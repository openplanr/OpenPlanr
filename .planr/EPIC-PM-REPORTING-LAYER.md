...
- **Local archive** under `.planr/reports/<YYYY-MM-DD>-<reportType>-report.md` (and `.html`, same basename).
...
- **`--push github`** — creates a new `planr:report` GitHub issue with the markdown body (uses the local `gh` CLI). Each run creates a separate issue; there is no title-based reuse in v1.
...
- `--strict-evidence` fails report generation when **substantive** list bullets under `##` sections lack a URL or `#NNN` reference. The **Evidence** appendix is skipped (it is link-oriented separately), and full-line italic placeholder bullets (e.g. `_Add links manually_`) are ignored so stock templates remain usable.
...
`distribution.slackChannel` and `distribution.weeklyRecipientAllowlist` are **reserved** for future routing / email allowlists; v1 Slack delivery only reads `slackWebhookUrl` (the webhook encodes the channel for Incoming Webhooks).
