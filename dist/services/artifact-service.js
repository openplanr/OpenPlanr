 ...
 */
const parseWarningsEmitted = new Set();
/**
 ...
 */
export async function updateArtifact(projectDir, config, type, id, content, { skipValidation = false } = {}) {
    const dir = path.join(projectDir, getArtifactDir(config, type));
    const files = await listFiles(dir, new RegExp(`^${id}-.*\\.md$`));
    if (files.length === 0)
 ...
 */
export async function updateArtifactFields(projectDir, config, type, id, fields) {
