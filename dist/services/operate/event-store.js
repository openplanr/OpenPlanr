...
import { OPERATE_AGENT_PROTOCOL_VERSION, OPERATE_MISSION_PROTOCOL_VERSION, OPERATE_PROTOCOL_VERSION, OPERATE_SCHEMA_VERSION, OperateError, } from './types.js';
 ...
 */
function recordEnvelopeProtocolVersion(recordType, content) {
    if (recordType === 'advisor-report')
        return OPERATE_AGENT_PROTOCOL_VERSION;
...
export function operatingRecordToLogEntry(record) {
    return {
        kind: 'operating-records-log-entry',
        schemaVersion: record.schemaVersion,
        protocolVersion: record.recordType === 'advisor-report'
...
export function logEntryToOperatingRecord(entry) {
    return {
...
export function operatingRecordsLogLine(record) {
    return canonicalize(operatingRecordToLogEntry(record));
}
async function fileExists(target) {
    return access(target).then(() => true, () => false);
}
async function durableAppend(target, line) {
...
async function atomicCanonicalWrite(target, value) {
...
function legacyRecordPath(recordsDir, digest) {
...
async function readRecordsLog(recordsFile) {
    ...
    for (const [index, line] of raw.split('\n').entries()) {
        ...
        try {
            ...
            throw new OperateError('E_OPERATE_STATE_INVALID', `Operating records.jsonl line ${index + 1} is not valid JSON.`);
        }
...
export class OperatingEventStore {
    ...
    async effectiveEventsPath() {
        if (await fileExists(this.paths.events))
    ...
    async effectiveCheckpointPath() {
        if (await fileExists(this.paths.checkpoint))
            return this.paths.checkpoint;
    ...
    async replay() {
        ...
        for (const [index, line] of raw.split('\n').entries()) {
            ...
            try {
                ...
                throw new OperateError('E_OPERATE_STATE_INVALID', `Operating event line ${index + 1} is not valid JSON.`);
            }
        }
        ...
        try {
            ...
            throw new OperateError('E_OPERATE_STATE_INVALID', error instanceof Error ? error.message : 'Operating event chain is invalid.');
        }
    }
    async putRecord(recordType, content, options) {
        ...
        for (const existing of await readRecordsLog(this.paths.records)) {
            ...
            if (canonicalDigest(existing) !== canonicalDigest(record)) {
                throw new OperateError('E_OPERATE_STATE_INVALID', `Content-addressed operating record ${digest} does not match its stored line.`);
            }
            return record;
        }
    ...
    async readRecord(digest) {
        let record = (await readRecordsLog(this.paths.records)).find((entry) => entry.digest === digest);
        if (!record) {
            ...
            if (await fileExists(legacy)) {
                record = JSON.parse(await readFile(legacy, 'utf8'));
            }
        }
        if (!record) {
            throw new OperateError('E_OPERATE_STATE_INVALID', `Content-addressed operating record ${digest} is not present in records.jsonl.`);
        }
        await assertOperatingArtifact('operating-record', record);
        if (record.digest !== digest ||
            ...
            throw new OperateError('E_OPERATE_STATE_INVALID', `Content-addressed operating record ${digest} failed integrity verification.`);
        }
        return record;
    }
    async append(input) {
        ...
        try {
            ...
            throw new OperateError('E_OPERATE_STATE_INVALID', error instanceof Error ? error.message : 'Operating event failed Protocol validation.');
        }
        await durableAppend(this.paths.events, `${canonicalize(event)}\n`);
        return event;
    }
    async state(checkpoint = null) {
        const replay = await this.replay();
        const protocol = await loadOperatingProtocol();
        ...
        return projectOperatingStalledItems(protocol.reduceOperatingEvents(events, { checkpoint }), replay.events);
    }
    async writeCheckpoint(state, options = {}) {
        await this.initialize();
        const protocol = await loadOperatingProtocol();
    ...
    async readCheckpoint(options = {}) {
        let checkpoint;
        try {
            checkpoint = JSON.parse(await readFile(await this.effectiveCheckpointPath(), 'utf8'));
        }
        catch (error) {
            if (error.code === 'ENOENT')
                return null;
        ...
        try {
            ...
            throw new OperateError('E_OPERATE_CHECKPOINT_INVALID', error instanceof Error ? error.message : 'Operating checkpoint is invalid.');
        }
    }
    async listRecordDigests() {
