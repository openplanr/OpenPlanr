 ...
 */
function hardBlockedSecretGap(citation, context, now) {
    const location = citation.repositoryPath ?? citation.gitRevision ?? citation.planrArtifactId ?? 'cited content';
    const createdAt = now.toISOString();
    return {
        kind: 'operating-data-gap',
        schemaVersion: '1.0.0',
        ...
        cycleId: context.cycleId,
        category: 'unresolvable-citation',
        question: `The cited content at "${location}" carries a hard-blocked secret; it is rejected as ` +
...
export async function resolveOperatingCitationAtPin(citation, context) {
    ...
    if (!observation.preRedacted &&
        detectSecretMetadata(rawContent).some((entry) => entry.hardBlock)) {
        const gap = await assertOperatingArtifact('operating-data-gap', hardBlockedSecretGap(keyed, context, now));
        return {
            citation: keyed,
            citationKey,
            outcome: 'rejected',
