...
const IMPORT_EVENT_TYPE = 'migration.legacy-imported';
const IMPORT_ACTOR = { kind: 'migration', id: 'openplanr-operate' };
...
function decodeUtf8(bytes, sourcePath) {
    try {
        ...
        throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Legacy board file is not valid UTF-8: ${sourcePath}`);
    }
}
async function readLegacyFiles(projectRoot) {
    ...
    if (rootInfo.isSymbolicLink() || !rootInfo.isDirectory()) {
        throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `${LEGACY_ROOT} must be a real directory, not a file or symlink.`);
    }
    ...
    async function visit(directory) {
        ...
        for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
            ...
            if (containsSecret(entry.name)) {
                throw new OperateError('E_OPERATE_SECRET_DETECTED', 'Legacy board contains a secret-shaped file name; rename or remove it before migration.');
            }
            if (entry.isSymbolicLink()) {
                throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Legacy board contains a symlink: ${relative}`);
            }
            ...
            if (!entry.isFile()) {
                throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Legacy board contains a non-file entry: ${relative}`);
            }
            if (files.length >= MAX_FILES) {
                throw new OperateError('E_OPERATE_INPUT_TOO_LARGE', `Legacy board exceeds the ${MAX_FILES}-file limit.`);
            }
            const bytes = await readFile(target);
            totalBytes += bytes.byteLength;
            if (totalBytes > MAX_BYTES) {
                throw new OperateError('E_OPERATE_INPUT_TOO_LARGE', `Legacy board exceeds the ${MAX_BYTES}-byte limit.`);
            }
            files.push({
                absolutePath: target,
                path: relative,
...
function legacyKindFromId(value) {
...
function legacyKindFromHeaders(headers, sourcePath) {
    if (headers.includes('question')) {
        return sourcePath.includes('gap') ? 'gap' : 'decision';
    }
    if (headers.includes('lane') || headers.includes('title'))
        return 'finding';
    if (sourcePath.includes('decision'))
        return 'decision';
    if (sourcePath.includes('gap'))
        return 'gap';
    if (sourcePath.includes('route'))
        return 'route';
    if (sourcePath.includes('outcome'))
        return 'outcome';
    return 'unknown';
}
function boundedLegacyId(value) {
    if (typeof value !== 'string')
        return null;
    const id = value.trim();
    if (containsSecret(id))
        return null;
...
function parseMarkdownCandidates(file) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        if (claimedLines.has(index))
            continue;
        const match = lines[index].match(/^\s*(?:[-*]\s*)?(?:\*\*)?([A-Za-z][A-Za-z0-9 /_-]{0,48})(?:\*\*)?\s*:\s*(.*?)\s*$/);
        if (!match) {
            if (!lines[index].trim())
                flushBlock();
            continue;
...
function parseJsonCandidates(file) {
    ...
    if (Array.isArray(parsed)) {
        ...
        for (const key of ['findings', 'decisions', 'gaps', 'routes', 'outcomes', 'records']) {
            ...
            entries.push(...collection.map((value) => ({
                key,
                value: value,
            })));
        }
        if (!foundCollection)
            entries.push({ key: null, value: parsed });
    }
    else {
...
function migrationEventPayload(event) {
    return event.payload;
}
async function applyDuplicateAndConflictRules(candidates, store, migrationId) {
    const conflicts = [];
    ...
    for (const row of candidates) {
        ...
        if (exact && migrationEventPayload(exact).migrationId === migrationId) {
            ...
            if (exactPayload.sourceDigest === row.sourceDigest) {
                ...
                row.disposition = 'conflict';
                conflicts.push(`${row.legacyKind}:${row.legacyId ?? row.recordDigest}:event-payload-mismatch:${exact.eventId}`);
            }
            continue;
        }
        const identity = `${row.legacyKind}:${row.legacyId ?? row.recordDigest}`;
...
function backupManifest(migrationId, sourceDigest, files) {
    return {
...
async function inspectPrepared(input) {
    ...
    const sourceDigest = canonicalDigest(files.map((file) => ({
        path: file.path,
        digest: file.digest,
        size: file.bytes.byteLength,
    })));
...
function publicInspection(prepared) {
    return {
...
async function writePrivateBytes(target, bytes) {
    ...
    try {
        ...
        if (sha256Digest(existing) !== sha256Digest(bytes)) {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Migration backup changed unexpectedly: ${path.basename(target)}`);
        }
    }
}
function backupRoot(projectRoot, migrationId, localRoot) {
    return path.join(resolveOperatingPaths(projectRoot, { localRoot }).localRoot, 'migration-backups', migrationId);
}
async function ensureBackup(input) {
    const { record, backupManifest } = input.prepared;
    if (!record || !backupManifest)
        return;
    const currentFiles = await readLegacyFiles(input.projectRoot);
    const byPath = new Map(currentFiles.map((file) => [file.path, file]));
    if (canonicalDigest(currentFiles.map((file) => ({
        path: file.path,
        digest: file.digest,
        size: file.bytes.byteLength,
    }))) !== record.sourceDigest) {
        throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', 'Legacy board changed after migration preview.');
    }
    ...
    for (const file of backupManifest.files) {
        ...
        if (!source || source.digest !== file.digest || source.bytes.byteLength !== file.size) {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Legacy board changed after preview: ${file.path}`);
        }
    ...
    if (canonicalDigest(backupManifest) !== record.backupManifestDigest) {
        throw new OperateError('E_OPERATE_STATE_INVALID', 'Migration backup manifest digest does not match the preview.');
    }
}
async function verifyBackup(input) {
    const root = backupRoot(input.projectRoot, input.record.id, input.localRoot);
    const manifest = JSON.parse(await readFile(path.join(root, 'manifest.json'), 'utf8'));
    if (manifest.migrationId !== input.record.id ||
        manifest.sourceDigest !== input.record.sourceDigest ||
        canonicalDigest(manifest) !== input.record.backupManifestDigest) {
        throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Migration ${input.record.id} backup manifest failed integrity verification.`);
    }
    for (const file of manifest.files) {
        ...
        if (bytes.byteLength !== file.size || sha256Digest(bytes) !== file.digest) {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Migration ${input.record.id} backup is corrupt: ${file.path}`);
        }
    }
    return manifest;
}
async function readMigrationRecord(projectRoot, migrationId, localRoot) {
    const target = path.join(resolveOperatingPaths(projectRoot, { localRoot }).migrations, `${migrationId}.json`);
    try {
...
async function persistMigrationHead(projectRoot, store, localRoot) {
...
export async function applyOperatingMigration(input) {
    const preview = await inspectPrepared(input);
    if (!preview.record)
        return null;
    const previewRecord = preview.record;
    if (previewRecord.state === 'conflict') {
        throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', 'Legacy board contains conflicts or corrupt input; no migration was applied.', { preview: publicInspection(preview) });
    }
    if (!input.confirmed) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'Migration requires explicit confirmation of its preview.', { preview: publicInspection(preview) });
    }
    const store = new OperatingEventStore(input.projectRoot, { localRoot: input.localRoot });
    ...
    return withOperatingLock(input.projectRoot, {
        ...
        const existing = await readMigrationRecord(input.projectRoot, previewRecord.id, input.localRoot);
        ...
        if (existing) {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Migration ${existing.id} is already ${existing.state}.`);
        }
        ...
        if (!current.record ||
            ...
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', 'Legacy migration input or conflicts changed after preview.');
        }
        ...
        const importRows = current.candidates.filter((row) => row.disposition === 'import' || row.disposition === 'already-imported');
        const immutablePlan = {
            ...current.record,
        ...
        const savedPlan = await store.putRecord('migration', immutablePlan, {
...
export async function rollbackOperatingMigration(input) {
    if (!input.confirmed) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'Migration rollback requires explicit confirmation.');
    }
    const store = new OperatingEventStore(input.projectRoot, { localRoot: input.localRoot });
    const initial = await store.replay();
    return withOperatingLock(input.projectRoot, {
        ...
        const lockedReplay = await store.replay();
        lock.assertEventHead(lockedReplay.eventHead);
        const current = await readMigrationRecord(input.projectRoot, input.migrationId, input.localRoot);
        if (!current) {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Unknown migration ${input.migrationId}.`);
        }
        ...
        if (current.state !== 'applied') {
            throw new OperateError('E_OPERATE_MIGRATION_CONFLICT', `Migration ${input.migrationId} is not applied.`);
        }
        await verifyBackup({
            projectRoot: input.projectRoot,
            localRoot: input.localRoot,
        ...
        if (!replay.events.some((event) => event.eventId === eventId)) {
            ...
            const saved = await store.putRecord('recovery', recovery, {
