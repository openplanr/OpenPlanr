...
import { applyOperatingRoute, electAcceptedFindingEpicRoutes, readOperatingRoute, rollbackOperatingRoute, } from './routes.js';
...
function stateCollection(state, collection) {
    ...
    if (collection === 'gaps')
        return state.dataGaps;
    if (collection === 'routes')
        return state.routes;
    if (collection === 'evidence')
        return state.evidenceSources;
    return [];
}
export async function readOperatingCollection(input) {
    ...
    if (input.collection === 'migrations') {
        ...
        const records = await Promise.all((await readdir(directory).catch(() => []))
            .filter((name) => name.endsWith('.json'))
            .sort()
            .map((name) => readFile(path.join(directory, name), 'utf8')
            .then((raw) => JSON.parse(raw))
            .catch(() => null)));
        const valid = records.filter((record) => Boolean(record));
        return input.id ? (valid.find((record) => record.id === input.id) ?? null) : valid;
    }
    if (input.collection === 'routes' && input.id) {
        return readOperatingRoute(input.projectRoot, input.id).catch((error) => {
...
export async function reconcileOperatingDecisionDeadlines(input) {
    ...
    if (due.length === 0)
        return { transitioned: 0, state: await store.state() };
    return withOperatingLock(input.projectRoot, {
        projectKey: operatingProjectKey(input.projectRoot),
        expectedEventHead: initial.eventHead,
        currentEventHead: initial.eventHead,
...
export async function transitionOperatingCycle(input) {
    if (!input.confirmed) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', `Cycle ${input.action} requires explicit confirmation.`);
    }
    const store = new OperatingEventStore(input.projectRoot, { localRoot: input.localRoot });
    const state = await store.state();
    ...
    const type = input.action === 'resume'
        ? 'cycle.collecting'
        : input.action === 'cancel'
            ? 'cycle.cancelled'
            : 'cycle.closed';
    return mutateEvent({
        ...
        validateState: input.action === 'close'
            ? (lockedState) => {
                const lockedCycle = lockedState.cycles.find((record) => record.id === input.cycleId);
                if (lockedCycle?.state !== 'reviewable') {
                    throw new OperateError('E_OPERATE_STATE_INVALID', `Cycle ${input.cycleId} must be reviewable before it can close.`);
                }
 ...
 */
export function assertOperatingCycleDisposable(state, cycleId) {
    const findings = state.findings.filter((finding) => finding.cycleId === cycleId && finding.parked !== true);
    const decisions = state.decisions.filter((decision) => decision.cycleId === cycleId);
    if (findings.length === 0 && decisions.length === 0)
        return;
    const appliedFindingIds = new Set(state.routes
        .filter((route) => route.cycleId === cycleId && route.state === 'applied')
        .flatMap((route) => Array.isArray(route.findingIds)
        ? route.findingIds.filter((id) => typeof id === 'string')
        : []));
    const terminalFindingStates = new Set(['done', 'rejected', 'superseded']);
    const blockingFindings = findings
        .filter((finding) => !terminalFindingStates.has(String(finding.status)) && !appliedFindingIds.has(finding.id))
    ...
    const blockingDecisions = decisions
        ...
        .sort((left, right) => left.id.localeCompare(right.id));
    if (blockingFindings.length === 0 && blockingDecisions.length === 0)
        return;
    throw new OperateError('E_OPERATE_CYCLE_NOT_DISPOSED', `Cycle ${cycleId} still has governed work that must be disposed before it can close.`, {
        blockingFindings,
        blockingDecisions,
        recoveryCommands: [
            ...blockingFindings.map(({ id }) => `planr operate findings accept ${id} --yes && planr operate routes apply <route-id> --yes`),
            ...blockingFindings.map(({ id }) => `planr operate findings reject ${id} --reason <reason> --yes`),
            ...blockingDecisions.map(({ id }) => `planr operate decisions decide ${id} --option <option> --yes`),
        ],
    });
}
function scoreAmendment(finding, values, timestamp) {
    const prior = {
    ...
    const next = { ...prior };
    for (const key of ['impact', 'confidence', 'ease']) {
        if (values[key] === undefined)
            continue;
        const score = Number(values[key]);
        if (!Number.isInteger(score) || score < 1 || score > 5) {
            throw new OperateError('E_OPERATE_CONFIG_INVALID', `${key} amendment must be an integer from 1 to 5.`);
        }
        next[key] = score;
    }
    const ceiling = Number(finding.confidenceCeiling ?? finding.confidence);
    if (next.confidence > ceiling) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', `Confidence amendment ${next.confidence} exceeds the evidence ceiling ${ceiling}.`);
    }
    const reason = sanitizeGeneratedPlainText(values.reason?.trim() || 'Accepted without changing the evidence-bounded score.').slice(0, 2_048);
    return {
...
export async function governOperatingFinding(input) {
    if (!input.confirmed) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', `Finding ${input.action} requires explicit confirmation.`);
    }
    const store = new OperatingEventStore(input.projectRoot, { localRoot: input.localRoot });
    const state = await store.state();
    ...
    if (input.action !== 'accept' && !input.reason?.trim()) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', `${input.action} requires an audit reason.`);
    }
    const timestamp = new Date().toISOString();
    const acceptedScore = input.action === 'accept' ? scoreAmendment(finding, input, timestamp) : null;
    ...
    for (const candidate of findingResult.state.routes) {
        const full = await readOperatingRoute(input.projectRoot, String(candidate.id)).catch(() => null);
        if (!full?.actions.some((action) => action.findingId === input.findingId))
            continue;
        acceptedRoute = full;
...
export async function decideOperatingDecision(input) {
    if (!input.confirmed || !input.value.trim()) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'A decision requires --yes and a non-empty value.');
    }
    ...
    if (decision.status === 'answered') {
        if (decision.selectedOption !== selectedOption) {
            throw new OperateError('E_OPERATE_STATE_INVALID', `Decision ${input.decisionId} was already answered with a different option.`);
        }
        const answeredEvent = [...(await store.replay()).events]
            .reverse()
            .find((event) => event.type === 'decision.answered' && event.entityId === input.decisionId);
        if (!answeredEvent) {
            throw new OperateError('E_OPERATE_STATE_INVALID', `Decision ${input.decisionId} is answered but its answer event is missing.`);
        }
        ...
        const answered = await mutateEvent({
            ...
            validateState: (lockedState) => {
                ...
                if (!lockedDecision || !['open', 'default-due'].includes(String(lockedDecision.status))) {
                    throw new OperateError('E_OPERATE_STATE_INVALID', `Decision ${input.decisionId} is no longer available to answer.`);
                }
            },
        });
    ...
    return mutateEvent({
        ...
        validateState: (lockedState) => {
            const lockedDecision = lockedState.decisions.find((record) => record.id === input.decisionId);
            if (lockedDecision?.status !== 'answered' ||
                lockedDecision.selectedOption !== selectedOption) {
                throw new OperateError('E_OPERATE_STATE_INVALID', `Decision ${input.decisionId} is not awaiting closure for the selected option.`);
            }
        },
    });
}
export async function answerOperatingGap(input) {
    if (!input.confirmed || !input.value.trim()) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'A gap answer requires --yes and a non-empty value.');
    }
    ...
    if (!gap)
        throw new OperateError('E_OPERATE_STATE_INVALID', `Unknown gap ${input.gapId}.`);
...
export async function verifyOperatingGap(input) {
    ...
    if (!input.confirmed || evidenceRefs.length === 0) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'Gap verification requires --yes and at least one verified evidence reference.');
    }
    const state = await new OperatingEventStore(input.projectRoot, {
        localRoot: input.localRoot,
    }).state();
    ...
    if (gap?.status !== 'answered') {
        throw new OperateError('E_OPERATE_STATE_INVALID', `Gap ${input.gapId} must be answered before it can be verified.`);
    }
...
export async function applyOrRollbackRoute(input) {
    ...
    if (input.action === 'apply') {
        const projected = (await new OperatingEventStore(input.projectRoot, {
            localRoot: input.localRoot,
        }).state()).routes.find((record) => record.id === input.routeId);
        const destination = await import('./routes.js').then(({ routeDestinationDigest }) => routeDestinationDigest(input.projectRoot, route));
        ...
        const generated = isAgentArtifact
            ...
            : null;
        const awaitingPlan = projected?.state === 'prepared' &&
            ['create-spec', 'create-instrumentation-spec'].includes(String(route.actions[0]?.kind));
        const awaitingArtifactReview = projected?.state === 'prepared' && generated?.state === 'generated';
        const preview = {
            route,
            previewDigest: awaitingArtifactReview && generated.exactPreviewDigest
                ? generated.exactPreviewDigest
                : route.previewDigest,
            destinationDigest: destination,
            destinationMatches: awaitingPlan || (isAgentArtifact && projected?.state === 'prepared')
                ? null
        ...
        if (input.preview)
            return preview;
        if (!input.confirmed) {
            throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', 'Review the route preview, then confirm the exact preview digest.', preview);
        }
        if (!input.previewDigest) {
            throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', `Preview this route and provide --preview-digest ${preview.previewDigest}.`, { previewDigest: preview.previewDigest, route });
        }
    ...
    if (!input.confirmed) {
        throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', 'Route rollback requires explicit confirmation.');
    }
    const state = await new OperatingEventStore(input.projectRoot, {
        localRoot: input.localRoot,
    }).state();
    ...
    if (!projected?.transactionId) {
        throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Route ${input.routeId} has no applied transaction to roll back.`);
    }
