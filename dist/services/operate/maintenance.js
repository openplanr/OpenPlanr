...
export async function operateAdapterLifecycle(input) {
    ...
    if (input.action === 'prepare') {
        ...
        const roleBriefs = Object.fromEntries(Object.keys(roleMandates).map((role) => [role, protocol.createOperatingAdvisorBrief(role)]));
    ...
    if (input.action === 'record') {
        ...
        for (const field of generatedText) {
            if (typeof field.value === 'string' && containsSecret(field.value)) {
                throw new OperateError('E_OPERATE_SECRET_DETECTED', `Recorded advisor output contains a secret at ${field.location}; nothing was persisted.`, { roleId: input.role, field: field.location });
            }
        }
