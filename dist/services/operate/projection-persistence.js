...
import { OPERATING_BOARD_ROLES, renderOperatingBoardReport, renderOperatingBrief, renderOperatingEvidenceIndex, selectCycleState, } from './projection.js';
...
function markdownCell(candidate, fallback = '—') {
    const rendered = typeof candidate === 'string' || typeof candidate === 'number' || typeof candidate === 'boolean'
        ? String(candidate)
        : fallback;
...
function evidenceRefs(record) {
    if (!Array.isArray(record.evidenceRefs))
        return '—';
    const refs = [
        ...new Set(record.evidenceRefs.filter((entry) => typeof entry === 'string')),
    ].sort();
    return refs.length > 0 ? refs.map((entry) => `\`${markdownCell(entry)}\``).join(', ') : '—';
}
function generatedHeader(state, title) {
    return [
        `# ${title}`,
        '',
...
function renderFindingRegister(state) {
    const findings = [...state.findings].sort((left, right) => left.id.localeCompare(right.id));
    const stalledById = new Map(operatingStalledItems(state).map((item) => [item.id, item.stalledCycles]));
    return [
        ...generatedHeader(state, 'Operating Findings Register'),
        '| Finding | Status | Lane | Owner | Severity | Score | Stalled cycles | Title | Evidence |',
        '|---|---|---|---|---|---:|---:|---|---|',
        ...(findings.length > 0
            ? findings.map((finding) => `| ${markdownCell(finding.id)} | ${markdownCell(finding.status)} | ${markdownCell(finding.lane)} | ${markdownCell(finding.owner)} | ${markdownCell(finding.severity)} | ${markdownCell(finding.score, '0')} | ${markdownCell(stalledById.get(finding.id) ?? 0, '0')} | ${markdownCell(finding.title)} | ${evidenceRefs(finding)} |`)
            : ['| — | — | — | — | — | 0 | 0 | No findings recorded. | — |']),
    ].join('\n');
}
function renderDecisionRegister(state) {
    const stalledById = new Map(operatingStalledItems(state).map((item) => [item.id, item.stalledCycles]));
    ...
    return [
        ...
        '|---|---|---|---|---:|---|---|---|',
        ...(decisions.length > 0
            ? decisions.map((decision) => `| ${markdownCell(decision.id)} | ${markdownCell(decision.status)} | ${markdownCell(decision.owner)} | ${markdownCell(decision.deadline)} | ${markdownCell(stalledById.get(decision.id) ?? 0, '0')} | ${markdownCell(decision.reversibility)} | ${markdownCell(decision.question)} | ${markdownCell(decision.recommendation)} |`)
            : ['| — | — | — | — | 0 | — | No decisions recorded. | — |']),
    ].join('\n');
}
function renderDataGapRegister(state) {
            ...
            ? gaps.map((gap) => {
                const unblocks = Array.isArray(gap.unblocks)
                    ? gap.unblocks
                        ...
                        .join(', ')
                    : '';
                return `| ${markdownCell(gap.id)} | ${markdownCell(gap.status)} | ${markdownCell(gap.owner)} | ${markdownCell(gap.question)} | ${markdownCell(gap.reason)} | ${markdownCell(unblocks)} |`;
            })
            : ['| — | — | — | No data gaps recorded. | — | — |']),
    ].join('\n');
}
function renderBacklogRegister(state) {
    ...
    return [
        ...
        '|---|---|---|---:|---|---|---|',
        ...(parked.length > 0
            ? parked.map((finding) => `| ${markdownCell(finding.id)} | ${markdownCell(finding.status)} | ${markdownCell(finding.owner)} | ${markdownCell(finding.score, '0')} | ${markdownCell(finding.title)} | ${markdownCell(finding.problem ?? finding.proposal, 'Deprioritized by the current attention cap.')} | ${evidenceRefs(finding)} |`)
            : ['| — | — | — | 0 | No parked findings. | — | — |']),
    ].join('\n');
}
function renderRouteRegister(state) {
        ...
        ...(routes.length > 0
            ? routes.map((route) => {
                ...
                const findingIds = Array.isArray(route.findingIds)
                    ? route.findingIds.filter((entry) => typeof entry === 'string')
                    : [
                        ...new Set(actions
                            .map((action) => action.findingId)
                            .filter((entry) => typeof entry === 'string')),
                    ];
                const actionCount = typeof route.actionCount === 'number' ? route.actionCount : actions.length;
                return `| ${markdownCell(route.id)} | ${markdownCell(route.cycleId)} | ${markdownCell(route.state)} | ${markdownCell(findingIds.sort().join(', '))} | ${markdownCell(actionCount, '0')} | ${markdownCell(route.routeDigest)} |`;
            })
            : ['| — | — | — | — | 0 | No routes recorded. |']),
    ].join('\n');
}
function renderCycleBrief(state, cycleId) {
    ...
    if (words.length > 900) {
        throw new OperateError('E_OPERATE_PROJECTION_DRIFT', `Cycle brief ${cycleId} exceeds the 900-word projection limit.`, { cycleId, words: words.length });
    }
    return brief;
}
function markerExpression(markerName, edge) {
    const escaped = markerName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return edge === 'begin'
        ? new RegExp(`^<!--\\s*##planr-${escaped}:begin##[^>]*-->[\\t ]*(?:\\r?\\n|$)`, 'gm')
        : new RegExp(`^<!--\\s*##planr-${escaped}:end##\\s*-->[\\t ]*`, 'gm');
}
function managedSection(content, markerName) {
...
function beginMarker(markerName) {
    return `<!-- ##planr-${markerName}:begin## (managed by planr CLI; preserve hand-edits outside this block) -->`;
}
function endMarker(markerName) {
    return `<!-- ##planr-${markerName}:end## -->`;
}
function spliceProjectionBlock(existing, markerName, managedContent) {
    ...
    if (beginMatches.length !== 1 || endMatches.length !== 1) {
        throw new OperateError('E_OPERATE_PROJECTION_DRIFT', `Projection markers for ${markerName} are malformed or duplicated.`, { markerName });
    }
    const begin = beginMatches[0];
    const end = endMatches[0];
    ...
    if (endStart < beginStart + begin[0].length) {
        throw new OperateError('E_OPERATE_PROJECTION_DRIFT', `Projection markers for ${markerName} are out of order.`, { markerName });
    }
    const afterStart = endStart + end[0].length;
    return `${existing.slice(0, beginStart)}${block}${existing.slice(afterStart)}`;
}
const OPERATING_ROLE_FILE_ALIASES = new Map([
...
function renderAgenticActionsMarkdown(cycleId, reports) {
    const actions = reports.flatMap((report) => (report.actions ?? []).map((action) => ({
    ...
    return [
        ...
        '',
        ...(actions.length > 0
            ? actions.map((action) => `- **${String(action.lane ?? 'OWNER')} · ${String(action.title ?? action.actionKey ?? 'Proposed action')}** (${String(action.roleId)}) — ${String(action.summary ?? '')}`)
...
export async function readOperatingRichCycleArtifacts(input) {
    ...
    const cycles = input.state.cycles.filter((cycle) => cycle.state === 'reviewable' || cycle.state === 'closed');
    if (cycles.length === 0)
        return artifacts;
    const { readOperatingReport, markdownLens, markdownCompleteRegisters } = await import('./reports.js');
        ...
        try {
            ...
            const draftSection = [
                ...
                ...(cycleDrafts.length > 0
                    ? cycleDrafts.map((draft) => `- **${draft.draftId}** [${draft.status} · ${draft.artifactKind}] \`${draft.path}\``)
            ...
            artifacts.set(cycle.id, {
                ...
                reportMarkdown: `${report.markdown}\n\n${draftSection}\n\n${markdownCompleteRegisters(selectCycleState(input.state, cycle.id))}`,
                ...
                evaluatedRoleIds: new Set(report.reports
...
function markdownProjectionSpecs(state, richArtifacts) {
    ...
    const cycleArtifacts = [...state.cycles]
        ...
        return [
            ...
            ...OPERATING_BOARD_ROLES.map((role) => ({
                ...
                managedContent: rich?.boardByRole.get(role.id) ??
                    renderOperatingBoardReport(state, cycle.id, role, rich?.evaluatedRoleIds ?? new Set()),
            })),
            ...(rich
                ? OPERATING_BOARD_ROLES.filter((role) => (OPERATING_ROLE_FILE_ALIASES.get(role.id) ?? role.id) !== role.id).map((role) => ({
                    relativePath: `${OPERATE_ROOT}/cycles/${cycle.id}/board/${OPERATING_ROLE_FILE_ALIASES.get(role.id) ?? role.id}.md`,
                    markerName: BOARD_MARKER,
                    managedContent: rich.boardByRole.get(role.id) ??
...
export function renderOperatingProjectionFiles(state, richArtifacts = new Map()) {
    const stateFile = {
        relativePath: STATE_PATH,
        content: `${canonicalize(state)}\n`,
    };
    const evidenceIndexFile = {
...
async function inspectProjectionFile(projectRoot, expected) {
    ...
    const current = await readFile(path.join(projectRoot, expected.relativePath), 'utf8').catch((error) => {
...
export async function inspectOperatingProjectionDrift(input) {
    ...
    const richArtifacts = input.richCycleArtifacts ??
...
export async function assertOperatingProjectionsCurrent(input) {
    ...
    if (mismatches.length > 0) {
        throw new OperateError('E_OPERATE_PROJECTION_DRIFT', `${mismatches.length} Operating Board projection(s) differ from event replay.`, { projections: mismatches });
    }
}
export async function prepareOperatingProjectionPersistence(input) {
    ...
    for (const file of rendered) {
        ...
        const current = await readFile(path.join(input.projectRoot, file.relativePath), 'utf8').catch((error) => {
            if (error.code === 'ENOENT')
                return '';
            throw error;
        });
    ...
    for (const file of files) {
        const current = await readFile(path.join(input.projectRoot, file.relativePath)).catch((error) => {
            if (error.code === 'ENOENT')
                return null;
            throw error;
        });
...
//# sourceMappingURL=projection-persistence.js.map