...
function semanticStem(token) {
    ...
    if (alias.length > 5 && alias.endsWith('ing'))
        return alias.slice(0, -3);
    if (alias.length > 4 && alias.endsWith('ed'))
        return alias.slice(0, -2);
    if (alias.length > 4 && alias.endsWith('es'))
        return alias.slice(0, -2);
    if (alias.length > 3 && alias.endsWith('s'))
...
function semanticallyEquivalentProposals(left, right) {
    if (left.type !== right.type)
        return false;
    if (exactDuplicateKey(left) === exactDuplicateKey(right))
        return true;
    if (['merge', 'sequence'].includes(left.type))
        return false;
...
function confidenceCeiling(evidenceRefs, evidence) {
    const items = evidence.items.filter((item) => evidenceRefs.includes(item.id));
    if (items.length === 0)
        return 1;
    let ceiling = items.some((item) => item.freshness === 'stale') ? 2 : 3;
    if (new Set(items.map((item) => item.source)).size >= 2)
        ceiling += 1;
    if (items.some((item) => item.metric?.observedFrom &&
...
function evidenceSensitivity(evidenceRefs, evidence) {
    ...
    if (missingRefs.length > 0) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', `Finding proposal cites unavailable evidence: ${missingRefs.join(', ')}.`);
    }
    return maximumSensitivity([...new Set(evidenceRefs)].map((reference) => byId.get(reference)?.sensitivity ?? 'public'));
}
function exactDuplicateKey(proposal) {
...
function findingFingerprint(proposal, sensitivity) {
    return canonicalDigest({
        category: proposal.type,
...
export function semanticallyEquivalentFindings(left, right) {
    if (left.category !== right.category || left.sensitivity !== right.sensitivity)
        return false;
    const leftTokens = semanticTokens(left.title, left.problem, left.proposal);
    const rightTokens = semanticTokens(right.title, right.problem, right.proposal);
    const similarity = semanticOverlap(leftTokens, rightTokens);
...
function laneFor(type) {
    if (type === 'decision')
        return 'OWNER';
    if (type === 'merge' || type === 'sequence')
        return 'AGENT';
    return 'DEV';
}
function preferredText(left, right) {
    return [normalizedDisplay(left), normalizedDisplay(right)].sort((a, b) => normalized(a).localeCompare(normalized(b)) || a.localeCompare(b))[0];
}
function preferredType(left, right) {
    return TYPE_RANK[left] >= TYPE_RANK[right] ? left : right;
}
function preferredSeverity(left, right) {
...
function normalizeProposal(proposal) {
    return {
        ...
        ...(proposal.sequenceProposalKeys
            ? {
                sequenceProposalKeys: proposal.sequenceProposalKeys.filter((key, index, values) => values.indexOf(key) === index),
            }
            : {}),
    };
}
function mergeProposalReferences(left, right) {
    ...
    const sequences = [left.sequenceProposalKeys, right.sequenceProposalKeys].filter((value) => Array.isArray(value));
...
function aggregateProposals(results) {
    const candidates = results
        .flatMap((result) => result.proposals.map((proposal) => ({
        roleId: result.roleId,
        proposal: structuredClone(proposal),
    })))
        .sort((left, right) => {
        const keyDelta = exactDuplicateKey(left.proposal).localeCompare(exactDuplicateKey(right.proposal));
        return (keyDelta ||
    ...
    return [...clusters.values()]
        .map((cluster) => {
        const ordered = [...cluster].sort((left, right) => left.roleId.localeCompare(right.roleId) ||
            left.proposal.proposalKey.localeCompare(right.proposal.proposalKey));
        const proposal = ordered.slice(1).reduce((current, candidate) => ({
...
function candidateOrder(left, right) {
    return (Number(right.finding.criticalOverride) - Number(left.finding.criticalOverride) ||
...
export async function consolidateOperatingResults(input) {
    ...
    const evaluatedCandidates = proposals
        ...
        const missingTechnologyRiskCoverage = hasVerifiedCriticalEvidence(proposal.evidenceRefs, input.evidence) &&
        ...
        return {
            ...
            convertedToGap: missingTechnologyRiskCoverage || (proposal.impact >= 4 && confidence <= 2 && !critical),
    ...
    const criticalCandidates = rankedCandidates.filter((candidate) => candidate.finding.criticalOverride);
    if (criticalCandidates.length > input.config.caps.surfacedFindings) {
        throw new OperateError('E_OPERATE_CRITICAL_CAP', `${criticalCandidates.length} verified critical findings exceed the surfaced-finding cap of ${input.config.caps.surfacedFindings}.`, {
    ...
    const findingCandidates = rankedCandidates.map(({ finding }, index) => ({
        ...finding,
        dependsOn: finding.lane === 'AGENT'
            ? rankedCandidates
                .slice(0, index)
                .map((candidate) => candidate.finding)
                .filter((candidate) => candidate.lane !== 'AGENT' &&
                candidate.evidenceRefs.some((reference) => finding.evidenceRefs.includes(reference)))
    ...
    for (const finding of findingCandidates) {
        if (findings.length < input.config.caps.surfacedFindings &&
    ...
    const chairConflictGaps = input.results
        ...
        .flatMap((result) => result.conflicts.map((conflict) => ({
    ...
    for (const gap of [...explicitGapProposals, ...confidenceGaps, ...chairConflictGaps].sort((left, right) => left.stableKey.localeCompare(right.stableKey))) {
    ...
    const gaps = [...uniqueGaps.values()].map((proposal, index) => ({
