...
export function projectMachineKey(projectRoot) {
...
export function resolveOperatingPaths(projectRoot, options = {}) {
    const resolvedProject = path.resolve(projectRoot);
    const root = path.join(resolvedProject, '.planr', 'operate');
    const state = path.join(root, '.state');
    const stateBase = path.resolve(options.localRoot ?? process.env.OPENPLANR_STATE_ROOT ?? path.join(homedir(), '.planr'));
...
export async function resolveContainedPath(projectRoot, relativePath, options = {}) {
    ...
    if (options.mustExist) {
        ...
        if (!isPathInside(root, resolved)) {
            throw new OperateError('E_OPERATE_PATH_ESCAPE', `Path follows a symlink outside the project: ${relativePath}`);
        }
    ...
    while (nearest !== root) {
        try {
            ...
            if (!isPathInside(root, resolvedParent)) {
                throw new OperateError('E_OPERATE_PATH_ESCAPE', `Destination parent follows a symlink outside the project: ${relativePath}`);
            }
            break;
        }
        catch (error) {
...
export async function resolveOperatingProject(projectRoot) {
    ...
    if (await access(config).then(() => true, () => false)) {
        return requested;
    }
    throw new OperateError('E_OPERATE_PROJECT_REQUIRED', gitFailure
        ...
        : 'Operating Board requires a Git worktree or initialized OpenPlanr project.', gitFailure ? { gitFailure, projectRoot: requested } : undefined);
}
export async function assertOperatingProject(projectRoot) {
...
function stableComponentId(remote) {
    const slug = remote
        .split('/')
        .at(-1)
        ?.toLowerCase()
...
async function repositoryDescriptor(root, readOnly, ignoredPaths = []) {
    ...
    const materialStatus = status
        ...
        const ignored = ignoredPaths.some((entry) => candidate === entry || candidate.startsWith(`${entry.replace(/\/$/, '')}/`));
...
export async function buildWorkspaceManifest(controlRoot, componentRoots = [], options = {}) {
    const resolvedControl = await resolveOperatingProject(controlRoot);
    const resolvedComponents = await Promise.all(componentRoots.map(async (candidate) => realpath(candidate)));
    const control = await repositoryDescriptor(resolvedControl, false, options.ignoredControlPaths ?? []);
    const components = await Promise.all([...new Set(resolvedComponents.filter((candidate) => candidate !== resolvedControl))].map(async (root) => ({ root, ...(await repositoryDescriptor(root, true)) })));
    ...
    if (new Set(identities.map((entry) => entry.componentId)).size !== identities.length) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Workspace component IDs collide; configure distinct canonical remotes.');
    }
    ...
    const manifest = {
        ...
        components: components
            .map((entry) => ({ ...entry.descriptor, readOnly: true }))
            .sort((a, b) => a.componentId.localeCompare(b.componentId)),
    };
 ...
 */
export async function refreshOperatingWorkspaceManifest(projectRoot, options = {}) {
    const paths = resolveOperatingPaths(projectRoot, options);
    let roots;
    let committed;
    try {
        roots = JSON.parse(await readFile(paths.roots, 'utf8'));
    }
    catch {
        throw new OperateError('E_OPERATE_NOT_INITIALIZED', 'Machine-local workspace roots are missing; run `planr operate init` on this machine.');
    }
    try {
        committed = JSON.parse(await readFile(paths.workspace, 'utf8'));
    }
    catch {
        throw new OperateError('E_OPERATE_NOT_INITIALIZED', 'Committed workspace metadata is missing; run `planr operate init`.');
    }
    ...
    if (!roots ||
        ...
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'The machine-local workspace root map is invalid.');
    }
    const componentRoots = committed.components.map((component) => roots.roots[component.componentId]);
    if (componentRoots.some((root) => typeof root !== 'string')) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'The workspace manifest and machine-local component roots disagree.');
    }
    return buildWorkspaceManifest(projectRoot, componentRoots, {
        localRoot: options.localRoot,
        persistRoots: false,
        ...(options.ignoredControlPaths ? { ignoredControlPaths: options.ignoredControlPaths } : {}),
    });
}
export async function ensureOperatingDirectories(projectRoot, options = {}) {
    const paths = resolveOperatingPaths(projectRoot, options);
    await Promise.all([
        paths.root,
...
export async function writeOperatingConfig(projectRoot, config, options = {}) {
...
export async function readOperatingConfig(projectRoot, options = {}) {
    const paths = resolveOperatingPaths(projectRoot, options);
    let parsed;
    try {
        parsed = JSON.parse(await readFile(paths.config, 'utf8'));
    }
    catch {
        throw new OperateError('E_OPERATE_NOT_INITIALIZED', 'Operating Board is not initialized for this project.');
    }
