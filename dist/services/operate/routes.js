...
import { assertPlanningProducer, completePipelinePoHandoff, hasPipelinePoCompletionProvenance, inspectPlanningProducer, loadPipelinePoBridge, preparePipelinePoHandoff, } from './pipeline-handoff.js';
...
import { refreshOperatingWorkspaceManifest, resolveContainedPath, resolveOperatingPaths, } from './workspace.js';
 ...
 */
function isSmallBoundedImplementation(finding) {
    return (finding.lane === 'DEV' &&
...
function actionKind(finding, isEpicAnchor = false) {
    ...
    if (finding.lane === 'AGENT')
        return 'create-cycle-artifact';
    if (finding.category.includes('instrument'))
        return 'create-instrumentation-spec';
    if (isSmallBoundedImplementation(finding))
...
export async function nextOperatingSpecOrdinal(projectRoot) {
...
export async function createOperatingRoutePlan(input) {
    ...
    const destinationDigest = canonicalDigest(routeDestinationPaths(destinationShape).map((destination) => ({
        path: destination,
        beforeDigest: null,
    })));
...
async function assertRouteWorkspaceCurrent(input) {
    ...
    if (observed.workspaceDigest !== input.route.workspaceDigest) {
        throw new OperateError('E_OPERATE_HEAD_DIVERGED', 'Workspace revisions, branches, remotes, or material dirty fingerprints changed after the route preview.', {
            expectedWorkspaceDigest: input.route.workspaceDigest,
            actualWorkspaceDigest: observed.workspaceDigest,
        });
    }
}
export async function readOperatingRoute(projectRoot, routeId) {
...
function localPlanningHandoffPath(projectRoot, routeId, localRoot) {
    return path.join(resolveOperatingPaths(projectRoot, { localRoot }).localRoot, 'planning-handoffs', `${routeId}.json`);
}
async function readOperatingRuntime(projectRoot, localRoot) {
    const target = path.join(resolveOperatingPaths(projectRoot, { localRoot }).localRoot, 'preferences.json');
    try {
...
function validateStoredPlanningHandoff(value, expected) {
    const handoff = value;
    if (handoff?.kind !== 'operating-planning-handoff' ||
        ...
        throw new OperateError('E_OPERATE_PLANNER_CONFLICT', 'The machine-local planning handoff does not match the accepted route.');
    }
    if (expected.planningEngine === 'pipeline-po' &&
        ...
        throw new OperateError('E_OPERATE_PLANNER_CONFLICT', 'The pipeline PO handoff digest is invalid.');
    }
    if (expected.planningEngine === 'openplanr' && handoff.prepared !== null) {
        throw new OperateError('E_OPERATE_PLANNER_CONFLICT', 'The OpenPlanr handoff contains an unexpected pipeline preparation.');
    }
    return handoff;
}
async function ensurePlanningHandoff(input) {
    const action = input.route.actions[0];
    const targetPath = path.posix.dirname(action.targetPath);
    const specId = action.targetPath?.match(/(?:^|\/)(SPEC-\d+)(?:-|\/)/)?.[1];
    if (!specId) {
        throw new OperateError('E_OPERATE_TRANSACTION_INVALID', 'DEV route target does not encode a canonical SPEC id.');
    }
    ...
    const prepared = input.config.planningEngine === 'pipeline-po'
    ...
    if (input.handoff.planningEngine === 'pipeline-po') {
        ...
        if (!completionRecorded) {
            await completePipelinePoHandoff({
                ...
                handoff: input.handoff.prepared,
        ...
        if (!(await hasPipelinePoCompletionProvenance({
            projectRoot: input.projectRoot,
            targetPath: input.handoff.targetPath,
            runId: input.handoff.prepared?.runId ?? input.handoff.transactionId,
        }))) {
            throw new OperateError('E_OPERATE_PLANNER_CONFLICT', 'Pipeline PO completion did not record the expected route-bound provenance.');
        }
    }
    return true;
}
function routeWritesPreviewDigest(inputDigest, writes, generationPlanDigest) {
...
async function routeWriteMaterialState(projectRoot, writes) {
    const observed = await Promise.all(writes.map(async (write) => {
    ...
    if (observed.every((state) => state === 'absent'))
        return 'absent';
    if (observed.every((state) => state === 'exact'))
        return 'exact';
    throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'A prepared route has partial or changed destination bytes; recover the journal before retrying.');
}
async function buildRouteWrites(input) {
    const action = input.route.actions[0];
    if (!action?.targetPath) {
        throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Route ${input.route.id} has no target path.`);
    }
    ...
    if (action.kind === 'create-cycle-artifact') {
        ...
        if (input.artifactGeneration.planDigest !== generationPlan.planDigest) {
            throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'The generated artifact no longer matches the accepted generation contract.');
        }
    ...
    const handoff = {
        ...
        planningEngine: input.config.planningEngine,
        state: 'awaiting-plan',
        invocation: input.config.planningEngine === 'pipeline-po'
...
export async function applyOperatingRoute(input) {
    ...
    const existingGeneration = isAgentArtifact
        ...
        : null;
    const acceptedApplyDigest = input.confirmationDigest === input.route.previewDigest ||
        (existingGeneration?.state === 'generated' &&
            input.confirmationDigest === existingGeneration.exactPreviewDigest);
    if (!acceptedApplyDigest) {
        throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', 'Route application requires confirmation of the exact preview digest.', {
            previewDigest: existingGeneration?.state === 'generated'
    ...
    const finding = initialState.findings.find((candidate) => candidate.id === input.route.actions[0]?.findingId);
    if (!finding) {
        throw new OperateError('E_OPERATE_STATE_INVALID', `Route ${input.route.id} references an unknown finding.`);
    }
    ...
    if (!['accepted', 'prepared'].includes(String(projectedRoute?.state)) ||
        typeof acceptedConfirmation !== 'string') {
        throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', 'Route must be accepted or recoverably prepared before it can be applied.');
    }
    return withOperatingLock(input.projectRoot, {
        projectKey: operatingProjectKey(input.projectRoot),
        expectedEventHead: initial.eventHead,
        ...
        try {
            ...
            if (!built.generationPlan && plannedDigest !== input.route.previewDigest) {
                throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'The exact route write set no longer matches the accepted preview.');
            }
            if (built.generationPlan) {
                const initialGenerationDigest = routeWritesPreviewDigest(input.route.inputDigest, [], built.generationPlan.planDigest);
                if (initialGenerationDigest !== input.route.previewDigest) {
                    throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'The artifact generation contract no longer matches the accepted route.');
                }
                if (artifactGeneration?.state !== 'generated') {
                    artifactGeneration = await generateOperatingRouteArtifact({
                        ...
                        adapter: input.artifactGenerator ??
                ...
                if (artifactGeneration.state !== 'generated' || !artifactGeneration.exactPreviewDigest) {
                    throw new OperateError('E_OPERATE_ARTIFACT_REJECTED', 'The AGENT artifact is not ready for exact-byte review.');
                }
                const exactDigest = routeWritesPreviewDigest(input.route.inputDigest, built.writes, built.generationPlan?.planDigest);
                if (exactDigest !== artifactGeneration.exactPreviewDigest ||
                    ...
                    return {
                        ...
                        artifact: {
                            ...
                            content: artifactGeneration.content,
                            outputDigest: artifactGeneration.session.outputDigest,
            ...
            await assertRouteWorkspaceCurrent(input);
            const transactionPreviewDigest = isAgentArtifact && artifactGeneration?.state === 'generated'
                ? artifactGeneration.exactPreviewDigest
                : input.route.previewDigest;
            ...
            const transaction = materialState === 'exact'
                ? {
                    root: path.join(resolveOperatingPaths(input.projectRoot, {
                        localRoot: input.localRoot,
                    }).transactions, transactionId),
                    manifestPath: path.join(resolveOperatingPaths(input.projectRoot, {
                        localRoot: input.localRoot,
                    }).transactions, transactionId, 'journal.json'),
                    record: await readJournal(path.join(resolveOperatingPaths(input.projectRoot, {
                        localRoot: input.localRoot,
                    }).transactions, transactionId, 'journal.json')),
                }
            ...
            if (materialState === 'exact' && transaction.record.state !== 'committed') {
                throw new OperateError('E_OPERATE_TRANSACTION_INVALID', 'Prepared route bytes exist without a committed journal.');
            }
            ...
            if (built.artifactSession &&
                !replayBeforeArtifact.events.some((event) => event.type === 'artifact.created' && event.entityId === built.artifactSession?.id)) {
                const artifactRecord = await store.putRecord('artifact-manifest', built.artifactSession, {
            ...
            if (committedTransaction && !bytesCommitted && !routeApplied) {
                await rollbackJournalTransaction(input.projectRoot, committedTransaction).catch(() => undefined);
            }
            ...
            const errorCode = error instanceof OperateError ? error.code : 'E_OPERATE_TRANSACTION_INVALID';
...
export async function rollbackOperatingRoute(input) {
    ...
    return withOperatingLock(input.projectRoot, {
        ...
        if (!projected ||
            ...
            throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'The route projection no longer matches the rollback transaction.');
        }
        ...
        if (revalidated.eventHead.sequence !== initial.eventHead.sequence ||
            revalidated.eventHead.hash !== initial.eventHead.hash) {
            throw new OperateError('E_OPERATE_HEAD_DIVERGED', 'Operating state changed before route rollback.');
        }
...
export async function routeDestinationDigest(projectRoot, route) {
    const destinations = await Promise.all(routeDestinationPaths(route).map(async (relativePath) => {
