...
export function assertGitReadOnlyArgs(args) {
    ...
    if (args.some((argument) => forbiddenRepositoryOverrides.some((option) => argument === option || argument.startsWith(`${option}=`)))) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', 'Git repository and configuration overrides are forbidden for evidence.');
    }
    const command = args.find((argument) => !argument.startsWith('-'));
    if (!command || !GIT_READ_COMMANDS.has(command)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Git operation is not allowlisted for evidence: ${command ?? '(missing)'}.`);
    }
...
export async function executeGitReadOnly(projectRoot, args, options = {}) {
...
export function assertGitCitationRevision(revision) {
    if (typeof revision !== 'string' || !GIT_CITATION_REVISION_PATTERN.test(revision)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Git citation revision must be a bounded hex object name: ${revision ?? '(missing)'}.`);
    }
}
function assertGitCitationRepositoryPath(relativePath) {
    if (typeof relativePath !== 'string' ||
        relativePath.length === 0 ||
        relativePath.length > 1024 ||
        relativePath.startsWith('/') ||
        !GIT_CITATION_PATH_PATTERN.test(relativePath)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Git citation path must be a bounded repository-relative path: ${relativePath ?? '(missing)'}.`);
    }
}
function assertGitPlanrCitationPath(relativePath) {
    if (typeof relativePath !== 'string' ||
        relativePath.length === 0 ||
        relativePath.length > 1024 ||
        relativePath.startsWith('/') ||
        ...
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Planr citation path must be a bounded .planr-relative path: ${relativePath ?? '(missing)'}.`);
    }
}
...
async function readGitBlob(projectRoot, revision, gitPath, options) {
    const maxBytes = Math.max(1, Math.min(options.maxBytes ?? MAX_CITATION_SNAPSHOT_BYTES, MAX_PROVIDER_OUTPUT));
    ...
    try {
        ...
        if (Buffer.byteLength(stdout, 'utf8') > maxBytes) {
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', `Cited content ${gitPath} exceeds the ${maxBytes}-byte snapshot bound.`);
        }
        return { exists: true, content: stdout, lineCount: countTextLines(stdout) };
    }
    catch (error) {
        if (error instanceof OperateError && error.code === 'E_OPERATE_EVIDENCE_REJECTED')
            throw error;
 ...
 */
export async function readGitPathAtRevision(projectRoot, revision, relativePath, options = {}) {
...
export async function readGitPlanrPathAtRevision(projectRoot, revision, planrPath, options = {}) {
    assertGitCitationRevision(revision);
    assertGitPlanrCitationPath(planrPath);
    return readGitBlob(projectRoot, revision, planrPath, options);
}
function parseGitTreeListing(stdout) {
    const entries = [];
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (index === 0 && /^tree \S+:/.test(lines[index]))
            continue;
 ...
 */
export async function listGitPlanrTreeAtRevision(projectRoot, revision, planrTreePath, options = {}) {
    assertGitCitationRevision(revision);
...
export async function gitRevisionResolves(projectRoot, revision, options = {}) {
    assertGitCitationRevision(revision);
    try {
        const stdout = await executeGitReadOnly(projectRoot, ['rev-parse', '--verify', '--quiet', `${revision}^{commit}`], { timeoutMs: options.timeoutMs });
...
export async function readGitCommitSummary(projectRoot, revision, options = {}) {
    assertGitCitationRevision(revision);
    try {
        const stdout = await executeGitReadOnly(projectRoot, ['log', '-1', '--no-color', '--format=%H%n%cI%n%s', revision], { timeoutMs: options.timeoutMs });
...
export function assertGitHubReadOnlyArgs(args) {
    const [resource, action] = args;
    if (!resource || !action || !GH_READ_COMMANDS[resource]?.has(action)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `GitHub operation is not allowlisted for evidence: ${resource ?? ''} ${action ?? ''}`.trim());
    }
    if (args.includes('--web') || args.includes('--jq') || args.includes('--template')) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', 'GitHub evidence cannot open browsers or execute output templates.');
    }
}
export async function executeGitHubReadOnly(projectRoot, args, options = {}) {
    ...
    if (!allowedHosts.includes(host)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `GitHub host ${host} is not allowlisted.`);
    }
...
export function assertReadOnlyRestRequest(endpoint, method, allowedHosts) {
    const url = new URL(endpoint);
    const normalizedMethod = method.toUpperCase();
    if (normalizedMethod !== 'GET' && normalizedMethod !== 'HEAD') {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Remote evidence permits GET and HEAD only, not ${normalizedMethod}.`);
    }
    if (url.protocol !== 'https:' ||
        ...
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', `Remote evidence host is not allowlisted: ${url.hostname || '(missing)'}.`);
    }
    return url;
}
function boundedPositiveInteger(value, fallback, maximum) {
    ...
    if (!Number.isSafeInteger(value) || value < 1) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Remote evidence limits must be positive integers.');
    }
    return Math.min(value, maximum);
}
async function readBoundedResponseText(response, maxBytes) {
    if (response.redirected || (response.status >= 300 && response.status < 400)) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', 'Remote evidence redirects are forbidden.');
    }
    if (!response.ok) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', `Remote evidence request failed with HTTP ${response.status}.`);
    }
    const rawLength = response.headers.get('content-length');
    if (rawLength !== null) {
        if (!/^\d+$/.test(rawLength) || Number(rawLength) > maxBytes) {
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Remote evidence Content-Length is invalid or exceeds the configured limit.');
        }
    }
    if (!response.body)
        return '';
        ...
        while (true) {
            ...
            if (bytes > maxBytes) {
                await reader
                    ...
                    .catch(() => undefined);
                throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Remote evidence streamed response exceeds the configured byte limit.');
            }
    ...
    const merged = Buffer.concat(chunks.map((chunk) => Buffer.from(chunk)), bytes);
    try {
...
async function fetchBoundedJson(endpoint, init, options) {
    const timeoutMs = boundedPositiveInteger(options.timeoutMs, DEFAULT_REMOTE_TIMEOUT_MS, 60_000);
    const maxBytes = boundedPositiveInteger(options.maxBytes, DEFAULT_REMOTE_MAX_BYTES, DEFAULT_REMOTE_MAX_BYTES);
    ...
    try {
        ...
        if (response.status !== 204 &&
            ...
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Remote evidence response is not JSON.');
        }
        ...
        try {
            ...
            if (error instanceof OperateError)
                throw error;
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Remote evidence response contains malformed JSON.');
        }
    }
    catch (error) {
        if (error.name === 'AbortError') {
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', `Remote evidence request exceeded the ${timeoutMs}ms timeout.`);
        }
        throw error;
    }
    finally {
...
export async function executeRestReadOnlyJson(endpoint, options) {
...
export function assertLinearReadOnlyQuery(endpoint, query) {
    const url = new URL(endpoint);
    if (url.protocol !== 'https:' ||
        ...
        url.search ||
        url.hash) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', 'Linear evidence endpoint must be https://api.linear.app/graphql.');
    }
    const normalized = stripGraphqlComments(query);
    if (/\b(?:mutation|subscription)\b/i.test(normalized) ||
        (!normalized.startsWith('query') && !normalized.startsWith('{'))) {
        throw new OperateError('E_OPERATE_PROVIDER_READ_ONLY', 'Linear evidence permits GraphQL query operations only.');
    }
}
export class ReadOnlyLinearTransport {
    ...
    async query(query, variables) {
        ...
        return this.transport.query(query, variables);
    }
}
export async function executeLinearReadOnlyQuery(input) {
    const endpoint = input.endpoint ?? 'https://api.linear.app/graphql';
    assertLinearReadOnlyQuery(endpoint, input.query);
    if (!input.token.trim()) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Linear evidence credentials are unavailable.');
    }
    ...
    if (Buffer.byteLength(requestBody) > 256 * 1024) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Linear evidence query exceeds the request-size limit.');
    }
    const response = await fetchBoundedJson(new URL(endpoint), {
        method: 'POST',
        headers: {
    ...
    if (response.errors?.length || response.data === undefined) {
        throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', 'Linear evidence query returned a GraphQL error.');
    }
    return response.data;
}
//# sourceMappingURL=read-only-providers.js.map