...
import { operatingInitQuestionRegistry, repeatedTextRenderability, } from './question-registry.js';
const OPERATE_PROFILE_IDS = [
    'saas',
    'product',
    'engineering',
    'custom',
 ...
 */
async function existingOperatingProfileId(projectRoot) {
    if (!projectRoot)
        return undefined;
    ...
    const id = parsed?.id;
    return typeof id === 'string' && OPERATE_PROFILE_IDS.includes(id)
        ? id
        : undefined;
}
/**
 ...
 */
async function withDetectedSuggestions(context) {
    if (context.existingProfileId !== undefined)
        return context;
 ...
 */
function decorateGuidedPresentation(questionnaire) {
    return {
        ...questionnaire,
        questions: questionnaire.questions.map((question) => {
            if (question.type !== 'repeated-text')
                return question;
...
const STAGES = ['foundation', 'product-charter', 'review'];
export function operatingInitAnswersFromOptions(options) {
    ...
    const charter = options.charter && typeof options.charter === 'object' && !Array.isArray(options.charter)
        ? options.charter
        : {
    ...
    return {
        ...
        ...(typeof options.sensitivityCeiling === 'string'
            ? {
                sensitivityCeiling: options.sensitivityCeiling,
            }
            : {}),
...
export function mergeOperatingInitAnswersIntoOptions(options, answers) {
    return {
        ...options,
        ...answers,
...
function conditionVisible(definition, definitions, answers) {
    return (definition.question.visibleWhen ?? []).every((condition) => {
        const dependency = definitions.find((candidate) => candidate.question.questionId === condition.questionId);
...
function normalizeValue(question, value) {
    ...
    if (choices &&
        ...
        throw new OperateError('E_OPERATE_QUESTIONNAIRE_INVALID', `${question.label} contains an unsupported choice.`);
    }
    ...
    if ((length !== undefined &&
        ...
        throw new OperateError('E_OPERATE_QUESTIONNAIRE_INVALID', `${question.label} does not satisfy its bounded validation rules.`);
    }
    return normalized;
}
function validateSemanticAnswers(answers) {
    if (answers.timezone) {
        try {
            new Intl.DateTimeFormat('en', { timeZone: answers.timezone }).format(new Date(0));
        }
        catch {
            throw new OperateError('E_OPERATE_QUESTIONNAIRE_INVALID', `Invalid IANA timezone: ${answers.timezone}.`);
        }
    }
}
export async function evaluateOperatingInitQuestions(input) {
    ...
    for (const stage of STAGES.slice(0, input.requireCharter === false ? 1 : 2)) {
        const visible = definitions.filter((definition) => definition.stage === stage && conditionVisible(definition, definitions, answers));
        const missing = visible.filter((definition) => definition.question.type !== 'informational' &&
            definition.question.required &&
            !answered(definition.read(answers)));
        if (missing.length > 0) {
            const suggestions = stage === 'product-charter'
            ...
            const toAsk = visible.filter((definition) => definition.question.type !== 'informational' &&
                !answered(definition.read(answers)) &&
            ...
            return {
                ...
                questions: toAsk.map((definition) => {
                    ...
                    const suggestion = suggestions?.suggestions.find((entry) => entry.field === question.questionId);
                    if (!suggestion)
                        return question;
                    return {
                        ...question,
...
export function applyOperatingInitAnswer(answers, context, questionId, value) {
    const definition = operatingInitQuestionRegistry(context).find((candidate) => candidate.question.questionId === questionId);
    if (!definition) {
        throw new OperateError('E_OPERATE_QUESTIONNAIRE_INVALID', `Unknown Operating Board question: ${questionId}.`);
    }
...
export async function createOperatingInitQuestionnaire(input) {
    ...
    const expiresAt = input.expiresAt ?? new Date(new Date(createdAt).getTime() + 24 * 60 * 60 * 1000).toISOString();
    const projectIdentity = input.context.projectIdentity ??
        canonicalDigest({
    ...
    const questionnaireBase = {
        ...
        description: input.stage === 'review'
    ...
    try {
        ...
        throw new OperateError(error instanceof Error && error.name === 'E_PIPELINE_NOT_INSTALLED'
            ...
            : 'E_PIPELINE_VERSION_INCOMPATIBLE', error instanceof Error ? error.message : 'Compatible guided validators are unavailable.');
    }
    const withoutDigest = {
        ...questionnaireBase,
    ...
    if (errors.length > 0) {
        throw new OperateError('E_OPERATE_QUESTIONNAIRE_INVALID', 'The generated Operating Board questionnaire did not satisfy Protocol v1.2.', { validationErrors: errors });
    }
