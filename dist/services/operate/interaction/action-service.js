...
import { enforceProposalCitations, } from '../citation-resolution.js';
import { containsSecret } from '../redaction.js';
import { OperateError, } from '../types.js';
import { createOperatingConfirmation, } from './confirmation-service.js';
function assertSafeCommand(command) {
    if (!/^planr [^\r\n]{1,1024}$/.test(command) ||
        ...
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Structured Operating Board actions must use a safe public planr command without authority flags or sensitive machine data.');
    }
}
async function validateStructuredAction(action) {
    const validators = await resolveGuidedInteractionValidators();
    if (validators.validateStructuredAction(action).length > 0) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', `Structured action ${action.id} failed Protocol v1.2 validation.`);
    }
}
export async function createOperatingAction(input) {
    ...
    if (Boolean(input.providerUse) !== providerUse && input.providerUse !== undefined) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Provider use must match the structured action effect.');
    }
    if (input.effect === 'read-only') {
        if (input.confirmation) {
            throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Read-only actions cannot carry confirmation authority.');
        }
        const action = {
    ...
    if (!input.confirmation) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', `Action ${input.id} requires digest-bound confirmation material.`);
    }
 ...
 */
export async function enforceRecordedProposalCitations(proposals, context) {
    return enforceProposalCitations(proposals, context);
}
export function sanitizeActionDestination(projectRoot, target) {
    const relative = path.relative(path.resolve(projectRoot), path.resolve(projectRoot, target));
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
        throw new OperateError('E_OPERATE_PATH_ESCAPE', 'Structured action destinations must stay inside the project.');
    }
