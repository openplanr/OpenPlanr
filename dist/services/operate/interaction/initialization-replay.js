 ...
 */
export function encodeOperatingInitializationReplay(answers) {
    ...
    if (raw.byteLength > MAX_REPLAY_BYTES) {
        throw new OperateError('E_OPERATE_INPUT_TOO_LARGE', `Initialization answers exceed the ${MAX_REPLAY_BYTES}-byte replay limit.`);
    }
    const token = deflateRawSync(raw, { level: 9 }).toString('base64url');
    if (token.length > MAX_TOKEN_LENGTH) {
        throw new OperateError('E_OPERATE_INPUT_TOO_LARGE', 'Encoded initialization replay token exceeds the supported command limit.');
    }
    return token;
}
export function decodeOperatingInitializationReplay(token) {
    if (token.length === 0 || token.length > MAX_TOKEN_LENGTH || !BASE64URL.test(token)) {
        throw new OperateError(token.length > MAX_TOKEN_LENGTH ? 'E_OPERATE_INPUT_TOO_LARGE' : 'E_OPERATE_SESSION_INVALID', 'Initialization replay token is malformed or exceeds the supported limit.');
    }
    try {
        ...
        const normalized = normalizeOperatingInitializationAnswers(parsed);
        ...
        if (error instanceof OperateError)
            throw error;
        throw new OperateError('E_OPERATE_SESSION_INVALID', 'Initialization replay token is invalid or corrupted.');
    }
}
//# sourceMappingURL=initialization-replay.js.map