export declare const OPERATE_PROTOCOL_VERSION: "1.2.0";
export declare const OPERATE_SCHEMA_VERSION: "1.0.0";
/**
 ...
 */
export declare const OPERATE_MISSION_PROTOCOL_VERSION: "1.3.0";
export declare const OPERATE_AGENT_PROTOCOL_VERSION: "1.4.0";
...
export type GuidedQuestionType = 'text' | 'secret' | 'single-select' | 'multi-select' | 'confirmation' | 'path' | 'repeated-text' | 'informational';
...
export type GuidedSessionState = 'created' | 'awaiting-input' | 'revising' | 'preview-ready' | 'confirmed' | 'applied' | 'cancelled' | 'expired' | 'stale' | 'invalid';
export interface GuidedSession {
    ...
    questionnaireVersion: '1.0.0';
    adapter: GuidedQuestionnaire['adapter'];
    persistedAnswers: Array<Omit<GuidedAnswer, 'sensitivity'> & {
...
export type OperatingActionEffect = 'read-only' | 'machine-local-write' | 'project-write' | 'provider-call' | 'external-effect';
...
export type OperatingRoleId = 'strategy-finance' | 'technology-risk' | 'product-activation' | 'growth-market' | 'operations-customer' | 'chair';
export type OperatingLane = 'DEV' | 'OWNER' | 'AGENT';
export type OperatingCycleStatus = 'preparing' | 'collecting' | 'advising' | 'consolidating' | 'reviewable' | 'closed' | 'blocked' | 'failed' | 'cancelled';
export type OperatingCriticalRiskCategory = 'security' | 'privacy' | 'payment-integrity' | 'legal' | 'tenant-isolation' | 'destructive-data';
...
export interface OperatingAdapterMachineAction {
    id: string;
    action: 'adapter.prepare' | 'adapter.record' | 'adapter.finalize' | 'adapter.resume' | 'adapter.cancel' | 'harness.prepare' | 'harness.record' | 'harness.finalize' | 'harness.resume' | 'harness.cancel' | 'run.continue';
    effect: OperatingActionEffect;
    role?: string;
    argv: string[];
    dispatch?: {
...
export interface OperatingAdapterHandoff extends ProtocolArtifact<'operating-adapter-handoff'> {
    ...
    state: 'prepare-required' | 'record-required' | 'finalize-required' | 'continue-required' | 'cancelled';
    binding: {
        cycleId: string;
...
export type OperateErrorCode = 'E_OPERATE_INTERNAL' | 'E_OPERATE_PROJECT_REQUIRED' | 'E_OPERATE_NOT_INITIALIZED' | 'E_OPERATE_CONFIG_INVALID' | 'E_OPERATE_CHARTER_INCOMPLETE' | 'E_OPERATE_PATH_ESCAPE' | 'E_OPERATE_STATE_INVALID' | 'E_OPERATE_STATE_UNAVAILABLE' | 'E_OPERATE_HEAD_DIVERGED' | 'E_OPERATE_CHECKPOINT_INVALID' | 'E_OPERATE_CYCLE_ACTIVE' | 'E_OPERATE_CYCLE_INPUT_CONFLICT' | 'E_OPERATE_CYCLE_NOT_DISPOSED' | 'E_OPERATE_STALE_LOCK_UNSAFE' | 'E_OPERATE_TRANSACTION_INVALID' | 'E_OPERATE_EVIDENCE_REJECTED' | 'E_OPERATE_EVIDENCE_BUDGET' | 'E_OPERATE_SECRET_DETECTED' | 'E_OPERATE_PROVIDER_READ_ONLY' | 'E_OPERATE_ADVISOR_ISOLATION' | 'E_OPERATE_ADVISOR_FAILED' | 'E_OPERATE_RUNTIME_MISMATCH' | 'E_OPERATE_DRAFT_UNAPPROVED' | 'E_RUNTIME_UNSUPPORTED' | 'E_OPERATE_EVIDENCE_NOT_READY' | 'E_OPERATE_CAP_EXCEEDED' | 'E_OPERATE_CRITICAL_CAP' | 'E_OPERATE_AUTHORITY_REQUIRED' | 'E_OPERATE_ROUTE_CONFIRMATION_REQUIRED' | 'E_OPERATE_ROUTE_DRIFT' | 'E_OPERATE_PLANNER_CONFLICT' | 'E_OPERATE_OUTCOME_NOT_READY' | 'E_OPERATE_PROJECTION_DRIFT' | 'E_OPERATE_ARTIFACT_REJECTED' | 'E_OPERATE_MIGRATION_CONFLICT' | 'E_OPERATE_SECURITY_REPAIR_REQUIRED' | 'E_OPERATE_INPUT_TOO_LARGE' | 'E_OPERATE_INPUT_REQUIRED' | 'E_OPERATE_QUESTIONNAIRE_INVALID' | 'E_OPERATE_SESSION_INVALID' | 'E_OPERATE_SESSION_EXPIRED' | 'E_OPERATE_SESSION_STALE' | 'E_OPERATE_SESSION_CANCELLED' | 'E_OPERATE_SESSION_REPLAY_CONFLICT' | 'E_PIPELINE_VERSION_INCOMPATIBLE' | 'E_OPERATE_ACTION_UNKNOWN' | 'E_OPERATE_MISSION_PACKET_BUDGET' | 'E_OPERATE_MISSION_UNAVAILABLE' | 'E_OPERATE_PROVIDER_DEPRECATED' | 'E_PIPELINE_NOT_INSTALLED';
export declare class OperateError extends Error {
    readonly code: OperateErrorCode;
    readonly details?: Readonly<Record<string, unknown>>;
    constructor(code: OperateErrorCode, message: string, details?: Readonly<Record<string, unknown>>);
}
...
export interface OperatingWorkspaceManifest extends ProtocolArtifact<'operating-workspace-manifest'> {
    capturedAt: string;
    workspaceDigest: `sha256:${string}`;
    controlRepository: OperatingWorkspaceComponent & {
        readOnly: false;
    };
    components: Array<OperatingWorkspaceComponent & {
        readOnly: true;
    }>;
}
...
export interface OperatingRecordEnvelope extends Omit<ProtocolArtifact<'operating-record'>, 'protocolVersion'> {
    /**
     ...
     */
    protocolVersion: typeof OPERATE_PROTOCOL_VERSION | typeof OPERATE_MISSION_PROTOCOL_VERSION | typeof OPERATE_AGENT_PROTOCOL_VERSION;
    digest: `sha256:${string}`;
    recordType: 'evidence-metadata' | 'advisor-result' | 'advisor-report' | 'finding' | 'decision' | 'data-gap' | 'route' | 'spec-link' | 'outcome' | 'artifact-manifest' | 'migration' | 'recovery';
    createdAt: string;
    correlationId: string;
    contentDigest: `sha256:${string}`;
    content: Record<string, unknown>;
}
export type OperatingEventType = `cycle.${OperatingCycleStatus}` | 'evidence.collected' | 'advisory.recorded' | 'finding.proposed' | 'finding.accepted' | 'finding.queued' | 'finding.in-progress' | 'finding.done' | 'finding.rejected' | 'finding.superseded' | 'decision.open' | 'decision.answered' | 'decision.closed' | 'decision.default-due' | 'decision.superseded' | 'gap.open' | 'gap.answered' | 'gap.verified' | 'gap.closed' | 'gap.superseded' | 'route.proposed' | 'route.accepted' | 'route.prepared' | 'route.applied' | 'route.failed' | 'route.rolled_back' | 'spec.linked' | 'artifact.created' | 'ship.observed' | 'outcome.registered' | 'outcome.observed' | 'outcome.evaluated' | 'learning.recorded' | 'projection.rebuilt' | 'migration.legacy-imported' | 'recovery.performed' | 'security.discontinuity';
export interface OperatingEvent extends ProtocolArtifact<'operating-event'> {
    ...
    actor: {
        kind: 'human' | 'engine' | 'runtime' | 'migration';
        id: string;
    };
...
export interface OperatingEvidenceReadiness extends ProtocolArtifact<'operating-evidence-readiness'> {
    cycleId: string;
    inputDigest: `sha256:${string}`;
    evaluatedAt: string;
    roles: Array<{
        ...
        requirements: Array<EvidenceRequirement & {
...
export interface OperatingProviderManifest extends ProtocolArtifact<'operating-provider-manifest'> {
    ...
    permittedDataClasses: Array<'source-code' | 'planning-artifacts' | 'git-metadata' | 'issue-metadata' | 'project-metadata' | 'outcome-observations' | 'imported-documents'>;
    retention: {
    ...
    consent: {
        ...
        renewalTriggers: Array<'policy-change' | 'scope-expansion' | 'credential-renewal' | 'scheduled-review'>;
    };
...
export interface OperatingRouteAction {
    ...
    lane: OperatingLane;
    owner: string;
    kind: 'create-spec' | 'create-instrumentation-spec' | 'create-decision' | 'create-cycle-artifact' | 'create-quick-task' | 'create-epic';
    dependsOn: string[];
    evidenceRefs: string[];
    reversible: true;
...
export interface OperatingRoutePlan extends Omit<ProtocolArtifact<'operating-route-plan'>, 'protocolVersion'> {
    /**
...
export interface OperatingOutcome extends ProtocolArtifact<'operating-outcome'> {
    ...
    aggregation: 'sum' | 'average' | 'median' | 'minimum' | 'maximum' | 'count' | 'rate' | 'percentile' | 'latest';
    baselineWindow: {
        from: string;
        to: string;
    };
...
export interface OperatingOutcomeObservation extends ProtocolArtifact<'operating-outcome-observation'> {
    id: string;
    outcomeId: string;
    observedAt: string;
    window: {
...
export interface OperatingState extends ProtocolArtifact<'operating-state'> {
    ...
    findings: Array<Record<string, unknown> & {
        id: string;
        status: OperatingFinding['status'];
    }>;
    ...
    dataGaps: Array<Record<string, unknown> & {
        id: string;
        status: OperatingDataGap['status'];
    }>;
    routes: Array<Record<string, unknown> & {
        id: string;
        state: OperatingRoutePlan['state'];
    }>;
    specLinks: Array<Record<string, unknown> & {
        specId: string;
    }>;
    outcomes: Array<Record<string, unknown> & {
        id: string;
        status: OperatingOutcome['status'];
    }>;
...
export interface OperatingTransactionJournal extends ProtocolArtifact<'operating-transaction-journal'> {
