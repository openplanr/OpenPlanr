...
import { applyOperatingInitialization, getOperatingProfile, listOperatingProfiles, normalizeCustomOperatingProfile, normalizeOperatingInitializationAnswers, prepareOperatingInitialization, readOperatingLastRunAt, validateOperatingConfiguration, } from './config.js';
import { operatingInitializationAnswersFromResearch, prepareOperatingContextResearch, readOperatingContextResearch, recordOperatingContextResearch, } from './context-research.js';
import { approveOperatingDraft, discardOperatingDraft, listOperatingDrafts, materializeOperatingDrafts, showOperatingDraft, } from './drafts.js';
...
import { persistableOperatingInitAnswers, probeGitUserName, probePipelineInstalled, resumeGuidedSession, submitGuidedAnswers, } from './interaction/answer-service.js';
...
import { cancelGuidedSession, createGuidedSession, createGuidedSessionId, currentGuidedSessionBindings, updateGuidedSession, } from './interaction/session-service.js';
...
import { assertOperatingProject, resolveContainedPath, resolveOperatingPaths, } from './workspace.js';
...
function option(request, name, fallback) {
...
async function resolvedOperatingRuntime(projectRoot, requested) {
    if (requested !== 'auto')
        return requested === 'claude' ? 'claude-code' : requested;
    const persisted = await readFile(path.join(resolveOperatingPaths(projectRoot).localRoot, 'preferences.json'), 'utf8')
        ...
        .catch(() => 'auto');
    if (persisted !== 'auto')
        return persisted === 'claude' ? 'claude-code' : persisted;
...
async function readCustomOperatingProfile(projectRoot, file) {
    ...
    try {
        ...
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Custom profile must be a valid bounded JSON object.');
    }
}
function success(action, value = {}) {
...
export function failure(action, error) {
    ...
    const confirmationAction = error instanceof OperateError &&
        ...
        ? error.details.action
        : null;
    const confirmationRecovery = code === 'E_OPERATE_ROUTE_CONFIRMATION_REQUIRED' &&
        ...
        : null;
    const explicitRecovery = error instanceof OperateError && typeof error.details?.recoveryCommand === 'string'
        ? [error.details.recoveryCommand]
        : null;
    const nextActions = explicitRecovery ??
    ...
    return {
        ...
        action,
        code,
        message: error instanceof OperateError
            ? error.message
...
async function actionEffect(request, command) {
    if (/\b(?:inspect|status|brief|report|review|list|show)\b/.test(command) ||
    ...
    if (/\boperate run\b/.test(command)) {
        if (hasFlag(command, '--offline'))
            return 'project-write';
        const commandRuntime = command.match(/(?:^|\s)--runtime\s+(\S+)/)?.[1];
        if (await usesNativeOperatingAdvisors(request.projectRoot, commandRuntime ?? option(request, 'runtime', 'auto'))) {
            return 'project-write';
        }
        return 'provider-call';
    }
    if (/\badapter resume\b/.test(command))
        return 'read-only';
    if (/\badapter finalize\b/.test(command))
        return 'project-write';
    if (/\b(?:adapter (?:prepare|record|cancel)|cache purge|diagnostics export)\b/.test(command)) {
        return 'machine-local-write';
    }
    if (/\bpipeline (?:plan|ship)\b/.test(command))
...
async function attachStructuredActions(request, result) {
    ...
    if (!resolveOperatingPipelineRoot())
        return result;
    ...
    const requestDigest = canonicalDigest({
        ...
        options: Object.fromEntries(Object.entries(request.options)
...
async function inspect(request) {
    ...
    const initialized = await readFile(paths.config, 'utf8').then(() => true, () => false);
    let project = false;
    try {
...
async function initializationApplyAction(input) {
    ...
    return createOperatingAction({
        ...
        description: 'Write only the reviewed charter, workspace, configuration, and machine-local preferences.',
        command,
        ...
        confirmation: {
            ...
            writes: input.preview.writes.map((write) => `${write.relativePath}:${write.operation ?? 'replace'}:${sha256Digest(write.content)}`),
        },
    });
}
async function initialize(request) {
    await loadOperatingProtocol();
    const replayToken = option(request, 'answersToken', undefined);
    ...
    const resumeId = option(request, 'resume', undefined);
    const localRoot = option(request, 'localRoot', undefined);
    if (option(request, 'cancelSession', false)) {
        if (!resumeId) {
            throw new OperateError('E_OPERATE_SESSION_INVALID', '--cancel-session requires --resume <session-id>.');
        }
        return success(request.action, {
    ...
    else if (supplied.profile !== undefined &&
    ...
    const preview = await prepareOperatingInitialization({
        ...
        localRoot,
        now: resumedSession?.session.createdAt ??
            option(request, 'previewCreatedAt', undefined),
    });
    ...
    const requestedConfirmation = option(request, 'confirm', undefined);
    ...
    if (!confirmed || !requestedConfirmation || !confirmation) {
        throw new OperateError('E_OPERATE_ROUTE_CONFIRMATION_REQUIRED', 'Initialization requires the exact named confirmation returned by its preview.', {
            preview: { ...previewData, confirmation },
            action: applyAction,
        });
    }
...
async function showConfig(request) {
...
async function profiles(request) {
    ...
    if (request.action === 'profiles.show') {
        ...
        return success(request.action, {
            data: getOperatingProfile(profile),
        });
    }
    const file = argument(request, 'file');
    if (!file)
        throw new OperateError('E_OPERATE_CONFIG_INVALID', 'Profile file is required.');
...
async function run(request) {
    ...
    if (resolvedRuntime !== 'auto' &&
        ...
        throw new OperateError('E_RUNTIME_UNSUPPORTED', `Runtime ${resolvedRuntime} does not expose a compatible agent-native Operate workflow.`, {
            runtime: resolvedRuntime,
            recovery: 'Run `planr setup --scope user`, then restart the runtime so its generated Operate workflow is available.',
        });
    }
    const deferAdvisors = !option(request, 'offline', false) &&
    ...
    const result = await runOperatingCycle({
        ...
        focus: stringList(request.options.focus),
    ...
    if (drafts.created.length > 0 && result.state) {
        await persistOperatingProjections({
            projectRoot: request.projectRoot,
            state: result.state,
            revalidateEventHead: async () => (await new OperatingEventStore(request.projectRoot).replay()).eventHead,
        });
    }
    const counts = {
        findings: result.state?.findings.filter((entry) => entry.cycleId === result.cycle.id).length ?? 0,
        decisions: result.state?.decisions.filter((entry) => entry.cycleId === result.cycle.id).length ?? 0,
...
async function drafts(request) {
    ...
    if (!option(request, 'yes', false)) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', `${request.action} requires explicit confirmation with --yes.`);
    }
...
async function contextResearch(request) {
    ...
    if (request.action === 'context.review') {
        return success(request.action, {
            ...
            message: 'Runtime-authored context was validated against the workspace and is ready for one compact owner review.',
            next: ['planr operate init'],
        });
    }
    const runtime = await resolvedOperatingRuntime(request.projectRoot, option(request, 'runtime', 'auto'));
    if (runtime === 'auto') {
        throw new OperateError('E_RUNTIME_UNSUPPORTED', 'Context research requires a selected Claude Code, Codex, or Cursor runtime.');
    }
    const researchMode = option(request, 'research', 'local');
    const consentDigest = researchMode === 'connected'
        ? canonicalDigest({
    ...
    if (researchMode === 'connected' && !preview && !option(request, 'yes', false)) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'Connected research requires the exact preview and explicit confirmation.', {
    ...
    const confirmed = option(request, 'confirm', undefined);
    if (researchMode === 'connected' && !preview && confirmed !== consentDigest) {
        throw new OperateError('E_OPERATE_AUTHORITY_REQUIRED', 'Connected research confirmation does not match the current project and runtime.', { confirmationDigest: consentDigest });
    }
...
async function reviewOrBrief(request) {
    ...
    return success(request.action, {
        data,
        message: request.action === 'review'
            ? 'This is the mandatory human review gate. No route has been applied.'
            : undefined,
    });
}
async function report(request) {
    const requestedFormat = option(request, 'format', undefined);
    if (requestedFormat && !['markdown', 'json'].includes(requestedFormat)) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', `Unknown report format ${requestedFormat}. Use markdown or json.`);
    }
...
async function collections(request) {
    const [collection, operation] = request.action.split('.');
    const singular = collection === 'cycles'
        ? 'cycleId'
        : collection === 'findings'
            ? 'findingId'
...
async function cycleMutation(request) {
    const cycleId = argument(request, 'cycleId');
    if (!cycleId)
        throw new OperateError('E_OPERATE_STATE_INVALID', 'Cycle ID is required.');
    const action = request.action.slice('cycles.'.length);
    return success(request.action, {
...
async function findingMutation(request) {
    const findingId = argument(request, 'findingId');
    if (!findingId)
        throw new OperateError('E_OPERATE_STATE_INVALID', 'Finding ID is required.');
    return success(request.action, {
        data: await governOperatingFinding({
            projectRoot: request.projectRoot,
            findingId,
            action: request.action.slice('findings.'.length),
            confirmed: option(request, 'yes', false),
...
async function routeMutation(request) {
    const routeId = argument(request, 'routeId');
    if (!routeId)
        throw new OperateError('E_OPERATE_STATE_INVALID', 'Route ID is required.');
    ...
    return success(request.action, {
        data,
        nextActions: data.state === 'awaiting-artifact-review'
            ? [
...
async function answerMutation(request) {
    if (request.action === 'gaps.verify') {
        const gapId = argument(request, 'gapId');
        if (!gapId)
            throw new OperateError('E_OPERATE_STATE_INVALID', 'Gap ID is required.');
        return success(request.action, {
    ...
    if (request.action === 'decisions.decide') {
        ...
        if (!decisionId)
            throw new OperateError('E_OPERATE_STATE_INVALID', 'Decision ID is required.');
        return success(request.action, {
...
async function maintenance(request) {
    ...
    const data = await operateAdapterLifecycle({
        projectRoot: request.projectRoot,
        action: request.action.slice(lifecycleNamespace.length),
        cycleId: option(request, 'cycleId', undefined),
    ...
    const handoff = data && typeof data === 'object' && 'handoff' in data && data.handoff
        ? data.handoff
        : undefined;
 ...
 */
export async function executeOperateAction(request) {
    try {
