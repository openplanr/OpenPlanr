 ...
 */
export const OPAQUE_ORIGIN_DECISION_BRIEF_SANDBOX = Object.freeze({
    network: 'none',
    filesystem: 'none',
    tools: Object.freeze([]),
 ...
 */
async function loadOperatingDecisionBriefBuilder() {
    const root = resolveOperatingPipelineRoot();
    if (!root) {
        throw new OperateError('E_PIPELINE_NOT_INSTALLED', 'Rendering a self-contained operating decision brief requires the full planr-pipeline package.', {
            recovery: 'Run `npm install -g openplanr@latest` (without `--omit=optional`), then `planr setup --scope user`.',
        });
    }
    const module = (await import(pathToFileURL(path.join(root, 'lib', 'pipeline', 'index.mjs')).href));
    if (typeof module.createOperatingDecisionBriefArtifact !== 'function') {
        throw new OperateError('E_PIPELINE_NOT_INSTALLED', 'The installed pipeline does not expose the self-contained decision-brief renderer.');
    }
 ...
 */
export function filterEvidenceByCeiling(evidence, ceiling) {
    const kept = [];
    const redactedRefs = [];
    for (const item of evidence) {
        if (compareSensitivity(item.sensitivity, ceiling) > 0)
            redactedRefs.push(item.ref);
        else
            kept.push(item);
    }
...
function redactedFacts(decision) {
    if (!decision)
        return null;
    const facts = {};
    for (const key of [
 ...
 */
export function buildDecisionBriefInput(source, ceiling) {
 ...
 */
export async function renderOperatingDecisionBriefArtifact(source, ceiling) {
    ...
    if (artifact?.kind !== 'html' || typeof artifact.html !== 'string') {
        throw new OperateError('E_OPERATE_ARTIFACT_REJECTED', 'The decision-brief renderer did not return a single HTML artifact.');
    }
    return {
        envelope,
        html: artifact.html,
 ...
 */
export async function readOperatingSensitivityCeiling(projectRoot, options = {}) {
...
//# sourceMappingURL=decision-brief.js.map