...
function isBoundaryRefusal(gap) {
    if (field(gap, 'category') === 'boundary-refusal')
        return true;
 ...
 */
export function buildOperatingIntegritySummary(state, cycleId) {
    const gaps = state.dataGaps.filter((gap) => gap.cycleId === undefined || gap.cycleId === cycleId);
    const citationRejections = [];
    const boundaryRefusals = [];
    const notEvaluatedRoles = [];
    for (const gap of gaps) {
        ...
        if (category === 'unresolvable-citation' || category === 'boundary-refusal') {
            ...
            if (isBoundaryRefusal(gap))
                boundaryRefusals.push(entry);
            else
                citationRejections.push(entry);
            continue;
        }
        if (category === 'missing-evidence') {
            const reason = field(gap, 'reason') || 'The role grounded no evidence and is recorded not_evaluated.';
    ...
    const byGapId = (left, right) => left.gapId.localeCompare(right.gapId);
    citationRejections.sort(byGapId);
    boundaryRefusals.sort(byGapId);
    notEvaluatedRoles.sort((left, right) => left.roleId.localeCompare(right.roleId) || byGapId(left, right));
    return {
        cycleId,
        citationRejections,
        boundaryRefusals,
 ...
 */
export function renderOperatingIntegritySection(summary) {
    ...
    lines.push(...(summary.citationRejections.length > 0
        ? summary.citationRejections.map((entry) => `- **${entry.gapId}** (${entry.reason}): ${entry.detail}${entry.affectedRoles.length > 0 ? ` Affected: ${entry.affectedRoles.join(', ')}.` : ''}`)
        : ['- None.']));
    lines.push('', '## Boundary refusals', '');
    lines.push(...(summary.boundaryRefusals.length > 0
        ? summary.boundaryRefusals.map((entry) => `- **${entry.gapId}** (${entry.reason}): ${entry.detail}${entry.affectedRoles.length > 0 ? ` Affected: ${entry.affectedRoles.join(', ')}.` : ''}`)
        : ['- None.']));
    lines.push('', '## Not-evaluated roles', '');
    lines.push(...(summary.notEvaluatedRoles.length > 0
        ? summary.notEvaluatedRoles.map((role) => `- **${role.roleId}** (${role.gapId}): ${role.reason}`)
        : ['- None.']));
    return lines.join('\n');
}
/**
 ...
 * artifacts.
 */
export function renderOperatingIntegrityDocument(summary) {
    return [
...
export async function detectGitignoredWorkspace(projectRoot) {
    try {
        ...
        return {
            ignored: true,
            message: 'This project’s `.planr/` is gitignored, so the Operating Board is not tracked or ' +
        ...
        if (code === 1) {
            return {
                ignored: false,
                message: '`.planr/` is not gitignored; the sanitized Operating Board content is eligible to be ' +
                    'committed and versioned alongside the code.',
            };
        }
        return {
            ignored: false,
            message: 'Git tracking status for `.planr/` could not be determined (this may not be a git ' +
