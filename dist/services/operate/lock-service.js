...
function assertLeaseDuration(value) {
    if (!Number.isInteger(value) ||
        ...
        throw new OperateError('E_OPERATE_STATE_INVALID', `Lock lease must be ${MINIMUM_LEASE_DURATION_MS}-${MAXIMUM_LEASE_DURATION_MS}ms.`);
    }
}
async function writeRecord(handle, record) {
...
function assertHead(expected, current) {
    if (!headsEqual(expected, current)) {
        throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'The operating event head changed after preview.', { expected, actual: current });
    }
}
export async function readOperatingLock(lockPath) {
    const record = JSON.parse(await readFile(lockPath, 'utf8'));
    if (typeof record.projectKey !== 'string' ||
...
export async function acquireOperatingLock(projectRoot, options) {
    ...
    return {
        ...
        async heartbeat(currentEventHead, heartbeatNow = new Date()) {
            ...
            if (heartbeatNow.getTime() > Date.parse(record.leaseExpiresAt)) {
                throw new OperateError('E_OPERATE_CYCLE_ACTIVE', 'Operating lock lease expired before heartbeat.');
            }
            record.heartbeatAt = heartbeatNow.toISOString();
            record.leaseExpiresAt = new Date(heartbeatNow.getTime() + record.leaseDurationMs).toISOString();
            await writeRecord(handle, record);
        },
        async release() {
            if (released)
...
export async function recoverStaleOperatingLock(projectRoot, options) {
    const lockPath = path.join(resolveOperatingPaths(projectRoot, { localRoot: options.localRoot }).locks, `${options.name ?? 'project'}.lock`);
    ...
    if (record.projectKey !== options.projectKey ||
        ...
        throw new OperateError('E_OPERATE_STALE_LOCK_UNSAFE', 'The lock changed after stale-lock preview.');
    }
    if (record.host !== hostname()) {
        throw new OperateError('E_OPERATE_STALE_LOCK_UNSAFE', 'Cross-host locks require manual owner verification and are never auto-removed.');
    }
    ...
    if (processIsAlive(record.pid)) {
        const liveIdentity = await (options.processIdentityReader ?? readProcessStartIdentity)(record.pid);
        if (!liveIdentity || liveIdentity === record.processStartedAt) {
            throw new OperateError('E_OPERATE_STALE_LOCK_UNSAFE', liveIdentity
    ...
    if (rechecked.nonce !== options.expectedNonce ||
        ...
        throw new OperateError('E_OPERATE_STALE_LOCK_UNSAFE', 'The lock changed immediately before recovery.');
    }
    await unlink(lockPath);
}
export async function withOperatingLock(projectRoot, options, operation) {
