...
import { executeGitReadOnly, gitRevisionResolves, readGitPathAtRevision, readGitPlanrPathAtRevision, } from './read-only-providers.js';
...
async function researchApi() {
    cachedApi ??= (async () => {
        ...
        if (!root) {
            throw new OperateError('E_PIPELINE_VERSION_INCOMPATIBLE', 'Agent-native context research requires planr-pipeline with Protocol v1.4.');
        }
        const loaded = (await import(pathToFileURL(path.join(root, 'lib', 'operate', 'research.mjs')).href));
        if (typeof loaded.createOperatingResearchMandate !== 'function' ||
            typeof loaded.validateOperatingContextClaims !== 'function') {
            throw new OperateError('E_PIPELINE_VERSION_INCOMPATIBLE', 'Installed planr-pipeline does not expose Protocol v1.4 research helpers.');
        }
        return loaded;
    })();
    return cachedApi;
}
function contextDirectory(projectRoot) {
...
function contextPath(projectRoot) {
...
export async function prepareOperatingContextResearch(input) {
    ...
    const session = {
        ...
        projectHead,
        runtime: String(mandate.runtimeBinding.runtime ?? input.runtime),
        researchMode,
    ...
    if (!input.preview)
        await atomicWrite(sessionPath(projectRoot), session);
    return {
        sessionId: session.sessionId,
        mandate,
        instruction: 'Inspect the workspace directly using the selected runtime. Return a JSON array of Protocol v1.4 context claims. Research before asking; label every claim observed, inferred, hypothesis, owner-confirmed, or unknown.',
    };
}
async function citationResolves(projectRoot, session, citation) {
    if (citation.kind === 'repository') {
        const blob = await readGitPathAtRevision(projectRoot, String(citation.revision), String(citation.path));
        return (blob.exists &&
    ...
    if (citation.kind === 'planr') {
        const blob = await readGitPlanrPathAtRevision(projectRoot, session.projectHead, String(citation.path));
...
export async function recordOperatingContextResearch(input) {
    const projectRoot = await assertOperatingProject(input.projectRoot);
    if (!input.stdin) {
        throw new OperateError('E_OPERATE_INPUT_REQUIRED', 'Context review requires one JSON array of runtime-authored context claims on stdin.');
    }
    const session = JSON.parse(await readFile(sessionPath(projectRoot), 'utf8'));
    if ((await gitHead(projectRoot)) !== session.projectHead) {
        throw new OperateError('E_OPERATE_SESSION_STALE', 'The workspace changed after context research was prepared. Refresh context research.');
    }
    let submitted;
    try {
    ...
    try {
        ...
        throw new OperateError('E_OPERATE_CONFIG_INVALID', error instanceof Error ? error.message : 'Context claims failed Protocol v1.4 validation.');
    }
    const accepted = [];
    const rejected = [];
    for (const claim of claims) {
        const results = await Promise.all(claim.citations.map((citation) => citationResolves(projectRoot, session, citation)));
...
export async function readOperatingContextResearch(projectRoot) {
    const root = await assertOperatingProject(projectRoot);
    return JSON.parse(await readFile(contextPath(root), 'utf8'));
}
function bestClaims(claims, field) {
    return claims
        .filter((claim) => claim.field === field && claim.epistemicStatus !== 'unknown')
        .sort((left, right) => right.confidence - left.confidence || left.id.localeCompare(right.id));
}
function firstValue(claims, field, fallback) {
    return bestClaims(claims, field)[0]?.value.trim() || fallback;
}
function normalizedStage(claims) {
    const raw = firstValue(claims, 'stage', 'launched').toLowerCase();
    if (/\bidea\b/.test(raw))
        return 'idea';
    if (/\bprototype|pre[- ]?launch|mvp\b/.test(raw))
        return 'prototype';
    if (/\bgrowth|scal(e|ing)|expansion\b/.test(raw))
        return 'growth';
    if (/\bmature|established|steady[- ]?state\b/.test(raw))
        return 'mature';
    return 'launched';
}
/**
 ...
 */
export async function operatingInitializationAnswersFromResearch(projectRoot) {
    let stored;
    try {
    ...
    if (!Array.isArray(record.claims))
        return null;
    const claims = record.claims.filter((claim) => Boolean(claim) && typeof claim === 'object' && !Array.isArray(claim));
    ...
    return {
        ...
        charter: {
            ...
            goals: goals.length > 0 ? goals : ['Clarify and prioritize the highest-leverage product outcome.'],
            constraints,
            successMetrics: metrics.length > 0
                ? metrics
...
//# sourceMappingURL=context-research.js.map