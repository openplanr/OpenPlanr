...
import { access, chmod, lstat, mkdir, open, readdir, readFile, rename, unlink, } from 'node:fs/promises';
...
async function atomicBytes(target, bytes, mode = 0o600, options = {}) {
    const directory = path.dirname(target);
    if (!options.parentSyncedByCaller)
...
async function writeJournal(target, record) {
...
export async function prepareJournalTransaction(projectRoot, input) {
    ...
    if (new Set(input.writes.map((write) => write.relativePath)).size !== input.writes.length) {
        throw new OperateError('E_OPERATE_TRANSACTION_INVALID', 'A transaction cannot target the same path twice.');
    }
    const transactionId = input.transactionId ?? `TXN-${randomUUID()}`;
    const root = path.join(resolveOperatingPaths(projectRoot, { localRoot: input.localRoot }).transactions, transactionId);
    ...
    for (const write of input.writes) {
        ...
        if (targetExists) {
            ...
            if (info.isSymbolicLink() || !info.isFile()) {
                throw new OperateError('E_OPERATE_PATH_ESCAPE', `Transaction target is not a regular file: ${write.relativePath}`);
            }
        }
        ...
        if (operation === 'create' && targetExists) {
            throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Create destination already exists: ${write.relativePath}`);
        }
        if (operation !== 'create' && !targetExists) {
            throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `${operation} destination does not exist: ${write.relativePath}`);
        }
        const after = operation === 'append' ? Buffer.concat([before, supplied]) : supplied;
...
async function restoreWrite(projectRoot, prepared, index) {
    ...
    if (sha256Digest(before) !== write.beforeDigest) {
        throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Transaction backup ${index} failed integrity verification.`);
    }
    await atomicBytes(target, before, parseMode(write.mode));
}
export async function rollbackJournalTransaction(projectRoot, prepared) {
...
export async function applyJournalTransaction(projectRoot, prepared, options) {
    if (prepared.record.state !== 'staged-fsynced' ||
    ...
    try {
        for (const [index, write] of prepared.record.writes.entries()) {
            ...
            if (!headsEqual(prepared.record.eventHead, currentHead)) {
                throw new OperateError('E_OPERATE_ROUTE_DRIFT', 'Event head changed while promoting a transaction.');
            }
            ...
            if (currentDigest !== write.beforeDigest) {
                throw new OperateError('E_OPERATE_ROUTE_DRIFT', `Destination changed after preview: ${write.path}`);
            }
            const after = await readFile(path.join(prepared.root, 'after', String(index)));
            if (sha256Digest(after) !== write.afterDigest) {
                throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Staged write ${index} failed integrity verification.`);
            }
...
export async function readJournal(manifestPath) {
...
export async function assertCommittedOperatingView(projectRoot, options = {}) {
    ...
    for (const name of names) {
        ...
        if (record && !['committed', 'rolled-back', 'failed'].includes(record.state)) {
            throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Transaction ${record.transactionId} is not committed; readers will not expose partial state.`, { transactionId: record.transactionId, state: record.state });
        }
    }
}
export async function recoverOperatingTransactions(projectRoot, options = {}) {
    const transactions = resolveOperatingPaths(projectRoot, options).transactions;
    const recovered = [];
