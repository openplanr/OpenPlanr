...
import { readPersistedOperatingAdvisorReports, readPersistedOperatingRoleResults, } from './maintenance.js';
...
function requestedLens(value) {
    if (!value || value.toLowerCase() === 'all')
        return null;
    const role = LENS_ALIASES[value.toLowerCase()];
    if (!role) {
        throw new OperateError('E_OPERATE_CONFIG_INVALID', `Unknown operating lens ${value}. Use CEO, CTO, CPO, CMO, COO, Chair, or all.`);
    }
    return role;
}
/**
 ...
 */
export function markdownLens(report) {
    const lines = [
        `## ${report.label}`,
        '',
        report.mandate,
        ...
        ...(report.claims && report.claims.length > 0
            ? [
                '### Material claims',
                '',
                ...report.claims.map((claim) => `- **${String(claim.epistemicStatus ?? 'unknown')} · ${String(claim.confidence ?? '?')}/5** ${String(claim.statement ?? '')}`),
                '',
            ]
            : []),
        '### Recommendations',
        ...
        ...(report.proposals.length > 0
            ? report.proposals.map((proposal) => `- **${proposal.title}** (${proposal.severity}; I${proposal.impact} C${proposal.confidence} E${proposal.ease}) — ${proposal.proposal} Evidence: ${proposal.evidenceRefs.map((reference) => `\`${reference}\``).join(', ')}.`)
 ...
 */
export function markdownCompleteRegisters(state) {
    ...
    return [
        ...
        '',
        ...(findings.length > 0
            ? findings.map((finding) => `- **${finding.id}** [${fieldText(finding, 'status') ?? 'proposed'} · ${fieldText(finding, 'lane') ?? 'OWNER'} · ${fieldText(finding, 'owner') ?? 'unassigned'} · ${fieldText(finding, 'severity') ?? 'low'}] ${fieldText(finding, 'title') ?? 'Untitled finding'} — ${fieldText(finding, 'proposal') ??
                fieldText(finding, 'problem') ??
                'Review the cited evidence.'} Evidence: ${evidence(finding)}.`)
            : ['- None recorded.']),
        '',
        `## All decisions (${decisions.length})`,
        '',
        ...(decisions.length > 0
            ? decisions.map((decision) => `- **${decision.id}** [${fieldText(decision, 'status') ?? 'open'} · ${fieldText(decision, 'owner') ?? 'unassigned'}${fieldText(decision, 'deadline') ? ` · due ${fieldText(decision, 'deadline')}` : ''}] ${fieldText(decision, 'question') ?? 'Decision required'} Recommendation: ${fieldText(decision, 'recommendation') ?? 'Review the available options.'} Evidence: ${evidence(decision)}.`)
            : ['- None recorded.']),
        '',
        `## All evidence gaps (${gaps.length})`,
        '',
        ...(gaps.length > 0
            ? gaps.map((gap) => `- **${gap.id}** [${fieldText(gap, 'status') ?? 'open'} · ${fieldText(gap, 'owner') ?? 'unassigned'}] ${fieldText(gap, 'question') ?? 'Evidence required'} — ${fieldText(gap, 'reason') ?? 'Evidence was not available.'} Evidence: ${evidence(gap)}.`)
            : ['- None recorded.']),
        '',
        ...
        ...(routes.length > 0
            ? routes.map((route) => {
                ...
                return `- **${route.id}** [${fieldText(route, 'state') ?? 'proposed'} · ${fieldText(route, 'cycleId') ?? ''}] findings: ${findingIds.length > 0 ? findingIds.join(', ') : 'none'}.`;
            })
            : ['- None recorded.']),
    ].join('\n');
}
...
export async function readOperatingReport(input) {
    ...
    if (!cycleId || !fullState.cycles.some((cycle) => cycle.id === cycleId)) {
        throw new OperateError('E_OPERATE_STATE_INVALID', input.cycleId
    ...
    const [roleResults, richReports, roles] = await Promise.all([
        ...
        loadOperatingProtocol().then((protocol) => protocol.listOperatingRoles()),
    ]);
    ...
    const notEvaluatedReasonsByRole = new Map();
    ...
    const reports = roles
        ...
        .map((role) => {
        const result = byRole.get(role.id);
    ...
    const actions = [
        ...
        ...state.routes
            .filter((route) => ['create-spec', 'create-instrumentation-spec'].includes(String(route.actions?.[0]?.kind)))
            .map((route) => ({
            kind: 'planning',
        ...
        ...groupRelatedAcceptedFindings(state.findings.filter((finding) => finding.status === 'accepted')).map((group) => ({
            kind: 'planning',
...
function findingSensitivityByRef(state) {
    const byRef = new Map();
    for (const finding of state.findings) {
        const sensitivity = fieldText(finding, 'sensitivity') ?? 'internal';
...
function decisionOptions(record) {
...
function selectedOptionLabel(record) {
    const selected = fieldText(record, 'selectedOption');
    if (!selected)
        return undefined;
    const match = (Array.isArray(record.options) ? record.options : []).find((option) => option && typeof option === 'object' && option.id === selected);
 ...
 */
async function readFullOperatingDecision(store, decisionId, projected) {
    const replay = await store.replay();
    let full;
    for (const event of replay.events) {
        if (event.entityId !== decisionId || event.type !== 'decision.open')
            continue;
...
function buildDecisionSource(decision, evidence) {
...
function buildBriefSource(state, cycleId, evidence) {
    const surfaced = state.findings.filter((finding) => finding.parked !== true && finding.status !== 'rejected');
    const options = surfaced.slice(0, 6).map((finding) => ({
    ...
    const openDecisions = state.decisions.filter((decision) => ['open', 'default-due'].includes(String(decision.status)));
    const openGaps = state.dataGaps.filter((gap) => gap.status === 'open');
    const blocksLines = [];
    if (openDecisions.length > 0) {
        blocksLines.push('Owner decisions pending:');
        blocksLines.push(...openDecisions
            .slice(0, 6)
            .map((decision) => `- ${decision.id}: ${fieldText(decision, 'question') ?? 'Decision required'}`));
    }
    if (openGaps.length > 0) {
        if (blocksLines.length > 0)
            blocksLines.push('');
        blocksLines.push('Evidence gaps blocking progress:');
        blocksLines.push(...openGaps
            .slice(0, 6)
...
export async function readOperatingDecisionBriefSource(input) {
    ...
    if (input.decisionId) {
        const projected = fullState.decisions.find((record) => record.id === input.decisionId);
