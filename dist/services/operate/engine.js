...
async function readPreferences(projectRoot, localRoot) {
    const paths = resolveOperatingPaths(projectRoot, { localRoot });
    return loadJson(path.join(paths.localRoot, 'preferences.json')).catch(() => ({
        runtime: 'auto',
...
function activeCycle(state) {
    return (state.cycles.find((cycle) => !['closed', 'cancelled', 'failed'].includes(cycle.state)) ?? null);
}
function maximumOrdinal(records, prefix) {
    return records.reduce((maximum, record) => {
        const match = String(record.id ?? '').match(new RegExp(`^${prefix}-(\\d+)$`));
        return Math.max(maximum, match ? Number(match[1]) : 0);
    }, 0);
}
async function remapReadinessGaps(readiness, state) {
    ...
    const remapped = {
        ...
        roles: readiness.roles.map((role) => ({
            ...role,
            gapId: role.gapId
                ? (state.dataGaps.find((gap) => gap.cycleId === readiness.cycleId &&
                    !['closed', 'superseded'].includes(String(gap.status)) &&
                    Array.isArray(gap.affectedRoles) &&
                    gap.affectedRoles.includes(role.roleId))?.id ?? `GAP-${String(++ordinal).padStart(3, '0')}`)
                : null,
        })),
    };
    return assertOperatingArtifact('operating-evidence-readiness', remapped);
}
function readinessGaps(readiness, decisionOwner, now) {
...
function requiredOpenEvidenceRefs(state) {
    const records = [
        ...state.findings.filter((finding) => !['done', 'rejected', 'superseded'].includes(String(finding.status))),
        ...state.decisions.filter((decision) => !['closed', 'superseded'].includes(String(decision.status))),
        ...state.outcomes.filter((outcome) => ['pending', 'observing', 'inconclusive'].includes(String(outcome.status))),
    ];
    return [
        ...new Set(records.flatMap((record) => Array.isArray(record.evidenceRefs)
            ? record.evidenceRefs.filter((reference) => typeof reference === 'string')
            : [])),
    ].sort();
}
export function buildChairEvidence(evidence, results, now) {
    ...
    const synthetic = results.flatMap((result) => {
        ...
        if (missingRefs.length > 0) {
            throw new OperateError('E_OPERATE_EVIDENCE_REJECTED', `${result.roleId} advisor result cites unavailable evidence: ${missingRefs.join(', ')}.`);
        }
            ...
            requirements: [
                {
                    ...
                    observationWindow: 'current-cycle',
...
async function assembleCitationEvidence(input) {
    const refs = [
        ...new Set(input.roleResults.flatMap((result) => result.proposals.flatMap((proposal) => proposal.evidenceRefs))),
    ].sort();
    const items = [];
...
const SENSITIVE_ROUTE_LANGUAGE = /\b(?:payment|billing|security|privacy|legal|tenant|isolation|destructive|delete|purge|credential|secret)\b/i;
function containsSensitiveRouteProposal(results) {
    return results.some((result) => result.proposals.some((proposal) => SENSITIVE_ROUTE_LANGUAGE.test(`${proposal.type} ${proposal.title} ${proposal.problem} ${proposal.proposal}`)));
}
...
async function appendLocked(store, lock, head, input) {
...
function remapConsolidation(state, findings, parked, decisions, gaps) {
    ...
    const remapFinding = (finding) => ({
        ...finding,
        id: findingIds.get(finding.id),
...
function suppressRepeatedFindings(state, findings, cycleId) {
    const priorByIdentity = new Map(state.findings
...
function committedCycleFindings(events, cycleId) {
    return events
        .filter((event) => event.type === 'finding.proposed' &&
        event.cycleId === cycleId &&
        event.payload.record &&
        typeof event.payload.record === 'object')
        .map((event) => structuredClone(event.payload.record));
}
function reconcileCycleFindings(events, cycleId, findings) {
    const existing = committedCycleFindings(events, cycleId);
    const existingByIdentity = new Map(existing.map((finding) => [findingIdentity(finding), finding]));
...
async function persistedCycleEvidence(store, events, cycleId) {
    const event = [...events]
        .reverse()
    ...
    if (record.recordType !== 'evidence-metadata') {
        throw new OperateError('E_OPERATE_STATE_INVALID', `Cycle ${cycleId} evidence event references a ${record.recordType} record.`);
    }
    return assertOperatingArtifact('operating-evidence', record.content);
}
async function persistedCycleRoleResults(store, events, cycleId) {
    ...
    for (const event of events) {
        ...
        if (record.recordType !== 'advisor-result') {
            throw new OperateError('E_OPERATE_STATE_INVALID', `Cycle ${cycleId} advisory event references a ${record.recordType} record.`);
        }
        ...
        if (result.cycleId !== cycleId) {
            throw new OperateError('E_OPERATE_STATE_INVALID', `Advisor result ${result.roleId} belongs to another cycle.`);
        }
 ...
 */
function roleEmptyGroundingGap(roleId, context, now) {
    ...
    return {
        ...
        reason: `Every citation ${roleId} returned failed to resolve at the pinned revision, so the role ` +
...
export async function gateRecordedProposalCitations(input) {
    const bearing = [];
    const citationBearingProposalKeys = new Set();
    ...
    const gaps = [...enforcement.gaps];
    const notEvaluatedRoleIds = [];
    const now = input.context.now ?? new Date();
    const roleResults = await Promise.all(input.roleResults.map(async (result) => {
        const gated = {
            ...result,
        ...
        const wasCitationBearing = result.proposals.some((proposal) => citationBearingProposalKeys.has(proposal.proposalKey));
        ...
        if (wasCitationBearing && !mintedAny) {
            ...
            gaps.push(await assertOperatingArtifact('operating-data-gap', roleEmptyGroundingGap(result.roleId, input.context, now)));
...
export async function runOperatingCycle(input) {
    ...
    if (input.cycleId && current?.id !== input.cycleId) {
        throw new OperateError('E_OPERATE_CYCLE_INPUT_CONFLICT', current
    ...
    if (input.reviewOnly) {
        ...
        if (!persisted) {
            throw new OperateError('E_OPERATE_STATE_INVALID', 'Review-only mode requires at least one committed operating cycle.');
        }
        const persistedCycle = await assertOperatingArtifact('operating-cycle-manifest', persisted);
    ...
    if (current && !resumableStates.has(current.state)) {
        throw new OperateError('E_OPERATE_CYCLE_ACTIVE', `Operating cycle ${current.id} is already ${current.state}; close or cancel it before starting another.`);
    }
    const depth = input.depth ?? current?.depth ?? 'standard';
    const focus = normalizeFocus(input.focus ?? current?.focus);
    const currentProducer = current ? current.producer : null;
    const requestedRuntime = input.runtime && input.runtime !== 'auto' ? input.runtime : preferences.runtime;
    if (current && requestedRuntime !== 'auto' && requestedRuntime !== currentProducer?.runtime) {
        throw new OperateError('E_OPERATE_RUNTIME_MISMATCH', `Operating cycle ${current.id} is bound to runtime ${currentProducer?.runtime}, not ${requestedRuntime}.`, {
            cycleId: current.id,
            expectedRuntime: currentProducer?.runtime,
            actualRuntime: requestedRuntime,
            recovery: 'Resume with the bound runtime, or cancel this cycle and create a new one with the desired runtime.',
        });
    }
    const resolvedRuntime = currentProducer?.runtime ?? requestedRuntime;
    const cycleStart = current
        ? initial.events.find((event) => event.type === 'cycle.preparing' &&
            event.cycleId === current.id &&
            event.entityId === current.id)
        : null;
    if (current && !cycleStart) {
        throw new OperateError('E_OPERATE_STATE_INVALID', `Operating cycle ${current.id} has no canonical preparing event.`);
    }
    ...
    if (current && current.inputDigest !== inputDigest) {
        throw new OperateError('E_OPERATE_CYCLE_INPUT_CONFLICT', `Operating cycle ${current.id} can resume only with its original workspace, configuration, depth, and focus.`, {
            cycleId: current.id,
    ...
    const cycle = current
        ? await assertOperatingArtifact('operating-cycle-manifest', current)
    ...
    const assertCycleWorkspace = async () => {
        const observed = await snapshotWorkspace(projectRoot, input.localRoot);
        if (observed.workspaceDigest !== workspace.workspaceDigest) {
            throw new OperateError('E_OPERATE_HEAD_DIVERGED', 'Workspace revisions, branches, remotes, or material dirty fingerprints changed during the operating cycle.', {
                expectedWorkspaceDigest: workspace.workspaceDigest,
                actualWorkspaceDigest: observed.workspaceDigest,
            });
        }
    };
    const existingEvidence = current && current.state !== 'blocked'
    ...
    const citationCache = new OperatingEvidenceCache(resolveOperatingPaths(projectRoot, { localRoot: input.localRoot }).evidence, preferences.sensitivityCeiling);
    ...
    const resolveCitations = (roleResults) => gateRecordedProposalCitations({
        roleResults,
        context: {
            projectRoot,
            cycleId: cycle.id,
    ...
    const evaluate = async (persist) => {
        ...
        const missingNonChair = readiness.roles.filter((role) => role.roleId !== 'chair' && !existingByRole.has(role.roleId));
        ...
        await assertCycleWorkspace();
        const first = runnableMissingNonChair.length > 0
        ...
        if (config.enabledRoles.includes('chair')) {
            ...
            chairReadiness = await remapReadinessGaps(chairReadiness, {
                ...
                dataGaps: [
                    ...
                    ...readiness.roles
                        .filter((role) => role.gapId)
                        .map((role) => ({ id: role.gapId, status: 'open' })),
                ],
            });
            readiness = {
                ...readiness,
            ...
            const runnableChair = chairReadiness.roles.filter((role) => role.modelCallAllowed && !persistedChair);
        ...
        const technologyRiskReady = readiness.roles.some((role) => role.roleId === 'technology-risk' && role.modelCallAllowed);
        ...
        const overflowDedup = suppressRepeatedFindings(initialState, consolidated.criticalOverflow, cycle.id);
        const parkedDedup = suppressRepeatedFindings(initialState, consolidated.parked, cycle.id);
        const suppressedFindings = surfacedDedup.suppressed + overflowDedup.suppressed + parkedDedup.suppressed;
        ...
        }, [...surfacedDedup.findings, ...overflowDedup.findings], parkedDedup.findings, consolidated.decisions, [...evidenceGaps, ...consolidated.gaps, ...failureGaps]);
        const warnings = [
            ...evidence.warnings,
        ...
        if (blocked)
            warnings.push('The cycle is blocked by required advisor evidence or failure.');
    ...
    const cycleResult = await withOperatingLock(projectRoot, {
        ...
        if (latest.eventHead.sequence !== initial.eventHead.sequence ||
            latest.eventHead.hash !== initial.eventHead.hash) {
            throw new OperateError('E_OPERATE_HEAD_DIVERGED', 'Operating state changed while preparing the cycle.');
        }
        ...
        try {
            ...
            if (phase === 'collecting') {
                if (!existingEvidence) {
                    const evidenceRecord = await store.putRecord('evidence-metadata', recordContent(evidence), { correlationId, createdAt: timestamp });
                    const cache = new OperatingEvidenceCache(resolveOperatingPaths(projectRoot, { localRoot: input.localRoot }).evidence, preferences.sensitivityCeiling);
                    await cache.put(evidence.fingerprint.slice('sha256:'.length), evidence, preferences.evidenceTtlMs, now);
            ...
            if (phase === 'advising') {
                ...
                for (const result of evaluated.roleResults ?? []) {
                    if (recordedRoles.has(result.roleId))
                        continue;
                ...
                const existingGapIds = new Set(initialState.dataGaps.filter((gap) => gap.cycleId === cycle.id).map((gap) => gap.id));
            ...
            const existingFindingIds = new Set(initialState.findings
            ...
            const providerDigest = evaluated.provider?.policyDigest ?? canonicalDigest({ provider: 'offline' });
            const routes = [];
            let routeOrdinal = maximumOrdinal(routeBase.routes, 'ACT');
            let specOrdinal = await nextOperatingSpecOrdinal(projectRoot);
            const existingRoutedFindingIds = new Set(initial.events
                .filter((event) => event.type === 'route.proposed' &&
                event.payload.record &&
                typeof event.payload.record === 'object')
                .flatMap((event) => {
            ...
            for (const finding of (evaluated.findings ?? []).filter((candidate) => !candidate.parked && !existingRoutedFindingIds.has(candidate.id))) {
                const isDev = finding.lane === 'DEV';
                routes.push(await createOperatingRoutePlan({
                    projectRoot,
                    cycleId: cycle.id,
                    finding,
                    config,
            ...
            if (routes.length > 0) {
                ...
                for (const route of routes) {
                    ...
                    const transactionRoot = path.join(resolveOperatingPaths(projectRoot, { localRoot: input.localRoot }).transactions, transactionId);
                    ...
                    if (existingBytes !== null) {
                        if (existingBytes !== content ||
                            ...
                            throw new OperateError('E_OPERATE_TRANSACTION_INVALID', `Orphaned route proposal ${route.id} does not match its committed journal.`);
                        }
                    }
                    else {
            ...
            const quiet = (evaluated.findings?.length ?? 0) === 0 &&
            ...
            if (started) {
                ...
                try {
                    const blockedByCriticalCap = error instanceof OperateError && error.code === 'E_OPERATE_CRITICAL_CAP';
