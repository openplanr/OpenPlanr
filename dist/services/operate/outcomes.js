...
function compare(operator, value, threshold) {
    ...
    if (operator === 'eq')
        return value === threshold.value;
    return (threshold.upperValue !== undefined && value >= threshold.value && value <= threshold.upperValue);
}
function assertDirectionOperator(direction, operator) {
    const allowed = {
        increase: ['gt', 'gte'],
        decrease: ['lt', 'lte'],
        maintain: ['eq'],
    ...
    if (!allowed[direction].includes(operator)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', `Outcome direction ${direction} is incompatible with operator ${operator}.`);
    }
}
export async function createOperatingOutcome(input) {
    if (Date.parse(input.baselineWindow.from) >= Date.parse(input.baselineWindow.to) ||
        Date.parse(input.targetWindow.from) >= Date.parse(input.targetWindow.to)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'Outcome windows must have ordered from/to timestamps.');
    }
    if (Date.parse(input.baselineWindow.to) >= Date.parse(input.targetWindow.from)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'Outcome baseline and target windows must not overlap.');
    }
    assertDirectionOperator(input.direction, input.operator);
    if (input.operator === 'between' && input.threshold.upperValue === undefined) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'A between outcome requires threshold.upperValue.');
    }
    if (Date.parse(input.verifyAfter) < Date.parse(input.targetWindow.to)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'Outcome verification cannot begin before the target window closes.');
    }
...
export async function evaluateOperatingOutcome(input) {
    if (input.unit !== input.outcome.unit ||
        input.queryIdentity !== input.outcome.queryIdentity ||
        input.aggregation !== input.outcome.aggregation) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'Observation identity does not match the outcome contract.');
    }
    if (input.window.from !== input.outcome.targetWindow.from ||
        input.window.to !== input.outcome.targetWindow.to ||
        Date.parse(input.observedAt) < Date.parse(input.outcome.verifyAfter)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', 'Observation must cover the exact target window and occur at or after verifyAfter.');
    }
    ...
    const insufficient = input.value === null ||
    ...
    if (insufficient) {
        ...
        evaluation = input.value === input.outcome.threshold.value ? 'neutral' : 'negative';
    }
    else if ((input.outcome.operator === 'gt' || input.outcome.operator === 'lt') &&
        input.value === input.outcome.threshold.value) {
        evaluation = 'neutral';
    }
    else {
        evaluation = compare(input.outcome.operator, input.value, input.outcome.threshold)
            ? 'positive'
            : 'negative';
    }
    if (input.outcome.guardrailPrecedence === 'block-on-breach' &&
...
async function readOutcomeContract(projectRoot, outcomeId, localRoot) {
    const target = path.join(resolveOperatingPaths(projectRoot, { localRoot }).outcomes, `${outcomeId}.json`);
...
export async function recordOperatingOutcomeObservation(input) {
    ...
    const observation = await assertOperatingArtifact('operating-outcome-observation', input.observation);
    ...
    const verified = await evaluateOperatingOutcome({
        ...
        guardrailValues: Object.fromEntries(observation.guardrails.map((guardrail) => [
            guardrail.metric,
    ...
    if (JSON.stringify(verified) !== JSON.stringify(observation)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', `Observation ${observation.id} does not match the deterministic outcome evaluation.`);
    }
    const store = new OperatingEventStore(projectRoot, { localRoot: input.localRoot });
    const initial = await store.replay();
    const registered = initial.events.find((event) => event.type === 'outcome.registered' && event.entityId === outcome.id && event.payload.record);
    if (!registered || canonicalDigest(registered.payload.record) !== canonicalDigest(outcome)) {
        throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', `Outcome ${outcome.id} does not match its immutable registered contract.`);
    }
    ...
    return withOperatingLock(projectRoot, {
        ...
        if (!state.outcomes.some((candidate) => candidate.id === outcome.id)) {
            throw new OperateError('E_OPERATE_OUTCOME_NOT_READY', `Outcome ${outcome.id} has not been registered by an applied route.`);
        }
        let head = initial.eventHead;
        let applied = false;
        const existingObserved = initial.events.some((event) => event.type === 'outcome.observed' &&
            event.entityId === outcome.id &&
            event.payload.record &&
            event.payload.record.id === observation.id);
        const existingEvaluated = initial.events.some((event) => event.type === 'outcome.evaluated' &&
            event.entityId === outcome.id &&
            event.correlationId === observation.id);
        const existingLearning = initial.events.some((event) => event.type === 'learning.recorded' && event.entityId === `LRN-${observation.id}`);
        const existingGap = initial.events.some((event) => event.type === 'gap.open' && event.correlationId === observation.id);
        const append = async (type, entityId, payload, evidenceRefs = []) => {
        ...
        if (!existingObserved) {
            await append('outcome.observed', outcome.id, { record: observation }, observation.evidenceRefs);
        }
        ...
        const needsGap = observation.evaluation === 'inconclusive' &&
...
export async function reconcileOperatingOutcomeFiles(input) {
    ...
    const store = new OperatingEventStore(projectRoot, { localRoot: input.localRoot });
    const inbox = path.join(resolveOperatingPaths(projectRoot, { localRoot: input.localRoot }).outcomes, 'observations');
    let reconciled = 0;
    let currentState = shipments.state;
    for (const name of (await readdir(inbox).catch(() => [])).sort()) {
        if (!name.endsWith('.json'))
            continue;
        const observation = JSON.parse(await readFile(path.join(inbox, name), 'utf8'));
        const outcome = currentState.outcomes.find((candidate) => candidate.id === observation.outcomeId);
        const linkedSpec = currentState.specLinks.find((candidate) => candidate.specId === outcome?.specId);
        if (outcome && linkedSpec?.state !== 'shipped')
            continue;
