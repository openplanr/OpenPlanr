...
function candidateRoots() {
    ...
    const roots = [process.env.OPENPLANR_PIPELINE_ROOT].filter((value) => Boolean(value));
    try {
...
export function resolvePipelinePackage(required = true) {
    for (const root of candidateRoots()) {
        ...
        if (![packagePath, binPath, adapterRegistryPath, roleRegistryPath].every(existsSync))
            continue;
    ...
    if (!required)
        return null;
    const error = new Error('The pipeline package is not installed. Run `npm install -g openplanr@latest` or rerun setup without `--minimal`.');
 ...
 */
export async function resolveGuidedInteractionValidators() {
    ...
    for (const candidate of candidates) {
        ...
        try {
            ...
            if (typeof loaded.createGuidedAnswerSubmission === 'function' &&
    ...
    const error = new Error(`Installed planr-pipeline version(s) ${versions} do not provide the Protocol v1.2 guided interaction validators. Install the compatible full package with \`npm install -g openplanr@latest\`.`);
    error.name = 'E_PIPELINE_VERSION_INCOMPATIBLE';
    throw error;
}
//# sourceMappingURL=pipeline-package-service.js.map