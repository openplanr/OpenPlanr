...
function readEnvKey(provider) {
    ...
    if (!envVar || !ALLOWED_ENV_VARS.has(envVar))
        return undefined;
...
export async function resolveApiKeySource(provider) {
    // Run one-time migration from legacy plaintext file
    await migrateCredentials();
    // 1. Environment variable (explicit allowlist)
    const envKey = readEnvKey(provider);
    ...
    const key = await encryptedFileBackend.get(provider);
    if (key)
        return { key, source: 'encrypted-file' };
    return undefined;
}
/**
...
export async function clearCredential(provider) {
...
export function _resetMigration() {
    migrationDone = false;
}
