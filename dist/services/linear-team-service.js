 ...
 */
export function resolveConfiguredLinearTeam(linear, selector) {
    const selectedTeams = linear.teams;
    const teams = selectedTeams && selectedTeams.length > 0
        ? selectedTeams
    ...
    const match = teams.find((team) => team.id.toLowerCase() === normalized || team.key?.toLowerCase() === normalized);
    if (match)
        return match;
    const available = teams
        .map((team) => (team.key ? `${team.name ?? team.key} (${team.key})` : (team.name ?? team.id)))
        .join(', ');
    throw new Error(`Linear team "${selector}" is not configured for this project. Available teams: ${available}. Run \`planr linear init\` to change team access.`);
}
/** Return a copy of config targeting one configured team for this command. */
export function withLinearTeam(config, selector) {
    if (!config.linear)
        return config;
...
//# sourceMappingURL=linear-team-service.js.map