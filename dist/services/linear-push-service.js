 ...
 */
async function pushOneFeatureAndDescendants(projectDir, config, client, sf, strategyCtx, typeLabelCache, teamId, updateOnly, noCascade = false) {
    const f = sf.data;
...
async function pushOneTaskListForFeature(projectDir, config, client, sf, featureIssueId, strategyCtx, typeLabelCache, teamId, updateOnly) {
    ...
    const taskFileMeta = await Promise.all(sf.taskFiles.map(async (tf) => {
 ...
 */
async function pushEpicScope(projectDir, config, client, epicId, updateOnly, teamId, leadId, override, noCascade = false) {
...
async function pushFeatureScope(projectDir, config, client, featureId, options, teamId, leadId) {
    ...
    if (!ctx.epic.linearProjectId) {
        if (options.pushParents) {
            logger.info(`Parent epic ${ctx.epic.id} is not in Linear yet — pushing the epic only (--push-parents).`);
            ...
            const epicPlan = await pushEpicScope(projectDir, config, client, ctx.epic.id, updateOnly, teamId, leadId, options.strategyOverride, true);
            if (!epicPlan)
                return null;
    ...
    const typeLabelCache = createTypeLabelCache(client, teamId, config);
    await pushOneFeatureAndDescendants(projectDir, config, client, ctx.sf, strategyCtx, typeLabelCache, teamId, updateOnly, noCascade);
...
async function pushStoryScope(projectDir, config, client, storyId, options, teamId, leadId) {
    ...
    if (!isUsableLinearIssueId(ctx.sf.data.linearIssueId, `Feature ${ctx.sf.data.id}`)) {
        if (options.pushParents) {
            logger.info(`Parent feature ${ctx.sf.data.id} is not in Linear yet — pushing the feature only (--push-parents).`);
            ...
            // attachment only.
            const parentPlan = await pushFeatureScope(projectDir, config, client, ctx.sf.data.id, { ...options, pushParents: true, noCascade: true }, teamId, leadId);
            if (!parentPlan)
                return null;
    ...
    if (!featureIssueId) {
        throw new Error(`Parent feature ${ctx.sf.data.id} has no Linear issue id after the parent push — this is a bug.`);
    }
...
async function resolveQuickOrBacklogContext(projectDir, config, client, artifactKind, artifactId, frontmatter, options, teamId, leadId) {
    ...
    if (linkedEpicId) {
        ...
        if (!epicScope.epic.linearProjectId) {
            if (options.pushParents) {
                logger.info(`Parent epic ${linkedEpicId} is not in Linear yet — pushing the epic only (--push-parents).`);
                ...
                await pushEpicScope(projectDir, config, client, linkedEpicId, options.updateOnly === true, teamId, leadId, options.strategyOverride, true);
                ...
                if (!refreshed?.epic.linearProjectId) {
                    throw new Error(`Pushed epic ${linkedEpicId} via --push-parents but no linearProjectId was written back. Re-run \`planr linear push ${linkedEpicId}\` to repair.`);
                }
...
export async function runLinearPush(projectDir, config, client, artifactId, options) {
    ...
    if (type === 'epic') {
        return pushEpicScope(projectDir, config, client, artifactId, updateOnly, teamId, leadId, opts.strategyOverride, noCascade);
    }
    if (type === 'feature') {
