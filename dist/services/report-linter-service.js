...
export function mergeLinterConfig(config) {
    return {
        rules: config?.rules ?? DEFAULT_RULES,
        vaguePhrases: config?.vaguePhrases ?? DEFAULT_VAGUE,
    };
}
export function validateReportMarkdown(markdown, reportType, linter) {
    const findings = [];
    const coaching = [];
...
export function buildCoachingHistoryKey(_user, ruleId) {
    return ruleId;
}
export function lintWithProjectConfig(markdown, reportType, projectConfig) {
    return validateReportMarkdown(markdown, reportType, mergeLinterConfig(projectConfig.reportLinter));
}
//# sourceMappingURL=report-linter-service.js.map