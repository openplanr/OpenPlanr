 ...
 */
export function displayTaskPreview(result) {
    ...
    if (result.acceptanceCriteriaMapping && result.acceptanceCriteriaMapping.length > 0) {
        ...
        for (const ac of result.acceptanceCriteriaMapping) {
            display.line(chalk.dim(`    ${ac.criterion} (${ac.sourceStoryId}) → [${ac.taskIds.join(', ')}]`));
        }
    }
 ...
 */
export async function displayValidationWarnings(relevantFiles, rawContext) {
    if (!rawContext || !relevantFiles?.length)
        return;
    try {
 ...
 */
export function buildTaskItems(result) {
 ...
 */
export function countTaskItems(tasks) {
    return tasks.reduce((sum, t) => sum + (t.subtasks || []).length + 1, 0);
}
/**
 * Display the "Next steps" block after successful creation.
 */
export function displayNextSteps(opts) {
    logger.heading('Next steps:');
    logger.dim(`  planr ${opts.command} list                          — View all ${opts.command} tasks`);
 ...
 */
export async function handleAIError(err) {
