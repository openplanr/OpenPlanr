 ...
 */
import { readArtifactRaw, updateArtifact, updateArtifactFields, } from '../../services/artifact-service.js';
...
export function resolveBulkStatusIntent(opts) {
    ...
    if (opts.status) {
        throw new Error(`--status and ${opts.allDone ? '--all-done' : '--all-pending'} are mutually exclusive — the bulk flag implies a status. Use one or the other.`);
    }
 ...
 */
export async function applyBulkCheckboxes(projectDir, config, type, id, bulkStatus) {
...
//# sourceMappingURL=bulk-checkbox-update.js.map