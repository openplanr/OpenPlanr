...
export function registerReportLinterCommand(program) {
    program
        ...
        for (const f of lint.findings) {
            ...
            if (f.suggestion)
                display.line(`  suggestion: ${f.suggestion}`);
        }
        for (const c of lint.coaching) {
            display.line(`(coaching) ${c.message}`);
            if (c.educational)
                display.line(`  ${c.educational}`);
        }
        if (!lint.ok)
            process.exit(1);
...
//# sourceMappingURL=report-linter.js.map