...
export function registerTemplateCommand(program) {
    ...
    template
        ...
        if (builtIn.length > 0) {
            ...
            for (const t of builtIn) {
                ...
                display.line(`    ${chalk.cyan(t.name)}  ${chalk.dim(`— ${t.description} (${taskCount} tasks)`)}`);
            }
        }
        if (custom.length > 0) {
            display.blank();
            ...
            for (const t of custom) {
                const taskCount = t.tasks.reduce((sum, tg) => sum + tg.subtasks.length + 1, 0);
                display.line(`    ${chalk.green(t.name)}  ${chalk.dim(`— ${t.description} (${taskCount} tasks)`)}`);
            }
        }
        display.blank();
    ...
    template
        ...
        if (!tpl) {
            logger.error(`Template "${name}" not found. Run \`planr template list\` to see available templates.`);
            return;
        }
    ...
    template
        ...
        const { id, filePath } = await createArtifact(projectDir, config, 'quick', 'quick/quick-task.md.hbs', { title, tasks });
    ...
    template
        ...
        const templateName = opts.name || (await promptText('Template name (lowercase, hyphenated):'));
...
async function loadCustomTemplates(projectDir, config) {
...
async function loadAllTemplates(projectDir, config) {
...
async function findTemplate(name, projectDir, config) {
