...
import { buildEpicPrompt, buildFeaturesPrompt, buildStoriesPrompt, } from '../../ai/prompts/prompt-builder.js';
import { aiEpicResponseSchema, aiFeaturesResponseSchema, aiStoriesResponseSchema, } from '../../ai/schemas/ai-response-schemas.js';
...
import { addChildReference, createArtifact, getArtifactDir, listArtifacts, readArtifact, readArtifactRaw, resolveArtifactFilename, } from '../../services/artifact-service.js';
...
async function continuePlanFromEpic(projectDir, config, provider, epicId) {
    const epicRaw = await readArtifactRaw(projectDir, config, 'epic', epicId);
    if (!epicRaw) {
        logger.error(`Epic ${epicId} not found.`);
        return;
    ...
    const needsStoriesAndTasks = [];
    const needsTasksOnly = [];
    let skipped = 0;
    ...
    for (const { featureId, storyIds } of needsTasksOnly) {
        ...
        const created = await generateTasksForFeature(projectDir, config, provider, featureId, storyIds);
        if (created)
            totalTasks++;
    }
    logger.heading('\nContinue Complete!');
...
async function planFromFeature(projectDir, config, provider, featureId) {
    ...
    const taskCreated = await generateTasksForFeature(projectDir, config, provider, featureId, storyIds);
    return { stories: storyIds.length, tasks: taskCreated ? 1 : 0 };
}
async function generateTasksForFeature(projectDir, config, provider, featureId, storyIds) {
    ...
    logger.success(`Created ${id}: ${result.title} (${total} tasks from ${ctx.stories.length} stories)`);
    return true;
}
async function generateTasksForSingleStory(projectDir, config, provider, storyId) {
