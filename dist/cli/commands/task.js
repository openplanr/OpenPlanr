...
import { addChildReference, createArtifact, listArtifacts, readArtifact, resolveArtifactFilename, updateArtifactFields, } from '../../services/artifact-service.js';
...
export function registerTaskCommand(program) {
    ...
    task
        ...
        if (intent.useBulk) {
            try {
                const result = await applyBulkCheckboxes(projectDir, config, 'task', taskId, intent.bulkStatus);
                const checkboxNote = result.flippedAny ? ' (subtasks flipped)' : '';
                logger.success(`Updated ${taskId}: status=${result.status}${checkboxNote}`);
            }
            catch (err) {
                logger.error(`Failed to update ${taskId}: ${err.message}`);
                process.exit(1);
            }
            return;
        }
...
// Display helpers
// ---------------------------------------------------------------------------
function buildArtifactSources(ctx, config) {
    const sources = [];
...
// ---------------------------------------------------------------------------
async function createTasksWithAI(projectDir, config, storyId) {
    ...
    for (const t of existingTasks) {
        ...
        if (data && data.data.storyId === storyId) {
            storyTasks.push({ id: t.id, title: data.data.title || t.title });
        }
    }
...
// ---------------------------------------------------------------------------
async function createTasksFromFeature(projectDir, config, featureId) {
    ...
    for (const t of existingTasks) {
        ...
        if (data && data.data.featureId === featureId) {
            featureTasks.push({ id: t.id, title: data.data.title || t.title });
        }
    }
...
// ---------------------------------------------------------------------------
async function createTasksManually(projectDir, config, opts) {
