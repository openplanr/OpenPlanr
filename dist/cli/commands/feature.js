...
import { addChildReference, createArtifact, listArtifacts, readArtifact, readArtifactRaw, resolveArtifactFilename, updateArtifactFields, } from '../../services/artifact-service.js';
...
export function registerFeatureCommand(program) {
    ...
    feature
        ...
        try {
            ...
            logger.error(`Failed to update ${featureId}: ${err.message}`);
            process.exit(1);
        }
    });
}
async function createFeaturesWithAI(projectDir, config, epicId, featureCount) {
    ...
    try {
        ...
        logger.dim(`  3. planr rules generate                     — Generate rules for your coding agent`);
        logger.dim('');
