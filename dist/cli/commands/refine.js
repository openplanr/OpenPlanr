...
import { accumulateUsage, generateJSON, getAIProvider, isAIConfigured, } from '../../services/ai-service.js';
...
export function registerRefineCommand(program) {
    program
        .command('refine')
 ...
 */
async function refineOne(projectDir, config, provider, type, artifactId, parentContext) {
    ...
    const hasDelta = (result.frontmatterChanges && Object.keys(result.frontmatterChanges).length > 0) ||
    ...
    if (hasDelta) {
        ...
        if (!validation.ok) {
            ...
            markdown = toMarkdownWithFrontmatter(improved, rawContent.split('---').slice(2).join('---').trim());
        }
        else {
            ...
            if (result.bodyChanges && result.bodyChanges.length > 0) {
                ...
                for (const change of result.bodyChanges) {
                    if (change.type === 'replaceSection')
                        ...
                        display.line(`  Replace text: "${change.findExact?.slice(0, 50)}..."`);
                }
            }
        }
        else {
 ...
 */
async function refineCascade(projectDir, config, provider, type, artifactId, parentContext, totalUsage) {
    let count = 0;
 ...
 */
async function findChildren(projectDir, config, parentType, parentId) {
 ...
 */
async function suggestNextSteps(projectDir, config, type, artifactId) {
    const children = await findChildren(projectDir, config, type, artifactId);
    if (children.length === 0)
        return;
    const mapping = CHILD_MAP[type];
    ...
    display.line(chalk.dim(`  This ${type} has ${children.length} ${mapping.label} that may need re-alignment:`));
    display.blank();
