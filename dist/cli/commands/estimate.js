...
function displayRollupTable(estimates) {
    ...
    display.line(`  ${chalk.dim('ID'.padEnd(12))} ${chalk.dim('Points'.padEnd(8))} ${chalk.dim('Hours'.padEnd(8))} ${chalk.dim('Complexity')}`);
    display.tableSeparator(55);
    let totalPoints = 0;
    let totalHours = 0;
    ...
    for (const { id, estimate } of estimates) {
        ...
        display.line(`  ${id.padEnd(12)} ${String(estimate.storyPoints).padEnd(8)} ${(`${estimate.estimatedHours}h`).padEnd(8)} ${complexityColor(estimate.complexity)}`);
    }
    display.tableSeparator(55);
    display.line(`  ${chalk.bold('TOTAL'.padEnd(12))} ${chalk.bold(String(totalPoints).padEnd(8))} ${chalk.bold(`${totalHours}h`.padEnd(8))} ${complexityColor(maxComplexity)}`);
    display.blank();
}
// ---------------------------------------------------------------------------
...
async function calibrate(projectDir, config) {
    ...
    // Display table
    display.line(`  ${chalk.dim('ID'.padEnd(12))} ${chalk.dim('Status'.padEnd(14))} ${chalk.dim('Points'.padEnd(8))} ${chalk.dim('Hours'.padEnd(8))} ${chalk.dim('Complexity')}`);
    ...
    for (const item of calibrationData) {
        const statusColor = item.status === 'done'
            ...
            : item.status === 'in-progress'
                ? chalk.yellow
                : chalk.dim;
        display.line(`  ${item.id.padEnd(12)} ${statusColor(item.status.padEnd(14))} ${String(item.points).padEnd(8)} ${(`${item.hours}h`).padEnd(8)} ${complexityColor(item.complexity)}`);
    }
    // Summary
    ...
    display.line(`  Artifacts:         ${calibrationData.length} estimated, ${doneItems.length} completed`);
