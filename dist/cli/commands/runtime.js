...
import { applySetup, detectRuntimes, listRuntimeAdapters, previewSetup, removeRuntime, rollbackRuntime, runtimeDoctor, } from '../../services/runtime-manager-service.js';
import { display, logger } from '../../utils/logger.js';
function print(value, json) {
    if (json)
        display.line(JSON.stringify(value));
    else
        display.line(JSON.stringify(value, null, 2));
}
export function registerRuntimeCommand(program, cliVersion) {
    ...
    runtime
        .command('list')
        .option('--json', 'machine-readable output', false)
        .action((opts) => print({ ok: true, adapters: listRuntimeAdapters(), detected: detectRuntimes() }, opts.json));
    ...
    runtime
        ...
        print(await removeRuntime(runtimeId, program.opts().projectDir), opts.json);
    });
    runtime
        .command('rollback')
        ...
        print(await rollbackRuntime(program.opts().projectDir, opts.backup), opts.json);
    });
    runtime
        .command('doctor')
        .option('--json', 'machine-readable output', false)
        .action(async (opts) => {
        const result = await runtimeDoctor(program.opts().projectDir);
        print(result, opts.json);
        if (!result.ok)
            process.exitCode = 1;
    });
