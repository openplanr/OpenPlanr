...
import { addChildReference, createArtifact, getArtifactDir, listArtifacts, readArtifact, readArtifactRaw, resolveArtifactFilename, updateArtifactFields, } from '../../services/artifact-service.js';
...
import { readStandupTranscriptSource, transcriptToStandupMarkdown, } from '../../services/voice-service.js';
...
export function registerStoryCommand(program) {
    ...
    story
        ...
        if (opts.epic) {
            ...
            display.blank();
            const confirmBatch = await promptConfirm(`Generate stories for all ${epicFeatures.length} features?`, true);
            if (!confirmBatch) {
    ...
    story
        ...
        try {
            ...
            logger.error(`Failed to update ${storyId}: ${err.message}`);
            process.exit(1);
        }
    });
    story
 ...
 */
async function createStoriesWithAI(projectDir, config, featureId) {
    ...
    for (const s of allStories) {
        ...
        if (data && data.data.featureId === featureId) {
            featureStories.push({ id: s.id, title: data.data.title || s.title });
        }
    }
    if (featureStories.length > 0) {
        ...
        const continueCreate = await promptConfirm('Generate additional stories? (AI will avoid duplicates)', false);
        if (!continueCreate) {
    ...
    try {
        ...
        display.separator(50);
        const confirmAll = await promptConfirm(`Create all ${result.stories.length} user stories?`, true);
        if (!confirmAll) {
            logger.info('Story creation cancelled.');
            return 0;
        }
        ...
        logger.dim(`  2. planr rules generate                    — Generate rules for your coding agent`);
...
async function createStoryManually(projectDir, config, opts) {
