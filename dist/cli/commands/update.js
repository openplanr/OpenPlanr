...
export function registerUpdateCommand(program) {
    program
        ...
        .option('--all-done', 'set status=done AND flip every `N.M` task checkbox to `[x]` (task / quick artifacts only)')
        .option('--all-pending', 'set status=pending AND flip every `N.M` task checkbox to `[ ]` (task / quick artifacts only)')
        ...
        if (!intent.useBulk && !opts.status && !opts.owner && !opts.priority) {
            logger.error('Provide at least one of: --status, --owner, --priority, --all-done, --all-pending');
            process.exit(1);
        }
        let hasError = false;
        for (const id of ids) {
            ...
            if (intent.useBulk) {
                if (type !== 'task' && type !== 'quick') {
                    logger.warn(`--all-done / --all-pending only flip checkboxes on task / quick artifacts. ${id} (${type}) will get a status update only.`);
                    try {
                        ...
                        logger.error(`Failed to update ${id}: ${err.message}`);
                        hasError = true;
                    }
                    continue;
                }
                try {
                    const result = await applyBulkCheckboxes(projectDir, config, type, id, intent.bulkStatus);
            ...
            if (opts.status && !opts.force) {
                ...
                if (allowed && !allowed.includes(opts.status)) {
                    logger.error(`Invalid status "${opts.status}" for ${type}. Valid: ${allowed.join(', ')}. Use --force to override.`);
                    hasError = true;
                    continue;
                }
            }
            ...
            if (opts.owner) {
                if (type === 'epic' || type === 'feature') {
                    ...
                    logger.warn(`--owner is not applicable to ${type} artifacts. Skipping owner for ${id}.`);
                }
            }
            if (opts.priority) {
                if (type === 'backlog') {
                    ...
                    logger.warn(`--priority is not applicable to ${type} artifacts. Skipping priority for ${id}.`);
                }
            }
