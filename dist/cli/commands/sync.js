...
import { listArtifacts, readArtifact, readArtifactRaw, resolveArtifactFilename, updateArtifact, } from '../../services/artifact-service.js';
...
async function syncParentChildLinks(projectDir, config, opts) {
    ...
    const actualChildrenByParent = new Map();
    const childTitleMap = new Map();
    ...
    if (opts.parentType === 'story' && opts.childType === 'task') {
        ...
        for (const parent of parents) {
            ...
            const featureId = data?.data.featureId;
            if (featureId) {
    ...
    for (const child of children) {
        ...
        if (!data)
            continue;
        childTitleMap.set(child.id, data.data.title || child.title);
        const parentId = data.data[opts.childForeignKey];
        if (parentId) {
    ...
    for (const parent of parents) {
        ...
        if (!parentRaw)
            continue;
        const actualChildren = actualChildrenByParent.get(parent.id) || new Set();
        ...
        const newSection = await buildLinksSection(projectDir, config, opts.childType, correctIds, childTitleMap, opts.parentType, parent.id);
 ...
 */
function extractLinkedIds(markdown, sectionHeading) {
    ...
    for (const line of lines) {
        if (line.startsWith('##') && line !== sectionHeading)
            break; // next section
 ...
 */
async function buildLinksSection(projectDir, config, childType, childIds, titleMap, parentType, parentId) {
 ...
 */
function replaceSection(markdown, sectionHeading, newContent) {
    const sectionIdx = markdown.indexOf(sectionHeading);
    if (sectionIdx === -1)
        return markdown;
