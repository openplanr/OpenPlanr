...
import { attachSpecDesigns, createSpec, decomposeSpec, destroySpec, getSpecStatus, listSpecStories, listSpecs, listSpecTasks, readSpec, resolveSpecDir, shapeSpec, syncAllSpecs, syncSpec, updateSpecFields, validateSpecForPromotion, } from '../../services/spec-service.js';
...
export function registerSpecCommand(program) {
    ...
    spec
        ...
        const projectDir = program.opts().projectDir;
        const config = await loadConfig(projectDir);
        const title = opts.title || titleParts.join(' ').trim();
        if (!title) {
        ...
        try {
            ...
            display.line(`  - Attach UI mockups (optional): planr spec attach-design ${id} --files <png>...`);
    ...
    spec
        .command('shape')
        .description('Interactive 4-question SPEC authoring (Context, Functional Requirements, Business Rules, Acceptance Criteria)')
        ...
        const userRole = await promptText('  Who is the primary user? (e.g., "PMs", "founders", "engineering leads")');
        const problem = await promptText('  What problem are they trying to solve today? (one sentence)');
        ...
        const functionalRequirements = await promptMultiText('What must the system DO? (comma-separated; one observable behavior per item)', 'e.g., "user can log in", "system validates password complexity", "session expires after 30min"');
        ...
        const businessRules = await promptText('  Key rule or constraint? (one line; leave empty if none — edit spec file later for longer)');
        // ── Question 4 — Acceptance criteria ────────────────────────────────
        logger.dim('');
        logger.dim('Question 4 of 4 — Acceptance Criteria');
        const acceptanceCriteria = await promptMultiText('How will you know this feature is done? (Given/When/Then format recommended; comma-separated)', 'e.g., "Given a logged-out user, when they submit valid creds, then they reach /dashboard"');
        ...
        logger.dim('');
        const wantExtras = await promptConfirm('Add Out-of-Scope items or Notes for Decomposition? (optional)', false);
        let outOfScope = [];
        let decompositionNotes = '';
        if (wantExtras) {
            outOfScope = await promptMultiText('Out of Scope (comma-separated; what this feature does NOT include)', 'e.g., "email notifications (covered in feat-notifications)"');
            decompositionNotes = await promptText('  Decomposition note (one line; hint for `planr spec decompose` — leave empty if none)');
        }
        // ── Confirm + write ─────────────────────────────────────────────────
        logger.dim('');
    ...
    spec
        ...
        .option('--max-stories <n>', 'cap the number of stories the AI emits (1-8)', (v) => Number.parseInt(v, 10))
        ...
        logger.dim(opts.codeContext === false
        ...
        try {
            ...
            logger.success(`Decomposed ${spec.id}: ${result.storiesCreated} stor${result.storiesCreated === 1 ? 'y' : 'ies'}, ${result.tasksCreated} tasks.`);
    ...
    spec
        ...
        if (tasks.length === 0) {
            ...
            for (const t of tasks) {
                display.line(`    ${t.id}  [${t.status.padEnd(13)}]  ${t.type.padEnd(4)} ${t.agent.padEnd(20)} ${t.title}`);
            }
        }
        display.line('');
    ...
    spec
        ...
        for (const s of report.specs) {
            display.line(`  ${s.id}  [${s.status.padEnd(18)}]  ${s.title}  (${s.storyCount} US, ${s.taskCount} tasks)`);
        }
    });
    // ------------------------------------------------------------------------
    ...
    spec
        ...
        const ok = await promptConfirm(`Delete ${specId} and ALL its stories, tasks, and design assets?`, false);
        if (!ok) {
            logger.info('Cancelled.');
            return;
        }
    ...
    spec
        .command('sync')
        .description('Validate spec integrity (orphaned tasks, stories without tasks, missing specId frontmatter, schema drift)')
        ...
        try {
            if (specId) {
                ...
                if (report.fixed.length > 0) {
                    logger.success(`${opts.dryRun ? 'Would fix' : 'Fixed'} ${report.fixed.length} issue${report.fixed.length === 1 ? '' : 's'}:`);
                    for (const f of report.fixed) {
                        display.line(`  ✓ ${f}`);
                    }
                }
                if (report.warnings.length > 0) {
                    logger.warn(`${report.warnings.length} warning${report.warnings.length === 1 ? '' : 's'} (require human review):`);
            ...
            logger.dim('');
            logger.dim(`Summary: ${opts.dryRun ? 'would fix' : 'fixed'} ${totalFixed}, warnings ${totalWarnings}.`);
        }
        catch (err) {
            logger.error(err.message);
            process.exit(1);
