...
import { createArtifact, listArtifacts, readArtifact, readArtifactRaw, updateArtifact, } from '../../services/artifact-service.js';
...
export function registerSprintCommand(program) {
    ...
    sprint
        ...
        if (activeSprint) {
            logger.error(`Sprint ${activeSprint.id} is already active. Close it first with \`planr sprint close\`.`);
            return;
        }
        ...
        const { id, filePath } = await createArtifact(projectDir, config, 'sprint', 'sprints/sprint.md.hbs', {
            title: name,
            name,
            startDate,
            endDate,
    ...
    sprint
        ...
        for (const taskId of taskIds) {
            ...
            taskMeta[taskId] = {
                title: artifact.data.title || taskId,
                filename,
            };
        }
        ...
        const currentIds = Array.isArray(data.taskIds) ? data.taskIds : [];
        ...
        let updated = raw.replace(/^taskIds: \[.*\]$/m, `taskIds: [${allIds.map((id) => `"${id}"`).join(', ')}]`);
        ...
        logger.success(`Added ${newIds.length} task${newIds.length !== 1 ? 's' : ''} to ${active.id}`);
    ...
    sprint
        ...
        for (const taskId of taskIds) {
            ...
            const taskRaw = await readArtifactRaw(projectDir, config, type, taskId);
            if (!taskRaw) {
    ...
    sprint
        ...
        for (const s of sprints) {
            const statusColor = s.status === 'active' ? chalk.green : s.status === 'closed' ? chalk.dim : chalk.yellow;
...
async function loadSprints(projectDir, config) {
...
async function getActiveSprint(projectDir, config) {
    const sprints = await loadSprints(projectDir, config);
...
async function autoSelectTasks(projectDir, config, sprintId) {
    ...
    try {
        ...
        for (const t of allTasks) {
            ...
            if (raw) {
                ...
                if (parentMatch)
                    parentInfo = ` (parent: ${parentMatch[1]})`;
            }
            taskSummaries.push({
                id: t.id,
        ...
        const { result } = await generateStreamingJSON(provider, messages, aiSprintAutoSelectResponseSchema, { maxTokens: TOKEN_BUDGETS.sprintAutoSelect });
...
function displayVelocityHistory(closedSprints) {
