...
export function registerInitCommand(program) {
    program
        ...
        .option('--no-pipeline-rules', 'deprecated compatibility flag; runtime installation belongs to `planr setup`', false)
        .action(async (opts) => {
        ...
        if (opts.ai === false) {
            ...
            logger.warn('AI features disabled (--no-ai). `planr spec decompose`, `planr refine`, ' +
            ...
            if (enableAI) {
                ...
                if (envVar) {
                    ...
                    if (existing) {
                        ...
                        const sourceLabel = existing.source === 'env'
                        ...
                        const apiKey = await promptSecret(`API key (or press Enter to set ${envVar} env var later):`);
                        if (apiKey.trim()) {
                            const storage = await saveCredential(provider, apiKey.trim());
                            const where = storage === 'keychain'
                                ? 'OS keychain'
