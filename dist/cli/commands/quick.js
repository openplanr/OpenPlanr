...
import { addChildReference, createArtifact, getArtifactDir, listArtifacts, readArtifact, readArtifactRaw, resolveArtifactFilename, updateArtifact, updateArtifactFields, } from '../../services/artifact-service.js';
...
import { buildTaskItems, countTaskItems, displayNextSteps, displayTaskPreview, displayValidationWarnings, handleAIError, } from '../helpers/task-creation.js';
export function registerQuickCommand(program) {
    ...
    quick
        ...
        .option('--epic <epicId>', 'link this quick task to an epic so `planr linear push EPIC-XXX` cascades to it')
    ...
    quick
        ...
        .option('--all-pending', 'set status=pending AND flip every `N.M` task checkbox to `[ ]` in the body')
        .action(async (qtId, opts) => {
        const projectDir = program.opts().projectDir;
        const config = await loadConfig(projectDir);
        let intent;
        ...
        if (intent.useBulk) {
            try {
                const result = await applyBulkCheckboxes(projectDir, config, 'quick', qtId, intent.bulkStatus);
                const checkboxNote = result.flippedAny ? ' (subtasks flipped)' : '';
                logger.success(`Updated ${qtId}: status=${result.status}${checkboxNote}`);
            }
            catch (err) {
                logger.error(`Failed to update ${qtId}: ${err.message}`);
                process.exit(1);
            }
            return;
        }
...
export async function createQuickWithAI(projectDir, config, description, options = {}) {
    ...
    try {
        ...
        const confirmCreate = await promptConfirm(confirmLabel ?? `Create quick task list with ${total} items?`, true);
        if (!confirmCreate) {
...
// ---------------------------------------------------------------------------
async function createQuickManually(projectDir, config, description, epicId) {
    ...
    const { id, filePath } = await createArtifact(projectDir, config, 'quick', 'quick/quick-task.md.hbs', { title, tasks, epicId });
...
// ---------------------------------------------------------------------------
async function promoteQuickTask(projectDir, config, qtId, opts) {
    ...
    const title = quickData.data.title || qtId;
    ...
    logger.dim(`  ${filePath}`);
    if (opts.story)
        logger.dim(`  Linked to story ${opts.story}`);
    if (opts.feature)
