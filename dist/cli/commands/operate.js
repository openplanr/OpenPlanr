...
function renderHuman(result) {
    ...
    if (typeof result.action === 'string' &&
        ...
        if (lease && typeof lease === 'object' && !Array.isArray(lease)) {
            ...
            display.line(status.expired === true
...
async function executeForResult(program, command, action, args, options) {
    ...
    let stdin;
    try {
        const richAgentDocument = ['context.review', 'harness.record', 'adapter.record'].includes(action);
        ...
        const recovery = code === 'E_OPERATE_STDIN_REQUIRED'
            ...
            : `Reduce the stdin payload to ${['context.review', 'harness.record', 'adapter.record'].includes(action)
...
export function registerOperateCommand(program) {
    ...
    json(operate
        ...
        .option('--format <format>', 'markdown or json')).action(function (cycleId, opts) {
