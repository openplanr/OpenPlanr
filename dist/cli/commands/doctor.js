...
export function registerDoctorCommand(program, cliVersion) {
    program
        ...
        .option('--yes', 'apply previewed owned-file and stale-daemon repairs without confirmation', false)
        .action(async (opts) => {
        const projectDir = program.opts().projectDir;
        let result = await runtimeDoctor(projectDir, opts.fix ? { pipelineRepair: 'preview' } : undefined);
        if (opts.fix) {
        ...
        if (opts.json)
            ...
            for (const diagnostic of result.diagnostics) {
                const label = diagnostic.status === 'pass' ? 'PASS' : diagnostic.status === 'warn' ? 'WARN' : 'FAIL';
