...
import { createArtifact, listArtifacts, readArtifact, readArtifactRaw, updateArtifact, updateArtifactFields, } from '../../services/artifact-service.js';
...
export function registerBacklogCommand(program) {
    ...
    backlog
        ...
        for (const item of filtered) {
            ...
            const tagStr = item.tags.length > 0 ? chalk.dim(` ${item.tags.map((t) => `#${t}`).join(' ')}`) : '';
    ...
    backlog
        ...
        try {
            ...
            const { result } = await generateStreamingJSON(provider, messages, aiBacklogPrioritizeResponseSchema, { maxTokens: TOKEN_BUDGETS.backlogPrioritize });
            ...
            for (const rec of result.items) {
                ...
                if (!item || item.priority === rec.priority)
                    continue;
    ...
    backlog
        ...
        try {
            ...
            logger.error(`Failed to update ${blId}: ${err.message}`);
            process.exit(1);
        }
    });
}
async function loadBacklogItems(projectDir, config) {
...
async function markPromoted(projectDir, config, blId, targetId) {
    const raw = await readArtifactRaw(projectDir, config, 'backlog', blId);
    if (!raw)
        return;
...
function truncateTitle(description, maxLength = 60) {
    if (!description)
        return 'Untitled';
