...
import { validateClaimsHaveAnchors, validateRemoteEvidence, } from '../../services/evidence-service.js';
...
export function registerReportCommand(program) {
    program
        .command('report <type>')
        .description('Generate stakeholder report (sprint, weekly, executive, standup, retro, release) from planr + GitHub')
        ...
        .option('--strict-evidence', 'fail if bullet claims under ## headings lack URLs or #issue references', false)
        ...
        if (!reportType) {
            logger.error(`Unknown report type "${typeArg}". Use: sprint, weekly, executive, standup, retro, release`);
            process.exit(1);
        }
        ...
        if (opts.lint) {
            ...
            for (const f of lint.findings) {
                ...
                if (f.suggestion)
                    display.line(` → ${f.suggestion}`);
            }
            for (const c of lint.coaching) {
                if (c.educational)
                    display.line(`  i ${c.educational}`);
            }
            if (!lint.ok) {
        ...
        if (mdPath)
            logger.success(`Wrote ${path.relative(projectDir, mdPath)}`);
        if (htmlPath)
            logger.success(`Wrote ${path.relative(projectDir, htmlPath)}`);
        if (opts.push) {
            ...
            for (const t of targets) {
                if (t === 'github') {
                    ...
                    if (res.ok)
                        logger.success(res.message + (res.url ? ` ${res.url}` : ''));
                    else
