...
import { promptCheckbox, promptMappingStrategy, promptSecret, promptSelect, promptStandaloneProject, } from '../../services/prompt-service.js';
...
export function registerLinearCommand(program) {
    ...
    linear
        ...
        if (teams.length === 1) {
            ...
            if (isNonInteractive()) {
                logger.error('Multiple teams are available. Run `planr linear init` in an interactive terminal to choose allowed teams and a default, or configure `linear.teams` and `linear.teamId` in `.planr/config.json`.');
                process.exit(1);
                return;
            }
            const scope = await promptSelect('Which Linear teams may OpenPlanr use?', [
            ...
            if (scope === 'choose') {
                ...
                const selectedIds = await promptCheckbox('Select Linear teams:', teams.map((team) => ({
        ...
        if (validatedTeams.length > 1) {
            const defaultTeamId = await promptSelect('Which team should OpenPlanr use by default?', validatedTeams.map((team) => ({
                name: `${team.name} (${team.key})`,
                value: team.id,
    ...
    linear
        ...
        .option('--push-parents', "If a parent in the chain is not yet pushed to Linear, push it first (upward attachment only — does NOT push the parent's siblings)", false)
        .option('--no-cascade', 'Push only the target artifact (and minimum parent chain when --push-parents is set). EPIC/FEAT pushes skip their descendants.', false)
        ...
        if (linearConfig.teamKey)
            display.line(`Team: ${linearConfig.teamKey}`);
        display.blank();
        if (o.dryRun) {
            try {
