...
import { findArtifactTypeById, listArtifacts, readArtifact, readArtifactRaw, updateArtifact, updateArtifactFields, } from '../../services/artifact-service.js';
import { loadConfig } from '../../services/config-service.js';
import { buildIssueBody, cleanTitle, createIssue, ensureLabel, ensureMilestone, getIssue, getIssueTypeForArtifact, getLabelForType, issueStateToStatus, setIssueType, statusToIssueState, updateIssue, verifyGitHubRepo, } from '../../services/github-service.js';
...
async function pushSingleArtifact(projectDir, config, artifactId, opts) {
    ...
    if (!raw)
        return null;
    const title = cleanTitle(artifactId, artifact.data.title);
    ...
    const existingIssueNumber = artifact.data.githubIssue;
    let issueNumber;
    let issueUrl;
    let action;
...
async function collectEpicArtifacts(projectDir, config, epicId) {
    const ids = [epicId];
...
export function registerGitHubCommand(program) {
    ...
    github
        ...
        if (opts.all) {
            // Push everything
            const types = ['epic', 'feature', 'story', 'task', 'quick'];
            ...
            const milestoneTitle = `${opts.epic}: ${epic.data.title}`;
            try {
        ...
        const milestoneTitle = opts.epic
            ? `${opts.epic}: ${(await readArtifact(projectDir, config, 'epic', opts.epic))?.data.title}`
            : undefined;
        for (const id of artifactIds) {
            try {
                ...
                logger.error(`  Failed to push ${id}: ${e.message}`);
            }
        }
        display.blank();
    ...
    github
        ...
        .option('--direction <dir>', 'sync direction: pull (GitHub→local), push (local→GitHub), or both', 'both')
        .action(async (opts) => {
        const projectDir = program.opts().projectDir;
        const config = await loadConfig(projectDir);
        let repoInfo;
            ...
            for (const a of artifacts) {
                ...
                if (data?.data.githubIssue) {
                    linkedArtifacts.push({
                        id: a.id,
                        type,
                        issueNumber: data.data.githubIssue,
        ...
        logger.dim(`Found ${linkedArtifacts.length} linked artifact${linkedArtifacts.length !== 1 ? 's' : ''}`);
        display.blank();
        let synced = 0;
        const conflicts = [];
        for (const artifact of linkedArtifacts) {
            try {
                ...
                if (opts.direction === 'pull') {
                    ...
                    display.line(`  ${chalk.blue('←')} ${artifact.id}: ${chalk.dim(localStatus)} → ${chalk.green(remoteStatus)} (from #${artifact.issueNumber})`);
                    synced++;
                }
                    ...
                    display.line(`  ${chalk.yellow('→')} ${artifact.id}: #${artifact.issueNumber} ${chalk.dim(issue.state)} → ${chalk.green(targetState)}`);
                    synced++;
                }
                else {
                ...
                logger.error(`  Failed to sync ${artifact.id}: ${e.message}`);
            }
        }
        ...
        if (conflicts.length > 0) {
            ...
            for (const conflict of conflicts) {
                ...
                display.line(`  ${chalk.red('!')} ${conflict.id} — local: ${chalk.yellow(conflict.localStatus)}, GitHub #${conflict.issueNumber}: ${chalk.cyan(conflict.remoteState)} (${remoteStatus})`);
                const action = await promptSelect(`  Resolve ${conflict.id}:`, [
                    {
                ...
                if (action === 'pull') {
                    await updateArtifactFields(projectDir, config, findArtifactTypeById(conflict.id) || 'task', conflict.id, { status: remoteStatus });
    ...
    github
        ...
        let linked = 0;
        let unlinked = 0;
        display.line(`  ${chalk.dim('Artifact'.padEnd(14))} ${chalk.dim('Status'.padEnd(14))} ${chalk.dim('Issue'.padEnd(10))} ${chalk.dim('State')}`);
        ...
        for (const type of types) {
            ...
            for (const a of artifacts) {
                ...
                const issueNumber = data?.data.githubIssue;
                if (issueNumber) {
                    let issueState = '';
                    try {
                    ...
                    const inSync = issueStateToStatus(issueState) === status ||
                        statusToIssueState(status) === issueState;
                    const syncIcon = inSync ? chalk.green('✓') : chalk.red('✗');
                    display.line(`  ${a.id.padEnd(14)} ${status.padEnd(14)} #${String(issueNumber).padEnd(8)} ${issueState} ${syncIcon}`);
                    linked++;
                }
                else {
                    display.line(`  ${a.id.padEnd(14)} ${status.padEnd(14)} ${chalk.dim('—'.padEnd(10))} ${chalk.dim('not linked')}`);
                    unlinked++;
                }
            }
        }
