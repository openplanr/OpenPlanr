...
import { applySetup, detectRuntimes, inspectProjectContext, listRuntimeAdapters, previewSetup, RuntimeManagerError, } from '../../services/runtime-manager-service.js';
...
function printRuntimeDetection() {
    display.heading('Detected coding agents');
    for (const item of detectRuntimes()) {
        if (item.installed)
            display.line(`  ✓ ${runtimeLabels[item.runtime]} (${item.command})`);
        else
            display.line(`  – ${runtimeLabels[item.runtime]} not detected (${item.command}; install or enable its shell command to use it)`);
    }
    display.blank();
...
function printPreview(preview) {
    ...
    for (const scope of ['user', 'project']) {
        const actions = preview.actions.filter((action) => action.scope === scope && action.operation !== 'unchanged');
        ...
        display.keyValue(`${scope === 'user' ? 'User' : 'Project'} scope`, `${creates} create, ${updates} update`);
    }
    if (isVerbose()) {
        display.blank();
    ...
    for (const diagnostic of preview.runtimeDiagnostics.filter((item) => item.status !== 'pass')) {
        ...
        if (diagnostic.fix)
            display.line(`       Fix: ${diagnostic.fix}`);
    }
}
export function registerSetupCommand(program, cliVersion) {
    program
        .command('setup')
        ...
        const guided = !isNonInteractive() &&
            opts.runtime === undefined &&
            opts.scope === undefined &&
            !opts.minimal;
        let minimal = Boolean(opts.minimal);
        let scope = opts.scope ?? 'user';
        let runtimes;
        if (guided) {
            logger.heading('Welcome to OpenPlanr');
            printRuntimeDetection();
            const mode = await promptSelect('What would you like to install?', [
                {
            ...
            if (!minimal) {
                ...
                if (detected.length === 0) {
                    throw new RuntimeManagerError('E_RUNTIME_NOT_FOUND', 'No supported coding agent was detected.', 'Install or enable Claude Code, Codex, or Cursor, then rerun setup.');
                }
                runtimes = await promptCheckbox('Which detected coding agents should OpenPlanr configure?', detected.map((item) => ({
                    name: `${runtimeLabels[item.runtime]} (${item.command})${adapters
            ...
            const adapters = listRuntimeAdapters();
            const requiresProject = !minimal &&
                (runtimes ?? []).some((runtime) => !adapters.find((adapter) => adapter.id === runtime)?.installScopes.includes('user'));
            display.blank();
            ...
            if (requiresProject && !context.valid) {
                throw new RuntimeManagerError('E_PROJECT_CONTEXT_REQUIRED', 'One or more selected coding agents require project-scoped installation.', 'Change into a Git or initialized OpenPlanr project, or select only user-scope agents.');
            }
            const scopeChoices = [];
            if (!requiresProject) {
            ...
            if (context.valid) {
                scopeChoices.push({ name: `Current project — ${context.path}`, value: 'project' }, { name: `Both user and current project — ${context.path}`, value: 'both' });
            }
            scope = await promptSelect('Where should integrations be installed?', scopeChoices, requiresProject ? 'project' : 'user');
        }
        const options = {
            projectDir,
            cliVersion,
        ...
        if (!opts.yes && !program.opts().yes && isNonInteractive()) {
            throw new RuntimeManagerError('E_CONFIRMATION_REQUIRED', 'Setup cannot apply changes without confirmation in a non-interactive terminal.', 'Review `planr setup --dry-run`, then rerun with explicit choices and `--yes`.');
        }
        const confirmed = opts.yes || program.opts().yes || (await promptConfirm('Apply these changes?', true));
        if (!confirmed) {
            if (opts.json)
                display.line(JSON.stringify({ ok: false, action: 'cancelled' }));
            else
                logger.warn('Setup cancelled; no files were changed.');
            return;
        }
        ...
        if (opts.json)
            ...
            if (result.backupDir)
                display.keyValue('Backup', result.backupDir);
            if (result.appliedRuntimeOperations?.length) {
                display.keyValue('Claude Code', `${result.appliedRuntimeOperations.length} marketplace/plugin operation(s) applied`);
            }
