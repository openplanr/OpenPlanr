...
export class CodexGenerator extends BaseGenerator {
    ...
    async generate(_artifacts) {
        ...
        if (this.includesAgile()) {
            const content = await renderTemplate('rules/codex/AGENTS.md.hbs', baseData, this.config.templateOverrides);
        ...
        if (this.includesPipeline()) {
            ...
            const content = await readFile(path.join(pipeline.root, 'adapters', 'codex', 'project-guidance.md'));
