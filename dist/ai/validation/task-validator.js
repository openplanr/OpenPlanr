 ...
 */
export function validateRelevantFiles(relevantFiles, sourceInventory) {
    const warnings = [];
    ...
    for (const file of relevantFiles) {
        ...
        if (file.action === 'modify' && !exists) {
            warnings.push(`${file.path} marked as "modify" but not found in source inventory — may not exist`);
        }
 ...
 */
export function parseSourceInventory(inventory) {
    const files = new Set();
...
function directoryExistsInInventory(dir, inventory) {
