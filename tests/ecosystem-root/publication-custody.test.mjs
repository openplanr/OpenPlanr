...
function pipelineFixture() {
  ...
  writeFileSync(join(pipeline, 'package.json'), JSON.stringify({
    name: 'planr-pipeline', version: '0.49.1', type: 'module', license: 'MIT',
    repository: 'git+https://github.com/openplanr/OpenPlanr.git', files: ['lib/'],
  }));
  ...
  const manifest = { entries: [{ source: 'packages/protocol/src/renderer.mjs', target: 'lib/artifact/renderer.mjs', sha256: digest, mode: '644' }] };
  ...
  for (const path of ['lib/generated/protocol-projection.json', 'lib/generated/domain-projections/design.json', 'lib/generated/domain-projections/operate.json']) {
...
assert.deepEqual(JSON.parse(readFileSync('packages/pipeline/${manifestFile}','utf8')),${JSON.stringify(manifest)});\n`;
  for (const script of ['scripts/protocol/project-protocol.mjs', 'scripts/domains/project-domains.mjs', 'packages/protocol/scripts/generate-protocol-assets.mjs']) writeFileSync(join(f.root, script), checker);
...
function preparePipeline(f, environment = {}) {
  return spawnSync(process.execPath, [join(f.root, 'scripts/prepare-publication.mjs'), join(f.temporary, 'publication')], {
    cwd: f.root, encoding: 'utf8', timeout: 30000,
    env: { ...process.env, RELEASE_PACKAGE: 'planr-pipeline', RELEASE_VERSION: '0.49.1', npm_config_audit: 'false', npm_config_fund: 'false', ...environment },
  });
}
...
test('publication includes reproduced pipeline outputs without tracking duplicate bytes', () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    const proof = JSON.parse(readFileSync(join(f.temporary, 'publication/publication.json'), 'utf8'));
    const files = execFileSync('tar', ['-tzf', join(f.temporary, 'publication', proof.filename)], { encoding: 'utf8' });
...
test('publication rejects an ignored extra file inside a generated pipeline directory', () => {
  const f = pipelineFixture();
  try {
    writeFileSync(join(f.pipeline, 'lib/artifact/private.mjs'), 'export const confidential = true;\n');
    const result = preparePipeline(f);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /not a reviewed source or declared build output: lib\/artifact\/private.mjs/u);
  } finally { rmSync(f.temporary, { recursive: true, force: true }); }
});
...
test('publication rejects modified generated bytes even in an otherwise clean checkout', () => {
  const f = pipelineFixture();
  try {
    writeFileSync(join(f.pipeline, 'lib/artifact/renderer.mjs'), 'export const renderer = "unreviewed";\n');
    const result = preparePipeline(f);
    assert.notEqual(result.status, 0);
...
test('publication rejects generated bytes changed or removed during packing', () => {
  for (const mutation of ['rewrite', 'remove']) {
    ...
    try {
      ...
      writeFileSync(mutate, mutation === 'rewrite'
      ...
      writeFileSync(join(wrapper, 'npm'), `#!/bin/sh\n${quote(process.execPath)} ${quote(mutate)}\nexec ${quote(npm)} "$@"\n`);
      chmodSync(join(wrapper, 'npm'), 0o755);
      const result = preparePipeline(f, { PATH: `${wrapper}:${process.env.PATH}` });
      assert.notEqual(result.status, 0);
      assert.match(result.stderr, /Generated archive bytes changed or missing: lib\/artifact\/renderer.mjs/u);
    } finally { rmSync(f.temporary, { recursive: true, force: true }); }
  }
});
