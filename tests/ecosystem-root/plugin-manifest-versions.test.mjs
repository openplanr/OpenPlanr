...
test('host plugin manifests and local marketplaces carry the CLI package version', () => {
  ...
  for (const path of [
    ...
    for (const plugin of marketplace.plugins) assert.equal(plugin.version, cliVersion, `${path} ${plugin.name}`);
  }
  ...
  for (const adapter of registry.adapters) assert.equal(adapter.version, cliVersion, `${adapter.id} adapter version`);
  assert.equal(json('packages/cli/lib/host-packages/capability-map.json').pluginVersion, cliVersion);
});
