...
test('role registry has exact canonical/legacy parity and fixed authority boundaries', () => {
  ...
  assert.deepEqual(getRole('planr-backend').outputContracts, [
    { id: 'implementation-result', version: '1.0.0' },
  ]);
  assert.deepEqual(getRole('planr-frontend').outputContracts, [
    { id: 'implementation-result', version: '1.0.0' },
  ]);
  for (const roleId of [
    'planr-database',
    ...
    assert.deepEqual(
      getRole(roleId).outputContracts,
      [{ id: 'task-output-manifest', version: '1.0.0' }],
      roleId,
    );
  }
});
...
test('alias collision fails with a stable typed diagnostic', () => {
  const values = clone();
  values['roles.json'].roles[1].legacyAliases[0].id =
    values['roles.json'].roles[0].legacyAliases[0].id;
  values['roles.json'] = redigest(values['roles.json']);
  assert.throws(
    () => validateCanonicalRegistries(values),
    (error) => error instanceof ProtocolError && error.code === 'E_PROTOCOL_DUPLICATE_ID',
  );
});

test('task-kind routing is exact and never inferred from prose', () => {
  assert.equal(
    routeLegacyTask({ legacyType: 'UI', legacyAgent: 'frontend-agent' }).taskKind,
    'frontend',
  );
  assert.equal(
    routeLegacyTask({ legacyType: 'Tech', legacyAgent: 'db-agent' }).taskKind,
    'database',
  );
  assert.equal(
    routeLegacyTask({ legacyType: 'Tech', legacyAgent: 'backend-agent' }).taskKind,
    'backend',
  );
  assert.equal(routeLegacyTask({ legacyType: 'Tech' }).taskKind, 'backend');
  assert.throws(
    () => routeLegacyTask({ legacyType: 'UI', legacyAgent: null }),
    (error) => error.code === 'E_PROTOCOL_REFERENCE_INVALID',
  );
  assert.throws(
    () => routeLegacyTask({ legacyType: 'Tech', legacyAgent: 'qa-agent', prose: 'database' }),
    (error) => error.code === 'E_PROTOCOL_REFERENCE_INVALID',
  );
  ...
  for (const binding of bindings) {
    ...
    assert.deepEqual(
      binding.outputContracts,
      [{ id: expected, version: '1.0.0' }],
      binding.taskKind,
    );
  }
});
...
test('catalog floors classify commands, skills, rules, and Class A-D outputs', () => {
  ...
  assert.ok(
    commands.negativeContracts.some(
      ({ forbiddenArgvPrefix }) => forbiddenArgvPrefix.join(' ') === 'pipeline operate',
    ),
  );
  ...
  assert.deepEqual(
    CANONICAL_REGISTRIES['rules.json'].rules.map(({ ruleId }) => ruleId),
    ['R1', 'R10', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R8', 'R9'],
  );
  assert.deepEqual(
    [
      ...new Set(
        CANONICAL_REGISTRIES['outputs.json'].outputs.map(({ outputClass }) => outputClass),
      ),
    ].sort(),
    ['A', 'B', 'C', 'D'],
  );
...
test('every declared skill CLI binding resolves to a registered command visible from root help', () => {
  ...
  for (const skill of CANONICAL_REGISTRIES['skills.json'].skills) {
    for (const requirement of skill.cliRequirements) {
      ...
      assert.equal(
        command.surface,
        requirement.commandId.startsWith('cli-') ? 'cli-root' : command.surface,
      );
      assert.deepEqual(requirement.argv, command.argv, `${skill.skillId}:${requirement.commandId}`);
      assert.match(
        result.stdout,
        new RegExp(`^  ${requirement.argv[0]}(?:[ \\[]|$)`, 'mu'),
        skill.skillId,
      );
    }
  }
