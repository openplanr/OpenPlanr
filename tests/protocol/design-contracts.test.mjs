...
const fixture = () => ({
  ...
  designSystem: { path: 'source/design-system', tokens: 'source/tokens.css', spacing: [0, 2.5, 5, 10, 20] },
  ...
  screens: [
    { id: 'overview', title: 'Overview', source: { html: 'source/overview.html', styles: ['source/theme.css'], scripts: ['source/app.js'] }, anchors: ['primary-action'] },
    { id: 'details', title: 'Account details', source: { html: 'source/details.html' }, anchors: ['primary-action'] },
  ],
  screenOrder: ['overview', 'details'],
  flows: [{ id: 'edit-account', title: 'Edit account', screens: ['overview', 'details', 'overview'] }],
  variants: [
    { id: 'quiet', label: 'Quiet clarity', status: 'ready' },
    { id: 'expressive', label: 'Expressive', status: 'ready', sources: { overview: { html: 'source/expressive/overview.html' } } },
    { id: 'incomplete', label: 'Incomplete', status: 'failed', issue: 'Rendering was interrupted.' },
  ],
...
test('authored design schema is additive, immutable, packaged identically and portable', async () => {
  const packaged = JSON.parse(readFileSync(new URL('../../packages/protocol/schemas/v1.9.0/design-document.schema.json', import.meta.url), 'utf8'));
  ...
  assert.equal((await import('@openplanr/protocol/design-contracts')).DESIGN_DOCUMENT_VERSION, '1.0.0');
  assert.deepEqual(validateJson(fixture(), packaged), []);
  assert.deepEqual(validateProtocolArtifact('design-document', fixture(), { protocolVersion: '1.9.0' }), []);
  assert.ok(listProtocolSchemas().some(({ kind, protocolVersion }) => kind === 'design-document' && protocolVersion === '1.9.0'));
});
...
test('walkthroughs are not limited to eight screens', () => {
  ...
  for (let index = 0; index < 12; index += 1) {
    document.screens.push({ id: `step-${index}`, title: `Step ${index}`, source: { html: `source/step-${index}.html` } });
...
test('document structure rejects malformed inputs and future or undeclared fields', () => {
  for (const value of [null, undefined, [], false, {}, { ...fixture(), schemaVersion: '2.0.0' }, { ...fixture(), runtime: {} }]) {
...
test('duplicate identities and feedback anchors cannot silently retarget review state', () => {
  for (const field of ['frames', 'screens', 'flows', 'variants']) {
    ...
    assert.ok(result.errors.some((error) => error.includes('duplicate identity')), result.errors.join('\n'));
  }
  const document = fixture();
...
test('screen order includes each screen exactly once and all flows and overrides resolve', () => {
  const mutations = [
    (document) => { document.screenOrder = ['overview']; },
    (document) => { document.screenOrder.push('missing'); },
    (document) => { document.screenOrder.push('overview'); },
    (document) => { document.flows[0].screens.push('missing'); },
    (document) => { document.variants[1].sources.missing = { html: 'source/missing.html' }; },
  ];
...
test('all authored source and asset paths reject escaping or external locations', () => {
  const badPaths = [
    '../outside.html', 'source/../../outside.html', './source/page.html', '/tmp/page.html',
    'C:\\temp\\page.html', 'source\\page.html', 'https://example.com/page.html',
    '//example.com/page.html', 'data:text/html,example', 'file:///tmp/page.html',
    'source/%2e%2e/page.html', 'source/page.html#fragment', 'source/page.html?query',
    'source/\u0000page.html', 'source//page.html', 'source/',
  ];
  const setters = [
    (document, path) => { document.screens[0].source.html = path; },
    (document, path) => { document.screens[0].source.styles = [path]; },
    (document, path) => { document.screens[0].source.scripts = [path]; },
    (document, path) => { document.variants[1].sources.overview.html = path; },
    (document, path) => { document.assets = [path]; },
    (document, path) => { document.designSystem.path = path; },
    (document, path) => { document.designSystem.tokens = path; },
  ];
