...
test('Protocol 1.6 owns nineteen closed additive schemas with exact identities', () => {
  const files = readdirSync(join(protocol, 'schemas', 'v1.6.0')).filter((name) => name.endsWith('.json')).sort();
  ...
  const completion = JSON.parse(readFileSync(join(protocol, 'schemas', 'v1.6.0', 'skill-completion-receipt.schema.json'), 'utf8'));
...
test('generated v1.6 and v1.7 registries validate against their contracts and RFC 8785 digests', () => {
  ...
  const generated = readdirSync(join(protocol, 'registries')).filter((file) => file in descriptors).sort();
  ...
  for (const [file, descriptor] of Object.entries(descriptors)) {
    ...
    assert.deepEqual(validateProtocolArtifact(descriptor.kind, value, { protocolVersion: descriptor.protocolVersion }), [], file);
  }
});
...
test('projection and ecosystem generators share canonical versioned registry descriptors', () => {
  ...
  for (const relativePath of consumers) {
    ...
    for (const file of Object.keys({ ...DIAGRAM_V16_REGISTRIES, ...PROTOCOL_V17_REGISTRIES })) {
      assert.doesNotMatch(source, new RegExp(`['\"]${file.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}['\"]`, 'u'), `${relativePath} duplicates ${file}`);
    }
  }
});

test('valid composed, markdown, module-registry, and consent contracts pass validation', () => {
  assert.deepEqual(validateProtocolArtifact('skill-source', load('skill-source-composed.json'), { protocolVersion: V16 }), []);
  assert.deepEqual(validateProtocolArtifact('skill-source', load('skill-source-markdown.json'), { protocolVersion: V16 }), []);
  assert.deepEqual(validateProtocolArtifact('skill-module-registry', load('skill-module-registry.json'), { protocolVersion: V16 }), []);
  assert.deepEqual(validateProtocolArtifact('skill-consent-record', load('skill-consent-record.json'), { protocolVersion: V16 }), []);
});

test('the composed-v1 and markdown-v1 modes are mutually exclusive', () => {
  const composed = load('skill-source-composed.json');
  const crossed = { ...composed, markdown: { path: 'skills/x/SKILL.md', digest: composed.template.digest } };
  assert.ok(
...
test('source-mode selectors require concrete sources and composed skills require a host', () => {
  const markdown = load('skill-source-markdown.json');
  assert.ok(
    validateProtocolArtifact('skill-source', { ...markdown, markdown: null }, { protocolVersion: V16 }).length > 0,
    'markdown-v1 may not select its source branch with a null source',
  );
  assert.deepEqual(
    validateProtocolArtifact('skill-source', { ...markdown, hostProfiles: [] }, { protocolVersion: V16 }),
    [],
  ...
  const composed = load('skill-source-composed.json');
  assert.ok(
    validateProtocolArtifact('skill-source', { ...composed, template: null }, { protocolVersion: V16 }).length > 0,
    'composed-v1 may not select its source branch with a null template',
  );
  assert.ok(
    validateProtocolArtifact('skill-source', { ...composed, hostProfiles: [] }, { protocolVersion: V16 }).length > 0,
...
test('closed objects reject an unexpected property', () => {
  ...
  assert.ok(
    validateProtocolArtifact('skill-source', source, { protocolVersion: V16 })
  ...
  assert.ok(
    validateProtocolArtifact('skill-module-registry', withExtra, { protocolVersion: V16 })
      .some(({ rule }) => rule === 'additionalProperties'),
  );
});
...
test('the authority ceiling encodes the component-wise monotone lattice domains', () => {
  ...
  for (const value of ['declared-paths', 'read-only', 'none']) {
    const ok = { ...source, authorityCeiling: { ...source.authorityCeiling, repositoryAccess: value } };
    assert.deepEqual(validateProtocolArtifact('skill-source', ok, { protocolVersion: V16 }), [], `repositoryAccess ${value}`);
  }
  const badRepo = { ...source, authorityCeiling: { ...source.authorityCeiling, repositoryAccess: 'write' } };
  ...
  const badExternal = { ...source, authorityCeiling: { ...source.authorityCeiling, externalDataAccess: 'declared-paths' } };
  assert.ok(validateProtocolArtifact('skill-source', badExternal, { protocolVersion: V16 }).length > 0);
  ...
  assert.ok(validateProtocolArtifact('skill-source', missingComponent, { protocolVersion: V16 }).length > 0);
});

test('a learning record may not imply consent without an explicit consent reference', () => {
  const base = {
    kind: 'skill-learning-record', schemaVersion: '1.0.0', protocolVersion: V16, documentVersion: '1.0.0',
    digestAlgorithm: 'sha256', canonicalization: 'rfc8785',
    documentDigest: `sha256:${'a'.repeat(64)}`,
    learningId: 'learn-1', skillId: 'planr-plan', observedAt: '2026-08-30T00:00:00Z',
    category: 'context', note: 'Observed a reusable context step.', consentGranted: false, consentRef: null,
  };
  assert.deepEqual(validateProtocolArtifact('skill-learning-record', base, { protocolVersion: V16 }), []);
  const impliedConsent = { ...base, consentGranted: true, consentRef: null };
  assert.ok(
    validateProtocolArtifact('skill-learning-record', impliedConsent, { protocolVersion: V16 }).length > 0,
