...
function task(overrides = {}) {
  const value = {
    ...
    task: {
      taskId: 'T-001',
      path: 'output/feats/feat-demo/us-001/tasks/task-1.md',
      schemaRef: { id: 'task', version: '1.0.0' },
      contentDigest: digest('task'),
    },
    ...
    expectedOutputs: [
      {
        ...
        contractRef: { id: 'task-output-manifest', version: '1.0.0' },
        paths: [],
        pathArguments: {},
      },
    ],
    acceptanceRefs: [
      {
        ...
        reference: 'output/feats/feat-demo/us-001/acceptance.feature',
        statement: 'Scenario: user sees the result',
      },
    ],
    ruleIds: ['R2', 'R5', 'R6', 'R9'],
    reviewBinding: {
      planDigest: digest('plan'),
      planningReviewReceiptDigest: digest('review'),
      ownerDecisionDigest: digest('owner'),
    },
...
function output(taskManifest, overrides = {}) {
  return withDocumentDigest({
    ...
    taskBinding: {
      taskManifestId: taskManifest.taskManifestId,
      taskManifestDigest: taskManifest.documentDigest,
    },
    roleBinding: {
      roleId: taskManifest.routing.roleId,
      roleVersion: taskManifest.routing.roleVersion,
      registryDigest: taskManifest.routing.roleRegistryDigest,
    },
    attempt: { initialAttempt: 1, correctionIteration: 0 },
    outcome: 'completed',
    ...
    preserveVerification: [
      {
        ...
        beforeDigest: digest('protected'),
        afterDigest: digest('protected'),
        unchanged: true,
      },
    ],
    commandEvidence: [
      {
        ...
        workingDirectory: { repositoryKey: 'project', path: 'src' },
        startedAt: '2026-08-29T00:01:00Z',
        completedAt: '2026-08-29T00:02:00Z',
        exitCode: 0,
    ...
    acceptanceEvidence: [
      {
        acceptanceRef: 'scenario-user-sees-result',
        verifier: 'qa',
        result: 'passed',
...
test('implementation result validates concise task and direct-request outcomes', () => {
  ...
  const direct = implementationResult({
    outcome: {
      status: 'partial',
      summary: 'The implementation works; one optional check was unavailable.',
    },
    ...
    checks: [
      {
        ...
        status: 'not-run',
        detail: 'No browser runtime is installed.',
      },
    ],
    issues: [
      {
        problem: 'Browser QA was not run.',
        impact: 'Interactive behavior is not browser-verified.',
        nextAction: 'Run browser QA when Chromium is available.',
      },
    ],
  });
  ...
  assert.ok(
    validateProtocolArtifact('implementation-result', { ...planned, attempt: 1 }).some(
  ...
  assert.ok(
    validateProtocolArtifact('implementation-result', withoutIssues).some(
      ({ rule }) => rule === 'required',
    ),
  );
});
...
test('legacy task output remains readable and satisfies its original semantic joins', () => {
  ...
  assert.deepEqual(
    validateProtocolArtifact('task-manifest', manifest, { protocolVersion: '1.5.0' }),
    [],
  );
  assert.deepEqual(
    validateProtocolArtifact('task-output-manifest', result, { protocolVersion: '1.5.0' }),
    [],
  );
...
test('routing mismatch, scope overlap, and unreviewed issuance have stable diagnostics', () => {
  ...
  const invalid = task({
    ...
    reviewBinding: {
      planDigest: null,
      planningReviewReceiptDigest: null,
      ownerDecisionDigest: null,
    },
  });
  assert.deepEqual(
    validateTaskManifestSemantics(invalid).map(({ code }) => code),
...
test('task graph catches cycles and R2 ignores preflight/quality work', () => {
  ...
  assert.ok(
    validateTaskGraph([first, secondBase]).some(({ code }) => code === 'E_TASK_GRAPH_CYCLE'),
  );
...
test('output semantics enforce Preserve, QA read-only, adaptive recovery, and terminal correlations', () => {
  ...
  const codes = validateTaskOutputSemantics(changedPreserve, { taskManifest: manifest }).map(
    ({ code }) => code,
  );
  ...
  const qa = output(manifest, {
    roleBinding: {
      roleId: 'planr-qa',
      roleVersion: '1.0.0',
      registryDigest: manifest.routing.roleRegistryDigest,
    },
  });
  ...
  assert.deepEqual(
    validateProtocolArtifact('task-output-manifest', fourth, { protocolVersion: '1.5.0' }),
    [],
  );

  const blocked = output(manifest, { outcome: 'blocked', diagnostics: [], errorHandoff: null });
  assert.ok(
    validateTaskOutputSemantics(blocked).some(
      ({ code }) => code === 'E_TASK_OUTPUT_BLOCKED_HANDOFF_REQUIRED',
    ),
  );
});
