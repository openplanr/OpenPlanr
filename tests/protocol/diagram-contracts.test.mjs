...
test('Protocol 1.6 exposes the complete closed diagram contract family', () => {
  ...
  assert.equal(diagramDocumentPath('system-context'), 'diagrams/system-context/system-context.planr-diagram.json');
...
test('diagram grammar and semantic-pattern registries are complete and self-validating', () => {
  ...
  assert.equal(new Set(DIAGRAM_GRAMMAR_REGISTRY.grammars.map(({ grammarId }) => grammarId)).size, 39);
  ...
  assert.deepEqual(validateProtocolArtifact('diagram-type-registry', DIAGRAM_GRAMMAR_REGISTRY, { protocolVersion: '1.6.0' }), []);
  assert.deepEqual(validateProtocolArtifact('diagram-semantic-pattern-registry', DIAGRAM_SEMANTIC_PATTERN_REGISTRY, { protocolVersion: '1.6.0' }), []);
  for (const grammar of DIAGRAM_GRAMMAR_REGISTRY.grammars) {
    assert.strictEqual(getDiagramGrammar(grammar.grammarId), grammar);
    assert.ok(grammar.requiredPrimitives.every((primitive) => grammar.allowedPrimitives.includes(primitive)));
...
test('every generated grammar fixture is a valid canonical diagram document', () => {
  for (const grammar of DIAGRAM_GRAMMAR_REGISTRY.grammars) {
    ...
    assert.deepEqual(validateProtocolArtifact('diagram-document', document, { protocolVersion: '1.6.0' }), [], grammar.grammarId);
  }
});

test('canonical diagram schema rejects renderer geometry and undeclared fields', () => {
  const fixture = JSON.parse(readFileSync(join(artifact, 'fixtures', 'diagram', 'grammars', 'flowchart.planr-diagram.json'), 'utf8'));
