...
const failWith = (kind, value, rule, options = {}) => {
  ...
  assert.ok(
    errors.some((error) => error.rule === rule),
    `Expected ${rule}, received ${JSON.stringify(errors)}`,
  );
...
test('blank drafts, empty containers and four populated authoring profiles retain their exact content', () => {
  const fixtures = [
    makeBundle('flowchart', { blank: true }),
    makeBundle('flowchart', { emptyContainer: true }),
  ...
  for (const bundle of fixtures) {
    ...
    assert.equal(
      ...
      encoded,
      'Validation cannot inject placeholder content or rewrite identifiers',
    );
  }
  assert.deepEqual(summarizeDiagramAuthoringContent(fixtures[0]), {
    elementCount: 0,
    hasVisibleContent: false,
  });
  assert.deepEqual(summarizeDiagramAuthoringContent(fixtures[1]), {
    elementCount: 1,
...
test('hostile inputs fail their domain or data constraints without mutation or accessor execution', () => {
  for (const result of evaluateAuthoringCases()) {
    ...
    assert.equal(
      ...
      `${result.name}: ${JSON.stringify(result.diagnostics)}`,
    );
    assert.equal(result.unchanged, true, result.name);
    assert.equal(result.getterReads, 0, result.name);
    if (result.expectedRule)
      assert.ok(
        result.diagnostics.some((error) => error.rule === result.expectedRule),
        `${result.name}: ${JSON.stringify(result.diagnostics)}`,
      );
  }
});
...
test('generic exact-version dispatch preserves dedicated domain rejection, not just schema validation', () => {
  for (const item of authoringCases().filter((item) => Object.keys(item.options).length === 0)) {
    ...
    const generic = validateProtocolArtifact(item.kind, item.value, {
...
test('generic version selection rejects an accessor before executing it', () => {
  ...
  let reads = 0;
  Object.defineProperty(bundle, 'protocolVersion', {
    enumerable: true,
    get() {
      reads++;
      return '1.13.0';
...
test('content digests exclude only their own field and bind each nested semantic and presentation basis', () => {
  ...
  assert.equal(
    bundle.presentation.presentationDigest,
    independent(bundle.presentation, 'presentationDigest'),
  );
  assert.equal(bundle.bundleDigest, independent(bundle, 'bundleDigest'));
  const transport = `sha256:${createHash('sha256')
  ...
  assert.equal(
    diagramAuthoringBundleDigest({ ...bundle, bundleDigest: 'ignored' }),
    bundle.bundleDigest,
  );
  assert.equal(
    diagramDocumentDigest({ ...bundle.document, documentDigest: 'ignored' }),
    bundle.document.documentDigest,
  );
  assert.equal(
    diagramPresentationDigest({ ...bundle.presentation, presentationDigest: 'ignored' }),
    bundle.presentation.presentationDigest,
  );
  const semantic = clone(bundle);
  semantic.document.nodes[0].label = 'Changed';
  semantic.document.documentDigest = diagramDocumentDigest(semantic.document);
  rehashBundle(semantic);
  failWith('diagram-authoring-bundle', semantic, 'basis');
  const moved = clone(bundle);
  moved.presentation.elements[0].bounds.x += 10;
  ...
  rehashBundle(moved);
  assert.deepEqual(validateDiagramAuthoringBundle(moved), []);
});
...
test('source correspondence retains exact Unicode and CRLF bytes and rejects substituted ranges or identities', () => {
  ...
  assert.equal(
    bundle.originalSource.sourceDigest,
    `sha256:${createHash('sha256').update(raw).digest('hex')}`,
  );
  assert.notEqual(
    bundle.originalSource.sourceDigest,
    digestText(SOURCE_TEXT.replaceAll('\r\n', '\n')),
  );
  ...
  failWith('diagram-source-map', bundle.sourceMap, 'source-digest', {
    document: bundle.document,
    sourceText: wrongBytes,
  });
  const duplicate = clone(bundle.sourceMap);
  duplicate.entries[1].sourceId = duplicate.entries[0].sourceId;
  failWith('diagram-source-map', duplicate, 'ambiguous-source-id', { bundle });
  const outOfRange = clone(bundle.sourceMap);
  outOfRange.entries[0].range.endByte = raw.length + 1;
  failWith('diagram-source-map', outOfRange, 'source-range', { bundle });
  const missing = clone(bundle.sourceMap);
  missing.entries[0].elementIds = ['missing'];
...
test('all six persisted edit classes describe bounded reversible data without arbitrary patch paths', () => {
  ...
  const afterMembership = state();
  afterMembership.groups[0].members = [];
  afterMembership.lanes[0].members.push('node-a');
  const operations = [
    {
      ...
      elements: [
        {
          collection: 'nodes',
          value: { id: 'new-node', label: 'Failure branch', kind: 'process', description: null },
        },
      ],
    ...
    makeTransaction(bundle).operations[0],
    {
      ...
      elements: [{ collection: 'annotations', value: clone(bundle.document.annotations[0]) }],
      presentation: [
        clone(bundle.presentation.elements.find((item) => item.elementId === 'note-a')),
      ],
    },
      ...
      changes: [
        {
          elementId: 'node-a',
          before: geometry(p),
          after: { ...geometry(p), bounds: { ...p.bounds, x: p.bounds.x + 20 } },
        },
      ],
    },
    {
      ...
      changes: [
        {
          elementId: 'node-a',
          before: { appearance: clone(p.appearance), locks: clone(p.locks) },
          after: {
            appearance: { ...p.appearance, fill: 'accent' },
            locks: { ...p.locks, position: true },
          },
        },
      ],
    },
  ...
  for (const operation of operations) {
    const transaction = {
      ...
      transactionId: `edit-${operation.type}`,
      operations: [operation],
    };
    assert.strictEqual(
      assertDiagramAuthoringArtifact('diagram-edit-transaction', transaction, {
        baseBundle: bundle,
      }),
      transaction,
    );
    const malformed = clone(transaction);
    malformed.operations[0].path = '/document/title';
    assert.ok(
      validate('diagram-edit-transaction', malformed, { baseBundle: bundle }).length,
      operation.type,
    );
  }
  const selfUndo = makeTransaction(bundle);
  selfUndo.undoOf = selfUndo.transactionId;
  ...
  duplicate.operations[0].elements[0].value.id = 'node-a';
  duplicate.operations[0].presentation[0].elementId = 'node-a';
  failWith('diagram-edit-transaction', duplicate, 'duplicate-id', { baseBundle: bundle });
  const noPlacement = { ...makeTransaction(bundle), operations: [clone(operations[0])] };
  noPlacement.operations[0].presentation[0].elementId = 'different-id';
  failWith('diagram-edit-transaction', noPlacement, 'presentation-coverage', {
    baseBundle: bundle,
  });
});
...
test('insertion positions preserve reversible collection ordering without an open patch surface', () => {
    ...
    operations: [
      {
        ...
        elements: [
          {
            collection: 'nodes',
            value: { id: 'restored-node', label: 'Restored', kind: 'process', description: null },
          },
        ],
  ...
  const append = clone(transaction);
  delete append.operations[0].positions;
  assert.deepEqual(validate('diagram-edit-transaction', append, { baseBundle: bundle }), []);
  const duplicate = clone(transaction);
  duplicate.operations[0].positions.push(clone(duplicate.operations[0].positions[0]));
  failWith('diagram-edit-transaction', duplicate, 'duplicate-id', { baseBundle: bundle });
  const missing = clone(transaction);
  missing.operations[0].positions[0].elementId = 'other-node';
  failWith('diagram-edit-transaction', missing, 'position-coverage', { baseBundle: bundle });
  for (const semanticIndex of [-1, 0.5, 10001]) {
    const invalid = clone(transaction);
    invalid.operations[0].positions[0].semanticIndex = semanticIndex;
    assert.ok(
      validate('diagram-edit-transaction', invalid, { baseBundle: bundle }).length,
...
test('emphasis insertion positions preserve order and cannot be attached to updates or removals', () => {
  ...
  for (const before of ['primary', 'secondary']) {
    const invalid = clone(transaction);
    invalid.operations[0].before = before;
    failWith('diagram-edit-transaction', invalid, 'insertion-position', { baseBundle: bundle });
  }
  const removal = clone(transaction);
  removal.operations[0].after = null;
  failWith('diagram-edit-transaction', removal, 'insertion-position', { baseBundle: bundle });
  const outOfBounds = clone(transaction);
  outOfBounds.operations[0].index = 1025;
...
test('source correspondence updates describe exact before and after maps without touching retained source bytes', () => {
  const bundle = makeBundle('flowchart', { source: true });
  const encoded = JSON.stringify(bundle);
  const after = clone(bundle.sourceMap);
  after.entries.splice(0, 1);
  const transaction = {
    ...makeTransaction(bundle),
    operations: [
      {
        ...
        before: clone(bundle.sourceMap),
        after,
      },
    ],
  };
  assert.deepEqual(validate('diagram-edit-transaction', transaction, { baseBundle: bundle }), []);
  const arbitrary = clone(transaction);
  arbitrary.operations[0].after.originalSource = { text: 'replacement' };
  assert.ok(validate('diagram-edit-transaction', arbitrary, { baseBundle: bundle }).length);
  const elementId = clone(transaction);
  elementId.operations[0].elementId = 'node-a';
...
test('proposal, report and publication contracts bind exact bases without granting portable authority', () => {
  ...
  const substituted = clone(proposal);
  substituted.transaction.base.presentationDigest = `sha256:${'a'.repeat(64)}`;
  ...
  const misleading = clone(report);
  misleading.presentation = 'lossless';
  failWith('diagram-fidelity-report', misleading, 'fidelity', { bundle });
  const publication = {
    kind: 'diagram-publication-state',
    schemaVersion: '1.0.0',
    protocolVersion: '1.13.0',
    diagramId: bundle.diagramId,
    draft: snapshot(bundle),
    published: null,
    audience: 'private',
  ...
  for (const [key, value] of Object.entries({
    ...
    roles: ['author'],
    credentials: { token: 'inert-test-token' },
    comments: ['private review'],
    camera: { x: 0 },
    selection: ['node-a'],
    publication,
  })) {
    const polluted = clone(bundle);
    polluted[key] = value;
    rehashBundle(polluted);
    failWith('diagram-authoring-bundle', polluted, 'additionalProperties');
  }
  assert.ok(
    validate('diagram-authoring-bundle', bundle, { trustDigest: true }).length,
...
test('capability certification separates renderer membership, authoring and interchange fidelity', () => {
  ...
  for (const direction of ['forward', 'both', 'none']) {
    const bundle = makeBundle();
    bundle.document.relations[0].direction = direction;
    sealBundle(bundle);
    assert.deepEqual(validateDiagramAuthoringBundle(bundle), []);
  }
  const misleading = clone(DIAGRAM_AUTHORING_CAPABILITIES);
  misleading.profiles[0].mermaid.constructs.find(
    (item) => item.construct === 'manual-geometry',
  ).semanticRoundTrip = 'lossless';
  failWith('diagram-authoring-capabilities', misleading, 'capability-certification');
  const unknown = clone(DIAGRAM_AUTHORING_CAPABILITIES);
  unknown.profiles[0].mermaid.constructs.push({
    construct: 'unknown-syntax',
    import: 'lossless',
    export: 'lossless',
    semanticRoundTrip: 'lossless',
    mapping: 'Uncertified claim',
  });
...
test('legacy inspection preserves all grammar bytes and IDs without silently migrating unsupported kinds', () => {
  for (const grammar of DIAGRAM_GRAMMAR_REGISTRY.grammars) {
    ...
    if (!getDiagramAuthoringCapability(grammar.grammarId) || report.unsupportedNodeKinds.length)
      assert.equal(report.authoringAvailable, false);
    assert.equal(
      Object.hasOwn(report, 'document'),
      false,
      'Inspection does not fabricate an adopted successor',
    );
  }
});
...
test('legacy inspection obeys the frozen schema for path, version and text boundaries', () => {
  const fixture = JSON.parse(
    readFileSync(
      join(root, 'packages/artifact/fixtures/diagram/grammars/flowchart.planr-diagram.json'),
      'utf8',
    ),
  );
  const cases = [
    [
      ...
      (doc) => {
        doc.source.path = 'diagrams/my flow.mmd';
      },
      true,
    ],
    [
    ...
    [
      ...
      (doc) => {
        doc.documentVersion = '01.0.0';
      },
      false,
    ],
    [
      'text beyond frozen limit',
      (doc) => {
        doc.title = 'x'.repeat(16385);
      },
      false,
    ],
  ];
  for (const [name, edit, expected] of cases) {
    ...
    edit(document);
    document.documentDigest = diagramDocumentDigest(document);
    const before = JSON.stringify(document);
    const frozen = validateProtocolArtifact('diagram-document', document, {
...
test('UTF-8 source and aggregate data limits reject excessive inputs before any adoption', () => {
  ...
  rehashBundle(source);
  assert.ok(
    validateDiagramAuthoringBundle(source).length,
    'UTF-8 bytes, not UTF-16 length, bound original source',
  );
  const enormous = makeBundle();
  enormous.extra = 'x'.repeat(8_388_609);
