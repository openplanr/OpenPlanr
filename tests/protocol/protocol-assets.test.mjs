...
test('all 180 schemas and 12 registries preserve exact source bytes and modes', () => {
  const additiveVersions = new Set([
    'v1.5.0',
    'v1.6.0',
    'v1.7.0',
    'v1.8.0',
...
test('Protocol 1.5 owns thirteen additive schemas with exact identities', () => {
  const files = walk(join(protocol, 'schemas', 'v1.5.0')).filter(({ key }) =>
    key.endsWith('.json'),
  );
...
test('all canonical registries satisfy schemas and their RFC 8785 digest', () => {
  const pairs = [
    ['roles.json', 'role-registry'],
    ['task-kinds.json', 'task-kind-registry'],
    ['rules.json', 'rule-catalog'],
    ['commands.json', 'command-catalog'],
    ['skills.json', 'skill-catalog'],
    ['outputs.json', 'output-catalog'],
  ...
  for (const [file, kind] of pairs) {
    ...
    assert.ok(
      validateProtocolArtifact(kind, invalid, { protocolVersion: '1.5.0' }).some(
...
test('generation check is deterministic and package exports are explicit', () => {
  ...
  for (const path of [
    ...
    'src/json-schema.mjs',
    'src/registries.mjs',
    'src/browser-contracts.mjs',
  ]) {
    assert.doesNotMatch(
      readFileSync(join(protocol, path), 'utf8'),
      /from ['"]node:/,
...
test('the Protocol root exposes v1.6 contracts while the Node validator subpath stays narrow', async () => {
  ...
  assert.equal(
    JSON.parse(readFileSync(skillSourceUrl, 'utf8'))['x-openplanr-contract'].version,
    '1.6.0',
  );
  const handoffUrl = rootModule.protocolAssetUrl('design-handoff-readiness', {
    protocolVersion: '1.11.0',
  });
  assert.equal(
    JSON.parse(readFileSync(handoffUrl, 'utf8'))['x-openplanr-contract'].version,
    '1.11.0',
  );
  ...
  assert.throws(
    () => rootModule.protocolAssetUrl('constructor', { protocolVersion: '1.13.0' }),
    RangeError,
  );
  assert.throws(
    () => rootModule.protocolAssetUrl('diagram-document', { protocolVersion: 'constructor' }),
    RangeError,
  );
  const authoringUrl = rootModule.protocolAssetUrl('diagram-authoring-bundle', {
    protocolVersion: '1.13.0',
  });
  assert.equal(
    JSON.parse(readFileSync(authoringUrl, 'utf8'))['x-openplanr-contract'].version,
    '1.13.0',
  );
  assert.equal(Object.hasOwn(nodeContracts, 'PROTOCOL_V16_CONTRACTS'), false);
  assert.doesNotMatch(
    readFileSync(join(protocol, 'src', 'contracts.d.mts'), 'utf8'),
    /browser-contracts/u,
  );
