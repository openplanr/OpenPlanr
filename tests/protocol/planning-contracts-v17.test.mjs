...
test('Protocol 1.7 publishes the planning and authority contracts without replacing v1.0 readers', () => {
  ...
  assert.match(resolveProtocolSchema('story', { protocolVersion: '1.0.0' }).path, /schemas\/v1\.0\.0\/story/u);
  assert.match(resolveProtocolSchema('story', { protocolVersion: '1.7.0' }).path, /schemas\/v1\.7\.0\/story/u);
});

test('Protocol 1.7 validates global IDs, stable acceptance IDs, metadata, and request scope', () => {
  const spec = {
    id: 'SPEC-1000', title: 'Global planning IDs', slug: 'global-planning-ids',
    schemaVersion: '1.7.0', status: 'decomposed', priority: 'P1',
    created: DATE, updated: DATE, ui_files: [], tech_dependencies: [],
  };
  const sequence = {
    kind: 'planning-id-sequence', schemaVersion: '1.0.0', protocolVersion: '1.7.0',
    next: { SPEC: 1001, US: 1001, T: 1001 },
  };
  const authority = {
    kind: 'request-authority', schemaVersion: '1.0.0', protocolVersion: '1.7.0',
  ...
  for (const [kind, value] of [['spec', spec], ['story', story], ['task', taskFrontmatter], ['planning-id-sequence', sequence], ['request-authority', authority]]) {
...
test('legacy snake-case metadata translates to canonical task fields', () => {
  assert.deepEqual(
    normalizePlanningTask({
      id: 'T-001', review_risks: ['security'], browser_surfaces: ['authentication'],
      acceptance_refs: ['AC-001'],
    }),
    {
      id: 'T-001', reviewRisks: ['security'], browserSurfaces: ['authentication'],
      acceptanceRefs: ['AC-001'],
    },
  );
  assert.deepEqual(normalizePlanningTask({ id: 'T-002' }), {
    id: 'T-002', reviewRisks: [], browserSurfaces: [], acceptanceRefs: [],
  });
});
...
test('acceptance coverage requires both task mapping and a verification statement', () => {
  ...
  assert.equal(validatePlanningAcceptanceCoverage([story], [{ ...task, acceptanceRefs: [] }])[0].rule, 'coverage');
  assert.equal(validatePlanningAcceptanceCoverage([story], [{ ...task, testRequirements: 'Run tests.' }])[0].rule, 'verification');
  assert.equal(validatePlanningAcceptanceCoverage([story], [{ ...task, acceptanceRefs: ['AC-999'] }]).length, 2);
});

test('request-scope authority may narrow but never widen', () => {
  const base = {
    repositoryAccess: 'request-scope', externalDataAccess: 'none',
    allowedCapabilities: ['context-gathering', 'local-execution', 'read', 'write'],
    allowedTools: ['read', 'edit', 'shell'], allowedOperations: ['compile', 'render'],
    allowedOutputClasses: ['A', 'B', 'C'], forbiddenEffects: ['external-publish'],
  };
  const edge = { from: 'ship', to: 'host' };
  assert.doesNotThrow(() => assertAuthorityNarrows(base, { ...base, repositoryAccess: 'declared-paths' }, { edge }));
  assert.throws(
