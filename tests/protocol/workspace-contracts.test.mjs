...
test('workspace schemas are additive generated Protocol contracts', () => {
  for (const [name, schema] of Object.entries(DESIGN_WORKSPACE_SCHEMAS)) {
    assert.deepEqual(
      JSON.parse(
        readFileSync(
          new URL(`../../packages/protocol/schemas/v1.9.0/${name}.schema.json`, import.meta.url),
        ),
      ),
      schema,
    );
...
test('persistent workspaces cannot leak auth capabilities or require expiry', () => {
  const metadata = {
    ...
    version: 1,
    epoch: 1,
    currentRevision: 'b'.repeat(24),
    commentsPaused: false,
    ownerPublicKey: { kty: 'EC', crv: 'P-256', x: 'a'.repeat(43), y: 'b'.repeat(43) },
    keyring: { iv: 'a'.repeat(16), ciphertext: 'a'.repeat(32) },
  };
  ...
  for (const field of ['token', 'ownerAuth', 'ownerPrivateKey', 'reviewerAuthHash', 'expiresAt']) {
    assert.throws(
      () => assertWorkspaceContract({ ...metadata, [field]: 'secret' }, DESIGN_WORKSPACE_SCHEMA),
      /Invalid/,
    );
  }
  assert.throws(
    () => assertWorkspaceContract({ ...metadata, epoch: 0 }, DESIGN_WORKSPACE_SCHEMA),
    /Invalid/,
  );
});
