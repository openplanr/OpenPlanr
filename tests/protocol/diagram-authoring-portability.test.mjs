...
test('authoring contracts return the same located diagnostics in Node, Chromium and a real Worker isolate', {
  timeout: 90_000,
}, async () => {
  const expected = evaluateAuthoringCases();
  assert.ok(
    expected.some((value) => value.valid),
    'The portability table must exercise valid content',
  );
  assert.ok(
    expected.some((value) => !value.valid),
    'The portability table must exercise rejected content',
  );
  for (const value of expected) {
    ...
    assert.equal(
      value.valid,
      value.expectedValid,
      `${value.name}: ${JSON.stringify(value.diagnostics)}`,
    );
    if (value.expectedRule)
      assert.ok(
        value.diagnostics.some((diagnostic) => diagnostic.rule === value.expectedRule),
        value.name,
      );
  ...
  try {
    ...
    await page.setContent(
      '<!doctype html><html lang="en"><title>Diagram contract portability</title><body></body></html>',
    );
    ...
    assert.deepEqual(
      actual.cases,
      expected,
      'Chromium must validate the same fixtures with identical located diagnostics',
    );
  } finally {
    await browser.close();
  }
  ...
  try {
    ...
    assert.deepEqual(
      actual.cases,
      expected,
      'Worker isolate must validate the same fixtures with identical located diagnostics',
    );
  } finally {
