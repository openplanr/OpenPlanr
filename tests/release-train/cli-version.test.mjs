...
import {
  ...
  cliReleaseVersion,
  releaseWeek,
} from '../../scripts/release-train/lib/cli-version.mjs';
...
test('release weeks follow ISO week-years across year boundaries', () => {
  ...
  assert.equal(
    releaseWeek(new Date('2026-09-27T23:59:59Z')),
    2639,
    'Sunday night UTC is still week 39',
  );
});
...
test('the first release of a week takes patch 0 and later ones count up', () => {
  ...
  assert.equal(
    cliReleaseVersion({ current: '2.7.0', published, date: on('2026-09-24') }),
    '2.2639.0',
  );
  assert.equal(
    cliReleaseVersion({
      current: '2.2639.1',
      published: [...published, '2.2639.0'],
      date: on('2026-09-25'),
    }),
    '2.2639.1',
  );
  assert.equal(
    cliReleaseVersion({
      current: '2.2640.0',
      published: [...published, '2.2639.0', '2.2639.1'],
      date: on('2026-09-26'),
    }),
    '2.2639.2',
...
test('a published version needs no renumbering and prereleases do not count', () => {
  assert.equal(
    cliReleaseVersion({ current: '2.6.3', published: ['2.6.3'], date: on('2026-09-24') }),
    null,
  );
  assert.equal(
    cliReleaseVersion({
      current: '2.7.0',
      published: ['2.6.3', '2.2639.0-beta.1'],
      date: on('2026-09-24'),
    }),
    '2.2639.0',
  );
...
test('the manifest version and the changelog heading are rewritten together', () => {
  ...
  try {
    ...
    assert.throws(
      () => applyCliVersion({ root, from: '2.7.0', to: '2.2639.1' }),
      /does not declare version 2\.7\.0 exactly once/u,
    );
    ...
    assert.throws(
      () => applyCliVersion({ root, from: '2.2639.1', to: '2.2639.0' }),
      /already has an unpublished 2\.2639\.0 section/u,
    );
  } finally {
