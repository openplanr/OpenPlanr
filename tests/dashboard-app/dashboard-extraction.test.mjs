...
test('dashboard app is a private 0.1 workspace with protocol-only internal dependency', () => {
  const manifest = JSON.parse(readFileSync(resolve(workspaceRoot, 'apps/dashboard/package.json'), 'utf8'));
...
test('built app and CLI copy retain exact digest custody', () => {
  assert.equal(existsSync(resolve(workspaceRoot, 'apps/dashboard/dist/dashboard-manifest.json')), true);
  assert.equal(existsSync(resolve(workspaceRoot, 'packages/cli/dist/dashboard/dashboard-manifest.json')), true);
...
test('path escape and symbolic-link asset inputs fail closed', { skip: process.platform === 'win32' }, () => {
  ...
  assert.equal(assertDashboardPathWithin(root, join(root, 'assets', 'ok.js')), join('assets', 'ok.js'));
  assert.throws(() => assertDashboardPathWithin(root, resolve(root, '..', 'escape.js')), /escapes custody root/u);
