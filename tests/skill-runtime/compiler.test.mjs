...
import { verifyDocumentDigest, withDocumentDigest } from '../../packages/protocol/src/canonical-json.mjs';
import {
  SkillRuntimeError,
...
import { readFrontmatter, renderFrontmatterBlock } from '../../packages/skill-runtime/src/compiler/frontmatter.mjs';
...
const readFixture = (relative) => readFileSync(join(composedDir, relative), 'utf8').replace(/\r\n/gu, '\n');
const FIXTURE_SOURCE_CUSTODY = Object.freeze({ path: 'manifest.json', digest: sha256(readFixture('manifest.json')) });
...
function loadComposedFixture() {
  ...
  const sourceByModule = new Map(manifest.modules.map((module) => [`${module.moduleId}@${module.moduleVersion}`, module.source]));
  const moduleRef = ({ moduleId, moduleVersion }) => ({ moduleId, moduleVersion, digest: digestOf(sourceByModule.get(`${moduleId}@${moduleVersion}`)) });
  ...
  const hostProfilesById = new Map(manifest.hostProfiles.map((profile) => [profile.hostProfileId, {
  ...
  const skillSource = withDocumentDigest({
    ...
    references: manifest.skill.references.map((reference) => ({ module: moduleRef(reference), routed: true })),
...
test('composed-v1 fixture documents validate against the Protocol 1.6 contracts', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('skill-source', skillSource, { protocolVersion: '1.6.0' }), []);
  assert.deepEqual(validateProtocolArtifact('skill-module-registry', moduleRegistry, { protocolVersion: '1.6.0' }), []);
  assert.deepEqual(validateProtocolArtifact('skill-host-profile-registry', hostProfileRegistry, { protocolVersion: '1.6.0' }), []);
...
test('a graph cycle fails with a typed diagnostic naming the source edge and repair action', () => {
  const { skillSource, modules } = loadComposedFixture();
  const lifecycleDigest = modules.find((module) => module.moduleId === 'lifecycle-core').source.digest;
  const cyclic = modules.map((module) => (
    module.moduleId === 'shared-intro'
      ? { ...module, dependsOn: [{ moduleId: 'lifecycle-core', moduleVersion: '1.0.0', digest: lifecycleDigest }] }
      : module
  ));
...
test('floating, ranged, and latest versions are rejected as typed source-edge diagnostics', () => {
  ...
  for (const bad of ['latest', '^1.0.0', '1.x', '>=1.0.0']) {
    const floated = { ...skillSource, modules: [{ moduleId: 'shared-intro', moduleVersion: bad, digest: 'sha256:' + 'a'.repeat(64) }] };
    ...
    assert.equal(error.details.edge.from, `skill:${skillSource.skillId}@${skillSource.skillVersion}`);
...
test('a missing pinned module version fails naming the exact edge and repair action', () => {
  const { skillSource, modules } = loadComposedFixture();
  const missing = { ...skillSource, modules: [{ moduleId: 'shared-intro', moduleVersion: '9.9.9', digest: 'sha256:' + 'b'.repeat(64) }] };
...
test('authority monotonicity accepts legal narrowing and rejects each widened component', () => {
  ...
  const narrowed = {
    repositoryAccess: 'none', externalDataAccess: 'none',
...
test('a widening host overlay fails composed generation before any byte is produced', () => {
  ...
  const selectedSkill = { ...skillSource, hostProfiles: [...skillSource.hostProfiles, { id: 'demo-widen', version: '1.0.0' }] };
  const error = caught(() => compileComposedV1({ skillSource: selectedSkill, skillSourceCustody: FIXTURE_SOURCE_CUSTODY, modules, hostProfile: hostProfilesById.get('demo-widen'), readSource: readFixture }));
  assert.ok(error instanceof SkillRuntimeError);
...
test('composed generation renders inline modules, keeps routed references separate, and previews both groups', () => {
  const { skillSource, modules, hostProfilesById } = loadComposedFixture();
  const result = compileComposedV1({ skillSource, skillSourceCustody: FIXTURE_SOURCE_CUSTODY, modules, hostProfile: hostProfilesById.get('demo-claude-code'), readSource: readFixture });
  ...
  assert.deepEqual(result.preview.inline.map(({ moduleId, moduleVersion }) => `${moduleId}@${moduleVersion}`), ['shared-intro@1.0.0', 'lifecycle-core@1.0.0']);
  assert.deepEqual(result.preview.routed.map(({ moduleId, moduleVersion }) => `${moduleId}@${moduleVersion}`), ['deep-reference@1.0.0']);
});
...
test('additive host overlay modules cannot alter workflow, outputs, capabilities, or effects', () => {
  ...
  for (const [semantic, body] of Object.entries(attempts)) {
    ...
    const profile = {
      ...hostProfile,
      overlayModules: [{ moduleId: overlay.moduleId, moduleVersion: overlay.moduleVersion, digest: overlay.source.digest }],
    };
    ...
    assert.deepEqual(error.details.policy.protectedSemantics, ['workflow', 'outputs', 'capabilities', 'effects']);
...
test('a validated presentation substitution changes only its owned output range', () => {
  ...
  const source = {
    ...skillSource,
    modules: [...skillSource.modules, { moduleId: base.moduleId, moduleVersion: base.moduleVersion, digest: base.source.digest }],
  };
  ...
  const withOverlay = compileComposedV1({
    ...
    hostProfile: {
      ...hostProfile,
      overlayModules: [{ moduleId: presentation.moduleId, moduleVersion: presentation.moduleVersion, digest: presentation.source.digest }],
    },
    readSource,
  });
  ...
  const owned = withOverlay.primary.sourceMap.find(({ owner: rangeOwner }) => (
    rangeOwner.pointer === 'modules/claude-presentation-range.json#/substitutions/AGENTS_ROOT'
  ));
  assert.ok(owned);
  assert.equal(
    Buffer.from(withOverlay.primary.bytes).subarray(owned.startByte, owned.endByte).toString('utf8'),
    '`agents`',
  );
...
test('compiler-owned host substitutions vary invocation syntax without changing the module graph', () => {
  ...
  assert.deepEqual(claude.preview.routed.map(({ moduleId }) => moduleId), codex.preview.routed.map(({ moduleId }) => moduleId));
...
test('routed assets compose dependencies and recursively expose module references', () => {
  ...
  const expandedReference = {
    ...reference,
    dependsOn: [{ moduleId: dependency.moduleId, moduleVersion: dependency.moduleVersion, digest: dependency.source.digest }],
    references: [{ moduleId: nested.moduleId, moduleVersion: nested.moduleVersion, digest: nested.source.digest }],
  };
  const extraSources = new Map([[dependency.source.path, dependencyBytes], [nested.source.path, nestedBytes]]);
  const result = compileComposedV1({
    skillSource,
    skillSourceCustody: FIXTURE_SOURCE_CUSTODY,
    modules: modules.map((module) => (module.moduleId === reference.moduleId ? expandedReference : module)).concat(dependency, nested),
    hostProfile: hostProfilesById.get('demo-claude-code'),
  ...
  assert.equal(result.references.some(({ moduleId }) => moduleId === 'nested-reference'), true);
  assert.equal(result.primary.bytes.includes('# Reference dependency'), false);
  assert.deepEqual(
    result.preview.routed.find(({ moduleId }) => moduleId === 'deep-reference').sources.map(({ moduleId }) => moduleId),
...
test('composed generation is byte-idempotent and covers every output byte exactly once across three host profiles', () => {
  ...
  for (const profileId of ['demo-claude-code', 'demo-codex', 'demo-cursor']) {
    ...
    const first = compileComposedV1({ skillSource: fixture.skillSource, skillSourceCustody: FIXTURE_SOURCE_CUSTODY, modules: fixture.modules, hostProfile: profile, cursorTemplate: CURSOR_TEMPLATE, readSource: readFixture });
    const second = compileComposedV1({ skillSource: fixture.skillSource, skillSourceCustody: FIXTURE_SOURCE_CUSTODY, modules: fixture.modules, hostProfile: profile, cursorTemplate: CURSOR_TEMPLATE, readSource: readFixture });
    ...
    assert.doesNotThrow(() => validateSourceMap(first.primary.sourceMap, first.primary.byteLength), profileId);
    assert.equal(first.primary.sourceMap[0].startByte, 0);
    assert.equal(first.primary.sourceMap[first.primary.sourceMap.length - 1].endByte, first.primary.byteLength);
    for (const reference of first.references) {
      assert.doesNotThrow(() => validateSourceMap(reference.sourceMap, reference.byteLength), `${profileId} ${reference.path}`);
    }
...
test('composed Codex and Cursor projections use exact host-native paths, frontmatter, and source maps', () => {
  const fixture = loadComposedFixture();
  const compile = (profileId) => compileComposedV1({
  ...
  assert.ok(codex.primary.bytes.startsWith(
  ...
  assert.ok(cursor.primary.bytes.startsWith(
    '---\ndescription: "Composed demo skill fixture."\nalwaysApply: false\n---\n\n',
  ));
  ...
  for (const result of [codex, cursor]) {
    assert.doesNotThrow(() => validateSourceMap(result.primary.sourceMap, result.primary.byteLength));
...
test('composed Cursor descriptions always round-trip as YAML strings with exact source-map coverage', () => {
  ...
  for (const { name, description } of cases) {
    ...
    const descriptionLine = result.primary.bytes.split('\n').find((line) => line.startsWith('description: '));
    ...
    assert.doesNotThrow(() => validateSourceMap(result.primary.sourceMap, result.primary.byteLength), name);
    assert.equal(
      result.primary.sourceMap.some((range) => range.owner.pointer.endsWith('#/serializeYamlScalar')),
      true,
      name,
    );
  }
...
test('composed generation binds a valid Protocol 1.6 generated-asset-manifest and deterministic custody', () => {
  ...
  const result = compileComposedV1({ skillSource: fixture.skillSource, skillSourceCustody: FIXTURE_SOURCE_CUSTODY, modules: fixture.modules, hostProfile: profile, readSource: readFixture });
  const assets = [
    { path: result.primary.path, host: result.host, byteLength: result.primary.byteLength, digest: result.primary.digest, sourceMap: result.primary.sourceMap },
    ...result.references.map((reference) => ({ path: reference.path, host: result.host, byteLength: reference.byteLength, digest: reference.digest, sourceMap: reference.sourceMap })),
  ];
  ...
  const custody = buildCustodyManifest({ assets, sourceFormat: 'composed-v1', assetSetId: manifest.assetSetId });
  assert.equal(custody.custodyDigest, buildCustodyManifest({ assets, sourceFormat: 'composed-v1', assetSetId: manifest.assetSetId }).custodyDigest);
...
test('markdown-v1 compatibility remains deterministic for legacy source bytes', () => {
  ...
  for (const host of ['claude-code', 'codex', 'cursor', 'pipeline']) {
    const input = {
      ...
      cursorTemplate,
      supportPaths: [],
      readSource: () => { throw new Error('unexpected source read'); },
      sourcePath,
    };
    ...
    assert.doesNotThrow(() => validateSourceMap(compiled.sourceMap, compiled.byteLength), `${host} source map`);
  }
});

test('frontmatter handling matches the markdown-v1 catalog shape and renders closed blocks', () => {
  const parsed = readFrontmatter(read('skills/planr-status/SKILL.md'), { expectedName: 'planr-status' });
  assert.equal(parsed.fields.name, 'planr-status');
  assert.ok(parsed.body.startsWith('# Planr Status'));
  const block = renderFrontmatterBlock([['name', 'planr-demo-compose'], ['description', 'Composed demo skill fixture.']]);
  assert.equal(block, '---\nname: planr-demo-compose\ndescription: Composed demo skill fixture.\n---\n');
...
test('the frozen 138-projection inventory stays migration-linked and self-consistent', async () => {
  const { canonicalizeJson, sha256Hex } = await import('../../packages/protocol/src/canonical-json.mjs');
  const inventory = JSON.parse(read('packages/skill-runtime/fixtures/markdown-v1/projection-inventory.json'));
