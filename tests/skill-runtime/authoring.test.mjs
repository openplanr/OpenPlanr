...
function startLockContender({ skillDir, role, signalsDir, pauseBeforeClaim = false }) {
  ...
  const child = spawn(
    process.execPath,
    ['--input-type=module', '--eval', source, skillDir, role, signalsDir, String(pauseBeforeClaim)],
...
test('the minimal composed-v1 example lints, previews, checks, and evaluates cleanly', () => {
  ...
  assert.deepEqual(lint.hostProfiles, [
    { id: 'minimal-claude-code', version: '1.0.0', host: 'claude-code' },
  ]);
  ...
  assert.deepEqual(
    host.inline.map((module) => `${module.moduleId}@${module.moduleVersion}`),
    ['hello-intro@1.0.0'],
  );
  assert.deepEqual(
    host.routed.map((reference) => reference.path),
    ['skills/planr-hello/references/hello-reference.md'],
  );
  ...
  assert.deepEqual(evaluate.hosts[0].checks, {
    idempotent: true,
...
test('generate materializes one complete deterministic host output set', () => {
  ...
  try {
    ...
    assert.deepEqual(
      first.assets.map(({ outputPath }) => outputPath),
      [
    ...
    const manifest = JSON.parse(
      readFileSync(join(outputRoot, 'manifests', 'generated-assets.json'), 'utf8'),
    );
    const custody = JSON.parse(
      readFileSync(join(outputRoot, 'manifests', 'generated-custody.json'), 'utf8'),
    );
    assert.deepEqual(
...
test('generate materializes exact Codex and Cursor host-native shapes and paths', () => {
  ...
  try {
    ...
    assert.deepEqual(
      result.assets.map(({ outputPath }) => outputPath),
      [
        'claude-code/skills/planr-hello/SKILL.md',
        'claude-code/skills/planr-hello/references/hello-reference.md',
    ...
    const codex = readFileSync(
      join(dir, 'dist', 'codex', 'skills', 'planr-hello', 'SKILL.md'),
      'utf8',
    );
    assert.ok(
      codex.startsWith(
    ...
    const manifest = JSON.parse(
      readFileSync(join(dir, 'dist', 'manifests', 'generated-assets.json'), 'utf8'),
    );
    assert.deepEqual(
      validateProtocolArtifact('generated-asset-manifest', manifest, { protocolVersion: '1.6.0' }),
      [],
...
test('generate validates completely before replacing a prior output set', () => {
  ...
  try {
    ...
    writeFileSync(
      templatePath,
      readFileSync(templatePath, 'utf8').replace('{{MODULES}}', '{{UNKNOWN}}\n\n{{MODULES}}'),
    );
...
test('concurrent composed generations serialize and converge on one complete output', async () => {
  ...
  try {
    const runs = await Promise.all(
      Array.from({ length: 8 }, () => runNode([script, dir, '--json'])),
    );
    ...
    const outputRoot = join(dir, 'dist');
    assert.deepEqual(
      snapshotTree(outputRoot).map(({ path }) => path),
      [
        'claude-code/skills/planr-hello/references/hello-reference.md',
        'claude-code/skills/planr-hello/SKILL.md',
        'manifests/generated-assets.json',
    ...
    assert.equal(
      readdirSync(dir).some(
        (name) => name.startsWith('.openplanr-dist-') || name.startsWith('.openplanr-backup-'),
      ),
      false,
    );
  } finally {
...
test('competing stale reclaimers never evict a freshly acquired fixed lock', {
  timeout: 15_000,
}, async () => {
  const dir = tempSkill();
...
test('the composed generate CLI has stable human, JSON, and usage contracts', () => {
  ...
  try {
    ...
    const json = JSON.parse(
      execFileSync(process.execPath, [script, dir, '--json'], { cwd: root, encoding: 'utf8' }),
    );
...
test('the legacy composed preview names the exact compiler-owned emitted ranges', () => {
  const output = execFileSync(
    process.execPath,
    [join(root, 'scripts', 'skills', 'preview.mjs'), example],
    { cwd: root, encoding: 'utf8' },
  );
  const ownerLines = output
    .split('\n')
    .map((line) => line.trim())
...
test('authoring operations are mutation-free', () => {
  const before = walk('examples/skills/minimal-composed').map((path) => ({
    path,
  ...
  const after = walk('examples/skills/minimal-composed').map((path) => ({
    path,
    size: statSync(path).size,
    digest: createHash('sha256').update(readFileSync(path)).digest('hex'),
  }));
...
test('an unresolved template token fails check with a typed, owning diagnostic', () => {
  ...
  try {
    ...
    writeFileSync(
      templatePath,
      `${readFileSync(templatePath, 'utf8').replace('{{MODULES}}', '{{UNRESOLVED}}\n\n{{MODULES}}')}`,
    );
...
test('runtime entry points resolve and every export is covered by a checked declaration', async () => {
  ...
  for (const [modulePath, declarationPath] of subpaths) {
    ...
    for (const match of declaration.matchAll(
      /export\s+declare\s+(?:function|class|const)\s+([A-Za-z0-9_]+)/gu,
    ))
    ...
    assert.ok(
      ...
      `${declarationPath} must be a non-empty declaration file`,
    );
  }
});
...
test('prompt purity scan: no generated skill or composed asset embeds deterministic algorithm prose', () => {
  ...
  const composed = compileComposedV1({
    skillSource,
    skillSourceCustody,
    modules,
  ...
  const subjects = [
    ...generated.map((path) => readFileSync(path, 'utf8')),
    composed.primary.bytes,
    ...composed.references.map((reference) => reference.bytes),
  ];
...
test('the optional completion-record contract is not an active workflow dependency', () => {
  const activeFiles = [
    ...
    ...walk('packages/pipeline/lib').filter(
      (path) => !path.includes('/lib/protocol/') && !path.includes('/lib/generated/'),
    ),
  ];
...
test('host profiles retain distinct exact versions and declared versions resolve without overwrite ambiguity', () => {
  ...
  try {
    ...
    const loaded = loadComposedSkill({ skillDir: dir });
    assert.deepEqual(
      [...loaded.hostProfilesByKey.keys()],
      ['minimal-claude-code@1.0.0', 'minimal-claude-code@2.0.0'],
    );
...
test('authoring preserves optional host capability and interaction bindings', () => {
  ...
  try {
    ...
    assert.deepEqual(
      profileValue.interactionBindings.map(({ surface }) => surface),
      ['native', 'chat', 'headless'],
    );
    assert.deepEqual(
      validateProtocolArtifact('skill-host-profile-registry', loaded.hostProfileRegistry, {
