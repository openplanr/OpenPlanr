...
function fixture(
  ...
  const document = {
    ...
    screenOrder: Array.from({ length: screens }, (_, index) => `screen-${index + 1}`),
  ...
  const specPath =
    mode === 'default' ? join(dirname(design), 'design-spec.md') : join(design, 'design-spec.md');
...
async function open(script, directory, file) {
  const child = spawn(process.execPath, [script, 'open', file, '--no-open', '--json'], {
    cwd: directory,
    env: environment(directory),
...
test('standalone and suite design skills render authored journeys with complete local resources', async () => {
  const temporary = realpathSync(mkdtempSync(join(tmpdir(), 'openplanr-design-install-')));
  try {
    for (const [index, skillId] of DESIGN_SKILL_IDS.entries()) {
      const directory = join(temporary, skillId);
      mkdirSync(directory);
      const script = install(directory, skillId, index === 1 ? 'claude' : 'openai', {
        suite: index === 1,
      });
      const skillRoot = resolve(dirname(script), '..');
      assert.match(
        readFileSync(join(skillRoot, 'SKILL.md'), 'utf8'),
        /review-context\.json/u,
        'installed entrypoint makes welcome authoring explicit',
      );
      const reviewGuidance = readFileSync(join(skillRoot, 'references/team-review.md'), 'utf8');
      assert.equal(
        reviewGuidance,
        readFileSync(join(root, 'packages/design/references/team-review.md'), 'utf8'),
        'installed company-review guidance matches its canonical source',
      );
      ...
      const brief = [
        {
          purpose: 'Review crew assignment before scoping the dispatch flow.',
          requests: ['Is the assigned crew clear before confirming delivery?'],
        },
        {
          purpose: 'Compare the three authored dispatch directions.',
          requests: [
            'Which direction makes crew assignment easiest to follow?',
            'Where does confirming delivery need more context?',
          ],
        },
        {
          purpose: 'Review the revised dispatch action beside its supplied reference.',
          requests: ['Does the revised action explain what will be confirmed?'],
        },
      ][index];
      const context = {
        kind: 'openplanr-design-review-context',
        schemaVersion: '1.0.0',
        designId: source.document.id,
        brief,
        implementation: {
          tokens: [{ name: '--ink', value: '#152637' }],
          components: [],
          responsive: [],
          accessibility: [],
        },
      ...
      assert.deepEqual(Object.keys(rendered.views).sort(), ['canvas', 'prototype', 'walkthrough']);
      const record = json(join(dirname(rendered.artifact), 'render.json'));
      assert.deepEqual(
        record.reviewContext,
        context,
        "the installed renderer retains this design's authored welcome and implementation guidance",
      );
      assert.equal(
      ...
      for (const view of ['canvas', 'prototype', 'walkthrough']) {
        ...
        assert.deepEqual(
          payload.reviewContext.brief,
          brief,
          'every view uses the same authored welcome',
        );
        assert.ok(
        ...
        assert.ok(
          html.includes('data-design-notes') &&
            html.includes('Implementation context stays outside the product UI.'),
        );
        assert.ok(html.includes('data:text/javascript;base64,'), 'portable runtime is embedded');
      }
      assert.ok(existsSync(rendered.manifest));
      assert.equal((readFileSync(source.specPath, 'utf8').match(/^## \d+\./gm) ?? []).length, 10);
    }
  } finally {
...
test('installed Artifact skill requires design-specific welcome preparation before sharing', () => {
  for (const host of ['openai', 'claude']) {
    ...
    assert.equal(
      readFileSync(join(installed, 'references/design-sharing.md'), 'utf8'),
      readFileSync(join(root, 'skills/planr-artifact/references/design-sharing.md'), 'utf8'),
    );
  }
});

test('installed review retains pins, selection and arrangement across restart, scoped revision and failed rendering', async () => {
  const directory = realpathSync(mkdtempSync(join(tmpdir(), 'openplanr-design-review-install-')));
  let session;
  try {
    ...
    assert.equal(json(selection.tastePath).designs.fieldwork.selected[0], 'direction-2');
    ...
    writeFileSync(
      target,
      readFileSync(target, 'utf8').replace('Confirm dispatch', 'Assign crew now'),
    );
    const revised = run(script, directory, ['render', source.path]);
    assert.notEqual(revised.revision, rendered.revision);
    assert.deepEqual(readFileSync(join(source.design, 'source/screen-2.html')), untouched);
    ...
    const workingPointer = readFileSync(join(source.design, '.design/current.json'));
    writeFileSync(target, '<img src="missing.png" alt="missing">');
    run(script, directory, ['render', source.path], { success: false });
    assert.deepEqual(readFileSync(join(source.design, '.design/current.json')), workingPointer);
    assert.ok(existsSync(rendered.artifact), 'previous completed revision remains recoverable');
  } finally {
    await session?.close();
...
test('design runtime bundling and release archives preserve binary assets without executable dependencies', async () => {
  ...
  assert.deepEqual(extracted.find(({ path }) => path === 'assets/opaque-font.woff2').bytes, binary);
  for (const entry of resources)
    assert.deepEqual(extracted.find(({ path }) => path === entry.path).bytes, entry.bytes);
  assert.ok(
...
test('isolated Design and Plan installations preserve explicit review handoff approval and freshness', () => {
  ...
  try {
    ...
    const source = fixture(directory);
    write(
      join(source.design, 'review-context.json'),
      JSON.stringify({
        kind: 'openplanr-design-review-context',
        schemaVersion: '1.0.0',
        designId: source.document.id,
        brief: {
          purpose: 'Review the dispatch journey before planning changes.',
          requests: ['Is the primary action clear?'],
        },
        implementation: {
          tokens: [{ name: '--ink', value: '#152637' }],
          components: [],
    ...
    write(
      ...
      JSON.stringify({
        content: {
          ...
          summary:
            'Review the authored dispatch journey and its verification gaps before scoping implementation.',
        },
      }),
    );
    const refined = run(designScript, directory, [
      'handoff',
      source.path,
      '--action',
      'update',
    ...
    const approved = run(designScript, directory, [
      'handoff',
      source.path,
      '--action',
      'approve',
    ...
    assert.deepEqual(
      readFileSync(approved.path),
      previous,
      'freshness never silently rewrites owner approval',
    );
    assert.equal(
      existsSync(join(directory, '.planr/tasks')),
      false,
      'inspecting a handoff never executes Plan or Ship',
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
