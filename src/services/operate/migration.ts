...
import { OperateError, type OperatingEventHead, type OperatingRecordEnvelope } from './types.js';
...
interface RecordsMigrationApi {
  migrateRecordsDirectoryToJsonl(
    legacyRecords: OperatingRecordEnvelope[],
    options?: { migratedAt?: string; eventCount?: number },
  ): { lines: string[]; migrationRecord: Record<string, unknown> };
  reconstructDirectoryLayoutFromJsonl(
    lines: string[],
  ): Map<string, Map<string, OperatingRecordEnvelope>>;
  flattenReconstructedLayout(
    layout: Map<string, Map<string, OperatingRecordEnvelope>>,
  ): OperatingRecordEnvelope[];
}

let cachedTransform: Promise<RecordsMigrationApi> | undefined;
 ...
 */
async function loadRecordsMigration(): Promise<RecordsMigrationApi> {
  const root = resolveOperatingPipelineRoot();
  if (!root) {
    throw new OperateError(
      'E_PIPELINE_NOT_INSTALLED',
      'The v1.3 storage-layout migration requires the installed pipeline package.',
      {
        recovery: 'Run `npm install -g openplanr@latest` (without `--omit=optional`), then re-run.',
      },
    );
  }
  cachedTransform ??= import(
    pathToFileURL(path.join(root, ...MIGRATION_TRANSFORM_MODULE)).href
  ).then((value) => value as unknown as RecordsMigrationApi);
 ...
 */
export async function detectOperatingStorageLayout(
  projectRoot: string,
  options: { localRoot?: string } = {},
): Promise<OperatingStorageLayout> {
  const paths = resolveOperatingPaths(projectRoot, options);
  if ((await fileExists(paths.state)) || (await fileExists(paths.events))) return 'v1.3';
  if (
...
async function readLegacyRecords(operateRoot: string): Promise<OperatingRecordEnvelope[]> {
  ...
  for (const prefix of await readdir(recordsDir).catch(() => [])) {
    ...
    for (const name of await readdir(path.join(recordsDir, prefix)).catch(() => [])) {
      if (!/^[a-f0-9]{62}\.json$/.test(name)) continue;
      const raw = await readFile(path.join(recordsDir, prefix, name), 'utf8');
      records.push(JSON.parse(raw) as OperatingRecordEnvelope);
    }
  }
  return records.sort((left, right) => left.digest.localeCompare(right.digest));
}

async function removeLegacyLayout(operateRoot: string): Promise<void> {
...
export async function applyStorageLayoutMigration(input: {
  ...
  if (layout === 'absent') return { migrated: false, layout };

  const paths = resolveOperatingPaths(input.projectRoot, { localRoot: input.localRoot });
  const store = new OperatingEventStore(input.projectRoot, { localRoot: input.localRoot });
  const head: OperatingEventHead = (await store.replay()).eventHead;
  ...
  const previewDigest = migrationRecord.previewDigest as `sha256:${string}`;
  const migrationId = migrationRecord.id as string;

  const eventsBytes = await readFile(legacyEventsPath(paths.root), 'utf8').catch((error) => {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return '';
    throw error;
  });
  const checkpointBytes = await readFile(legacyCheckpointPath(paths.root), 'utf8').catch(
    (error) => {
  ...
  const writes: JournalWrite[] = [
    ...
    ...(checkpointBytes !== null
      ? [{ relativePath: STATE_CHECKPOINT, content: checkpointBytes, operation: 'create' as const }]
      : []),
  ];
 ...
 */
export async function migrateOperatingStorageLayoutOnOpen(
  projectRoot: string,
  options: { localRoot?: string } = {},
): Promise<OperatingStorageMigrationResult> {
...
export async function inspectStorageLayoutMigration(input: {
  projectRoot: string;
  localRoot?: string;
}): Promise<OperatingStorageMigrationResult & { recordCount: number }> {
  const layout = await detectOperatingStorageLayout(input.projectRoot, {
    localRoot: input.localRoot,
  });
  const recordCount =
