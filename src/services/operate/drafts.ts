...
import { OperateError, type OperatingFinding } from './types.js';
...
export interface OperatingMaterializedDraft {
  ...
  causality: { findingIds: string[]; citationDigests: `sha256:${string}`[] };
  reversible: true;
  userEdited?: boolean;
}
...
interface AdvisorAction {
  ...
  routeKind: OperatingDraftKind | 'experiment' | 'metric';
...
interface DraftApi {
  qualifyOperatingDraftCandidates(
    ...
    eligible: Array<{ action: AdvisorAction; digest: `sha256:${string}` }>;
    rejected: Array<{ action: AdvisorAction; digest: `sha256:${string}`; reason: string }>;
  };
  createOperatingMaterializedDraft(input: Record<string, unknown>): OperatingMaterializedDraft;
  assertOperatingDraftApproved(draft: OperatingMaterializedDraft): OperatingMaterializedDraft;
}

let cachedApi: Promise<DraftApi> | null = null;

async function draftApi(): Promise<DraftApi> {
  cachedApi ??= (async () => {
    const root = resolveOperatingPipelineRoot({ requireMission: true });
    if (!root) {
      throw new OperateError(
        'E_PIPELINE_VERSION_INCOMPATIBLE',
        'Operating drafts require planr-pipeline with Protocol v1.4.',
      );
    }
    return (await import(
      pathToFileURL(path.join(root, 'lib', 'operate', 'drafts.mjs')).href
    )) as DraftApi;
  })();
  return cachedApi;
}

function draftsDirectory(projectRoot: string): string {
  return path.join(resolveOperatingPaths(projectRoot).root, 'drafts');
}

function storedDraftPath(projectRoot: string, draftId: string): string {
  return path.join(draftsDirectory(projectRoot), `${draftId}.json`);
}

function relativeStoredDraftPath(draftId: string): string {
...
async function readStoredDraft(target: string): Promise<StoredDraft> {
  const parsed = JSON.parse(await readFile(target, 'utf8')) as StoredDraft;
...
export async function listOperatingDrafts(projectRoot: string): Promise<StoredDraft[]> {
  const names = (await readdir(draftsDirectory(projectRoot)).catch(() => []))
    .filter((name) => /^DRAFT-[A-Za-z0-9._-]+\.json$/.test(name))
    .sort();
  return Promise.all(
    names.map((name) => readStoredDraft(path.join(draftsDirectory(projectRoot), name))),
  );
}

export async function showOperatingDraft(
  projectRoot: string,
  draftId: string,
): Promise<StoredDraft> {
...
function findingIdsForAction(findings: OperatingFinding[], action: AdvisorAction): string[] {
  const title = normalized(action.title);
  const summary = normalized(action.summary);
  return findings
    .filter(
      (finding) =>
        normalized(finding.title) === title ||
...
function nextOrdinal(entries: string[], prefix: string): number {
...
function proposalNotice(cycleId: string, draftId: string): string {
  return [
    '> [!IMPORTANT]',
...
function insertAfterFrontmatter(content: string, notice: string): string {
  const end = content.indexOf('\n---\n', 4);
  if (!content.startsWith('---\n') || end < 0) return `${notice}${content}`;
  const offset = end + '\n---\n'.length;
  return `${content.slice(0, offset)}\n${notice}${content.slice(offset)}`;
}

async function directoryEntries(projectRoot: string, relative: string): Promise<string[]> {
...
async function renderDraftArtifact(input: {
  ...
  projectName: string;
  prefixes: { quick: string; spec: string; epic: string };
}): Promise<{
  path: string;
  content: string;
  ...
  if (input.action.routeKind === 'spec') {
    ...
    const shaped = insertAfterFrontmatter(content, notice).replace(
  ...
  if (input.action.routeKind === 'epic') {
    ...
    return {
      ...
      content: insertAfterFrontmatter(
        content.replace('status: "planning"', 'status: "pending"'),
        notice,
      ),
    };
  }
...
function ordinalRoot(kind: OperatingDraftKind): string {
  if (kind === 'quick-task') return '.planr/quick';
  if (kind === 'spec') return '.planr/specs';
  if (kind === 'epic') return '.planr/epics';
  if (kind === 'decision') return '.planr/operate/decisions';
...
export async function materializeOperatingDrafts(input: {
  ...
  rejected: Array<{ actionKey: string; reason: string }>;
}> {
  ...
  for (const candidate of qualified.eligible) {
    const findingIds = findingIdsForAction(
      state.findings.filter(
        (finding) => finding.cycleId === input.cycleId,
      ) as unknown as OperatingFinding[],
      candidate.action,
    );
    ...
    const kind = candidate.action.routeKind as OperatingDraftKind;
    ...
    const ordinal =
      reservations.get(reservationKey) ??
    ...
    const storedRecord: StoredDraft = {
      version: '1.0.0',
    ...
    const writes = [
      { relativePath: rendered.path, operation: 'create' as const, content: rendered.content },
...
async function replaceStoredDraft(
  projectRoot: string,
  stored: StoredDraft,
  nextDraft: OperatingMaterializedDraft,
): Promise<StoredDraft> {
  const store = new OperatingEventStore(projectRoot);
  const head = (await store.replay()).eventHead;
  const next: StoredDraft = { ...stored, draft: nextDraft, updatedAt: new Date().toISOString() };
...
export async function approveOperatingDraft(
  projectRoot: string,
  draftId: string,
): Promise<StoredDraft> {
  ...
  const nextDraft: OperatingMaterializedDraft = {
    ...stored.draft,
    status: 'approved',
...
export async function discardOperatingDraft(
  projectRoot: string,
  draftId: string,
): Promise<{ discarded: OperatingMaterializedDraft; artifactPreserved: boolean }> {
  const stored = await showOperatingDraft(projectRoot, draftId);
  if (stored.draft.status === 'approved') {
    throw new OperateError(
      'E_OPERATE_STATE_INVALID',
      `Approved draft ${draftId} cannot be discarded automatically. Revert its approved work explicitly.`,
    );
  }
  ...
  const discarded: OperatingMaterializedDraft = {
    ...stored.draft,
    status: 'discarded',
...
export async function assertOperatingDraftTargetApproved(
  projectRoot: string,
  target: string,
): Promise<void> {
  ...
  for (const stored of await listOperatingDrafts(projectRoot)) {
    ...
    if (
      ...
      throw new OperateError(
        ...
        `Draft ${draft.draftId} is proposed and cannot enter PLAN or SHIP.`,
        { approvalCommand: `planr operate drafts approve ${draft.draftId}` },
      );
    }
  }
}
