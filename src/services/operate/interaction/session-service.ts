...
export async function currentGuidedSessionBindings(
  projectRoot: string,
): Promise<GuidedSessionBindings> {
  const resolved = await resolveOperatingProject(projectRoot).catch(() =>
...
function sessionPath(projectRoot: string, sessionId: string, localRoot?: string): string {
...
function sessionMacPath(projectRoot: string, sessionId: string, localRoot?: string): string {
  return `${sessionPath(projectRoot, sessionId, localRoot)}.mac`;
}

function sessionKeyPath(projectRoot: string, localRoot?: string): string {
  return path.join(resolveOperatingPaths(projectRoot, { localRoot }).sessions, '.integrity-key');
}

async function loadOrCreateSessionKey(projectRoot: string, localRoot?: string): Promise<Buffer> {
  const target = sessionKeyPath(projectRoot, localRoot);
  const existing = await readFile(target).catch(() => null);
  if (existing) return existing;
  await mkdir(path.dirname(target), { recursive: true, mode: 0o700 });
  const key = randomBytes(32);
  await writeFile(target, key, { mode: 0o600, flag: 'wx' }).catch(
    async (error: NodeJS.ErrnoException) => {
      if (error.code !== 'EEXIST') throw error;
    },
  );
  return (await readFile(target).catch(() => key)) as Buffer;
}

function sessionMac(raw: string, key: Buffer): string {
  return createHmac('sha256', key).update(raw).digest('base64url');
}

async function validateSession(session: GuidedSession): Promise<void> {
  try {
    const errors = (await resolveGuidedInteractionValidators()).validateGuidedSession(session);
    if (errors.length > 0) {
      throw new OperateError(
        'E_OPERATE_SESSION_INVALID',
        'Guided session failed Protocol v1.2 validation.',
      );
    }
  } catch (error) {
    ...
    throw new OperateError(
      ...
      error instanceof Error ? error.message : 'Guided session validators are unavailable.',
    );
  }
}

async function atomicSessionWrite(
  projectRoot: string,
  ...
  switch (session.state) {
    case 'cancelled':
      return new OperateError(
        ...
        'Guided session was cancelled. Start a new initialization session.',
        { state: session.state, recoveryCommand: 'planr operate init --json' },
      );
    case 'expired':
      return new OperateError(
        'E_OPERATE_SESSION_EXPIRED',
        'Guided session expired after 24 hours. Start a new initialization session.',
        { state: session.state, recoveryCommand: 'planr operate init --json' },
      );
    case 'stale':
      return new OperateError(
    ...
    case 'invalid':
      return new OperateError(
        'E_OPERATE_SESSION_INVALID',
        'Guided session is invalid or was tampered with.',
        { state: session.state, recoveryCommand: 'planr operate init --json' },
      );
    default:
      return null;
...
export async function readGuidedSession(input: {
  ...
  try {
    ...
    throw new OperateError(
      'E_OPERATE_SESSION_INVALID',
      'Guided session does not exist or was already purged.',
  ...
  try {
    ...
    const actual = Buffer.from(
      (
        await readFile(sessionMacPath(input.projectRoot, input.sessionId, input.localRoot), 'utf8')
      ).trim(),
    );
    if (expected.length !== actual.length || !timingSafeEqual(expected, actual)) {
      throw new OperateError(
        'E_OPERATE_SESSION_INVALID',
        'Guided session integrity verification failed.',
        { state: 'invalid', recoveryCommand: 'planr operate init --json' },
      );
    }
    ...
    if (parsed.sessionId !== input.sessionId) {
      throw new OperateError(
        'E_OPERATE_SESSION_INVALID',
        'Guided session identity does not match its machine-local record.',
      );
    }
    ...
    if (now.getTime() >= Date.parse(parsed.expiresAt)) {
      ...
      throw terminalSessionError({ ...parsed, state: 'expired' }) as OperateError;
    }
    const bindings = input.bindings ?? (await currentGuidedSessionBindings(input.projectRoot));
    if (
      parsed.projectIdentity !== bindings.projectIdentity ||
...
export async function cancelGuidedSession(input: {
  ...
  sessionId: string;
  localRoot?: string;
}): Promise<{ sessionId: string; state: 'cancelled'; removed: true }> {
  const target = sessionPath(input.projectRoot, input.sessionId, input.localRoot);
  await Promise.all([
...
export async function guidedSessionStatus(input: {
  ...
  localRoot?: string;
  now?: Date;
}): Promise<{ active: number; expired: number; files: number }> {
  ...
  for (const name of names.filter((entry) => entry.endsWith('.json'))) {
    const record = await readFile(path.join(directory, name), 'utf8')
      .then((raw) => JSON.parse(raw) as Partial<GuidedSession>)
      .catch(() => null);
    if (record?.expiresAt && Date.parse(record.expiresAt) <= now) expired += 1;
    else active += 1;
  }
...
export async function purgeGuidedSessions(input: {
  ...
  return {
    removed: new Set(
      names
        .filter((entry) => entry.startsWith('GIS-'))
        .map((entry) => entry.replace(/\.json(?:\.mac)?$|\.replays$/, '')),
    ).size,
  };
}

export function guidedSessionState(
  session: GuidedSession,
  state: GuidedSessionState,
  now: Date,
  values: Partial<GuidedSession> = {},
): GuidedSession {
