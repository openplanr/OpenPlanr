...
const projectedStalls = new WeakMap<OperatingState, OperatingStalledItem[]>();

function closedCyclesAfter(events: OperatingEvent[], sequence: number): number {
  return new Set(
    events
...
function routeFindingIds(events: OperatingEvent[]): Map<string, string[]> {
  const routes = new Map<string, string[]>();
  for (const event of events) {
    if (event.type !== 'route.proposed') continue;
    const record = event.payload.record;
    if (!record || typeof record !== 'object' || Array.isArray(record)) continue;
    const route = record as Record<string, unknown>;
    const actions: unknown[] = Array.isArray(route.actions) ? route.actions : [];
    routes.set(
      event.entityId,
      [
        ...new Set(
          actions
            .map((action: unknown) =>
              action && typeof action === 'object' && !Array.isArray(action)
                ? (action as Record<string, unknown>).findingId
                : null,
            )
...
function latestSequence(events: OperatingEvent[], predicate: (event: OperatingEvent) => boolean) {
  return events.reduce(
    (latest, event) => (predicate(event) ? Math.max(latest, event.sequence) : latest),
    0,
  );
}
 ...
 */
export function deriveOperatingStalledItems(
  state: OperatingState,
  events: OperatingEvent[],
): OperatingStalledItem[] {
  const routeFindings = routeFindingIds(events);
  const routeIdsByFinding = new Map<string, string[]>();
  ...
  const items: OperatingStalledItem[] = [];
  for (const finding of state.findings) {
    const acceptedAt = latestSequence(
      events,
      (event) => event.type === 'finding.accepted' && event.entityId === finding.id,
    );
    if (acceptedAt === 0) continue;
    const routeIds = new Set(routeIdsByFinding.get(finding.id) ?? []);
    const routeApplied = state.routes.some(
      (route) => routeIds.has(route.id) && route.state === 'applied',
    );
    const terminal =
      routeApplied || ['done', 'rejected', 'superseded'].includes(String(finding.status));
    const progressAt = latestSequence(
      events,
      (event) =>
        (event.entityId === finding.id &&
          ['finding.accepted', 'finding.queued', 'finding.in-progress', 'finding.done'].includes(
            event.type,
          )) ||
  ...
  for (const decision of state.decisions) {
    const openedAt = latestSequence(
      events,
      (event) => event.type === 'decision.open' && event.entityId === decision.id,
    );
...
export function projectOperatingStalledItems(
  state: OperatingState,
  events: OperatingEvent[],
): OperatingState {
  const stalls = deriveOperatingStalledItems(state, events);
  const findingCycles = new Map(
    stalls.filter((item) => item.kind === 'finding').map((item) => [item.id, item.stalledCycles]),
  );
  const projected: OperatingState = {
    ...state,
...
export function operatingStalledItems(state: OperatingState): OperatingStalledItem[] {
  return (
    projectedStalls.get(state)?.map((item) => ({ ...item })) ??
    state.findings
...
export function attachOperatingStalledItems(
  state: OperatingState,
  stalls: OperatingStalledItem[],
): OperatingState {
  projectedStalls.set(
    state,
...
export function overdueOperatingDecisionIds(state: OperatingState, now: Date): string[] {
