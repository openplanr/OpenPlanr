...
class StrictJsonParser {
  ...
  constructor(
    ...
    private readonly limits: JsonImportLimits,
  ) {}

  parse(): unknown {
    if (Buffer.byteLength(this.source) > this.limits.maxBytes) this.fail('JSON exceeds byte limit');
    this.skipWhitespace();
  ...
  private value(depth: number): unknown {
    if (depth > this.limits.maxDepth) this.fail('JSON exceeds nesting limit');
    this.skipWhitespace();
    const char = this.source[this.index];
    if (char === '{') return this.object(depth + 1);
    if (char === '[') return this.array(depth + 1);
    if (char === '"') return this.scalar(this.string());
  ...
  private object(depth: number): Record<string, unknown> {
    ...
    while (true) {
      ...
      if (FORBIDDEN_KEYS.has(key)) this.fail(`Forbidden JSON key: ${key}`);
  ...
  private string(): string {
    ...
    while (this.index < this.source.length) {
      ...
      if (!escaped && this.source[this.index] === '"') {
        ...
        if (value.length > this.limits.maxStringLength) this.fail('JSON string exceeds limit');
        return value;
      }
      if (!escaped && this.source[this.index] === '\\') {
        escaped = true;
  ...
  private scalar<T>(value: T): T {
    this.scalars += 1;
    if (this.scalars > this.limits.maxScalars) this.fail('JSON exceeds scalar limit');
    return value;
  }
...
export function parseStrictJson(source: string, limits: Partial<JsonImportLimits> = {}): unknown {
...
export function parseStrictCsv(source: string, limits: Partial<CsvImportLimits> = {}): string[][] {
  const resolved: CsvImportLimits = {
...
export function serializeSafeCsv(rows: readonly (readonly string[])[]): string {
  return `${rows
    .map((row) =>
      row
        .map((raw) => {
...
export async function resolveEvidenceImportPath(
  projectRoot: string,
  configuredPath: string,
  roots: readonly EvidenceImportRoot[],
): Promise<{ absolutePath: string; location: string }> {
  if (!configuredPath.trim() || configuredPath.includes('\0')) {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence path is empty or contains NUL.',
    );
  }
  ...
  try {
    ...
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      `Imported evidence file does not exist: ${configuredPath}.`,
    );
  }
  ...
  if (!owner) {
    throw new OperateError(
      ...
      'Imported evidence must remain inside the configured product workspace.',
    );
  }
  ...
  if (!info.isFile()) {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence must be a regular file.',
    );
  }
  ...
  if (extension !== '.json' && extension !== '.csv') {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence supports .json and .csv files only.',
    );
  }
  return {
...
export async function readImportedEvidenceFile(input: {
  ...
  if (!Number.isSafeInteger(input.maxBytes) || input.maxBytes < 1) {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence byte limit must be a positive integer.',
    );
  }
  const resolved = await resolveEvidenceImportPath(
    input.projectRoot,
  ...
  if (before.size > input.maxBytes) {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence exceeds the configured byte limit.',
    );
  }
  ...
  if (buffer.byteLength > input.maxBytes) {
    throw new OperateError(
      'E_OPERATE_EVIDENCE_REJECTED',
      'Imported evidence changed while reading and exceeds the byte limit.',
    );
  }
