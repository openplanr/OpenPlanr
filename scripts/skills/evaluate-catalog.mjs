...
const questions = readJson(join(root, 'packages/skill-runtime/fixtures/resolver/question-fallbacks.json'));
const skills = registry.skills.map(({ skillId, source }) => {
  ...
  return { skillId, passed: result.ok, hosts: result.ok ? result.hosts.length : 0, diagnostics: result.diagnostics };
});
const registryCases = buildRegistryRoutingCases(registry);
const routing = evaluateRoutingCorpus({ registry, cases: [...corpus.cases, ...registryCases], threshold: corpus.threshold });
...
if (process.argv.includes('--json')) process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  ...
  process.stdout.write(`Catalog evaluation ${report.passed ? 'passed' : 'failed'}: ${skills.filter(({ passed }) => passed).length}/${skills.length} skills, ${routing.totals.passed}/${routing.totals.cases} routing cases, ${journeys.totals.passed}/${journeys.totals.journeys} resilience journeys.\n`);
}
