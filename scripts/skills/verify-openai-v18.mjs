...
if (manifest.name !== 'planr' || manifest.skills !== './skills/') throw new Error('Invalid OpenAI plugin manifest.');
...
if (JSON.stringify(skillIds) !== JSON.stringify(expectedSkillNames)) {
  throw new Error(`OpenAI skill membership differs from the ${expectedSkillNames.length}-skill canonical registry.`);
}
function files(directory, prefix = '') {
  return readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    const absolute = join(directory, entry.name);
    const relative = prefix ? `${prefix}/${entry.name}` : entry.name;
    if (entry.isSymbolicLink() || lstatSync(absolute).isSymbolicLink()) throw new Error(`Plugin contains a symlink: ${relative}`);
...
for (const hostSkillName of skillIds) {
  ...
  if (!primary || !metadata || !packageManifest) throw new Error(`${skillId} is not a complete standard skill package.`);
  ...
  if (!metadata.bytes.includes(`$planr:${hostSkillName}`)) throw new Error(`${skillId} lacks native OpenAI invocation metadata.`);
}
