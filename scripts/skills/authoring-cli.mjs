...
const COMMANDS = Object.freeze({
  lint: Object.freeze({
    ...
    format: (result) => [
      ...
      ...result.modules.map((module) => `  ${module.mode} ${module.moduleId}@${module.moduleVersion}`),
      ...result.hostProfiles.map((profile) => `  host-profile ${profile.id}@${profile.version} (${profile.host})`),
    ],
  }),
  ...
  preview: Object.freeze({
    ...
    format: (result) => {
      ...
      for (const host of result.hosts) {
        ...
        for (const module of host.inline) lines.push(`    inline ${module.moduleId}@${module.moduleVersion} (${module.source})`);
        for (const module of host.overlayModules) lines.push(`    overlay-module ${module.moduleId}@${module.moduleVersion} (${module.source})`);
        for (const reference of host.routed) lines.push(`    routed ${reference.moduleId}@${reference.moduleVersion} -> ${reference.path}`);
        lines.push(`    overlay ${host.overlay.hostProfile}: ${formatAuthority(host.overlay.authority)}`);
        if (host.capabilityDecision) {
          const fallbacks = host.capabilityDecision.fallbacks.map(({ surface }) => surface).join(' -> ');
          lines.push(`    capability ${host.capabilityDecision.preferred.surface}: declared${fallbacks ? `; fallback ${fallbacks}` : ''}`);
        }
        for (const output of host.outputs) lines.push(`    output ${output.kind} ${output.path} (${output.byteLength} bytes)`);
        for (const owner of host.owners) lines.push(`    owner ${owner.ownerKind} ${owner.pointer}@${owner.version}`);
        lines.push(...(host.repairs.length === 0 ? ['    repair none'] : host.repairs.map((repair) => `    repair ${repair}`)));
      }
      return lines;
    },
  }),
  evaluate: Object.freeze({
    ...
    format: (result) => [
      ...
      ...result.hosts.map((host) => `  ${host.host} ${host.pass ? 'pass' : 'fail'}: ${host.inlineModules} inline, ${host.routedReferences} routed, ${host.byteLength} bytes${host.reasons.length ? ` (${host.reasons.join('; ')})` : ''}`),
    ],
  }),
});
