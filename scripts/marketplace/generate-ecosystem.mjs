...
function absolutePath(path) {
  if (typeof path !== 'string' || path.length === 0 || path.startsWith('/') || path.split(/[\\/]/u).some((part) => part === '' || part === '.' || part === '..')) {
    fail('E_ECOSYSTEM_PATH', 'Ecosystem paths must be safe repository-relative paths.', { path });
  }
  const absolute = resolve(repoRoot, path);
  if (!absolute.startsWith(`${repoRoot}${sep}`)) fail('E_ECOSYSTEM_PATH_ESCAPE', 'Ecosystem path escapes the repository.', { path });
  return absolute;
}

function read(path) {
  const absolute = absolutePath(path);
  if (!existsSync(absolute)) fail('E_ECOSYSTEM_INPUT_MISSING', `Required ecosystem input is missing: ${path}`, { path });
  const stat = lstatSync(absolute);
  if (stat.isSymbolicLink() || !stat.isFile()) fail('E_ECOSYSTEM_INPUT_TYPE', `Required ecosystem input must be a regular file: ${path}`, { path });
...
function readJson(path) {
  try {
    ...
    fail('E_ECOSYSTEM_JSON', `Cannot parse ecosystem input ${path}.`, { path, cause: error instanceof Error ? error.message : String(error) });
  }
}
...
function assertEqual(actual, expected, code, message, details = {}) {
  if (JSON.stringify(actual) !== JSON.stringify(expected)) fail(code, message, { ...details, expected, actual });
}

function listFiles(path) {
  const base = absolutePath(path);
  if (!existsSync(base)) return [];
  if (lstatSync(base).isSymbolicLink() || !lstatSync(base).isDirectory()) fail('E_ECOSYSTEM_DIRECTORY_TYPE', `${path} must be a regular directory.`);
  const files = [];
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
      const target = resolve(directory, entry.name);
      if (entry.isSymbolicLink()) fail('E_ECOSYSTEM_SYMLINK', `Ecosystem custody does not accept symlinks: ${relative(repoRoot, target)}`);
      if (entry.isDirectory()) visit(target);
      else if (entry.isFile()) files.push(relative(repoRoot, target).split(sep).join('/'));
      else fail('E_ECOSYSTEM_FILE_TYPE', `Unsupported file type in ecosystem custody: ${relative(repoRoot, target)}`);
    }
  };
  visit(base);
  return files.sort();
...
function component(path, publication) {
  ...
  if (typeof manifest.name !== 'string' || typeof manifest.version !== 'string') fail('E_ECOSYSTEM_PACKAGE', `${path}/package.json does not declare name and version.`);
  return {
...
function verifyGeneratedAssets(manifestPath) {
  ...
  if (!Array.isArray(manifest.assets)) fail('E_ECOSYSTEM_ASSET_MANIFEST', `${manifestPath} does not expose an assets array.`);
  for (const asset of manifest.assets) {
    if (typeof asset?.path !== 'string' || typeof asset?.digest !== 'string') fail('E_ECOSYSTEM_ASSET_ROW', `${manifestPath} has an invalid asset row.`, { asset });
    const actual = sha256(read(asset.path));
    if (actual !== asset.digest) fail('E_ECOSYSTEM_ASSET_DIGEST', `${asset.path} differs from ${manifestPath}.`, { expected: asset.digest, actual });
  }
  return manifest;
}

function renderAdapterDoc(ecosystem) {
  const hosts = ecosystem.adapters.hosts.map((host) => `| \`${host.id}\` | ${host.version} | ${host.capabilityLevel} |`).join('\n');
...
function renderEcosystemDoc(ecosystem) {
  const rows = Object.values(ecosystem.components).map((row) => `| \`${row.package}\` | ${row.version ?? 'pending'} | \`${row.path}\` | ${row.publication} |`).join('\n');
  return `<!-- Generated by scripts/marketplace/generate-ecosystem.mjs. -->
...
function renderSupport(support) {
  ...
  return others === 0 ? listed : `${listed}, and ${others} packaged schema, script, and runtime resources`;
}
...
function renderSkillCatalog(sourceRegistry, contentManifest) {
  ...
  const sections = sourceRegistry.skills.map((skill) => {
    ...
    const support = content.assets
      .map(({ path }) => {
        if (!path.startsWith(entryPrefix)) {
          fail('E_ECOSYSTEM_SKILL_RESOURCE', `${skill.skillId} resource is outside its packaged directory.`, { path });
        }
        return path.slice(entryPrefix.length);
      })
      .filter((path) => path !== 'SKILL.md' && path !== 'openplanr.skill.json' && path !== 'agents/openai.yaml');
...
async function buildOutputs() {
  ...
  components.dashboard = existsSync(dashboardManifest)
    ...
    : { package: '@openplanr/dashboard', version: null, path: 'apps/dashboard', publication: 'private', state: 'deferred-local-extraction', manifestDigest: null };

  const manifestByPath = new Map(Object.values(components).map(({ path }) => [path, readJson(`${path}/package.json`)]));
  const releaseFailures = validateWorkspaceManifests(workspace, manifestByPath);
  if (releaseFailures.length > 0) fail('E_ECOSYSTEM_RELEASE_BOUNDARY', 'Package identities or release dependencies are invalid.', { failures: releaseFailures });
  ...
  const expectedExports = [...preservedExports,
    ...
    './diagram-authoring-store', './diagram-authoring-export',
  ].sort();
  ...
  for (const [name, catalog] of Object.entries({ commands, skills, roles, rules, taskKinds, outputs, outputPaths })) {
    if (catalog.protocolVersion !== '1.5.0') fail('E_ECOSYSTEM_PROTOCOL_CATALOG', `${name} is not a Protocol 1.5 catalog.`);
  }
  ...
  assertEqual([...canonicalSkills.skillIds].sort(), protocolSkillIds, 'E_ECOSYSTEM_SKILL_MEMBERSHIP', 'Generated skill membership differs from Protocol.');
  assertEqual([...roleAssets.roleIds].sort(), protocolRoleIds, 'E_ECOSYSTEM_ROLE_MEMBERSHIP', 'Generated role membership differs from Protocol.');
  assertEqual(protocolRoleIds.length, 9, 'E_ECOSYSTEM_ROLE_COUNT', 'Canonical role count drifted.');
  if (!protocolRoleIds.includes('planr-documentation') || protocolRoleIds.includes('planr-docs')) fail('E_ECOSYSTEM_DOCUMENTATION_ROLE', 'Canonical documentation role taxonomy drifted.');
  ...
  assertEqual(adapterIds, ['claude-code', 'codex', 'cursor'], 'E_ECOSYSTEM_ADAPTER_MEMBERSHIP', 'Runtime adapter membership drifted.');
  ...
  assertEqual(evaluationHostProfiles.profiles.map(({ host }) => host), ['claude-code', 'codex', 'cursor'], 'E_ECOSYSTEM_EVALUATION_HOSTS', 'Evaluation host-profile membership drifted.');
  ...
  for (const [file, descriptor] of Object.entries({ ...DIAGRAM_V16_REGISTRIES, ...PROTOCOL_V17_REGISTRIES })) {
    const catalog = readJson(`packages/protocol/registries/${file}`);
    if (catalog.protocolVersion !== descriptor.protocolVersion || catalog.kind !== descriptor.kind) {
      fail('E_ECOSYSTEM_PROTOCOL_CATALOG', `${file} does not match its canonical Protocol registry descriptor.`);
    }
  }
  ...
  const landingAssetsPath = 'packages/pipeline/conformance/fixtures/landing-workflow/generated-assets.json';
  ...
  const ecosystem = {
    ...
    components,
    compatibility: {
      cliOptionalPipeline: { package: 'planr-pipeline', version: components.pipeline.version, exact: true },
    ...
    catalogs: {
      commands: { ...digestRef('packages/protocol/registries/commands.json'), rootCommands: commands.inventory.rootCommandModules, pipelineMachineLeaves: commands.inventory.pipelineMachineLeaves, frozenClaudeDocuments: commands.inventory.frozenClaudeDocuments, negativeContracts: commands.negativeContracts },
      skills: { ...digestRef('packages/protocol/registries/skills.json'), count: protocolSkillIds.length, ids: protocolSkillIds, aliases: canonicalSkills.aliases.map(({ id }) => id).sort(), membershipDigest: canonicalSkills.membershipDigest },
      roles: { ...digestRef('packages/protocol/registries/roles.json'), count: protocolRoleIds.length, ids: protocolRoleIds, aliases: roleAssets.aliases },
      rules: { ...digestRef('packages/protocol/registries/rules.json'), count: rules.rules.length },
      taskKinds: { ...digestRef('packages/protocol/registries/task-kinds.json'), count: taskKinds.bindings.length },
      outputs: { ...digestRef('packages/protocol/registries/outputs.json'), count: outputs.outputs.length },
      outputPaths: { ...digestRef('packages/protocol/registries/output-paths.json'), count: outputPaths.outputs.length, outputCatalogDigest: outputPaths.outputCatalogDigest },
    },
    adapters: {
    ...
    registries: {
      ...
      capturedEvaluationContracts: {
        ...
        graders: { ...digestRef(evaluationGradersPath), registryId: evaluationGraders.registryId, count: evaluationGraders.graders.length },
      },
    },
    generatedSurfaces: {
      artifactShell: { ...digestRef(artifactShellPath), shellVersion: artifactShell.shellVersion, assetCount: artifactShell.assets.length },
      landingWorkflow: {
        catalog: { ...digestRef(landingCatalogPath), workflowId: landingCatalog.workflow.workflowId, workflowVersion: landingCatalog.workflow.workflowVersion, catalogHash: landingCatalog.catalogHash, authority: landingCatalog.authority },
        generatedAssets: { ...digestRef(landingAssetsPath), assetCount: landingAssets.assets.length, workflowCatalogDigest: landingAssets.workflowCatalogDigest },
      },
    },
  };
  ...
  const custody = {
    ...
    ].map((path) => digestRef(path)).sort((left, right) => left.path.localeCompare(right.path)),
    outputs: [...outputsMap.entries()].map(([path, bytes]) => ({ path, digest: sha256(Buffer.from(bytes, 'utf8')) })).sort((left, right) => left.path.localeCompare(right.path)),
  };
...
function writeAtomic(path, bytes) {
  ...
  if (existsSync(absolute) && lstatSync(absolute).isSymbolicLink()) fail('E_ECOSYSTEM_OUTPUT_SYMLINK', `Generated output may not be a symlink: ${path}`);
...
async function run() {
  ...
  if (mode === 'write') {
    for (const [path, bytes] of [...outputs.entries()].sort(([left], [right]) => left.localeCompare(right))) writeAtomic(path, bytes);
  ...
  for (const [path, expected] of outputs) {
    ...
    else if (lstatSync(absolute).isSymbolicLink() || !lstatSync(absolute).isFile()) drift.push({ path, reason: 'type' });
    else {
      const actual = canonicalText(readFileSync(absolute, 'utf8'));
      if (actual !== expected) drift.push({ path, reason: 'content', expected: sha256(Buffer.from(expected)), actual: sha256(Buffer.from(actual)) });
    }
  }
  if (drift.length > 0) fail('E_ECOSYSTEM_DRIFT', 'Generated ecosystem and marketplace assets are stale.', { drift });
...
try {
  ...
  if (error?.details && Object.keys(error.details).length > 0) process.stderr.write(`${JSON.stringify(error.details, null, 2)}\n`);
  process.exitCode = 1;
}
