...
export const PUBLIC_PACKAGE_PATHS = Object.freeze([
  'packages/cli', 'packages/pipeline', 'packages/protocol',
]);
...
export function validateWorkspaceManifests(rootManifest, manifestByPath) {
  ...
  check(rootManifest.name === 'openplanr-workspace', 'root package name must be openplanr-workspace');
  ...
  check(JSON.stringify(rootManifest.workspaces) === JSON.stringify(Object.keys(WORKSPACE_IDENTITIES)), 'root workspace list must be the explicit ordered package list');

  const byName = new Map([...manifestByPath.values()].map((manifest) => [manifest.name, manifest]));
  const internalName = (name) => name.startsWith('@openplanr/') || Object.values(WORKSPACE_IDENTITIES).includes(name);
  for (const [path, name] of Object.entries(WORKSPACE_IDENTITIES)) {
    ...
    check(isPublic ? manifest?.private !== true : manifest?.private === true,
    ...
    for (const section of ['dependencies', 'optionalDependencies', 'peerDependencies', 'devDependencies']) {
      ...
      for (const target of allowed) {
        ...
        check(validVersion(version) && dependencies[target] === version,
      ...
      for (const [target, version] of Object.entries(dependencies)) {
        if (internalName(target)) check(allowed.includes(target), `${path} ${section} has forbidden internal dependency ${target}`);
        if (isPublic && section !== 'devDependencies') {
          check(typeof version === 'string' && !/^(?:file:|link:|workspace:)|^(?:\/|\.\.?[\\/]|[A-Za-z]:[\\/])/u.test(version),
    ...
    if (isPublic) {
      ...
      if (Array.isArray(bundled)) {
        for (const target of bundled) check(!internalName(target), `${path} must not bundle internal workspace ${target}`);
      }
    }
  }
  const cli = manifestByPath.get('packages/cli');
  check(JSON.stringify(cli?.bin) === JSON.stringify({ planr: './bin/planr.js', openplanr: './bin/planr.js', opr: './bin/planr.js' }),
