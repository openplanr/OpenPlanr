...
if (
  ...
  throw new Error('Package identity/version/license is not the reviewed release');
const repository =
  typeof manifest.repository === 'string' ? manifest.repository : manifest.repository?.url;
if (!/^git\+https:\/\/github\.com\/openplanr\/OpenPlanr(?:\.git)?$/.test(repository ?? ''))
  throw new Error('Repository metadata must name the public monorepo');
...
if (!outputRelative || (!outputRelative.startsWith(`..${sep}`) && outputRelative !== '..'))
  throw new Error('Publication output must be outside the source checkout');
...
const tracked = new Set(
  execFileSync('git', ['ls-files', '--cached', '-z'], { cwd: root, encoding: 'utf8' }).split('\0'),
);
...
if (name === 'planr-pipeline') {
  ...
  for (const file of [
    ...
    ...['artifact', 'design', 'operate'].map(
      (domain) => `lib/generated/domain-projections/${domain}.json`,
    ),
  ]) {
    ...
    for (const entry of projection.entries) {
      if (
        ...
        throw new Error('Unsafe generated package path');
      const absolute = join(root, targets[name], entry.target);
      if (createHash('sha256').update(readFileSync(absolute)).digest('hex') !== entry.sha256)
        throw new Error(`Generated package bytes changed: ${entry.target}`);
...
const packed = JSON.parse(
  execFileSync(
    'npm',
    ['pack', '--workspace', name, '--ignore-scripts', '--json', '--pack-destination', output],
...
if (!/^[a-zA-Z0-9._-]+\.tgz$/.test(archive.filename) || !Array.isArray(archive.files))
  throw new Error('Invalid npm archive report');
for (const entry of archive.files) {
  ...
  if (
    ...
    throw new Error('Unsafe packed file path');
  if (
    !tracked.has(`${targets[name]}/${file}`) &&
...
for (const [dependency, range] of Object.entries({
  ...
  if (Object.hasOwn(targets, dependency)) {
    ...
    if (range !== owner.version)
      throw new Error('Public workspace dependencies must pin the tested version');
...
const proof = {
  ...
  sha256: createHash('sha256')
    .update(readFileSync(join(output, archive.filename)))
    .digest('hex'),
  publicDependencies,
};
