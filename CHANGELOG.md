...
- `--push slack` via Incoming Webhooks (`distribution.slackWebhookUrl` in `.planr/config.json`); `--dry-run` works without a webhook configured
...
See [docs/EPIC-PM-REPORTING-LAYER.md](docs/EPIC-PM-REPORTING-LAYER.md) for the design and shipped-vs-deferred matrix.
