...
const source = JSON.parse(readFileSync(resolve(packageRoot, 'command-boundary-source.json'), 'utf8'));
...
if (!existsSync(entrypoint)) throw new Error('Build the CLI before generating its command catalog.');
...
if (JSON.stringify(activeRoots) !== JSON.stringify(classifiedRoots)) {
  throw new Error(`Active command roots are not fully classified: ${JSON.stringify({ activeRoots, classifiedRoots })}`);
}
for (const retired of Object.keys(source.retired)) {
  if (activePaths.includes(retired)) throw new Error(`Retired command is still callable: planr ${retired}`);
}
...
const output = `${JSON.stringify({
  ...
  retired: Object.entries(source.retired).map(([path, classification]) => ({ path, classification })),
}, null, 2)}\n`;

if (mode === 'write') writeFileSync(outputPath, output);
else if (!existsSync(outputPath) || readFileSync(outputPath, 'utf8') !== output) {
  throw new Error('docs/generated/utility-command-catalog.json is stale; run npm run generate:command-catalog --workspace=openplanr.');
}

process.stdout.write(`${mode === 'write' ? 'Generated' : 'Checked'} ${activePaths.length} active command paths and ${Object.keys(source.retired).length} retired paths.\n`);
