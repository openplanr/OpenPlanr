...
function makeFakeClient(
  ...
  const team = vi.fn(async () => ({
    ...
    // tests that do not exercise estimate synchronization. Estimate-specific tests live in
...
describe('runLinearPush — BL push', () => {
  ...
  it('pushes the full body — description, acceptance criteria, and notes (regression for empty-Linear-issue bug)', async () => {
    ...
    // The top-level `# <title>` heading is also stripped — Linear
...
describe('runLinearPush — BL status → Linear stateId', () => {
  ...
  it('omits stateId on update when BL pushStateIds is not configured (Linear rejects null)', async () => {
    // Regression: re-pushing a backlog item after the first successful push hit
...
describe('runLinearPush — zero-config auto-derived stateIds', () => {
  ...
  it('QT status "done" auto-resolves to the team\'s completed state when pushStateIds is missing', async () => {
    ...
    // the issue estimation type. Both are cached for cascades.
