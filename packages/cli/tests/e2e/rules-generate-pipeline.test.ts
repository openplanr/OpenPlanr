...
describe('init and setup have separate responsibilities', () => {
  it(
    ...
    () => {
      ...
      expect(existsSync(rulesRoot)).toBe(false);
  ...
  it(
    ...
    () => {
      ...
      const rulesRoot = join(dir, '.cursor', 'rules');
      expect(existsSync(join(rulesRoot, 'openplanr', 'planr-plan.mdc'))).toBe(true);
...
describe('rules generate --scope pipeline', () => {
  it(
    'cursor + scope=pipeline produces one concise host-native guidance rule',
    () => {
      const dir = makeTempDir();
      ...
      // Retired workflow aliases must not remain active scan inputs.
      for (const name of [
        'planr-pipeline.mdc',
  ...
  it(
    ...
    () => {
      ...
      // ...and only the canonical pipeline rule must exist.
      expect(existsSync(join(rulesRoot, 'openplanr.mdc'))).toBe(true);
  ...
  it(
    ...
    () => {
      ...
      expect(content).toContain('OpenPlanr host-native workflows');
      expect(content).not.toContain('Agent Instructions');
      expect(content).not.toContain('##planr-agile:begin##');
  ...
  it(
    'claude + scope=pipeline produces one namespaced host-native CLAUDE.md block',
    () => {
      const dir = makeTempDir();
  ...
  it(
    ...
    () => {
      ...
      expect(master).not.toMatch(/CLAUDE_PLUGIN_ROOT|Sonnet|Opus|\/planr-pipeline:/);
    },
    TIMEOUT_MS,
  );
});
