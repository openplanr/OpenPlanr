 ...
 *
 * Resolution order:
 * 1. Environment variable (`PLANR_LINEAR_TOKEN`)
...
const ENV_KEY_MAP: Readonly<Record<string, string>> = Object.freeze({
...
export async function resolveApiKey(provider: string): Promise<string | undefined> {
  // Run one-time migration from the legacy plaintext file.
...
/** Save a rotating credential without treating an unreadable fallback file as empty. */
export async function saveRecordedCredential(
  provider: string,
  value: string,
): Promise<'keychain' | 'encrypted-file'> {
  if (await keychainBackend.isAvailable()) {
    try {
      await keychainBackend.set(provider, value);
      return 'keychain';
    } catch {
      /* Use strict fallback. */
    }
...
/** Read a credential from its recorded backend, avoiding stale fallback copies. */
export async function readStoredCredential(
  provider: string,
  source: 'keychain' | 'encrypted-file',
): Promise<string | undefined> {
  try {
    return await (source === 'keychain' ? keychainBackend : encryptedFileBackend).getStrict(
      provider,
    );
  } catch {
    throw new Error(
      'The recorded credential backend is unavailable. Unlock or repair it before retrying.',
    );
  }
}

/** Delete the recorded credential copy; false means removal was not confirmed. */
export async function removeStoredCredential(
  provider: string,
  source: 'keychain' | 'encrypted-file',
): Promise<boolean> {
  try {
    return await (source === 'keychain' ? keychainBackend : encryptedFileBackend).deleteStrict(
      provider,
    );
  } catch {
    return false;
