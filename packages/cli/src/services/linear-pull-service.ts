...
import { reconcileStatus } from '../../lib/integrations.mjs';
...
export function resolveStatusFinalState(
  ...
  const decision = reconcileStatus(c, strategy === 'linear' ? 'remote' : strategy);
  return {
    ...decision,
    side: decision.side === 'remote' ? 'linear' : decision.side,
  };
}
...
export async function runLinearTaskCheckboxSync(
  ...
  for (const t of allTasks) {
    ...
    // pushed issue ids are UUIDs and must pass through.
