...
import { findGherkinContent } from './gherkin-service.js';
import {
...
async function pushOneTaskListForFeature(
  ...
  // Merged TaskLists receive an aggregated stateId, but no estimate.
  // Per-issue estimate synchronization belongs to the granular push path.
  const f = sf.data;
  const { projectId } = strategyCtx;
  ...
  // aggregate per-task-file statuses into a canonical TaskStatus,
