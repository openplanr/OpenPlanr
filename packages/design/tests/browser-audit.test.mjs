...
const entry = { artifactId: 'overview-desktop', screenId: 'overview', frameId: 'desktop', variantId: 'primary' };
function mockPage(result, { attribute = 'data-planr-artifact-frame', throws = false } = {}) {
  const panel = { hidden: true };
  const handle = {
    async getAttribute(name) { return name === attribute ? entry.artifactId : null; },
    async evaluate(fn, argument) { return fn({ closest: () => panel }, argument); },
    async dispose() {},
  };
  const frame = {
    async frameElement() { return handle; },
    async waitForLoadState() { if (throws) throw new Error('Frame timed out.'); },
    async evaluate() { assert.equal(panel.hidden, false); return result; },
  };
  return { frames: () => [frame], panel };
}
const clearResult = () => ({ screenId: 'overview', checkedElements: 3, checkedContrast: 1, checkedFocus: 1, skippedContrast: 0, viewport: { width: 900, height: 700 }, issues: [] });
...
test('rendered checks remain unverified until caller supplies screenshots and completed journeys', async () => {
  ...
  const verified = await auditDesignPage(page, { revision: 'revision-one', entries: [entry], screenshotPaths: ['screenshots/overview.png'], scenarios: [{ name: 'Save account', status: 'passed' }] });
  assert.equal(verified.status, 'verified');
});

test('missing, mismatched and failed frames can never claim complete artifact coverage', async () => {
  const options = { revision: 'revision-one', entries: [entry], screenshotPaths: ['overview.png'], scenarios: [{ name: 'Save', status: 'passed' }] };
  for (const page of [
    { frames: () => [] },
...
test('computed findings retain artifact identity and browser script failures remain visible', async () => {
  const page = mockPage({ ...clearResult(), issues: [{ rule: 'contrast-below-aa', severity: 'error', message: 'Computed contrast is 2:1.' }] });
  page.pageErrors = async () => [new Error('Action handler failed.')];
  const report = await auditDesignPage(page, { revision: 'revision-one', entries: [entry] });
  assert.equal(report.status, 'failed');
  assert.deepEqual(report.issues[0], { artifactId: entry.artifactId, rule: 'contrast-below-aa', severity: 'error', message: 'Computed contrast is 2:1.' });
  assert.ok(report.issues.some(({ rule }) => rule === 'browser-script-error'));
});

async function browser(t) {
  let chromium;
  try { ({ chromium } = createRequire(new URL('../../cli/package.json', import.meta.url))('playwright')); }
  catch { t.skip('Browser fixture requires the workspace Playwright development dependency.'); return null; }
  const systemChrome = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
  const executablePath = existsSync(chromium.executablePath()) ? chromium.executablePath() : existsSync(systemChrome) ? systemChrome : null;
  if (!executablePath) { t.skip('No existing Chromium executable is available; no runtime download is attempted.'); return null; }
...
test('real browser resolves token contrast and diagnoses overflow, clipping, focus and missing media', async (t) => {
  ...
  await page.setContent(`<!doctype html><html lang="en"><head><style>
    ...
    <img src="data:image/png;base64,invalid" alt="Company logo"><button>Continue</button></body></html>`, { waitUntil: 'load' });
  ...
  for (const rule of ['contrast-below-aa', 'clipped-content', 'horizontal-overflow', 'missing-image', 'missing-focus-indicator']) {
    assert.ok(report.issues.some((issue) => issue.rule === rule), `${rule}: ${JSON.stringify(report)}`);
  }
});
...
test('real browser audits hidden frames and restores their review visibility', async (t) => {
  ...
  const content = '<!doctype html><html lang="en"><body data-planr-screen="overview"><h1>Account</h1><p>Review your account information.</p><button onclick="this.textContent=\'Saved\'">Save account</button></body></html>';
  const srcdoc = content.replaceAll('&', '&amp;').replaceAll('"', '&quot;');
  await page.setContent(`<section data-artifact-id="overview-desktop" hidden><iframe title="Account" data-planr-artifact-frame="overview-desktop" width="900" height="700" srcdoc="${srcdoc}"></iframe></section>`, { waitUntil: 'load' });
  const report = await auditDesignPage(page, { revision: 'browser-revision', entries: [entry] });
  assert.deepEqual(report.checkedArtifacts, [entry.artifactId]);
  assert.equal(report.issues.filter(({ severity }) => severity === 'error').length, 0, JSON.stringify(report.issues));
