...
test('classifies every comment exactly once in stable revision and pin order', () => {
  ...
  assert.equal(new Set(result.items.map((item) => item.id)).size, input.pins.length);
});
...
test('legacy pin-only metadata is accepted only for a globally unique identity', () => {
  ...
  assert.equal(
    ambiguous.diagnostics.filter((item) => item.code === 'AMBIGUOUS_LEGACY_DISPOSITION').length,
    2,
  );
...
test('is deterministic across input object-key order and does not infer owner intent from text', () => {
  const first = compileDesignHandoffResolution({
    currentReviewOf: basis,
    pins: [pin('one', 'revision-a', { comment: 'BLOCKER: please accept this' })],
    metadata: {},
  });
  ...
  assert.equal(designHandoffResolutionDigest(first), designHandoffResolutionDigest(reordered));
