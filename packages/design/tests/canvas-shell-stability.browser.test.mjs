...
test(`canvas zoom preserves shell geometry, minimap focus and frame identity (${engine})`, { timeout:90000 }, async () => {
  ...
  try {
    ...
    for (const url of [pathToFileURL(rendered.views.canvas).href,review.url]) {
      ...
      await page.evaluate(async()=>{await document.fonts.ready;window.__openPlanrDesignStudio.setPanels({navOpen:true,reviewOpen:true});});
      await page.locator('.design-minimap-board').first().waitFor();
      await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
      await page.evaluate(()=>Promise.all([...document.querySelectorAll('.planr-workspace,.design-navigator,.planr-review-rail')].flatMap(n=>n.getAnimations()).map(a=>a.finished.catch(()=>{}))));
      const result=await page.evaluate(async()=>{
        const studio=window.__openPlanrDesignStudio;
        const nodes=[...document.querySelectorAll('.design-toolbar,.design-navigator,.planr-review-rail,.design-stage-context,.design-canvas-tools')];
        const geometry=()=>nodes.map(n=>{const r=n.getBoundingClientRect();return [r.x,r.y,r.width,r.height]});
        const initial=geometry(); const first=document.querySelector('.design-minimap-board');first.focus();
        const boards=[...document.querySelectorAll('.design-minimap-board')];
        const boardGeometry=boards.map(n=>['x','y','width','height'].map(k=>n.getAttribute(k)));
        const frames=[...document.querySelectorAll('[data-planr-artifact-frame]')];
        let rootChanges=0,structuralRenders=0;
        const onRender=()=>structuralRenders++;
        const shell=document.querySelector('.planr-shell');shell.addEventListener('planr:design-render',onRender);
        const observer=new MutationObserver(records=>rootChanges+=records.length);observer.observe(shell,{attributes:true});
        const samples=[];
        for(const zoom of [.09,.1,.11,.99,1,1.11,1.2,.98]) {
          studio.setCamera({x:28.7,y:62.3,zoom});
          await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
          samples.push(geometry());
        }
        observer.disconnect();shell.removeEventListener('planr:design-render',onRender);
        return {initial,samples,rootChanges,structuralRenders,focusRetained:document.activeElement===first,
          mapNodesStable:boards.every((n,i)=>n===document.querySelectorAll('.design-minimap-board')[i]),
          mapGeometryStable:JSON.stringify(boardGeometry)===JSON.stringify(boards.map(n=>['x','y','width','height'].map(k=>n.getAttribute(k)))),
          framesStable:frames.every((n,i)=>n===document.querySelectorAll('[data-planr-artifact-frame]')[i])};
      });
      assert.ok(result.samples.every(sample=>JSON.stringify(sample)===JSON.stringify(result.initial)),`Zoom must not resize or recenter shell controls: ${JSON.stringify(result)}`);
      assert.equal(result.rootChanges,0,'Camera updates do not invalidate inherited styles across the shell');
      assert.equal(result.structuralRenders,0,'Zoom does not dispatch structural renders');
      assert.ok(result.focusRetained && result.mapNodesStable && result.mapGeometryStable,'Only the minimap viewport moves on zoom; artboards and keyboard focus remain stable');
      ...
      await page.mouse.move(scroll.x+8,scroll.y+20);await page.keyboard.down('Control');
      for(const delta of [30,-30,80,-80])await page.mouse.wheel(0,delta);
      await page.keyboard.up('Control');
      assert.deepEqual(await page.locator('.design-canvas-tools').boundingBox(),before);
      await page.evaluate(()=>window.__openPlanrDesignStudio.setCamera({x:40,y:40,zoom:.5}));
      ...
      const cameraBeforeProductPan=await page.evaluate(()=>window.__openPlanrDesignStudio.getState().camera);
      ...
      assert.deepEqual(await page.locator('.design-canvas-tools').boundingBox(),before,'Trackpad pan over a product keeps the shell stable');
      ...
      await page.waitForFunction(prior=>window.__openPlanrDesignStudio.getState().zoom<prior,prior);
      assert.deepEqual(await page.evaluate(()=>({width:innerWidth,height:innerHeight,ratio:devicePixelRatio})),viewport,'Modifier-wheel over a product zooms the canvas without zooming the browser shell');
      assert.deepEqual(await page.locator('.design-canvas-tools').boundingBox(),before);
