...
function fixture() {
	...
	const sources = sourceKinds.map((kind, index) => {
		...
		const anchor = kind === "selected-direction" ? { section: "direction-one" }
			: kind === "screen" ? { screenId: "checkout" }
			: kind === "element-anchor" ? { screenId: "checkout", elementId: "submit" }
			: kind === "review-decision" ? { reviewId: "review-1", pinId: "pin-1" }
			: undefined;
...
test("composes all reference and requirement kinds without copying canonical bodies", () => {
	...
	try {
		...
		assert.ok(value.requirements.every((item) => item.verification.length && item.sourceRefs.length));
		...
		assert.throws(() => verifyImplementationHandoffSources(value, (path, source) => ({
...
test("requirement identities derive from canonical content and ordered references", () => {
	...
	assert.equal(deriveImplementationRequirementId(base), deriveImplementationRequirementId({
		verification: ["A confirmation is visible."],
		sourceRefs: ["source-a", "source-b"],
		statement: "Keep checkout explicit.",
		kind: "behavior",
...
test("JSON and Markdown exports are byte stable and exact across key and line-ending changes", () => {
	...
	try {
		...
		assert.deepEqual(exportImplementationHandoffPackage(first), exportImplementationHandoffPackage(second));
		...
		assert.deepEqual(
			exportImplementationHandoffPackage(importImplementationHandoffPackage({ json: reordered, markdown: exported.markdown })),
			exported,
		);
		...
		assert.throws(() => importImplementationHandoffPackage({
			json: exported.json,
...
test("hostile and foreign paths fail closed", () => {
	...
	try {
		for (const path of ["../secret", "/etc/passwd", "https://example.test/design", "user:pass@example.test/file", "file%2fescape"])
			assert.throws(() => composeImplementationHandoff({
				...input,
...
test("draft publication recovers one exact JSON and Markdown pair after interruption", () => {
	...
	try {
		...
		writeFileSync(paths.journal, `${JSON.stringify({ package: value, markdown: value.markdown }, null, 2)}\n`);
...
test("composition leaves an existing review handoff byte-identical", () => {
	...
	try {
		...
		assert.equal(readFileSync(join(root, "review-handoff.md"), "utf8"), "# Existing review handoff\n");
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
});
