...
import { DESIGN_REVIEW_BUNDLE_SCHEMA, DESIGN_WORKSPACE_EVENT_SCHEMA, assertWorkspaceContract } from '@openplanr/protocol/workspace-contracts';
import {
...
const bundle = () => ({
  schemaVersion: '1.0.0', kind: 'openplanr-design-review-bundle', revision: 'render-1',
  design: { id: 'work', title: 'Secret product design', screens: [{ id: 'one', title: 'Overview', description: 'External notes' }], frames: [{ id: 'desktop', label: 'Desktop', width: 1440, height: 1024 }], variants: [{ id: 'A', label: 'First', status: 'ready' }], screenOrder: ['one'], selectedVariant: 'A', defaultView: 'canvas' },
  envelope: { schemaVersion: '1.0.0', artifacts: [{ id: 'one', html: '<h1>Confidential UI</h1>' }], viewer: { mode: 'single' } },
  entries: [{ artifactId: 'one', screenId: 'one', variantId: 'A', frameId: 'desktop' }], state: { positions: { one: { x: -100, y: 10 } } }, verification: { status: 'unverified' },
});
function service(custody) {
  ...
  const meta = { schemaVersion: '1.0.0', id: custody.id, version: 1, epoch: 1, currentRevision: create.revision.id, ownerPublicKey: create.ownerPublicKey, keyring: create.keyring, commentsPaused: false };
  const requests = [];
  let reviewerHash = create.reviewerAuthHash;
  const fetchImpl = async (input, init) => {
    requests.push({ url: input, ...init });
    const url = new URL(input); const suffix = url.pathname.split(custody.id)[1];
    ...
    if (init.method === 'POST') {
      if (!owner || !await verifyWorkspaceSignature(body, meta.ownerPublicKey)) return Response.json({}, { status: 403 });
      if (body.expectedVersion !== meta.version) return Response.json({}, { status: 409 });
      if (suffix === '/rotate') { meta.keyring = body.keyring; meta.epoch = body.epoch; reviewerHash = body.reviewerAuthHash; }
      if (suffix === '/publish') { revisions.set(body.revision.id, body.revision); meta.currentRevision = body.revision.id; }
      meta.version++;
...
test('share contracts reject authored paths and unrelated metadata', () => {
  ...
  for (const field of ['brief', 'designSystem', 'assets']) {
    const value = bundle(); value.design[field] = { path: '/private/project' };
    assert.throws(() => assertWorkspaceContract(value, DESIGN_REVIEW_BUNDLE_SCHEMA), /Invalid/);
  }
  const value = bundle(); value.design.screens[0].source = { html: '/secret/file.html' };
  assert.throws(() => assertWorkspaceContract(value, DESIGN_REVIEW_BUNDLE_SCHEMA), /Invalid/);
});
...
test('creation encrypts content, isolates capabilities and preserves exact retry request', async () => {
  ...
  for (const secret of [custody.token, custody.ownerAuth, custody.ownerPrivateKey, 'Confidential UI', 'Secret product design']) assert.ok(!pending.includes(secret));
  ...
  await assert.rejects(commitWorkspace(custody, { fetchImpl: async () => { throw new Error('timeout'); } }), /unreachable/);
...
test('recipient unlocks signed revisions; wrong token and content tampering fail', async () => {
  const custody = await prepareWorkspace(bundle()); const server = service(custody);
  ...
  await assert.rejects(getWorkspace({ ...access, token: auth }, { fetchImpl: async () => Response.json(server.meta) }));
...
test('rotation preserves prior revisions and prevents old tokens from future access', async () => {
  ...
  await assert.rejects(commitWorkspaceMutation(custody, { fetchImpl: async () => { throw Error('offline'); } }));
  ...
  assert.equal((await decryptWorkspaceRevision(current, originalRevision, server)).design.title, bundle().design.title);
  const updated = bundle(); updated.design.title = 'A new revision';
  await prepareWorkspaceMutation(custody, 'publish', updated); await commitWorkspaceMutation(custody, server);
  await getWorkspace(current, server);
  assert.equal((await decryptWorkspaceRevision(current, current.currentRevision, server)).design.title, 'A new revision');
...
test('feedback is encrypted, signed, author-required, revision-bound and survives rotation', async () => {
  ...
  await assert.rejects(prepareWorkspaceEvent(custody, { kind: 'review', author: '', reviewOf }), /name/);
  const event = await prepareWorkspaceEvent(custody, { kind: 'review', author: 'Reviewer', reviewOf, review: { comment: 'Private comment' } });
...
test('signed change-request metadata round-trips alongside older categories without transport changes', async () => {
  const custody = await prepareWorkspace(bundle()); const server = service(custody); await commitWorkspace(custody, server);
  const common = { kind: 'category', author: 'Reviewer', reviewOf: workspaceEnvelopeDigest(bundle().envelope), pinId: 'pin-1', updatedAt: '2026-09-11T10:00:00Z' };
  const payloads = [{ ...common, schemaVersion: '1.0.0', category: 'suggestion' }, { ...common, schemaVersion: '1.1.0', category: 'change-request' }];
  const events = await Promise.all(payloads.map(payload => prepareWorkspaceEvent(custody, payload)));
  for (const event of events) {
    assertWorkspaceContract(event, DESIGN_WORKSPACE_EVENT_SCHEMA);
    assert.ok(!JSON.stringify(event).includes('change-request'));
  }
  assert.deepEqual(Object.keys(events[1]), Object.keys(events[0]), 'encrypted transport fields remain unchanged');
  const result = await readWorkspaceEvents(custody, { fetchImpl: async () => Response.json({ events: events.map((event, index) => ({ ...event, sequence: index + 1 })), cursor: 2, hasMore: false }) });
  assert.deepEqual(result.events.map(event => event.payload), payloads);
  assert.equal(result.issues.length, 0);
  await assert.rejects(prepareWorkspaceEvent(custody, { ...payloads[1], schemaVersion: '1.0.0' }), /Invalid/);
});

test('unverifiable successful responses retain pending operations for safe retry', async () => {
  const custody = await prepareWorkspace(bundle()); const server = service(custody);
  await assert.rejects(commitWorkspace(custody, { fetchImpl: async () => Response.json({ ok: true }) }), /Invalid/);
