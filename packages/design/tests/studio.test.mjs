...
const runtime = readFileSync(
	new URL("../templates/studio/studio.js", import.meta.url),
	"utf8",
);
...
async function mount({ data = fixture(), options = {}, stored = null, experience = false } = {}) {
	...
	window.eval(runtime);
	await stage.ready;
	for (let index = 0; index < 20 && !window.__openPlanrDesignStudio; index += 1)
		await delay(10);
	assert.ok(
		window.__openPlanrDesignStudio,
		"studio boots after artifact stage",
	);
	if (experience) { window.eval(enhancements); await delay(20); }
	return {
		...data,
		window,
...
test("Notes dismisses without losing context and restores keyboard focus", async () => {
	...
	try {
		...
		assert.equal(notes.open, false);
		summary.click();
		document.querySelector(".planr-toolbar").dispatchEvent(new window.Event("pointerdown", { bubbles: true }));
		assert.equal(notes.open, false);
		summary.click();
		studio.setView("prototype");
		assert.equal(notes.open, false);
		assert.equal(notes.querySelectorAll("[data-design-note-screen]").length, 3, "dismissal preserves the notes");
	} finally {
		fixture.close();
	}
});
...
test("studio composes one toolbar and retains the artifact-owned review, bridge sandbox and failed directions", () => {
	...
	assert.equal(
		document
			.querySelector("[data-design-toggle-nav]")
	...
	assert.equal(
		document.querySelectorAll('[data-planr-slot="feedback-rail"]').length,
		1,
	);
	assert.equal(
		document.querySelectorAll("[data-planr-artifact-frame]").length,
		12,
	);
	assert.equal(
	...
	const generic = new JSDOM(
		renderArtifactShellDocument({ envelope: data.envelope }),
	);
	assert.equal(
	...
	assert.equal(
		document.querySelector('option[value="unfinished"]').disabled,
		true,
	);
	dom.window.close();
});
...
test("compact navigation and review rails collapse independently and persist", async () => {
	...
	try {
		...
		const feedback = app.documentNode.querySelector(
		...
		assert.equal(
			app.documentNode.querySelector("[data-planr-share-dialog]").hidden,
			false,
		);
...
test("stable artifact ids are unambiguous and bounded without losing long source identities", () => {
	assert.notEqual(
		designStudioArtifactId("a.b", "c", "d"),
		designStudioArtifactId("a", "b.c", "d"),
	);
...
test("three views, responsive frames and variant comparison reuse the original loaded frames", async () => {
	const app = await mount();
	try {
		const frameSources = [...app.documentNode.querySelectorAll("iframe")].map(
			(frame) => frame.src,
		);
		const visible = () =>
		...
		assert.equal(
			app.documentNode.querySelector(".design-walkthrough-caption").hidden,
			false,
		);
		...
		assert.deepEqual(
			[...app.documentNode.querySelectorAll("iframe")].map(
				(frame) => frame.src,
			),
			frameSources,
		);
...
test("canvas camera pans freely, artboards cross the origin, and each view restores its viewport", async () => {
	...
	try {
		...
		const drag = app.documentNode.querySelector(
			`[data-design-drag="${entry.artifactId}"]`,
		);
		drag.dispatchEvent(
		...
		scroll.dispatchEvent(
			new app.window.MouseEvent("pointerup", { bubbles: true, button: 0 }),
		);
		...
		assert.equal(
			app.studio.getState().viewports.canvas.x,
			movedCanvasViewport.x,
		);
		assert.equal(
			app.studio.getState().viewports.canvas.y,
			movedCanvasViewport.y,
		);
		assert.equal(
			app.studio.getState().viewports.canvas.zoom,
			app.studio.getState().zoom,
		);
		...
		assert.ok(
			Math.abs((point.x - afterZoom.camera.x) / afterZoom.zoom - worldPoint.x) <
				1e-8,
		);
		assert.ok(
			Math.abs((point.y - afterZoom.camera.y) / afterZoom.zoom - worldPoint.y) <
				1e-8,
		);
	} finally {
		app.close();
...
test("screen guidance stays outside product frames in compact tooltips and an accordion", async () => {
	...
	try {
		assert.equal(
			app.documentNode.querySelectorAll("[data-design-notes]").length,
			1,
		);
		const note = app.documentNode.querySelector(
			'[data-design-note="screen-2"]',
		);
		assert.match(note.title, /Journey step 2/);
		note.click();
		const notes = app.documentNode.querySelector("[data-design-notes]");
		const target = app.documentNode.querySelector(
			'[data-design-note-screen="screen-2"]',
		);
		assert.equal(notes.open, true);
		assert.equal(target.open, true);
		assert.equal(target.dataset.active, "true");
		assert.equal(
			app.documentNode.querySelectorAll(
				".planr-artifact-panel [data-design-note-screen]",
			).length,
			0,
		);
	} finally {
...
test("walkthroughs do not truncate beyond eight screens and reject unknown navigation", async () => {
	...
	try {
		...
		assert.equal(
			app.documentNode.querySelector("[data-design-step]").textContent,
			"Step 12 of 12",
		);
		assert.equal(
			app.documentNode.querySelector('[data-design-step-change="1"]').disabled,
			true,
		);
...
test("the preview default is not a vote and the first direction can be explicitly selected", async () => {
	...
	try {
		const select = app.documentNode.querySelector(
			"[data-design-select-direction]",
		);
		assert.equal(select.disabled, false);
		assert.equal(select.textContent, "Use this direction");
		assert.equal(
			app.documentNode.querySelector("[data-design-selected-direction]")
				.textContent,
			"",
		);
...
test("ratings, remix, selection and keyboard arrangement persist serially with optimistic state versions", async () => {
	...
	try {
		...
		app.documentNode.querySelector("[data-design-remix]").value =
			"Keep the compact navigation.";
		app.documentNode.querySelector("[data-design-save-remix]").click();
		const entry = app.entries.find(
			(item) =>
				item.variantId === "compact" &&
				item.screenId === "screen-1" &&
				item.frameId === "desktop",
		);
		const label = app.documentNode.querySelector(
			`[data-design-drag="${entry.artifactId}"]`,
		);
		label.dispatchEvent(
...
test("save failure keeps an unsaved draft visible and never reports success", async () => {
	...
	try {
		...
		const draft = JSON.parse(
			app.window.localStorage.getItem("openplanr.design-studio.fieldwork"),
		);
		assert.equal(draft.unsaved, true);
		assert.equal(draft.state.ratings.editorial, 2);
		assert.match(
			app.documentNode.querySelector(".design-notice").textContent,
			/another window/,
		);
	} finally {
		app.close();
...
test("pins remain owned by the artifact review controller across studio navigation", async () => {
	...
	try {
		...
		assert.equal(
			app.documentNode.querySelectorAll(
				".planr-annotation-layer [data-planr-pin-id]",
			).length,
			1,
		);
		assert.equal(
			app.documentNode
				.querySelector('[data-design-rating="1"]')
...
test('review experience preserves read-only modes, welcome context and personal sidebar widths', async () => {
  const value = await mount({ experience: true, options: { loadExperience: async () => ({ capabilities: { owner: false }, reviewContext: { brief: { purpose: 'Check the assignment flow.', requests: ['Can you find the next action?'] }, implementation: { tokens: [{name:'accent',value:'#008577'}], components: [], responsive: ['One column on mobile.'], accessibility: ['Labels remain visible.'] } } }) } });
  try {
    ...
    assert.equal(document.querySelector('.planr-shell').style.getPropertyValue('--design-nav-width'), '248px');
    ...
    document.querySelector('#design-inspect-tab').dispatchEvent(new window.KeyboardEvent('keydown', { key:'ArrowLeft', bubbles:true }));
    ...
    [...document.querySelectorAll('dialog button')].find(item => item.textContent === 'Start walkthrough').click();
    assert.equal(document.querySelector('dialog').dataset.designWelcomeStep, 'profile');
    [...document.querySelectorAll('dialog button')].find(item => item.textContent === 'Browse first').click();
...
test('review filters search and classify without losing unsent reply text', async () => {
  const value = await mount({ experience:true, options:{ loadExperience:async () => ({ capabilities:{owner:false}, metadata:{version:1,categories:{'pin-1':'blocker'},dispositions:{}} }) } });
  try {
    ...
    const pin = (id,screen,comment) => ({id,artifactId:entries.find(item=>item.screenId===screen).artifactId,author:{name:'Rae'},intent:'improve',status:'open',comment,region:{x:.1,y:.1,w:.05,h:.05},viewport:{width:1280,height:800},anchor:{planrId:'main',screen},createdAt:'2026-09-10T00:00:00.000Z',updatedAt:'2026-09-10T00:00:00.000Z',replies:[]});
    stage.review.replaceReview({schemaVersion:'1.0.0',reviewId:'review',reviewOf:digestArtifactEnvelope(envelope),decision:'pending',overall:'',pins:[pin('pin-1','screen-1','Missing action'),pin('pin-2','screen-2','Looks clear')]});
    assert.equal(document.querySelectorAll('.planr-thread').length,1);
    const reply = document.querySelector('[data-planr-reply-form] textarea'); reply.value='Unsent detailed answer'; reply.dispatchEvent(new value.window.Event('input',{bubbles:true}));
    const scope = document.querySelector('[aria-label="Review scope"]');scope.value='all';scope.dispatchEvent(new value.window.Event('change',{bubbles:true}));
    assert.equal(document.querySelectorAll('.planr-thread').length,2);
    assert.equal(document.querySelector('[data-planr-pin-id="pin-1"] textarea').value,'Unsent detailed answer');
    const type = document.querySelector('[aria-label="Comment type"]');type.value='blocker';type.dispatchEvent(new value.window.Event('change',{bubbles:true}));
    assert.equal(document.querySelectorAll('.planr-thread').length,1);
    assert.equal(document.querySelector('.planr-intent').textContent,'Blocker');
    const search=document.querySelector('[aria-label="Search comments"]');search.value='nothing';search.dispatchEvent(new value.window.Event('input',{bubbles:true}));
    assert.equal(document.querySelectorAll('.planr-thread').length,0);
    search.value='action';search.dispatchEvent(new value.window.Event('input',{bubbles:true}));
    assert.equal(document.querySelector('[data-planr-pin-id="pin-1"] textarea').value,'Unsent detailed answer');
  } finally { value.close(); }
});
...
test('owner handoff keeps source quotes immutable and requires saving refinements before approval', async () => {
  const calls = [];
  const source = { pinId:'p1', reviewId:'review', reviewOf:'f'.repeat(64), text:'The primary action is unclear.', refinement:'', author:'Rae', source:'local', stale:false };
  let result = { current:true, metadata:{version:0,categories:{},dispositions:{}}, revision:'current', draft:{version:1,status:'draft',contentHash:'hash-one',content:{summary:'Clarify the action',agreedChanges:[source],openQuestions:[],deferred:[],rejected:[]},markdown:'Review brief'} };
  const value = await mount({experience:true, options:{loadExperience:async()=>({revision:'current',capabilities:{owner:true,handoff:true}}), loadHandoff:async()=>result, updateHandoff:async body=>{calls.push(body);result={...result,draft:{...result.draft,version:2,contentHash:'hash-two',...(body.content ? {content:body.content}:{}),status:body.action==='approve'?'approved':'draft'}};return result;}}});
  try {
    ...
    const refinement = document.querySelector('[aria-label="Accepted changes: p1"]');refinement.value='Use a clear verb and keep the action above the fold.';refinement.dispatchEvent(new value.window.Event('input',{bubbles:true}));
    const find = label => [...document.querySelectorAll('dialog button')].find(button=>button.textContent===label);
    assert.equal(find('Approve handoff').disabled,true);
    find('Save draft').click(); await delay(10);
    assert.equal(calls[0].content.agreedChanges[0].text,source.text);
    assert.equal(calls[0].content.agreedChanges[0].refinement,'Use a clear verb and keep the action above the fold.');
    find('Approve handoff').click();await delay(10);
    assert.equal(calls[1].action,'approve');assert.equal(calls[1].contentHash,'hash-two');
    assert.equal(find('Approve handoff').disabled,true);
  } finally {value.close();}
});
