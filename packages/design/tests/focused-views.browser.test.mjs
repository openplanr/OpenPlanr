  ...
  await page
    ...
    .catch(async (error) => {
      const geometry = await page.evaluate(() => ({
        view: window.__openPlanrDesignStudio.getState().view,
        frame: document
          .querySelector('.planr-artifact-panel:not([hidden]) iframe')
      ...
      throw new Error(`Focused screen escaped its viewport: ${JSON.stringify(geometry)}`, {
        cause: error,
      });
    });
...
async function pointerAction(page, name) {
  ...
  await page.mouse.click(
    bounds.x + ((position.rect.x + position.rect.width / 2) * bounds.width) / position.width,
    bounds.y + ((position.rect.y + position.rect.height / 2) * bounds.height) / position.height,
  );
  return child;
}

test(`focused journeys contain the screen, preserve product interaction and return to the free canvas (${engine})`, {
  timeout: 90000,
}, async () => {
  ...
  try {
    ...
    for (const [host, url] of [
      ...
      await page.waitForFunction(
        () => !document.documentElement.hasAttribute('data-design-opening'),
      );
      ...
      assert.equal(
        await page.locator('[data-planr-mode="interact"]').getAttribute('aria-pressed'),
        'true',
        'entering Prototype clears a prior comment cursor',
      );
      ...
      for (const view of ['prototype', 'walkthrough']) {
        if (view === 'walkthrough') {
          ...
          assert.equal(
            await page.locator('.planr-shell').getAttribute('data-design-inspect'),
            'true',
          );
          ...
          assert.notEqual(
            await page.locator('.planr-shell').getAttribute('data-design-inspect'),
            'true',
            'entering Walkthrough clears inspection overlay',
          );
          assert.equal(
            await page.locator('[data-planr-mode="interact"]').getAttribute('aria-pressed'),
            'true',
        ...
        for (const { button, space } of [
          { button: 'left', space: false },
          { button: 'middle', space: false },
          { button: 'left', space: true },
        ]) {
          await page.mouse.move(background.x, background.y);
        ...
        assert.equal(
          await page
            .locator('.planr-artifact-panel:not([hidden]) .design-artboard-label')
            .isVisible(),
          false,
          'focused screens do not expose draggable artboard chrome',
        );
        await page.evaluate(() =>
          window.__openPlanrDesignStudio.setCamera({ x: -100000, y: 100000, zoom: 0.05 }),
        );
        const after = await page.evaluate(() => window.__openPlanrDesignStudio.getState());
        assert.deepEqual(
          after.camera,
          before.camera,
          `${host}/${view}: canvas gestures cannot move a focused screen`,
        );
        assert.equal(
          after.zoom,
          before.zoom,
          `${host}/${view}: canvas zoom cannot shrink a focused screen`,
        );
        assert.deepEqual(await focusedGeometry(page), rectBefore);
        assert.equal(await page.locator('[data-design-pan]').isVisible(), false);
        for (const control of await page.locator('[data-design-zoom]').all())
          assert.equal(await control.isVisible(), false);
        assert.equal(
          await page.getByRole('button', { name: 'Fit screen', exact: true }).isVisible(),
          true,
        );
        assert.equal(
          await page.locator('[data-planr-mode="interact"]').getAttribute('aria-pressed'),
        ...
        await page.waitForFunction(
          (previous) => window.__openPlanrDesignStudio.getState().screenId !== previous,
          before.screenId,
        );
        await focusedGeometry(page);
        const product = await (
        ...
        await page.mouse.move(
          ...
          frameRect.y + frameRect.height * 0.75,
        );
        ...
        assert.deepEqual(
          await focusedGeometry(page),
          frameRect,
          'product scroll stays inside the selected screen',
        );

        await page.locator('[data-planr-mode="comment"]').click();
        const annotation = page.locator(
          '.planr-artifact-panel:not([hidden]) [data-planr-annotation-layer]',
        );
        ...
        assert.equal(
          await page.locator('[data-planr-composer-comment]').isVisible(),
          true,
          'annotation remains an explicit usable tool',
        );
        ...
        if (evidence) {
          ...
          await page.screenshot({ path: join(evidence, `${host}-${view}-${engine}.png`) });
        }
      ...
      assert.deepEqual(
        restored.camera,
        canvas.camera,
        'returning to Canvas restores its free camera',
      );
      assert.equal(restored.zoom, canvas.zoom);
      assert.deepEqual(
        restored.positions,
        canvas.positions,
        'focused journeys never alter the freeform artboard arrangement',
      );
      assert.equal(
        restored.compare,
        canvas.compare,
        'Canvas retains its direction-comparison setting',
      );
      assert.ok((await page.locator('.planr-artifact-panel:visible').count()) > 1);
      await page.evaluate(() =>
        window.__openPlanrDesignStudio.setCamera({ x: 160, y: 140, zoom: 0.65 }),
      );
      assert.deepEqual(
        (await page.evaluate(() => window.__openPlanrDesignStudio.getState())).camera,
        { x: 160, y: 140 },
        'Canvas remains freely movable',
      );
    }
