...
async function fixture(t) {
  ...
  document.brief.text = 'PRIVATE_LOCAL_BRIEF';
  ...
  const review = (current, id, comment) => ({
    schemaVersion: '1.0.0',
    reviewId: id,
    reviewOf: digestArtifactEnvelope(current.envelope),
    decision: 'pending',
    overall: id === 'local-current' ? 'Current overall note' : '',
    pins: [
      {
        id: 'pin',
        artifactId: current.entries[0].artifactId,
        author: { id: 'morgan', name: 'Morgan' },
        intent: 'improve',
        status: 'open',
        comment,
  ...
  document.screens[0].title = 'Updated workspace';
  atomicJson(file, document);
  ...
  const current = currentDesign(file),
    nowReview = review(current, 'local-current', 'Current review text');
  const unknown = {
    ...review(current, 'shared-missing-bundle', 'Do not relocate this pin'),
  ...
  writeArtifactReviewState(
    ...
    createReviewLedger({
      ...
      currentReviewOf: digestArtifactEnvelope(current.envelope),
      reviews: [oldReview, nowReview, unknown].map((review) => ({
        review,
        stale: review.reviewOf !== digestArtifactEnvelope(current.envelope),
      })),
    }),
  );
  atomicJson(join(root, '.design/shared-feedback.json'), {
    importedReviews: { [oldReview.reviewId]: oldReview },
    metadataByRevision: { 'published-old': { categories: { pin: 'question' }, dispositions: {} } },
    issues: [],
    events: [],
  });
  atomicJson(join(root, '.design/review-metadata.json'), {
    ...
    byRevision: {
      'local-current': {
        categories: { pin: 'change' },
        dispositions: {
          pin: {
            disposition: 'accept',
            reason: 'Owner explanation',
            author: 'Owner',
...
test('local agent export preserves original revision, screen, frame, metadata and quotations without project material', async (t) => {
  const f = await fixture(t),
    before = readFileSync(designReviewPath(f.file, f.env), 'utf8');
  ...
  assert.equal(exported.summary.threads, 3);
  assert.equal(exported.summary.replies, 3);
  const previous = exported.groups.find((group) => group.sourceRevisionId === 'published-old');
  assert.equal(previous.screen.title, 'Workspace overview');
  assert.equal(previous.screen.id, 'screen-1');
  assert.equal(previous.frame.id, 'desktop');
  assert.equal(previous.frame.width, 1440);
  assert.equal(previous.threads[0].category, 'question');
  assert.equal(previous.threads[0].comment, 'Historical review text');
  assert.equal(previous.threads[0].location.coordinateSpace, 'anchor-normalized');
  ...
  assert.equal(latest.screen.title, 'Updated workspace');
  assert.equal(latest.threads[0].category, 'change');
  assert.equal(latest.threads[0].disposition.value, 'accept');
  ...
  assert.equal(missing.threads[0].stale, true);
  assert.equal(missing.screen.id, null);
  assert.ok(missing.threads[0].staleReasons.some((reason) => reason.includes('do not relocate')));
  const raw = JSON.stringify(exported);
  assert.ok(!raw.includes(f.root));
  assert.doesNotMatch(
    raw,
    /PRIVATE_LOCAL_BRIEF|PRIVATE_UNRELATED_FILE|ownerAuth|ownerPrivateKey|accessToken|sourceDigests|file:\/\//,
  );
  assert.equal(
    readFileSync(designReviewPath(f.file, f.env), 'utf8'),
    before,
    'Export cannot resolve pins or rewrite feedback',
  );
...
test('bundled feedback utility exports JSON and Markdown without overwriting and reports scoped summaries', async (t) => {
  const f = await fixture(t),
    output = join(f.root, 'exports/review.json'),
    printed = [];
  const result = await designUtility(
    ...
    { env: f.env, stdout: (value) => printed.push(value) },
  );
  assert.equal(result.summary.threads, 1);
  assert.equal(printed.length, 1);
  assert.equal(
    JSON.parse(readFileSync(output, 'utf8')).groups[0].threads[0].comment,
    'Current review text',
  );
  ...
  await assert.rejects(
    () =>
      designUtility(['feedback', f.file, '--action', 'export', '--output', output], {
        env: f.env,
        stdout() {},
      }),
    /already exists/,
  ...
  assert.match(readFileSync(markdown, 'utf8'), /Historical review text/);
  assert.match(readFileSync(markdown, 'utf8'), /anchor-normalized/);
  for (const flags of [['--format', 'html'], ['--scope', 'invalid'], []]) {
    await assert.rejects(
      () =>
        designUtility(['feedback', f.file, '--action', 'export', ...flags], {
          env: f.env,
          stdout() {},
        }),
      /format|scope|output/,
...
test('local export API requires the owner session capability and same-origin JSON, then returns only review data', async (t) => {
  const f = await fixture(t),
    server = await startDesignReview(f.file, { env: f.env, noOpen: true });
  t.after(() => server.close());
  const origin = new URL(server.url).origin,
    url = `${server.url}api/design-feedback-export`;
  ...
  for (const bad of [
    ...
    const response = await fetch(url, { method: 'POST', headers: bad, body: '{}' });
    assert.equal(response.status, 403);
    await response.arrayBuffer();
  }
  assert.equal(
    (await fetch(`${origin}/api/design-feedback-export`, { method: 'POST', headers, body: '{}' }))
      .status,
    404,
  );
  const wrong = new URL(url),
    segments = wrong.pathname.split('/');
  segments[3] = 'A'.repeat(43);
  wrong.pathname = segments.join('/');
  ...
  assert.equal(snapshot.summary.threads, 1);
  assert.ok(!text.includes(f.root));
  assert.doesNotMatch(text, /PRIVATE_LOCAL_BRIEF|PRIVATE_UNRELATED_FILE|ownerPrivateKey/);
  const runtime = await fetch(`${server.url}runtime.js`).then((response) => response.text());
  assert.match(runtime, /loadReviewExport/);
  assert.match(runtime, /api\/design-feedback-export/);
});
