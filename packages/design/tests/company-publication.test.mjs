...
import { createCompanyDesignSourceReader, prepareCompanyDesignPublication } from '../lib/design/company-publication.mjs';
...
function save(file, document) { writeFileSync(file, `${JSON.stringify(document, null, 2)}\n`); }
...
test('publishes all screens, frames and ready variants deterministically without render cache writes', t => {
  ...
  const context = emptyReviewContext(document); context.brief.purpose = 'Review the order confirmation screens.';
  save(join(root, 'review-context.json'), context);
  const names = readdirSync(root), first = prepareCompanyDesignPublication(file), second = prepareCompanyDesignPublication(file);
...
test('embeds linked CSS, nested imports, images and fonts while omitting authored scripts and handlers', t => {
  ...
  writeFileSync(join(root, 'source/assets/logo.svg'), '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M0 0h20v20H0z"/></svg>');
  writeFileSync(join(root, 'source/assets/font.woff2'), Buffer.from([0x77, 0x4f, 0x46, 0x32, 0, 1, 2]));
  writeFileSync(join(root, 'source/nested.css'), '@font-face{font-family:Test;src:url("assets/font.woff2")}main{background-image:url("assets/logo.svg")}');
  ...
  document.screens[0].source.scripts = ['source/runtime.js']; save(file, document);
  const source = join(root, 'source/screen-1.html');
  writeFileSync(source, readFileSync(source, 'utf8').replace('</head>', '<link rel="stylesheet" href="linked.css"><script type="module" src="runtime.js"></script></head>').replace('</body>', '<img src="assets/logo.svg" alt="ACME"><script>window.inline = true;</script></body>'));
  const publication = prepareCompanyDesignPublication(file), html = publication.bundle.envelope.artifacts[0].html;
  ...
  for (const name of ['source/linked.css', 'source/nested.css', 'source/assets/logo.svg', 'source/assets/font.woff2', 'source/runtime.js']) assert.ok(publication.sourceFiles.includes(name));
...
test('rejects unsafe dependencies before their contents can be bundled', t => {
  for (const [name, path, setup, expected] of [
    ...
    ['secret', '.env.local', root => writeFileSync(join(root, 'source/.env.local'), 'TOKEN=secret'), /secrets|internal/],
    ['internal', '.local/data.css', root => { mkdirSync(join(root, 'source/.local')); writeFileSync(join(root, 'source/.local/data.css'), 'body{}'); }, /secrets|internal/],
    ['symlink', 'linked.css', root => symlinkSync(join(root, 'source/style.css'), join(root, 'source/linked.css')), /symbolic/],
    ['missing', 'missing.css', () => {}, /ENOENT/],
  ]) {
    const { root, file } = fixture(t, { count: 1 }); setup(root);
    const source = join(root, 'source/screen-1.html');
    writeFileSync(source, readFileSync(source, 'utf8').replace('</head>', `<link rel="stylesheet" href="${path}"></head>`));
...
test('SVG image payloads omit executable content, bundle referenced assets and reject remote dependencies', t => {
  const { root, file } = fixture(t, { count: 1 });
  const svg = join(root, 'source/logo.svg'), source = join(root, 'source/screen-1.html');
  writeFileSync(join(root, 'source/pixel.png'), Buffer.from([137, 80, 78, 71]));
  writeFileSync(svg, '<svg xmlns="http://www.w3.org/2000/svg" onload="bad()"><script>bad()</script><foreignObject><button>Unsafe</button></foreignObject><image href="pixel.png"/></svg>');
  writeFileSync(source, readFileSync(source, 'utf8').replace('</main>', '<img src="logo.svg" alt="Logo"></main>'));
  const publication = prepareCompanyDesignPublication(file);
  const encoded = /data:image\/svg\+xml;base64,([A-Za-z\d+/=]+)/u.exec(publication.bundle.envelope.artifacts[0].html)[1];
  ...
  writeFileSync(svg, '<svg xmlns="http://www.w3.org/2000/svg"><image href="https://example.invalid/private.png"/></svg>');
...
test('rejects symlink documents and credential-bearing sources even when scripts are omitted', t => {
  ...
  assert.throws(() => prepareCompanyDesignPublication(join(root, 'linked-document.json')), /symbolic/);
  writeFileSync(join(root, 'source/runtime.js'), `const authorization = "Bearer ${'a'.repeat(30)}";`);
  document.screens[0].source.scripts = ['source/runtime.js']; save(file, document);
...
test('rejects local path leakage, missing anchors and source/output budget overflow', t => {
  ...
  writeFileSync(source, original.replace('</main>', '<p>/Users/example/private-project</p></main>'));
  ...
  document.frames = Array.from({ length: 16 }, (_, i) => ({ id: `frame${i}`, label: `Frame ${i}`, width: 1440, height: 1024 }));
  document.variants = Array.from({ length: 4 }, (_, i) => ({ id: `V${i}`, label: `Variant ${i}`, status: 'ready' })); document.selectedVariant = 'V0'; save(file, document);
...
test('source mutation, inode replacement and new symlinks invalidate the prepared snapshot', t => {
  ...
  for (const mutate of [
    ...
    () => { const value = readFileSync(source); rmSync(source); writeFileSync(source, value); },
    () => { rmSync(source); symlinkSync(file, source); },
  ]) {
    const reader = createCompanyDesignSourceReader(file, 1048576);
    reader.readSource('source/style.css', root); mutate();
...
test('company publication bounds acyclic import expansion before producing large intermediate CSS', t => {
  const { root, file, document } = fixture(t, { count: 1 });
  document.screens[0].source.styles = ['source/level-0.css']; save(file, document);
  for (let level = 0; level < 20; level++) writeFileSync(join(root, `source/level-${level}.css`), `@import "level-${level + 1}.css";@import "level-${level + 1}.css";`);
