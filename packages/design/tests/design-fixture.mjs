...
export function designFixture(
  ...
  const document = {
    ...
    brief: {
      text: 'A calm, grounded operations workspace for a product team.',
      source,
      provenance: 'inferred',
    },
    frames,
    screens,
    screenOrder: screens.map(({ id }) => id),
    variants: Array.from({ length: variants }, (_, i) => ({
      id: String.fromCharCode(65 + i),
      label: `Direction ${String.fromCharCode(65 + i)}`,
      status: 'ready',
    })),
    selectedVariant: 'A',
    defaultView: 'canvas',
  ...
  const file = join(root, 'design-document.json');
