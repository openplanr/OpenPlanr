  ...
  for (const invalid of [
    ...
    { ...value, brief: { purpose: 'Review', requests: ['a', 'b', 'c', 'd'] } },
    {
      ...
      implementation: {
        ...
        components: [{ id: 'nav', name: 'Nav', screenIds: ['missing'] }],
      },
    },
  ]) {
    atomicJson(path, invalid);
    assert.throws(() => loadReviewContext(root, document));
  }
});
test('fingerprints scope source edits and distinguish guidance from authored content', (t) => {
  ...
  for (const before of first.fingerprints) {
    const after = edited.fingerprints.find(
      (item) => item.screenId === before.screenId && item.frameId === before.frameId,
    );
  ...
  context.implementation.components = [
    {
      id: 'button',
      name: 'Action',
      screenIds: ['screen-1'],
      states: [{ name: 'disabled', description: 'Wait for validation.' }],
    },
  ];
  ...
  for (const before of edited.fingerprints) {
    const after = guided.fingerprints.find(
      (item) => item.screenId === before.screenId && item.frameId === before.frameId,
    );
...
test('runtime-only revisions keep authored fingerprints and preserve prior review bundles', async (t) => {
  ...
  assert.deepEqual(
    readDesignRevision(file, first.revision).fingerprints,
    readDesignRevision(file, second.revision).fingerprints,
  );
...
test('categorizing a requested change preserves the open pin and requires a separate owner disposition', async (t) => {
  ...
  const { revision } = await renderDesignDocument(file);
  seedReview(file, env);
  const changed = await updateDesignHandoff(
    file,
    { action: 'category', revision, version: 0, pinId: 'pin-1', category: 'change-request' },
    { env },
  );
  ...
  const drafted = await updateDesignHandoff(
    file,
    { action: 'draft', revision, version: 0 },
    { env },
  );
  ...
  await assert.rejects(
    updateDesignHandoff(
      file,
      { action: 'approve', revision, version: 1, contentHash: drafted.draft.contentHash },
      { env },
    ),
    /blocking comment/,
  );
...
test('resolved comments remain explicit and legacy metadata normalizes on the next owner write', async (t) => {
  ...
  atomicJson(join(root, '.design/review-metadata.json'), {
    ...
    dispositions: {
      'pin-1': {
        ...
        updatedAt: '2026-09-10T11:01:00Z',
        author: 'Design owner',
      },
    },
  });
  await assert.rejects(
    updateDesignHandoff(
      file,
      { action: 'approve', revision, version: 1, contentHash: result.draft.contentHash },
      { env },
    ),
    /out of date/,
  );
  ...
  await updateDesignHandoff(
    file,
    { action: 'category', revision, version: 0, pinId: 'pin-1', category: 'question' },
    { env },
  );
...
test('owner handoff groups dispositions, binds citations, rejects conflicting writers and invalidates approval on new feedback', async (t) => {
  ...
  assert.equal(
    readDesignHandoff(file, { env }).feedback.pins[0].status,
    'open',
    'Accepting work must not resolve it',
  );
  let result = await updateDesignHandoff(file, { action: 'draft', revision, version: 0 }, { env });
  ...
  const saves = await Promise.allSettled(
    [1, 2].map(() =>
      updateDesignHandoff(file, { action: 'update', revision, version: 1, content }, { env }),
    ),
  );
  ...
  assert.equal(
    result.draft.content.agreedChanges[0].refinement,
    'Use a prominent action with an explicit label.',
  );
  result = await updateDesignHandoff(
    file,
    { action: 'approve', revision, version: 2, contentHash: result.draft.contentHash },
    { env },
  );
  ...
  await assert.rejects(
    updateDesignHandoff(
      file,
      { action: 'approve', revision, version: 3, contentHash: result.draft.contentHash },
      { env },
    ),
    /out of date/,
  );
  const saved = readDesignHandoff(file, { env });
  saved.draft.content.summary = 'tampered';
  atomicJson(saved.path, saved.draft);
...
test('duplicate pin identities stay scoped to original reviews and overall notes invalidate approval', async (t) => {
  ...
  seedReview(file, env);
  const path = designReviewPath(file, env),
    ledger = readArtifactReviewState(path);
  const second = structuredClone(ledger.reviews[0]);
  second.review.reviewId = 'other-review';
  second.review.pins[0].comment = 'Different original quotation';
  writeArtifactReviewState(
    path,
    createReviewLedger({ ...ledger, reviews: [...ledger.reviews, second] }),
  );
  await assert.rejects(
    updateDesignHandoff(
      file,
      { action: 'disposition', revision, version: 0, pinId: 'pin-1', disposition: 'accepted' },
      { env },
    ),
    /ambiguous/,
  );
  ...
  result = await updateDesignHandoff(
    file,
    {
      ...
      revision,
      version: 1,
      content: { ...result.draft.content, summary: 'Keep both original comments.' },
    },
    { env },
  );
  result = await updateDesignHandoff(
  ...
  const updated = readArtifactReviewState(path);
  updated.reviews[0].review.overall = 'A new overall requirement.';
...
test('owner APIs serve guidance/history and reject cross-origin handoff writes', async (t) => {
  ...
  const server = await startDesignReview(file, { env });
  t.after(() => server.close());
  const response = await fetch(`${server.url}api/design-experience`);
  assert.equal((await response.json()).capabilities.owner, true);
  const rejected = await fetch(`${server.url}api/design-handoff`, {
    method: 'POST',
    headers: {
      origin: 'https://outside.test',
      'content-type': 'application/json',
      'x-openplanr-design': '1',
    },
    body: '{}',
  });
  assert.equal(rejected.status, 403);
  const history = await fetch(`${server.url}api/design-revisions`).then((response) =>
    response.json(),
  );
