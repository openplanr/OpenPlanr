...
test('failed alternatives with unfinished source paths do not block available designs', async (t) => {
  ...
  document.variants[2] = {
    ...document.variants[2],
    status: 'failed',
    issue: 'Interrupted generation',
    sources: { 'screen-1': { html: 'source/unfinished.html' } },
  };
...
test('portable and live exports retain the board selection, layout and refinement state', async (t) => {
  ...
  const state = {
    ...
    selectedVariant: 'B',
    variantId: 'A',
    positions: { [current.entries[0].artifactId]: { x: 42, y: 84 } },
    ratings: { A: 2, B: 5 },
    remix: { B: 'Keep the dense layout' },
  ...
  assert.equal(
    json(file).selectedVariant,
    'A',
    'a provisional board selection does not silently author the semantic handoff',
  );
...
test('independent design boards preserve each others project preferences under concurrent saves', async (t) => {
  ...
  const designs = ['alpha', 'beta'].map((id) => {
    ...
    writeFileSync(value.file, JSON.stringify(value.document));
    return value;
  });
  for (const design of designs) await renderDesignDocument(design.file);
  const saves = await Promise.all(
    designs.map((design) =>
      saveDesignState(design.file, {
        revision: currentDesign(design.file).revision,
        stateVersion: 0,
        state: {
          selectedVariant: 'B',
          ratings: { B: 5 },
          remix: { B: design.document.id },
          preferences: { selected: ['B'], rejected: [] },
        },
      }),
    ),
  );
...
test('publication failure restores exact compatibility bytes while retaining the previous live pointer', async (t) => {
  ...
  await assert.rejects(
    renderDesignDocument(file, {
      writePointer() {
        throw new Error('Pointer replacement failed');
      },
    }),
    /Pointer replacement failed/u,
  );
...
test('dead publication journals recover correctly before and after the pointer was committed', async (t) => {
  const { root, file } = fixture(t);
  const first = await renderDesignDocument(file),
    firstPointer = json(join(root, '.design/current.json'));
  ...
  const secondPointer = json(join(root, '.design/current.json')),
    secondManifest = json(join(root, 'finalized.json'));
  const journal = {
    revision: second.revision,
    previousRevision: first.revision,
    previousManifest: firstManifest.toString('base64'),
...
test('renderer upgrades produce a new snapshot while identical renders retain immutable metadata', async (t) => {
  const { root, file } = fixture(t);
  const initial = await renderDesignDocument(file, {
    rendererRevision: 'renderer-one',
    now: () => '2026-09-09T09:00:00Z',
  });
  const repeated = await renderDesignDocument(file, {
    rendererRevision: 'renderer-one',
...
test('verification needs valid screenshots and identity-matched rendered evidence for every frame', async (t) => {
  ...
  const report = {
    ...
    status: 'passed',
    checkedArtifacts: current.entries.map((entry) => entry.artifactId),
    screenshots: [path],
    scenarios: [{ name: 'Save workspace', status: 'passed' }],
    issues: [],
    screens: current.entries.map((entry) => ({
      artifactId: entry.artifactId,
      screenId: entry.screenId,
      viewport: current.document.frames.find((frame) => frame.id === entry.frameId),
      checkedElements: 5,
      issues: [],
    })),
  };
  ...
  assert.equal(
    verifyDesignDocument(file, { ...report, scenarios: [{ status: 'passed' }] }).status,
    'unverified',
  );
  const wrongFrame = structuredClone(report);
  wrongFrame.screens[0].viewport.width = 1;
  ...
  failing.screens[0].issues.push({ severity: 'error', message: 'Content clipped' });
