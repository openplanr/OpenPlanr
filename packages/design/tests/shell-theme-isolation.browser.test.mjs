...
for (const darkBrowser of engine === 'chromium' ? [false, true] : [false]) {
  test(`shell appearance leaves authored screens unchanged (${engine}, browser ${darkBrowser ? 'dark' : 'default'})`, {
    timeout: 45000,
  }, async () => {
    ...
    try {
      const { file, document } = designFixture(root, {
        count: 4,
        frames: [{ id: 'desktop', label: 'Desktop', width: 640, height: 480 }],
      });
      const styles = [
      ...
      document.screens.forEach((screen, i) => {
        ...
        writeFileSync(
          ...
          `<!doctype html><html><head><style>${styles[i]}</style></head><body><h1>Product ${i + 1}</h1><input value="Authored field"><button data-planr-id="action-${i + 1}" onclick="this.textContent='Saved'">Save product</button></body></html>`,
        );
      });
