...
const actor = {
	id: "local-owner",
	role: "owner",
	capabilities: [IMPLEMENTATION_HANDOFF_APPROVE_CAPABILITY, IMPLEMENTATION_HANDOFF_REVOKE_CAPABILITY],
};
...
test("approval binds the exact loaded package and exact retries reuse one immutable result", () => {
	...
	try {
		...
		assert.throws(() => approveImplementationHandoff(f.root, {
			...request,
			expectedContentDigest: fixed("d"),
		}, options(f)), /already used for different input/u);
	} finally {
...
test("stale tabs, changed bases, wrong roles, missing capabilities, and expired sessions fail closed", () => {
	...
	try {
		...
		const changed = composeImplementationHandoff({
			...f.input,
			requirements: [{ ...f.input.requirements[0], statement: "Show the checkout summary and taxes." }],
		});
		...
		}, options(f)), /changed after it was loaded/u);
		...
		}, options(f, { actor: { ...actor, role: "reviewer" } })), /owner or maintainer/u);
		assert.throws(() => approveImplementationHandoff(f.root, {
			requestId: "missing-capability",
			expectedVersion: changed.version,
			expectedContentDigest: changed.contentDigest,
		}, options(f, { actor: { ...actor, capabilities: [] } })), /lacks the required/u);
		assert.throws(() => approveImplementationHandoff(f.root, {
			requestId: "expired-session",
			expectedVersion: changed.version,
			expectedContentDigest: changed.contentDigest,
		}, options(f, { actor: { ...actor, id: "hosted-owner", sessionExpiresAt: "2026-09-21T11:59:59.000Z" } })), /expired/u);
		assert.throws(() => approveImplementationHandoff(f.root, {
			requestId: "changed-basis",
			expectedVersion: changed.version,
			expectedContentDigest: changed.contentDigest,
		}, options(f, { currentBasis: { ...f.basis, selectedVariant: "direction-two" } })), /basis changed/u);
	} finally {
		rmSync(f.root, { recursive: true, force: true });
	}
});
...
test("regeneration supersedes only the current pointer while history stays immutable", () => {
	const f = fixture();
	try {
		const first = approveImplementationHandoff(f.root, {
		...
		const regenerated = regenerateImplementationHandoffDraft(f.root, {
			...f.input,
			requirements: [{ ...f.input.requirements[0], statement: "Keep the checkout summary and taxes visible." }],
		}, { requestId: "regenerate-v2" }, options(f));
		...
		const repeatedRegeneration = regenerateImplementationHandoffDraft(f.root, {
			...f.input,
			requirements: [{ ...f.input.requirements[0], statement: "Keep the checkout summary and taxes visible." }],
		}, { requestId: "regenerate-v2" }, options(f));
		assert.equal(repeatedRegeneration.draft.version, 2);
		assert.equal(repeatedRegeneration.supersession.repeated, true);
		assert.throws(() => regenerateImplementationHandoffDraft(f.root, {
			...f.input,
			requirements: [{ ...f.input.requirements[0], statement: "Use different content under the same request." }],
		}, { requestId: "regenerate-v2" }, options(f)), /already used for different package content/u);
		assert.equal(readFileSync(join(paths.history, firstDirectory, "handoff.json"), "utf8"), firstBytes);
		assert.equal(regenerated.supersession.event.actor.capability, IMPLEMENTATION_HANDOFF_APPROVE_CAPABILITY);

		const comparison = compareImplementationHandoffVersions(
			f.root,
			{ id: first.package.id, version: first.package.version, contentDigest: first.package.contentDigest },
			"draft",
		);
		...
		const second = approveImplementationHandoff(f.root, {
		...
		const revoked = revokeImplementationHandoff(f.root, {
			requestId: "revoke-v2",
			expectedVersion: 2,
		...
		const retry = revokeImplementationHandoff(f.root, {
			requestId: "revoke-v2",
			expectedVersion: 2,
			expectedContentDigest: second.package.contentDigest,
			reason: "The release scope changed.",
...
test("an interrupted lifecycle publication recovers one complete immutable commit", () => {
	const f = fixture();
	try {
		const result = approveImplementationHandoff(f.root, {
		...
		writeFileSync(paths.journal, `${JSON.stringify({
...
test("approval changes only handoff custody and has no planning, source, Git, release, deployment, or agent effect", () => {
	const f = fixture();
	try {
		const sentinels = ["plan.md", "source/checkout.txt", "git-ref", "release.json", "deployment.json", "agent.log"];
		...
		const before = Object.fromEntries(sentinels.map((path) => [path, readFileSync(join(f.root, path), "utf8")]));
		approveImplementationHandoff(f.root, {
			requestId: "no-side-effects",
			expectedVersion: 1,
			expectedContentDigest: f.draft.contentDigest,
		}, options(f));
		for (const [path, bytes] of Object.entries(before)) assert.equal(readFileSync(join(f.root, path), "utf8"), bytes);
		const moduleSource = readFileSync(new URL("../lib/design/implementation-handoff-approval.mjs", import.meta.url), "utf8");
		assert.doesNotMatch(moduleSource, /node:child_process|\b(?:spawn|execFile|fork)\s*\(|\bgit\s+(?:add|commit|push)|planr\s+(?:plan|ship)|deploy\s*\(/u);
	} finally {
		rmSync(f.root, { recursive: true, force: true });
	}
});
