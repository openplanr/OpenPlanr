...
test('camera movement preserves artboard geometry and quiet personal saves while review edits retain save status', {
  ...
  try {
    ...
    if (evidence) {
      ...
      await page.screenshot({ path: join(evidence, 'canvas-before.png') });
    }
    await page.evaluate(() => {
      window.motionMutations = [];
      window.saveChanges = [];
      const geometryObserver = new MutationObserver((records) =>
        window.motionMutations.push(
          ...records.map((record) => `${record.target.tagName}:${record.attributeName}`),
        ),
      );
      for (const node of document.querySelectorAll(
        '.planr-artifact-panel, [data-planr-artifact-frame]',
      ))
        geometryObserver.observe(node, { attributes: true });
    ...
    const geometry = await page.evaluate(async () => {
      ...
      for (const zoom of [0.23, 0.31, 0.47, 0.63, 0.83]) {
        ...
        samples.push({
          ...
          intrinsicWidth: frame.clientWidth,
          intrinsicHeight: frame.clientHeight,
          renderedWidth: rect.width,
          renderedHeight: rect.height,
          borderWidth: getComputedStyle(outline).borderLeftWidth,
          offsetX: rect.x - border.x,
          offsetY: rect.y - border.y,
          extraWidth: border.width - rect.width,
          extraHeight: border.height - rect.height,
        });
      }
      await studio.flush();
      await new Promise((resolve) => requestAnimationFrame(resolve));
    ...
    assert.deepEqual(
      geometry.mutations,
      [],
      'pan/zoom never rewrites artboard attributes, geometry, visibility or iframe tab stops',
    );
    assert.deepEqual(
      geometry.saveChanges,
      [],
      'camera changes do not touch the review save status',
    );
    for (const sample of geometry.samples) {
      ...
      assert.ok(
        Math.abs(sample.extraWidth - 6) <= 0.51 && Math.abs(sample.extraHeight - 6) <= 0.51,
      );
    }
    ...
    assert.deepEqual(
      ...
      [],
      'sidebar, screen and view changes persist without review save announcements',
    );
    assert.equal(
      await page.evaluate(() => window.__openPlanrDesignStudio.getSaveState().dirty),
      false,
    );
    ...
    assert.deepEqual(
      restored.camera,
      state.camera,
      'personal camera remains recoverable after reload',
    );
    ...
    await page.evaluate(() => {
      ...
      options.savePersonalState = async () => {
        window.hostSaves.personal++;
        return {};
      };
      options.saveState = async () => {
        window.hostSaves.content++;
        return {};
      };
    });
    await page.evaluate(async () => {
    ...
    await page.evaluate(() => {
      document.documentElement.dataset.designOpening = 'true';
    });
    for (const selector of ['.planr-workspace', '.design-navigator', '.planr-review-rail'])
      assert.equal(
        await page.locator(selector).evaluate((node) => getComputedStyle(node).transitionDuration),
        '0s',
        'restored chrome never animates halfway through opening',
      );
