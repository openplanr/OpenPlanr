...
async function settled(page, screenId) {
  await page.waitForFunction((id) => {
    ...
    return (
      ...
      !document.querySelector('[data-design-transition]') &&
...
async function observeNavigation(page, steps, expectedScreen, { cancelToView = null } = {}) {
  return page.evaluate(
    async ({ steps, expectedScreen, cancelToView }) => {
      ...
      const observed = new Promise((resolve, reject) => {
        function sample() {
          ...
          const transitionAnimations = [
            ...document.querySelectorAll('[data-design-transition-visual]'),
          ...
          samples.push({
            ...
            outgoingInert: outgoing.every(
              (panel) => panel.inert || Boolean(panel.closest('[inert]')),
            ),
          ...
          const final =
            state.screenId === expectedScreen && (!cancelToView || state.view === cancelToView);
          stable = complete && !mode && final ? stable + 1 : 0;
          if (stable >= 3) return resolve({ baseline, artboardBaseline, frameCount, samples });
          if (performance.now() - started > 5000)
            return reject(
              new Error(`Walkthrough did not settle: ${JSON.stringify({ state, mode })}`),
            );
...
function assertContinuousTransition(result, label, { animated = true } = {}) {
  ...
  for (const sample of result.samples) {
    ...
    assert.ok(
      sample.outgoing <= 1,
      `${label}: rapid navigation does not accumulate outgoing screens`,
    );
    assert.ok(
      sample.outgoingInert,
      `${label}: outgoing product cannot receive clicks or keyboard focus`,
    );
    assert.deepEqual(
      sample.geometry,
      result.baseline,
      `${label}: stage, toolbar and caption remain fixed`,
    );
    for (const artboard of sample.artboards) {
      for (const key of ['x', 'y', 'width', 'height']) {
        assert.ok(
          Math.abs(artboard[key] - result.artboardBaseline[key]) <= 0.25,
...
test(`walkthrough navigation stays painted, preserves product state and handles interruption (${engine})`, {
  timeout: 90000,
}, async () => {
  ...
  try {
    ...
    document.screens[1].description = Array.from(
      { length: 6 },
      () =>
        'Review the detailed permissions, ownership boundaries, validation rules, responsive behavior and empty states before continuing. Keep this implementation guidance outside the product screen so reviewers can explore the working journey without losing its context.',
    ).join(' ');
    ...
    for (const [host, url] of [
      ...
      assertContinuousTransition(
        await observeNavigation(page, [-1], 'screen-1'),
        `${host}/previous`,
      );
      assert.equal(
        await (await activeFrame(page))
          .getByRole('textbox', { name: 'Workspace name' })
          .inputValue(),
        'Keep my product draft',
      );
      assert.equal(
        await (await activeFrame(page)).getByRole('button', { name: 'Saved', exact: true }).count(),
        1,
      ...
      assert.equal(
        await page.locator('[data-design-transition-outgoing]').count(),
        0,
        'view switch clears outgoing panels',
      );
      assert.equal(await page.locator('.planr-artifact-panel:not([hidden])').count(), 1);
      assert.equal(
        (await page.evaluate(() => window.__openPlanrDesignStudio.getState())).view,
        'prototype',
      );
      assert.deepEqual(
        (await page.evaluate(() => window.__openPlanrDesignStudio.getState())).positions,
        beforeCancel.positions,
      ...
      assertContinuousTransition(
        await observeNavigation(page, [-1], 'screen-2'),
        `${host}/reduced motion`,
        { animated: false },
      );
      assert.deepEqual(
      ...
      if (evidence) {
        ...
        await page.waitForFunction(
          () =>
            document.querySelector('[data-design-transition-visual]')?.getAnimations().length > 0,
        );
        await page.evaluate(() => {
          for (const animation of document
            .querySelector('[data-design-transition-visual]')
            .getAnimations()) {
            animation.pause();
        ...
        await page.screenshot({
          path: join(evidence, `${host}-walkthrough-mid-transition-${engine}.png`),
        });
        await page.evaluate(() => {
          for (const animation of document
            .querySelector('[data-design-transition-visual]')
