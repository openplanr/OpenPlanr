...
const digest='a'.repeat(64),revisionId='revision-abcdefghijklmnop';
const pin=(id='pin-1')=>({id,artifactId:'screen',intent:'improve',status:'open',comment:'Original',author:{name:'spoofed'},position:{x:.2,y:.2},region:{x:.2,y:.2,w:0,h:0},viewport:{width:1440,height:1024},replies:[],createdAt:'2026-09-09T10:00:00Z',updatedAt:'2026-09-09T10:00:00Z'});
const event=(sequence,author,pins)=>({id:`event-${sequence}`,sequence,revisionId,reviewOf:digest,publicKey:{kty:'EC',crv:'P-256',x:author,y:author},payload:{kind:'review',author,reviewOf:digest,review:{schemaVersion:'1.0.0',reviewId:'arbitrary',reviewOf:digest,overall:'',decision:'approved',pins}}});
test('signed author owns pin, immutable geometry and replies; copied snapshots cannot take over',()=>{
 const original=pin();const forged={...pin(),comment:'Replaced',status:'resolved',position:{x:.8,y:.8},replies:[{id:'reply',author:{name:'Fake owner'},comment:'Review response',createdAt:'2026-09-09T11:00:00Z'}]};
 const result=mergeWorkspaceFeedback([event(1,'Alice',[original]),event(2,'Bob',[forged]),event(3,'Alice',[{...original,status:'addressed',position:{x:.9,y:.9}}])],{revisionId,reviewOf:digest});
 assert.equal(result.review.decision,'pending');assert.equal(result.review.pins[0].comment,'Original');assert.equal(result.review.pins[0].status,'addressed');assert.equal(result.review.pins[0].region.x,.2);assert.equal(result.review.pins[0].author.name,'Alice');assert.equal(result.review.pins[0].replies[0].author.name,'Bob');
});
test('quarantines invalid snapshots and digest mismatches without losing later feedback',()=>{
 const bad=event(2,'Bob',[pin()]);bad.payload.review.reviewOf='b'.repeat(64);
 const malformed=event(3,'Eve',[{id:'bad'}]);
 const result=mergeWorkspaceFeedback([event(1,'Alice',[pin()]),bad,malformed,event(4,'Sam',[pin('new')])],{revisionId,reviewOf:digest});
 assert.equal(result.issues.length,2);assert.equal(result.review.pins.length,2);assert.deepEqual(result.acceptedEventIds,['event-1','event-4']);
});
test('reviewer badges distinguish identical display names using verified signer identities', () => {
 ...
 assert.deepEqual(result.review.pins.map(value=>value.author.name),['Alex','Alex']);
...
test('union overflow is quarantined before any event mutation',()=>{
 const full=event(1,'Alice',Array.from({length:10000},(_,i)=>pin(`pin-${i}`)));
 const extra=event(2,'Bob',[pin('overflow')]);
 const result=mergeWorkspaceFeedback([full,extra],{revisionId,reviewOf:digest});assert.equal(result.review.pins.length,10000);assert.equal(result.issues.length,1);assert.deepEqual(result.acceptedEventIds,['event-1']);
});
...
test('categories retain author ownership and owner dispositions cannot be forged by reviewers', () => {
 ...
 const metadata=(sequence,author,kind,fields)=>({...event(sequence,author,[]),payload:{schemaVersion:'1.0.0',kind,author,reviewOf:digest,pinId:'pin-1',updatedAt:'2026-09-10T10:00:00Z',...fields}});
 const values=[event(1,'Alice',[pin()]),metadata(2,'Alice','category',{category:'blocker'}),metadata(3,'Bob','category',{category:'suggestion'}),metadata(4,'Bob','disposition',{disposition:'accepted',reason:'Pretend owner'}),metadata(5,'Owner','disposition',{disposition:'deferred',reason:'Separate change'})];
 ...
 assert.equal(mergeWorkspaceFeedback(values,{revisionId,reviewOf:digest}).metadata.dispositions['pin-1'],undefined);
});
...
test('change request metadata preserves original pins, independent legacy intent and owner authority', () => {
 ...
 const category=(sequence,author,schemaVersion,category)=>({...event(sequence,author,[]),payload:{schemaVersion,kind:'category',author,reviewOf:digest,pinId:original.id,category,updatedAt:'2026-09-11T10:00:00Z'}});
