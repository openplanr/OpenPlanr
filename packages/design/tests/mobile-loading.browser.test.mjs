...
async function settled(page, screenId, frameId = 'desktop') {
  await page.waitForFunction(
    ({ screenId, frameId }) => {
      ...
      return (
        studio?.getState().screenId === screenId &&
        studio.getState().frameId === frameId &&
...
async function tapProductButton(page, name) {
  ...
  const frameSize = await frameElement.evaluate((value) => ({
  ...
  assert.equal(
    await page.evaluate(({ x, y }) => document.elementFromPoint(x, y)?.tagName, screenPoint),
    'IFRAME',
...
test(`mobile reviews demand-load bounded product documents and remain usable (${engine})`, {
  timeout: 180000,
}, async () => {
  ...
  try {
    ...
    for (const [host, url] of [
      ...
      await context.addInitScript(() => {
        ...
        const sample = () => {
          const count = document.querySelectorAll(
            '.planr-artifact-panel iframe[src], .planr-artifact-panel iframe[srcdoc]',
          ).length;
      ...
      assert.equal(
        ...
        true,
        'test uses a touch device',
      );
      await settled(page, 'screen-1');
      assert.equal(
        await page.locator(loadedSelector).count(),
        1,
      ...
      for (let index = 2; index <= 7; index++) {
        ...
        await (await productFrame(page))
          .getByRole('heading', { name: document.screens[index - 1].title, exact: true })
          .waitFor();
      }
      ...
      await settled(page, 'screen-1', 'mobile');
      await focusedGeometry(page);
      const mobile = await productFrame(page);
      assert.equal(
        await mobile.evaluate(() => innerWidth),
        390,
        'responsive selection renders its authored frame width',
      );
      ...
      await page.evaluate(() => {
        window.__openPlanrArtifactStage.review.setIdentity({ name: 'Mobile reviewer' });
        window.__openPlanrDesignStudio.setTool('annotate');
      });
      await page
      ...
      assert.ok(
        (await page.locator('[data-design-load-frame]:visible').count()) >= 4,
        'canvas exposes useful unloaded screen placeholders',
      );
      await page.evaluate(() => {
        const target = [
          ...document.querySelectorAll(
            '.planr-artifact-panel:not([hidden]) [data-design-load-frame]',
          ),
      ...
      await page.waitForFunction(() => {
        const stage = window.__openPlanrArtifactStage;
        return (
          stage.getState().activeArtifactId === window.__selectedPlaceholder &&
      ...
      await focusedGeometry(page);
      assert.equal(
        await page.locator('.design-thumbnail img').count(),
        0,
        'mobile does not rasterize thumbnails',
      );
      ...
      if (evidence) {
        ...
        await page.screenshot({ path: join(evidence, `${host}-mobile-${engine}.png`) });
      }
      ...
      assert.equal(
        await page.locator(loadedSelector).count(),
        1,
        'reload returns to a single ready focused screen',
      );
