...
test("pins, ratings, selected direction and arrangement survive server restart", async (t) => {
	...
	const restored = await fetch(`${restarted.url}api/design-state`).then(
...
test("a stale state writer waiting on a lock cannot overwrite a newly rendered revision", async (t) => {
	...
	const release = await acquireStartLock(
		join(context.root, ".design/studio-state.json.lock"),
	);
	...
	const pending = assert.rejects(
		save,
		(error) =>
			error.statusCode === 409 && /design changed/u.test(error.message),
	);
	try {
		await delay(30);
		writeFileSync(
...
test("occupied requested ports are preserved and incomplete browser readiness never reports ready", async (t) => {
	...
	assert.equal(
		(
			await fetch(`${session.url}api/design-status`).then((response) =>
				response.json(),
			)
		).status,
		"loading",
...
test("pin resolution requires verified revision and a surviving current anchor", async (t) => {
	...
	atomicJson(
		join(context.root, ".design/verification", `${updated.revision}.json`),
		{ status: "verified", revision: updated.revision },
	);
	await assert.rejects(
		resolveDesignPins(context.file, {
			pinIds: ["pin-one"],
