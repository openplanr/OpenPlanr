...
const selectors = { interact: '[data-planr-mode="interact"]', annotate: '[data-planr-mode="comment"]', pan: '[data-design-pan]', inspect: '[data-design-inspect-mode]' };

async function activeTool(page, tool) {
  for (const [name, selector] of Object.entries(selectors)) assert.equal(await page.locator(selector).getAttribute('aria-pressed'), String(name === tool), `Only ${tool} should be selected (${name})`);
  assert.equal(await page.locator('.planr-shell').getAttribute('data-design-pan-active'), String(tool === 'pan'));
  assert.equal(await page.locator('.design-inspect-overlay:visible').count() > 0, tool === 'inspect');
}

test(`canvas tools are exclusive and interrupted gestures release without a view change (${engine})`, { timeout: 90000 }, async () => {
  const root = await mkdtemp(join(tmpdir(), 'openplanr-tools-'));
  let browser, review;
  try {
    const { file } = designFixture(root, {count:1,frames:[{id:'desktop',label:'Desktop',width:1000,height:720}]});
    ...
    for (const url of [pathToFileURL(rendered.views.canvas).href,review.url]) {
      ...
      for (const target of ['interact','annotate','inspect']) {
        await page.locator(selectors.pan).click(); await activeTool(page,'pan');
        await page.locator(selectors[target]).click(); await activeTool(page,target);
      ...
      await page.getByRole('button',{name:'Add comment',exact:true}).click(); await activeTool(page,'annotate');
      await page.locator('.planr-annotation-layer').click({position:{x:100,y:80}});
      await page.locator('[data-planr-composer-comment]').fill('Preserve this draft during tool changes');
      await page.locator(selectors.pan).click(); await activeTool(page,'pan');
      assert.equal(await page.locator('[data-planr-annotation-composer]').count(),0);
      await page.locator(selectors.annotate).click();
      assert.equal(await page.locator('[data-planr-composer-comment]').inputValue(),'Preserve this draft during tool changes');
      ...
      const control = await frame.getByRole('button',{name:'Save workspace',exact:true}).evaluate(button => ({rect:button.getBoundingClientRect().toJSON(),width:innerWidth,height:innerHeight}));
      const bounds = await page.locator('[data-planr-artifact-frame]').boundingBox();
      await page.mouse.click(bounds.x+(control.rect.x+control.rect.width/2)*bounds.width/control.width,bounds.y+(control.rect.y+control.rect.height/2)*bounds.height/control.height);
      ...
      await page.locator(selectors.pan).click();
      const cameraBeforeFramePan = await page.evaluate(()=>window.__openPlanrDesignStudio.getState().camera);
      await page.mouse.move(bounds.x+bounds.width/2,bounds.y+bounds.height/2);
      await page.mouse.down();
      await page.mouse.move(bounds.x+bounds.width/2+84,bounds.y+bounds.height/2+52);
      await page.mouse.up();
      const cameraAfterFramePan = await page.evaluate(()=>window.__openPlanrDesignStudio.getState().camera);
      assert.ok(cameraAfterFramePan.x-cameraBeforeFramePan.x>70&&cameraAfterFramePan.y-cameraBeforeFramePan.y>40,'Pan starts over a product frame');
      await page.locator(selectors.interact).click();
      ...
      await page.locator(selectors.pan).click(); await page.keyboard.press('i'); await activeTool(page,'interact');
      await page.keyboard.press('h'); await activeTool(page,'pan');
      await page.keyboard.press('c'); await activeTool(page,'annotate');
      await page.locator(selectors.inspect).click(); await page.keyboard.press('Escape'); await activeTool(page,'interact');
      ...
      for (const interruption of ['blur','lostpointercapture','pointercancel','tool']) {
        await page.locator(selectors.pan).click();
        await page.mouse.move(area.x+8,area.y+20); await page.mouse.down();
        ...
        await page.mouse.move(area.x+130,area.y+70); await page.mouse.up();
        assert.deepEqual(await page.evaluate(()=>window.__openPlanrDesignStudio.getState().camera),before,`${interruption} releases the old drag`);
        await page.locator(selectors.interact).click(); await activeTool(page,'interact');
      }
      // Holding Space is temporary and returns to the chosen annotation tool.
      await page.locator(selectors.annotate).click();
      await page.evaluate(()=>document.activeElement?.blur());
      await page.keyboard.down('Space'); await activeTool(page,'pan');
      await page.keyboard.up('Space'); await activeTool(page,'annotate');
      await page.locator(selectors.pan).click(); await page.keyboard.press('Escape'); await activeTool(page,'interact');
