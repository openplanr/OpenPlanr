    ...
    requirements: [
      {
        id: 'REQ-001',
        kind: 'behavior',
        statement: 'Confirm order.',
        sourceRefs: ['screen'],
        verification: ['Confirmation is visible.'],
      },
    ],
...
function lineage(value) {
  return {
    ...
    schemaVersion: '1.0.0',
    handoff: { id: value.id, version: value.version, contentDigest: value.contentDigest },
    specId: 'SPEC-014',
    mappings: [
      {
...
test('Continue to Plan returns host-native forms and causes no filesystem effect', () => {
  ...
  try {
    ...
    const before = createHash('sha256')
      .update(readFileSync(join(root, 'sentinel')))
      .digest('hex');
...
test('delivery projection advances from approved to verified without mutating the package', () => {
  ...
  assert.equal(
    projectDesignDeliveryStatus({ handoff: value, currentHandoff: current }).phase,
    'approved',
  );
  assert.equal(
    projectDesignDeliveryStatus({
      handoff: value,
      currentHandoff: current,
      lineage: lineage(value),
      tasks: [{ id: 'T-046', status: 'outstanding' }],
    }).phase,
    'planned',
  );
  assert.equal(
    projectDesignDeliveryStatus({
      ...
      currentHandoff: current,
      lineage: lineage(value),
      tasks: [{ id: 'T-046', status: 'in_progress' }],
      shipClosure: { runId: 'ship_1', state: 'implementing' },
    }).phase,
    'implementing',
  );
  const verified = projectDesignDeliveryStatus({
    handoff: value,
    currentHandoff: current,
    lineage: lineage(value),
    tasks: [{ id: 'T-046', status: 'done' }],
    shipClosure: { runId: 'ship_1', state: 'passed' },
  });
  ...
  assert.equal(
    projectDesignDeliveryStatus({
      handoff: value,
      currentHandoff: { ...current, version: 2 },
      lineage: lineage(value),
    }).stale,
    true,
  );
