...
const readyInput = () => ({
  document: {
    ...
    variants: [
      { id: 'calm', status: 'ready' },
      { id: 'dense', status: 'rejected' },
    ],
  },
  ...
  verification: {
    path: '.design/verification/current.json',
    revision: raw('a'),
    digest: `sha256:${raw('c')}`,
    status: 'verified',
  },
...
test('ready evidence compiles all stable checks and exposes only prepare-plan continuation', () => {
  ...
  assert.equal(
    designHandoffReadinessDigest(value),
    designHandoffReadinessDigest(compileDesignHandoffReadiness(structuredClone(input))),
  );
});
...
test('missing, blocked, attention, and stale evidence produce exact recovery states', () => {
  ...
  stale.verification.revision = raw('9');
...
test('typed absence leaves ordinary work ungoverned and user guidance exposes no integrity mechanics', () => {
  ...
  for (const fixture of [
    readyInput(),
    (() => {
      const value = readyInput();
      delete value.reviewHandoff;
      return value;
    })(),
  ]) {
    const report = compileDesignHandoffReadiness(fixture);
    for (const check of report.checks)
      values.push(check.message, check.recoveryAction?.label ?? '');
  }
...
test('evidence order is canonical while hostile identities, anchors, versions, and paths fail closed', () => {
  ...
  second.review.pins.reverse();
  assert.equal(
    JSON.stringify(compileDesignHandoffReadiness(first)),
    JSON.stringify(compileDesignHandoffReadiness(second)),
  );

  for (const mutate of [
    (value) => {
      value.document.schemaVersion = '2.0.0';
    },
    (value) => {
      value.document.variants.push(structuredClone(value.document.variants[0]));
    },
    (value) => {
      value.review.pins = [{ id: 'pin', revisionId: raw('a'), anchorCount: 2 }];
    },
    (value) => {
      value.specification.path = '../design-spec.md';
    },
    (value) => {
      value.verification.digest = `sha256:${raw('A')}`;
    },
  ]) {
    const value = readyInput();
    mutate(value);
