...
const require = createRequire(
	new URL("../../pipeline/package.json", import.meta.url),
);
...
test("native local submit requires allow-forms even when form navigation is denied by CSP", {
	...
	try {
		...
		assert.equal(
			await local.locator("body").evaluate(() => location.href),
			"about:srcdoc",
		);
	} finally {
		await browser.close();
		...
		screens: [
			{
				...
				title: "Team overview",
				description:
					"Start with the teams and assignments that need your attention.",
			...
			},
			{
				...
				description:
					"A single place for the brief, assigned team, and next action.",
...
test("browser studio boots through the protected artifact server and supports a real reviewed journey", {
					...
					stalePins: [
						{
							...
							comment:
								"Clarify the earlier action before resolving this feedback.",
						},
					],
				},
	...
	try {
		...
		const response = await fetch(
			`http://127.0.0.1:${port}/internal/v1/sessions`,
			{
				method: "POST",
				headers: {
		...
		assert.equal(
			await page.locator('[data-design-stale-pin="earlier-pin"]').isVisible(),
			true,
		);
		assert.equal(
			await page
				.locator('[data-planr-slot="feedback-rail"] [data-design-stale-pin]')
				.count(),
			0,
		);
		await page.reload();
		...
		assert.equal(
			await page.locator(".planr-artifact-panel:visible").count(),
			6,
		);
		const chrome = await page.evaluate(() => ({
			toolbar: document.querySelector(".planr-toolbar").getBoundingClientRect()
				.height,
			navigator: document
				.querySelector(".design-navigator")
				.getBoundingClientRect().width,
			review: document
				.querySelector(".planr-review-rail")
				.getBoundingClientRect().width,
			stage: document.querySelector(".planr-stage").getBoundingClientRect()
				.width,
		}));
    ...
    assert.equal(await page.locator('[data-design-screen]').first().evaluate(node => getComputedStyle(node).fontSize), '13px');
    assert.ok(await page.locator('[data-design-screen]').first().evaluate(node => node.getBoundingClientRect().height) < 90, 'screen rows stay compact');
    ...
    await resizeScreens.focus(); await page.keyboard.press('ArrowRight');
    ...
    assert.equal(await page.getByRole('dialog', {name:'Welcome to this review'}).isVisible(), true);
    ...
    const inspection = page.locator('.planr-artifact-panel:visible .design-inspect-overlay').first();
    ...
    assert.ok(await page.evaluate(() => window.__openPlanrDesignStudio.getState().zoom) > initialCamera.zoom);
    await page.locator('.design-tools-menu > summary').click();
    ...
    await page.evaluate(() => window.__openPlanrDesignStudio.setCamera({x:-345,y:147,zoom:.42}));
    ...
    assert.equal(await page.locator('.planr-shell').getAttribute('data-design-presentation'),'true');
    await page.waitForFunction(() => {
      ...
      const frame = panel.getBoundingClientRect(), viewport = stage.getBoundingClientRect();
      return viewport.width > 100 && viewport.height > 100 && frame.width > 100 && frame.height > 100
        && frame.right > viewport.left && frame.left < viewport.right && frame.bottom > viewport.top && frame.top < viewport.bottom;
    });
    assert.ok(await page.locator('.planr-stage-scroll').evaluate(node => node.getBoundingClientRect().height) > 800, 'presentation stage fills the fullscreen shell');
    await page.getByRole('button',{name:'Exit presentation',exact:true}).click();
    await page.waitForFunction(() => !document.querySelector('.planr-shell').hasAttribute('data-design-presentation'));
    await page.waitForFunction(() => !document.querySelector('.planr-shell').hasAttribute('data-design-restoring'));
    const afterPresentation = await page.evaluate(() => window.__openPlanrDesignStudio.getState());
    for (const key of ['view','screenId','variantId','frameId','camera','zoom','viewports','navOpen','reviewOpen']) assert.deepEqual(afterPresentation[key],beforePresentation[key], `Presentation restores ${key}`);
    assert.equal(await page.locator('.design-tools-menu > summary').evaluate(node => node===document.activeElement), true, 'focus returns to the canvas controls');
    await page.evaluate(() => { document.querySelector('.planr-shell').requestFullscreen = async () => { throw new Error('Fullscreen denied'); }; });
    await page.locator('.design-tools-menu > summary').click();
    await page.getByRole('button', {name:'Present fullscreen',exact:true}).click();
    await page.waitForFunction(() => {
      const panel = document.querySelector('.planr-artifact-panel[data-design-active="true"]');
      ...
      if (!panel || !stage || panel.hidden) return false;
      const frame = panel.getBoundingClientRect(), viewport = stage.getBoundingClientRect();
      return frame.width > 100 && frame.height > 100 && frame.right > viewport.left && frame.left < viewport.right && frame.bottom > viewport.top && frame.top < viewport.bottom;
    });
    await page.getByRole('button', {name:'Exit presentation',exact:true}).focus();
    await page.keyboard.press('Escape');
    await page.waitForFunction(() => !document.querySelector('.planr-shell').hasAttribute('data-design-restoring'));
    assert.equal(await page.locator('.planr-shell').getAttribute('data-design-presentation'), null, 'Escape exits presentation even when native fullscreen is denied');
    assert.deepEqual(await page.evaluate(() => window.__openPlanrDesignStudio.getState().camera), beforePresentation.camera);
    await page.evaluate(value => window.__openPlanrDesignStudio.setCamera({...value.camera,zoom:value.zoom}), initialCamera);
		assert.match(
			await page
				.locator('[data-design-frame] option[value="desktop"]')
				.textContent(),
			/1440 × 1024/,
		);
		...
		const firstCanvasEntry = data.entries.find(
			(entry) =>
				entry.screenId === "overview" &&
				entry.frameId === "desktop" &&
				entry.variantId === "calm",
		);
		...
		const movedPosition = await page.evaluate(
			(artifactId) =>
				window.__openPlanrDesignStudio.getState().positions[artifactId],
		...
		assert.equal(
			await page
				.locator('[data-design-note-screen="overview"]')
				.getAttribute("open"),
			"",
		);
		assert.equal(
			await page
				.locator(".planr-artifact-panel [data-design-note-screen]")
				.count(),
			0,
		...
		assert.equal(await page.locator('[data-design-note="overview"]').first().evaluate((node) => node === document.activeElement), true);
		...
		assert.equal(await page.locator("[data-design-notes]").getAttribute("open"), "", "accordion stays open");
		await page.keyboard.press("Escape");
		assert.equal(await page.locator("[data-design-notes]").getAttribute("open"), null);
		assert.equal(await page.locator("[data-design-notes] > summary").evaluate((node) => node === document.activeElement), true);
		...
		await page.waitForFunction(
			(width) =>
				document.querySelector(".planr-stage").getBoundingClientRect().width >=
				width + 180,
			chrome.stage,
		);
		...
		assert.equal(
			await page.locator(".planr-shell").getAttribute("data-design-nav-open"),
			"false",
		);
		assert.equal(
			await page.locator(".planr-shell").getAttribute("data-planr-rail-open"),
			"false",
		);
		await page.getByRole("button", { name: "Screens", exact: true }).click();
		await page.locator('[data-planr-action="feedback"]').click();
		...
		assert.equal(
			await page.locator("[data-planr-share-dialog]").isVisible(),
			true,
		);
		assert.match(
		...
		assert.match(await page.locator("[data-planr-share-error]").textContent(), /updated OpenPlanr installation/);
		assert.equal(await page.locator("[data-planr-share-fragment-size]").textContent(), "Unavailable");
		...
		assert.equal(
			await page
				.locator('button[data-design-view="prototype"]')
		...
		const iframe = page.locator(
			`[data-planr-artifact-frame="${first.artifactId}"]`,
		);
		...
		await page.mouse.click(
			frameBounds.x +
				((buttonBounds.rect.x + buttonBounds.rect.width / 2) *
					frameBounds.width) /
					buttonBounds.width,
			frameBounds.y +
				((buttonBounds.rect.y + buttonBounds.rect.height / 2) *
					frameBounds.height) /
					buttonBounds.height,
		);
		await page.waitForFunction(
		...
		const target = data.entries.find(
			(entry) =>
				entry.screenId === "assignment" &&
				entry.frameId === "mobile" &&
				entry.variantId === "calm",
		);
		await page
		...
		assert.equal(
			await page.locator(".planr-annotation-layer [data-planr-pin-id]").count(),
			1,
		);
		await page.screenshot({
		...
		assert.match(
			await page.locator("[data-design-step]").textContent(),
			/Step 3 of 3/,
		);
		assert.equal(
			await page.locator(".planr-annotation-layer [data-planr-pin-id]").count(),
		...
		assert.equal(
			await page.evaluate(
				() => window.__openPlanrDesignStudio.getState().ratings.calm,
			),
			4,
		);
		assert.equal(
		...
		await page.waitForSelector('.planr-shell[data-planr-rail-open="false"]');
		await page.screenshot({ path: "/tmp/openplanr-design-studio-mobile.png", animations: "disabled" });
		assert.equal(
			await page.evaluate(
		...
		assert.equal(await page.locator('.planr-artifact-panel[data-design-active="true"]').evaluate((panel) => {
			const rect = panel.getBoundingClientRect();
			return rect.right > 0 && rect.left < innerWidth && rect.bottom > 130 && rect.top < innerHeight;
		}), true, "resizing a journey view keeps the active product screen in view");
		await page.locator("[data-design-notes] > summary").click();
		await page.locator("[data-design-note-screen]").first().evaluate((item) => {
			item.open = true;
			item.querySelector("p").textContent = "A long implementation note remains readable without covering the controls. ".repeat(100);
		});
		const noteBounds = await page.locator(".design-guidance-panel").evaluate((panel) => {
			const rect = panel.getBoundingClientRect();
			return { left: rect.left, right: rect.right, top: rect.top, bottom: rect.bottom, scrolls: panel.scrollHeight > panel.clientHeight };
		});
		assert.ok(noteBounds.left >= 0 && noteBounds.right <= 390 && noteBounds.top >= 0 && noteBounds.bottom <= 844,
    ...
    assert.equal(await page.getByRole("button", { name: "Screens", exact: true }).evaluate(node => node === document.activeElement), true);
    await page.getByRole("button", { name: "Review", exact: true }).click();
    await page.locator('.design-refinement > summary').click();
    await page.getByLabel('Refinement note', { exact: true }).fill('Keep these compact review controls.');
    await page.getByRole('button', { name: 'Save refinement note', exact: true }).click();
    await page.evaluate(() => window.__openPlanrDesignStudio.flush());
    assert.equal(await page.evaluate(() => window.__openPlanrDesignStudio.getState().remix.calm), 'Keep these compact review controls.');
    await page.getByRole("button", { name: "Close review", exact: true }).click();
    assert.equal(await page.getByRole("button", { name: "Review", exact: true }).evaluate(node => node === document.activeElement), true);
...
test("journey-start thumbnails capture later screens when they become visible without retrying on camera motion", {
  ...
  try {
    for (const view of ['walkthrough', 'prototype']) {
      ...
      const data = designFixture(); data.document.defaultView = view;
      const server = createArtifactReviewServer({
        env: { ...process.env, PLANR_HOME: temporary },
        renderDocument: ({ model, base }) => renderDesignStudio({ ...data, envelope: model.envelope }, { stageRuntimeUrl: `${base}runtime.js` }),
      ...
      try {
        ...
        assert.equal(await page.locator('[data-design-screen="assignment"] .design-thumbnail img').count(), 0, 'hidden journey frames retain a placeholder until paintable');
        const iframeCount = await page.locator('iframe').count();
        const failingArtifact = data.entries.find(item => item.screenId === 'confirmed' && item.variantId === basis.variantId && item.frameId === basis.frameId).artifactId;
        await page.evaluate(id => {
          const frame = [...document.querySelectorAll('iframe')].find(frame => frame.dataset.planrArtifactFrame === id);
          window.thumbnailFailureAttempts = 0;
          Object.defineProperty(frame, '__openPlanrBridge', { configurable: true, value: { ...frame.__openPlanrBridge, thumbnail: async () => { window.thumbnailFailureAttempts++; throw new Error('Capture unavailable'); } } });
        }, failingArtifact);
        ...
        assert.deepEqual(await page.evaluate(() => { const {view, variantId, frameId} = window.__openPlanrDesignStudio.getState(); return {view, variantId, frameId}; }), basis);
        await page.locator('[data-design-screen="confirmed"]').click();
        await page.waitForFunction(() => window.thumbnailFailureAttempts === 1);
        await page.evaluate(() => { for (let i=0; i<5; i++) window.__openPlanrDesignStudio.setCamera({x: i*8,y: i*4}); });
        await page.waitForTimeout(200);
        assert.equal(await page.evaluate(() => window.thumbnailFailureAttempts), 1, 'camera renders do not poll or retry a genuine capture failure');
        assert.equal(await page.locator('[data-design-screen="confirmed"] .design-thumbnail img').count(), 0);
        await page.locator('[data-design-screen="assignment"]').click();
        await page.locator('[data-design-screen="confirmed"]').click();
        await page.waitForFunction(() => window.thumbnailFailureAttempts === 2);
        assert.equal(await page.locator('iframe').count(), iframeCount, 'captures reuse the original frames');
      } finally { await page.close(); await server.close(); await rm(temporary, { recursive: true, force: true }); }
    }
	...
	try {
		...
		assert.equal(
			await firstFrame
				...
				.evaluate(
					(input) =>
						input.matches(":invalid") && document.activeElement === input,
				),
			true,
		);
		...
		await page.evaluate(() => window.__openPlanrDesignStudio.flush());
		assert.equal(
			readDesignFeedback(file, { ...process.env, PLANR_HOME: temporary }).pins
				.length,
			1,
		);
		await page.reload();
		...
		assert.equal(
			await page.evaluate(
				() => window.__openPlanrDesignStudio.getState().selectedVariant,
			),
			"B",
		);
		assert.equal(
		...
		assert.equal(
			await page.locator("[data-design-save-state]").textContent(),
			"All changes saved",
		);
		await review.close();
		review = await startDesignReview(file, {
