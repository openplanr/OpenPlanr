  ...
  options.fetchImpl = async (url, init = {}) => {
    ...
    if (route === 'events') {
      if (init.method === 'POST') {
        ...
        if (!event) {
          event = { ...body, sequence: state.events.length + 1 };
          state.events.push(event);
        }
        if (state.badEventReceipt)
          return Response.json({
            event: { ...event, id: 'changed-receipt-abcdefghijkl' },
  ...
  const record = () =>
    JSON.parse(
      readFileSync(
        join(
          options.custodyRoot,
          readdirSync(options.custodyRoot).find((name) => name.endsWith('.json')),
        ),
        'utf8',
      ),
    );
...
test('design publishing keeps credentials local and bundles only presentation material', async (t) => {
  ...
  for (const secret of [custody.token, custody.ownerAuth, custody.ownerPrivateKey])
    assert.ok(!JSON.stringify(result).includes(secret));
  const remoteBytes = JSON.stringify(
    state.requests.map(({ url, init }) => ({ url, body: init.body })),
  );
  assert.ok(!remoteBytes.includes(custody.token));
  assert.ok(!remoteBytes.includes(custody.ownerPrivateKey));
...
test('lost creation response retries the same request and keeps original revision identity', async (t) => {
  ...
  const original = record(),
    first = state.requests[0].init.body;
  writeFileSync(
  ...
  assert.equal(published.url, retried.url);
  assert.equal(published.hasUpdate, false);
...
test('hosted review remains tied to its original revision and never imports reviewer approval', async (t) => {
  ...
  const review = {
    ...
    pins: [
      {
        id: 'pin-1',
        artifactId: bundle.envelope.artifacts[0].id,
        author: { name: 'Teammate' },
        intent: 'improve',
        status: 'open',
        comment: 'Increase label contrast',
        region: { x: 0.1, y: 0.1, w: 0, h: 0 },
        viewport: bundle.envelope.artifacts[0].viewport,
        replies: [],
        createdAt: date,
        updatedAt: date,
      },
  ...
  const event = await prepareWorkspaceEvent(
    custody,
    { kind: 'review', author: 'Teammate', reviewOf: bundle.reviewOf, review },
    { reviewOf: bundle.reviewOf },
  );
  state.events.push({ ...event, sequence: 1 });
  const synced = await syncDesignShare(file, options);
  ...
  state.failNext = true;
  const queued = await publishDesignReviewMetadata(
    file,
    {
      schemaVersion: '1.0.0',
      kind: 'disposition',
  ...
  assert.equal(
    ...
    1,
    'Unsent owner intent is durable before retry.',
  );
  await syncDesignShare(file, options);
  assert.equal(record().pendingReviewMetadata.length, 0);
  assert.equal(
    readDesignFeedback(file, options.env).shared.metadataByRevision[custody.currentRevision]
      .dispositions['pin-1'].disposition,
    'accepted',
  );
  const upload = state.requests.find(
    ({ url, init }) => url.endsWith('/events') && init.method === 'POST',
  );
...
test('a mismatched hosted event receipt retains the exact owner decision for retry', async (t) => {
  ...
  const first = await publishDesignReviewMetadata(
    file,
    {
      schemaVersion: '1.0.0',
      kind: 'disposition',
  ...
  const sent = state.requests
    .filter(({ url, init }) => url.endsWith('/events') && init.method === 'POST')
...
test('unsafe custody permissions are rejected rather than loaded', async (t) => {
  ...
  assert.throws(() => getDesignShareStatus(file, options), /0600/);
});
...
test('studio supplies design share adapter and rejects cross-origin owner requests', async (t) => {
  ...
  assert.match(page, /design-share-dialog/);
  const runtime = await fetch(`${session.url}runtime.js`).then((r) => r.text());
  assert.match(runtime, /design-share-runtime/);
  const adapter = await fetch(`${session.url}api/design-share-runtime`).then((r) => r.text());
  assert.match(adapter, /Copy access token/);
  const denied = await fetch(`${session.url}api/design-share`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', origin: 'https://untrusted.test' },
...
test('recovery restores matching authority without overwriting another workspace', async (t) => {
  ...
  invalid.custody.ownerPublicKey.x = 'x'.repeat(43);
  writeFileSync(output, JSON.stringify(invalid), { mode: 0o600 });
  await assert.rejects(
    importDesignShareRecovery(file, { ...restoredOptions, input: output }),
    /owner identity/,
  );
});
...
test('custody refuses project paths and symbolic-link ancestors', async (t) => {
  ...
  assert.throws(
    () => getDesignShareStatus(file, { ...options, custodyRoot: join(root, 'credentials') }),
    /outside the project/,
  );
  const link = join(root, '..', 'linked');
  symlinkSync(join(root, '..'), link);
  await assert.rejects(
    shareDesign(file, { ...options, custodyRoot: join(link, 'credentials') }),
    /symbolic links/,
  );
});
...
test('Share design completes in the real browser without generic missing-handler controls', {
  timeout: 30000,
}, async (t) => {
  const { file, options } = await fixture(t);
  ...
  assert.match(
    await dialog.getByLabel('Design review link').inputValue(),
    /^https:\/\/share.test\/d\//,
  );
  ...
  assert.equal(
    await page
      .getByRole('button', { name: 'Share design', exact: true })
...
test('definite publication conflict preserves intent and allows an authenticated retry', async (t) => {
  const { root, file, options, state, record } = await fixture(t);
  await shareDesign(file, options);
  const original = state.workspace.currentRevision;
  writeFileSync(
    join(root, 'source/screen-1.html'),
    readFileSync(join(root, 'source/screen-1.html'), 'utf8').replace(
      '12 active tasks',
...
test('arrangement changes can be explicitly published without changing authored source revision', async (t) => {
  ...
  writeFileSync(
    join(root, '.design/studio-state.json'),
    JSON.stringify({
      stateVersion: 1,
      state: {
        positions: { [artifactId]: { x: -240, y: 650 } },
        ratings: { A: 5 },
        remix: { A: 'Private preference' },
      },
    }),
  );
