...
const forbiddenPart =
  /^(?:\.git|node_modules|\.ssh|\.local|\.design|\.codex|\.claude|\.aws|\.azure|\.config|\.env(?:\..*)?|credentials(?:\..*)?|id_rsa(?:\..*)?|id_ed25519(?:\..*)?)$/iu;
const sameFile = (a, b) =>
  a.dev === b.dev &&
  a.ino === b.ino &&
  a.size === b.size &&
...
function assertPublicContent(content) {
  ...
  while (pending.length) {
    ...
    if (localPath.test(value) || sensitive.test(value))
      throw new Error(
        'Company design output contains a local filesystem path or credential. Use share-safe screen content and review guidance.',
      );
...
export function createCompanyDesignSourceReader(file, maxBytes) {
  ...
  function readSource(path, from = root) {
    if (
      ...
      throw new Error('Company design sources must be local relative files.');
    // The initial document may be an absolute caller selection; dependencies may not.
    if (
      isAbsolute(path) &&
      resolve(path) !== selected &&
      resolve(path) !== join(root, basename(selected))
    )
      throw new Error('Absolute dependency paths cannot be published.');
    const candidate = resolve(
      path === selected ? root : from,
      path === selected ? basename(selected) : path,
    );
    ...
    if (
      ...
      name.startsWith('../') ||
      isAbsolute(name) ||
      name.split('/').some((part) => forbiddenPart.test(part))
    )
      throw new Error(
        'Company design sources cannot include secrets, internal state or files outside the design directory.',
      );
    let current = root;
    for (const part of name.split('/')) {
      current = join(current, part);
      if (lstatSync(current).isSymbolicLink())
        throw new Error('Company design source paths must not contain symbolic links.');
    }
    const before = lstatSync(candidate, { bigint: true });
    if (!before.isFile() || before.size > BigInt(maxBytes))
      throw new Error('Company design source exceeds the regular-file publication budget.');
    let descriptor, value, after;
    try {
      descriptor = openSync(candidate, constants.O_RDONLY | constants.O_NOFOLLOW);
      const opened = fstatSync(descriptor, { bigint: true });
      if (!sameFile(before, opened) || realpathSync(candidate) !== candidate)
      ...
      if (!sameFile(opened, after) || !sameFile(after, lstatSync(candidate, { bigint: true })))
        throw new Error('Company design source changed during preparation. Retry after saving.');
    } finally {
    ...
    if (prior && (prior.digest !== digest || !sameFile(prior.stat, after)))
      throw new Error('Company design source changed during preparation. Retry after saving.');
    if (!prior) {
      ...
      if (sensitive.test(value.toString('utf8')))
        throw new Error(
          'A selected design source appears to contain credentials. Remove them before sharing.',
        );
...
export function prepareCompanyDesignPublication(
  ...
  const variantCount = Array.isArray(document.variants)
    ? document.variants.filter((item) => item?.status === 'ready').length
    : 0;
  if (
  ...
  return {
    ...
    screenCount: bundle.design.screens.length,
    frameCount: bundle.design.frames.length,
    variantCount: bundle.design.variants.length,
    mediaKinds: [
      ...new Set(
