...
function reviewExportTools() {
  ...
  const quote = value => { if (typeof value !== 'string' || value.length > 16384) throw new TypeError('Review text must be a string of at most 16384 characters.'); return value; };
  ...
  const fragment = values => '#' + Object.entries(values).filter(([, value]) => value !== null && value !== undefined).map(([key, value]) => `${key}=${encodeURIComponent(value).replace(/[!'()*]/gu, char => '%' + char.charCodeAt(0).toString(16).toUpperCase())}`).join('&');
  ...
  function location(pin) {
    ...
    return { kind: region.w > 0 || region.h > 0 ? 'region' : 'point', coordinateSpace: pin.anchor ? 'anchor-normalized' : 'viewport-normalized', anchor,
      region: normalizedRegion, point, capturedViewport: viewport,
  ...
  function sourceBundle(value, fallback = {}) {
    ...
    return { bundle: object(bundle), revisionId: field(value?.revisionId ?? fallback.revisionId ?? bundle?.revision), reviewOf: digest(value?.reviewOf ?? fallback.reviewOf ?? bundle?.reviewOf) };
  }
  function flatten(input) {
    if (Array.isArray(input.feedback?.pins)) return input.feedback.pins;
    if (input.review) return list(input.review.pins).map(pin => ({ ...pin, reviewId: input.review.reviewId, reviewOf: input.review.reviewOf, revisionId: pin.revisionId ?? input.revisionId }));
    return list(input.feedback?.ledger?.reviews).flatMap(entry => list(entry.review?.pins).map(pin => ({ ...pin, reviewId: entry.review.reviewId, reviewOf: entry.review.reviewOf, stale: pin.stale || entry.stale })));
  }
  function revisionFor(pin) { return field(pin.revisionId) ?? (pin.reviewId?.startsWith('shared-') ? field(pin.reviewId.slice(7)) : null); }
  function resolveSource(pin, sources) {
    ...
    const distinct = matches.filter((item, index) => matches.findIndex(other => other.revisionId === item.revisionId && other.reviewOf === item.reviewOf) === index);
    return distinct.length === 1 ? distinct[0] : null;
  }
  function pinMetadata(metadata, pin, current) {
    const key = revisionFor(pin) ?? pin.reviewId;
    return object(metadata.byRevision?.[pin.reviewId] ?? metadata.byRevision?.[key] ?? (!metadata.byRevision && (!revisionFor(pin) || revisionFor(pin) === current.revisionId) ? metadata : {}));
  }
  ...
  function createDesignReviewExport(input = {}) {
    ...
    for (const pin of [...pins].sort(order)) {
      ...
      if (replies.length > 1000) throw new TypeError('Review export exceeds 1000 replies in one thread.');
      const thread = { id: pin.id, source: { revisionId: sourceRevisionId, reviewId, reviewOf, artifactId: pin.artifactId, navigation: fragment(refs) },
        category, originalIntent: field(pin.intent), status: field(pin.status), resolved: pin.status === 'resolved', stale: staleReasons.length > 0, staleReasons,
        author: identity(pin.author), createdAt: timestamp(pin.createdAt), updatedAt: timestamp(pin.updatedAt), comment: quote(pin.comment),
        location: location(pin), disposition: field(decision.disposition) ? { value: field(decision.disposition), explanation: quote(decision.reason ?? ''), author: typeof decision.author === 'string' ? { id: null, name: quote(decision.author) } : identity(decision.author), updatedAt: timestamp(decision.updatedAt) } : null,
      ...
      if (!groups.has(groupKey)) groups.set(groupKey, { sourceRevisionId, reviewId, reviewOf, artifactId: pin.artifactId, sourceMapping: entry ? 'original-bundle' : 'unavailable',
        screen: { id: refs.screen, title: field(screen?.title) }, direction: { id: refs.direction, label: field(direction?.label) },
        frame: { id: refs.frame, label: field(frame?.label), ...(dimensions(frame) ?? { width: null, height: null }) }, threads: [] });
    ...
    const overallNotes = reviews.filter(entry => entry.review?.overall).map(({ review }) => ({ reviewId: field(review.reviewId), reviewOf: digest(review.reviewOf), comment: quote(review.overall) })).sort((a, b) => compare(a.reviewId ?? '', b.reviewId ?? ''));
    return { kind: 'openplanr-design-review-export', schemaVersion: '1.0.0', design: { id: field(design.id), title: field(design.title) ?? 'Design review' },
      currentRevisionId: current.revisionId, currentArtifactDigest: current.reviewOf, ...(timestamp(input.generatedAt) ? { generatedAt: timestamp(input.generatedAt) } : {}),
      ...
      ], groups: orderedGroups, overallNotes };
  }

  const inline = value => String(value ?? 'Unavailable').replaceAll('\\', '\\\\').replace(/[\r\n]/gu, ' ').replace(/[\[\]<>`*#|]/gu, char => '\\' + char);
  const quoted = value => { const runs = value.match(/`+/gu) ?? []; const fence = '`'.repeat(Math.max(3, ...runs.map(run => run.length + 1))); return `${fence}text\n${value}\n${fence}`; };
  function serializeDesignReviewExport(snapshot, format = 'json') {
    if (snapshot?.kind !== 'openplanr-design-review-export' || snapshot.schemaVersion !== '1.0.0') throw new TypeError('Expected a design review export snapshot.');
    ...
    const lines = [`# ${inline(snapshot.design.title)} — review feedback`, '', `Revision: ${inline(snapshot.currentRevisionId)} · ${snapshot.summary.threads} threads · ${snapshot.summary.replies} replies · ${snapshot.summary.open} open · ${snapshot.summary.stale} stale`, '',
      ...
      '## How to use this review', '', ...snapshot.resolutionGuidance.map(value => '- ' + value), '',
    ];
    for (const group of snapshot.groups) {
      lines.push(`## ${inline(group.screen.title ?? group.screen.id ?? 'Unmapped screen')} · ${inline(group.direction.label ?? group.direction.id)} · ${inline(group.frame.label ?? group.frame.id)}`, '',
        `Source revision: ${inline(group.sourceRevisionId)} · review: ${inline(group.reviewId)}`, `Artifact: ${inline(group.artifactId)} · digest: ${inline(group.reviewOf)}`,
      ...
      for (const thread of group.threads) {
        lines.push(`### ${inline(thread.id)} · ${inline(thread.category)} · ${inline(thread.status)}${thread.stale ? ' · STALE' : ''}`, '',
          ...
          `Original intent: ${inline(thread.originalIntent)} · [Open original pin](${thread.source.navigation})`, '', quoted(thread.comment), '',
        ...
        if (thread.disposition) lines.push(`Owner disposition: ${inline(thread.disposition.value)} · ${inline(thread.disposition.author.name)} · ${inline(thread.disposition.updatedAt)}`, '', quoted(thread.disposition.explanation), '');
        for (const reply of thread.replies) lines.push(`Reply ${inline(reply.id)} — ${inline(reply.author.name)} (reviewer ID: ${inline(reply.author.id)}) · ${inline(reply.createdAt)}`, '', quoted(reply.comment), '');
      }
    }
    if (snapshot.overallNotes.length) lines.push('## Overall review notes', '', ...snapshot.overallNotes.flatMap(note => [`Review: ${inline(note.reviewId)} · digest: ${inline(note.reviewOf)}`, '', quoted(note.comment), '']));
    return lines.join('\n');
  }
...
export function renderDesignReviewExportSource() { return `globalThis.OpenPlanrDesignReviewExport = Object.freeze((${reviewExportTools.toString()})());`; }
