...
export function auditRenderedScreen() {
  ...
  const label = (node) =>
    node.getAttribute('data-planr-id') ||
    node.id ||
    `${node.tagName.toLowerCase()} “${(node.textContent || node.getAttribute('aria-label') || '').trim().replace(/\s+/gu, ' ').slice(0, 60)}”`;
    ...
    if (directText) {
      ...
      if (background && foreground) {
        ...
        const large =
          ...
          (parseFloat(style.fontSize) >= 18.666 && parseFloat(style.fontWeight) >= 700);
  ...
  for (const node of focusTargets.slice(0, 150)) {
    ...
    const outlined =
      ...
      (color(focused.outlineColor)?.[3] ?? 0) > 0;
    const changed = [
      focused.boxShadow,
 ...
 */
export async function auditDesignPage(
  page,
  { revision, entries = [], screenshotPaths = [], scenarios = [] } = {},
) {
  const issues = [],
    checkedArtifacts = [],
    screens = [];
  const expected = new Map(entries.map((entry) => [entry.artifactId, entry]));
  const add = (artifactId, rule, severity, message) =>
    issues.push({ artifactId, rule, severity, message });
  ...
  for (const frame of page.frames()) {
    ...
    try {
      ...
      artifactId =
        (await handle.getAttribute('data-planr-artifact-frame')) ??
        (await handle.getAttribute('data-planr-artifact-id'));
