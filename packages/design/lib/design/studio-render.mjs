...
const icons = {
  ...
  walkthrough:
    '<rect x="3" y="4" width="18" height="14" rx="2"/><path d="m9 22 3-4 3 4M9 9h6M9 13h3"/>',
...
export function renderDesignStudioMarkup(
  ...
  for (const entry of entries) {
    if (
      ...
      !document.variants.some(({ id, status }) => id === entry.variantId && status === 'ready') ||
  ...
  const payload = {
    ...
    staticArtifacts: envelope.artifacts
      .filter(
        (artifact) =>
          artifact.kind !== 'html' ||
  ...
  html = html.replace(/<header class="planr-toolbar">[\s\S]*?<\/header>/, toolbar(document));
  html = html.replace(
