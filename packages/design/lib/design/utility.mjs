...
export function verifyDesignDocument(file, report) {
  ...
  const coverage = current.entries.every((entry) => {
    ...
    return (
      checkedArtifacts.includes(entry.artifactId) &&
      matches.length === 1 &&
      matches[0].screenId === entry.screenId &&
      matches[0].viewport?.width === frame.width &&
      matches[0].viewport?.height === frame.height &&
      Number.isInteger(matches[0].checkedElements) &&
      matches[0].checkedElements > 0
    );
  });
  ...
  const journeys =
    ...
    report.scenarios.every(
      (item) =>
        item?.status === 'passed' &&
        [item.name, item.id].some((value) => typeof value === 'string' && value.trim().length > 0),
    );
  const issues = [
  ...
  const status =
    issues.some((item) => item?.severity === 'error') || report.status === 'failed'
      ? 'failed'
      : coverage && images.length > 0 && journeys && report.status !== 'unverified'
        ? 'verified'
        : 'unverified';
  const saved = {
    ...report,
...
export async function designUtility(
  argv,
  {
  ...
  if (command === 'handoff') {
    ...
    if (action === 'inspect') result = readDesignHandoff(file);
      ...
      result = await updateDesignHandoff(file, {
        ...input,
        action,
        revision: input.revision ?? snapshot.revision,
        version: input.version ?? snapshot.draft?.version ?? 0,
      });
    }
  }
  ...
  if (
    command === 'manage' &&
    !['rotate', 'pause', 'resume', 'revoke', 'delete', 'recovery', 'restore'].includes(flags.action)
  )
    throw new Error('manage requires --action rotate|pause|resume|revoke|delete|recovery|restore.');
  if (command === 'manage')
    result =
      flags.action === 'restore'
        ? await importDesignShareRecovery(file, { input: flags.input })
        : flags.action === 'recovery'
          ? await exportDesignShareRecovery(file, { output: flags.output })
          : await manageDesignShare(file, flags.action);
  ...
  if (command === 'export') {
    const current = currentDesign(file),
      view = flags.view ?? current.document.defaultView;
  ...
  if (command === 'feedback') {
    ...
    if (action === 'inspect') result = readDesignFeedback(file, env);
      ...
      const current = currentDesign(file),
        saved = readJson(join(current.root, '.design/studio-state.json'), {
          state: {},
          stateVersion: 0,
        });
      ...
      const selected = [variant],
        rejected = (saved.state.preferences?.rejected ?? []).filter((id) => id !== variant);
