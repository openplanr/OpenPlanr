...
export function projectDesignDeliveryStatus({
  handoff,
  currentHandoff,
  lineage = null,
  tasks = [],
  ...
  const stale =
    ...
    (currentHandoff
      ? !identityMatches(currentHandoff, handoff) || currentHandoff.status !== 'approved'
      : false) ||
  ...
  if (
    lineage &&
    (relevantTasks.some(({ status }) => ['in_progress', 'addressed'].includes(status)) ||
      (shipClosure && !['passed', 'blocked'].includes(shipClosure.state)))
  )
    phase = 'implementing';
  if (
    lineage &&
    shipClosure?.state === 'passed' &&
    relevantTasks.length > 0 &&
    relevantTasks.every(({ status }) => status === 'done')
  )
    phase = 'verified';
  ...
  const result = Object.freeze({
    ...
    blocked,
    detail: stale
      ? 'The approved design package no longer matches current planning context.'
      : blocked
        ? 'Delivery has a recorded blocker.'
        : phase === 'verified'
          ? 'The separate Ship workflow recorded a passing closure for every mapped task.'
          : phase === 'implementing'
            ? 'A separate Ship workflow is implementing mapped tasks.'
            : phase === 'planned'
              ? 'Plan recorded complete requirement lineage.'
  ...
  if (canonicalizeJson(handoff) !== inputDigest)
    throw new TypeError('Delivery projection mutated the approved implementation handoff.');
  return result;
}
