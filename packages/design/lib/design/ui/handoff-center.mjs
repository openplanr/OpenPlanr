...
/**
 * Mounts the owner Handoff Center over the review experience's handoff bridge.
 * Returns its API, or null without a bridge or shell root.
 */
export function mountDesignHandoffCenter({ bridge: handoffBridge, root: shellRoot }) {
  ...
  const plainPackage = (value) => ({
    ...
    requirements: (value.requirements || []).map(({ id: _id, ...requirement }) =>
  ...
  const approvedReview = () =>
    Boolean(state.review?.current && state.review?.draft?.status === 'approved');
  ...
  const statusFor = (id) => {
    if (
      state.errors[id === 'readiness' ? 'readiness' : id === 'review' ? 'review' : 'implementation']
    )
      return 'attention';
    ...
    if (id === 'approval')
      return approvedPackage()
        ? 'pass'
        : currentPackage()?.status || (packageDraft() ? 'attention' : 'blocked');
    return approvedPackage() ? 'ready' : 'blocked';
  };
  const badge = (value) =>
    node('span', labels[value] || value, { class: 'design-handoff-badge', 'data-status': value });
  const paragraph = (text, className = '') =>
    node('p', text, className ? { class: className } : {});
  ...
  function renderReadiness(content) {
    ...
    summary.append(
      ...
      node(
        'strong',
        value.status === 'ready'
          ? 'This design can move into implementation packaging.'
          : `${value.blockers?.length || 0} blocking checks remain.`,
      ),
    );
  ...
  function renderReview(content) {
    ...
    summary.append(
      ...
      node(
        ...
        draft.content?.summary ||
          (response.current
            ? 'Review decisions match this design.'
            : 'The design or feedback changed after this handoff was prepared.'),
      ),
    );
    content.append(summary);
    ...
    const label = approvedReview()
      ? 'Continue to implementation package'
      : response.current
        ? 'Review and approve decisions'
        : 'Regenerate current decisions';
    content.append(
      action(
        label,
        () => {
  ...
  function packageEditor(source, { regenerate = false } = {}) {
    ...
    const editors = editable.requirements.map((requirement, index) => {
      ...
      card.append(
        node('legend', `${requirement.kind.replaceAll('-', ' ')} requirement ${index + 1}`),
      );
      ...
      const verification = node('textarea', '', {
        required: '',
        maxlength: '16384',
        'aria-label': `Requirement ${index + 1} verification`,
      });
    ...
    form.addEventListener('submit', (event) => {
      ...
      void perform(
        ...
        regenerate
          ? 'A new package version is ready for approval.'
          : 'Implementation package saved.',
      );
    });
    return form;
  }
  ...
  function renderPackage(content) {
    ...
    const response = state.implementation,
      draft = response?.draft;
        ...
        action(
          ...
          () => {
            ...
            void perform(
              'Revoking approval…',
              async () =>
                bridge.updateImplementationHandoff({
                  action: 'revoke',
      ...
      content.append(
        empty(
          current?.status === 'revoked'
            ? 'The current approval was revoked'
            : 'This package cannot be approved yet',
  ...
  async function copyText(value, output) {
    try {
      ...
      output.textContent =
        'Invocation copied. Paste it into your active coding agent when you are ready.';
    } catch {
    ...
    if (!approvedPackage()) {
      content.append(
        empty(
          ...
          currentPackage()?.status === 'revoked'
            ? 'This package was revoked. Prepare and approve a current version before planning.'
            : 'Complete owner approval before preparing the Plan invocation.',
        ),
      );
      content.append(
        action(
        ...
        async () => {
          ...
          try {
            ...
            if (typeof bridge.continueToActiveHost === 'function') {
              ...
              result.textContent =
                'The approved package was handed to the active host. Invoke Plan there to continue.';
            } else {
              const host = String(context.host || '').toLowerCase();
              const key = host.includes('claude')
                ? 'claudeCode'
                : host.includes('codex')
                  ? 'codex'
                  : host.includes('cursor')
                    ? 'cursor'
                    : 'fallback';
  ...
  async function refresh() {
    ...
    setStatus(
      Object.keys(state.errors).length
        ? 'Some handoff state is unavailable. The current design remains usable.'
