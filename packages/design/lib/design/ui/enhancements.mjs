...
import { mountDesignHandoffCenter } from './handoff-center.mjs';

/**
 * Mounts the review experience onto a mounted studio and stage.
 * Returns the experience API and the owner Handoff Center.
 */
export function mountDesignEnhancements({ payload, studio: designStudio, stage: artifactStage }) {
  ...
  const node = (tag, text = '', attrs = {}) => {
    ...
    for (const [key, entry] of Object.entries(attrs))
      if (entry !== undefined && entry !== null && entry !== false) value.setAttribute(key, entry);
    return value;
  };
  ...
  const write = (key, value) => {
    try {
      ...
      /* Personal preferences remain usable in memory. */
    }
  };
  const avatarColors = [
    ['teal', 'Teal', '#0f766e'],
  ...
  const colorFor = (value) =>
    [...String(value)].reduce((n, char) => (n * 31 + char.codePointAt(0)) >>> 0, 0) %
    avatarColors.length;
  const normalizedProfile = (value) => ({
    name: typeof value?.name === 'string' ? value.name.trim().slice(0, 160) : '',
    color: avatarColors.some(([key]) => key === value?.color)
      ? value.color
      : avatarColors[colorFor(value?.name || '')][0],
  });
  let profile = normalizedProfile(read('profile', null)),
  ...
  const applyTheme = () => {
    const resolved =
      themePreference === 'system' ? (systemTheme?.matches ? 'light' : 'dark') : themePreference;
  ...
  const reviewerAudience = () =>
    options().audience === 'reviewer' || location.pathname.startsWith('/d/');
  const commentTypes = [
  ...
  const categoryLabel = (value) =>
    commentTypes.find(([id]) => id === value)?.[1] ||
    { fix: 'Fix', improve: 'Improve' }[value] ||
    value;
  const owner = () => experience?.capabilities?.owner === true || experience?.owner === true;
  const state = () => studio.getState();
  const entry = (screenId = state().screenId) =>
    payload.entries.find(
      (item) =>
        item.screenId === screenId &&
        item.variantId === state().variantId &&
        item.frameId === state().frameId,
    );
  const pinScope = (pin) => {
    const reviewId = pin.reviewId || stage.review.getState().review?.reviewId;
    const revisionId =
      pin.revisionId || (reviewId?.startsWith('shared-') ? reviewId.slice(7) : undefined);
  ...
  function about(auto = false) {
    ...
    content.append(
      node(
        'p',
        brief?.purpose ||
          'Explore this design and leave feedback on the screens that matter to you.',
      ),
    );
    if (brief?.audience)
  ...
  function avatar(author, { color, large = false } = {}) {
    ...
    const own =
      typeof options().isOwnReviewAuthor === 'function'
        ? options().isOwnReviewAuthor(author)
  ...
  function installProfileAndTheme() {
    ...
    destroyers.push(
      stage.review.controller.subscribe((state, change) => {
        if (change.type !== 'identity') return;
        profile = normalizedProfile({ ...profile, name: state.identity?.name || '' });
        write('profile', profile);
        profileChanged();
      }),
    );
    ...
    const restore = () => {
      ...
      if (saved) {
        try {
          ...
          /* Invalid personal storage must not block the studio. */
        }
      }
    };
    restore();
  ...
  function installReadability() {
    ...
    const apply = () => {
      root.style.setProperty('--design-nav-width', `${clamp(widths.left, 200, 360)}px`);
      root.style.setProperty('--planr-review-rail-width', `${clamp(widths.right, 300, 480)}px`);
    };
    apply();
    for (const [side, panel, min, max] of [
      ...
      panel.append(handle);
      const set = (value) => {
        widths[side] = clamp(value, min, max);
        apply();
      ...
      on(handle, 'keydown', (event) => {
        ...
        const delta =
          ...
          (side === 'left' ? 1 : -1) *
          (event.shiftKey ? 40 : 8);
        set(event.key === 'Home' ? min : event.key === 'End' ? max : widths[side] + delta);
      });
  ...
  function installThumbnails() {
    ...
    const paintable = (current) => {
      ...
      return (
        state().navOpen &&
        frame &&
        !frame.closest('.planr-artifact-panel')?.hidden &&
    ...
    const renderThumb = async (screenButton) => {
      ...
      const fingerprint = (experience?.fingerprints || payload.fingerprints)?.find(
        (item) =>
          item.screenId === current.screenId &&
          item.frameId === current.frameId &&
      ...
      while (queue.length && !disposed) {
        ...
        try {
          ...
          if (
            ...
            !/^data:image\/png;base64,/u.test(result.dataUrl) ||
            result.dataUrl.length > 360000
          )
            continue;
          ...
          /* The labelled screen button remains usable when capture is unavailable. */
        } finally {
          queued.delete(key);
        }
    ...
    on(root, 'planr:design-ready', start);
    if (stage.getState().status === 'ready') setTimeout(start, 200);
  }
  let updateFilters = () => {};
  function categoryPicker(initial, onChange) {
    ...
    const sync = () => {
      for (const value of qa('[role="radio"]', group)) {
        ...
        value.tabIndex =
          selected ||
          (!commentTypes.some(([id]) => id === field.value) && value === q('[role="radio"]', group))
            ? 0
            : -1;
      }
    ...
    group.addEventListener('keydown', (event) => {
      if (!['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key))
        return;
      event.preventDefault();
      const buttons = qa('[role="radio"]', group),
  ...
  function installComposerCategory() {
    ...
    const available = () =>
      typeof options().updateReviewMetadata === 'function' ||
      (owner() && (options().handoffUrl || options().updateHandoff));
    const flush = async () => {
      ...
      try {
        for (const [pinId, category] of Object.entries(outbox)) {
          ...
          const result = options().updateReviewMetadata
            ? await options().updateReviewMetadata(body)
            : await request('updateHandoff', 'handoffUrl', body);
    ...
    on(root, 'planr:artifact-annotation-draft', () => {
      ...
      const { group, field: category } = categoryPicker('suggestion', (value) => {
        const intent =
          { question: 'question', suggestion: 'improve', 'change-request': 'fix', blocker: 'fix' }[
            value
          ] || 'improve';
      ...
      picker.after(label);
      q('[data-planr-composer-comment]', composer).placeholder =
        'Describe your feedback for this area…';
      composer.addEventListener(
        'submit',
        () => {
          if (
            !q('[data-planr-composer-comment]', composer).value.trim() ||
            !q('[data-planr-composer-identity]', composer).value.trim()
          ) {
            pending = null;
            return;
          }
  ...
  function installReview() {
    ...
    const activity = (pin) =>
      [pin.updatedAt, pin.createdAt, ...(pin.replies || []).map((reply) => reply.createdAt)]
        .filter(Boolean)
        .sort()
        .at(-1) || '';
    const unread = (pin) => (seen[pin.id] || '') < activity(pin);
    const category = (pin) =>
      pinMetadata(pin).categories?.[pin.id] ||
      (pin.intent === 'improve' ? 'suggestion' : pin.intent);
    const filter = (pin) => {
      ...
      return (
        (scopeFilter.value === 'all' || screenId === state().screenId) &&
        (statusFilter.value === 'all' ||
          (statusFilter.value === 'resolved'
            ? pin.status === 'resolved'
            : pin.status !== 'resolved')) &&
        (type.value === 'all' || category(pin) === type.value) &&
        (!unreadOnly.checked || unread(pin)) &&
        (!search.value.trim() ||
          [
            pin.comment,
            pin.author.name,
    ...
    const decorate = ({ element, pin }) => {
      ...
      const canCategorize =
        (typeof options().updateReviewMetadata === 'function' ||
          (owner() && (options().handoffUrl || options().updateHandoff))) &&
        (typeof options().canCategorizePin !== 'function' || options().canCategorizePin(pin.id));
      if (canCategorize) {
        const edit = button(
          label(pin),
          async () => {
    ...
    const updateUnread = () => {
      const count = (stage.review.getState().review?.pins || []).filter(unread).length;
      unreadCount.textContent = `Unread · ${count}`;
    };
    updateFilters = () => {
      if (disposed) return;
      updateUnread();
  ...
  function exportReviews() {
    ...
    const download = async (format) => {
      ...
      try {
        ...
        const snapshot = canLoad
          ? await options().loadReviewExport({ scope: scopeChoice.value })
        ...
        const text = serializeDesignReviewExport(snapshot, format),
  ...
  function installCanvas() {
    ...
    function draw() {
      ...
      map.hidden =
        current.view !== 'canvas' || !minimapOpen || root.dataset.designPresentation === 'true';
      toggleMap.setAttribute('aria-pressed', String(minimapOpen));
      if (map.hidden) return;
      const boards = payload.entries
        .filter((item) => !stage.getPanel(item.artifactId)?.hidden)
        .map((item) => {
          const panel = stage.getPanel(item.artifactId),
            frame = design.frames.find((value) => value.id === item.frameId);
          return {
            ...item,
      ...
      const minX = Math.min(...boards.map((item) => item.x)),
        minY = Math.min(...boards.map((item) => item.y));
      const width = Math.max(...boards.map((item) => item.x + item.width)) - minX,
        height = Math.max(...boards.map((item) => item.y + item.height)) - minY;
    ...
    const move = (event) => {
      ...
      const x = ((event.clientX - bounds.left) * 200) / bounds.width,
        y = ((event.clientY - bounds.top) * 128) / bounds.height;
      const worldX = geometry.minX + (x - geometry.offsetX) / geometry.scale,
        worldY = geometry.minY + (y - geometry.offsetY) / geometry.scale;
      studio.setCamera({
    ...
    const restore = () => {
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => {
          ...
          const target = hiddenMenu
            ? q('summary', hiddenMenu)
            : focusBeforePresentation?.isConnected &&
                focusBeforePresentation.getClientRects().length
              ? focusBeforePresentation
              : q('summary', more);
          target?.focus();
          schedule();
        }),
  ...
  function installInspector() {
    ...
    const selectTarget = async (value, overlay, point) => {
      ...
      try {
        const result = point
          ? await bridge?.inspectAt?.(point.x, point.y)
          : await bridge?.inspect?.(value);
        if (!result)
          throw new Error(
    ...
    for (const value of payload.entries) {
      const panel = stage.getPanel(value.artifactId),
        frame = stage.getFrame(value.artifactId);
      ...
      on(overlay, 'click', (event) => {
        ...
        const rect = frame.getBoundingClientRect(),
          viewport = design.frames.find((item) => item.id === value.frameId);
        studio.selectEntry(value);
        void selectTarget(value, overlay, {
          x: clamp(((event.clientX - rect.left) * viewport.width) / rect.width, 0, viewport.width),
          y: clamp(
    ...
    on(root, 'planr:design-tool-change', (event) =>
      applyInspector(event.detail.tool === 'inspect'),
    );
  ...
  function renderInspector(computed = inspectedTarget) {
    ...
    if (computed) {
      ...
      detail.append(
        ...
        node(
          'p',
          asset
            ? 'This is an image or vector asset. Its depicted component internals cannot be inspected. Use authored implementation guidance below.'
            : 'Computed in this frame. These values describe the current render, not a reusable component contract.',
          { class: 'design-muted' },
        ),
      );
    ...
    const implementation =
      (experience?.reviewContext || payload.reviewContext)?.implementation || {};
    ...
    try {
      ...
      for (const [index, item] of revisions.entries()) {
        ...
        card.append(
          node(
            'strong',
            `${index === 0 ? 'Latest · ' : ''}${item.createdAt ? new Date(item.createdAt).toLocaleString() : id.slice(0, 12)}`,
          ),
        );
        ...
        if (item.fingerprints?.length && previous?.fingerprints?.length) {
          ...
          const changed = Object.values(diffs).filter((value) => value !== 'Unchanged');
          card.append(
            node(
              'span',
              changed.length
                ? `${changed.length} screens changed · ${[...new Set(changed)].join(', ')}`
                : 'Design unchanged · runtime or publication only',
        ...
        if (revisions.length > 1)
          card.append(
            button(
              ...
              {
                disabled:
                  id === (value.currentRevision || revisions[0].revision || revisions[0].id)
                    ? ''
                    : undefined,
              },
            ),
  ...
  function screenDifferences(before, after) {
    ...
    const signature = (items, field) =>
      JSON.stringify(
        items
          .map((item) => [`${item.variantId}:${item.frameId}`, item[field]])
          .sort((a, b) => a[0].localeCompare(b[0])),
      );
    return Object.fromEntries(
      ids.map((id) => {
        const a = before.filter((item) => item.screenId === id),
          b = after.filter((item) => item.screenId === id);
        return [
          id,
          !a.length
            ? 'Added'
            : !b.length
              ? 'Removed'
              : signature(a, 'contentDigest') !== signature(b, 'contentDigest')
                ? 'Changed'
                : signature(a, 'guidanceDigest') !== signature(b, 'guidanceDigest')
                  ? 'Notes'
                  : 'Unchanged',
        ];
      }),
  ...
  async function updateScreenBadges() {
    if (badgeRevision === revision() || !(options().revisionsUrl || options().listRevisions))
      return;
    badgeRevision = revision();
    try {
      const value = options().listRevisions
        ? await options().listRevisions({ brief: true })
        : await request('listRevisions', 'revisionsUrl');
      const revisions = Array.isArray(value) ? value : value.revisions || [];
      ...
      const previous = revisions[index + 1];
      if (!previous) return;
      const old = previous.fingerprints?.length
        ? previous
        : await loadRevision(previous.revision || previous.id);
      ...
      /* History retains an explicit retry state without blocking the current design. */ badgeRevision =
        null;
    }
  }
  async function compareRevisions(beforeId, afterId) {
    ...
    try {
      ...
      beforeId ||=
        revisions.find((item) => (item.revision || item.id) !== afterId)?.revision ||
        revisions.find((item) => (item.revision || item.id) !== afterId)?.id ||
        afterId;
      ...
      const beforeSelect = select('Before revision', revisionChoices, beforeId),
        afterSelect = select('After revision', revisionChoices, afterId);
      ...
      const screenSelect = select('Comparison screen', [], ''),
        frameSelect = select('Comparison frame', [], ''),
        variantSelect = select('Comparison direction', [], '');
      const controls = node('div', '', { class: 'design-comparison-controls' });
      controls.append(screenSelect, frameSelect, variantSelect);
      content.append(controls);
      const summary = node('p', '', { class: 'design-comparison-summary' }),
        boards = node('div', '', { class: 'design-comparison-boards' });
      ...
      const title = (id) =>
        after.design.screens.find((item) => item.id === id)?.title ||
        before.design.screens.find((item) => item.id === id)?.title ||
        id;
      const render = async () => {
        ...
        const matches = (item) =>
          item.screenId === selectedScreen &&
          item.frameId === selectedFrame &&
          item.variantId === selectedVariant;
        const a = before.fingerprints?.find(matches),
          b = after.fingerprints?.find(matches);
        const beforeEntry = before.entries.find(matches),
          afterEntry = after.entries.find(matches);
        summary.textContent = !beforeEntry
          ? 'Added screen, frame or direction'
          : !afterEntry
            ? 'Removed screen, frame or direction'
            : !a || !b
              ? 'Rendered-output comparison · authored source fingerprints are unavailable for this revision'
              : a.contentDigest !== b.contentDigest
                ? 'Product design changed'
                : a.guidanceDigest !== b.guidanceDigest
                  ? 'Guidance changed · product design unchanged'
                  : 'Product design and guidance unchanged · publication or studio runtime only';
        const windows = [];
        for (const [name, bundle, id, selectedEntry] of [
          ...
          const viewport =
            artifact.viewport || bundle.design.frames.find((item) => item.id === selectedFrame);
          ...
          for (const [index, pin] of comments.entries()) {
            ...
            const marker = button(
              ...
              },
              {
                ...
                'aria-label': `Read ${name.toLowerCase()} comment ${index + 1}`,
              },
            );
        ...
        for (const windowNode of windows)
          windowNode.addEventListener('scroll', () => {
            ...
            for (const other of windows)
              if (other !== windowNode)
                other.scrollTop =
                  (windowNode.scrollTop /
                    Math.max(1, windowNode.scrollHeight - windowNode.clientHeight)) *
        ...
        status.textContent =
          'Read-only comparison · scrolling is synchronized. Return to the design to interact or comment.';
      };
      const loadPair = async () => {
        ...
        try {
          ...
          [before, after] = pair;
          const ids = [...new Set([...after.design.screenOrder, ...before.design.screenOrder])];
          choices(
            screenSelect,
          ...
          const variants = [
            ...new Map(
              [...after.design.variants, ...before.design.variants]
  ...
  async function reviewHandoff() {
    ...
    const render = () => {
        ...
        node(
          ...
          response.current
            ? draft.status === 'approved'
              ? 'Approved for this design and feedback snapshot.'
              : 'Ready for your review.'
            : 'Outdated: the design or feedback changed. Regenerate before approval.',
      ...
      for (const [key, title] of [
        ...
        for (const item of draft.content[key] || []) {
          const field = node('textarea', '', {
            'aria-label': `${title}: ${item.pinId || 'note'}`,
            maxlength: '8192',
          });
            ...
            group.append(
              button(
                ...
                () => {
                  ...
                  if (
                    (item.revisionId && item.revisionId !== revision()) ||
                    (item.reviewOf && item.reviewOf !== currentReview?.reviewOf) ||
                    (item.reviewId && item.reviewId !== currentReview?.reviewId)
                  ) {
                    void history();
                    announce(
                  ...
                  if (pin) {
                    const value = payload.entries.find(
                      (value) => value.artifactId === pin.artifactId,
                    );
      ...
      const regenerate = button('Regenerate from reviews', () => {
        if (
          !edited ||
          window.confirm('Replace your unsaved handoff edits with current review decisions?')
        )
      ...
      try {
        ...
        status.textContent =
          input.action === 'approve'
            ? 'Handoff approved. Invoke Plan separately when you are ready.'
            : 'Draft saved.';
        updateFilters();
      } catch (error) {
        status.textContent = error.message;
  ...
  async function refreshExperience() {
    ...
    try {
      ...
      experience = {
        ...
        capabilities: {
          owner: Boolean(options().handoffUrl),
          revisions: Boolean(options().revisionsUrl || options().listRevisions),
          handoff: Boolean(options().handoffUrl || options().updateHandoff),
        },
      };
    }
    if (disposed) return;
    ...
    if (actionBar.dataset.signature !== signature) {
      ...
      if (experience.capabilities?.revisions || options().listRevisions || options().revisionsUrl)
        actionBar.append(button('Revision history', history));
      if (owner() && (options().handoffUrl || options().updateHandoff))
  ...
  function mount() {
    ...
    const bridge = Object.freeze({
      ...
      updateReviewHandoff: (input) =>
        request('updateHandoff', 'handoffUrl', { ...input, revision: revision() }),
      loadImplementationHandoff: () =>
        request('loadImplementationHandoff', 'implementationHandoffUrl'),
      updateImplementationHandoff: (input) =>
        request('updateImplementationHandoff', 'implementationHandoffUrl', input),
      continueToActiveHost:
        typeof options().continueToPlan === 'function'
          ? (handoff) => options().continueToPlan(handoff)
          : null,
      context: () => ({
        designId: design.id,
        title: design.title,
        revision: revision(),
        host: options().activeHost || options().host || '',
      }),
    });
