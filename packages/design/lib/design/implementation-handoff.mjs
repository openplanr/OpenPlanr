...
export function deriveImplementationRequirementId(requirement) {
  const hexadecimal = createHash('sha256')
    ...
    .digest('hex');
  const numeric = (BigInt(`0x${hexadecimal}`) % 1_000_000_000_000n).toString(10).padStart(12, '0');
  return `REQ-${numeric}`;
}
...
export function assertImplementationHandoffProjection(value) {
  assertDesignImplementationHandoff(value);
  if (
    value.sources.map((source) => source.id).join('\n') !==
    [...value.sources]
      .sort((left, right) => left.id.localeCompare(right.id))
  ...
  if (
    value.requirements.map((item) => item.id).join('\n') !==
    [...value.requirements]
      .sort((left, right) => left.id.localeCompare(right.id))
      .map((item) => item.id)
      .join('\n')
...
function discoverAnchor(bytes, anchor) {
  ...
  const values = anchor.elementId
    ? [anchor.elementId]
    : anchor.pinId
      ? [anchor.reviewId, anchor.pinId]
      : anchor.section
        ? [anchor.section]
        : [anchor.screenId];
...
export function importImplementationHandoffPackage(input, { resolveSource } = {}) {
  ...
  if (
    Buffer.byteLength(input.json) > MAX_PACKAGE_BYTES ||
    Buffer.byteLength(input.markdown) > MAX_PACKAGE_BYTES
  )
