...
export function loadReviewContext(root, document, { readSource } = {}) {
  ...
  const context = JSON.parse(
    readSource
      ? readSource('review-context.json', root).value.toString('utf8')
      : readFileSync(resolveLocalDocumentFile(root, 'review-context.json'), 'utf8'),
  );
  ...
  if (
    /(?:file:\/\/|\/(?:Users|home|private|tmp|var|etc|opt|Volumes)\/|[A-Za-z]:\\\\|\\\\\\\\)/u.test(
  ...
  return {
    ...
    design: {
      ...
      screens: document.screens.map((screen) =>
        pick(screen, ['id', 'title', 'description', 'anchors']),
      ),
      ...
      variants: document.variants
        ...
        .map((variant) => pick(variant, ['id', 'label', 'status'])),
    },
...
export function listDesignRevisions(file) {
  ...
  const revisions = readdirSync(join(root, '.design/revisions'))
    .filter((name) => /^[a-f0-9]{64}$/u.test(name))
    .map((revision) => {
      const value = JSON.parse(
        readFileSync(join(root, '.design/revisions', revision, 'render.json'), 'utf8'),
      );
      return {
        revision,
