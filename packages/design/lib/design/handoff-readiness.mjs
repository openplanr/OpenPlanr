...
const optionalDigest = (value, label) =>
  value === undefined || value === null ? undefined : normalizeDigest(value, label);
const plainClone = (value) =>
  value === undefined ? undefined : JSON.parse(canonicalizeJson(value));
...
function normalizedPins(review, revision) {
  ...
  for (const pin of pins) {
    ...
    if (pin.anchorCount !== undefined && pin.anchorCount !== 1)
      throw new TypeError('Review comments must identify exactly one anchor.');
  }
  return pins
    .sort((left, right) =>
      `${left.revisionId ?? left.reviewId}:${left.id}`.localeCompare(
    ...
    .map((pin) => ({
      ...
      current:
        ...
        (!revision ||
          !pin.revisionId ||
          normalizeDigest(pin.revisionId, 'review revision') === revision),
    }));
}
...
function reviewState(input, revision) {
  ...
  return {
    ...
    stale:
      input.current === false ||
      (recordedRevision !== undefined && recordedRevision !== revision) ||
      pins.some((pin) => !pin.current),
...
function handoffState(handoff, designId, revision, selectedVariant) {
  ...
  if (
    ...
    !handoff.approval ||
    handoff.approval.contentHash !== handoff.contentHash
  )
    return 'blocked';
  const basisRevision = optionalDigest(handoff.basis?.sourceRevision, 'review handoff revision');
  if (
    handoff.current === false ||
    handoff.basis?.designId !== designId ||
    basisRevision !== revision ||
    handoff.basis?.selectedVariant !== selectedVariant
  )
    return 'stale';
  return 'pass';
}
...
export function compileDesignHandoffReadiness(input) {
  ...
  if (
    !document ||
    document.kind !== 'openplanr-design-document' ||
  ...
  const revision =
    source.sourceRevision === null || source.sourceRevision === undefined
      ? null
  ...
  const directionStatus = !direction.ready
    ? 'blocked'
    : handoffDirection && handoffDirection !== direction.selected
      ? 'stale'
      : 'pass';
  checks.push(
  ...
  if (specification?.complete === true) {
    ...
    specificationStatus =
      revision && specificationRevision && specificationRevision !== revision ? 'stale' : 'pass';
  }
  checks.push(
    checked(
      'design-specification',
  ...
  const dispositionStatus =
    invalidDisposition || review.ambiguous ? 'blocked' : undecided.length ? 'attention' : 'pass';
  checks.push(checked('review-dispositions', dispositionStatus, reviewRefs));

  const openBlockers = review.pins.filter(
    (pin) =>
      pin.current &&
      ['blocker', 'change-request'].includes(pin.category) &&
      !['accepted', 'deferred', 'rejected'].includes(pin.disposition),
  );
  ...
  const blockers = checks
    .filter((item) => ['blocked', 'stale'].includes(item.status))
    .map((item) => item.id);
  const nextActions = checks
    .filter((item) => item.status !== 'pass')
    .map((item) => item.recoveryAction);
