...
export function buildRegistryRoutingCases(registry) {
  if (!Array.isArray(registry?.skills)) throw new TypeError('registry must contain a skills array.');
  return Object.freeze(registry.skills.flatMap((skill) => {
    ...
    return [
      { ...base, id: `${skill.skillId}:positive`, kind: 'positive', input: primary, expectedSkillId: skill.skillId },
      { ...base, id: `${skill.skillId}:paraphrase`, kind: 'paraphrase', input: paraphrase, expectedSkillId: skill.skillId },
      { ...base, id: `${skill.skillId}:near-miss`, kind: 'near-miss', input: nearMiss, forbiddenSkillId: skill.skillId },
      { ...base, id: `${skill.skillId}:ambiguous`, kind: 'ambiguous', input: `${primary}. Some optional details remain ambiguous, but that is the requested outcome.`, expectedSkillId: skill.skillId },
      { ...base, id: `${skill.skillId}:explicit`, kind: 'explicit', input: `Use $${skill.skillId} for this request`, expectedSkillId: skill.skillId, expectedReason: 'explicit-name' },
      { ...base, id: `${skill.skillId}:unrelated`, kind: 'unrelated', input: 'Translate this unrelated paragraph into French', forbiddenSkillId: skill.skillId },
      { ...base, id: `${skill.skillId}:optional-context`, kind: 'optional-context', input: `${primary} using the current repository and any available project context`, expectedSkillId: skill.skillId },
    ];
  }));
}
...
export function evaluateRoutingCorpus({ registry, cases, threshold = 0.95 } = {}) {
  ...
  const families = new Map(registry.skills.map(({ family }) => [family, {
    family,
    truePositive: 0,
    falsePositive: 0,
    falseNegative: 0,
  ...
  const results = cases.map((fixture) => {
    ...
    const pass = (fixture.forbiddenSkillId
      ? actual.skillId !== fixture.forbiddenSkillId
      : actual.status === expectedStatus && (expectedSkillId === null || actual.skillId === expectedSkillId))
  ...
  const familyMetrics = [...families.values()]
    .filter(({ truePositive, falsePositive, falseNegative }) => (
  ...
  const falsePositives = results.filter(({ actualSkillId, expectedSkillId, forbiddenSkillId }) => (
    forbiddenSkillId === null && actualSkillId !== null && actualSkillId !== expectedSkillId
  ));
  const falseNegatives = results.filter(({ expectedSkillId, actualSkillId }) => (
  ...
  const coverage = registry.skills.map(({ skillId }) => {
    ...
    return { skillId, cases: relevant.length, kinds, missingKinds, pass: missingKinds.length === 0 && relevant.every(({ pass }) => pass) };
  });
  return Object.freeze({
    kind: 'skill-routing-evaluation',
    schemaVersion: '1.0.0',
    threshold,
    passed: failedFamilies.length === 0 && results.every(({ pass }) => pass) && coverage.every(({ pass }) => pass),
    totals: {
