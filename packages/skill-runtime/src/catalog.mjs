...
const generatedSkillMembership = JSON.parse(
  readFileSync(new URL('../contributions/canonical-skills.json', import.meta.url), 'utf8'),
);
...
export function assertSafeRelativePath(path, label = 'path') {
  if (typeof path !== 'string' || path.length === 0 || isAbsolute(path)) {
    fail('E_SKILL_PATH_INVALID', `${label} must be a non-empty repository-relative path.`, {
      path,
    });
  }
  const parts = path.split(/[\\/]/u);
  if (parts.some((part) => part === '' || part === '.' || part === '..')) {
    fail('E_SKILL_PATH_INVALID', `${label} may not contain empty, current, or parent segments.`, {
      path,
    });
  }
  return path;
...
export function validateContributionGraph(manifests, { expectedSkillIds, repoRoot } = {}) {
  ...
  for (const manifest of manifests) {
    ...
    if (!/^[a-z][a-z0-9-]*$/u.test(manifest.id) || contributionIds.has(manifest.id)) {
      fail(
        'E_SKILL_CONTRIBUTION_ID_INVALID',
        `Contribution id ${String(manifest.id)} is invalid or duplicated.`,
      );
    }
    ...
    for (const row of manifest.skills) {
      ...
      if (
        ...
        row.commands.some((command) => !/^[a-z][a-z0-9-]*$/u.test(command))
      ) {
  ...
  if (duplicates.length > 0) {
    fail('E_SKILL_ID_DUPLICATE', 'A canonical skill is contributed more than once.', {
  ...
  if (expectedSkillIds) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      fail(
        'E_SKILL_MEMBERSHIP_INVALID',
        'Canonical skill membership differs from the source registry.',
        { expected, actual },
      );
    }
  }
...
export function validateRoleContributions(
  manifest,
  { expectedRoleIds = EXPECTED_ROLE_IDS, repoRoot } = {},
) {
  if (
    manifest?.kind !== 'openplanr-role-contributions' ||
  ...
  for (const role of manifest.roles) {
    if (!/^planr-[a-z0-9-]+$/u.test(role.id) || ids.has(role.id))
      fail('E_ROLE_ID_INVALID', `Role id ${String(role.id)} is invalid or duplicated.`);
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail(
      'E_ROLE_MEMBERSHIP_INVALID',
      'Canonical role membership differs from the locked catalog.',
      { expected, actual },
    );
  }
...
export function readSkillSourceRegistry({ repoRoot }) {
  ...
  if (
    ...
    fail(
      'E_SKILL_REGISTRY_INVALID',
      'skills/registry.json is not a supported skill source registry.',
    );
  }
  const ids = registry.skills.map(({ skillId }) => skillId);
  if (ids.some((id) => !/^planr-[a-z0-9-]+$/u.test(id)) || new Set(ids).size !== ids.length) {
    fail(
      'E_SKILL_REGISTRY_MEMBERSHIP_INVALID',
      'skills/registry.json contains invalid or duplicate skill IDs.',
      { ids },
    );
  }
  if (JSON.stringify(ids) !== JSON.stringify([...ids].sort())) {
    fail(
      'E_SKILL_REGISTRY_ORDER_INVALID',
      'skills/registry.json skills must be sorted by canonical ID.',
    );
  }
  for (const row of registry.skills) {
    ...
    if (row.source !== expectedSource) {
      fail(
        'E_SKILL_REGISTRY_SOURCE_INVALID',
        `${row.skillId} does not point at its canonical source manifest.`,
...
export function readStandardSkillPackage({ repoRoot, registryRow }) {
  const root = resolve(repoRoot);
  const manifestPath = resolveRegularFile(
    root,
    registryRow.source,
    `manifest for ${registryRow.skillId}`,
  );
  const manifest = readJson(manifestPath);
  const errors = validateProtocolArtifact('skill-package', manifest, { protocolVersion: '1.8.0' });
  if (errors.length > 0) {
    fail(
      'E_SKILL_PACKAGE_SCHEMA_INVALID',
      `${registryRow.skillId} does not satisfy Protocol 1.8.`,
      { errors },
    );
  }
  if (
    manifest.skillId !== registryRow.skillId ||
    manifest.skillVersion !== registryRow.skillVersion
  ) {
    fail(
      'E_SKILL_PACKAGE_IDENTITY_DRIFT',
      `${registryRow.skillId} manifest identity differs from the registry.`,
      {
  ...
  const entrypoint = resolveRegularFile(
    skillDir,
    manifest.entrypoint,
    `entrypoint for ${registryRow.skillId}`,
  );
  const markdown = readFileSync(entrypoint, 'utf8');
  if (!new RegExp(`^name:\\s*["']?${registryRow.skillId}["']?\\s*$`, 'mu').test(markdown)) {
    fail(
      'E_SKILL_PACKAGE_FRONTMATTER_INVALID',
      `${registryRow.skillId} SKILL.md has a different or missing name.`,
    );
  }
  const seen = new Set();
  const resources = manifest.resources.map((resource) => {
    if (seen.has(resource.path))
      fail(
        'E_SKILL_PACKAGE_RESOURCE_DUPLICATE',
        `${registryRow.skillId} declares ${resource.path} more than once.`,
      );
    seen.add(resource.path);
    const absolute = resolveRegularFile(
      skillDir,
      resource.path,
      `resource for ${registryRow.skillId}`,
    );
    const executable = (lstatSync(absolute).mode & 0o111) !== 0;
    if (resource.executable !== executable) {
      fail(
        'E_SKILL_PACKAGE_EXECUTABLE_DRIFT',
        `${registryRow.skillId}/${resource.path} executable mode differs from its declaration.`,
        {
...
export function readContributionGraph({ repoRoot }) {
  ...
  const manifests = [...contributions.entries()]
  ...
  const skills = validateContributionGraph(manifests, {
    expectedSkillIds: registry.skills.map(({ skillId }) => skillId),
    repoRoot: root,
  });
  const aliases =
    registry.sourceFormat === 'package-v1'
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true }).sort((a, b) =>
      ...
      if (entry.isSymbolicLink())
        fail('E_GENERATED_SYMLINK_INVALID', 'Generated custody trees may not contain symlinks.', {
          path: relative(relativeTo, absolute),
        });
      if (entry.isDirectory()) visit(absolute);
      else if (entry.isFile()) result.push(relative(relativeTo, absolute).split(sep).join('/'));
      else
        fail(
          'E_GENERATED_FILE_TYPE_INVALID',
          'Generated custody trees may contain regular files only.',
          { path: relative(relativeTo, absolute) },
        );
    }
  };
