...
function assertArtifactBytes(artifact, bytes, label) {
  try {
    ...
    throw materializationError(
      'ARTIFACT_NOT_FOUND',
      `${label} metadata is not a valid v2 Artifact.`,
      {
  ...
  if (bytes.byteLength !== artifact.sizeBytes || rawHashForBytes(bytes) !== artifact.rawHash) {
    throw materializationError(
      ...
      `${label} bytes do not match its immutable Artifact identity.`,
      {
...
function requireByteStore(store) {
  if (!store || typeof store.readRaw !== 'function' || typeof store.stageRaw !== 'function') {
    throw materializationError(
      'ARTIFACT_NOT_FOUND',
      'Evidence materialization requires the runtime Artifact byte store.',
    );
  }
  return store;
}
...
export function createOperatingArtifactByteStoreV2() {
  ...
  return Object.freeze({
    stageRaw({ artifact, rawBytes }) {
      ...
      if (existing) {
        if (existing.rawHash !== artifact.rawHash || !existing.bytes.equals(bytes)) {
          throw materializationError(
            ...
            'Artifact identity is already bound to different exact bytes.',
            {
              artifactId: artifact.artifactId,
            },
          );
      ...
      entries.set(
        artifact.artifactId,
        Object.freeze({ rawHash: artifact.rawHash, bytes: Buffer.from(bytes) }),
      );
      return true;
    },
    readRaw({ artifactId, rawHash }) {
      ...
      if (!entry) {
        throw materializationError(
          'ARTIFACT_NOT_FOUND',
          'Artifact raw bytes are unavailable from the runtime Artifact store.',
          {
            artifactId,
          },
        );
      ...
      if (entry.rawHash !== rawHash) {
        throw materializationError(
          'ARTIFACT_HASH_MISMATCH',
          'Artifact raw bytes do not match the requested immutable hash.',
          {
            artifactId,
            rawHash,
          },
      ...
      if (rawHashForBytes(bytes) !== rawHash) {
        throw materializationError(
          'ARTIFACT_HASH_MISMATCH',
          'Artifact raw bytes failed immutable hash verification.',
          {
            artifactId,
            rawHash,
          },
...
export function readOperatingArtifactRawBytesV2(store, { artifactId, rawHash }) {
  if (
    ...
    !/^sha256:[a-f0-9]{64}$/u.test(rawHash)
  ) {
    throw materializationError(
      'ARTIFACT_NOT_FOUND',
      'Artifact raw retrieval requires an immutable Artifact identity and SHA-256 hash.',
    );
  }
  const bytes = exactBytes(
    requireByteStore(store).readRaw({ artifactId, rawHash }),
    'Stored Artifact raw bytes',
  );
  if (rawHashForBytes(bytes) !== rawHash) {
    throw materializationError(
      'ARTIFACT_HASH_MISMATCH',
      'Artifact raw retrieval failed immutable hash verification.',
      {
        artifactId,
        rawHash,
      },
...
function resolutionError(candidate, resolved, draft) {
  ...
  if (!error || typeof error !== 'object') {
    throw materializationError(
      ...
      'A rejected evidence resolution requires a structured safe error.',
    );
  }
  const resolution = {
    ...
    sourceContract: null,
    provider: clone(candidate.provider),
    resolver: clone(candidate.resolver),
    outcome: 'rejected',
    evidenceRefId: null,
  ...
  try {
    assertProtocolArtifact('operating-evidence-resolution', resolution, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The resolver returned an invalid evidence rejection.',
      {
        candidateId: candidate.candidateId,
        cause: cause.code ?? null,
      },
...
function classificationFor(sourceArtifact, capture) {
  ...
  if (source === undefined || captured === undefined) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence classification must be an explicit known value.',
    );
  }
  return Object.keys(CLASSIFICATION_RANK).find(
    (key) => CLASSIFICATION_RANK[key] === Math.max(source, captured),
  );
}
...
function buildSnapshotArtifact({ candidate, sourceArtifact, capture, draft }) {
  ...
  try {
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The runtime-built evidence snapshot Artifact is invalid.',
      {
        candidateId: candidate.candidateId,
        cause: cause.code ?? null,
      },
...
function buildEvidenceRef({ candidate, capture, artifact, draft }) {
  ...
  try {
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The runtime-built EvidenceRef is invalid.',
      {
        candidateId: candidate.candidateId,
        cause: cause.code ?? null,
      },
...
function buildResolvedResolution({ candidate, evidenceRef, artifact, draft }) {
  ...
  try {
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The runtime-built evidence resolution is invalid.',
      {
        candidateId: candidate.candidateId,
        cause: cause.code ?? null,
      },
...
function normalizeClaimLinks(claimLinks, { candidate, sourceArtifact, evidenceRef, timestamp }) {
  ...
  const edges = claimLinks.map((link) => {
    if (
      ...
      Object.keys(link).some(
        (key) => !['sourceArtifactId', 'localClaimId', 'relation', 'confidence'].includes(key),
      ) ||
    ...
    try {
      assertProtocolArtifact('operating-evidence-edge', edge, {
        protocolVersion: PROTOCOL_VERSION,
      });
    } catch (cause) {
      throw materializationError(
        'RESULT_CONTRACT_INVALID',
        'The runtime-built evidence edge is invalid.',
        {
          sourceArtifactId: candidate.sourceArtifactId,
          cause: cause.code ?? null,
        },
  ...
  for (const edge of edges) {
    ...
    if (duplicate.has(key)) {
      throw materializationError(
        'STATE_TRANSITION_INVALID',
        'Duplicate source-Artifact-local evidence links are forbidden.',
        {
...
function assertSourceBinding(candidate, sourceArtifact, inputBinding) {
  try {
    assertProtocolArtifact('operating-evidence-candidate', candidate, {
      protocolVersion: PROTOCOL_VERSION,
    });
    assertProtocolArtifact('operating-artifact', sourceArtifact, {
      protocolVersion: PROTOCOL_VERSION,
    });
    assertProtocolArtifact('operating-cycle-input-binding', inputBinding, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence materialization requires valid v2 candidate, source Artifact, and input binding.',
      {
        cause: cause.code ?? null,
      },
    );
  ...
  if (
    ...
    throw materializationError(
      ...
      'Evidence candidate, source Artifact, and Cycle input binding must share one immutable scope/domain binding.',
      {
...
function assertDraft(draft) {
  if (!draft || typeof draft !== 'object' || Array.isArray(draft)) {
    throw materializationError(
      'STATE_TRANSITION_INVALID',
      'Evidence materialization requires a runtime-owned draft.',
    );
  }
  ...
  if (
    ...
    typeof draft.resolutionId !== 'string' ||
    draft.resolutionId.length === 0 ||
    ...
    (draft.evidenceRefId !== undefined &&
      (typeof draft.evidenceRefId !== 'string' || draft.evidenceRefId.length === 0)) ||
    (draft.evidenceArtifactId !== undefined &&
      (typeof draft.evidenceArtifactId !== 'string' || draft.evidenceArtifactId.length === 0))
  ) {
    throw materializationError(
      'STATE_TRANSITION_INVALID',
      'Evidence materialization draft is incomplete or contains unsupported fields.',
    );
  }
  return {
    resolutionId: draft.resolutionId,
    ...
    ...(draft.evidenceArtifactId === undefined
      ? {}
      : { evidenceArtifactId: draft.evidenceArtifactId }),
  };
}
...
export function deriveOperatingEvidenceRequestHashV2({ candidate, claimLinks, draft }) {
  ...
  try {
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence materialization inputs must be deterministic JSON values.',
    );
  }
}
...
function normalizeProofLinks(links, { candidateId, sourceArtifactId, allowCandidateId }) {
  if (!Array.isArray(links)) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence claim links must be an array.',
      { candidateId },
    );
  }
...
function readDeclaredEvidencePayload(
  sourceArtifact,
  artifactStore,
  candidate,
  requestedClaimLinks,
  ...
  if (sourceArtifact.mediaType !== 'application/json' || sourceArtifact.encoding !== 'utf-8') {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence candidates require a validated JSON source Artifact.',
      {
  ...
  if (!Buffer.from(text, 'utf8').equals(rawBytes)) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence candidates require a UTF-8 source Artifact body.',
      {
        artifactId: sourceArtifact.artifactId,
      },
    );
  ...
  try {
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Evidence candidates require a JSON source Artifact body.',
      {
        artifactId: sourceArtifact.artifactId,
      },
    );
  ...
  if (
    ...
    typeof body !== 'object' ||
    Array.isArray(body) ||
    !Array.isArray(body.evidenceCandidates) ||
    !Array.isArray(body.evidenceClaimLinks)
  ) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
  ...
  const candidates = body.evidenceCandidates.filter(
    (entry) => entry?.candidateId === candidate?.candidateId,
  );
  if (candidates.length !== 1) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The requested evidence candidate is not uniquely declared by the validated source Artifact.',
      {
        artifactId: sourceArtifact.artifactId,
        candidateId: candidate?.candidateId ?? null,
      },
  ...
  try {
    assertProtocolArtifact('operating-evidence-candidate', declaredCandidate, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'The validated source Artifact declares an invalid evidence candidate.',
      {
        artifactId: sourceArtifact.artifactId,
        candidateId: candidate?.candidateId ?? null,
        cause: cause.code ?? null,
  ...
  if (sha256Jcs(declaredCandidate) !== sha256Jcs(candidate)) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Caller-supplied evidence candidate does not match the validated source Artifact.',
      {
        artifactId: sourceArtifact.artifactId,
        candidateId: candidate?.candidateId ?? null,
      },
  ...
  if (!compareProofLinks(declaredLinks, suppliedLinks)) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Caller-supplied evidence proof links do not match the validated source Artifact.',
      {
        artifactId: sourceArtifact.artifactId,
        candidateId: declaredCandidate.candidateId,
      },
...
function capturedRawBytes(capture) {
  ...
  if (
    ...
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
      'Resolver capture content must be exact base64 bytes.',
    );
  }
  ...
  if (capture.sizeBytes !== bytes.byteLength || capture.rawHash !== rawHashForBytes(bytes)) {
    throw materializationError(
      'ARTIFACT_HASH_MISMATCH',
      'Resolver capture bytes do not match their immutable metadata.',
    );
  }
  return bytes;
}
...
function assertResolverSourceContract(dispatch) {
  ...
  if (
    !sourceContract ||
    typeof sourceContract !== 'object' ||
    Array.isArray(sourceContract) ||
    Object.keys(sourceContract).sort().join(',') !== 'id,version' ||
    typeof sourceContract.id !== 'string' ||
    typeof sourceContract.version !== 'string' ||
    !Array.isArray(supported) ||
    !supported.some(
      (entry) => entry.id === sourceContract.id && entry.version === sourceContract.version,
    )
  ) {
    throw materializationError(
      'RESULT_CONTRACT_INVALID',
...
export function buildOperatingEvidenceMaterializationV2({
  ...
  if (
    resolvedDispatch &&
    (typeof normalizedDraft.evidenceRefId !== 'string' ||
      typeof normalizedDraft.evidenceArtifactId !== 'string')
  ) {
    throw materializationError(
      'STATE_TRANSITION_INVALID',
  ...
  const artifact = buildSnapshotArtifact({
    candidate,
    sourceArtifact,
  ...
  const resolution = buildResolvedResolution({
    candidate,
    evidenceRef,
    artifact,
    draft: normalizedDraft,
...
export function validateOperatingEvidenceSourcePayloadV2({
  ...
  return readDeclaredEvidencePayload(
    sourceArtifact,
    requireByteStore(artifactStore),
    candidate,
    claimLinks,
  );
}
