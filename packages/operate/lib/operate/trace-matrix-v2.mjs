...
const ALLOWED_CONTRACTS_BY_KIND = Object.freeze(
  Object.fromEntries(
    Object.entries(CONTRACT_BY_KIND).map(([kind, contractId]) => [
      kind,
      new Set(kind === 'absence' ? [contractId, 'operating-intelligence-plan'] : [contractId]),
    ]),
  ),
);
const ALLOWED_EDGE_PAIRS = Object.freeze({
  ...
  informs: new Set([
    'claim>finding',
    'claim>decision',
    'evidence-ref>finding',
    'evidence-ref>decision',
  ]),
...
function validateEventHead(eventHead) {
  if (
    ...
    eventHead.sequence < 0 ||
    (eventHead.sequence === 0
      ? eventHead.hash !== null
      : !/^sha256:[a-f0-9]{64}$/u.test(eventHead.hash ?? ''))
  ) {
    fail(
      'E_OPERATE_TRACE_BINDING_INVALID',
      'Trace matrix requires one exact canonical Event head.',
    );
  }
}
...
function acceptedArtifactForAssignment({ assignment, artifacts, submissions }) {
  ...
  if (accepted.length > 1) {
    fail(
      'E_OPERATE_TRACE_GRAPH_INVALID',
      'An Assignment has more than one accepted Artifact identity.',
      {
  ...
  if (
    ...
    fail(
      'E_OPERATE_TRACE_DANGLING',
      'Accepted Submission does not retain its exact Artifact identity.',
      {
        assignmentId: assignment.assignmentId,
  ...
  if (assignment.state !== 'validated') {
    fail(
      'E_OPERATE_TRACE_GRAPH_INVALID',
      'Only a validated Assignment may contribute an accepted Artifact node.',
      {
        assignmentId: assignment.assignmentId,
...
function resolvePlan(state, cycle, assignments, planId) {
  const candidates = (state.intelligencePlans ?? []).filter(
    (plan) =>
  ...
  if (planId !== undefined && candidates.length !== 1) {
    fail(
      'E_OPERATE_TRACE_BINDING_INVALID',
      'Trace matrix cannot resolve the requested intelligence plan exactly.',
      {
  ...
  if (planId === undefined && candidates.length > 1) {
    fail(
      'E_OPERATE_TRACE_BINDING_INVALID',
      'Trace matrix requires an explicit plan when multiple Cycle plans exist.',
      {
        cycleId: cycle.cycleId,
        matchCount: candidates.length,
      },
...
function currentPlanMetricBinding(
  state,
  cycle,
  plan,
  artifacts,
  ...
  if (!plan) {
    fail(
      'E_OPERATE_TRACE_DANGLING',
      'Action verification proof requires one exact intelligence plan.',
    );
  }
  const snapshots = new Map(
    (state.operatingSnapshots ?? []).map((record) => [record.snapshotId, record]),
  );
  const modelStates = new Map(
    (state.operatingModelStates ?? []).map((record) => [record.stateId, record]),
  );
  const candidates = [...snapshots.values()].filter(
    (snapshot) =>
      sameScope(snapshot, cycle) && snapshotDescendsFrom(snapshot, plan.snapshotId, snapshots),
  );
  const candidateIds = new Set(candidates.map(({ snapshotId }) => snapshotId));
  const leaves = candidates.filter(
    ({ snapshotId }) =>
      !candidates.some(
        (entry) => entry.previousSnapshotId === snapshotId && candidateIds.has(entry.snapshotId),
      ),
  );
  if (leaves.length !== 1) {
    fail(
      'E_OPERATE_TRACE_DANGLING',
      'Action verification proof requires one exact current Snapshot on the plan lineage.',
      {
        planId: plan.planId,
        metricId,
  ...
  const metricMatches =
    operatingState?.metrics?.filter((metric) => metric.metricId === metricId) ?? [];
  ...
  if (
    ...
    acceptedArtifactByAssignment.get(sourceArtifact.assignmentId)?.artifactId !==
    ...
    fail(
      'E_OPERATE_TRACE_DANGLING',
      'Current plan Metric lost its exact Snapshot, state, source Artifact, or Evidence binding.',
      {
        planId: plan.planId,
        metricId,
        snapshotId: snapshot.snapshotId,
...
function stableMetricValue(metric) {
  return Object.fromEntries(
    Object.entries(metric).filter(
      ([field]) => !['observationIds', 'freshness', 'updatedAt'].includes(field),
    ),
  );
}

function proofCounts(nodes) {
  const count = (kind, states) =>
    nodes.filter(({ locator, proofState }) => locator.kind === kind && states.includes(proofState))
      .length;
  return {
...
export function buildOperatingTraceMatrixV2(
  state,
  { cycleId, planId = undefined, accessLevel = 'restricted', eventHead = state?.eventHead } = {},
) {
  ...
  if (eventHead.sequence !== state.eventHead.sequence || eventHead.hash !== state.eventHead.hash) {
    fail(
      'E_OPERATE_TRACE_BINDING_INVALID',
      'Trace matrix Event head must equal the exact input runtime-state head.',
    );
  }
  ...
  if (cycleMatches.length !== 1) {
    fail('E_OPERATE_TRACE_BINDING_INVALID', 'Trace matrix requires one exact Cycle.', {
      cycleId,
      matchCount: cycleMatches.length,
    });
  }
  ...
  const addNode = (
    ...
    if (existing) {
      if (
        existing.proofState !== proofState ||
        existing.accessState !== accessState ||
        existing.locator.contract.id !== contractId
      ) {
        fail(
          'E_OPERATE_TRACE_DUPLICATE',
          'A trace node identity has divergent proof or access truth.',
          { kind, id },
        );
      }
      return existing;
    ...
    if (nodes.size >= MAX_NODES) {
      fail('E_OPERATE_TRACE_LIMIT_EXCEEDED', 'Trace matrix node limit exceeded.', {
        maxNodes: MAX_NODES,
      });
    }
  ...
  const addEdge = (relation, fromKind, fromId, toKind, toId) => {
    ...
    if (!ALLOWED_EDGE_PAIRS[relation]?.has(pair)) {
      fail(
        'E_OPERATE_TRACE_EDGE_INVALID',
        'Trace edge relation does not permit its endpoint kinds.',
        { relation, pair },
      );
    }
    if (KIND_RANK[fromKind] >= KIND_RANK[toKind]) {
      fail('E_OPERATE_TRACE_CYCLE', 'Trace edge would violate the forward lifecycle order.', {
        relation,
        pair,
      });
    }
    if (!nodes.has(endpointKey(from)) || !nodes.has(endpointKey(to))) {
      fail('E_OPERATE_TRACE_DANGLING', 'Trace edge references an unavailable endpoint.', {
        relation,
        from,
        to,
      });
    ...
    if (existing && sha256Jcs(existing) !== sha256Jcs({ edgeId, ...identity })) {
      fail('E_OPERATE_TRACE_DUPLICATE', 'Trace edge identity collision has divergent content.', {
        edgeId,
      });
    }
  ...
  for (const assignment of cycleAssignments) {
    ...
    if (artifact) {
      ...
      const accessState =
        ACCESS_ORDER.indexOf(artifact.sensitivity) <= ACCESS_ORDER.indexOf(accessLevel)
          ? 'available'
          : 'restricted';
      addNode(
        'artifact',
        artifact.artifactId,
        accessState === 'available' ? 'accepted' : 'restricted',
        accessState,
      );
    ...
    for (const absence of assignment.inputAbsences ?? []) {
      const absenceId =
        absence.absenceId ??
        `absence:${assignment.assignmentId}:${absence.roleId ?? absence.requirementId}`;
  ...
  if (plan) {
    for (const [role, omitted] of [
      ...
      addNode(
        'seat',
        seatId,
        omitted ? 'absent' : 'not-applicable',
        omitted ? 'absent' : 'available',
      );
      seatByRole.set(role.roleId, seatId);
      if (omitted) {
        const absenceId = deriveOperatingOmittedRoleAbsenceIdV2(
          plan.planId,
          role.roleId,
          role.roleVersion,
        );
      ...
      if (!omitted) {
        ...
        if (!assignments.has(assignmentId)) {
          fail('E_OPERATE_TRACE_DANGLING', 'Selected board seat has no exact issued Assignment.', {
            planId: plan.planId,
            roleId: role.roleId,
          });
        }
  ...
  for (const assignment of cycleAssignments) {
    ...
    for (const absence of assignment.inputAbsences ?? []) {
      ...
      if (absence.kind === 'evidence') {
        ...
        if (candidates.length !== 1) {
          fail(
            'E_OPERATE_TRACE_DANGLING',
            'Evidence absence does not resolve one exact declared Cycle requirement.',
            {
              assignmentId: assignment.assignmentId,
    ...
    if (artifact)
      addEdge('produces', 'assignment', assignment.assignmentId, 'artifact', artifact.artifactId);
  }
  ...
  for (const record of state.evidenceRefs ?? []) {
    ...
    const sourceAssignment =
      sourceArtifact === undefined ? undefined : assignments.get(sourceArtifact.assignmentId);
    ...
    if (
      ...
      fail('E_OPERATE_TRACE_DANGLING', 'EvidenceRef lost its exact evidence Artifact identity.', {
        evidenceRefId: record.evidenceRefId,
      });
    }
    const resolutions = (state.evidenceResolutions ?? []).filter(
      (entry) =>
    ...
    const candidateResolutions = (state.evidenceResolutions ?? []).filter(
      (entry) => entry.candidateId === record.candidateId,
    );
    if (candidateResolutions.length !== 1 || resolutions.length !== 1) {
      fail(
        'E_OPERATE_TRACE_DANGLING',
        'EvidenceRef requires one exact resolved evidence outcome.',
        {
          evidenceRefId: record.evidenceRefId,
    ...
    if (
      ...
      fail(
        'E_OPERATE_TRACE_FOREIGN_SCOPE',
        'Evidence resolution lost its exact EvidenceRef source and resolver binding.',
        {
          evidenceRefId: record.evidenceRefId,
    ...
    const available =
      ACCESS_ORDER.indexOf(record.classification) <= ACCESS_ORDER.indexOf(accessLevel);
    const proofState = !available
      ? 'restricted'
      : record.freshness === 'stale'
        ? 'stale'
        : ['current', 'historical'].includes(record.freshness)
          ? 'verified'
          : 'unverified';
    addNode(
      'evidence-ref',
  ...
  for (const assignment of cycleAssignments) {
    for (const issued of assignment.intelligenceContext?.inputBundle?.issuedEvidence ?? []) {
      ...
      if (!nodes.has(`requirement:${requirementNodeId}`) || !evidence.has(issued.evidenceRefId)) {
        fail(
          'E_OPERATE_TRACE_DANGLING',
          'Issued evidence does not resolve its exact requirement and EvidenceRef.',
          {
            assignmentId: assignment.assignmentId,
  ...
  for (const claim of cycleClaims) {
    ...
    if (!sourceArtifact)
      fail('E_OPERATE_TRACE_DANGLING', 'Claim references an unavailable source Artifact.', {
        claimId: claim.claimId,
      });
    const sourceAvailable =
      ACCESS_ORDER.indexOf(sourceArtifact.sensitivity) <= ACCESS_ORDER.indexOf(accessLevel);
    const support = claim.supportingEvidenceRefIds.map((id) => {
      const node = nodes.get(`evidence-ref:${id}`);
      if (!node)
        fail(
          'E_OPERATE_TRACE_DANGLING',
          'Claim references an unavailable supporting EvidenceRef.',
          { claimId: claim.claimId, evidenceRefId: id },
        );
      return node;
    });
    const contradiction = claim.contradictingEvidenceRefIds.map((id) => {
      ...
      if (!node)
        fail(
          'E_OPERATE_TRACE_DANGLING',
          'Claim references an unavailable contradicting EvidenceRef.',
          { claimId: claim.claimId, evidenceRefId: id },
        );
      return node;
    });
    ...
    const proofState = !sourceAvailable
      ? 'restricted'
      : verifiedSupport > 0 && verifiedContradiction > 0
        ? 'contested'
        : verifiedContradiction > 0
          ? 'contradicted'
          : verifiedSupport > 0
            ? 'supported'
            : 'unverified';
    ...
    if (!acceptedArtifactByAssignment.has(sourceArtifact.assignmentId)) {
      fail('E_OPERATE_TRACE_DANGLING', 'Claim source is not one exact accepted Artifact.', {
        claimId: claim.claimId,
        artifactId: sourceArtifact.artifactId,
      });
    }
    addEdge('contains', 'artifact', sourceArtifact.artifactId, 'claim', claim.claimId);
    for (const id of claim.supportingEvidenceRefIds)
      addEdge('supported-by', 'claim', claim.claimId, 'evidence-ref', id);
    for (const id of claim.contradictingEvidenceRefIds)
      addEdge('contradicted-by', 'claim', claim.claimId, 'evidence-ref', id);
  }

  const cycleFindings = (state.findings ?? []).filter(
    (record) => record.sourceCycleId === cycle.cycleId,
  );
  ...
  for (const finding of cycleFindings) {
    ...
    if (!sourceArtifact)
      fail('E_OPERATE_TRACE_DANGLING', 'Finding references an unavailable source Artifact.', {
    ...
    addNode(
      'finding',
      finding.findingId,
      available ? 'proposed' : 'restricted',
      available ? 'available' : 'restricted',
    );
    findings.set(finding.findingId, finding);
    for (const id of finding.supportingEvidenceRefIds ?? []) {
      if (!evidence.has(id))
        fail('E_OPERATE_TRACE_DANGLING', 'Finding references unavailable supporting evidence.', {
          findingId: finding.findingId,
          evidenceRefId: id,
        });
    ...
    for (const id of finding.contradictingEvidenceRefIds ?? []) {
      if (!evidence.has(id))
        fail('E_OPERATE_TRACE_DANGLING', 'Finding references unavailable contradicting evidence.', {
          findingId: finding.findingId,
          evidenceRefId: id,
        });
      addEdge('informs', 'evidence-ref', id, 'finding', finding.findingId);
    ...
    for (const target of finding.targets ?? []) {
      for (const localClaimId of target.claimIds ?? []) {
        const claimId = deriveOperatingMaterializedClaimIdV2(
          target.advisorArtifactId,
          localClaimId,
        );
        if (!claimById.has(claimId))
          fail('E_OPERATE_TRACE_DANGLING', 'Finding target does not resolve one durable Claim.', {
            findingId: finding.findingId,
            claimId,
          });
  ...
  const cycleDecisions = (state.decisions ?? []).filter(
    (record) => record.sourceCycleId === cycle.cycleId,
  );
  ...
  for (const decision of cycleDecisions) {
    ...
    if (!sourceArtifact)
      fail('E_OPERATE_TRACE_DANGLING', 'Decision references an unavailable source Artifact.', {
    ...
    addNode(
      'decision',
      decision.decisionId,
      available ? (decision.state === 'approved' ? 'governed' : 'proposed') : 'restricted',
      available ? 'available' : 'restricted',
    );
    decisions.set(decision.decisionId, decision);
    if (!nodes.has(`artifact:${sourceArtifact.artifactId}`)) {
      fail('E_OPERATE_TRACE_DANGLING', 'Decision source is not one exact accepted Artifact.', {
        decisionId: decision.decisionId,
        artifactId: sourceArtifact.artifactId,
      });
    }
    ...
    for (const claimId of decision.claimIds ?? []) {
      if (!claimById.has(claimId))
        fail('E_OPERATE_TRACE_DANGLING', 'Decision references an unavailable Claim.', {
          decisionId: decision.decisionId,
          claimId,
        });
    ...
    for (const findingId of decision.findingIds ?? []) {
      if (!findings.has(findingId))
        fail('E_OPERATE_TRACE_DANGLING', 'Decision references an unavailable Finding.', {
          decisionId: decision.decisionId,
          findingId,
        });
    ...
    for (const evidenceRefId of decision.evidenceRefIds ?? []) {
      if (!evidence.has(evidenceRefId))
        fail('E_OPERATE_TRACE_DANGLING', 'Decision references unavailable EvidenceRef.', {
          decisionId: decision.decisionId,
          evidenceRefId,
        });
  ...
  for (const action of (state.actions ?? []).filter(
    ...
    if (!decisions.has(action.sourceDecisionId)) {
      fail('E_OPERATE_TRACE_DANGLING', 'Action references an unavailable source Decision.', {
  ...
  for (const verification of state.verificationPlans ?? []) {
    ...
    if (!action) continue;
    requireExactRecordScope(
      verification,
      cycle,
      'Verification plan',
      verification.verificationPlanId,
    );
    if (action.verificationPlanId !== verification.verificationPlanId) {
      fail('E_OPERATE_TRACE_DANGLING', 'Action lost its exact verification plan binding.', {
        actionId: action.actionId,
      });
    }
  ...
  const observations = new Map(
    (state.metricObservations ?? []).map((record) => [record.observationId, record]),
  );
  ...
  for (const outcome of state.outcomes ?? []) {
    ...
    if (!verification)
      fail('E_OPERATE_TRACE_DANGLING', 'Outcome references an unavailable verification plan.', {
    ...
    if (
      action.verificationPlanId !== verification.verificationPlanId ||
      ...
      fail(
        'E_OPERATE_TRACE_DANGLING',
        'Outcome verification lost its exact Action, plan, Metric, baseline, target, or window.',
        {
          outcomeId: outcome.outcomeId,
    ...
    const acceptedSource =
      sourceArtifact === undefined || sourceAssignment === undefined
        ? null
    ...
    const matchedObservations = outcome.observationIds.map((observationId) => {
      ...
      const snapshot =
        observation === undefined ? undefined : current.snapshots.get(observation.snapshotId);
      const operatingState =
        snapshot === undefined ? undefined : current.modelStates.get(snapshot.stateId);
      const snapshotMetrics =
        operatingState?.metrics?.filter((entry) => entry.metricId === metric.metricId) ?? [];
      ...
      if (
        ...
        fail(
          'E_OPERATE_TRACE_DANGLING',
          'Outcome observation lost its exact Metric, accepted source, snapshot, or temporal binding.',
          {
            outcomeId: outcome.outcomeId,
            observationId,
          },
    ...
    const observationEvidenceIds = [
      ...new Set(matchedObservations.flatMap(({ evidenceRefIds }) => evidenceRefIds)),
    ].sort();
    const outcomeEvidenceIds = [...new Set(outcome.evidenceRefIds)].sort();
    const evidenceVerified =
      outcomeEvidenceIds.length > 0 &&
    ...
    if (
      ...
      latestOutcomeProof.set(action.actionId, {
        outcomeId: outcome.outcomeId,
  ...
  for (const action of actions.values()) {
    ...
    nodes.get(`action:${action.actionId}`).proofState =
      latestOutcomeProof.get(action.actionId)?.verified === true ? 'completed' : 'insufficient';
  }

  const ledgers = (state.decisionLedgers ?? []).filter(
    (entry) => sameScope(entry, cycle) && (!plan || entry.intelligencePlanId === plan.planId),
  );
  if (ledgers.length > 1)
    fail(
      'E_OPERATE_TRACE_BINDING_INVALID',
      'Cycle trace resolves more than one current Chair ledger.',
      { cycleId },
    );
  const ledger = ledgers[0] ?? null;
  if (ledger) {
    const decisionByLocalId = new Map(
      cycleDecisions.map((entry) => [entry.sourceLocalDecisionId, entry]),
    );
    const findingByLocalId = new Map(
      cycleFindings.map((entry) => [
        `${entry.sourceArtifactId}:${entry.sourceLocalFindingId}`,
        entry,
      ]),
    );
    for (const [position, disposition] of ledger.sourceDispositions.entries()) {
      ...
      if (!sourceArtifact || sourceArtifact.cycleId !== cycle.cycleId) {
        fail('E_OPERATE_TRACE_DANGLING', 'Source disposition references an unavailable Artifact.', {
          dispositionId,
        });
      }
      if (nodes.has(`artifact:${sourceArtifact.artifactId}`)) {
      ...
      if (disposition.sourceKind === 'challenger-finding') {
        const finding = findingByLocalId.get(
          `${disposition.sourceArtifactId}:${disposition.localSourceId}`,
        );
        if (!finding)
          fail(
            'E_OPERATE_TRACE_DANGLING',
            'Source disposition references an unavailable Finding.',
            { dispositionId },
          );
      ...
      if (disposition.localDecisionId !== null) {
        ...
        if (!decision)
          fail(
            'E_OPERATE_TRACE_DANGLING',
            'Source disposition references an unavailable Decision.',
            { dispositionId },
          );
  ...
  const edges = [...rawEdges.values()]
    .filter((edge) => {
      const fromRestricted = restrictedKeys.has(endpointKey(edge.from));
      const toRestricted = restrictedKeys.has(endpointKey(edge.to));
      if (fromRestricted)
        incidentRestricted.set(
          endpointKey(edge.from),
          incidentRestricted.get(endpointKey(edge.from)) + 1,
        );
      if (toRestricted)
        incidentRestricted.set(
          endpointKey(edge.to),
          incidentRestricted.get(endpointKey(edge.to)) + 1,
        );
      return !fromRestricted && !toRestricted;
    })
    .sort((left, right) => left.edgeId.localeCompare(right.edgeId));
  if (edges.length > MAX_EDGES)
    fail('E_OPERATE_TRACE_LIMIT_EXCEEDED', 'Trace matrix edge limit exceeded.', {
      maxEdges: MAX_EDGES,
    });
  ...
  const omissions = [
    ...[...incidentRestricted.entries()].map(([key, incidentEdgeCount]) => {
      ...
      return {
        ...
        kind: 'restricted',
        subject: node.locator.kind,
        subjectId: node.locator.id,
...
export function assertOperatingTraceMatrixV2(value) {
  ...
  if (
    !value ||
    value.kind !== 'operating-trace-matrix' ||
  ...
  if (
    value.nodes.map(({ locator }) => locatorKey(locator)).join('\0') !==
      [...value.nodes]
        .map(({ locator }) => locatorKey(locator))
        .sort()
        .join('\0') ||
    ...
    value.omissions.map(({ omissionId }) => omissionId).join('\0') !==
      [...value.omissions]
        .map(({ omissionId }) => omissionId)
        .sort()
        .join('\0')
  ) {
    fail(
      'E_OPERATE_TRACE_GRAPH_INVALID',
      'Trace nodes, edges, and omissions must be canonically ordered.',
    );
  }
  if (value.matrixHash !== sha256Jcs(without(value, 'matrixHash'))) {
    fail(
      'E_OPERATE_TRACE_HASH_MISMATCH',
      'Trace matrix hash does not equal its canonical content.',
    );
  }
  const nodes = new Map();
  for (const node of value.nodes) {
    ...
    if (!key || nodes.has(key))
      fail('E_OPERATE_TRACE_DUPLICATE', 'Trace node identities must be unique.', {
        key: key ?? null,
      });
    if (
      locator.scopeId !== value.scopeId ||
      ...
      fail(
        'E_OPERATE_TRACE_FOREIGN_SCOPE',
        'Trace node locator lost its contract, scope, Cycle, or version binding.',
        { key },
      );
    }
    if (
      !Array.isArray(node.inboundEdgeIds) ||
      !Array.isArray(node.outboundEdgeIds) ||
      ...
      fail(
        'E_OPERATE_TRACE_RECIPROCAL_INVALID',
        'Trace node adjacency must be unique and canonically ordered.',
        { key },
      );
    }
    nodes.set(key, node);
  ...
  for (const edge of value.edges) {
    ...
    if (!nodes.has(fromKey) || !nodes.has(toKey))
      fail('E_OPERATE_TRACE_DANGLING', 'Trace edge endpoint is unavailable.', {
        edgeId: edge?.edgeId ?? null,
      });
    if (
      !ALLOWED_EDGE_PAIRS[edge.relation]?.has(pair) ||
      KIND_RANK[edge.from.kind] >= KIND_RANK[edge.to.kind]
    ) {
      fail('E_OPERATE_TRACE_CYCLE', 'Trace edge violates the closed acyclic lifecycle.', {
        edgeId: edge.edgeId,
        pair,
      });
    }
    if (edges.has(edge.edgeId) || edgeIdentities.has(identity))
      fail('E_OPERATE_TRACE_DUPLICATE', 'Trace edge identity is duplicated.', {
        edgeId: edge.edgeId,
      });
    if (
      edge.edgeId !==
      shortIdentity('tre', { relation: edge.relation, from: edge.from, to: edge.to })
    ) {
      fail(
        'E_OPERATE_TRACE_HASH_MISMATCH',
        'Trace edge ID does not bind its canonical endpoints.',
        { edgeId: edge.edgeId },
      );
    }
  ...
  for (const [key, node] of nodes) {
    for (const edgeId of node.outboundEdgeIds) {
      ...
      if (
        !edge ||
        endpointKey(edge.from) !== key ||
        !nodes.get(endpointKey(edge.to))?.inboundEdgeIds.includes(edgeId)
      ) {
        fail(
          'E_OPERATE_TRACE_RECIPROCAL_INVALID',
          'Trace forward edge is missing its exact inverse locator.',
          { key, edgeId },
        );
      }
    }
    for (const edgeId of node.inboundEdgeIds) {
      ...
      if (
        !edge ||
        endpointKey(edge.to) !== key ||
        !nodes.get(endpointKey(edge.from))?.outboundEdgeIds.includes(edgeId)
      ) {
        fail(
          'E_OPERATE_TRACE_RECIPROCAL_INVALID',
          'Trace inverse edge is missing its exact forward locator.',
          { key, edgeId },
        );
      }
    }
  ...
  for (const edge of edges.values()) {
    if (
      !nodes.get(endpointKey(edge.from)).outboundEdgeIds.includes(edge.edgeId) ||
      !nodes.get(endpointKey(edge.to)).inboundEdgeIds.includes(edge.edgeId)
    ) {
      fail(
        'E_OPERATE_TRACE_RECIPROCAL_INVALID',
        'Trace edge is not reciprocally indexed by both endpoints.',
        { edgeId: edge.edgeId },
      );
    }
  }
  ...
  for (const omission of value.omissions) {
    ...
    const expectedReason =
      expectedKind === 'restricted'
        ...
        : node?.locator.kind === 'absence' &&
            node.locator.contract.id === 'operating-intelligence-plan'
          ? 'OPERATE_TRACE_ROLE_NOT_SELECTED'
          : expectedKind === 'absent'
            ? 'OPERATE_TRACE_TYPED_ABSENCE'
            : null;
    ...
    const visibleIncidentCount =
      node === undefined ? -1 : node.inboundEdgeIds.length + node.outboundEdgeIds.length;
    if (
      !node ||
      expectedKind !== omission.kind ||
      expectedReason !== omission.reasonCode ||
      expectedId !== omission.omissionId ||
      omissions.has(key) ||
      (omission.kind === 'absent' && omission.incidentEdgeCount !== visibleIncidentCount)
    ) {
      fail(
        'E_OPERATE_TRACE_OMISSION_INVALID',
        'Trace omission does not bind one exact restricted or absent node.',
        {
  ...
  for (const [key, node] of nodes) {
    if (
      ...
      fail(
        'E_OPERATE_TRACE_OMISSION_INVALID',
        'Restricted and absent trace nodes require an explicit typed omission.',
        { key },
      );
    }
  }
  if (sha256Jcs(value.proof) !== sha256Jcs(proofCounts([...nodes.values()]))) {
    fail(
      'E_OPERATE_TRACE_PROOF_INVALID',
      'Trace proof summary contradicts its node classifications.',
    );
  }
  return value;
}
...
export function locateOperatingTraceNodeV2(matrix, locator, { maxResults = 256 } = {}) {
  ...
  if (!Number.isSafeInteger(maxResults) || maxResults < 1 || maxResults > 4096) {
    fail(
      'E_OPERATE_TRACE_LIMIT_EXCEEDED',
      'Trace locator result bound must be between 1 and 4096.',
    );
  }
