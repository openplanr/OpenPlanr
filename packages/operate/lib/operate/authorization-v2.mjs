...
const AUTHORITY_TOOL_CAPABILITIES = Object.freeze({
  'operate.review.submit': Object.freeze({
    id: 'operate-review-submit',
    version: PROTOCOL_VERSION,
  }),
  'operate.action.approve': Object.freeze({
    id: 'operate-action-approve',
    version: PROTOCOL_VERSION,
  }),
  'operate.action.execute': Object.freeze({
    id: 'operate-action-execute',
    version: PROTOCOL_VERSION,
  }),
  'operate.action.rollback': Object.freeze({
...
const AUTHORITY_CONTEXT_FIELDS = Object.freeze([
  ...
  'operation',
  'operationHistory',
  'currentPreconditionArtifactIds',
  'executor',
  'governedExtensions',
...
function hasToolCapability(context, operation) {
  ...
  return capabilities.some(
    (entry) =>
      ...
      versionedIdentityEqual(entry, expected),
  );
}
...
function validateActor(operation, context, allowedKinds, checks) {
  ...
  if (!actor || typeof actor.actorId !== 'string' || !allowedKinds.includes(actor.kind)) {
    return deniedDecision(
      ...
      `actor.kind is not authorized for ${operation}.`,
      { state: 'actor.kind' },
      checks,
    );
  }
...
function evaluateReviewSubmit(context) {
  ...
  if (!contractValid('operating-review', review)) {
    return deniedDecision(
      ...
      'review must satisfy operating-review@2.0.0.',
      { reviewId: review.reviewId, state: 'review' },
      checks,
    );
  }
  if (!cycle || !contractValid('operating-cycle', cycle) || cycle.cycleId !== review.cycleId) {
    return deniedDecision(
      ...
      context,
      'REVIEW_NOT_FOUND',
      'review.cycleId must bind its durable source Cycle.',
  ...
  if (review.state !== 'pending') {
    return deniedDecision(
      ...
      { reviewId: review.reviewId, state: review.state },
      checks,
    );
  }
  if (
    ...
    return deniedDecision(
      ...
      context,
      'REVIEW_NOT_FOUND',
      'A Cycle Review must be the active awaiting-review gate.',
      { reviewId: review.reviewId, state: 'cycle.activeReviewId' },
      checks,
    );
  }
  ...
  if (
    !request ||
    !contractValid('operate-tool-call', {
    ...
    return deniedDecision(
      ...
      context,
      'RESULT_CONTRACT_INVALID',
      'request must contain one complete typed review disposition.',
      { reviewId: review.reviewId, state: 'request' },
      checks,
    );
  }
  if (request.reviewId !== review.reviewId) {
    return deniedDecision(
      ...
      context,
      'REVIEW_NOT_FOUND',
      'request.reviewId must equal review.reviewId.',
      { reviewId: review.reviewId, state: 'request.reviewId' },
      checks,
    );
  }
  if (
    request.cycleId !== cycle.cycleId ||
  ...
  if (review.subject?.type === 'action') {
    if (
      !context.action ||
      !contractValid('operating-action', context.action) ||
...
function validateActionBase(operation, context, allowedStates, checks) {
  ...
  if (!contractValid('operating-action', action)) {
    return deniedDecision(
      ...
      context,
      'RESULT_CONTRACT_INVALID',
      'action must satisfy operating-action@2.0.0.',
      { state: 'action' },
      checks,
    );
  }
  ...
  if (requiredAuthorityFields.some((field) => !Object.hasOwn(action, field))) {
    return deniedDecision(
      ...
      context,
      'ACTION_REVISION_MISMATCH',
      'action authority tuple is incomplete.',
  ...
  if (allowedStates !== null && !allowedStates.includes(action.state)) {
    return deniedDecision(
      ...
      `action.state ${action.state} cannot authorize ${operation}.`,
      { state: action.state },
      checks,
    );
  }
  ...
  if (
    !scope ||
    scope.scopeId !== action.scopeId ||
    scope.domainId !== action.domainId ||
    ...
    return deniedDecision(
      ...
      context,
      'OPERATING_SCOPE_INVALID',
      'scope must bind the exact Action scopeId, domainId, and domainVersion.',
      { state: 'scope' },
      checks,
    );
  }
  if (!targetEqual(context.target, action.targetBinding)) {
    return deniedDecision(
      ...
      'target must equal action.targetBinding including revision.',
      { state: 'target' },
      checks,
    );
  }
  if (
    (operation === 'operate.action.execute' || operation === 'operate.action.rollback') &&
  ...
  if (action.effectClass === 'destructive') {
    return deniedDecision(
      ...
      context,
      'POLICY_PROHIBITED',
      'action.effectClass destructive is prohibited.',
  ...
  for (const dependency of context.dependencyActions ?? []) {
    if (
      action.dependsOnActionIds.includes(dependency.actionId) &&
      ...
      return deniedDecision(
        ...
        context,
        'STATE_TRANSITION_INVALID',
        'action.dependsOnActionIds contains an incomplete dependency.',
        { state: 'action.dependsOnActionIds' },
        checks,
      );
    }
  }
  const knownDependencies = new Set(
    (context.dependencyActions ?? []).map(({ actionId }) => actionId),
  );
  if (action.dependsOnActionIds.some((id) => !knownDependencies.has(id))) {
    return deniedDecision(
      ...
      context,
      'STATE_TRANSITION_INVALID',
      'every action.dependsOnActionIds entry must be supplied and completed.',
      { state: 'action.dependsOnActionIds' },
      checks,
    );
  }
...
function validateDomainNarrowing(operation, context, checks) {
  ...
  if (
    !contractValid('operate-domain-registration', registration) ||
    ...
    return deniedDecision(
      ...
      'domainRegistration must match the Action domain exactly.',
      { state: 'domainRegistration' },
      checks,
    );
  }
  if (
    registration.actionKinds &&
    !registration.actionKinds.some((entry) =>
      versionedIdentityEqual(entry, context.action.actionKind),
    )
  ) {
    return deniedDecision(
      operation,
      context,
      'DOMAIN_CONTRACT_UNSUPPORTED',
      'domainRegistration.actionKinds does not declare the exact Action kind.',
      { state: 'domainRegistration.actionKinds' },
      checks,
    );
  }
  if (
    registration.requestedCapabilities &&
    !registration.requestedCapabilities.some((entry) =>
      versionedIdentityEqual(entry, context.action.requestedCapability),
    )
  ) {
    return deniedDecision(
      operation,
...
function validatePolicy(operation, context, checks, nowMs) {
  ...
  if (
    !policy ||
    !contractValid('operating-action-policy', policy) ||
    !evaluation ||
    !contractValid('operating-policy-evaluation', evaluation)
  ) {
    return deniedDecision(
      operation,
      context,
  ...
  try {
    ...
    if (
      !policyEqual(canonical.policy, policyRef) ||
    ...
    return deniedDecision(
      ...
      error.message,
      { state: 'policyEvaluation.precedence' },
      checks,
    );
  }
  if (evaluation.outcome === 'prohibited') {
    return deniedDecision(
      ...
      context,
      'POLICY_PROHIBITED',
      'policyEvaluation.outcome prohibits the operation.',
      { state: 'policyEvaluation.outcome' },
      checks,
    );
  }
  if (evaluation.outcome === 'rejected') {
    return deniedDecision(
      ...
      context,
      'POLICY_EVALUATION_REJECTED',
      'policyEvaluation.outcome rejects the operation.',
      { state: 'policyEvaluation.outcome' },
      checks,
    );
  }
  if (evaluation.outcome === 'deferred') {
    return deniedDecision(
      ...
      'policyEvaluation.outcome defers the operation.',
      { state: 'policyEvaluation.outcome' },
      checks,
    );
  }
...
function validateApprovalAction(operation, context, checks, nowMs) {
  ...
  if (!['approved', 'rejected', 'deferred'].includes(request.decision)) {
    return deniedDecision(
      ...
      context,
      'APPROVAL_INVALID',
      'request.decision must be approved, rejected, or deferred.',
  ...
  if (evaluation.outcome === 'automatic') {
    ...
    if (context.actor.kind !== 'engine') {
      return deniedDecision(
        ...
        context,
        'REVIEW_NOT_AUTHORIZED',
        'automatic policy decisions may be recorded only by the engine.',
        { state: 'actor.kind' },
        checks,
      );
    }
  ...
  if (!uniqueScalarSet(approvalIds)) {
    return deniedDecision(
      ...
      context,
      'APPROVAL_INVALID',
      'approval records must have unique identities.',
      { state: 'approvals' },
      checks,
    );
  }
  try {
    ...
    return deniedDecision(
      ...
      error.message,
      { state: 'approvalRecord.canonical' },
      checks,
    );
  }
  for (const requirementId of evaluation.approvalRequirementIds) {
    ...
    const requirementExpiry =
      requirement.expiresAt === null ? null : parseTime(requirement.expiresAt);
    if (requirementExpiry !== null && requirementExpiry <= nowMs) continue;
    const eligibleParty = requirement.parties.find(
      (party) =>
    ...
    const existing = (context.approvals ?? []).find(
      (record) =>
        (record.requirementId === requirement.requirementId &&
          record.partyId === eligibleParty.partyId) ||
        (record.actor?.kind === context.actor.kind &&
          record.actor?.actorId === context.actor.actorId),
    );
    if (existing) {
      return deniedDecision(
        operation,
  ...
  return deniedDecision(
    ...
    context,
    'APPROVAL_REQUIRED',
    'actor does not satisfy an unsatisfied exact approval party.',
    { state: 'approvalRequirements' },
    checks,
  );
}
  ...
  try {
    ...
    if (!result.complete || result.disposition !== 'approved') {
      return deniedDecision(
        ...
        context,
        'APPROVAL_REQUIRED',
        'the exact current approval set is incomplete or not approved.',
...
function validateOperationBinding(
  ...
  if (!operation || !contractValid('operating-governed-operation', operation)) {
    return deniedDecision(
      ...
      context,
      'OPERATION_CONFLICT',
      'operation must be a valid runtime-issued governed operation.',
      { state: 'operation' },
      checks,
    );
  }
  ...
  const approvedInputArtifactIds = [
  ...
  const rollbackClassificationValid = action.executionBinding.rollbackRequired
    ...
    : operation.rollbackClass === 'not-applicable';
  if (
    !exactScalarSet(operation.preconditionArtifactIds, action.preconditionArtifactIds) ||
  ...
  if (
    ...
    operationCreatedAt > operationUpdatedAt ||
    operationUpdatedAt > nowMs ||
...
function replayDecision(operationName, context, checks, nowMs) {
  ...
  if (!uniqueScalarSet(historyIds)) {
    return deniedDecision(
      ...
      context,
      'OPERATION_CONFLICT',
      'operation history contains duplicate operation identities.',
      { state: 'operationHistory.operationId' },
      checks,
    );
  }
  ...
  const exact =
    contractValid('operating-governed-operation', entry) &&
  ...
  if (
    !grant ||
    !contractValid('operating-capability-grant', grant) ||
  ...
  const proofHash =
    proof &&
    sha256Jcs(
      Object.fromEntries(Object.entries(proof).filter(([field]) => field !== 'reconciliationHash')),
    );
  if (
  ...
  return deniedDecision(
    ...
    context,
    'OPERATION_CONFLICT',
    'operationId already has non-terminal durable history.',
    { state: entry.state ?? 'operationHistory' },
    checks,
  );
}
...
function validateCapability(operationName, context, checks, nowMs) {
  ...
  if (
    !availability ||
    !contractValid('operating-capability-availability', availability) ||
  ...
  if (
    ...
    availabilityCheckedAt > nowMs ||
    availabilityExpiresAt === null ||
    availabilityExpiresAt <= nowMs
  ) {
    return deniedDecision(
      operationName,
      context,
  ...
  const scopeHashes = [
    ...new Set((context.approvalRequirements ?? []).map(({ scopeHash }) => scopeHash)),
  ];
  ...
  const approvalIds = (context.approvals ?? []).map(({ approvalId }) => approvalId);
  if (
    !exactScalarSet(grant.approvalIds, approvalIds) ||
    !exactScalarSet(operation.approvalIds, approvalIds) ||
    operation.grantId !== grant.grantId ||
    grant.useLimit !== 1
  ) {
    return deniedDecision(
  ...
  if (
    ...
    grantIssuedAt === null ||
    grantIssuedAt < latestApprovalAt ||
    operationCreatedAt === null ||
    operationCreatedAt < grantIssuedAt ||
    operationCreatedAt > nowMs ||
    grantExpiresAt === null ||
  ...
  if (grant.consumedAt !== null) {
    return deniedDecision(
      ...
      { assignmentId: operation.assignmentId, state: 'grant.consumedAt' },
      checks,
    );
  }
...
function validatePreconditions(operationName, context, checks) {
  ...
  if (
    !sameSet(context.operation.preconditionArtifactIds, expected) ||
    ...
    return deniedDecision(
      ...
      {
        assignmentId: context.operation.assignmentId,
        state: 'preconditionArtifactIds',
      },
      checks,
    );
  }
...
function validateExecutor(operationName, context, checks, nowMs) {
  ...
  if (
    !executor ||
    !contractValid('operate-executor-registration', executor) ||
    registeredExecutor === null ||
    ...
    return deniedDecision(
      ...
      {
        assignmentId: operation.assignmentId,
        state: 'executor.binding',
      },
      checks,
    );
  }
  if (
    executor.health.status !== 'available' ||
    parseTime(executor.health.checkedAt) === null ||
    parseTime(executor.health.checkedAt) > nowMs ||
    parseTime(executor.health.expiresAt) === null ||
    parseTime(executor.health.expiresAt) <= nowMs ||
    !executor.operationKinds.includes(operationKind) ||
    !executor.capabilities.some((entry) =>
      versionedIdentityEqual(entry, action.requestedCapability),
    ) ||
    !executor.supportedActionKinds.some((entry) =>
      versionedIdentityEqual(entry, action.actionKind),
    ) ||
  ...
  if (operation.connector === null) {
    ...
    try {
      ...
      if (!versionedIdentityEqual(binding.connector, operation.connector))
...
function validateRollback(operationName, context, checks, nowMs) {
  ...
  if (
    !rollbackPlan ||
    !contractValid('operating-rollback-plan', rollbackPlan) ||
  ...
  if (
    ...
    parseTime(rollbackPlan.expiresAt) <= nowMs
  ) {
    return deniedDecision(
      operationName,
      context,
      ...
      {
        assignmentId: operation.assignmentId,
        state: 'rollbackPlan.eligibility',
      },
      checks,
    );
  }
  const originalMatches = (context.operationHistory ?? []).filter(
    ({ operationId }) => operationId === rollbackPlan.operationId,
  );
  const [original] = originalMatches;
  if (
    originalMatches.length !== 1 ||
    !contractValid('operating-governed-operation', original) ||
    ...
    return deniedDecision(
      ...
      {
        assignmentId: operation.assignmentId,
        state: 'operationHistory.originalOperationId',
      },
      checks,
    );
  }
...
function evaluateActionOperation(operationName, context) {
  const checks = [];
  const allowedKinds =
    operationName === 'operate.action.approve' ? ['human', 'engine'] : ['engine'];
  const actorFailure = validateActor(operationName, context, allowedKinds, checks);
  if (actorFailure) return actorFailure;
  const states =
    operationName === 'operate.action.approve'
      ? ['proposed']
  ...
  if (nowMs === null) {
    return deniedDecision(
      ...
      context,
      'RESULT_CONTRACT_INVALID',
      'now must be an explicit valid timestamp.',
      { state: 'now' },
      checks,
    );
  }
  ...
  if (operationName !== 'operate.action.approve') {
    const replayOperationFailure = validateOperationBinding(operationName, context, checks, {
      live: false,
      nowMs,
    });
  ...
  const operationFailure = validateOperationBinding(operationName, context, checks, {
    live: true,
    nowMs,
  });
...
export function evaluateOperateAuthorityV2(operation, context = {}) {
  if (!GOVERNED_OPERATIONS.has(operation)) {
    return deniedDecision(
      ...
      `operation ${operation} is not governed by authorization-v2.`,
      { state: 'operation' },
    );
  }
...
export function assertOperatingActionAuthorityTupleV2(action) {
  ...
  if (
    !contractValid('operating-action', action) ||
    fields.some((field) => !Object.hasOwn(action, field))
  ) {
    throw new PipelineError(
      'ACTION_REVISION_MISMATCH',
      'Action lacks a complete contract-valid authority tuple.',
      '',
      {
        retryable: false,
...
export function deriveOperateAuthorityAllowedActionsV2(context = {}) {
  const labels = new Map(
    OPERATE_CONTRACT_CATALOG_V2.actions.map(({ operationId, label }) => [operationId, label]),
  );
  const effects = new Map(
    OPERATE_CONTRACT_CATALOG_V2.operations.map(({ id, effect }) => [id, effect]),
  );
  const result = [];
