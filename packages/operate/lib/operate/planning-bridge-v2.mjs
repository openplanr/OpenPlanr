  ...
  for (const record of records) {
    ...
    if (typeof identity !== 'string' || identity.length === 0 || index.has(identity)) {
      fail(
        ...
        `Planning bridge requires unique scope-local ${entityName} identities.`,
...
function scopedRecord(records, scope, identityField, identity, entityName) {
  return uniqueIdentityIndex(
    (records ?? []).filter((record) => sameScope(record, scope)),
    identityField,
    entityName,
  ).get(identity);
}
...
function assertCurrentAction(action, state, scope) {
  ...
  if (
    ...
    fail(
      ...
      'Planning bridge requires the exact current governed Action revision.',
...
function assertCurrentDecision(decision, state, scope) {
  ...
  if (
    ...
    sha256Jcs(current) !== decision.decisionHash ||
    current.state !== 'approved'
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Planning bridge requires the exact current approved Decision revision.',
...
function canonicalPlanningMetric(state, currentAction, plan, sourceVerificationPlanEvent, scope) {
  const currentMetrics = (state.metrics ?? []).filter(
    (metric) => sameScope(metric, scope) && metric.metricId === currentAction.metricId,
  );
  if (currentMetrics.length > 1) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning proposal metric identity is ambiguous in the current operating scope.',
    );
  }
  ...
  if (!sourceVerificationPlanEvent) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning proposal requires the exact verification-plan Event for canonical metric recovery.',
    );
  }
  let reconstructed;
  try {
    reconstructed = reconstructOperatingVerificationPlanActionV2({
      state,
      event: sourceVerificationPlanEvent,
    });
  } catch (cause) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning proposal could not recover the canonical plan-bound metric.',
      {
  ...
  const immutableFields = Object.keys(expectedAction).filter(
    (field) => !['state', 'updatedAt'].includes(field),
  );
  if (
    reconstructed.verificationPlan.verificationPlanId !== plan.verificationPlanId ||
    sha256Jcs(reconstructed.verificationPlan) !== sha256Jcs(plan) ||
    immutableFields.some(
      (field) => sha256Jcs(expectedAction[field]) !== sha256Jcs(currentAction[field]),
    ) ||
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Recovered Planning metric does not retain the exact Action, plan, scope, and model-state binding.',
    );
  }
  const snapshots = (state.operatingSnapshots ?? []).filter(
    (candidate) => sameScope(candidate, scope) && candidate.snapshotId === reconstructed.snapshotId,
  );
  if (snapshots.length !== 1) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Recovered Planning metric lost its unique immutable snapshot binding.',
    );
  }
...
export function createOperatingDeliveryRouteV1({
  state,
  action,
  route,
  rationale,
  ...
  if (!ROUTES.includes(route))
    fail('RESULT_CONTRACT_INVALID', 'Delivery route must be one of the four canonical values.');
...
export function assertOperatingDeliveryRouteRevisionV1(previous, next) {
  ...
  if (
    previous.routeHash !== sha256Jcs(without(previous, 'routeHash')) ||
    next.routeHash !== sha256Jcs(without(next, 'routeHash'))
  )
    fail('E_OPERATE_BINDING_MISMATCH', 'Delivery route canonical hash is invalid.');
  if (
    previous.action.actionId === next.action.actionId &&
    previous.action.revision === next.action.revision &&
    previous.route !== next.route
  )
    fail(
      'ACTION_REVISION_MISMATCH',
      'Changing delivery route requires a new bound Action revision.',
    );
  return next;
}

function assertRoleCustody({
  assignment,
  submission,
  artifact,
  output,
  ...
  if (
    ...
    output.assignmentId !== assignment.assignmentId ||
    output.roleId !== assignment.roleId ||
    output.roleKind !== assignment.assignmentKind ||
    sha256Jcs(output.sourceArtifactValue) !== artifact.canonicalHash
  ) {
    fail(
      'ARTIFACT_HASH_MISMATCH',
      'Planning perspectives require exact cycle-scoped validated Assignment/Submission/Artifact proof.',
...
function exactRoleInputBundle({ assignment, value, artifacts, state, plan, snapshot }) {
  try {
    assertProtocolArtifact('operating-intelligence-input-bundle', value, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning perspectives require a contract-valid role-scoped intelligence bundle.',
      {
        assignmentId: assignment.assignmentId,
        cause: cause?.code ?? null,
      },
  ...
  if (
    ...
    fail(
      'ARTIFACT_HASH_MISMATCH',
      'Planning perspective input bundle lost exact Assignment, Artifact, or scope custody.',
      {
        assignmentId: assignment.assignmentId,
  ...
  const operatingStates = (state.operatingModelStates ?? []).filter(
    (candidate) =>
  ...
  if (
    operatingStates.length !== 1 ||
    operatingStates[0].runtimeHash !== value.operatingState.sourceRuntimeHash
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Planning perspective bundle lost its exact bounded operating-state source.',
      {
        assignmentId: assignment.assignmentId,
...
function perspectiveProjection(roleKind, projected) {
  if (roleKind === 'advisor') {
    ...
    return {
      stance:
        projected.outcome === 'recommendation'
          ? 'support'
  ...
  const constraints = [
    ...
    ...projected.alternatives.flatMap((alternative) => [
...
function chairPerspective({ value, artifact, decision, plan, snapshot, scope }) {
  try {
    assertProtocolArtifact('operating-decision-ledger', value, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'The accepted Chair output is not a canonical Decision ledger.',
      { cause: cause?.code ?? null },
    );
  }
  if (
    ...
    value.sourceArtifactId !== plan.sourceArtifactId ||
    artifact.artifactId !== decision.sourceArtifactId
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'The accepted Chair ledger does not bind the exact Decision source chain.',
    );
  }
  const candidates = value.decisions.filter(
    (entry) =>
  ...
  if (candidates.length !== 1)
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'The normative Chair synthesis is not the unique canonical source of the current Decision.',
    );
...
function derivePerspectiveSummaries(state, scope, cycleId, decision, accessLevel, acceptedOutputs) {
  const artifacts = uniqueIdentityIndex(
    state.artifacts.filter((artifact) => sameScope(artifact, scope)),
    'artifactId',
    'Artifact',
  );
  const outputAssignmentIds = new Set(
    (acceptedOutputs ?? []).map((output) => output?.assignmentId),
  );
  ...
  const submissions = uniqueIdentityIndex(
    state.submissions.filter(
      (submission) => submission.cycleId === cycleId && submission.state === 'accepted',
    ),
    'assignmentId',
    'accepted Submission',
  );
  if (!Array.isArray(acceptedOutputs))
    fail('RESULT_CONTRACT_INVALID', 'Planning perspectives require exact accepted role outputs.');
  ...
  for (const output of acceptedOutputs) {
    ...
    if (
      ...
      keys.some((key) => !(key in output)) ||
      !['advisor', 'challenger', 'chair'].includes(output.roleKind) ||
      [output.assignmentId, output.artifactId, output.roleId].some(
        (value) => typeof value !== 'string' || value.length === 0,
      ) ||
      !output.sourceArtifactValue ||
      typeof output.sourceArtifactValue !== 'object' ||
      Array.isArray(output.sourceArtifactValue) ||
      !output.inputBundleValue ||
      typeof output.inputBundleValue !== 'object' ||
      Array.isArray(output.inputBundleValue) ||
      outputs.has(output.assignmentId)
    )
      fail('RESULT_CONTRACT_INVALID', 'Accepted perspective output is not closed or unique.');
    outputs.set(output.assignmentId, output);
  }
  if (ACCESS_ORDER.indexOf(accessLevel) < 0)
    fail('RESULT_CONTRACT_INVALID', 'Proposal actor access level is invalid.');
  const intelligencePlans = uniqueIdentityIndex(
    (state.intelligencePlans ?? []).filter((record) => sameScope(record, scope)),
    'planId',
    'intelligence plan',
  );
  const snapshots = uniqueIdentityIndex(
    (state.operatingSnapshots ?? []).filter((record) => sameScope(record, scope)),
    'snapshotId',
    'operating snapshot',
  );
  ...
  const summaries = assignments
    .map((assignment) => {
      ...
      if (!output)
        fail(
          'ARTIFACT_HASH_MISMATCH',
          'Planning perspectives require the exact accepted role body proof.',
          { assignmentId: assignment.assignmentId },
        );
      assertRoleCustody({ assignment, submission, artifact, output, cycleId, scope, accessLevel });
      const planId =
        output.sourceArtifactValue.intelligencePlanId ??
        state.intelligencePlans.find(
          (candidate) => candidate.snapshotId === output.sourceArtifactValue.snapshotId,
        )?.planId;
      const snapshotId =
        output.sourceArtifactValue.snapshotId ??
        state.intelligencePlans.find(
          (candidate) => candidate.planId === output.sourceArtifactValue.intelligencePlanId,
        )?.snapshotId;
      const plan = intelligencePlans.get(planId);
      const snapshot = snapshots.get(snapshotId);
      if (!plan || !snapshot || plan.snapshotId !== snapshot.snapshotId)
        fail(
          'E_OPERATE_BINDING_MISMATCH',
          'Accepted role output lost its exact intelligence-plan snapshot binding.',
          {
      ...
      if (assignment.assignmentKind === 'advisor')
        advisorProofs.push({ artifact, value: output.sourceArtifactValue });
      const perspective = chair
        ? projected
        : perspectiveProjection(assignment.assignmentKind, projected);
      assertNoSensitiveContext(perspective);
      return {
        roleId: assignment.roleId,
        roleVersion: PROTOCOL_VERSION,
        roleKind: assignment.assignmentKind,
        artifactId: artifact.artifactId,
        artifactHash: artifact.rawHash,
        accessClassification: artifact.sensitivity,
    ...
    .sort(
      (left, right) =>
        left.roleKind.localeCompare(right.roleKind) || left.roleId.localeCompare(right.roleId),
    );
  if (
    outputs.size !== assignments.length ||
    counts.advisor < 1 ||
    counts.challenger !== 1 ||
    counts.chair !== 1
  )
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning proposal requires only the exact accepted advisor outputs, exactly one Challenger, and exactly one normative Chair synthesis.',
    );
  return summaries;
}
...
function safeEvidence(state, scope, cycleId, decision, accessLevel, sourceSnapshot = null) {
  ...
  const refs = uniqueIdentityIndex(
    (state.evidenceRefs ?? []).filter((record) => sameScope(record, scope)),
    'evidenceRefId',
    'EvidenceRef',
  );
  const artifacts = uniqueIdentityIndex(
  ...
  return [...decision.evidenceRefIds].sort().map((evidenceRefId) => {
    ...
    if (
      ...
      artifact.sensitivity !== ref.classification
    )
      fail(
        'EVIDENCE_SOURCE_SCOPE_MISMATCH',
        'Planning evidence must derive from one exact Decision-linked, cycle-scoped EvidenceRef and Artifact.',
        { evidenceRefId },
      );
...
export function buildOperatingPlanningProposalV1(state, options = {}) {
  ...
  if (foreignField)
    fail(
      'RESULT_CONTRACT_INVALID',
      `Planning proposal input contains unsupported caller context: ${foreignField}.`,
    );
  const {
    scope,
    decision,
  ...
  assertProtocolArtifact('operating-runtime-state', state, { protocolVersion: PROTOCOL_VERSION });
  assertInstant(createdAt, 'createdAt');
  assertInstant(previewExpiresAt, 'previewExpiresAt');
  if (
    !scope ||
    typeof scope !== 'object' ||
    ['scopeId', 'domainId', 'domainVersion'].some(
      (field) => typeof scope[field] !== 'string' || scope[field].length === 0,
    ) ||
    !actor ||
    ...
    fail(
      ...
      'Planning proposal requires explicit scope and actor bindings.',
    );
  }
  if (actor.kind !== 'human')
    fail('E_OPERATE_BINDING_MISMATCH', 'Planning proposal review must be bound to a human actor.');
  ...
  if (currentAction.sourceDecisionId !== currentDecision.decisionId)
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Planning Action does not originate from the selected Decision.',
    );
  assertOperateExperienceArtifactV2('operating-delivery-route', deliveryRoute);
  if (
    deliveryRoute.route !== 'planning-work' ||
    !sameScope(deliveryRoute, scope) ||
    ...
    deliveryRoute.routeHash !== sha256Jcs(without(deliveryRoute, 'routeHash'))
  )
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Only an exact current planning-work route can produce a planning proposal.',
    );
  assertNoSensitiveContext({ framing, omissions });
  const plan = scopedRecord(
    state.verificationPlans,
    scope,
    'verificationPlanId',
    currentAction.verificationPlanId,
    'verification plan',
  );
  if (
    !plan ||
    !sameScope(plan, scope) ||
    plan.actionId !== currentAction.actionId ||
    plan.metricId !== currentAction.metricId ||
    plan.baseline !== currentAction.baseline ||
    plan.target !== currentAction.target ||
    plan.window !== currentAction.verificationWindow
  )
    fail(
      'RESULT_CONTRACT_INVALID',
      'Planning proposal requires the exact original Action verification plan.',
    );
  const canonicalMetric = canonicalPlanningMetric(
    state,
    currentAction,
    plan,
    sourceVerificationPlanEvent,
  ...
  const metricHash = sha256Jcs(metric);
  const verificationPlanHash = sha256Jcs(plan);
  const acceptedPerspectiveSummaries = derivePerspectiveSummaries(
    state,
    scope,
    currentAction.sourceCycleId,
    currentDecision,
    actor.accessLevel,
    acceptedOutputs,
  );
  const safeEvidenceRefs = safeEvidence(
    state,
    scope,
    currentAction.sourceCycleId,
    currentDecision,
    actor.accessLevel,
    canonicalMetric.snapshot,
  );
  const correlationId = stableId('corr', {
    scope,
    decision: decision.decisionHash,
    action: action.actionHash,
  });
  const proposalId = stableId('oprop', {
    correlationId,
    eventHead: state.eventHead,
    framing,
    evidence: safeEvidenceRefs.map(({ evidenceRefId, artifactHash }) => ({
      evidenceRefId,
      artifactHash,
    })),
  });
  ...
  const base = {
    ...
    confirmation: null,
    ...scope,
    cycleId: currentAction.sourceCycleId,
    eventHead: clone(state.eventHead),
    decision: {
      decisionId: currentDecision.decisionId,
      revision: currentDecision.revision,
      decisionHash: decision.decisionHash,
      sourceArtifactId: currentDecision.sourceArtifactId,
      title: currentDecision.title,
      outcome: currentDecision.outcome,
      rationale: currentDecision.rationale,
      confidence: currentDecision.confidence,
      evidenceRefIds: [...currentDecision.evidenceRefIds].sort(),
    },
    action: {
      actionId: currentAction.actionId,
      revision: currentAction.revision,
      actionHash: currentAction.actionHash,
      sourceArtifactId: currentAction.sourceArtifactId,
      title: currentAction.title,
      expectedResult: currentAction.expectedResult,
      metricId: currentAction.metricId,
      verificationPlanId: currentAction.verificationPlanId,
    },
    deliveryRoute: clone(deliveryRoute),
    acceptedPerspectiveSummaries,
    evidence: safeEvidenceRefs,
    omissions: omissions
      .map(clone)
      .sort(
        (a, b) =>
          a.reason.localeCompare(b.reason) || a.classification.localeCompare(b.classification),
      ),
    ...
    verification: {
      ...
      window: plan.window,
      method: plan.method,
      revisitConditions: [...plan.evaluationRules].sort(),
    },
    actor: { actorId: actor.actorId, kind: actor.kind },
    preview,
...
export function confirmOperatingPlanningProposalV1(
  proposal,
  { actorId, proposalRevision, proposalHash, eventHead, previewDigest, confirmedAt } = {},
) {
  assertOperatingPlanningProposalV1(proposal);
  assertInstant(confirmedAt, 'confirmedAt');
  if (
    proposal.state !== 'review-required' ||
    proposal.revision !== 1 ||
    proposal.confirmation !== null
  )
    fail(
      'STATE_TRANSITION_INVALID',
      'Only the current review-required proposal may be confirmed once.',
    );
  if (
    actorId !== proposal.actor.actorId ||
    proposal.actor.kind !== 'human' ||
    proposalRevision !== proposal.revision ||
    proposalHash !== proposal.proposalHash ||
    !sameHead(eventHead, proposal.eventHead) ||
    previewDigest !== proposal.preview.digest
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Confirmation actor, proposal revision/hash, Event head, or preview digest differs from the reviewed proposal.',
    );
  }
  if (
    Date.parse(confirmedAt) < Date.parse(proposal.preview.issuedAt) ||
    Date.parse(confirmedAt) > Date.parse(proposal.preview.expiresAt)
  )
    fail(
      'STATE_TRANSITION_INVALID',
      'Confirmation must occur within the exact preview validity window.',
    );
  const confirmationBase = {
    confirmationId: stableId('pcfm', { proposalHash: proposal.proposalHash, actorId, confirmedAt }),
    reviewProposalHash: proposal.proposalHash,
...
export function createOperatingOriginV1({ proposal, spec, actor, transaction, createdAt } = {}) {
  ...
  if (proposal.state !== 'approved' || proposal.revision !== 2)
    fail(
      'STATE_TRANSITION_INVALID',
      'Only the exact human-confirmed current proposal may produce an operating origin.',
    );
  if (actor.actorId !== proposal.confirmation.actorId || actor.kind !== 'human')
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Operating origin actor differs from the human confirmation proof.',
    );
  ...
  const base = {
    ...
    eventHead: clone(proposal.eventHead),
    decision: {
      id: proposal.decision.decisionId,
      revision: proposal.decision.revision,
      hash: proposal.decision.decisionHash,
    },
    action: {
      id: proposal.action.actionId,
      revision: proposal.action.revision,
      hash: proposal.action.actionHash,
    },
    metric: clone(proposal.metric),
    verification: {
      verificationPlanId: proposal.verification.verificationPlanId,
      verificationPlanHash: proposal.verification.verificationPlanHash,
      metricId: proposal.metric.metricId,
      metricHash: proposal.metric.metricHash,
      window: proposal.verification.window,
    },
    evidence: proposal.evidence.map(
      ({ evidenceRefId, artifactId, artifactHash, classification, accessState }) => ({
        evidenceRefId,
        artifactId,
        artifactHash,
        classification,
...
export function buildOperatingDeliveryEvidenceV1({
  origin,
  planRun,
  shipRun,
  tasks,
  ...
  if (
    'verificationPlanId' in shipRun ||
    'verificationPlanHash' in shipRun ||
    'metricId' in shipRun ||
    'metricHash' in shipRun
  )
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'SHIP run cannot substitute proposal metric or verification bindings.',
    );
  if (
    (deliveryStatus === 'rolled-back') !== (rollback !== null) ||
    (rollback !== null &&
      (rollback.originalShipRunId !== shipRun.runId || rollback.status !== 'succeeded'))
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Rolled-back delivery requires one exact successful rollback custody bound to the SHIP run.',
    );
  }
  ...
  const base = {
    ...
    schemaVersion: '1.0.0',
    protocolVersion: PROTOCOL_VERSION,
    deliveryEvidenceId: stableId('dlev', {
    ...
    proposalRevision: origin.proposalRevision,
    scopeId: origin.scopeId,
    domainId: origin.domainId,
    domainVersion: origin.domainVersion,
    decision: clone(origin.decision),
    action: clone(origin.action),
    metric: clone(origin.metric),
    verification: clone(origin.verification),
    spec: {
      specId: origin.spec.specId,
      contentHash: origin.spec.contentHash,
      originHash: origin.originHash,
    },
    planRun: clone(planRun),
    shipRun: clone(shipRun),
    tasks: tasks.map(clone).sort((a, b) => a.taskId.localeCompare(b.taskId)),
    changedSurfaces: [...changedSurfaces].sort(),
    qa: clone(qa),
    limitations: [...limitations].sort(),
    artifacts: artifacts.map(clone).sort((a, b) => a.artifactId.localeCompare(b.artifactId)),
    classification,
    summary,
    deliveryStatus,
