...
function contractRecord(kind, value, subject, details = {}) {
  try {
    ...
    fail(`${subject} is not a valid Protocol 2.0 record.`, {
      ...details,
...
export function deriveOperatingIntelligenceAssignmentIdV2(planId, roleId, roleVersion = '2.0.0') {
  if (![planId, roleId, roleVersion].every(nonEmptyString)) {
    fail(
      'An intelligence Assignment identity requires explicit plan, role, and role-version identities.',
    );
  }
 ...
 */
export function assertOperatingValidatedDependencyProofV2({
  assignment,
  submission,
  artifact,
  replay,
  ...
  if (
    ...
    fail(
      'A validated dependency lacks an exact accepted Submission, Artifact, Assignment, claim, contract, or replay binding.',
      {
        assignmentId: assignment.assignmentId,
        submissionId: submission.submissionId,
        artifactId: artifact.artifactId,
...
function baseEvidenceAbsences(assignment) {
  ...
  return (assignment.inputAbsences ?? [])
    .filter((absence) => {
      ...
      const expectedId =
        context?.inputBundle?.bundleId && assignment.roleVersion
          ? deriveOperatingEvidenceAbsenceIdV2({
...
export function resolveOperatingAssignmentInputAbsencesV2(
  assignment,
  { dependencyProofs = [], assignments = [], intelligencePlans = [] } = {},
) {
  ...
  const plan = intelligencePlans.find((candidate) =>
    candidate.selectedRoles.some(
      ({ roleId, roleVersion }) =>
        deriveOperatingIntelligenceAssignmentIdV2(candidate.planId, roleId, roleVersion) ===
  ...
  const ordered = absences.sort(
    (left, right) =>
      left.kind.localeCompare(right.kind) ||
      String(left.roleKind ?? '').localeCompare(String(right.roleKind ?? '')) ||
      String(left.roleId ?? left.requirementId ?? '').localeCompare(
        String(right.roleId ?? right.requirementId ?? ''),
      ) ||
...
function assertPolicy(policy, assignment) {
  if (!plainObject(policy))
    fail('Assignment dependencyPolicy must be an object.', {
      assignmentId: assignment.assignmentId,
    });
  const dependencies = assignment.dependsOn;
  const compiled = COMPILED_DEPENDENCY_POLICIES.get(policy.kind);
  if (!compiled)
    fail('Assignment dependencyPolicy kind is not compiled.', {
      assignmentId: assignment.assignmentId,
      kind: policy.kind,
    });
  if (policy.kind === 'none') {
    if (
      compiled.minimumValidated !== 0 ||
      Object.keys(policy).length !== 1 ||
      dependencies.length !== 0
    ) {
      fail('The none dependency policy is valid only for zero-dependency Assignments.', {
        assignmentId: assignment.assignmentId,
      });
    }
    return;
  ...
  if (policy.kind === 'all-required') {
    if (
      compiled.minimumValidated !== 'all' ||
      Object.keys(policy).length !== 1 ||
      dependencies.length === 0
    ) {
      fail('The all-required dependency policy requires one or more declared dependencies.', {
        assignmentId: assignment.assignmentId,
      });
    }
    return;
  ...
  if (policy.kind === 'threshold') {
    if (
      ...
      policy.allowedTerminalOutcomes.some(
        (outcome) => !compiled.allowedTerminalOutcomes.includes(outcome),
      )
    ) {
  ...
  fail('Assignment dependencyPolicy kind is not implemented.', {
    assignmentId: assignment.assignmentId,
    kind: policy.kind,
  });
}
...
export function validateOperatingAssignmentGraphV2(assignments) {
  ...
  for (const assignment of assignments) {
    ...
    if (byId.has(assignment.assignmentId))
      fail('Duplicate Assignment identity in dependency graph.', {
        assignmentId: assignment.assignmentId,
      });
    if (
      typeof assignment.cycleId !== 'string' ||
      assignment.cycleId.length === 0 ||
      !uniqueStrings(assignment.dependsOn)
    ) {
      fail('An Assignment has malformed cycle or dependency references.', {
        assignmentId: assignment.assignmentId,
      });
    }
    ...
    if (assignment.assignmentKind === 'verification') {
      ...
      if (
        ...
        fail(
          'Verification Assignment must be directly owned by one governed operation and produce one Outcome or live-evidence ingestion.',
          {
            assignmentId: assignment.assignmentId,
  ...
  for (const assignment of byId.values()) {
    for (const dependencyId of assignment.dependsOn) {
      ...
      if (!dependency)
        fail('Assignment dependency references an unknown Assignment.', {
          assignmentId: assignment.assignmentId,
          dependencyId,
        });
      if (dependencyId === assignment.assignmentId)
        fail('Assignment cannot depend on itself.', {
          assignmentId: assignment.assignmentId,
        });
      if (dependency.cycleId !== assignment.cycleId)
        fail('Assignment dependencies cannot cross cycle boundaries.', {
          assignmentId: assignment.assignmentId,
          dependencyId,
  ...
  const visit = (assignmentId) => {
    ...
    if (visiting.has(assignmentId))
      fail('Assignment dependency graph contains a cycle.', { assignmentId });
...
export function validateOperatingIntelligenceAssignmentGraphV2(
  plan,
  assignments,
  { allowLifecycleProgress = false, canonicalContext = null } = {},
) {
  try {
    assertOperateIntelligencePlanContractV2(plan);
  } catch (cause) {
    fail('Intelligence Assignment graph requires a contract-valid plan.', {
      cause: cause?.code ?? null,
    });
  }
  ...
  if (byId.size !== plan.selectedRoles.length) {
    fail(
      'Intelligence Assignment graph must contain exactly one Assignment for every selected role.',
      {
        planId: plan.planId,
      },
    );
  ...
  for (const assignment of byId.values()) {
    ...
    if (byRole.has(assignment.roleId))
      fail('Intelligence Assignment graph repeats a selected role.', { roleId: assignment.roleId });
    if (
      !allowLifecycleProgress &&
      (assignment.state !== 'pending' ||
        assignment.availableAt !== null ||
        assignment.completedAt !== null ||
        assignment.claim !== null ||
        assignment.terminalOutcome !== null)
    ) {
      fail('Intelligence Assignment intents must begin pending without lifecycle proof.', {
        assignmentId: assignment.assignmentId,
      });
    }
  ...
  if (plan.omittedRoles.some(({ roleId }) => byRole.has(roleId))) {
    fail('An explicitly omitted role cannot receive an Assignment intent.', {
      planId: plan.planId,
    });
  }
  ...
  const canonicalDomain = canonicalContext
    ? OPERATE_CONTRACT_CATALOG_V2.extensions.domains.find(
        (domain) =>
  ...
  if (
    ...
    fail(
      'Intelligence Assignment checkpoint context differs from its canonical plan, snapshot, or Cycle.',
      {
        planId: plan.planId,
      },
    );
  ...
  for (const role of plan.selectedRoles) {
    ...
    const canonicalRole =
      canonicalDomain?.roles.find((candidate) => candidate.roleId === role.roleId) ?? null;
    const context = assignment?.intelligenceContext;
    const baseInputArtifactIds = context
      ? [
          ...new Set([
            context.inputBundle.bundleArtifactId,
            ...context.inputBundle.issuedEvidence.map(
              ({ evidenceArtifactId }) => evidenceArtifactId,
            ),
          ]),
        ].sort()
      : [];
    const contextMatchesPlan =
      context &&
      ...
      (!canonicalContext ||
        (sha256Jcs(context.sourceArtifactIds) ===
          sha256Jcs([...canonicalContext.snapshot.sourceArtifactIds].sort()) &&
          sha256Jcs(context.evidenceRefIds) === sha256Jcs(contextEvidenceRefIds)));
    const inputMatchesPlan =
      assignment &&
      role.inputArtifactIds.every((artifactId) =>
    ...
    const expectedDependencies = role.dependsOnRoleIds
      .map((roleId) => {
        if (!selectedIds.has(roleId) || !byRole.has(roleId)) {
          fail('Selected role dependency does not resolve to another selected Assignment.', {
            roleId: role.roleId,
            dependencyRoleId: roleId,
          });
        }
    ...
    if (sha256Jcs(assignment.dependsOn) !== sha256Jcs(expectedDependencies)) {
      fail('Assignment dependencies differ from the explainable role plan.', {
        roleId: role.roleId,
      });
    }
    if (canonicalContext) {
      const expectedDependencyPolicy =
      ...
      if (
        ...
        fail(
          'Intelligence Assignment immutable intent differs from the compiler-owned role registry or plan.',
          {
            planId: plan.planId,
            roleId: role.roleId,
            assignmentId: assignment.assignmentId,
  ...
  if (!chair || chair.dependsOn.length === 0) {
    fail('Chair work requires at least one declared selected-role dependency.', {
      planId: plan.planId,
    });
  }
  return byId;
...
function dependencyProofs(
  ...
  for (const dependencyId of [...assignment.dependsOn].sort()) {
    ...
    if (dependency.state === 'validated') {
      proofs.push(
        validatedProof(dependency, acceptedByAssignment, artifactsById, replayBySubmission),
      );
      continue;
    }
...
function policySatisfied(assignment, proofs) {
  ...
  const qualified = proofs.filter(
    ({ outcome }) => outcome === 'validated' || permitted.has(outcome),
  );
...
export function deriveOperatingAssignmentTerminalIntentsV2({
  ...
  const replayBySubmission = uniqueIndex(
    submissionReplayIndex,
    'submissionId',
    'Submission replay',
  );
  ...
  for (const assignment of [...byId.values()]
    ...
    if (
      assignment.dependsOn.some(
        (dependencyId) => byId.get(dependencyId)?.assignmentKind !== 'advisor',
      )
    ) {
    ...
    if (
      proofs === null ||
      proofs.length === 0 ||
      proofs.some(({ outcome }) => outcome === 'validated')
    )
      continue;
...
export function assertOperatingAssignmentTerminalPayloadV2(assignment, payload, state) {
  const intent = deriveOperatingAssignmentTerminalIntentsV2(state).find(
...
function acceptedSubmissionIndex(submissions) {
  ...
  for (const submission of submissions ?? []) {
    ...
    if (accepted.has(submission.assignmentId))
      fail('An Assignment has multiple accepted submission proofs.', {
        assignmentId: submission.assignmentId,
      });
...
function uniqueIndex(records, key, label) {
  ...
  for (const record of records ?? []) {
    const id = record?.[key];
    if (!nonEmptyString(id) || index.has(id))
      fail(`${label} identities must be present and unique.`, { [key]: id ?? null });
    index.set(id, record);
  }
  return index;
}
...
export function deriveOperatingAssignmentReleaseIntentsV2({
  ...
  for (const plan of intelligencePlans ?? []) {
    ...
    const planAssignments = plan.selectedRoles.map((role) => {
      ...
      if (!assignment)
        fail('A persisted intelligence plan is missing its canonical Assignment.', {
          planId: plan.planId,
          assignmentId,
        });
      return assignment;
    });
    validateOperatingIntelligenceAssignmentGraphV2(plan, planAssignments, {
    ...
    for (const role of plan.selectedRoles) {
      ...
      if (assignment.availableAt === null) {
        if (
          sha256Jcs(assignment.inputArtifactIds) !== sha256Jcs(baseIntent.inputArtifactIds) ||
      ...
      if (
        sha256Jcs(assignment.inputArtifactIds) !== sha256Jcs(expectedInputs) ||
        sha256Jcs(assignment.inputAbsences) !== sha256Jcs(expectedAbsences)
      ) {
        fail(
          'Released intelligence Assignment custody differs from durable dependency and absence proof.',
          {
            planId: plan.planId,
            assignmentId: assignment.assignmentId,
          },
  ...
  for (const assignment of [...byId.values()]
    ...
    const orderedProofs = [...satisfied].sort((left, right) =>
      left.assignmentId.localeCompare(right.assignmentId),
    );
...
export function assertOperatingAssignmentAvailabilityPayloadV2(assignment, payload, state) {
  const intent = deriveOperatingAssignmentReleaseIntentsV2(state).find(
    ({ assignmentId }) => assignmentId === assignment.assignmentId,
  );
  if (!intent || !plainObject(payload)) {
    fail('Assignment availability has no scheduler-owned release intent.', {
      assignmentId: assignment.assignmentId,
    });
  }
  const expected = {
