...
function sortedBy(records, id) {
  ...
  if (new Set(normalized.map((record) => record[id])).size !== normalized.length) {
    fail(
      ...
      'Scenario/trigger transition contains duplicate durable identities.',
      { id },
    );
  }
  return normalized;
...
function assertSnapshotState(snapshot, state) {
  ...
  try {
    ...
    fail(
      ...
      'Scenario and trigger records require one valid immutable snapshot and state.',
      {
...
function evidenceArtifactMap(records, snapshot, selectedEvidenceRefs) {
  if (!Array.isArray(records))
    fail('RESULT_CONTRACT_INVALID', 'Scenario/trigger records require Evidence Artifact metadata.');
  ...
  for (const artifact of records) {
    ...
    try {
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Scenario/trigger evidence requires typed Evidence Artifacts.',
        { cause: cause?.code ?? null },
      );
    }
    if (
      ...
      fail(
        ...
        'Scenario/trigger Evidence Artifacts must be unique and match the snapshot scope.',
        {
...
function evidenceMap(records, snapshot, evidenceArtifacts) {
  if (!Array.isArray(records))
    fail('RESULT_CONTRACT_INVALID', 'Scenario/trigger records require typed EvidenceRefs.');
  ...
  for (const record of records) {
    ...
    try {
      assertProtocolArtifact('operating-evidence-ref', record, {
        protocolVersion: PROTOCOL_VERSION,
      });
    } catch (cause) {
      fail('RESULT_CONTRACT_INVALID', 'Scenario/trigger evidence must use typed EvidenceRefs.', {
        cause: cause?.code ?? null,
      });
    }
    if (
      ...
      fail(
        'OPERATING_SCOPE_INVALID',
        'Scenario/trigger EvidenceRefs selected by a snapshot must be unique and in scope.',
        {
  ...
  for (const record of selected) {
    ...
    if (
      ...
      fail(
        'OPERATING_SCOPE_INVALID',
        'Scenario/trigger EvidenceRefs must bind their exact selected Evidence Artifact.',
        {
...
function assertEvidence(ids, evidence, subject, { sourceArtifactId, required = false } = {}) {
  if (!Array.isArray(ids) || (required && ids.length === 0) || new Set(ids).size !== ids.length) {
    fail(
      'RESULT_CONTRACT_INVALID',
      `${subject} requires a unique ${required ? 'nonempty ' : ''}typed EvidenceRef list.`,
    );
  }
  for (const id of ids) {
    ...
    if (!ref || ref.freshness === 'stale' || ref.sourceArtifactId !== sourceArtifactId) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} cannot use stale, foreign, or unresolved evidence.`,
        { evidenceRefId: id },
      );
    }
  }
...
function assertSnapshotArtifact(record, snapshot, subject) {
  if (!snapshot.sourceArtifactIds.includes(record.sourceArtifactId)) {
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} must name an accepted source Artifact declared by its snapshot.`,
      {
...
function assertScenario(record, snapshot, assumptions, evidence) {
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', 'Scenario record is not contract-valid.', {
      cause: cause?.code ?? null,
    });
  }
  if (
    ...
    fail(
      'OPERATING_SCOPE_INVALID',
      'Scenario must bind to the exact immutable snapshot and domain scope.',
      { scenarioId: record.scenarioId },
    );
  }
  assertSnapshotArtifact(record, snapshot, 'Scenario');
  assertEvidence(record.evidenceRefIds, evidence, 'Scenario', {
    sourceArtifactId: record.sourceArtifactId,
  });
  for (const [name, entry] of [
    ...
    assertEvidence(entry.evidenceRefIds, evidence, `Scenario ${name} case evidence`, {
  ...
  for (const assumptionId of assumptionIds) {
    if (!assumptions.has(assumptionId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Scenario assumptions must resolve to immutable operating state.',
        {
          scenarioId: record.scenarioId,
          assumptionId,
        },
  ...
  if (/\b(?:approve|execute|grant|capability|connector|policy)\b/iu.test(record.breakEven)) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Scenario analysis cannot encode an approval, capability, policy, connector, or execution instruction.',
      {
        scenarioId: record.scenarioId,
      },
    );
...
function assertTrigger(record, snapshot, evidence) {
  try {
    assertProtocolArtifact('operating-event-trigger', record, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    fail('RESULT_CONTRACT_INVALID', 'Event trigger record is not contract-valid.', {
      cause: cause?.code ?? null,
    });
  }
  if (
    ...
    fail(
      'OPERATING_SCOPE_INVALID',
      'Event trigger must bind to the exact immutable snapshot and domain scope.',
      { triggerId: record.triggerId },
    );
  }
  assertSnapshotArtifact(record, snapshot, 'Event trigger');
  assertEvidence(record.evidenceRefIds, evidence, 'Event trigger', {
    sourceArtifactId: record.sourceArtifactId,
    required: true,
  });
  if (
    /\b(?:approve|execute|grant|capability|connector|policy|assignment)\b/iu.test(record.condition)
  ) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Event trigger conditions may request ordinary observation/cycle work only; authority or execution language is not a trigger condition.',
      {
        triggerId: record.triggerId,
      },
    );
...
export function buildOperatingTriggerScenarioTransitionV2({
  ...
  for (const scenario of normalized.scenarios)
    assertScenario(scenario, checked.snapshot, assumptions, evidence);
