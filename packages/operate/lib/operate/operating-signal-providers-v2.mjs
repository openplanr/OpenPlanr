...
function explicitDomainContract(domainContract) {
  if (
    !domainContract ||
    typeof domainContract !== 'object' ||
    ['apiDomainId', 'id', 'version'].some((key) => typeof domainContract[key] !== 'string')
  ) {
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'Provider selection requires one explicit API-domain contract binding.',
      {},
    );
  }
...
function canonicalRegistry(registry) {
  try {
    ...
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'Provider selection requires one complete validated Operate extension registry.',
      {
...
function supportsDomain(registration, domainContract) {
  return registration.supportedDomains.some(
    (entry) =>
...
function select(registry, kind, providerId, { providerVersion, domainContract } = {}) {
  ...
  if (typeof providerVersion !== 'string' || providerVersion.length === 0) {
    fail(
      'OPERATING_PROVIDER_VERSION_REQUIRED',
      'Provider selection requires an explicit provider version.',
      {
        providerId,
      },
    );
  ...
  if (
    provider === null ||
    provider.implementation?.kind !== 'built-in' ||
...
export function selectOperatingSnapshotProviderV2(
...
export function selectOperatingMetricProviderV2(
  registry = OPEN_REFERENCE_OPERATE_EXTENSIONS_V2,
  request = {},
) {
...
export function selectOperatingVerificationProviderV2(
  registry = OPEN_REFERENCE_OPERATE_EXTENSIONS_V2,
  request = {},
) {
...
function exactAcceptedArtifacts(artifacts, scope, sourceArtifactIds) {
  if (!Array.isArray(artifacts)) {
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'A provider accepts only declared accepted Artifact descriptors.',
      {},
    );
  }
  const seen = new Set();
  ...
  for (const artifact of artifacts) {
    try {
      ...
      fail('OPERATING_PROVIDER_INPUT_INVALID', 'A provider input Artifact is invalid.', {
        cause: cause.code ?? null,
      });
    }
    if (!sameDomain(artifact, scope) || seen.has(artifact.artifactId)) {
      fail(
        'OPERATING_PROVIDER_INPUT_INVALID',
        'Provider Artifacts must be unique and match the declared scope.',
        {
  ...
  if (ids.sort().join('\u0000') !== expected.join('\u0000')) {
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'Provider Artifacts must exactly match their declared accepted inputs.',
      {
...
export function createOperatingSnapshotCandidateV2({
  ...
  return freeze({
    ...
    candidate: clone(verifiedSnapshot),
  });
}
...
export function createOperatingMetricObservationCandidateV2({
  ...
  try {
    ...
    assertProtocolArtifact('operating-metric-observation', observation, {
      protocolVersion: VERSION,
    });
  } catch (cause) {
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'A metric provider requires valid metric and observation contracts.',
      {
        cause: cause.code ?? null,
      },
    );
  ...
  if (selection.status !== 'selected') return selection;
  const accepted = exactAcceptedArtifacts(
    acceptedArtifacts,
    verifiedSnapshot,
    verifiedSnapshot.sourceArtifactIds,
  );
  if (
    !sameDomain(metric, verifiedSnapshot) ||
    !sameDomain(observation, verifiedSnapshot) ||
    observation.metricId !== metric.metricId ||
    observation.snapshotId !== verifiedSnapshot.snapshotId ||
    !accepted.has(observation.sourceArtifactId)
  ) {
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'A metric observation must bind the selected metric, snapshot, and accepted source Artifact.',
      {
...
export function createOperatingVerificationCandidateV2({
  ...
  try {
    ...
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'A verification provider requires valid plan and Outcome contracts.',
      {
        cause: cause.code ?? null,
      },
    );
  ...
  if (
    !sameDomain(outcome, verificationPlan) ||
    ...
    fail(
      'OPERATING_PROVIDER_INPUT_INVALID',
      'An Outcome must bind the selected verification plan and accepted source Artifact.',
      {
