...
function canonicalJsonArtifactBody(artifact, rawBytes, subject) {
  ...
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', `${subject} bytes are not valid UTF-8 JSON.`, {
      artifactId: artifact.artifactId,
    });
  }
  if (sha256Jcs(value) !== artifact.canonicalHash) {
    fail(
      ...
      `${subject} JSON does not match the accepted Artifact canonical hash.`,
      {
        artifactId: artifact.artifactId,
      },
    );
...
export function decodeOperatingIntelligenceArtifactBodyV2({
  artifact,
  rawBytes,
  subject = 'Intelligence result',
} = {}) {
...
export function projectOperatingAcceptedIntelligenceOutputV2({
  ...
  try {
    ...
    fail(
      ...
      'Accepted intelligence projection requires contract-valid public custody.',
      {
        cause: cause?.code ?? null,
      },
    );
  ...
  if (
    !assignment ||
    assignment.assignmentKind !== roleKind ||
    ...
    fail(
      ...
      'Accepted intelligence projection requires one exact role, plan, snapshot, and scope binding.',
    );
  }
  ...
  return freeze({
    ...
    alternatives:
      roleKind === 'advisor' ? clone(checked.alternatives) : clone(checked.missingAlternatives),
...
function indexArtifacts(records) {
  if (!Array.isArray(records))
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires an Artifact collection.',
    );
  ...
  for (const artifact of records) {
    try {
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Decision-ledger materialization requires valid Artifact metadata.',
        { cause: cause?.code ?? null },
      );
    }
    if (result.has(artifact.artifactId))
      fail('STATE_TRANSITION_INVALID', 'Artifact identities must be unique.');
...
function indexEvidence(records, snapshot) {
  if (!Array.isArray(records))
    fail('RESULT_CONTRACT_INVALID', 'Decision-ledger materialization requires EvidenceRefs.');
  ...
  for (const evidenceRef of records) {
    ...
    try {
      assertProtocolArtifact('operating-evidence-ref', evidenceRef, {
        protocolVersion: PROTOCOL_VERSION,
      });
    } catch (cause) {
      fail(
        'RESULT_CONTRACT_INVALID',
        'Decision-ledger materialization requires typed EvidenceRefs.',
        { cause: cause?.code ?? null },
      );
    }
...
function assertRoleArtifact({ artifact, assignment, plan, snapshot, roleId }) {
  ...
  if (
    ...
    fail(
      ...
      'A role result must remain bound to its validated Assignment, role, output contract, input ceiling, and producer provenance.',
      {
        roleId,
        artifactId: artifact?.artifactId ?? null,
  ...
  if (
    assignment.assignmentId !==
      deriveOperatingIntelligenceAssignmentIdV2(plan.planId, roleId, selected.roleVersion) ||
    assignment.roleVersion !== selected.roleVersion ||
    !selected.inputArtifactIds.every((artifactId) =>
      assignment.inputArtifactIds.includes(artifactId),
    ) ||
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'A role result is not bound to the exact immutable intelligence-plan role.',
      {
        roleId,
        planId: plan.planId,
      },
...
function assertAcceptedRoleArtifact({
  artifact,
  assignment,
  submissions,
  replayBySubmission,
  ...
  try {
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'A role result requires one exact accepted Submission and durable replay proof.',
      {
        roleId,
        artifactId: artifact.artifactId,
        cause: cause?.code ?? null,
...
function currentDecisionForQuestion(existingDecisions, decision, snapshot) {
  const records = existingDecisions.filter(
    (record) =>
  ...
  const predecessorIds = new Set(
    records.map(({ predecessorDecisionId }) => predecessorDecisionId).filter(Boolean),
  );
  const current = records.filter(({ decisionId }) => !predecessorIds.has(decisionId));
  if (current.length > 1) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Decision ledger cannot revise an ambiguous current Decision lineage.',
      {
...
export function buildOperatingDecisionLedgerMaterializationV2({
  ...
  try {
    ...
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires contract-valid snapshot, state, plan, assignments, and ledger.',
      {
        cause: cause?.code ?? null,
        causeMessage: cause?.message ?? null,
      },
  ...
  if (
    ...
    fail(
      'OPERATING_SCOPE_INVALID',
      'Decision ledger, intelligence plan, immutable snapshot, and Event timestamp must share one exact binding.',
      {
  ...
  if (!Array.isArray(assignments))
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires an Assignment collection.',
    );
  if (!Array.isArray(inputBundles))
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires role-scoped input bundles.',
    );
  const selectedAssignmentIds = new Set(
    checkedPlan.selectedRoles.map(({ roleId, roleVersion }) =>
  ...
  if (planAssignments.some((assignment) => !assignment)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Decision-ledger materialization requires every selected Assignment in exact plan order.',
      {
        planId: checkedPlan.planId,
      },
    );
  ...
  try {
    validateOperatingIntelligenceAssignmentGraphV2(checkedPlan, planAssignments, {
      allowLifecycleProgress: true,
    });
  } catch (cause) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires the exact recorded intelligence-plan Assignments.',
      {
        cause: cause?.code ?? null,
        causeMessage: cause?.message ?? null,
      },
  ...
  if (!Array.isArray(submissions) || !Array.isArray(submissionReplayIndex)) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Decision-ledger materialization requires Submission and replay-proof collections.',
    );
  }
  ...
  for (const replay of submissionReplayIndex) {
    if (
      !replay ||
      typeof replay.submissionId !== 'string' ||
      replayBySubmission.has(replay.submissionId)
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Decision-ledger Submission replay identities must be unique.',
      );
    }
  ...
  const assignmentsByRole = new Map(
    planAssignments.map((assignment) => [assignment.roleId, assignment]),
  );
  ...
  for (const entry of inputBundles) {
    if (
      ...
      bundlesByAssignment.has(entry.assignmentId)
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Decision-ledger input-bundle custody must contain unique Assignment identities.',
      );
    }
  ...
  if (planAssignments.some(({ assignmentId }) => !bundlesByAssignment.has(assignmentId))) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Every intelligence-plan Assignment requires its exact role-scoped input bundle.',
    );
  }
  const advisorAssignments = planAssignments.filter(
    ({ assignmentKind }) => assignmentKind === 'advisor',
  );
  const challengerAssignment = planAssignments.find(
  ...
  for (const advisorAssignment of advisorAssignments) {
    if (advisorAssignment.state === 'validated') {
      const entry = advisorResultEntries.find(
        ({ artifactId }) =>
      ...
      if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Every validated advisor seat requires one validated Advisor result.',
          {
            planId: checkedPlan.planId,
    ...
    if (TERMINAL_ADVISOR_OUTCOMES.has(advisorAssignment.state)) {
      if (
        !advisorAssignment.terminalOutcome ||
        advisorAssignment.terminalOutcome.outcome !== advisorAssignment.state
      ) {
        fail(
          'STATE_TRANSITION_INVALID',
          'An abandoned advisor seat requires an exact typed terminal absence proof.',
          {
            planId: checkedPlan.planId,
            roleId: advisorAssignment.roleId,
      ...
      const issuedGap = chairAssignment?.inputAbsences.find(
        (gap) =>
      ...
      if (!issuedGap) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Assignment is missing the exact typed advisor absence input.',
          {
            planId: checkedPlan.planId,
            roleId: advisorAssignment.roleId,
          },
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'Every selected advisor seat must be terminal before Chair ledger materialization.',
      {
        planId: checkedPlan.planId,
        roleId: advisorAssignment.roleId,
  ...
  if (!exactArray(ledger.advisorArtifactIds, normalizedAdvisorArtifactIds)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair ledger must name exactly its validated Advisor result Artifact.',
      {
  ...
  if (checkedPlan.challengerRequired) {
    if (
      !challengerReview ||
      typeof challengerReview !== 'object' ||
      Array.isArray(challengerReview)
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'A required Challenger review is missing before Chair ledger materialization.',
        {
          planId: checkedPlan.planId,
        },
      );
    ...
    assertAcceptedRoleArtifact({
      ...
      plan: checkedPlan,
      snapshot: checkedSnapshot,
      roleId: challengerAssignment.roleId,
    });
    ...
    if (ledger.challengerArtifactId !== artifact.artifactId) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair ledger must name the exact required Challenger review Artifact.',
        {
          ledgerId: ledger.ledgerId,
        },
      );
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'A plan that omitted Challenger cannot inject an unplanned Challenger review.',
    );
  } else if (ledger.challengerArtifactId !== null) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair ledger must represent an explicitly omitted Challenger as null.',
    );
  }
  const checkedChairArtifact = artifactIndex.get(chairArtifact?.artifactId);
  assertAcceptedRoleArtifact({
    artifact: checkedChairArtifact,
  ...
  if (
    ledger.sourceArtifactId !== checkedPlan.sourceArtifactId ||
    !chairInputBundle.sourceArtifactIds.includes(ledger.sourceArtifactId)
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      "Chair ledger must retain the intelligence plan's exact bundle-proven source Artifact provenance.",
      {
        ledgerId: ledger.ledgerId,
  ...
  const priorDecisions = existingDecisions.filter(
    (record) => record?.sourceArtifactId !== checkedChairArtifact.artifactId,
  );
  if (
    new Set(validatedLedger.decisions.map(({ question }) => question)).size !==
    validatedLedger.decisions.length
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      'One Chair ledger may not contain two revisions for the same Decision question.',
      {
  ...
  const claims = normalizedAdvisorResults.flatMap(({ artifactId, output }) =>
  ...
  const findings =
    normalizedChallengerReview === null
      ? []
  ...
  const durableClaimIds = new Map(
    normalizedAdvisorResults.flatMap(({ artifactId, output }) =>
  ...
  const durableFindingIds = new Map(
    (normalizedChallengerReview?.output.findings ?? []).map(({ localFindingId }) => [
      localFindingId,
      deriveOperatingMaterializedFindingIdV2(normalizedChallengerReview.artifactId, localFindingId),
    ]),
  );
  const dissentById = new Map(
    validatedLedger.dissent.map((entry) => [entry.localDissentId, entry]),
  );
  const decisions = validatedLedger.decisions.map((decision) => {
    if (decision.ownerActorId !== checkedPlan.decisionOwnerActorId) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair Decision owner must equal the immutable intelligence-plan owner.',
        {
          ledgerId: validatedLedger.ledgerId,
    ...
    const record = {
      ...
      domainId: checkedSnapshot.domainId,
      domainVersion: checkedSnapshot.domainVersion,
      sourceCycleId: checkedChairArtifact.cycleId,
      sourceArtifactId: checkedChairArtifact.artifactId,
      sourceAssignmentId: chairAssignment.assignmentId,
      sourceLocalDecisionId: decision.localDecisionId,
      title: decision.title,
      question: decision.question,
      outcome: decision.outcome,
      rationale: decision.rationale,
      sourceClaimRefs: clone(decision.sourceClaimRefs),
      claimIds: decision.sourceClaimRefs.map((reference) =>
        durableClaimIds.get(`${reference.advisorArtifactId}:${reference.localClaimId}`),
      ),
      sourceRecommendationRefs: clone(decision.sourceRecommendationRefs),
      challengerFindingIds: clone(decision.challengerFindingIds),
      findingIds: decision.challengerFindingIds.map((localFindingId) =>
        durableFindingIds.get(localFindingId),
      ),
      ...
      assumptionIds: clone(decision.assumptionIds),
      expectedUpside: decision.upside,
      expectedDownside: decision.downside,
      uncertainty: decision.uncertainty,
      reversibility: decision.reversibility,
      dissentIds: clone(decision.dissentIds),
      dissent: decision.dissentIds.map(
        (localDissentId) => dissentById.get(localDissentId).statement,
      ),
      ...
      revisitAt: null,
      revision,
      predecessorDecisionId: predecessor?.decisionId ?? null,
      historyDecisionIds:
        predecessor === null
          ? []
  ...
  for (const [history, derived, idField, subject] of histories) {
    ...
    for (const record of history) {
      if (!record || typeof record[idField] !== 'string' || byId.has(record[idField])) {
        fail(
          'STATE_TRANSITION_INVALID',
          `${subject} history identities must be explicit and unique.`,
        );
      }
    ...
    for (const record of derived) {
      ...
      if (existing && sha256Jcs(existing) !== sha256Jcs(record)) {
        fail(
          ...
          `Runtime-derived ${subject} identity already carries different bytes.`,
          {
  ...
  if (
    sha256Jcs(validatedLedger.advisorAbsenceGaps) !== sha256Jcs(issuedRoleGaps) ||
    sha256Jcs(validatedLedger.evidenceGaps) !== sha256Jcs(issuedEvidenceGaps)
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair ledger advisor absence gaps must exactly equal its issued typed absence inputs.',
      {
        ledgerId: validatedLedger.ledgerId,
      },
    );
  ...
  return freeze({
    ...
    actionHypotheses: validatedLedger.decisions.flatMap(({ actionHypotheses }, decisionIndex) =>
      actionHypotheses.map((hypothesis, hypothesisIndex) =>
        freeze({ decisionIndex, hypothesisIndex, ...clone(hypothesis) }),
      ),
    ),
  });
}
