...
const compiledAssignmentTransition = (from, to) => (
  OPERATE_CONTRACT_CATALOG_V2.transitions.find((transition) => (
...
const EVENT_HANDLER_TYPES = Object.freeze({
  assignment: Object.freeze([
    ...
    'assignment.submitted', 'artifact.created', 'assignment.validated', 'assignment.rejected',
    'assignment.abandoned', 'assignment.failed',
  ]),
  workflow: Object.freeze([
    'cycle.input-bound', 'executive-board.materialized', 'review.created', 'review.submitted', 'work-change-set.materialized',
    'action.authority-promoted', 'action.approved', 'action.rejected', 'action.deferred',
    'action.reopened', 'action.queued', 'action.started', 'action.completed', 'action.blocked',
    'action.cancelled', 'cycle.approved', 'cycle.executing', 'cycle.verifying', 'cycle.closed',
  ]),
  intelligence: Object.freeze([
    'evidence.resolved', 'planning-delivery.ingested', 'evidence.rejected', 'snapshot.materialized',
    'operating-state.materialized', 'metric.observed', 'claim.recorded', 'finding.recorded', 'risk.recorded',
    'assumption.recorded', 'decision.revised', 'delta.derived', 'intelligence.plan-recorded',
    'decision-ledger.materialized', 'verification.plan-recorded', 'outcome.recorded',
    'learning.recorded', 'scenario.recorded', 'trigger.recorded',
  ]),
  authority: Object.freeze([
    'policy.evaluated', 'approval.recorded', 'capability.availability-recorded',
    'capability.granted', 'rollback.plan-recorded', 'operation.intent-recorded',
    'execution.result-recorded', 'rollback.result-recorded',
  ]),
});
const EVENT_HANDLER_NAME_BY_TYPE = Object.freeze(Object.fromEntries(
...
export function createOperatingRuntimeEventReducerV2({
  ...
  const dependencies = [
    runtimeError, computeEventHash, verifyEventChain, indexRuntimeState, transitionAssignment,
    materializeRuntimeState, createEmptyState, createNoModelReplayHook,
  ];
  ...
  if (!eventHandlers || Object.keys(EVENT_HANDLER_TYPES).filter((handlerName) => handlerName !== 'assignment').some((handlerName) => (
    typeof eventHandlers[handlerName] !== 'function'
  ))) {
  ...
  function requireAssignment(index, event) {
    ...
    if (!assignment) {
      throw runtimeError('ASSIGNMENT_NOT_AVAILABLE', `${event.type} references unknown assignment ${assignmentId}.`, { assignmentId });
    }
    if (event.entityId !== assignmentId) {
      throw runtimeError('STATE_TRANSITION_INVALID', `${event.type} entityId does not match assignmentId.`, {
        assignmentId,
  ...
  function setAssignment(index, event, nextState, patch = {}) {
    ...
    if (!transition || transition.event !== event.type) {
      throw runtimeError('STATE_TRANSITION_INVALID', `${event.type} is not the compiled transition for Assignment ${assignment.assignmentId}.`, {
  ...
  function requireSubmission(index, event, assignmentId) {
    ...
    if (!submission || submission.assignmentId !== assignmentId || submission.cycleId !== event.cycleId) {
      throw runtimeError('SUBMISSION_ID_CONFLICT', `${event.type} submission identity does not match the assignment.`, {
        assignmentId,
  ...
  function dependencyValidationProof(index, assignment, dependencyId) {
    ...
    const acceptedSubmissions = [...index.submissions.values()].filter((submission) => (
      submission.assignmentId === dependencyId && submission.state === 'accepted'
    ));
  ...
  function applyAssignmentRuntimeEvent(index, event) {
    switch (event.type) {
      case 'assignment.created': {
        if (event.entityId !== event.payload.assignmentId || event.cycleId !== event.payload.cycleId) {
          throw runtimeError('STATE_TRANSITION_INVALID', 'assignment.created identifiers do not match its record.');
        }
        ...
        if (index.assignments.has(event.entityId)) {
          throw runtimeError('CONCURRENT_MODIFICATION', `Assignment ${event.entityId} already exists.`);
        }
        if (
          ...
          || (event.payload.dependencyPolicy.kind === 'none' && event.payload.dependsOn.length !== 0)
          || (event.payload.dependencyPolicy.kind === 'all-required' && event.payload.dependsOn.length === 0)
        ) {
          throw runtimeError('STATE_TRANSITION_INVALID',
        ...
        if (event.payload.assignmentKind === 'verification') {
          if (['operate-planning-delivery-verifier', 'operate-planning-decision-revisit'].includes(event.payload.roleId)
            && event.payload.governedOperationId === null
            && event.payload.capabilityGrantId === null) {
            throw runtimeError('RESULT_CONTRACT_INVALID', 'Planning delivery verification Assignment must be created atomically by its dedicated ingestion Event.');
          }
          ...
          const verificationPlan = action ? index.verificationPlans.get(action.verificationPlanId) : null;
          let expected = null;
          try {
          ...
          if (!expected || sha256Jcs(expected) !== sha256Jcs(event.payload)) {
            throw runtimeError('RESULT_CONTRACT_INVALID', 'Verification Assignment must be deterministically owned by one existing plan and terminal operation result.', {
              assignmentId: event.payload.assignmentId,
      ...
      case 'assignment.available': {
        ...
        try {
          for (const proof of event.payload.dependencyProofs) {
            ...
            if (sha256Jcs(expected) !== sha256Jcs(proof)) {
              throw runtimeError('STATE_TRANSITION_INVALID', 'Assignment availability contains a forged dependency proof.', {
        ...
        setAssignment(index, event, 'available', {
          ...
          inputArtifactIds: resolveOperatingAssignmentInputArtifactIdsV2(current, event.payload.dependencyProofs),
      ...
      case 'assignment.claimed': {
        ...
        if (index.submissions.has(event.payload.submissionId)) {
          throw runtimeError('SUBMISSION_ID_CONFLICT', `Submission ${event.payload.submissionId} already exists.`);
        }
      ...
      case 'assignment.started': {
        const assignment = requireAssignment(index, event);
        if (event.payload.attempt !== assignment.attemptPolicy.attempt + 1
          || event.payload.attempt > assignment.attemptPolicy.maxAttempts) {
          throw runtimeError('STATE_TRANSITION_INVALID', `Invalid attempt ${event.payload.attempt} for ${assignment.assignmentId}.`);
        }
        if (assignment.state === 'rejected') {
          const rejected = [...index.submissions.values()].filter((item) => (
            item.assignmentId === assignment.assignmentId && item.state === 'rejected'
          ));
          const materializedArtifacts = [...index.artifacts.values()].filter((artifact) => (
          ...
          if (attemptsRemaining !== assignment.attemptPolicy.maxAttempts - assignment.attemptPolicy.attempt
            || attemptsRemaining < 1) {
            throw runtimeError('STATE_TRANSITION_INVALID',
        ...
        const issued = [...index.submissions.values()].filter((item) => (
          item.assignmentId === assignment.assignmentId && item.state === 'issued'
        ));
        if (issued.length !== 1) {
          throw runtimeError('SUBMISSION_ID_CONFLICT',
      ...
      case 'assignment.submitted': {
        ...
        if (submission.state !== 'issued') {
          throw runtimeError('ASSIGNMENT_ALREADY_SUBMITTED', `Submission ${submission.submissionId} is already resolved.`);
        }
        if (event.payload.mediaType !== assignment.outputContract.mediaType
          || event.payload.encoding !== assignment.outputContract.encoding) {
          throw runtimeError('RESULT_CONTRACT_INVALID', 'Submission media type or encoding does not match the assignment contract.');
        }
        if (event.payload.sizeBytes > assignment.outputContract.maxBytes) {
          throw runtimeError('RESULT_CONTRACT_INVALID', 'Submission exceeds the assignment byte limit.');
        }
      ...
      case 'artifact.created': {
        ...
        if (!assignment || assignment.state !== 'submitted' || assignment.cycleId !== event.cycleId) {
          throw runtimeError('STATE_TRANSITION_INVALID', 'artifact.created requires the matching submitted assignment.');
        }
        const matchingSubmissions = [...index.submissions.values()].filter((item) => (
        ...
        if (!submission
          ...
          throw runtimeError('RESULT_CONTRACT_INVALID', 'Artifact bytes do not match the submitted bytes.');
        }
        ...
        if (!cycle
          ...
          || event.payload.encoding !== assignment.outputContract.encoding) {
          throw runtimeError('RESULT_CONTRACT_INVALID', 'Artifact metadata does not match the bound cycle and assignment contract.');
        }
        if (event.payload.sizeBytes > assignment.outputContract.maxBytes
        ...
        if (event.entityId !== event.payload.artifactId || index.artifacts.has(event.entityId)) {
          throw runtimeError('STATE_TRANSITION_INVALID', `Artifact identity ${event.entityId} is invalid or duplicated.`);
        }
      ...
      case 'assignment.validated': {
        ...
        if (!artifact
          ...
          throw runtimeError('ARTIFACT_NOT_FOUND', 'Validated artifact does not match the submission.');
        }
      ...
      case 'assignment.rejected': {
        ...
        if (event.payload.attempt !== assignment.attemptPolicy.attempt) {
          throw runtimeError('STATE_TRANSITION_INVALID', 'Rejected attempt does not match the running attempt.');
        }
        const expectedAttemptsRemaining = assignment.attemptPolicy.maxAttempts - event.payload.attempt;
        ...
        if (submission.state !== 'issued'
          ...
          || [...index.artifacts.values()].some((artifact) => artifact.assignmentId === assignment.assignmentId)) {
          throw runtimeError('SUBMISSION_ID_CONFLICT',
      ...
      case 'assignment.failed':
        if (requireAssignment(index, event).state === 'pending') {
          try {
            const intent = assertOperatingAssignmentTerminalPayloadV2(requireAssignment(index, event), event.payload, {
              assignments: [...index.assignments.values()],
              submissions: [...index.submissions.values()],
              artifacts: [...index.artifacts.values()],
              submissionReplayIndex: [...index.replay.values()],
            ...
            if (event.eventId !== `evt_${intent.terminalId}`
              || event.actor.kind !== 'engine'
              || event.actor.id !== 'openplanr-scheduler'
              || event.causationId !== intent.dependencyEventIds.at(-1)) {
              throw runtimeError('STATE_TRANSITION_INVALID', 'Pending Assignment failure differs from its scheduler-owned terminal identity.', {
        ...
        return;
      default:
        throw runtimeError('CONTRACT_VERSION_UNSUPPORTED', `Unsupported Assignment Event ${event.type}.`);
    }
  }
  ...
  function dispatchRuntimeEvent(index, event, options) {
    ...
    if (!handlerName) {
      throw runtimeError('CONTRACT_VERSION_UNSUPPORTED', `Unsupported Protocol 2.0 Event ${event.type}.`);
    }
  ...
  function validateEventForIdentityReplay(event) {
    ...
    if (computeEventHash(event) !== event.eventHash) {
      throw runtimeError('STATE_TRANSITION_INVALID', `Event ${event.eventId} failed its JCS hash check.`, {
        eventId: event.eventId,
      });
    }
  }
  ...
  function reduceOperatingRuntimeEventsV2(events, options = {}) {
    ...
    replayHook.assertUnused();
    return materializeRuntimeState(index, appendEvents.at(-1)?.timestamp ?? initialState.generatedAt, eventHead);
  }
