...
function assertPortableDeclaration(value, path = '$', prose = false, identity = false) {
  if (Array.isArray(value)) {
    value.forEach((entry, index) =>
      assertPortableDeclaration(entry, `${path}[${index}]`, prose, identity),
    );
    return;
  }
  ...
  if (
    ...
    (!prose &&
      !identity &&
      /\b(?:private[-_ ]?consumer|customer|tenant|credential|secret|api[-_ ]?key|access[-_ ]?token|codex|claude|cursor|openai|anthropic|gpt|sonnet|opus)\b/iu.test(
        value,
      ))
  ) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      `Extension registration contains a non-portable declaration at ${path}.`,
      { path },
    );
  }
}
...
function assertDataOnly(value, path = '$', prose = false) {
  ...
  if (value && typeof value === 'object') {
    Object.entries(value).forEach(([key, entry]) => {
      ...
      if (
        !isPolicyRequirementPath(path) &&
        !declaredPolicyBinding &&
        /^(?:function|path|module|url|connector|credential|policy|approval|executor|transition|grant|dynamicImport)$/iu.test(
          key,
        )
      ) {
        fail(
          'E_EXTENSION_REGISTRATION_INVALID',
          `Extension registration has an authority-bearing field at ${childPath}.`,
          { path: childPath },
        );
      }
      assertDataOnly(
        entry,
        childPath,
        prose || PROSE_FIELDS.has(key) || isPolicyRequirementPath(childPath),
      );
    });
    return;
  }
  ...
  if (
    !prose &&
    !isPolicyRequirementPath(path) &&
    /\b(?:dynamic[- ]?import|external[- ]?effect|network|filesystem|connector|executor|policy|approval|capability[- ]?grant)\b/iu.test(
      value,
    )
  ) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      `Extension registration contains an authority-bearing declaration at ${path}.`,
      { path },
    );
  }
}
...
function providerArtifact(kind, registration) {
  return {
    kind: SIGNAL_PROVIDER_KINDS[kind].contractKind,
    schemaVersion: '1.0.0',
    protocolVersion: VERSION,
    ...clone(registration),
  };
...
function assertRubric(domain, role) {
  ...
  const clauses =
    rubric === null || typeof rubric !== 'object' || Array.isArray(rubric)
      ? null
  ...
  if (!complete) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'A domain role requires a complete versioned analysis rubric.',
      {
...
function assertRoles(value) {
  ...
  for (const role of value.roles) {
    ...
    if (roles.has(identity)) {
      fail(
        ...
        'A domain cannot register the same role identity twice.',
        {
    ...
    if (!kinds.has(role.roleKind)) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'A domain role must declare one kernel scheduling kind.',
        {
          domainId: value.domainId,
          roleId: role.roleId,
    ...
    if (role.roleId === role.label) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'A presentation label cannot stand in for a role identity.',
        {
          domainId: value.domainId,
          roleId: role.roleId,
        },
    ...
    if (
      value.domainId === 'business' &&
      value.domainVersion === '1.0.0' &&
      FORBIDDEN_BUSINESS_ROLE_IDS_V1.has(role.roleId)
    ) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'A business registration cannot introduce a forbidden finance or product-owner seat.',
        {
          domainId: value.domainId,
          roleId: role.roleId,
        },
    ...
    if (
      value.domainId === 'software' &&
      value.domainVersion === '1.0.0' &&
      FORBIDDEN_SOFTWARE_EXECUTIVE_ROLE_IDS_V1.has(role.roleId)
    ) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'A software registration cannot copy a business executive identity.',
        {
          domainId: value.domainId,
          roleId: role.roleId,
        },
  ...
  const choreography =
    kinds.get('advisor') >= 1 && kinds.get('challenger') === 1 && kinds.get('chair') === 1;
  if (!choreography) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'A public domain requires at least one advisor and exactly one challenger and chair.',
      {
        domainId: value.domainId,
...
function assertDomain(value, { allowSyntheticDomains }) {
  ...
  if (expected) {
    if (
      value.domainVersion !== expected.version ||
      !sameJson(value.domainContract, {
      ...
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'Public API domains require their exact explicit contract and projection bindings.',
        {
          domainId: value.domainId,
          domainVersion: value.domainVersion,
        },
    ...
    if (
      ...
      value.domainId !== 'synthetic-domain' ||
      !sameJson(value.domainContract, expectedSyntheticBinding(value)) ||
      value.projectionContracts.length !== 1 ||
      !sameJson(value.projectionContracts[0], {
        schemaId: 'operating-domain-projection',
        schemaVersion: '2.0.0',
      })
    ) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'Only an explicit test-only synthetic domain may accompany the two public domains.',
        {
          domainId: value.domainId,
        },
      );
  ...
  if (value.requestedCapabilities.length !== 0) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Domain registration is data-only and cannot request an authority-bearing capability.',
      {
        domainId: value.domainId,
      },
    );
    ...
    roles: value.roles
      ...
      .sort(
        (left, right) =>
          ...
          left.roleVersion.localeCompare(right.roleVersion),
      ),
    vocabulary: value.vocabulary
      .map(clone)
      .sort((left, right) => left.term.localeCompare(right.term)),
    projectionContracts: value.projectionContracts
      .map(clone)
      .sort(
        (left, right) =>
...
function expectedSupportedDomains() {
  return Object.values(PUBLIC_OPERATE_DOMAIN_BINDINGS_V2)
    .map(({ apiDomainId, id, version }) => ({
      apiDomainId,
      domainContractId: id,
      domainContractVersion: version,
    }))
...
function assertSignalProvider(kind, value) {
  ...
  const actualDomains = [...value.supportedDomains].sort((left, right) =>
    left.apiDomainId.localeCompare(right.apiDomainId),
  );
  if (
    ...
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Signal providers must use one exact public built-in declaration and public domain bindings.',
      {
        providerKind: kind,
...
function index(entries, identity, label) {
  ...
  for (const entry of entries) {
    ...
    if (indexed.has(key))
      fail('E_EXTENSION_REGISTRATION_CONFLICT', `Duplicate ${label} registration ${key}.`, { key });
...
function publicRegistry({
  ...
  const domainIndex = index(
    domains.map((entry) => assertDomain(entry, { allowSyntheticDomains })),
    ({ domainId, domainVersion }) => `${domainId}@${domainVersion}`,
    'domain',
  );
  const publicDomainKeys = Object.keys(PUBLIC_OPERATE_DOMAIN_BINDINGS_V2).map(
    (domainId) => `${domainId}@1.0.0`,
  );
  if (!publicDomainKeys.every((key) => domainIndex.has(key))) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'The public registry must contain both exact business and software domains.',
      {},
    );
  }
  const runtimeIndex = index(
    agentRuntimeManifests.map(assertRuntime),
  ...
  const providerIndexes = Object.fromEntries(
    Object.entries({
      ...
      metric: metricProviders,
      verification: verificationProviders,
    }).map(([kind, registrations]) => [
      kind,
      index(
        registrations.map((entry) => assertSignalProvider(kind, entry)),
  ...
  for (const [kind, expected] of Object.entries(SIGNAL_PROVIDER_KINDS)) {
    ...
    if (
      providers.size !== 1 ||
      !providers.has(`${expected.providerId}@${expected.providerVersion}`)
    ) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'The public registry must contain one exact reference provider for each signal kind.',
        {
          providerKind: kind,
        },
      );
  ...
  const evidence = createOperateEvidenceRegistryV2({
    providers: evidenceProviders,
    resolvers: evidenceResolvers,
  });
  ...
  for (const manifest of runtimeIndex.values()) {
    if (manifest.fallback.kind === 'runtime') {
      ...
      if (
        ...
        !runtimeIndex.has(fallbackKey)
      ) {
        fail(
          'E_EXTENSION_REGISTRATION_CONFLICT',
          'An agent runtime fallback must name a different registered runtime.',
          {
  ...
  return freeze({
    domains: [...domainIndex.values()].sort((left, right) =>
      `${left.domainId}@${left.domainVersion}`.localeCompare(
        `${right.domainId}@${right.domainVersion}`,
      ),
    ),
    agentRuntimeManifests: [...runtimeIndex.values()].sort((left, right) =>
      `${left.runtimeId}@${left.runtimeVersion}`.localeCompare(
        `${right.runtimeId}@${right.runtimeVersion}`,
      ),
    ),
    evidenceProviders: evidence.providers,
    evidenceResolvers: evidence.resolvers,
    snapshotProviders: [...providerIndexes.snapshot.values()].sort((left, right) =>
      `${left.providerId}@${left.providerVersion}`.localeCompare(
        `${right.providerId}@${right.providerVersion}`,
      ),
    ),
    metricProviders: [...providerIndexes.metric.values()].sort((left, right) =>
      `${left.providerId}@${left.providerVersion}`.localeCompare(
        `${right.providerId}@${right.providerVersion}`,
      ),
    ),
    verificationProviders: [...providerIndexes.verification.values()].sort((left, right) =>
      `${left.providerId}@${left.providerVersion}`.localeCompare(
        `${right.providerId}@${right.providerVersion}`,
      ),
    ),
...
export function createOperateExtensionRegistryV2(input = {}) {
  if (
    ...
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Extension registrations must be an object of arrays.',
      {},
    );
  }
  ...
  if (
    ...
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Extension registration input must contain data properties only.',
      {},
    );
  }
  ...
  if (unknown.length > 0)
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Extension registration input contains unsupported fields.',
      { unknown: unknown.sort() },
    );
  const {
    domains = OPERATE_CONTRACT_CATALOG_V2.extensions.domains.map(domainArtifact),
    agentRuntimeManifests = OPERATE_CONTRACT_CATALOG_V2.extensions.agentRuntimeManifests.map(
      runtimeArtifact,
    ),
    ...
    snapshotProviders = OPERATE_CONTRACT_CATALOG_V2.extensions.snapshotProviders.map((entry) =>
      providerArtifact('snapshot', entry),
    ),
    metricProviders = OPERATE_CONTRACT_CATALOG_V2.extensions.metricProviders.map((entry) =>
      providerArtifact('metric', entry),
    ),
    verificationProviders = OPERATE_CONTRACT_CATALOG_V2.extensions.verificationProviders.map(
      (entry) => providerArtifact('verification', entry),
    ),
  ...
  if (
    ...
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'Extension registrations must contain arrays and an optional boolean synthetic-domain test flag.',
      {},
    );
  }
...
export function canonicalizeOperateExtensionRegistryV2(registry) {
  if (!registry || typeof registry !== 'object' || Array.isArray(registry)) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'A public Operate extension registry must be a complete object.',
      {},
    );
  }
  ...
  if (prototype !== Object.prototype || Object.getOwnPropertySymbols(registry).length > 0) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'A public Operate extension registry must be a plain data object.',
      {},
    );
  }
  ...
  if (
    keys.length !== COMPLETE_REGISTRY_FIELDS.length ||
    keys.some((key, index) => key !== COMPLETE_REGISTRY_FIELDS[index])
  ) {
    fail(
      'E_EXTENSION_REGISTRATION_INVALID',
      'A public Operate extension registry must declare exactly every registry field.',
      {
        fields: keys,
      },
    );
  ...
  for (const field of COMPLETE_REGISTRY_FIELDS) {
    ...
    if (!descriptor || !Object.hasOwn(descriptor, 'value') || !Array.isArray(descriptor.value)) {
      fail(
        'E_EXTENSION_REGISTRATION_INVALID',
        'A public Operate extension registry must contain only data-array fields.',
        {
          field,
        },
      );
...
export function findOperateDomainRegistrationV2(registry, domainId, { domainVersion } = {}) {
  ...
  const match = registry?.domains?.find(
    (entry) => entry.domainId === domainId && entry.domainVersion === domainVersion,
  );
...
export function findAgentRuntimeManifestV2(registry, runtimeId, { runtimeVersion } = {}) {
  ...
  const match = registry?.agentRuntimeManifests?.find(
    (entry) => entry.runtimeId === runtimeId && entry.runtimeVersion === runtimeVersion,
  );
  return match === undefined ? null : clone(match);
}
...
function findSignalProvider(registry, kind, providerId, { providerVersion } = {}) {
  ...
  const match = registry?.[field]?.find(
...
export function selectAgentRuntimeManifestV2(registry, runtimeId, { runtimeVersion } = {}) {
  ...
  if (manifest.implementation.availability === 'available')
    return freeze({ status: 'available', manifest, fallback: null });
  if (manifest.fallback.kind === 'runtime') {
    const fallback = findAgentRuntimeManifestV2(registry, manifest.fallback.runtimeId, {
      runtimeVersion: manifest.fallback.runtimeVersion,
    });
    if (fallback?.implementation.availability === 'available') {
      return freeze({
        status: 'fallback',
        manifest: fallback,
        fallback: { runtimeId: fallback.runtimeId, runtimeVersion: fallback.runtimeVersion },
      });
    }
  }
