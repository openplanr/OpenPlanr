...
function applyLearningRecorded(index, event, { artifactStore } = {}) {
  ...
  const verificationPlan = outcome ? index.verificationPlans.get(outcome.verificationPlanId) : null;
  const sourceDecision = action ? index.decisions.get(action.sourceDecisionId) : null;
  const { observation, snapshot } = observationForOutcome(index, outcome ?? {});
  let expected;
  try {
...
function applyRuntimeEvent(index, event, options = {}) {
  switch (event.type) {
...
function applyWorkflowRuntimeEvent(index, event, options) {
  applyRuntimeEvent(index, event, options);
}
...
function applyAuthorityRuntimeEvent(index, event, options) {
  applyRuntimeEvent(index, event, options);
}
