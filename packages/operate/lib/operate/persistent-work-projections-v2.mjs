...
function linksForScope(state, scope, records) {
  ...
  const add = (entityType, entityId, cycleId, relation) => {
    ...
    links.set(key, {
      entityKind: entityType.replace('operating-', ''),
      entityId,
      cycleId,
      relation,
    });
  ...
  for (const [entityType, id, field] of ENTITY_TYPES) {
    for (const record of records[field])
      add(entityType, record[id], record.sourceCycleId, 'source');
  }
  for (const review of state.reviews) {
    const cycle = state.cycles.find(({ cycleId }) => cycleId === review.cycleId);
    if (
      !cycle ||
      cycle.scopeId !== scope.scopeId ||
      cycle.domainId !== scope.domainId ||
      cycle.domainVersion !== scope.domainVersion
    )
      continue;
    for (const disposition of review.workDispositions) {
      ...
      if (disposition.disposition === 'deferred')
        add(disposition.entityType, disposition.entityId, review.cycleId, 'carried-forward');
    }
  }
  for (const action of records.actions) {
    const sourceCycle = state.cycles.find(({ cycleId }) => cycleId === action.sourceCycleId);
    const hasOwnedVerification = state.assignments.some(
      (assignment) =>
        assignment.assignmentKind === 'verification' &&
        state.governedOperations?.some(
          (operation) =>
...
export function buildOperatingCycleWorkViewV2(
  ...
  const sourceOrTouched = new Set(
    ledger.cycleLinks
      .filter((link) => link.cycleId === cycleId)
      .map((link) => `${link.entityKind}:${link.entityId}`),
  );
  const select = (field, id, kind) =>
    ledger[field].filter((record) => sourceOrTouched.has(`${kind}:${record[id]}`)).map(clone);
  return Object.freeze({
    kind: 'operating-cycle-work-view',
    schemaVersion: '1.0.0',
    protocolVersion: PROTOCOL_VERSION,
    ...
    cycleLinks: Object.freeze(
      ledger.cycleLinks.filter((link) => link.cycleId === cycleId).map(clone),
    ),
  });
}
