...
export const OPERATE_GOVERNED_EFFECT_RANK_V2 = Object.freeze(Object.assign(
  Object.create(null),
...
const SHIPPED_GOVERNED_REGISTRATION_RESERVATIONS = Object.freeze(
  Object.entries(REGISTRATION_KINDS).flatMap(([kind, definition]) => (
    OPERATE_CONTRACT_CATALOG_V2.extensions[definition.field].map((entry) => Object.freeze({
      kind,
...
function semverParts(value, field) {
  ...
  if (match === null) fail('E_EXTENSION_VERSION_REQUIRED', `${field} requires one exact semantic version.`, { field });
...
function explicitTime(value, field) {
  ...
  if (Number.isNaN(parsed)) fail('E_EXTENSION_REGISTRATION_INVALID', `${field} requires an explicit RFC 3339 timestamp.`, { field });
  return parsed;
}
...
function assertClosedData(value, path = '$') {
  if (value === null || ['string', 'number', 'boolean'].includes(typeof value)) {
    if (typeof value === 'number' && !Number.isFinite(value)) {
      fail('E_EXTENSION_REGISTRATION_INVALID', `Registration data must be finite at ${path}.`, { path });
    }
    if (typeof value === 'string' && (
      ...
      || /\b(?:dynamic[- ]?import|private[-_ ]?consumer|access[-_ ]?token|api[-_ ]?key)\b/iu.test(value)
    )) {
      fail('E_EXTENSION_REGISTRATION_INVALID', `Registration contains a non-portable or private declaration at ${path}.`, { path });
    }
    return;
  }
  if (!value || typeof value !== 'object') {
    fail('E_EXTENSION_REGISTRATION_INVALID', `Registration must contain closed JSON data at ${path}.`, { path });
  }
  const prototype = Object.getPrototypeOf(value);
  const expectedPrototype = Array.isArray(value) ? Array.prototype : Object.prototype;
  if (prototype !== expectedPrototype) {
    fail('E_EXTENSION_REGISTRATION_INVALID', `Registration must use plain data objects at ${path}.`, { path });
  }
  if (Object.getOwnPropertySymbols(value).length > 0) {
    fail('E_EXTENSION_REGISTRATION_INVALID', `Registration cannot contain symbol fields at ${path}.`, { path });
  }
  for (const [key, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(value))) {
    const field = `${path}.${key}`;
    if (!Object.hasOwn(descriptor, 'value') || descriptor.get || descriptor.set) {
      fail('E_EXTENSION_REGISTRATION_INVALID', `Registration cannot contain an accessor at ${field}.`, { path: field });
    }
    if (/^(?:module|path|url|connector|factory|loader|import|require|credential|secret|token|grant|approval|transition|dispatch|targetAdapter)$/iu.test(key)) {
      fail('E_EXTENSION_REGISTRATION_INVALID', `Registration contains an implementation or authority field at ${field}.`, { path: field });
    }
...
function governedEffectRank(value) {
  if (typeof value !== 'string' || !Object.hasOwn(OPERATE_GOVERNED_EFFECT_RANK_V2, value)) return null;
  const rank = OPERATE_GOVERNED_EFFECT_RANK_V2[value];
  return Number.isSafeInteger(rank) ? rank : null;
}

function sortIdentities(values) {
  return values.map(clone).sort((left, right) => (
...
function canonicalizeRegistration(kind, registration) {
  ...
  if (prohibition !== null) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'Reference-governed registrations cannot declare a prohibited semantic class.', {
      registrationId: registration?.[definition.idField] ?? null,
      prohibition,
    });
  }
  try {
    assertProtocolArtifact(definition.contractKind, registration, { protocolVersion: PROTOCOL_VERSION });
  } catch (cause) {
  ...
  const reserved = SHIPPED_GOVERNED_REGISTRATION_RESERVATIONS.find((entry) => (
  ...
  if (reserved && (
    ...
    fail('E_EXTENSION_REGISTRATION_CONFLICT', 'A shipped open-reference identity must equal its immutable catalog declaration.', {
...
function canonicalizeList(kind, registrations) {
  if (!Array.isArray(registrations) || registrations.length === 0) {
    fail('E_EXTENSION_REGISTRATION_INVALID', `${REGISTRATION_KINDS[kind].field} requires at least one registration.`);
  }
  const seen = new Set();
  const canonical = registrations.map((entry) => {
    ...
    if (seen.has(identity)) {
      fail('E_EXTENSION_REGISTRATION_CONFLICT', `Duplicate governed extension registration ${identity}.`, { identity });
    }
    seen.add(identity);
    return registration;
  });
  return canonical.sort((left, right) => registrationIdentity(kind, left).localeCompare(registrationIdentity(kind, right)));
}

export function createOperateGovernedExtensionRegistryV2(input = {}) {
  if (!plainRecord(input)) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'Governed extension registrations must be one object of arrays.');
  }
  const descriptors = Object.getOwnPropertyDescriptors(input);
  if (Object.values(descriptors).some((descriptor) => !Object.hasOwn(descriptor, 'value')
    || descriptor.get !== undefined || descriptor.set !== undefined)) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'Governed extension registry fields must be closed data properties.');
  }
  const unknown = Object.getOwnPropertyNames(input)
    .filter((key) => !['capabilityProviders', 'policyProviders', 'executors'].includes(key));
  if (unknown.length > 0) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'Governed extension registry contains unsupported fields.', { unknown: unknown.sort() });
  }
  const capabilityProviders = input.capabilityProviders
    ?? OPERATE_CONTRACT_CATALOG_V2.extensions.capabilityProviders.map((entry) => registrationArtifact('capability', entry));
  const policyProviders = input.policyProviders
    ?? OPERATE_CONTRACT_CATALOG_V2.extensions.policyProviders.map((entry) => registrationArtifact('policy', entry));
  const executors = input.executors
    ?? OPERATE_CONTRACT_CATALOG_V2.extensions.executors.map((entry) => registrationArtifact('executor', entry));
  return freeze({
...
export const OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2 = createOperateGovernedExtensionRegistryV2();
...
export function canonicalizeOperateGovernedExtensionRegistryV2(registry) {
  if (!plainRecord(registry)) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'A governed extension registry must be a complete plain object.');
  }
  ...
  if (!sameJson(keys, expected)) {
    fail('E_EXTENSION_REGISTRATION_INVALID', 'A governed extension registry must declare exactly its three closed collections.', { fields: keys });
  }
  const input = {};
  for (const field of expected) {
    const descriptor = descriptors[field];
    if (!Object.hasOwn(descriptor, 'value') || !Array.isArray(descriptor.value)) {
      fail('E_EXTENSION_REGISTRATION_INVALID', 'Governed registry collections must be data arrays.', { field });
    }
...
function findRegistration(kind, registry, id, version) {
  ...
  const match = canonical[definition.field].find((entry) => (
    entry[definition.idField] === id && entry[definition.versionField] === version
  ));
  return match === undefined ? null : clone(match);
}

export function findOperateCapabilityProviderRegistrationV2(registry, providerId, { providerVersion } = {}) {
  return findRegistration('capability', registry, providerId, providerVersion);
}

export function findOperatePolicyProviderRegistrationV2(registry, providerId, { providerVersion } = {}) {
...
function selectRegistration(kind, registry, criteria) {
  ...
  if (criteria.protocolVersion !== PROTOCOL_VERSION) return selectionFailure(kind, 'protocol-version-incompatible', registration);
  ...
  if (compareSemver(runtimeVersion, compatibility.minimumRuntimeVersion) < 0
    || (compatibility.maximumRuntimeVersion !== null && compareSemver(runtimeVersion, compatibility.maximumRuntimeVersion) > 0)) {
  ...
  if (kind === 'policy' && criteria.policyRef && !exactIdentity(registration.policyRefs, criteria.policyRef)) {
...
export function classifyOperateExecutorRecoveryCapabilityV2(executor) {
  ...
  if (registration.reconciliation.supported === true
...
export function assertOperateExecutorRegistrationV2(executor, { registry = OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2 } = {}) {
  const registered = findOperateExecutorRegistrationV2(registry, executor?.executorId, {
    executorVersion: executor?.executorVersion,
  });
  if (registered === null || !sameJson(registered, canonicalizeRegistration('executor', executor))) {
...
export function createTrustedExecutorBindingV2({ selection, trustedHost }) {
  if (selection?.status !== 'available' || !selection.registration) {
    fail('EXECUTOR_UNAVAILABLE', 'A trusted connector can bind only beneath an available exact Executor selection.');
  }
  ...
  if (host === null
    ...
    fail('EXECUTOR_UNAVAILABLE', 'Trusted host implementation must bind the selected Executor and one host-owned connector exactly.', {
  ...
  const binding = {
    ...
    schemaVersion: '1.0.0',
    protocolVersion: PROTOCOL_VERSION,
    executor: { executorId: registration.executorId, executorVersion: registration.executorVersion },
...
export function assertTrustedExecutorBindingV2(binding, { executor }) {
  ...
  if (!TRUSTED_BINDINGS.has(binding)
    || !exactRecord(binding, [
      'kind', 'schemaVersion', 'protocolVersion', 'executor', 'implementation',
      'connector', 'registrationHash', 'bindingHash',
    ])
    ...
    || binding.schemaVersion !== '1.0.0' || binding.protocolVersion !== PROTOCOL_VERSION
    ...
    || binding.bindingHash !== sha256Jcs(Object.fromEntries(Object.entries(binding).filter(([key]) => key !== 'bindingHash')))) {
    fail('EXECUTOR_UNAVAILABLE', 'Trusted connector binding is invalid or does not belong to the selected Executor.', {
      executorId: executor?.executorId ?? null,
    });
  }
  return binding;
...
const EXECUTOR_INPUT_FIELDS = Object.freeze([
  'kind', 'schemaVersion', 'protocolVersion', 'operation', 'payload',
  'rollbackBaseline', 'requestFingerprint', 'envelopeHash',
]);
...
function assertExecutorPayload(value, field, operation) {
  if (!exactRecord(value, PAYLOAD_FIELDS)
    ...
    fail('OPERATION_CONFLICT', `${field} must be one exact reviewed Artifact identity, content hash, and closed value.`, {
      operationId: operation.operationId,
      field,
    });
  }
...
function assertClosedExecutorValue(value, path) {
  ...
  for (const [key, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(value))) {
    if (!Object.hasOwn(descriptor, 'value') || descriptor.get !== undefined || descriptor.set !== undefined
      ...
      fail('OPERATION_CONFLICT', `Executor input cannot contain executable data at ${path}.${key}.`, { path: `${path}.${key}` });
    }
    assertClosedExecutorValue(descriptor.value, `${path}.${key}`);
  }
}

export function deriveContainedExecutorRequestFingerprintV2({ operation, payload, rollbackBaseline } = {}) {
  ...
  try {
    ...
    fail('OPERATION_CONFLICT', 'Executor request fingerprint requires one contract-valid governed operation.', {
      cause: cause?.code ?? null,
    });
  }
  ...
  if ((operation.rollbackClass === 'not-applicable') !== (rollbackBaseline === null)) {
    fail('OPERATION_CONFLICT', 'Rollback baseline presence must equal the governed rollback classification.', {
      operationId: operation.operationId,
    });
  }
  if (rollbackBaseline !== null) assertExecutorPayload(rollbackBaseline, 'rollbackBaseline', operation);
...
export function deriveContainedExecutorRequestFingerprintFromBindingV2({
  ...
  if (!exactRecord(payload, ['artifactId', 'contentHash'])
    ...
    fail('OPERATION_CONFLICT', 'Executor request binding must contain only exact reviewed Artifact identities and canonical hashes.', {
...
export function createContainedExecutorInputEnvelopeV2({ operation, payload, rollbackBaseline = null } = {}) {
  const requestFingerprint = deriveContainedExecutorRequestFingerprintV2({ operation, payload, rollbackBaseline });
  if (operation.requestFingerprint !== requestFingerprint) {
    fail('OPERATION_CONFLICT', 'Governed operation requestFingerprint must commit the complete executor input envelope.', {
      operationId: operation.operationId,
    });
  }
  const envelope = {
...
export function assertContainedExecutorInputEnvelopeV2(envelope, { operation, rollbackPlan = null } = {}) {
  assertClosedExecutorValue(envelope, 'executorInput');
  if (!exactRecord(envelope, EXECUTOR_INPUT_FIELDS)
    || !exactRecord(envelope?.payload, PAYLOAD_FIELDS)
    || (envelope?.rollbackBaseline !== null && !exactRecord(envelope?.rollbackBaseline, PAYLOAD_FIELDS))
    ...
    || envelope.envelopeHash !== sha256Jcs(Object.fromEntries(
      Object.entries(envelope).filter(([key]) => key !== 'envelopeHash'),
    ))) {
    fail('OPERATION_CONFLICT', 'Executor input envelope must be plain, closed, exact, and bound to the governed operation.', {
      operationId: operation?.operationId ?? null,
    });
  }
  ...
  if (expected !== envelope.requestFingerprint) {
    fail('OPERATION_CONFLICT', 'Executor payload or rollback baseline diverges from the authorized request fingerprint.', {
      operationId: operation.operationId,
    });
  }
  if (operation.operationKind === 'rollback' && (
    ...
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback executor input must equal the authorized rollback-plan baseline identity and hash.', {
      operationId: operation.operationId,
    });
  }
  return envelope;
