...
export const LOCAL_OPERATE_ARTIFACT_EVIDENCE_PROVIDER_ID_V2 = 'local-operate-artifact-evidence-provider';
export const LOCAL_OPERATE_ARTIFACT_EVIDENCE_RESOLVER_ID_V2 = 'local-operate-artifact-evidence-resolver';
...
function acceptedRecord(records, artifactId) {
  return records.find((record) => record?.accepted === true && record?.artifact?.artifactId === artifactId) ?? null;
}
...
function accessAllows(record, capabilities) {
  ...
  return Array.isArray(required)
    && required.length > 0
    && required.every((capability) => typeof capability === 'string' && capabilities.includes(capability));
}

function sameBinding(candidate, artifact) {
  return candidate.scopeId === artifact.scopeId
...
function assertSelection(candidate, provider, resolver) {
  return candidate?.evidenceKind === 'operate-artifact'
 ...
 */
export function resolveLocalOperateArtifactEvidenceV2(candidate, {
  provider,
  resolver,
  capabilities = [],
  ...context
  ...
  if (!locator || typeof locator !== 'object' || typeof locator.artifactId !== 'string'
    || typeof locator.expectedArtifactType !== 'string' || typeof locator.expectedSchemaId !== 'string'
    || typeof locator.expectedSchemaVersion !== 'string'
    || (locator.expectedRawHash !== undefined && (typeof locator.expectedRawHash !== 'string' || !DIGEST.test(locator.expectedRawHash)))
    || (locator.expectedCanonicalHash !== undefined && (typeof locator.expectedCanonicalHash !== 'string' || !DIGEST.test(locator.expectedCanonicalHash)))) {
  ...
  if (!validArtifact(artifact)) return safeError(candidate, provider, resolver, 'ARTIFACT_NOT_FOUND');
  if (!record.sourceContract || typeof record.sourceContract !== 'object'
  ...
  if (!accessAllows(record, capabilities)) return safeError(candidate, provider, resolver, 'CAPABILITY_DENIED');
  if (!sameBinding(candidate, artifact)) return safeError(candidate, provider, resolver, 'EVIDENCE_SOURCE_SCOPE_MISMATCH');
  if (artifact.artifactType !== locator.expectedArtifactType
    || artifact.schemaId !== locator.expectedSchemaId
    || artifact.artifactSchemaVersion !== locator.expectedSchemaVersion) {
    return safeError(candidate, provider, resolver, 'OBJECT_TYPE_MISMATCH');
  }
  if ((locator.expectedRawHash !== undefined && locator.expectedRawHash !== artifact.rawHash)
    || (locator.expectedCanonicalHash !== undefined && locator.expectedCanonicalHash !== artifact.canonicalHash)) {
  ...
  return freeze({
    ...
    capture: {
      ...
      locator: {
        ...
        ...(artifact.canonicalHash === null ? {} : { expectedCanonicalHash: artifact.canonicalHash }),
      },
      provenance: {
        artifactId: artifact.artifactId,
        artifactType: artifact.artifactType,
