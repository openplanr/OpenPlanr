...
function sortedChange(change) {
  const normalized = {
    ...
    kind: change.kind,
    ...(change.evidenceRefIds === undefined ? {} : { evidenceRefIds: sortedUniqueIds(change.evidenceRefIds, 'Delta evidence references') }),
  };
  return normalized;
}

function canonicalChanges(changes) {
  return changes.map(sortedChange).sort((left, right) => (
...
function assertSnapshotState(snapshot, state, subject) {
  ...
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', `${subject} requires a valid snapshot and exact operating-model state.`, {
...
function diffRevisions(priorSnapshot, currentSnapshot) {
  ...
  return canonicalChanges(ids.flatMap((artifactId) => {
    const previous = prior.get(artifactId);
    const next = current.get(artifactId);
    if (!previous) return [{ subjectId: artifactId, kind: 'added', evidenceRefIds: next.evidenceRefIds }];
    if (!next) return [{ subjectId: artifactId, kind: 'removed', evidenceRefIds: previous.evidenceRefIds }];
    if (previous.revision !== next.revision) {
      return [{
        subjectId: artifactId,
        kind: 'changed',
        evidenceRefIds: [...new Set([...previous.evidenceRefIds, ...next.evidenceRefIds])].sort(),
      }];
    }
    return [];
  }));
...
function latestObservations(observations, snapshot, evidenceRefs) {
  if (!Array.isArray(observations)) fail('RESULT_CONTRACT_INVALID', 'Metric observations must be an array.');
  ...
  for (const candidate of observations) {
    ...
    try {
      assertProtocolArtifact('operating-metric-observation', candidate, { protocolVersion: PROTOCOL_VERSION });
    } catch (cause) {
      fail('RESULT_CONTRACT_INVALID', 'Metric delta comparison accepts only typed metric observations.', { cause: cause?.code ?? null });
    }
    if (candidate.scopeId !== snapshot.scopeId
      || candidate.domainId !== snapshot.domainId
      || candidate.domainVersion !== snapshot.domainVersion) {
      fail('OPERATING_SCOPE_INVALID', 'Metric observation does not bind to its compared immutable snapshot.', {
        observationId: candidate.observationId,
        snapshotId: snapshot.snapshotId,
      });
    }
    if (!snapshot.sourceArtifactIds.includes(candidate.sourceArtifactId)) {
      fail('STATE_TRANSITION_INVALID', 'Metric observations must retain a source Artifact declared by their bound snapshot.', {
        observationId: candidate.observationId,
    ...
    for (const evidenceRefId of candidate.evidenceRefIds) {
      ...
      if (!evidenceRef || evidenceRef.sourceArtifactId !== candidate.sourceArtifactId) {
        fail('STATE_TRANSITION_INVALID', 'Metric observations may cite only selected, same-source EvidenceRefs.', {
          observationId: candidate.observationId,
          evidenceRefId,
        });
      }
    ...
    if (!existing || candidate.observedAt > existing.observedAt
      || (candidate.observedAt === existing.observedAt && candidate.observationId > existing.observationId)) {
...
function diffMetrics(priorSnapshot, currentSnapshot, priorObservations, currentObservations, priorEvidenceRefs, currentEvidenceRefs) {
  const prior = priorSnapshot ? latestObservations(priorObservations, priorSnapshot, priorEvidenceRefs) : new Map();
  const current = latestObservations(currentObservations, currentSnapshot, currentEvidenceRefs);
  const ids = [...new Set([...prior.keys(), ...current.keys()])].sort();
  return canonicalChanges(ids.flatMap((metricId) => {
    const before = prior.get(metricId);
    const after = current.get(metricId);
    if (!before) return [{ subjectId: metricId, kind: 'added', evidenceRefIds: after.evidenceRefIds }];
    if (!after) return [{ subjectId: metricId, kind: 'removed', evidenceRefIds: before.evidenceRefIds }];
    if (before.value !== after.value || before.unit !== after.unit) {
      return [{
        subjectId: metricId,
        kind: 'changed',
        evidenceRefIds: [...new Set([...before.evidenceRefIds, ...after.evidenceRefIds])].sort(),
      }];
    }
    return [];
  }));
...
function evidenceArtifactIndex(records, snapshot, evidenceRefs) {
  if (!Array.isArray(records)) fail('RESULT_CONTRACT_INVALID', 'Delta comparison requires Evidence Artifact metadata.');
  const selectedArtifactIds = new Set([...evidenceRefs.values()].map(({ evidenceArtifactId }) => evidenceArtifactId));
  const result = new Map();
  for (const artifact of records) {
    ...
    try {
      ...
      fail('RESULT_CONTRACT_INVALID', 'Delta comparison accepts only typed Evidence Artifacts.', { cause: cause?.code ?? null });
    }
    if (artifact.scopeId !== snapshot.scopeId || artifact.domainId !== snapshot.domainId
      ...
      fail('OPERATING_SCOPE_INVALID', 'Delta Evidence Artifacts must be unique and match the selected immutable snapshot.', {
...
function evidenceIndex(records, evidenceArtifacts, snapshot) {
  ...
  for (const evidence of records ?? []) {
    ...
    try {
      ...
      fail('RESULT_CONTRACT_INVALID', 'Delta comparison accepts only typed EvidenceRefs.', { cause: cause?.code ?? null });
    }
    if (evidence.scopeId !== snapshot.scopeId || evidence.domainId !== snapshot.domainId
      || evidence.domainVersion !== snapshot.domainVersion || index.has(evidence.evidenceRefId)
      || !snapshot.sourceArtifactIds.includes(evidence.sourceArtifactId)) {
      fail('STATE_TRANSITION_INVALID', 'Delta comparison requires snapshot-declared, unique EvidenceRef identities.', {
  ...
  for (const evidence of index.values()) {
    ...
    if (!artifact
      ...
      fail('STATE_TRANSITION_INVALID', 'Delta EvidenceRefs must resolve to their exact selected Evidence Artifact.', {
        evidenceRefId: evidence.evidenceRefId,
      });
    }
  }
...
function derivedEvidenceStatus({ evidenceRefs, claims, snapshot }) {
  ...
  for (const claim of scopedRecords(claims, snapshot, 'Claim')) {
    ...
    try {
      ...
      fail('RESULT_CONTRACT_INVALID', 'Delta claim analysis accepts only typed Claim records.', { cause: cause?.code ?? null });
    }
    const support = new Set(claim.supportingEvidenceRefIds);
    const contradiction = new Set(claim.contradictingEvidenceRefIds);
    if (support.size !== claim.supportingEvidenceRefIds.length
      || contradiction.size !== claim.contradictingEvidenceRefIds.length
      || [...support].some((id) => contradiction.has(id))) {
      fail('STATE_TRANSITION_INVALID', 'A Delta Claim may not overlap or duplicate support and contradiction EvidenceRefs.', {
        claimId: claim.claimId,
      });
    }
    for (const evidenceRefId of support) {
      if (!evidenceRefs.has(evidenceRefId) || evidenceRefs.get(evidenceRefId).sourceArtifactId !== claim.sourceArtifactId) {
        fail('STATE_TRANSITION_INVALID', 'A Claim references unavailable or cross-source typed evidence.', { claimId: claim.claimId, evidenceRefId });
      }
    }
...
function recordsById(records, id, snapshot, kind, { snapshotSourcesOnly = false } = {}) {
  ...
  for (const record of scopedRecords(records, snapshot, kind)) {
    if (snapshotSourcesOnly && !snapshot.sourceArtifactIds.includes(record?.sourceArtifactId)) continue;
    try {
      assertProtocolArtifact(kind, record, { protocolVersion: PROTOCOL_VERSION });
    } catch (cause) {
      fail('RESULT_CONTRACT_INVALID', `Delta ${kind} analysis requires typed records.`, { cause: cause?.code ?? null });
    }
    if ((kind === 'operating-risk' && record.revisionId !== record.riskId)
      || (kind === 'operating-assumption' && record.revisionId !== record.assumptionId)) {
      fail('STATE_TRANSITION_INVALID', 'Delta lifecycle records must retain their canonical immutable revision identity.', {
        entityId: record[id],
        revisionId: record.revisionId,
      });
    }
    if (result.has(record[id])) fail('STATE_TRANSITION_INVALID', `Delta analysis cannot resolve duplicate ${kind} identities.`, { entityId: record[id] });
...
function verificationLearningRevisits(outcomes, learnings, snapshot, evidenceRefs) {
  if (!Array.isArray(outcomes) || !Array.isArray(learnings)) {
    fail('RESULT_CONTRACT_INVALID', 'Delta verification-revisit analysis requires Outcome and Learning collections.');
  }
  ...
  for (const outcome of outcomes) {
    ...
    try {
      ...
      fail('RESULT_CONTRACT_INVALID', 'Delta verification-revisit analysis accepts only typed Outcomes.', { cause: cause?.code ?? null });
    }
    if (!snapshot.sourceArtifactIds.includes(outcome.sourceArtifactId)
      || outcome.evidenceRefIds.some((id) => evidenceRefs.get(id)?.sourceArtifactId !== outcome.sourceArtifactId)
      || outcomesById.has(outcome.outcomeId)) {
      fail('STATE_TRANSITION_INVALID', 'Outcome revisit analysis requires one snapshot-declared accepted source and matching typed EvidenceRefs.', {
  ...
  for (const learning of learnings) {
    ...
    try {
      ...
      fail('RESULT_CONTRACT_INVALID', 'Delta verification-revisit analysis accepts only typed Learnings.', { cause: cause?.code ?? null });
    }
    const outcome = outcomesById.get(learning.outcomeId);
    if (!outcome || learning.sourceArtifactId !== outcome.sourceArtifactId
      || !snapshot.sourceArtifactIds.includes(learning.sourceArtifactId)
      || learning.evidenceRefIds.some((id) => evidenceRefs.get(id)?.sourceArtifactId !== learning.sourceArtifactId)) {
      fail('STATE_TRANSITION_INVALID', 'Learning revisit analysis requires its retained Outcome and snapshot-declared typed evidence.', {
...
function deltaMateriality(delta) {
  const fields = [
    'sourceRevisionChanges', 'metricChanges', 'staleEvidenceRefIds', 'conflictingEvidenceRefIds',
...
export function classifyOperatingDeltaMaterialityV2(delta) {
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', 'Delta materiality requires a valid operating Delta.', { cause: cause?.code ?? null });
  }
  return freeze({ material: deltaMateriality(delta), reason: deltaMateriality(delta) ? 'material-change' : 'no-material-change' });
}
...
export function assertOperatingDeltaV2(delta, { currentSnapshot, priorSnapshot = null } = {}) {
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', 'Operating Delta is not a valid Protocol 2.0 record.', { cause: cause?.code ?? null });
  }
  ...
  for (const field of ['sourceRevisionChanges', 'metricChanges']) {
    ...
    if (!sameCanonical(delta[field], canonical)) {
      fail('STATE_TRANSITION_INVALID', 'Operating Delta change records must use stable canonical ordering.', { deltaId: delta.deltaId, field });
    }
  }
  for (const field of ['staleEvidenceRefIds', 'conflictingEvidenceRefIds', 'invalidatedAssumptionIds', 'exposedRiskIds', 'decisionRevisitIds']) {
    const canonical = sortedUniqueIds(delta[field], field);
    if (!sameCanonical(delta[field], canonical)) {
      fail('STATE_TRANSITION_INVALID', 'Operating Delta identity collections must use stable canonical ordering.', { deltaId: delta.deltaId, field });
    }
  }
  if (currentSnapshot !== undefined) {
    if (delta.currentSnapshotId !== currentSnapshot.snapshotId || delta.scopeId !== currentSnapshot.scopeId
      || delta.domainId !== currentSnapshot.domainId || delta.domainVersion !== currentSnapshot.domainVersion) {
      fail('OPERATING_SCOPE_INVALID', 'Delta must bind to its exact current snapshot and scope.', { deltaId: delta.deltaId });
    }
  }
  if (priorSnapshot !== null && priorSnapshot !== undefined && delta.priorSnapshotId !== priorSnapshot.snapshotId) {
    fail('STATE_TRANSITION_INVALID', 'Delta priorSnapshotId must match the supplied immutable predecessor.', { deltaId: delta.deltaId });
  }
...
export function deriveOperatingDeltaV2({
  ...
  if (typeof deltaId !== 'string' || deltaId.length === 0 || typeof sourceArtifactId !== 'string' || sourceArtifactId.length === 0) {
    fail('STATE_TRANSITION_INVALID', 'Delta derivation requires runtime-issued Delta and source Artifact identities.');
  }
  ...
  if ((priorSnapshot === null) !== (priorState === null)) {
    fail('STATE_TRANSITION_INVALID', 'Delta derivation requires both predecessor snapshot and state, or neither.');
  }
  if (priorSnapshot !== null) {
    ...
    if (current.snapshot.previousSnapshotId !== prior.snapshot.snapshotId) {
      fail('STATE_TRANSITION_INVALID', 'Delta predecessor must be the current snapshot’s immutable prior link.', {
    ...
    fail('STATE_TRANSITION_INVALID', 'A snapshot with a prior link requires that exact predecessor for Delta derivation.', {
      currentSnapshotId: current.snapshot.snapshotId,
      priorSnapshotId: current.snapshot.previousSnapshotId,
    });
  }
  if (!current.snapshot.sourceArtifactIds.includes(sourceArtifactId)) {
    fail('STATE_TRANSITION_INVALID', 'Delta source Artifact must be named by the current immutable snapshot.', {
      sourceArtifactId,
      currentSnapshotId: current.snapshot.snapshotId,
    });
  }
  ...
  const priorEvidence = prior
    ...
    : new Map();
  const status = derivedEvidenceStatus({ evidenceRefs: evidence, claims, snapshot: current.snapshot });
  const scopedAssumptions = recordsById(current.state.assumptions, 'revisionId', current.snapshot, 'operating-assumption');
  const scopedRisks = recordsById(current.state.risks, 'revisionId', current.snapshot, 'operating-risk');
  ...
  const priorRisks = prior
    ...
    : new Map();
  const scopedDecisions = recordsById(decisions, 'decisionId', current.snapshot, 'operating-decision', { snapshotSourcesOnly: true });
  ...
  const exposedRiskIds = [...scopedRisks.values()]
    .filter((risk) => {
      if (risk.state === 'closed' || risk.state === 'superseded' || risk.exposure <= 0) return false;
      const before = priorEquivalent(risk, priorRisks);
      return !before || before.state === 'closed' || before.state === 'superseded' || before.exposure !== risk.exposure;
    })
    .map(({ riskId }) => riskId)
    .sort();
  ...
  const learnedDecisionRevisitIds = verificationLearningRevisits(outcomes, learnings, current.snapshot, evidence);
  const decisionRevisitIds = [...scopedDecisions.values()]
    ...
    .sort();
  const delta = {
    kind: 'operating-delta', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    deltaId,
  ...
  return assertOperatingDeltaV2(delta, { currentSnapshot: current.snapshot, priorSnapshot: prior?.snapshot ?? null });
}
