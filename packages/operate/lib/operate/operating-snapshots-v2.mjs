...
import { assertOperatingModelStateV2, buildOperatingModelStateV2 } from './operating-state-v2.mjs';
...
function assertScope(scope) {
  if (
    ...
    throw snapshotError(
      ...
      'Operating snapshots require one explicit scope and domain binding.',
    );
  }
}
...
function normalizedIds(ids, label, { minimum = 0 } = {}) {
  if (!Array.isArray(ids) || ids.some((id) => typeof id !== 'string' || id.length === 0)) {
    throw snapshotError(
      ...
      `${label} must be an array of durable identities.`,
    );
  }
  ...
  if (unique.length !== ids.length || unique.length < minimum) {
    throw snapshotError(
      'RESULT_CONTRACT_INVALID',
      `${label} must be unique${minimum > 0 ? ' and nonempty' : ''}.`,
    );
  }
  return unique.sort();
}
  ...
  const normalized = sourceRevisions.map((entry) => {
    if (
      ...
      Object.keys(entry).some(
        (key) => !['sourceArtifactId', 'revision', 'evidenceRefIds'].includes(key),
      ) ||
      ...
      throw snapshotError(
        'RESULT_CONTRACT_INVALID',
        'Snapshot revisions must bind an included source Artifact to one explicit revision.',
      );
    }
    const refs =
      entry.evidenceRefIds === undefined
        ? undefined
        : normalizedIds(entry.evidenceRefIds, 'Snapshot revision evidence references');
    if (refs?.some((id) => !knownEvidence.has(id))) {
      throw snapshotError(
        'OPERATING_SCOPE_INVALID',
        'Snapshot revisions may reference only included typed EvidenceRefs.',
        {
    ...
    if (semantic.has(key)) {
      throw snapshotError(
        'RESULT_CONTRACT_INVALID',
        'Snapshot source revisions must not duplicate a source Artifact and revision pair.',
        {
          sourceArtifactId: entry.sourceArtifactId,
        },
      );
...
export function deriveOperatingSnapshotRuntimeHashV2(snapshot) {
  try {
    ...
    throw snapshotError(
      'RESULT_CONTRACT_INVALID',
      'Operating snapshots must contain deterministic JSON values.',
    );
  }
}
...
export function assertOperatingSnapshotV2(snapshot, { state, previousSnapshotId } = {}) {
  try {
    ...
    throw snapshotError(
      'RESULT_CONTRACT_INVALID',
      'Operating snapshot is not a valid Protocol 2.0 record.',
      {
  ...
  if (!validTimestamp(snapshot.createdAt)) {
    throw snapshotError(
      ...
      'Operating snapshots require an Event-owned createdAt timestamp.',
      {
        snapshotId: snapshot.snapshotId,
      },
    );
  }
  const sourceArtifactIds = normalizedIds(snapshot.sourceArtifactIds, 'Snapshot source Artifacts', {
    minimum: 1,
  });
  ...
  if (
    !sameCanonicalArray(snapshot.sourceArtifactIds, sourceArtifactIds) ||
    !sameCanonicalArray(snapshot.evidenceRefIds, evidenceRefIds) ||
    !sameCanonicalArray(snapshot.sourceRevisions, sourceRevisions)
  ) {
    throw snapshotError(
      'STATE_TRANSITION_INVALID',
      'Operating snapshots must retain canonically ordered safe provenance descriptors.',
      {
        snapshotId: snapshot.snapshotId,
      },
    );
  ...
  if (
    snapshot.domainContract.apiDomainId !== snapshot.domainId ||
    snapshot.domainContract.version !== snapshot.domainVersion
  ) {
    throw snapshotError(
      'OPERATING_SCOPE_INVALID',
      'Snapshot domain-contract identity must match its explicit API domain and version.',
      {
        snapshotId: snapshot.snapshotId,
      },
    );
  ...
  if (previousSnapshotId !== undefined && snapshot.previousSnapshotId !== previousSnapshotId) {
    throw snapshotError(
      'STATE_TRANSITION_INVALID',
      'Snapshot prior linkage is not the deterministic current-scope predecessor.',
      {
        snapshotId: snapshot.snapshotId,
        previousSnapshotId,
      },
  ...
  if (snapshot.runtimeHash !== deriveOperatingSnapshotRuntimeHashV2(snapshot)) {
    throw snapshotError(
      'STATE_TRANSITION_INVALID',
      'Operating snapshot runtimeHash does not match its immutable safe descriptor.',
      {
        snapshotId: snapshot.snapshotId,
      },
    );
  ...
  if (state !== undefined) {
    ...
    if (
      linked.stateId !== snapshot.stateId ||
      ...
      throw snapshotError(
        'STATE_TRANSITION_INVALID',
        'Snapshot and operating-model state must have one exact immutable binding.',
        {
          snapshotId: snapshot.snapshotId,
...
function expectedPreviousSnapshotId(previousSnapshots, scope) {
  const matches = (previousSnapshots ?? [])
    .filter(
      (snapshot) =>
        snapshot.scopeId === scope.scopeId &&
...
export function buildOperatingSnapshotStateTransactionV2(
  ...
  if (
    ...
    throw snapshotError(
      'RESULT_CONTRACT_INVALID',
      'Snapshot materialization requires a validated manifest and runtime-owned draft.',
    );
  }
  const supportedRequest = new Set([
    'scope',
    'domainContract',
    'sourceArtifactIds',
    'evidenceRefIds',
  ...
  if (
    ...
    throw snapshotError(
      'STATE_TRANSITION_INVALID',
      'Snapshot materialization must use only complete runtime-owned identities and timestamp.',
    );
  }
  assertScope(request.scope);
  const scope = request.scope;
  if (
    !request.domainContract ||
    typeof request.domainContract !== 'object' ||
    Array.isArray(request.domainContract) ||
    Object.keys(request.domainContract).some(
      (key) => !['apiDomainId', 'id', 'version'].includes(key),
    ) ||
    request.domainContract.apiDomainId !== scope.domainId ||
    request.domainContract.version !== scope.domainVersion ||
    typeof request.domainContract.id !== 'string' ||
    request.domainContract.id.length === 0
  ) {
    throw snapshotError(
      'OPERATING_SCOPE_INVALID',
      'Snapshot materialization requires an explicit matching public domain-contract identity.',
    );
  }
  const sourceArtifactIds = normalizedIds(request.sourceArtifactIds, 'Snapshot source Artifacts', {
    minimum: 1,
  });
