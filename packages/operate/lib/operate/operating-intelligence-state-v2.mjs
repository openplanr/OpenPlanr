...
function sortedBy(records, id) {
  ...
  if (new Set(normalized.map((record) => record[id])).size !== normalized.length) {
    fail(
      ...
      'One intelligence transition cannot contain duplicate durable identities.',
      { id },
    );
  }
  return normalized;
...
function assertSnapshotState(snapshot, state) {
  ...
  try {
    ...
    fail(
      ...
      'Operating intelligence facts require a valid immutable snapshot and matching model state.',
      {
...
function evidenceArtifactMap(records, snapshot, selectedEvidenceRefs) {
  if (!Array.isArray(records))
    fail(
      'RESULT_CONTRACT_INVALID',
      'Intelligence fact materialization requires Evidence Artifact metadata.',
    );
  const selectedArtifactIds = new Set(
    selectedEvidenceRefs.map(({ evidenceArtifactId }) => evidenceArtifactId),
  );
  ...
  for (const artifact of records) {
    ...
    try {
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Intelligence fact materialization accepts only typed Evidence Artifacts.',
        { cause: cause?.code ?? null },
      );
    }
    if (!sameScope(artifact, snapshot) || result.has(artifact.artifactId)) {
      fail(
        ...
        'Evidence Artifacts must be unique and match the immutable snapshot scope.',
        {
...
function evidenceMap(records, snapshot, evidenceArtifacts) {
  if (!Array.isArray(records))
    fail(
      'RESULT_CONTRACT_INVALID',
      'Intelligence fact materialization requires an EvidenceRef collection.',
    );
  ...
  for (const evidence of records) {
    ...
    try {
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Intelligence fact materialization accepts only typed EvidenceRefs.',
        { cause: cause?.code ?? null },
      );
    }
    if (!sameScope(evidence, snapshot) || seen.has(evidence.evidenceRefId)) {
      fail(
        'OPERATING_SCOPE_INVALID',
        'EvidenceRefs selected by an immutable snapshot must be unique and in scope.',
        {
  ...
  for (const evidence of selected) {
    ...
    if (
      ...
      fail(
        'OPERATING_SCOPE_INVALID',
        'EvidenceRefs must bind their exact selected Evidence Artifact.',
        {
...
function sourceArtifactMap(records, snapshot) {
  if (!Array.isArray(records)) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Intelligence fact materialization requires a producer Artifact collection.',
    );
  }
  const result = new Map();
  for (const artifact of records) {
    try {
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Intelligence fact materialization accepts only typed producer Artifacts.',
        {
          cause: cause?.code ?? null,
        },
      );
    ...
    if (!sameScope(artifact, snapshot) || result.has(artifact.artifactId)) {
      fail(
        'OPERATING_SCOPE_INVALID',
        'Producer Artifacts must be unique and match the immutable snapshot scope.',
        {
          artifactId: artifact?.artifactId ?? null,
        },
      );
...
function assertEvidence(
  ids,
  evidence,
  { subject, sourceArtifactId, producerArtifact = null, required = false } = {},
) {
  if (!Array.isArray(ids) || (required && ids.length === 0)) {
    fail(
      'RESULT_CONTRACT_INVALID',
      `${subject} requires ${required ? 'typed evidence' : 'an evidence-reference array'}.`,
    );
  }
  if (new Set(ids).size !== ids.length)
    fail('RESULT_CONTRACT_INVALID', `${subject} evidence references must be unique.`);
  for (const evidenceRefId of ids) {
    ...
    if (!ref || ref.freshness === 'stale' || !retainsCustody) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} requires current or historical typed EvidenceRefs; stale or unavailable evidence fails closed.`,
        {
          evidenceRefId,
          sourceArtifactId,
        },
...
function assertSourceArtifact(record, snapshot, { subject, producerArtifacts = null } = {}) {
  ...
  if (
    ...
    ].includes(producerArtifact.schemaId)
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} source Artifact must be explicitly declared by the immutable snapshot.`,
      {
...
function assertTyped(kind, record, snapshot, id) {
  try {
    ...
    fail('RESULT_CONTRACT_INVALID', `Intelligence transition contains an invalid ${kind} record.`, {
      cause: cause?.code ?? null,
    });
  }
  if (!sameScope(record, snapshot)) {
    fail('OPERATING_SCOPE_INVALID', `${kind} must match the immutable snapshot scope.`, {
...
function assertClaim(record, snapshot, evidence, assumptions, producerArtifacts) {
  ...
  const producerArtifact = assertSourceArtifact(record, snapshot, {
    subject: 'Claim',
    producerArtifacts,
  });
  ...
  assertEvidence(record.contradictingEvidenceRefIds, evidence, {
    subject: 'Claim contradiction',
    sourceArtifactId: record.sourceArtifactId,
    producerArtifact,
  });
  if (
    new Set([...record.supportingEvidenceRefIds, ...record.contradictingEvidenceRefIds]).size !==
    record.supportingEvidenceRefIds.length + record.contradictingEvidenceRefIds.length
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      'A Claim may not use one EvidenceRef as both support and contradiction.',
  ...
  for (const assumptionId of record.assumptionIds) {
    if (!assumptions.has(assumptionId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Claim assumptions must resolve within the immutable operating state.',
        {
          claimId: record.claimId,
          assumptionId,
        },
...
function assertObservation(record, snapshot, evidence, metrics) {
  ...
  if (record.snapshotId !== snapshot.snapshotId) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Metric observations must bind to the exact immutable snapshot used for the transition.',
      {
  ...
  if (!metric || metric.unit !== record.unit) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Metric observations require one in-scope metric with a matching unit.',
      {
        observationId: record.observationId,
...
function revisionIndex(records, idField, subject) {
  ...
  for (const record of records) {
    ...
    if (record.revisionId !== record[idField]) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} history has a duplicate or reset revision identity.`,
        {
    ...
    if (existing) {
      ...
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} history has hash-divergent records for one revision identity.`,
        {
          entityId: record?.[idField] ?? null,
          revisionId: record?.revisionId ?? null,
        },
...
function assertRevision(
  ...
  if (
    ...
    record.revision < 1 ||
    record.revisionId !== record[idField] ||
    !Object.hasOwn(record, 'predecessorRevisionId')
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} must carry explicit append-only revision metadata.`,
      {
        entityId: record?.[idField] ?? null,
      },
    );
  ...
  if (
    revisions.has(record.revisionId) ||
    [...revisions.values()].some((entry) => entry[idField] === record[idField])
  ) {
    fail(
      'CONCURRENT_MODIFICATION',
      `${subject} may not reuse an immutable record or revision identity.`,
      {
  ...
  if (record.revision === 1) {
    if (record.predecessorRevisionId !== null || record[stateField] !== initialState) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} initial revision must begin in its initial lifecycle state without a predecessor.`,
        {
          entityId: record[idField],
        },
      );
  ...
  if (
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} revision must retain exactly one predecessor and its complete canonical history.`,
      {
        entityId: record[idField],
  ...
  for (const field of immutableFields) {
    if (record[field] !== predecessor[field]) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} revisions may not alter immutable identity field ${field}.`,
        {
          entityId: record[idField],
          field,
        },
  ...
  if (!transitions[predecessor[stateField]]?.includes(record[stateField])) {
    fail('STATE_TRANSITION_INVALID', `${subject} revision uses an illegal lifecycle transition.`, {
      entityId: record[idField],
      from: predecessor[stateField],
      to: record[stateField],
    });
  }
}
...
function assertRisk(record, snapshot, evidence, risks, producerArtifacts) {
  ...
  if (record.state === 'closed' && record.closedAt !== record.updatedAt) {
    fail(
      'STATE_TRANSITION_INVALID',
      'A closed Risk must retain its Event timestamp as closedAt and updatedAt.',
...
function assertAssumption(record, snapshot, evidence, decisions, actions, assumptions) {
  ...
  if (['invalidated', 'closed'].includes(record.status) && record.closedAt !== record.updatedAt) {
    fail(
      'STATE_TRANSITION_INVALID',
      'An invalidated or closed Assumption must retain its Event timestamp as closedAt and updatedAt.',
      {
  ...
  for (const decisionId of record.affectedDecisionIds) {
    if (!decisions.has(decisionId))
      fail(
        'STATE_TRANSITION_INVALID',
        'Assumption affected decision must resolve to durable history.',
        { assumptionId: record.assumptionId, decisionId },
      );
  }
  for (const actionId of record.affectedActionIds) {
    if (!actions.has(actionId))
      fail(
        'STATE_TRANSITION_INVALID',
        'Assumption affected action must resolve to durable history.',
        { assumptionId: record.assumptionId, actionId },
      );
  }
}
...
function assertDecisionRevision(record, snapshot, evidence, assumptions, decisions) {
  ...
  assertEvidence(record.evidenceRefIds, evidence, {
    subject: 'Decision revision',
    sourceArtifactId: record.sourceArtifactId,
    required: true,
  });
  if (record.predecessorDecisionId === null) {
    fail('STATE_TRANSITION_INVALID', 'A decision revision must name its immutable predecessor.', {
  ...
  if (!predecessor || record.revision !== predecessor.revision + 1) {
    fail(
      'STATE_TRANSITION_INVALID',
      'A decision revision must increment exactly one existing predecessor revision.',
      {
        decisionId: record.decisionId,
  ...
  if (sha256Jcs(record.historyDecisionIds) !== sha256Jcs(expectedHistory)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Decision revision history must retain every predecessor identity in canonical order.',
      {
        decisionId: record.decisionId,
      },
    );
  ...
  for (const assumptionId of record.assumptionIds) {
    if (!assumptions.has(assumptionId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Decision revision assumptions must resolve to immutable operating history.',
        {
          decisionId: record.decisionId,
          assumptionId,
        },
...
export function buildOperatingIntelligenceStateTransitionV2({
  ...
  for (const key of Object.keys(arguments[0] ?? {})) {
    if (!allowed.has(key))
      fail('RESULT_CONTRACT_INVALID', 'Unsupported intelligence-state transition field.', {
        field: key,
      });
  }
  ...
  const assumptionsById = new Map(
    checked.state.assumptions.map((record) => [record.assumptionId, record]),
  );
  const decisions = new Map(
    [...checked.state.decisions, ...existingDecisions].map((record) => [record.decisionId, record]),
  );
  const actions = new Map(
    [...checked.state.actions, ...existingActions].map((record) => [record.actionId, record]),
  );
  const risksByRevision = revisionIndex(
    [...checked.state.risks, ...existingRisks],
    'riskId',
    'Risk',
  );
  const assumptionsByRevision = revisionIndex(
    [...checked.state.assumptions, ...existingAssumptions],
    'assumptionId',
    'Assumption',
  );
  const normalized = {
  ...
  for (const record of normalized.claims)
    assertClaim(record, checked.snapshot, evidence, assumptionsById, producers);
  for (const record of normalized.metricObservations)
    assertObservation(record, checked.snapshot, evidence, metrics);
  ...
  for (const record of normalized.decisionRevisions)
    assertDecisionRevision(record, checked.snapshot, evidence, assumptionsById, decisions);
  return freeze({
