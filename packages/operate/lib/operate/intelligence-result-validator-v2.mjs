  ...
  for (const [position, value] of values.entries()) {
    ...
    if (value?.[field] !== expected) {
      fail(
        ...
        `${subject} identities must be the exact ordered Assignment-derived sequence.`,
        {
          assignmentId,
          expected,
...
function evidenceSet(assignment, advisorOutputs = [], challengerOutput = null) {
  ...
  return new Set([...issued.map(({ evidenceRefId }) => evidenceRefId), ...inherited]);
}
...
function assertEvidenceIds(ids, allowed, subject) {
  for (const evidenceRefId of ids) {
    if (!allowed.has(evidenceRefId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        `${subject} cites Evidence that was not issued to this Assignment.`,
        {
          evidenceRefId,
        },
      );
...
function assertIdentityBinding(value, assignment, plan, snapshot) {
  if (
    ...
    fail(
      ...
      'Intelligence result must retain its exact Assignment, role, profile, plan, snapshot, and scope.',
      {
  ...
  if (
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'Intelligence result must account for the exact issued typed input absences.',
      {
        assignmentId: assignment.assignmentId,
      },
    );
...
const TARGET_FIELDS = Object.freeze({
  ...
  'question-coverage': Object.freeze({
    challenger: ['questionCoverage'],
    chair: ['questionCoverage'],
  }),
});
...
function resultRequirementCount(value, assignment, requirement) {
  ...
  if (!fields) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Assignment result requirement does not map to its role output contract.',
      {
        assignmentId: assignment.assignmentId,
        requirementId: requirement.requirementId,
...
function assertResultRequirements(value, assignment) {
  ...
  for (const requirement of assignment.resultRequirements ?? []) {
    if (
      !requirement.appliesToOutcomes.includes('always') &&
      requirement.appliesToOutcomes.length > 0 &&
      !requirement.appliesToOutcomes.includes(outcome)
    )
      continue;
    const count = resultRequirementCount(value, assignment, requirement);
    if (
      count < requirement.minimumItems ||
      (requirement.maximumItems !== null && count > requirement.maximumItems)
    ) {
      fail(
        'RESULT_CONTRACT_INVALID',
        'Intelligence result does not satisfy its registry-owned result requirement.',
        {
          assignmentId: assignment.assignmentId,
          requirementId: requirement.requirementId,
          target: requirement.target,
...
function approximately(actual, expected) {
  return (
    Math.abs(actual - expected) <= Math.max(1e-6, Math.abs(expected) * NUMERIC_RELATIVE_TOLERANCE)
  );
}
...
function assertMeasurementCoherence(measurement) {
  if (measurement.kind === 'runway') {
    ...
    if (!approximately(measurement.runwayPeriods, expected)) {
      measurementFailure(
        measurement,
        'Runway periods must equal cash balance divided by net burn.',
        { expected },
      );
    }
    return;
  ...
  if (measurement.kind === 'margin') {
    ...
    if (
      !approximately(measurement.marginAmount, expectedAmount) ||
      !approximately(measurement.marginRate, expectedRate)
    ) {
      measurementFailure(
        measurement,
        'Margin amount and rate must agree with revenue and cost; zero revenue uses rate 0.',
        {
          expectedAmount,
          expectedRate,
        },
  ...
  if (measurement.kind === 'funnel') {
    for (const stage of measurement.stages) {
      ...
      if (stage.advanced > stage.entered || !approximately(stage.conversionRate, expected)) {
        measurementFailure(
          measurement,
          'Funnel advanced volume cannot exceed entered volume and conversion must equal advanced divided by entered; zero entered uses rate 0.',
          {
            stage: stage.stage,
            expected,
          },
  ...
  if (measurement.kind === 'channel-economics') {
    if (measurement.acquiredCustomers === 0) {
      if (measurement.customerAcquisitionCost !== null) {
        measurementFailure(
          measurement,
          'Channel CAC must be null when no customers were acquired.',
        );
      }
    } else {
      const expected = measurement.spend / measurement.acquiredCustomers;
      if (
        measurement.customerAcquisitionCost === null ||
        !approximately(measurement.customerAcquisitionCost, expected)
      ) {
        measurementFailure(
          measurement,
          'Channel CAC must equal spend divided by acquired customers.',
          { expected },
        );
      }
    }
  ...
  if (measurement.kind === 'capacity') {
    const expected =
      measurement.available === 0 ? 0 : measurement.committed / measurement.available;
    if (
      (measurement.available === 0 && measurement.committed !== 0) ||
      !approximately(measurement.utilizationRate, expected)
    ) {
      measurementFailure(
        measurement,
        'Capacity utilization must equal committed divided by available; zero available requires zero committed and utilization.',
        { expected },
      );
    }
    return;
  ...
  if (measurement.kind === 'throughput') {
    if (
      (measurement.leadTimeP50 === null) !== (measurement.leadTimeP95 === null) ||
      (measurement.leadTimeP50 !== null && measurement.leadTimeP95 < measurement.leadTimeP50)
    ) {
      measurementFailure(
        measurement,
        'Throughput lead-time percentiles must be jointly absent or satisfy p95 >= p50.',
      );
    }
    return;
  }
  if (measurement.kind === 'exposure') {
    ...
    if (!approximately(measurement.exposureScore, expected)) {
      measurementFailure(
        measurement,
        'Exposure score must equal likelihood multiplied by impact score.',
        { expected },
      );
    }
  }
...
function assertAdvisorAnalysis(
  value,
  assignment,
  { allowedEvidence, allowedAbsences, claimIds, measurementIds, riskIds, alternativeIds },
) {
  if (value.analysis.profileId !== assignment.analysisProfile.id) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Advisor analysis must use the exact registry-owned analysis profile.',
      {
        assignmentId: assignment.assignmentId,
  ...
  for (const answer of value.analysis.executiveQuestionAnswers) {
    ...
    if (
      ...
      fail(
        'RESULT_CONTRACT_INVALID',
        'Each required executive question answer must map to a claim, measurement, risk, issued EvidenceRef, or typed absence.',
        {
          questionId: answer.questionId,
        },
      );
  ...
  if (
    Array.isArray(assignment.analysisProfile.questionIds) &&
    !exactArray(
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'Advisor executive question answers must cover the exact ordered registry-owned question identities.',
      {
        assignmentId: assignment.assignmentId,
      },
    );
  ...
  const linked = [
    ...value.analysis.executiveQuestionAnswers.map((entry) => ({
      entry,
      subject: 'Advisor executive question answer',
    })),
...
function assertAdvisor(value, assignment, { allowedEvidence, knownAssumptions }) {
  ...
  if (
    ...
    value.recommendation.localRecommendationId !==
      deriveOperatingRoleLocalRecommendationIdV2(assignment.assignmentId)
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Advisor recommendation identity must be derived from its Assignment.',
      {
        assignmentId: assignment.assignmentId,
      },
    );
  ...
  const measurementIds = new Set(
    value.measurements.map(({ localMeasurementId }) => localMeasurementId),
  );
  const riskIds = new Set(value.risks.map(({ localRiskId }) => localRiskId));
  const alternativeIds = new Set(
    value.alternatives.map(({ localAlternativeId }) => localAlternativeId),
  );
  ...
  for (const claim of value.claims) {
    if (
      claim.supportingEvidenceRefIds.some((id) => claim.contradictingEvidenceRefIds.includes(id))
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Advisor claim support and contradiction Evidence must be disjoint.',
        {
    ...
    for (const assumptionId of claim.assumptionIds) {
      if (!knownAssumptions.has(assumptionId)) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Advisor claim references an Assumption outside the immutable operating state.',
          {
            localClaimId: claim.localClaimId,
            assumptionId,
          },
  ...
  for (const risk of value.risks) {
    if (risk.claimIds.some((id) => !claimIds.has(id))) {
      fail('STATE_TRANSITION_INVALID', 'Advisor risk references a foreign claim.', {
  ...
  for (const gap of value.gaps) {
    if (
      !absenceSet(assignment).has(gap.inputAbsenceId) ||
      !(assignment.evidenceRequirements ?? []).some(
        ({ requirementId }) => requirementId === gap.requirementId,
      )
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Advisor gap must name an issued absence and registry-owned evidence requirement.',
        {
  ...
  if (value.recommendation) {
    ...
    if (
      recommendation.rationaleClaimIds.some((id) => !claimIds.has(id)) ||
      ...
      fail(
        'STATE_TRANSITION_INVALID',
        'Advisor recommendation contains a foreign claim, alternative, risk, or measurement reference.',
        {
...
function advisorSourceIndex(advisorOutputs) {
  ...
  for (const entry of advisorOutputs) {
    if (!entry || typeof entry.artifactId !== 'string' || byArtifact.has(entry.artifactId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Advisor result custody must contain unique exact Artifact identities.',
      );
    }
...
function assertAdvisorTarget(target, advisors, subject) {
  ...
  if (!output)
    fail('STATE_TRANSITION_INVALID', `${subject} names a foreign Advisor Artifact.`, target);
  const analysis = new Set(
    profileInsights(output.analysis, output.analysis.profileId).map(
      ({ localAnalysisId }) => localAnalysisId,
    ),
  );
  const claims = new Set(output.claims.map(({ localClaimId }) => localClaimId));
  const measurements = new Set(
    output.measurements.map(({ localMeasurementId }) => localMeasurementId),
  );
  const risks = new Set(output.risks.map(({ localRiskId }) => localRiskId));
  const recommendations = new Set(
    output.recommendation ? [output.recommendation.localRecommendationId] : [],
  );
  if (
    (target.analysisIds ?? []).some((id) => !analysis.has(id)) ||
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} names a foreign Advisor analysis, claim, measurement, risk, or recommendation.`,
      target,
    );
  }
}
...
function assertExactQuestionCoverage(value, assignment, subject) {
  if (
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      `${subject} question coverage must preserve the exact ordered registry-owned question identities.`,
      {
        assignmentId: assignment.assignmentId,
      },
    );
...
function assertChallengerQuestionCoverage(
  value,
  assignment,
  { findingIds, alternativeIds, dissentIds, gapIds },
) {
  assertExactQuestionCoverage(value, assignment, 'Challenger');
  for (const entry of value.questionCoverage) {
    assertLocalReferenceArray(
      entry.findingIds,
      findingIds,
      'Challenger question coverage',
      'Finding',
    );
    assertLocalReferenceArray(
...
function assertChairQuestionCoverage(
  value,
  assignment,
  { decisionIds, findingIds, dissentIds, absenceIds },
) {
...
function assertChallenger(value, assignment, advisorOutputs, { allowedEvidence }) {
  ...
  if (!exactArray(value.advisorArtifactIds, [...advisors.keys()])) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Challenger must name the exact ordered validated Advisor Artifact custody.',
      {
        assignmentId: assignment.assignmentId,
      },
    );
  ...
  if (!exactArray(value.reviewedClaims, expectedClaims)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Challenger must account exactly once for every Advisor claim.',
      {
        assignmentId: assignment.assignmentId,
      },
    );
  ...
  const alternativeIds = new Set(
    value.missingAlternatives.map(({ localAlternativeId }) => localAlternativeId),
  );
  ...
  for (const finding of value.findings) {
    for (const target of finding.targets)
      assertAdvisorTarget(target, advisors, 'Challenger Finding');
    if (
      finding.type === 'correlated-reasoning' &&
      new Set(finding.targets.map(({ advisorArtifactId }) => advisorArtifactId)).size < 2
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'A correlated-reasoning Finding must span two distinct Advisor Artifacts.',
        {
    ...
    if (
      finding.supportingEvidenceRefIds.some((id) =>
        finding.contradictingEvidenceRefIds.includes(id),
      )
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Challenger Finding support and contradiction Evidence must be disjoint.',
        {
          localFindingId: finding.localFindingId,
        },
      );
  ...
  for (const alternative of value.missingAlternatives) {
    for (const target of alternative.targets)
      assertAdvisorTarget(target, advisors, 'Challenger alternative');
  }
  ...
  for (const gap of value.gaps) {
    const absence = assignment.inputAbsences.find(
      ({ absenceId }) => absenceId === gap.inputAbsenceId,
    );
    const exactBinding =
      absence?.kind === gap.kind &&
      (gap.kind === 'evidence'
    ...
    if (!exactBinding) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Challenger gap references an absence that was not issued.',
        {
          localGapId: gap.localGapId,
        },
      );
...
function assertChair(
  value,
  assignment,
  advisorOutputs,
  challengerOutput,
  { allowedEvidence, knownAssumptions, knownObjectiveIds, knownMetricIds, decisionOwnerActorId },
) {
  const advisors = advisorSourceIndex(advisorOutputs);
  if (!exactArray(value.advisorArtifactIds, [...advisors.keys()])) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair must name the exact ordered validated Advisor Artifact custody.',
    );
  }
  if (value.challengerArtifactId !== (challengerOutput?.artifactId ?? null)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair must name the exact Challenger Artifact or explicit null absence.',
    );
  }
  localIds(
    assignment.assignmentId,
  ...
  if (actionHypotheses.length > MAX_OPERATING_ACTION_HYPOTHESES_PER_LEDGER_V2) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Chair Action hypotheses exceed the bounded owner Review capacity.',
      {
        assignmentId: assignment.assignmentId,
  ...
  localIds(
    assignment.assignmentId,
    actionHypotheses,
    'localActionHypothesisId',
    'action-hypothesis',
    'Chair action hypotheses',
  );
  ...
  for (const [artifactId, output] of advisors) {
    ...
    if (output.recommendation)
      recommendations.set(
        `${artifactId}:${output.recommendation.localRecommendationId}`,
        output.recommendation,
      );
    for (const alternative of output.alternatives)
      alternatives.set(`${artifactId}:${alternative.localAlternativeId}`, alternative);
  }
  const challengerArtifactId = challengerOutput?.artifactId ?? null;
  const findings = new Map(
    (challengerOutput?.output.findings ?? []).map((finding) => [finding.localFindingId, finding]),
  );
  const dissent = new Map(
    (challengerOutput?.output.dissent ?? []).map((entry) => [entry.localDissentId, entry]),
  );
  ...
  const actionIds = new Set(
    actionHypotheses.map(({ localActionHypothesisId }) => localActionHypothesisId),
  );
  ...
  for (const decision of value.decisions) {
    if (decision.ownerActorId !== decisionOwnerActorId) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair Decision owner must equal the immutable plan owner.',
        {
    ...
    const decisionClaimRefs = new Set(
      decision.sourceClaimRefs.map((ref) => `${ref.advisorArtifactId}:${ref.localClaimId}`),
    );
    ...
    for (const ref of decision.sourceClaimRefs) {
      ...
      if (!claim)
        fail('STATE_TRANSITION_INVALID', 'Chair Decision references a foreign Advisor claim.', ref);
      for (const id of [...claim.supportingEvidenceRefIds, ...claim.contradictingEvidenceRefIds])
        relevantEvidence.add(id);
    }
    for (const ref of decision.sourceRecommendationRefs) {
      const recommendation = recommendations.get(
        `${ref.advisorArtifactId}:${ref.localRecommendationId}`,
      );
      if (!recommendation)
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Decision references a foreign Advisor recommendation.',
          ref,
        );
    ...
    for (const findingId of decision.challengerFindingIds) {
      ...
      if (!finding)
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Decision references a foreign Challenger Finding.',
          { findingId },
        );
      for (const id of [
    ...
    for (const dissentId of decision.dissentIds) {
      ...
      if (!item)
        fail('STATE_TRANSITION_INVALID', 'Chair Decision references foreign dissent.', {
          dissentId,
        });
      for (const id of item.evidenceRefIds) relevantEvidence.add(id);
    }
    for (const alternative of decision.alternativeDispositions) {
      const source = alternatives.get(
        `${alternative.sourceArtifactId}:${alternative.localAlternativeId}`,
      );
      if (!source || source.title !== alternative.title) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Decision references a foreign or substituted alternative.',
          alternative,
        );
      }
    ...
    if (
      new Set(
        decision.alternativeDispositions.map(
          ({ sourceArtifactId, localAlternativeId }) => `${sourceArtifactId}:${localAlternativeId}`,
        ),
      ).size !== decision.alternativeDispositions.length
    ) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair Decision alternative dispositions must identify unique relevant source alternatives.',
        {
          localDecisionId: decision.localDecisionId,
        },
      );
    ...
    if (decision.evidenceRefIds.some((id) => !relevantEvidence.has(id))) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair Decision may cite only Evidence relevant to its exact source references.',
        {
          localDecisionId: decision.localDecisionId,
        },
      );
    ...
    if (decision.assumptionIds.some((id) => !knownAssumptions.has(id))) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair Decision references an Assumption omitted from the issued bounded operating-state projection.',
        {
          localDecisionId: decision.localDecisionId,
        },
      );
    ...
    for (const action of decision.actionHypotheses) {
      const actionClaimRefs = action.sourceClaimRefs.map(
        (ref) => `${ref.advisorArtifactId}:${ref.localClaimId}`,
      );
      const hasOwner = typeof action.ownerActorId === 'string' && action.ownerActorId.length > 0;
      if (
        new Set(actionClaimRefs).size !== actionClaimRefs.length ||
        actionClaimRefs.some((ref) => !decisionClaimRefs.has(ref)) ||
        action.sourceFindingIds.some((id) => !decisionFindingIds.has(id)) ||
        action.dependsOnActionHypothesisIds.some(
          (id) => !actionIds.has(id) || id === action.localActionHypothesisId,
        )
      ) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Action hypothesis references a claim, Finding, or dependency outside its accepted Decision ledger custody.',
          {
      ...
      if (!knownObjectiveIds.has(action.objectiveId) || !knownMetricIds.has(action.metricId)) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Action hypothesis must reference an Objective and Metric issued in the bounded operating-state projection.',
          {
            localActionHypothesisId: action.localActionHypothesisId,
      ...
      if (
        (hasOwner && action.accountabilityDisposition !== null) ||
        (!hasOwner && !['unowned', 'blocked'].includes(action.accountabilityDisposition))
      ) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair Action hypothesis requires one owner or an explicit unowned/blocking disposition.',
          {
            localActionHypothesisId: action.localActionHypothesisId,
          },
        );
  ...
  const expectedSources = [
    ...[...advisors.entries()].map(([artifactId, output]) =>
      output.recommendation
  ...
  const actualSources = value.sourceDispositions.map(
  ...
  if (!exactArray(actualSources, expectedSources)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair sourceDispositions must account in canonical source order exactly once for every Advisor outcome/recommendation, Finding, and dissent item.',
    );
  }
  for (const disposition of value.sourceDispositions) {
    if (disposition.sourceKind === 'advisor-outcome') {
      ...
      if (
        ...
        fail(
          'STATE_TRANSITION_INVALID',
          'Advisor outcome disposition must preserve the exact quiet or partial source outcome.',
          disposition,
        );
      }
    }
    if (disposition.localDecisionId !== null && !decisionIds.has(disposition.localDecisionId)) {
      fail(
        'STATE_TRANSITION_INVALID',
        'Chair source disposition references a foreign Decision.',
        disposition,
      );
    }
    if (disposition.localDecisionId !== null) {
      const decision = value.decisions.find(
        ({ localDecisionId }) => localDecisionId === disposition.localDecisionId,
      );
      const relevant =
        disposition.sourceKind === 'advisor-recommendation'
          ? decision.sourceRecommendationRefs.some(
              (ref) =>
      ...
      if (!relevant) {
        fail(
          'STATE_TRANSITION_INVALID',
          'Chair source disposition may bind only to a Decision that cites the exact source item.',
          disposition,
        );
      }
    }
  ...
  if (sha256Jcs(value.dissent) !== sha256Jcs(expectedDissent)) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair ledger must preserve exact structured Challenger dissent.',
    );
  }
  ...
  if (
    sha256Jcs(value.advisorAbsenceGaps) !== sha256Jcs(roleAbsences) ||
    sha256Jcs(value.evidenceGaps) !== sha256Jcs(evidenceAbsences)
  ) {
    fail(
      'STATE_TRANSITION_INVALID',
      'Chair ledger must preserve exact runtime-issued role and Evidence gaps.',
    );
  }
  return freeze(clone(value));
}
...
export function validateOperatingIntelligenceResultV2({
  ...
  try {
    ...
    fail(
      'RESULT_CONTRACT_INVALID',
      'Intelligence result does not satisfy its advertised closed result contract.',
      {
  ...
  if (
    ...
    fail(
      'STATE_TRANSITION_INVALID',
      'Intelligence result validation requires its exact durable plan, snapshot, operating state, and issued input bundle.',
      {
        assignmentId: assignment?.assignmentId ?? null,
      },
    );
  ...
  try {
    assertProtocolArtifact('operating-intelligence-input-bundle', inputBundle, {
      protocolVersion: PROTOCOL_VERSION,
    });
  } catch (cause) {
    fail(
      'RESULT_CONTRACT_INVALID',
      'Intelligence result validation requires a contract-valid issued input bundle.',
      {
        assignmentId: assignment.assignmentId,
        cause: cause?.code ?? null,
      },
  ...
  const assignmentBundleAbsences = assignment.inputAbsences.filter(({ absenceId }) =>
    bundleAbsenceIds.has(absenceId),
  );
  ...
  if (
    inputBundle.bundleId !== assignment.intelligenceContext.inputBundle.bundleId ||
    ...
    assignment.intelligenceContext.inputBundle.sourceArtifactIds.some(
      (artifactId, position) => inputBundle.sourceArtifactIds[position] !== artifactId,
    ) ||
    sha256Jcs(roleBinding.issuedEvidence) !==
      sha256Jcs(assignment.intelligenceContext.inputBundle.issuedEvidence) ||
    sha256Jcs(roleBinding.evidenceAbsences) !== sha256Jcs(assignmentBundleAbsences)
  ) {
    fail(
      'OPERATING_SCOPE_INVALID',
      'Intelligence result input bundle must retain the exact Assignment role, requirements, Evidence, Snapshot, and scope binding.',
      {
        assignmentId: assignment.assignmentId,
  ...
  const context = {
    ...
    knownAssumptions: new Set(
      inputBundle.operatingState.assumptions.map(({ recordId }) => recordId),
    ),
    knownObjectiveIds: new Set(
      inputBundle.operatingState.objectives.map(({ recordId }) => recordId),
    ),
  ...
  if (schemaId === 'operating-challenger-review')
    return assertChallenger(value, assignment, advisorOutputs, context);
...
export function preflightOperatingIntelligenceResultV2(input = {}) {
  ...
  if (
    ...
    let schemaErrors;
    try {
      schemaErrors = validateProtocolArtifact(schemaId, input.value, {
        protocolVersion: PROTOCOL_VERSION,
      });
    } catch (error) {
      schemaErrors = [
