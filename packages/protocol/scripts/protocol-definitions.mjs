...
const nullableString = (schema = {}) => ({
  anyOf: [{ type: 'string', ...schema }, { type: 'null' }],
});
...
export function buildSchemas() {
  ...
  const legacyAlias = closed(['id', 'status', 'since', 'removalVersion'], {
    id: ref('identifier'),
    status: { const: 'compatibility' },
    since: ref('semver'),
    removalVersion: nullableRef('semver'),
  });
  ...
  const adapterMapping = closed(['host', 'nativeId', 'generatedPath'], {
    host: { enum: ['claude-code', 'codex', 'cursor'] },
    nativeId: ref('identifier'),
    generatedPath: ref('relativePath'),
  });
  const role = closed(
    [
      'roleId',
    ...
    {
      roleId: ref('roleId'),
      roleVersion: ref('semver'),
      displayName: ref('nonBlankText'),
      ...
      authorityClass: ref('identifier'),
      writeBoundary,
      source: sourceRef,
      taskKinds: array(ref('identifier'), 0, 32, true),
      outputContracts: array(contractRef, 1, 32),
      ruleIds: array(ref('ruleId'), 1, 10, true),
      adapterMappings: array(adapterMapping, 1, 8),
  ...
  const binding = closed(
    ...
    {
      taskKind: ref('identifier'),
      roleId: ref('roleId'),
      assignmentLevel: { enum: ['story', 'preflight', 'feature', 'quality'] },
      countsTowardR2: { type: 'boolean' },
      implementationAuthority: { type: 'boolean' },
      description: ref('nonBlankText'),
      inputContracts: array(contractRef, 0, 32),
      outputContracts: array(contractRef, 1, 32),
      requiredRuleIds: array(ref('ruleId'), 1, 10, true),
    },
  );
  const legacyMapping = closed(
    ['legacyType', 'legacyAgent', 'taskKind', 'evidence', 'precedence'],
    {
      legacyType: { enum: ['UI', 'Tech'] },
      legacyAgent: nullableString({ pattern: '^[a-z][a-z0-9-]+-agent$' }),
      taskKind: ref('identifier'),
      evidence: { enum: ['explicit-type-and-agent', 'explicit-type-default'] },
  ...
  const taskKindRegistry = documentSchema(
    ...
    {
      roleRegistryDigest: ref('digest'),
      bindings: array(binding, 1, 64),
      legacyMappings: array(legacyMapping, 1, 32),
    },
  );

  const taskManifest = documentSchema(
    'task-manifest',
    'task-manifest',
    '1.0.0',
    [
    ...
    ],
    {
      ...
      parent: closed(
        ['projectMode', 'storyId', 'specId', 'featureSlug'],
        {
          projectMode: { enum: ['spec-driven', 'default'] },
          storyId: ref('storyId'),
          specId: nullableRef('specId'),
        ...
        },
        {
          oneOf: [
            {
              properties: {
                projectMode: { const: 'spec-driven' },
                specId: ref('specId'),
                featureSlug: { type: 'null' },
              },
            },
            {
              properties: {
                projectMode: { const: 'default' },
                specId: { type: 'null' },
                featureSlug: { type: 'string', minLength: 1 },
              },
            },
          ],
        },
      ...
      routing: closed(
        ['taskKind', 'roleId', 'roleVersion', 'taskKindRegistryDigest', 'roleRegistryDigest'],
        {
          taskKind: ref('identifier'),
          roleId: ref('roleId'),
          roleVersion: ref('semver'),
          taskKindRegistryDigest: ref('digest'),
          roleRegistryDigest: ref('digest'),
        },
      ),
      dependencies: array(
        closed(['taskId', 'taskManifestDigest'], {
          taskId: ref('taskId'),
          taskManifestDigest: ref('digest'),
        }),
        0,
        256,
      ),
      scope: closed(['create', 'modify', 'preserve'], {
        create: array(repositoryPath, 0, 1024),
        modify: array(repositoryPath, 0, 1024),
        preserve: array(repositoryPath, 0, 1024),
      }),
      inputs: array(
        closed(['artifactId', 'contractRef', 'path', 'digest', 'required'], {
          artifactId: ref('identifier'),
          contractRef,
          path: ref('relativePath'),
          digest: ref('digest'),
      ...
      expectedOutputs: array(
        closed(['outputId', 'contractRef', 'paths', 'pathArguments'], {
          outputId: ref('identifier'),
          contractRef,
          paths: array(repositoryPath, 0, 128),
          pathArguments: {
            type: 'object',
      ...
      acceptanceRefs: array(
        closed(['kind', 'reference', 'statement'], {
          kind: { enum: ['gherkin-scenario', 'verification-statement'] },
          reference: ref('relativePath'),
          statement: ref('nonBlankText'),
        }),
        1,
        256,
      ),
      ...
      reviewBinding: closed(['planDigest', 'planningReviewReceiptDigest', 'ownerDecisionDigest'], {
        ...
        planningReviewReceiptDigest: ref('digest'),
        ownerDecisionDigest: ref('digest'),
      }),
      producer: ref('producer'),
      issuedAt: ref('dateTime'),
    },
  );

  const digestChange = closed(
    ['repositoryKey', 'path', 'operation', 'beforeDigest', 'afterDigest', 'byteLength'],
    {
      repositoryKey: ref('repositoryKey'),
      path: ref('relativePath'),
      operation: { enum: ['create', 'modify', 'delete'] },
      beforeDigest: nullableRef('digest'),
      afterDigest: nullableRef('digest'),
      byteLength: { type: 'integer', minimum: 0 },
    },
  );
  const taskOutput = documentSchema(
    ...
    ],
    {
      ...
      attempt: closed(['initialAttempt', 'correctionIteration'], {
        initialAttempt: { const: 1 },
        correctionIteration: { type: 'integer', minimum: 0 },
      }),
      outcome: { enum: ['completed', 'blocked'] },
      startedAt: ref('dateTime'),
      completedAt: ref('dateTime'),
      changes: array(digestChange, 0, 4096),
      preserveVerification: array(
        closed(['repositoryKey', 'path', 'beforeDigest', 'afterDigest', 'unchanged'], {
          repositoryKey: ref('repositoryKey'),
          path: ref('relativePath'),
          beforeDigest: ref('digest'),
          afterDigest: ref('digest'),
      ...
      commandEvidence: array(
        closed(
          ...
          {
            commandId: ref('identifier'),
            argvDigest: ref('digest'),
            workingDirectory: repositoryPath,
            startedAt: ref('dateTime'),
            completedAt: ref('dateTime'),
            exitCode: { anyOf: [{ type: 'integer', minimum: 0, maximum: 255 }, { type: 'null' }] },
            stdoutDigest: ref('digest'),
            stderrDigest: ref('digest'),
            status: { enum: ['passed', 'failed', 'skipped-not-configured'] },
          },
        ),
        0,
        128,
      ...
      acceptanceEvidence: array(
        closed(['acceptanceRef', 'verifier', 'result', 'evidenceDigest'], {
          acceptanceRef: ref('nonBlankText'),
          verifier: ref('identifier'),
      ...
      outputs: array(
        closed(['outputId', 'path', 'mediaType', 'byteLength', 'digest'], {
          outputId: ref('identifier'),
          path: repositoryPath,
      ...
      errorHandoff: {
        anyOf: [
          closed(['path', 'digest'], { path: repositoryPath, digest: ref('digest') }),
          { type: 'null' },
        ],
      },
      externalEffects: array(
        closed(['effectClass', 'description'], {
          effectClass: ref('identifier'),
          description: ref('nonBlankText'),
        }),
        0,
        0,
      ...
      allOf: [
        {
          ...
          then: {
            ...
            not: {
              properties: {
                diagnostics: { contains: { properties: { severity: { const: 'error' } } } },
              },
            },
          },
        },
        ...
        {
          if: { properties: { roleBinding: { properties: { roleId: { const: 'planr-qa' } } } } },
  ...
  const implementationResult = {
    ...
    ...closed(['outcome', 'task', 'changed', 'checks', 'issues'], {
      ...
      changed: array(
        closed(['path', 'purpose'], {
          path: ref('relativePath'),
          purpose: ref('nonBlankText'),
        }),
        0,
        4096,
      ),
      checks: array(
        closed(['command', 'status', 'detail'], {
          command: ref('nonBlankText'),
          status: { enum: ['passed', 'failed', 'not-run'] },
          detail: ref('nonBlankText'),
        }),
        0,
        256,
      ),
      issues: array(
        closed(['problem', 'impact', 'nextAction'], {
          problem: ref('nonBlankText'),
          impact: ref('nonBlankText'),
          nextAction: ref('nonBlankText'),
        }),
        0,
        256,
      ),
  ...
  const ruleCatalog = documentSchema('rule-catalog', 'rule-catalog', '1.0.0', ['rules'], {
    rules: array(
      closed(
        ...
        {
          ruleId: ref('ruleId'),
          ruleVersion: ref('semver'),
          title: ref('nonBlankText'),
          status: { enum: ['active', 'deprecated', 'retired'] },
          normativeText: ref('nonBlankText'),
          enforcementPoints: array(
            closed(['package', 'path', 'symbol'], {
              package: ref('nonBlankText'),
              path: ref('relativePath'),
              symbol: ref('nonBlankText'),
            }),
            1,
            64,
          ),
          testRefs: array(ref('relativePath'), 1, 64, true),
          citedBy: array(ref('relativePath'), 1, 256, true),
          source: sourceRef,
        },
      ),
      1,
  ...
  const commandCatalog = documentSchema(
    'command-catalog',
    'command-catalog',
    '1.0.0',
    ['binaries', 'inventory', 'commands', 'negativeContracts'],
    {
      binaries: array(
        closed(['id', 'lifecycle', 'aliasOf'], {
          id: ref('identifier'),
          lifecycle: { enum: ['canonical', 'exact-alias', 'deprecated-exact-alias'] },
          aliasOf: nullableString({ pattern: '^[a-z][a-z0-9-]*$' }),
        }),
        3,
        3,
      ),
      inventory: closed(
        [
          'rootCommandModules',
          'rootCommandSlugs',
          'pipelineMachineLeaves',
          'pipelineMachineGrammar',
          'frozenClaudeDocuments',
          'frozenClaudeSlugs',
        ],
        {
          rootCommandModules: { const: 39 },
          rootCommandSlugs: array(ref('identifier'), 39, 39, true),
          pipelineMachineLeaves: { const: 31 },
          pipelineMachineGrammar: array(
            array({ type: 'string', minLength: 1, maxLength: 128 }, 1, 8),
            31,
            31,
          ),
          frozenClaudeDocuments: { const: 8 },
          frozenClaudeSlugs: array(ref('identifier'), 8, 8, true),
        },
      ),
      commands: array(
        closed(
          ...
          {
            commandId: ref('identifier'),
            surface: { enum: ['cli-root', 'cli-leaf', 'pipeline-machine', 'frozen-host-alias'] },
            argv: array({ type: 'string', minLength: 1, maxLength: 128 }, 1, 16),
            lifecycle: { enum: ['active', 'compatibility-alias', 'deprecated'] },
            ownerPackage: ref('nonBlankText'),
            source: sourceRef,
            authorityClass: ref('identifier'),
            machineJson: { type: 'boolean' },
            outputContracts: array(contractRef, 0, 32),
            skillId: nullableRef('skillId'),
            aliases: array(array({ type: 'string', minLength: 1 }, 1, 16), 0, 32),
            testRefs: array(ref('relativePath'), 1, 64, true),
          },
        ),
        1,
      ...
      negativeContracts: array(
        closed(
          ['negativeContractId', 'forbiddenArgvPrefix', 'forbiddenPaths', 'reason', 'testRefs'],
          {
            negativeContractId: ref('identifier'),
            forbiddenArgvPrefix: array({ type: 'string', minLength: 1 }, 1, 16),
            forbiddenPaths: array(ref('relativePath'), 0, 64, true),
            reason: ref('nonBlankText'),
            testRefs: array(ref('relativePath'), 1, 64, true),
          },
        ),
        1,
  ...
  const skillCatalog = documentSchema(
    ...
    'skill-catalog',
    '1.0.0',
    ['matchingPolicyVersion', 'skills', 'compatibilityAliases'],
    {
        ...
        closed(
          ...
          ],
          {
            ...
            triggerPolicy: closed(['include', 'exclude', 'deferTo'], {
              include: array(ref('nonBlankText'), 1, 64, true),
              exclude: array(ref('nonBlankText'), 1, 64, true),
              deferTo: array(ref('skillId'), 0, 32, true),
            }),
            contracts: closed(['inputs', 'outputs'], {
              inputs: array(contractRef, 0, 32),
              outputs: array(contractRef, 1, 32),
            }),
            cliRequirements: array(
              closed(['commandId', 'argv', 'requiredOptions'], {
                commandId: ref('identifier'),
                argv: array({ type: 'string', minLength: 1 }, 1, 16),
                requiredOptions: array({ type: 'string', pattern: '^--[a-z0-9-]+$' }, 0, 32, true),
              }),
              0,
              64,
            ),
            ruleIds: array(ref('ruleId'), 1, 10, true),
            contributionManifestRefs: array(ref('relativePath'), 1, 64, true),
            hosts: array(
              closed(['host', 'entrypoint', 'path'], {
                host: { enum: ['claude-code', 'codex', 'cursor'] },
                entrypoint: ref('nonBlankText'),
                path: ref('relativePath'),
              }),
              1,
              8,
            ...
            testRefs: array(ref('relativePath'), 1, 64, true),
            certificationRefs: array(ref('relativePath'), 1, 64, true),
          },
        ),
        1,
        256,
      ...
      compatibilityAliases: array(
        closed(['aliasId', 'canonicalSkillId', 'lifecycle', 'generatedPaths'], {
          aliasId: ref('identifier'),
          canonicalSkillId: ref('skillId'),
          lifecycle: { const: 'deprecated' },
          generatedPaths: array(ref('relativePath'), 1, 16, true),
        }),
        0,
        64,
      ),
    ...
    outputs: array(
      closed(
        ...
        ],
        {
          outputId: ref('identifier'),
          title: ref('nonBlankText'),
          outputClass: { enum: ['A', 'B', 'C', 'D'] },
          ownerPackage: ref('nonBlankText'),
          pathFunctionId: ref('identifier'),
          pathTemplate: ref('relativePath'),
          sourceOfTruth: { type: 'boolean' },
          schemaRef: { anyOf: [contractRef, { type: 'null' }] },
          manifestRef: { anyOf: [contractRef, { type: 'null' }] },
          generator: closed(['id', 'version', 'sourceDigest'], {
            id: ref('identifier'),
            version: ref('semver'),
            sourceDigest: ref('digest'),
          }),
          validator: closed(['id', 'version'], { id: ref('identifier'), version: ref('semver') }),
          compatibilityReaders: array(
            closed(['package', 'entrypoint'], {
              package: ref('nonBlankText'),
              entrypoint: ref('nonBlankText'),
            }),
            1,
          ...
          retentionPolicy: { enum: ['project', 'run', 'release', 'ephemeral'] },
          cleanupPolicy: { enum: ['never', 'on-success', 'bounded-expiry', 'explicit-owner'] },
          mediaTypes: array(
  ...
  const generatedAssetManifest = documentSchema(
    ...
    ],
    {
      ...
      inputs: array(
        closed(['inputKind', 'path', 'version', 'digest'], {
          inputKind: {
            enum: ['canonical-source', 'schema', 'registry', 'template', 'overlay', 'contribution'],
          },
          path: ref('relativePath'),
          version: ref('semver'),
          digest: ref('digest'),
      ...
      assets: array(
        closed(
          ...
          {
            path: ref('relativePath'),
            assetKind: ref('identifier'),
            host: nullableString({ enum: ['claude-code', 'codex', 'cursor'] }),
            mediaType: { type: 'string', pattern: '^[a-z0-9.+-]+/[a-z0-9.+-]+$' },
            byteLength: { type: 'integer', minimum: 0 },
            ...
            outputId: ref('identifier'),
            sourceMap: array(
              closed(['path', 'digest', 'moduleVersion', 'sectionIds'], {
                path: ref('relativePath'),
                digest: ref('digest'),
                moduleVersion: ref('semver'),
                sectionIds: array(ref('identifier'), 1, 256, true),
              }),
              1,
              256,
            ),
      ...
      compatibility: closed(
        ['protocolVersions', 'packageVersions', 'adapterProfiles', 'hostProfiles'],
        {
          protocolVersions: array(ref('semver'), 1, 32, true),
          packageVersions: array(ref('nonBlankText'), 1, 64, true),
          adapterProfiles: array(ref('identifier'), 1, 32, true),
          hostProfiles: array(ref('identifier'), 1, 32, true),
        },
      ),
      purity: closed(['checkerVersion', 'checkerDigest', 'violations'], {
        checkerVersion: ref('semver'),
        checkerDigest: ref('digest'),
        violations: array(ref('diagnostic'), 0, 0),
      }),
      custody: closed(
        ['ownerPackage', 'distributionPackage', 'editPolicy', 'includedInPublicPackage'],
        {
          ownerPackage: ref('nonBlankText'),
          distributionPackage: ref('nonBlankText'),
          editPolicy: { const: 'generated-do-not-edit' },
          includedInPublicPackage: { type: 'boolean' },
        },
      ),
    },
  );

  const migrationPreservation = documentSchema(
    ...
    ],
    {
      ...
      sources: array(
        closed(
          ...
          {
            sourceId: {
              enum: ['openplanr-cli', 'planr-pipeline', 'skills', 'marketplace', 'openplanr-web'],
            },
            cutoffCommit: { type: 'string', pattern: '^[0-9a-f]{40}$' },
            branch: ref('nonBlankText'),
            includedState: {
              enum: ['clean', 'tracked-dirty', 'tracked-and-untracked-dirty', 'external-reference'],
            },
            trackedPatchDigest: nullableRef('digest'),
            untrackedInventoryDigest: ref('digest'),
            refsDigest: ref('digest'),
            fileInventoryDigest: ref('digest'),
            licenseRefs: array(ref('relativePath'), 1, 16, true),
            custodyArtifacts: array(
              closed(['name', 'byteLength', 'digest'], {
      ...
      tagMappings: array(
        closed(['sourceId', 'originalRef', 'tagObjectId', 'peeledCommitId', 'destinationRef'], {
          sourceId: ref('identifier'),
          originalRef: ref('nonBlankText'),
          tagObjectId: { type: 'string', pattern: '^[0-9a-f]{40}$' },
          peeledCommitId: { type: 'string', pattern: '^[0-9a-f]{40}$' },
          destinationRef: { type: 'string', pattern: '^source/[a-z0-9-]+/.+$' },
        }),
        0,
        4096,
      ),
      pathMappings: array(
        closed(
          ...
          ],
          {
            ...
            sources: array(
              closed(
                ['sourceId', 'path', 'cutoffPresent', 'currentPresent', 'mode', 'digest', 'state'],
                {
                  sourceId: ref('identifier'),
                  path: ref('relativePath'),
                  cutoffPresent: { type: 'boolean' },
                  currentPresent: { type: 'boolean' },
                  mode: nullableString({ pattern: '^(?:100644|100755|120000)$' }),
                  digest: nullableRef('digest'),
                  state: { enum: ['tracked', 'untracked', 'deleted'] },
                },
              ),
              1,
              32,
            ...
            disposition: {
              enum: ['exact', 'moved', 'merged', 'regenerated', 'retired', 'external', 'excluded'],
            },
            owner: ref('nonBlankText'),
            reasonCode: ref('identifier'),
            contractRefs: array(contractRef, 0, 32),
            verificationRefs: array(ref('relativePath'), 1, 64, true),
          },
        ),
        1,
        131072,
      ),
      catalogBindings: closed(
        [
          'roles',
          'taskKinds',
          'rules',
        ...
        {
          roles: ref('digest'),
          taskKinds: ref('digest'),
          rules: ref('digest'),
          commands: ref('digest'),
          skills: ref('digest'),
          outputs: ref('digest'),
          exports: ref('digest'),
          generators: ref('digest'),
          generatedAssets: ref('digest'),
        },
      ),
      coverage: closed(
        [
        ...
        {
          originalSchemas: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 180 },
            mapped: { const: 180 },
            unmapped: { const: 0 },
          }),
          originalRegistries: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 12 },
            mapped: { const: 12 },
            unmapped: { const: 0 },
          }),
          canonicalRoles: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 9 },
            mapped: { const: 9 },
            unmapped: { const: 0 },
          }),
          canonicalSkills: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 23 },
            mapped: { const: 23 },
            unmapped: { const: 0 },
          }),
          cliRootCommands: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 38 },
            mapped: { const: 38 },
            unmapped: { const: 0 },
          }),
          pipelineExportKeys: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 37 },
            mapped: { const: 37 },
            unmapped: { const: 0 },
          }),
          pipelineRootSymbols: closed(['expected', 'mapped', 'unmapped'], {
            expected: { const: 229 },
            mapped: { const: 229 },
            unmapped: { const: 0 },
          }),
      ...
      historyProof: closed(
        [
          'firstParentSeed',
          'importedSourceCommits',
          'subtreeMergeCommits',
          'preservedAuthorsDigest',
          'absentCommits',
        ],
        {
          firstParentSeed: { type: 'string', pattern: '^[0-9a-f]{40}$' },
          importedSourceCommits: array({ type: 'string', pattern: '^[0-9a-f]{40}$' }, 4, 16, true),
          subtreeMergeCommits: array({ type: 'string', pattern: '^[0-9a-f]{40}$' }, 3, 16, true),
          preservedAuthorsDigest: ref('digest'),
          absentCommits: array({ enum: ['0a4f3b2', '7ad18f4', '2c22a45'] }, 3, 3, true),
        },
      ),
      verificationGates: array(
        closed(['gateId', 'status', 'commandRef', 'reportDigest', 'evidenceDigest'], {
          gateId: ref('identifier'),
          status: { enum: ['passed', 'failed', 'blocked'] },
          commandRef: ref('nonBlankText'),
      ...
      sourceUnchangedProof: array(
        closed(['sourceId', 'pre', 'post'], {
          sourceId: ref('identifier'),
          pre: closed(
            ['head', 'refsDigest', 'trackedPatchDigest', 'untrackedManifestDigest', 'statusDigest'],
            {
              head: { type: 'string', pattern: '^[0-9a-f]{40}$' },
              refsDigest: ref('digest'),
              trackedPatchDigest: ref('digest'),
              untrackedManifestDigest: ref('digest'),
              statusDigest: ref('digest'),
            },
          ),
          post: closed(
            ['head', 'refsDigest', 'trackedPatchDigest', 'untrackedManifestDigest', 'statusDigest'],
...
const digestForCanonicalPath = (path, fallbackNamespace) => {
  ...
  return existsSync(absolute)
    ? digestFor(readFileSync(absolute))
    : digestFor(`${fallbackNamespace}:${path}`);
};
...
const producer = (id) => ({
  id,
  version: '1.0.0',
  sourceDigest: digestFor(`producer:${id}@1.0.0`),
});
const document = (kind, schemaVersion, fields) =>
  withDocumentDigest({
    kind,
    schemaVersion,
...
const ROLE_DEFINITIONS = [
  [
    ...
    'none',
    ['B', 'C'],
    ['scope-expansion', 'deploy', 'publish', 'credential-write'],
    ['backend'],
    ['R3', 'R5', 'R6', 'R7', 'R9', 'R10'],
  ],
  [
    ...
    'read-only',
    ['A'],
    ['database-mutation', 'deploy', 'publish', 'credential-write', 'scope-expansion'],
    ['database'],
    ['R3', 'R8', 'R10'],
  ],
  [
    ...
    'none',
    ['A', 'B'],
    ['deploy', 'publish', 'credential-write', 'scope-expansion'],
    [],
    ['R1', 'R3', 'R10'],
  ],
  [
...
function roleRecord([
  ...
  const phaseDirectory =
    phase === 'po-preflight' ? 'po/preflight' : phase === 'post-build' ? 'post-build' : phase;
  return {
    roleId,
    roleVersion: '1.0.0',
    displayName: slug
    ...
    legacyAliases: [
      { id: legacyId, status: 'compatibility', since: '1.5.0', removalVersion: null },
    ],
    phase,
    activation,
    capabilityTier,
    ...
    adapterMappings: [
      {
        host: 'claude-code',
        nativeId: legacyId,
        generatedPath: `adapters/claude-code/agents/${legacyId}.md`,
      },
  ...
  return document('task-kind-registry', '1.0.0', {
    ...
    bindings: TASK_BINDINGS.map(
      ([
        ...
        outputContracts: [
          contract(
            ['backend', 'frontend'].includes(taskKind)
              ? 'implementation-result'
              : 'task-output-manifest',
          ),
        ],
...
const RULES = [
  [
    'R1',
    'Request-directed workflow',
    'Planning requests produce planning artifacts. Implementation requests continue through code and the relevant engineering checks.',
    'plan-ship-boundary',
  ],
  [
    'R2',
  ...
  [
    ...
    'Roles use registry-owned capability tiers and adapters own host model mapping; vendor model names are not protocol truth.',
    'role-resolution',
  ],
  [
    'R4',
    'Generated task custody',
    'Generated task documents are not manually edited; their reviewed parent source is corrected and deterministic generation is rerun.',
    'task-generation',
  ],
  [
    'R5',
    'Immutable Preserve boundaries',
    'Every Preserve repository path is immutable for the complete task and SHIP run and is verified against its bound baseline.',
    'preserve-verification',
  ],
  [
    'R6',
    'Adaptive recovery',
    'Use command results and diagnostics to choose useful corrections, continue while meaningful engineering progress is possible, and report a genuine impasse with the relevant error context.',
    'adaptive-recovery',
  ],
  [
    'R7',
    'Proportional verification',
    'Local work uses verification proportional to its risk and directly corrects relevant failures. Dedicated release certification owns release closure.',
    'ship-closure',
  ],
  [
    'R8',
    'Read-only database inspection',
    'Database inspection is read-only and cannot issue DDL, DML, or any external mutation.',
    'database-authority',
  ],
  [
    'R9',
    'Implementation ownership',
    'Frontend and backend ownership boundaries are binding; companion changes must be acceptance-required, disclosed, and outside Preserve.',
    'ownership-boundary',
  ],
  [
    'R10',
    'Role QA parity',
    'No role is releasable without corresponding QA verification coverage and generated adapter parity.',
    'role-parity',
  ],
];
      ...
      enforcementPoints: [
        {
          ...
          path: `packages/pipeline/lib/pipeline/${symbol}.mjs`,
          symbol,
        },
      ],
      testRefs: [`tests/protocol/rule-catalog.test.mjs`],
      citedBy: ['docs/rules.md', 'agents/shared/rules.md'],
      source: source('docs/rules.md'),
...
export const FROZEN_COMMAND_SLUGS = [
  'dashboard',
  'design',
  'design-loop',
  'design-review',
...
const commandRecord = ({
  ...
  lifecycle,
  ownerPackage,
  source: commandSource(sourcePath),
  authorityClass,
  machineJson,
  outputContracts: [],
  skillId,
  aliases: [],
  testRefs: ['tests/protocol/command-catalog.test.mjs'],
});
...
function buildCommands() {
  const commands = [
    ...ROOT_COMMAND_SLUGS.map((slug) =>
      commandRecord({
        ...
        sourcePath:
          RETIRED_SEMANTIC_ROOT_COMMANDS.get(slug) ??
    ...
    ...PIPELINE_MACHINE_GRAMMAR.map((tokens) =>
      commandRecord({
  ...
  return document('command-catalog', '1.0.0', {
    ...
    inventory: {
      rootCommandModules: ROOT_COMMAND_SLUGS.length,
      rootCommandSlugs: ROOT_COMMAND_SLUGS,
      pipelineMachineLeaves: 31,
      pipelineMachineGrammar: PIPELINE_MACHINE_GRAMMAR,
      frozenClaudeDocuments: 8,
      frozenClaudeSlugs: FROZEN_COMMAND_SLUGS,
    },
    commands,
    negativeContracts: [
      {
        negativeContractId: 'retired-pipeline-operate',
        forbiddenArgvPrefix: ['pipeline', 'operate'],
        forbiddenPaths: [
          'packages/pipeline/commands/operate.md',
          'packages/pipeline/skills/planr-operate/SKILL.md',
        ],
        reason:
          'Operate remains a CLI-owned planr operate facade; pipeline-owned Operate grammar and plugin assets are retired.',
        testRefs: [
  ...
  return document('skill-catalog', '1.0.0', {
    ...
    skills: CANONICAL_SKILL_REGISTRY.skills.map((row) => ({
      ...
      hosts: [
        {
          host: 'claude-code',
          entrypoint: `/planr:${projectedName(row.skillId)}`,
          path: `dist/plugins/claude/openplanr/skills/${projectedName(row.skillId)}/SKILL.md`,
        },
        {
          host: 'codex',
          entrypoint: `$planr:${projectedName(row.skillId)}`,
          path: `dist/plugins/openai/openplanr/skills/${projectedName(row.skillId)}/SKILL.md`,
        },
        {
          host: 'cursor',
          entrypoint: row.skillId,
          path: `dist/plugins/cursor/openplanr/rules/${row.skillId}.mdc`,
        },
      ],
...
const OUTPUTS = [
  [
    ...
    '.planr/artifacts/{artifactId}/bundle.html',
    false,
    null,
    'generated-asset-manifest',
    'release',
  ...
  [
    ...
    '@openplanr/protocol',
    'dashboard-data-path',
    '.planr/dashboard/{projectId}.json',
    false,
    'dashboard-bootstrap',
    null,
    'run',
  ...
  [
    ...
    '@openplanr/design',
    'design-session-path',
    '.planr/design/{feature}/session.json',
    false,
    'design-session',
    null,
    'run',
  ...
  [
    ...
    '@openplanr/design',
    'design-spec-path',
    'output/feats/{feature}/design-spec.md',
    true,
    null,
    null,
    'project',
  ...
  [
    'diagram-set',
    'Verified diagram set',
    'D',
    '@openplanr/artifact',
    'diagram-manifest-path',
    'diagrams/{slug}/{slug}.manifest.json',
    false,
    'diagram-manifest',
    'diagram-manifest',
    'project',
    'explicit-owner',
    ['application/json', 'image/png', 'image/svg+xml', 'text/html'],
  ],
  [
    'diagram-source',
    ...
    '@openplanr/artifact',
    'diagram-source-path',
    'diagrams/{slug}/{slug}.planr-diagram.json',
    true,
    'diagram-document',
    'diagram-manifest',
    'project',
  ...
  [
    ...
    'Generated host plugin assets',
    'D',
    '@openplanr/skill-runtime',
    'host-asset-path',
    'dist/plugins/{host}/openplanr/{assetPath}',
    false,
    null,
    'generated-asset-manifest',
    'release',
  ...
  [
    'gherkin-feature',
    'Gherkin acceptance feature',
    'A',
    '@openplanr/pipeline',
    'gherkin-path',
    'output/feats/{feature}/us-{story}/acceptance.feature',
    true,
    null,
    null,
    'project',
  ...
  [
    'operate-state',
    'Operate runtime state',
    'B',
    '@openplanr/operate',
    'operate-state-path',
    '.planr/operate/{cycleId}/state.json',
    true,
  ...
  [
    ...
    'B',
    '@openplanr/pipeline',
    'planning-review-path',
    '.planr/pipeline/{feature}/planning-review-receipt.json',
    true,
    'planning-review-receipt',
    null,
    'release',
  ...
  [
    'ship-closure',
    'SHIP closure receipt',
    'B',
    '@openplanr/pipeline',
    'ship-closure-path',
    '.planr/pipeline/{feature}/ship-closure.json',
    true,
    'ship-closure',
    null,
    'release',
  ...
  [
    ...
    '@openplanr/pipeline',
    'task-path',
    'output/feats/{feature}/us-{story}/tasks/task-{task}.md',
    true,
    'task',
    'task-manifest',
    'project',
  ...
  [
    'task-error-handoff',
    'Blocked task error handoff',
    'C',
    '@openplanr/pipeline',
    'task-error-path',
    'output/feats/{feature}/us-{story}/tasks/{taskId}-error-report.md',
    false,
    null,
    'task-output-manifest',
    'run',
  ...
  [
    ...
    '@openplanr/pipeline',
    'task-output-path',
    '.planr/pipeline/{feature}/tasks/{taskId}-output.json',
    true,
    'task-output-manifest',
    null,
    'run',
  ...
  [
    ...
    '@openplanr/pipeline',
    'story-path',
    'output/feats/{feature}/us-{story}/us-{story}.md',
    true,
    'story',
    null,
    'project',
    ...
    outputs: OUTPUTS.map(
      ([
        ...
        schemaRef: schemaId
          ? contract(
              schemaId,
              schemaId.startsWith('operating-')
                ? '2.0.0'
                : schemaId === 'evaluation-aggregate-report'
                  ? '1.4.0'
                  : schemaId === 'dashboard-bootstrap'
                    ? '1.2.0'
                    : schemaId === 'ship-closure' ||
                        schemaId === 'planning-review-receipt' ||
                        schemaId === 'professional-specification'
                      ? '1.1.0'
                      : '1.0.0',
            )
          : null,
        manifestRef: manifestId ? contract(manifestId) : null,
        generator: producer(`generate-${outputId}`),
        validator: { id: `validate-${outputId}`, version: '1.0.0' },
        compatibilityReaders: [
          { package: ownerPackage, entrypoint: `${ownerPackage}/${outputId}` },
        ],
        retentionPolicy,
        cleanupPolicy,
        mediaTypes,
        testRefs: ['tests/protocol/output-catalog.test.mjs'],
      }),
    ),
  });
}
