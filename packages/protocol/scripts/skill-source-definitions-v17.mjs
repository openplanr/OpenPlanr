...
function successor(value) {
  return JSON.parse(JSON.stringify(value).replaceAll('1.6.0', '1.7.0').replaceAll('Protocol 1.6', 'Protocol 1.7'));
}

/** Additive successors; Protocol 1.6 generation remains byte-identical. */
export function buildSkillSourceSchemasV17() {
  const schemas = new Map([...buildSkillSourceSchemas()].map(([name, value]) => [name, successor(value)]));
...
export function buildSkillSourceRegistriesV17() {
  return new Map([...buildSkillSourceRegistries()].map(([name, value]) => {
    ...
    return [name, withDocumentDigest({
      ...value,
