...
export const DIAGRAM_GRAMMARS = Object.freeze([
  grammar('architecture', 'Architecture', 'graph', [
    'system architecture',
    'software architecture',
    'components',
  ]),
  grammar('it-current-state', 'IT current state', 'graph', [
    'legacy landscape',
    'current state',
    'modernization',
  ]),
  grammar('flowchart', 'Flowchart', 'graph', ['decision flow', 'workflow', 'process flow'], {
    mermaid: 'editable',
  }),
  grammar(
    'sequence',
    'Sequence',
    'chronology',
    ['message sequence', 'interaction sequence', 'request trace'],
    { required: ['node', 'relation', 'event'], mermaid: 'editable' },
  ),
  grammar(
    'state-machine',
    'State machine',
    'graph',
    ['state diagram', 'lifecycle states', 'transitions'],
    { mermaid: 'editable' },
  ),
  grammar(
    'er-data-model',
    'ER data model',
    'entity',
    ['entity relationship', 'logical data model', 'er diagram'],
    { mermaid: 'editable' },
  ),
  grammar('timeline', 'Timeline', 'chronology', ['milestones', 'history', 'roadmap'], {
    required: ['event'],
  }),
  grammar(
    'swimlane',
    'Swimlane',
    'lanes',
    ['cross functional', 'handoff flow', 'responsibility lanes'],
    { required: ['node', 'lane'] },
  ),
  grammar('quadrant', 'Quadrant', 'chart', ['two axis matrix', 'impact effort', 'four quadrants'], {
    required: ['node', 'axis'],
    directions: ['radial'],
  }),
  grammar('radar', 'Radar chart', 'chart', ['spider chart', 'multi axis comparison'], {
    required: ['series', 'axis'],
    directions: ['radial'],
    mermaid: 'unsupported',
  }),
  grammar('polar', 'Polar chart', 'chart', ['polar area', 'cyclic magnitude'], {
    required: ['series', 'axis'],
    directions: ['radial'],
    mermaid: 'unsupported',
  }),
  grammar(
    ...
    'Loop and flywheel',
    'graph',
    ['flywheel', 'reinforcing loop', 'feedback loop'],
    { directions: ['radial'] },
  ),
  grammar('nested', 'Nested containment', 'containment', ['nested boxes', 'containment map'], {
    required: ['node', 'group'],
  }),
  grammar('tree', 'Tree', 'hierarchy', ['hierarchy tree', 'parent child'], { mermaid: 'editable' }),
  grammar('org-chart', 'Organization chart', 'hierarchy', [
    'org chart',
    'reporting lines',
    'team structure',
  ]),
  grammar('layer-stack', 'Layer stack', 'hierarchy', [
    'layered architecture',
    'stacked abstractions',
  ]),
  grammar('venn', 'Venn', 'containment', ['set overlap', 'venn diagram'], {
    required: ['set'],
    directions: ['radial'],
    mermaid: 'unsupported',
  }),
  grammar('pyramid-funnel', 'Pyramid and funnel', 'hierarchy', [
    'pyramid',
    'funnel',
    'ranked hierarchy',
  ]),
  grammar('bar', 'Bar chart', 'chart', ['bar graph', 'categorical comparison'], {
    required: ['series', 'axis'],
    mermaid: 'unsupported',
  }),
  grammar('treemap', 'Treemap', 'containment', ['part of whole', 'area hierarchy'], {
    required: ['node', 'group'],
    mermaid: 'unsupported',
  }),
  grammar('line', 'Line chart', 'chart', ['line graph', 'trend chart'], {
    required: ['series', 'axis'],
    mermaid: 'unsupported',
  }),
  grammar('gantt', 'Gantt', 'chronology', ['gantt chart', 'project schedule', 'task timeline'], {
    required: ['event'],
    mermaid: 'editable',
  }),
  grammar(
    ...
    'Scatter plot',
    'chart',
    ['scatter chart', 'correlation', 'distribution plot'],
    { required: ['series', 'axis'], mermaid: 'unsupported' },
  ),
  grammar('high-level', 'High-level system view', 'graph', [
    'high level architecture',
    'end to end stack',
  ]),
  grammar('process', 'Process', 'lanes', ['business process', 'multi actor process'], {
    required: ['node', 'relation', 'lane'],
  }),
  ...
  grammar('dp-integration', 'Data platform integration', 'graph', [
  ...
  grammar('sankey', 'Sankey', 'sankey', ['flow quantities', 'split and merge'], {
    required: ['node', 'relation', 'series'],
    mermaid: 'unsupported',
    maxFanIn: 8,
  }),
  grammar('fishbone', 'Fishbone', 'cause-effect', ['ishikawa', 'cause and effect', 'root causes'], {
    required: ['node', 'relation', 'group'],
    mermaid: 'unsupported',
  }),
  grammar('wardley', 'Wardley map', 'wardley', ['value chain evolution', 'wardley'], {
    required: ['node', 'relation', 'axis'],
    mermaid: 'unsupported',
  }),
  grammar('kanban', 'Kanban', 'lanes', ['kanban board', 'work in progress'], {
    required: ['node', 'lane'],
    mermaid: 'unsupported',
  }),
  grammar('user-journey', 'User journey', 'lanes', ['customer journey', 'experience map'], {
    required: ['event', 'lane'],
    mermaid: 'partial',
  }),
  grammar(
    ...
    'Deployment',
    'containment',
    ['deployment diagram', 'zones hosts artifacts'],
    { required: ['node', 'group'], mermaid: 'partial' },
  ),
  grammar(
    'dependency-graph',
    'Dependency graph',
    'graph',
    ['dependencies', 'fan in graph', 'package graph'],
    { mermaid: 'editable', maxFanIn: 10 },
  ),
  grammar('uml-class', 'UML class', 'entity', ['class diagram', 'typed relations'], {
    mermaid: 'editable',
  }),
  grammar('story-map', 'Story map', 'lanes', ['user story map', 'release slices'], {
    required: ['node', 'lane'],
    mermaid: 'unsupported',
  }),
  grammar(
    ...
    'Database schema',
    'entity',
    ['physical schema', 'database tables', 'foreign keys'],
    { mermaid: 'editable' },
  ),
]);
...
export const DIAGRAM_SEMANTIC_PATTERNS = Object.freeze(
  [
    [
      'fan-in-bottleneck',
      ['queue', 'bottleneck', 'fan in'],
      ['dependency-graph', 'sankey', 'data-flow'],
    ],
    [
      'repeated-stages',
      ['stages', 'pipeline slots', 'repeat'],
      ['process', 'swimlane', 'data-flow'],
    ],
    [
      'unstructured-transformation',
      ['unstructured input', 'transform', 'normalize'],
      ['data-flow', 'dp-integration'],
    ],
    [
      'paired-policy-trace',
      ['policy trace', 'allow deny', 'decision path'],
      ['sequence', 'flowchart'],
    ],
    [
      'secure-paved-road',
      ['secure path', 'paved road', 'guardrails'],
      ['architecture', 'deployment'],
    ],
    [
      'governance-catalog',
      ['catalog', 'governance', 'ownership'],
      ['tree', 'org-chart', 'layer-stack'],
    ],
    [
      'compensating-layers',
      ['defense in depth', 'compensating controls'],
      ['layer-stack', 'dp-security-matrix'],
    ],
  ].map(([patternId, triggers, candidateGrammars]) =>
    Object.freeze({
      patternId,
...
const node = closed(['id', 'label', 'kind'], {
  ...
  semanticPosition: nullable(
    closed(['horizontal', 'vertical', 'meaning'], {
...
const relation = closed(['id', 'from', 'to', 'kind'], {
  ...
  from: itemId,
  to: itemId,
  kind: { enum: ['association', 'dependency', 'flow', 'message', 'transition', 'containment'] },
  label: nullable(nonBlank),
  weight: nullable({ type: 'number', minimum: 0 }),
});
const group = closed(['id', 'label', 'members'], {
  id: itemId,
  label: nonBlank,
  members: array(itemId, 1, 256, true),
});
const lane = closed(['id', 'label', 'members'], {
  id: itemId,
  label: nonBlank,
  members: array(itemId, 0, 256, true),
});
const event = closed(['id', 'label', 'order'], {
  id: itemId,
  label: nonBlank,
  order: { type: 'integer', minimum: 0 },
  at: nullable({ type: 'string', minLength: 1, maxLength: 128 }),
});
...
const axis = closed(['id', 'label', 'scale'], {
  id: itemId,
  label: nonBlank,
  scale: { enum: ['categorical', 'linear', 'logarithmic', 'ordinal', 'temporal'] },
});
const set = closed(['id', 'label', 'members'], {
  id: itemId,
  label: nonBlank,
  members: array(itemId, 1, 256, true),
});
const annotation = closed(['id', 'text', 'targetId'], {
  id: itemId,
  text: nonBlank,
  targetId: nullable(itemId),
});
const emphasis = closed(['targetId', 'level'], {
  targetId: itemId,
  level: { enum: ['primary', 'secondary', 'muted'] },
});

export function buildDiagramSchemas() {
  const diagramDocument = documentSchema(
    'diagram-document',
    'planr-diagram',
    [
      'diagramId',
    ...
    ],
    {
      ...
      accessibility: closed(['title', 'description', 'readingOrder'], {
        ...
        readingOrder: array(itemId, 0, 4096, true),
      }),
    },
    {
      anyOf: [
  ...
  const quality = documentSchema(
    ...
    ['diagramId', 'status', 'checks', 'splitPlan'],
    {
      ...
      splitPlan: nullable(
        closed(['strategy', 'panels'], {
          strategy: { enum: ['by-group', 'by-lane', 'by-sequence', 'by-subgraph'] },
          panels: array(
            closed(['id', 'title', 'itemIds'], {
              id: itemId,
              title: nonBlank,
              itemIds: array(itemId, 1, 256, true),
            }),
            2,
            32,
          ),
  ...
  const fidelityReport = documentSchema(
    ...
    ['diagramId', 'sourceFormat', 'targetFormat', 'status', 'interpreted', 'omitted', 'notes'],
    {
      ...
      status: fidelity,
      interpreted: array(
        closed(['source', 'targetId', 'construct'], {
          source: nonBlank,
          targetId: itemId,
  ...
  const consumerReference = documentSchema(
  ...
  const reviewBinding = documentSchema(
    ...
    ['bindingId', 'diagramId', 'manifest', 'artifact', 'review'],
    {
      ...
      review: nullable(
        closed(['reviewId', 'reviewDigest', 'pinIds'], {
  ...
  const grammarEntry = closed(
    ...
    ],
    {
      ...
      readability: closed(
        ['maxLabelCharacters', 'maxCrossings', 'maxFanIn', 'minTextSize', 'aspectRatios'],
        {
      ...
      rendererContract: closed(['layoutFamily', 'contractVersion', 'staticByDefault'], {
        ...
        staticByDefault: { const: true },
      }),
      accessibilityTemplate: closed(['title', 'description'], {
        title: nonBlank,
        description: nonBlank,
      }),
    },
  ...
  const typeRegistry = documentSchema(
    ...
    ['registryVersion', 'primitives', 'layoutFamilies', 'grammars'],
    {
      ...
      layoutFamilies: array(
        { enum: DIAGRAM_LAYOUT_FAMILIES },
        DIAGRAM_LAYOUT_FAMILIES.length,
        DIAGRAM_LAYOUT_FAMILIES.length,
        true,
      ),
  ...
  const semanticPatternRegistry = documentSchema(
    ...
    {
      patterns: array(
        closed(['patternId', 'patternVersion', 'triggers', 'candidateGrammars', 'selection'], {
...
export function buildDiagramRegistries() {
  ...
  return new Map(
    Object.entries(DIAGRAM_V16_REGISTRIES).map(([file, descriptor]) => [
      file,
