...
const readinessRecord = closed({
  ...
  continuation: closed({ action: { const: DESIGN_HANDOFF_AUTHORITY }, available: { type: 'boolean' } }),
...
const implementationHandoff = closed({
  ...
  approval: closed({ actorId: id, approvedAt: timestamp, contentDigest: digest, authority: { const: DESIGN_HANDOFF_AUTHORITY } }),
...
export const DESIGN_PLANNING_LINEAGE_SCHEMA = contract('design-planning-lineage', closed({
...
export function isDesignHandoffRelativePath(value) {
  return typeof value === 'string'
    ...
    && value.split('/').every(part => part && part !== '.' && part !== '..' && !['__proto__', 'prototype', 'constructor'].includes(part));
}
...
export function assertDesignHandoffContract(value, schemaOrName) {
  ...
  if (errors.length) throw new TypeError(`Invalid ${schema['x-openplanr-contract']?.id ?? 'design handoff data'}: ${errors.slice(0, 5).map(error => `${error.path} (${error.rule})`).join('; ')}`);
  return value;
}

function assertEvidenceReferences(evidence) {
  distinct(evidence, item => item.id, 'evidence reference identity');
  for (const item of evidence) if (!isDesignHandoffRelativePath(item.path)) throw new TypeError('Design handoff evidence requires a repository-relative logical path.');
}
...
export function assertDesignHandoffReadiness(value) {
  ...
  const worst = value.checks.reduce((current, check) => priority[check.status] > priority[current] ? check.status : current, 'pass');
...
export function assertDesignImplementationHandoff(value) {
  ...
  for (const item of value.requirements) {
    ...
    if (item.sourceRefs.some(reference => !sourceIds.has(reference))) throw new TypeError('Implementation requirement references missing evidence.');
  }
  ...
  if (value.approval?.contentDigest !== undefined && value.approval.contentDigest !== value.contentDigest) throw new TypeError('Implementation handoff approval does not match its content.');
...
export function assertDesignPlanningLineage(value, handoff) {
  ...
  for (const mapping of value.mappings) {
    distinct(mapping.acceptanceRefs, item => `${item.storyId}:${item.acceptanceId}`, 'lineage acceptance identity');
    distinct(mapping.taskIds, item => item, 'lineage task identity');
  }
  if (handoff) {
    assertDesignImplementationHandoff(handoff);
    if (handoff.status !== 'approved') throw new TypeError('Planning lineage requires an approved handoff.');
    if (value.handoff.id !== handoff.id || value.handoff.version !== handoff.version || value.handoff.contentDigest !== handoff.contentDigest) throw new TypeError('Planning lineage identifies a different handoff package.');
    const expected = handoff.requirements.map(item => item.id).sort();
    const actual = value.mappings.map(item => item.requirementId).sort();
    if (actual.join('\n') !== expected.join('\n')) throw new TypeError('Planning lineage must map every implementation requirement exactly once.');
  }
  return value;
}
