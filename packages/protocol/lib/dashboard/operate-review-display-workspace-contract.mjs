  ...
  return [
    {
      path: '$',
      rule: 'closedContract',
      detail: 'does not satisfy the closed Review display workspace contract',
    },
  ];
}
...
function validSummary(summary) {
  ...
  if (
    ...
    stageStates.some(
      (state) =>
        summary.stages[state] !==
        summary.stages.byStage.reduce((total, stage) => total + stage[state], 0),
    ) ||
    summary.seats.terminal !==
      summary.seats.validated +
        summary.seats.rejected +
        summary.seats.failed +
        summary.seats.abandoned ||
    ...
    summary.evidence.total !==
      summary.evidence.current + summary.evidence.historical + summary.evidence.stale ||
    ...
    summary.proof.unresolvedClaims !==
      summary.claims.uncertain + summary.claims.unknown + summary.claims.restricted ||
    summary.claims.total !== summary.proof.resolvedClaims + summary.proof.unresolvedClaims ||
    summary.verification.total !==
      summary.verification.verified +
        summary.verification.unverified +
        summary.verification.notRequired +
        summary.verification.insufficientEvidence ||
    summary.proof.verifiedOutcomes !== summary.verification.verified ||
    summary.proof.unverifiedOutcomes !==
      summary.verification.unverified + summary.verification.insufficientEvidence ||
...
function validCapability(payload) {
  ...
  if (!capability.available)
    return capability.actions.length === 0 && payload.mutationEnabled === false;
  if (
    payload.sourceArtifactKind !== 'operating-review-read' ||
    payload.status !== 'ready' ||
    payload.mutationEnabled !== true ||
    capability.actions.length !== choices.length
  )
    return false;
  const choiceByArguments = new Map(
    choices.map((choice) => [jcsHash(choice.submitArguments), choice]),
  );
...
function validWorkspaceSemantics(value, expected) {
  ...
  const { data } = payload;
  if (
    integrity.domain !== OPERATE_REVIEW_DISPLAY_WORKSPACE_DOMAIN ||
    ...
    !exactExpected(payload, expected)
  )
    return false;
  try {
...
export function validateOperateReviewDisplayWorkspaceV1(value, expected) {
  try {
    ...
    return clone.integrity.contentHash ===
      contentHash(clone.payload, OPERATE_REVIEW_DISPLAY_WORKSPACE_DOMAIN)
      ? []
      : invalid();
  } catch {
    return invalid();
...
export function assertOperateReviewDisplayWorkspaceV1(value, expected) {
  if (validateOperateReviewDisplayWorkspaceV1(value, expected).length > 0) {
    const error = new TypeError(
      'The Review display workspace does not satisfy its closed integrity contract.',
    );
