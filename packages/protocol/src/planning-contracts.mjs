...
export function validatePlanningAcceptanceCoverage(stories, tasks) {
  ...
  for (const story of stories ?? []) {
    ...
    if (ids.size !== criteria.length) {
      issues.push({
        path: `${story.id}.acceptanceCriteria`,
        rule: 'unique',
        detail: 'Acceptance IDs must be unique within the story.',
      });
    }
    for (const criterion of criteria) {
      const mapped = (tasksByStory.get(story.id) ?? []).filter((task) =>
        task.acceptanceRefs.includes(criterion.id),
      );
      if (mapped.length === 0) {
        issues.push({
          path: `${story.id}.${criterion.id}`,
          rule: 'coverage',
          detail: 'Acceptance criterion is not mapped to a task.',
        });
        continue;
      }
      if (mapped.every((task) => !String(task.testRequirements ?? '').includes(criterion.id))) {
        issues.push({
          path: `${story.id}.${criterion.id}`,
          rule: 'verification',
          detail: 'A mapped task must name the acceptance ID in Test Requirements.',
        });
      }
    }
    for (const task of tasksByStory.get(story.id) ?? []) {
      for (const reference of task.acceptanceRefs) {
        if (!ids.has(reference))
          issues.push({
            path: `${task.id}.acceptanceRefs`,
            rule: 'reference',
            detail: `${reference} is not declared by ${story.id}.`,
          });
      }
    }
  }
