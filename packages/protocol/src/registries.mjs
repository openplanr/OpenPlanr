...
function assertDocument(name, value, kind) {
  if (
    ...
    Array.isArray(value) ||
    value.kind !== kind ||
    value.protocolVersion !== '1.5.0' ||
    value.digestAlgorithm !== 'sha256' ||
    value.canonicalization !== 'rfc8785' ||
    !verifyDocumentDigest(value)
  ) {
    fail(
      PROTOCOL_ERROR_CODES.DOCUMENT_INVALID,
      `${name} is not a digest-bound Protocol 1.5 document.`,
      { name },
    );
  }
}
...
function assertSortedUnique(values, key, name) {
  ...
  if (new Set(keys).size !== keys.length)
    fail(PROTOCOL_ERROR_CODES.DUPLICATE_ID, `${name} contains duplicate ${key} values.`, { keys });
  const sorted = [...keys].sort();
  if (!equal(keys, sorted))
    fail(PROTOCOL_ERROR_CODES.SORT_ORDER_INVALID, `${name} must be sorted by ${key}.`, {
      keys,
      sorted,
    });
}
...
const EXPECTED_TASK_KINDS = Object.freeze({
  backend: 'planr-backend',
  database: 'planr-database',
  devops: 'planr-devops',
  documentation: 'planr-documentation',
  'entity-scaffold': 'planr-entity-scaffold',
  frontend: 'planr-frontend',
  qa: 'planr-qa',
});
...
export function validateCanonicalRegistries(registries = CANONICAL_REGISTRIES) {
  ...
  if (roles.roles.length !== 9)
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Protocol 0.1 requires exactly nine canonical roles.',
    );
  ...
  for (const role of roles.roles) {
    if (
      role.legacyAliases.length !== 1 ||
      aliases.has(role.legacyAliases[0].id) ||
      canonicalRoleIds.has(role.legacyAliases[0].id)
    ) {
    ...
    if (
      ...
      !equal(
        [
          role.legacyAliases[0]?.id,
          role.phase,
          role.activation,
          role.capabilityTier,
          role.outputContracts[0]?.id,
        ],
        expected,
      )
    ) {
      fail(
        PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
        `Role compatibility mapping drifted for ${role.roleId}.`,
      );
    }
    if (role.adapterMappings.length < 3 || role.qaCoverage.length < 1) {
      fail(
        PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
        `Role ${role.roleId} lacks adapter parity or QA coverage.`,
      );
    }
  }
  ...
  if (
    qa.writeBoundary.repositoryAccess !== 'read-only' ||
    qa.writeBoundary.allowedOutputClasses.includes('C')
  ) {
    fail(PROTOCOL_ERROR_CODES.REGISTRY_INVALID, 'QA must remain read-only.');
  }
  if (
    database.writeBoundary.externalDataAccess !== 'read-only' ||
    !database.writeBoundary.forbiddenEffects.includes('database-mutation')
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Database inspection must remain externally read-only.',
    );
  }
  if (!devops.writeBoundary.forbiddenEffects.includes('deploy')) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'DevOps configuration authority must forbid deploy.',
    );
  }

  if (taskKinds.roleRegistryDigest !== roles.documentDigest || taskKinds.bindings.length !== 7) {
    fail(
      PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
      'Task-kind registry is not bound to the exact role registry.',
    );
  }
  assertSortedUnique(taskKinds.bindings, 'taskKind', 'task-kinds.json bindings');
  for (const binding of taskKinds.bindings) {
    if (
      EXPECTED_TASK_KINDS[binding.taskKind] !== binding.roleId ||
      !canonicalRoleIds.has(binding.roleId)
    ) {
      fail(
        PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
        `Task kind ${binding.taskKind} has a mismatched role binding.`,
      );
    }
    ...
    if (!equal(binding.outputContracts, [{ id: expectedOutput, version: '1.0.0' }])) {
      fail(
        PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
        `Task kind ${binding.taskKind} has a mismatched output contract.`,
      );
    }
  }
  if (
    !equal(
      taskKinds.legacyMappings.map(({ legacyType, legacyAgent, taskKind, precedence }) => [
        legacyType,
        legacyAgent,
        taskKind,
        precedence,
      ...
      [
        ['UI', 'frontend-agent', 'frontend', 1],
        ['Tech', 'db-agent', 'database', 2],
        ['Tech', 'backend-agent', 'backend', 3],
        ['Tech', null, 'backend', 4],
      ],
    )
  )
  ...
  if (
    !equal(
      rules.rules.map(({ ruleId }) => ruleId),
      ['R1', 'R10', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R8', 'R9'].sort(),
    )
  ) {
  ...
  if (
    rules.rules.some(
      (rule) => !rule.enforcementPoints.length || !rule.testRefs.length || !rule.citedBy.length,
    )
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Every rule requires enforcement, tests, and citations.',
    );
  }

  const inventory = commands.inventory;
  if (
    inventory.rootCommandModules !== 39 ||
    inventory.rootCommandSlugs.length !== 39 ||
    inventory.pipelineMachineLeaves !== 31 ||
    inventory.pipelineMachineGrammar.length !== 31 ||
    inventory.frozenClaudeDocuments !== 8 ||
    inventory.frozenClaudeSlugs.length !== 8
  ) {
  ...
  if (new Set(grammar).size !== grammar.length)
    fail(PROTOCOL_ERROR_CODES.DUPLICATE_ID, 'Command grammar contains duplicate argv entries.');
  if (commands.commands.some(({ argv }) => argv[0] === 'pipeline' && argv[1] === 'operate')) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Retired pipeline Operate grammar became callable.',
    );
  }
  if (
    !commands.commands.some(
      ({ argv, lifecycle }) => argv[0] === 'operate' && lifecycle === 'active',
    ) ||
    !commands.negativeContracts.some(({ forbiddenArgvPrefix }) =>
      equal(forbiddenArgvPrefix, ['pipeline', 'operate']),
    )
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Operate facade/retirement negative contract drifted.',
    );
  }
  ...
  if (!equal(skillIds, EXPECTED_SKILLS)) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Canonical skill membership differs from the active source registry.',
    );
  }
  if (
    skillIds.some((id) => canonicalRoleIds.has(id)) ||
    skills.skills.some((skill) => skill.hosts.length < 3 || !skill.certificationRefs.length)
  ) {
    fail(
      PROTOCOL_ERROR_CODES.DUPLICATE_ID,
      'Skill/role IDs collide or a skill lacks host/certification parity.',
    );
  }
  ...
  if (!equal(ship?.contracts?.outputs, [{ id: 'implementation-result', version: '1.0.0' }])) {
    fail(
      PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
      'Planr Ship must produce the canonical implementation result.',
    );
  }
  ...
  if (aliasIds.length !== 0) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Protocol 1.8 canonical skills must not expose compatibility aliases.',
    );
  }
  for (const command of commands.commands.filter(
    ({ surface }) => surface === 'frozen-host-alias',
  )) {
    if (!skillIds.includes(command.skillId))
      fail(
        PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
        `Frozen command ${command.commandId} has no canonical skill.`,
      );
  }

  assertSortedUnique(outputs.outputs, 'outputId', 'outputs.json');
  if (
    !['A', 'B', 'C', 'D'].every((outputClass) =>
      outputs.outputs.some((output) => output.outputClass === outputClass),
    )
  ) {
  ...
  if (
    outputs.outputs.some(
      (output) =>
        !output.generator ||
        !output.validator ||
        !output.compatibilityReaders.length ||
        !output.testRefs.length,
    )
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
      'Every output requires generator, validator, reader, and test custody.',
    );
  }
  const modeAwarePlanningIds = [
    'gherkin-feature',
  ...
  if (
    outputPaths.outputCatalogDigest !== outputs.documentDigest ||
    !equal(
      outputPaths.outputs.map(({ outputId }) => outputId),
      modeAwarePlanningIds,
    )
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
      'Project-mode paths must bind the exact output catalog and complete planning output set.',
    );
  }
  for (const outputPath of outputPaths.outputs) {
    const output = outputs.outputs.find(({ outputId }) => outputId === outputPath.outputId);
    if (
      outputPath.pathTemplates.default !== output?.pathTemplate ||
      !outputPath.pathTemplates['spec-driven']
    ) {
      fail(
        PROTOCOL_ERROR_CODES.REGISTRY_INVALID,
        `Output ${outputPath.outputId} has an invalid project-mode path contract.`,
      );
    }
  }
  return true;
...
const roleIndex = new Map(
  CANONICAL_REGISTRIES['roles.json'].roles.map((role) => [role.roleId, role]),
);
const aliasIndex = new Map(
  CANONICAL_REGISTRIES['roles.json'].roles.flatMap((role) =>
    role.legacyAliases.map(({ id }) => [id, role]),
  ),
);
const taskKindIndex = new Map(
  CANONICAL_REGISTRIES['task-kinds.json'].bindings.map((binding) => [binding.taskKind, binding]),
);
...
export function getOutputPathTemplate(
  ...
  const outputPath = registries['output-paths.json']?.outputs?.find(
    (candidate) => candidate.outputId === outputId,
  );
...
export function resolveOutputPath(outputId, options = {}, registries = CANONICAL_REGISTRIES) {
  ...
  const missing = [...new Set([...template.matchAll(PATH_TOKEN)].map((match) => match[1]))].filter(
    (name) => typeof pathArguments[name] !== 'string' || pathArguments[name].length === 0,
  );
  if (missing.length > 0) {
    fail(
      PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
      `Output ${outputId} path is missing arguments: ${missing.join(', ')}.`,
      { outputId, projectMode, missing },
    );
  }
  const resolved = template.replace(PATH_TOKEN, (_token, name) => pathArguments[name]);
  if (
    resolved.length > MAX_RELATIVE_PATH_LENGTH ||
    !SAFE_RELATIVE_PATH.test(resolved) ||
    /[\u0000\r\n]/u.test(resolved)
  ) {
    fail(
      PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
      `Output ${outputId} resolved to an invalid relative path.`,
      { outputId, projectMode },
    );
  }
  return resolved;
}
...
export function routeLegacyTask({ legacyType, legacyAgent = null }) {
  ...
  const exact = mappings.find(
    (mapping) => mapping.legacyType === legacyType && mapping.legacyAgent === legacyAgent,
  );
  if (exact) return resolveTaskKind(exact.taskKind);
  if (legacyType === 'Tech' && legacyAgent === null) return resolveTaskKind('backend');
  fail(
    PROTOCOL_ERROR_CODES.REFERENCE_INVALID,
    `Legacy routing requires an explicit reviewed mapping for ${legacyType}/${legacyAgent ?? '<none>'}.`,
  );
}
