...
export interface EnterpriseAccessContext {
  ...
  projectMembership?: EnterpriseScope & {
    actorId: EnterpriseId;
    role: EnterpriseProjectRole;
    status: 'active' | 'revoked';
  };
  guestGrant?: EnterpriseScope & {
    artifactId: EnterpriseId;
    actorId: EnterpriseId;
    role: 'reviewer' | 'viewer';
    status: 'active' | 'revoked';
    expiresAt: EnterpriseTimestamp;
  };
...
export interface EnterpriseArtifactRevision extends EnterpriseScope {
  ...
  id: EnterpriseId;
  artifactId: EnterpriseId;
  parentRevisionId: EnterpriseId | null;
  contentDigest: EnterpriseDigest;
  contentType: 'application/json' | 'text/markdown' | 'image/svg+xml' | 'text/html';
  byteLength: number;
  createdAt: EnterpriseTimestamp;
  actorId: EnterpriseId;
}
export interface EnterpriseReviewAnchor {
  revisionId: EnterpriseId;
  elementId?: EnterpriseId;
  screenId?: EnterpriseId;
  frameId?: EnterpriseId;
  x?: number;
  y?: number;
}
export interface EnterpriseReviewReply {
  id: EnterpriseId;
...
export interface EnterpriseReviewThread extends EnterpriseScope {
  ...
  id: EnterpriseId;
  artifactId: EnterpriseId;
  anchor: EnterpriseReviewAnchor;
  category: EnterpriseReviewCategory;
  status: 'open' | 'addressed' | 'resolved';
  authorId: EnterpriseId;
  body: string;
  createdAt: EnterpriseTimestamp;
  updatedAt: EnterpriseTimestamp;
  replies: EnterpriseReviewReply[];
  assigneeId?: EnterpriseId | null;
  addressedRevisionId?: EnterpriseId;
  resolvedAt?: EnterpriseTimestamp;
}
...
export type EnterpriseChangeOperation =
  ...
  | {
      ...
      field: 'label' | 'description' | 'status' | 'title';
      value: string;
    }
  | {
      op: 'add-element';
      collection: EnterpriseElementCollection;
      element: { id: EnterpriseId; [key: string]: EnterpriseJson };
    }
...
export interface EnterpriseChangeProposal extends EnterpriseScope {
  ...
  id: EnterpriseId;
  artifactId: EnterpriseId;
  baseRevisionId: EnterpriseId;
  authorId: EnterpriseId;
  createdAt: EnterpriseTimestamp;
  status: 'draft' | 'proposed' | 'accepted' | 'rejected' | 'applied' | 'conflicted';
  summary: string;
  operations: EnterpriseChangeOperation[];
  validation: {
    status: 'pending' | 'passed' | 'failed';
    issues: { code: string; message: string; targetId?: EnterpriseId }[];
  };
  application?: {
    revisionId: EnterpriseId;
    appliedAt: EnterpriseTimestamp;
    actorId: EnterpriseId;
    gitCommit?: string;
  };
}
...
export interface EnterpriseEvidenceReference extends EnterpriseScope {
  ...
  capturedAt: EnterpriseTimestamp;
...
export interface EnterpriseSyncState extends EnterpriseScope {
  ...
  schemaVersion: '1.0.0';
  repositoryId: EnterpriseId;
  direction: 'push' | 'pull';
  status: 'preview' | 'pending' | 'synchronized' | 'failed' | 'conflicted';
  scope: EnterpriseId[];
  cursor: string | null;
  operationId: EnterpriseId;
  updatedAt: EnterpriseTimestamp;
  items: {
    artifactId: EnterpriseId;
    baseRevisionId: EnterpriseId | null;
    revisionId: EnterpriseId | null;
    contentDigest: EnterpriseDigest;
...
export interface EnterpriseAgentHandoff extends EnterpriseScope {
  ...
  threads: EnterpriseReviewThread[];
...
export declare function assertEnterpriseContract<T>(
  value: T,
  schemaOrName: EnterpriseSchema | string,
): T;
export declare function assertEnterpriseRevision<T extends EnterpriseArtifactRevision>(value: T): T;
export declare function assertEnterpriseRevisionAppend<T extends EnterpriseArtifactRevision>(
  revision: T,
  previous: EnterpriseArtifactRevision | null,
...
export declare function assertEnterpriseEvidence<T extends EnterpriseEvidenceReference>(
  value: T,
): T;
...
export declare function authorizeEnterpriseAccess(
  context: EnterpriseAccessContext,
): EnterpriseAccessDecision;
/** Stable UI projection only; every resource request still requires server authorization. */
export declare function enterpriseProjectCapabilities(
  role: EnterpriseProjectRole | string,
): readonly EnterpriseAction[];
export declare function createEnterpriseHandoff(
...
export interface EnterpriseDocumentChange {
  op: 'add' | 'remove' | 'replace';
  plane: 'semantic' | 'layout';
  collection?: string;
  targetId?: string;
  field?: string;
  before?: EnterpriseJson;
  after?: EnterpriseJson;
}
export interface EnterpriseDocumentDiff {
  semanticChanged: boolean;
  layoutChanged: boolean;
  changes: EnterpriseDocumentChange[];
}
export declare function diffEnterpriseDocuments(
  before: { [key: string]: EnterpriseJson },
  after: { [key: string]: EnterpriseJson },
  options?: { beforeLayout?: EnterpriseLayout; afterLayout?: EnterpriseLayout },
): EnterpriseDocumentDiff;
export declare function applyEnterpriseOperations(
  document: { [key: string]: EnterpriseJson },
  proposal: EnterpriseChangeProposal,
  options?: { layout?: EnterpriseLayout },
): EnterpriseDocumentDiff & {
  document: { [key: string]: EnterpriseJson };
