...
export const DIAGRAM_EDIT_OPERATION_CLASSES = deepFreeze([
  'insert-elements', 'update-semantics', 'remove-elements', 'set-membership-order',
  'set-geometry', 'set-appearance-locks',
]);
export const DIAGRAM_AUTHORING_LIMITS = deepFreeze({
  depth: 48, values: 500000, textCodeUnits: 8388608, sourceBytes: 1048576,
  elements: 10000, operations: 256, coordinate: 1000000,
});

const closed = (properties, required = Object.keys(properties)) => ({ type: 'object', additionalProperties: false, properties, required });
const arr = (items, maxItems, minItems = 0, uniqueItems = false) => ({ type: 'array', items, minItems, maxItems, ...(uniqueItems ? { uniqueItems } : {}) });
...
const id = { type: 'string', pattern: '^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$', minLength: 1, maxLength: 128 };
const digest = { type: 'string', pattern: '^sha256:[a-f0-9]{64}$' };
const token = { type: 'string', pattern: '^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$', maxLength: 64 };
const relativePath = { type: 'string', minLength: 1, maxLength: 1024, pattern: '^[A-Za-z0-9][A-Za-z0-9._/-]*$' };
...
const node = closed({ id, label: str(), kind: enumOf(...semanticKinds), description: nullable(str()) });
const relation = closed({ id, from: id, to: id, kind: enumOf(...relationKinds), direction: enumOf('forward', 'both', 'none'), label: nullable(str()), weight: nullable({ type: 'number', minimum: 0, maximum: 1000000 }) });
...
const semanticCollections = {
  nodes: arr(node, 4096), relations: arr(relation, 8192), groups: arr(container, 1024),
  lanes: arr(container, 256), events: arr(false, 0), series: arr(false, 0), axes: arr(false, 0),
  sets: arr(false, 0), annotations: arr(annotation, 1024), emphasis: arr(emphasis, 1024),
};
...
const attachment = closed({ side: enumOf('top', 'right', 'bottom', 'left'), offset: { type: 'number', minimum: 0, maximum: 1 } });
const route = closed({ mode: enumOf('automatic', 'manual'), strategy: enumOf('straight', 'orthogonal'), from: attachment, to: attachment, points: arr(point, 256) });
...
const locks = closed({ position: { type: 'boolean' }, size: { type: 'boolean' }, route: { type: 'boolean' } });
const placement = closed({ elementId: id, bounds: nullable(bounds), route: nullable(route), label: nullable(labelPlacement), zIndex: index, appearance, locks });
const snapshot = closed({ bundleDigest: digest, semanticDigest: digest, presentationDigest: digest });
...
const documentSchema = schema('diagram-document', 'planr-diagram', {
  diagramId: id, title: str(), summary: str(), audience: enumOf('engineer', 'executive', 'mixed'),
  grammar: closed({ id: enumOf('flowchart', 'process', 'swimlane', 'architecture'), version: { const: '1.0.0' } }),
  ...semanticCollections, laneOrder: ids,
...
const presentationSchema = schema('diagram-presentation', 'diagram-presentation', {
  diagramId: id, semanticDigest: digest, coordinateSystem: { const: 'global-canvas' },
  ...
  elements: arr(placement, 10000), presentationDigest: digest,
});
const range = closed({ startByte: { type: 'integer', minimum: 0, maximum: 1048576 }, endByte: { type: 'integer', minimum: 0, maximum: 1048576 } });
const sourceMapSchema = schema('diagram-source-map', 'diagram-source-map', {
  diagramId: id, semanticDigest: digest, sourceDigest: digest,
  sourceByteLength: { type: 'integer', minimum: 0, maximum: 1048576 },
  encoding: { const: 'utf-8' }, parser: closed({ id: token, version: str(64, 1) }),
  certificationVersion: { const: 'flowchart-copy-v1' },
  entries: arr(closed({ sourceId: nullable(str(128, 1)), elementIds: ids, range: nullable(range), construct: token, confidence: enumOf('exact', 'ambiguous'), losses: arr(str(512, 1), 64) }), 10000),
});
...
const fidelitySchema = schema('diagram-fidelity-report', 'diagram-fidelity-report', {
  diagramId: id, basis: snapshot, sourceDigest: nullable(digest),
  sourceFormat: enumOf('mermaid', 'planr-diagram-bundle'), targetFormat: enumOf('mermaid', 'planr-diagram-bundle', 'svg', 'html', 'png'),
  semantic: fidelityValue, presentation: fidelityValue, sourceText: fidelityValue,
...
const bundleSchema = schema('diagram-authoring-bundle', 'diagram-authoring-bundle', {
  diagramId: id, document: documentSchema, presentation: presentationSchema,
  originalSource: nullable(closed({ format: { const: 'mermaid' }, text: str(1048576), sourceDigest: digest })),
  sourceMap: nullable(sourceMapSchema), bundleDigest: digest,
});
const semanticEntry = { oneOf: [
  closed({ collection: { const: 'nodes' }, value: node }), closed({ collection: { const: 'relations' }, value: relation }),
  closed({ collection: { const: 'groups' }, value: container }), closed({ collection: { const: 'lanes' }, value: container }),
  closed({ collection: { const: 'annotations' }, value: annotation }),
] };
const editableSemanticValue = (collection, properties) => closed({
  type: { const: 'update-semantics' }, collection: { const: collection }, elementId: id,
  before: closed(properties), after: closed(properties),
});
const updateSchemas = [
...
const operation = { oneOf: [
  ...
  closed({ type: { const: 'set-geometry' }, changes: arr(closed({ elementId: id, before: geometryValue, after: geometryValue }), 10000, 1) }),
  closed({ type: { const: 'set-appearance-locks' }, changes: arr(closed({ elementId: id, before: appearanceValue, after: appearanceValue }), 10000, 1) }),
] };
const transactionSchema = schema('diagram-edit-transaction', 'diagram-edit-transaction', {
  transactionId: id, diagramId: id, base: snapshot, operations: arr(operation, 256, 1), undoOf: nullable(id),
});
const proposalSchema = schema('diagram-change-proposal', 'diagram-change-proposal', {
  proposalId: id, diagramId: id, base: snapshot, transaction: transactionSchema,
  summary: str(4096, 1), author: closed({ kind: enumOf('human', 'agent'), id }),
...
const publicationSchema = schema('diagram-publication-state', 'diagram-publication-state', {
  diagramId: id, draft: nullable(snapshot), published: nullable(snapshot),
...
const manifestSchema = schema('diagram-manifest', 'diagram-manifest', {
  diagramId: id, basis: snapshot, bundle: closed({ path: relativePath, transportDigest: digest }),
...
const capabilityProfile = closed({
  ...
  nodeKinds: arr(enumOf(...semanticKinds), 6, 1, true), relationKinds: arr(enumOf(...relationKinds), 5, 1, true),
...
const capabilitiesSchema = schema('diagram-authoring-capabilities', 'diagram-authoring-capabilities', {
  version: { const: '1.0.0' }, liveCollaboration: { const: false },
  profiles: arr(capabilityProfile, 4, 4),
  unsupportedGrammars: arr(token, 128, 0, true),
});
const schemas = {
  'diagram-document': documentSchema, 'diagram-presentation': presentationSchema,
  'diagram-authoring-bundle': bundleSchema, 'diagram-edit-transaction': transactionSchema,
  'diagram-source-map': sourceMapSchema, 'diagram-fidelity-report': fidelitySchema,
  'diagram-manifest': manifestSchema, 'diagram-change-proposal': proposalSchema,
  'diagram-publication-state': publicationSchema, 'diagram-authoring-capabilities': capabilitiesSchema,
};
export const DIAGRAM_AUTHORING_SCHEMAS = deepFreeze(schemas);
export const DIAGRAM_AUTHORING_CONTRACT_FILES = deepFreeze(Object.fromEntries(Object.keys(schemas).map((name) => [name, `${name}.schema.json`])));
...
export const DIAGRAM_AUTHORING_CAPABILITIES = deepFreeze({
  kind: 'diagram-authoring-capabilities', schemaVersion: '1.0.0', protocolVersion: '1.13.0',
  version: '1.0.0', liveCollaboration: false,
  profiles: profileIds.map((grammarId) => ({
    grammarId, authoring: true, primitives: ['node', 'relation', 'group', ...(grammarId === 'process' || grammarId === 'swimlane' ? ['lane'] : []), 'annotation', 'emphasis'],
    nodeKinds: [...semanticKinds], relationKinds: [...relationKinds], operations: [...DIAGRAM_EDIT_OPERATION_CLASSES],
    mermaid: { mode: 'copy', certificationVersion: 'flowchart-copy-v1', constructs: mermaidConstructs.map((entry) => ({ ...entry })), linkedSource: false },
  })),
...
export function getDiagramAuthoringCapability(grammarId) {
  return DIAGRAM_AUTHORING_CAPABILITIES.profiles.find((entry) => entry.grammarId === grammarId) ?? null;
}
...
function inspectData(value) {
  ...
  function visit(current, path, depth) {
    ...
    if (++count > DIAGRAM_AUTHORING_LIMITS.values || depth > DIAGRAM_AUTHORING_LIMITS.depth) {
      issues.push(error(path, 'resource-limit', 'Data exceeds the portable value or nesting limit.')); return;
    }
    ...
    if (typeof current === 'number') { if (!Number.isFinite(current)) issues.push(error(path, 'finite-number', 'Numbers must be finite.')); return; }
    if (typeof current !== 'object') { issues.push(error(path, 'plain-data', 'Only JSON data is accepted.')); return; }
    const proto = Object.getPrototypeOf(current);
    if (Array.isArray(current) ? proto !== Array.prototype : proto !== Object.prototype && proto !== null) {
      issues.push(error(path, 'plain-data', 'Custom prototypes are not accepted.')); return;
    }
    if (ancestors.has(current)) { issues.push(error(path, 'cycle', 'Cyclic data is not accepted.')); return; }
    ancestors.add(current);
    const descriptors = Object.getOwnPropertyDescriptors(current);
    for (const key of Reflect.ownKeys(descriptors)) {
      if (typeof key !== 'string' || key === '__proto__' || key === 'constructor' || key === 'prototype') {
        issues.push(error(path, 'unsafe-key', 'Unsafe or symbolic object key.')); break;
      }
      const descriptor = descriptors[key];
      if (descriptor.get || descriptor.set || !('value' in descriptor)) { issues.push(error(path, 'accessor', 'Accessors are not accepted.')); break; }
      if (Array.isArray(current) && key === 'length') continue;
      if (!descriptor.enumerable || (Array.isArray(current) && (!/^(0|[1-9][0-9]*)$/u.test(key) || Number(key) >= current.length))) {
        issues.push(error(path, 'plain-data', 'Non-JSON properties are not accepted.')); break;
      }
  ...
  try { visit(value, '$', 0); } catch { issues.push(error('$', 'plain-data', 'Data cannot be inspected safely.')); }
  return issues;
}
...
const semanticNames = ['nodes', 'relations', 'groups', 'lanes', 'events', 'series', 'axes', 'sets', 'annotations'];
const allElements = (doc) => semanticNames.flatMap((name) => doc[name]);
const expectedSnapshot = (bundle) => ({ bundleDigest: bundle.bundleDigest, semanticDigest: bundle.document.documentDigest, presentationDigest: bundle.presentation.presentationDigest });
...
function documentIssues(doc, path, issues) {
  ...
  const byId = new Map();
  if (entries.length + doc.emphasis.length > 10000) issues.push(error(path, 'resource-limit', 'Too many semantic primitive entries.'));
  for (const collection of semanticNames) for (let i = 0; i < doc[collection].length; i += 1) {
    const entry = doc[collection][i];
    if (byId.has(entry.id)) issues.push(error(`${path}.${collection}[${i}].id`, 'duplicate-id', 'Semantic element IDs must be globally unique.'));
  ...
  doc.annotations.forEach((entry, i) => { if (entry.targetId !== null && !byId.has(entry.targetId)) issues.push(error(`${path}.annotations[${i}].targetId`, 'reference', 'Annotation target is missing.')); });
  const emphasisIds = new Set();
  doc.emphasis.forEach((entry, i) => {
    if (!byId.has(entry.targetId)) issues.push(error(`${path}.emphasis[${i}].targetId`, 'reference', 'Emphasis target is missing.'));
    if (emphasisIds.has(entry.targetId)) issues.push(error(`${path}.emphasis[${i}].targetId`, 'duplicate-id', 'Only one emphasis entry is allowed per target.'));
    emphasisIds.add(entry.targetId);
  });
  doc.accessibility.readingOrder.forEach((entry, i) => { if (!byId.has(entry)) issues.push(error(`${path}.accessibility.readingOrder[${i}]`, 'reference', 'Reading order target is missing.')); });
...
function presentationIssues(value, doc, path, issues) {
  ...
  value.elements.forEach((entry, i) => {
    ...
    if (idsSeen.has(entry.elementId)) issues.push(error(`${location}.elementId`, 'duplicate-id', 'Presentation element IDs must be unique.'));
    idsSeen.add(entry.elementId);
    if (byId && !byId.has(entry.elementId)) issues.push(error(`${location}.elementId`, 'reference', 'Presentation element is not in the semantic document.'));
    if (entry.route !== null && entry.bounds !== null) issues.push(error(location, 'geometry-kind', 'A connector has a route; a shape has bounds.'));
    if (entry.route === null && entry.bounds === null) issues.push(error(location, 'geometry-kind', 'A placed element requires bounds or a route.'));
    if (entry.route?.mode === 'manual' && entry.route.points.length < 2) issues.push(error(`${location}.route.points`, 'manual-route', 'A manual route requires at least two global points.'));
    if (entry.route?.mode === 'automatic' && entry.route.points.length !== 0) issues.push(error(`${location}.route.points`, 'automatic-route', 'Automatic route geometry is derived and must not carry authored points.'));
    if (entry.route && entry.appearance.shape !== 'connector') issues.push(error(`${location}.appearance.shape`, 'shape-kind', 'Routes must use the connector shape.'));
    if (entry.bounds && entry.appearance.shape === 'connector') issues.push(error(`${location}.appearance.shape`, 'shape-kind', 'Bounded elements cannot use the connector shape.'));
    if (relations?.has(entry.elementId) && !entry.route) issues.push(error(location, 'geometry-kind', 'Semantic relations require route geometry.'));
    if (doc && !relations.has(entry.elementId) && !entry.bounds) issues.push(error(location, 'geometry-kind', 'Non-relation elements require bounds.'));
    if (containers?.has(entry.elementId) && entry.appearance.shape !== 'container') issues.push(error(`${location}.appearance.shape`, 'shape-kind', 'Containers require the container visual shape.'));
    if (nodes?.has(entry.elementId) && ['container', 'text'].includes(entry.appearance.shape)) issues.push(error(`${location}.appearance.shape`, 'shape-kind', 'Nodes require a node visual shape.'));
  });
...
function sourceMapIssues(value, doc, text, path, issues) {
  ...
  if (text !== undefined) {
    ...
    if (value.sourceDigest !== sourceDigest(text) || value.sourceByteLength !== byteLength) issues.push(error(path, 'source-digest', 'Source digest and byte length must match exact UTF-8 source text.'));
    const requested = new Set(value.entries.flatMap((entry) => entry.range ? [entry.range.startByte, entry.range.endByte] : []));
  ...
  value.entries.forEach((entry, i) => {
    ...
    if (entry.sourceId !== null && sourceIds.has(entry.sourceId)) issues.push(error(`${location}.sourceId`, 'ambiguous-source-id', 'Duplicate explicit source IDs must be represented by one ambiguous entry.'));
    if (entry.sourceId !== null) sourceIds.add(entry.sourceId);
    if (entry.confidence === 'exact' && ((entry.sourceId === null && entry.range === null) || entry.elementIds.length !== 1)) issues.push(error(location, 'source-identity', 'Exact correspondence requires a source ID or exact range and one semantic element.'));
    const certification = mermaidConstructs.find((item) => item.construct === entry.construct);
    if (!certification || (certification.import === 'unsupported' && (entry.confidence === 'exact' || entry.losses.length === 0))) issues.push(error(`${location}.construct`, 'source-certification', 'Unsupported syntax requires the certified loss record and cannot claim exact correspondence.'));
    if (elementIds) entry.elementIds.forEach((item, j) => { if (!elementIds.has(item)) issues.push(error(`${location}.elementIds[${j}]`, 'reference', 'Source map semantic element is missing.')); });
    if (entry.range && (entry.range.startByte > entry.range.endByte || entry.range.endByte > value.sourceByteLength || (boundaries && (!boundaries.has(entry.range.startByte) || !boundaries.has(entry.range.endByte))))) issues.push(error(`${location}.range`, 'source-range', 'Source range must lie on UTF-8 boundaries within the original source.'));
  });
}
...
function transactionIssues(value, bundle, path, issues) {
  ...
  const collectionById = bundle ? new Map(semanticNames.flatMap((name) => bundle.document[name].map((entry) => [entry.id, name]))) : null;
  ...
  const semanticAppearance = (collection, appearance, location) => {
    ...
    if ((collection === 'relations' && shape !== 'connector')
      || (collection !== 'relations' && shape === 'connector')
      || (['groups', 'lanes'].includes(collection) && shape !== 'container')
      || (collection === 'nodes' && ['container', 'text'].includes(shape))) issues.push(error(`${location}.shape`, 'shape-kind', 'Appearance must match the semantic element class.'));
  };
    ...
    if (op.type === 'insert-elements' || op.type === 'remove-elements') {
      ...
      for (const entry of op.elements) {
        if (op.type === 'insert-elements') {
          ...
          if (inserted.has(entry.value.id) || available?.has(entry.value.id)) issues.push(error(`${location}.elements`, 'duplicate-id', 'An insert must allocate a new stable element ID.'));
          inserted.add(entry.value.id);
          insertedCollections.set(entry.value.id, entry.collection);
        } else {
          if (removed.has(entry.value.id)) issues.push(error(`${location}.elements`, 'duplicate-id', 'An element cannot be removed twice.'));
          if (available && !available.has(entry.value.id) && !inserted.has(entry.value.id)) issues.push(error(`${location}.elements`, 'reference', 'Removed element is not in the base or earlier inserts.'));
          if (collectionById && (collectionById.get(entry.value.id) ?? insertedCollections.get(entry.value.id)) !== entry.collection) issues.push(error(`${location}.elements`, 'element-class', 'Removed entry must identify the actual semantic element class.'));
          removed.add(entry.value.id);
        }
      }
      if (op.type === 'insert-elements' && collectionById) op.elements.forEach((entry, j) => {
        if (entry.collection === 'relations') for (const endpoint of ['from', 'to']) {
          if ((collectionById.get(entry.value[endpoint]) ?? insertedCollections.get(entry.value[endpoint])) !== 'nodes') issues.push(error(`${location}.elements[${j}].value.${endpoint}`, 'endpoint', 'Relationship endpoints must identify nodes in the base or this insertion.'));
        }
        if (['groups', 'lanes'].includes(entry.collection)) membershipReferences(entry.collection, entry.value, `${location}.elements[${j}].value`);
        if (entry.collection === 'annotations' && entry.value.targetId !== null && !collectionOf(entry.value.targetId)) issues.push(error(`${location}.elements[${j}].value.targetId`, 'reference', 'Annotation target is not in the base or inserts.'));
      });
      ...
      for (const [j, entry] of op.presentation.entries()) {
        ...
        semanticAppearance(semantic?.collection, entry.appearance, `${location}.presentation[${j}].appearance`);
      }
...
function geometryIssues(entry, path, issues) {
  if ((entry.bounds === null) === (entry.route === null)) issues.push(error(path, 'geometry-kind', 'Geometry requires exactly one of bounds or route.'));
  if (entry.route?.mode === 'manual' && entry.route.points.length < 2) issues.push(error(`${path}.route.points`, 'manual-route', 'Manual routes require at least two global points.'));
  if (entry.route?.mode === 'automatic' && entry.route.points.length !== 0) issues.push(error(`${path}.route.points`, 'automatic-route', 'Automatic route points are derived.'));
  if (entry.route?.mode === 'manual' && entry.route.strategy === 'straight' && entry.route.points.length !== 2) issues.push(error(`${path}.route.points`, 'route-strategy', 'A straight manual route has exactly two endpoint points.'));
  if (entry.route?.mode === 'manual' && entry.route.strategy === 'orthogonal' && entry.route.points.some((point, i, points) => i > 0 && point.x !== points[i - 1].x && point.y !== points[i - 1].y)) issues.push(error(`${path}.route.points`, 'route-strategy', 'Every orthogonal segment must be horizontal or vertical.'));
}
function fidelityIssues(value, bundle, path, issues) {
  ...
  for (const [i, loss] of value.losses.entries()) {
    ...
    if (idsKnown) loss.elementIds.forEach((entry, j) => { if (!idsKnown.has(entry)) issues.push(error(`${path}.losses[${i}].elementIds[${j}]`, 'reference', 'Fidelity loss refers to an unknown semantic element.')); });
  }
  ...
  if (bundle && value.sourceFormat === 'mermaid') {
    const uncertain = bundle.sourceMap?.entries.some((entry) => entry.confidence === 'ambiguous' || entry.losses.length || mermaidConstructs.find((item) => item.construct === entry.construct)?.import !== 'lossless');
    ...
    const lostPresentation = bundle.sourceMap?.entries.some((entry) => ['styles-and-directives', 'manual-geometry', 'unsupported-construct'].includes(entry.construct));
  ...
  if (bundle && value.targetFormat === 'mermaid') {
    ...
    const appearances = new Map(bundle.presentation.elements.map((entry) => [entry.elementId, entry.appearance.shape]));
    const supportedNodes = bundle.document.nodes.every((entry) => (
      (entry.kind === 'process' && ['rectangle', 'rounded-rectangle'].includes(appearances.get(entry.id)))
      || (entry.kind === 'decision' && appearances.get(entry.id) === 'diamond')
      || (entry.kind === 'data-store' && appearances.get(entry.id) === 'cylinder')
    ));
    const supportedRelations = bundle.document.relations.every((entry) => (entry.kind === 'flow' && ['forward', 'both'].includes(entry.direction)) || (entry.kind === 'association' && entry.direction === 'none'));
...
const validRelativePath = (value) => !value.split('/').some((part) => part === '' || part === '.' || part === '..') && !/^[A-Za-z]:/u.test(value);
...
export function validateDiagramAuthoringArtifact(kind, value, options = {}) {
  ...
  if (optionIssues.length) return optionIssues.map((entry) => ({ ...entry, path: entry.path.replace(/^\$/u, '$options') }));
  ...
  if (options.sourceText !== undefined && typeof options.sourceText !== 'string') return [error('$options.sourceText', 'type', 'Source text context must be a string.')];
  ...
  for (const name of ['document', 'bundle', 'baseBundle']) if (options[name] !== undefined) {
    ...
    issues.push(...failures.map((entry) => ({ ...entry, path: entry.path.replace(/^\$/u, `$options.${name}`) })));
  }
  if (issues.length) return issues;
  ...
  if (options.sourceText !== undefined && contextBundle?.originalSource && options.sourceText !== contextBundle.originalSource.text) return [error('$options.sourceText', 'source-digest', 'Source text context must match the supplied bundle.')];
  switch (kind) {
    case 'diagram-document': documentIssues(value, '$', issues); break;
    case 'diagram-presentation': presentationIssues(value, contextDoc, '$', issues); break;
    case 'diagram-authoring-bundle': bundleIssues(value, '$', issues); break;
    case 'diagram-source-map': sourceMapIssues(value, contextDoc, options.sourceText ?? contextBundle?.originalSource?.text, '$', issues); break;
    case 'diagram-edit-transaction': transactionIssues(value, contextBundle, '$', issues); break;
    case 'diagram-change-proposal':
      ...
      break;
    case 'diagram-fidelity-report': fidelityIssues(value, contextBundle, '$', issues); break;
    case 'diagram-manifest':
      ...
      value.outputs.forEach((entry, i) => {
        if (!validRelativePath(entry.path)) issues.push(error(`$.outputs[${i}].path`, 'relative-path', 'Output path must remain within its artifact scope.'));
        if (entry.path === value.bundle.path) issues.push(error(`$.outputs[${i}].path`, 'canonical-output-collision', 'A derived output must not overwrite the canonical bundle.'));
        if (entry.fidelity.targetFormat !== ({ 'image/svg+xml': 'svg', 'text/html': 'html', 'image/png': 'png' })[entry.mediaType]) issues.push(error(`$.outputs[${i}].fidelity.targetFormat`, 'output-format', 'Output fidelity must describe the declared media type.'));
        fidelityIssues(entry.fidelity, contextBundle, `$.outputs[${i}].fidelity`, issues);
        if (entry.fidelity.diagramId !== value.diagramId || !same(entry.fidelity.basis, value.basis)) issues.push(error(`$.outputs[${i}].fidelity`, 'basis', 'Every output must identify the manifest snapshot.'));
      });
      if (new Set(value.outputs.map((entry) => entry.path)).size !== value.outputs.length) issues.push(error('$.outputs', 'duplicate-path', 'Output paths must be unique.'));
      break;
    case 'diagram-publication-state':
      ...
      if (contextBundle && (value.diagramId !== contextBundle.diagramId || ![value.draft, value.published].some((entry) => entry && same(entry, expectedSnapshot(contextBundle))))) issues.push(error('$', 'basis', 'Publication metadata does not reference the supplied snapshot.'));
      break;
...
export function assertDiagramAuthoringArtifact(kind, value, options) {
  ...
  if (errors.length) {
    const failure = new TypeError(`Invalid ${kind}: ${errors.map((entry) => `${entry.path}: ${entry.rule}`).join('; ')}`);
    failure.errors = errors;
    throw failure;
  }
  return value;
}
export const validateDiagramDocument = (value, options) => validateDiagramAuthoringArtifact('diagram-document', value, options);
export const assertDiagramDocument = (value, options) => assertDiagramAuthoringArtifact('diagram-document', value, options);
export const validateDiagramPresentation = (value, options) => validateDiagramAuthoringArtifact('diagram-presentation', value, options);
export const assertDiagramPresentation = (value, options) => assertDiagramAuthoringArtifact('diagram-presentation', value, options);
export const validateDiagramAuthoringBundle = (value, options) => validateDiagramAuthoringArtifact('diagram-authoring-bundle', value, options);
export const assertDiagramAuthoringBundle = (value, options) => assertDiagramAuthoringArtifact('diagram-authoring-bundle', value, options);
export const validateDiagramEditTransaction = (value, options) => validateDiagramAuthoringArtifact('diagram-edit-transaction', value, options);
export const assertDiagramEditTransaction = (value, options) => assertDiagramAuthoringArtifact('diagram-edit-transaction', value, options);
...
export function inspectLegacyDiagramDocument(value) {
  ...
  if (!errors.length && value && typeof value === 'object' && (value.protocolVersion !== '1.6.0' || value.schemaVersion !== '1.0.0')) errors.push(error('$', 'unsupported-version', 'This inspector accepts only the frozen v1.6 diagram contract.'));
  if (!errors.length) errors = validateJson(value, LEGACY_DIAGRAM_DOCUMENT_SCHEMA).slice(0, 128).map(({ path, rule }) => error(path, rule, 'Value does not satisfy the legacy diagram contract.'));
  let grammar = null;
  if (!errors.length) {
    grammar = DIAGRAM_REGISTRIES['diagram-grammars.json'].grammars.find((entry) => entry.grammarId === value.grammar.id);
    if (!grammar) errors.push(error('$.grammar.id', 'grammar', 'Unknown legacy grammar.'));
    if (value.documentDigest !== diagramDocumentDigest(value)) errors.push(error('$.documentDigest', 'digest', 'Legacy semantic digest does not match.'));
    const elements = allElements(value);
    if (!elements.length && !value.emphasis.length) errors.push(error('$', 'visible-content', 'The legacy document contract requires content.'));
    if (elements.length + value.emphasis.length > 10000) errors.push(error('$', 'resource-limit', 'Too many legacy semantic primitive entries.'));
    const itemIds = new Set(elements.map((entry) => entry.id));
    if (itemIds.size !== elements.length) errors.push(error('$', 'duplicate-id', 'Legacy semantic IDs must be unique.'));
    for (const collection of ['relations', 'groups', 'lanes', 'sets', 'annotations', 'emphasis']) value[collection].forEach((entry, i) => {
      const refs = collection === 'relations' ? [entry.from, entry.to] : entry.members ?? (entry.targetId == null ? [] : [entry.targetId]);
      if (refs.some((ref) => !itemIds.has(ref))) errors.push(error(`$.${collection}[${i}]`, 'reference', 'Legacy semantic reference is missing.'));
    });
    value.accessibility.readingOrder.forEach((entry, i) => { if (!itemIds.has(entry)) errors.push(error(`$.accessibility.readingOrder[${i}]`, 'reference', 'Legacy reading order target is missing.')); });
    if (grammar) {
      if (!grammar.directions.includes(value.layout.direction)) errors.push(error('$.layout.direction', 'profile-direction', 'Direction is not supported by the legacy grammar.'));
      if (!['quadrant', 'scatter', 'wardley'].includes(grammar.grammarId) && value.nodes.some((entry) => entry.semanticPosition != null)) errors.push(error('$.nodes', 'semantic-position', 'Semantic position is not supported by this legacy grammar.'));
      const plural = { node: 'nodes', relation: 'relations', group: 'groups', lane: 'lanes', event: 'events', series: 'series', axis: 'axes', set: 'sets', annotation: 'annotations', emphasis: 'emphasis' };
      for (const [primitive, collection] of Object.entries(plural)) {
        if (value[collection].length && !grammar.allowedPrimitives.includes(primitive)) errors.push(error(`$.${collection}`, 'profile-primitive', 'Primitive is not allowed by this legacy grammar.'));
        if (!value[collection].length && grammar.requiredPrimitives.includes(primitive)) errors.push(error(`$.${collection}`, 'required-primitive', 'Required legacy primitive is missing.'));
      }
    }
  }
  const valid = errors.length === 0;
  const unsupportedNodeKinds = valid ? [...new Set(value.nodes.filter((entry) => !semanticKinds.includes(entry.kind)).map((entry) => entry.kind))] : [];
  const authoringAvailable = valid && getDiagramAuthoringCapability(value.grammar.id) !== null && !unsupportedNodeKinds.length && !value.relations.some((entry) => entry.kind === 'containment') && !['events', 'series', 'axes', 'sets'].some((key) => value[key].length) && !(value.grammar.id === 'flowchart' || value.grammar.id === 'architecture' ? value.lanes.length : false);
  return {
    valid, errors, diagramId: valid ? value.diagramId : null, grammarId: valid ? value.grammar.id : null,
    authoringAvailable, requiresDerivedPresentation: valid, unsupportedNodeKinds,
    adoption: 'explicit-save-required', sourceModified: false,
  };
}
