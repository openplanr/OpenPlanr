export interface WorkspaceCiphertext { iv: string; ciphertext: string }
export interface WorkspacePublicKey { kty: 'EC'; crv: 'P-256'; x: string; y: string; ext?: boolean; key_ops?: ['verify'] }
export interface WorkspaceRevision extends WorkspaceCiphertext {
  id: string; epoch: number; reviewOf: string; createdAt: string; signature: string;
}
export interface WorkspaceEvent extends WorkspaceCiphertext {
  id: string; revisionId: string; reviewOf: string; epoch: number;
  publicKey: WorkspacePublicKey; signature: string;
}
export interface DesignReviewWorkspace {
  schemaVersion: '1.0.0'; id: string; version: number; epoch: number;
  currentRevision: string; commentsPaused: boolean;
  ownerPublicKey: WorkspacePublicKey; keyring: WorkspaceCiphertext;
}
...
export declare const DESIGN_WORKSPACE_SCHEMAS: Readonly<Record<string, Readonly<Record<string, unknown>>>>;
export declare function assertWorkspaceContract<T>(value: T, contract: Readonly<Record<string, unknown>>): T;
