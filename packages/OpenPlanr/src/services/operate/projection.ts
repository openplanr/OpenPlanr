...
function bounded(value: string, maximumWords: number): string {
  const words = value.split(/\s+/).filter(Boolean);
  return words.length <= maximumWords ? value : `${words.slice(0, maximumWords).join(' ')}…`;
}

function evidence(record: Record<string, unknown>): string {
...
function rankedFindings(state: OperatingState): Array<Record<string, unknown>> {
  const severity = { critical: 4, high: 3, medium: 2, low: 1 };
  return [...state.findings]
    .filter((finding) => finding.parked !== true && finding.status !== 'rejected')
    .sort((left, right) => {
      const severityDelta =
        (severity[String(right.severity) as keyof typeof severity] ?? 0) -
        (severity[String(left.severity) as keyof typeof severity] ?? 0);
...
export function renderOperatingBrief(state: OperatingState): string {
  ...
  const activeDecisions = state.decisions.filter((decision) =>
  ...
  const decisions = activeDecisions
    .sort((left, right) => {
      const overdueDelta =
        Number(right.status === 'default-due') - Number(left.status === 'default-due');
      return (
        overdueDelta ||
  ...
  const lines = [
    ...
    ...(decisions.length > 0
      ? decisions.map(
          (decision) =>
            `- **${decision.id}:**${decision.status === 'default-due' ? ' **DEFAULT DUE — no action executed.**' : ''} ${bounded(text(decision, 'question', 'Decision required'), 32)}${(stalledById.get(decision.id) ?? 0) >= 2 ? ` Stalled: ${stalledById.get(decision.id)} closed cycles.` : ''} Recommendation: ${bounded(text(decision, 'recommendation', 'Review the available options.'), 28)} Evidence: ${evidence(decision)}.`,
        )
    ...
    ...(gaps.length > 0
      ? gaps.map(
          (gap) =>
            `- **${gap.id}:** ${bounded(text(gap, 'question', 'Evidence required'), 28)} Owner: ${text(gap, 'owner', 'unassigned')}. Evidence: ${evidence(gap)}.`,
        )
    ...
    ...(outcomes.length > 0
      ? outcomes.map(
          (outcome) =>
            `- **${outcome.id}:** ${bounded(text(outcome, 'metric', 'Tracked outcome'), 20)} — ${text(outcome, 'status', 'pending')}. Evidence: ${evidence(outcome)}.`,
        )
...
export function selectCycleState(state: OperatingState, cycleId?: string): OperatingState {
  ...
  return attachOperatingStalledItems(
    selected,
    sourceStalls.filter(
      (item) =>
        selected.findings.some((finding) => finding.id === item.id) ||
