...
export type DiagramJsonValue = null | boolean | number | string | DiagramJsonValue[] | { [key: string]: DiagramJsonValue };
...
export interface DiagramFieldChange {
  collection: 'nodes' | 'relations' | 'groups' | 'lanes' | 'annotations' | 'emphasis' | 'document' | 'elements' | 'presentation' | 'source-map';
...
export interface DiagramTransactionIdentity { transactionId: string }
...
export type DiagramCommand =
  ...
  | { type: 'geometry'; changes: Extract<DiagramEditOperation, { type: 'set-geometry' }>['changes'] }
  | { type: 'appearance'; changes: Extract<DiagramEditOperation, { type: 'set-appearance-locks' }>['changes'] }
  | { type: 'reparent'; ids: string[]; parentId: string | null; index?: number }
  | { type: 'group'; group: Pick<DiagramAuthoringContainer, 'id' | 'label'>; ids: string[]; placement: DiagramPlacement; parentId?: string | null }
  | { type: 'ungroup' | 'reorder-lanes'; ids: string[] }
  | { type: 'duplicate'; ids: string[]; idMap: Record<string, string>; dx?: number; dy?: number }
  | { type: 'paste'; sourceBundle: DiagramAuthoringBundle; ids: string[]; idMap: Record<string, string>; dx?: number; dy?: number }
  | { type: 'delete'; ids: string[]; confirmedImpact?: DiagramDeletionImpact }
  | { type: 'cancel' };
export type DiagramCommandResult = DiagramPreviewResult
...
export declare function compileDiagramCommand(bundle: DiagramAuthoringBundle, command: DiagramCommand, options: DiagramTransactionIdentity): DiagramCommandResult;
export declare function compileDiagramCommand(bundle: DiagramAuthoringBundle, command: { type: 'cancel' }, options?: DiagramTransactionIdentity): { ok: true; cancelled: true; transaction: null } | DiagramKernelFailure;

/** Validates inert content and final diagram invariants without modifying input. */
export declare function validateAuthoringBundle(bundle: unknown): { ok: true; diagnostics: [] } | DiagramKernelFailure;
/** Applies an exact-base transaction to an isolated preview; it does not persist it. */
export declare function previewDiagramTransaction(bundle: DiagramAuthoringBundle, transaction: DiagramEditTransaction): DiagramPreviewResult;
/** Compares identity-keyed content; derived digests are excluded from user changes. */
export declare function diffDiagramBundles(before: DiagramAuthoringBundle, after: DiagramAuthoringBundle):
