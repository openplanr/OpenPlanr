import { validateDiagramEditTransaction, diagramDocumentDigest } from '@openplanr/protocol/diagram-authoring-contracts';
import {
  clone, same, diagnostic, failure, validateAuthoringBundle, snapshot, elementIndex,
  semanticFields, geometryFields, appearanceFields, membershipState, sealBundle,
} from './model.mjs';
...
export function applyOperation(bundle, op, path, diagnostics) {
  ...
  if (op.type === 'insert-elements') {
    for (const { value } of op.elements) if (entries.has(value.id)) diagnostics.push(diagnostic(path, 'duplicate-id', `Element ${value.id} already exists.`));
    ...
    if (op.positions) {
      ...
      for (const collection of ['nodes', 'relations', 'groups', 'lanes', 'annotations']) {
        const inserted = op.elements.filter(entry => entry.collection === collection).sort((a, b) => positions.get(a.value.id).semanticIndex - positions.get(b.value.id).semanticIndex);
        let previous = -1;
        for (const { value } of inserted) {
          const index = positions.get(value.id).semanticIndex;
          if (index <= previous || index > document[collection].length) { diagnostics.push(diagnostic(path, 'insertion-position', `Invalid semantic insertion position for ${value.id}.`)); return; }
          document[collection].splice(index, 0, clone(value)); previous = index;
        }
      }
      let previous = -1;
      for (const value of [...op.presentation].sort((a, b) => positions.get(a.elementId).presentationIndex - positions.get(b.elementId).presentationIndex)) {
        ...
        if (index <= previous || index > bundle.presentation.elements.length) { diagnostics.push(diagnostic(path, 'insertion-position', `Invalid presentation insertion position for ${value.elementId}.`)); return; }
        bundle.presentation.elements.splice(index, 0, clone(value)); previous = index;
      }
    } else {
    ...
    for (const { collection, value } of op.elements) if (collection === 'lanes' && !document.laneOrder.includes(value.id)) document.laneOrder.push(value.id);
  } else if (op.type === 'remove-elements') {
    for (const { collection, value } of op.elements) {
      const actual = entries.get(value.id);
      if (!actual || actual.collection !== collection) { diagnostics.push(diagnostic(path, 'element-class', `Element ${value.id} does not belong to ${collection}.`)); return; }
    ...
    for (const collection of ['nodes', 'relations', 'groups', 'lanes', 'annotations']) document[collection] = document[collection].filter(value => !ids.has(value.id));
    bundle.presentation.elements = bundle.presentation.elements.filter(value => !ids.has(value.elementId));
    ...
    if (op.collection === 'source-map') {
      ...
      if (precondition(semanticFields('document', document), op.before, path, diagnostics)) Object.assign(document, clone(op.after));
    } else if (op.collection === 'emphasis') {
      const actual = document.emphasis.find(value => value.targetId === op.elementId)?.level ?? null;
      ...
      if (op.after === null) document.emphasis.splice(index, index < 0 ? 0 : 1);
        ...
        if (target > document.emphasis.length) { diagnostics.push(diagnostic(path, 'insertion-position', 'Emphasis insertion index is out of range.')); return; }
        document.emphasis.splice(target, 0, { targetId: op.elementId, level: op.after });
      }
    } else {
      const entry = entries.get(op.elementId);
      if (!entry || entry.collection !== op.collection) { diagnostics.push(diagnostic(path, 'element-class', `Element ${op.elementId} does not belong to ${op.collection}.`)); return; }
      if (precondition(semanticFields(op.collection, entry.value), op.before, path, diagnostics)) Object.assign(entry.value, clone(op.after));
    }
    ...
    for (const collection of ['groups', 'lanes']) {
      if (!same(document[collection].map(value => value.id).sort(), op.after[collection].map(value => value.id).sort())) { diagnostics.push(diagnostic(path, 'containment-reference', 'Membership must include exactly the existing containers.')); return; }
      for (const value of op.after[collection]) entries.get(value.id).value.members = clone(value.members);
    }
    ...
    for (const change of op.changes) {
      ...
      if (!placement) { diagnostics.push(diagnostic(path, 'reference', `Missing placement ${change.elementId}.`)); return; }
      if (!precondition(geometry ? geometryFields(placement) : appearanceFields(placement), change.before, `${path}.${change.elementId}`, diagnostics)) return;
      if (geometry) {
        const oldBounds = placement.bounds;
        const newBounds = change.after.bounds;
        const positionChanged = !same(oldBounds && { x: oldBounds.x, y: oldBounds.y }, newBounds && { x: newBounds.x, y: newBounds.y }) || !same(placement.label, change.after.label) || placement.zIndex !== change.after.zIndex;
        const sizeChanged = !same(oldBounds && { width: oldBounds.width, height: oldBounds.height }, newBounds && { width: newBounds.width, height: newBounds.height });
        if ((placement.locks.position && positionChanged) || (placement.locks.size && sizeChanged) || (placement.locks.route && !same(placement.route, change.after.route))) {
          diagnostics.push(diagnostic(`${path}.${change.elementId}`, 'geometry-lock', `Geometry is locked for ${change.elementId}; explicitly unlock it first.`)); return;
        }
      }
...
function containmentIssues(before, after) {
  ...
  for (const parent of [...after.document.groups, ...after.document.lanes]) {
    ...
    const childrenChanged = parent.members.some(id => !same(oldPlacements.get(id)?.bounds, placements.get(id)?.bounds));
    if (!bounds || (old && same(old.members, parent.members) && same(oldBounds, bounds) && !childrenChanged)) continue;
    for (const id of parent.members) {
      const child = placements.get(id)?.bounds;
      if (child && (child.x < bounds.x || child.y < bounds.y || child.x + child.width > bounds.x + bounds.width || child.y + child.height > bounds.y + bounds.height)) diagnostics.push(diagnostic(`$.presentation.${parent.id}`, 'container-bounds', `Container ${parent.id} must contain child ${id}.`));
    }
  }
  return diagnostics;
}
function attachmentPoint(bounds, attachment) {
  if (attachment.side === 'left' || attachment.side === 'right') return { x: bounds.x + (attachment.side === 'right' ? bounds.width : 0), y: bounds.y + bounds.height * attachment.offset };
  return { x: bounds.x + bounds.width * attachment.offset, y: bounds.y + (attachment.side === 'bottom' ? bounds.height : 0) };
}
function resolveIncidentRoutes(before, after) {
  ...
  for (const relation of after.document.relations) {
    ...
    if (!from || !to) continue;
    if (old && old.from === relation.from && old.to === relation.to && same(oldPlacements.get(old.from)?.bounds, from) && same(oldPlacements.get(old.to)?.bounds, to) && same(oldPlacements.get(relation.id)?.route, placement.route)) continue;
    ...
    if (route.strategy === 'straight') route.points = [start, end];
      ...
      for (let index = 1; index < points.length; index++) {
        ...
        if (previous.x !== next.x && previous.y !== next.y) {
          ...
          const horizontal = index === 1 ? ['left', 'right'].includes(route.from.side) : !['left', 'right'].includes(route.to.side);
    ...
    if (!same(geometry, geometryFields(placement))) changes.push({ elementId: relation.id, before: geometryFields(placement), after: geometry });
  }
  return changes;
}
...
export function previewDiagramTransaction(bundle, transaction) {
  ...
  let diagnostics;
  try { diagnostics = validateDiagramEditTransaction(transaction); } catch { return failure('$', 'plain-data', 'Transaction could not be inspected as inert JSON data.'); }
  if (diagnostics.length) return { ok: false, diagnostics };
  if (transaction.diagramId !== bundle.diagramId || !same(transaction.base, snapshot(bundle))) return failure('$.base', 'stale-base', 'The transaction requires the exact current diagram snapshot.');
  const originalIds = elementIndex(bundle.document);
  for (const op of transaction.operations) if (op.type === 'insert-elements' && op.elements.some(entry => originalIds.has(entry.value.id))) return failure('$.operations', 'duplicate-id', 'Inserted identities must be fresh even when the same transaction removes an existing element.');
  ...
  if (bundle.sourceMap && !transaction.operations.some(op => op.type === 'update-semantics' && op.collection === 'source-map')) {
  ...
  if (next.sourceMap && transaction.operations.some(op => op.type === 'update-semantics' && op.collection === 'source-map') && next.sourceMap.semanticDigest !== diagramDocumentDigest(next.document)) return failure('$.sourceMap.semanticDigest', 'basis', 'The updated source map must identify the resulting semantic document.');
  ...
  for (const [id, entry] of elementIndex(bundle.document)) if (!finalIds.has(id)) positions.push({ elementId: id, semanticIndex: bundle.document[entry.collection].findIndex(value => value.id === id), presentationIndex: bundle.presentation.elements.findIndex(value => value.elementId === id) });
  return { ok: true, bundle: sealed, transaction: canonicalTransaction,
    diff: { semantic: diff.semantic, presentation: diff.presentation }, impact: diff.impact,
    inverse: { diagramId: bundle.diagramId, transactionId: transaction.transactionId,
      ...
      dependencies: inverseDependencies(sealed, diff.impact), positions, emphasisOrder: bundle.document.emphasis.map(value => value.targetId) } };
}
