import { clone, same, failure, inspectPlainData, validateAuthoringBundle, elementIndex, parentIndex, descendants, geometryFields, snapshot } from './model.mjs';
import { previewDiagramTransaction } from './transactions.mjs';

const inside = (bounds, frame) => !frame || (bounds.x >= frame.x && bounds.y >= frame.y && bounds.x + bounds.width <= frame.x + frame.width && bounds.y + bounds.height <= frame.y + frame.height);
...
function check(bundle, options, keys) {
  ...
  if (options.targetIds.some(id => typeof id !== 'string' || !byId.has(id))) return failure('$.options.targetIds', 'reference', 'Every layout target must exist in this diagram.');
  return { ok: true };
}
function preview(bundle, changes, transactionId) {
  ...
  return previewDiagramTransaction(bundle, { kind: 'diagram-edit-transaction', schemaVersion: '1.0.0', protocolVersion: '1.13.0', diagramId: bundle.diagramId, transactionId, base: snapshot(bundle), undoOf: null, operations: [{ type: 'set-geometry', changes }] });
}
function translate(geometry, dx, dy) {
  const after = clone(geometry);
  if (after.bounds) { after.bounds.x += dx; after.bounds.y += dy; }
  if (after.route?.mode === 'manual') after.route.points = after.route.points.map(point => ({ x: point.x + dx, y: point.y + dy }));
  if (after.label) { after.label.x += dx; after.label.y += dy; }
  return after;
}
...
export function previewAutomaticLayout(bundle, options) {
  ...
  if (!Number.isFinite(gap) || gap < 8 || gap > 512 || (options.columns !== undefined && (!Number.isInteger(options.columns) || options.columns < 1 || options.columns > 32))) return failure('$.options', 'layout-options', 'Gap must be 8–512 canvas units; columns must be an integer from 1 to 32.');
  ...
  if (options.targetIds.some(id => byId.get(id).collection === 'relations')) return failure('$.options.targetIds', 'layout-target', 'Select shapes or containers for layout; resetting a connector route is a separate explicit operation.');
  const selected = new Set(options.targetIds);
  const roots = options.targetIds.filter(id => { let parent = parents.get(id); while (parent) { if (selected.has(parent)) return false; parent = parents.get(parent); } return true; });
  const lockedIds = [];
  const movable = [];
  const closureById = new Map();
  for (const id of roots) {
    const closure = descendants(bundle.document, [id]); closureById.set(id, closure);
  ...
  for (const id of movable) { const parent = parents.get(id) ?? null; if (!groups.has(parent)) groups.set(parent, []); groups.get(parent).push(id); }
  ...
  for (const [parentId, ids] of groups) {
    ...
    for (const id of ids) { let parent = parents.get(id); while (parent) { ancestors.add(parent); parent = parents.get(parent); } }
    ...
    const columns = Math.min(options.columns ?? Math.max(1, Math.ceil(Math.sqrt(ids.length))), availableColumns);
    const origin = { x: Math.min(...ids.map(id => placements.get(id).bounds.x)), y: Math.min(...ids.map(id => placements.get(id).bounds.y)) };
    if (frame) { origin.x = Math.max(frame.x, Math.min(origin.x, frame.x + frame.width - width)); origin.y = Math.max(frame.y, Math.min(origin.y, frame.y + frame.height - height)); }
    const ordered = ['right-left', 'bottom-up'].includes(bundle.presentation.layout.direction) ? [...ids].reverse() : ids;
    let slot = 0;
    ...
    for (const id of ordered) {
      ...
      while (slot < maximumSlots) {
        const column = slot % columns, row = Math.floor(slot / columns); slot++;
        ...
        if (collisionChecks > 250000) return failure('$.options.targetIds', 'focused-region-required', 'This layout exceeds the bounded obstacle-search budget; select a smaller region.');
        if (!inside(candidate, frame) || [...obstacles, ...occupied].some(bounds => overlaps(candidate, bounds, gap / 2))) continue;
        proposed = candidate; break;
      }
      if (!proposed) return failure(`$.presentation.${id}`, 'layout-space', `The selected region has insufficient space around fixed objects for ${id}; expand its container or select a smaller region.`);
      occupied.push(proposed);
      for (const member of closureById.get(id)) deltas.set(member, { dx: proposed.x - old.x, dy: proposed.y - old.y });
    }
  }
  const changes = [];
  for (const [id, delta] of deltas) {
    const before = geometryFields(placements.get(id)), after = translate(before, delta.dx, delta.dy);
    if (!same(before, after)) changes.push({ elementId: id, before, after });
  }
  for (const relation of bundle.document.relations) {
    const from = deltas.get(relation.from), to = deltas.get(relation.to);
    if (!from || !to || !same(from, to)) continue;
    const before = geometryFields(placements.get(relation.id)), after = translate(before, from.dx, from.dy);
...
export function previewResetRoute(bundle, options) {
  ...
  if (!checked.ok) return checked;
  const byId = elementIndex(bundle.document);
  if (options.targetIds.some(id => byId.get(id).collection !== 'relations')) return failure('$.options.targetIds', 'route-target', 'Reset route selects connectors only.');
  const placements = new Map(bundle.presentation.elements.map(value => [value.elementId, value]));
  const changes = options.targetIds.flatMap(elementId => {
    const before = geometryFields(placements.get(elementId));
    const after = clone(before); after.route.mode = 'automatic'; after.route.points = [];
