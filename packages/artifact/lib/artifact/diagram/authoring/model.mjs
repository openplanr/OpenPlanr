import {
  validateDiagramAuthoringBundle, diagramDocumentDigest,
  diagramPresentationDigest, diagramAuthoringBundleDigest, DIAGRAM_AUTHORING_LIMITS,
...
export const failure = (path, rule, detail) => ({ ok: false, diagnostics: [diagnostic(path, rule, detail)] });
...
export function inspectPlainData(value, allowedKeyPaths = []) {
  ...
  let text = 0;
  let issue;
  const reject = (path, rule, detail) => { issue ??= diagnostic(path, rule, detail); };
  function visit(current, path, depth) {
    ...
    if (typeof current === 'string') {
      text += current.length;
      if (text > DIAGRAM_AUTHORING_LIMITS.textCodeUnits) reject(path, 'resource-limit', 'Input exceeds the text limit.');
      return;
    }
    if (current === null || typeof current === 'boolean') return;
    if (typeof current === 'number') { if (!Number.isFinite(current)) reject(path, 'finite-number', 'Numbers must be finite.'); return; }
    ...
    for (const key of Reflect.ownKeys(descriptors)) {
      if (typeof key !== 'string' || key === '__proto__' || (['constructor', 'prototype'].includes(key) && !allowedKeyPaths.includes(path))) { reject(path, 'unsafe-key', 'Unsafe object key.'); break; }
      const descriptor = descriptors[key];
      if (!Object.hasOwn(descriptor, 'value')) { reject(path, 'accessor', 'Accessors are not accepted.'); break; }
      if (Array.isArray(current) && key === 'length') continue;
      if (!descriptor.enumerable || (Array.isArray(current) && !/^(0|[1-9][0-9]*)$/u.test(key))) { reject(path, 'plain-data', 'Non-JSON properties are not accepted.'); break; }
  ...
  try { visit(value, '$', 0); } catch { reject('$', 'plain-data', 'Input could not be inspected as inert JSON data.'); }
  return issue ? [issue] : [];
}
...
export function validateAuthoringBundle(bundle) {
  ...
  if (!diagnostics.length) {
    try { diagnostics = validateDiagramAuthoringBundle(bundle); } catch { diagnostics = [diagnostic('$', 'plain-data', 'Bundle could not be inspected as inert JSON data.')]; }
  }
...
export const elementIndex = document => new Map(COLLECTIONS.flatMap(collection => document[collection].map(value => [value.id, { collection, value }])));
export const parentIndex = document => new Map([...document.groups, ...document.lanes].flatMap(value => value.members.map(id => [id, value.id])));
export function descendants(document, ids) {
  const byId = elementIndex(document);
  const result = new Set(ids);
  const pending = [...ids];
  while (pending.length) for (const member of byId.get(pending.pop())?.value.members ?? []) if (!result.has(member)) { result.add(member); pending.push(member); }
  return [...result];
}
...
export const geometryFields = ({ bounds, route, label, zIndex }) => clone({ bounds, route, label, zIndex });
export const appearanceFields = ({ appearance, locks }) => clone({ appearance, locks });
export const membershipState = document => clone({ groups: document.groups.map(({ id, members }) => ({ id, members })), lanes: document.lanes.map(({ id, members }) => ({ id, members })), laneOrder: document.laneOrder });
