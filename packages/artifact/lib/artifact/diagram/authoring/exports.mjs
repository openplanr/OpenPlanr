...
async function workspace(root) {
  if (typeof root !== 'string' || !root || root.includes('\0'))
    abort('unsafe-path', 'An existing workspace root is required.');
  const path = resolve(root),
    status = await info(path);
  if (!status?.isDirectory() || status.isSymbolicLink())
    abort('unsafe-path', 'Workspace root must be an existing real directory.');
...
async function directory(root, relativePath, create = false) {
  ...
  if (
    ...
    abort('unsafe-path', 'Export paths must remain below their workspace.');
  let path = root;
  for (const part of segments) {
    ...
    if (status.isSymbolicLink() || !status.isDirectory())
      abort('unsafe-path', 'Every export directory must be a real directory.');
  }
  return path;
}
async function regularBytes(path) {
  ...
  try {
    ...
    if (!status.isFile() || status.nlink !== 1 || status.size > MAX_FILE_BYTES)
      abort('unsafe-member', 'Export members must be bounded regular files without links.');
...
function prepare(bundle, options) {
  ...
  if (
    ...
    Object.keys(options).some((key) => !['root', 'slug', 'rasterize'].includes(key))
  )
    return fail(
      'invalid-options',
      'Only root, slug and an optional rasterize adapter are accepted.',
    );
  if (
  ...
  if (options.rasterize !== undefined && typeof options.rasterize !== 'function')
    return fail('invalid-options', 'The rasterize adapter must be a function.');
  ...
  if (
    ...
    return fail('unsafe-path', 'The export slug must be a bounded lowercase identifier.');
...
function fidelity(bundle, targetFormat) {
  return {
    ...
    losses: [
      {
        dimension: 'semantic',
        code: 'visual-snapshot',
        elementIds: [],
        message: 'This visual output does not replace the editable bundle snapshot.',
      },
      {
        dimension: 'sourceText',
        code: 'source-in-bundle-only',
        elementIds: [],
        message:
          'Original source and correspondence remain in the separate immutable bundle snapshot.',
      },
    ],
  };
}
function reportSection(bundle, rendered) {
  const report = {
    ...
    theme: { id: rendered.theme.id, version: rendered.theme.version },
    quality: rendered.quality,
    canvas: { width: rendered.scene.width, height: rendered.scene.height },
    editableSource: 'snapshot.planr-diagram-bundle.json',
  };
...
function reviewHtml(bundle, rendered) {
  return renderDiagramHtml(
    { ...bundle.document, layout: bundle.presentation.layout },
    rendered.svg,
...
async function verifyDirectory(root, bundle, prepared) {
  ...
  if (!target)
    return fail('exports-missing', 'No complete export set exists for this snapshot and renderer.');
  const manifestBytes = await regularBytes(join(target, 'manifest.json'));
  let manifest;
  try {
    manifest = JSON.parse(manifestBytes.toString('utf8'));
  } catch {
    return fail('invalid-manifest', 'Export manifest is not valid JSON.');
  }
  ...
  if (
    !sameVersion(manifest.renderer, prepared.rendered.renderer) ||
    !sameVersion(manifest.theme, prepared.theme)
  )
    return fail(
      'stale-renderer',
      'Export renderer and theme must match the current supported renderer.',
    );
  ...
  if (
    manifest.bundle.path !== `${prefix}snapshot.planr-diagram-bundle.json` ||
    manifest.bundle.transportDigest !== digestBytes(prepared.bundleBytes)
  )
    return fail(
      'snapshot-mismatch',
      'The export manifest must identify these exact immutable bundle bytes.',
    );
  const source = await regularBytes(join(target, 'snapshot.planr-diagram-bundle.json'));
  if (!source.equals(prepared.bundleBytes))
    return fail(
      'snapshot-mismatch',
      'The immutable export source does not match the requested bundle.',
    );
  if (manifest.outputs.length !== expectedOutputs.size)
    return fail(
      'incomplete-exports',
      'The export manifest must name the complete visual output set.',
    );
  const names = new Set(['manifest.json', 'snapshot.planr-diagram-bundle.json']);
  for (const output of manifest.outputs) {
    const name = basename(output.path);
    if (
      output.path !== `${prefix}${name}` ||
      !expectedOutputs.has(name) ||
      expectedOutputs.get(name) !== output.mediaType ||
      names.has(name)
    )
      return fail(
        'unexpected-output',
        'The manifest contains an unexpected or substituted output member.',
      );
    names.add(name);
    const bytes = await regularBytes(join(target, name));
    if (name === 'diagram.svg' && !bytes.equals(Buffer.from(prepared.rendered.svg)))
      return fail('stale-output', 'SVG content does not match the current authored renderer.');
    if (name === 'review.html' && !bytes.equals(Buffer.from(reviewHtml(bundle, prepared.rendered))))
      return fail(
        'stale-output',
        'Review HTML does not match the current authored scene and quality report.',
      );
    if (name === 'diagram.png') {
      const dimensions = inspectDiagramPng(bytes);
      if (
        dimensions.width !== Math.ceil(prepared.rendered.scene.width) ||
        dimensions.height !== Math.ceil(prepared.rendered.scene.height)
      )
        return fail('raster-dimensions', 'PNG dimensions do not match the authored scene.');
    }
    if (digestBytes(bytes) !== output.transportDigest)
      return fail('output-mismatch', `The ${name} bytes do not match the export manifest.`);
  }
  if ((await readdir(target)).some((name) => !names.has(name)))
    return fail('unowned-collision', 'The export directory contains unowned files.');
...
export async function exportAuthoredDiagram(bundle, options) {
  try {
    ...
    return fail(
      'export-cleanup-pending',
      'Export cleanup could not be confirmed. Verify this snapshot output and recover its inactive lock or staging directory before retrying.',
    );
  }
}
async function stageAuthoredDiagramExports(bundle, options) {
  ...
  try {
    ...
    const raster = await (options.rasterize ?? renderDiagramPng)(rendered.svg, {
      scale: 1,
      theme: rendered.theme,
    });
    ...
    if (
      dimensions.width !== Math.ceil(rendered.scene.width) ||
      dimensions.height !== Math.ceil(rendered.scene.height)
    )
      return fail(
        'raster-dimensions',
        'PNG dimensions do not match the authored scene at its declared scale.',
      );
    const files = new Map([
      ['snapshot.planr-diagram-bundle.json', prepared.bundleBytes],
      ['diagram.svg', Buffer.from(rendered.svg)],
      ['diagram.png', png],
    ...
    const parentRelative = prepared.relativeDirectory.slice(
      0,
      prepared.relativeDirectory.lastIndexOf('/'),
    );
    ...
    for (const [file, bytes] of files) {
      ...
      if (!(await regularBytes(join(stage, file))).equals(bytes))
        abort('staging-mismatch', 'Staged output bytes changed before promotion.');
    }
    await flushDirectory(stage);
    await directory(root, parentRelative); // Recheck containment immediately before promotion.
    if (await info(join(parent, name)))
      abort('unowned-collision', 'The immutable output target appeared during rendering.');
    ...
    return fail(
      ...
      error.exportCode
        ? error.message
        : 'Export did not complete; canonical content and prior complete exports were preserved.',
    );
  } finally {
    let cleanupError;
    if (stage) {
