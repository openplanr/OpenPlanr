import {
  COLLECTIONS, clone, same, diagnostic, failure, inspectPlainData, validateAuthoringBundle,
  snapshot, elementIndex, semanticFields, geometryFields, appearanceFields, membershipState, sealBundle,
} from './model.mjs';
...
function restoreChange(bundle, change, positions, emphasisOrder) {
  if (!change.path.length) {
    if (change.collection === 'source-map') { bundle.sourceMap = clone(change.before); return; }
    ...
    if (change.before !== null) {
      ...
      collection.splice(requested === undefined || requested < 0 ? collection.length : Math.min(requested, collection.length), 0, clone(change.before));
    }
    return;
  }
...
function validateInverse(value) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value) || typeof value.diagramId !== 'string' || typeof value.transactionId !== 'string' || !value.changes || !Array.isArray(value.dependencies) || !Array.isArray(value.positions) || !Array.isArray(value.emphasisOrder)) return fail('Expected conditional inverse data from a successful preview.');
  if (Object.keys(value).some(key => !['diagramId', 'transactionId', 'changes', 'dependencies', 'positions', 'emphasisOrder'].includes(key))) return fail('Unknown inverse field.');
  for (const name of ['semantic', 'presentation', 'sourceMap']) {
    ...
    for (const change of value.changes[name]) if (!change || typeof change.collection !== 'string' || ![...COLLECTIONS, 'document', 'emphasis', 'elements', 'presentation', 'source-map'].includes(change.collection) || (change.elementId !== null && typeof change.elementId !== 'string') || !Array.isArray(change.path) || change.path.some(key => typeof key !== 'string' || ['__proto__', 'constructor', 'prototype'].includes(key)) || !Object.hasOwn(change, 'before') || !Object.hasOwn(change, 'after')) return fail('Malformed inverse change.');
  }
  if (value.dependencies.some(item => !item || typeof item.elementId !== 'string' || !Object.hasOwn(item, 'value'))) return fail('Malformed inverse dependency.');
  if (value.positions.some(item => !item || typeof item.elementId !== 'string' || !Number.isInteger(item.semanticIndex) || item.semanticIndex < 0 || !Number.isInteger(item.presentationIndex) || item.presentationIndex < 0)) return fail('Malformed inverse insertion position.');
  return [];
}
...
function compileCompensation(current, target, inverse, transactionId) {
  ...
  const operations = [];
  const diagnostics = [];
  const add = operation => { operations.push(operation); applyOperation(working, operation, '$.inverse', diagnostics); };
  ...
  const unlock = current.presentation.elements.flatMap(placement => {
    ...
    if (!wanted || same(geometryFields(placement), geometryFields(wanted)) || !Object.values(placement.locks).some(Boolean)) return [];
    return [{ elementId: placement.elementId, before: appearanceFields(placement), after: { appearance: clone(placement.appearance), locks: { position: false, size: false, route: false } } }];
  });
  if (unlock.length) add({ type: 'set-appearance-locks', changes: unlock });
  const removed = [...oldEntries].filter(([id]) => !targetEntries.has(id));
  if (removed.length) add({ type: 'remove-elements', elements: removed.map(([, entry]) => clone(entry)), presentation: working.presentation.elements.filter(value => removed.some(([id]) => id === value.elementId)).map(clone) });
  const inserted = [...targetEntries].filter(([id]) => !oldEntries.has(id));
  if (inserted.length) add({ type: 'insert-elements', elements: inserted.map(([, entry]) => ({ collection: entry.collection, value: { ...clone(entry.value), ...(entry.value.members ? { members: [] } : {}) } })), presentation: inserted.map(([id]) => clone(targetPlacements.get(id))), positions: inserted.map(([id, entry]) => ({ elementId: id, semanticIndex: target.document[entry.collection].findIndex(value => value.id === id), presentationIndex: target.presentation.elements.findIndex(value => value.elementId === id) })) });
  ...
  for (const [id, entry] of targetEntries) {
    ...
    if (!same(semanticFields(entry.collection, currentEntry.value), semanticFields(entry.collection, entry.value))) add({ type: 'update-semantics', collection: entry.collection, elementId: id, before: semanticFields(entry.collection, currentEntry.value), after: semanticFields(entry.collection, entry.value) });
  }
  if (!same(membershipState(working.document), membershipState(target.document))) add({ type: 'set-membership-order', before: membershipState(working.document), after: membershipState(target.document) });
  if (!same(semanticFields('document', working.document), semanticFields('document', target.document))) add({ type: 'update-semantics', collection: 'document', before: semanticFields('document', working.document), after: semanticFields('document', target.document) });
  ...
  for (const id of emphasisIds) {
    ...
    if (before !== after) add({ type: 'update-semantics', collection: 'emphasis', elementId: id, before, after, ...(before === null && after !== null ? { index: target.document.emphasis.findIndex(value => value.targetId === id) } : {}) });
  }
  const geometry = [];
  const appearance = [];
  for (const placement of working.presentation.elements) {
    const wanted = targetPlacements.get(placement.elementId);
    if (!same(geometryFields(placement), geometryFields(wanted))) geometry.push({ elementId: placement.elementId, before: geometryFields(placement), after: geometryFields(wanted) });
    if (!same(appearanceFields(placement), appearanceFields(wanted))) appearance.push({ elementId: placement.elementId, before: appearanceFields(placement), after: appearanceFields(wanted) });
  }
  ...
  if (!same(working.sourceMap, target.sourceMap)) add({ type: 'update-semantics', collection: 'source-map', before: clone(working.sourceMap), after: clone(target.sourceMap) });
  if (diagnostics.length) return { ok: false, diagnostics };
  if (!operations.length) return failure('$.inverse', 'no-change', 'This inverse has no remaining content change.');
  return { ok: true, transaction: { kind: 'diagram-edit-transaction', schemaVersion: '1.0.0', protocolVersion: '1.13.0', diagramId: current.diagramId, transactionId, base: snapshot(current), operations, undoOf: inverse.transactionId } };
}
...
export function createConditionalInverse(current, inverse, options) {
  ...
  for (const change of changes) if (!same(atPath(record(current, change), change.path), change.after)) diagnostics.push(diagnostic(`$.inverse.${change.elementId ?? change.collection}`, 'inverse-conflict', 'An affected field changed after the original transaction.'));
  ...
  if (!same(actual, inverse.dependencies)) diagnostics.push(diagnostic('$.inverse.dependencies', 'inverse-conflict', 'An affected dependency changed after the original transaction.'));
  if (diagnostics.length) return { ok: false, diagnostics };
  ...
  try {
    ...
    for (const change of structural.filter(change => change.before === null)) restoreChange(target, change, inverse.positions, inverse.emphasisOrder);
  ...
  } catch { return failure('$.inverse', 'inverse-shape', 'Inverse fields cannot be restored in the current diagram.'); }
  let sealed;
  try { sealed = sealBundle(target); } catch { return failure('$.inverse', 'inverse-shape', 'Inverse fields do not describe a complete diagram.'); }
  ...
  if (!same(preview.bundle, sealed)) return failure('$.inverse', 'inverse-conflict', 'Compensation would alter content beyond the guarded inverse.');
