...
export const AUTHORED_DIAGRAM_RENDERER = Object.freeze({ id: 'openplanr-authored-svg', version: '1.0.0' });
const palettes = {
...
function attributes(element, theme, fill = theme.fills[element.appearance.fill]) {
  ...
  const dash = appearance.strokeStyle === 'dashed' ? ' stroke-dasharray="7 5"' : appearance.strokeStyle === 'dotted' ? ' stroke-dasharray="2 4" stroke-linecap="round"' : '';
...
function shape(element, theme) {
  ...
  switch (element.appearance.shape) {
    case 'text': return '';
    case 'ellipse': return `<ellipse cx="${n(x + width / 2)}" cy="${n(y + height / 2)}" rx="${n(width / 2)}" ry="${n(height / 2)}" ${common}/>`;
    case 'diamond': return `<path d="M ${n(x + width / 2)} ${n(y)} L ${n(x + width)} ${n(y + height / 2)} L ${n(x + width / 2)} ${n(y + height)} L ${n(x)} ${n(y + height / 2)} Z" ${common}/>`;
    case 'cylinder': {
    ...
    default: return `<rect x="${n(x)}" y="${n(y)}" width="${n(width)}" height="${n(height)}" rx="${element.appearance.shape === 'rounded-rectangle' ? n(Math.min(14, width / 2, height / 2)) : 0}" ${common}/>`;
  }
}
...
export function renderAuthoredSceneElement(element, theme, diagramId) {
  ...
  const markerDefinition = element.direction === 'none' ? '' : `<defs><marker id="${marker}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 Z" fill="${color}"/></marker></defs>`;
  const start = element.direction === 'both' ? ` marker-start="url(#${marker})"` : '';
  const end = element.direction !== 'none' ? ` marker-end="url(#${marker})"` : '';
  const dash = element.appearance.strokeStyle === 'dashed' ? ' stroke-dasharray="7 5"' : element.appearance.strokeStyle === 'dotted' ? ' stroke-dasharray="2 4" stroke-linecap="round"' : '';
  const path = element.points.map((point, index) => `${index === 0 ? 'M' : 'L'} ${svgNumber(point.x)} ${svgNumber(point.y)}`).join(' ');
...
export function renderAuthoredDiagramSvg(bundle, options = {}) {
  ...
  if (!options || typeof options !== 'object' || Array.isArray(options) || Object.keys(options).some(key => key !== 'theme') || (options.theme !== undefined && !Object.hasOwn(palettes, options.theme))) return { ok: false, code: 'invalid-options', diagnostics: [{ path: '$.options', rule: 'theme', detail: 'Choose the paper, slate or midnight theme.' }] };
