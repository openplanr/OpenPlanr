...
const hex = (digest) => {
  if (typeof digest !== 'string' || !DIGEST.test(digest))
    fail('IDENTITY', 'Expected a SHA-256 content identity.');
  return digest.slice(7);
};
const identifier = (value) => {
  if (typeof value !== 'string' || value.length > 128 || !ID.test(value))
    fail('IDENTITY', 'Expected a bounded lowercase identifier.');
  return value;
};
...
export function createDiagramAuthoringStore({
  ...
  if (
    ...
    fail('CAPACITY', 'Bundle capacity must be between 1 byte and 128 MiB.');
  if (faultInjector !== undefined && typeof faultInjector !== 'function')
    fail('OPTIONS', 'faultInjector must be a function.');
  ...
  const receiptPath = (transactionId) =>
    join(receipts, `${digestBytes(identifier(transactionId)).slice(7)}.json`);
  ...
  async function inspect(
    path,
    { missing = false, directory: wantDirectory = false, allowLinks = false } = {},
  ) {
    ...
    for (let index = 0; index < parts.length; index++) {
      ...
      if (info.isSymbolicLink() || (!last || wantDirectory ? !info.isDirectory() : !info.isFile()))
        fail('PATH', 'Store paths must contain real directories and regular files.', {
          path: cursor,
        });
      if (last && !wantDirectory && !allowLinks && info.nlink !== 1)
        fail('PATH', 'Hard-linked store files are not accepted.', { path: cursor });
      if (last) return info;
    }
  ...
  async function ensureDirectory(path) {
    ...
    if (child === '..' || child.startsWith(`..${sep}`))
      fail('PATH', 'Store directory escapes its workspace.');
    let cursor = base;
  ...
  async function readBytes(path, limit = metadataLimit, optional = false, allowLinks = false) {
    ...
    if (before.size > limit)
      fail('CAPACITY', 'Stored content exceeds its bounded capacity.', { path });
    const handle = await open(path, constants.O_RDONLY | NOFOLLOW);
    try {
      ...
      if (
        ...
        (!allowLinks && opened.nlink !== 1)
      )
        fail('PATH', 'Store member changed while opening it.', { path });
  ...
  async function readJson(path, optional = false, limit = metadataLimit, allowLinks = false) {
    ...
    try {
      ...
      fail('CORRUPT', 'Store metadata is not valid JSON.', { path });
    }
  }
  ...
  async function temporary(path, bytes) {
    ...
    if (
      digestBytes(await readBytes(target, Math.max(metadataLimit, bytes.length))) !==
      digestBytes(bytes)
    )
      fail('CORRUPT', 'Temporary write did not preserve exact bytes.');
    return target;
  }
  ...
  async function immutable(path, bytes, { allowExisting = true } = {}) {
    ...
    try {
      ...
      if (raced) {
        if (!allowExisting || !raced.equals(bytes))
          fail('COLLISION', 'Existing immutable content changed before publication.', { path });
  ...
  async function acquire() {
    ...
    let reclaimed = false;
    try {
      if (await inspect(reclaimPath, { missing: true }))
        fail('LOCKED', 'Lock recovery is already in progress.');
      try {
        ...
        if (
          ...
          fail('LOCKED', 'Lock owner cannot be safely identified.');
        try {
        ...
        if (current && current.token !== previous.token)
          fail('LOCKED', 'Lock ownership changed during recovery.');
  ...
  function validBundle(bundle) {
    ...
    if (!validity.ok)
      fail('INVALID', 'The complete bundle is invalid.', { diagnostics: validity.diagnostics });
    ...
    if (bytes.length > maxBundleBytes)
      fail('CAPACITY', 'The bundle exceeds the configured storage quota.');
    return bytes;
  }
  ...
  async function owned({ initialize = false } = {}) {
    ...
    if (owner) {
      if (
        owner.kind !== 'diagram-authoring-store' ||
        owner.version !== 1 ||
        owner.diagramId !== slug
      )
        fail('COLLISION', 'Store ownership is not recognized.');
      await inspect(snapshots, { directory: true });
      await inspect(receipts, { directory: true });
      return true;
    }
    if (await inspect(canonical, { missing: true }))
      fail('COLLISION', 'An unowned canonical bundle already exists.');
    ...
    if (incomplete) {
      ...
      for (const name of await readdir(metadata)) {
        if (!['snapshots', 'receipts'].includes(name))
          fail('COLLISION', 'Unowned or incomplete store metadata needs explicit inspection.');
        await inspect(join(metadata, name), { directory: true });
        if ((await readdir(join(metadata, name))).length)
          fail('COLLISION', 'Unowned store history already exists.');
      }
    }
    ...
    if (
      present &&
      (await readdir(directory)).filter((name) => !incomplete || name !== '.authoring').length
    ) {
      ...
      const changes = legacy?.generatedChanges.filter(
        (change) =>
          !(
            incomplete &&
            change.reason === 'unowned' &&
    ...
    await immutable(
      ownerPath,
      jsonBytes({ kind: 'diagram-authoring-store', version: 1, diagramId: slug }),
    );
    return true;
  }
  ...
  async function loadSnapshot(byteDigest) {
    ...
    try {
      ...
      fail('CORRUPT', 'Historical snapshot is unreadable.');
    }
    validBundle(bundle);
    return { bundle, bytes };
  }
  ...
  function validateReceipt(receipt, transactionId) {
    if (
      receipt?.kind !== 'diagram-authoring-receipt' ||
      receipt.version !== 1 ||
      receipt.diagramId !== slug ||
      receipt.transactionId !== transactionId ||
      !Number.isSafeInteger(receipt.sequence) ||
      receipt.sequence < 1 ||
      !DIGEST.test(receipt.fingerprint) ||
      !DIGEST.test(receipt.resultBytesDigest) ||
      !DIGEST.test(receipt.result?.bundleDigest)
    )
      fail('CORRUPT', 'Invalid transaction receipt.');
    return receipt;
  }
  ...
  async function saved(receipt, replayed = false) {
    ...
    if (!same(snapshot(bundle), receipt.result))
      fail('CORRUPT', 'Receipt and snapshot identities disagree.');
  ...
  async function current() {
    ...
    if (pending)
      return {
        ...
        reason: 'A durable transaction is awaiting recovery.',
      };
    ...
    if (
      !head ||
      !bytes ||
      head.kind !== 'diagram-authoring-head' ||
      head.version !== 1 ||
      head.diagramId !== slug ||
      digestBytes(bytes) !== head.byteDigest
    )
      fail('CHANGED', 'Canonical bytes do not match the last known complete save.');
    const receipt = validateReceipt(
      await readJson(receiptPath(head.transactionId)),
      head.transactionId,
    );
    if (
      receipt.resultBytesDigest !== head.byteDigest ||
      receipt.result.bundleDigest !== head.bundleDigest ||
  ...
  async function settle(pending) {
    ...
    if (
      pending.kind !== 'diagram-authoring-pending' ||
      pending.version !== 1 ||
      pending.fingerprint !== receipt.fingerprint ||
      pending.baseBytesDigest !== receipt.baseBytesDigest
    )
      fail('CORRUPT', 'The pending recovery identity is invalid.');
    const { bundle, bytes } = await loadSnapshot(receipt.resultBytesDigest);
    if (!same(snapshot(bundle), receipt.result))
      fail('CORRUPT', 'Pending result and immutable snapshot disagree.');
    if (pending.transaction) {
      if (
        inspectPlainData(pending.transaction).length ||
        digestBytes(jsonBytes(pending.transaction)) !== receipt.fingerprint ||
        pending.transaction.transactionId !== receipt.transactionId
      )
        fail('CORRUPT', 'Pending transaction fingerprint changed.');
      ...
      if (
        !replay.ok ||
        !same(snapshot(prior.bundle), receipt.base) ||
        digestBytes(validBundle(replay.bundle)) !== receipt.resultBytesDigest ||
        !same(replay.inverse, receipt.inverse)
      )
        fail('CORRUPT', 'Recovery does not reproduce the exact transaction result.');
      ...
      if (
        ...
        digestBytes(
          jsonBytes({
            replace: pending.replacement.bundle,
      ...
      receipt.base !== null ||
      receipt.baseBytesDigest !== null ||
      digestBytes(jsonBytes({ initialize: bundle, transactionId: receipt.transactionId })) !==
        receipt.fingerprint
    )
      fail('CORRUPT', 'Pending initialization fingerprint changed.');
    ...
    if (observed !== receipt.baseBytesDigest && observed !== receipt.resultBytesDigest)
      return {
        ok: false,
        status: 'unknown',
        transactionId: receipt.transactionId,
        ...
        reason:
          'Canonical bytes changed outside the pending transaction; no content was overwritten.',
      };
    if (observed !== receipt.resultBytesDigest) {
      ...
      try {
        ...
        if ((reread ? digestBytes(reread) : null) !== receipt.baseBytesDigest)
          fail('CHANGED', 'Canonical bytes changed before replacement.');
    ...
    await replace(
      headPath,
      jsonBytes({
        kind: 'diagram-authoring-head',
        version: 1,
        diagramId: slug,
        sequence: receipt.sequence,
        transactionId: receipt.transactionId,
        byteDigest: receipt.resultBytesDigest,
        bundleDigest: receipt.result.bundleDigest,
      }),
    );
  ...
  async function recoverInternal({ transactionId, fingerprint } = {}) {
    ...
    if (pending) {
      if (
        ...
        (fingerprint && pending.fingerprint !== fingerprint)
      )
        fail('IDENTITY', 'Recovery request does not identify the pending transaction.');
    ...
    if (transactionId) {
      ...
      if (fingerprint && receipt.fingerprint !== fingerprint)
        fail('ID_REUSE', 'Transaction ID was already used with different bytes.');
  ...
  async function writeRequest({
    ...
    const request =
      transaction ??
      (replacement
        ? { replace: bundle, transactionId, expectedBase }
        : { initialize: bundle, transactionId });
    const diagnostics = inspectPlainData(request);
    if (diagnostics.length)
      fail('INVALID', 'The save request must contain inert bounded JSON.', { diagnostics });
    ...
    if (
      pendingIdentity?.transactionId === transactionId &&
      pendingIdentity.fingerprint !== fingerprint
    )
      fail('ID_REUSE', 'Transaction ID is pending with different bytes.');
    ...
    if (transaction) {
      if (before.status !== 'ready')
        fail('MISSING', 'Initialize the canonical bundle before editing it.');
      preview = previewDiagramTransaction(before.bundle, transaction);
      if (!preview.ok)
        fail('INVALID_TRANSACTION', 'The transaction does not apply to the current exact base.', {
      ...
      if (
        ...
        fail(
          'CHANGED',
          'The complete-bundle replacement no longer matches its exact reviewed base.',
        );
    } else if (before.status !== 'absent')
      fail('COLLISION', 'Initialization never replaces an existing diagram.');
    ...
    const receipt = {
      ...
      version: 1,
      diagramId: slug,
      sequence: (before.receipt?.sequence ?? 0) + 1,
      transactionId,
      fingerprint,
      base: before.basis ?? null,
      baseBytesDigest: before.byteDigest ?? null,
      result: snapshot(bundle),
      resultBytesDigest: digestBytes(bytes),
      inverse: preview?.inverse ?? null,
    };
    const pending = {
      kind: 'diagram-authoring-pending',
      version: 1,
      transactionId,
      fingerprint,
      baseBytesDigest: receipt.baseBytesDigest,
      transaction: transaction ?? null,
      ...(replacement ? { replacement: { bundle, expectedBase } } : {}),
      receipt,
    };
    const pendingBytes = jsonBytes(pending);
    if (pendingBytes.length > metadataLimit)
      fail('CAPACITY', 'Recovery metadata exceeds its bounded capacity.');
    ...
    try {
      ...
      return {
        ...
        code: error.code ?? 'E_DIAGRAM_STORE_INTERRUPTED',
      };
    }
  }
  ...
  return Object.freeze({
    path: canonical,
    read: () => locked(current),
    initialize: (bundle, identity = {}) =>
      captured({ bundle, identity }, (value) =>
        writeRequest({ bundle: value.bundle, transactionId: value.identity.transactionId }),
      ),
    preview: (transaction) =>
      captured(transaction, async (transaction) => {
        const state = await current();
        if (!state.ok) return state;
        if (state.status !== 'ready')
          fail('MISSING', 'Initialize the canonical bundle before previewing edits.');
    ...
    commitSnapshot: (bundle, identity = {}) =>
      captured({ bundle, identity }, (value) =>
        writeRequest({
          bundle: value.bundle,
          transactionId: value.identity.transactionId,
          replacement: true,
          expectedBase: value.identity.expectedBase,
        }),
      ),
    recover: (options = {}) => captured(options, recoverInternal),
    readSnapshot: (byteDigest) =>
      locked(async () => {
        if (!(await owned())) fail('MISSING', 'The diagram has no snapshot history.');
        return (await loadSnapshot(byteDigest)).bundle;
      }),
    history: ({ limit = 100, beforeSequence = Infinity } = {}) =>
      locked(async () => {
        if (
          ...
          fail('OPTIONS', 'Invalid history page.');
        if (!(await owned())) return [];
        const entries = [];
        for (const name of await readdir(receipts)) {
          if (!/^[a-f0-9]{64}\.json$/u.test(name))
            fail('COLLISION', 'An unowned history entry exists.');
          const receipt = await readJson(join(receipts, name));
          validateReceipt(receipt, receipt.transactionId);
          if (receiptPath(receipt.transactionId) !== join(receipts, name))
            fail('CORRUPT', 'Receipt filename does not match its identity.');
