import { COLLECTIONS, clone, same, elementIndex, parentIndex, descendants, validateAuthoringBundle, failure } from './model.mjs';
...
function fields(before, after, base, result, path = []) {
  ...
  if (before && after && !Array.isArray(before) && !Array.isArray(after) && typeof before === 'object' && typeof after === 'object') {
    for (const key of [...new Set([...Object.keys(before), ...Object.keys(after)])].sort()) fields(before[key], after[key], base, result, [...path, key]);
  } else result.push({ ...base, path, before: before === undefined ? null : clone(before), after: after === undefined ? null : clone(after) });
}
function records(before, after, collection, identity, result) {
  ...
  for (const id of [...new Set([...old.keys(), ...next.keys()])].sort()) {
    ...
    if (!old.has(id) || !next.has(id)) result.push({ ...base, path: [], before: clone(old.get(id) ?? null), after: clone(next.get(id) ?? null) });
...
export function affectedState(before, after, semantic, presentation) {
  ...
  for (const bundle of [before, after]) {
    ...
    for (const edge of document.relations) if (reads.has(edge.id) || reads.has(edge.from) || reads.has(edge.to)) { reads.add(edge.id); reads.add(edge.from); reads.add(edge.to); }
    for (const note of document.annotations) if (reads.has(note.id) || reads.has(note.targetId)) { reads.add(note.id); if (note.targetId !== null) reads.add(note.targetId); }
    for (const id of [...reads]) {
      let parent = parents.get(id);
      while (parent) { reads.add(parent); parent = parents.get(parent); }
    }
  }
...
export function diffDiagramBundles(before, after) {
  for (const bundle of [before, after]) { const checked = validateAuthoringBundle(bundle); if (!checked.ok) return checked; }
  ...
  for (const field of ['title', 'summary', 'audience', 'grammar', 'laneOrder', 'accessibility']) fields(before.document[field], after.document[field], { collection: 'document', elementId: null }, semantic, [field]);
  records(before.presentation.elements, after.presentation.elements, 'elements', 'elementId', presentation);
  for (const field of ['layout', 'theme']) fields(before.presentation[field], after.presentation[field], { collection: 'presentation', elementId: null }, presentation, [field]);
...
export function inverseDependencies(bundle, impact) {
  ...
  for (const { id, from, to } of bundle.document.relations) for (const endpoint of new Set([from, to])) {
  ...
  for (const { id, targetId } of bundle.document.annotations) if (targetId !== null) {
  ...
  return impact.readIds.map(elementId => {
    ...
    if (!writes.has(elementId)) return { elementId, value: clone({ semantic: entry, placement: placements.get(elementId), parent: parents.get(elementId) ?? null }) };
    const { from, to, targetId, members } = entry.value;
    return { elementId, value: clone({ collection: entry.collection, parent: parents.get(elementId) ?? null,
      ...(from === undefined ? {} : { from, to }), ...(targetId === undefined ? {} : { targetId }), ...(members === undefined ? {} : { members }),
      locks: placements.get(elementId).locks,
      incident: incident.get(elementId) ?? [], annotations: annotations.get(elementId) ?? [] }) };
  });
}
...
export function sourceMapChanges(before, after) {
  ...
  if (before === null || after === null) {
    if (!same(before, after)) changes.push({ collection: 'source-map', elementId: null, path: [], before: clone(before), after: clone(after) });
    return changes;
  }
  ...
  for (const key of Object.keys(before).filter(key => key !== 'semanticDigest' && key !== 'entries')) fields(before[key], after[key], { collection: 'source-map', elementId: null }, changes, [key]);
  ...
  else for (let index = 0; index < before.entries.length; index++) fields(before.entries[index], after.entries[index], { collection: 'source-map', elementId: null }, changes, ['entries', String(index)]);
  return changes;
}
