...
import { assertContainedPath, diagramRelativeDirectory } from './paths.mjs';
...
async function assertRegularFile(target) {
  ...
  if (!info.isFile())
    diagramFail(
      ...
      'Diagram output member is not a regular file.',
      { path: target },
    );
}
...
async function ensureOwnedDirectory(target) {
  ...
  if (info.isSymbolicLink() || !info.isDirectory()) {
    diagramFail(
      ...
      'Diagram runtime directory must be a real directory.',
      { path: target },
    );
  }
}
...
export async function readDiagramSet(root, slug) {
  ...
  if (!info.isDirectory())
    diagramFail(DIAGRAM_ERROR_CODES.OUTPUT_CONFLICT, 'Diagram output target is not a directory.', {
      path: directory,
    });
  ...
  if (!(await exists(manifestPath))) {
    diagramFail(
      DIAGRAM_ERROR_CODES.OUTPUT_CONFLICT,
      'Existing diagram output has no OpenPlanr manifest.',
      {
        path: directory,
  ...
  try {
    ...
    diagramFail(DIAGRAM_ERROR_CODES.OUTPUT_CONFLICT, 'Existing diagram manifest is unreadable.', {
      cause: error.message,
    });
  }
  try {
    assertDiagramRenderManifest(manifest, { slug });
  } catch (error) {
    diagramFail(DIAGRAM_ERROR_CODES.OUTPUT_CONFLICT, 'Existing diagram manifest is invalid.', {
      cause: error.message,
    });
  }
  ...
  for (const output of manifest.outputs) {
    ...
    if (relative(directory, target).startsWith('..')) {
      diagramFail(
        DIAGRAM_ERROR_CODES.OUTPUT_CONFLICT,
        'Manifest output escapes its diagram directory.',
        { path: output.path },
      );
    }
    ...
    if (!(await exists(target))) {
      ...
      (branch ? sourceChanges : generatedChanges).push({
        path: output.path,
        reason: 'missing',
        branch,
      });
  ...
  if (unexpected.length > 0)
    generatedChanges.push(
      ...unexpected.map((name) => ({ path: `${relativeDirectory}/${name}`, reason: 'unowned' })),
    );
  return Object.freeze({ directory, files, generatedChanges, manifest, sourceChanges });
}

export async function cleanupAbandonedDiagramStages(
  root,
  { maximumAgeMs = DEFAULT_ABANDONED_MS } = {},
) {
  ...
  if (stagingInfo.isSymbolicLink() || !stagingInfo.isDirectory()) {
    diagramFail(
      DIAGRAM_ERROR_CODES.OUTPUT_ESCAPE,
      'Diagram staging root must be a real directory.',
      { path: stagingRoot },
    );
  }
  let removed = 0;
...
export async function recoverInterruptedDiagramPromotion(root, slug) {
  ...
  if (diagramsInfo.isSymbolicLink() || !diagramsInfo.isDirectory()) {
    diagramFail(
      DIAGRAM_ERROR_CODES.OUTPUT_ESCAPE,
      'Diagram output collection must be a real directory.',
      { path: diagramsRoot },
    );
  }
  ...
  const backups = (await readdir(diagramsRoot, { withFileTypes: true }))
    .filter(
      (entry) => entry.isDirectory() && !entry.isSymbolicLink() && entry.name.startsWith(prefix),
    )
...
export async function promoteDiagramSet(root, slug, contentId, files, { current = null } = {}) {
  ...
  const stageDirectory = assertContainedPath(
    stagingRoot,
    join(stagingRoot, `${slug}-${contentId.slice(0, 16)}-${nonce}`),
  );
  ...
  try {
    for (const [name, bytes] of [...files.entries()].sort(([left], [right]) =>
      ...
      if (basename(name) !== name)
        diagramFail(
          DIAGRAM_ERROR_CODES.OUTPUT_ESCAPE,
          'Staged diagram members must be direct filenames.',
          { name },
        );
      const target = assertContainedPath(stageDirectory, join(stageDirectory, name));
      await writeFile(target, bytes, { flag: 'wx' });
      if (digestBytes(await readFile(target)) !== digestBytes(bytes))
        throw new Error(`Staged byte verification failed: ${name}`);
    }
    if (await setsAreEqual(current, files))
      return Object.freeze({ status: 'unchanged', directory: current.directory });
    const diagramsRoot = assertContainedPath(root, join(root, 'diagrams'));
    await ensureOwnedDirectory(diagramsRoot);
    const finalDirectory = assertContainedPath(diagramsRoot, join(diagramsRoot, slug));
    const backupDirectory = assertContainedPath(
      diagramsRoot,
      join(diagramsRoot, `.${slug}.backup-${nonce}`),
    );
    ...
    try {
      ...
      if (replacing && (await exists(backupDirectory)) && !(await exists(finalDirectory)))
