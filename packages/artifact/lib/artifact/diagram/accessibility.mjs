...
const attribute = (bytes, name) =>
  bytes.match(new RegExp(`\\b${name}=["']([^"']+)["']`, 'iu'))?.[1] ?? null;

export function validateDiagramSvg(
  svg,
  { foreground = '#111111', background = '#ffffff', clipped = false } = {},
) {
  ...
  if (
    /<(?:script|foreignObject)\b|\son[a-z]+\s*=|(?:href|src)=["'](?:https?:|\/\/|data:)/iu.test(
      bytes,
    )
  )
    errors.push('external-or-executable-resource');
  const fontSizes = [...bytes.matchAll(/font-size\s*[:=]\s*["']?(\d+(?:\.\d+)?)/giu)].map((match) =>
    Number(match[1]),
  );
...
export function assertDiagramSvg(svg, options) {
  ...
  if (!result.ok) {
    diagramFail(
      DIAGRAM_ERROR_CODES.ACCESSIBILITY_INVALID,
      'SVG accessibility contract failed.',
      result,
    );
  }
  return result;
