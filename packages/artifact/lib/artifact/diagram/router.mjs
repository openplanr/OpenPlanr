...
export function routeDiagramIntent({
  ...
  if (grammarId) {
    ...
    if (exact.length > 1) {
      diagramFail(
        ...
        `Diagram intent matches multiple exact aliases: ${intent}`,
        { candidates: exact },
      );
    }
    if (!grammar) {
      ...
      for (const candidate of DIAGRAM_GRAMMARS) {
        for (const phrase of [candidate.title, ...candidate.aliases]) {
          if (phraseMatch(text, phrase))
            scores.set(
              candidate.grammarId,
              scores.get(candidate.grammarId) + normalize(phrase).split(' ').length,
            );
        }
      }
      for (const pattern of DIAGRAM_SEMANTIC_PATTERNS) {
        if (!pattern.triggers.some((trigger) => phraseMatch(text, trigger))) continue;
        pattern.candidateGrammars.forEach((id, index) =>
          scores.set(id, scores.get(id) + Math.max(1, 3 - index)),
        );
      }
      ...
      if (leaders.length !== 1) {
        diagramFail(
          DIAGRAM_ERROR_CODES.GRAMMAR_AMBIGUOUS,
          'Diagram intent does not select one compatible grammar.',
          {
            intent,
            candidates:
              leaders.length > 0 ? leaders : DIAGRAM_GRAMMARS.map(({ grammarId: id }) => id),
  ...
  if (!grammar.directions.includes(selectedDirection)) {
    diagramFail(
      ...
      `Direction ${selectedDirection} is not supported by ${grammar.grammarId}.`,
      {
  ...
  if (!grammar.readability.aspectRatios.includes(destination)) {
    diagramFail(
      DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID,
      `Destination ${destination} is not supported by ${grammar.grammarId}.`,
      {
