...
export function createDiagramLocalOwnerTransport({ apiBase, fetch: request = globalThis.fetch, origin = globalThis.location?.origin, maxResponseBytes = 64 * 1024 * 1024 }) {
  const base = new URL(apiBase);
  if (base.protocol !== 'http:' || !['127.0.0.1', '[::1]'].includes(base.hostname) || base.username || base.password || base.search || base.hash
    || !/^\/o\/[A-Za-z0-9_-]+\/[A-Za-z0-9_-]+\/api\/$/u.test(base.pathname)
    || (origin && origin !== base.origin)) throw new TypeError('Owner transport requires this page’s exact loopback owner capability URL.');
  ...
  async function call(action, body) {
    const response = await request(new URL(action, base).href, {
      ...
      headers: { 'X-OpenPlanr-Owner': '1', ...(body === undefined ? {} : { 'Content-Type': 'application/json' }) },
    ...
    const decoder = new TextDecoder('utf-8', { fatal: true }); let text = ''; let count = 0;
    try {
      for (;;) {
