...
export function createDiagramEditorRecovery({ storage, scope, maxBytes = MAX_RECOVERY_BYTES }) {
  ...
  const unavailable = message => { mode = 'memory-only'; warning = message; return { ok: false, mode, warning }; };
  return {
    status: () => ({ mode, warning }),
    load() {
      ...
      try {
        ...
        if (inspectPlainData(value).length || value?.version !== 1 || value.scope?.sessionId !== boundScope.sessionId || value.scope?.diagramId !== boundScope.diagramId
          ...
          || Object.keys(value).some(name => !['version', 'scope', 'draft'].includes(name))) throw new Error('Recovery does not match this owner session and diagram.');
        return value.draft;
      } catch { quarantined = true; unavailable('Stored recovery could not be read safely. Pending edits remain in memory only.'); return null; }
    },
    save(draft) {
    ...
    clear() {
      ...
      try { storage.removeItem(key); return { ok: true, mode, warning }; }
      catch { return unavailable('Recovery storage could not be cleared. Close this browser session before changing access.'); }
    },
  };
}
