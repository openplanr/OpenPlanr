import {
  compileDiagramCommand, previewDiagramTransaction, createConditionalInverse,
  validateAuthoringBundle, diffDiagramBundles, previewAutomaticLayout,
...
const validId = value => typeof value === 'string' && value.length <= 128 && /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/u.test(value);
...
export function createDiagramEditorSession({ bundle, acknowledged = false, transport = null, recovery = null, nextTransactionId = defaultId, capabilities = { read: true, write: true } }) {
  ...
  function emit(type, affectedIds = []) {
    ...
    for (const listener of [...listeners]) { try { listener(clone(event)); } catch { /* A subscriber cannot interrupt a content commit. */ } }
  }
  function persist() {
  ...
  function guard() {
    ...
    if (!capability.read || !capability.write) return fail('access-changed', 'This session no longer has owner editing access.');
    return null;
  }
  function pruneView() {
  ...
  function accept(preview, history = 'edit') {
    ...
    if (idOf(preview.bundle) === idOf(current)) return { ok: true, changed: false, transaction: null };
    if (pending.length >= MAX_PENDING) return fail('pending-limit', 'Save the pending edits before adding more.');
    if (usedIds.has(preview.transaction.transactionId)) return fail('transaction-id', 'Each new edit needs a fresh transaction identity.');
    usedIds.add(preview.transaction.transactionId);
    updateGeometry(preview.bundle, preview.impact.affectedIds);
    pending.push({ transaction: clone(preview.transaction), bundle: clone(preview.bundle), inverse: clone(preview.inverse) });
    current = clone(preview.bundle);
    if (history === 'edit') { undo.push(clone(preview.inverse)); undo = undo.slice(-MAX_HISTORY); redo = []; }
    diagnostics = [];
    saveState = comparison ? 'conflict' : saving ? 'saving' : 'unsaved';
    pruneView(); persist(); emit('content', preview.impact.affectedIds);
    return clone(preview);
  }
  function checkIdentity(transactionId) {
    return validId(transactionId) && !usedIds.has(transactionId) ? null : fail('transaction-id', 'Each new edit needs a fresh transaction identity.');
  }
  function submit(command, options = {}) {
    ...
    const invalid = checkIdentity(transactionId); if (invalid) return invalid;
  ...
  function beginGesture({ transactionId = nextTransactionId() } = {}) {
    ...
    emit('gesture-start'); return { ok: true };
  }
  ...
  function completeGesture() {
    ...
    if (comparison || gesture.basis !== idOf(current)) return fail('stale-gesture', 'The authoritative revision changed. Retain or cancel this preview and compare before applying it.');
  ...
  function compensate(source, target, transactionId) {
    ...
    if (comparison) return fail('conflict', 'Compare the authoritative revision before undo or redo.');
    ...
    if (!inverse.ok) { diagnostics = clone(inverse.diagnostics); emit('undo-conflict'); return inverse; }
    const preview = previewDiagramTransaction(current, inverse.transaction);
    const result = accept(preview, 'compensation');
    if (result.ok && result.transaction) { source.pop(); target.push(clone(result.inverse)); if (target.length > MAX_HISTORY) target.shift(); emit('history'); }
    return result;
  }
  function refresh(authoritative) {
    const blocked = disposed || !capability.read; if (blocked) return fail('access-changed', 'This session cannot read a revision.');
    const validation = validateAuthoringBundle(authoritative);
    if (!validation.ok) return validation;
    if (authoritative.diagramId !== current.diagramId) return fail('diagram-scope', 'A refresh cannot switch the session to another diagram.');
    ...
    if (pending.length || initialization || gesture || saving) {
      ...
      saveState = 'conflict'; emit('conflict');
    ...
    current = clone(authoritative); base = clone(authoritative); saved = clone(authoritative);
    saveState = 'saved'; comparison = null; diagnostics = []; pruneView(); emit('refresh', diff.impact.affectedIds);
  ...
  function acknowledge(result, expected, transactionId) {
    if (!result?.ok || result.status !== 'saved' || !validateAuthoringBundle(result.bundle).ok || !same(result.bundle, expected)
      || result.receipt?.transactionId !== transactionId || !same(result.receipt.result, snapshot(expected))) return false;
  ...
  async function performSave(runEpoch, count) {
    ...
    try {
      ...
      for (let index = 0; index < count; index++) {
        ...
        pending.shift(); persist(); emit('acknowledged', item.inverse.changes.semantic.map(change => change.elementId).filter(Boolean));
      }
      if (!live()) return fail('disposed', 'This session is closed.');
      saveState = comparison ? 'conflict' : pending.length ? 'unsaved' : 'saved';
      diagnostics = []; return { ok: true, status: saveState, bundle: clone(saved) };
    } catch (error) {
      if (!live()) return fail('disposed', 'This session is closed.');
      return await failureState({ ok: false, httpStatus: error?.httpStatus, diagnostics: error?.details?.diagnostics ?? [{ path: '$save', rule: 'save-unavailable', detail: 'The owner did not acknowledge this save. Retry without discarding pending work.' }] });
    } finally {
      if (live()) { persist(); emit('save'); }
    }
  }
  function save() {
    const blocked = guard(); if (blocked) return Promise.resolve(blocked);
    if (saving) return saving;
    if (!transport) { saveState = 'offline'; emit('save'); return Promise.resolve(fail('no-transport', 'No owner store is connected. Pending changes remain unsaved.')); }
    ...
    const settled = Promise.resolve().then(() => performSave(runEpoch, count)).finally(() => { if (saving === settled) saving = null; });
    saving = settled;
    emit('save');
    return settled;
  }
  function setView(patch) {
    if (disposed || !capability.read) return fail('access-changed', 'This session cannot update view state.');
    if (inspectPlainData(patch).length || !patch || Array.isArray(patch) || Object.keys(patch).some(key => !Object.hasOwn(view, key))) return fail('view', 'Unknown or non-JSON view preference.');
    const next = { ...view, ...clone(patch) };
    const camera = next.camera;
    if (!camera || Object.keys(camera).some(key => !['x', 'y', 'scale', 'fit'].includes(key)) || !Number.isFinite(camera.x) || !Number.isFinite(camera.y) || !Number.isFinite(camera.scale) || camera.scale <= 0
      || ![null, 'all', 'width'].includes(camera.fit) || typeof next.snap !== 'boolean'
      || ['selection', 'collapsedGroups', 'trace'].some(key => !Array.isArray(next[key]) || next[key].length > 10000 || next[key].some(id => typeof id !== 'string'))) return fail('view', 'Invalid camera or selection preference.');
    view = next; pruneView(); emit('view'); return { ok: true };
  }
  function restore() {
    ...
    try {
      ...
      for (const transaction of record.transactions) {
        ...
        if (!preview.ok || ids.has(transaction.transactionId)) throw new Error('Invalid recovery transaction.');
        ids.add(transaction.transactionId); draft = preview.bundle;
        entries.push({ transaction: preview.transaction, bundle: preview.bundle, inverse: preview.inverse });
      }
    ...
    } catch { recoveryWarning = 'Stored edits failed validation and were not applied. The authoritative diagram remains unchanged.'; }
  }
  restore();
  ...
  return {
    getState() {
      return {
        ...
        pendingCount: pending.length, needsInitialization: initialization !== null,
    ...
    query: options => disposed || !capability.read ? fail('access-changed', 'This session cannot inspect geometry.') : geometry.query({ ...options, camera: { x: view.camera.x, y: view.camera.y, scale: view.camera.scale } }),
    geometry: id => disposed || !capability.read ? null : geometry.get(id),
    geometryStats: () => geometry.stats(),
    subscribe(listener) { if (disposed) return () => {}; if (typeof listener !== 'function') throw new TypeError('Listener must be a function.'); listeners.add(listener); return () => listeners.delete(listener); },
    ...
    useAuthoritative() {
      const blocked = guard(); if (blocked) return blocked;
      if (saving) return fail('save-in-flight', 'Wait for the save result before discarding a draft.');
...
export async function openDiagramEditorSession({ transport, create, ...options }) {
  ...
  if (result?.ok && result.status === 'ready') return createDiagramEditorSession({ ...options, transport, bundle: result.bundle, acknowledged: true });
  if (result?.ok && result.status === 'absent' && create) return createDiagramEditorSession({ ...options, transport, bundle: create, acknowledged: false });
  throw new Error('The owner could not provide a complete diagram. Recover or retry before opening an editor.');
}
