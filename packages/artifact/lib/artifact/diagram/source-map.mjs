...
const idFor = (sourceId) =>
  sourceId
    ...
    .replace(/[^a-z0-9]+/gu, '-')
    .replace(/^-|-+$/gu, '');
const issue = (code, severity, line, column, range, message, repair, elementIds = []) => ({
  code,
  severity,
  line,
  column,
...
function sourceLines(text) {
  ...
  while (cursor < text.length) {
    ...
    const newline = text.slice(
      end,
      end + (text.slice(end, end + 2) === '\r\n' ? 2 : end < text.length ? 1 : 0),
    );
...
function nodeExpression(raw) {
  ...
  return {
    sourceId,
    label,
    kind: shape === 'diamond' ? 'decision' : shape === 'cylinder' ? 'data-store' : 'process',
    shape,
    declared: true,
  };
}
...
function parseEdge(line) {
  ...
  if (!match) return null;
  const from = nodeExpression(match[1]),
    to = nodeExpression(match[4]);
  if (!from || !to) return null;
  const label = match[3] === undefined ? null : parseLabel(match[3], '');
  if (
    match[3] !== undefined &&
    (label === null || label.length > 4096 || /[\u0000-\u001f]/u.test(label))
  )
    return null;
  return {
    from,
...
function placement(elementId, shape, bounds, zIndex, previous) {
  ...
  if (old && old.appearance.shape === shape && Boolean(old.bounds) === Boolean(bounds))
    return clone(old);
  return {
    elementId,
    bounds,
    ...
    appearance: {
      shape,
      fill: shape === 'container' || shape === 'connector' ? 'transparent' : 'surface',
      stroke: 'default',
      strokeWidth: 2,
      strokeStyle: 'solid',
      fontSize: shape === 'connector' ? 14 : 16,
      textAlign: 'center',
    },
...
export function previewMermaidCopy(source, options = {}) {
  if (
    ...
    Object.keys(options).some((key) => !['diagramId', 'title', 'previousBundle'].includes(key))
  )
    return failure(
      issue(
        'invalid-options',
  ...
  if (
    ...
    !SAFE_ID.test(diagramId) ||
    diagramId.length > 128 ||
    typeof title !== 'string' ||
    title.length > 4096
  )
    return failure(
  ...
  if (previousBundle) {
    ...
    if (!checked.ok || previousBundle.diagramId !== diagramId)
      return failure(
        issue(
  ...
  const previousIds = new Map(
    (previousBundle?.sourceMap?.entries ?? [])
      .filter(
        (entry) =>
          entry.sourceId &&
          entry.elementIds.length === 1 &&
          (entry.confidence === 'exact' ||
            (entry.losses.length > 0 && entry.losses.every((loss) => loss === SEMANTIC_EDIT_LOSS))),
      )
      .map((entry) => [entry.sourceId, entry.elementIds[0]]),
  );
  ...
  const used = new Map();
  let header = null;
  const reject = (code, line, message, repair, elementIds = []) =>
    diagnostics.push(
  ...
  const identity = (raw, line, type) => {
    ...
    if (
      previousIds.has(raw) &&
      previous &&
      !(type === 'node' ? previous.document.nodes : previous.document.groups).some(
        (item) => item.id === id,
      )
    ) {
      reject(
  ...
  const addMember = (id, line) => {
    ...
    for (const group of groups.values())
      if (group.id !== parent.id && group.members.includes(id))
        reject(
          'multiple-parents',
          line,
  ...
  const addNode = (node, line) => {
    ...
    if (
      ...
      node.declared &&
      current.declared &&
      (current.label !== node.label || current.shape !== node.shape)
    )
      reject(
        'conflicting-node',
        line,
  ...
  for (const line of lines) {
    if (
      /%%\{|<\s*(?:script|iframe|foreignObject)\b|javascript:|https?:\/\/|(?:^|\s)(?:click|href)\s+/iu.test(
        line.text,
      )
    ) {
      reject(
    ...
    if (edge) {
      const from = addNode(edge.from, line),
        to = addNode(edge.to, line);
      if (!from || !to) continue;
      const signature = JSON.stringify([from, to, edge.direction, edge.label]);
      const continuing =
        previous?.document.relations.filter(
  ...
  for (const annotation of previous?.document.annotations ?? []) {
    if (
      semanticIds.has(annotation.id) ||
      (annotation.targetId && !semanticIds.has(annotation.targetId))
    )
      diagnostics.push(
        issue(
          'orphaned-authoring',
          ...
          1,
          null,
          `Authored annotation ${annotation.id} would be lost or collide with source.`,
  ...
  for (const emphasis of previous?.document.emphasis ?? [])
    if (!semanticIds.has(emphasis.targetId))
      diagnostics.push(
        issue(
          'orphaned-authoring',
          'error',
          ...
          1,
          null,
          `Authored emphasis on ${emphasis.targetId} would be lost.`,
  ...
  const document = {
    ...
    nodes: [...nodes.values()].map(({ id, label, kind }) => ({
      id,
      label,
      kind,
      description: previous?.document.nodes.find((item) => item.id === id)?.description ?? null,
    })),
    relations: [...edges.values()],
    groups: [...groups.values()].map(({ id, label, members }) => ({ id, label, members })),
    lanes: [],
    events: [],
    series: [],
    axes: [],
  ...
  const nodeElements = orderedNodes.map((node, index) => {
    ...
    const bounds = vertical
      ? {
          x: 100 + line * 320,
          y: 100 + along * 320,
          width: node.shape === 'diamond' ? 200 : 180,
          height: node.shape === 'diamond' ? 104 : 80,
        }
      : {
          x: 100 + along * 420,
          y: 100 + line * 280,
  ...
  for (const group of [...groups.values()].reverse()) {
    ...
    const bounds = members.length
      ? {
          ...
          width:
            Math.max(...members.map((item) => item.x + item.width)) -
            Math.min(...members.map((item) => item.x)) +
            88,
          height:
            Math.max(...members.map((item) => item.y + item.height)) -
  ...
  const reportIssues = validateDiagramAuthoringArtifact('diagram-fidelity-report', fidelity, {
    bundle,
  });
  if (reportIssues.length)
    return {
      ok: false,
      sourceModified: false,
      diagnostics: reportIssues.map((item) =>
        issue(
          item.rule,
          'error',
          1,
  ...
  const acknowledgement = losses.length
    ? digest(JSON.stringify({ bundleDigest: bundle.bundleDigest, losses }))
    : null;
  return {
    ok: true,
    bundle,
...
export function adoptMermaidCopy(preview, acknowledgement = null) {
  ...
  if (!preview?.ok || !preview.bundle || !validateAuthoringBundle(preview.bundle).ok)
    return failure(
      issue(
        'invalid-preview',
        'error',
  ...
  if (
    validateDiagramAuthoringArtifact('diagram-fidelity-report', preview.fidelity, {
  ...
  const expected = preview.fidelity.losses.length
    ? digest(
        JSON.stringify({
          bundleDigest: preview.bundle.bundleDigest,
          losses: preview.fidelity.losses,
        }),
      )
    : null;
  if (expected !== preview.acknowledgement || expected !== acknowledgement)
    return failure(
      issue(
...
function sourceIds(bundle) {
  const ids = new Map();
  for (const entry of bundle.sourceMap?.entries ?? [])
    if (entry.sourceId && entry.elementIds.length === 1 && ID.test(entry.sourceId))
      ids.set(entry.elementIds[0], entry.sourceId);
  return ids;
}
...
export function exportMermaidCopy(bundle) {
  ...
  if (!checked.ok)
    return {
      ok: false,
      diagnostics: checked.diagnostics.map((item) =>
        issue(item.rule, 'error', 1, 1, null, item.detail, 'Use a valid editable bundle.'),
      ),
    };
    ...
    if (!ID.test(name) || occupied.has(name))
      return failure(
        issue(
          ...
          1,
          null,
          `Element ${entry.id} has no unique Mermaid ID.`,
  ...
  const nodeText = (node) => {
    ...
    if (node.kind === 'process' && shape === 'rounded-rectangle')
      return `${names.get(node.id)}(${label})`;
    if (node.kind === 'decision' && shape === 'diamond') return `${names.get(node.id)}{${label}}`;
    if (node.kind === 'data-store' && shape === 'cylinder')
      return `${names.get(node.id)}[(${label})]`;
    lost(
      'semantic',
      'unsupported-node-shape',
      [node.id],
      `Node ${node.id} has a role or shape outside certified Mermaid flowchart copy.`,
    );
    return `${names.get(node.id)}[${label}]`;
  };
  ...
  const append = (parent, depth) => {
    ...
    for (const node of bundle.document.nodes.filter(
      (item) => (byParent.get(item.id) ?? null) === parent,
    ))
  ...
  for (const edge of bundle.document.relations) {
    const op =
      edge.kind === 'flow' && edge.direction === 'forward'
        ? '-->'
        : edge.kind === 'flow' && edge.direction === 'both'
          ? '<-->'
          : edge.kind === 'association' && edge.direction === 'none'
            ? '---'
            : null;
    if (!op) {
      lost(
