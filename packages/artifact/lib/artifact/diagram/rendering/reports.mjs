...
import { LABEL_EDGE_DISTANCE, SHARED_SEGMENT_LENGTH, labelAttached, routesMerge } from './layout.mjs';
...
function foreignLabelCount(scene) {
  ...
  return (scene.labelBounds ?? []).filter((label) => {
    ...
    const halo = { x: label.x - FOREIGN_LABEL_MARGIN, y: label.y - FOREIGN_LABEL_MARGIN,
      width: label.width + 2 * FOREIGN_LABEL_MARGIN, height: label.height + 2 * FOREIGN_LABEL_MARGIN };
    return (scene.boxes ?? []).some((box) => box.id !== edge.from && box.id !== edge.to && overlaps(halo, box));
  }).length;
}
...
function labelOnContainerTitleCount(scene) {
  ...
  const titles = [...(scene.groups ?? []), ...(scene.lanes ?? [])]
    .map((frame) => ({ x: frame.x, y: frame.y, width: frame.width, height: 34 }));
  return (scene.labelBounds ?? []).filter((label) => titles.some((title) => overlaps(label, title))).length;
}
...
export function createRenderQualityReport(document, { scene, png, svgValidation }) {
  ...
  const checks = [
    ...
    { id: 'label-foreign-node', status: foreignLabels ? 'warning' : 'pass', message: foreignLabels ? `${foreignLabels} relation label${foreignLabels === 1 ? ' sits' : 's sit'} within ${FOREIGN_LABEL_MARGIN}px of a node the relation does not connect; readers will attach the label to the wrong node.` : 'Every relation label keeps clear of nodes it does not connect.' },
    { id: 'label-edge-distance', status: detachedLabels ? 'warning' : 'pass', message: detachedLabels ? `${detachedLabels} relation label${detachedLabels === 1 ? ' sits' : 's sit'} more than ${LABEL_EDGE_DISTANCE}px from the path they name; readers cannot tell which connection they belong to.` : 'Every relation label sits on the path it names.' },
    { id: 'edge-merge', status: mergedEdges ? 'warning' : 'pass', message: mergedEdges ? `${mergedEdges} pair${mergedEdges === 1 ? '' : 's'} of unrelated connections share a run of at least ${SHARED_SEGMENT_LENGTH}px and read as one line.` : 'Unrelated connections do not share a straight run.' },
    { id: 'label-cluster-overlap', status: titleLabels ? 'fail' : 'pass', message: titleLabels ? `${titleLabels} relation labels overlap a group or lane title.` : 'Relation labels keep clear of group and lane titles.' },
