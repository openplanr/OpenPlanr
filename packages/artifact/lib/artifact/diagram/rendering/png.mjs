...
const INTER_REGULAR_DIGEST = 'sha256:a414b48aa577ef2c62ebb135341ddeef33ee26a4f5dc9f787f93c1aab08ebb50';
...
function loadRasterAssets() {
  try {
    ...
    if (digest !== INTER_REGULAR_DIGEST) {
      diagramFail(DIAGRAM_ERROR_CODES.RENDER_FAILED, 'The packaged diagram font failed its custody check.', {
        actual: digest,
...
export function inspectDiagramPng(bytes) {
  ...
  if (value.length < 24 || !value.subarray(0, 8).equals(PNG_SIGNATURE) || value.toString('ascii', 12, 16) !== 'IHDR') {
  ...
  if (width === 0 || height === 0 || width > 16_384 || height > 16_384 || value.length > MAX_DIAGRAM_PNG_BYTES) {
