...
function parseLiveResultUrl(value, capability) {
  ...
  if (url.username || url.password || url.search
    ...
    || !roomId || !LIVE_ROOM_ID_RE.test(roomId)
    || keys.length !== 2 || !keys.includes('k') || !keys.includes(capability)
...
function validateLiveResultUrls({ url, ownerUrl, manageUrl }) {
  ...
  if (reviewer.origin !== owner.origin || reviewer.origin !== management.origin
    || reviewer.pathname !== owner.pathname || reviewer.pathname !== management.pathname
    || reviewer.key !== owner.key || reviewer.key !== management.key
...
function normalizeOwnerCustody(value) {
  if (record(value?.prepared) && record(value?.recovery)) {
    ...
    if (!exactKeys(prepared, [
      'schemaVersion', 'kind', 'protocolVersion', 'id', 'roomId', 'reviewOf', 'ttl', 'ownerKey',
    ])
    ...
    if (prepared.ownerKey.algorithm !== signerCustody.signer.algorithm
  ...
  if (signer.role !== 'owner' || typeof signer.sign !== 'function'
...
export async function establishArtifactOwnerCustody({ prepareOwnerCustody, saveOwnerCustody } = {}) {
...
export function reduceArtifactShareDialog(state, action = {}) {
  ...
  if (current.phase === 'ambiguous'
    && !['create-start', 'create-success', 'failure'].includes(action.type)) return current;
  if (current.phase === 'custody-ready'
    && ['select-transport', 'set-ttl'].includes(action.type)) return current;
  switch (action.type) {
    case 'open':
...
export function artifactShareCapabilities({ prepareShare, prepareOwnerCustody, createShare, supportedTransports } = {}) {
  const canCreate = typeof prepareShare === 'function' && typeof createShare === 'function';
  return Object.freeze(ARTIFACT_SHARE_TRANSPORTS.filter((transport) => canCreate
    && (transport !== 'live' || typeof prepareOwnerCustody === 'function')
    && (supportedTransports === undefined || (Array.isArray(supportedTransports) && supportedTransports.includes(transport)))));
}
...
export function mountArtifactShareDialog({
  ...
  const unavailable = text(unavailableReason) || 'Sharing is not configured in this viewer. Open the review with an updated OpenPlanr installation that supports sharing.';
  ...
  const handlers = {
    ...
    saveOwnerCustody: typeof saveOwnerCustody === 'function'
      ? saveOwnerCustody
  ...
  function render() {
    ...
    for (const button of dialog.querySelectorAll('[data-planr-share-transport]')) {
      ...
      button.disabled = ['previewing', 'custody-preparing', 'custody-ready', 'creating', 'ambiguous', 'created'].includes(state.phase)
        || !supports(transport);
      button.setAttribute('aria-disabled', String(button.disabled));
      button.title = capabilities.includes(transport) ? '' : transport === 'live' && capabilities.length
    ...
    if (fragmentSize) fragmentSize.textContent = !capabilities.includes('fragment') ? 'Unavailable' : preview
      ? `${preview.fragmentLength.toLocaleString('en-US')} chars · ${formatArtifactShareBytes(preview.compressedBytes)}`
      : state.phase === 'previewing' ? 'Calculating…' : 'Size unavailable';
    const shortSize = dialog.querySelector('[data-planr-share-short-size]');
    if (shortSize) shortSize.textContent = !capabilities.includes('short') ? 'Unavailable' : preview
    ...
    if (threshold) {
      ...
      threshold.textContent = preview?.fragmentEligible === false
        ...
        : preview ? 'Private fragment snapshot available for links up to 8,000 characters.' : 'Preparing the snapshot size before sharing.';
    }
    ...
    if (ttlSelect) {
      ...
      ttlSelect.disabled = ['custody-preparing', 'custody-ready', 'creating', 'ambiguous', 'created'].includes(state.phase);
    }
    ...
    if (primary) {
      ...
      primary.disabled = !supports(state.transport) || (!preview && state.phase !== 'error') || state.phase === 'previewing'
        || state.phase === 'custody-preparing' || state.phase === 'creating' || state.phase === 'created';
      primary.textContent = !supports(state.transport) ? 'Sharing unavailable'
        : state.phase === 'error' && !preview ? 'Retry preparation'
        ...
        : state.transport === 'live'
          ? state.ownerCustodyEstablished
            ? state.phase === 'error' ? 'Retry live review room' : 'I saved it — create live room'
    ...
    if (custodyStatus) custodyStatus.textContent = state.ownerCustodyEstablished
  ...
  async function open() {
    ...
    resetCopyButtons();
    if (!state.open) returnFocus = document.activeElement instanceof window.HTMLElement ? document.activeElement : trigger;
    ...
    try {
      ...
      render();
      announce(capabilities.includes('fragment') && state.preview.fragmentEligible
  ...
  function close() {
    if (state.phase === 'creating' || state.phase === 'ambiguous') {
      announce(state.phase === 'ambiguous'
  ...
  async function confirm() {
    ...
    if (transport === 'live' && !state.ownerCustodyEstablished) {
      ...
      try {
        ...
        render();
        announce('Private recovery-bundle download started. Verify the file is saved, then confirm room creation.');
      } catch (error) {
        if (request !== generation || !state.open) return state;
        clearPendingOwnerSigner();
  ...
  async function copy(value, successMessage, button) {
    ...
    try {
      ...
      announce(error?.message ?? 'The value could not be copied. The share receipt remains visible.');
    }
  }
