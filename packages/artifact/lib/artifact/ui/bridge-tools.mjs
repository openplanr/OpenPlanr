...
export function normalizeArtifactViewportZoom(value, viewport) {
  ...
  if (!keys.every((key) => Object.hasOwn(descriptors[key], 'value')
    && typeof descriptors[key].value === 'number' && Number.isFinite(descriptors[key].value))) return null;
  const { x, y, deltaY } = value;
  if (!Number.isFinite(viewport?.width) || !Number.isFinite(viewport?.height)
    || viewport.width < 1 || viewport.height < 1 || viewport.width > 16384 || viewport.height > 16384
    || x < 0 || y < 0 || x > viewport.width || y > viewport.height
    || deltaY === 0 || Math.abs(deltaY) > ARTIFACT_VIEWPORT_ZOOM_MAX_DELTA) return null;
...
export function normalizeArtifactViewportPan(value) {
  ...
  if ((!deltaX && !deltaY) || Math.abs(deltaX) > ARTIFACT_VIEWPORT_PAN_MAX_DELTA
    || Math.abs(deltaY) > ARTIFACT_VIEWPORT_PAN_MAX_DELTA) return null;
...
export function createArtifactViewportGestures(window, emit) {
  ...
  const native = {
    x: getter(window.MouseEvent.prototype, 'clientX'), y: getter(window.MouseEvent.prototype, 'clientY'),
    ctrl: getter(window.MouseEvent.prototype, 'ctrlKey'), meta: getter(window.MouseEvent.prototype, 'metaKey'),
    shift: getter(window.MouseEvent.prototype, 'shiftKey'),
    deltaX: getter(window.WheelEvent.prototype, 'deltaX'), deltaY: getter(window.WheelEvent.prototype, 'deltaY'),
    mode: getter(window.WheelEvent.prototype, 'deltaMode'),
  };
  let enabled = false, disposed = false, timer = 0, pending = null, pendingType = '';
  const clear = () => { if (timer) cancel(timer); timer = 0; pending = null; pendingType = ''; };
  const flush = () => {
    timer = 0; const value = pending; const type = pendingType; pending = null; pendingType = '';
  ...
  const wheel = (event) => {
    ...
    try {
      x = native.x.call(event); y = native.y.call(event); mode = native.mode.call(event);
      zooming = native.ctrl.call(event) || native.meta.call(event); shift = native.shift.call(event);
      deltaX = native.deltaX.call(event); deltaY = native.deltaY.call(event);
    } catch { return; }
    ...
    if (zooming) {
      ...
      value = normalizeArtifactViewportZoom({ x, y, deltaY }, { width: window.innerWidth, height: window.innerHeight });
    } else {
      type = 'viewport.pan';
      if (shift && !deltaX) { deltaX = deltaY; deltaY = 0; }
    ...
    if (!value) return;
    prevent.call(event); stop.call(event);
    if (pendingType && pendingType !== type) flush();
    pendingType = type;
    pending = type === 'viewport.zoom'
      ? { ...value, deltaY: Math.max(-ARTIFACT_VIEWPORT_ZOOM_MAX_DELTA,
        Math.min(ARTIFACT_VIEWPORT_ZOOM_MAX_DELTA, (pending?.deltaY || 0) + deltaY)) }
      : { deltaX: Math.max(-ARTIFACT_VIEWPORT_PAN_MAX_DELTA,
        Math.min(ARTIFACT_VIEWPORT_PAN_MAX_DELTA, (pending?.deltaX || 0) + deltaX)),
      deltaY: Math.max(-ARTIFACT_VIEWPORT_PAN_MAX_DELTA,
        Math.min(ARTIFACT_VIEWPORT_PAN_MAX_DELTA, (pending?.deltaY || 0) + deltaY)) };
    if (!timer) timer = schedule(flush, 16);
  };
  const destroy = () => { disposed = true; enabled = false; clear(); remove('wheel', wheel, true); remove('pagehide', destroy); };
  add('wheel', wheel, { capture: true, passive: false });
  add('pagehide', destroy, { once: true });
  return Object.freeze({ setEnabled(value) { if (disposed || typeof value !== 'boolean') return false; enabled = value; if (!value) clear(); return true; }, destroy });
}
...
export const ARTIFACT_INSPECTION_PROPERTIES = Object.freeze([
  ...
  'color', 'background-color', 'display', 'position', 'gap',
  'padding-top', 'padding-right', 'padding-bottom', 'padding-left',
  'margin-top', 'margin-right', 'margin-bottom', 'margin-left',
  'border-radius', 'border-width', 'border-color', 'box-sizing',
]);
...
export function normalizeArtifactInspection(value, viewport) {
  ...
  const exact = (entry, keys) => plain(entry) && Object.keys(entry).length === keys.length
  ...
  if (!['width', 'height'].every((key) => Number.isInteger(own(size, key)) && own(size, key) >= 1
    && own(size, key) <= 16384 && own(size, key) === viewport?.[key])) return null;
  if (!['x', 'y', 'width', 'height'].every((key) => typeof own(rect, key) === 'number'
    && Number.isFinite(own(rect, key)) && own(rect, key) >= 0)) return null;
  if (rect.x + rect.width > size.width || rect.y + rect.height > size.height) return null;
  if (anchor !== null && (!exact(anchor, own(anchor, 'screen') === undefined ? ['planrId'] : ['planrId', 'screen'])
    || typeof own(anchor, 'planrId') !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,511}$/.test(anchor.planrId)
    || (own(anchor, 'screen') !== undefined && (typeof anchor.screen !== 'string'
      || !/^[^\u0000-\u001f\u007f]{1,128}$/.test(anchor.screen))))) return null;
  if (!exact(styles, ARTIFACT_INSPECTION_PROPERTIES) || !ARTIFACT_INSPECTION_PROPERTIES.every((key) => {
    const text = own(styles, key);
    return typeof text === 'string' && text.length <= 256 && !/[\u0000-\u001f\u007f]/.test(text)
      && !/(?:url\s*\(|https?:|file:)/i.test(text);
  })) return null;
  if (!exact(accessibility, ['role', 'ariaLabel', 'alt', 'tabIndex', 'disabled'])
    || !['role', 'ariaLabel', 'alt'].every((key) => typeof own(accessibility, key) === 'string'
      && own(accessibility, key).length <= 256 && !/[\u0000-\u001f\u007f]/.test(own(accessibility, key)))
    || !Number.isInteger(own(accessibility, 'tabIndex')) || accessibility.tabIndex < -1 || accessibility.tabIndex > 32767
    || typeof own(accessibility, 'disabled') !== 'boolean') return null;
  return Object.freeze({ tagName, anchor: anchor === null ? null : Object.freeze({ ...anchor }),
    rect: Object.freeze({ ...rect }), viewport: Object.freeze({ ...size }),
    styles: Object.freeze({ ...styles }), accessibility: Object.freeze({ ...accessibility }) });
}

export function normalizeArtifactThumbnail(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    || Object.keys(value).length !== 4 || !Object.keys(value).every((key) => ['dataUrl', 'width', 'height', 'label'].includes(key))
    || !Object.values(Object.getOwnPropertyDescriptors(value)).every((descriptor) => Object.hasOwn(descriptor, 'value'))) return null;
  const { dataUrl, width, height, label } = value;
  if (typeof dataUrl !== 'string' || dataUrl.length > ARTIFACT_THUMBNAIL_MAX_DATA_URL
    || !/^data:image\/png;base64,[A-Za-z0-9+/]+=*$/.test(dataUrl)
    || !Number.isInteger(width) || !Number.isInteger(height) || width < 1 || height < 1
    || width > ARTIFACT_THUMBNAIL_MAX_EDGE || height > ARTIFACT_THUMBNAIL_MAX_EDGE
    || typeof label !== 'string' || label.length < 1 || label.length > 128) return null;
...
export function normalizeArtifactBridgeToolResult(requestType, message, viewport) {
  ...
  const exact = (keys) => message && typeof message === 'object' && !Array.isArray(message)
    && Object.keys(message).length === keys.length && Object.keys(message).every((key) => keys.includes(key))
    && Object.values(Object.getOwnPropertyDescriptors(message)).every((descriptor) => Object.hasOwn(descriptor, 'value'));
  ...
  if (requestType === 'thumbnail.request') {
    if (exact([...base, 'reason']) && message.type === 'thumbnail.error'
      && typeof message.reason === 'string' && message.reason.length <= 256) return { valid: true, value: null };
...
export function createArtifactBridgeTools(document, window) {
  ...
  const screen = (element) => {
    ...
    return typeof value === 'string' && /^[^\u0000-\u001f\u007f]{1,128}$/.test(value) ? value : undefined;
  };
  ...
  const inspectElement = (element) => {
    ...
    if (typeof tagName !== 'string' || !/^[a-z][a-z0-9-]{0,63}$/.test(tagName)) return null;
    const rect = bounds.call(element), width = window.innerWidth, height = window.innerHeight;
    ...
    return { tagName, anchor: validId(planrId) ? { planrId, ...(anchorScreen ? { screen: anchorScreen } : {}) } : null,
      rect: { x, y, width: Math.max(0, clamp(rect.right, 0, width) - x), height: Math.max(0, clamp(rect.bottom, 0, height) - y) },
      viewport: { width, height }, styles,
      accessibility: { role: clean(get(element, 'role')), ariaLabel: clean(get(element, 'aria-label')),
        alt: clean(get(element, 'alt')), tabIndex: clamp(Number(element.tabIndex) || 0, -1, 32767),
  ...
  return Object.freeze({
    ...
    inspect(anchor) {
      if (!anchor || !validId(anchor.planrId) || Object.keys(anchor).some((key) => !['planrId', 'screen'].includes(key))
        || (anchor.screen !== undefined && (typeof anchor.screen !== 'string' || !/^[^\u0000-\u001f\u007f]{1,128}$/.test(anchor.screen)))) return null;
      let count = 0;
      for (const element of query('[data-planr-id]')) {
        if (++count > 10000) return null;
        if (get(element, 'data-planr-id') === anchor.planrId && (anchor.screen === undefined || screen(element) === anchor.screen)) return inspectElement(element);
      }
      return null;
    },
    async thumbnail() {
      ...
      try {
        ...
        const cloneStyled = (source) => {
          if (++count > 4000 || Date.now() > deadline) throw new Error('Thumbnail capture limit exceeded.');
          if (source.nodeType === 8 || (source.nodeType === 1 && ['SCRIPT', 'STYLE', 'LINK', 'META'].includes(source.tagName))) return document.createTextNode('');
          const target = cloneNode.call(source, false);
          if (source.nodeType === 1) {
            const styles = computed(source); let css = '';
            for (let index = 0; index < styles.length; index++) {
              if (index >= 2048 || Date.now() > deadline) throw new Error('Thumbnail style limit exceeded.');
              const name = styles[index]; if (!name.startsWith('--')) css += name + ':' + styles.getPropertyValue(name) + ';';
            }
            ...
            if (source.tagName === 'INPUT' || source.tagName === 'TEXTAREA') { target.value = ''; target.textContent = ''; }
            if (source.tagName === 'CANVAS') {
              try { const replacement = create('img'); replacement.src = source.toDataURL('image/png'); setAttribute.call(replacement, 'style', css); return replacement; } catch {}
            }
          }
        ...
        const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + outputWidth + '" height="' + outputHeight
          + '" viewBox="0 0 ' + width + ' ' + height + '"><foreignObject width="' + width + '" height="' + height + '">' + markup + '</foreignObject></svg>';
        image = new Image();
        await new Promise((resolve, reject) => {
          const timer = setTimeout(() => reject(new Error('Thumbnail capture timed out.')), Math.max(1, deadline - Date.now()));
          image.onload = () => { clearTimeout(timer); resolve(); };
          image.onerror = () => { clearTimeout(timer); reject(new Error('Thumbnail rendering unavailable.')); };
          image.src = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
        });
        const canvas = create('canvas'); canvas.width = outputWidth; canvas.height = outputHeight;
      ...
      } finally { if (image) { image.onload = null; image.onerror = null; } capturing = false; }
    },
  });
}
