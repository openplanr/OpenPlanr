...
import { appearanceFields, clone, elementIndex, geometryFields, snapshot } from '../diagram/authoring/model.mjs';
...
export function mountDiagramEditor({ root, session, host = {} }) {
  ...
  const canvasButton = (container, label, action, options = {}) => { const node = options.icon ? iconButton(doc,label,action,{...options,labelClassName:'de-control-label'}) : button(doc, label, action, options); container.append(node); return node; };
  canvasButton(modes, 'Select', 'select-tool', { icon:'select','aria-pressed': 'true' }); canvasButton(modes, 'Pan', 'pan-tool', { icon:'pan','aria-pressed': 'false' });
  ...
  canvasButton(zoomTools, '+', 'zoom-in', { 'aria-label': 'Zoom in' }); canvasButton(zoomTools, 'Fit', 'fit', { icon:'fit' });
  ...
  const guardPropertyDraft = () => {
    ...
    setRightTab('properties'); setRail('right',true,{restoreFocus:false});
    report(PROPERTY_DRAFT_MESSAGE);
    propertyController?.focus?.();
    return false;
  };
  const select = (ids, { force = false } = {}) => {
    const next = [...new Set(ids)], previous = current().view.selection;
    ...
    else { report(''); notice(next.length ? next.length + ' object' + (next.length === 1 ? '' : 's') + ' selected.' : 'Selection cleared.'); }
    return result;
  };
  const submit = (command, selectIds) => {
    if (controllerIsDirty()) { guardPropertyDraft(); return { ok: false, status: 'property-draft' }; }
  ...
  const submitTransaction = (value, { allowDirty = false } = {}) => {
    if (!allowDirty && controllerIsDirty()) { guardPropertyDraft(); return { ok: false, status: 'property-draft' }; }
  ...
  function toggleInert(node, inert) {
    if (inert) { node.setAttribute('inert', ''); node.setAttribute('aria-hidden', 'true'); }
  ...
  function syncPanelState({ focusPanel = false, focusTarget = null } = {}) {
    ...
    for (const [node, open] of [[left, leftOpen], [right, rightOpen]]) {
      if (open) { node.setAttribute('aria-hidden', 'false'); node.removeAttribute('inert'); }
    }
    const outlineControl = bar.querySelector('[data-action="outline"]'), propertiesControl = bar.querySelector('[data-action="properties"]');
    outlineControl?.setAttribute('aria-expanded', String(leftOpen)); outlineControl?.setAttribute('aria-controls', 'diagram-outline-panel');
    propertiesControl?.setAttribute('aria-expanded', String(rightOpen)); propertiesControl?.setAttribute('aria-controls', 'diagram-inspector-panel');
    ...
    for (const [node, open] of [[left, leftOpen], [right, rightOpen]]) {
      if (!open) { node.setAttribute('aria-hidden', 'true'); node.setAttribute('inert', ''); }
    }
    synchronizeBackgroundInteractivity();
  }
  function setRail(side, open, { focusPanel = false, restoreFocus = true } = {}) {
    ...
    if (open && drawer) {
      if (side === 'left') rightOpen = false; else leftOpen = false;
      drawerOpener = control;
    }
    if (side === 'left') leftOpen = open; else rightOpen = open;
  ...
  function setLeftTab(next, { focus = false } = {}) {
    ...
    outlinePane.hidden = !outlineSelected; shapesPane.hidden = outlineSelected;
  ...
  function renderChrome(state) {
    ...
    snapControl?.setAttribute('aria-pressed', String(state.view.snap)); if (snapControl) snapControl.dataset.active = String(state.view.snap);
  ...
  function renderLeft(state) {
    ...
    outlinePane.replaceChildren(); shapesPane.replaceChildren(); setLeftTab(tab);
    ...
    const search = field(doc,'Find in diagram','',{type:'search',placeholder:'Search objects'});outlinePane.append(search.label);
  ...
  function renderRight(state) {
    ...
    propertiesStamp=nextStamp;propertiesSelection=selectionKey;propertiesDirty=false;
    if (!state.bundle) {
      propertiesPane.replaceChildren(element(doc,'p',{},'Access changed. Reopen this diagram.'));
      propertyController=null;updateCommandState(state);return;
    }
    ...
    const submitPropertiesTransaction=value=>{
      ...
      if(result?.ok){propertiesStamp='';synchronizePropertiesAfterApply();}
      return result;
    };
    ...
    propertiesDirty=controllerIsDirty();updateCommandState(state);
  ...
  async function applyPropertiesDraft() {
    ...
    if(form&&!form.checkValidity()){form.reportValidity();form.querySelector(':invalid')?.focus();return false;}
    if (!controller?.apply) {
      report('Apply or revert property changes before saving.'); controller?.focus?.(); return false;
    }
    const result=await controller.apply();
    if (result === false || result?.ok === false) { controller.focus?.(); return false; }
    propertiesDirty=false;updateCommandState(current(),false);synchronizePropertiesAfterApply();return true;
  }
  async function save() {
  ...
  function act(action,value,options={}) {
    ...
    if(action==='outline'||action==='close-outline'){setRail('left',action==='outline'?!leftOpen:false,{focusPanel:action==='outline'&&!leftOpen});return;}
    ...
    if(action==='show-source'||action==='show-revision'){
      ...
      openDialog(action==='show-source'?'Source':'Current revision',body);return;
    }
    if(action==='export-json'){setOverflow(false,{restoreFocus:false});downloadJson(doc,bundle,bundle.diagramId+'.planr-diagram-bundle.json');moreButton.focus({preventScroll:true});return;}
    ...
    if(action==='create'){
      ...
      if(result.ok){if(state.view.camera.fit)fit();stage.focus();}
      return;
    }
    ...
    if(action==='confirm-delete'){const impact=JSON.parse(dialog.dataset.impact);const result=submit({type:'delete',ids,confirmedImpact:impact});if(result.ok){select([],{force:true});closeDialog();}return;}
  ...
  function pointerDown(event) {
    ...
    if(controllerIsDirty()){event.preventDefault();guardPropertyDraft();return;}
    if(handle){
  ...
  function handleKey(event) {
    ...
    if(overflowOpen && event.key==='Escape'){event.preventDefault();setOverflow(false);return;}
    if(overflowOpen && event.key==='Tab'){setOverflow(false);return;}
    if(event.target===moreButton && ['ArrowDown','ArrowUp'].includes(event.key)){
      event.preventDefault();setOverflow(true);
      const items=[...moreMenu.querySelectorAll('[role="menuitem"]')];(event.key==='ArrowUp'?items.at(-1):items[0])?.focus();return;
    }
    if(overflowOpen && event.target.matches?.('[role="menuitem"]')) {
      const items=[...moreMenu.querySelectorAll('[role="menuitem"]:not(:disabled)')],index=items.indexOf(event.target);
      let next=-1;
    ...
    if(event.target.matches?.('[role="tab"]') && ['ArrowLeft','ArrowRight','Home','End'].includes(event.key)) {
      const list=event.target.closest('[role="tablist"]'),tabs=[...list.querySelectorAll('[role="tab"]')],index=tabs.indexOf(event.target);
    ...
    if(event.key.toLowerCase()==='v'&&!event.metaKey&&!event.ctrlKey){tool='select';controlsStamp='';renderChrome(state);notice('Select mode');return;}
    if(event.key.toLowerCase()==='h'&&!event.metaKey&&!event.ctrlKey){tool='pan';controlsStamp='';renderChrome(state);notice('Pan mode');return;}
  ...
  const onResize=()=>{
    ...
    if(breakpointChanged&&breakpoint==='drawer'){
      ...
      const focusTarget=left.contains(active)
        ...
        : right.contains(active) ? bar.querySelector('[data-action="properties"]') : null;
      ...
      leftOpen=false;rightOpen=false;drawerOpener=null;
    }
    ...
    if(!lastBreakpoint&&breakpoint==='drawer'){leftOpen=false;rightOpen=false;}
    if(breakpointChanged&&breakpoint==='desktop'){leftOpen=true;rightOpen=true;drawerOpener=null;}
    ...
    controlsStamp='';renderChrome(current());focusAfterRender?.focus({preventScroll:true});
  };
