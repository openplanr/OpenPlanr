...
export async function mountDiagramOwnerStudio(document = globalThis.document) {
  ...
  try {
    ...
    if (!authoritative.ok || !['ready', 'absent'].includes(authoritative.status)
      || authoritative.diagramId !== config.diagramId || authoritative.capabilities?.read !== true
      || typeof authoritative.capabilities?.write !== 'boolean' || typeof authoritative.recoveryScope !== 'string') {
    ...
    let storage;
    try { storage = window.sessionStorage; } catch { /* Explicit memory-only recovery. */ }
    ...
    session = createDiagramEditorSession({ bundle, transport, recovery,
      capabilities: authoritative.capabilities, acknowledged: authoritative.status === 'ready' });
    mount = mountDiagramEditor({ root, session, host: {
      async readCurrent() {
        const current = await transport.read();
        if (current?.ok && current.status === 'ready' && current.diagramId === authoritative.diagramId
          && current.recoveryScope === authoritative.recoveryScope && current.capabilities?.read === true) return current.bundle;
    ...
    retry.type = 'button'; retry.textContent = 'Retry opening diagram';
