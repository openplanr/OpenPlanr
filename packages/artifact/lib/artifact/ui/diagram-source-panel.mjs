...
function sourceOffset(source, displayedSource, byteOffset) {
	...
	while (
		displayedIndex < displayedSource.length &&
		logicalIndex < logicalOffset
	) {
		if (
			displayedSource[displayedIndex] === "\r" &&
			displayedSource[displayedIndex + 1] === "\n"
		) {
			displayedIndex += 2;
		} else {
			displayedIndex++;
...
function sourceLineRange(source, lineNumber) {
	...
	return ranges[
		Math.max(0, Math.min(ranges.length - 1, (lineNumber ?? 1) - 1))
	];
}
...
export function mountDiagramSourcePanel({
	...
	if (typeof onClose === "function")
		footer.append(button(document, "Close", "source-close"));
	...
	const click = (event) => {
		...
		if (action === "source-import-tab" || action === "source-export-tab") {
			...
			status.textContent =
				"Mermaid copy adopted as an unsaved diagram. Save to keep it.";
