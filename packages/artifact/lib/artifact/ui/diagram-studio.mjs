...
export function mountDiagramStudio(document = globalThis.document) {
  ...
  function resolveGroupMembers(id, trail = new Set()) {
    ...
    const members = new Set(); trail.add(id);
    ...
    trail.delete(id); groupMembers.set(id, members); return members;
  }
  ...
  function hitItem(x, y) {
    return config.items.map((item, index) => ({ item, index, rect: elementBounds(item.id) }))
      .filter(({ item, rect }) => !groupHiddenIds.has(item.id) && rect && x >= rect.x && x <= rect.x + rect.width && y >= rect.y && y <= rect.y + rect.height)
  ...
  drawing.__openPlanrBridge = {
    hitTest(x, y) { const match = hitItem(x, y); return match ? { planrId: match.item.id, screen: config.artifact.id, rect: match.rect } : null; },
    resolve(id) { const rect = elementBounds(id); return rect ? { planrId: id, rect, viewport: config.artifact.viewport } : null; },
  };
  const pointers = new Map();
  const saveState = root.querySelector('[data-save-state]');
  let pendingReview = null, saving = false, failed = false;
  const status = text => { root.querySelector('[data-canvas-status]').textContent = text; };
  const emit = () => root.dispatchEvent(new window.CustomEvent('planr:stage-change', { detail: { ...state } }));
  const setMode = mode => {
    state.reviewMode = mode; root.dataset.planrReviewMode = mode;
  ...
  function setRail(open) {
    ...
    const rail = document.getElementById('planr-review-rail'); rail.inert = !open; rail.setAttribute('aria-hidden', String(!open));
  ...
  function zoom(scale, x = canvas.clientWidth / 2, y = (canvas.clientHeight - 80) / 2) {
    ...
    camera.scale = next; camera.fit = null; paint();
  }
  function focusPoint(x, y, scale = Math.max(.8, camera.scale)) {
    camera.fit = null; camera.scale = Math.min(2, scale);
    camera.x = canvas.clientWidth / 2 - x * camera.scale;
    camera.y = (canvas.clientHeight - 90) / 2 - y * camera.scale;
    canvas.scrollTop = 0; canvas.scrollLeft = 0; paint();
  }
  ...
  function updateOutlineFilter() {
    const value = root.querySelector('[data-search]').value.trim().toLocaleLowerCase(); let count = 0;
  ...
  function expandForItem(id) {
    ...
    for (const groupId of [...collapsedGroups]) {
      if (groupMembers.get(groupId)?.has(id)) { collapsedGroups.delete(groupId); changed = true; }
    }
    if (changed) applyGroupVisibility();
  }
  const feedback = mountArtifactFeedbackRail({ root, document, window, reviewOf: config.reviewOf, initialReview: config.review,
    presentation: { compact: true, pageSize: 30, replyLimit: 2,
      ...
      decorateThread({ element, pin }) {
        ...
        const target = document.createElement('p'); target.className = 'diagram-thread-target';
  ...
  annotations = mountArtifactAnnotations({ root, document, window, stageController: stage, reviewController: feedback,
    onFocusPin: pin => {
      ...
      if (connectionFocus) { connectionFocus = false; updateSelection(); }
      focusPoint(rect ? rect.x + x * rect.width : x * width, rect ? rect.y + y * rect.height : y * height);
    } });

  async function save() {
    if (saving || !pendingReview) return;
    saving = true; failed = false; saveState.disabled = true; saveState.dataset.failed = 'false'; saveState.textContent = 'Saving…';
    while (pendingReview) {
      const review = pendingReview; pendingReview = null;
      try {
        ...
        pendingReview ??= review; failed = true; saveState.disabled = false; saveState.dataset.failed = 'true'; saveState.textContent = 'Save failed · click to retry'; break;
      }
    }
    saving = false;
  ...
  listen(saveState, 'click', () => { if (failed) void save(); });
  listen(window, 'online', () => { if (failed) void save(); });
  listen(window, 'beforeunload', event => { if (pendingReview || saving) { event.preventDefault(); event.returnValue = ''; } });
  ...
  function updateSelection() {
    ...
    for (const relation of config.relations ?? []) {
      if (relation.id === item?.id || relation.from === item?.id || relation.to === item?.id) {
        connected.add(relation.id); connected.add(relation.from); connected.add(relation.to);
      }
    }
    for (const [id, element] of elementIndex) {
  ...
  function selectItem(index, { focus = true } = {}) {
    const item = config.items[index]; if (!item) return;
    ...
    const details = root.querySelector('[data-element-details]'); details.hidden = false;
    ...
    updateSelection();
    if (focus) { if (window.innerWidth <= 700) setOutline(false); focusPoint(item.x, item.y); }
    status(item.label);
  }
  const search = root.querySelector('[data-search]');
  listen(search, 'input', updateOutlineFilter);
  listen(search, 'keydown', event => { if (event.key === 'Enter') { const first = root.querySelector('[data-item-index]:not([hidden])'); if (first) selectItem(Number(first.dataset.itemIndex)); } });
  ...
  function resetPresentation() {
    ...
    const button = root.querySelector('[data-action=present]'); button.textContent = 'Present'; button.setAttribute('aria-pressed', 'false');
    fit();
  }
  async function present() {
    const active = root.dataset.present !== 'true';
    root.dataset.present = String(active); setRail(false);
    root.querySelector('[data-presentation-nav]').hidden = !active;
    const button = root.querySelector('[data-action=present]'); button.textContent = active ? 'Exit presentation' : 'Present'; button.setAttribute('aria-pressed', String(active));
    try { if (active && root.requestFullscreen) await root.requestFullscreen(); else if (!active && document.fullscreenElement) await document.exitFullscreen(); } catch { /* The same fitted workspace remains usable when fullscreen is unavailable. */ }
    if (active) showChapter(0); else fit();
  }
  listen(document, 'fullscreenchange', () => { if (!document.fullscreenElement && root.dataset.present === 'true') resetPresentation(); });
  ...
  actions.connections = () => { connectionFocus = !connectionFocus; updateSelection(); status(connectionFocus ? 'Direct connections highlighted · Click again to show all' : 'Showing the complete diagram'); };
  actions['toggle-group'] = () => {
    const item = config.items[selectedIndex]; if (item?.kind !== 'Group') return;
    if (collapsedGroups.has(item.id)) collapsedGroups.delete(item.id); else collapsedGroups.add(item.id);
    applyGroupVisibility(); selectItem(selectedIndex, { focus: false });
    status(collapsedGroups.has(item.id) ? `${item.label} details collapsed` : `${item.label} details expanded`);
  };
  actions['close-details'] = () => { selectedIndex = -1; connectionFocus = false; updateSelection(); root.querySelector('[data-element-details]').hidden = true; root.querySelectorAll('[aria-current=true]').forEach(element => element.removeAttribute('aria-current')); };
  actions['comment-element'] = () => {
    const item = config.items[selectedIndex]; if (!item) return;
    const rect = elementBounds(item.id); if (!rect) return;
  ...
  listen(root, 'click', event => {
    const button = event.target.closest('[data-action]'); if (button) actions[button.dataset.action]?.();
    const item = event.target.closest('[data-item-index]'); if (item) selectItem(Number(item.dataset.itemIndex));
    if (event.target.closest('[data-planr-close-feedback]')) { setRail(false); root.querySelector('[data-action=review]').focus(); }
  ...
  function resetGesture() {
    pointers.clear(); gesture = null; root.querySelector('[data-selection]').hidden = true; delete canvas.dataset.dragging;
  }
  function beginPinch() {
    const [a, b] = [...pointers.values()];
    const center = { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
    gesture = { type: 'pinch', distance: Math.max(1, Math.hypot(a.x - b.x, a.y - b.y)), scale: camera.scale,
      diagramX: (center.x - camera.x) / camera.scale, diagramY: (center.y - camera.y) / camera.scale };
  ...
  listen(canvas, 'pointerdown', event => {
    ...
    gesture = { type: comment ? 'comment' : 'pan', start: p, x: camera.x, y: camera.y,
      client: { x: event.clientX, y: event.clientY }, item: hitItem((p.x - camera.x) / camera.scale, (p.y - camera.y) / camera.scale) };
    if (!comment) canvas.dataset.dragging = 'true';
  });
  listen(canvas, 'pointermove', event => {
    if (!pointers.has(event.pointerId)) return;
    pointers.set(event.pointerId, point(event)); if (!gesture) return;
    if (gesture.type === 'pinch' && pointers.size === 2) {
      const [a,b] = [...pointers.values()];
      camera.scale = Math.max(.04, Math.min(4, gesture.scale * Math.hypot(a.x-b.x, a.y-b.y) / gesture.distance));
      camera.x = (a.x+b.x)/2 - gesture.diagramX * camera.scale; camera.y = (a.y+b.y)/2 - gesture.diagramY * camera.scale;
      camera.fit = null; paint(); return;
    }
    const p = point(event);
    if (gesture.type === 'pan') { camera.fit = null; camera.x = gesture.x + p.x - gesture.start.x; camera.y = gesture.y + p.y - gesture.start.y; paint(); }
    if (gesture.type === 'comment') {
      ...
      const selection = root.querySelector('[data-selection]'); selection.hidden = false;
  ...
  listen(canvas, 'pointerup', event => {
    ...
    if (gesture?.type === 'pan' && gesture.item && !space && Math.hypot(event.clientX - gesture.client.x, event.clientY - gesture.client.y) < 4) {
      selectItem(gesture.item.index, { focus: false }); if (window.innerWidth <= 700) setOutline(true);
    }
  ...
  listen(canvas, 'lostpointercapture', event => { if (pointers.has(event.pointerId)) resetGesture(); });
  listen(window, 'blur', () => { space = false; resetGesture(); });
  listen(canvas, 'wheel', event => {
    ...
    if (event.ctrlKey || event.metaKey) zoom(camera.scale * Math.exp(-event.deltaY * units * .004), p.x, p.y);
    else { camera.fit = null; camera.x -= (event.shiftKey ? event.deltaY : event.deltaX) * units; camera.y -= (event.shiftKey ? 0 : event.deltaY) * units; paint(); }
  }, { passive: false });
  ...
  listen(document, 'keyup', event => { if (event.key === ' ') space = false; });
  ...
  const resize = new window.ResizeObserver(() => {
    ...
    else { camera.x += (w - lastWidth) / 2; camera.y += (h - lastHeight) / 2; paint(); }
