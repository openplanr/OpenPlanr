...
export type DiagramEditorIconName =
  | 'panel' | 'undo' | 'redo' | 'save' | 'more' | 'share' | 'history' | 'review' | 'properties'
  | 'copy' | 'duplicate' | 'lock' | 'unlock' | 'trash' | 'plus' | 'search';
...
export interface DiagramEditorHostAction {
  ...
  onSelect(context: { session: DiagramEditorSession; state: DiagramEditorState; trigger: HTMLButtonElement }): void;
}
...
export interface DiagramEditorHostPanel {
  ...
  hidden?: (state: DiagramEditorState) => boolean;
  mount(options: {
    root: HTMLElement; session: DiagramEditorSession;
    select: (ids: string[]) => unknown; close: () => void;
  }): (() => void) | null | void;
}
