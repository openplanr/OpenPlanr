import { clone, geometryFields, appearanceFields, semanticFields, snapshot, same, elementIndex, parentIndex, descendants } from '../diagram/authoring/model.mjs';
...
export const transaction = (bundle, operations) => ({ kind: 'diagram-edit-transaction', schemaVersion: '1.0.0', protocolVersion: '1.13.0', transactionId: freshId(), diagramId: bundle.diagramId, base: snapshot(bundle), operations, undoOf: null });
export function placement(id, shape, bounds) {
  return { elementId: id, bounds, route: null, label: null, zIndex: shape === 'container' ? 0 : 1,
    appearance: { shape, fill: shape === 'text' || shape === 'container' ? 'transparent' : 'surface', stroke: shape === 'text' ? 'none' : 'default', strokeWidth: 1.5, strokeStyle: 'solid', fontSize: 14, textAlign: shape === 'container' || shape === 'text' ? 'left' : 'center' },
...
export function createObject(kind, position) {
  ...
  let collection = 'nodes', value, shape = 'rectangle';
  if (kind === 'annotation') { collection = 'annotations'; value = { id, text: 'Add a note', targetId: null }; shape = 'text'; }
  else if (kind === 'container' || kind.endsWith('-lane')) {
    collection = kind === 'container' ? 'groups' : 'lanes'; value = { id, label: kind === 'container' ? 'Container' : 'Lane', members: [] }; shape = 'container';
    Object.assign(bounds, kind === 'vertical-lane' ? { width: 240, height: 480 } : { width: 540, height: 240 });
  } else {
    ...
    shape = { start: 'ellipse', end: 'ellipse', decision: 'diamond', 'data-store': 'cylinder', component: 'rounded-rectangle' }[kind] ?? 'rectangle';
    if (kind === 'decision') bounds.height = 100;
  }
  return { type: 'create', elements: [{ collection, value }], presentation: [placement(id, shape, bounds)] };
}
export function connector(from, to, label = '') {
  const id = freshId('connector');
  const entry = placement(id, 'connector', null);
  entry.route = { mode: 'automatic', strategy: 'orthogonal', from: { side: 'right', offset: .5 }, to: { side: 'left', offset: .5 }, points: [] };
  return { type: 'create', elements: [{ collection: 'relations', value: { id, from, to, label: label || null, kind: 'flow', direction: 'forward', weight: null } }], presentation: [entry] };
}
...
export function duplicateSelection(bundle, ids, copied = null) {
  ...
  const preview = pasteDiagramSelection(bundle, result.value, { idMap, transactionId: freshId(), dx: 24, dy: 24 });
...
export function propertyTransaction(bundle, id, { semantic, geometry, appearance }) {
  ...
  const operations = [];
  if (semantic) operations.push({ type: 'update-semantics', collection: entry.collection, elementId: id, before: semanticFields(entry.collection, entry.value), after: semantic });
  if (geometry) {
    ...
    if (before.bounds && geometry.bounds && (before.bounds.x !== geometry.bounds.x || before.bounds.y !== geometry.bounds.y)) {
      const moved = compileDiagramCommand(bundle, { type: 'move', ids: [id],
        dx: geometry.bounds.x - before.bounds.x, dy: geometry.bounds.y - before.bounds.y }, { transactionId: freshId() });
      if (moved.ok && moved.transaction) {
        ...
        if (target) target.after = { ...target.after, bounds: geometry.bounds,
          route: same(geometry.route, before.route) ? target.after.route : geometry.route,
          label: same(geometry.label, before.label) ? target.after.label : geometry.label, zIndex: geometry.zIndex };
      }
    }
    operations.push({ type: 'set-geometry', changes });
  }
  if (appearance) operations.push({ type: 'set-appearance-locks', changes: [{ elementId: id, before: appearanceFields(current), after: appearance }] });
...
export function arrangementCommand(bundle, ids, mode) {
  const parents = parentIndex(bundle.document), selected = new Set(ids);
  const roots = ids.filter(id => { let parent = parents.get(id); while (parent) { if (selected.has(parent)) return false; parent = parents.get(parent); } return true; });
  ...
  for (const item of mode.startsWith('distribute') ? ordered : boxes) {
    ...
    if (mode === 'align-center') dx = boxes[0].bounds.x + boxes[0].bounds.width / 2 - bounds.x - bounds.width / 2;
    if (mode.startsWith('distribute')) { if (horizontal) dx = offset - bounds.x; else dy = offset - bounds.y; offset += bounds[horizontal ? 'width' : 'height'] + gap; }
    const preview = compileDiagramCommand(bundle, { type: 'move', ids: [item.elementId], dx, dy }, { transactionId: freshId() });
    if (!preview.ok) throw new Error(preview.diagnostics[0].detail);
    for (const op of preview.transaction?.operations ?? []) for (const change of op.changes ?? []) changes.set(change.elementId, change);
  }
...
export function laneArrangementCommand(bundle, laneId, direction) {
  ...
  const container = byId.get(laneId), before = geometryFields(container), horizontal = direction === 'horizontal';
  let x = container.bounds.x + 24, y = container.bounds.y + 48, cross = 0;
  const changes = new Map();
  for (const member of lane.members) {
    const bounds = byId.get(member).bounds;
    if (!bounds) continue;
    const preview = compileDiagramCommand(bundle, { type: 'move', ids: [member], dx: x - bounds.x, dy: y - bounds.y }, { transactionId: freshId() });
    ...
    if (preview.ok) for (const op of preview.transaction?.operations ?? []) for (const change of op.changes ?? []) changes.set(change.elementId, change);
    else {
      ...
      for (const id of moved) {
        const old = geometryFields(byId.get(id)), next = clone(old), dx = x - bounds.x, dy = y - bounds.y;
    ...
    if (horizontal) x += bounds.width + 40; else y += bounds.height + 40;
...
export function addOrthogonalDetour(points) {
  ...
  const detour = horizontal
    ? [{ x: Math.round((start.x + end.x) / 2), y: start.y }, { x: Math.round((start.x + end.x) / 2), y: start.y + 40 }, { x: end.x, y: start.y + 40 }]
    : [{ x: start.x, y: Math.round((start.y + end.y) / 2) }, { x: start.x + 40, y: Math.round((start.y + end.y) / 2) }, { x: start.x + 40, y: end.y }];
