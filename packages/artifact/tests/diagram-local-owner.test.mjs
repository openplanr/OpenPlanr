...
import { createDiagramLocalOwnerAdapter, startDiagramOwner } from '../lib/artifact/diagram/editor/local-owner.mjs';
...
import { makeBundle, makeTransaction, placement } from '../../../tests/protocol/fixtures/diagram-authoring.mjs';
...
function request(url, { method = 'GET', headers = {}, body } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = httpRequest({ hostname: target.hostname, port: Number(target.port), path: target.pathname + target.search, method, headers }, res => {
      const chunks = [];
...
const post = (owner, action, value, headers = {}) => request(`${owner.apiBase}${action}`, {
  method: 'POST', headers: { ...owner.headers, origin: new URL(owner.baseUrl).origin, 'content-type': 'application/json', ...headers }, body: JSON.stringify(value),
});
...
test('offline blank and template create/edit/save/reopen retain complete paired content', async t => {
  ...
  assert.equal((await post(owner, 'initialize', { bundle: blank, transactionId: 'create-blank' })).value.status, 'saved');
  const edit = compileDiagramCommand(blank, { type: 'create', elements: [{ collection: 'nodes', value: { id: 'first-node', label: 'First authored object', kind: 'process', description: null } }], presentation: [placement('first-node')] }, { transactionId: 'create-first-object' });
  ...
  assert.equal((await read(owner)).value.recoveryScope, owner.recoveryScope, 'refresh retains one recovery namespace');
  await owner.close();
  const reopened = await startDiagramOwner({ root, slug: 'checkout' });
  t.after(() => reopened.close());
  assert.notEqual(reopened.recoveryScope, owner.recoveryScope, 'a new authority session has a new browser recovery namespace');
  ...
  assert.equal((await post(templateOwner, 'initialize', { bundle: template, transactionId: 'create-template' })).status, 200);
...
test('review capability and HTTP registration cannot acquire owner read or write authority', async t => {
  ...
  const registration = server.registerOwnerSession(createDiagramLocalOwnerAdapter({ root, slug: 'checkout' }));
  await server.listen();
  const origin = `http://127.0.0.1:${server.port}`;
  const envelope = createArtifactEnvelope({ artifacts: [{ id: 'checkout', title: 'Reviewer copy', html: '<!doctype html><html><body>Published review only</body></html>', viewport: { width: 800, height: 600 }, colorScheme: 'light' }] });
  const registered = await request(`${origin}/internal/v1/sessions`, { method: 'POST', headers: { authorization: `Bearer ${server.controlToken}`, 'content-type': 'application/json' }, body: JSON.stringify({ envelope, cwd: root, owner: true, root, slug: 'checkout' }) });
  assert.equal(registered.status, 201);
  assert.equal(Object.hasOwn(registered.value, 'recoveryScope'), false);
  const reviewer = registered.value;
  const owner = { baseUrl: origin + registration.path, apiBase: origin + registration.path + 'api/', headers: { 'x-openplanr-owner': '1' } };
  await post(owner, 'initialize', { bundle: makeBundle(), transactionId: 'create-draft' });
  ...
  assert.equal((await read(owner)).value.bundle.document.nodes[0].label, makeBundle().document.nodes[0].label);
...
test('owner APIs enforce host, origin, capability, headers, method and closed path boundaries', async t => {
  ...
  const missingOrigin = await request(`${owner.apiBase}initialize`, { method: 'POST', headers: { ...owner.headers, 'content-type': 'application/json' }, body: JSON.stringify(init) });
  assert.equal(missingOrigin.status, 403);
  assert.equal((await post(owner, 'initialize', init, { origin: 'https://attacker.example' })).status, 403);
  assert.equal((await post(owner, 'initialize', init, { 'content-type': 'application/jsonish' })).status, 415);
  assert.equal((await post(owner, 'initialize', init, { 'content-encoding': 'gzip' })).status, 415);
  assert.equal((await post(owner, 'initialize', { ...init, root: '/tmp/other', slug: 'other' })).status, 400);
  assert.equal((await request(`${owner.apiBase}commit`, { headers: owner.headers })).status, 404);
  assert.equal((await request(url, { method: 'OPTIONS', headers: { origin: 'https://attacker.example' } })).status, 403);
  assert.equal((await request(`${url}?path=outside`, { headers: owner.headers })).status, 400);
  assert.equal((await request(`${owner.apiBase}%2foutside`, { headers: owner.headers })).status, 400);
...
test('bounded UTF-8 JSON and prototype rejection leave the connection and source usable', async t => {
  ...
  await createDiagramAuthoringStore({ root, slug: 'checkout' }).initialize(bundle, { transactionId: 'initialize-direct' });
  const headers = { ...owner.headers, origin: new URL(owner.baseUrl).origin, 'content-type': 'application/json' };
  assert.equal((await request(`${owner.apiBase}commit`, { method: 'POST', headers, body: JSON.stringify({ transaction: 'x'.repeat(400) }) })).status, 413);
  assert.equal((await request(`${owner.apiBase}recover`, { method: 'POST', headers: { ...headers, 'content-length': '500' } })).status, 413);
  assert.equal((await request(`${owner.apiBase}recover`, { method: 'POST', headers, body: Buffer.from([123, 34, 120, 34, 58, 34, 255, 34, 125]) })).status, 400);
  const hostile = '{"transaction":{"__proto__":{"polluted":true}}}';
  assert.equal((await request(`${owner.apiBase}commit`, { method: 'POST', headers, body: hostile })).status, 422);
  assert.equal({}.polluted, undefined);
  assert.equal((await request(`${owner.apiBase}recover`, { method: 'POST', headers, body: '[' })).status, 400);
...
test('HTTP idempotency, stale bases and historical recovery preserve the authoritative head', async t => {
  ...
  const stale = await post(owner, 'commit', { transaction: { ...transaction, transactionId: 'stale-edit' } });
  assert.equal(stale.status, 409);
  assert.equal(stale.value.status, 'conflict');
  const changed = structuredClone(transaction); changed.operations[0].after.label = 'Changed bytes with same ID';
  assert.equal((await post(owner, 'commit', { transaction: changed })).status, 409);
  const historical = await post(owner, 'recover', { transactionId: 'create-draft', fingerprint: initialized.value.receipt.fingerprint });
...
test('uncertain HTTP save is recoverable by exact identity without leaking filesystem errors', async t => {
  const root = await workspace(t);
  let interrupt = false;
  const { owner } = await ownerFor(t, { root, storeOptions: { faultInjector: phase => {
    if (interrupt && phase === 'after-replacement') { interrupt = false; throw new Error(`Private failure ${root}/private-data`); }
  } } });
  const bundle = makeBundle();
  ...
  const recovered = await post(owner, 'recover', { transactionId: unknown.value.transactionId, fingerprint: unknown.value.fingerprint });
...
test('HTTP callers cannot redirect local custody or overwrite unowned files', async t => {
  ...
  const failed = await post(owner, 'initialize', { bundle: makeBundle(), transactionId: 'create-draft' });
  ...
  assert.equal((await post(owner, 'initialize', { bundle: makeBundle(), transactionId: 'create-draft' })).status, 409);
...
test('owner shutdown drains the in-flight store operation before closing its authority', async t => {
  ...
  let hold = false;
  const { root, owner } = await ownerFor(t, { storeOptions: { faultInjector: async phase => {
    if (hold && phase === 'after-journal') { entered(); await held; }
  } } });
  const bundle = makeBundle();
  await post(owner, 'initialize', { bundle, transactionId: 'create-draft' });
  hold = true;
...
test('an aborted JSON request releases its handler and leaves later owner reads usable', async t => {
  ...
  await new Promise(resolve => {
    const req = httpRequest({ hostname: target.hostname, port: Number(target.port), path: target.pathname, method: 'POST', agent: false,
      headers: { ...owner.headers, origin: target.origin, 'content-type': 'application/json', 'content-length': '200' } });
    req.on('error', resolve);
    req.on('response', res => { res.resume(); res.once('end', resolve); });
    req.once('socket', socket => socket.once('connect', () => {
      req.write('{');
  ...
  assert.equal((await post(owner, 'commit', { transaction: makeTransaction(bundle) })).value.status, 'saved');
});
