...
test('diagram authoring has a closed browser-safe Artifact and Protocol import graph', async () => {
  ...
  const renderingHelpers = new Set(
    [
      ...
      'diagram/errors.mjs',
      'internal/contrast.mjs',
    ].map((path) => resolve(packageRoot, 'lib/artifact', path)),
  );
  const result = await build({
    absWorkingDir: root,
    entryPoints: [resolve(authoringRoot, 'index.mjs')],
    bundle: true,
    platform: 'browser',
    format: 'esm',
    treeShaking: false,
  ...
  for (const [path, input] of inputs) {
    ...
    assert.ok(
      absolute.startsWith(`${authoringRoot}${sep}`) ||
        absolute.startsWith(`${protocolRoot}${sep}`) ||
        renderingHelpers.has(absolute),
      `Unexpected diagram authoring dependency: ${path}`,
    );
    for (const dependency of input.imports) {
      assert.equal(
        Boolean(dependency.external),
        false,
        `Unbundled platform or model dependency: ${dependency.path}`,
      );
    }
  }
...
test('portable editor bundles without filesystem, Design, hosted identity or Node adapters', async () => {
  ...
  const result = await build({
    absWorkingDir: root,
    entryPoints: [resolve(packageRoot, 'lib/artifact/diagram/editor/index.mjs')],
    bundle: true,
    platform: 'browser',
    format: 'esm',
    treeShaking: false,
  ...
  for (const [file, input] of Object.entries(result.metafile.inputs)) {
    assert.doesNotMatch(
      file,
      /(?:local-owner|review-server|authoring\/(?:store|exports|migration))\.mjs$/u,
      file,
    );
    assert.ok(
    ...
    for (const dependency of input.imports)
      assert.equal(Boolean(dependency.external), false, dependency.path);
  }
});
