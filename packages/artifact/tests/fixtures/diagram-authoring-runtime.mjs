...
export function evaluateKernelCases() {
  ...
  bundle.presentation.elements.push({
    ...
    bounds: { x: 10, y: 10, width: 170, height: 120 }, zIndex: 0,
  });
  sealBundle(bundle);
  freezeData(bundle);
  ...
  const moved = required(compileDiagramCommand(bundle, moveCommand, { transactionId: 'move-nested' }), 'move');
  const movedAgain = required(previewDiagramTransaction(bundle, moved.transaction), 'replay compiled transaction');
  freezeData(moved.bundle);
  const renamed = required(compileDiagramCommand(moved.bundle, freezeData({
    type: 'rename', id: 'node-a', label: 'Accept Café ☕ order',
  }), { transactionId: 'rename-node' }), 'rename');
  freezeData(renamed.bundle);
  const undo = required(createConditionalInverse(renamed.bundle, renamed.inverse, { transactionId: 'undo-rename' }), 'inverse');
  const undone = required(previewDiagramTransaction(renamed.bundle, undo.transaction), 'undo');
  const redo = required(createConditionalInverse(undone.bundle, undone.inverse, { transactionId: 'redo-rename' }), 'redo inverse');
  ...
  hostileBatch.operations.push({ type: 'remove-elements',
    elements: [{ collection: 'annotations', value: { id: 'node-a', text: 'Wrong semantic class', targetId: null } }],
  ...
  lockedBundle.presentation.elements.find(value => value.elementId === 'node-a').locks.position = true;
  ...
  Object.defineProperty(hostileCommand, 'label', { enumerable: true, get() { getterReads++; return 'Do not evaluate'; } });
  const hostile = compileDiagramCommand(bundle, hostileCommand, { transactionId: 'hostile-command' });
  const duplicated = required(compileDiagramCommand(bundle, { type: 'duplicate', ids: ['group-a'], dx: 500, dy: 0,
  ...
  return {
    ...
    initialBounds: Object.fromEntries(bundle.presentation.elements.map(value => [value.elementId, value.bounds])),
  };
}
...
export function runKernelWithoutAmbientEffects() {
  ...
  const replace = (owner, key, label = key) => {
    ...
    const blocked = function () { calls.push(label); throw new Error(`Unexpected kernel effect: ${label}`); };
    Object.defineProperty(owner, key, { configurable: true, writable: true, value: blocked });
    restore.push(() => descriptor ? Object.defineProperty(owner, key, descriptor) : delete owner[key]);
  };
  try {
    for (const name of ['Date', 'fetch', 'setTimeout', 'setInterval', 'WebSocket', 'XMLHttpRequest']) replace(globalThis, name);
