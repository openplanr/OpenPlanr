...
const tx = (bundle, operations, transactionId = 'edit-one') => ({ kind: 'diagram-edit-transaction', schemaVersion: '1.0.0', protocolVersion: '1.13.0', diagramId: bundle.diagramId, base: snapshot(bundle), transactionId, operations, undoOf: null });
const command = (bundle, value, transactionId = 'command-one') => compileDiagramCommand(bundle, value, { transactionId });
function roundTrip(bundle, preview) {
  assert.equal(preview.ok, true, JSON.stringify(preview.diagnostics));
  const inverse = createConditionalInverse(preview.bundle, preview.inverse, { transactionId: 'undo-one' });
...
test('failed second operation never yields a partial result or mutates the input', () => {
  ...
  const cases = [
    { type: 'remove-elements', elements: [{ collection: 'annotations', value: { id: 'node-a', text: 'wrong class', targetId: null } }], presentation: [bundle.presentation.elements[0]] },
    { type: 'update-semantics', collection: 'nodes', elementId: 'node-b', before: { label: 'stale', kind: 'end', description: 'Checkout finished.' }, after: { label: 'Renamed', kind: 'end', description: 'Checkout finished.' } },
    { type: 'unrecognized-operation' },
    { type: 'insert-elements', elements: [{ collection: 'nodes', value: bundle.document.nodes[0] }], presentation: [bundle.presentation.elements[0]] },
  ];
  for (const invalid of cases) {
    const result = previewDiagramTransaction(bundle, tx(bundle, [...makeTransaction(bundle).operations, invalid]));
...
test('a batch may construct forward references but final dangling references reject', () => {
  ...
  const edge = { id: 'edge', from: 'a', to: 'b', kind: 'flow', direction: 'forward', label: null, weight: null };
  const route = { ...placement('edge', 'connector'), bounds: null, route: { mode: 'automatic', strategy: 'straight', from: { side: 'right', offset: 0.5 }, to: { side: 'left', offset: 0.5 }, points: [] } };
  const operations = [{ type: 'insert-elements', elements: [{ collection: 'relations', value: edge }], presentation: [route] }, { type: 'insert-elements', elements: [{ collection: 'nodes', value: a }, { collection: 'nodes', value: b }], presentation: [placement('a'), placement('b')] }];
...
test('locks and containment reject atomically and explicit unlock permits geometry', () => {
  ...
  const after = structuredClone(geometry); after.bounds.x += 10;
  const move = { type: 'set-geometry', changes: [{ elementId: 'node-a', before: geometry, after }] };
  assert.equal(previewDiagramTransaction(bundle, tx(bundle, [move])).ok, false);
  assert.deepEqual(bundle, before);
  const locks = bundle.presentation.elements[0];
  const unlock = { type: 'set-appearance-locks', changes: [{ elementId: 'node-a', before: { appearance: locks.appearance, locks: locks.locks }, after: { appearance: locks.appearance, locks: { ...locks.locks, position: false } } }] };
  roundTrip(bundle, previewDiagramTransaction(bundle, tx(bundle, [unlock, move])));
  const shrink = command(makeBundle(), { type: 'resize', id: 'group-a', bounds: { x: 0, y: 0, width: 60, height: 60 } });
...
test('conditional inverse preserves a newer unrelated field and rejects changed dependencies', () => {
  ...
  const described = command(renamed.bundle, { type: 'describe', id: 'node-a', description: 'New independent description' }, 'describe-one');
  const inverse = createConditionalInverse(described.bundle, renamed.inverse, { transactionId: 'undo-rename' });
  assert.equal(inverse.ok, true, JSON.stringify(inverse.diagnostics));
  ...
  const overlapping = command(renamed.bundle, { type: 'rename', id: 'node-a', label: 'Newer label' }, 'rename-two');
  assert.equal(createConditionalInverse(overlapping.bundle, renamed.inverse, { transactionId: 'undo-two' }).ok, false);
  const edge = { ...bundle.document.relations[0], id: 'edge-new' };
  const added = command(renamed.bundle, { type: 'create', elements: [{ collection: 'relations', value: edge }], presentation: [{ ...bundle.presentation.elements[2], elementId: 'edge-new' }] }, 'new-edge');
  assert.equal(added.ok, true, JSON.stringify(added.diagnostics));
  assert.equal(createConditionalInverse(added.bundle, renamed.inverse, { transactionId: 'undo-three' }).ok, false);
});
...
test('delete and undo restore collection order, source correspondence and emphasis exactly', () => {
  ...
  bundle.document.emphasis = [{ targetId: 'node-a', level: 'primary' }, { targetId: 'node-b', level: 'secondary' }];
  sealBundle(bundle);
  const proposed = command(bundle, { type: 'delete', ids: ['node-a'] });
  assert.equal(proposed.ok, false);
  const deleted = command(bundle, { type: 'delete', ids: ['node-a'], confirmedImpact: proposed.deletionImpact });
...
test('removing an identity does not make it fresh again in the same transaction', () => {
  ...
  const result = previewDiagramTransaction(bundle, tx(bundle, [
    { type: 'remove-elements', elements: [entry], presentation: [bundle.presentation.elements[0]] },
    { type: 'insert-elements', elements: [{ collection: 'nodes', value: { ...entry.value, kind: 'decision' } }], presentation: [bundle.presentation.elements[0]] },
  ]));
  assert.equal(result.ok, false);
...
test('malformed conditional inverse data returns diagnostics without throwing', () => {
  const bundle = makeBundle();
  const inverse = { diagramId: bundle.diagramId, transactionId: 'prior', changes: { semantic: [], presentation: [], sourceMap: [{ collection: 'source-map', elementId: null, path: [], before: 1, after: null }] }, dependencies: [], positions: [], emphasisOrder: [] };
...
test('new manual routes resolve endpoint attachments and the returned transaction replays', () => {
  ...
  route.route.points = [{ x: 999, y: 999 }, { x: 888, y: 888 }];
  const result = command(bundle, { type: 'create', elements: [{ collection: 'relations', value: edge }], presentation: [route] });
  assert.equal(result.ok, true, JSON.stringify(result.diagnostics));
  assert.deepEqual(result.bundle.presentation.elements.find(value => value.elementId === edge.id).route.points, [{ x: 160, y: 65 }, { x: 260, y: 65 }]);
...
test('document order edits identify their semantic references in read and affected sets', () => {
  ...
  const before = { title: bundle.document.title, summary: bundle.document.summary, audience: bundle.document.audience, accessibility: structuredClone(bundle.document.accessibility) };
  const after = structuredClone(before); after.accessibility.readingOrder.reverse();
  const result = previewDiagramTransaction(bundle, tx(bundle, [{ type: 'update-semantics', collection: 'document', before, after }]));
...
test('raw geometry cannot move only a parent away or move its child outside containment', () => {
  for (const [id, x] of [['group-a', 500], ['node-a', 500]]) {
    const bundle = makeBundle();
    const original = structuredClone(bundle);
    const before = geometryFields(bundle.presentation.elements.find(value => value.elementId === id));
    const after = structuredClone(before); after.bounds.x = x;
    const result = previewDiagramTransaction(bundle, tx(bundle, [{ type: 'set-geometry', changes: [{ elementId: id, before, after }] }]));
    assert.equal(result.ok, false);
