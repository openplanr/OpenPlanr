...
test('bundle resolves imported CSS, fonts, images and classic JavaScript without modifying authored files', (t) => {
  const files = {
    'screen.html': '<!doctype html><head><link rel="STYLESHEET" href="css/main.css"></head><body><img src="media/logo.svg"><script src="app.js"></script></body>',
    'css/main.css': '@import "nested/tokens.css" screen; body { background: url("../media/background.png") }',
    'css/nested/tokens.css': '@font-face { font-family: Brand; src: url("../../media/brand.woff2") }',
  ...
  for (const [path, content] of Object.entries(files)) assert.deepEqual(readFileSync(join(root, path)), Buffer.from(content));
});

test('external deferred scripts bind controls after parsing the body and preserve source order', (t) => {
  const root = fixture(t, {
    'screen.html': '<head><script src="first.js" defer></script><script src="second.js" defer></script></head><body><button id="action">Save</button><script>order.push("body");</script></body>',
  ...
  function visit(node) {
    ...
    if (node.tagName === 'script') new Script(node.childNodes.map(({ value }) => value ?? '').join('')).runInNewContext(context);
...
test('nested deferred scripts keep document order when the HTML hierarchy differs', (t) => {
  const root = fixture(t, {
    'screen.html': '<body><section><div><script src="first.js" defer></script></div></section><script src="second.js" defer></script></body>',
...
test('local forms and validation attributes remain interactive while submit destinations are rejected', (t) => {
  const root = fixture(t, { 'screen.html': '<form><label>Email<input name="email" type="email" required></label><button type="submit">Continue</button></form>' });
  assert.match(bundle(root).html, /<form>/u);
  assert.match(bundle(root).html, /type="email" required/u);
  for (const [attribute, value] of [['action', 'https://example.com'], ['action', '/submit'], ['target', '_blank']]) {
    writeFileSync(join(root, 'screen.html'), `<form ${attribute}="${value}"><input name="email"></form>`);
    assert.throws(() => bundle(root), /local submit handlers/u);
  }
  writeFileSync(join(root, 'screen.html'), '<form><button formaction="/submit">Continue</button></form>');
  assert.throws(() => bundle(root), /local submit handlers/u);
});

test('SVG image and use references include their local media and preserve symbol fragments', (t) => {
  const root = fixture(t, {
    'screen.html': '<svg><image href="media/logo.svg"/><use href="media/logo.svg#mark"/><use href="#local"/></svg>',
  ...
  for (const attribute of ['href', 'xlink:href']) {
    writeFileSync(join(root, 'screen.html'), `<svg><image ${attribute}="https://example.com/tracker.svg"/></svg>`);
...
test('relative source escapes, external references, symlink escapes and missing assets fail closed', (t) => {
  ...
  for (const path of ['../external.html', join(outside, 'external.html'), 'https://example.com/screen.html', 'escape.html', 'missing.html']) {
    assert.throws(() => bundle(root, { html: path }), undefined, path);
  }
  assert.throws(() => resolveLocalDocumentFile(root, '.'), /regular file/u);
  writeFileSync(join(root, 'screen.html'), '<style>body{background:url(https://example.com/image.png)}</style>');
  assert.throws(() => bundle(root), /local relative asset/u);
});
...
test('style and source order stay explicit and imported stylesheet cycles fail with a diagnosis', (t) => {
  ...
  const html = bundle(root, { html: 'screen.html', styles: ['project.css'], scripts: ['app.js'] }, { sharedStyles: ['tokens.css'] }).html;
  ...
  assert.throws(() => bundle(root, { html: 'screen.html', styles: ['cycle.css'] }), /Circular stylesheet import/u);
});

test('budget and unsupported source diagnostics never publish a partial result', (t) => {
  const root = fixture(t, { 'screen.html': '<p>Content</p>', 'app.js': 'import thing from "package-name";' });
  assert.throws(() => bundle(root, { html: 'screen.html' }, { maxBytes: 1 }), /budget/u);
  assert.throws(() => bundle(root, { html: 'screen.html', scripts: ['app.js'] }), /self-contained browser JavaScript/u);
  writeFileSync(join(root, 'screen.html'), '<script type="module">document.title = "Module";</script>');
...
test('repeated acyclic imports fail before allocating exponential CSS output', t => {
  ...
  for (let level = 0; level < 20; level++) files[`level-${level}.css`] = `@import "level-${level + 1}.css";@import "level-${level + 1}.css";`;
  files['level-20.css'] = `/*${'x'.repeat(1024)}*/`;
  const root = fixture(t, files);
  assert.throws(() => bundle(root, { html: 'screen.html' }, { maxBytes: 1048576 }), /Stylesheet expansion.*budget/);
  // A zero-byte leaf must not permit unbounded traversal either.
  writeFileSync(join(root, 'level-20.css'), '');
  assert.throws(() => bundle(root, { html: 'screen.html' }, { maxBytes: 1048576 }), /complexity.*budget/);
});
