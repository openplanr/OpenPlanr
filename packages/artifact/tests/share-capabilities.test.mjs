...
function mount(t, options = {}) {
  const envelope = createArtifactEnvelope({
    artifacts: [{ id: 'screen', title: 'Screen', html: '<main>Review me</main>' }],
  });
...
test('capabilities require real preparation and creation handlers and explicit owner custody for live rooms', () => {
  ...
  assert.deepEqual(artifactShareCapabilities({ ...snapshots, prepareOwnerCustody() {} }), [
    'live',
    'fragment',
    'short',
  ]);
  assert.deepEqual(artifactShareCapabilities({ ...snapshots, supportedTransports: ['short'] }), [
    'short',
  ]);
});
...
test('a host without sharing handlers shows actionable unavailable state before creation', async (t) => {
  ...
  for (const button of document.querySelectorAll(
    '[data-planr-share-transport], [data-planr-share-confirm]',
  ))
    assert.equal(button.disabled, true);
  assert.equal(
    document.querySelector('[data-planr-share-fragment-size]').textContent,
    'Unavailable',
  );
...
test('snapshot hosts skip unsupported live room custody and actually create the selected snapshot', async (t) => {
  ...
  const { controller, document } = mount(t, {
    ...
    createShare: async (input) => {
      creation = input;
      return { transport: input.transport, url: 'https://share.openplanr.dev/#v1.fixture' };
    },
  ...
  assert.equal(
    controller.getState().transport,
    'fragment',
    'programmatic selection cannot bypass capability gating',
  );
  await controller.confirm();
...
test('preparation failure has accurate unavailable metrics and can retry', async (t) => {
  ...
  const { controller, document } = mount(t, {
    ...
    prepareShare: async () => {
      if (++attempts === 1) throw new Error('Design is still loading. Retry preparation.');
      return preview;
    },
    createShare: async () => {
  ...
  await controller.open();
  assert.equal(
    document.querySelector('[data-planr-share-short-size]').textContent,
    'Size unavailable',
  );
  assert.equal(
    document.querySelector('[data-planr-share-confirm]').textContent,
    'Retry preparation',
  );
  await controller.confirm();
...
test('unknown preview sizes are rejected instead of being fabricated as zero bytes', async (t) => {
  assert.throws(() => normalizeArtifactSharePreview({}), {
    code: 'E_ARTIFACT_SHARE_PREVIEW_INVALID',
  });
