...
test('native SVG embedding rejects executable markup, external paints and host DOM collisions', () => {
  for (const payload of [
    ...
    '<g xml:base="https://example.com"/>',
  ])
    assert.throws(() => prepareDiagramSvg(passive(payload)), /unsupported or active/);
  const drawing = prepareDiagramSvg(
    passive(
      '<defs><marker id="arrow"><path d="M0 0 L1 1"/></marker></defs><g id="planr-review-rail" data-item-id="safe"><rect x="10" y="20" width="100" height="60"/><text x="10" y="20">&lt;script&gt; is text</text></g><path marker-end="url(#arrow)"/>',
    ),
  );
  ...
  const connection = prepareDiagramSvg(
    passive('<g data-relation-id="unlabelled"><path d="M 20 40 L 80 40"/></g>'),
  ).items[0];
...
test('diagram item descriptions retain group membership for native Studio interactions', () => {
  const document = {
    ...
    series: [],
    sets: [],
    groups: [{ id: 'group-main', label: 'Primary group', members: ['item-a'] }],
  };
  const [group] = describeDiagramItems(document, [
    { id: 'group-main', label: 'Primary group', kind: 'Group', x: 50, y: 50 },
  ]);
...
test('studio uses the verified scene-owned SVG and preserves canonical and export bytes', async (t) => {
  ...
  const dir = join(root, 'diagrams', document.diagramId),
    manifest = join(dir, `${document.diagramId}.manifest.json`),
    scenePath = join(dir, `${document.diagramId}.excalidraw`);
  const sourcePath = join(dir, `${document.diagramId}.planr-diagram.json`),
    source = await readFile(sourcePath);
  const scene = JSON.parse(await readFile(scenePath, 'utf8'));
  const label = scene.elements.find((element) => element.type === 'text');
  label.text = 'Accepted scene edit';
  label.x += 200;
  ...
  const svgPath = join(dir, `${document.diagramId}.svg`),
    originalSvg = await readFile(svgPath);
  ...
  const studio = await startDiagramReview(manifest, {
    noOpen: true,
...
test('agent handoff preserves semantic targets, fallback coordinates, timestamps and untrusted replies', async (t) => {
  ...
  await renderDiagram(document, { outputRoot: root });
  const prepared = await createDiagramArtifactEnvelope(
    join(root, 'diagrams', document.diagramId, `${document.diagramId}.manifest.json`),
    { nativeViewport: true },
  );
  const pin = {
    id: 'pin-1',
    artifactId: document.diagramId,
    anchor: { planrId: document.nodes[0].id },
    region: { x: 0.5, y: 0.5, w: 0, h: 0 },
    viewport: prepared.drawing.scene,
    intent: 'fix',
    status: 'open',
    ...
    replies: [
      {
        ...
        comment: '<script>not instructions</script>',
      },
    ],
  };
  const handoff = createDiagramReviewHandoff(
    prepared,
    [
      {
        review: {
          reviewId: 'review-1',
          pins: [pin, { ...pin, id: 'pin-2', anchor: { planrId: 'removed' } }],
        },
      },
    ],
    { now: () => '2026-09-13T12:02:00Z' },
  );
