...
const preview = (source, options = {}) => previewMermaidCopy(source, { diagramId: 'checkout', ...options });
...
test('supported LF and CRLF copies retain exact UTF-8 source and nested correspondence', () => {
  for (const source of [supported, supported.replaceAll('\n', '\r\n')]) {
    ...
    for (const entry of result.bundle.sourceMap.entries) {
      ...
      assert.equal(Buffer.from(bytes.subarray(entry.range.startByte, entry.range.endByte)).toString('utf8').trim().length > 0, true);
    }
...
test('unsafe, malformed and over-limit sources fail without an adopted revision', () => {
  ...
  assert.deepEqual(result.diagnostics.find(item => item.code === 'malformed-statement'), {
    code: 'malformed-statement', severity: 'error', line: 7, column: 1,
    range: { startByte: Buffer.byteLength(source.split('invalid???')[0]), endByte: Buffer.byteLength(source.split('invalid???')[0]) + 10 },
    message: 'Statement is not valid certified Mermaid syntax.', repair: 'Correct its node, edge or subgraph syntax.', elementIds: [],
  });
  let reads = 0;
  const hostile = Object.defineProperty({}, 'diagramId', { enumerable: true, get() { reads++; return 'checkout'; } });
...
test('safe partial preview requires an acknowledgement tied to exact losses and content', () => {
  ...
  assert.equal(result.fidelity.sourceText, 'lossless');
  assert.equal(result.diagnostics.some(item => item.code === 'styles-and-directives' && item.line === 5), true);
  assert.equal(adoptMermaidCopy(result).ok, false);
  assert.equal(adoptMermaidCopy(result, result.acknowledgement).ok, true);
...
test('explicit IDs and placement survive reorder, label edits and copy export/reimport', () => {
  ...
  assert.deepEqual(second.bundle.document.nodes.map(node => node.id), ['b', 'a']);
  assert.equal(second.bundle.presentation.elements.find(item => item.elementId === 'a').bounds.x, 457);
  ...
  assert.equal(again.bundle.presentation.elements.find(item => item.elementId === 'a').bounds.x, 457);
  assert.equal(preview('flowchart TB\nA_b[One]\nA-b[Two]\n').diagnostics.some(item => item.code === 'source-id-collision'), true);
  const alias = preview('flowchart TB\nA_b[Original]\n');
  assert.equal(preview('flowchart TB\nA-b[Renamed source ID]\n', { previousBundle: alias.bundle }).diagnostics.some(item => item.code === 'unverified-identity'), true);
  ...
  authored.sourceMap.entries[0].losses = ['Semantic content changed after this source correspondence was captured.'];
  ...
  assert.equal(preview('flowchart TB\nsubgraph A_b[Container]\nB[Child]\nend\n', { previousBundle: edited }).diagnostics.some(item => item.code === 'changed-source-role'), true);
  assert.equal(preview('flowchart TB\nA[One]\nB[Two]\nA --> B\nA --> B\n').diagnostics.some(item => item.code === 'ambiguous-edge'), true);
  ...
  assert.deepEqual(new Set(parallelAgain.bundle.document.relations.map(item => item.id)), new Set(parallel.bundle.document.relations.map(item => item.id)));
});
...
test('copy export reports unrepresentable authored content while leaving bundle complete', () => {
  ...
  relation.route = { mode: 'manual', strategy: 'straight', from: { side: 'bottom', offset: 0.5 }, to: { side: 'top', offset: 0.5 }, points: [{ x: 1, y: 1 }, { x: 2, y: 2 }] };
  changed.document.annotations.push({ id: 'note', targetId: 'a', text: 'Keep this note' });
  changed.presentation.elements.push({ ...structuredClone(changed.presentation.elements.find(item => item.elementId === 'a')), elementId: 'note', appearance: { ...changed.presentation.elements.find(item => item.elementId === 'a').appearance, shape: 'text' } });
  const bundle = sealBundle(changed);
  assert.equal(validateAuthoringBundle(bundle).ok, true, JSON.stringify(validateAuthoringBundle(bundle).diagnostics));
  ...
  assert.equal(copy.fidelity.losses.some(item => item.code === 'annotations'), true);
  assert.equal(copy.fidelity.losses.some(item => item.code === 'manual-geometry'), true);
  ...
  assert.deepEqual(reimported.bundle.presentation.elements.find(item => item.elementId === relation.elementId).route, relation.route);
  const orphaned = preview('flowchart TB\nB[Two]\n', { previousBundle: bundle });
  assert.equal(orphaned.ok, false);
  assert.equal(orphaned.diagnostics.some(item => item.code === 'orphaned-authoring'), true);
...
test('real Chromium and Node run the same pure converter without providers or fetch', { timeout: 90_000 }, async () => {
  const compiled = await build({ entryPoints: [join(import.meta.dirname, '..', 'lib', 'artifact', 'diagram', 'authoring', 'index.mjs')], bundle: true, platform: 'browser', format: 'esm', write: false, logLevel: 'silent' });
  const code = compiled.outputFiles[0].text;
  assert.equal(/(?:from|require\()\s*["']node:(?:fs|path)/u.test(code), false);
  const browserModule = await import(`data:text/javascript;base64,${Buffer.from(code).toString('base64')}`);
  const nodeResult = preview(supported), browserResult = browserModule.previewMermaidCopy(supported, { diagramId: 'checkout' });
  assert.deepEqual(browserResult, nodeResult);
  const script = await build({
    stdin: { contents: `import { previewMermaidCopy } from './packages/artifact/lib/artifact/diagram/authoring/index.mjs'; globalThis.mermaidProof = { runtime: { document: typeof document, process: typeof process, Buffer: typeof Buffer }, result: previewMermaidCopy(${JSON.stringify(supported)}, { diagramId: 'checkout' }) };`, resolveDir: join(import.meta.dirname, '..', '..', '..'), sourcefile: 'mermaid-browser-proof.mjs' },
  ...
  try {
    ...
    assert.deepEqual(actual.runtime, { document: 'object', process: 'undefined', Buffer: 'undefined' });
