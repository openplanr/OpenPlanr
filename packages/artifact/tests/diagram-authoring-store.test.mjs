...
const fixtureUrl = new URL(
  '../../../tests/protocol/fixtures/diagram-authoring.mjs',
  import.meta.url,
).href;
const create = (root, options = {}) =>
  createDiagramAuthoringStore({ root, slug: 'checkout', ...options });
...
const rejectsCode = (promise, code) =>
  assert.rejects(promise, (error) => error.code === `E_DIAGRAM_STORE_${code}`);
...
function child(root, body) {
  ...
  const processHandle = spawn(process.execPath, ['--input-type=module', '-e', script], {
    env: { ...process.env, DIAGRAM_TEST_ROOT: root },
    stdio: ['pipe', 'pipe', 'pipe'],
  });
  let output = '';
  let errors = '';
  processHandle.stdout.setEncoding('utf8');
  processHandle.stdout.on('data', (text) => {
    output += text;
  });
  processHandle.stderr.setEncoding('utf8');
  processHandle.stderr.on('data', (text) => {
    errors += text;
  });
  return {
    process: processHandle,
    done: once(processHandle, 'exit').then(([code, signal]) => ({ code, signal, output, errors })),
  };
}
...
test('complete snapshots, exact retry receipts and conditional undo survive store restart', async (t) => {
  ...
  assert.deepEqual(
    (await restarted.history()).map((r) => r.transactionId),
    ['rename-checkout', 'initialize-checkout'],
  );
  const inverse = createConditionalInverse(saved.bundle, saved.receipt.inverse, {
  ...
  assert.deepEqual(
    await restarted.commit(transaction),
    { ...saved, replayed: true },
    'an old receipt still identifies its historical result',
  );
  assert.equal(
    (await restarted.history({ limit: 1, beforeSequence: 3 }))[0].transactionId,
    transaction.transactionId,
  );
  assert.equal(
    (await readdir(join(root, 'diagrams', 'checkout', '.authoring', 'snapshots'))).length,
    2,
    'undo reuses the exact immutable snapshot',
  );
});
...
test('reviewed complete successor commits retain custody and recover exactly after interruption', async (t) => {
  ...
  const change = compileDiagramCommand(
    ...
    { transactionId: 'move-for-review' },
  );
  assert.equal(change.ok, true);
  const expectedBase = { byteDigest: first.receipt.resultBytesDigest, basis: first.receipt.result };
  const interrupted = create(root, {
    faultInjector: (phase) => {
      if (phase === 'after-replacement') throw new Error('interrupted replacement');
    },
  });
  const uncertain = await interrupted.commitSnapshot(change.bundle, {
    transactionId: 'company-successor',
    expectedBase,
  });
  assert.equal(uncertain.ok, false);
  assert.equal(uncertain.status, 'unknown');
  const recovered = await create(root).recover({
    transactionId: 'company-successor',
  ...
  const replay = await store.commitSnapshot(change.bundle, {
    transactionId: 'company-successor',
    expectedBase,
  });
  assert.equal(replay.replayed, true);
  await rejectsCode(
    store.commitSnapshot(bundle, { transactionId: 'stale-successor', expectedBase }),
    'CHANGED',
  );
...
test('each interrupted durability boundary remains unacknowledged until exact recovery', async (t) => {
  for (const phase of [
    ...
    await t.test(phase, async (t) => {
      ...
      const interrupted = create(root, {
        faultInjector: (point) => {
          if (point === phase) throw new Error(`interrupted ${phase}`);
        },
      });
      if (phase === 'before-journal') {
        ...
        assert.equal(
          (await create(root).recover({ transactionId: transaction.transactionId })).status,
          'not-found',
        );
      } else {
...
test('process death during a save is recovered after restart without an age-based lock timeout', async (t) => {
  for (const phase of [
    ...
    await t.test(phase, async (t) => {
      ...
      const run = child(
        root,
        `const store=createDiagramAuthoringStore({root,slug:'checkout',faultInjector:phase=>{if(phase===${JSON.stringify(phase)})process.kill(process.pid,'SIGKILL');}});const state=await store.read();await store.commit(makeTransaction(state.bundle));`,
      );
...
test('competing writers and stale bases cannot overwrite a complete save', async (t) => {
  ...
  const second = compileDiagramCommand(
    bundle,
    { type: 'rename', id: 'node-b', label: 'A competing name' },
    { transactionId: 'competing-rename' },
  ).transaction;
  const settled = await Promise.allSettled([
    create(root).commit(first),
    create(root).commit(second),
  ]);
  assert.equal(
    settled.filter((r) => r.status === 'fulfilled' && r.value.status === 'saved').length,
    1,
  );
  const failure = settled.find((r) => r.status === 'rejected');
  assert.ok(
    ['E_DIAGRAM_STORE_LOCKED', 'E_DIAGRAM_STORE_INVALID_TRANSACTION'].includes(failure.reason.code),
  );
...
test('transaction identity reuse rejects different bytes, including recovery identity mismatch', async (t) => {
  ...
  await rejectsCode(
    store.initialize(bundle, { transactionId: transaction.transactionId }),
    'ID_REUSE',
  );
  await rejectsCode(
    store.recover({
...
test('changed unknown canonical bytes stop recovery without overwriting source or recovery intent', async (t) => {
  const { root, store, bundle } = await initialized(t);
  const transaction = makeTransaction(bundle);
  const uncertain = await create(root, {
    faultInjector: (phase) => {
      if (phase === 'after-journal') throw new Error('disconnect');
    },
  }).commit(transaction);
  const different = compileDiagramCommand(
    bundle,
    { type: 'rename', id: 'node-b', label: 'Unrecognized edit' },
    { transactionId: 'unknown-edit' },
  ).bundle;
...
test('traversal, symlink components, hard links, and unowned collisions leave outside bytes untouched', async (t) => {
  ...
  await rejectsCode(
    create(alias).initialize(makeBundle(), { transactionId: 'initialize' }),
    'PATH',
  );
  ...
  await rejectsCode(
    create(root).initialize(makeBundle(), { transactionId: 'initialize' }),
    'COLLISION',
  );
...
test('post-initialization canonical and snapshot substitution is detected before committing', async (t) => {
  ...
  const snapshotFile = join(
    ...
    '.authoring',
    'snapshots',
    `${result.receipt.resultBytesDigest.slice(7)}.json`,
  );
...
test('blank and 1000-element mixed bundles save without any renderer or rasterizer', async (t) => {
  ...
  dense.document.annotations.push({
    id: 'note-a',
    text: 'A valid dense source is independent of export budgets.',
    targetId: null,
  });
...
test('first initialization can resume after process death before claiming empty metadata folders', async (t) => {
  ...
  const run = child(
    root,
    `const {makeBundle}=await import(${JSON.stringify(fixtureUrl)});const store=createDiagramAuthoringStore({root,slug:'checkout',faultInjector:phase=>{if(phase==='before-ownership')process.kill(process.pid,'SIGKILL');}});await store.initialize(makeBundle(),{transactionId:'initialize-checkout'});`,
  );
  assert.equal((await run.done).signal, 'SIGKILL');
  assert.equal((await create(root).read()).status, 'absent');
  assert.equal(
    (await create(root).initialize(makeBundle(), { transactionId: 'initialize-checkout' })).status,
    'saved',
  );
});
...
test('accessor transactions are rejected without evaluating properties or saving any edit', async (t) => {
  ...
  let reads = 0;
  const transaction = makeTransaction(bundle);
  Object.defineProperty(transaction, 'transactionId', {
    enumerable: true,
    get() {
      reads++;
      return 'hostile';
...
test('legacy adoption resumes after real interruption only while legacy custody remains unchanged', async (t) => {
  for (const changed of [false, true]) {
    await t.test(
      changed
        ? 'changed legacy source rejects adoption'
        : 'unchanged legacy source resumes adoption',
      async (t) => {
        const root = await workspace(t);
        const legacy = JSON.parse(
          await readFile(
            new URL('../fixtures/diagram/grammars/flowchart.planr-diagram.json', import.meta.url),
            'utf8',
          ),
        );
        ...
        const run = child(
          root,
          `const store=createDiagramAuthoringStore({root,slug:'checkout',faultInjector:phase=>{if(phase==='before-ownership')process.kill(process.pid,'SIGKILL');}});await store.initialize(${JSON.stringify(preview.bundle)},{transactionId:'adopt-checkout'});`,
        );
        const exit = await run.done;
        assert.equal(exit.signal, 'SIGKILL', exit.errors);
        if (changed) {
          ...
          await rejectsCode(
            create(root).initialize(preview.bundle, { transactionId: 'adopt-checkout' }),
            'COLLISION',
          );
          ...
          const saved = await create(root).initialize(preview.bundle, {
            transactionId: 'adopt-checkout',
          });
          assert.equal(saved.status, 'saved');
