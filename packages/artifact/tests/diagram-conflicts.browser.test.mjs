...
test('two real owner pages compare base, current and pending edits without overwriting the other author', {
  ...
  await pending
    .getByText(/Conflict.*Unsaved/u)
    .first()
    .waitFor();
  const panel = pending.getByRole('dialog', { name: /compare|conflict/iu });
  await panel.waitFor();
  for (const text of ['Done', 'Another author confirmed', 'My pending change'])
    assert.ok((await panel.textContent()).includes(text), `Comparison shows ${text}`);
  assert.deepEqual(
    (await store.read()).bundle,
    authoritative,
    'A rejected stale save retains the current store revision',
  );
  await panel.getByRole('button', { name: 'Keep my draft', exact: true }).click();
  assert.equal(
    await pending
      .locator('[data-editor-svg] [data-element-id="node-b"]')
      .getAttribute('aria-label'),
    'My pending change',
  );
  await pending.reload();
  await pending.locator('[data-editor-svg]').waitFor();
  assert.equal(
    await pending
      .locator('[data-editor-svg] [data-element-id="node-b"]')
      .getAttribute('aria-label'),
    'My pending change',
    'Refresh retains the pending draft while another revision exists',
  );
  ...
  assert.deepEqual(
    (await store.read()).bundle,
    authoritative,
    'Explicit draft disposal does not add or mutate a stored revision',
  );
});
