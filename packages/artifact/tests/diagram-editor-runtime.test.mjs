...
async function bundled(contents, format = 'iife') {
  const result = await build({
    stdin: { contents, resolveDir: root, sourcefile: 'diagram-editor-proof.mjs' },
    bundle: true,
    platform: 'browser',
    format,
    write: false,
...
test('browser session uses real scoped owner HTTP, saves offline and recovers a lost acknowledgement after refresh', {
  timeout: 45_000,
}, async (t) => {
  ...
  await page.route(owner.baseUrl, (route) =>
    route.fulfill({
      ...
      body: '<!doctype html><title>Diagram session integration</title><main>Session test</main>',
    }),
  );
  const requests = [];
  page.on('request', (request) => requests.push(request.url()));
  const code = await bundled(
    `import * as api from ${JSON.stringify(entry)}; globalThis.editorApi = api;`,
  );
  async function mount() {
    ...
    return page.evaluate(
      async ({ apiBase }) => {
        ...
        const recovery = api.createDiagramEditorRecovery({
          storage: sessionStorage,
          scope: { sessionId: read.recoveryScope, diagramId: read.diagramId },
        });
        const draft = api.createDiagramEditorDraft({
          diagramId: 'browser',
          title: 'Offline browser draft',
        });
        globalThis.editor = await api.openDiagramEditorSession({
          transport,
          recovery,
          create: draft.bundle,
        });
  ...
  const created = await page.evaluate(async () => {
    ...
    const result = editor.submit({
      ...
      elements: [
        {
          collection: 'nodes',
          value: { id: 'step', label: 'Receive order', kind: 'process', description: null },
        },
      ],
      presentation: [
        {
    ...
    return { result, saved, state: editor.getState() };
  });
  assert.equal(created.result.ok, true, JSON.stringify(created.result));
  assert.equal(created.saved.status, 'saved', JSON.stringify(created.saved));
  assert.equal(created.state.bundle.document.nodes[0].id, 'step');
  await page.route(
    `${owner.apiBase}commit`,
    async (route) => {
      await route.fetch();
  ...
  const uncertain = await page.evaluate(async () => {
    ...
    const result = await globalThis.editor.save();
    return { result, state: globalThis.editor.getState(), recovery: Object.values(sessionStorage) };
  });
  assert.equal(uncertain.result.ok, false);
  assert.equal(uncertain.state.saveState, 'offline');
  assert.equal(uncertain.state.pendingCount, 1);
  assert.ok(uncertain.recovery.length);
  assert.ok(
    uncertain.recovery.every((value) => !value.includes('/o/')),
    'No capability URLs in recovery',
  );
  const restored = await mount();
  assert.equal(restored.pendingCount, 1);
  assert.equal(restored.bundle.document.nodes[0].label, 'Saved but response lost');
  const retried = await page.evaluate(async () => ({
    result: await globalThis.editor.save(),
    state: globalThis.editor.getState(),
  }));
  assert.equal(retried.result.ok, true);
  assert.equal(retried.state.saveState, 'saved');
  assert.equal(retried.state.pendingCount, 0);
  const store = createDiagramAuthoringStore({ root: directory, slug: 'browser' });
  assert.equal(
    (await store.history()).length,
    3,
    'Initialization, creation and one rename; no duplicate revision',
  );
  const cancelled = await page.evaluate(() => {
    const editor = globalThis.editor;
    editor.beginGesture();
    editor.previewGesture({ type: 'move', ids: ['step'], dx: 40, dy: 20 });
    window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }));
    return editor.getState();
  });
  assert.equal(cancelled.gesture, null);
  assert.equal(cancelled.pendingCount, 0);
  assert.equal(cancelled.bundle.presentation.elements[0].bounds.x, 20);
  assert.ok(
    requests.every((url) => url.startsWith(owner.baseUrl)),
    'Manual editing needs no external network or company credential',
  );
});

test('portable session and indexed interaction run in a real Worker isolate without DOM or Node globals', {
  timeout: 30_000,
}, async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'planr-editor-worker-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const code = await bundled(
    `
  ...
  const response = await worker.dispatchFetch('https://editor.test');
  ...
  assert.equal(value.undo, true);
  assert.equal(value.redo, true);
  assert.ok(value.hit.hits.some((item) => item.id === 'step'));
  assert.equal(value.state.bundle.document.nodes[0].id, 'step');
  assert.equal(value.state.saveState, 'unsaved');
});
