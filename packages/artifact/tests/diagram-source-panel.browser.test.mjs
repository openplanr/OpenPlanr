...
const dialog = (page) =>
	page.getByRole("dialog", { name: "Mermaid import and export" });
...
test("the exported source panel mounts in a minimal company host through the declared session seam", {
	...
	assert.equal(
		await standalone.evaluate((node) =>
			node.classList.contains("planr-diagram-source-panel"),
		),
		true,
	...
	const lightStyles = await standalone.evaluate((node) => {
		...
		return {
			...
			panelDisplay: getComputedStyle(node.querySelector(".de-source-panel"))
				.display,
	...
	await page.waitForFunction(
		() =>
			getComputedStyle(
				document.querySelector("#company-source-panel .de-primary"),
	...
	assert.deepEqual(
		await standalone.evaluate((node) => ({
			...
			inputBackground: getComputedStyle(node.querySelector(".de-source-input"))
				.backgroundColor,
			primaryBackground: getComputedStyle(node.querySelector(".de-primary"))
				.backgroundColor,
			transitionDuration: getComputedStyle(node.querySelector(".de-primary"))
				.transitionDuration,
		})),
		{
	...
	assert.equal(
		await page
			.getByRole("button", { name: /link|watch|write|overwrite|repository/iu })
			.count(),
		0,
	...
	assert.equal(
		await page.evaluate(() => globalThis.companyHostProof.calls.adopt),
		0,
	);
	...
	assert.ok(
		responsive.right <= 640,
		"the standalone panel stays inside the viewport",
	);
	assert.ok(
	...
	assert.match(
		...
		/Company owner access was revoked/u,
	);
	assert.equal(
	...
	assert.equal(
		await page.getByRole("button", { name: "Download Mermaid copy" }).count(),
		1,
	);
...
test("the canonical source panel bounds pasted input, blocks stale upload adoption and maps displayed CRLF ranges", {
	...
	assert.equal(
		byteRangeSelection.start,
		byteRangeSelection.value.indexOf("invalid???"),
	);
	assert.equal(
		byteRangeSelection.end,
		byteRangeSelection.start + "invalid???".length,
	);
...
test("a new local diagram previews and adopts an exact Mermaid copy only after loss acknowledgement", {
	...
	assert.equal(
		await panel
			.getByText("Container · container: Platform", { exact: false })
			.count(),
		1,
	);
	assert.equal(
		await panel.getByText("Node · data-store", { exact: false }).count(),
		1,
	);
...
test("unsafe and changed source leave the diagram untouched and diagnostics navigate original bytes", {
	...
	assert.equal(
		await panel.getByText("No object was created.", { exact: true }).count(),
		1,
	);
	...
	const fallbackSelection = await panel
		.getByLabel("Mermaid source")
		.evaluate((input) => ({
			start: input.selectionStart,
			end: input.selectionEnd,
	...
	assert.equal(
		...
		fallbackSelection.value.indexOf("A[One]"),
	);
	assert.equal(
...
test("unsafe, oversized, invalid UTF-8 and unsupported copies are rejected without replacing the last source", {
	...
	await panel
		.getByRole("status")
		.filter({ hasText: "This file is not valid UTF-8." })
		.waitFor();
...
test("ambiguous correspondence diagnostics select their original range and continuing canvas object", {
	...
	const original = previewMermaidCopy(
		"flowchart TB\nA[One]\nB[Two]\nA --> B\n",
		{ diagramId: "checkout" },
	);
...
test("the shared source dialog guards property drafts, traps focus and remains readable at desktop and tablet widths", {
	...
	await page
		...
		.filter({
			hasText:
				"Apply or revert property changes before selecting another object.",
		})
		.waitFor();
	assert.equal(
		await label.evaluate((input) => input === document.activeElement),
		true,
	);
	...
	for (const selector of [".de-bar", ".de-work"]) {
		...
		assert.equal(
			await page.locator(selector).getAttribute("aria-hidden"),
			"true",
		);
	}
	...
	assert.equal(
		await exportTab.evaluate((tab) => tab === document.activeElement),
		true,
	);
...
test("export discloses losses and offers the complete bundle, Mermaid copy and SVG separately", {
	...
	const downloadedBundle = JSON.parse(
		readFileSync(await bundleFile.path(), "utf8"),
	);
	...
	assert.match(
		readFileSync(await mermaidFile.path(), "utf8"),
		/^flowchart LR/u,
	);
...
test("uploaded copies stay unlinked and produce an inspectable visual snapshot", {
	...
	await panel
		.getByRole("status")
		.filter({ hasText: "unlinked local copy" })
		.waitFor();
	assert.equal(
		await panel.getByLabel("Mermaid source").inputValue(),
		`\uFEFF${supported}`,
	...
	assert.equal(
		(await store.read()).status,
		"absent",
		"recovery before save creates no revision",
	);
	await page.getByRole("button", { name: "Save diagram" }).click();
	await page.getByText("Saved", { exact: true }).waitFor();
	const saved = await store.read();
