...
import { exportAuthoredDiagram, verifyAuthoredDiagramExports } from '../lib/artifact/diagram/authoring/exports.mjs';
...
function authoredFixture() {
  ...
  bundle.document.relations = [{ id: 'edge-a', from: 'node-a', to: 'node-b', kind: 'flow', direction: 'forward', label: 'Accepted', weight: null }];
  ...
  const bounded = (id, shape, bounds, zIndex, fill = 'surface') => ({ ...placement(id, shape), bounds, zIndex, appearance: { ...placement(id, shape).appearance, fill } });
  bundle.presentation.elements = [
    ...
    { ...placement('edge-a', 'connector'), bounds: null, zIndex: 2,
      route: { mode: 'manual', strategy: 'straight', from: { side: 'right', offset: 0.5 }, to: { side: 'left', offset: 0.5 }, points: [{ x: 220, y: 140 }, { x: 360, y: 140 }] },
...
async function snapshot(directory) {
  return Object.fromEntries(await Promise.all((await readdir(directory)).sort().map(async name => [name, (await readFile(join(directory, name))).toString('base64')])));
}
async function replace(path, bytes) { await chmod(path, 0o600); await writeFile(path, bytes); }
...
function decodePng(bytes) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < stride; x++) {
      ...
      const predicted = [0, left, above, Math.floor((left + above) / 2), paeth(left, above, diagonal)][filter];
      pixels[y * stride + x] = (raw[y * (stride + 1) + x + 1] + predicted) & 255;
    }
  }
  return { width, height, pixel(x, y) { return [...pixels.subarray((Math.round(y) * width + Math.round(x)) * 4, (Math.round(y) * width + Math.round(x)) * 4 + 4)]; } };
}
...
test('SVG, decoded PNG and portable review HTML use one authored snapshot and retain previous exports', async t => {
  const root = await temporaryRoot(t), bundle = authoredFixture(), before = jsonBytes(bundle);
  const exported = good(await exportAuthoredDiagram(bundle, { root }));
  const rendered = good(renderAuthoredDiagramSvg(bundle));
  assert.deepEqual(await readFile(join(exported.directory, 'snapshot.planr-diagram-bundle.json')), before);
  ...
  assert.deepEqual(at(45, 110), color(exported.theme.fills.accent), 'Saved node bounds map directly to raster pixels');
  assert.deepEqual(at(365, 125), color(exported.theme.fills.surface), 'Unrelated node placement and fill are preserved');
  assert.deepEqual(at(10, 190), color(exported.theme.fills.surface), 'Container interior remains visible below the node');
  ...
  const moved = good(compileDiagramCommand(bundle, { type: 'move', ids: ['node-a'], dx: 20, dy: 0 }, { transactionId: 'move-export' }));
  ...
  assert.deepEqual(afterPixel(45, 110), color(next.theme.fills.surface), 'Old node position is no longer filled after the move');
...
test('blank, dense and obstructed diagrams return truthful export diagnostics without invoking rasterization', async t => {
  ...
  let rasterCalls = 0;
  const rasterize = () => { rasterCalls++; throw new Error('Must not run for a rejected scene'); };
  const blank = await exportAuthoredDiagram(makeBundle('flowchart', { blank: true }), { root, rasterize });
  ...
  for (let index = 0; index < 1000; index++) {
    ...
    dense.presentation.elements.push(placement(id, 'rectangle', (index % 40) * 180, Math.floor(index / 40) * 100));
  }
  assert.equal((await exportAuthoredDiagram(sealBundle(dense), { root, rasterize })).code, 'focused-output-required');
  const obstructed = authoredFixture();
  obstructed.document.nodes.push({ id: 'obstacle', label: '', kind: 'process', description: null });
  obstructed.presentation.elements.push({ ...placement('obstacle'), bounds: { x: 270, y: 110, width: 40, height: 60 } });
  const blocked = await exportAuthoredDiagram(sealBundle(obstructed), { root, rasterize });
  assert.equal(blocked.ok, false);
  assert.ok(blocked.diagnostics.some(value => value.rule === 'route-obstruction' && value.elementIds.includes('edge-a')));
...
test('raster failure preserves canonical save bytes and every prior complete derived member', async t => {
  const root = await temporaryRoot(t), bundle = authoredFixture();
  const first = good(await exportAuthoredDiagram(bundle, { root }));
  const prior = await snapshot(first.directory);
  const canonical = join(root, 'diagrams', bundle.diagramId, `${bundle.diagramId}.planr-diagram-bundle.json`);
  const next = good(compileDiagramCommand(bundle, { type: 'rename', id: 'node-a', label: 'Intake' }, { transactionId: 'rename-export' })).bundle;
  const saved = jsonBytes(next); await writeFile(canonical, saved);
  const result = await exportAuthoredDiagram(next, { root, rasterize() { throw new Error('Injected raster failure'); } });
  assert.equal(result.ok, false); assert.equal(result.code, 'export-failed');
...
test('verification rejects another basis, changed renderer and self-consistent substituted SVG', async t => {
  const root = await temporaryRoot(t), bundle = authoredFixture();
  const first = good(await exportAuthoredDiagram(bundle, { root }));
  const nextBundle = good(compileDiagramCommand(bundle, { type: 'rename', id: 'node-a', label: 'Intake' }, { transactionId: 'next-snapshot' })).bundle;
  ...
  good(await verifyAuthoredDiagramExports(bundle, { root }));
  const wrongTheme = structuredClone(first.manifest); wrongTheme.theme.version = '99.0.0';
  ...
  const stale = structuredClone(first.manifest); stale.renderer.version = '99.0.0';
  await replace(manifestPath, jsonBytes(stale));
  assert.equal((await verifyAuthoredDiagramExports(bundle, { root })).code, 'stale-renderer');
  const substituted = structuredClone(first.manifest), svg = Buffer.from('<svg xmlns="http://www.w3.org/2000/svg"></svg>');
  substituted.outputs.find(value => value.mediaType === 'image/svg+xml').transportDigest = digestBytes(svg);
  await replace(join(first.directory, 'diagram.svg'), svg); await replace(manifestPath, jsonBytes(substituted));
  assert.equal((await verifyAuthoredDiagramExports(bundle, { root })).code, 'stale-output');
  assert.equal((await exportAuthoredDiagram(bundle, { root })).ok, false, 'Unowned changed bytes are never replaced');
});

test('path escape, symlink directories, linked members and unowned collisions fail closed', async t => {
  const root = await temporaryRoot(t), outside = await temporaryRoot(t), bundle = authoredFixture();
...
async function unlinkForTest(path) { await rm(path); }

test('exports isolate caller mutation while rasterization is pending', async t => {
  const root = await temporaryRoot(t), bundle = authoredFixture(), expected = structuredClone(bundle);
  const result = good(await exportAuthoredDiagram(bundle, { root, rasterize(svg, options) {
  ...
  assert.deepEqual(await readFile(join(result.directory, 'snapshot.planr-diagram-bundle.json')), jsonBytes(expected));
...
test('an interrupted exporter lock fails closed until explicitly recovered', async t => {
  const basisRoot = await temporaryRoot(t), root = await temporaryRoot(t), bundle = authoredFixture();
