...
test('new comment composer remains viewport sized across canvas zoom and narrow screens, with accessible dismissal', { skip: !enabled }, async (t) => {
  const page = await browserFixture(t);
  for (const scale of [.15, 1, 2.6]) {
    await page.evaluate((value) => { document.querySelector('#canvas').style.transform = 'scale(' + value + ')'; }, scale);
    ...
    const composer = page.locator('[data-planr-annotation-composer]'); await composer.waitFor({ state: 'visible' });
    const box = await composer.boundingBox();
    assert.ok(Math.abs(box.width - 360) < 1, `Composer width must stay 360px at canvas scale ${scale}`);
    assert.ok(box.x >= 11 && box.y >= 11 && box.x + box.width <= 1429 && box.y + box.height <= 889);
    assert.equal(await composer.evaluate((form) => form.parentElement.classList.contains('planr-shell')), true);
    assert.equal(await composer.getAttribute('role'), 'dialog');
    assert.equal(await page.locator('[data-planr-composer-comment]').evaluate((field) => document.activeElement === field), true);
    await page.keyboard.press('Escape'); await composer.waitFor({ state: 'detached' });
    assert.equal(await page.locator('#open').evaluate((button) => document.activeElement === button), true);
  }
  ...
  await page.waitForFunction(() => { const r = document.querySelector('[data-planr-annotation-composer]').getBoundingClientRect(); return r.bottom <= innerHeight - 11; });
  box = await page.locator('[data-planr-annotation-composer]').boundingBox();
  assert.ok(box.width <= 296 && box.height <= 256);
  assert.equal(await page.locator('[data-planr-annotation-composer]').evaluate((form) => getComputedStyle(form).overflowY), 'auto');
...
test('comment drafts suspend during product interaction and resume only on their original screen', { skip: !enabled }, async (t) => {
  const page = await browserFixture(t);
  await page.getByRole('button', { name: 'Add comment', exact: true }).click();
  ...
  await page.evaluate(() => fixture.stage.dispatch({reviewMode:'comment',activeArtifactId:'other'}));
  assert.equal(await page.locator('[data-planr-annotation-composer]').count(), 0, 'A draft cannot move to another screen');
  await page.evaluate(() => fixture.stage.dispatch({activeArtifactId:'screen'}));
  assert.equal(await field.inputValue(), before.comment);
  assert.deepEqual(await page.evaluate(() => fixture.annotations.snapshotDraft()), before);
  await page.getByRole('button', {name:'Close new comment',exact:true}).click();
  await page.evaluate(() => { fixture.stage.dispatch({reviewMode:'interact'}); fixture.stage.dispatch({reviewMode:'comment'}); });
  assert.equal(await page.locator('[data-planr-annotation-composer]').count(), 0, 'Explicit dismissal does not reopen a discarded draft');
  assert.equal(await page.evaluate(() => fixture.annotations.snapshotDraft()), null);
});

test('replies expand compactly, disable empty send, and preserve drafts, caret, filtering and refresh', { skip: !enabled }, async (t) => {
  const page = await browserFixture(t);
  ...
  const field = thread.getByLabel('Reply to thread'); const send = thread.getByRole('button', { name: 'Send reply', exact: true });
  assert.equal(await send.isDisabled(), true); await field.fill('   '); assert.equal(await send.isDisabled(), true);
  await field.fill('Keep my reply draft'); assert.equal(await send.isEnabled(), true);
  const sendBox = await send.boundingBox(); assert.ok(sendBox.width <= 32 && sendBox.height <= 32);
  await field.evaluate((input) => input.setSelectionRange(2, 6));
  await page.evaluate(() => { const next = structuredClone(fixture.rail.getReview()); next.pins[0].replies.push({ id: 'remote', author: { name: 'Other reviewer' }, comment: 'Incoming reply', createdAt: '2026-09-10T10:01:00Z' }); fixture.rail.replaceReview(next); });
  assert.equal(await field.inputValue(), 'Keep my reply draft'); assert.equal(await field.evaluate((input) => document.activeElement === input && input.selectionStart === 2 && input.selectionEnd === 6), true);
  await page.evaluate(() => fixture.rail.setPresentation({ filterPin: pin => pin.id === 'second' }));
  assert.equal(await thread.count(), 0); await page.evaluate(() => fixture.rail.setPresentation({ filterPin: null }));
  assert.equal(await field.inputValue(), 'Keep my reply draft');
  await page.reload(); await page.waitForFunction(() => window.fixture);
  assert.equal(await field.isVisible(), true); assert.equal(await field.inputValue(), 'Keep my reply draft');
  await field.focus(); await page.keyboard.press('Escape');
  assert.equal(await field.isVisible(), false); assert.equal(await thread.getByRole('button', { name: '+ Reply', exact: true }).evaluate((button) => document.activeElement === button), true);
  await thread.getByRole('button', { name: '+ Reply', exact: true }).click(); assert.equal(await field.inputValue(), 'Keep my reply draft');
  await field.press('Control+Enter');
  assert.equal(await field.isVisible(), false); assert.equal(await thread.locator('.planr-reply p').last().textContent(), 'Keep my reply draft');
  assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem('fixture-review-drafts')).replies.first), '');
});
