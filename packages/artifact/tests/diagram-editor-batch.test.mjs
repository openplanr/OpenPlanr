...
const recovery = storage => createDiagramEditorRecovery({ storage, scope: { sessionId: 'hosted-one', diagramId: 'checkout' } });
  ...
  const transport = {
    ...
    async saveBatch(batch) {
      ...
      if (!receipts.has(batch.batchId)) {
        if (batch.initialization) good(await store.initialize(batch.initialization.bundle, { transactionId: batch.initialization.transactionId }));
        for (const transaction of batch.transactions) good(await store.commit(transaction));
        const head = good(await store.read());
        receipts.set(batch.batchId, { ok: true, status: 'saved', bundle: head.bundle, receipt: { batchId: batch.batchId, result: snapshot(head.bundle) } });
      }
...
test('a batch transport saves a new diagram and its edits in one request with a stable identity', async t => {
  ...
  good(editor.submit({ type: 'create', elements: [{ collection: 'nodes', value: { id: 'node-a', label: 'Receive order', kind: 'process', description: null } }], presentation: [placement('node-a')] }));
...
test('an unconfirmed batch is resent exactly before newer edits and is not duplicated', async t => {
  ...
  const { transport, calls } = batchTransport(store, { lose: call => call === 1 ? { ok: false, status: 'unknown' } : null });
  const editor = session(t, { bundle: good(await store.read()).bundle, acknowledged: true, transport });
  ...
  assert.equal(calls.length, 3, 'The unconfirmed batch is retried, then the newer edit is sent separately');
  ...
  assert.equal((await store.history()).length, 4, 'Each edit is stored once even though the first response was lost');
  assert.equal(editor.getState().saveState, 'saved');
});

test('the unconfirmed batch boundary survives recovery after a reload', async t => {
  const store = await storeFor(t, makeBundle()), storage = memoryStorage();
  const lost = batchTransport(store, { lose: () => ({ ok: false, status: 'unknown' }) });
  const bundle = good(await store.read()).bundle;
  const first = session(t, { bundle, acknowledged: true, transport: lost.transport, recovery: recovery(storage) });
  ...
  const second = session(t, { bundle, acknowledged: true, transport: reopened.transport, recovery: recovery(storage) });
  assert.equal(second.getState().pendingCount, 3);
  good(await second.save());
  assert.equal(reopened.calls[0].batchId, lost.calls[0].batchId, 'The reloaded session resends the same batch first');
...
test('a rejected batch commits nothing, so later edits are sent together in a new batch', async t => {
  const store = await storeFor(t, makeBundle());
  const { transport, calls } = batchTransport(store, { reject: call => call === 1 ? { ok: false, httpStatus: 413 } : null });
  const editor = session(t, { bundle: good(await store.read()).bundle, acknowledged: true, transport });
...
test('a stale-base conflict clears the unconfirmed batch and keeps the draft for comparison', async t => {
  const store = await storeFor(t, makeBundle());
  const { transport, calls } = batchTransport(store, { reject: call => call === 1 ? { ok: false, httpStatus: 409 } : null });
  const editor = session(t, { bundle: good(await store.read()).bundle, acknowledged: true, transport });
  ...
  await editor.save();
  assert.equal(calls.at(-1).transactions.length, 2, 'After a definitive rejection the next save rebatches every pending edit');
});

for (const retain of [false, true]) test(`access loss reported by a save updates subscribers and ${retain ? 'retains' : 'clears'} recovery`, async t => {
  const store = await storeFor(t, makeBundle()), storage = memoryStorage();
  const { transport } = batchTransport(store, { reject: () => ({ ok: false, httpStatus: 401 }) });
  const editor = session(t, { bundle: good(await store.read()).bundle, acknowledged: true, transport, recovery: recovery(storage), retainRecoveryOnAccessLoss: retain });
  ...
  assert.equal(events.at(-1).saveState, 'access-changed', 'Subscribers receive the final access state instead of a stuck Saving state');
...
test('revoked read access retains same-identity recovery only when the host asks for it', async t => {
  ...
  for (const retain of [false, true]) {
    ...
    const editor = session(t, { bundle: good(await store.read()).bundle, acknowledged: true, recovery: recovery(storage), retainRecoveryOnAccessLoss: retain });
