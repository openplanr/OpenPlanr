...
test('pixel tolerance scales through the camera and segment lookup does not use an edge bounding rectangle', () => {
  const bundle = mixedBundle(1),
    index = accepted(createDiagramGeometryIndex(bundle)).index;
  const edge = index.get('edge-0'),
    middle = { x: (edge.points[0].x + edge.points[1].x) / 2, y: edge.points[0].y };
  for (const scale of [0.25, 1, 4]) {
    const camera = { x: -101, y: 89, scale };
    const hit = accepted(
      index.query({
        ...screen({ x: middle.x, y: middle.y + 5 / scale }, camera),
        camera,
        tolerance: 6,
      }),
    ).hits.find((value) => value.id === 'edge-0');
    assert.equal(hit.kind, 'segment');
    assert.ok(Math.abs(hit.distance - 5) < 1e-9);
    assert.ok(
      !accepted(
        index.query({
          ...screen({ x: middle.x, y: middle.y + 7 / scale }, camera),
          camera,
          tolerance: 6,
        }),
      ).hits.some((value) => value.id === 'edge-0'),
    );
  }
});
...
test('one moved node updates only it and its incident route among 1,000 cached elements', () => {
  const bundle = mixedBundle(),
    index = accepted(createDiagramGeometryIndex(bundle)).index;
  const unrelated = index.get('node-599'),
    previous = index.stats().geometryResolved;
  const moved = accepted(
    compileDiagramCommand(
      bundle,
      { type: 'move', ids: ['node-7'], dx: 20, dy: 0 },
      { transactionId: 'indexed-move' },
    ),
  );
...
test('nested lane and container movement refreshes descendants and connected endpoints in global coordinates', () => {
  const bundle = mixedBundle(),
    index = accepted(createDiagramGeometryIndex(bundle)).index;
  const oldNode = index.get('node-0'),
    oldEdge = index.get('edge-0'),
    fixed = index.get('group-49');
  const moved = accepted(
    compileDiagramCommand(
      bundle,
      { type: 'move', ids: ['lane-0'], dx: 60, dy: 40 },
      { transactionId: 'indexed-lane' },
    ),
  );
  ...
  assert.deepEqual(index.get('node-0').bounds, {
    ...oldNode.bounds,
    x: oldNode.bounds.x + 60,
    y: oldNode.bounds.y + 40,
  });
  assert.deepEqual(
    index.get('edge-0').points,
    oldEdge.points.map((point) => ({ x: point.x + 60, y: point.y + 40 })),
  );
  assert.deepEqual(index.get('group-49'), fixed);
  assert.equal(
    accepted(index.query({ ...screen(center(index.get('node-0').bounds), camera), camera })).hits[0]
      .id,
    'node-0',
  );
});
...
test('reconnect, manual bend and label changes share exact renderer geometry and retain unrelated routes', () => {
  ...
  const index = accepted(createDiagramGeometryIndex(bundle)).index,
    fixed = index.get('edge-5');
  bundle = accepted(
    compileDiagramCommand(
      bundle,
      { type: 'reconnect', id: 'edge-0', from: 'node-0', to: 'node-2' },
      { transactionId: 'indexed-reconnect' },
    ),
  ).bundle;
  ...
  const placement = bundle.presentation.elements.find((value) => value.elementId === 'edge-0'),
    before = geometryFields(placement),
    after = structuredClone(before);
  const start = index.get('edge-0').points[0],
    end = index.get('edge-0').points.at(-1);
  after.route.strategy = 'orthogonal';
  after.route.points = [
    start,
    { x: start.x + 40, y: start.y },
    { x: start.x + 40, y: start.y - 80 },
    { x: end.x - 40, y: start.y - 80 },
    { x: end.x - 40, y: end.y },
    end,
  ];
  ...
  bundle = accepted(
    compileDiagramCommand(
      bundle,
      { type: 'geometry', changes: [{ elementId: 'edge-0', before, after }] },
      { transactionId: 'indexed-bend-label' },
    ),
  ).bundle;
  assert.deepEqual(accepted(index.update(bundle, ['edge-0'])).updatedIds, ['edge-0']);
  const scene = accepted(resolveDiagramScene(bundle)).scene,
    edge = scene.edges.find((value) => value.id === 'edge-0');
  assert.deepEqual(index.get('edge-0').points, edge.points);
  assert.deepEqual(index.get('edge-0').labelBounds, edge.text.bounds);
  const labelHit = accepted(
    index.query({ ...screen(center(edge.text.bounds), camera), camera }),
  ).hits.find((value) => value.id === 'edge-0');
  assert.equal(labelHit.kind, 'label');
  const bendHit = accepted(
    index.query({ ...screen({ x: start.x + 60, y: start.y - 80 }, camera), camera }),
  ).hits.find((value) => value.id === 'edge-0');
  assert.equal(bendHit.kind, 'segment');
  assert.ok(
    !accepted(
      index.query({
        ...screen({ x: start.x + 80, y: start.y - 30 }, camera),
        camera,
        tolerance: 2,
      }),
    ).hits.some((value) => value.id === 'edge-0'),
...
test('equal revisions do no geometry work, deleted IDs leave no hit entries, and returned records are detached', () => {
  const bundle = mixedBundle(1),
    index = accepted(createDiagramGeometryIndex(bundle)).index;
  const node = index.get('node-0');
  node.bounds.x = 999999;
  ...
  const old = index.get('edge-0'),
    point = { x: (old.points[0].x + old.points[1].x) / 2, y: old.points[0].y };
  ...
  bundle.presentation.elements = bundle.presentation.elements.filter(
    (value) => value.elementId !== 'edge-0',
  );
  sealBundle(bundle);
  const update = accepted(index.update(bundle, ['edge-0']));
  assert.deepEqual(update.removedIds, ['edge-0']);
  assert.deepEqual(update.updatedIds, []);
  assert.equal(index.get('edge-0'), null);
  assert.equal(index.stats().lastUpdateResolved, 0);
  assert.ok(
    !accepted(index.query({ ...screen(point, camera), camera })).hits.some(
      (value) => value.id === 'edge-0',
    ),
  );
});
...
test('query and update reject unsafe data or extreme selection work without changing cached geometry', () => {
  ...
  let reads = 0;
  const input = { x: 0, y: 0 };
  Object.defineProperty(input, 'camera', {
    enumerable: true,
    get() {
      reads++;
      return camera;
  ...
  assert.equal(index.query(input).ok, false);
  assert.equal(reads, 0);
  const invalid = {};
  Object.defineProperty(invalid, 'presentation', {
    enumerable: true,
    get() {
      reads++;
  ...
  assert.equal(index.update(invalid, []).ok, false);
  assert.equal(reads, 0);
  ...
  for (const camera of [
    ...
    assert.equal(index.query({ x: 1, y: 1, camera }).ok, false);
...
test('reordering equal-z objects updates hit priority without recomputing their geometry', () => {
  ...
  bundle.presentation.elements.reverse();
  sealBundle(bundle);
