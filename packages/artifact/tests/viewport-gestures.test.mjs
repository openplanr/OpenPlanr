...
import { normalizeArtifactViewportPan, normalizeArtifactViewportZoom } from '../lib/artifact/ui/bridge-tools.mjs';
...
test('viewport zoom requires opt-in, exact authenticated identity and bounded coordinates', () => {
  ...
  for (const value of [{ ...zoom, x: -1 }, { ...zoom, y: 601 }, { ...zoom, deltaY: Infinity },
    { ...zoom, deltaY: 1001 }, { ...zoom, deltaY: 0 }, { ...zoom, x: '120' }, { ...zoom, scale: 1.2 }]) {
    assert.equal(normalizeArtifactViewportZoom(value, viewport), null);
  }
  const getter = { ...zoom }; Object.defineProperty(getter, 'x', { get() { throw new Error('Do not execute getters'); } });
  assert.equal(normalizeArtifactViewportZoom(getter, viewport), null);
  const source = {}, nonce = createArtifactBridgeNonce();
  const data = { channel: 'openplanr.artifact-anchor', schemaVersion: '1.0.0', type: 'viewport.zoom', artifactId: 'screen', nonce, ...zoom };
  ...
  for (const bad of [{ ...event, source: {} }, { ...event, origin: 'https://example.test' },
    { ...event, data: { ...data, nonce: createArtifactBridgeNonce() } },
    { ...event, data: { ...data, artifactId: 'different' } }, { ...event, data: { ...data, extra: true } },
    { ...event, data: { ...data, deltaY: NaN } }]) assert.equal(validateArtifactBridgeMessage(bad, contract).ok, false);
  assert.equal(validateArtifactBridgeMessage(event, { ...contract, viewportGesturesEnabled: false }).ok, false);
  assert.equal(validateArtifactBridgeMessage(event, { ...contract, viewportGesturesEnabled: undefined }).ok, false);
});
...
test('viewport pan requires opt-in, exact authenticated identity and bounded deltas', () => {
  ...
  for (const value of [{ ...pan, deltaX: Infinity }, { ...pan, deltaY: 1001 },
    { deltaX: 0, deltaY: 0 }, { ...pan, deltaX: '84' }, { ...pan, extra: true }]) {
    assert.equal(normalizeArtifactViewportPan(value), null);
  }
  const source = {}, nonce = createArtifactBridgeNonce();
  const data = { channel: 'openplanr.artifact-anchor', schemaVersion: '1.0.0', type: 'viewport.pan', artifactId: 'screen', nonce, ...pan };
  const contract = { source, nonce, artifactId: 'screen', viewport, viewportGesturesEnabled: true };
  const event = { source, origin: 'null', data };
...
for (const kind of ['local', ...(process.env.PLANR_HOSTED_BRIDGE_SOURCE ? ['hosted'] : [])]) {
  test(`${kind}: real iframe wheel gestures are opt-in, scale-independent and restored after load`, { skip: process.env.PLANR_BROWSER_TESTS !== '1' }, async (t) => {
    ...
    let hostedRuntime;
    if (kind === 'hosted') {
      const bundle = await build({ stdin: { resolveDir: root, contents: `import {prepareHostedArtifact,createHostedBridgeClient} from ${JSON.stringify(process.env.PLANR_HOSTED_BRIDGE_SOURCE)};${setup}window.cleanup=createHostedBridgeClient(new Map([['screen',${JSON.stringify(nonce)}]])).attach({artifact,frame});document.body.append(frame);frame.src=URL.createObjectURL(new Blob([prepareHostedArtifact(${JSON.stringify(product)},{artifactId:'screen',nonce:${JSON.stringify(nonce)}})],{type:'text/html'}));` },
        alias: { '#artifact-bridge-tools': fileURLToPath(new URL('../lib/artifact/ui/bridge-tools.mjs', import.meta.url)) }, bundle: true, write: false, platform: 'browser', format: 'iife' });
      hostedRuntime = bundle.outputFiles[0].text;
    }
    const server = createServer((req, res) => {
      if (req.url === '/stage.js') { res.setHeader('content-type', 'application/javascript'); return res.end(`(async()=>{${setup}const options=__OPENPLANR_ARTIFACT_STAGE_OPTIONS__;window.cleanup=options.bridgeClient.attach({artifact,frame});document.body.append(frame);frame.src=URL.createObjectURL(await options.resolveArtifactSource(artifact));})()`); }
      if (req.url === '/hosted.js') { res.setHeader('content-type', 'application/javascript'); return res.end(hostedRuntime); }
      if (req.url === '/artifact/screen') { res.setHeader('content-type', 'application/octet-stream'); return res.end(prepareArtifactDocument({ html: product, artifactId: 'screen', nonce, parentOrigin: origin }).html); }
      res.setHeader('content-type', 'text/html'); res.end(`<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;height:100vh;overflow:hidden}</style><body>${kind === 'hosted' ? '<script src="/hosted.js"></script>' : '<script>' + renderArtifactParentRuntime({ artifactBaseUrl: '/artifact/', stageRuntimeUrl: '/stage.js', nonce }) + '</script>'}`);
    });
    await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
    t.after(async () => { await browser.close(); await new Promise((resolve) => server.close(resolve)); });
    origin = `http://127.0.0.1:${server.address().port}`;
    const page = await browser.newPage({ viewport: { width: 1200, height: 800 } }); page.setDefaultTimeout(10000); await page.goto(origin);
    ...
    await page.mouse.move(180, 150); await page.mouse.wheel(0, 60);
    await child().waitForFunction(() => window.productWheels > 0);
    assert.equal(await page.evaluate(() => zooms.length), 0);
    assert.equal(await page.evaluate(() => frame.__openPlanrBridge.setViewportGestures('true')), false);
    ...
    await page.mouse.move(180, 150); await page.mouse.wheel(32, 48);
    await page.waitForFunction(() => pans.length > 0);
    assert.deepEqual(await page.evaluate(() => pans.at(-1)), { deltaX: 32, deltaY: 48 });
    assert.equal(await child().evaluate(() => window.productWheels), productWheels, 'Canvas pan must not activate product wheel handlers');
    ...
    assert.ok(Math.abs(zoom.x - 300) < 2 && Math.abs(zoom.y - 250) < 2, 'Coordinates must use unscaled iframe CSS pixels');
    assert.ok(zoom.deltaY < 0 && zoom.deltaY >= -1000);
    assert.equal(await child().evaluate(() => productModifiers), before, 'Camera gestures must not activate product zoom handlers');
    assert.equal(await page.evaluate(() => window.visualViewport.scale), 1, 'Camera gesture must not zoom browser chrome');
    const count = await page.evaluate(() => zooms.length);
    await child().evaluate(() => dispatchEvent(new WheelEvent('wheel', { ctrlKey: true, deltaY: -50, clientX: 50, clientY: 50, cancelable: true })));
    await page.evaluate((nonce) => {
      const base={channel:'openplanr.artifact-anchor',schemaVersion:'1.0.0',type:'viewport.zoom',artifactId:'screen',nonce,x:10,y:10,deltaY:10};
      for(const [data,source,origin] of [[{...base,nonce:'wrong'},frame.contentWindow,'null'],[base,window,'null'],[base,frame.contentWindow,location.origin],[{...base,deltaY:1001},frame.contentWindow,'null'],[{...base,extra:true},frame.contentWindow,'null']])dispatchEvent(new MessageEvent('message',{data,source,origin}));
    }, nonce);
    await page.waitForTimeout(40); assert.equal(await page.evaluate(() => zooms.length), count);
    // Reload of the trusted execution copy keeps the caller's explicit opt-in.
    await page.evaluate(() => { frame.dataset.planrBridgeTrusted='false';frame.src=frame.src; });
    await page.waitForFunction(() => frame.dataset.planrBridgeTrusted === 'true');
    await page.mouse.move(180,150); await page.keyboard.down('Control'); await page.mouse.wheel(0, -50); await page.keyboard.up('Control');
    await page.waitForFunction((previous) => zooms.length > previous, count);
    await page.evaluate(() => frame.__openPlanrBridge.setViewportGestures(false)); await page.waitForTimeout(40);
    ...
    await page.evaluate(() => { frame.__openPlanrBridge.setViewportGestures(true);window.savedBridge=frame.__openPlanrBridge;cleanup(); });
