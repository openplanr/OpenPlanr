...
import { previewLegacyDiagramDocument, previewLegacyDiagramMigration } from '../lib/artifact/diagram/authoring/migration.mjs';
...
const fixturePath = new URL('../fixtures/diagram/grammars/flowchart.planr-diagram.json', import.meta.url);
const fixture = JSON.parse(await readFile(fixturePath, 'utf8'));
function supported() {
  const doc = structuredClone(fixture);
  doc.nodes.forEach(node => { node.kind = 'process'; });
  doc.relations = [{ id: 'flow', from: 'item-a', to: 'item-b', kind: 'flow', label: 'Continue', weight: null }];
...
async function bytes(directory) {
  ...
  return Object.fromEntries(await Promise.all(entries.map(async name => [name, await readFile(join(directory, name))])));
}

test('migration captures the legacy scene and semantic IDs in preview only', async t => {
  const root = await workspace(t), doc = supported();
  ...
  assert.deepEqual(preview.bundle.document.nodes.map(node => node.id), doc.nodes.map(node => node.id));
  for (const box of legacy.boxes) {
    assert.deepEqual(preview.bundle.presentation.elements.find(item => item.elementId === box.id).bounds,
      { x: box.x, y: box.y, width: box.width, height: box.height });
  }
  const route = preview.bundle.presentation.elements.find(item => item.elementId === 'flow').route;
  assert.deepEqual(route.points, legacy.edges[0].routePoints.map(([x, y]) => ({ x, y })));
  assert.deepEqual(await bytes(directory), before, 'Preview never creates or alters any source or output member');
});
...
test('migration refuses changed bytes, substituted output manifests and symlinked collections', async t => {
  ...
  assert.equal((await previewLegacyDiagramMigration({ root: other, slug: doc.diagramId })).diagnostics[0].code, 'unsafe-legacy-path');
});
...
test('explicit adoption preserves the legacy source and prevents a legacy renderer overwrite', async t => {
  ...
  for (const [name, contents] of Object.entries(before)) assert.deepEqual(await readFile(join(directory, name)), contents);
  assert.equal((await store.read()).bundle.bundleDigest, preview.bundle.bundleDigest);
});
