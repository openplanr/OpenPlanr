...
const session = (bundle = makeBundle(), options = {}) =>
  createDiagramEditorSession({ bundle, acknowledged: true, nextTransactionId: ids(), ...options });
...
test('offline blank creation, edit, save and reopen retain the complete paired revision', async (t) => {
  ...
  const editor = await openDiagramEditorSession({
    transport: store,
    create: draft.bundle,
    nextTransactionId: ids(),
  });
  assert.equal(editor.getState().saveState, 'unsaved');
  const created = editor.submit({
    type: 'create',
    elements: [
      {
        collection: 'nodes',
        value: { id: 'first', label: 'First step', kind: 'process', description: null },
      },
    ],
    presentation: [placement('first')],
  });
...
test('validated templates receive diagram identity without retaining linked source custody', () => {
  ...
  invalid.document.nodes[0].label = '<script>alert(1)</script>';
  assert.equal(
    createDiagramEditorDraft({ diagramId: 'invalid', title: 'Invalid', template: invalid }).ok,
    false,
  );
});
...
test('the named process template starts a new diagram with a connected start, step and end', () => {
  ...
  assert.equal(draft.ok, true, JSON.stringify(draft));
  assert.deepEqual(
    draft.bundle.document.nodes.map((node) => node.kind),
    ['start', 'process', 'end'],
  );
  ...
  assert.equal(
    createDiagramEditorSession({ bundle: draft.bundle }).getState().pendingCount,
    0,
    'The template belongs to the new diagram, not to its pending edits',
  );
  const unknown = createDiagramEditorDraft({
    diagramId: 'kanban',
    title: 'Kanban',
    template: 'kanban',
  });
  ...
  assert.equal(
    pending.submit({
      ...
      elements: [
        {
          collection: 'nodes',
          value: { id: 'draft-node', label: 'Draft', kind: 'process', description: null },
        },
      ],
      presentation: [placement('draft-node')],
    }).ok,
    true,
  );
...
test('one multi-object gesture commits one inverse; cancel events persist no partial edit', () => {
  ...
  for (const eventName of ['keydown', 'pointercancel', 'lostpointercapture', 'blur']) {
    ...
    if (eventName === 'keydown') Object.defineProperty(event, 'key', { value: 'Escape' });
  ...
  for (let dx = 1; dx <= 5; dx++)
    assert.equal(editor.previewGesture({ type: 'move', ids: ['lane-a'], dx, dy: 10 }).ok, true);
  assert.equal(editor.completeGesture().ok, true);
  assert.equal(editor.getState().pendingCount, 1);
  const changed = editor.getState().bundle;
  assert.equal(
    changed.presentation.elements[0].bounds.x,
    bundle.presentation.elements[0].bounds.x + 5,
  );
...
test('explicit layout preview cancels without content and applies as one transaction', () => {
  ...
  assert.deepEqual(editor.getState().bundle, bundle);
  editor.beginGesture();
  editor.previewLayout({ targetIds: ['node-a', 'node-b'], columns: 1 });
  assert.equal(editor.completeGesture().ok, true);
  assert.equal(editor.getState().pendingCount, 1);
  assert.equal(editor.undo().ok, true);
  assert.deepEqual(editor.getState().bundle, bundle);
...
test('clipboard copies a bounded fragment and kernel remaps internal connectors and annotations', () => {
  ...
  const copied = copyDiagramSelection(source, ['group-a', 'node-b']);
  ...
  const pasted = pasteDiagramSelection(blank, JSON.stringify(copied.value), {
    idMap,
    transactionId: 'paste',
    dx: 40,
    dy: 50,
  ...
  const editor = session(blank);
  assert.equal(editor.submitTransaction(pasted.transaction).ok, true);
  assert.equal(
    pasteDiagramSelection(blank, copied.value, { idMap: {}, transactionId: 'invalid-paste' }).ok,
    false,
  );
  assert.equal(
    pasteDiagramSelection(blank, ' '.repeat(1024 * 1024 + 1), { idMap, transactionId: 'oversized' })
      .ok,
    false,
  );
  const hostile = structuredClone(copied.value);
  hostile.sourceBundle.presentation.elements[0].appearance.fill = 'url(https://invalid.test)';
  assert.equal(
    pasteDiagramSelection(blank, hostile, { idMap, transactionId: 'hostile' }).ok,
    false,
  );
});
...
test('equal revision and review refresh preserve personal view without changing document digests', () => {
  ...
  const editor = session(bundle);
  editor.setView({
    camera: { x: -25, y: 35, scale: 0.5, fit: null },
    selection: ['node-a'],
  ...
  assert.ok(hit.hits.some((item) => item.id === 'node-a'));
...
test('recovery replays exact pending transactions after refresh and rejects foreign scopes', () => {
  ...
  const scope = { sessionId: 'owner-1', diagramId: 'checkout' };
  const editor = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({ storage: memory, scope }),
  });
  editor.submit(rename('Recovered label'));
  const draft = editor.getState().bundle;
  const reopened = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({ storage: memory, scope }),
  });
  assert.deepEqual(reopened.getState().bundle, draft);
  assert.equal(reopened.getState().pendingCount, 1);
  assert.equal(reopened.getState().saveState, 'unsaved');
  const alien = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({
      storage: memory,
      scope: { ...scope, sessionId: 'owner-2' },
    }),
  });
  assert.equal(alien.getState().pendingCount, 0);
  const copied = [...memory.values.values()][0];
  memory.setItem('openplanr:diagram-editor:1:owner-3:checkout', copied);
  const rejected = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({
      storage: memory,
      scope: { ...scope, sessionId: 'owner-3' },
    }),
  });
  assert.equal(rejected.getState().pendingCount, 0);
  assert.match(rejected.getState().recovery.warning, /safely/u);
});
...
test('blocked or quota-limited recovery reports memory-only while preserving edits', () => {
  ...
  const editor = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({
      storage: blocked,
      scope: { sessionId: 'owner', diagramId: 'checkout' },
    }),
  });
  ...
  const limited = session(makeBundle(), {
    recovery: createDiagramEditorRecovery({
      storage: storage(),
      scope: { sessionId: 'owner', diagramId: 'checkout' },
      maxBytes: 20,
  ...
  limited.submit(rename('Quota limit'));
  assert.equal(limited.getState().recovery.mode, 'memory-only');
});

test('conditional undo after authoritative overlapping changes never replaces the newer revision', async (t) => {
  const bundle = makeBundle();
  const store = await storeFor(t, 'checkout');
  await store.initialize(bundle, { transactionId: 'initialize' });
  const editor = session(bundle, { transport: store });
  editor.submit(rename('My change'));
  await editor.save();
  const external = compileDiagramCommand(editor.getState().bundle, rename('Another author'), {
    transactionId: 'external',
  });
  await store.commit(external.transaction);
  editor.refresh(external.bundle);
  const before = editor.getState().bundle;
  const undo = editor.undo();
  assert.equal(undo.ok, false);
  assert.deepEqual(editor.getState().bundle, before);
