...
for (const transport of ['blob', 'srcdoc']) {
  test(`phone-sized ${transport} frame pool stays bounded through authenticated navigation and disposal`, {
    ...
    const envelope = createArtifactEnvelope({
      artifacts,
      viewer: { mode: 'single', activeArtifactId: 'screen-0', presentation: 'canvas' },
    });
    ...
    const server = createServer((req, res) => {
      ...
      if (req.url.startsWith('/artifact/')) {
        const id = decodeURIComponent(req.url.slice('/artifact/'.length)),
          artifact = artifacts.find((value) => value.id === id);
        if (!artifact) {
        ...
        requested.push(id);
        preparing++;
        peakPreparing = Math.max(peakPreparing, preparing);
      ...
      const html = renderArtifactShellDocument({ envelope }, { stageRuntimeUrl: '/unused.js' })
        ...
        .replace(
          ...
          `<script>${renderArtifactParentRuntime({ artifactBaseUrl: '/artifact/', stageRuntimeUrl: '/stage.js', nonce })}</script><script>globalThis.__OPENPLANR_ARTIFACT_STAGE_OPTIONS__={...globalThis.__OPENPLANR_ARTIFACT_STAGE_OPTIONS__,sourceTransport:${JSON.stringify(transport)}};</script>`,
        );
    ...
    assert.deepEqual(
      requested,
      ['screen-0'],
      'Opening a phone review must not initialize the other 26 documents',
    );
    assert.equal(
      await page.evaluate(() => fixture.stage.getFrame('screen-0').dataset.planrBridgeTrusted),
      'true',
    );
    await page.evaluate(() => {
    ...
    assert.equal(
      await page.evaluate(() => fixture.stage.getFrame('screen-0').__openPlanrBridge === undefined),
      true,
    );
    const returned = await page.evaluate(async () => {
      await fixture.stage.ensureFrames(['screen-0']);
      return fixture.stage.getFrame('screen-0').__openPlanrBridge !== window.firstBridge;
    });
    assert.equal(returned, true, 'Revisiting an evicted screen establishes a fresh bridge');
    await page.evaluate(() =>
      fixture.stage.dispatch({ type: 'set-active', artifactId: 'screen-0' }),
    );
    ...
    const disposed = await page.evaluate(async () => {
      const { stage } = fixture;
      stage.destroy();
      const result = await stage.ensureFrames(['screen-2']).then(
        () => '',
        (error) => error.name,
      );
      return {
