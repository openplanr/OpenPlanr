...
test('an adopted initial copy retains exact source and initialization identity through recovery and first save', async (t) => {
  ...
  const preview = good(
    previewMermaidCopy(SOURCE_TEXT, { diagramId: blank.diagramId, title: blank.document.title }),
  );
  const copy = good(adoptMermaidCopy(preview, preview.acknowledgement)).bundle;
  const store = createDiagramAuthoringStore({ root, slug: blank.diagramId }),
    storage = memoryStorage();
  let initialization = null;
  const transport = {
    ...store,
    async initialize(bundle, identity) {
  ...
  const original = session(t, blank, {
    ...
    nextTransactionId: () => 'initial-copy',
  });
  ...
  assert.deepEqual(
    Buffer.from(state.bundle.originalSource.text, 'utf8'),
    Buffer.from(SOURCE_TEXT, 'utf8'),
  );
  ...
  assert.equal(state.needsInitialization, true);
  assert.equal(state.canUndo, false);
  assert.equal(state.canRedo, false);
  assert.equal(
    (await store.read()).status,
    'absent',
    'Adoption remains an unsaved local initialization',
  );
  ...
  const reopened = session(t, blank, {
    ...
    transport,
    recovery: recovery(storage),
    nextTransactionId: () => 'replacement-copy',
  });
  ...
  assert.equal(
    ...
    1,
    'The adopted copy is the sole initial durable revision',
  );
...
test('an older save acknowledgement retains edits created while that save was in flight', async (t) => {
  ...
  await delayed.committed.promise;
...
test('lost acknowledgement followed by refresh and exact retry settles recovery without a false conflict', async (t) => {
  ...
  const transport = {
    ...
    async commit(transaction) {
      ...
      if (loseResponse) {
        loseResponse = false;
        throw new Error('Connection closed after durable save');
      }
      return result;
    },
  };
  ...
  assert.equal(original.getState().pendingCount, 1);
  original.dispose();
  const reopened = session(t, good(await store.read()).bundle, {
    transport,
    recovery: recovery(storage),
    nextTransactionId: ids('reopened'),
  });
  assert.deepEqual(reopened.getState().bundle, expected);
  good(await reopened.save());
  assert.equal(
    reopened.getState().saveState,
    'saved',
    'An exact replay that matches the refreshed head resolves uncertainty',
  );
...
test('a historical acknowledgement cannot replace a newer authoritative comparison', async (t) => {
  ...
  await delayed.committed.promise;
  const newer = await externalEdit(
    store,
    rename('node-b', 'External completion'),
    'external-completion',
  );
  ...
  assert.deepEqual(state.bundle, draft);
...
test('the latest invalid gesture cannot commit an earlier valid preview; indexed selection uses the session camera', (t) => {
  ...
  good(editor.beginGesture());
  good(editor.previewGesture({ type: 'move', ids: ['node-a'], dx: 5, dy: 0 }));
...
test('undo after a conflicting authoritative refresh preserves newer content and the undo opportunity', async (t) => {
  const { store, bundle } = await persisted(t),
    editor = session(t, bundle, { transport: store });
  good(editor.submit(rename('node-a', 'My label')));
  good(await editor.save());
  const newer = await externalEdit(
    store,
    rename('node-a', 'Another author label'),
    'external-rename',
  );
  good(editor.refresh(newer.bundle));
  const result = editor.undo({ transactionId: 'undo-original' });
  assert.equal(result.ok, false);
  assert.ok(result.diagnostics.some((item) => item.rule === 'inverse-conflict'));
  assert.deepEqual(editor.getState().bundle, newer.bundle);
  assert.equal(editor.getState().pendingCount, 0);
...
test('recovery copied across owner scopes is concealed and unavailable storage retains an honest in-memory draft', (t) => {
  ...
  assert.deepEqual(other.getState().bundle, bundle);
  assert.equal(other.getState().pendingCount, 0);
  ...
  assert.deepEqual(offline.getState().bundle, edited);
  assert.equal(offline.getState().pendingCount, 1);
  assert.equal(offline.getState().saveState, 'unsaved');
  assert.equal(offline.getState().recovery.mode, 'memory-only');
  assert.ok(offline.getState().recovery.warning);
});

for (const action of ['dispose', 'revoke'])
  test(`late save responses after ${action} cannot repopulate cleared recovery or emit events`, async (t) => {
    const { store, bundle } = await persisted(t),
      delayed = delayedCommit(store),
      storage = memoryStorage();
    const editor = session(t, bundle, {
    ...
    else editor.setCapabilities({ read: false, write: false });
    ...
    assert.equal(
      (await store.history()).length,
      2,
      'The response can be ignored without denying the actual durable save',
    );
  });
...
test('a mismatched successful acknowledgement is returned as failure and leaves the exact edit pending', async (t) => {
  ...
  assert.equal(
    (await editor.save()).ok,
    false,
    'An unusable ok:true payload is not a successful save result',
  );
  assert.equal(editor.getState().pendingCount, 1);
  assert.notEqual(editor.getState().saveState, 'saved');
...
test('reentrant save subscribers share one in-flight queue head and cannot drop a newer edit', async (t) => {
  ...
  release.resolve();
  await Promise.all([saving, reentered]);
  assert.equal(
    editor.getState().pendingCount,
    1,
    'The second edit has not been submitted or acknowledged',
  );
...
test('invalid same-scope recovery stays available for explicit recovery instead of being silently deleted', (t) => {
  ...
  const invalid = good(
    compileDiagramCommand(bundle, rename('node-a', 'Recoverable user intent'), {
      transactionId: 'damaged-pending',
    }),
  ).transaction;
  ...
  assert.equal(
    ...
    original.length,
    'Validation rejection is not permission to discard the stored draft',
  );
  assert.ok(
    original.every(([key, value]) => storage.entries.get(key) === value),
    'Rejected recovery bytes remain unchanged',
  );
});
...
test('unknown initialization can be retried after refresh with its original identity before later edits', async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const bundle = makeBundle(),
    store = createDiagramAuthoringStore({ root, slug: bundle.diagramId }),
    storage = memoryStorage();
  let loseResponse = true;
  const transport = {
    ...store,
    async initialize(value, identity) {
  ...
  const original = session(t, bundle, {
    acknowledged: false,
    transport,
    recovery: recovery(storage),
  });
  ...
  assert.equal((await original.save()).ok, false);
  assert.equal(original.getState().needsInitialization, true);
  original.dispose();
  const reopened = session(t, good(await store.read()).bundle, {
    transport,
    recovery: recovery(storage),
  ...
  assert.equal(
    (await store.history()).length,
    2,
    'Replayed initialization and the later edit are each durable once',
  );
});
...
test('default transaction IDs remain valid when UUID entropy starts with a digit', async (t) => {
  ...
  Object.defineProperty(globalThis.crypto, 'randomUUID', {
    configurable: true,
    value: () => `${++sequence}0000000-0000-4000-8000-000000000000`,
  });
  try {
...
test('a malformed recovered initialization identity cannot replace the current usable session', (t) => {
  ...
  const transaction = good(
    compileDiagramCommand(bundle, rename('node-a', 'Retained intent'), {
      transactionId: 'valid-pending',
    }),
  ).transaction;
  good(
    archive.save({ base: bundle, initialization: '1-invalid-init', transactions: [transaction] }),
  );
  const retained = [...storage.entries];
  const editor = session(t, bundle, { recovery: archive });
  assert.deepEqual(editor.getState().bundle, bundle);
  assert.equal(editor.getState().pendingCount, 0);
  assert.equal(editor.getState().needsInitialization, false);
  assert.ok(editor.getState().recovery.warning);
...
test('malformed recovery envelopes are retained without silently clearing or replacing them', (t) => {
  for (const draft of [undefined, null, false, 0, 'damaged', []]) {
    ...
    const raw = JSON.stringify({
      version: 1,
      scope: { sessionId: 'owner-one', diagramId: 'checkout' },
      draft,
    });
