...
import { compileDiagramCommand, previewDiagramTransaction, createConditionalInverse } from '../lib/artifact/diagram/authoring/index.mjs';
import { makeBundle, placement, sealBundle, clone } from '../../../tests/protocol/fixtures/diagram-authoring.mjs';

const compile = (bundle, command, id = 'gesture-a') => compileDiagramCommand(bundle, command, { transactionId: id });
const accepted = result => { assert.equal(result.ok, true, JSON.stringify(result.diagnostics)); assert.ok(result.transaction); return result; };
const bounds = (bundle, id) => bundle.presentation.elements.find(item => item.elementId === id).bounds;
...
test('create, rename and describe compile precise edits without inventing IDs or moving other objects', () => {
  ...
  const created = accepted(compile(blank, { type: 'create', elements: [{ collection: 'nodes', value: { id: 'new-node', label: 'Checkout', kind: 'process', description: null } }], presentation: [placement('new-node')] }));
  assert.deepEqual(created.bundle.document.nodes, [{ id: 'new-node', label: 'Checkout', kind: 'process', description: null }]);
  const renamed = accepted(compile(created.bundle, { type: 'rename', id: 'new-node', label: 'Accept payment' }));
  const described = accepted(compile(renamed.bundle, { type: 'describe', id: 'new-node', description: 'Reserve inventory before capture.' }));
...
test('moving an ancestor and a selected descendant transforms each shape and internal connector exactly once', () => {
  ...
  const result = accepted(compile(bundle, { type: 'move', ids: ['lane-a', 'group-a', 'node-a'], dx: 40, dy: 20 }));
  ...
  assert.deepEqual(edge.route.points, [{ x: 200, y: 85 }, { x: 300, y: 85 }]);
...
test('reparent and group/ungroup use one semantic parent and retain global geometry', () => {
  const bundle = makeBundle('swimlane');
  const reparented = accepted(compile(bundle, { type: 'reparent', ids: ['node-a'], parentId: 'lane-a', index: 1 }));
  ...
  const groupPlacement = { ...placement('new-group', 'container'), bounds: { x: 0, y: 0, width: 430, height: 150 } };
  const grouped = accepted(compile(reparented.bundle, { type: 'group', group: { id: 'new-group', label: 'Payment' }, ids: ['node-a', 'node-b'], placement: groupPlacement, parentId: 'lane-a' }));
  assert.deepEqual(grouped.bundle.document.groups.find(item => item.id === 'new-group').members, ['node-a', 'node-b']);
  ...
  assert.equal(cyclic.ok, false); assert.equal(cyclic.bundle, undefined);
});

test('shrinking around children and changing locked geometry fail atomically; explicit unlock enables movement', () => {
  const bundle = makeBundle();
  const shrink = compile(bundle, { type: 'resize', id: 'group-a', bounds: { x: 0, y: 0, width: 50, height: 50 } });
  assert.equal(shrink.ok, false); assert.equal(shrink.bundle, undefined);
  bundle.presentation.elements[0].locks.position = true; sealBundle(bundle);
  const before = JSON.stringify(bundle);
  const locked = compile(bundle, { type: 'move', ids: ['group-a'], dx: 10, dy: 0 });
  assert.equal(locked.ok, false); assert.equal(locked.bundle, undefined); assert.equal(JSON.stringify(bundle), before);
  const item = bundle.presentation.elements[0];
  const unlocked = accepted(compile(bundle, { type: 'appearance', changes: [{ elementId: item.elementId, before: { appearance: item.appearance, locks: item.locks }, after: { appearance: item.appearance, locks: { ...item.locks, position: false } } }] }));
...
test('duplicate/paste use supplied IDs, retain originals and disclose excluded external links', () => {
  const bundle = makeBundle();
  const before = JSON.stringify(bundle);
  const command = { type: 'duplicate', ids: ['group-a'], idMap: { 'group-a': 'group-copy', 'node-a': 'node-copy', 'note-a': 'note-copy' }, dx: 30, dy: 50 };
  ...
  assert.equal(result.bundle.document.annotations.find(item => item.id === 'note-copy').targetId, 'node-copy');
  assert.deepEqual(result.bundle.document.groups.find(item => item.id === 'group-copy').members, ['node-copy']);
  assert.equal(result.bundle.document.nodes[0].id, 'node-a');
  assert.equal(JSON.stringify(bundle), before);
  assertContent(undo(result.bundle, result.inverse).bundle, bundle);
  const pasted = accepted(compile(makeBundle('flowchart', { blank: true }), { ...command, type: 'paste', sourceBundle: bundle }));
  assert.equal(pasted.bundle.document.nodes.length, 1);
  assert.equal(pasted.bundle.document.relations.length, 0);
  assert.equal(compile(bundle, { ...command, idMap: { ...command.idMap, 'node-a': 'node-a' } }).ok, false);
});
...
test('delete requires its exact connector, annotation, membership and reading-order impact; undo restores ordering and source correspondence', () => {
  ...
  assert.equal(preview.ok, false); assert.equal(preview.transaction, undefined); assert.equal(preview.bundle, undefined);
  ...
  const result = accepted(compile(bundle, { type: 'delete', ids: ['node-a'], confirmedImpact: preview.deletionImpact }));
  assert.deepEqual(result.bundle.document.nodes.map(item => item.id), ['node-b']);
...
test('conditional undo permits an unrelated edit and rejects overlap or newly introduced incident dependencies', () => {
  const bundle = makeBundle();
  bundle.document.nodes.push({ id: 'unrelated', label: 'Elsewhere', kind: 'process', description: null });
  ...
  const unrelated = accepted(compile(edited.bundle, { type: 'rename', id: 'unrelated', label: 'Updated unrelated node' }, 'later-a'));
  ...
  assert.equal(restored.bundle.document.nodes.find(item => item.id === 'unrelated').label, 'Updated unrelated node');
  const overlapping = accepted(compile(edited.bundle, { type: 'rename', id: 'node-a', label: 'Pay later' }, 'later-b'));
  assert.equal(createConditionalInverse(overlapping.bundle, edited.inverse, { transactionId: 'undo-conflict' }).ok, false);
  ...
  newEdge.elementId = 'edge-new'; newEdge.route.mode = 'automatic'; newEdge.route.points = [];
  const attached = accepted(compile(moved.bundle, { type: 'create', elements: [{ collection: 'relations', value: { ...bundle.document.relations[0], id: 'edge-new' } }], presentation: [newEdge] }, 'later-edge'));
  assert.equal(createConditionalInverse(attached.bundle, moved.inverse, { transactionId: 'undo-move' }).ok, false);
});

test('closed command validation and cancellation leave input unchanged and return no applicable transaction', () => {
  const bundle = makeBundle(), original = JSON.stringify(bundle);
  for (const command of [null, { type: 'execute', script: 'bad' }, { type: 'move', ids: ['node-a'], dx: 1, dy: 1, authority: 'owner' }, { type: 'move', ids: ['node-a'], dx: Infinity, dy: 0 }, { type: 'reparent', ids: ['edge-a'], parentId: 'group-a' }]) {
    const result = compile(bundle, command); assert.equal(result.ok, false); assert.equal(result.bundle, undefined);
  }
  const canceled = compileDiagramCommand(bundle, { type: 'cancel' });
  assert.equal(canceled.ok, true); assert.equal(canceled.transaction, null); assert.equal(canceled.bundle, undefined);
...
test('copy maps accept valid reserved-looking IDs and detach excluded annotation targets by own key', () => {
  ...
  bundle.document.annotations.push({ id: 'note-a', text: 'Keep this note', targetId: 'constructor' });
  bundle.presentation.elements.push(placement('note-a', 'text', 20, 200));
  sealBundle(bundle);
  const copied = accepted(compile(bundle, { type: 'duplicate', ids: ['constructor', 'prototype'], idMap: { constructor: 'copy-a', prototype: 'copy-b', 'note-a': 'copy-note' } }));
  assert.equal(copied.bundle.document.annotations.find(value => value.id === 'copy-note').targetId, 'copy-a');
  const detached = accepted(compile(bundle, { type: 'duplicate', ids: ['note-a'], idMap: { 'note-a': 'note-copy' } }));
  assert.equal(detached.bundle.document.annotations.find(value => value.id === 'note-copy').targetId, null);
...
test('optional numeric command fields reject explicit null without mutating the source', () => {
  const bundle = makeBundle('swimlane'), before = JSON.stringify(bundle);
  const duplicate = { type: 'duplicate', ids: ['node-b'], idMap: { 'node-b': 'node-copy' } };
  for (const command of [{ ...duplicate, dx: null }, { ...duplicate, dy: null }, { type: 'reparent', ids: ['node-a'], parentId: 'lane-a', index: null }]) {
