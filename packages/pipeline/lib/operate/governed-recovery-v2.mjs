...
import { canonicalizeJson, sha256Jcs } from '../protocol/canonical-json.mjs';
...
const ROLLBACK_TOOL_CAPABILITY = Object.freeze({ id: 'operate-action-rollback', version: PROTOCOL_VERSION });
...
function provenTerminalError(error, context = {}) {
  ...
  return new PipelineError(
    ...
    error?.message ?? 'Rollback terminalization failed after exact baseline restoration was proved.',
    error?.fix ?? '',
    {
      ...inherited,
      retryable: false,
...
function runtimeHead(state) {
  ...
  if (!entry || entry.eventHash !== state.eventHead.hash) fail('STATE_TRANSITION_INVALID', 'Runtime checkpoint Event head is incomplete.');
  return { previousEvent: { sequence: entry.sequence, eventHash: entry.eventHash }, causationId: entry.eventId };
}
...
async function readCheckpoint(checkpointStore, currentState) {
  if (typeof checkpointStore?.readSnapshot !== 'function') {
    fail('RESULT_CONTRACT_INVALID', 'Governed recovery requires a durable read-only checkpoint primitive.');
  }
  let snapshot;
  try {
  ...
  const ancestry = sameHead(durable.eventHead, currentState.eventHead)
  ...
  if (durable.eventHead.sequence < currentState.eventHead.sequence || !ancestry
...
async function commitCheckpoint(checkpointStore, expectedState, nextState, context) {
  ...
  try {
    response = await checkpointStore.compareAndSwap(freeze({
      expectedEventHead: clone(expectedState.eventHead), nextState: clone(nextState),
      phase: context.phase, operationId: context.operationId,
      requestFingerprint: context.requestFingerprint,
    }));
  } catch (error) {
    fail('OPERATION_UNCERTAIN', 'Governed recovery checkpoint commit failed.', {
      ...context, cause: error?.code ?? error?.name ?? 'checkpoint-error',
    });
  }
  const receipt = snapshotExactDataRecord(response, ['committed', 'state']);
  if (receipt === null || typeof receipt.committed !== 'boolean') {
    fail('RESULT_CONTRACT_INVALID', 'Governed recovery checkpoint store returned an invalid CAS receipt.');
  }
  ...
  if (receipt.committed && sha256Jcs(state) !== sha256Jcs(nextState)) {
    fail('CONCURRENT_MODIFICATION', 'Governed recovery checkpoint committed divergent bytes.', context);
  }
  if (!receipt.committed && sameHead(state.eventHead, expectedState.eventHead)) {
    fail('CONCURRENT_MODIFICATION', 'Governed recovery checkpoint rejected CAS without a newer durable state.', context);
  }
...
export function buildOperatingRollbackPlanV2({
  operation, result, baseline, rollbackPlanId, eligibility = 'eligible', expiresAt,
} = {}) {
  try {
    ...
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback planning requires contract-valid immutable operation and result history.', {
      cause: error?.code ?? null,
    });
  }
  if (!operation || !result || !baseline
    ...
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback planning requires one exact reversible terminal operation, result, and pre-effect baseline.', {
      operationId: operation?.operationId ?? null,
    });
  }
  const identity = rollbackPlanId ?? `rbp_${sha256Jcs({
    operationId: operation.operationId, executionResultId: result.resultId,
    baselineArtifactId: baseline.artifactId, baselineHash: baseline.contentHash,
  }).slice('sha256:'.length, 31)}`;
  const plan = {
    kind: 'operating-rollback-plan', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    ...
    steps: [{
      ...
      description: 'Restore the exact immutable pre-effect baseline after proving the expected current target hash.',
...
export function recordOperatingRollbackPlanV2({ state, plan, eventId, runtimeActorId = 'operate-runtime-v2' } = {}) {
  try {
    assertProtocolArtifact('operating-rollback-plan', plan, { protocolVersion: PROTOCOL_VERSION });
  } catch (error) {
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback plan recording requires one contract-valid immutable plan.', {
      cause: error?.code ?? null,
    });
  }
  if (plan.planHash !== sha256Jcs(without(plan, 'planHash'))) {
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback plan hash does not equal its canonical immutable content.');
  }
  const durable = reduceOperatingRuntimeEventsV2([], { initialState: state });
  const result = durable.executionResults.find(({ resultId }) => resultId === plan?.executionResultId);
  const operation = durable.governedOperations.find(({ operationId }) => operationId === plan?.operationId);
  const action = durable.actions.find(({ actionId }) => actionId === plan?.action?.actionId);
  if (!result || !operation || !action || typeof eventId !== 'string') {
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback plan recording requires its exact durable execution history.');
  }
  const event = runtimeEvent(durable, {
    eventId, timestamp: result.completedAt, cycleId: action.sourceCycleId,
    type: 'rollback.plan-recorded', entityId: plan.rollbackPlanId,
    actor: { kind: 'engine', id: runtimeActorId },
    correlationId: `rollback-plan:${plan.rollbackPlanId}`, payload: clone(plan),
  });
...
export function classifyOperatingGovernedRecoveryV2({ state, operationId, observedAt } = {}) {
  const durable = reduceOperatingRuntimeEventsV2([], { initialState: state });
  const operation = durable.governedOperations.find((candidate) => candidate.operationId === operationId);
  if (!operation) fail('ACTION_NOT_FOUND', 'Recovery requires one exact durable governed operation.', { operationId });
  const result = operation.operationKind === 'execute'
    ? durable.executionResults.find(({ resultId }) => resultId === operation.resultId)
    : durable.rollbackResults.find(({ rollbackResultId }) => rollbackResultId === operation.resultId);
  const classifications = {
    succeeded: 'applied', partial: 'partial', failed: 'not-applied', blocked: 'not-applied',
    uncertain: 'unknown', dispatching: 'unknown', authorized: 'unknown', 'intent-recorded': 'unknown',
  };
  const proof = {
    kind: 'operating-reconciliation-proof', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    operationId: operation.operationId,
...
export function classifyOperatingRollbackReconciliationReceiptV2({
  ...
  const durable = reduceOperatingRuntimeEventsV2([], { initialState: state });
  const operations = durable.governedOperations.filter((candidate) => (
  ...
  if (!operation || operation.operationKind !== 'rollback'
    ...
    fail('OPERATION_CONFLICT', 'Rollback reconciliation requires one exact durable dispatching rollback operation.', {
      operationId: operationId ?? null,
    });
  }
  const authority = assertOperatingRollbackAuthorityChainV2({ state: durable, operationId });
  const plans = durable.rollbackPlans.filter(({ rollbackPlanId }) => (
  ...
  if (!rollbackPlan
    ...
    fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback reconciliation requires exact canonical durable operation and plan bindings.', {
      operationId: operation.operationId,
      rollbackPlanId: operation.rollbackPlanId,
    });
  }
  if (exact(response, ['status', 'receipt'])
    && response.status === 'not-found' && response.receipt === null) {
    return freeze({ classification: 'not-applied', receipt: null });
  }
  if (exact(response, ['status', 'receipt'])
    && response.status === 'succeeded' && response.receipt !== null) {
    try {
      ...
      return freeze({
        classification: receipt.after.stateHash === rollbackPlan.baselineHash
          ? 'applied'
          : 'partial',
        receipt,
      });
...
export async function reconcileOperatingGovernedDispatchV2({
  state, operationId, request, trustedHost, targetAdapter,
  registry = OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2,
  observedAt,
} = {}) {
  const durable = reduceOperatingRuntimeEventsV2([], { initialState: state });
  const operation = durable.governedOperations.find((candidate) => candidate.operationId === operationId);
  const replay = durable.operationReplayIndex.find((candidate) => candidate.operationId === operationId);
  ...
  const registration = findOperateExecutorRegistrationV2(
    registry, operation.executor.executorId, { executorVersion: operation.executor.executorVersion },
  );
  const host = resolveOpenReferenceExecutorHostV2(trustedHost);
  if (!registration || !host || !targetAdapter
    ...
    const unknown = clone(classifyOperatingGovernedRecoveryV2({ state: durable, operationId, observedAt }));
  ...
  const rollbackPlans = operation.operationKind === 'rollback'
    ? durable.rollbackPlans.filter(({ rollbackPlanId }) => rollbackPlanId === operation.rollbackPlanId)
    : [];
  const rollbackPlan = operation.operationKind === 'rollback' && rollbackPlans.length === 1
    ? rollbackPlans[0]
    : null;
  if (operation.operationKind === 'rollback') {
    if (rollbackPlan === null) {
      fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback reconciliation requires one exact durable rollback plan.', {
        operationId: operation.operationId,
        rollbackPlanId: operation.rollbackPlanId,
      });
    }
  ...
  const selection = { status: 'available', registration, reasonCode: 'exact-registration-available', fallback: null };
  const binding = createTrustedExecutorBindingV2({ selection, trustedHost });
  const response = await Promise.resolve(trustedHost.reconcile({
    targetAdapter, executorInput, binding, executor: registration, rollbackPlan,
  }));
  let classification = 'unknown';
  let receipt = null;
  if (operation.operationKind === 'rollback') {
    ({ classification, receipt } = classifyOperatingRollbackReconciliationReceiptV2({
      state: durable, operationId: operation.operationId, response,
    }));
  } else {
    if (!exact(response, ['status', 'receipt']) || !['not-found', 'succeeded'].includes(response.status)
      || (response.status === 'not-found') !== (response.receipt === null)) {
      fail('OPERATION_UNCERTAIN', 'Executor reconciliation returned an unsupported or ambiguous classification.', {
        operationId,
      });
    }
    classification = response.status === 'succeeded' ? 'applied' : 'not-applied';
    receipt = response.status === 'succeeded'
...
const ROLLBACK_DRAFT_FIELDS = [
  'assignmentId', 'submissionId', 'operationId', 'grantId', 'rollbackResultId',
  'resultArtifactId', 'claimId', 'preparedAt', 'completedAt', 'grantExpiresAt',
  'availabilityExpiresAt', 'correlationId', 'eventIds', 'uncertainty',
];
const EVENT_FIELDS = [
  'assignmentCreated', 'assignmentClaimed', 'assignmentStarted', 'availabilityRecorded',
  'capabilityGranted', 'intentRecorded', 'submitted', 'artifactCreated', 'validated', 'resultRecorded',
];
const TERMINAL_EVENT_FIELDS = ['submitted', 'artifactCreated', 'validated', 'resultRecorded'];

function assertRollbackRequest(request) {
  if (!exact(request, ['actionId', 'originalOperationId', 'rollbackPlanId', 'payload', 'rollbackBaseline'])
    ...
    fail('RESULT_CONTRACT_INVALID', 'Governed rollback requires one closed exact plan, payload, and baseline request.');
  }
  return clone(request);
}

function assertRollbackDraft(draft) {
  if (!exact(draft, ROLLBACK_DRAFT_FIELDS)
    || !exact(draft?.eventIds, EVENT_FIELDS)
    || !exact(draft?.uncertainty, ['rollbackResultId', 'resultArtifactId', 'submissionId', 'eventIds'])
    ...
    || new Set([...Object.values(draft.eventIds), ...Object.values(draft.uncertainty.eventIds)]).size
      !== EVENT_FIELDS.length + TERMINAL_EVENT_FIELDS.length) {
    fail('RESULT_CONTRACT_INVALID', 'Governed rollback requires one complete runtime-owned identity draft.');
  }
  const prepared = validTime(draft.preparedAt, 'draft.preparedAt');
  if (validTime(draft.completedAt, 'draft.completedAt') < prepared
...
function assertRuntimeReconcileInput(input) {
  ...
  if (!input || Object.getPrototypeOf(input) !== Object.prototype
    ...
    fail('RESULT_CONTRACT_INVALID', 'Runtime reconciliation accepts only the documented operation, request, host, target, and observation fields.');
  }
  return {
...
function exactAuthority(state, action, plan, now) {
  const evaluation = resolveOperatingLatestActionEvaluationV2({
    evaluations: state.policyEvaluations, action, configuredPolicies: state.actionPolicies, at: now,
  });
  ...
  const approvals = evaluation ? state.approvalRecords.filter(({ evaluationId }) => evaluationId === evaluation.evaluationId) : [];
  let disposition;
  try {
    assertOperatingRollbackPolicyV2({ action, evaluation, rollbackPlan: plan, at: now });
    disposition = evaluateOperatingRollbackApprovalSetV2({ evaluation, action, requirements, approvals, now });
  } catch (error) {
    fail(error.code ?? 'APPROVAL_REQUIRED', error.message, error.details?.context ?? {});
  }
  if (!evaluation || !actionPolicy || requirements.some((entry) => !entry)
    || !disposition.complete || disposition.disposition !== 'approved') {
    fail('APPROVAL_REQUIRED', 'Rollback requires one fresh exact current policy and approval disposition.', {
...
function buildAssignment(action, draft, inputArtifactIds) {
  return {
    ...
    assignmentId: draft.assignmentId, cycleId: action.sourceCycleId, assignmentKind: 'execution',
    ...
    state: 'pending', dependsOn: [], dependencyPolicy: { kind: 'none' }, inputArtifactIds, inputAbsences: [],
    ...
    capabilityGrantId: draft.grantId, governedOperationId: draft.operationId,
    attemptPolicy: { maxAttempts: 1, attempt: 0, timeoutMs: 30000 }, claim: null,
    terminalOutcome: null, createdAt: draft.preparedAt, availableAt: null, completedAt: null,
  };
}

function buildGrant({ action, authority, availability, draft, runtimeActorId }) {
  const grant = {
    kind: 'operating-capability-grant', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    grantId: draft.grantId, issuer: { kind: 'runtime', id: runtimeActorId },
    assignmentId: draft.assignmentId, action: actionIdentity(action),
    evaluationId: authority.evaluation.evaluationId,
    approvalIds: [...authority.disposition.approvalIds].sort(), operationId: draft.operationId,
    capability: clone(action.requestedCapability), target: clone(action.targetBinding),
    effectClass: action.effectClass, useLimit: 1, issuedAt: draft.preparedAt,
    expiresAt: draft.grantExpiresAt, consumedAt: null, revokedAt: null,
    scopeHash: authority.requirements[0]?.scopeHash ?? sha256Jcs({
      action: actionIdentity(action), evaluationId: authority.evaluation.evaluationId,
...
function terminalReservation(draft) {
  return {
    resultId: draft.rollbackResultId, resultArtifactId: draft.resultArtifactId,
    submissionId: draft.submissionId,
    eventIds: Object.fromEntries(TERMINAL_EVENT_FIELDS.map((field) => [field, draft.eventIds[field]])),
    completedAt: draft.completedAt, correlationId: draft.correlationId,
    uncertainty: {
...
function buildOperation({ action, authority, assignment, plan, selection, binding, request, draft }) {
  const operation = {
    kind: 'operating-governed-operation', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    operationId: draft.operationId, operationKind: 'rollback', action: actionIdentity(action),
    assignmentId: assignment.assignmentId, requestFingerprint: `sha256:${'0'.repeat(64)}`,
    evaluationId: authority.evaluation.evaluationId, approvalIds: [...authority.disposition.approvalIds].sort(),
    grantId: draft.grantId, capability: clone(action.requestedCapability), target: clone(action.targetBinding),
    effectClass: action.effectClass,
    executor: { executorId: selection.registration.executorId, executorVersion: selection.registration.executorVersion },
    connector: clone(binding.connector), preconditionArtifactIds: [...action.preconditionArtifactIds],
    inputArtifactIds: [...assignment.inputArtifactIds], verificationPlanId: plan.verificationPlanId,
    rollbackClass: 'reversible', state: 'dispatching', intentEventId: draft.eventIds.intentRecorded,
    resultId: null, rollbackPlanId: plan.rollbackPlanId, parentOperationId: plan.operationId,
    createdAt: draft.preparedAt, updatedAt: draft.preparedAt, operationHash: `sha256:${'0'.repeat(64)}`,
  };
...
function buildAuthorityContext({ state, action, authority, availability, grant, operation, plan, selection, binding, executorInput, draft, runtimeActorId, registry }) {
  return {
    actor: { actorId: runtimeActorId, kind: 'engine', capabilities: [clone(action.requestedCapability)] },
    capabilities: [clone(ROLLBACK_TOOL_CAPABILITY)], now: draft.preparedAt, action,
    actionRequest: { action: actionIdentity(action), originalOperationId: plan.operationId, rollbackPlanId: plan.rollbackPlanId },
    request: { action: actionIdentity(action), originalOperationId: plan.operationId, rollbackPlanId: plan.rollbackPlanId },
    ...
    actionPolicy: authority.actionPolicy, actionPolicies: state.actionPolicies,
    policyEvaluation: authority.evaluation, approvalRequirements: authority.requirements,
    approvals: authority.approvals, capabilityAvailability: availability, grant, operation,
    operationHistory: state.governedOperations, currentPreconditionArtifactIds: [...action.preconditionArtifactIds],
    executor: selection.registration, governedExtensions: registry,
    trustedExecutorBinding: binding, executorInput, rollbackPlan: plan,
  };
}
...
function buildRollbackResult({ operation, plan, draft, reservation, receiptProof, status }) {
  ...
  const summaries = {
    succeeded: changed ? 'Restored the exact immutable pre-effect baseline.' : 'The target already equalled the exact immutable baseline.',
  ...
  const result = {
    kind: 'operating-rollback-result', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    rollbackResultId: reservation.resultId, rollbackOperationId: operation.operationId,
    operationKind: 'rollback', requestFingerprint: operation.requestFingerprint,
    originalOperationId: plan.operationId, executionResultId: plan.executionResultId,
    rollbackPlanId: plan.rollbackPlanId, action: clone(operation.action), assignmentId: operation.assignmentId,
    evaluationId: operation.evaluationId, approvalIds: clone(operation.approvalIds), grantId: operation.grantId,
    capability: clone(operation.capability), target: clone(operation.target), effectClass: operation.effectClass,
    executor: clone(operation.executor), connector: clone(operation.connector), status,
    ...
    baselineArtifactId: plan.baselineArtifactId, baselineHash: plan.baselineHash,
    inputArtifactIds: clone(operation.inputArtifactIds), outputArtifactIds: [reservation.resultArtifactId],
    effectSummary: { changed, summary: summaries[status], affectedTargetIds: ['failed', 'blocked'].includes(status) ? [] : [operation.target.id] },
    resultArtifactId: reservation.resultArtifactId, verificationPlanId: operation.verificationPlanId,
    eventIds: [operation.intentEventId, reservation.eventIds.submitted, reservation.eventIds.artifactCreated, reservation.eventIds.validated, reservation.eventIds.resultRecorded],
    completedAt: draft.completedAt,
  };
  result.resultHash = sha256Jcs(result);
  assertProtocolArtifact('operating-rollback-result', result, { protocolVersion: PROTOCOL_VERSION });
  return result;
}

function createRollbackReconciliationReceiptProof(operation, plan, parentReceipt, receipt) {
  if (!exact(receipt, ['operationId', 'requestFingerprint', 'target', 'before', 'after', 'changed', 'synthetic', 'effectCount'])
    || !exact(parentReceipt, ['operationId', 'requestFingerprint', 'target', 'before', 'after', 'changed', 'synthetic', 'effectCount'])
    ...
    || receipt.target.kind !== operation.target.kind || receipt.target.id !== operation.target.id
    || parentReceipt.operationId !== plan.operationId
    || parentReceipt.target.kind !== operation.target.kind || parentReceipt.target.id !== operation.target.id
    || parentReceipt.after.stateHash !== plan.steps[0].expectedTargetHash
    || typeof parentReceipt.before.revision !== 'string' || parentReceipt.before.revision.length === 0
    || typeof parentReceipt.after.revision !== 'string' || parentReceipt.after.revision.length === 0
    || typeof receipt.before.revision !== 'string' || receipt.before.revision.length === 0
    ...
    || typeof receipt.synthetic !== 'boolean' || receipt.effectCount !== 1) {
    fail('RESULT_CONTRACT_INVALID', 'Rollback receipt must prove one exact plan-bound compensation attempt.', {
      operationId: operation.operationId,
    });
  }
  return freeze({
    operationId: receipt.operationId, requestFingerprint: receipt.requestFingerprint,
    ...
    changed: receipt.changed, synthetic: receipt.synthetic, effectCount: 1,
  });
}
...
function createRollbackReceiptProof(operation, plan, parentReceipt, receipt) {
  ...
  if (proof.after.stateHash !== plan.baselineHash) {
    fail('RESULT_CONTRACT_INVALID', 'Rollback receipt must prove the exact plan-bound baseline postcondition.', {
      operationId: operation.operationId,
...
function materializeRollbackResult({ state, operation, assignment, action, result, draft, reservation, runtimeActorId }) {
  const bytes = Buffer.from(canonicalizeJson(result), 'utf8');
  const claimedAssignment = state.assignments.find(({ assignmentId }) => (
    assignmentId === assignment.assignmentId
  ));
  if (!claimedAssignment?.claim || claimedAssignment.state !== 'running') {
    fail('ASSIGNMENT_NOT_AVAILABLE', 'Rollback result materialization requires the exact running Assignment claim.', {
      assignmentId: assignment.assignmentId,
    });
  }
  const accepted = acceptOperatingAssignmentSubmissionV2({
    assignmentId: assignment.assignmentId, submissionId: reservation.submissionId,
    actor: {
      actorId: claimedAssignment.claim.actorId,
      kind: claimedAssignment.claim.actorKind,
      runtime: claimedAssignment.claim.runtime,
    },
    contentBase64: bytes.toString('base64'), mediaType: 'application/json', encoding: 'utf-8',
  }, {
    artifactId: reservation.resultArtifactId, artifactType: 'operating-rollback-result',
    storageClass: 'machine-local', sensitivity: 'internal', retentionClass: 'project',
    inputArtifactIds: operation.inputArtifactIds, timestamp: draft.completedAt,
    validatorVersion: 'operate-governed-recovery-v2@1.0.0',
    eventIds: { submitted: reservation.eventIds.submitted, artifactCreated: reservation.eventIds.artifactCreated, validated: reservation.eventIds.validated },
    correlationId: draft.correlationId,
  }, { initialState: state });
  const resultRecorded = runtimeEvent(accepted.state, {
    eventId: reservation.eventIds.resultRecorded, timestamp: draft.completedAt,
    cycleId: action.sourceCycleId, type: 'rollback.result-recorded',
    entityId: result.rollbackResultId, actor: { kind: 'engine', id: runtimeActorId },
    correlationId: draft.correlationId, payload: result,
  });
  let finalState = reduceOperatingRuntimeEventsV2([...accepted.events, resultRecorded], { initialState: state });
  const lifecycleEvents = [];
  const terminalActions = finalState.actions.filter(({ actionId }) => actionId === action.actionId);
  const terminalCycles = finalState.cycles.filter(({ cycleId }) => cycleId === action.sourceCycleId);
  const terminalOperations = finalState.governedOperations.filter(({ operationId }) => (
    operationId === operation.operationId
  ));
  const verificationPlans = finalState.verificationPlans.filter(({ verificationPlanId }) => (
  ...
  if (terminalActions.length !== 1 || terminalCycles.length !== 1 || terminalOperations.length !== 1
    || verificationPlans.length > 1 || !sameScope(terminalAction) || !sameScope(terminalCycle)
    || (verificationPlan && (!sameScope(verificationPlan) || verificationPlan.actionId !== action.actionId))) {
    fail('OPERATING_SCOPE_INVALID', 'Rollback lifecycle selection requires one exact Action, Cycle, and verification-plan scope and domain version.', {
      actionId: action.actionId,
...
async function rollbackReplay(state, request, draft, artifactStore) {
  ...
  if (operation.operationKind !== 'rollback'
    ...
    || deriveContainedExecutorRequestFingerprintV2({ operation, payload: request.payload, rollbackBaseline: request.rollbackBaseline })
      !== operation.requestFingerprint) {
    fail('OPERATION_CONFLICT', 'Rollback operation identity was reused with divergent input.', { operationId: draft.operationId });
  }
  if (!TERMINAL_STATES.has(operation.state) || operation.resultId === null) {
    fail('OPERATION_UNCERTAIN', 'Rollback has durable dispatch intent without a terminal result; blind retry is forbidden.', {
      operationId: operation.operationId, recoveryDisposition: 'reconcile-before-retry',
    });
  }
  const result = state.rollbackResults.find(({ rollbackResultId }) => rollbackResultId === operation.resultId);
  const artifact = result && state.artifacts.find(({ artifactId }) => artifactId === result.resultArtifactId);
  if (!result || !artifact || artifact.canonicalHash !== sha256Jcs(result)) {
    fail('RESULT_CONTRACT_INVALID', 'Rollback replay is missing its exact immutable result Artifact.');
  }
  ...
  if (!await hasExactArtifactCustody(artifactStore, artifact, resultBytes)) {
    const provenTerminal = ['succeeded', 'partial'].includes(result.status)
      && operation.state === result.status
      && operation.resultId === result.rollbackResultId;
    fail('RESULT_CONTRACT_INVALID', 'Rollback replay cannot prove custody of the exact accepted result bytes.', {
      operationId: operation.operationId,
  ...
  return freeze({ state: clone(state), operation: clone(operation), result: clone(result), artifact: clone(artifact), events: [], receipt: null, replayed: true, dispatchCount: 0, effectCount: 0 });
}

export function createOperatingGovernedRecoveryRuntimeV2({
  initialState, artifactStore, checkpointStore, runtimeVersion = DEFAULT_RUNTIME_VERSION,
  runtimeActorId = 'operate-runtime-v2', registry = OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2,
} = {}) {
  if (!initialState || typeof checkpointStore?.readSnapshot !== 'function' || typeof checkpointStore?.compareAndSwap !== 'function') {
    fail('RESULT_CONTRACT_INVALID', 'Governed recovery runtime requires one explicit state and durable CAS checkpoint store.');
  }
  if (artifactStore !== undefined
    && (typeof artifactStore?.stageRaw !== 'function' || typeof artifactStore?.readRaw !== 'function')) {
    fail('RESULT_CONTRACT_INVALID', 'Rollback Artifact byte storage must implement exact raw staging and retrieval.');
  }
  ...
  async function rollbackOnce(requestInput, draftInput, { trustedHost, targetAdapter } = {}) {
    ...
    const plan = currentState.rollbackPlans.find(({ rollbackPlanId }) => rollbackPlanId === request.rollbackPlanId);
    const parent = currentState.governedOperations.find(({ operationId }) => operationId === request.originalOperationId);
    const parentResult = parent && currentState.executionResults.find(({ resultId }) => resultId === parent.resultId);
    if (!action || !plan || !parent || !parentResult
      || plan.operationId !== parent.operationId || plan.executionResultId !== parentResult.resultId
      || parent.operationKind !== 'execute' || !['succeeded', 'partial'].includes(parent.state)
      ...
      || currentState.governedOperations.some((candidate) => candidate.operationKind === 'rollback'
        && candidate.parentOperationId === parent.operationId && candidate.rollbackPlanId === plan.rollbackPlanId)) {
      fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback request is stale, repeated, unsupported, or divergent from immutable execution history.', {
        rollbackPlanId: request.rollbackPlanId,
      });
    }
    const authority = exactAuthority(currentState, action, plan, draft.preparedAt);
    const host = resolveOpenReferenceExecutorHostV2(trustedHost);
    if (!host || !targetAdapter) fail('EXECUTOR_UNAVAILABLE', 'Rollback requires one package-owned contained host and target.');
    const selection = selectOperateExecutorV2(configuredRegistry, {
      executorId: host.executorId, executorVersion: host.executorVersion,
      protocolVersion: PROTOCOL_VERSION, runtimeVersion, now: draft.preparedAt,
      domainId: action.domainId, actionKind: action.actionKind,
      capability: action.requestedCapability, targetKind: action.targetBinding.kind,
      effectClass: action.effectClass, operationKind: 'rollback',
    });
    if (selection.status !== 'available'
      ...
      fail('EXECUTOR_UNAVAILABLE', 'Rollback requires the exact healthy executor version recorded by its plan.');
    }
    const binding = createTrustedExecutorBindingV2({ selection, trustedHost });
    const availability = createOpenReferenceCapabilityAvailabilityV2({
      action, runtimeVersion, checkedAt: draft.preparedAt,
      expiresAt: draft.availabilityExpiresAt, registry: configuredRegistry,
    });
    if (availability.status !== 'available') fail('CAPABILITY_UNAVAILABLE', 'Rollback capability is unavailable.');
    ...
    if (!inputArtifactIds.includes(request.rollbackBaseline.artifactId)) {
      fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback baseline is not one of the reviewed immutable Action inputs.');
    }
    ...
    const operation = buildOperation({ action, authority, assignment, plan, selection, binding, request, draft });
    const executorInput = createContainedExecutorInputEnvelopeV2({ operation, payload: request.payload, rollbackBaseline: request.rollbackBaseline });
    const authorityContext = buildAuthorityContext({ state: currentState, action, authority, availability, grant, operation, plan, selection, binding, executorInput, draft, runtimeActorId, registry: configuredRegistry });
    ...
    try {
      ...
      fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback target no longer equals the plan-bound post-execution state.', {
        rollbackPlanId: plan.rollbackPlanId,
        cause: error?.code ?? error?.name ?? 'target-inspection-error',
      });
    }
    if (!exact(targetBefore, ['value', 'revision', 'stateHash'])
      || targetBefore.stateHash !== sha256Jcs(targetBefore.value)
      || targetBefore.stateHash !== plan.steps[0].expectedTargetHash) {
      fail('ROLLBACK_NOT_ELIGIBLE', 'Rollback target no longer equals the plan-bound post-execution state.', { rollbackPlanId: plan.rollbackPlanId });
    }
    const finalDecision = assertOperateAuthorityV2('operate.action.rollback', authorityContext);
    if (sha256Jcs(authorityDecision) !== sha256Jcs(finalDecision)) fail('OPERATION_CONFLICT', 'Rollback authority changed during preflight.');
    const reservation = terminalReservation(draft);
    let staged = currentState;
    const events = [];
    const created = runtimeEvent(staged, {
      eventId: draft.eventIds.assignmentCreated, timestamp: draft.preparedAt,
      cycleId: action.sourceCycleId, type: 'assignment.created', entityId: assignment.assignmentId,
      actor: { kind: 'engine', id: runtimeActorId }, correlationId: draft.correlationId, payload: assignment,
    });
    ...
    for (const input of [
      { eventId: draft.eventIds.assignmentClaimed, type: 'assignment.claimed', entityId: assignment.assignmentId, payload: {
        assignmentId: assignment.assignmentId,
      ...
      { eventId: draft.eventIds.assignmentStarted, type: 'assignment.started', entityId: assignment.assignmentId, payload: { assignmentId: assignment.assignmentId, attempt: 1 } },
      { eventId: draft.eventIds.availabilityRecorded, type: 'capability.availability-recorded', entityId: availability.availabilityId, payload: availability },
      { eventId: draft.eventIds.capabilityGranted, type: 'capability.granted', entityId: grant.grantId, payload: grant },
      { eventId: draft.eventIds.intentRecorded, type: 'operation.intent-recorded', entityId: operation.operationId, requestHash: operation.requestFingerprint, payload: { operation, request: { payload: { artifactId: request.payload.artifactId, contentHash: request.payload.contentHash }, rollbackBaseline: { artifactId: request.rollbackBaseline.artifactId, contentHash: request.rollbackBaseline.contentHash }, targetBeforeHash: targetBefore.stateHash }, terminal: reservation } },
    ]) {
      const event = runtimeEvent(staged, { ...input, timestamp: draft.preparedAt, cycleId: action.sourceCycleId, actor: { kind: 'engine', id: runtimeActorId }, correlationId: draft.correlationId });
    ...
    const committedIntent = await commitCheckpoint(checkpointStore, currentState, staged, {
      phase: 'rollback-intent', operationId: operation.operationId, requestFingerprint: operation.requestFingerprint,
    });
    ...
    let receipt;
    try {
      receipt = await Promise.resolve(trustedHost.rollback({ authorityContext, authorityDecision, executorInput, binding, executor: selection.registration, targetAdapter }));
      ...
      if (receiptProof.after.stateHash !== plan.baselineHash) fail('OPERATION_UNCERTAIN', 'Rollback receipt did not prove the exact baseline postcondition.');
      provenTerminalOperations.add(operation.operationId);
      const result = buildRollbackResult({ operation, plan, draft, reservation, receiptProof, status: 'succeeded' });
      ...
      for (let rebaseAttempt = 0; rebaseAttempt < 16; rebaseAttempt += 1) {
        const durableOperation = currentState.governedOperations.find(({ operationId }) => (
          operationId === operation.operationId
        ));
        if (!durableOperation) {
          fail('OPERATION_CONFLICT', 'The durable rollback owner disappeared during proven-success rebasing.', {
            operationId: operation.operationId, provenTerminal: true,
          });
        }
        if (TERMINAL_STATES.has(durableOperation.state) && durableOperation.resultId !== null) {
          ...
          if (durable.result.resultHash !== result.resultHash) {
            fail('OPERATION_CONFLICT', 'A different terminal rollback result owns the reserved identities.', {
              operationId: operation.operationId, provenTerminal: true,
            });
          }
          return durable;
        ...
        const terminal = materializeRollbackResult({
          state: currentState, operation, assignment, action, result, draft, reservation, runtimeActorId,
        });
        if (!await stageAndProveArtifactCustody(artifactStore, terminal.accepted.artifact, terminal.bytes)) {
          fail('OPERATION_UNCERTAIN', 'Proven rollback success Artifact custody could not be established.', {
            operationId: operation.operationId, rollbackResultId: result.rollbackResultId,
            resultArtifactId: result.resultArtifactId, provenTerminal: true,
            recoveryDisposition: 'commit-proven-result-before-retry',
          });
        }
        try {
          const committed = await commitCheckpoint(checkpointStore, currentState, terminal.finalState, {
            phase: 'rollback-result', operationId: operation.operationId,
            requestFingerprint: operation.requestFingerprint,
          });
          ...
          return freeze({
            ...
            operation: clone(terminal.finalState.governedOperations.find(({ operationId }) => operationId === operation.operationId)),
            result: clone(result), artifact: clone(terminal.accepted.artifact),
            events: [
              ...events,
            ...
            receipt: clone(receipt), replayed: false, dispatchCount: 1, effectCount: receipt.effectCount,
          });
        } catch (commitError) {
      ...
      fail('OPERATION_UNCERTAIN', 'Proven rollback success could not acquire the durable checkpoint after bounded rebasing.', {
        operationId: operation.operationId, rollbackResultId: result.rollbackResultId,
        provenTerminal: true, recoveryDisposition: 'commit-proven-result-before-retry',
      });
    } catch (error) {
      if (provenTerminalOperations.has(operation.operationId)
      ...
      const uncertainResult = buildRollbackResult({
        operation, plan, draft, reservation: uncertaintyReservation,
        receiptProof: null, status: 'uncertain',
      });
      const terminal = materializeRollbackResult({
        state: currentState, operation, assignment, action, result: uncertainResult,
        draft, reservation: uncertaintyReservation, runtimeActorId,
      });
      if (!await stageAndProveArtifactCustody(artifactStore, terminal.accepted.artifact, terminal.bytes)) {
        fail('OPERATION_UNCERTAIN', 'Rollback uncertainty bytes could not be placed in exact Artifact custody.', {
          operationId: operation.operationId, cause: error?.code ?? error?.name ?? 'rollback-error',
          rollbackResultId: null, recoveryDisposition: 'reconcile-before-retry',
        });
      }
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (durableOperation && TERMINAL_STATES.has(durableOperation.state) && durableOperation.resultId !== null) {
          const durable = await rollbackReplay(currentState, request, draft, artifactStore);
          fail('OPERATION_UNCERTAIN', 'Rollback dispatch has one durable terminal non-success result.', {
            operationId: durable.operation.operationId, cause: error?.code ?? error?.name ?? 'rollback-error',
            rollbackResultId: durable.result.rollbackResultId, recoveryDisposition: 'reconcile-before-retry',
          });
        }
        ...
        try {
          const committed = await commitCheckpoint(checkpointStore, currentState, rebasedTerminal.finalState, {
            phase: 'rollback-result', operationId: operation.operationId,
            requestFingerprint: operation.requestFingerprint,
          });
          currentState = committed.state;
          if (!committed.committed) continue;
          fail('OPERATION_UNCERTAIN', 'Rollback dispatch outcome is uncertain and cannot be retried blindly.', {
            operationId: operation.operationId, cause: error?.code ?? error?.name ?? 'rollback-error',
            causeMessage: error?.message ?? null, rollbackResultId: uncertainResult.rollbackResultId,
            recoveryDisposition: 'reconcile-before-retry',
          });
        } catch (commitError) {
          if (commitError?.code !== 'OPERATION_UNCERTAIN'
            || commitError?.details?.context?.rollbackResultId === uncertainResult.rollbackResultId) throw commitError;
        }
      }
      fail('OPERATION_UNCERTAIN', 'Rollback dispatch remains non-redispatchable without a durable uncertainty result.', {
        operationId: operation.operationId, cause: error?.code ?? error?.name ?? 'rollback-error',
        rollbackResultId: null, recoveryDisposition: 'reconcile-before-retry',
      });
    }
  ...
  async function rollback(request, draft, environment = {}) {
    ...
    if (active) {
      if (active.invocationHash !== invocationHash) fail('OPERATION_CONFLICT', 'Concurrent rollback identity reuse is divergent.', { operationId });
      await active.promise;
      return rollbackReplay(currentState, assertRollbackRequest(request), assertRollbackDraft(draft), artifactStore);
    }
    const promise = rollbackOnce(request, draft, environment);
    inFlight.set(operationId, { invocationHash, promise });
    try { return await promise; } finally { if (inFlight.get(operationId)?.promise === promise) inFlight.delete(operationId); }
  }
  ...
  return Object.freeze({
    ...
    reconcile: async (input) => {
      ...
      currentState = await readCheckpoint(checkpointStore, currentState);
      return reconcileOperatingGovernedDispatchV2({ ...checked, state: currentState, registry: configuredRegistry });
    },
    checkpoint: () => clone(currentState), getState: () => clone(currentState),
