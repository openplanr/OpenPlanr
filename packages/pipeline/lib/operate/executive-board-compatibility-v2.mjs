...
function selectReview(state, cycleId, reviewId) {
  ...
  if (candidates.length !== 1) {
    fail('E_OPERATE_BOARD_COMPATIBILITY_UNAVAILABLE', 'Compatibility board requires one exact Cycle Review.', {
      cycleId,
...
function selectPlanAndLedger(state, cycleId) {
  const planIds = new Set(state.assignments
    .filter((assignment) => assignment.cycleId === cycleId)
    .map((assignment) => assignment.intelligenceContext?.intelligencePlanId)
    .filter((value) => typeof value === 'string'));
  const candidates = (state.decisionLedgers ?? []).filter((ledger) => planIds.has(ledger.intelligencePlanId));
  if (candidates.length !== 1) {
    fail('E_OPERATE_BOARD_COMPATIBILITY_UNAVAILABLE', 'Compatibility board requires one exact Chair ledger for the Cycle.', {
      cycleId,
      matchCount: candidates.length,
    });
  }
...
function selectReviewCreationCustody(state, review) {
  const events = state.eventReplayIndex.filter((entry) => (
  ...
  if (events.length === 1) {
    ...
    if (event.timestamp !== review.createdAt) {
      fail('E_OPERATE_BOARD_BINDING_INVALID', 'Cycle Review creation custody has a divergent timestamp.', {
  ...
  fail('E_OPERATE_BOARD_COMPATIBILITY_UNAVAILABLE', 'Compatibility board requires one exact pending Review or durable review.created custody.', {
    cycleId: review.cycleId,
    reviewId: review.reviewId,
...
function assertStoredBoardCustody(state, board, review, reviewCreation) {
  ...
  const submissionEvents = state.eventReplayIndex.filter((entry) => (
  ...
  if (!event
    ...
    || !currentReviewBound) {
    fail('E_OPERATE_BOARD_BINDING_INVALID', 'Materialized Executive Board lost its exact Event and adjacent Review-creation custody.', {
 ...
 */
export function readOperatingExecutiveBoardV2(state, {
  cycleId,
  reviewId = undefined,
  ...
  const stored = (state.executiveBoards ?? []).filter((board) => (
    board.cycleId === cycleId && board.reviewId === review.reviewId
  ));
  if (stored.length > 1) {
    fail('E_OPERATE_BOARD_BINDING_INVALID', 'Cycle Review has more than one materialized Executive Board.', {
      cycleId,
      reviewId: review.reviewId,
    });
  }
...
export function buildExistingOperatingExecutiveBoardCompatibilityV2(state, options = {}) {
  const board = readOperatingExecutiveBoardV2(
    Object.hasOwn(state ?? {}, 'executiveBoards')
      ? { ...state, executiveBoards: [] }
      : state,
    options,
  );
  if (board.projectionMode !== 'compatibility'
    || board.authoritativeForMutation !== false
    || board.materializedEventId !== null) {
    fail('E_OPERATE_BOARD_CONTRACT_INVALID', 'Existing-Cycle compatibility projection became mutation-authoritative.');
  }
  return board;
}
