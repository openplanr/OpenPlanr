...
import { buildOperateExperienceLivePatchV2 } from '../protocol/operate-experience-live-patch.mjs';
...
const STAGES = Object.freeze(['observe', 'understand', 'decide', 'govern', 'act', 'verify', 'learn']);
const CYCLE_STAGE = Object.freeze({
  created: 0, observing: 0, advising: 1, challenging: 1, synthesizing: 1,
  awaiting_review: 2, approved: 3, executing: 4, verifying: 5, closed: 6,
  blocked: 0, failed: 0, cancelled: 0,
});
...
function intelligencePlanForCycle(indexes, cycleAssignments) {
  ...
  for (const plan of plans) {
    if (plan.selectedRoles.some(({ roleId, roleVersion }) => (
      assignmentIds.has(deriveOperatingIntelligenceAssignmentIdV2(plan.planId, roleId, roleVersion))
    ))) return plan;
  }
  return null;
}
...
function assertBinding(binding, field) {
  if (!binding || typeof binding !== 'object'
    || ['scopeId', 'domainId', 'domainVersion'].some((key) => typeof binding[key] !== 'string' || binding[key].length === 0)) {
    fail('OPERATING_SCOPE_INVALID', `${field} requires one explicit scope/domain/version binding.`);
  }
}

function sameScope(value, scope) {
  return value?.scopeId === scope.scopeId
...
function uniqueIdentityIndex(records, identityField, entityName) {
  ...
  for (const record of records) {
    ...
    if (typeof identity !== 'string' || identity.length === 0 || index.has(identity)) {
      fail('RESULT_CONTRACT_INVALID', `Experience projection requires unique scope-local ${entityName} identities.`, { identity: identity ?? null });
    }
...
function buildScopeIdentityIndexes(state, scope) {
  ...
  for (const cycleId of cycleIds) {
    if (state.cycles.filter((record) => record.cycleId === cycleId).length !== 1) {
      fail('RESULT_CONTRACT_INVALID', 'Experience projection requires globally unambiguous Cycle identities for scope-less dependent records.', { cycleId });
    }
  }
  ...
  const actionMatches = (record) => {
    ...
    return Boolean(action) && (!binding || (binding.revision === action.revision && binding.actionHash === action.actionHash));
  };
  return {
    cycles,
    inputBindings: uniqueIdentityIndex(scoped(state.inputBindings).filter(({ cycleId }) => cycleIds.has(cycleId)), 'cycleId', 'cycle input binding'),
    assignments: uniqueIdentityIndex(state.assignments.filter(({ cycleId }) => cycleIds.has(cycleId)), 'assignmentId', 'Assignment'),
    reviews: uniqueIdentityIndex(state.reviews.filter(({ cycleId }) => cycleIds.has(cycleId)), 'reviewId', 'Review'),
    executiveBoards: uniqueIdentityIndex(scoped(state.executiveBoards ?? []), 'boardId', 'Executive Board'),
    ...
    policyEvaluations: uniqueIdentityIndex((state.policyEvaluations ?? []).filter((record) => {
      const action = actions.get(record?.action?.actionId);
      return Boolean(action) && record.action.revision === action.revision && record.action.actionHash === action.actionHash;
    }), 'evaluationId', 'policy evaluation'),
    approvalRequirements: uniqueIdentityIndex(scoped(state.approvalRequirements), 'requirementId', 'approval requirement'),
    approvalRecords: uniqueIdentityIndex(scoped(state.approvalRecords), 'approvalId', 'approval record'),
    metrics: uniqueIdentityIndex(scoped(state.metrics), 'metricId', 'Metric'),
    metricObservations: uniqueIdentityIndex(scoped(state.metricObservations), 'observationId', 'metric observation'),
    ...
    verificationPlans: uniqueIdentityIndex(scoped(state.verificationPlans), 'verificationPlanId', 'verification plan'),
    evidenceRefs: uniqueIdentityIndex(scoped(state.evidenceRefs), 'evidenceRefId', 'evidence reference'),
    evidenceResolutions: uniqueIdentityIndex(scoped(state.evidenceResolutions), 'resolutionId', 'evidence resolution'),
    evidenceEdges: uniqueIdentityIndex(scoped(state.evidenceEdges), 'edgeId', 'evidence edge'),
    intelligencePlans: uniqueIdentityIndex(scoped(state.intelligencePlans ?? []), 'planId', 'intelligence plan'),
    decisionLedgers: uniqueIdentityIndex(scoped(state.decisionLedgers ?? []), 'ledgerId', 'decision ledger'),
    governedOperations: uniqueIdentityIndex((state.governedOperations ?? []).filter(actionMatches), 'operationId', 'governed operation'),
    executionResults: uniqueIdentityIndex((state.executionResults ?? []).filter(actionMatches), 'resultId', 'execution result'),
    rollbackPlans: uniqueIdentityIndex((state.rollbackPlans ?? []).filter(actionMatches), 'rollbackPlanId', 'rollback plan'),
    rollbackResults: uniqueIdentityIndex((state.rollbackResults ?? []).filter(actionMatches), 'rollbackResultId', 'rollback result'),
...
function cycleStages(cycle) {
  ...
  return STAGES.map((id, index) => {
    ...
    return { id, state, reason: state === 'blocked' || state === 'failed' || state === 'skipped' ? `Cycle is ${cycle.state}.` : null };
  });
}
...
function routeFor(action, routes, scope, eventHead) {
  ...
  if (candidates.length !== 1) fail('RESULT_CONTRACT_INVALID', 'Every current Action requires exactly one delivery route.', { actionId: action.actionId, candidates: candidates.length });
  const route = candidates[0];
  assertOperateExperienceArtifactV2('operating-delivery-route', route);
  if (!sameScope(route, scope)
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Delivery route does not bind the exact Action revision, scope, domain, and Event head.', { actionId: action.actionId });
  }
  return route;
}
...
function approvalComplete(requirement, indexes, now) {
  ...
  const requirements = evaluation.approvalRequirementIds.map((requirementId) => indexes.approvalRequirements.get(requirementId) ?? null);
  if (requirements.some((entry) => entry === null) || !requirements.some(({ requirementId }) => requirementId === requirement.requirementId)) return false;
  const approvals = [...indexes.approvalRecords.values()].filter(({ evaluationId }) => evaluationId === evaluation.evaluationId);
  try {
    const result = evaluateOperatingApprovalSetV2({ evaluation, action, requirements, approvals, now });
...
function currentApprovalRequirement(requirement, indexes) {
  ...
  return current.evaluationId === requirement.evaluationId
...
function approvalRequirementConsumed(requirement, indexes) {
  return [...indexes.approvalRecords.values()].some((entry) => {
    if (entry.requirementId !== requirement.requirementId || entry.consumedByOperationId === null) return false;
    const operation = indexes.governedOperations.get(entry.consumedByOperationId);
    return operation?.evaluationId === requirement.evaluationId
...
function exactRollbackApprovalTemplate(indexes, action, actorId, at) {
  ...
  if (!evaluation
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval requires one exact current Action policy evaluation.');
  }
  const policies = indexes.actionPolicies.filter((policy) =>
  ...
  if (policies.length !== 1) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval requires one exact current Action policy.');
  }
  const eligible = evaluation.approvalRequirementIds.flatMap((requirementId) => {
    ...
    if (!requirement
      ...
      || requirement.scopeHash !== sha256Jcs(without(requirement, 'scopeHash'))) return [];
    return requirement.parties
      .filter((party) => party.actorKind === 'human'
  ...
  if (eligible.length !== 1) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval requires one exact human actor party and approval template.');
  }
  return eligible[0];
}
...
function exactRollbackApprovalContext(indexes, action, rollback, actorId, at) {
  ...
  if (!plan || !operation || !result
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval requires one exact current plan, operation, result, Action, target, and effect binding.');
  }
  return {
    plan,
    operation,
...
function openApprovalRequirement(requirement, indexes, now) {
  return currentApprovalRequirement(requirement, indexes)
    && !approvalRequirementConsumed(requirement, indexes)
    && !approvalComplete(requirement, indexes, now);
}

export function createOperateExperienceReplayCheckpointV2(state, {
...
function validateReplayCheckpoint(state, checkpoint, checkpointState) {
  if (checkpoint === null) {
    if (checkpointState !== null) {
      fail('E_OPERATE_BINDING_MISMATCH', 'A retained replay base requires its exact checkpoint contract.');
    }
    return null;
  }
  assertProtocolArtifact('operating-checkpoint', checkpoint, { protocolVersion: PROTOCOL_VERSION });
  const baseState = checkpointState ?? state;
  assertProtocolArtifact('operating-runtime-state', baseState, { protocolVersion: PROTOCOL_VERSION });
  ...
  if (checkpoint.runtimeStateHash !== runtimeStateHash
    ...
    || sha256Jcs(state.eventReplayIndex.slice(0, baseSequence)) !== sha256Jcs(baseState.eventReplayIndex)) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Replay checkpoint does not bind the exact retained base, replay prefix, and Event head.');
  }
...
function expectedEventEntityId(event) {
  ...
  if (event.type === 'planning-delivery.ingested') return payload.deliveryEvidence.deliveryEvidenceId;
  ...
  if (['metric.observed', 'claim.recorded', 'finding.recorded', 'risk.recorded', 'assumption.recorded', 'decision.revised', 'delta.derived', 'scenario.recorded', 'trigger.recorded'].includes(event.type)) {
    const field = {
      'metric.observed': 'observationId', 'claim.recorded': 'claimId', 'finding.recorded': 'findingId', 'risk.recorded': 'riskId', 'assumption.recorded': 'assumptionId',
      'decision.revised': 'decisionId', 'delta.derived': 'deltaId', 'scenario.recorded': 'scenarioId', 'trigger.recorded': 'triggerId',
    }[event.type];
...
function scopedEventPayloadRecords(value, records = []) {
  ...
  if (['scopeId', 'domainId', 'domainVersion'].some((field) => Object.hasOwn(value, field))) records.push(value);
...
function uniqueStateRecord(records, identityField, identity, collection) {
  ...
  if (matches.length !== 1) fail('E_OPERATE_BINDING_MISMATCH', 'Operation authority requires one exact current-state owner.', { collection, identity, matches: matches.length });
  return matches[0];
}

function intentOperationReplayBinding(state, event) {
  const { operation, request, terminal } = event.payload;
  const replay = uniqueStateRecord(state.operationReplayIndex, 'operationId', operation.operationId, 'operationReplayIndex');
  const exact = {
  ...
  for (const [field, value] of Object.entries(exact)) {
    if (sha256Jcs(replay[field]) !== sha256Jcs(value)) fail('E_OPERATE_BINDING_MISMATCH', 'Operation replay ownership diverges from the exact intent Event.', { operationId: operation.operationId, field });
  }
...
function validateOperationIntentAuthority(state, event, derived, promotedActionKeys) {
  ...
  const actions = [...derived.actions.values()].filter((candidate) => candidate.actionId === operation.action.actionId && sameScope(candidate, cycle));
  const assignments = [...derived.assignments.values()].filter((candidate) => candidate.assignmentId === operation.assignmentId);
  const evaluations = [...derived.policyEvaluations.values()].filter((candidate) => candidate.evaluationId === operation.evaluationId);
  const grants = [...derived.capabilityGrants.values()].filter((candidate) => candidate.grantId === operation.grantId);
  const availability = [...derived.capabilityAvailability.values()].filter((candidate) => candidate.checkedAt === event.timestamp
    && candidate.capability?.id === operation.capability.id && candidate.capability?.version === operation.capability.version
    && candidate.target?.kind === operation.target.kind && candidate.target?.id === operation.target.id
    && candidate.target?.revision === operation.target.revision);
  if (actions.length !== 1 || assignments.length !== 1 || evaluations.length !== 1 || grants.length !== 1 || availability.length !== 1) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Operation intent authority must have one exact prior canonical Event owner.', { operationId: operation.operationId });
  }
  const [action] = actions;
  const [assignment] = assignments;
  const [evaluation] = evaluations;
  const [grant] = grants;
  if (!sameScope(action, cycle) || action.sourceCycleId !== cycle.cycleId || assignment.cycleId !== cycle.cycleId) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Operation authority crosses its exact Action scope or Cycle binding.', { operationId: operation.operationId, cycleId: cycle.cycleId });
  }
  const actionKey = scopedRecordKey(action, 'actionId');
  const sameActionOwners = [...derived.governedOperations.values()].filter((candidate) => candidate.action?.actionId === operation.action.actionId
    && candidate.action?.revision === operation.action.revision && candidate.action?.actionHash === operation.action.actionHash);
  const duplicateOperation = [...derived.governedOperations.values()].some((candidate) => candidate.operationId === operation.operationId);
  const parent = operation.operationKind === 'rollback'
    ? derived.governedOperations.get([...derived.governedOperations.keys()].find((key) => derived.governedOperations.get(key).operationId === operation.parentOperationId))
    : null;
  const ownerValid = operation.operationKind === 'execute'
    ? sameActionOwners.length === 0
    : parent && parent.operationKind === 'execute' && parent.operationId === operation.parentOperationId
      && sameActionOwners.filter((candidate) => candidate.operationKind === 'rollback').length === 0;
  if (!promotedActionKeys.has(actionKey) || duplicateOperation || !ownerValid
    || operation.action.revision !== action.revision || operation.action.actionHash !== action.actionHash
    || assignment.state !== 'running' || grant.consumedAt !== null) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Operation intent authority must exist in exact prior canonical Event-derived state.', { operationId: operation.operationId });
  }
  const requirements = evaluation.approvalRequirementIds.map((requirementId) => uniqueStateRecord(state.approvalRequirements, 'requirementId', requirementId, 'approvalRequirements'));
  const eventApprovals = [...derived.approvalRecords.values()].filter((record) => record.evaluationId === evaluation.evaluationId);
  const intentReplayEntry = intentOperationReplayBinding(state, event);
  try {
    const dispatchProof = operation.operationKind === 'execute'
      ? assertOperatingExecuteDispatchAuthorityChainV2({ state, operationId: operation.operationId })
      : assertOperatingRollbackAuthorityChainV2({ state, operationId: operation.operationId });
    if (sha256Jcs(dispatchProof.intentOperation) !== sha256Jcs(operation)
    ...
    const disposition = evaluateOperatingApprovalSetV2({ evaluation, action, requirements, approvals: eventApprovals, now: event.timestamp });
    if (disposition.complete !== true || disposition.disposition !== 'approved'
      || sha256Jcs(disposition.approvalIds) !== sha256Jcs(operation.approvalIds)) throw new Error('approval-set-divergence');
    if (operation.operationKind === 'execute') {
      assertOperatingExecuteOperationV2({
        operation, action, assignment, evaluation, evaluations: [...derived.policyEvaluations.values()],
        configuredPolicies: state.actionPolicies, grant, requirements, approvals: eventApprovals,
        capabilityAvailability: availability[0], request: {
          payload: { artifactId: request.payload.artifactId, contentHash: request.payload.contentHash },
          rollbackBaseline: request.rollbackBaseline === null ? null : {
            artifactId: request.rollbackBaseline.artifactId, contentHash: request.rollbackBaseline.contentHash,
          },
        }, timestamp: event.timestamp,
      });
    }
    if (operation.approvalIds.length > 0) {
      const consumed = consumeOperatingApprovalRecordsV2({
        approvals: eventApprovals, requirements, approvalIds: operation.approvalIds,
        operationId: operation.operationId,
      });
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Operation intent does not satisfy the canonical execution-authority chain.', { operationId: operation.operationId, cause: error.code ?? error.message });
  }
  return intentReplayEntry;
}

function validateProjectedStateEventParity(state, events, cycles, baseState = null) {
  const derived = Object.fromEntries(Object.keys(EVENT_PARITY_COLLECTIONS).map((name) => [name, new Map()]));
  ...
  const put = (name, record) => {
    ...
    if (derived[name].has(key)) fail('E_OPERATE_BINDING_MISMATCH', 'Canonical Event history duplicates one immutable full-record identity.', { collection: EVENT_PARITY_COLLECTIONS[name][0], identity: record[identityField] });
    derived[name].set(key, clone(record));
  };
  const putGlobal = (index, identity, record, collection) => {
    if (index.has(identity)) fail('E_OPERATE_BINDING_MISMATCH', 'Canonical Event history duplicates one runtime-owned identity.', { collection, identity });
  ...
  const putCanonicalMetric = (metric) => {
    ...
    if (prior && sha256Jcs(prior) !== sha256Jcs(metric)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Verification-plan Events resolve one Metric identity to conflicting immutable model states.', {
    ...
    for (const [name, [stateField, identityField]] of Object.entries(EVENT_PARITY_COLLECTIONS)) {
      for (const record of baseState[stateField] ?? []) {
        ...
        if (derived[name].has(key)) {
          fail('E_OPERATE_BINDING_MISMATCH', 'Retained replay base duplicates one canonical record identity.', {
  ...
  for (const event of events) {
    ...
    if (event.type === 'planning-delivery.ingested') {
      ...
      try {
        ...
        fail('E_OPERATE_BINDING_MISMATCH', 'Planning delivery evidence cannot reconstruct its original canonical verification plan.', {
      ...
      const decision = state.decisions.find(({ decisionId }) => decisionId === payload.origin.decision.id);
      if (event.actor.kind !== 'runtime' || event.actor.id !== 'openplanr'
        ...
        || payload.deliveryEvidence.deliveryEvidenceHash !== sha256Jcs(without(payload.deliveryEvidence, 'deliveryEvidenceHash'))
        ...
        || !action || action.revision !== payload.origin.action.revision || action.actionHash !== payload.origin.action.hash
        || !decision || decision.revision !== payload.origin.decision.revision || sha256Jcs(decision) !== payload.origin.decision.hash
        || reconstructed.action.actionId !== action.actionId
        || reconstructed.verificationPlan.verificationPlanId !== payload.origin.verification.verificationPlanId
        || sha256Jcs(reconstructed.verificationPlan) !== payload.origin.verification.verificationPlanHash
        ...
        fail('E_OPERATE_BINDING_MISMATCH', 'Planning delivery Event lost its exact Action, Decision, metric, verification, evidence, or Assignment custody.', {
          deliveryEvidenceId: payload.deliveryEvidence?.deliveryEvidenceId ?? null,
        });
      }
    ...
    if (event.type.startsWith('assignment.') && event.type !== 'assignment.created') {
      const assignmentKey = [...derived.assignments.keys()].find((key) => derived.assignments.get(key).assignmentId === payload.assignmentId);
      ...
      if (assignment) {
        const patches = {
          ...
          'assignment.claimed': { state: 'claimed', claim: { actorId: payload.actorId, actorKind: payload.actorKind, runtime: payload.runtime, claimId: payload.claimId } },
          'assignment.started': { state: 'running', attemptPolicy: { ...assignment.attemptPolicy, attempt: payload.attempt } },
          ...
          'assignment.abandoned': { state: 'abandoned', completedAt: event.timestamp, terminalOutcome: { outcome: 'abandoned', eventId: event.eventId, code: payload.reasonCode, reason: payload.reason, recoveryDisposition: payload.recoveryDisposition } },
          'assignment.failed': { state: 'failed', completedAt: event.timestamp, terminalOutcome: { outcome: 'failed', eventId: event.eventId, code: payload.errorCode, reason: payload.reason, recoveryDisposition: payload.recoveryStatus } },
        };
        if (patches[event.type]) derived.assignments.set(assignmentKey, { ...assignment, ...patches[event.type] });
        if (event.type === 'assignment.claimed') {
          putGlobal(submissions, payload.submissionId, {
            kind: 'operating-submission', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
            submissionId: payload.submissionId, assignmentId: assignment.assignmentId, cycleId: assignment.cycleId,
            state: 'issued', rawHash: null, canonicalHash: null, sizeBytes: null, artifactId: null,
            acceptanceEventIds: [], responseData: null, issuedAt: event.timestamp, resolvedAt: null,
          }, 'submissions');
        }
        if (event.type === 'assignment.submitted') {
          const submission = submissions.get(payload.submissionId);
          if (!submission || submission.assignmentId !== assignment.assignmentId || submission.state !== 'issued') {
            fail('E_OPERATE_BINDING_MISMATCH', 'Assignment submission lacks one prior runtime-issued Submission.', { submissionId: payload.submissionId });
          }
          submissions.set(payload.submissionId, {
            ...submission, rawHash: payload.rawHash, canonicalHash: payload.canonicalHash, sizeBytes: payload.sizeBytes,
            acceptanceEventIds: [event.eventId],
          });
        }
        if (event.type === 'assignment.validated') {
          const submission = submissions.get(payload.submissionId);
          const artifact = [...derived.artifacts.values()].find(({ artifactId }) => artifactId === payload.artifactId);
          if (!submission || !artifact || submission.assignmentId !== assignment.assignmentId) {
            fail('E_OPERATE_BINDING_MISMATCH', 'Assignment validation lacks its prior Submission and Artifact.', { submissionId: payload.submissionId, artifactId: payload.artifactId });
          }
          const responseData = {
            accepted: true, artifactId: artifact.artifactId, rawHash: artifact.rawHash,
            sizeBytes: artifact.sizeBytes, assignmentState: 'validated',
          };
          const acceptanceEventIds = [...submission.acceptanceEventIds, event.eventId];
          submissions.set(payload.submissionId, {
            ...submission, state: 'accepted', artifactId: artifact.artifactId, acceptanceEventIds,
            responseData, resolvedAt: event.timestamp,
          });
          putGlobal(submissionReplay, payload.submissionId, {
            submissionId: payload.submissionId, assignmentId: assignment.assignmentId,
            rawHash: artifact.rawHash, canonicalHash: artifact.canonicalHash, sizeBytes: artifact.sizeBytes,
            artifactId: artifact.artifactId, acceptanceEventIds, responseData,
          }, 'submissionReplayIndex');
        }
        if (event.type === 'assignment.rejected') {
          const submission = submissions.get(payload.submissionId);
          if (!submission || submission.assignmentId !== assignment.assignmentId) {
            fail('E_OPERATE_BINDING_MISMATCH', 'Assignment rejection lacks its prior Submission.', { submissionId: payload.submissionId });
          }
          submissions.set(payload.submissionId, {
            ...submission, state: 'rejected', acceptanceEventIds: [event.eventId],
            responseData: { accepted: false, violations: clone(payload.violations), attemptsRemaining: payload.attemptsRemaining },
    ...
    if (event.type === 'artifact.created') {
      ...
      const matchingSubmissions = [...submissions.values()].filter((candidate) => candidate.assignmentId === payload.assignmentId
        && candidate.rawHash === payload.rawHash && candidate.canonicalHash === payload.canonicalHash
        && candidate.sizeBytes === payload.sizeBytes && candidate.acceptanceEventIds.at(-1) === event.causationId);
      if (matchingSubmissions.length > 1) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Artifact creation has ambiguous prior Submission custody.', { assignmentId: payload.assignmentId, artifactId: payload.artifactId });
      }
      if (matchingSubmissions.length === 1) {
        const [submission] = matchingSubmissions;
        submissions.set(submission.submissionId, {
          ...submission, acceptanceEventIds: [...submission.acceptanceEventIds, event.eventId],
        });
      }
    }
    ...
    if (event.type === 'action.authority-promoted') {
      ...
      try {
        expectedPromotion = promotePersistentOperatingActionAuthorityV2(payload.before, payload.authority);
      } catch {
        expectedPromotion = null;
      }
      if (!prior || sha256Jcs(prior) !== sha256Jcs(payload.before)
        || Object.hasOwn(prior, 'revisionId') || payload.action.revision !== 1
        || payload.action.predecessorRevisionId !== null
        || !expectedPromotion || sha256Jcs(expectedPromotion) !== sha256Jcs(payload.action)
        || event.requestHash !== sha256Jcs({ contract: 'operating-action-authority-promotion-v2', before: payload.before, authority: payload.authority })
        || event.actor.kind !== 'engine' || event.actor.id !== 'openplanr') {
        fail('E_OPERATE_BINDING_MISMATCH', 'Action authority promotion requires one exact prior Event-backed ungoverned Action.', { actionId: payload.action.actionId });
      }
      derived.actions.set(key, clone(payload.action));
      promotedActionKeys.add(key);
    }
    if (event.type === 'evidence.resolved') {
      put('evidenceResolutions', payload.resolution); put('evidenceRefs', payload.evidenceRef); put('artifacts', payload.evidenceArtifact);
    ...
    if (event.type === 'verification.plan-recorded') {
      ...
      try {
        ...
        fail('E_OPERATE_BINDING_MISMATCH', 'Verification-plan Event cannot reconstruct its exact ledger-derived Action.', {
      ...
      if (prior && sha256Jcs(prior) !== sha256Jcs(reconstructed.action)) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Verification-plan Event Action conflicts with an earlier Event owner.', {
    ...
    if (event.type === 'outcome.recorded') {
      const sourceArtifact = [...derived.artifacts.values()].find(
      ...
      if (payload.status === 'insufficient-evidence'
        ...
        const plan = [...derived.verificationPlans.values()].find(
          ({ verificationPlanId }) => verificationPlanId === payload.verificationPlanId,
        ) ?? null;
        const action = [...derived.actions.values()].find(
        ...
        if (event.actor.kind !== 'runtime' || event.actor.id !== 'openplanr'
          ...
          fail('E_OPERATE_BINDING_MISMATCH', 'Insufficient-evidence Outcome lacks its exact accepted terminal verification Event chain.', {
    ...
    if (event.type === 'learning.recorded') {
      const outcome = [...derived.outcomes.values()].find(
        ({ outcomeId }) => outcomeId === payload.outcomeId,
      ) ?? null;
      const sourceArtifact = outcome ? [...derived.artifacts.values()].find(
      ...
      if (outcome?.status === 'insufficient-evidence'
        ...
        const action = [...derived.actions.values()].find(
          ({ actionId }) => actionId === outcome.actionId,
        ) ?? null;
        ...
        if (event.actor.kind !== 'runtime' || event.actor.id !== 'openplanr'
          ...
          fail('E_OPERATE_BINDING_MISMATCH', 'Insufficient-evidence Learning lacks its exact accepted Outcome and terminal verification Event chain.', {
    ...
    if (event.type === 'policy.evaluated') {
      if (event.actor.kind !== 'runtime' || event.actor.id !== 'openplanr'
        || event.requestHash !== payload.inputHash || event.timestamp !== payload.evaluatedAt) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Policy evaluation authority requires its exact prior runtime Event.', { evaluationId: payload.evaluationId });
      }
      put('policyEvaluations', payload);
    }
    if (event.type === 'approval.recorded') {
      if (event.actor.kind !== payload.actor.kind || event.actor.id !== payload.actor.actorId
        || event.timestamp !== payload.issuedAt) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Approval authority requires its exact prior actor and issuance Event.', { approvalId: payload.approvalId });
      }
    ...
    if (event.type === 'operation.intent-recorded') {
      const intentReplayEntry = validateOperationIntentAuthority(state, event, derived, promotedActionKeys);
      put('governedOperations', payload.operation);
      putGlobal(operationReplay, payload.operation.operationId, intentReplayEntry, 'operationReplayIndex');
      const successSubmission = submissions.get(payload.terminal.submissionId);
      if (!successSubmission || successSubmission.assignmentId !== payload.operation.assignmentId || successSubmission.state !== 'issued') {
        fail('E_OPERATE_BINDING_MISMATCH', 'Operation intent lacks its exact runtime-issued success Submission.', { operationId: payload.operation.operationId });
      }
      putGlobal(submissions, payload.terminal.uncertainty.submissionId, {
        kind: 'operating-submission', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
        submissionId: payload.terminal.uncertainty.submissionId, assignmentId: payload.operation.assignmentId,
        cycleId: event.cycleId, state: 'issued', rawHash: null, canonicalHash: null, sizeBytes: null,
        artifactId: null, acceptanceEventIds: [], responseData: null, issuedAt: event.timestamp, resolvedAt: null,
      }, 'submissions');
      const grantKey = [...derived.capabilityGrants.keys()].find((key) => derived.capabilityGrants.get(key).grantId === payload.operation.grantId);
      if (!grantKey) fail('E_OPERATE_BINDING_MISMATCH', 'Operation intent lacks one prior exact grant Event.', { operationId: payload.operation.operationId });
      const consumedGrant = { ...derived.capabilityGrants.get(grantKey), consumedAt: event.timestamp };
    ...
    if (event.type === 'execution.result-recorded') {
      ...
      const operationKey = [...derived.governedOperations.keys()].find((key) => derived.governedOperations.get(key).operationId === payload.result.operationId);
      ...
      const terminalReservation = ['succeeded', 'partial'].includes(payload.result.status) ? {
        resultId: replay?.reservedResultId, artifactId: replay?.reservedResultArtifactId, submissionId: replay?.reservedSubmissionId,
      } : {
        resultId: replay?.reservedUncertaintyResultId, artifactId: replay?.reservedUncertaintyResultArtifactId, submissionId: replay?.reservedUncertaintySubmissionId,
      };
      const assignment = operation ? [...derived.assignments.values()].find(({ assignmentId }) => assignmentId === operation.assignmentId) : null;
      const submission = submissions.get(terminalReservation.submissionId);
      const artifact = [...derived.artifacts.values()].find(({ artifactId }) => artifactId === terminalReservation.artifactId);
      if (!operation || !replay || operation.state !== 'dispatching' || operation.resultId !== null) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Execution result lacks one prior dispatching operation and replay owner.', { resultId: payload.result.resultId });
      }
      if (!assignment || assignment.state !== 'validated' || !submission || submission.state !== 'accepted' || !artifact
        || payload.result.resultId !== terminalReservation.resultId || payload.result.resultArtifactId !== terminalReservation.artifactId) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Execution result precedes or diverges from its canonical terminal Assignment, Submission, and Artifact.', { resultId: payload.result.resultId });
      }
      const next = { ...operation, state: payload.result.status, resultId: payload.result.resultId, updatedAt: payload.result.completedAt };
      next.operationHash = sha256Jcs(without(next, 'operationHash'));
      derived.governedOperations.set(operationKey, next);
      operationReplay.set(operation.operationId, { ...replay, terminalResultId: payload.result.resultId, terminalReceipt: clone(payload.receipt) });
    }
    ...
    if (event.type === 'rollback.result-recorded') {
      ...
      const operationKey = [...derived.governedOperations.keys()].find((key) => derived.governedOperations.get(key).operationId === payload.rollbackOperationId);
      const operation = operationKey ? derived.governedOperations.get(operationKey) : null;
      ...
      const submission = submissions.get(terminalReservation.submissionId);
      const artifact = [...derived.artifacts.values()].find(({ artifactId }) => artifactId === terminalReservation.artifactId);
      if (!operation || !replay || operation.operationKind !== 'rollback' || operation.state !== 'dispatching' || operation.resultId !== null) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Rollback result lacks one prior dispatching rollback operation and replay owner.', { rollbackResultId: payload.rollbackResultId });
      }
      if (!assignment || assignment.state !== 'validated' || !submission || submission.state !== 'accepted' || !artifact
        || payload.rollbackResultId !== terminalReservation.resultId || payload.resultArtifactId !== terminalReservation.artifactId) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Rollback result precedes or diverges from its canonical terminal Assignment, Submission, and Artifact.', { rollbackResultId: payload.rollbackResultId });
      }
      const next = { ...operation, state: payload.status, resultId: payload.rollbackResultId, updatedAt: payload.completedAt };
      next.operationHash = sha256Jcs(without(next, 'operationHash'));
      derived.governedOperations.set(operationKey, next);
      operationReplay.set(operation.operationId, { ...replay, terminalResultId: payload.rollbackResultId, terminalReceipt: null });
    }
    if (event.type === 'review.created') {
      const key = `${event.cycleId}\u0000${payload.reviewId}`;
      if (reviews.has(key)) fail('E_OPERATE_BINDING_MISMATCH', 'Review lifecycle contains duplicate canonical creation Events.', { reviewId: payload.reviewId });
    ...
    if (event.type === 'review.submitted') {
      ...
      if (record) {
        if (record.state !== 'pending') fail('E_OPERATE_BINDING_MISMATCH', 'Review lifecycle contains a non-contiguous canonical submission Event.', { reviewId: payload.reviewId });
        reviews.set(key, { ...record, state: payload.disposition, disposition: payload.disposition, workDispositions: clone(payload.workDispositions), updatedAt: event.timestamp });
        if (payload.disposition === 'approved' && record.subject?.type === 'cycle') {
          ...
          try {
            ...
            fail('E_OPERATE_BINDING_MISMATCH', 'Approved Review work dispositions do not match canonical Cycle closure semantics.', {
              reviewId: payload.reviewId,
              cause: error.code ?? error.message,
            });
          }
          derived.findings = new Map(projected.findings.map((entry) => [scopedRecordKey(entry, 'findingId'), entry]));
          derived.decisions = new Map(projected.decisions.map((entry) => [scopedRecordKey(entry, 'decisionId'), entry]));
          derived.actions = new Map(projected.actions.map((entry) => [scopedRecordKey(entry, 'actionId'), entry]));
        }
        if (record.subject?.type === 'cycle') {
          ...
          if (!priorCycle || priorCycle.state !== 'awaiting_review') {
            fail('E_OPERATE_BINDING_MISMATCH', 'Cycle Review submission lacks one canonical awaiting-review predecessor.', {
              reviewId: payload.reviewId,
              cycleId: event.cycleId,
            });
          }
    ...
    if (event.type.startsWith('action.') && event.type !== 'action.authority-promoted') {
      if (event.type === 'action.approved' && (event.actor.kind !== 'engine' || event.actor.id !== 'openplanr')) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Action approval requires its exact prior governance Event.', { actionId: payload.action.actionId });
      }
      const key = `${cycle.scopeId}\u0000${cycle.domainId}\u0000${cycle.domainVersion}\u0000${payload.action.actionId}`;
      const current = derived.actions.get(key);
      if (current) {
        if (current.revision !== payload.action.revision || current.actionHash !== payload.action.actionHash || current.state !== payload.from) {
          fail('E_OPERATE_BINDING_MISMATCH', 'Action Event lineage diverges from its canonical projected record.', { eventId: event.eventId, actionId: payload.action.actionId });
        }
        derived.actions.set(key, { ...current, state: payload.to, updatedAt: event.timestamp });
      }
      const previous = actionTransitions.get(key);
      if (previous && previous.state !== payload.from) fail('E_OPERATE_BINDING_MISMATCH', 'Action lifecycle contains non-contiguous canonical transition Events.', { actionId: payload.action.actionId, eventId: event.eventId });
      actionTransitions.set(key, { ...clone(payload.action), state: payload.to, updatedAt: event.timestamp });
    }
    if (event.type.startsWith('cycle.') && event.type !== 'cycle.input-bound') {
      const previous = cycleTransitions.get(event.cycleId);
      if (previous && previous.state !== payload.from) fail('E_OPERATE_BINDING_MISMATCH', 'Cycle lifecycle contains non-contiguous canonical transition Events.', { cycleId: event.cycleId, eventId: event.eventId });
  ...
  for (const [name, [stateField, identityField]] of Object.entries(EVENT_PARITY_COLLECTIONS)) {
    ...
    if (baseState !== null && records.length !== derived[name].size) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Retained checkpoint and tail do not own the exact final record membership.', {
        collection: stateField,
      });
    }
    for (const [key, expected] of derived[name]) {
      ...
      if (candidates.length !== 1 || sha256Jcs(candidates[0]) !== sha256Jcs(expected)) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Current projection state diverges from its canonical Event-backed record.', { collection: stateField, identity: expected[identityField] });
      }
    }
  }
  if (baseState !== null && (state.submissions.length !== submissions.size
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Retained checkpoint and tail do not own the exact final lifecycle membership.');
  }
  for (const [identity, expected] of submissions) {
    const candidates = state.submissions.filter(({ submissionId }) => submissionId === identity);
    if (candidates.length !== 1 || sha256Jcs(candidates[0]) !== sha256Jcs(expected)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Submission diverges from its canonical Event-derived lifecycle.', { submissionId: identity });
    }
  }
  for (const [identity, expected] of submissionReplay) {
    const candidates = state.submissionReplayIndex.filter(({ submissionId }) => submissionId === identity);
    if (candidates.length !== 1 || sha256Jcs(candidates[0]) !== sha256Jcs(expected)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Submission replay diverges from its canonical Event-derived lifecycle.', { submissionId: identity });
    }
  }
  for (const [identity, expected] of operationReplay) {
    const candidates = state.operationReplayIndex.filter(({ operationId }) => operationId === identity);
    if (candidates.length !== 1 || sha256Jcs(candidates[0]) !== sha256Jcs(expected)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current operation replay diverges from its canonical Event-derived lifecycle.', { operationId: identity });
    }
  }
  for (const [key, expected] of actionTransitions) {
    const candidates = state.actions.filter((record) => scopedRecordKey(record, 'actionId') === key);
    const current = candidates[0];
    if (candidates.length !== 1 || current.revision !== expected.revision || current.actionHash !== expected.actionHash
      || current.state !== expected.state || current.updatedAt !== expected.updatedAt) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Action does not equal its final canonical lifecycle Event.', { actionId: expected.actionId });
    }
  }
  for (const [cycleId, expected] of cycleTransitions) {
    ...
    if (!current || current.state !== expected.state || current.updatedAt !== expected.updatedAt) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Cycle does not equal its final canonical lifecycle Event.', { cycleId });
    }
  }
  for (const [key, expected] of reviews) {
    const [cycleId, reviewId] = key.split('\u0000');
    const candidates = state.reviews.filter((record) => record.cycleId === cycleId && record.reviewId === reviewId);
    if (candidates.length !== 1 || sha256Jcs(candidates[0]) !== sha256Jcs(expected)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Review diverges from its canonical Event-backed record.', { reviewId });
    }
  }
...
function validateEventReplayIndex(state, events, checkpointValidation) {
  if (!Array.isArray(events)) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Experience projection requires a bounded canonical Event suffix.');
  }
  const checkpoint = checkpointValidation?.projection ?? null;
  const checkpointState = checkpointValidation?.state ?? null;
  if (events.length === 0) {
    const genesis = state.eventHead.sequence === 0
      && state.eventHead.hash === null
      && state.eventReplayIndex.length === 0
      && checkpoint === null;
    const exactCheckpointBase = checkpoint !== null
    ...
    if (!genesis && !exactCheckpointBase) {
      fail('E_OPERATE_BINDING_MISMATCH', 'An empty retained Event suffix requires genesis or one exact validated checkpoint base.');
    }
    return { events: [], stateParityVerified: exactCheckpointBase, canonicalMetrics: [] };
  }
  const first = events[0];
  const startsAtGenesis = first?.sequence === 1;
  const startsAfterCheckpoint = checkpoint !== null
    && first?.sequence === checkpoint.eventHead.sequence + 1;
  if (!startsAtGenesis && !startsAfterCheckpoint) fail('E_OPERATE_BINDING_MISMATCH', 'Event replay must start at genesis or immediately after the validated checkpoint.');
  ...
  if (events.length !== expectedEventCount) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Experience projection Event suffix does not cover its exact retained replay interval.');
  }
  let finalHead;
  try {
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Experience Event replay is not one exact canonical contiguous chain.', { cause: error.code ?? 'STATE_TRANSITION_INVALID' });
  }
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (event.eventHash !== computeOperatingRuntimeEventHashV2(event)
      || sha256Jcs(replay) !== sha256Jcs(exactReplayEntry(event))) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Replay metadata does not equal its indexed canonical Event.', { eventId: event.eventId });
    }
    const cycle = cycles.get(event.cycleId);
    if (!cycle) fail('E_OPERATE_BINDING_MISMATCH', 'Canonical Event references an unknown Cycle.', { eventId: event.eventId, cycleId: event.cycleId });
    if (expectedEventEntityId(event) !== event.entityId) fail('E_OPERATE_BINDING_MISMATCH', 'Canonical Event entity metadata does not equal its validated payload.', { eventId: event.eventId, entityId: event.entityId });
    if (scopedEventPayloadRecords(event.payload).some((record) => !sameScope(record, cycle))) {
      fail('OPERATING_SCOPE_INVALID', 'Canonical Event payload has a foreign scope/domain/version binding.', { eventId: event.eventId });
    }
  }
  if (finalHead.sequence !== state.eventHead.sequence || finalHead.hash !== state.eventHead.hash) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Canonical Event replay does not equal the runtime final Event head.');
  }
...
function createAccessScreen(artifacts, accessLevel) {
  ...
  const inspect = (artifactIds) => {
    ...
    for (const id of ids) {
      ...
      if (ACCESS_ORDER.indexOf(artifact.sensitivity) > ACCESS_ORDER.indexOf(classification)) classification = artifact.sensitivity;
    }
  ...
  const route = (value, artifactIds) => {
    ...
    omit(result.classification, 'body-never-projected');
    const base = { ...without(value, 'routeHash'), rationale: 'Route details are unavailable at this access level.' };
    return { ...base, routeHash: sha256Jcs(base) };
  };
  const artifactIds = (values) => (values ?? []).filter((id) => {
    const result = inspect([id]);
    if (result.visible) return true;
    omit(result.classification, 'identity-redacted');
    return false;
  });
  return { inspect, omit, text, identity, route, artifactIds, omissions: () => [...omissions.values()] };
}

function inboxEvidence(evidence, evidenceRefIds) {
  const selected = new Set(evidenceRefIds);
  return evidence.filter(({ evidenceRefId }) => selected.has(evidenceRefId)).map((entry) => ({
    evidenceRefId: entry.evidenceRefId,
    classification: entry.classification,
    accessState: entry.accessState,
    relation: entry.claimStatus === 'contradicted' ? 'contradiction'
      : entry.claimStatus === 'supported' ? 'support' : 'informs',
  }));
}

function inboxParty(party, indexes, actorId, requirementId) {
  const recorded = [...indexes.approvalRecords.values()].some((entry) =>
...
function inboxLocator(candidates, unavailableCode = 'OPERATE_ACTION_NOT_AVAILABLE') {
  if (candidates.length > 1) {
    fail('RESULT_CONTRACT_INVALID', 'An Inbox item maps to more than one exact runtime-issued action digest.');
  }
  if (candidates.length === 1) {
    return {
      actionLocator: {
  ...
  return {
    ...
    unavailableReason: {
      ...
      message: unavailableCode === 'OPERATE_APPROVAL_EXPIRED'
...
function inboxReviewLocators(submitCandidates, readCandidates, cycleId) {
  if (readCandidates.length > 1) {
    fail('RESULT_CONTRACT_INVALID', 'A Review Inbox item maps to more than one exact runtime-issued read action.');
  }
  ...
  if (reviewIds.size > 1) {
    fail('E_OPERATE_BINDING_MISMATCH', 'A Review Inbox item cannot combine actions for different Reviews.');
  }
  const navigationLocator = readCandidates.length === 1
    ? (() => {
        const [{ subjectId, action }] = readCandidates;
        const reviewId = action.arguments?.reviewId;
        if (action.tool !== 'operate.review.get'
          || action.effect !== 'read-only'
          || subjectId !== reviewId
          || action.arguments?.cycleId !== cycleId) {
          fail('E_OPERATE_BINDING_MISMATCH', 'Review Inbox navigation requires one exact read-only Review action.');
        }
        return {
          kind: 'review',
          cycleId,
  ...
  if (submitCandidates.length > 1) {
    if (navigationLocator === null) {
      fail('RESULT_CONTRACT_INVALID', 'Multiple Review choices require one exact read-only Review navigation action.');
    }
...
function buildInbox(indexes, cycles, actorId, screen, generatedAt, evidence, allowedActions) {
  ...
  const items = [];
  for (const decision of [...indexes.decisions.values()].filter((entry) => entry.ownerActorId === actorId && ['proposed', 'deferred'].includes(entry.state))) {
    ...
    const reviewCandidates = allowedActions.filter(({ action }) => {
      ...
      return review?.state === 'pending'
        ...
        && args.workDispositions.some((entry) => entry?.entityType === 'operating-decision'
          && entry.entityId === decision.decisionId);
    });
    const reviewIds = new Set(reviewCandidates.map(({ action }) => action.arguments.reviewId));
    const reviewReadCandidates = allowedActions.filter(({ action }) => (
      action.tool === 'operate.review.get'
      && reviewIds.has(action.arguments?.reviewId)
    ));
    items.push({
      itemId: `decision:${decision.decisionId}`, kind: 'decision', subjectId: decision.decisionId,
      ownerActorId: actorId, state: decision.state,
      title: screen.text(decision.title, 'Restricted Decision', sources),
      consequence: screen.text(decision.expectedDownside, 'Decision details are unavailable at this access level.', sources),
      expiresAt: decision.revisitAt, blocking: decision.state === 'proposed',
      evidence: inboxEvidence(evidence, decision.evidenceRefIds),
      requiredParties: [{
        partyId: `review-owner:${actorId}`, actorKind: 'human', actorId,
        requiredCapability: { id: 'review-submit-authorized', version: '1.0.0' },
        state: 'required', redacted: false,
      }],
  ...
  for (const review of [...indexes.reviews.values()].filter((entry) =>
    ...
    if (!action
      ...
      fail('E_OPERATE_BINDING_MISMATCH', 'Action Review Inbox work requires one exact current Action subject.');
    }
    const candidates = allowedActions.filter(({ subjectId, action: candidate }) =>
    ...
    const readCandidates = allowedActions.filter(({ subjectId, action: candidate }) => (
      subjectId === review.reviewId
      && candidate.tool === 'operate.review.get'
      && candidate.arguments?.reviewId === review.reviewId
    ));
    const decision = action.sourceDecisionId === null
      ? null
      : indexes.decisions.get(action.sourceDecisionId);
    const evidenceRefIds = decision?.evidenceRefIds ?? [];
    const safeEvidence = inboxEvidence(evidence, evidenceRefIds);
    items.push({
      itemId: `action-review:${review.reviewId}`, kind: 'approval',
      subjectId: review.reviewId, ownerActorId: actorId, state: review.state,
      title: screen.text(`Review ${action.title}`, 'Restricted Action Review', [action.sourceArtifactId]),
      consequence: screen.text(action.expectedResult, 'Action Review details are unavailable at this access level.', [action.sourceArtifactId]),
      expiresAt: null, blocking: true, evidence: safeEvidence,
      requiredParties: [{
        partyId: `action-review-owner:${actorId}`, actorKind: 'human', actorId,
        requiredCapability: { id: 'review-submit-authorized', version: '1.0.0' },
        state: 'required', redacted: false,
      }],
  ...
  const approvalRequirements = [...indexes.approvalRequirements.values()].filter((entry) =>
    (entry.namedActorIds.includes(actorId) || entry.parties.some((party) => party.actorId === actorId))
  ...
  for (const requirement of approvalRequirements) {
    const candidates = allowedActions.filter(({ action }) =>
    ...
    if (candidates.length === 1) {
      ...
      if (approvalActionKeys.has(key)) {
        fail('RESULT_CONTRACT_INVALID', 'Distinct Inbox approvals cannot share one ambiguous action digest.');
      }
      approvalActionKeys.add(key);
    }
    const parties = requirement.parties.map((party) => inboxParty(party, indexes, actorId, requirement.requirementId));
    const expired = requirement.expiresAt !== null && Date.parse(requirement.expiresAt) <= Date.parse(generatedAt);
    items.push({
      itemId: `approval:${requirement.requirementId}`, kind: 'approval',
      subjectId: requirement.action.actionId, ownerActorId: actorId, state: expired ? 'expired' : 'waiting',
      ...
      expiresAt: requirement.expiresAt, blocking: true, evidence: [], requiredParties: parties,
      redactions: parties.filter(({ redacted }) => redacted)
  ...
  for (const candidate of allowedActions.filter(({ action }) =>
    action.tool === 'operate.action.approve'
    && action.arguments?.decision === 'approved'
    ...
    if (!action
      ...
      fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval Inbox work requires one exact current plan and terminal result binding.');
    }
    ...
    approvalActionKeys.add(key);
    const parties = template.requirement.parties.map((party) => inboxUnrecordedParty(party, actorId));
    const decision = action.sourceDecisionId === null ? null : indexes.decisions.get(action.sourceDecisionId);
    ...
    items.push({
      itemId: `approval:rollback:${plan.rollbackPlanId}`, kind: 'approval',
      subjectId: action.actionId, ownerActorId: actorId, state: expired ? 'expired' : 'waiting',
      title: screen.text(`Rollback approval for ${action.title}`, 'Restricted rollback approval', [action.sourceArtifactId]),
      consequence: screen.text(`Approve the exact rollback plan before reversing ${action.effectClass}.`, 'Rollback approval details are unavailable at this access level.', [action.sourceArtifactId]),
      expiresAt: plan.expiresAt, blocking: true, evidence: safeEvidence, requiredParties: parties,
      redactions: parties.filter(({ redacted }) => redacted)
        .map(() => ({ classification: 'restricted', count: 1, reason: 'identity-redacted' })),
      ...inboxLocator(expired ? [] : [candidate], expired ? 'OPERATE_APPROVAL_EXPIRED' : undefined),
    });
  }
  for (const assignment of [...indexes.assignments.values()].filter((entry) => cycleIds.has(entry.cycleId) && entry.assignmentKind === 'verification' && ['available', 'claimed', 'running'].includes(entry.state))) {
    if (assignment.claim?.actorId === actorId || assignment.state === 'available') {
      ...
      items.push({
        itemId: `verification:${assignment.assignmentId}`, kind: 'verification',
        subjectId: assignment.assignmentId, ownerActorId: actorId, state: assignment.state,
        title: screen.text(assignment.objective, 'Restricted verification work', assignment.inputArtifactIds),
        consequence: 'The expected outcome remains unverified.', expiresAt: null, blocking: false,
        evidence: [],
        requiredParties: [{
          partyId: `verification-owner:${actorId}`, actorKind: 'human', actorId,
          requiredCapability: { id: 'assignment-submit', version: '1.0.0' },
          state: 'required', redacted: false,
        }],
        redactions: [], ...inboxLocator(candidates),
      });
    }
  }
  return items.sort((left, right) => Number(right.blocking) - Number(left.blocking) || left.itemId.localeCompare(right.itemId));
}
...
function attentionPriority(indexes, item, actorId, generatedAt) {
  ...
  const approval = item.kind === 'approval'
    ...
    : null;
  const action = ['action', 'uncertainty'].includes(item.kind) ? indexes.actions.get(item.subjectId) : null;
  const outcome = item.kind === 'outcome' ? indexes.outcomes.get(item.subjectId) : null;
  const effectMateriality = { 'read-only': 0, 'machine-local-write': 40, 'project-write': 100, 'provider-call': 125, 'external-effect': 180, destructive: 240 };
  const materiality = decision ? Math.round(decision.confidence * 160) + Math.min(decision.evidenceRefIds.length, 4) * 20
    : approval ? (effectMateriality[approval.effectClass] ?? 0)
      : action ? Math.min(220, Math.round(Math.abs(action.target - action.baseline) * 220)) + (action.state === 'blocked' ? 60 : 0)
        : outcome ? 140 : item.kind === 'verification' ? 100 : 0;
  const urgency = ({ blocked: 170, uncertain: 170, failed: 150, pending: 110, proposed: 105, waiting: 100, claimed: 90, running: 90, in_progress: 80, queued: 65, approved: 55, available: 45 }[item.state] ?? 25);
  const owned = decision?.ownerActorId === actorId || item.kind === 'approval' ? 110
    : action?.ownerActorId === actorId ? 90 : item.kind === 'verification' ? 70 : 0;
  const unresolvedDependencies = action ? action.dependsOnActionIds.filter((dependencyId) => indexes.actions.get(dependencyId)?.state !== 'completed').length : 0;
  const dependencyBlocking = Math.min(200, unresolvedDependencies * 70 + (item.state === 'blocked' || item.blocking ? 90 : 0));
  const expiry = expiryProximityScore(item.dueAt, generatedAt);
  const uncertainty = item.kind === 'uncertainty' || ['uncertain', 'partial', 'failed'].includes(item.state) ? 150 : 0;
  const dueVerification = item.kind === 'verification' || item.kind === 'outcome' ? 150
    : action?.state === 'completed' && ![...indexes.outcomes.values()].some((entry) => entry.actionId === action.actionId && entry.status === 'succeeded') ? 120 : 0;
  return Math.min(1000, materiality + urgency + owned + dependencyBlocking + expiry + uncertainty + dueVerification);
}

function sortAttention(items) {
  return items.sort((left, right) => right.priority - left.priority || left.attentionId.localeCompare(right.attentionId));
}

function buildAttention(indexes, inbox, actions, outcomes, actorId, generatedAt) {
  const items = inbox.map((item) => {
    const base = {
      attentionId: `attention:${item.itemId}`, inboxItemId: item.itemId,
      kind: item.kind, subjectId: item.subjectId,
      title: item.title, whyNow: item.expiresAt ? `This item expires at ${item.expiresAt}.` : item.blocking ? 'Other work is waiting on this item.' : 'This work is ready for attention.',
      consequence: item.consequence, state: item.state, dueAt: item.expiresAt, evidenceRefIds: [], blocking: item.blocking,
    };
    return {
  ...
  for (const action of actions) {
    ...
    const base = { attentionId: `attention:action:${action.actionId}`, kind: action.state === 'blocked' ? 'uncertainty' : 'action', subjectId: action.actionId, title: action.title, whyNow: action.state === 'blocked' ? 'The Action cannot continue safely.' : 'The Action has current operating work.', consequence: action.expectedResult, state: action.state, dueAt: null, evidenceRefIds: [] };
  ...
  for (const outcome of outcomes.filter(({ status }) => status !== 'succeeded')) {
    const base = { attentionId: `attention:outcome:${outcome.outcomeId}`, kind: 'outcome', subjectId: outcome.outcomeId, title: `Outcome ${outcome.status}`, whyNow: 'The expected result is not confirmed.', consequence: 'Inspect verification evidence before closing the work.', state: outcome.status, dueAt: null, evidenceRefIds: [...outcome.evidenceRefIds] };
    items.push({ ...base, priority: attentionPriority(indexes, base, actorId, generatedAt) });
  }
...
function accessSafeEvidence(indexes, scope, accessLevel) {
  ...
  for (const item of [...indexes.evidenceRefs.values()].sort((a, b) => a.evidenceRefId.localeCompare(b.evidenceRefId))) {
    ...
    if (!sourceArtifact || !artifact
      ...
      fail('E_OPERATE_BINDING_MISMATCH', 'EvidenceRef does not bind the exact Artifact identity, hashes, media, size, and access classification.', { evidenceRefId: item.evidenceRefId });
    }
    const resolutions = [...indexes.evidenceResolutions.values()].filter(({ candidateId }) => candidateId === item.candidateId);
    const edges = [...indexes.evidenceEdges.values()].filter(({ evidenceRefId, classification }) => evidenceRefId === item.evidenceRefId && ACCESS_ORDER.indexOf(classification) <= maximum);
    const claims = [...indexes.claims.values()];
    const supportingClaimIds = claims.filter(({ supportingEvidenceRefIds }) => supportingEvidenceRefIds.includes(item.evidenceRefId)).map(({ claimId }) => claimId).sort();
    const contradictingClaimIds = claims.filter(({ contradictingEvidenceRefIds }) => contradictingEvidenceRefIds.includes(item.evidenceRefId)).map(({ claimId }) => claimId).sort();
    const claimStatus = contradictingClaimIds.length > 0 && supportingClaimIds.length > 0 ? 'uncertain'
      : contradictingClaimIds.length > 0 ? 'contradicted'
        : supportingClaimIds.length > 0 ? 'supported' : 'unknown';
    evidence.push({
      ...
      source: allowed ? { evidenceKind: item.evidenceKind, provider: clone(item.provider), resolver: clone(item.resolver) } : null,
      ...
      confidence: allowed && edges.length > 0 ? Math.max(...edges.map(({ confidence }) => confidence)) : null,
      gaps: [],
      errors: allowed ? resolutions.filter(({ outcome }) => outcome === 'rejected').map(({ resolutionId, error, resolvedAt }) => ({ resolutionId, error: clone(error), resolvedAt })) : [],
      accessReason: allowed ? null : 'access-denied',
      causalLinks: allowed ? [
        ...supportingClaimIds.map((claimId) => ({ kind: 'claim', subjectId: claimId, relation: 'support', deepLink: deepLink('evidence', item.evidenceRefId) })),
        ...contradictingClaimIds.map((claimId) => ({ kind: 'claim', subjectId: claimId, relation: 'contradiction', deepLink: deepLink('evidence', item.evidenceRefId) })),
        ...edges.map(({ localClaimId, relation }) => ({ kind: 'local-claim', subjectId: localClaimId, relation: relation === 'supportedBy' ? 'support' : 'contradiction', deepLink: deepLink('evidence', item.evidenceRefId) })),
      ].sort((left, right) => left.subjectId.localeCompare(right.subjectId) || left.relation.localeCompare(right.relation)) : [],
  ...
  return {
    evidence,
    omissions: [...omitted].map(([classification, count]) => ({ classification, count, reason: 'access-denied' })),
  };
}
...
function buildClaims(indexes, evidence, scope, screen) {
  ...
  return [...indexes.claims.values()].map((claim) => {
    const source = accessSafeArtifactMetadata(indexes, screen, claim.sourceArtifactId);
    const support = claim.supportingEvidenceRefIds.filter((id) => evidenceAccess.get(id)?.accessState === 'available').sort();
    const contradiction = claim.contradictingEvidenceRefIds.filter((id) => evidenceAccess.get(id)?.accessState === 'available').sort();
    const verifiedSupport = support.filter((id) => evidenceAccess.get(id)?.freshness !== 'stale');
    const verifiedContradiction = contradiction.filter((id) => evidenceAccess.get(id)?.freshness !== 'stale');
    const status = !source.access.visible ? 'restricted'
      : verifiedContradiction.length > 0 && verifiedSupport.length > 0 ? 'uncertain'
        : verifiedContradiction.length > 0 ? 'contradicted'
          : verifiedSupport.length > 0 ? 'supported'
            : claim.epistemicStatus === 'unknown' ? 'unknown' : 'uncertain';
    const decisions = [...indexes.decisions.values()].filter((decision) => decision.evidenceRefIds.some((id) => support.includes(id) || contradiction.includes(id)));
    const actions = [...indexes.actions.values()].filter((action) => decisions.some(({ decisionId }) => decisionId === action.sourceDecisionId));
    const outcomes = [...indexes.outcomes.values()].filter((outcome) => actions.some(({ actionId }) => actionId === outcome.actionId));
    const errors = [...new Set([...support, ...contradiction])].flatMap((id) => evidence.find(({ evidenceRefId }) => evidenceRefId === id)?.errors ?? []);
    return {
      ...
      provenance: source.artifact ? { artifactId: source.artifact.artifactId, rawHash: source.artifact.rawHash, canonicalHash: source.artifact.canonicalHash } : null,
      ...
      causalLinks: source.access.visible ? [
        ...decisions.map(({ decisionId }) => ({ kind: 'decision', subjectId: decisionId, relation: 'informs', deepLink: deepLink('decision', decisionId) })),
        ...actions.map(({ actionId }) => ({ kind: 'action', subjectId: actionId, relation: 'informs', deepLink: deepLink('action', actionId) })),
        ...outcomes.map(({ outcomeId }) => ({ kind: 'outcome', subjectId: outcomeId, relation: 'evaluates', deepLink: deepLink('outcome', outcomeId) })),
      ].sort((left, right) => left.kind.localeCompare(right.kind) || left.subjectId.localeCompare(right.subjectId)) : [],
...
function safeRationale(state, indexes, scope, screen) {
  ...
  for (const claim of (state.claims ?? []).filter((entry) => sameScope(entry, scope))) add({ nodeId: `claim:${claim.claimId}`, kind: 'claim', subjectId: claim.claimId, summary: claim.statement, artifactId: claim.sourceArtifactId, evidenceRefIds: [...claim.supportingEvidenceRefIds, ...claim.contradictingEvidenceRefIds].sort() });
  for (const decision of indexes.decisions.values()) add({ nodeId: `decision:${decision.decisionId}`, kind: 'decision', subjectId: decision.decisionId, summary: decision.rationale, artifactId: decision.sourceArtifactId, evidenceRefIds: [...decision.evidenceRefIds].sort() });
  for (const action of indexes.actions.values()) add({ nodeId: `action:${action.actionId}`, kind: 'action', subjectId: action.actionId, summary: action.expectedResult, artifactId: action.sourceArtifactId, evidenceRefIds: [] });
  for (const outcome of indexes.outcomes.values()) add({ nodeId: `outcome:${outcome.outcomeId}`, kind: 'outcome', subjectId: outcome.outcomeId, summary: `Verification status: ${outcome.status}.`, artifactId: outcome.sourceArtifactId, evidenceRefIds: [...outcome.evidenceRefIds].sort() });
  for (const learning of indexes.learnings.values()) add({ nodeId: `learning:${learning.learningId}`, kind: 'learning', subjectId: learning.learningId, summary: learning.statement, artifactId: learning.sourceArtifactId, evidenceRefIds: [...learning.evidenceRefIds].sort() });
...
function exactInsufficientEvidenceChain(indexes, plan, action, outcome) {
  ...
  if (!plan || !action || !artifact || !assignment || !operation || !result
    ...
    fail('E_OPERATE_BINDING_MISMATCH', 'Insufficient-evidence Outcome requires one exact accepted terminal verification Assignment, Artifact, operation result, Action, and plan.', {
...
function exactVerificationChain(indexes, plan, outcome = null) {
  ...
  if (!plan || !action || !metric
    ...
    || (outcome !== null && (outcome.actionId !== action.actionId || outcome.verificationPlanId !== plan.verificationPlanId))) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Outcome verification must bind the exact Action, verification plan, Metric, baseline, target, and window.', { outcomeId: outcome?.outcomeId ?? null, verificationPlanId: plan?.verificationPlanId ?? null });
  }
  if (outcome?.status === 'insufficient-evidence') {
    const artifact = [...indexes.artifacts.values()].find(
      ({ artifactId }) => artifactId === outcome.sourceArtifactId,
    ) ?? null;
    const assignment = artifact
      ? [...indexes.assignments.values()].find(
  ...
  const observations = outcome === null ? [] : outcome.observationIds.map((observationId) => {
    const observation = indexes.metricObservations.get(observationId);
    if (!observation || observation.metricId !== metric.metricId || !metric.observationIds.includes(observationId)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Outcome observation does not bind the exact verified Metric.', { outcomeId: outcome.outcomeId, observationId });
    }
    return observation;
  });
  return {
...
function buildOutcomes(indexes, screen) {
  return [...indexes.outcomes.values()].map((outcome) => {
    ...
    const { action, metric, observations: matchedObservations, metricHash, verificationPlanHash } = exact;
    ...
    const delta = snapshot ? newest([...indexes.deltas.values()].filter((entry) => entry.currentSnapshotId === snapshot.snapshotId), 'derivedAt') : null;
    const decision = action?.sourceDecisionId ? indexes.decisions.get(action.sourceDecisionId) ?? null : null;
    const planAccess = plan ? screen.inspect([plan.sourceArtifactId]) : { visible: true, classification: 'public' };
    const execution = [...indexes.executionResults.values()].filter((entry) => entry.action.actionId === outcome.actionId).map((entry) => resultProjection(entry, screen, 'resultId')).sort((left, right) => left.completedAt.localeCompare(right.completedAt));
    const rollback = [...indexes.rollbackResults.values()].filter((entry) => entry.action.actionId === outcome.actionId).map((entry) => resultProjection(entry, screen, 'rollbackResultId')).sort((left, right) => left.completedAt.localeCompare(right.completedAt));
    return {
      ...
      evidenceRefIds: planAccess.visible ? outcome.evidenceRefIds.filter((id) => {
        const evidence = indexes.evidenceRefs.get(id);
        return evidence ? screen.inspect([evidence.evidenceArtifactId]).visible : false;
      }).sort() : [],
      observedAt: outcome.observedAt,
      decision: decision ? { decisionId: decision.decisionId, revision: decision.revision, state: decision.state, deepLink: deepLink('decision', decision.decisionId) } : null,
      execution,
      rollback,
      verification: plan ? {
      ...
      nextObservation: planAccess.visible && plan ? { ...clone(plan.observationRequest), dueAt: null } : null,
      revisit: planAccess.visible && plan ? {
        decisionIds: [...plan.revisitDecisionIds].sort(),
        conditions: plan.revisitDecisionIds.flatMap((decisionId) => indexes.decisions.get(decisionId)?.revisitConditions ?? []),
      } : null,
      snapshot: snapshot ? { snapshotId: snapshot.snapshotId, createdAt: snapshot.createdAt } : null,
      delta: delta ? { deltaId: delta.deltaId, priorSnapshotId: delta.priorSnapshotId, currentSnapshotId: delta.currentSnapshotId, derivedAt: delta.derivedAt } : null,
      accessReason: planAccess.visible ? null : 'access-denied',
...
function historySubject(indexes, entityId) {
  const candidates = [
    ['cycle', indexes.cycles.get(entityId)], ['input-binding', [...indexes.inputBindings.values()].find(({ inputBindingId }) => inputBindingId === entityId)],
    ['assignment', indexes.assignments.get(entityId)], ['review', indexes.reviews.get(entityId)],
    ['artifact', indexes.artifacts.get(entityId)], ['decision', indexes.decisions.get(entityId)], ['action', indexes.actions.get(entityId)],
    ['outcome', indexes.outcomes.get(entityId)], ['learning', indexes.learnings.get(entityId)], ['operation', indexes.governedOperations.get(entityId)],
    ['execution-result', indexes.executionResults.get(entityId)], ['rollback-result', indexes.rollbackResults.get(entityId)],
  ];
...
function historySourceArtifacts(indexes, kind, record) {
  ...
  if (['decision', 'action', 'outcome', 'learning'].includes(kind)) return record.sourceArtifactId ? [record.sourceArtifactId] : [];
...
function historyEvidence(indexes, kind, record) {
  if (!record) return [];
  if (kind === 'decision' || kind === 'outcome' || kind === 'learning') return [...record.evidenceRefIds].sort();
  if (kind === 'action') return indexes.decisions.get(record.sourceDecisionId)?.evidenceRefIds?.slice().sort() ?? [];
  return [];
}
...
function historySummary(indexes, kind, record) {
  ...
  if (kind === 'execution-result' || kind === 'rollback-result') return record.effectSummary.summary;
...
function historyWhy(indexes, kind, record) {
  ...
  if (kind === 'action') return indexes.decisions.get(record.sourceDecisionId)?.rationale ?? record.expectedResult;
  if (kind === 'outcome') {
    const action = indexes.actions.get(record.actionId);
    return action ? indexes.decisions.get(action.sourceDecisionId)?.rationale ?? action.expectedResult : null;
  }
  if (kind === 'learning') return record.statement;
  return null;
}
...
function historyAuthority(indexes, kind, record) {
  const operation = kind === 'operation' ? record
    : ['execution-result', 'rollback-result'].includes(kind) ? indexes.governedOperations.get(record.operationId ?? record.rollbackOperationId) : null;
  return operation ? {
...
function buildHistory(indexes, cycles, allowedActions, screen) {
  const cycleIds = new Set(cycles.map(({ cycleId }) => cycleId));
  const nextBySubject = new Map(allowedActions.map(({ subjectId, action }) => [subjectId, { label: action.label, tool: action.tool }]));
  return indexes.stateEvents.filter((entry) => cycleIds.has(entry.cycleId)).map((entry) => {
    ...
    const lifecycleTransition = (kind === 'action' || kind === 'cycle') && entry.payload
      && typeof entry.payload.from === 'string' && typeof entry.payload.to === 'string';
    const canonicalTransitionSummary = lifecycleTransition ? `${entry.payload.from} → ${entry.payload.to}` : null;
    const change = lifecycleTransition ? canonicalTransitionSummary : screen.text(historySummary(indexes, kind, record), null, sourceArtifactIds);
    const why = lifecycleTransition
      ? (typeof entry.payload.reasonCode === 'string' ? entry.payload.reasonCode : null)
      : screen.text(historyWhy(indexes, kind, record), null, sourceArtifactIds);
    const delta = newest([...indexes.deltas.values()].filter((candidate) => [...candidate.sourceRevisionChanges, ...candidate.metricChanges].some(({ subjectId }) => subjectId === entry.entityId)), 'derivedAt');
    const changeRecord = delta ? [...delta.sourceRevisionChanges, ...delta.metricChanges].find(({ subjectId }) => subjectId === entry.entityId) : null;
    const result = kind === 'execution-result' || kind === 'rollback-result' ? { status: record.status, completedAt: record.completedAt }
      : record?.state ? { status: record.state, completedAt: record.updatedAt ?? null }
        : record?.status ? { status: record.status, completedAt: record.observedAt ?? null } : null;
    ...
    return {
      ...
      actorId: sourceArtifactIds.length > 0 ? screen.identity(entry.actor.id, sourceArtifactIds) : null,
      ...
      beforeAfter: delta && changeRecord ? {
...
function buildReplay(state, checkpoint, sourceStateHash, stateParityVerified, omissions) {
  ...
  const startSequence = checkpoint ? checkpoint.eventHead.sequence + 1 : state.eventReplayIndex[0]?.sequence ?? null;
  const tail = checkpoint ? state.eventReplayIndex.filter(({ sequence }) => sequence > checkpoint.eventHead.sequence) : state.eventReplayIndex;
  return {
    ...
    parityProof: {
      ...
      stateParityVerified,
      finalEventHashMatches: state.eventHead.sequence === 0
        ? state.eventHead.hash === null
...
function mergeOmissions(...groups) {
  ...
  return [...merged.values()].sort((left, right) => left.classification.localeCompare(right.classification) || left.reason.localeCompare(right.reason));
}
...
function newest(records, field) {
  return [...records].sort((left, right) => right[field].localeCompare(left[field]) || JSON.stringify(left).localeCompare(JSON.stringify(right)))[0] ?? null;
}

function assignmentStage(assignment) {
  return { advisor: 'understand', challenger: 'understand', chair: 'understand', execution: 'act', verification: 'verify' }[assignment.assignmentKind] ?? 'observe';
}
...
function buildDomainMetrics(indexes, screen) {
  ...
  return [...indexes.metrics.values()].map((metric) => {
    ...
    const metricObservations = observations
      .filter((entry) => entry.metricId === metric.metricId && metric.observationIds.includes(entry.observationId))
    ...
    const delta = newest(deltas.filter((entry) => entry.currentSnapshotId === current?.snapshotId
      && entry.metricChanges.some(({ subjectId }) => subjectId === metric.metricId)), 'derivedAt');
    const deltaChange = delta?.metricChanges.find(({ subjectId }) => subjectId === metric.metricId) ?? null;
    const verification = plans.filter((entry) => entry.metricId === metric.metricId).map((plan) => {
      const { action } = exactVerificationChain(indexes, plan);
      const outcome = newest([...indexes.outcomes.values()].filter((entry) => entry.verificationPlanId === plan.verificationPlanId), 'observedAt');
      return {
        verificationPlanId: plan.verificationPlanId,
    ...
    return {
      ...
      change: access.visible && (current || deltaChange) ? {
      ...
      evidenceRefIds: access.visible ? metric.evidenceRefIds.filter((id) => {
        const evidence = indexes.evidenceRefs.get(id);
        return evidence ? screen.inspect([evidence.evidenceArtifactId]).visible : false;
      }).sort() : [],
      snapshot: snapshot ? { snapshotId: snapshot.snapshotId, createdAt: snapshot.createdAt } : null,
...
function buildCycleDetails(indexes, cycles, screen, replayCheckpoint, generatedAt, scope, state, accessLevel) {
  ...
  const uncertainClaims = [...indexes.claims.values()].filter(({ epistemicStatus }) => ['speculative', 'unknown'].includes(epistemicStatus));
  const uncertainOperations = [...indexes.governedOperations.values()].filter(({ state }) => ['partial', 'uncertain', 'blocked'].includes(state));
  return cycles.map((cycle) => {
    ...
    const rows = cycleAssignments.map((assignment) => {
      const outputArtifactIds = [...indexes.artifacts.values()].filter(({ assignmentId }) => assignmentId === assignment.assignmentId).map(({ artifactId }) => artifactId).sort();
      const dependencies = assignment.dependsOn.map((assignmentId) => {
        const dependency = indexes.assignments.get(assignmentId);
        return { assignmentId, state: dependency?.state ?? null, resolved: dependency?.state === 'validated' };
      });
      return {
        ...
        ownerLabel: screen.identity(assignment.claim?.actorId ?? null, assignment.inputArtifactIds),
        ...
        dependencies,
        blockers: dependencies.filter(({ resolved }) => !resolved).map(({ assignmentId }) => assignmentId),
    ...
    const evidenceGapIds = resolutions.filter(({ outcome, sourceArtifactId }) => outcome === 'rejected' && screen.inspect([sourceArtifactId]).visible).map(({ resolutionId }) => resolutionId).sort();
    const uncertaintyIds = [
      ...uncertainClaims.filter(({ sourceArtifactId }) => screen.inspect([sourceArtifactId]).visible).map(({ claimId }) => claimId),
      ...uncertainOperations.filter(({ action }) => cycleActions.some(({ actionId }) => actionId === action.actionId)).map(({ operationId }) => operationId),
    ].sort();
    const reviews = [...indexes.reviews.values()].filter((entry) => entry.cycleId === cycle.cycleId);
    const requirements = [...indexes.approvalRequirements.values()].filter((requirement) =>
      currentApprovalRequirement(requirement, indexes)
      && cycleActions.some(({ actionId, revision, actionHash }) =>
        actionId === requirement.action.actionId
        && revision === requirement.action.revision
        && actionHash === requirement.action.actionHash));
    const gatesByStage = {
      decide: reviews.map(({ reviewId, state }) => ({ kind: 'review', subjectId: reviewId, state })),
      govern: requirements.map((requirement) => ({ kind: 'approval', subjectId: requirement.requirementId, state: approvalRequirementConsumed(requirement, indexes) || approvalComplete(requirement, indexes, generatedAt) ? 'complete' : 'waiting' })),
      act: [...indexes.governedOperations.values()].filter(({ action }) => cycleActions.some(({ actionId }) => actionId === action.actionId)).map(({ operationId, state }) => ({ kind: 'execution', subjectId: operationId, state })),
      verify: [...indexes.outcomes.values()].filter(({ actionId }) => cycleActions.some((action) => action.actionId === actionId)).map(({ outcomeId, status }) => ({ kind: 'verification', subjectId: outcomeId, state: status })),
      learn: [...indexes.learnings.values()].filter(({ outcomeId }) => [...indexes.outcomes.values()].some((outcome) => outcome.outcomeId === outcomeId && cycleActions.some((action) => action.actionId === outcome.actionId))).map(({ learningId }) => ({ kind: 'learning', subjectId: learningId, state: 'recorded' })),
    };
    const stages = cycle.stages.map((stage) => {
      const stageAssignments = rows.filter((entry) => assignmentStage(indexes.assignments.get(entry.assignmentId)) === stage.id);
      return {
        ...stage,
        inputArtifactIds: [...new Set(stageAssignments.flatMap(({ inputArtifactIds }) => inputArtifactIds))].sort(),
        outputArtifactIds: [...new Set(stageAssignments.flatMap(({ outputArtifactIds }) => outputArtifactIds))].sort(),
        gates: (gatesByStage[stage.id] ?? []).sort((left, right) => left.subjectId.localeCompare(right.subjectId)),
        evidenceGapIds: stage.id === 'understand' ? evidenceGapIds : [],
        uncertaintyIds: ['understand', 'act', 'verify'].includes(stage.id) ? uncertaintyIds : [],
        persistentActionIds: ['act', 'verify', 'learn'].includes(stage.id) ? cycleActions.map(({ actionId }) => actionId).sort() : [],
      };
    });
    const dependencies = [
      ...rows.filter(({ dependencies: values }) => values.length > 0).map(({ assignmentId, dependencies: values }) => ({ subjectKind: 'assignment', subjectId: assignmentId, dependsOn: values })),
      ...cycleActions.filter(({ dependsOnActionIds }) => dependsOnActionIds.length > 0).map((action) => ({ subjectKind: 'action', subjectId: action.actionId, dependsOn: action.dependsOnActionIds.map((actionId) => ({ actionId, state: indexes.actions.get(actionId)?.state ?? null, resolved: indexes.actions.get(actionId)?.state === 'completed' })) })),
    ];
    const lensAbsences = lensAbsencesForPlan(intelligencePlan);
    const cycleRecord = indexes.cycles.get(cycle.cycleId);
    const cycleReviews = [...indexes.reviews.values()].filter((entry) => (
      entry.cycleId === cycle.cycleId && entry.subject?.type !== 'action'
    ));
    ...
    if (intelligencePlan && cycleReviews.length === 1) {
      try {
        ...
        if (cycleRecord?.state === 'awaiting_review') {
          fail('E_OPERATE_BINDING_MISMATCH', 'Awaiting-Review Cycle cannot resolve one exact Executive Board truth.', {
            cycleId: cycle.cycleId,
            cause: error.code ?? error.message,
          });
        }
    ...
    return {
      ...
      dependencies,
      blockers: dependencies.filter(({ dependsOn }) => dependsOn.some(({ resolved }) => !resolved)).map(({ subjectKind, subjectId, dependsOn }) => ({ subjectKind, subjectId, blockingSubjectIds: dependsOn.filter(({ resolved }) => !resolved).map((entry) => entry.assignmentId ?? entry.actionId).sort() })),
...
const ALLOWED_ACTION_SUBJECTS = Object.freeze({
  'operate.cycle.get': { kind: 'cycle', index: 'cycles', stateField: 'cycles', identityField: 'cycleId', argumentField: 'cycleId' },
  'operate.cycle.resume': { kind: 'cycle', index: 'cycles', stateField: 'cycles', identityField: 'cycleId', argumentField: 'cycleId' },
  'operate.assignment.claim': { kind: 'assignment', index: 'assignments', stateField: 'assignments', identityField: 'assignmentId', argumentField: 'assignmentId' },
  'operate.assignment.submit': { kind: 'assignment', index: 'assignments', stateField: 'assignments', identityField: 'assignmentId', argumentField: 'assignmentId' },
  'operate.artifact.get': { kind: 'artifact', index: 'artifacts', stateField: 'artifacts', identityField: 'artifactId', argumentField: 'artifactId' },
  'operate.review.get': { kind: 'review', index: 'reviews', stateField: 'reviews', identityField: 'reviewId', argumentField: 'reviewId' },
  'operate.review.submit': { kind: 'review', index: 'reviews', stateField: 'reviews', identityField: 'reviewId', argumentField: 'reviewId' },
  'operate.action.approve': { kind: 'action', index: 'actions', stateField: 'actions', identityField: 'actionId', argumentField: 'action' },
  'operate.action.execute': { kind: 'action', index: 'actions', stateField: 'actions', identityField: 'actionId', argumentField: 'action' },
  'operate.action.rollback': { kind: 'action', index: 'actions', stateField: 'actions', identityField: 'actionId', argumentField: 'action' },
});
...
function resolveAllowedActionSubject(state, indexes, entry, actorId) {
  if (!entry || typeof entry.subjectId !== 'string') fail('RESULT_CONTRACT_INVALID', 'Allowed actions require one explicit subject identity.');
  assertProtocolArtifact('operate-allowed-action', entry.action, { protocolVersion: PROTOCOL_VERSION });
  const binding = ALLOWED_ACTION_SUBJECTS[entry.action.tool];
  if (!binding) fail('CAPABILITY_DENIED', 'Allowed actions require an existing operation-specific canonical subject.', { tool: entry.action.tool });
  ...
  if (argumentId !== entry.subjectId) fail('E_OPERATE_BINDING_MISMATCH', 'Allowed action arguments do not bind the exact operation-specific subject.', { tool: entry.action.tool, subjectId: entry.subjectId });
  if (actorArgumentIds(entry.action.arguments).some((value) => value !== actorId)) {
    fail('CAPABILITY_DENIED', 'Allowed action actor arguments do not bind the current experience actor.', { tool: entry.action.tool, subjectId: entry.subjectId });
  }
  const record = indexes[binding.index].get(argumentId);
  if (!record) fail('E_OPERATE_BINDING_MISMATCH', 'Allowed action subject is foreign or absent from the selected scope/domain/version.', { tool: entry.action.tool, subjectId: entry.subjectId });
  ...
  if (candidates.length !== 1) fail('E_OPERATE_BINDING_MISMATCH', 'Allowed action subject identity is ambiguous across runtime state.', { tool: entry.action.tool, subjectId: entry.subjectId, candidates: candidates.length });
  if (binding.kind === 'action' && (record.revision !== argument.revision || record.actionHash !== argument.actionHash)) {
    fail('ACTION_REVISION_MISMATCH', 'Allowed action arguments contain a stale or foreign Action tuple.', { actionId: argumentId });
  }
  return {
    kind: binding.kind,
    id: argumentId,
    revision: binding.kind === 'action' ? record.revision : null,
    hash: binding.kind === 'action' ? record.actionHash : binding.kind === 'artifact' ? record.rawHash : sha256Jcs(record),
    record,
  };
}

function subjectArtifactIds(indexes, subject) {
  if (subject.kind === 'cycle') return indexes.inputBindings.get(subject.id)?.sourceArtifactIds ?? [];
  if (subject.kind === 'assignment') return subject.record.inputArtifactIds;
  if (subject.kind === 'artifact') return [subject.id];
  if (subject.kind === 'review') return indexes.inputBindings.get(subject.record.cycleId)?.sourceArtifactIds ?? [];
...
function boundAllowedActions(entries, state, indexes, screen, actorId) {
  ...
  return entries.map((entry) => {
    ...
    if (seen.has(key)) fail('RESULT_CONTRACT_INVALID', 'Allowed actions require one unique operation-specific subject tuple.');
    seen.add(key);
    const action = clone(entry.action);
    action.label = screen.text(action.label, 'Restricted action', subjectArtifactIds(indexes, subject));
...
export function buildOperateExperienceViewV2(state, {
  scope,
  actor,
  deliveryRoutes = [],
  allowedActions = [],
  ...
  if (!actor || typeof actor.actorId !== 'string' || actor.actorId.length === 0 || !ACCESS_ORDER.includes(actor.accessLevel)) fail('RESULT_CONTRACT_INVALID', 'Experience projection requires an actor and access level.');
  ...
  for (const metric of replayValidation.canonicalMetrics) {
    ...
    if (prior && sha256Jcs(prior) !== sha256Jcs(metric)) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Current Metric conflicts with its exact verification-plan snapshot/state provenance.', {
        metricId: metric.metricId,
      });
    }
  ...
  const baseCycles = [...indexes.cycles.values()].map((cycle) => {
    const sourceArtifactIds = indexes.inputBindings.get(cycle.cycleId)?.sourceArtifactIds ?? [];
    return { cycleId: cycle.cycleId, state: cycle.state, health: cycle.health, focus: cycle.focus.map((value) => screen.text(value, 'Restricted cycle focus', sourceArtifactIds)), createdAt: cycle.createdAt, updatedAt: cycle.updatedAt, stages: cycleStages(cycle) };
  }).sort(compareLatest);
  const actionRecords = [...indexes.actions.values()].sort((a, b) => a.actionId.localeCompare(b.actionId));
  if (deliveryRoutes.length !== actionRecords.length
    || deliveryRoutes.some((route) => !actionRecords.some(({ actionId }) => actionId === route?.action?.actionId))) {
    fail('RESULT_CONTRACT_INVALID', 'Delivery routes must describe exactly the current scope-local Action set.');
  }
  const actions = actionRecords.map((action) => {
    ...
    const executions = [...indexes.executionResults.values()].filter((entry) => entry.action.actionId === action.actionId).map((entry) => resultProjection(entry, screen, 'resultId'));
    const rollbacks = [...indexes.rollbackResults.values()].filter((entry) => entry.action.actionId === action.actionId).map((entry) => resultProjection(entry, screen, 'rollbackResultId'));
    return {
      actionId: action.actionId, revision: action.revision, actionHash: action.actionHash,
      title: screen.text(action.title, 'Restricted Action', source), state: action.state,
      ownerActorId: screen.identity(action.ownerActorId, source), expectedResult: screen.text(action.expectedResult, 'Action details are unavailable at this access level.', source),
      verificationPlanId: action.verificationPlanId, deliveryRoute: screen.route(route, source),
      dependencyActionIds: [...action.dependsOnActionIds].sort(), executions, rollbacks,
      deepLink: deepLink('action', action.actionId),
    };
  });
  const outcomes = buildOutcomes(indexes, screen);
  const learnings = [...indexes.learnings.values()].map((entry) => ({
    learningId: entry.learningId, outcomeId: entry.outcomeId,
    statement: screen.text(entry.statement, 'Learning details are unavailable at this access level.', [entry.sourceArtifactId]),
    decisionIds: [...entry.decisionIds].sort(), evidenceRefIds: entry.evidenceRefIds.filter((id) => {
      const evidence = indexes.evidenceRefs.get(id);
      return evidence ? screen.inspect([evidence.evidenceArtifactId]).visible : false;
    }).sort(), createdAt: entry.createdAt,
  ...
  const cycleDetails = buildCycleDetails(indexes, baseCycles, screen, replayCheckpoint, generatedAt, scope, state, actor.accessLevel);
  const projectedAllowedActions = boundAllowedActions(allowedActions, state, indexes, screen, actor.actorId);
  ...
  const nextBySubject = new Map(projectedAllowedActions.map(({ subjectId, action }) => [subjectId, { label: action.label, tool: action.tool }]));
  const cycles = cycleDetails.map((cycle) => ({ ...cycle, assignments: cycle.assignments.map((assignment) => ({ ...assignment, next: nextBySubject.get(assignment.assignmentId) ?? null })) }));
  ...
  const replay = buildReplay(state, replayCheckpoint, sourceStateHash, replayValidation.stateParityVerified, omissions);
  const base = {
    kind: 'operate-experience-view', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    viewId: stableId('xview', { scope, actorId: actor.actorId, eventHead: state.eventHead, sourceStateHash }),
    ...scope, actorId: actor.actorId, accessLevel: actor.accessLevel, generatedAt, eventHead: clone(state.eventHead), sourceStateHash, status,
    attention: buildAttention(indexes, inbox, actions, outcomes, actor.actorId, generatedAt), domainMetrics, cycles, inbox, actions,
    evidence: safe.evidence, claims, rationale: safeReasoning, outcomes, learnings, history, replay,
    allowedActions: projectedAllowedActions, omissions,
    export: { formats: ['html', 'json'], accessSafe: true, redactionCount: omissions.reduce((sum, entry) => sum + entry.count, 0) },
  };
...
function buildPreviewTransition(state, indexes, current, allowedAction, actorId) {
  ...
  if (allowedAction.tool === 'operate.review.submit') {
    ...
    const targets = [previewTarget('review', review.reviewId, null, sha256Jcs(review), disposition)];
    for (const work of args.workDispositions ?? []) {
      const kind = work.entityType === 'operating-decision' ? 'decision'
        : work.entityType === 'operating-action' ? 'action' : 'finding';
      const index = kind === 'decision' ? indexes.decisions
        : kind === 'action' ? indexes.actions : indexes.findings;
      const record = index.get(work.entityId);
      if (!record) fail('E_OPERATE_BINDING_MISMATCH', 'Review preview target is absent from current canonical state.');
  ...
  if (allowedAction.tool === 'operate.action.approve') {
    if (args.rollback !== undefined) {
      ...
      if (!action || action.revision !== current.revision || action.actionHash !== current.hash) {
        fail('E_OPERATE_BINDING_MISMATCH', 'Rollback approval preview requires one exact current Action.');
      }
      const { plan, template } = exactRollbackApprovalContext(
        indexes,
        action,
      ...
      return {
        ...
        targets: [
          ...
          previewTarget('rollback-plan', plan.rollbackPlanId, null, plan.planHash, 'approval-recorded'),
        ],
        reversible: false,
        ...
        threshold: {
          ...
          recorded,
          remaining,
          parties: template.requirement.parties.map((party) => inboxUnrecordedParty(party, actorId)),
        },
      };
    }
    const requirements = [...indexes.approvalRequirements.values()].filter((entry) =>
    ...
    if (requirements.length !== 1) {
      fail('E_OPERATE_BINDING_MISMATCH', 'Approval preview requires one exact current requirement and party binding.');
    }
    const requirement = requirements[0];
    const recorded = [...indexes.approvalRecords.values()].filter((entry) =>
      entry.requirementId === requirement.requirementId
      && entry.decision === 'approved'
      && entry.consumedByOperationId === null).length;
    ...
    return {
      ...
      targets: [
        previewTarget('action', current.id, current.revision, current.hash, args.decision),
        previewTarget(requirement.target.kind, requirement.target.id, requirement.target.revision, null, 'unchanged'),
      ],
      reversible: false,
      nextState: remaining === 0 ? 'threshold-satisfied' : 'recorded-parties-remain',
      threshold: {
        ...
        remaining,
        parties: requirement.parties.map((party) => inboxParty(party, indexes, actorId, requirement.requirementId)),
      },
    };
  }
  if (allowedAction.tool === 'operate.assignment.submit') {
    return {
      ...
      reversible: false, nextState: 'submitted', threshold: null,
    };
  }
  if (allowedAction.tool === 'operate.action.rollback') {
    return {
      ...
      reversible: false, nextState: 'rollback-dispatching', threshold: null,
    };
  }
  return {
    kind: 'execution',
    targets: [previewTarget(current.kind, current.id, current.revision, current.hash, 'dispatching')],
    reversible: current.kind === 'action'
      && (state.rollbackPlans?.some((entry) => entry.action?.actionId === current.id) ?? false),
    nextState: 'dispatching', threshold: null,
  };
}

/** Build the exact, expiring confirmation preview for one runtime-provided action. */
export function createOperateExperiencePreviewV1(state, {
  scope,
  view,
  actionDigest,
  authority,
  ...
  assertCanonicalView(view);
  if (!sameScope(view, scope) || view.sourceStateHash !== sha256Jcs(state)
    || view.eventHead.sequence !== state.eventHead.sequence || view.eventHead.hash !== state.eventHead.hash
    || view.viewHash !== sha256Jcs(without(view, 'viewHash'))) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Experience preview requires the exact current actor/scope/Event-head view.');
  }
  ...
  if (typeof actionDigest !== 'string' || !/^sha256:[a-f0-9]{64}$/u.test(actionDigest)) {
    fail('RESULT_CONTRACT_INVALID', 'Experience preview requires one exact owner-issued action digest.');
  }
  validInstant(issuedAt, 'issuedAt'); validInstant(expiresAt, 'expiresAt');
  if (Date.parse(expiresAt) <= Date.parse(issuedAt)) fail('RESULT_CONTRACT_INVALID', 'Experience preview expiry must follow issuance.');
  const indexes = buildScopeIdentityIndexes(state, scope);
  const issued = view.allowedActions.filter((entry) => sha256Jcs(entry.action) === actionDigest);
  if (issued.length !== 1) {
    fail('CAPABILITY_DENIED', 'Experience preview action is not in the exact current actor-bound allowed-action set.');
  }
  const allowedAction = issued[0].action;
  const current = resolveAllowedActionSubject(state, indexes, issued[0], view.actorId);
  const subject = { kind: current.kind, id: current.id, revision: current.revision, hash: current.hash };
  if ((allowedAction.effect === 'read-only') !== (authority === 'read-only')) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Preview authority does not match the allowed action effect.');
  }
  const effectCopy = {
  ...
  const projectedAction = subject.kind === 'action' ? view.actions.find(({ actionId }) => actionId === subject.id) ?? null : null;
  ...
  if (consequence !== undefined && consequence !== canonicalConsequence) {
    fail('E_OPERATE_BINDING_MISMATCH', 'Preview consequence must equal the canonical runtime-derived consequence.');
  }
  ...
  const base = {
    kind: 'operate-experience-preview', schemaVersion: '1.0.0', protocolVersion: PROTOCOL_VERSION,
    previewId: stableId('xprv', { scope, actorId: view.actorId, actionDigest, eventHead: state.eventHead }),
    ...scope, actorId: view.actorId, subject: clone(subject), eventHead: clone(state.eventHead),
    sourceViewHash: view.viewHash, actionDigest, allowedAction: clone(allowedAction),
    authority, consequence: canonicalConsequence, reasonCodes: sortedReasons,
    transition, issuedAt, expiresAt,
  };
...
export function applyOperateExperienceLivePatchV2(previous, patch, next) {
  ...
  if (patch.patchHash !== sha256Jcs(without(patch, 'patchHash')) || patch.fromViewHash !== previous.viewHash || patch.toViewHash !== next.viewHash
    || patch.fromEventHead.sequence !== previous.eventHead.sequence || patch.fromEventHead.hash !== previous.eventHead.hash
    || patch.toEventHead.sequence !== next.eventHead.sequence || patch.toEventHead.hash !== next.eventHead.hash) fail('E_OPERATE_BINDING_MISMATCH', 'Live patch hash or exact Event-head/view binding is invalid.');
  const expected = buildOperateExperienceLivePatchV2(previous, next, { createdAt: patch.createdAt });
  if (sha256Jcs(expected) !== sha256Jcs(patch)) fail('E_OPERATE_BINDING_MISMATCH', 'Live patch operations do not equal the deterministic projection delta.');
  return next;
}
