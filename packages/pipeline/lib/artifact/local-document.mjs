...
const MIME = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.svg': 'image/svg+xml', '.webp': 'image/webp', '.avif': 'image/avif', '.gif': 'image/gif', '.woff': 'font/woff', '.woff2': 'font/woff2', '.ttf': 'font/ttf', '.ico': 'image/x-icon' };
const children = (node) => [...(node.childNodes ?? []), ...(node.content?.childNodes ?? [])];
const attr = (node, name) => node.attrs?.find((item) => item.name === name);
const text = (node) => children(node).map((item) => item.value ?? '').join('');
function setText(node, value) { node.childNodes = [{ nodeName: '#text', value, parentNode: node }]; }
function element(name, value) { const node = parseFragment(`<${name}></${name}>`).childNodes[0]; setText(node, value); return node; }
...
export function resolveLocalDocumentFile(root, path, from = root) {
  ...
  const inside = (file) => { const r = relative(root, file); return r !== '..' && !r.startsWith(`..${process.platform === 'win32' ? '\\' : '/'}`) && !isAbsolute(r); };
  if (!inside(candidate)) throw new Error(`Asset escapes design root: ${path}`);
  const actual = realpathSync(candidate);
  if (!inside(actual) || !statSync(actual).isFile()) throw new Error(`Asset is not a contained regular file: ${path}`);
  return actual;
}

/** No network, package resolution, transpiler, native binary, or code execution. */
export function bundleLocalDocument({ root: inputRoot, source, sharedStyles = [], screenId, maxBytes = 10 * 1024 * 1024, readSource, passive = false }) {
  ...
  function read(path, from = root) {
    ...
    if (!files.has(file)) {
      const value = checked?.value ?? readFileSync(file); bytes += value.length;
      if (bytes > maxBytes || files.size >= 1000) throw new Error('Design source exceeds the local asset budget.');
  ...
  function asset(ref, from) {
    ...
    if (passive && extname(file).toLowerCase() === '.svg') {
      ...
      mediaStack.add(file);
      try { payload = Buffer.from(passiveSvg(value.toString('utf8'), dirname(file))); }
      finally { mediaStack.delete(file); }
    }
  ...
  function css(value, from, stack = new Set()) {
    ...
    let result = replaceCss(value, /@import\s+(?:url\(\s*)?["']([^"']+)["']\s*\)?\s*([^;]*);/giu, (_all, ref, media) => {
      ...
      const expanded = css(next.value.toString('utf8'), dirname(next.file), new Set([...stack, next.file]));
  ...
  function js(value, label) {
    try { new Script(value, { filename: label }); } catch (error) { throw new Error(`Use compiled, self-contained browser JavaScript in ${label}: ${error.message}`); }
  ...
  while (queue.length) {
    ...
    if (node.tagName === 'link' && attr(node, 'rel')?.value.toLowerCase().split(/\s+/u).includes('stylesheet')) {
      ...
      node.tagName = 'style'; node.nodeName = 'style'; node.attrs = [];
      setText(node, css(linked.value.toString('utf8'), dirname(linked.file), new Set([linked.file])));
    }
    if (node.tagName === 'script') {
      ...
      if (!type || /(?:javascript|ecmascript)/u.test(type)) {
        ...
        node.attrs = (node.attrs ?? []).filter((item) => !['src', 'async', 'defer'].includes(item.name));
      }
    }
    ...
    for (const item of node.attrs ?? []) {
      ...
      if (['src', 'poster'].includes(item.name)) item.value = asset(item.value, dirname(input.file));
      if (item.name === 'srcset') throw new Error('Use a local src and responsive CSS for portable design images.');
      if (['action', 'formaction', 'target', 'formtarget'].includes(item.name) && item.value.trim()) throw new Error('Design forms must use local submit handlers without navigation targets.');
      if (item.name === 'href' && !item.value.startsWith('#')) {
        if (node.tagName === 'link' || node.namespaceURI === 'http://www.w3.org/2000/svg') item.value = asset(item.value, dirname(input.file));
  ...
  const prepend = [];
  for (const path of [...sharedStyles, ...(source.styles ?? [])]) { const linked = read(path); prepend.push(element('style', css(linked.value.toString('utf8'), dirname(linked.file), new Set([linked.file])))); }
  head.childNodes = [...prepend, ...head.childNodes];
  for (const node of prepend) node.parentNode = head;
  for (const path of source.scripts ?? []) { const linked = read(path); if (passive) continue; const node = element('script', js(linked.value.toString('utf8'), path)); node.parentNode = body; body.childNodes.push(node); }
  ...
  for (const node of deferredScripts) { node.parentNode = body; body.childNodes.push(node); }
  ...
  return {
    ...
    sourceDigests: Object.fromEntries([...files].map(([path, value]) => [relative(root, path), createHash('sha256').update(value).digest('hex')])),
    inputBytes: bytes,
