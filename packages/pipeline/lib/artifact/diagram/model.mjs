...
const RENDERER_COORDINATE_KEYS = new Set([
  'x', 'y', 'x1', 'x2', 'y1', 'y2', 'width', 'height', 'coordinates', 'position', 'points', 'viewBox',
]);
...
function collectCoordinatePath(value, path = '$', insideSemanticPosition = false) {
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      const found = collectCoordinatePath(value[index], `${path}[${index}]`, insideSemanticPosition);
...
function schemaCheck(document) {
  try {
    ...
    diagramFail(DIAGRAM_ERROR_CODES.SCHEMA_INVALID, 'Diagram document does not satisfy Protocol 1.6.', {
      cause: error.message,
...
function assertUniqueIds(document) {
  ...
  for (const collection of Object.values(COLLECTIONS).filter((name) => name !== 'emphasis')) {
    for (const [index, value] of document[collection].entries()) {
      ...
      if (previous) {
        diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, `Duplicate semantic item id: ${value.id}`, {
          itemId: value.id,
          first: previous,
  ...
  for (const collection of ['groups', 'lanes', 'sets']) {
    for (const [index, value] of document[collection].entries()) {
      for (const member of value.members) if (!ids.has(member)) missing.push(`${collection}[${index}].members=${member}`);
    }
  }
  ...
  if (missing.length > 0) {
    diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, 'Diagram references unknown semantic item IDs.', { missing });
  }
}

export function assertDiagramDocument(document) {
  const primitiveItems = Object.values(COLLECTIONS).reduce(
    (total, collection) => total + (Array.isArray(document?.[collection]) ? document[collection].length : 0),
    0,
  );
  if (primitiveItems > MAX_DIAGRAM_PRIMITIVE_ITEMS) {
    diagramFail(DIAGRAM_ERROR_CODES.RESOURCE_BUDGET_EXCEEDED, 'Diagram exceeds the semantic primitive resource budget.', {
      primitiveItems,
  ...
  if (coordinatePath) {
    diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, 'Renderer coordinates are forbidden in canonical diagram documents.', {
      path: coordinatePath,
  ...
  if (missing.length > 0 || forbidden.length > 0) {
    diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, `Diagram violates the ${grammar.grammarId} primitive contract.`, {
  ...
  if (!grammar.directions.includes(document.layout.direction)) {
    diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, `Direction ${document.layout.direction} is not valid for ${grammar.grammarId}.`, {
  ...
  if (semanticPositions.length > 0 && !SEMANTIC_POSITION_GRAMMARS.has(grammar.grammarId)) {
    diagramFail(DIAGRAM_ERROR_CODES.GRAMMAR_RULE_INVALID, `semanticPosition is not meaningful for ${grammar.grammarId}.`, {
