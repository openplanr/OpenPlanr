...
import { ARTIFACT_ERROR_CODES, PipelineError } from '../protocol/errors.mjs';
...
function error(code, message, fix = '', details = undefined) { return new PipelineError(code, message, fix, details); }
...
function assertAuthor(value, label) {
  if (!exactKeys(value, ['name'], ['id']) || !bounded(value.name, 1, 256) || (value.id !== undefined && !bounded(value.id, 1, 128))) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, `${label} is invalid.`);
  }
}
function assertReply(value) {
  if (!exactKeys(value, ['id', 'author', 'comment', 'createdAt'])
    || !bounded(value.id, 1, 128) || !bounded(value.comment, 1, 65_536) || !validIso(value.createdAt)) {
...
function assertPin(value) {
  const required = ['id', 'author', 'artifactId', 'region', 'viewport', 'intent', 'status', 'comment', 'replies', 'createdAt', 'updatedAt'];
  if (!exactKeys(value, required, ['variant', 'anchor'])
    || !bounded(value.id, 1, 128) || !bounded(value.artifactId, 1, 128)
    ...
    || !Array.isArray(value.replies) || value.replies.length > 10_000
    || !exactKeys(value.region, ['x', 'y', 'w', 'h'])
    || ['x', 'y', 'w', 'h'].some((key) => typeof value.region[key] !== 'number' || value.region[key] < 0 || value.region[key] > 1)
    || value.region.x + value.region.w > 1 || value.region.y + value.region.h > 1
    || !exactKeys(value.viewport, ['width', 'height'])
    || !Number.isInteger(value.viewport.width) || value.viewport.width < 1 || value.viewport.width > 16_384
  ...
  if (value.anchor !== undefined && (!exactKeys(value.anchor, ['planrId'], ['screen'])
...
function assertReview(value, reviewOf) {
  if (!exactKeys(value, ['schemaVersion', 'reviewId', 'reviewOf', 'decision', 'overall', 'pins'], ['createdAt', 'updatedAt'])
    || value.schemaVersion !== '1.0.0' || !bounded(value.reviewId, 1, 128) || value.reviewOf !== reviewOf
    || !['pending', 'approved', 'changes_requested'].includes(value.decision)
    || !bounded(value.overall, 0, 65_536) || !Array.isArray(value.pins) || value.pins.length > 10_000
    || (value.createdAt !== undefined && !validIso(value.createdAt))
    || (value.updatedAt !== undefined && !validIso(value.updatedAt))) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review snapshot is invalid or belongs to another artifact.');
  }
  for (const pin of value.pins) assertPin(pin);
}
function assertEventPayload(kind, payload, reviewOf) {
  if (!isRecord(payload)) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review event payload is invalid.');
  if (kind === 'pin') return assertPin(payload);
  if (kind === 'reply') {
    if (!exactKeys(payload, ['pinId', 'reply']) || !bounded(payload.pinId, 1, 128)) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review reply event is invalid.');
    return assertReply(payload.reply);
  }
  if (kind === 'pin_status') {
    if (!exactKeys(payload, ['pinId', 'status']) || !bounded(payload.pinId, 1, 128) || !['open', 'addressed', 'resolved'].includes(payload.status)) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review status event is invalid.');
    return;
  }
  if (kind === 'recommendation') {
    if (!exactKeys(payload, ['author', 'decision', 'overall'])
      || !['pending', 'approved', 'changes_requested'].includes(payload.decision)
      || !bounded(payload.overall, 0, 65_536)) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review recommendation is invalid.');
    return assertAuthor(payload.author, 'Live review recommendation author');
  }
  if (kind === 'owner_decision') {
    if (!exactKeys(payload, ['decision'], ['overall'])
      || !['approved', 'changes_requested'].includes(payload.decision)
      || (payload.overall !== undefined && !bounded(payload.overall, 0, 65_536))) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review owner decision is invalid.');
    return;
  }
  if (kind === 'review_snapshot') {
    if (!exactKeys(payload, ['review'])) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Live review snapshot event is invalid.');
...
function assertEnvelope(value) {
  if (!exactKeys(value, ['schemaVersion', 'artifacts', 'viewer'], ['review'])
    || value.schemaVersion !== '1.0.0' || !Array.isArray(value.artifacts) || value.artifacts.length < 1 || value.artifacts.length > 256
...
async function digestEnvelope(value, crypto = globalThis.crypto) {
  ...
  for (const artifact of value.artifacts) {
    if (!exactKeys(artifact, ['id', 'kind', 'title', 'sha256', 'html', 'viewport', 'colorScheme'])
      || !bounded(artifact.id, 1, 128) || !ARTIFACT_ID_RE.test(artifact.id) || ids.has(artifact.id) || artifact.kind !== 'html'
      || !bounded(artifact.title, 1, 512) || !bounded(artifact.html, 1, Number.MAX_SAFE_INTEGER)
      ...
      || !Number.isInteger(artifact.viewport.width) || artifact.viewport.width < 1 || artifact.viewport.width > 16_384
      || !Number.isInteger(artifact.viewport.height) || artifact.viewport.height < 1 || artifact.viewport.height > 16_384
      || !['light', 'dark'].includes(artifact.colorScheme)) throw error(ARTIFACT_ERROR_CODES.ENVELOPE_INVALID, 'Live review artifact envelope is invalid.');
    ids.add(artifact.id);
    ...
    if (actual !== artifact.sha256) throw error(ARTIFACT_ERROR_CODES.DIGEST_MISMATCH, 'Live review artifact HTML digest is invalid.');
  }
  if (!exactKeys(value.viewer, ['mode', 'activeArtifactId'], ['presentation'])
    || !['single', 'variants'].includes(value.viewer.mode)
    || !bounded(value.viewer.activeArtifactId, 1, 128) || !ids.has(value.viewer.activeArtifactId)
    || (value.viewer.presentation !== undefined && !['document', 'canvas'].includes(value.viewer.presentation))) {
...
function normalizeServiceUrl(source, { roomLink = false } = {}) {
  let url;
  try { url = new URL(source); } catch { throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review service URL is malformed.'); }
 ...
 */
export async function prepareLiveReviewRoom(envelope, {
  ...
  const ownerSigner = suppliedOwnerSigner ?? await createLiveRoomSigner({ role: 'owner', crypto });
...
export async function importLiveRoomRecoveryBundle(value, { crypto } = {}) {
  if (!exactKeys(value, [
    'schemaVersion', 'kind', 'protocolVersion', 'id', 'roomId', 'reviewOf', 'ttl',
    'ownerKey', 'url', 'ownerUrl', 'manageUrl', 'ownerSigner',
  ])
  ...
  if (review.role !== 'reviewer' || owner.role !== 'owner' || management.role !== 'management'
    || review.origin !== owner.origin || review.origin !== management.origin
    || review.roomId !== value.roomId || owner.roomId !== value.roomId || management.roomId !== value.roomId
    || review.key !== owner.key || review.key !== management.key
    || new Set([review.write, owner.owner, management.manage]).size !== 3) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review recovery bundle capabilities do not describe one room.');
  }
  const ownerSigner = await importLiveRoomSignerSecret(value.ownerSigner, { crypto });
  if (ownerSigner.role !== 'owner'
    || ownerSigner.keyId !== descriptor.ownerKey.keyId
    || (ownerSigner.publicKey ?? ownerSigner.value) !== descriptor.ownerKey.value) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_FORBIDDEN, 'Live review recovery owner key does not match the room.');
  }
  const recovered = {
...
export async function recoverLiveReviewRoom(value, { client, crypto } = {}) {
  ...
  if (room.descriptor?.roomId !== recovered.roomId
    ...
    throw error(ARTIFACT_ERROR_CODES.ROOM_FORBIDDEN, 'Live review recovery bundle does not match the committed room descriptor.');
  }
  const result = { ...room };
...
export function parseLiveRoomLink(source) {
  ...
  if (!room || url.search || keys.some((key) => !['k', 'w', 'o', 'm'].includes(key))
  ...
  if ([write, owner, manage].filter((value) => value !== null).length > 1) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'A live review URL can expose only one scoped capability.');
  }
  return Object.freeze({
    origin: url.origin,
    roomId: room,
...
export function createLiveRoomLinks({ baseUrl = 'https://share.openplanr.dev', roomId, key, writeCapability, ownerCapability, manageCapability }) {
  const base = normalizeServiceUrl(baseUrl);
  validateId(roomId);
  ...
  return Object.freeze({ url: review.toString(), ownerUrl: owner.toString(), manageUrl: manage.toString() });
}
...
export function createLiveRoomEvent({ roomId, reviewOf, kind, payload, eventId = globalThis.crypto?.randomUUID?.(), createdAt = nowIso() } = {}) {
  ...
  if (typeof eventId !== 'string' || !EVENT_ID_RE.test(eventId)) throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Room event ID is invalid.');
...
export function createLiveRoomEventFromReviewChange({
  ...
  if (role === 'owner') {
    if (!equalJson(previousReview.pins, nextReview.pins)
      || !['approved', 'changes_requested'].includes(nextReview.decision)
      || (previousReview.decision === nextReview.decision && previousReview.overall === nextReview.overall)) {
      throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Owner review changes must be one explicit verdict over the current reviewed digest.');
    }
  ...
  if (role !== 'reviewer') throw error(ARTIFACT_ERROR_CODES.ROOM_FORBIDDEN, 'This room authority cannot author review events.');

  if (nextReview.pins.length === previousReview.pins.length + 1
    && previousReview.pins.every((pin) => nextReview.pins.some((candidate) => equalJson(pin, candidate)))) {
    const added = nextReview.pins.find((pin) => !previousReview.pins.some(({ id }) => id === pin.id));
  ...
  if (nextReview.pins.length === previousReview.pins.length) {
    for (const previousPin of previousReview.pins) {
      ...
      if (!nextPin) continue;
      if (nextPin.replies.length === previousPin.replies.length + 1
        && previousPin.replies.every((reply) => nextPin.replies.some((candidate) => equalJson(reply, candidate)))
        && equalJson(withoutUpdatedAt({ ...previousPin, replies: [] }), withoutUpdatedAt({ ...nextPin, replies: [] }))) {
        const reply = nextPin.replies.find((candidate) => !previousPin.replies.some(({ id }) => id === candidate.id));
        if (reply) return createLiveRoomEvent({ ...eventOptions, kind: 'reply', payload: { pinId: nextPin.id, reply } });
      }
      if (nextPin.status !== previousPin.status
        && equalJson(withoutUpdatedAt({ ...previousPin, status: nextPin.status }), withoutUpdatedAt(nextPin))) {
        return createLiveRoomEvent({ ...eventOptions, kind: 'pin_status', payload: { pinId: nextPin.id, status: nextPin.status } });
      }
    }
  }
  if (equalJson(previousReview.pins, nextReview.pins)
  ...
  throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Review change is not one coherent event for this room authority.');
}
...
export function reduceLiveRoomEvents({ roomId, reviewOf, events = [], reviewId = roomId } = {}) {
  validateId(roomId); validateDigest(reviewOf);
  const pins = new Map(); const recommendations = new Map(); const seen = new Set(); let ownerDecision = 'pending'; let ownerOverall = ''; let snapshot = null;
  ...
  const review = Object.freeze(snapshot ?? { schemaVersion: '1.0.0', reviewId, reviewOf, decision: ownerDecision, overall: ownerOverall, createdAt, updatedAt, pins: [...pins.values()] });
...
export async function reduceSignedLiveRoomEvents({ descriptor, entries = [], reviewId, key, crypto } = {}) {
...
export async function reduceResilientSignedLiveRoomEvents({ descriptor, records = [], reviewId, key, crypto } = {}) {
  ...
  for (const record of verified.records) {
    try {
      const semantic = await verifySignedLiveRoomEvent({ descriptor: verified.descriptor, record, key, crypto });
...
function responseError(status, body) {
  ...
  if (status === 409 && body?.error === 'legacy_read_only') {
    return error(ARTIFACT_ERROR_CODES.ROOM_LEGACY_READ_ONLY, 'Legacy unsigned live review rooms are readable but cannot be mutated.');
  }
  if (status === 409 && ['room_conflict', 'room_stale', 'room_clock_skew'].includes(body?.error)) {
    return error(ARTIFACT_ERROR_CODES.ROOM_EVENT_REPLAY, 'Live review room generation is stale or the event identity diverged.');
  }
...
function assertCiphertext(value, label = 'Live review ciphertext') {
  if (!exactKeys(value, ['version', 'iv', 'ciphertext']) || value.version !== ARTIFACT_ROOM_VERSION
    || !bounded(value.iv, 16, 16) || !bounded(value.ciphertext, 22, 7_000_000)) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, `${label} is invalid.`);
  }
  return value;
}
...
function assertCreateResponse(value, { reviewOf, ownerSigner }) {
  if (!exactKeys(value, ['id', 'expiresAt', 'descriptor', 'generation', 'head'])
    || !validIso(value.expiresAt) || value.generation !== 0 || value.head !== ARTIFACT_ROOM_GENESIS_HASH) {
  ...
  if (descriptor.roomId !== value.id || descriptor.reviewOf !== reviewOf
    || descriptor.ownerKey.keyId !== ownerSigner.keyId
    || descriptor.ownerKey.value !== (ownerSigner.publicKey ?? ownerSigner.value)) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review service returned a descriptor for another room or owner.');
  }
  return { ...value, descriptor };
}

function assertV2RoomResponse(value, roomId) {
  if (!exactKeys(value, ['schemaVersion', 'operation', 'descriptor', 'iv', 'ciphertext', 'expiresAt', 'commentsEnabled', 'generation', 'head', 'events'])
    || value.schemaVersion !== '2.0.0' || value.operation !== 'read'
    || !validIso(value.expiresAt) || typeof value.commentsEnabled !== 'boolean'
    || !Number.isSafeInteger(value.generation) || value.generation < 0
    || !HASH_RE.test(value.head) || !Array.isArray(value.events) || value.events.length > 10_000
    || value.events.length !== value.generation) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review service returned an invalid signed room.');
  }
  const descriptor = normalizeLiveRoomDescriptor(value.descriptor);
  if (descriptor.roomId !== roomId) throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review service returned another room.');
  assertCiphertext({ version: ARTIFACT_ROOM_VERSION, iv: value.iv, ciphertext: value.ciphertext }, 'Live review room ciphertext');
  return Object.freeze({ ...value, descriptor });
}

function isLegacyRoomResponse(value) {
  return isRecord(value) && !Object.hasOwn(value, 'descriptor')
...
export function createLiveRoomClient({ baseUrl = 'https://share.openplanr.dev', fetchImpl = globalThis.fetch } = {}) {
  ...
  const json = async (path, options) => {
    let response;
    try { response = await request(base, fetchImpl, path, options); } catch { throw error(ARTIFACT_ERROR_CODES.SHARE_NETWORK, 'Live review service is unavailable.'); }
    let value;
    try {
      const body = await response.text();
      if (new TextEncoder().encode(body).byteLength > 32 * 1024 * 1024) throw new Error('response too large');
      value = JSON.parse(body);
    } catch {
      if (!response.ok) throw responseError(response.status);
      throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review service returned malformed JSON.');
    }
    if (!response.ok) throw responseError(response.status, value);
    return value;
  };
  return Object.freeze({
    async create({ prepared: suppliedPreparation, envelope, ttl = '7d', ownerSigner, crypto } = {}) {
      const prepared = suppliedPreparation ?? await prepareLiveReviewRoom(envelope, {
        baseUrl: base.origin,
        ttl,
        ownerSigner,
        crypto,
      ...
      if (state.baseOrigin !== base.origin) {
        throw error(ARTIFACT_ERROR_CODES.ROOM_INVALID, 'Live review preparation belongs to another service origin.');
      }
      ...
      if (!response.ok) {
        const definitelyRejected = !hadPriorDispatch
          && [400, 403, 413, 429].includes(response.status);
      ...
      let created;
      try {
        created = assertCreateResponse(createdValue, { reviewOf: prepared.reviewOf, ownerSigner: state.ownerSigner });
    ...
    async append(roomId, capability, { expectedGeneration, record } = {}) {
      validateId(roomId); validateKey(capability, 'Live review event capability');
      if (!Number.isSafeInteger(expectedGeneration) || expectedGeneration < 0
        || !isRecord(record) || record.schemaVersion !== '2.0.0'
        || record.roomId !== roomId || record.sequence !== expectedGeneration + 1) {
        throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_INVALID, 'Signed live review append request is invalid.');
      }
      return json(`/api/v1/rooms/${encodeURIComponent(roomId)}/events`, {
        ...
        body: JSON.stringify({ schemaVersion: '2.0.0', operation: 'append', expectedGeneration, record }),
      });
    },
...
export async function appendLiveRoomEvent(link, event, { client = createLiveRoomClient({ baseUrl: parseLiveRoomLink(link).origin }), signer: suppliedSigner, crypto } = {}) {
  const parsed = parseLiveRoomLink(link);
  const roomValue = await client.read(parsed.roomId);
  if (isLegacyRoomResponse(roomValue)) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_LEGACY_READ_ONLY, 'Legacy unsigned live review rooms are readable but cannot be mutated.');
  }
  const room = assertV2RoomResponse(roomValue, parsed.roomId);
  const normalizedEvent = createLiveRoomEvent(event);
  if (normalizedEvent.roomId !== parsed.roomId || normalizedEvent.reviewOf !== room.descriptor.reviewOf) {
    throw error(ARTIFACT_ERROR_CODES.DIGEST_MISMATCH, 'Live review event belongs to another room or artifact.');
  }
  const ownerKind = normalizedEvent.kind === 'owner_decision' || normalizedEvent.kind === 'review_snapshot';
  const capability = ownerKind ? parsed.owner : parsed.write;
  if (!capability) throw error(ARTIFACT_ERROR_CODES.ROOM_FORBIDDEN, ownerKind
  ...
  const signer = suppliedSigner ?? (ownerKind ? null : await createLiveRoomSigner({ role: 'reviewer', crypto }));
  if (!signer) throw error(ARTIFACT_ERROR_CODES.ROOM_FORBIDDEN, 'Owner events require the client-held room owner signer.');
...
export async function hydrateLiveReviewRoom(link, { client = createLiveRoomClient({ baseUrl: parseLiveRoomLink(link).origin }), crypto } = {}) {
  ...
  if (declaredReviewOf !== reviewOf) throw error(ARTIFACT_ERROR_CODES.DIGEST_MISMATCH, 'Live review room artifact digest is invalid.');
  if (legacy) {
    const events = [];
  ...
  if (room.descriptor.reviewOf !== reviewOf) throw error(ARTIFACT_ERROR_CODES.DIGEST_MISMATCH, 'Live review room descriptor digest is invalid.');
  ...
  if (projection.integrity.generation !== room.generation || projection.integrity.head !== room.head) {
    throw error(ARTIFACT_ERROR_CODES.ROOM_EVENT_REPLAY, 'Live review room head does not match its authenticated event chain.');
  }
  ...
  return Object.freeze({
    ...
    protocolVersion: ARTIFACT_ROOM_PROTOCOL_VERSION,
    descriptor: room.descriptor,
    mutation: Object.freeze({ enabled, reason: enabled ? null : parsed.role === 'management' ? 'management-only' : room.commentsEnabled ? 'read-only-link' : 'comments-paused' }),
    ...projection,
  });
}
