...
export const EVALUATION_IDENTITY_BINDINGS = Object.freeze({
  'evaluation-scenario': Object.freeze({
    prefix: 'esc',
    width: 64,
    idField: 'scenarioId',
    digestField: 'scenarioDigest',
  }),
  'evaluation-corpus': Object.freeze({
    prefix: 'ecp',
    width: 64,
    idField: 'corpusId',
    digestField: 'corpusDigest',
  }),
  'evaluation-fixture': Object.freeze({
    prefix: 'efx',
    width: 64,
    idField: 'fixtureId',
    digestField: 'fixtureDigest',
  }),
  'evaluation-host-profile': Object.freeze({
    prefix: 'ehp',
    width: 32,
    idField: 'hostProfileId',
    digestField: 'profileDigest',
  }),
  'evaluation-host-profile-registry': Object.freeze({
    prefix: 'ehpr',
    width: 32,
    idField: 'registryId',
    digestField: 'registryDigest',
  }),
  'evaluation-grader-registration': Object.freeze({
    prefix: 'egr',
    width: 32,
    idField: 'graderId',
    digestField: 'registrationDigest',
  }),
  'evaluation-grader-registry': Object.freeze({
    prefix: 'egrr',
    width: 32,
    idField: 'registryId',
    digestField: 'registryDigest',
  }),
  'evaluation-budget': Object.freeze({
    prefix: 'ebg',
    width: 32,
    idField: 'budgetId',
    digestField: 'budgetDigest',
  }),
  'evaluation-gate-policy': Object.freeze({
    prefix: 'egp',
    width: 32,
    idField: 'policyId',
    digestField: 'gatePolicyDigest',
  }),
  'evaluation-observation': Object.freeze({
    prefix: 'eob',
    width: 32,
    idField: 'observationId',
    digestField: 'observationDigest',
  }),
  'evaluation-run-result': Object.freeze({
    prefix: 'ers',
    width: 32,
    idField: 'runResultId',
    digestField: 'runResultDigest',
  }),
  'evaluation-aggregate-report': Object.freeze({
    prefix: 'ear',
    width: 32,
    idField: 'reportId',
    digestField: 'reportDigest',
  }),
  'evaluation-waiver': Object.freeze({
    prefix: 'evw',
    width: 32,
    idField: 'waiverId',
    digestField: 'waiverDigest',
  }),
  'skill-certification-receipt': Object.freeze({
    prefix: 'scr',
    width: 32,
    idField: 'receiptId',
...
export const EVALUATION_EVIDENCE_INVALIDATION_REASONS = Object.freeze([
...
function plainObject(value, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    fail(
      'E_EVALUATION_IDENTITY_INVALID',
      `${label} must be one JSON object.`,
      'Pass the parsed contract record itself, not a wrapper or array.',
    );
  }
  return value;
}
...
function bindingFor(kind) {
  if (!Object.prototype.hasOwnProperty.call(EVALUATION_IDENTITY_BINDINGS, kind)) {
    fail(
      'E_EVALUATION_IDENTITY_INVALID',
      `Contract kind "${String(kind)}" has no evaluation identity binding.`,
      `Use one of: ${Object.keys(EVALUATION_IDENTITY_BINDINGS).join(', ')}.`,
    );
  }
...
export function evaluationContentDigest(bytes) {
  if (typeof bytes !== 'string' && !ArrayBuffer.isView(bytes) && !(bytes instanceof ArrayBuffer)) {
    fail(
      'E_EVALUATION_IDENTITY_INVALID',
      'Content digest input must be a string, TypedArray, or ArrayBuffer.',
      'Read the source file as bytes and pass the buffer.',
    );
  }
  let buffer;
...
export function evaluationPayloadDigest(value, omitFields = []) {
  ...
  if (!Array.isArray(omitFields))
    fail(
      'E_EVALUATION_IDENTITY_INVALID',
      'omitFields must be an array of field names.',
      'Pass the identity and digest field names to omit.',
    );
  const payload = { ...value };
...
export function evaluationIdentityFrom(digest, kind) {
  ...
  if (typeof digest !== 'string' || !DIGEST.test(digest)) {
    fail(
      'E_EVALUATION_IDENTITY_INVALID',
      `${kind} identity requires an exact SHA-256 digest.`,
      'Compute the digest with evaluationSelfDigest before deriving the identity.',
    );
  }
...
export function deriveEvaluationIdentity(value, kind) {
  ...
  return Object.freeze({
    id: evaluationIdentityFrom(digest, kind),
    digest,
...
export function assertEvaluationIdentity(value, kind) {
  ...
  if (value[binding.digestField] !== derived.digest) {
    fail(
      ...
      `${kind}.${binding.digestField} does not bind this record's canonical bytes.`,
      `Recompute ${binding.digestField} from the record with ${binding.idField} and ${binding.digestField} removed.`,
      { expected: derived.digest, actual: value[binding.digestField] ?? null },
    );
  }
  if (value[binding.idField] !== derived.id) {
    fail(
      'E_EVALUATION_IDENTITY_FOREIGN',
      `${kind}.${binding.idField} is a foreign identity for these bytes.`,
      `Set ${binding.idField} to ${derived.id}.`,
      { expected: derived.id, actual: value[binding.idField] ?? null },
    );
  }
  return value;
}
...
export function assertUniqueJsonKeys(text, label = 'source document') {
  if (typeof text !== 'string')
    fail(
      ...
      `${label} must be decoded JSON text.`,
      'Decode the source bytes as UTF-8 before parsing.',
    );
  const stack = [];
  let expectKey = false;
  let index = 0;
  while (index < text.length) {
    ...
    if (character === '"') {
      ...
      if (index >= text.length)
        fail(
          'E_EVALUATION_CONTRACT_INVALID',
          `${label} ends inside a string literal.`,
          'Repair the document so it is well-formed JSON.',
        );
      ...
      if (expectKey) {
        ...
        if (!keys || keys.has(key))
          fail(
            'E_EVALUATION_CONTRACT_INVALID',
            `${label} repeats the object key "${key}".`,
            'Remove the duplicate field; a repeated key is refused, never merged.',
          );
        keys.add(key);
        expectKey = false;
      }
...
export function evaluationScenarioIdentityFromSource(sourceText, label = 'scenario source') {
  ...
  try {
    ...
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} is not parseable JSON: ${cause.message}`,
      'Repair the source document before deriving a scenario identity.',
    );
  }
...
function evidenceIndex(entries, label) {
  if (!Array.isArray(entries) || entries.length > 4_096) {
    fail(
      'E_EVALUATION_EVIDENCE_STALE',
      `${label} must be a bounded array of scenario evidence bindings.`,
      'Pass at most 4096 entries, one per scenario.',
    );
  }
  const index = new Map();
  entries.forEach((entry, position) => {
    ...
    if (keys.length !== 2 || keys[0] !== 'inputs' || keys[1] !== 'scenarioId') {
      fail(
        'E_EVALUATION_EVIDENCE_STALE',
        `${cursor} must carry exactly scenarioId and inputs.`,
        'Remove unknown fields; evidence bindings are closed.',
      );
    }
    if (typeof entry.scenarioId !== 'string' || !SCENARIO_ID.test(entry.scenarioId)) {
      fail(
        'E_EVALUATION_EVIDENCE_STALE',
        `${cursor}.scenarioId is not a scenario identity.`,
    ...
    if (
      inputKeys.length !== EVALUATION_EVIDENCE_INPUTS.length ||
      inputKeys.some((key, position2) => key !== EVALUATION_EVIDENCE_INPUTS[position2])
    ) {
      fail(
        'E_EVALUATION_EVIDENCE_STALE',
        `${cursor}.inputs must declare every bound input digest.`,
        `Declare exactly: ${EVALUATION_EVIDENCE_INPUTS.join(', ')}.`,
      );
    }
    for (const field of EVALUATION_EVIDENCE_INPUTS) {
      if (typeof entry.inputs[field] !== 'string' || !DIGEST.test(entry.inputs[field])) {
        fail(
          'E_EVALUATION_EVIDENCE_STALE',
          `${cursor}.inputs.${field} must be an exact SHA-256 digest.`,
          'Bind every input by digest; a name or version is not a binding.',
        );
      }
    }
    if (index.has(entry.scenarioId)) {
      fail(
        'E_EVALUATION_EVIDENCE_STALE',
        `${label} repeats scenario ${entry.scenarioId}.`,
        'Record one evidence binding per scenario identity.',
      );
    }
...
export function evaluationEvidenceReuse(priorEvidence, currentInputs) {
  ...
  for (const [scenarioId, entry] of prior) {
    ...
    if (!now) {
      invalidated.push(
        Object.freeze({
          scenarioId,
          reason: 'scenario-absent',
          changedInputs: Object.freeze([...EVALUATION_EVIDENCE_INPUTS]),
        }),
      );
      continue;
    }
    const changedInputs = EVALUATION_EVIDENCE_INPUTS.filter(
      (field) => entry.inputs[field] !== now.inputs[field],
    );
  ...
  return Object.freeze({
    ...
    invalidated: Object.freeze(
      invalidated.sort((a, b) => a.scenarioId.localeCompare(b.scenarioId)),
    ),
