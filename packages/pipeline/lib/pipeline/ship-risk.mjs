...
const registry = Object.freeze(JSON.parse(readFileSync(
...
function canonicalPath(entry) {
  ...
  if (!/^[a-z][a-z0-9-]*$/.test(entry.repositoryKey ?? '')
    || typeof entry.path !== 'string' || entry.path.length === 0 || entry.path.length > 1024
    || entry.path.startsWith('/') || /^[A-Za-z]:[\\/]/.test(entry.path)) {
  ...
  if (!path || path.split('/').some((part) => part === '' || part === '.' || part === '..')) {
    fail('E_SHIP_RISK_PATH_INVALID', `Risk path contains traversal or an empty segment: ${entry.path}`);
  }
...
function canonicalEnum(values, allowed, field) {
  ...
  if (new Set(values).size !== values.length) fail('E_SHIP_RISK_INPUT_INVALID', `${field} contains duplicates.`);
...
export function shipReviewSpecialistRegistry() {
  ...
  for (const specialist of registry.specialists) {
    if (JSON.stringify(specialist.capabilities) !== JSON.stringify(['candidate.read', 'evidence.read', 'finding.contribute'])
      || specialist.authority !== 'read-only-consolidated-review') {
      fail('E_SHIP_SPECIALIST_AUTHORITY_INVALID', `Specialist ${specialist.id} exceeds read-only consolidated review authority.`);
    }
  }
...
function pathSignals(paths) {
  ...
  if (any(/(?:^|\/)(?:src\/)?(?:dashboard|frontend|ui|pages)(?:\/|$)|\.(?:tsx|jsx|css)$/u)) browserSurfaces.add('ui');
  if (any(/(?:^|\/)(?:auth|authentication|login|oauth)(?:\/|[-_.])/u)) browserSurfaces.add('authentication');
  ...
  if (any(/(?:^|\/)(?:network|client|fetch|http)(?:\/|[-_.])|openapi|graphql/u)) browserSurfaces.add('network');
  return {
    ...
    data: any(/database|(?:^|\/)db(?:\/|[-_.])|storage|persistence|provenance|receipt|data-integrity/u),
  };
}

export function classifyShipRisk(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) fail('E_SHIP_RISK_INPUT_INVALID', 'Risk input must be a closed object.');
  const expected = ['browserSurfaces', 'changedPaths', 'contractChanges', 'dataWrites', 'explicitRisks', 'migrationChanges', 'performanceBudgets', 'permissionEffects', 'subjectDigest'];
  if (JSON.stringify(Object.keys(value).sort()) !== JSON.stringify(expected)) fail('E_SHIP_RISK_INPUT_INVALID', `Risk input fields must be exactly: ${expected.join(', ')}.`);
  const changedPaths = value.changedPaths.map(canonicalPath).sort((left, right) => pathKey(left).localeCompare(pathKey(right)));
  if (new Set(changedPaths.map(pathKey)).size !== changedPaths.length) fail('E_SHIP_RISK_INPUT_INVALID', 'changedPaths contains duplicate repository paths.');
  const explicitRisks = canonicalEnum(value.explicitRisks, SHIP_SPECIALIST_IDS, 'explicitRisks');
  const declaredSurfaces = canonicalEnum(value.browserSurfaces, BROWSER_SURFACES, 'browserSurfaces');
  const input = {
  ...
  schema('ship-risk-classification', {
    kind: 'ship-risk-classification', schemaVersion: '1.0.0', protocolVersion: '1.1.0', input,
    inputDigest: sha256Jcs(input), selectedSpecialists: [], reviewerRoster: ['qa-agent'],
    browserQa: { required: false, triggers: [], requirementDigest: sha256Jcs({ required: false, triggers: [] }) },
  ...
  if (value.migrationChanges || signals.migration) { selected.add('migration'); selected.add('data-integrity'); }
  ...
  const triggers = BROWSER_SURFACES.filter((surface) => declaredSurfaces.includes(surface) || signals.browserSurfaces.includes(surface));
  const required = triggers.length > 0;
  const browserQa = { required, triggers, requirementDigest: sha256Jcs({ required, triggers }) };
  const classification = {
    kind: 'ship-risk-classification', schemaVersion: '1.0.0', protocolVersion: '1.1.0', input,
    inputDigest: sha256Jcs(input), selectedSpecialists,
    reviewerRoster: ['qa-agent', ...selectedSpecialists], browserQa,
  };
...
export function assertShipRiskClassification(value) {
  ...
  if (sha256Jcs(value) !== sha256Jcs(expected)) fail('E_SHIP_RISK_CLASSIFICATION_INVALID', 'Risk classification is not the canonical deterministic result.');
  return value;
}
...
export function assertSpecialistReviewResult(value, { classification, candidateDigest } = {}) {
  ...
  if (!classification.selectedSpecialists.includes(value.specialistId)
    || value.candidateDigest !== candidateDigest
    || value.classificationDigest !== classification.classificationDigest) {
    fail('E_SHIP_SPECIALIST_RESULT_FOREIGN', 'Specialist result does not bind the selected specialist, candidate, and classification.');
  }
  if (/(?:-----BEGIN .*PRIVATE KEY-----|\b(?:cookie|password|secret|token)\s*[:=]|\/(?:Users|home)\/)/iu.test(value.summary)) {
    fail('E_SHIP_SPECIALIST_RESULT_PRIVATE', 'Specialist result contains private or credential-like text.');
  }
  return value;
}
