...
export interface EvaluationIdentityBinding { prefix: string; width: 32 | 64; idField: string; digestField: string }
export type EvaluationEvidenceInput =
  | 'budgetDigest' | 'fixtureSetDigest' | 'graderRegistrationDigest' | 'hostProfileDigest'
  | 'packageDigest' | 'scenarioDigest' | 'sourceDigest';
export type EvaluationEvidenceInvalidationReason = 'input-digest-changed' | 'scenario-absent';
export interface EvaluationEvidenceBinding { scenarioId: string; inputs: Record<EvaluationEvidenceInput, string> }
...
export interface EvaluationDerivedIdentity { id: string; digest: string; idField: string; digestField: string }
export interface EvaluationScenarioIdentity { scenarioId: string; scenarioDigest: string; record: Record<string, unknown> }

export declare const EVALUATION_IDENTITY_BINDINGS: Readonly<Record<EvaluationContractKind, Readonly<EvaluationIdentityBinding>>>;
...
export declare function evaluationContentDigest(bytes: string | ArrayBuffer | ArrayBufferView): string;
export declare function evaluationPayloadDigest(value: unknown, omitFields?: readonly string[]): string;
export declare function evaluationSelfDigest(value: unknown, kind: EvaluationContractKind): string;
export declare function evaluationIdentityFrom(digest: string, kind: EvaluationContractKind): string;
export declare function deriveEvaluationIdentity(value: unknown, kind: EvaluationContractKind): Readonly<EvaluationDerivedIdentity>;
...
export declare function evaluationScenarioIdentityFromSource(sourceText: string, label?: string): Readonly<EvaluationScenarioIdentity>;
