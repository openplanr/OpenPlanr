...
function digest(value, label) {
  if (!/^sha256:[a-f0-9]{64}$/.test(value ?? ''))
    fail('E_BROWSER_QA_ATTESTATION_INVALID', `${label} must be an exact SHA-256 digest.`);
}
function identifier(value, label) {
  if (typeof value !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,127}$/.test(value))
    fail('E_BROWSER_QA_ATTESTATION_INVALID', `${label} must be a closed identifier.`);
}
function canonicalDate(value, label) {
  if (
    ...
    fail('E_BROWSER_QA_ATTESTATION_INVALID', `${label} must be a canonical timestamp.`);
}
...
function bytes(value, label) {
  if (!(Buffer.isBuffer(value) || value instanceof Uint8Array))
    fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} must be engine-owned bytes.`);
  const result = Buffer.from(value);
  if (result.length > MAX_ARTIFACT_BYTES)
    fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} exceeds the bounded artifact size.`);
  return result;
}
...
function countArtifact(value, label) {
  ...
  try {
    ...
    fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} is not one JSON event array.`);
  }
  if (!Array.isArray(events) || events.length > MAX_COUNT_EVENTS)
    fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} is not a bounded event array.`);
  const ids = new Set();
  let failureCount = 0;
  for (const [index, event] of events.entries()) {
    exact(event, ['id', 'status'], `${label}[${index}]`);
    identifier(event.id, `${label}[${index}].id`);
    if (!['passed', 'failed'].includes(event.status) || ids.has(event.id))
      fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} contains a duplicate or unsupported event.`);
    ids.add(event.id);
...
function binaryArtifacts(values, label) {
  if (!Array.isArray(values) || values.length > 256)
    fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} must be a bounded artifact array.`);
  const ids = new Set();
  let byteLength = 0;
  const claims = values.map((entry, index) => {
    ...
    if (ids.has(entry.id))
      fail('E_BROWSER_QA_EVIDENCE_INVALID', `${label} contains duplicate artifact IDs.`);
    ids.add(entry.id);
...
function evidenceClaims(artifacts) {
  exact(
    artifacts,
    ['accessibility', 'console', 'network', 'screenshots', 'traces'],
  ...
  const totalBytes =
    accessibility.byteLength +
    console.byteLength +
    network.byteLength +
    screenshots.byteLength +
    traces.byteLength;
  if (totalBytes > MAX_BUNDLE_BYTES)
    fail(
      'E_BROWSER_QA_EVIDENCE_INVALID',
      'Browser evidence bundle exceeds the bounded total size.',
    );
  const claims = {
    accessibility: {
      failureCount: accessibility.failureCount,
      evidenceDigest: accessibility.evidenceDigest,
    },
...
function assertClaims(result, claims) {
  for (const key of ['accessibility', 'console', 'network', 'screenshots', 'traces']) {
    if (JSON.stringify(result?.[key]) !== JSON.stringify(claims[key])) {
      fail(
        'E_BROWSER_QA_EVIDENCE_MISMATCH',
        `Browser result ${key} claims do not match engine-owned artifact bytes.`,
      );
    }
  }
}
...
export function createTrustedBrowserQaRuntimeHost({ hostId, hostVersion } = {}) {
  ...
  if (typeof hostVersion !== 'string' || !/^[0-9]+\.[0-9]+\.[0-9]+$/.test(hostVersion))
    fail('E_BROWSER_QA_HOST_UNTRUSTED', 'Browser runtime host version is invalid.');
...
export function establishBrowserQaRuntimeCapability({
  ...
  if (!trustedHosts.has(host))
    fail(
      ...
      'Browser evidence requires an engine-owned runtime host configuration.',
    );
  ...
  if (typeof signedIn !== 'boolean')
    fail('E_BROWSER_QA_ATTESTATION_INVALID', 'Browser session signedIn binding is invalid.');
  canonicalDate(issuedAt, 'issuedAt');
  canonicalDate(expiresAt, 'expiresAt');
  if (
    Date.parse(expiresAt) <= Date.parse(issuedAt) ||
    Date.parse(expiresAt) - Date.parse(issuedAt) > 60 * 60 * 1000
  ) {
    fail(
      'E_BROWSER_QA_ATTESTATION_STALE',
      'Browser runtime capability must expire within one hour.',
    );
  }
  const capability = Object.freeze({
    kind: 'browser-qa-runtime-capability',
    capabilityId: `bqc_${randomUUID().replaceAll('-', '')}`,
    hostId: host.hostId,
...
export function attestBrowserQaEvidence({
  capability,
  result,
  artifacts,
  ...
  if (!runtimeCapabilities.has(capability))
    fail(
      ...
      'Browser evidence requires a runtime capability established before the result.',
    );
  ...
  if (
    result?.candidateDigest !== capability.candidateDigest ||
    result?.requirementDigest !== capability.requirementDigest ||
    result?.signedIn !== capability.signedIn ||
    (result.signedIn && result.sessionId !== capability.sessionId) ||
    (!result.signedIn && result.sessionId !== null)
  ) {
    fail(
      'E_BROWSER_QA_ATTESTATION_FOREIGN',
      'Browser result is foreign to the established runtime capability.',
    );
  }
  const custody = evidenceClaims(artifacts);
  assertClaims(result, custody.claims);
  const core = {
    kind: 'browser-qa-host-attestation',
    schemaVersion: '1.0.0',
    ...
    requirementDigest: capability.requirementDigest,
    sessionBindingDigest: capability.sessionBindingDigest,
    resultDigest: sha256Jcs(result),
    evidenceBundleDigest: custody.evidenceBundleDigest,
    issuedAt: now,
...
export function assertBrowserQaEvidenceAttestation(
  attestation,
  {
    result,
    candidateDigest,
  ...
  if (!issuedAttestations.has(attestation))
    fail(
      'E_BROWSER_QA_ATTESTATION_REQUIRED',
      'Browser evidence requires the original runtime-issued host attestation.',
    );
  canonicalDate(now, 'now');
  if (
    ...
    fail('E_BROWSER_QA_ATTESTATION_STALE', 'Browser host attestation is stale.');
  if (
    attestation.candidateDigest !== candidateDigest ||
    attestation.requirementDigest !== requirementDigest ||
    attestation.sessionBindingDigest !== sessionBindingDigest ||
    attestation.resultDigest !== sha256Jcs(result)
  ) {
    fail(
      'E_BROWSER_QA_ATTESTATION_FOREIGN',
      'Browser host attestation does not bind the exact result, candidate, requirement, and ephemeral session.',
    );
  }
  const { attestationDigest, ...core } = attestation;
  if (attestationDigest !== sha256Jcs(core))
    fail('E_BROWSER_QA_ATTESTATION_INVALID', 'Browser host attestation digest is invalid.');
  return attestation;
}
