...
import {
  ...
  assertEvaluationNoPrivateMaterial,
} from './evaluation-redaction.mjs';

export const EVALUATION_PROTOCOL_VERSION = '1.4.0';
export const EVALUATION_SCHEMA_VERSION = '1.0.0';

export const EVALUATION_PROMPT_CLASSES = Object.freeze([
  'positive',
  'negative',
  'ambiguous',
  'permission-denied',
]);
export const EVALUATION_TRIGGER_OUTCOMES = Object.freeze([
  'invoke',
  'decline',
  'clarify',
  'refuse',
]);
export const EVALUATION_RISK_CLASSES = Object.freeze([
  'read-only',
  'local-write',
  'destructive',
  'external-effect',
]);
export const EVALUATION_CAPABILITIES = Object.freeze([
  'file-read',
  'file-write',
  'command-execute',
  'network',
...
export const EVALUATION_EFFECTFUL_CAPABILITIES = Object.freeze([
  'file-write',
  'command-execute',
  'network',
  'browser',
...
export const EVALUATION_GRADER_TYPES = Object.freeze([
  'deterministic-schema',
  'deterministic-behaviour',
  'live-model-judgement',
]);
export const EVALUATION_ABSENCE_CODES = Object.freeze([
  'grader-missing',
  'grader-errored',
  'grader-unavailable',
  'host-unavailable',
...
export const EVALUATION_UNWAIVABLE_METRICS = Object.freeze([
  'schema-validity',
  'package-parity',
  'finding-severity-p0',
  'finding-severity-p1',
]);
export const EVALUATION_WAIVABLE_METRICS = Object.freeze(
  EVALUATION_METRICS.filter((metric) => !EVALUATION_UNWAIVABLE_METRICS.includes(metric)),
);
export const EVALUATION_WAIVER_REASON_CODES = Object.freeze([
  'accepted-risk',
  'known-defect',
...
export const EVALUATION_GATE_THRESHOLDS = Object.freeze({
  'trigger-precision': Object.freeze({
    comparator: '>=',
    unit: 'basis-points',
    threshold: 9500,
    waivable: true,
  }),
  'trigger-recall': Object.freeze({
    comparator: '>=',
    unit: 'basis-points',
    threshold: 9500,
    waivable: true,
  }),
  'journey-completion': Object.freeze({
    comparator: '>=',
    unit: 'basis-points',
    threshold: 9000,
    waivable: true,
  }),
  'schema-validity': Object.freeze({
    comparator: '==',
    unit: 'basis-points',
    threshold: 10_000,
    waivable: false,
  }),
  'asset-parity': Object.freeze({
    comparator: '==',
    unit: 'basis-points',
    threshold: 10_000,
    waivable: true,
  }),
  'export-parity': Object.freeze({
    comparator: '==',
    unit: 'basis-points',
    threshold: 10_000,
    waivable: true,
  }),
  'package-parity': Object.freeze({
    comparator: '==',
    unit: 'basis-points',
    threshold: 10_000,
    waivable: false,
  }),
  'finding-severity-p0': Object.freeze({
    comparator: '<=',
    unit: 'count',
    threshold: 0,
    waivable: false,
  }),
  'finding-severity-p1': Object.freeze({
    comparator: '<=',
    unit: 'count',
    threshold: 0,
    waivable: false,
  }),
  'latency-regression': Object.freeze({
    comparator: '<=',
    unit: 'basis-points',
    threshold: 2_000,
    waivable: true,
...
const SCENARIO_SOURCE_PATH =
  /^[A-Za-z0-9_][A-Za-z0-9._-]*(?:\/[A-Za-z0-9_][A-Za-z0-9._-]*)*\.json$/u;
const CREDENTIAL_REF_NAME = /^[A-Za-z][A-Za-z0-9_.-]{0,127}$/u;
const WAIVER_STATEMENT = /^[A-Za-z0-9][A-Za-z0-9 ,.()'-]{9,279}$/u;
const MARKUP = /(?:<[^>]+>|\{\{[^}]+\}\})/u;
const AUTHORITY_KEY =
  /^(?:authority|authorities|grant|grants|granted|capability|capabilities|entitlement|permit|permission(?:s)?Granted|promotion|activation|approval|approvals|releaseAuthority|publishAuthority|deployAuthority|tagAuthority|pushAuthority|marketplacePromotion|roomActivation)$/iu;
...
function object(value, label) {
  if (
    ...
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must be one plain JSON object.`,
      'Pass the parsed contract record itself.',
    );
  }
  return value;
}
...
function exact(value, keys, label) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    ...
    fail(
      ...
      'Contracts are closed; remove unknown fields and supply every required field.',
      { missing, unknown },
    );
  }
  return value;
...
function envelope(value, kind, label) {
  ...
  if (value.kind !== kind)
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} is not a ${kind} record.`,
      `Set kind to "${kind}".`,
      { kind: value.kind ?? null },
    );
  if (
    value.schemaVersion !== EVALUATION_SCHEMA_VERSION ||
    value.protocolVersion !== EVALUATION_PROTOCOL_VERSION
  ) {
    fail(
      'E_EVALUATION_VERSION_UNSUPPORTED',
      `${label} carries an implicit or unsupported contract version.`,
      `Declare schemaVersion "${EVALUATION_SCHEMA_VERSION}" and protocolVersion "${EVALUATION_PROTOCOL_VERSION}" explicitly.`,
      {
...
function constant(value, expected, label) {
  if (value !== expected)
    fail(
      ...
      'This field is a constant; no other value is representable.',
      { actual: value ?? null },
    );
  return value;
}
...
function member(value, allowed, label) {
  if (!allowed.includes(value))
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} is not a member of its closed vocabulary.`,
...
function pattern(value, expression, label, hint) {
  if (typeof value !== 'string' || !expression.test(value))
    fail('E_EVALUATION_CONTRACT_INVALID', `${label} does not match its closed pattern.`, hint, {
      actual: typeof value === 'string' ? value.slice(0, 48) : null,
    });
  return value;
}
...
function boolean(value, label) {
  if (typeof value !== 'boolean')
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must be an explicit boolean.`,
      'Declare the value; an absent declaration is never a default.',
    );
  return value;
}
...
function integer(value, label, min, max) {
  if (!Number.isSafeInteger(value) || value < min || value > max) {
    fail(
      ...
      'Use whole units; measurements are integers, never floats.',
      { actual: value ?? null },
    );
  }
  return value;
...
function timestamp(value, label) {
  if (
    ...
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must be one canonical RFC 3339 UTC timestamp.`,
      'Use the exact form 2026-01-01T00:00:00.000Z so identical instants canonicalize identically.',
      { actual: value ?? null },
    );
  }
  return value;
...
function boundedText(value, label, max) {
  if (
    ...
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must be non-blank text of at most ${max} characters.`,
      'Supply a statement a reader can act on.',
    );
  }
...
function title(value, label) {
  ...
  if (value !== value.trim())
    fail(
      ...
      'Trim the title so identical titles canonicalize identically.',
    );
  if (MARKUP.test(value))
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must not carry markup or template syntax.`,
...
function boundedArray(value, label, min, max) {
  if (!Array.isArray(value) || value.length < min || value.length > max) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} must be an array of ${min} to ${max} entries.`,
      'Bound the collection explicitly.',
    );
  }
  if (new Set(value.map((entry) => JSON.stringify(entry))).size !== value.length) {
    fail(
      ...
      'Duplicate members are refused, never collapsed.',
    );
  }
  return value;
}
...
function ascending(values, label, compare) {
  for (let index = 1; index < values.length; index += 1) {
    if (compare(values[index - 1], values[index]) >= 0) {
      fail(
        'E_EVALUATION_MEMBER_ORDER',
        `${label} is not in canonical ascending order.`,
        'Sort members canonically so identical membership always yields an identical digest.',
        { at: index },
      );
    }
  }
...
const byNameThenVersion = (nameField, versionField) => (left, right) =>
  ...
  compareVersion(left[versionField], right[versionField]);
...
function skillIdentity(value, label) {
  ...
  if (value.id.length < 2 || value.id.length > 64)
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.id must be 2 to 64 characters.`,
      'Use a lowercase closed skill identifier.',
    );
  ...
  const visit = (entry, cursor, depth) => {
    ...
    for (const [key, item] of Object.entries(entry)) {
      if (AUTHORITY_KEY.test(key)) {
        fail(
          'E_EVALUATION_RECEIPT_AUTHORITY_CLAIM',
          `${cursor}.${key} would read as release authority.`,
          'A certification receipt records readiness only; it grants nothing.',
        );
      }
...
export function assertEvaluationScenario(value, label = 'scenario') {
  ...
  pattern(
    value.hostProfileRef.hostProfileId,
    IDENTIFIER,
    `${label}.hostProfileRef.hostProfileId`,
    'Reference the host profile by its closed identifier.',
  );
  ...
  pattern(
    value.budgetRef.budgetId,
    IDENTIFIER,
    `${label}.budgetRef.budgetId`,
    'Reference the budget by its closed identifier.',
  );
  ...
  member(
    value.expectedTrigger.outcome,
    EVALUATION_TRIGGER_OUTCOMES,
    `${label}.expectedTrigger.outcome`,
  );
  ...
  value.declaredPermissions.forEach((entry, index) => {
    ...
    if (capabilities.has(entry.capability))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.capability is declared twice.`,
        'Declare each capability once; a repeated decision is ambiguous.',
      );
  ...
  value.fixtureRefs.forEach((entry, index) => {
    ...
    pattern(
      ...
      /^efx_[a-f0-9]{64}$/u,
      `${cursor}.fixtureId`,
      'Use the efx_ identity derived from the fixture record.',
    );
  ...
  if (promptFixtures !== 1) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.fixtureRefs must reference exactly one prompt fixture.`,
      'Prompt text lives in a fixture so no identity-bearing record carries a raw prompt.',
      { promptFixtures },
    );
  }
  ...
  const expectedOutcome = {
    ...
    'permission-denied': ['refuse'],
  }[value.promptClass];
  if (!expectedOutcome.includes(value.expectedTrigger.outcome)) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.expectedTrigger.outcome is not the correct terminal behaviour for prompt class ${value.promptClass}.`,
      `Use one of: ${expectedOutcome.join(', ')}.`,
    );
  }
  if (
    value.promptClass === 'permission-denied' &&
    !value.declaredPermissions.some((entry) => entry.decision === 'denied')
  ) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} declares no denied capability for a permission-denied prompt class.`,
      'Declare the capability the host denies; an absent capability is denied but never assumed.',
    );
  }
  if (
    value.riskClass === 'read-only' &&
    value.declaredPermissions.some(
      (entry) =>
        entry.decision === 'granted' &&
        EVALUATION_EFFECTFUL_CAPABILITIES.includes(entry.capability),
    )
  ) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} grants an effectful capability under a read-only risk class.`,
      'Raise the risk class or withdraw the grant; the risk class states the widest blast radius.',
    );
  }
...
export function assertEvaluationCorpus(value, { sourceDigests = null, label = 'corpus' } = {}) {
  ...
  value.members.forEach((entry, index) => {
    ...
    pattern(
      ...
      /^esc_[a-f0-9]{64}$/u,
      `${cursor}.scenarioId`,
      'Use the esc_ identity derived from the scenario source bytes.',
    );
    ...
    pattern(
      ...
      `${cursor}.sourcePath`,
      'Use a normalized repository-relative .json path.',
    );
    if (entry.scenarioId.slice(4) !== entry.scenarioDigest.slice(7)) {
      fail(
        'E_EVALUATION_IDENTITY_FOREIGN',
        `${cursor}.scenarioId is a foreign identity for its own digest.`,
        'Derive the member identity from the member digest.',
      );
    }
    if (seenScenarios.has(entry.scenarioId))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.scenarioId is already a member.`,
        'A duplicate scenario identity is refused, never deduplicated.',
      );
    if (seenPaths.has(entry.sourcePath))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.sourcePath is already a member.`,
        'Each member has its own source document.',
      );
    seenScenarios.add(entry.scenarioId);
    seenPaths.add(entry.sourcePath);
  });
  ascending(value.members, `${label}.members`, (left, right) =>
    ...
    for (const entry of value.members) {
      ...
      if (recomputed === undefined) {
        fail(
          ...
          `${label} member ${entry.scenarioId} has no recomputed digest for ${entry.sourcePath}.`,
          'Recompute every member digest from its source bytes; an unread member is not a valid member.',
        );
      }
      if (recomputed !== entry.scenarioDigest) {
        fail(
          'E_EVALUATION_DIGEST_MISMATCH',
          `${label} member ${entry.scenarioId} no longer matches its source bytes.`,
          'Reseal the corpus against the current scenario sources; evidence never carries across a changed digest.',
          { sourcePath: entry.sourcePath, expected: entry.scenarioDigest, actual: recomputed },
        );
      }
    }
  }
...
export function assertEvaluationFixture(value, label = 'fixture') {
  ...
  member(
    value.mediaKind,
    ['text', 'markdown', 'json', 'source', 'html', 'image', 'archive', 'binary'],
    `${label}.mediaKind`,
  );
  ...
  pattern(
    ...
    RELATIVE_PATH,
    `${label}.sourcePath`,
    'Use a normalized repository-relative path; absolute paths and dot segments are refused.',
  );
...
export function assertEvaluationHostProfile(value, label = 'host profile') {
  ...
  member(
    value.runtime,
    ['cli', 'desktop', 'web', 'mobile', 'ide-extension', 'headless-browser'],
    `${label}.runtime`,
  );
  ...
  exact(
    ...
    ['mode', 'promptSurface', 'toolIsolation', 'escalation'],
    `${label}.permissionModel`,
  );
  member(
    value.permissionModel.mode,
    ['prompted', 'preauthorized', 'sandboxed', 'denied'],
    `${label}.permissionModel.mode`,
  );
  member(
    value.permissionModel.promptSurface,
    ['terminal', 'chat', 'native-dialog', 'none'],
    `${label}.permissionModel.promptSurface`,
  );
  member(
    value.permissionModel.toolIsolation,
    ['enforced', 'advisory', 'none', 'enforced-read-only'],
    `${label}.permissionModel.toolIsolation`,
  );
  ...
  value.outputModes.forEach((mode, index) =>
    member(
      mode,
      ['text', 'markdown', 'json', 'file', 'artifact', 'image', 'web-page'],
      `${label}.outputModes[${index}]`,
    ),
  );
  ...
  if (value.adapterBinding !== null) {
    ...
    constant(
      value.adapterBinding.protocolVersion,
      EVALUATION_PROTOCOL_VERSION,
      `${cursor}.protocolVersion`,
    );
    member(
      value.adapterBinding.adapterId,
      ['claude-code', 'codex', 'cursor'],
      `${cursor}.adapterId`,
    );
    pattern(
      value.adapterBinding.adapterVersion,
      SEMVER,
      `${cursor}.adapterVersion`,
      'Use an explicit semantic version.',
    );
    ...
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} claims a certified full pipeline runtime with no adapter binding.`,
      'Bind the registered adapter record by digest, or declare certifiedFullPipelineRuntime false.',
    );
  }
...
export function assertEvaluationHostProfileRegistry(value, label = 'host profile registry') {
  ...
  value.profiles.forEach((profile, index) => {
    ...
    if (seen.has(profile.hostProfileId))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${label}.profiles[${index}].hostProfileId is already registered.`,
        'Register each host profile identity once.',
      );
...
export function assertEvaluationGraderRegistration(value, label = 'grader registration') {
  ...
  value.declaredInputs.forEach((entry, index) => {
    ...
    if (seenInputs.has(entry.inputKind))
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${cursor}.inputKind is declared twice.`,
        'Declare each input kind once.',
      );
  ...
  exact(
    value.determinism,
    ['deterministic', 'modelCall', 'network', 'credentialUse', 'filesystemWrite', 'replay'],
    determinism,
  );
  ...
  exact(
    ...
    ['contractId', 'schemaVersion', 'protocolVersion'],
    `${label}.outputContract`,
  );
  constant(
    value.outputContract.contractId,
    'evaluation-observation',
    `${label}.outputContract.contractId`,
  );
  constant(
    value.outputContract.schemaVersion,
    EVALUATION_SCHEMA_VERSION,
    `${label}.outputContract.schemaVersion`,
  );
  constant(
    value.outputContract.protocolVersion,
    EVALUATION_PROTOCOL_VERSION,
    `${label}.outputContract.protocolVersion`,
  );
  provenance(value.provenance, `${label}.provenance`);
  ...
  if (deterministic) {
    if (
      ...
      value.determinism.modelCall ||
      value.determinism.network ||
      value.determinism.credentialUse ||
      value.determinism.replay !== 'identical-output'
    ) {
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${label} is a deterministic grader that declares a model call, network access, a credential use, or non-identical replay.`,
        'A deterministic grader must declare no model call, no network, no credential use, and identical-output replay.',
      );
    }
    if (value.consent !== null || value.credentialReference !== null) {
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${label} is a deterministic grader carrying consent or a credential reference.`,
        'Set consent and credentialReference to null; the deterministic path holds neither.',
      );
    }
  } else {
    if (
      value.determinism.deterministic ||
      !value.determinism.modelCall ||
      value.determinism.replay !== 'not-guaranteed'
    ) {
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${label} is a live-model grader that claims determinism.`,
        'Declare deterministic false, modelCall true, and not-guaranteed replay.',
      );
    }
    const consent = `${label}.consent`;
    exact(
      value.consent,
      ['required', 'mode', 'scope', 'recordDigest', 'grantedAt', 'expiresAt', 'noDefaultChoice'],
      consent,
    );
    ...
    if (Date.parse(value.consent.expiresAt) <= Date.parse(value.consent.grantedAt)) {
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${consent}.expiresAt must be after grantedAt.`,
        'Consent is bounded; an unbounded grant is refused.',
      );
    }
    const credential = `${label}.credentialReference`;
    exact(value.credentialReference, ['refKind', 'refName', 'custody'], credential);
    member(
      value.credentialReference.refKind,
      ['environment-name', 'os-custody-key'],
      `${credential}.refKind`,
    );
    pattern(
      value.credentialReference.refName,
      CREDENTIAL_REF_NAME,
      `${credential}.refName`,
      'Reference the credential by logical name; a credential value is never representable here.',
    );
...
export function assertEvaluationGraderRegistry(value, label = 'grader registry') {
  ...
  value.graders.forEach((grader, index) => {
    ...
    if (seen.has(grader.graderId))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${label}.graders[${index}].graderId is already registered.`,
        'Register each grader identity once.',
      );
...
export function assertEvaluationGraderIndependence(
  registration,
  subject,
  ...
  pattern(
    subject.skillId,
    PUBLISHED_NAME,
    `${label} subject.skillId`,
    'Name the skill under evaluation.',
  );
  digest(subject.skillSourceDigest, `${label} subject.skillSourceDigest`);
  if (
    registration.graderName === subject.skillId ||
    registration.provenance.sourceDigest === subject.skillSourceDigest
  ) {
    fail(
      'E_EVALUATION_GRADER_NOT_INDEPENDENT',
      `${label} shares its identity or source bytes with the skill under evaluation.`,
      'Grade a skill with a grader that is not part of it.',
      { graderName: registration.graderName, skillId: subject.skillId },
    );
  }
  return registration;
}
...
export function assertEvaluationBudget(value, label = 'budget') {
  ...
  pattern(
    ...
    IDENTIFIER,
    `${label}.scope.id`,
    'Name the scope by its closed identifier.',
  );
  ...
  exact(
    ...
    [
      ...
      'costEstimateMicrosCeiling',
      'permissionPromptCeiling',
      'retryCeiling',
    ],
    ceilings,
  );
  ...
  integer(
    value.ceilings.costEstimateMicrosCeiling,
    `${ceilings}.costEstimateMicrosCeiling`,
    0,
    1_000_000_000_000,
  );
  ...
  if (value.ceilings.latencyMsP95Ceiling < value.ceilings.latencyMsP50Ceiling) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${ceilings}.latencyMsP95Ceiling is below the p50 ceiling.`,
      'A p95 ceiling is never tighter than its p50 ceiling.',
    );
  }
  if (
    value.ceilings.totalTokensCeiling < value.ceilings.inputTokensCeiling ||
    value.ceilings.totalTokensCeiling < value.ceilings.outputTokensCeiling
  ) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${ceilings}.totalTokensCeiling is below a component ceiling.`,
      'The total ceiling covers both the input and the output ceiling.',
    );
  }
...
export function assertEvaluationGatePolicy(value, label = 'gate policy') {
  ...
  for (const metric of EVALUATION_METRICS) {
    ...
    for (const [field, expectedValue] of Object.entries(expected)) {
      if (value.gates[metric][field] !== expectedValue) {
        fail(
          ...
          `${cursor}.${field} does not state the mandatory certification threshold.`,
          `This gate is fixed at ${JSON.stringify(expectedValue)}; a policy that lowers or widens it is not this policy.`,
          { metric, field, expected: expectedValue, actual: value.gates[metric][field] ?? null },
        );
      }
    }
    if (EVALUATION_UNWAIVABLE_METRICS.includes(metric) && value.gates[metric].waivable !== false) {
      fail(
        'E_EVALUATION_GATE_POLICY_INVALID',
        `${cursor} marks an unwaivable gate as waivable.`,
        'Schema validity, package parity, and P0/P1 findings can never be waived.',
      );
    }
  }
...
export function assertEvaluationObservation(value, { subject = null, label = 'observation' } = {}) {
  ...
  pattern(
    ...
    /^eru_[a-f0-9]{32}$/u,
    `${label}.runId`,
    'Bind the observation to its run identity.',
  );
  ...
  if (value.scenario.scenarioId.slice(4) !== value.scenario.scenarioDigest.slice(7)) {
    fail(
      'E_EVALUATION_IDENTITY_FOREIGN',
      `${scenario}.scenarioId is a foreign identity for its own digest.`,
      'Observations bind the scenario whose bytes were actually run.',
    );
  }
  ...
  exact(
    value.grader,
    ['graderId', 'graderVersion', 'graderType', 'registrationDigest', 'registryDigest'],
    grader,
  );
  pattern(
    value.grader.graderId,
    /^egr_[a-f0-9]{32}$/u,
    `${grader}.graderId`,
    'Bind the registered grader identity.',
  );
  pattern(
    value.grader.graderVersion,
    SEMVER,
  ...
  pattern(
    value.hostProfile.hostProfileId,
    /^ehp_[a-f0-9]{32}$/u,
    `${hostProfile}.hostProfileId`,
    'Bind the registered host profile identity.',
  );
  ...
  pattern(
    ...
    'Use an upper-case terminal reason code.',
  );
  ...
  if (value.outcome === 'absent') {
    ...
    member(
      value.absence.recoveryDisposition,
      ['rerun', 'register-grader', 'refresh-fixture', 'escalate'],
      `${absence}.recoveryDisposition`,
    );
    if (value.absence.treatedAsPass !== false) {
      fail(
        'E_EVALUATION_ABSENCE_INVALID',
        `${absence}.treatedAsPass must be false.`,
        'A missing, errored, or unavailable grader is a typed absence and can never be read as a pass.',
      );
    }
    if (value.graderOutputDigest !== null) {
      fail(
        'E_EVALUATION_ABSENCE_INVALID',
        `${label}.graderOutputDigest must be null for a typed absence.`,
        'An absent observation has no grader output to bind.',
      );
    }
  } else {
    if (value.absence !== null) {
      fail(
        'E_EVALUATION_ABSENCE_INVALID',
        `${label}.absence must be null for a ${value.outcome} outcome.`,
        'A recorded result and a typed absence are mutually exclusive.',
      );
    }
  ...
  if (subject !== null) {
    ...
    if (
      subject.registration.graderId !== value.grader.graderId ||
      subject.registration.registrationDigest !== value.grader.registrationDigest
    ) {
      fail(
        'E_EVALUATION_GRADER_INVALID',
        `${label} subject.registration is not the grader that produced this observation.`,
        'Pass the registration the observation binds by digest.',
      );
    }
...
export function assertEvaluationRunResult(value, label = 'run result') {
  ...
  exact(
    ...
    [
      ...
      'evidenceTransport',
      'corpusDigest',
      'graderRegistryDigest',
      'hostProfileRegistryDigest',
      'hostProfiles',
      'budgetDigest',
      'gatePolicyDigest',
      'packageDigest',
  ...
  value.observations.forEach((entry, index) => {
    ...
    pattern(
      ...
      'Bind the observation identity.',
    );
    digest(entry.observationDigest, `${cursor}.observationDigest`);
    digest(entry.scenarioDigest, `${cursor}.scenarioDigest`);
    member(entry.outcome, ['pass', 'fail', 'absent'], `${cursor}.outcome`);
    if (seenObservations.has(entry.observationId))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.observationId is recorded twice.`,
        'Record each observation once.',
      );
  ...
  integer(
    value.measurements.costEstimateMicros,
  ...
  if (value.measurements.latencyMsP95 < value.measurements.latencyMsP50) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${measurements}.latencyMsP95 is below the observed p50.`,
      'Report percentiles from the same sample.',
    );
  }

  exact(value.findingCounts, ['p0', 'p1', 'p2', 'p3'], `${label}.findingCounts`);
  for (const severity of ['p0', 'p1', 'p2', 'p3'])
    counter(value.findingCounts[severity], `${label}.findingCounts.${severity}`);
  ...
  value.rawEvidence.forEach((entry, index) => {
    ...
    pattern(
      ...
      /^eev_[a-f0-9]{32}$/u,
      `${cursor}.evidenceId`,
      'Bind the local evidence identity.',
    );
    ...
    if (seenEvidence.has(entry.evidenceId))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.evidenceId is recorded twice.`,
        'Record each evidence reference once.',
      );
...
export function assertEvaluationAggregateReport(value, label = 'aggregate report') {
  ...
  for (const field of [
    ...
    digest(value.inputs[field], `${inputs}.${field}`);
  boundedArray(value.inputs.hostProfileDigests, `${inputs}.hostProfileDigests`, 1, 32);
  value.inputs.hostProfileDigests.forEach((entry, index) =>
    digest(entry, `${inputs}.hostProfileDigests[${index}]`),
  );
  boundedArray(value.inputs.runResultDigests, `${inputs}.runResultDigests`, 1, 256);
  value.inputs.runResultDigests.forEach((entry, index) =>
    digest(entry, `${inputs}.runResultDigests[${index}]`),
  );
  ...
  const counterFields = [
    ...
    'scenariosAbsent',
    'scenariosWaived',
    'triggerTruePositives',
    'triggerFalsePositives',
    'triggerFalseNegatives',
    'journeysAttempted',
    'journeysCompleted',
    'outputsValidated',
    'outputsSchemaValid',
    ...
    'exportsDeclared',
    'exportsPresent',
    'packageMembersDeclared',
    'packageMembersPresent',
    'permissionPrompts',
    'retries',
    'findingsP0',
    'findingsP1',
  ...
  const outcomeSum =
    value.counters.scenariosPassed +
    value.counters.scenariosFailed +
    value.counters.scenariosBlocked +
    value.counters.scenariosAbsent +
    value.counters.scenariosWaived;
  if (outcomeSum !== value.counters.scenariosTotal) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.counters outcome breakdown does not sum to scenariosTotal.`,
      'Every scenario lands in exactly one terminal bucket; an unaccounted scenario is never a pass.',
      { outcomeSum, scenariosTotal: value.counters.scenariosTotal },
    );
  }
  for (const [part, whole] of [
    ['journeysCompleted', 'journeysAttempted'],
    ['outputsSchemaValid', 'outputsValidated'],
    ['assetsPresent', 'assetsDeclared'],
    ['exportsPresent', 'exportsDeclared'],
    ['packageMembersPresent', 'packageMembersDeclared'],
  ]) {
    if (value.counters[part] > value.counters[whole]) {
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${label}.counters.${part} exceeds ${whole}.`,
        'A subset counter never exceeds its population.',
      );
    }
  }
  ...
  exact(
    ...
    [
      ...
      'latencyMsObserved',
      'latencyRegression',
      'costEstimateMicrosBaseline',
      'costEstimateMicrosObserved',
      'costRegression',
      'permissionPromptCeiling',
      'retryCeiling',
    ],
  ...
  integer(
    value.budget.costEstimateMicrosBaseline,
    `${budget}.costEstimateMicrosBaseline`,
    0,
    1_000_000_000_000,
  );
  integer(
    value.budget.costEstimateMicrosObserved,
    `${budget}.costEstimateMicrosObserved`,
    0,
    1_000_000_000_000,
  );
  ...
  if (value.budget.baselineDigest !== value.inputs.budgetBaselineDigest) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${budget}.baselineDigest does not match the declared input baseline.`,
      'A regression is measured against the frozen baseline this report was built from.',
    );
  }
  ...
  if (absenceSum !== value.counters.scenariosAbsent) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.absences does not sum to counters.scenariosAbsent.`,
      'Every typed absence is counted once and stays an absence.',
      { absenceSum, scenariosAbsent: value.counters.scenariosAbsent },
    );
  }
  ...
  value.skills.forEach((entry, index) => {
    ...
    pattern(
      ...
      PUBLISHED_NAME,
      `${cursor}.skillId`,
      'Use the published skill identifier.',
    );
    pattern(
      entry.skillVersion,
      SKILL_VERSION,
    ...
    for (const field of [
      ...
      counter(entry[field], `${cursor}.${field}`);
    const sum =
      entry.scenariosPassed +
      entry.scenariosFailed +
      entry.scenariosBlocked +
      entry.scenariosAbsent +
      entry.scenariosWaived;
    if (sum !== entry.scenariosTotal) {
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor} outcome breakdown does not sum to scenariosTotal.`,
        'Every scenario lands in exactly one terminal bucket.',
      );
    }
  ...
  if (skillScenarioTotal !== value.counters.scenariosTotal) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.skills does not partition counters.scenariosTotal.`,
      'Each scenario belongs to exactly one skill summary.',
      { skillScenarioTotal, scenariosTotal: value.counters.scenariosTotal },
    );
  }
  ...
  value.scenarios.forEach((entry, index) => {
    ...
    for (const field of [
      ...
      digest(entry[field], `${cursor}.${field}`);
    pattern(
      entry.skillId,
      PUBLISHED_NAME,
      `${cursor}.skillId`,
    ...
    if (entry.status === 'absent')
      member(entry.absenceReason, EVALUATION_ABSENCE_CODES, `${cursor}.absenceReason`);
    else if (entry.absenceReason !== null)
      fail(
        'E_EVALUATION_ABSENCE_INVALID',
        `${cursor}.absenceReason must be null unless the scenario is absent.`,
        'An absence reason belongs only to a typed absence.',
      );
    if (entry.status === 'waived') digest(entry.waiverDigest, `${cursor}.waiverDigest`);
    else if (entry.waiverDigest !== null)
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.waiverDigest must be null unless the scenario is waived.`,
        'Bind a waiver only where one was applied.',
      );
  });
...
export function assertEvaluationWaiver(value, label = 'waiver') {
  ...
  if (EVALUATION_UNWAIVABLE_METRICS.includes(value.metric)) {
    fail(
      ...
      `${label}.metric targets an unwaivable gate.`,
      'Schema validity, package parity, and P0/P1 findings can never be waived; fix the finding instead.',
      { metric: value.metric },
    );
  }
  ...
  if (value.scope.kind === 'scenario') {
    ...
    pattern(
      ...
      PUBLISHED_NAME,
      `${scope}.skillId`,
      'Name the skill the waiver covers.',
    );
    pattern(
      value.scope.skillVersion,
      SKILL_VERSION,
      `${scope}.skillVersion`,
      'Use an explicit semantic version.',
    );
    ...
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${scope} names no scenario or skill.`,
      'A scope-free waiver is refused; bind the waiver to one scenario digest or one skill version.',
      { kind: value.scope.kind ?? null },
    );
  }
  ...
  pattern(
    ...
    'State the accepted risk in 10 to 280 plain characters.',
  );
  ...
  pattern(
    value.ownerSignature.identity,
    ACTOR_IDENTITY,
    `${signature}.identity`,
    'Name the accountable owner.',
  );
  pattern(
    value.ownerSignature.keyIdentity,
    ACTOR_IDENTITY,
    `${signature}.keyIdentity`,
    'Reference the signing key by logical identity, never by value.',
  );
  ...
  if (expires <= issued) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label}.expiresAt is not after issuedAt.`,
      'A waiver that never opens a window is refused.',
    );
  }
  if (expires - issued > EVALUATION_WAIVER_MAX_HORIZON_DAYS * 86_400_000) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label} is unbounded in practice.`,
      `A waiver expires within ${EVALUATION_WAIVER_MAX_HORIZON_DAYS} days of issue; a longer horizon is a permanent exemption.`,
      { horizonDays: Math.round((expires - issued) / 86_400_000) },
    );
  }
...
export function assertEvaluationWaiverApplicable(
  waiver,
  { now, gatePolicyDigest = null, scenarioDigest = null, label = 'waiver' } = {},
) {
  ...
  if (instant < Date.parse(waiver.issuedAt)) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label} is not yet in force.`,
      'Apply a waiver only after it is issued.',
    );
  }
  if (instant >= Date.parse(waiver.expiresAt)) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label} expired at ${waiver.expiresAt}.`,
      'Re-issue the waiver or fix the finding; an expired waiver never applies.',
    );
  }
  if (gatePolicyDigest !== null && waiver.gatePolicyDigest !== gatePolicyDigest) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label} was issued against a different gate policy.`,
      'A waiver binds the exact policy it was granted under.',
      { expected: gatePolicyDigest, actual: waiver.gatePolicyDigest },
    );
  }
  if (
    scenarioDigest !== null &&
    waiver.scope.kind === 'scenario' &&
    waiver.scope.scenarioDigest !== scenarioDigest
  ) {
    fail(
      'E_EVALUATION_WAIVER_REFUSED',
      `${label} was issued against a different scenario.`,
      'A scenario waiver never carries to a scenario whose bytes differ.',
      { expected: scenarioDigest, actual: waiver.scope.scenarioDigest },
    );
  }
  return waiver;
}

export function assertSkillCertificationReceipt(
  value,
  { aggregateReport = null, now = null, label = 'certification receipt' } = {},
) {
  ...
  pattern(
    value.subject.skillId,
    PUBLISHED_NAME,
    `${label}.subject.skillId`,
    'Name the certified skill.',
  );
  pattern(
    value.subject.skillVersion,
    SKILL_VERSION,
    `${label}.subject.skillVersion`,
    'Use an explicit semantic version.',
  );
  ...
  value.inputs.hostProfiles.forEach((entry, index) => {
    ...
    pattern(
      ...
      PUBLISHED_NAME,
      `${cursor}.hostProfileId`,
      'Name the host profile the run covered.',
    );
  ...
  value.gateEvaluation.forEach((entry, index) => {
    ...
    if (evaluated.has(entry.metric))
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.metric is evaluated twice.`,
    ...
    if (entry.status === 'waived') {
      if (!entry.waivable || EVALUATION_UNWAIVABLE_METRICS.includes(entry.metric)) {
        fail(
          'E_EVALUATION_WAIVER_REFUSED',
          `${cursor} waives a gate that cannot be waived.`,
          'Fix the finding; this gate has no waiver path.',
        );
      }
      ...
      fail(
        'E_EVALUATION_CONTRACT_INVALID',
        `${cursor}.waiverDigest must be null unless the gate is waived.`,
        'Bind a waiver only to a waived gate.',
      );
    }
  ...
  if (unevaluated.length > 0) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.gateEvaluation omits a mandatory gate.`,
      'Evaluate every mandatory gate; an unevaluated gate is never a met gate.',
      { unevaluated },
    );
  }
  ...
  value.appliedWaivers.forEach((entry, index) => {
    ...
    pattern(
      entry.ownerSignatureIdentity,
      ACTOR_IDENTITY,
      `${cursor}.ownerSignatureIdentity`,
      'Name the accountable owner.',
    );
    ...
    if (instant !== null && Date.parse(entry.expiresAt) <= instant) {
      fail(
        'E_EVALUATION_WAIVER_REFUSED',
        `${cursor} expired at ${entry.expiresAt}.`,
        'An expired waiver never counts toward readiness.',
      );
    }
    applied.add(entry.waiverDigest);
  });
  const waivedDigests = new Set(
    value.gateEvaluation
      .filter((entry) => entry.status === 'waived')
      .map((entry) => entry.waiverDigest),
  );
  for (const waiverDigest of waivedDigests) {
    if (!applied.has(waiverDigest))
      fail(
        'E_EVALUATION_WAIVER_REFUSED',
        `${label} waives a gate with a waiver it does not record.`,
        'Record every applied waiver so a reader can see what was accepted.',
        { waiverDigest },
      );
  }
  for (const waiverDigest of applied) {
    if (!waivedDigests.has(waiverDigest))
      fail(
        'E_EVALUATION_WAIVER_REFUSED',
        `${label} records a waiver that waives no gate.`,
        'Remove waivers that were not applied.',
        { waiverDigest },
      );
  }
  ...
  value.blockingMetrics.forEach((metric, index) =>
    member(metric, EVALUATION_METRICS, `${label}.blockingMetrics[${index}]`),
  );
  ...
  if (JSON.stringify([...value.blockingMetrics].sort()) !== JSON.stringify(notMet.sort())) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label}.blockingMetrics does not match the gates that were not met.`,
      'List exactly the gates whose status is not-met.',
      { declared: value.blockingMetrics, notMet },
    );
  }
  if (value.result === 'release-ready' && notMet.length > 0) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} reports readiness while a gate is not met.`,
      'A receipt reports readiness only when every gate is met or validly waived.',
      { notMet },
    );
  }
  if (value.result === 'blocked' && notMet.length === 0) {
    fail(
      'E_EVALUATION_CONTRACT_INVALID',
      `${label} reports blocked with no unmet gate.`,
      'Name the gate that blocks, or report readiness.',
    );
  }
  ...
  if (aggregateReport !== null) {
    ...
    if (value.inputs.aggregateReportDigest !== aggregateReport.reportDigest) {
      fail(
        'E_EVALUATION_DIGEST_MISMATCH',
        `${label}.inputs.aggregateReportDigest does not bind this aggregate report.`,
        'A receipt reports on the exact report it was computed from.',
        { expected: aggregateReport.reportDigest, actual: value.inputs.aggregateReportDigest },
      );
    }
  }
