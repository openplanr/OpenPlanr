...
export type EvaluationCapability =
  | 'file-read' | 'file-write' | 'command-execute' | 'network' | 'browser' | 'credential' | 'git' | 'publish' | 'deploy';
export type EvaluationGraderType = 'deterministic-schema' | 'deterministic-behaviour' | 'live-model-judgement';
export type EvaluationAbsenceCode =
  | 'grader-missing' | 'grader-errored' | 'grader-unavailable' | 'host-unavailable'
  | 'fixture-unavailable' | 'budget-exceeded' | 'not-run';
export type EvaluationMetric =
  | 'trigger-precision' | 'trigger-recall' | 'journey-completion' | 'schema-validity'
  | 'asset-parity' | 'export-parity' | 'package-parity' | 'finding-severity-p0'
  | 'finding-severity-p1' | 'latency-regression' | 'cost-regression';
export type EvaluationUnwaivableMetric = 'schema-validity' | 'package-parity' | 'finding-severity-p0' | 'finding-severity-p1';
...
export type EvaluationWaiverReasonCode =
  | 'accepted-risk' | 'known-defect' | 'environment-limitation' | 'deferred-remediation' | 'external-dependency';
...
export interface EvaluationGraderSubject { skillId: string; skillSourceDigest: string }
export interface EvaluationObservationSubject { registration: Record<string, unknown>; skill: EvaluationGraderSubject }
...
export declare const EVALUATION_GATE_THRESHOLDS: Readonly<Record<EvaluationMetric, Readonly<EvaluationGateThreshold>>>;

export declare function assertEvaluationScenario<T>(value: T, label?: string): T;
export declare function assertEvaluationCorpus<T>(value: T, options?: { sourceDigests?: Record<string, string> | null; label?: string }): T;
...
export declare function assertEvaluationGraderIndependence<T>(registration: T, subject: EvaluationGraderSubject, label?: string): T;
...
export declare function assertEvaluationObservation<T>(value: T, options?: { subject?: EvaluationObservationSubject | null; label?: string }): T;
...
export declare function assertEvaluationWaiverApplicable<T>(waiver: T, options: { now: string; gatePolicyDigest?: string | null; scenarioDigest?: string | null; label?: string }): T;
export declare function assertSkillCertificationReceipt<T>(value: T, options?: { aggregateReport?: unknown; now?: string | null; label?: string }): T;
