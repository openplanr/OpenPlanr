...
function ensureDirectory(path) {
  if (existsSync(path)) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory())
      fail(
        'E_PLAN_REVIEW_STORAGE_UNSAFE',
        'Planning review custody contains an unsafe filesystem node.',
      );
    return;
  }
...
function receiptPath(value, receiptHash) {
  if (!/^sha256:[a-f0-9]{64}$/.test(receiptHash ?? ''))
    fail(
      'E_PLAN_REVIEW_RECEIPT_HASH_INVALID',
      'Planning review receipt hash must be exact SHA-256 custody.',
    );
...
function readRecord(path, expectedType = undefined) {
  ...
  try {
    ...
    fail('E_PLAN_REVIEW_STORAGE_INVALID', 'Planning review custody contains invalid JSON.');
  }
  assertPlanningReview(value);
  if (expectedType && value.recordType !== expectedType)
    fail('E_PLAN_REVIEW_STORAGE_INVALID', `Expected ${expectedType} planning review custody.`);
  return value;
}

function receiptFiles(value) {
  if (!existsSync(value.receiptDir)) return [];
  return readdirSync(value.receiptDir)
...
function receipts(value) {
  return receiptFiles(value).map((name) => {
    ...
    if (`${record.receiptHash.slice(7)}.json` !== name)
      fail(
        'E_PLAN_REVIEW_STORAGE_INVALID',
        'Planning review receipt filename does not match its immutable content hash.',
      );
    return record;
  });
}
...
function activeFiles(value) {
  ...
  return readdirSync(value.activeDir)
    .filter((name) => /^prr_[a-f0-9]{32}\.json$/.test(name))
    .sort();
}
...
function writeReceipt(path, receipt) {
  ...
  if (existsSync(path)) {
    ...
    if (readFileSync(path, 'utf8') !== bytes)
      fail(
        'E_PLAN_REVIEW_RECEIPT_IMMUTABLE',
        'Content-addressed planning review receipt already exists with divergent bytes.',
      );
    return;
  }
...
function summary(record, { replayed = false } = {}) {
  return {
    ...
    professionalSpecificationDigest:
      record.candidateRevisions.at(-1).professionalSpecificationDigest,
...
export function issuePlanningReviewOwnerDecisionCapability({
  ...
  if (!/^prr_[a-f0-9]{32}$/.test(runId ?? ''))
    fail(
      'E_PLAN_REVIEW_OWNER_AUTHORITY_INVALID',
      'Owner decision capability requires one exact planning-review run ID.',
    );
...
function ownerDecisionCapability(value) {
  const binding =
    value && typeof value === 'object' ? OWNER_DECISION_CAPABILITIES.get(value) : undefined;
  if (!binding) {
    fail(
...
export function preparePlanningReview({ projectRoot, featureRoot, mode, slug } = {}) {
  ...
  const result = {
    ...
    activeRunIds: existsSync(value.activeDir)
      ? activeFiles(value).map((name) => name.slice(0, -5))
      : [],
...
export function prepareStoredPlanningReviewOwnerDecision({ prepared, runId } = {}) {
  ...
  if (record.recordType === 'receipt')
    return { ...summary(record, { replayed: true }), allowedDecisions: [] };
  if (!['awaiting_owner', 'correction_required'].includes(record.state)) {
    fail(
      'E_PLAN_REVIEW_TRANSITION_INVALID',
      'Owner decision is unavailable until the consolidated review reaches an owner disposition boundary.',
    );
  }
  const allowedDecisions =
    record.state === 'awaiting_owner' && record.reviewOutcome === 'pass-candidate'
      ? ['approved', 'blocked']
      : ['blocked'];
  return { ...summary(record), allowedDecisions };
}

export function startStoredPlanningReview({
  prepared,
  runtime = 'unknown',
  runId = `prr_${randomUUID().replaceAll('-', '')}`,
  now = new Date().toISOString(),
} = {}) {
  if (!/^prr_[a-f0-9]{32}$/.test(runId))
    fail('E_PLAN_REVIEW_RUN_ID_INVALID', 'Planning review runId must use prr_<32 lowercase hex>.');
  if (!['claude-code', 'codex', 'cursor', 'unknown'].includes(runtime))
    fail('E_RUNTIME_INVALID', `Unsupported planning review runtime ${runtime}.`);
  const value = paths(prepared.featureRoot, prepared.projectRoot);
  ensureDirs(value);
  const roster = planningReviewerRoster(
    prepared.plan.professionalSpecification.declaredSpecialists,
  );
  const requested = createPlanningReview({
    runId,
    feature: prepared.plan.feature,
    plan: prepared.plan,
  ...
  return withLock(value.featureLock, () => {
    const exactReceipt = receipts(value).find(
      ({ candidateRevisions }) => candidateRevisions.at(-1).planDigest === prepared.plan.planDigest,
    );
    ...
    if (existsSync(target)) {
      ...
      const identity = (record) => ({
        ...
        reviewerRoster: record.reviewerRoster,
        candidateRevisions: record.candidateRevisions,
      });
      if (sha256Jcs(identity(prior)) === sha256Jcs(identity(requested)))
        return summary(prior, { replayed: true });
      fail(
        'E_PLAN_REVIEW_RUN_ID_CONFLICT',
        `Planning review run ${runId} already exists with different custody.`,
      );
    }
    const other = activeFiles(value);
    if (other.length)
      fail(
        'E_PLAN_REVIEW_ACTIVE',
        `Feature already has active planning review ${other[0].slice(0, -5)}.`,
      );
...
function load(value, runId) {
  ...
  if (receipt)
    return { record: receipt, active: null, receipt: receiptPath(value, receipt.receiptHash) };
  fail('E_PLAN_REVIEW_RUN_NOT_FOUND', `Planning review run ${runId} was not found.`);
}

export function advanceStoredPlanningReview({
  prepared,
  runId,
  event,
  now = new Date().toISOString(),
    ...
    if (prior) {
      if (prior.inputDigest !== inputDigest)
        fail(
          ...
          `Event ${normalized.eventId} replayed with divergent bytes.`,
        );
...
export function decideStoredPlanningReview({
  prepared,
  runId,
  capability,
  now = new Date().toISOString(),
  ...
  if (capability.runId !== runId)
    fail(
      'E_PLAN_REVIEW_OWNER_AUTHORITY_FOREIGN',
      'Owner decision capability belongs to a different planning-review run.',
    );
  const value = paths(prepared.featureRoot, prepared.projectRoot);
  ensureDirs(value);
  return withLock(value.featureLock, () => {
    ...
    if (loaded.record.recordType === 'receipt') {
      ...
      const exact =
        decision?.ownerId === binding.event.ownerId &&
        ...
        loaded.record.events.some(
          ({ eventId, inputDigest: digest }) =>
            eventId === binding.event.eventId && digest === inputDigest,
        );
      if (!exact)
        fail(
          'E_PLAN_REVIEW_OWNER_AUTHORITY_FOREIGN',
          'Owner decision capability does not match the immutable terminal receipt.',
        );
      return summary(loaded.record, { replayed: true });
    }
    if (binding.consumed)
      fail(
        'E_PLAN_REVIEW_OWNER_AUTHORITY_REPLAYED',
        'Owner decision capability was already consumed by a different terminal result.',
      );
    const candidate = loaded.record.candidateRevisions.at(-1);
    if (
      binding.event.expectedGeneration !== loaded.record.generation ||
      binding.event.candidateRevision !== candidate.revision ||
      binding.event.planDigest !== candidate.planDigest
    ) {
      fail(
        'E_PLAN_REVIEW_OWNER_AUTHORITY_FOREIGN',
        'Owner decision capability does not bind the current generation and exact plan candidate.',
      );
    }
...
export function readPlanningReviewReceipt({ prepared, receiptHash, requirePassed = true } = {}) {
  ...
  if (!existsSync(value.receiptDir))
    fail(
      'E_PLAN_REVIEW_RECEIPT_NOT_FOUND',
      'No planning review receipt custody exists for this feature.',
    );
  const path = receiptPath(value, receiptHash);
  if (!existsSync(path))
    fail('E_PLAN_REVIEW_RECEIPT_NOT_FOUND', `No planning review receipt matches ${receiptHash}.`);
  const receipt = readRecord(path, 'receipt');
  if (receipt.receiptHash !== receiptHash)
    fail(
      'E_PLAN_REVIEW_RECEIPT_INVALID',
      'Planning review receipt hash and file custody disagree.',
    );
  if (requirePassed && receipt.state !== 'passed')
    fail('E_PLAN_REVIEW_NOT_APPROVED', 'SHIP requires a terminal PASS planning review receipt.');
  if (
    receipt.feature.mode !== prepared.mode ||
    receipt.feature.slug !== prepared.slug ||
    receipt.candidateRevisions.at(-1).planDigest !== prepared.plan.planDigest
  ) {
    fail(
      'E_PLAN_REVIEW_STALE',
      'Planning review receipt does not match the exact current specification and task graph.',
    );
  }
  return receipt;
}
