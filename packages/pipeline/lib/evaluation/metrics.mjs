...
export const EVALUATION_FRICTION_CLASSES = Object.freeze(['permission-prompt', 'retry', 'clarification', 'typed-unavailable']);
...
function wholeNumber(value, label) {
  if (!Number.isSafeInteger(value) || value < 0) {
    fail('E_EVALUATION_METRIC_INVALID', `${label} must be a non-negative integer.`, 'Measurements are whole units; a float or a negative count is refused.', { actual: value ?? null });
  }
  return value;
}
...
export function basisPoints(part, whole) {
  ...
  if (part > whole) {
    fail('E_EVALUATION_METRIC_INVALID', 'A metric numerator exceeds its population.', 'A subset counter never exceeds the population it is measured against.', { part, whole });
  }
...
export function percentileMs(samples, percentile) {
  if (!Array.isArray(samples)) fail('E_EVALUATION_METRIC_INVALID', 'Latency samples must be an array.', 'Pass one integer millisecond sample per observation.');
  if (!Number.isSafeInteger(percentile) || percentile < 1 || percentile > 100) {
    fail('E_EVALUATION_METRIC_INVALID', 'A percentile must be a whole number between 1 and 100.', 'Use 50 or 95.', { percentile });
  }
...
export function estimateTokens(text) {
  if (typeof text !== 'string') fail('E_EVALUATION_METRIC_INVALID', 'A token estimate needs decoded text.', 'Pass the prompt or output text.');
...
export function deriveTriggerCounts(decisions) {
  if (!Array.isArray(decisions)) fail('E_EVALUATION_METRIC_INVALID', 'Trigger decisions must be an array.', 'Pass one decision per scenario.');
  let truePositives = 0;
  ...
  decisions.forEach((decision, index) => {
    ...
    if (!decision || typeof decision !== 'object' || Array.isArray(decision)) fail('E_EVALUATION_METRIC_INVALID', `${cursor} must be an object.`, 'Pass { expected, observed } per scenario.');
    const keys = Object.keys(decision).sort();
    if (keys.length !== 2 || keys[0] !== 'expected' || keys[1] !== 'observed') {
      fail('E_EVALUATION_METRIC_INVALID', `${cursor} must carry exactly expected and observed.`, 'Trigger decisions are closed; remove unknown fields.');
    }
...
export function aggregateMeasurements(samples) {
  if (!Array.isArray(samples)) fail('E_EVALUATION_METRIC_INVALID', 'Measurement samples must be an array.', 'Pass one sample per scenario.');
  const latencies = [];
  let inputTokens = 0;
  let outputTokens = 0;
  ...
  for (const [index, sample] of samples.entries()) {
    ...
    if (!sample || typeof sample !== 'object' || Array.isArray(sample)) fail('E_EVALUATION_METRIC_INVALID', `${cursor} must be an object.`, 'Pass the per-scenario measurement record.');
...
export function frictionCount({ permissionPrompts = 0, retries = 0, clarifications = 0, typedUnavailable = 0 } = {}) {
  return wholeNumber(permissionPrompts, 'permissionPrompts')
...
export function assertEvaluationBaseline(value, label = 'baseline') {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) {
    fail('E_EVALUATION_BASELINE_INVALID', `${label} must be one plain JSON object.`, 'Pass the parsed baseline record itself.');
  }
  const expected = ['kind', 'schemaVersion', 'frozen', 'scope', 'capturedAt', 'latencyMsP50', 'latencyMsP95', 'costEstimateCurrency', 'costEstimateMicros'].sort();
  const actual = Object.keys(value).sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('E_EVALUATION_BASELINE_INVALID', `${label} has missing or unknown fields.`, `Declare exactly: ${expected.join(', ')}.`, { missing: expected.filter((key) => !actual.includes(key)), unknown: actual.filter((key) => !expected.includes(key)) });
  }
  if (value.kind !== 'evaluation-baseline' || value.schemaVersion !== '1.0.0') {
    fail('E_EVALUATION_BASELINE_INVALID', `${label} carries an implicit or foreign baseline version.`, 'Declare kind "evaluation-baseline" and schemaVersion "1.0.0" explicitly.');
  }
  if (value.frozen !== true) fail('E_EVALUATION_BASELINE_INVALID', `${label}.frozen must be true.`, 'A regression is only meaningful against a frozen baseline.');
  if (typeof value.scope !== 'string' || !IDENTIFIER.test(value.scope)) {
    fail('E_EVALUATION_BASELINE_INVALID', `${label}.scope must name the corpus by its closed identifier.`, 'Use the same scope identifier the budget declares.');
  }
  if (typeof value.capturedAt !== 'string' || new Date(value.capturedAt).toISOString() !== value.capturedAt) {
    fail('E_EVALUATION_BASELINE_INVALID', `${label}.capturedAt must be one canonical RFC 3339 UTC timestamp.`, 'Use the exact form 2026-01-01T00:00:00.000Z.');
  }
  ...
  if (value.costEstimateCurrency !== 'USD') fail('E_EVALUATION_BASELINE_INVALID', `${label}.costEstimateCurrency must be USD.`, 'Cost is recorded in one currency.');
  if (value.latencyMsP95 < value.latencyMsP50) fail('E_EVALUATION_BASELINE_INVALID', `${label}.latencyMsP95 is below the p50 baseline.`, 'Capture both percentiles from the same frozen sample.');
  return value;
}
...
export function assertBaselineDigest(baseline, declaredDigest, label = 'baseline') {
  if (typeof declaredDigest !== 'string' || !DIGEST.test(declaredDigest)) {
    fail('E_EVALUATION_BASELINE_INVALID', `${label} digest must be an exact sha256 digest.`, 'Bind the baseline by digest, never by name.');
  }
  const actual = evaluationBaselineDigest(baseline);
  if (actual !== declaredDigest) {
    fail('E_EVALUATION_DIGEST_MISMATCH', `${label} no longer matches the digest the policy binds.`, 'Recapture the frozen baseline and reseal the budget and gate policy against it.', { expected: declaredDigest, actual });
  }
  return baseline;
}
