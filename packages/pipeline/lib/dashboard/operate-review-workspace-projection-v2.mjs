...
function assertCanonicalView(view, { allowUnavailable = false } = {}) {
  ...
  if (view.viewHash !== sha256Jcs(without(view, 'viewHash'))) {
    fail(
      ...
      'Review workspace source viewHash does not equal its canonical content.',
    );
  }
  if (!allowUnavailable && ['incompatible', 'corrupt'].includes(view.status)) {
    fail(
      'E_OPERATE_REVIEW_WORKSPACE_UNAVAILABLE',
      'An incompatible or corrupt experience view cannot project a Review workspace.',
      {
        status: view.status,
      },
    );
...
function assertReviewSource(source) {
  if (source?.kind === 'operating-review-read') {
    ...
    if (source.review.state !== 'pending' || source.review.disposition !== null) {
      fail(
        'E_OPERATE_BINDING_MISMATCH',
        'An operating-review-read workspace source must describe one pending Review.',
      );
    }
    return source;
  }
  if (source?.kind === 'operating-review-receipt') {
    assertProtocolArtifact('operating-review-receipt', source, {
  ...
  fail(
    'E_OPERATE_REVIEW_WORKSPACE_SOURCE_INVALID',
    'Review workspace source must be an exact operating-review-read or operating-review-receipt.',
  );
}
...
function assertTerminalReceiptIntegrity(receipt) {
  if (
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Terminal Review receipt state, disposition, or work dispositions diverge.',
    );
  }
  const applied = receipt.dispositionChoices.filter(
    (choice) =>
  ...
  if (applied.length !== 1) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Terminal Review receipt must retain exactly one applied advertised choice.',
    );
  }
  const [choice] = applied;
  if (
    choice.choiceHash !== sha256Jcs(choice.submitArguments) ||
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Terminal Review receipt applied choice lost its exact owner, scope, Review, or disposition binding.',
    );
  }
  const summary = receipt.summary;
  if (
    summary.decisionCount !== receipt.decisions.length ||
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Terminal Review receipt summary counts contradict its retained records.',
    );
  }
}
...
function assertSourceViewBinding(source, view) {
  ...
  if (
    !exactEventHead(source.eventHead, view.eventHead) ||
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Review source and experience view do not bind the exact Event head, actor, scope, domain, Cycle, and Review.',
      {
  ...
  if (
    ...
    !exactEventHead(sourceReadEventHead(source), source.eventHead)
  ) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Pending Review sourceReadEventHead must equal its source Event head.',
    );
  }
}
...
function assertExperienceEvidenceGraph(view) {
  ...
  for (const entry of view.evidence) {
    if (evidence.has(entry.evidenceRefId))
      fail(
        'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
        'Review workspace evidence identities must be unique.',
      );
    evidence.set(entry.evidenceRefId, entry);
  }
  for (const entry of view.claims) {
    if (claims.has(entry.claimId))
      fail(
        'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
        'Review workspace claim identities must be unique.',
      );
  ...
  for (const entry of view.evidence) {
    for (const claimId of entry.supportClaimIds) {
      if (!claims.get(claimId)?.supportEvidenceRefIds.includes(entry.evidenceRefId)) {
        fail(
          'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
          'Review workspace contains a broken reciprocal evidence support edge.',
          { evidenceRefId: entry.evidenceRefId, claimId },
        );
      }
    }
    for (const claimId of entry.contradictClaimIds) {
      if (!claims.get(claimId)?.contradictEvidenceRefIds.includes(entry.evidenceRefId)) {
        fail(
          'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
          'Review workspace contains a broken reciprocal evidence contradiction edge.',
          { evidenceRefId: entry.evidenceRefId, claimId },
        );
      }
    }
  ...
  for (const entry of view.claims) {
    for (const evidenceRefId of entry.supportEvidenceRefIds) {
      if (!evidence.get(evidenceRefId)?.supportClaimIds.includes(entry.claimId)) {
        fail(
          'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
          'Review workspace contains a broken reciprocal claim support edge.',
          { evidenceRefId, claimId: entry.claimId },
        );
      }
    }
    for (const evidenceRefId of entry.contradictEvidenceRefIds) {
      if (!evidence.get(evidenceRefId)?.contradictClaimIds.includes(entry.claimId)) {
        fail(
          'E_OPERATE_REVIEW_WORKSPACE_GRAPH_INVALID',
          'Review workspace contains a broken reciprocal claim contradiction edge.',
          { evidenceRefId, claimId: entry.claimId },
        );
      }
    }
  ...
  return (
    ...
    verify.gates.every(
      (gate) =>
        ...
        affirmative.filter(
          ({ outcomeId, status }) => outcomeId === gate.subjectId && status === gate.state,
        ).length === 1,
    ) &&
    affirmative.every(
      ({ outcomeId, status }) =>
...
function cycleProofIsIncomplete(cycle, outcomes) {
  const verify = cycle.stages.find(({ id }) => id === 'verify');
  const noProofRequired =
    cycle.persistentActionIds.length === 0 &&
...
export function deriveOperateSharedTruthSummaryV1(inputView) {
  ...
  const byStage = STAGE_IDS.map((id) => {
    ...
    if (stages.some((stage) => stage === undefined)) {
      fail(
        'E_OPERATE_REVIEW_WORKSPACE_SUMMARY_INVALID',
        'Every Cycle must contribute exactly one state for every canonical stage.',
        { stageId: id },
      );
    }
  ...
  const stageTotals = Object.fromEntries(
    STAGE_STATES.map((state) => [state, byStage.reduce((total, stage) => total + stage[state], 0)]),
  );

  const assignments = view.cycles.flatMap((cycle) => cycle.assignments);
  const typedAbsences =
    assignments.filter((assignment) => assignment.absence !== null).length +
  ...
  const evidenceLinked = countBy(
    view.evidence,
    (entry) => entry.supportClaimIds.length + entry.contradictClaimIds.length > 0,
  );
  const claimResolved = countBy(view.claims, (entry) =>
    ['supported', 'contradicted'].includes(entry.status),
  );
  ...
  const outcomeInsufficient = countBy(
    view.outcomes,
    (entry) => entry.status === 'insufficient-evidence',
  );
  const outcomeUnverified = view.outcomes.length - outcomeVerified - outcomeInsufficient;
  const cycleProofIncomplete = countBy(view.cycles, (cycle) =>
    cycleProofIsIncomplete(cycle, view.outcomes),
  );
  const omissionRestricted = view.omissions.reduce(
    (total, entry) => total + (entry.classification === 'restricted' ? entry.count : 0),
    0,
  );
  const omissionAbsent = view.omissions.reduce(
    (total, entry) => total + (entry.classification === 'restricted' ? 0 : entry.count),
    0,
  );

  const proofReasons = [];
  if (evidenceRestricted > 0 || omissionRestricted > 0)
    proofReasons.push('OPERATE_PROOF_RESTRICTED');
  ...
  if (outcomeUnverified + outcomeInsufficient + cycleProofIncomplete > 0)
    proofReasons.push('OPERATE_VERIFICATION_EVIDENCE_MISSING');
  if (view.evidence.length === 0 && view.claims.length > 0)
    proofReasons.push('OPERATE_EVIDENCE_ABSENT');
  const proofPositive =
    claimResolved +
    outcomeVerified +
    countBy(view.evidence, (entry) => entry.accessState === 'available');
  ...
  const unavailableSubjects = new Set(
    view.inbox
      .filter(({ unavailableReason }) => unavailableReason !== null)
      .map(({ subjectId }) => subjectId),
  );
  const blockingSubjects = new Set(
    view.inbox.filter(({ blocking }) => blocking).map(({ subjectId }) => subjectId),
  );
  const terminalSeats = countBy(assignments, (entry) =>
    TERMINAL_ASSIGNMENT_STATES.has(entry.state),
  );
  ...
  if (assignments.length !== terminalSeats + activeSeats + pendingSeats) {
    fail(
      'E_OPERATE_REVIEW_WORKSPACE_SUMMARY_INVALID',
      'Shared seat truth encountered an undeclared Assignment state.',
    );
  }
  const summary = {
    ...
    attention: {
      ...
      blocking: countBy(
        view.attention,
        (entry) =>
          blockingSubjects.has(entry.subjectId) ||
      ...
      unavailable: countBy(
        view.attention,
        (entry) =>
          unavailableSubjects.has(entry.subjectId) || !allowedSubjects.has(entry.subjectId),
      ),
    },
    omissions: {
      ...
      reasonCodes: [
        ...new Set(
          view.omissions.map(
            (entry) => `OPERATE_OMISSION_${entry.reason.replaceAll('-', '_').toUpperCase()}`,
          ),
        ),
      ].sort(),
    },
...
function selectReviewEvidenceGraph(source, view) {
  ...
  const selectedEvidenceIds = new Set(
    [...requestedEvidenceIds].filter((id) => evidenceById.has(id)),
  );
  ...
  for (const claim of view.claims) {
    if (
      [...claim.supportEvidenceRefIds, ...claim.contradictEvidenceRefIds].some((id) =>
  ...
  for (const claimId of selectedClaimIds) {
    ...
    if (!claim) continue;
    claim.supportEvidenceRefIds.forEach((id) => {
      if (evidenceById.has(id)) selectedEvidenceIds.add(id);
    });
    claim.contradictEvidenceRefIds.forEach((id) => {
      if (evidenceById.has(id)) selectedEvidenceIds.add(id);
    });
  }
  const missing = [...requestedEvidenceIds].filter((id) => !evidenceById.has(id)).sort();
  const evidence = [...selectedEvidenceIds]
    .map((id) => clone(evidenceById.get(id)))
    .sort(compareBy('evidenceRefId'));
  const claims = [...selectedClaimIds]
    .map((id) => clone(claimById.get(id)))
    .filter(Boolean)
...
function choiceConsequence(disposition) {
  const consequences = {
    ...
    cancelled:
      'Records owner cancellation and applies only the listed work dispositions; it does not authorize any follow-on effect.',
  };
...
function assertChoiceBinding(choice, source) {
  ...
  const actor = sourceActor(source);
  if (
    choice.choiceHash !== sha256Jcs(args) ||
    ...
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Review choice lost its exact hash, actor, scope, Cycle, or Review binding.',
...
function enrichedChoices(source) {
  ...
  return [...source.dispositionChoices].sort(compareBy('choiceId')).map((choice) => {
...
function matchingSubmitActions(source, view, choices) {
  ...
  const matches = view.allowedActions.filter(
    ({ subjectId, action }) =>
  ...
  const exactChoiceIds = new Set(
    matches.map(({ action }) => byArguments.get(sha256Jcs(action.arguments)).choiceId),
  );
  if (matches.length !== exactChoiceIds.size) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Review submit capability contains duplicate actions for one advertised choice.',
    );
  }
  return matches.sort((left, right) =>
    sha256Jcs(left.action.arguments).localeCompare(sha256Jcs(right.action.arguments)),
  );
}
...
function capabilityFor(source, view, choices) {
  ...
  if (view.status !== 'ready') {
    return {
      ...
      actions: [],
      reason: {
        code: `VIEW_${view.status.replaceAll('-', '_').toUpperCase()}`,
        message: `The verified view is ${view.status}.`,
      },
    };
  }
  if (matches.length !== choices.length) {
    return {
      ...
      actions: [],
      reason: {
        code: 'REVIEW_SUBMIT_NOT_AVAILABLE',
        message: 'No exact owner submit capability is available.',
      },
    };
  }
...
function sourceOmissions(view, graph, redactionCount) {
  const omissions = [];
  const restrictedIds = graph.evidence
  ...
  if (restrictedIds.length > 0)
    omissions.push({
      kind: 'restricted',
      subject: 'evidence',
      count: restrictedIds.length,
      subjectIds: restrictedIds,
      reasonCode: 'REVIEW_EVIDENCE_RESTRICTED',
    });
  if (graph.missing.length > 0)
    omissions.push({
      kind: 'absent',
      subject: 'evidence',
      count: graph.missing.length,
      subjectIds: graph.missing,
      reasonCode: 'REVIEW_EVIDENCE_NOT_VISIBLE',
    });
  ...
  if (redactionCount > 0)
    omissions.push({
      ...
      subjectIds: [],
      reasonCode: 'DISPLAY_SOURCE_REDACTED',
    });
...
function statusAndReasons(source, view, capability) {
  if (source.kind === 'operating-review-receipt')
    return { status: 'terminal', reasonCodes: ['REVIEW_TERMINAL'] };
  const reasons = [];
  if (view.status !== 'ready')
    reasons.push(`VIEW_${view.status.replaceAll('-', '_').toUpperCase()}`);
...
export function buildOperateReviewWorkspacePayloadV1(inputSource, inputView) {
  ...
  const recommendation =
    redacted.recommendationItems.length > 0
      ? { status: 'available', items: redacted.recommendationItems, absence: null }
      : {
          status: 'absent',
          items: [],
          absence: {
            code: 'REVIEW_RECOMMENDATION_ABSENT',
            message: 'No Chair recommendation is available.',
          },
        };
  const summaryCounts =
  ...
  if (sourceCycles.length !== 1) {
    fail(
      'E_OPERATE_BINDING_MISMATCH',
      'Review workspace requires one exact source Cycle trace owner.',
      {
