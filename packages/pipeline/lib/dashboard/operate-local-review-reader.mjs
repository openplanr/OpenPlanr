...
function safeMarkdownFile(cycleRoot, filename) {
	...
	try {
		...
		if (link.isSymbolicLink() || !link.isFile() || link.size > MAX_REPORT_BYTES)
			return null;
...
function readCycleFiles(operateRoot, cycleId) {
	...
	for (const filename of REVIEW_FILES) {
		...
		try {
			...
			if (bytes > MAX_REPORT_BYTES || totalBytes + bytes > MAX_CYCLE_BYTES)
				continue;
...
function sectionText(markdown, heading, limit = 360) {
	return cleanInlineMarkdown(sectionMarkdown(markdown, heading)).slice(
		0,
		limit,
	);
}
...
function fieldsFromMarkdown(markdown) {
	...
	for (const line of markdown.split(/\r?\n/u)) {
		...
		if (!match) continue;
		fields[cleanInlineMarkdown(match[1]).toLowerCase()] = cleanInlineMarkdown(
			match[2],
		);
	}
	return fields;
...
function parseDecisions(markdown) {
	return Object.freeze(
		headingEntries(sectionMarkdown(markdown, "decision queue"), "D").map(
			(entry) => {
		...
		tableRows(sectionMarkdown(markdown, "action plan"))
			...
			.map((row) =>
				Object.freeze({
					...
					state: /unassigned/iu.test(row["suggested owner"] ?? "")
						? "needs-owner"
						: "proposed",
				}),
			),
...
function parseNamedList(markdown, heading, prefix) {
	...
	for (const line of section.split(/\r?\n/u)) {
		...
		const named = new RegExp(`^(${prefix}\\d+)\\s+[—-]\\s+(.+)$`, "u").exec(
			label,
		);
		fallback += 1;
		entries.push(
...
function findingCount(markdown, prefix = "F") {
	return (markdown.match(new RegExp(`^###\\s+${prefix}\\d+\\b`, "gmu")) ?? [])
		.length;
}

function parseLenses(files, board) {
	const coverage = new Map(
		lensOutcomeRows(board).map((row) => [row.lens?.toLowerCase(), row]),
	);
	return Object.freeze(
		LENS_FILES.map(([name, filename]) => {
			...
			return Object.freeze({
				...
				signal: cleanInlineMarkdown(
					signalMatch?.[1] ?? (note ? "reported" : "missing"),
				),
				informed: row.informed || "",
				findings: note ? findingCount(note, prefix) : 0,
				recommendation: note
					? sectionText(note, "recommended next move", 520)
					: "",
			});
		}),
	);
...
function parseEvidence(files, decisions) {
	...
	for (const [filename, markdown] of Object.entries(files)) {
		...
		if (filename === "brief.md")
			candidates.push(sectionMarkdown(markdown, "source pointers"));
	}
	const seen = new Set();
	const evidence = [];
	const tokenPattern = /`([^`\n]{1,240})`/gu;
	for (const sourceText of candidates) {
		for (const match of sourceText.matchAll(tokenPattern)) {
			...
			const looksLikePath =
				/[/.]/u.test(reference) &&
				/(?:\/|\.(?:md|ts|tsx|js|mjs|yml|yaml|json))(?:[:#]|$)/u.test(
					reference,
				);
			...
			evidence.push(
				Object.freeze({
					...
					reference,
					kind:
						reference.includes("#") || /:\d/u.test(reference)
							? "source location"
							: "source file",
...
function reportCounts(markdown) {
	return Object.freeze({
		decisions: headingEntries(sectionMarkdown(markdown, "decision queue"), "D")
			.length,
...
function buildStructuredReview(cycleId, cycle) {
	...
	const presentFiles = REVIEW_FILES.filter(
		(filename) => typeof cycle.files[filename] === "string",
	);
	const expectedFiles = REVIEW_FILES.filter(
		(filename) => filename !== "brief.md",
	);
