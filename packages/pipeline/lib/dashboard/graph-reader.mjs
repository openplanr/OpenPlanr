...
//
// Supports the bounded scalar / list shape planr artifacts use:
//   key: "quoted string" | unquoted scalar | true/false | 123
//   key: [a, b, c]                          (inline flow array)
//   key:                                    (block array follows)
//     - item
//     - item
//   key:                                    (block array of flat maps)
//     - id: "AC-001"
//       statement: "..."
// Arbitrary nested objects remain outside this dependency-free reader.

const FLAT_MAP_ARRAY_KEYS = new Set(['acceptanceCriteria', 'preserve']);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (value === '') {
      ...
      while (j < lines.length && /^\s*-\s+/.test(lines[j])) {
        const rawItem = lines[j].replace(/^\s*-\s+/, '');
        const itemColon = rawItem.indexOf(':');
        if (!FLAT_MAP_ARRAY_KEYS.has(key) || itemColon === -1) {
          items.push(coerceScalar(rawItem));
          j++;
          continue;
        }
        const item = {};
