...
function assertCanonicalViewHash(view) {
  ...
  if (view.viewHash !== sha256Jcs(without(view, 'viewHash'))) {
    throw Object.assign(
      new Error('The experience projection failed its canonical integrity check.'),
      {
...
function assertCanonicalReplayCustody(view) {
  ...
  const invalid =
    ...
    view.cycles.some((cycle) => !exactJson(cycle.replayCheckpoint, checkpoint)) ||
    (checkpoint !== null &&
      ((exactCheckpoint &&
  ...
  if (invalid) {
    throw Object.assign(
      new Error('The experience projection failed its canonical replay custody check.'),
      { code: 'OPERATE_PROJECTION_INVALID' },
    );
  }
}
...
function exactInboxAction(view, item) {
  ...
  if (item.kind === 'decision') {
    return (
      ...
      action.arguments.workDispositions.some(
        (entry) => entry?.entityType === 'operating-decision' && entry.entityId === item.subjectId,
      )
    );
  }
  if (item.kind === 'approval') {
    ...
    const projected = view.actions.find((entry) => entry.actionId === item.subjectId);
    return (
      action.tool === 'operate.action.approve' &&
      action.arguments?.decision === 'approved' &&
      (rollback === undefined || item.itemId === `approval:rollback:${rollback.rollbackPlanId}`) &&
...
function exactInboxNavigation(view, item) {
  ...
  const candidates = view.allowedActions.filter(
    (entry) =>
      entry.subjectId === locator.reviewId && sha256Jcs(entry.action) === locator.readActionDigest,
  );
  ...
  if (item.kind === 'decision') {
    const reviewChoices = view.allowedActions.filter(
      (entry) =>
        entry.subjectId === locator.reviewId && entry.action.tool === 'operate.review.submit',
    );
    return (
      ...
      reviewChoices.some(
        ({ action: choice }) =>
          ...
          choice.arguments.workDispositions.some(
            (entry) =>
              entry?.entityType === 'operating-decision' && entry.entityId === item.subjectId,
          ),
      )
    );
  }
  ...
  for (const item of view.inbox) {
    ...
    for (const evidence of item.evidence) {
      ...
      const expectedRelation =
        ownerEvidence?.claimStatus === 'contradicted'
          ? 'contradiction'
...
export function assertOperateExperienceTransportView(view) {
  ...
  if (
    ...
    throw Object.assign(new Error('The experience projection contains an unsafe actor identity.'), {
      code: 'OPERATE_PROJECTION_INVALID',
    });
  }
  return view;
...
function safeReadJson(path, maxBytes) {
  ...
  if (!stats.isFile()) throw new Error('Experience projection is not a regular file.');
  if (stats.size > maxBytes) throw new Error('Experience projection exceeds the local read limit.');
...
export function readOperateExperienceProjection(
  planrDir,
  { maxBytes = DEFAULT_MAX_BYTES, relativePath = PROJECTION_RELATIVE_PATH } = {},
) {
  ...
  try {
    const view = buildOperateExperienceTransportView(safeReadJson(path, maxBytes));
    return Object.freeze({
      available: true,
      readOnly: true,
      status: view.status,
  ...
  return view.history.filter(
    (entry) =>
      ...
      entry.deepLinks.some(
        (deepLink) =>
          resolveOperateExperienceSearchDestination(view, deepLink)?.subjectId === actionId,
      ),
  );
}
...
function validExecutiveBoardChairSynthesis(chair) {
  ...
  return Array.isArray(chair.unresolvedGaps) && chair.unresolvedGaps.every(validExecutiveBoardGap);
}
...
function exactDisplayBinding(view, binding, surface, subjectId) {
  return (
    ...
    (surface !== 'cycle' || (binding.subjectId === subjectId && binding.cycleId === subjectId)) &&
    (surface !== 'actions' ||
...
function exactAuditDisplayBinding(view, binding, { surface, subjectId, cycleId, query, format }) {
  if (
    !exactBinding(view, binding) ||
    binding.generatedAt !== view.generatedAt ||
    binding.viewHash !== view.viewHash ||
  ...
  if (surface === 'export')
    return ['json', 'html'].includes(format) && binding.query === null && binding.format === format;
...
function linkedPersistentRows(records, links, entityKind, idField) {
  return records
    .filter((record) =>
      links.some((link) => link.entityKind === entityKind && link.entityId === record[idField]),
    )
    .map((record) => ({
      kind: entityKind,
      ...
      state: record.state,
      relations: links
        .filter((link) => link.entityKind === entityKind && link.entityId === record[idField])
...
function exactPersistentLinkCoverage(ledger, cycleLinks, cycleId, view) {
  ...
  if (
    ledger.cycleLinks.some((link, index) =>
      ledger.cycleLinks.slice(0, index).some((candidate) => sameLink(candidate, link)),
    )
  )
    return false;
  if (cycleLinks.some((link) => link.cycleId !== cycleId)) return false;
  if (
    cycleLinks.some((link) => !ledger.cycleLinks.some((candidate) => sameLink(candidate, link)))
  ) {
    return false;
  }
  if (
  ...
  for (const { entityKind, idField, record } of records) {
    ...
    const sourceLinks = ledger.cycleLinks.filter(
      (link) =>
        link.entityKind === entityKind && link.entityId === entityId && link.relation === 'source',
    );
    if (sourceLinks.length !== 1 || sourceLinks[0].cycleId !== record.sourceCycleId) return false;
...
function outcomeCarriesRestrictedAffirmativeProof(outcome) {
  return ['succeeded', 'failed'].includes(outcome.status) && outcomeCarriesRestrictedProof(outcome);
}

function entityDeepLink(view, kind, subjectId) {
  if (kind === 'cycle')
    return view.cycles.find((entry) => entry.cycleId === subjectId)?.deepLink ?? null;
  if (kind === 'assignment') {
    return (
  ...
  if (kind === 'evidence')
    return view.evidence.find((entry) => entry.evidenceRefId === subjectId)?.deepLink ?? null;
  if (kind === 'claim')
    return view.claims.find((entry) => entry.claimId === subjectId)?.deepLink ?? null;
  if (kind === 'outcome')
    return view.outcomes.find((entry) => entry.outcomeId === subjectId)?.deepLink ?? null;
  if (kind === 'learning') {
    const outcomeId = view.learnings.find((entry) => entry.learningId === subjectId)?.outcomeId;
    return view.outcomes.find((entry) => entry.outcomeId === outcomeId)?.deepLink ?? null;
  }
  return null;
}
...
export function resolveOperateExperienceSearchDestination(view, value) {
  ...
  if (parts.length === 2) {
    subjectId = parts[1];
    if (!subjectId || subjectId === '.' || subjectId === '..' || /[/\\]/.test(subjectId))
      return null;
  }
  const route = parts[0];
  if (route === 'cycles') {
    if (subjectId === null) return { route, surface: 'cycles', subjectId: null };
  ...
  if (route === 'evidence') {
    if (
      ...
      !view.claims.some((entry) => entry.claimId === subjectId) &&
      !view.rationale.some(
        (entry) => entry.subjectId === subjectId || entry.evidenceRefIds.includes(subjectId),
      )
    )
      return null;
  ...
  if (route === 'outcomes') {
    if (subjectId === null) return { route, surface: 'outcomes', subjectId: null };
    if (!view.outcomes.some((entry) => entry.outcomeId === subjectId)) return null;
    return { route, surface: 'outcome', subjectId };
  }
  if (route === 'today' && subjectId === null) return { route, surface: 'today', subjectId: null };
  if (route === 'history' && subjectId === null)
    return { route, surface: 'history', subjectId: null };
  if (route === 'actions') {
    if (subjectId === null) return { route, surface: 'actions', subjectId: null };
...
function exactSearchDeepLink(view, candidates) {
  for (const candidate of candidates) {
    if (resolveOperateExperienceSearchDestination(view, candidate)) return candidate;
  }
  return null;
}
...
function surfaceData(
  ...
  if (surface === 'inbox') {
    if (
      ...
      return null;
    const items =
      subjectId === null ? view.inbox : view.inbox.filter((item) => item.itemId === subjectId);
  ...
  if (surface === 'outcome') {
    const outcome = view.outcomes.find((entry) => entry.outcomeId === subjectId);
    return outcome
      ? {
          outcome,
          learnings: view.learnings.filter((entry) => entry.outcomeId === subjectId),
        }
      : null;
  }
  if (surface === 'history') return { history: view.history, replay: view.replay };
...
function searchView(view, query) {
  ...
  const rows = [
    ...view.attention.map((entry) => ({
      ...
      state: entry.state,
      deepLink: exactSearchDeepLink(view, [entityDeepLink(view, entry.kind, entry.subjectId)]),
    })),
    ...view.domainMetrics
    ...
    ...view.evidence
      .filter((entry) => entry.accessReason === null && entry.accessState === 'available')
      .map((entry) => ({
        kind: 'evidence',
    ...
    ...view.claims
      .filter((entry) => entry.accessReason === null && entry.statement !== null)
      .map((entry) => ({
        kind: 'claim',
    ...
    ...view.rationale.map((entry) => ({
      ...
      deepLink: exactSearchDeepLink(view, [
        ...
        ...entry.evidenceRefIds.map((id) => entityDeepLink(view, 'evidence', id)),
      ]),
    })),
    ...
    ...view.learnings.map((entry) => ({
      ...
      summary: 'Learning',
      state: 'recorded',
      deepLink: exactSearchDeepLink(view, [entityDeepLink(view, 'learning', entry.learningId)]),
    })),
...
function exportValue(view) {
  return {
    ...
    actions: view.actions.map(({ ownerActorId: _ownerActorId, ...entry }) => entry),
    evidence: view.evidence,
    claims: view.claims,
    rationale: view.rationale,
    outcomes: view.outcomes,
...
function exactEvidenceClaimCustody(view) {
  const evidence = new Map(view.evidence.map((entry) => [entry.evidenceRefId, entry]));
...
function exactOutcomeCustody(view) {
  if (!hasUniqueIds(view.outcomes, 'outcomeId') || !hasUniqueIds(view.learnings, 'learningId'))
    return false;
...
function exactOwnedAuditDeepLink(view, value, surface, subjectId) {
  const destination = resolveOperateExperienceSearchDestination(view, value);
  return destination?.surface === surface && destination.subjectId === subjectId ? value : null;
}

function sanitizeAuditNode(view, value, { omitUnsafeDeepLink = false } = {}) {
  if (Array.isArray(value))
    return value.map((entry) => sanitizeAuditNode(view, entry, { omitUnsafeDeepLink }));
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (key === 'error' && child !== null && typeof child === 'object') {
      result[key] = sanitizeAuditNode(view, { ...child, context: {} }, { omitUnsafeDeepLink });
      continue;
    }
...
function sanitizeAuditExportNode(view, value) {
  if (Array.isArray(value)) return value.map((entry) => sanitizeAuditExportNode(view, entry));
  if (value === null || typeof value !== 'object') return value;
  const result = {};
  for (const [key, child] of Object.entries(value)) {
    if (
...
function canonicalizeAuditExportValue(value) {
  if (Array.isArray(value)) return value.map((entry) => canonicalizeAuditExportValue(entry));
  if (value === null || typeof value !== 'object') return value;
...
function closeEvidenceLinks(view, data) {
  return {
    ...
    evidence: data.evidence.map((entry) => ({
      ...entry,
      deepLink: exactOwnedAuditDeepLink(view, entry.deepLink, 'evidence', entry.evidenceRefId),
      causalLinks: entry.causalLinks.map((link) => ({
        ...link,
        deepLink:
          link.kind === 'outcome'
            ? exactOwnedAuditDeepLink(view, link.deepLink, 'outcome', link.subjectId)
            : null,
      })),
    })),
    claims: data.claims.map((entry) => ({
      ...entry,
      deepLink: exactOwnedAuditDeepLink(view, entry.deepLink, 'evidence', entry.claimId),
      causalLinks: entry.causalLinks.map((link) => ({
        ...link,
        deepLink:
          link.kind === 'outcome'
...
function closeOutcomeLinks(view, data) {
  const closeOutcome = (entry) => ({
    ...entry,
    deepLink: exactOwnedAuditDeepLink(view, entry.deepLink, 'outcome', entry.outcomeId),
    decision: entry.decision === null ? null : { ...entry.decision, deepLink: null },
    execution: entry.execution.map((result) => ({ ...result, deepLink: null })),
    rollback: entry.rollback.map((result) => ({ ...result, deepLink: null })),
  });
  if ('outcome' in data) return { ...data, outcome: closeOutcome(data.outcome) };
  return {
    ...data,
...
function searchResultCandidateDeepLink(view, entry) {
  if (entry.kind === 'action') {
    return (
      view.actions.find((item) => item.actionId === entry.subjectId)?.deepLink ?? entry.deepLink
    );
  }
  if (entry.kind === 'cycle') {
    return view.cycles.find((item) => item.cycleId === entry.subjectId)?.deepLink ?? entry.deepLink;
  }
  if (entry.kind === 'assignment') {
    for (const cycle of view.cycles) {
      const assignment = cycle.assignments.find((item) => item.assignmentId === entry.subjectId);
      if (assignment) {
    ...
    results: data.results.map((entry) => {
      ...
      if (candidate !== undefined) {
        if (entry.kind === 'evidence' || entry.kind === 'claim') {
          deepLink = exactOwnedAuditDeepLink(view, candidate, 'evidence', entry.subjectId);
        } else if (entry.kind === 'outcome') {
          deepLink = exactOwnedAuditDeepLink(view, candidate, 'outcome', entry.subjectId);
        } else if (entry.kind === 'action') {
          deepLink = exactOwnedAuditDeepLink(view, candidate, 'action', entry.subjectId);
        } else if (entry.kind === 'cycle') {
          deepLink = exactOwnedAuditDeepLink(view, candidate, 'cycle', entry.subjectId);
        } else if (entry.kind === 'assignment') {
          const destination = resolveOperateExperienceSearchDestination(view, candidate);
          if (destination?.surface === 'cycle' && destination.subjectId) {
            deepLink = exactOwnedAuditDeepLink(view, candidate, 'cycle', destination.subjectId);
          }
        }
      }
  ...
  return {
    ...
    history: data.history.map((entry) => ({
      ...
      deepLinks: entry.deepLinks.filter((deepLink) => {
        const destination = resolveOperateExperienceSearchDestination(view, deepLink);
        ...
        return destination?.surface === 'outcome' && destination.subjectId === entry.entityId;
      }),
    })),
  };
}
...
function closeAuditLinks(view, surface, data) {
  ...
  if (surface === 'outcomes' || surface === 'outcome') return closeOutcomeLinks(view, data);
...
export function selectOperateExperienceSurface(
  ...
  if ((surface === 'cycle' || surface === 'outcome' || surface === 'action') && !subjectId) {
    return safeError('OPERATE_SUBJECT_REQUIRED', 'This operating view requires a subject.', 400);
  }
  ...
  const reasonCodes = STATUS_REASON[view.status] ?? ['OPERATE_PROJECTION_UNAVAILABLE'];
  return Object.freeze(
...
export function selectOperateExperienceAuditDisplaySurface(
  ...
  try {
    ...
    if (surface === 'export') {
      ...
      payload.data = closeAuditLinks(
        view,
        surface,
        sanitizeAuditNode(view, payload.data, {
...
export function selectOperateExperienceDisplaySurface(
  view,
  { surface = 'today', binding, subjectId = null, cycleId = null } = {},
) {
  try {
    view = canonicalTransportView(view);
  } catch {
  ...
  const exactSubjectId = surface === 'cycle' || surface === 'inbox' ? subjectId : null;
  if (!binding || !exactDisplayBinding(view, binding, surface, exactSubjectId)) {
    return safeError(
      'OPERATE_BINDING_MISMATCH',
      'The requested operating view is unavailable for this actor and scope.',
      403,
 ...
 */
export function selectOperateInboxItemDisplaySurface(view, { binding, subjectId = null } = {}) {
 ...
 */
export function selectOperateCycleWorkspace(view, cycleRead, { binding, subjectId } = {}) {
  let truthSummary;
  try {
    view = canonicalTransportView(view);
    truthSummary = transportTruthSummary(view);
  ...
  const expectedLinks = ledger.cycleLinks.filter((link) => link.cycleId === subjectId);
  const canonicalStages = ['observe', 'understand', 'decide', 'govern', 'act', 'verify', 'learn'];
  const route = cycle ? resolveOperateExperienceSearchDestination(view, cycle.deepLink) : null;
  ...
  const findings = linkedPersistentRows(ledger.findings, links, 'finding', 'findingId');
  const decisions = linkedPersistentRows(ledger.decisions, links, 'decision', 'decisionId');
  const actions = linkedPersistentRows(ledger.actions, links, 'action', 'actionId');
  const assignments = cycle.assignments.map((assignment) => {
    const destination = resolveOperateExperienceSearchDestination(view, assignment.deepLink);
    return {
      ...assignment,
      deepLink:
  ...
  const outcomes = view.outcomes
    ...
    .map((outcome) => {
      const destination = resolveOperateExperienceSearchDestination(view, outcome.deepLink);
      return {
        ...outcome,
        deepLink:
          destination?.surface === 'outcome' && destination.subjectId === outcome.outcomeId
            ? outcome.deepLink
            : null,
      };
    });
  const outcomeIds = outcomes.map((entry) => entry.outcomeId);
  const learnings = view.learnings.filter((entry) => outcomeIds.includes(entry.outcomeId));
...
export function selectOperateCycleDisplayWorkspace(view, cycleRead, options = {}) {
 ...
 */
export function selectOperateReviewWorkspace(view, reviewSource, { binding, subjectId } = {}) {
  let payload;
  try {
...
export function selectOperateReviewDisplayWorkspace(view, reviewSource, options = {}) {
...
export function selectOperateActionWorkspace(
  ...
  const action = view.actions.find((entry) => entry.actionId === subjectId);
  const route = action ? resolveOperateExperienceSearchDestination(view, action.deepLink) : null;
  const outcome = view.outcomes.find((entry) => entry.actionId === subjectId) ?? null;
  const learnings = outcome
    ? view.learnings.filter((entry) => entry.outcomeId === outcome.outcomeId)
    : [];
  const allowedActions = view.allowedActions.filter((entry) => entry.subjectId === subjectId);
...
export function decodeOperateExperienceCheckpoint(value) {
  ...
  try {
    ...
    if (
      ...
      parsed.sequence < 0 ||
      (parsed.sequence === 0 ? parsed.hash !== null : !/^sha256:[a-f0-9]{64}$/.test(parsed.hash)) ||
