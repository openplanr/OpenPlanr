 ...
 * @param {() => object} [options.getExperience] access-safe experience reader
 * @param {(change:{previous:object|null,next:object}) => void} [options.onExperience]
 *        called only when the validated experience head/view/status changes
 * @param {object|null} [options.initialExperience] initial experience read result
 ...
 *             recomputeExperience: () => object|null }}
 */
export function createWatcher(planrDir, options = {}) {
  ...
  let experienceSnapshot = options.initialExperience ?? null;
  ...
  function experienceIdentity(result) {
    const view = result?.view;
    return JSON.stringify({
      available: result?.available ?? false,
      status: result?.status ?? 'absent',
      reasonCodes: result?.reasonCodes ?? [],
      eventHead: view?.eventHead ?? null,
      viewHash: view?.viewHash ?? null,
    });
  }

  /** Re-read only the public access-safe view; never opens Artifact bodies. */
  function recomputeExperience() {
    if (!getExperience) return null;
    let next;
    try {
      next = getExperience();
    } catch {
      // A half-written projection is not broadcast. Its stable invalid state is
      // emitted by the reader once the write settles and the next watch fires.
      return null;
    }
    if (experienceIdentity(experienceSnapshot) === experienceIdentity(next)) return null;
    const change = { previous: experienceSnapshot, next };
  ...
  function flush() {
    ...
    const experienceChange = recomputeExperience();
    if (experienceChange) onExperience(experienceChange);
  }
  ...
  return { start, stop, recompute, recomputeExperience };
}
