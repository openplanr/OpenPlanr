...
} from "../artifact/local-document.mjs";
import { assertDesignDocument } from "../protocol/design-contracts.mjs";
...
export const DESIGN_VIEWS = Object.freeze([
	"canvas",
	"prototype",
	"walkthrough",
]);
...
export function recoverDesignPublication(
	root,
	{ ownsRenderLock = false } = {},
) {
	...
	try {
		...
		if (pointer?.revision === journal.revision)
			...
			atomicJson(
				manifestPath,
				readJson(
					join(root, ".design/revisions", pointer.revision, "render.json"),
				).manifest,
			);
		} else
			throw new Error(
...
export function loadDesignDocument(file, { readSource } = {}) {
  ...
  const document = assertDesignDocument(checked ? JSON.parse(checked.value.toString('utf8')) : readJson(path));
  ...
  const { path, root, document } = loadDesignDocument(file, { readSource });
	...
	for (const screen of document.screens)
		for (const source of [
			...
			for (const item of [...(source.styles ?? []), ...(source.scripts ?? [])])
				sources.add(item);
		}
	const missing = [];
  ...
  const reviewContext = loadReviewContext(root, document, { readSource });
	...
	const sourceContents = new Map(
		inspected.sources.map((source) => [
			source,
      readSource ? readSource(source, root).value : readFileSync(resolveLocalDocumentFile(root, source)),
		]),
	);
	for (const variant of document.variants.filter(
		...
		for (const screenId of document.screenOrder) {
			...
			const bundled = bundleLocalDocument({
				...
				sharedStyles: document.designSystem?.tokens
					? [document.designSystem.tokens]
					: [],
        screenId,
        readSource,
        passive,
      ...
      if (!passive) bundled.html = bundled.html.replace("</body>", `${navigate}</body>`);
			for (const name of bundled.files) {
				...
				if (!sourceContents.has(name))
					sourceContents.set(
						name,
            readSource ? readSource(name, root).value : readFileSync(resolveLocalDocumentFile(root, name)),
					);
			...
			for (const frame of document.frames) {
				fingerprints.push(reviewFingerprints({ document, context: reviewContext, screen, variant, frame, sourceDigests: bundled.sourceDigests }));
	...
	const activeArtifactId = [...artifacts].sort((a, b) =>
	...
	return {
		...inspected,
		reviewContext, contextDigest, fingerprints,
		envelope,
		entries,
		revision,
		lint,
...
export function currentDesign(file) {
	...
	return {
		...
		directory,
		file: resolve(file),
		verification: readJson(
			join(root, ".design/verification", `${pointer.revision}.json`),
...
export async function renderDesignDocument(
	...
	try {
		...
		const manifest = existsSync(directory)
			...
			: buildManifest({
					...
					screens: prepared.document.screenOrder.map(
						(id) =>
							prepared.document.screens.find((item) => item.id === id).title,
					),
					iterations:
		...
		const record = {
			reviewContext: prepared.reviewContext, contextDigest: prepared.contextDigest, fingerprints: prepared.fingerprints,
		...
		if (!existsSync(directory)) {
			...
			writeFileSync(
				join(temporary, "design-document.json"),
