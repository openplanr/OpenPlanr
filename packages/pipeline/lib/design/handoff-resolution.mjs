import {
	canonicalizeJson,
	sha256Hex,
} from "../protocol/canonical-json.mjs";
...
function issue(
	...
	severity,
	message,
	{ pinId, revisionId, recoveryAction } = {},
) {
	return {
		code,
		severity,
...
function validatePin(pin) {
	...
	if (
		pin.screenId !== undefined &&
		(typeof pin.screenId !== "string" || !pin.screenId)
	)
...
function scopedMetadata(metadata, pin, duplicatePinIds, diagnostics) {
	...
	if (
		category === undefined &&
		Object.hasOwn(metadata.categories ?? {}, pin.id)
	) {
	...
	if (
		...
		Object.hasOwn(metadata.dispositions ?? {}, pin.id)
	) {
		if (duplicatePinIds.has(pin.id))
			diagnostics.push(
				issue(
	...
	const dispositionValue =
		typeof disposition === "string" ? disposition : disposition?.disposition;
...
export function compileDesignHandoffResolution(input) {
	...
	const items = [...pins]
		...
		.map((pin) => {
			...
			const current =
				pin.stale !== true &&
				(!source.currentReviewOf || pin.reviewOf === source.currentReviewOf);
	...
	return {
		...
		status,
		complete:
			source.historyComplete !== false &&
			!["blocked", "stale"].includes(status),
		items,
		implementationScope: items
			.filter((item) => item.implementationScope)
			.map((item) => item.id),
		diagnostics,
	};
}
