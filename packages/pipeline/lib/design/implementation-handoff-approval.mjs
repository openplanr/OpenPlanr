...
function lifecycleConflict(message) {
	return Object.assign(new Error(message), { code: "E_IMPLEMENTATION_HANDOFF_CONFLICT", statusCode: 409 });
}

function lifecycleForbidden(message) {
	return Object.assign(new Error(message), { code: "E_IMPLEMENTATION_HANDOFF_FORBIDDEN", statusCode: 403 });
}
...
function archivePaths(root, value) {
	...
	return { directory, json: join(directory, "handoff.json"), markdown: join(directory, "handoff.md") };
}
...
export function readImplementationHandoffVersion(root, identity) {
	...
	const matches = listImplementationHandoffHistory(root).filter((value) =>
		value.id === identity.id && value.version === identity.version &&
...
export function listImplementationHandoffHistory(root) {
	...
	return readdirSync(directory, { withFileTypes: true })
		...
		.map((entry) => assertImplementationHandoffProjection(readJson(join(directory, entry.name, "handoff.json"))))
...
export function readImplementationHandoffLifecycle(root) {
	...
	const events = existsSync(paths.events)
		? readdirSync(paths.events).filter((name) => name.endsWith(".json")).map((name) => readJson(join(paths.events, name))).sort((left, right) => left.at.localeCompare(right.at) || left.eventId.localeCompare(right.eventId))
		: [];
...
export function previewImplementationHandoffApproval(root) {
	...
	const alreadyCurrent = lifecycle.current?.status === "approved" &&
		lifecycle.current.id === draft.id && lifecycle.current.version === draft.version &&
...
function assertExpectedDraft(draft, request, currentBasis) {
	if (!Number.isInteger(request.expectedVersion) || request.expectedVersion < 1 || !DIGEST.test(request.expectedContentDigest ?? ""))
...
function assertUniqueVersion(root, value) {
	const existing = listImplementationHandoffHistory(root).find((item) => item.id === value.id && item.version === value.version);
...
export function approveImplementationHandoff(root, request, options = {}) {
	...
	if (existing) {
		...
		if (lifecycle.current?.status === "approved" && lifecycle.current.id === approved.id && lifecycle.current.version === approved.version && lifecycle.current.contentDigest === approved.contentDigest)
			return { package: existing, current: lifecycle.current, event: lifecycle.events.find((item) => item.type === "approved" && item.package.contentDigest === approved.contentDigest) ?? null, repeated: true };
...
export function supersedeImplementationHandoff(root, replacement, request, options = {}) {
	...
	if (checked.status !== "draft") throw new TypeError("A superseding package must still be a draft.");
	const requestId = normalizeRequestId(request?.requestId);
	const at = timestamp(options.clock);
	const actor = authorizeActor(options.actor, APPROVE_CAPABILITY, at);
	const lifecycle = readImplementationHandoffLifecycle(root);
	...
	if (lifecycle.current.id === checked.id && lifecycle.current.version === checked.version && lifecycle.current.contentDigest === checked.contentDigest) return null;
	...
	const pointer = pointerFor(prior, "superseded", eventId, { supersededBy: signature.replacement, updatedAt: at });
	writeLifecycleJournal(root, {
		kind: "openplanr-design-implementation-handoff-lifecycle-publication",
		schemaVersion: "1.0.0",
		event,
...
export function regenerateImplementationHandoffDraft(root, input, request, options = {}) {
	...
	if (existingEvent) {
		...
		const draft = readImplementationHandoffDraft(root, { allowMissing: false });
		if (draft.id !== candidate.id || draft.version !== candidate.version || draft.contentDigest !== candidate.contentDigest)
	...
	const maximum = Math.max(0, currentDraft?.version ?? 0, ...lifecycle.history.map((item) => item.version));
...
export function compareImplementationHandoffVersions(root, leftIdentity, rightIdentity) {
	const resolveValue = (identity) => identity === "draft"
