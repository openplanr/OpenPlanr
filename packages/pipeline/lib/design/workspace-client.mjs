...
import {
  DESIGN_WORKSPACE_API, DESIGN_WORKSPACE_VERSION, DESIGN_WORKSPACE_MAX_BYTES,
  DESIGN_WORKSPACE_MAX_EVENT_BYTES, DESIGN_WORKSPACE_CREATE_SCHEMA,
  DESIGN_WORKSPACE_REVISION_SCHEMA, DESIGN_WORKSPACE_EVENT_SCHEMA,
  DESIGN_WORKSPACE_SCHEMA, DESIGN_REVIEW_BUNDLE_SCHEMA, assertWorkspaceContract,
} from '../protocol/workspace-contracts.mjs';

import { assertDesignReviewBundle, assertDesignReviewMetadata } from '../protocol/review-experience-contracts.mjs';
...
export function newWorkspaceToken() { return encodeWorkspaceBytes(crypto.getRandomValues(new Uint8Array(32))); }
export function newWorkspaceId() { return encodeWorkspaceBytes(crypto.getRandomValues(new Uint8Array(18))); }
...
async function tokenMaterial(token, id, purpose) {
  if (!tokenPattern.test(token) || decodeWorkspaceBytes(token).length !== 32 || !idPattern.test(id)) throw new TypeError('Enter the complete generated access token.');
  const key = await crypto.subtle.importKey('raw', decodeWorkspaceBytes(token), 'HKDF', false, ['deriveBits']);
  return new Uint8Array(await crypto.subtle.deriveBits({ name: 'HKDF', hash: 'SHA-256', salt: encoder.encode(id), info: encoder.encode(`openplanr-design-workspace/v1/${purpose}`) }, key, 256));
}
...
export async function signWorkspaceValue(value, privateKey) {
  ...
  const signature = await crypto.subtle.sign(signatureAlgorithm, key, encoder.encode(canonicalizeJson(omitSignature(value))));
...
const revisionContext = (id, revision) => ({ type: 'revision', workspaceId: id, id: revision.id, epoch: revision.epoch, reviewOf: revision.reviewOf });
const eventContext = (id, event) => ({ type: 'event', workspaceId: id, id: event.id, epoch: event.epoch, revisionId: event.revisionId, reviewOf: event.reviewOf });
export function workspaceEnvelopeDigest(envelope) {
  return sha256Hex(canonicalizeJson({ schemaVersion: envelope.schemaVersion, artifacts: envelope.artifacts, viewer: envelope.viewer }));
}
async function wrapKeyring(custody, token = custody.token, epoch = custody.epoch, keys = custody.keys) {
  return seal({ keys, ownerPublicKey: custody.ownerPublicKey }, await tokenMaterial(token, custody.id, 'key-wrap'), keyringContext(custody.id, epoch));
}
async function prepareRevision(custody, bundle) {
  assertDesignReviewBundle(bundle);
  const header = { id: newWorkspaceId(), epoch: custody.epoch, reviewOf: workspaceEnvelopeDigest(bundle.envelope), createdAt: new Date().toISOString() };
...
async function request(access, suffix = '', { method = 'GET', body, fetchImpl = globalThis.fetch, owner = Boolean(access.ownerAuth), timeoutMs = 20_000 } = {}) {
  ...
  let response;
  try {
    response = await fetchImpl(`${normalizeWorkspaceBase(access.baseUrl)}${DESIGN_WORKSPACE_API}/${access.id}${suffix}`, { method, headers, body: body ? JSON.stringify(body) : undefined, redirect: 'error', cache: 'no-store', credentials: 'omit', signal: AbortSignal.timeout(timeoutMs) });
  } catch { throw new Error('Design sharing is unreachable. Check the connection and retry; the previous review is unchanged.'); }
  if (!response.ok) {
  ...
  try {
    for (;;) {
      ...
      length += value.length;
      if (length > limit) { await reader.cancel(); throw new RangeError('Design service returned an oversized response.'); }
      chunks.push(value);
    }
  } catch (error) {
...
export async function listWorkspaceRevisions(access, { after = '', ...options } = {}) {
  ...
  return request(access, `/revisions${after ? `?after=${encodeURIComponent(after)}` : ''}`, options);
}
export async function decryptWorkspaceRevision(access, revisionId = access.currentRevision, options = {}) {
...
export async function prepareWorkspaceMutation(custody, action, payload = undefined) {
  ...
  if (action === 'publish') body.revision = await prepareRevision(custody, payload);
    ...
    Object.assign(body, { epoch, reviewerAuthHash: sha256Hex(await deriveWorkspaceAuthentication(token, custody.id)), keyring });
...
export async function publishWorkspace(custody, bundle, options = {}) { await prepareWorkspaceMutation(custody, 'publish', bundle); return commitWorkspaceMutation(custody, options); }
export async function rotateWorkspace(custody, options = {}) { await prepareWorkspaceMutation(custody, 'rotate'); return commitWorkspaceMutation(custody, options); }
export async function manageWorkspace(custody, action, options = {}) { await prepareWorkspaceMutation(custody, action); return commitWorkspaceMutation(custody, options); }
...
export async function prepareWorkspaceEvent(access, payload, { revisionId = access.currentRevision, reviewOf = payload.reviewOf, signer } = {}) {
  ...
  const event = await signWorkspaceValue({ ...header, ...await seal(payload, decodeWorkspaceBytes(access.keys[access.epoch]), eventContext(access.id, header), DESIGN_WORKSPACE_MAX_EVENT_BYTES), publicKey }, identity.privateKey);
