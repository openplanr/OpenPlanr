...
- 7a56faf: `startDashboard` is available from `planr-pipeline/dashboard`, which loads the dashboard server without the package root. The root `startDashboard` export is deprecated; it still works and still loads the dashboard server when the root is imported.
...
- 080d66b: The packaged third-party notices now cover pako 2.1.0 (MIT and zlib), which is bundled into `templates/design/design-board-adapter.js`.
- c7a64c6: The Operate runtime Event reducer applies workflow, evidence-state, intelligence and authority Events through four handler modules under `lib/operate/runtime-foundation/`. Event handling and the `planr-pipeline/operate/runtime-v2` exports are unchanged.
...
- f93a973: Regenerate the Protocol projections and skill assets for the `planr-sprint` skill, and skip `sprints/SPRINT-NNN/` refinement note directories in the dashboard's native graph reader.
...
This is a large pre-1.0 release with breaking package-boundary changes. Consumers
of 0.42.0 must audit imports from the package root and `./protocol`, plus direct
lookups through the schema and registry wildcard exports. Consumers of
unpublished Operate 2.0 snapshots must also replace the seven retracted Operate
subpaths listed below. The main product change is that Operate is now a
skill-first review workflow instead of a governed CLI session.

### Added

- Seven evidence-bound Operate review lenses now cover strategy and finance,
  technology risk, product activation, growth and market, operations and
  customer, challenge, and chair synthesis. Each lens owns one Markdown report;
  the chair can propose follow-up actions only at a human approval gate, and the
  unified dashboard renders the resulting review workspace without requiring
  consumers to reconstruct private runtime state.
- A deterministic skill-evaluation runner executes versioned corpora and
  journeys against declared host profiles, records grader evidence, and applies
  quality thresholds, freshness checks, waiver rules, and regression gates.
  Certification is now reproducible evidence rather than a prose assertion.
- Protocol v1.4.0 now includes closed evaluation-corpus, journey, run, result,
  quality-gate, waiver, grader, and host-profile contracts. Unknown properties,
  inconsistent runtime identities, stale evidence, and incomplete coverage fail
  validation instead of being silently accepted.
- Release candidates can be proven from two deterministic package archives and
  an isolated real installation. Compatibility claims and release-ledger rows
  are derived from the exact package payload hash and installed export probes,
  preventing a version string or workspace symlink from standing in for the
  bytes consumers receive.
- Operate gained explicit role identity and rubric contracts, an executive-board
  projection, governed Assignment authority and result identities, input-custody
  proofs, and review actions whose evidence remains traceable into planning.

### Changed

- Operate role execution is now a lens-review skill graph. `planr-operate`
  prepares one shared evidence brief, dispatches the five advisory lenses, then
  the challenger and chair, and records a missing lens as absent rather than
  synthesizing its result. The former `/planr-pipeline:operate` governed
  packet/session workflow is retired, and `bin/planr-pipeline.mjs` intentionally
  exposes no Operate command. Consumers should invoke the installed skills and
  read their review artifacts instead of driving a session state machine.
- PLAN and SHIP instructions are context envelopes again: they gather the
  planning and repository evidence an implementation agent needs while
  deterministic transition, locking, Preserve, correction, receipt, and
  finalization rules remain in `lib/pipeline/`.
- The contract catalog and live-evidence contracts now belong to
  `lib/protocol/`; executive-board and trace-matrix types moved to protocol
  declarations; and the review-workspace display projection moved to
  `lib/dashboard/`. `lib/operate/` is no longer a second owner for those shared
  contracts and is public only through its declared package subpaths.
- Operate role IDs are distinct from reusable role kinds, with versioned rubrics
  and explicit review order. Custom reviewers can therefore share a lens kind
  without colliding with the canonical board identity.
- The Operate experience now renders through the unified dashboard composition,
  including empty, unavailable, legacy, and verified states under the same
  security headers and route boundary as the planning dashboard.
- Artifact-room routing now accepts the complete resource-key grammar used by
  encrypted room clients, avoiding false route rejections for valid identifiers.
- The v1.4 adapter registry no longer declares an Operate entrypoint or the
  retired operating-board, advisor-dispatch, and runtime-policy capabilities.
  Registries using those now-invalid closed-schema properties must be updated.
- Artifact-room events now require canonical event IDs and kind-specific closed
  payloads. Producers that relied on arbitrary IDs or extra payload fields must
  emit the typed event shape before upgrading.

### Fixed

- Assignment authority, role-result identity, evidence bindings, and runtime
  context claims now use one canonical digest path, so previews, replay,
  recovery, dashboard projections, and final results cannot disagree about the
  work or evidence they authorize.
- Board action creation preserves the exact human gate and planning bridge:
  review output can recommend work, but it cannot create planning artifacts or
  trigger PLAN, SHIP, deployment, publication, spending, or customer contact by
  implication.
- Custody pins were ratified after the protocol moves, and stale generated-file,
  adapter-manifest, extension-declaration, skill-documentation, dashboard, and
  package assertions were repaired so the repository suite validates the new
  ownership boundaries from a clean checkout.

### Removed

- **Breaking for consumers of unpublished Operate 2.0 snapshots:** the package
  no longer exports `planr-pipeline/operate/action-verification-v2`,
  `planr-pipeline/operate/operating-delta-v2`,
  `planr-pipeline/operate/operating-intelligence-state-v2`,
  `planr-pipeline/operate/operating-snapshots-v2`,
  `planr-pipeline/operate/operating-state-v2`,
  `planr-pipeline/operate/operating-triggers-v2`, or
  `planr-pipeline/operate/executive-board-compatibility-v2`. Their modules remain
  internal implementation details; direct package imports now fail.
- **Breaking for 0.42.0 library consumers:** the legacy Operate 1.x API was
  removed from the package root and `planr-pipeline/protocol`, including the
  advisor-brief, mandate, session, checkpoint, provider, event-chain, artifact
  generation, replay, and runtime-binding helpers. Migrate to the declared
  Protocol 2.0 exports and lens review artifacts.
- The schema wildcard no longer contains the 56 legacy `operating-*` contracts
  from v1.2.0 through v1.4.0, and the registry wildcard no longer contains
  `operating-providers.json` or `operating-roles.json`. Consumers that load those
  paths must move to the current closed Protocol 2.0 contracts and registries.
- The old governed Operate command, generated runtime packets, advisor session
  machinery, and compatibility shims were removed. Runtime adapters now install
  the lens skills and shared evidence workflow instead.
