...
## Pipeline contributor context

- Keep deterministic transitions, validation, dependency ordering, artifact
  generation, and finalization in `lib/pipeline/`; prompts and host adapters
  should orchestrate those contracts rather than duplicate them.
- Runtime adapters resolve portable assets and capability tiers. Core code must
  not contain host-specific paths, vendor model names, or host-only commands.
- The canonical nine roles live in `registry/roles.json`; generated host files
  are projections of that registry.
- Spec-driven projects use `.planr/specs/SPEC-NNN-{slug}/`. Default-mode projects
  use `input/specs/spec-{slug}.md` and `output/feats/feat-{slug}/`.
- Run focused tests for the changed domain, then the relevant conformance and
  package checks before reporting completion.
