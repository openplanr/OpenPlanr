...
assert(
  isOnSpacingScale(16) && isOnSpacingScale(24) && !isOnSpacingScale(14) && !isOnSpacingScale(13),
  'spacing scale: 16/24 on the 4-point grid, 13/14 off it',
);
assert(
  isCanonicalFrame({ w: 1440, h: 1024 }) && !isCanonicalFrame({ w: 1440, h: 760 }),
  'canonical frame: 1440×1024 ok, 1440×760 (the per-screen drift) rejected',
);
assert(
  lintDesign('<style>.a{margin-bottom:14px}</style>').ok === false,
  'linter FAILS off-grid spacing (14px)',
);
assert(
  lintDesign('<style>.a{margin-bottom:16px;padding:24px}</style>').ok === true,
  'linter PASSES on-grid spacing',
);
assert(
  lintCanvasData({ sections: [{ artboards: [{ id: 'x', width: 1440, height: 760 }] }] }).ok ===
    false,
...
assert(
  isCanonicalFrame(FRAMES.desktop) && isCanonicalFrame(FRAMES.mobile),
  'desktop + mobile canonical',
);
assert(
...
assert(
  designSystemStatus({ hasPackage: true }).source === 'package' &&
...
assert(
  dsFix.found && dsFix.source === 'package' && dsFix.tokens.length >= 5,
...
assert(
  contrastRatio('oklch(0.21 0.015 268)', 'oklch(1 0 0)') > 15,
  'oklch contrast resolves to a WCAG-grade value (dark on white)',
);
assert(
  isReadable('#000', '#fff') && !isReadable('#999', '#fff'),
  'isReadable: black ok, mid-gray fails AA',
);
assert(
  lintDesign('<style>.a{color:#999;background:#fff}</style>').ok === false,
  'linter FAILS sub-AA text/background contrast',
);
assert(
  lintDesign('<style>.a{color:#111;background:#fff}</style>').ok === true,
  'linter PASSES AA contrast',
);
assert(
  lintDesign('<style>.a{background:#11151c}</style>').warnings.some(
    (w) => w.rule === 'color-not-token',
  ),
...
assert(
  !('format' in schema.properties) && 'design_format' in schema.properties,
...
const fixtureSpec = readFileSync(
  join(root, 'conformance/fixture-design/SPEC-900-design-sample.md'),
  'utf-8',
);
const screens = resolveScreens(fixtureSpec);
assert(
  countScreens(fixtureSpec) === 5,
  `fixture resolves 5 screens (got ${screens.length}: ${screens.join(', ')})`,
);
assert(
  recommendFormat({ screenCount: screens.length }).format === 'walkthrough',
  '5 linear screens → walkthrough recommended',
);
assert(recommendFormat({ screenCount: 1 }).format === 'prototype', '1 screen → prototype');
assert(
  recommendFormat({ screenCount: 6, intentText: 'explore options' }).format === 'canvas',
  '6 exploratory screens → canvas',
);
assert(
  chooseWalkthroughNav(5) === 'anchor' && chooseWalkthroughNav(20) === 'lazy',
  'walkthrough nav: 5 → anchor, 20 → lazy',
);

// thin-spec decision (v0.13.1 — interactive asks, only headless dead-ends)
assert(
  decideThinSpec({ screenCount: 0 }).action === 'clarify',
  '0 screens interactive → clarify (no dead-end)',
);
assert(
  decideThinSpec({ screenCount: 0, format: 'walkthrough', from: 'spec' }).action === 'abort',
  '0 screens headless (both flags, non-describe) → abort',
);
assert(
  decideThinSpec({ screenCount: 0, from: 'describe' }).action === 'proceed',
...
assert(
  JSON.parse(embedJson({ label: hostile })).label === hostile,
  'embedJson preserves the value',
);
...
function* walkFiles(rel) {
  ...
  if (stat.isFile()) {
    if (TEXT_EXT.test(rel)) yield rel;
    return;
  }
...
for (const name of FOREIGN_NAMES) {
  ...
  for (const start of SCAN_ROOTS) {
    for (const rel of walkFiles(start)) {
      if (re.test(readFileSync(join(root, rel), 'utf-8'))) {
        where = rel;
        break;
      }
    }
  ...
  assert(
    !where,
    `tracked sources are clean of codename #${FOREIGN_NAMES.indexOf(name) + 1}`,
...
log('');
if (failures === 0) {
  log('✓ all design-asset conformance checks passed');
  process.exit(0);
}
