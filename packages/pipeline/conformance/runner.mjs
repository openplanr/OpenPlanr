...
//   - ND4 coherent invocation: Codex native dispatch and Cursor sequential
...
// ── ND4 — coherent invocation ──────────────────────────────────────────────
// Native dispatch and sequential fallback change scheduling only. Both drain
// all ready work before returning control to the user.
const verifyND4Coherent = (fixtureDir) => {
  let f = 0;
  const fl = (label, detail) => {
    fail(label, detail);
    f++;
  ...
  if (JSON.stringify(ready) === JSON.stringify(expEnd.ready_tasks)) {
    pass(`${ready.length} ready tasks exist (${ready.join(', ')})`);
  } else {
...
if (wantVerifyPO || wantVerifyShip) {
  ...
  if (wantVerifyShip && isNativeDispatchFixture(root)) {
    ...
    if (sentinel.gate === 'ND1') {
      ...
      failures += verifyND4Coherent(root);
    } else {
