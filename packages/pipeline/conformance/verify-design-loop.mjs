...
const { DESIGN_BOARD_ENVELOPE_FILE, DESIGN_BOARD_SOURCES_FILE, renderBoardHtml } = await import(
  moduleUrl('lib/design-engine/board.mjs')
);
const { createDesignBoardArtifactEnvelope } = await import(
  moduleUrl('lib/design-engine/artifact-adapter.mjs')
);
...
const loopBoardProcedure = readFileSync(
  join(root, 'procedures/design-loop-step3-board.md'),
  'utf8',
);
assert(
...
assert(
  loopBoardProcedure.includes('never wrap') && loopBoardProcedure.includes('never imply approval'),
  'board sharing stays explicit, immutable, non-nested, and separate from approval',
);
assert(
  !loopBoardProcedure.includes('planr-pipeline artifact'),
...
assert(
  contractInstructions(contract).includes('section-mark'),
  'contract instructions name the required sections',
);
...
assert(
  validateSheet(readFileSync(v1, 'utf-8'), contract).pass,
  'authored sheet passes the structural quality gate',
);
...
assert(
  existsSync(join(sessionDir, 'session-A.json')),
  'session persisted in the session dir (not /tmp)',
);
...
writeFileSync(
  ...
  JSON.stringify({ variants: { A: 'done' }, versions: { A: ['variant-A.svg'] } }),
);
...
assert(
  served.includes('conf') &&
    served.includes('planr-review-rail') &&
    served.includes('./runtime.js'),
...
assert(
  noSlash.status === 301 && (noSlash.headers.get('location') ?? '').endsWith('/boards/conf-loop/'),
...
assert(
  followed.includes('planr-review-rail'),
  'following the redirect lands on the working shared board',
);
assert(
  followed.includes('data-planr-mode="interact"') && followed.includes('data-planr-mode="comment"'),
  'board ships the shared Interact/Comment mode toggle (canvas artifacts stay pannable)',
);
assert(
  followed.includes('data-planr-share-dialog') && followed.includes('Private fragment'),
  'board ships the explicit Share privacy receipt',
);
assert(
  (followed.match(/class="planr-shell"/g) ?? []).length === 1 &&
    !followed.includes('legacy-board-shell'),
  'board contains one review shell and never nests legacy chrome',
);
assert(
  followed.includes('data-planr-view="split"') &&
    followed.includes('data-planr-slot="domain-rail"'),
...
assert(
  runtime.includes('/boards/conf-loop/artifacts/') && adapter.includes('api/feedback'),
...
assert(
  artifact.status === 200 &&
    (artifact.headers.get('content-type') ?? '').startsWith('application/octet-stream'),
...
assert(
  sources.sources?.[0]?.kind === 'svg' &&
    sourceExport.status === 200 &&
...
assert(
  ...
  'daemon /health reports its version (stale-daemon restart guard)',
);
  ...
  pins: [
    {
      ...
      w: 0.2,
      h: 0.1,
      comment: 'stroke too thin here',
      intent: 'fix',
    },
  ],
};
const postPending = await (
...
assert(
  round1?.kind === 'pending' && round1.feedback.pins[0].intent === 'fix',
  'agent read the pending round with its pin',
);
...
assert(
  validateSheet(readFileSync(v2, 'utf-8'), contract).pass,
  'iterated sheet still passes the gate',
);
session = appendRound(loadSession(sessionDir, 'A'), {
  outputPath: v2,
  feedback: 'bolder mark (pin: stroke too thin)',
});
saveSession(sessionDir, 'A', session);
assert(loadSession(sessionDir, 'A').outputPaths.length === 2, 'session chained the iterate round');
const reload = await (
  await fetch(`${base}/boards/conf-loop/api/reload`, {
    method: 'POST',
    headers: { origin: base },
  })
).json();
...
assert(
  finalRound?.kind === 'submit' && finalRound.feedback.preferred === 'A',
  'submit round carries preferred',
);

const approvedSchema = JSON.parse(
  readFileSync(join(root, 'schemas/v1.0.0/design-approved.schema.json'), 'utf-8'),
);
const approved = {
  schema_version: '1.0.0',
  boardId: 'conf-loop',
...
let profile = updateTaste(emptyProfile(), {
  verdict: 'approved',
  attributes: { aesthetics: ['minimal'], colors: ['indigo'] },
  sessionId: 'conf-A',
  artifact: v2,
});
profile = updateTaste(profile, {
  verdict: 'rejected',
  attributes: { aesthetics: ['playful'] },
  sessionId: 'conf-B',
  artifact: v1,
});
...
assert(
  read.dimensions.aesthetics.some((e) => e.value === 'minimal' && e.approved_count === 1),
  'approved attribute present',
);
...
log('');
if (failures === 0) {
  log('✓ all design-loop conformance checks passed');
  process.exit(0);
}
