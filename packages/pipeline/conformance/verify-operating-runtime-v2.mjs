...
function baseAssignment(state = 'pending') {
  ...
  value.claim = ['claimed', 'running', 'submitted', 'rejected', 'validated', 'failed'].includes(
    state,
  )
...
function patchFor(nextState, source) {
  ...
  if (nextState === 'running') {
    return {
      attemptPolicy: { ...source.attemptPolicy, attempt: source.attemptPolicy.attempt + 1 },
    };
  }
...
function releasePayload(assignments, assignmentId) {
  const intent = deriveOperatingAssignmentReleaseIntentsV2({ assignments }).find(
...
function happyPathEvents() {
  ...
  append(
    ...
    'asg_00000001',
    releasePayload([baseAssignment()], 'asg_00000001'),
  );
...
const registeredV2 = listProtocolSchemas().filter(
...
pass(
  registeredV2.length === OPERATE_RUNTIME_CONTRACT_KINDS.length,
...
for (const kind of OPERATE_RUNTIME_CONTRACT_KINDS) {
  ...
  pass(
    validateProtocolArtifact(kind, valid[kind], { protocolVersion: VERSION }).length === 0,
    `${kind}: valid fixture`,
  );
  pass(
...
for (const kind of OPERATE_EXTENSION_CONTRACT_KINDS_V2) {
  pass(
    loadOperateExtensionContract(kind, { protocolVersion: VERSION }).kind === kind,
...
pass(
  validateProtocolArtifact('operating-delta', deltaFixture, { protocolVersion: VERSION }).length ===
    0,
  'evidence-backed Delta fixture is a strict public contract',
);
pass(
  validateProtocolArtifact('operating-delta', invalidDeltaFixture, { protocolVersion: VERSION })
    .length > 0,
  'malformed Delta fixture fails closed',
);
pass(
  classifyOperatingDeltaMaterialityV2(deltaFixture).material,
  'public runtime surface explicitly reports a material Delta',
);
pass(
  assertOperateIntelligencePlanContractV2(intelligencePlanFixture) === intelligencePlanFixture,
  'public protocol helper accepts the exact explainable intelligence plan',
);
pass(
  sha256Jcs(assertOperatingIntelligencePlanV2(intelligencePlanFixture)) ===
    sha256Jcs(intelligencePlanFixture),
  'router semantics accept the minimum useful Advisor/Challenger/Chair board',
);
pass(
  validateProtocolArtifact('operating-intelligence-plan', invalidIntelligencePlanFixture, {
    protocolVersion: VERSION,
  }).length > 0,
  'malformed intelligence plan fails closed',
);
pass(
  validateProtocolArtifact('operating-decision-ledger', decisionLedgerFixture, {
    protocolVersion: VERSION,
  }).length === 0,
  'challenged decision ledger carries mandatory uncertainty, reversibility, and complete Action hypotheses',
);
pass(
  validateProtocolArtifact('operating-decision-ledger', invalidDecisionLedgerFixture, {
    protocolVersion: VERSION,
  }).length > 0,
  'decision ledger without mandatory reversibility fails closed',
);
const topologyBypassPlan = clone(intelligencePlanFixture);
topologyBypassPlan.selectedRoles.find(({ roleId }) => roleId === 'chair').dependsOnRoleIds = [
  'advisor',
];
...
for (const [kind, records] of [
  ...
  pass(
    records.every(
      (record) => validateProtocolArtifact(kind, record, { protocolVersion: VERSION }).length === 0,
    ),
    `${kind}: intelligence fixture keeps typed evidence-linked facts`,
  );
}
pass(
  invalidIntelligenceStateFixture.claims[0].supportingEvidenceRefIds.length === 0 &&
...
pass(
  validateProtocolArtifact('operating-scenario', triggerScenarioFixture.scenario, {
    protocolVersion: VERSION,
  }).length === 0 &&
    validateProtocolArtifact('operating-event-trigger', triggerScenarioFixture.trigger, {
      protocolVersion: VERSION,
    }).length === 0,
  'analytical Scenario and Trigger fixtures are strict contracts',
);
pass(
  validateProtocolArtifact('operating-event-trigger', invalidTriggerScenarioFixture.trigger, {
    protocolVersion: VERSION,
  }).length > 0,
  'invalid Trigger fixture fails closed',
);
...
pass(
  validateProtocolArtifact('operating-action', invalidActionVerificationFixture.action, {
    protocolVersion: VERSION,
  }).length > 0,
  'an Action-to-Assignment coercion fails contract conformance',
);
...
pass(
  validateProtocolArtifact('operate-domain-registration', businessDomain, {
    protocolVersion: VERSION,
  }).length === 0,
  'business public domain registration is valid',
);
pass(
  validateProtocolArtifact('operate-domain-registration', softwareDomain, {
    protocolVersion: VERSION,
  }).length === 0,
  'software public domain registration is valid',
);
pass(
  validateProtocolArtifact('operate-domain-registration', invalidDomain, {
    protocolVersion: VERSION,
  }).length > 0,
  'invalid public domain registration fails closed',
);
pass(
  OPEN_REFERENCE_OPERATE_EXTENSIONS_V2.domains.length === 2 &&
...
pass(
  selectOperatingSnapshotProviderV2(undefined, {
    ...
    domainContract: publicBusinessBinding,
  }).status === 'selected',
  'snapshot provider selection is explicit and local',
);
pass(
  ...
  'metric provider selection is explicit and local',
);
pass(
  ...
  'verification provider selection is explicit and local',
);
...
const evidenceDispatch = prepareOperateEvidenceDispatchV2(
  evidenceRegistry,
  evidenceRegistryFixture.candidate,
  {
...
pass(
  evidenceDispatch.status === 'authorized',
  'evidence registry selects only the exact capability-checked local pair',
);
const unavailableEvidenceDispatch = dispatchOperateEvidenceResolverV2(
  evidenceRegistry,
  evidenceRegistryFixture.candidate,
  {
    scope: evidenceRegistryFixture.scope,
...
pass(
  unavailableEvidenceDispatch.status === 'unavailable' &&
...
const planrSourceMissing = dispatchOperateEvidenceResolverV2(
  evidenceRegistry,
  planrFixture.candidate,
  {
...
pass(
  planrSourceMissing.status === 'rejected' && planrSourceMissing.error.code === 'SOURCE_NOT_FOUND',
...
const artifactReference = dispatchOperateEvidenceResolverV2(
  evidenceRegistry,
  artifactFixture.candidate,
  {
...
pass(
  artifactReference.status === 'resolved' &&
...
pass(
  validateProtocolArtifact('operating-evidence-candidate', evidenceResolutionFixture.candidate, {
    protocolVersion: VERSION,
  }).length === 0,
  'evidence materialization candidate fixture remains a typed v2 input',
);
pass(
  evidenceResolutionFixture.claimLinks.every(
    (link) =>
...
pass(
  evidenceResolutionInvalid.foreignClaimLink.sourceArtifactId !==
    evidenceResolutionFixture.candidate.sourceArtifactId &&
...
pass(
  validateProtocolArtifact('operating-evidence-graph', evidenceGraphValid, {
    protocolVersion: VERSION,
  }).length === 0,
  'empty evidence graph is a valid read-only scope projection',
);
pass(
  validateProtocolArtifact('operating-evidence-graph', evidenceGraphInvalid, {
    protocolVersion: VERSION,
  }).length > 0,
  'unsupported evidence graph edge relation fails schema conformance',
);
const generatedCatalog = fixture('generated-contract-catalog-valid.json');
pass(
  generatedCatalog.protocol.id === 'operate' && generatedCatalog.protocol.version === VERSION,
  'generated contract fixture declares explicit v2 identity',
);
pass(
  sha256Jcs(generatedCatalog.roles) === sha256Jcs(OPERATE_ROLE_MANDATES_V2),
...
for (const mandate of OPERATE_ROLE_MANDATES_V2) {
  ...
  pass(
    resolved.schemaId === mandate.output.schemaId &&
      resolved.schemaVersion === mandate.output.schemaVersion,
    `${mandate.id}: advertised schema identity is exact`,
  );
  pass(
    resolved.maxBytes === mandate.output.maxBytes,
    `${mandate.id}: advertised byte maximum is exact`,
  );
}
...
pass(
  readOperatingRuntimeStateV2(valid['operating-runtime-state'], { protocolVersion: VERSION }) ===
    valid['operating-runtime-state'],
  'explicit v2 state reader',
);
...
for (const assignmentKind of fixture('deferred-assignment-kinds-invalid.json').assignmentKinds) {
  pass(
    ...
    `deferred Assignment kind ${assignmentKind}`,
  );
}
...
pass(
  `sha256:${createHash('sha256').update(decoded).digest('hex')}` === exactBytes.rawHash,
  'exact raw hash',
);
const directBytes = exactBytes;
const directDecoded = Buffer.from(directBytes.contentBase64, 'base64');
pass(
  directDecoded.toString('utf8') === directBytes.text,
  'direct submission fixture retains exact Unicode/whitespace bytes',
);
pass(
  `sha256:${createHash('sha256').update(directDecoded).digest('hex')}` === directBytes.rawHash,
  'direct submission fixture raw hash',
);
pass(
  assertOperateRuntimeBindingsV2(bindingBaseline()).cycle.cycleId === 'cyc_00000001',
...
for (const [field, descriptor] of Object.entries(fixture('binding-field-tamper-invalid.json'))) {
  ...
  assert.throws(
    ...
    (error) => error?.code === 'E_OPERATE_BINDING_MISMATCH' && error?.details?.field === field,
    field,
  );
  checks += 1;
}
...
for (const [from, nextStates] of Object.entries(OPERATING_ASSIGNMENT_TRANSITIONS_V2)) {
  ...
  for (const to of assignmentStates) {
    ...
    assert.throws(
      () => transitionOperatingAssignmentV2(source, to, patchFor(to, source)),
...
for (const [from, nextStates] of Object.entries(OPERATING_REVIEW_TRANSITIONS_V2)) {
  ...
  for (const to of reviewStates) {
    ...
    const before = sha256Jcs(source);
    assert.throws(
      () => transitionOperatingReviewV2(source, to, { updatedAt: NEXT_TIME }),
      (error) => error?.code === 'STATE_TRANSITION_INVALID',
      `${from} -> ${to}`,
    );
...
pass(
  Object.keys(OPERATE_GUARD_TABLE_V2).length === 11,
  'eleven bounded runtime tools including the three governed Action operations',
);
pass(
  OPERATE_GOVERNED_TOOL_OPERATIONS_V2.length === 3 && OPERATE_AUTHORITY_GUARD_IDS_V2.length === 4,
...
const actionContexts = {
  ...
  'operate.cycle.resume': {
    capabilities: ['operate.cycle.resume'],
    cycle: clone(valid['operating-cycle']),
  },
  ...
  'operate.assignment.submit': {
    ...
    submitRequest: {
      ...
      submissionId: issuedSubmission.submissionId,
      actor: { actorId: 'agent-001', kind: 'agent', runtime: 'codex' },
      mediaType: 'application/json',
      encoding: 'utf-8',
      contentBase64: intelligenceSubmissionBase64('advisor'),
    },
  },
  'operate.artifact.get': {
    capabilities: ['operate.artifact.get'],
    artifact: clone(valid['operating-artifact']),
    ...
    scopeMembership: {
      active: true,
      actorId: 'owner-artifact-read-001',
      scopeId: valid['operating-artifact'].scopeId,
      domainId: valid['operating-artifact'].domainId,
      domainVersion: valid['operating-artifact'].domainVersion,
    },
  ...
  'operate.review.get': {
    ...
    cycle: {
      ...
      activeReviewId: valid['operating-review'].reviewId,
    },
    ...
    reviewReadRequest: {
      ...
      actor: {
        actorId: valid['operating-review'].ownerActorId,
        kind: 'human',
        runtime: 'portable',
      },
      scope: {
...
pass(
  validateProtocolArtifact('operating-governed-operation', governedOperationValid, {
    protocolVersion: VERSION,
  }).length === 0,
  'at-most-once runtime fixture is one strict runtime-issued governed operation',
);
pass(
  validateProtocolArtifact('operating-governed-operation', governedOperationInvalid, {
    protocolVersion: VERSION,
  }).length > 0,
  'caller identity/fingerprint and prohibited-effect governed operation fixture fails closed',
);
pass(
  readOperatingGovernedOperationV2(governedOperationValid, { protocolVersion: VERSION })
    .operationId === governedOperationValid.operationId,
  'public protocol reader accepts only the explicit governed operation contract',
);
pass(
  JSON.stringify(OPERATING_GOVERNED_EXECUTION_TERMINAL_STATES_V2) ===
...
pass(
  typeof createOperatingGovernedExecutionRuntimeV2 === 'function' &&
...
pass(
  validateProtocolArtifact('operating-rollback-plan', governedRollbackValid.rollbackPlan, {
    protocolVersion: VERSION,
  }).length === 0 &&
    validateProtocolArtifact('operating-rollback-result', governedRollbackValid.rollbackResult, {
      protocolVersion: VERSION,
    }).length === 0,
  'governed rollback fixtures use the strict plan and immutable result contracts',
);
pass(
  sha256Jcs(governedRollbackValid.reconciliation.classifications) ===
...
pass(
  readOperatingRollbackPlanV2(governedRollbackValid.rollbackPlan, { protocolVersion: VERSION })
    .rollbackPlanId === governedRollbackValid.rollbackPlan.rollbackPlanId &&
    readOperatingRollbackResultV2(governedRollbackValid.rollbackResult, {
      protocolVersion: VERSION,
    }).rollbackResultId === governedRollbackValid.rollbackResult.rollbackResultId,
  'public protocol readers accept only explicit immutable rollback plan and result contracts',
);
pass(
  governedRollbackValid.rollbackResult.originalOperationId ===
    governedRollbackValid.rollbackPlan.operationId &&
    governedRollbackValid.rollbackResult.executionResultId ===
      governedRollbackValid.rollbackPlan.executionResultId &&
    governedRollbackValid.rollbackResult.rollbackPlanId ===
      governedRollbackValid.rollbackPlan.rollbackPlanId &&
    governedRollbackValid.rollbackResult.baselineArtifactId ===
      governedRollbackValid.rollbackPlan.baselineArtifactId &&
    governedRollbackValid.rollbackResult.baselineHash ===
      governedRollbackValid.rollbackPlan.baselineHash &&
    governedRollbackValid.rollbackResult.targetAfterHash ===
      governedRollbackValid.rollbackPlan.baselineHash,
  'valid rollback fixture restores only the exact plan-bound baseline under a new operation identity',
);
pass(
  governedRollbackInvalid.vectors.length === 12 &&
    new Set(governedRollbackInvalid.vectors.map(({ id }) => id)).size === 12 &&
    governedRollbackInvalid.vectors.every(
      ({ path, expectedCode }) => typeof path === 'string' && typeof expectedCode === 'string',
    ),
...
pass(
  ...
  'the package exposes typed bounded recovery, reconciliation, plan, and rollback entry points',
);
pass(
  sha256Jcs(policyApprovalValid.policyPrecedence) === sha256Jcs(OPERATE_GOVERNED_POLICY_TIERS_V2),
  'policy fixture uses compiler-owned core, project, and domain precedence',
);
pass(
  sha256Jcs(policyApprovalValid.effectClasses) === sha256Jcs(OPERATE_GOVERNED_EFFECT_CLASSES_V2),
  'policy fixture covers every compiler-owned effect class',
);
pass(
  sha256Jcs([...policyApprovalValid.outcomes].sort()) ===
    sha256Jcs([...OPERATE_GOVERNED_POLICY_OUTCOMES_V2].sort()),
  'policy fixture covers every compiler-owned outcome',
);
pass(
  sha256Jcs(policyApprovalValid.hardProhibitions) ===
    sha256Jcs(OPERATE_GOVERNED_CORE_PROHIBITIONS_V2),
  'policy fixture covers every non-overridable reference prohibition',
);
pass(
  policyApprovalValid.approvalCases.length === 7 &&
    policyApprovalValid.approvalCases.every(
...
pass(
  policyApprovalInvalid.vectors.length === 16 &&
    new Set(policyApprovalInvalid.vectors.map(({ id }) => id)).size === 16 &&
    policyApprovalInvalid.vectors.every(
...
const approvalRequirement = createOperatingApprovalRequirementV2({
  policyRequirementId: 'aprq_conform01',
...
const approvalRecord = createOperatingApprovalRecordV2({
  approvalId: 'aprv_conform001',
  requirement: approvalRequirement,
  evaluation: policyEvaluation,
  action: policyAction,
  partyId: approvalParty.partyId,
  actor: {
    kind: approvalParty.actorKind,
    actorId: approvalParty.actorId,
    capability: approvalParty.requiredCapability,
  },
  decision: 'approved',
...
pass(
  evaluateOperatingApprovalSetV2({
    ...
    approvals: approvalAppend.records,
    now: '2026-08-10T08:02:00Z',
  }).disposition === 'approved',
  'public policy and approval modules produce one exact complete authority disposition',
);
pass(
  appendOperatingApprovalRecordV2({
    records: approvalAppend.records,
    record: approvalRecord,
    requirement: approvalRequirement,
  }).replayed,
  'identical approval replay is harmless',
);
...
const executeAuthorityContext = {
  ...
  action: clone(authorizationFixture.action),
  actionRequest: clone(authorizationFixture.executeRequest),
  scope: clone(authorizationFixture.scope),
  ...
  capabilityAvailability: clone(governedFixture['operating-capability-availability']),
...
const approveAuthorityContext = {
  ...
  actionRequest: clone(authorizationFixture.approveRequest),
  approvals: [],
};
...
rollbackAuthorityContext.capabilities = [
  clone(authorizationFixture.toolCapabilities.actionRollback),
];
...
rollbackAuthorityContext.operation = {
  ...
  requestFingerprint: governedFixture['operating-rollback-result'].requestFingerprint,
  grantId: 'cgr_00000002',
  verificationPlanId: rollbackAuthorityContext.rollbackPlan.verificationPlanId,
...
function policyForConformance(
  action,
  { policyId, tier, decisionMode, approvalRequirementIds = [] } = {},
) {
  ...
  return createOperatingActionPolicyV2({
    ...
    tier,
    provenance: {
      providerId: `${tier}-negative-policy-provider`,
      providerVersion: '1.0.0',
...
function executePolicyApprovalNegativeMutation(mutation) {
  ...
  if (mutation.type === 'lower-tier-widens-outcome') {
    ...
    evaluateOperatingActionPolicyV2({
      action: policyAction,
      configuredPolicies: [strongCore, weakDomain],
  ...
  if (mutation.type === 'required-policy-omitted') {
    const projectPolicy = policyForConformance(policyAction, {
      policyId: 'project-required-policy',
      tier: 'project',
      decisionMode: 'named-single-party',
    ...
    assertOperatingPolicyEvaluationV2(policyEvaluation, {
      action: policyAction,
      configuredPolicies: [corePolicy, projectPolicy, policy],
    });
    return null;
  }
  if (mutation.type === 'core-prohibition-weakened') {
    ...
    const prohibitedCore = policyForConformance(prohibitedAction, {
      policyId: 'core-prohibition-policy',
      tier: 'core',
      decisionMode: 'prohibited',
    });
    const weakDomain = policyForConformance(prohibitedAction, {
      policyId: prohibitedAction.executionBinding.policyId,
      tier: 'domain',
      decisionMode: 'automatic',
    });
    evaluateOperatingActionPolicyV2({
      action: prohibitedAction,
      configuredPolicies: [prohibitedCore, weakDomain],
      evaluatedAt: policyEvaluation.evaluatedAt,
    });
    return null;
  }
  ...
  if (mutation.type === 'actor-reused-across-requirements') {
    ...
    const evaluation = evaluateOperatingActionPolicyV2({
      action: policyAction,
      configuredPolicies: [corePolicy, multiPolicy],
      evaluatedAt: policyEvaluation.evaluatedAt,
    });
    const requirements = templates.map((policyRequirementId, index) =>
      createOperatingApprovalRequirementV2({
        policyRequirementId,
        evaluation,
        action: policyAction,
        parties: [{ ...clone(approvalParty), partyId: `global-${index + 1}-party` }],
        expiresAt: approvalRequirement.expiresAt,
        consumable: true,
      }),
    );
    evaluateOperatingApprovalSetV2({
  ...
  if (mutation.type === 'partial-quorum') {
    ...
    const evaluation = evaluateOperatingActionPolicyV2({
      action: policyAction,
      configuredPolicies: [corePolicy, thresholdPolicy],
      evaluatedAt: policyEvaluation.evaluatedAt,
    });
    const parties = [1, 2].map((index) => ({
      ...clone(approvalParty),
      partyId: `threshold-${index}-party`,
      actorId: `owner-000${index}`,
    }));
    ...
    const partial = createOperatingApprovalRecordV2({
      approvalId: 'aprv_partial001',
      requirement,
      evaluation,
      action: policyAction,
      partyId: parties[0].partyId,
      actor: {
        kind: parties[0].actorKind,
        actorId: parties[0].actorId,
        capability: parties[0].requiredCapability,
      },
      decision: 'approved',
      issuedAt: approvalRecord.issuedAt,
    ...
    if (!disposition.complete)
      throw Object.assign(new Error('partial approval quorum'), { code: 'APPROVAL_REQUIRED' });
    return null;
  }
  if (mutation.type === 'expired-approval') {
    const expired = createOperatingApprovalRecordV2({
      approvalId: 'aprv_expired001',
      requirement: approvalRequirement,
      evaluation: policyEvaluation,
      action: policyAction,
      partyId: approvalParty.partyId,
  ...
  if (mutation.type === 'consumed-approval') {
    const consumed = consumeOperatingApprovalRecordsV2({
      ...
      requirements: [approvalRequirement],
      approvalIds: [approvalRecord.approvalId],
      operationId: 'op_conformconsume',
    });
    evaluateOperatingApprovalSetV2({
      evaluation: policyEvaluation,
      action: policyAction,
      requirements: [approvalRequirement],
      approvals: consumed.records,
      now: authorizationFixture.now,
    });
    return null;
  }
  ...
  if (mutation.type === 'cycle-review-substitution') {
    ...
    if (!decision.allowed)
      throw Object.assign(new Error(decision.error.message), { code: decision.error.code });
    return null;
  }
  if (mutation.type === 'prior-evaluation-reuse') {
    const nextEvaluation = evaluateOperatingActionPolicyV2({
      action: policyAction,
      configuredPolicies: [corePolicy, policy],
      evaluatedAt: '2026-08-10T08:02:00Z',
    });
    evaluateOperatingApprovalSetV2({
...
pass(
  invalidAuthorizationFixture.vectors.length >= 12,
...
pass(
  evaluateOperateAuthorityV2('operate.action.execute', executeAuthorityContext).allowed,
...
const governedSelections = [
  selectOperateCapabilityProviderV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
    ...governedSelectionBase,
    ...governedExtensionFixture.capabilitySelection,
  }),
  selectOperatePolicyProviderV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
    ...governedSelectionBase,
    ...governedExtensionFixture.policySelection,
  }),
  selectOperateExecutorV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
    ...governedSelectionBase,
    ...governedExtensionFixture.projectExecutorSelection,
  }),
  selectOperateExecutorV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
    ...governedSelectionBase,
    ...governedExtensionFixture.containmentExecutorSelection,
  }),
];
pass(
  governedSelections.every(({ status, fallback }) => status === 'available' && fallback === null),
...
pass(
  sha256Jcs(
    createOperateGovernedExtensionRegistryV2({
      capabilityProviders: [
        ...OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2.capabilityProviders,
      ].reverse(),
...
pass(
  governedExtensionInvalid.vectors.length >= 19 &&
    governedExtensionInvalid.vectors.every(
      ({ name, kind }) => typeof name === 'string' && typeof kind === 'string',
    ),
...
pass(
  governedExtensionFixture.catalogDigest ===
    OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSION_CATALOG_DIGEST_V2,
...
pass(
  true,
  'reserved shipped executor ID remains immutable across every version and implementation identity',
);
...
pass(
  governedExtensionInvalid.vectors
    .find(({ kind }) => kind === 'effect-rank')
    .values.every((effectClass) =>
      selectorCases.every(
        ([select, selection]) =>
          select(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
            ...governedSelectionBase,
            ...selection,
            effectClass,
...
pass(
  governedExtensionInvalid.vectors
    .find(({ kind }) => kind === 'registration-semantics')
...
pass(
  createOperateGovernedExtensionRegistryV2(payOnlyRegistration).executors.some(
    ({ executorId }) => executorId === payOnlyExecutor.executorId,
  ),
  'pay alone remains a non-transfer semantic without send or transfer composition',
);
pass(
  ['ship', 'message', 'email'].every((identifier) => {
    const input = clone(payOnlyRegistration);
    input.executors.at(-1).supportedActionKinds[0].id = identifier;
    try {
      return createOperateGovernedExtensionRegistryV2(input).executors.some(
        ({ executorId }) => executorId === payOnlyExecutor.executorId,
      );
    } catch {
      return false;
...
containedAuthorityContext.operation.requestFingerprint =
  deriveContainedExecutorRequestFingerprintV2({
...
pass(
  containedTarget.describe().effectCount === 1,
...
const containedRollbackDecision = evaluateOperateAuthorityV2(
  'operate.action.rollback',
  containedRollbackContext,
);
pass(
  containedRollbackDecision.allowed,
...
pass(
  containedTarget.read().value.status === 'after' && containedTarget.describe().effectCount === 1,
...
pass(
  ...
  'caller-constructed binding clone is denied by authorization',
);
...
assert.throws(
  () =>
    OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
      ...
      binding: projectBinding,
      executor: projectExecutorSelection.registration,
      targetAdapter: substitutedContainedTarget,
    }),
  { code: 'OPERATION_CONFLICT' },
);
pass(
  substitutedContainedTarget.read().value.status === 'before',
...
pass(
  evaluateOperateAuthorityV2('operate.action.execute', {
    ...containedAuthorityContext,
    executorInput: divergentContainedInput,
  }).allowed === false,
  'divergent initial payload is denied before target access',
);

pass(
  OPERATE_GOVERNED_CORE_PROHIBITIONS_V2.every((identifier) => {
    try {
      createDisposableLocalProjectTargetV2({
...
for (const [operation, context] of Object.entries(actionContexts)) {
  ...
  if (operation === 'operate.assignment.claim') {
    pass(
      !Object.hasOwn(action.arguments.actor, 'sessionId'),
      'claim action excludes internal session binding',
    );
  }
...
for (const [label, overrides, code] of [
  [
    ...
    {
      submitRequest: {
        ...actionContexts['operate.assignment.submit'].submitRequest,
        assignmentId: 'asg_other0001',
      },
    },
...
const firstReplay = reduceOperatingRuntimeEventsV2(events, {
  initialState: replayBase(),
  replayHook,
});
...
pass(
  sha256Jcs(replayedEvent) === sha256Jcs(firstReplay),
  'same Event identity/content replays without append',
);
const conflictingEvent = createOperatingRuntimeEventV2(
  {
    ...
    actor: { kind: 'engine', id: 'openplanr' },
    causationId: firstReplay.eventReplayIndex.at(-1).eventId,
...
const scheduledCreation = scheduleOperatingRuntimeEventsV2(creationOnly, {
  initialState: replayBase(),
});
const [zeroDependencyRelease] = scheduledCreation.releaseEvents;
const zeroDependencyAvailable = scheduledCreation.state;
pass(
  zeroDependencyAvailable.assignments[0].state === 'available',
  'zero-dependency Assignment becomes available through scheduler Event',
);
pass(
  zeroDependencyAvailable.eventReplayIndex.filter(
    ({ eventId }) => eventId === zeroDependencyRelease.eventId,
  ).length === 1,
  'zero-dependency release has exactly one durable Event provenance',
);
pass(
  sha256Jcs(
    scheduleOperatingRuntimeEventsV2(creationOnly, {
      initialState: clone(zeroDependencyAvailable),
    }).state,
  ) === sha256Jcs(zeroDependencyAvailable),
  'replayed creation contributes no second availability Event',
);
...
assert.throws(
  () =>
    createOperatingRuntimeEventV2(
      {
        eventId: 'evt-zero-created-bypass',
        timestamp: NEXT_TIME,
        cycleId: 'cyc_00000001',
        type: 'assignment.created',
        entityId: 'asg_00000002',
        actor: { kind: 'runtime', id: 'openplanr' },
        causationId: 'evt-002',
        correlationId: 'corr-zero-created-bypass',
        payload: {
          ...baseAssignment('available'),
          assignmentId: 'asg_00000002',
      ...
      {
        previousEvent: {
          sequence: pendingOnly.eventHead.sequence,
          eventHash: pendingOnly.eventHead.hash,
        },
      },
    ),
...
pass(
  sha256Jcs(pendingOnly) === beforeAvailabilityBypass,
  'direct available creation bypass does not mutate projection',
);

const schedulerFixture = fixture('scheduler-valid.json');
pass(
  schedulerFixture.zeroDependency.expectedReleaseOrder[0] === zeroDependencyCreation.assignmentId,
...
const beforeDirectAcceptance = reduceOperatingRuntimeEventsV2(events.slice(0, 5), {
  initialState: replayBase(),
});
...
const directRequest = {
  ...
  mediaType: 'application/json',
  encoding: 'utf-8',
  contentBase64: directBytes.contentBase64,
};
const directDraft = {
...
pass(
  Buffer.compare(directAccepted.stagedRawBytes, directDecoded) === 0,
  'direct submission stages exact bytes',
);
pass(
  directAccepted.artifact.rawHash === directBytes.rawHash,
  'Artifact records exact-byte raw hash',
);
pass(
  directAccepted.artifact.canonicalHash === directBytes.canonicalHash,
  'Artifact records separate canonical hash',
);
pass(
  directAccepted.events.length === 3 && directAccepted.state.assignments[0].state === 'validated',
  'direct submission commits Artifact/Event/projection together',
);
const directReplay = acceptOperatingAssignmentSubmissionV2(
  directRequest,
  {
    ...directDraft,
...
pass(
  directReplay.replayed &&
    directReplay.events.length === 0 &&
    sha256Jcs(directReplay.response) === sha256Jcs(directAccepted.response),
  'same submission ID and exact bytes replay without a duplicate effect',
);
const directConflict = Buffer.from(
  '{"kind":"operating-advisor-result","schemaVersion":"1.0.0","protocolVersion":"2.0.0","café":"Δ","items":[]}',
  'utf8',
).toString('base64');
assert.throws(
  () =>
    acceptOperatingAssignmentSubmissionV2(
      { ...directRequest, contentBase64: directConflict },
      directDraft,
      {
        initialState: directAccepted.state,
      },
