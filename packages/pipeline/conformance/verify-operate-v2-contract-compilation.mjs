...
import {
  runOperateContractGenerator,
} from '../scripts/generate-operate-contracts.mjs';
...
function transitionEdges(table, entityId) {
  return Object.entries(table).flatMap(([from, targets]) => targets.map((to) => ({
    entityId,
    from,
    to,
  }))).sort((left, right) => (
...
try {
  ...
  const sourceTargetsAvailable = sourceVerificationTargets.every((path) => existsSync(join(root, path)));
  ...
  assert.deepEqual(OPERATE_RUNTIME_CONTRACT_KINDS, catalogKinds,
    'public loader kinds must be compiler-owned in runtime order');
  checks += 1;
  assert.equal(catalogKinds.length, registry.contracts.length,
  ...
  assert.deepEqual(first.evidence, {
    ...
    resolverErrorCodes: [
      'ANCESTRY_MISMATCH', 'ARTIFACT_HASH_MISMATCH', 'ARTIFACT_NOT_FOUND', 'CAPABILITY_DENIED',
      'CONSENT_REQUIRED', 'EVIDENCE_LOCATOR_INVALID', 'EVIDENCE_PROVIDER_UNAVAILABLE',
      'EVIDENCE_RESOLVER_UNAVAILABLE', 'EVIDENCE_RESOLVER_UNREGISTERED',
      'EVIDENCE_RESOLVER_VERSION_UNSUPPORTED', 'EVIDENCE_SOURCE_SCOPE_MISMATCH',
      'LINE_RANGE_INVALID', 'OBJECT_TYPE_MISMATCH', 'PATH_NOT_FOUND', 'REVISION_NOT_FOUND',
      'SECRET_DETECTED', 'SENSITIVITY_BLOCKED', 'SOURCE_NOT_FOUND', 'SOURCE_STALE',
      'SOURCE_UNTRACKED', 'UNSUPPORTED_EVIDENCE_KIND',
    ],
    sourceContracts: [
  ...
  checks += 1;
  assert.deepEqual(OPERATE_OPERATING_INTELLIGENCE_CONTRACT_KINDS_V2, first.operatingIntelligence.contractIds,
  ...
  assert.deepEqual(OPERATE_OPERATING_PROVIDER_REGISTRATION_CONTRACT_KINDS_V2,
  ...
  checks += 3;
  assert.deepEqual(OPERATE_GOVERNED_EXECUTION_CONTRACT_KINDS_V2, first.governedExecution.contractIds,
    'Phase 6 governed-execution identities must be compiler-owned and exact');
  assert.deepEqual(OPERATE_GOVERNED_EFFECT_CLASSES_V2, first.governedExecution.effectClasses,
    'Phase 6 effect classifications must be compiler-owned and confer no authority');
  checks += 2;
  assert.deepEqual(OPERATE_EXPERIENCE_CONTRACT_KINDS_V2, first.experience.contracts.map(({ id }) => id),
    'product-experience contract identities must be compiler-owned and exact');
  assert.deepEqual(OPERATING_DELIVERY_ROUTES_V1, first.experience.deliveryRoutes,
    'Operate-to-Planning delivery routes must be closed and compiler-owned');
  checks += 2;
  pass(first.generation.verifiedTargets.every(({ path }) => (
    path !== 'package.json'
  ...
  const registeredSchemas = listProtocolSchemas()
    ...
    .map(({ kind }) => kind)
    .sort();
  assert.deepEqual(registeredSchemas, [...catalogKinds].sort(),
  ...
  assert.deepEqual(Object.keys(valid).sort(), [...catalogKinds].sort(), 'valid fixture must cover every contract');
  assert.deepEqual(Object.keys(invalid).sort(), [...catalogKinds].sort(), 'invalid fixture must cover every contract');
  checks += 2;
  for (const kind of catalogKinds) {
    const contract = loadOperateRuntimeContract(kind, { protocolVersion: first.protocol.version });
    pass(contract.path === first.contracts.find(({ id }) => id === kind).schemaPath,
      `${kind}: loader path must equal catalog path`);
    pass(validateProtocolArtifact(kind, valid[kind], { protocolVersion: first.protocol.version }).length === 0,
    ...
    }).length > 0, `${kind}: emitted invalid fixture must fail validation`);
  }

  const validExperience = fixture('experience-bridge-valid.json');
  const invalidExperience = fixture('experience-bridge-invalid.json');
  assert.deepEqual(Object.keys(validExperience).sort(), [...OPERATE_EXPERIENCE_CONTRACT_KINDS_V2].sort(),
    'experience fixture must cover every public experience contract');
  checks += 1;
  for (const contract of first.experience.contracts) {
    const loaded = loadOperateExperienceContract(contract.id, { protocolVersion: first.protocol.version });
    pass(loaded.path === contract.schemaPath, `${contract.id}: experience loader path must equal catalog path`);
    pass(validateOperateExperienceArtifactV2(contract.id, validExperience[contract.id]).length === 0,
      `${contract.id}: valid experience fixture must validate`);
    pass(validateOperateExperienceArtifactV2(contract.id, invalidExperience[contract.id]).length > 0,
  ...
  const reviewEdges = first.transitions
    ...
    .map(({ entityId, from, to }) => ({ entityId, from, to }));
  assert.deepEqual(assignmentEdges, transitionEdges(OPERATING_ASSIGNMENT_TRANSITIONS_V2, 'operating-assignment'));
  assert.deepEqual(reviewEdges, transitionEdges(OPERATING_REVIEW_TRANSITIONS_V2, 'operating-review'));
  checks += 2;
  assert.deepEqual(Object.keys(OPERATE_GUARD_TABLE_V2).sort(), first.operations.map(({ id }) => id).sort(),
    'every implemented public operation must have exactly one compiled guard row');
  assert.deepEqual(
    Object.values(OPERATE_GUARD_TABLE_V2).map(({ operation, label }) => ({ operationId: operation, label }))
  ...
  for (const operation of first.operations) {
    pass(!/(?:finalize|prepare)/u.test(operation.id),
  ...
  for (const contract of first.contracts) {
    pass(documentation.includes(`\`${contract.id}\``), `${contract.id}: documentation must name the contract`);
  }
  for (const contract of first.experience.contracts) {
    pass(documentation.includes(`\`${contract.id}\``), `${contract.id}: documentation must name the experience contract`);
  }
  ...
  for (const subpath of operateSubpaths) {
    ...
    pass(typeof target?.import === 'string' && typeof target?.types === 'string',
      `${subpath}: package export must declare runtime and types`);
    pass(existsSync(join(root, target.import)) && existsSync(join(root, target.types)),
      `${subpath}: declared export targets must exist`);
    pass(documentation.includes(`\`planr-pipeline${subpath.slice(1)}\``),
      `${subpath}: protocol documentation must list the declared export`);
  }
  const documentedSubpaths = [...documentation.matchAll(/`planr-pipeline(\/operate\/[a-z0-9-]+)`/gu)]
    .map(([, path]) => `.${path}`);
  for (const subpath of new Set(documentedSubpaths)) {
    pass(operateSubpaths.includes(subpath),
      `${subpath}: protocol documentation lists an export package.json does not declare`);
  }

  const driftTargets = [registry.generation.catalogPath, ...first.generation.verifiedTargets.map(({ path }) => path)];
  if (sourceTargetsAvailable) {
