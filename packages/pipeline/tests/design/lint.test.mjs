...
test('off-grid spacing in a <style> rule is an ERROR with a snap suggestion', () => {
  ...
  assert.ok(
    r.errors.find((e) => e.value === '13px' && e.suggestion === '12px'),
    'flags 13px → 12px',
  );
});
...
test('sub-AA text/background contrast is a hard ERROR', () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => e.rule === 'contrast-below-aa'),
    'flags #999 on #fff (~2.85:1)',
  );
});
...
test('raw color usage is a WARNING; a --token definition is not flagged', () => {
  ...
  assert.ok(
    !r.warnings.some((w) => w.value === '#fff'),
    'a --custom-prop definition is not raw-color usage',
  );
...
test('font-not-token warns against a known design system only', () => {
  ...
  const bad = lintDesign('<style>.a{font-family:"Comic Sans MS",cursive}</style>', {
    designSystem: ds,
  });
  ...
  assert.ok(
    !ok.warnings.some((w) => w.rule === 'font-not-token'),
    'allowed DS font + system fallback is fine',
  );
...
test('off-grid spacing expressed in rem/em is caught (normalized to px at 16px root)', () => {
  assert.equal(
    lintDesign('<style>.a{padding:0.8125rem}</style>').errors.some(
      (e) => e.rule === 'spacing-off-grid',
    ),
    true,
    '0.8125rem = 13px is off-grid',
  );
  assert.equal(
    lintDesign('<style>.a{padding:1rem;gap:0.5rem}</style>').errors.length,
    0,
    '1rem/0.5rem = 16px/8px are on-grid',
  );
});

test('lintDesign reports a declarations count (0 when no <style>/inline CSS is present)', () => {
  assert.equal(
    lintDesign('.a{padding:13px}').declarations,
    0,
    'raw CSS (no <style>) parses to zero declarations',
  );
  assert.ok(
    lintDesign('<style>.a{padding:16px;color:#111}</style>').declarations >= 2,
    'real <style> CSS parses declarations',
  );
});
...
const runLint = (args) => {
  try {
    execFileSync('node', [LINT, ...args], { stdio: 'pipe' });
    return 0;
  } catch (e) {
    return e.status;
  }
...
test('CLI auto-wraps a bare .css so compiled stylesheets (Tailwind arbitrary values) are caught', () => {
  ...
  try {
    ...
    assert.equal(
      runLint([join(dir, 'app.css')]),
      1,
      'off-grid in a bare .css is caught (auto-wrapped) → exit 1',
    );
...
test('CLI --expect-styles fails (exit 3) when a style-less file parses zero declarations — no fabricated clean pass', () => {
  ...
  try {
    ...
    assert.equal(
      runLint([join(dir, 'shell.html')]),
      0,
      'back-compat: no flag → 0 even with nothing parsed',
    );
    assert.equal(
      runLint(['--expect-styles', join(dir, 'shell.html')]),
      3,
      'zero-decl + --expect-styles → exit 3 (no fabricated pass)',
    );
    assert.equal(
      runLint(['--expect-styles', join(dir, 'real.html')]),
      0,
      'real on-grid CSS → exit 0',
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
