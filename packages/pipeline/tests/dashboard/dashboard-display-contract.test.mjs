...
function cycle(focus = ['API/CLI and C#/.NET delivery'], cycleState = 'approved') {
...
test('display owner enforces the exact canonical lifecycle matrix', () => {
  for (const cycleState of Object.keys(lifecycleStage)) {
    ...
    assert.equal(assertOperateExperienceDisplaySurfaceV1(value, expected), value, cycleState);
    const hostile = structuredClone(value);
    hostile.payload.data.cycles[0].stages[6].state = 'revisited';
    assert.throws(() => assertOperateExperienceDisplaySurfaceV1(hostile, expected), cycleState);
  }
});
...
test('display verification rejects hidden fields, accessors, and proxies without evaluating private values', () => {
  ...
  symbol.payload[Symbol('privateBody')] = '/private/symbol-value-must-not-echo';
  assert.throws(
