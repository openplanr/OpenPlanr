...
const fixtureView = JSON.parse(readFileSync(new URL(
...
function runtimeState() {
  return {
    ...
    cycles: [{
      ...
      cycleId: CYCLE_ID, scopeId: SCOPE.scopeId, domainId: SCOPE.domainId,
      domainVersion: SCOPE.domainVersion, state: 'awaiting_review',
      inputBindingId: 'inb_bound_00000001', contractVersions: { 'advisor-result': '1.0.0' },
      trigger: { kind: 'manual' }, focus: ['strategy'], health: 'normal',
      activeReviewId: REVIEW_ID, createdAt: TIME, updatedAt: TIME,
    }],
    reviews: [{
      kind: 'operating-review', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
      reviewId: REVIEW_ID, cycleId: CYCLE_ID, ownerActorId: ACTOR.actorId,
...
function readReview(state) {
  return readOperatingReviewV2({
    reviewId: REVIEW_ID, cycleId: CYCLE_ID, actor: ACTOR, scope: SCOPE,
  }, { initialState: state, capabilities: ['operate.review.get'], readAt: COMMIT_TIME }).data;
}

function stages() {
  return ['observe', 'understand', 'decide', 'govern', 'act', 'verify', 'learn']
    .map((id, index) => ({
      id,
...
test('Review REST route is exact and rejects traversal, encoded separators, and surplus segments', () => {
  assert.deepEqual(
    parseOperateApiRoute(`/api/operate/cycles/${CYCLE_ID}/reviews/${REVIEW_ID}`),
    { surface: 'review', cycleId: CYCLE_ID, subjectId: REVIEW_ID },
  );
  for (const path of [
...
test('Review route is owner-bound and confirmation preserves the exact immutable receipt and refreshed workspace', async () => {
  ...
  const [choice] = pending.dispositionChoices.filter(({ submitArguments }) => (
  ...
  const committed = submitBoundOperatingReviewV2(bound, {
  ...
  const allowedAction = pendingView.allowedActions.find(({ subjectId }) => subjectId === REVIEW_ID).action;
  ...
  const previewBase = {
    ...
    transition: {
      kind: 'review',
      targets: [{ kind: 'operating-review', id: REVIEW_ID, revision: null, hash: HASH, disposition: 'approved' }],
      reversible: false,
      nextState: 'approved',
      threshold: null,
    },
  ...
  const gateway = {
    async issueSession() {
      return {
        ...
        sessionId: 'opsess_bound_review_0001', sessionCapability: CAPABILITY,
        issuedAt: TIME, expiresAt: '2026-08-23T08:05:00.000Z',
        binding: structuredClone(sessionBinding), readOnly: false,
        allowedActions: [{ actionReference, subjectId: REVIEW_ID, actionDigest }],
      };
    },
    assertSessionBinding() { return structuredClone(sessionBinding); },
    async preview() { return structuredClone(preview); },
  ...
  const dashboard = createDashboardServer({
    ...
    getOperatingExperience: () => ({
      available: true, readOnly: true, status: 'ready', view: currentView, reasonCodes: [],
    }),
  ...
  try {
    ...
    currentWorkspace = { ...structuredClone(pendingWorkspace), privateOwnerContent: 'must-not-escape' };
    ...
    const stale = await request(port, `/api/operate/commands/confirm?${QUERY}`, {
      method: 'POST', body: confirmBody, origin, authorization: `Bearer ${CAPABILITY}`,
    });
