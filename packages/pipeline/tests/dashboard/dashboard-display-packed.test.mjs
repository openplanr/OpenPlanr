...
test('display verifier source, packed runtime, and declarations have exact public parity', {
  ...
  assert.equal(
    JSON.parse(readFileSync(join(root, 'package.json'), 'utf8')).dependencies['@noble/hashes'],
    '1.8.0',
  );
  ...
  try {
    ...
    const installedWorkspace = await import(
      pathToFileURL(join(installedPackage, 'schemas/v1.2.0/operate-cycle-display-workspace.mjs'))
        .href
    );
    const installedBoard = await import(
      pathToFileURL(
        join(installedPackage, 'schemas/v1.2.0/operate-executive-board-display-surface.mjs'),
      ).href
    );
    const installedContract = await import(
      pathToFileURL(join(installedPackage, 'lib/dashboard/operate-experience-display-contract.mjs'))
        .href
    );
    ...
    assert.equal(
      installedSurface.assertOperateExperienceDisplaySurfaceV1(display, binding).integrity
        .contentHash,
