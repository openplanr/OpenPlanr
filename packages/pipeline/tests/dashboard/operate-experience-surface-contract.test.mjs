...
const SURFACE_SCHEMA_EXPORTS = Object.freeze([
	["OPERATE_ACTION_DISPLAY_WORKSPACE_SCHEMA", "schemas/v1.2.0/operate-action-display-workspace.schema.json"],
	["OPERATE_CYCLE_DISPLAY_WORKSPACE_SCHEMA", "schemas/v1.2.0/operate-cycle-display-workspace.schema.json"],
	["OPERATE_RECOVERY_DISPLAY_SURFACE_SCHEMA", "schemas/v1.2.0/operate-recovery-display-surface.schema.json"],
	["OPERATE_EXECUTIVE_BOARD_DISPLAY_SURFACE_SCHEMA", "schemas/v1.2.0/operate-executive-board-display-surface.schema.json"],
	["OPERATE_EXPERIENCE_DISPLAY_SURFACE_SCHEMA", "schemas/v1.2.0/operate-experience-display-surface.schema.json"],
	["OPERATE_EXPERIENCE_AUDIT_DISPLAY_SURFACE_SCHEMA", "schemas/v1.2.0/operate-experience-audit-display-surface.schema.json"],
	...
	["OPERATE_REVIEW_BOUND_SUBMISSION_SCHEMA", "schemas/v2.0.0/operate-review-bound-submission.schema.json"],
	["OPERATE_REVIEW_DISPLAY_WORKSPACE_SCHEMA", "schemas/v2.0.0/operate-review-display-workspace.schema.json"],
	...
	["OPERATING_EVIDENCE_RESOLUTION_SCHEMA", "schemas/v2.0.0/operating-evidence-resolution.schema.json"],
	["OPERATING_EXECUTION_RESULT_SCHEMA", "schemas/v2.0.0/operating-execution-result.schema.json"],
	["OPERATING_GOVERNED_OPERATION_SCHEMA", "schemas/v2.0.0/operating-governed-operation.schema.json"],
...
test("generated surface schema data is deterministic and check mode detects drift", async () => {
	const temporaryRoot = await mkdtemp(
		join(tmpdir(), "planr-surface-generation-"),
	);
	const generated = join(
		temporaryRoot,
		"operate-experience-surface-schema-data.mjs",
	);
	...
	try {
		for (let index = 0; index < 2; index += 1) {
			...
			if (index === 0) {
				...
				assert.equal(
					bytes,
					await readFile(join(temporaryRoot, "first.mjs"), "utf8"),
				);
				assert.equal(
					reviewBytes,
				...
				assert.equal(
					codecBytes,
					await readFile(join(temporaryRoot, "codec-first.mjs"), "utf8"),
				);
			}
		}
		const reviewExports = [...(await readFile(reviewGenerated, "utf8"))
			.matchAll(/^export const ([A-Z0-9_]+) =/gmu)]
			.map((match) => match[1]);
		...
		assert.match(
			surfaceBytes,
			/from "\.\/operate-review-schema-data\.mjs";/u,
		);
		assert.match(surfaceBytes, /\/\* @__PURE__ \*\/ decodeDashboardSchema/u);
		assert.match(await readFile(reviewGenerated, "utf8"), /\/\* @__PURE__ \*\/ decodeDashboardSchema/u);
		...
		assert.deepEqual(Object.keys(surfaceModule).sort(), SURFACE_SCHEMA_EXPORTS
		...
		for (const [exportName, filename] of REVIEW_SCHEMA_EXPORTS) {
			const canonical = JSON.parse(await readFile(
				join(root, "schemas/v2.0.0", filename),
				"utf8",
			));
			assert.deepEqual(surfaceModule[exportName], canonical, exportName);
		...
		const { decodeDashboardSchema } = await import(
			`${pathToFileURL(codecGenerated).href}${nonce}`
		);
		const firstPayload = (await readFile(reviewGenerated, "utf8"))
			.match(/decodeDashboardSchema\("([A-Za-z0-9+/=]+)"\)/u)?.[1];
		...
		for (const invalidPayload of [
			...
			Buffer.concat([
				Buffer.from(firstPayload, "base64"),
				Buffer.from([0x00]),
			]).toString("base64"),
		]) {
...
test("Today schema recursively rejects hostile collection values and contradictory status reasons", () => {
	...
	const hostile = [
		...
		[
			...
			(value) => {
				value.data.domainMetrics = [
					{ metricId: "metric-1", unknown: "private" },
				];
			},
		],
		[
