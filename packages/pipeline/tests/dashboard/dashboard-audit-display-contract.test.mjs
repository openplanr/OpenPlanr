...
const PRIVATE_ERROR_LOCATOR = 'private-repository-locator-must-not-cross-audit-export';
const PRIVATE_CLAIM = 'private-restricted-claim-must-not-cross-the-owner-boundary';
  ...
  return [
    {
      ...
      statement: "Retention <script>alert('public-text')</script> remains measurable.",
...
function privateInboxItem() {
  return {
    ...
    consequence: 'The read-only audit export retains only safe aggregate state.',
    expiresAt: null,
    blocking: true,
    evidence: [],
    requiredParties: [
...
function auditView(overrides = {}) {
  const omissions = [{ classification: 'restricted', count: 2, reason: 'access-denied' }];
  const base = {
    ...
    learnings: [
      {
        ...
        outcomeId: 'outcome-1',
        statement: 'Missing evidence is itself a reason to revisit the Decision.',
        decisionIds: ['decision-1'],
        evidenceRefIds: ['evidence-available'],
        createdAt: emptyFixture.generatedAt,
      },
...
function auditBinding(view, surface, { subjectId = null, query = null, format = null } = {}) {
  return {
...
function selected(view, surface, { subjectId = null, query = null, format = null } = {}) {
...
test('audit owner issues and synchronously verifies all six exact selected surfaces', () => {
  ...
  for (const [surface, options] of cases) {
    ...
    assert.equal(assertOperateExperienceAuditDisplaySurfaceV1(value, expected), value);
    assert.deepEqual(issueOperateExperienceAuditDisplaySurfaceV1(value.payload, expected), value);
  }
  ...
  assert.equal(
    assertOperateExperienceAuditDisplaySurfaceV1(evidenceDetail.value, evidenceDetail.expected),
...
test('restricted affirmative Outcomes preserve honest state without reopening hidden proof', () => {
  ...
  for (const [surface, options] of [
    ...
    assert.equal(assertOperateExperienceAuditDisplaySurfaceV1(value, expected), value);
    const projected =
      surface === 'outcome' ? value.payload.data.outcome : value.payload.data.outcomes[0];
...
test('restricted insufficient-evidence Outcomes close verification and metric custody', () => {
  ...
  assert.equal(
    assertOperateExperienceAuditDisplaySurfaceV1(selectedOutcome.value, selectedOutcome.expected),
    selectedOutcome.value,
  );
  assert.equal(selectedOutcome.value.payload.data.outcome.metric.target, null);
  assert.equal(selectedOutcome.value.payload.data.outcome.verification.method, null);
  assert.equal(selectedOutcome.value.payload.data.outcome.nextObservation, null);
  ...
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(reissued, selectedOutcome.expected),
  );
});
...
test('public issuer refuses re-committed privacy, truth, and route lookalikes', () => {
  ...
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(restoredPrivateClaim, evidence.expected),
  );

  const swappedEvidenceLink = structuredClone(evidence.value.payload);
  swappedEvidenceLink.data.evidence[0].deepLink = '#/operate/evidence/claim-retention';
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(swappedEvidenceLink, evidence.expected),
  );

  const falseSuccess = structuredClone(outcomes.value.payload);
  falseSuccess.data.outcomes[0].status = 'succeeded';
  assert.throws(() => issueOperateExperienceAuditDisplaySurfaceV1(falseSuccess, outcomes.expected));

  const foreignHistoryLink = structuredClone(history.value.payload);
  foreignHistoryLink.data.history[0].deepLinks = ['#/operate/outcomes/outcome-foreign'];
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(foreignHistoryLink, history.expected),
  );
  ...
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(privateExport, exported.expected),
  );
  ...
  assert.throws(() =>
    issueOperateExperienceAuditDisplaySurfaceV1(reorderedExport, exported.expected),
  );
});
...
test('audit JCS commitment rejects domain, digest, surface, order, and hostile JSON substitutions', () => {
  ...
  const mutations = [
    ...
    (candidate) => {
      candidate.integrity.contentHash = sha256Jcs(withoutContentHash(candidate));
    },
    (candidate) => {
    ...
    (candidate) => {
      candidate.payload.data.rationale[0].summary = 'same-schema rationale tamper';
    },
    (candidate) => {
  ...
  for (const mutate of mutations) {
    ...
    mutate(candidate);
    assert.equal(
      validateOperateExperienceAuditDisplaySurfaceV1(candidate, evidence.expected).length,
      1,
    );
    assert.throws(
      () => assertOperateExperienceAuditDisplaySurfaceV1(candidate, evidence.expected),
      (error) =>
        error.code === 'E_OPERATE_EXPERIENCE_AUDIT_DISPLAY_INVALID' &&
        !String(error.message).includes('private-envelope-member-must-not-echo'),
    );
  }

  const reordered = reverseTopLevelKeys(evidence.value);
  assert.equal(
    assertOperateExperienceAuditDisplaySurfaceV1(reordered, evidence.expected).integrity
      .contentHash,
...
test('audit verification refuses non-data graphs, accessors, proxies, and non-finite values without echo', () => {
  ...
  assert.throws(
    ...
    (error) => !String(error.message).includes('private-accessor-value-must-not-echo'),
  );
  assert.equal(reads, 0);
  assert.throws(() =>
    assertOperateExperienceAuditDisplaySurfaceV1(new Proxy(structuredClone(value), {}), expected),
  );
});
...
test('audit assertion independently requires every exact current selection binding field', () => {
  ...
  for (const { value, expected } of matrices) {
    for (const field of Object.keys(expected)) {
      ...
      assert.throws(() => assertOperateExperienceAuditDisplaySurfaceV1(value, substituted), field);
...
test('audit commitment rejects same-schema leaf changes for every selected surface', () => {
  ...
  for (const [{ value, expected }, mutate] of cases) {
    const candidate = structuredClone(value);
    mutate(candidate);
    assert.throws(() => assertOperateExperienceAuditDisplaySurfaceV1(candidate, expected));
  }
});
...
test('audit owner preserves row order and omits every unsupported or unowned navigation target', () => {
  ...
  assert.equal(evidence.evidence[0].deepLink, '#/operate/evidence/evidence-available');
  assert.equal(evidence.evidence[0].causalLinks[0].deepLink, '#/operate/outcomes/outcome-1');
  assert.equal(evidence.evidence[0].causalLinks[1].deepLink, null);
  assert.equal(evidence.evidence[1].deepLink, null);
  assert.equal(evidence.claims[0].deepLink, '#/operate/evidence/claim-retention');
  ...
  for (const query of ['act_00000001', 'learning-1', 'retention']) {
    const search = selected(view, 'search', { query }).value.payload.data.results;
  ...
  rehashView(encoded);
  assert.equal(selected(encoded, 'evidence').value.payload.data.evidence[0].deepLink, null);
});
...
test('audit search and Export preserve exact safe text while excluding restricted indexes and locators', () => {
  ...
  for (const query of [PRIVATE_ERROR_LOCATOR, 'claim-restricted', HASH_A]) {
    assert.deepEqual(selected(view, 'search', { query }).value.payload.data.results, [], query);
  }
  const publicSearch = selected(view, 'search', { query: '<script>' }).value.payload.data;
  assert.equal(publicSearch.query, '<script>');
  assert.equal(publicSearch.results[0].title.includes('<script>'), true);

  const jsonExport = selected(view, 'export', { format: 'json' }).value.payload.data;
  ...
  assert.equal(JSON.stringify(parsed).includes('private-owner-actor-must-not-cross-export'), false);
  assert.equal(JSON.stringify(parsed).includes('private-party-must-not-cross-export'), false);
  ...
  assert.equal(JSON.stringify(parsed).includes('#/operate/history/operation-1'), false);

  const first = selected(view, 'export', { format: 'html' }).value;
  const second = selected(structuredClone(view), 'export', {
    format: 'html',
  }).value;
  assert.deepEqual(second, first, 'same replayed state produces exact Export bytes');
  ...
  assert.equal(html.includes("&lt;script&gt;alert('public-text')&lt;/script&gt;"), true);
  assert.equal(html.includes(PRIVATE_ERROR_LOCATOR), false);
  assert.equal(html.includes('private-owner-actor-must-not-cross-export'), false);
...
test('audit restart bytes are deterministic and stale or late foreign envelopes fail expected custody', () => {
  ...
  assert.throws(() => assertOperateExperienceAuditDisplaySurfaceV1(early.value, later.expected));
  assert.throws(() => assertOperateExperienceAuditDisplaySurfaceV1(later.value, early.expected));
});
