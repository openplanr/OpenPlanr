...
function request(port, path, headers = {}) {
	return new Promise((resolvePromise, reject) => {
		...
		requestHandle.setTimeout(4_000, () =>
...
test("audit REST signs all exact surfaces and closes hostile transport input", async () => {
	...
	try {
		...
		const closedBeforeRead = [
			...
			[
				`/api/operate/evidence?${base}&cycleId=cycle-1&cycleId=cycle-1`,
			...
			[
				`/api/operate/%65vidence?${base}&cycleId=cycle-1`,
			...
			[
				`/api/operate/evidence/private/extra?${base}&cycleId=cycle-1`,
				"OPERATE_ROUTE_INVALID",
			],
			[
				`/api/operate/search?${base}&cycleId=cycle-1&q=a&q=b`,
				"OPERATE_QUERY_INVALID",
			],
			[
			...
			[
				`/api/operate/search?${base}&cycleId=cycle-1&q=${"x".repeat(513)}`,
				"OPERATE_QUERY_INVALID",
			],
			[
		...
		for (const [path, reasonCode] of closedBeforeRead) {
			...
			assert.equal(
				providerReads,
				readsBeforeRequest,
				`${path} must fail before owner access`,
			);
		}
		assert.equal(
			providerReads,
			0,
			"invalid grammar must fail before owner access",
		);

		for (const [path, foreignHeaders] of [
			[
				`/api/operate/evidence?${base}&cycleId=cycle-1`,
				{ "X-OpenPlanr-Actor": PRIVATE_QUERY },
			],
			[
			...
			assert.equal(response.body.includes(PRIVATE_QUERY), false);
			assert.equal(
				JSON.parse(response.body).error.reasonCode,
		...
		const cases = [
			...
			[
				"evidence",
				"/api/operate/evidence/evidence-available",
				{ subjectId: "evidence-available" },
			],
			["outcomes", "/api/operate/outcomes", {}],
			[
				"outcome",
				"/api/operate/outcomes/outcome-1",
				{ subjectId: "outcome-1" },
			],
		...
		for (const [surface, route, options] of cases) {
			const response = await request(
				port,
				`${route}?${queryFor(surface, options)}`,
				headers,
			);
			...
			assert.equal(
				assertOperateExperienceAuditDisplaySurfaceV1(envelope, expected),
				envelope,
			);
