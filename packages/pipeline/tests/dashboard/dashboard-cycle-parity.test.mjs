...
const stages = () =>
  ['observe', 'understand', 'decide', 'govern', 'act', 'verify', 'learn'].map((id, index) => ({
    id,
    state: 'complete',
    reason: null,
    ...
    dependencies: [
      {
        ...
        dependsOn: [{ actionId: 'act_dependency', state: 'completed', resolved: true }],
      },
    ],
    blockers: [
      {
...
test('Cycle workspace preserves canonical spine, owner work-link order, proof, and closed learning', () => {
  ...
  assert.deepEqual(validateOperateExperienceArtifactV2('operate-experience-view', view), []);
  for (const [kind, value] of [
    ['operating-cycle', cycleRead.data.cycle],
    ['operating-work-ledger', cycleRead.data.persistentWork.ledger],
  ]) {
    assert.deepEqual(validateProtocolArtifact(kind, value, { protocolVersion: '2.0.0' }), [], kind);
  }
  assert.deepEqual(
  ...
  assert.deepEqual(selected.data.cycle.dependencies, view.cycles[0].dependencies);
  ...
  assert.equal(selected.data.persistentWork.outcomes[0].status, 'insufficient-evidence');
  assert.equal(selected.data.persistentWork.learnings[0].statement, 'More observation is needed.');
  ...
  assert.equal(JSON.stringify(selected).includes('Revenue is below plan.'), false);
  assert.equal(JSON.stringify(selected).includes('Retention is the largest risk.'), false);
});
...
test('Cycle display workspace commits the sanitized owner DTO and rejects every post-owner substitution', () => {
  ...
  const display = selectOperateCycleDisplayWorkspace(input.view, input.cycleRead, {
  ...
  const mutations = [
    ...
    (candidate) => {
      candidate.payload.data.persistentWork.findings[0].privateBody = 'private-value-must-not-echo';
    },
  ];
...
test('Cycle HTTP detail composes the strict workspace from one exact owner read', async () => {
  ...
  try {
    ...
    const response = await request(port, `/api/operate/cycles/${input.binding.cycleId}?${query}`, {
...
test('Cycle and Executive Board display issuers preserve a structured durable Finding', () => {
  ...
  const cycleDisplay = selectOperateCycleDisplayWorkspace(input.view, input.cycleRead, {
    binding: input.binding,
    subjectId: input.binding.cycleId,
  });
...
test('Cycle verification requires affirmative bound proof and keeps result success separate', () => {
  ...
  for (const status of ['blocked', 'cancelled', 'partial', 'uncertain', 'unknown']) {
  ...
  for (const stage of noAction.view.cycles[0].stages) stage.persistentActionIds = [];
  ...
  rehashView(noAction);
  const selected = selectOperateCycleWorkspace(noAction.view, noAction.cycleRead, {
...
test('Cycle workspace fails closed for stale time, foreign binding, replay, route, and link drift', () => {
  ...
  const cases = [
    ...
    },
    {
      ...
      cycleRead: structuredClone(original.cycleRead),
      mutate(value) {
        value.data.persistentWork.cycleLinks = [...value.data.persistentWork.cycleLinks].reverse();
      },
      reason: 'OPERATE_PROJECTION_STALE',
    },
    ...
    })),
    {
      ...
      cycleRead: structuredClone(original.cycleRead),
      mutate(value) {
        value.data.persistentWork.ledger.actions[0].sourceCycleId = 'cyc_foreign_0001';
      },
      reason: 'OPERATE_PROJECTION_STALE',
    },
    {
      ...
      mutate(value) {
        const retained = value.data.persistentWork.ledger.cycleLinks.filter(
          (link) => !(link.entityKind === 'finding' && link.relation === 'source'),
        );
    ...
    },
    {
      ...
      cycleRead: structuredClone(original.cycleRead),
      mutate(value) {
        const duplicate = structuredClone(value.data.persistentWork.ledger.cycleLinks[0]);
...
test('packed installed Node 20 Cycle reader matches source bytes and behavior', {
  ...
  assert.deepEqual(Object.keys(operateExperienceReader).sort(), EXPECTED_READER_EXPORTS);
  assert.deepEqual(
    Object.keys(operateReviewDisplayContract).sort(),
    EXPECTED_REVIEW_CONTRACT_EXPORTS,
  );
  ...
  try {
    ...
    assert.equal(sha256(join(installedRoot, relativeTypes)), sha256(join(root, relativeTypes)));
    for (const relativePath of reviewFiles) {
      assert.equal(
        sha256(join(installedRoot, relativePath)),
        sha256(join(root, relativePath)),
        relativePath,
      );
    }
    ...
    assert.deepEqual(
      Object.keys(installedReviewProjection).sort(),
      EXPECTED_REVIEW_PROJECTION_EXPORTS,
    );
    ...
    assert.deepEqual(
      installedReviewContract.validateOperateReviewDisplayWorkspaceV1(installedFixture),
      [],
    );
    const input = cycleWorkspace();
    assert.deepEqual(
