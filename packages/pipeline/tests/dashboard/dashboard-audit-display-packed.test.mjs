...
const SCHEMA_SUBPATH =
	"planr-pipeline/schemas/v1.2.0/operate-experience-audit-display-surface.mjs";
const CONTRACT_SUBPATH =
	"planr-pipeline/dashboard/operate-experience-audit-display-contract";
...
function auditFixture() {
	...
	return {
		binding,
		display: auditContract.issueOperateExperienceAuditDisplaySurfaceV1(
			payload,
			binding,
		),
	};
...
test("audit verifier source, packed bytes, public exports, and declarations have exact parity", {
	...
	assert.deepEqual(
		metadata.exports["./dashboard/operate-experience-audit-display-contract"],
		{
	...
	const temporaryRoot = mkdtempSync(
		join(tmpdir(), "planr-audit-display-pack-"),
	);
	try {
		...
		assert.equal(
			installedContract.assertOperateExperienceAuditDisplaySurfaceV1(
				display,
				binding,
...
test("packed audit verifier bundles synchronously without Node built-ins", {
	...
	const temporaryRoot = mkdtempSync(
		join(tmpdir(), "planr-audit-display-vite-"),
	);
	try {
		...
		assert.doesNotMatch(
			bundle,
			/(?:createHash|readFileSync|protocol\/loader)/u,
		);
...
test("packed audit verifier executes under exact Node 20.0.0", {
	...
	const temporaryRoot = mkdtempSync(
		join(tmpdir(), "planr-audit-display-node20-"),
	);
	try {
