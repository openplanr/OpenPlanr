...
function unavailableApproval() {
	return {
		...
		redactions: [
			{ classification: "confidential", count: 1, reason: "identity-redacted" },
		],
		actionLocator: null,
		navigationLocator: null,
		unavailableReason: {
...
test("owner issues one standalone exact-bound Inbox display without Today or full-view fallback", () => {
	...
	assert.deepEqual(
		[...new Set(display.payload.data.inbox.map(({ kind }) => kind))].sort(),
	...
	assert.equal(
		decision.actionLocator.actionDigest,
		sha256Jcs(view.allowedActions[0].action),
	);
...
test("Inbox assertion rejects every binding, generation, order, evidence, locator, and XOR substitution", () => {
	...
	for (const field of ["actorId", "projectId", "scopeId", "domainId", "domainVersion", "generation", "subjectId", "eventHead", "viewHash", "surface"]) {
	...
	const mutations = [
		...
		(candidate) => {
			itemBySubject(candidate.inbox, "act_00000001").requiredParties[0].actorId = "restricted-actor-private";
		},
		(candidate) => {
...
test("browser-safe preview assertion binds action digest and hash-covers every transition claim", () => {
	...
	assert.throws(() =>
		assertOperateExperiencePreviewV1(
			digestSubstitution,
			previewBinding(digestSubstitution),
		),
	);
...
test("approval preview distinguishes recorded parties remaining from threshold satisfied without claiming execution", () => {
	...
	for (const preview of [partiesRemain, thresholdSatisfied]) {
		assert.deepEqual(
			assertOperateExperiencePreviewV1(preview, previewBinding(preview)),
			preview,
		);
	...
	assert.throws(() =>
		assertOperateExperiencePreviewV1(hostile, previewBinding(hostile)),
	);
});
...
test("display and preview assertions reject accessors, proxies, and unsupported members as whole envelopes", () => {
	...
	Object.defineProperty(
		itemBySubject(accessor.payload.data.inbox, "dec_00000001"),
		"title",
		{
		enumerable: true,
		get() {
	...
	assert.throws(() =>
		assertOperateExperienceDisplaySurfaceV1(new Proxy(display, {}), binding),
	);
	...
	assert.throws(() =>
		assertOperateExperiencePreviewV1(previewAccessor, previewBinding(preview)),
	);
	assert.throws(() =>
		assertOperateExperiencePreviewV1(
			new Proxy(preview, {}),
...
test("Inbox display and preview verifier have source, packed, public-import, and browser-bundle parity", {
	...
	try {
		...
		const preview = reviewPreview();
		assert.equal(
			installed.assertOperateExperienceDisplaySurfaceV1(display, binding)
				.integrity.contentHash,
			display.integrity.contentHash,
		);
		assert.equal(
			installed.assertOperateExperiencePreviewV1(
				preview,
				previewBinding(preview),
			).previewHash,
			preview.previewHash,
