...
function recoveryFixture(status = 'repairable') {
  ...
  const recoveryRead = {
    ...
    data: {
      ...
      allowedRecovery: status === 'repairable' ? 'clear-stale-lock' : 'none',
      lock: {
...
test('recovery display sanitizes inspection and commits stable recovery state', () => {
  ...
  const display = selectOperateRecoveryDisplay(input.view, input.recoveryRead, {
  ...
  assert.equal(JSON.stringify(display).includes('externally-anchored-or-signed-custody'), false);
});
...
test('recovery display rejects stale inspection and post-owner substitutions', () => {
  ...
  const staleDisplay = selectOperateRecoveryDisplay(stale.view, stale.recoveryRead, {
  ...
  assert.throws(() => assertOperateRecoveryDisplaySurfaceV1(hostile, input.binding));
  ...
  for (const mutate of [
    ...
    mutate(candidate);
    assert.equal(validateOperateRecoveryDisplaySurfaceV1(candidate, input.binding).length, 1);
  }
});
