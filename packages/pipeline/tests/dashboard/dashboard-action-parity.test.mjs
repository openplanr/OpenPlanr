...
test("action display workspace commits exact custody and rejects deep-link substitution", () => {
	...
	assert.throws(() =>
		assertOperateActionDisplayWorkspaceV1(hostile, input.binding),
	);
});
...
test("action commandability is owner-issued only for a ready Action with an exact command and gateway", () => {
	...
	assert.equal(
		assertOperateActionDisplayWorkspaceV1(commandable, input.binding),
		commandable,
	);
...
test("action display rejects post-owner integrity substitutions", () => {
	...
	for (const mutate of mutations) {
		...
		mutate(candidate);
		assert.equal(
			validateOperateActionDisplayWorkspaceV1(candidate, input.binding).length,
			1,
		);
		assert.throws(() =>
			assertOperateActionDisplayWorkspaceV1(candidate, input.binding),
		);
	}
});
