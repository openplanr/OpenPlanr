...
import { createShipClosure, finalizeStoredShipClosure } from '../../lib/pipeline/ship-closure.mjs';
...
function taskMarkdown({ id, storyId = 'US-001', dependsOn = [], preserve = [] }) {
  const preserveYaml =
    preserve.length === 0
      ...
      : [
          'preserve:',
          ...preserve.flatMap(({ repositoryKey, path }) => [
...
function project(slug = 'hostile', { tasks = [{ id: 'T-001' }], commit = true } = {}) {
  ...
  writeFileSync(
    join(prepared.specDir, 'stories', 'US-001-hostile.md'),
    ['---', 'id: "US-001"', 'status: "pending"', 'updated: "2026-08-21"', '---', ''].join('\n'),
  );
...
function addFeature(root, slug, { taskId = 'T-001' } = {}) {
  ...
  writeFileSync(
    join(prepared.specDir, 'tasks', `${taskId}-hostile.md`),
    taskMarkdown({ id: taskId }),
  );
  git(root, 'add', '.');
...
function taskCompleted(
  generation,
  taskId = 'T-001',
  { filesWritten = [], filesModified = [repositoryPath('src/app.js')] } = {},
) {
  return {
    type: 'task.completed',
...
function reviewClosed(
  state,
  { phase, findings = [], reviewedFindingIds = [], reviewerIds, contributions } = {},
) {
  ...
  return {
    ...
    contributions:
      contributions ??
      state.reviewerRoster.map((reviewerId) => ({
        reviewerId,
...
function rebindSingleCandidate(receipt, mutateInventory) {
  ...
  for (const repository of candidate.repositories) {
    repository.inventoryDigest = sha256Jcs(
      candidate.inventory.filter(({ repositoryKey }) => repositoryKey === repository.repositoryKey),
    );
  }
  candidate.digest = sha256Jcs({
    repositories: candidate.repositories,
    inventory: candidate.inventory,
  });
  for (const review of value.reviews)
    if (review.candidateDigest === oldDigest) review.candidateDigest = candidate.digest;
  for (const evidence of value.gateEvidence)
    if (evidence.candidateDigest === oldDigest) evidence.candidateDigest = candidate.digest;
  if (value.terminal.candidateDigest === oldDigest)
...
function correctedReceipt(root, slug) {
  ...
  let summary = advance(
    ...
    slug,
    started.runId,
    reviewClosed(state, { phase: 'initial', findings: [finding()] }),
  );
  ...
  runShipGates({ projectRoot: root, feature: slug, runId: started.runId, phase: 'final' });
  const summaryReceipt = finalizeShipClosure({
    projectRoot: root,
    feature: slug,
    runId: started.runId,
  });
...
test('feature-wide start serialization permits exactly one concurrent active run', async () => {
  ...
  assert.ok(
    ['E_SHIP_ACTIVE', 'E_SHIP_LOCKED'].includes(loser.code),
    `unexpected untyped concurrent-start failure: ${loser.code}`,
  );
...
test('active event replay is exact-only and illegal/backward transitions are rejected', () => {
  ...
  errorCode(
    () => advance(root, 'illegal', started.runId, reviewOpened(0)),
  ...
  const eventId = getShipClosure({ projectRoot: root, feature: 'illegal', runId: started.runId })
    .events[0].eventId;
  errorCode(
    () =>
      advance(root, 'illegal', started.runId, {
        ...completion,
        eventId,
  ...
  errorCode(
    () => advance(root, 'illegal', started.runId, taskCompleted(opened.generation)),
    'E_SHIP_STATE_TRANSITION_INVALID',
  );
  let state = getShipClosure({ projectRoot: root, feature: 'illegal', runId: started.runId });
  errorCode(
    () => advance(root, 'illegal', started.runId, reviewClosed(state, { phase: 'targeted' })),
    'E_SHIP_STATE_TRANSITION_INVALID',
  );
  errorCode(
    () => advance(root, 'illegal', started.runId, reviewOpened(state.generation)),
    'E_SHIP_STATE_TRANSITION_INVALID',
  );
});
...
test('review close requires exact roster membership and one contribution per declared reviewer', () => {
  ...
  errorCode(
    () => advance(root, 'roster', started.runId, { ...valid, reviewerIds: ['qa-agent'] }),
    'E_SHIP_REVIEWER_UNDECLARED',
  );
  errorCode(
    () =>
      advance(root, 'roster', started.runId, {
        ...valid,
        contributions: valid.contributions.slice(0, 1),
      }),
    'E_SHIP_REVIEWER_UNDECLARED',
  );
  errorCode(
    () =>
      advance(root, 'roster', started.runId, {
        ...
        contributions: [
          ...
          { ...valid.contributions[1], reviewerId: 'qa-agent' },
        ],
      }),
    'E_SHIP_REVIEWER_UNDECLARED',
  );
  errorCode(
    () =>
      advance(root, 'roster', started.runId, {
        ...
        contributions: [
          valid.contributions[0],
          { ...valid.contributions[1], reviewerId: 'foreign-agent' },
        ],
      }),
    'E_SHIP_REVIEWER_UNDECLARED',
  );
...
test('cyclic task and gate DAGs are rejected before custody is written', () => {
  ...
  errorCode(
    () =>
      startShip({ projectRoot: taskCycle.root, feature: 'task-cycle', humanReviewConfirmed: true }),
  ...
  const gates = [
    ...
    },
    {
      ...
      repositoryKey: 'project',
      argv: ['node', '--version'],
      inputs: [repositoryPath('tests/smoke.test.mjs')],
      dependsOn: ['a'],
        ...
        gates: [
          {
            ...
            repositoryKey: 'project',
            argv: ['node', '--version'],
            inputs: [repositoryPath('src/app.js', 'undeclared')],
            dependsOn: [],
            finalRelevantSuite: true,
          },
        ],
...
test('one correction is the hard revision ceiling and arbitrary late review events cannot reopen it', () => {
  ...
  errorCode(
    () =>
      advance(root, 'finite', started.runId, {
        ...correction,
        expectedGeneration: summary.generation,
      }),
    'E_SHIP_STATE_TRANSITION_INVALID',
...
test('a sealed correction successor must receive its targeted review before finalization', () => {
  ...
  assert.equal(
    getShipClosure({ projectRoot: root, feature: 'targeted-required', runId: started.runId })
      .recordType,
    'active',
  );
});
...
test('receipt validation rejects circular or future final gate evidence reuse', () => {
  ...
  const receipt = JSON.parse(readFileSync(summary.receiptPath, 'utf8'));
  const reusable = receipt.gateEvidence.find(
    ({ phase, status }) => phase === 'final' && status === 'reused',
  );
...
test('storage rejects symlink custody, reclaims provably stale locks, and preserves live locks', () => {
  ...
  errorCode(
    () =>
      startShip({ projectRoot: unsafe.root, feature: 'unsafe-store', humanReviewConfirmed: true }),
  ...
  writeFileSync(
    featureLock,
    `${JSON.stringify({ pid: 2147483647, host: hostname(), processStart: 'not-running', createdAt: new Date().toISOString() })}\n`,
  );
  assert.equal(
    startShip({ projectRoot: stale.root, feature: 'stale-lock', humanReviewConfirmed: true }).state,
    'implementing',
  );
  ...
  writeFileSync(
    join(liveLocks, 'feature-start.lock'),
    `${JSON.stringify({ pid: process.pid, host: hostname(), processStart: null, createdAt: new Date().toISOString() })}\n`,
  );
  errorCode(
    () => startShip({ projectRoot: live.root, feature: 'live-lock', humanReviewConfirmed: true }),
    'E_SHIP_ACTIVE',
  );
  ...
  errorCode(
    () =>
      startShip({
        projectRoot: unsafeLockDir.root,
        feature: 'unsafe-lock-dir',
        humanReviewConfirmed: true,
      }),
    'E_SHIP_STORAGE_UNSAFE',
  );
  ...
  const linkedTarget = join(
    mkdtempSync(join(tmpdir(), 'openplanr-linked-custody-')),
    'record.json',
  );
...
test('structured Preserve covers file bytes, modes, directory trees, rename/deletion, and broken symlink targets', () => {
  ...
  writeFileSync(
    ...
    taskMarkdown({
      id: 'T-001',
      preserve: [
        repositoryPath('protected-file'),
        repositoryPath('protected-tree'),
        repositoryPath('broken-link'),
      ],
    }),
  );
  git(root, 'add', '.');
  ...
  const expectViolation = (mutate, restore, expectedPath) => {
    ...
    assert.throws(
      ...
      (error) =>
        error?.code === 'E_PRESERVE_VIOLATION' &&
  ...
  expectViolation(
    () => {
      unlinkSync(join(root, 'broken-link'));
      symlinkSync('missing-b', join(root, 'broken-link'));
    },
    () => {
      unlinkSync(join(root, 'broken-link'));
      symlinkSync('missing-a', join(root, 'broken-link'));
  ...
  assert.equal(
    advance(root, 'preserve-hostile', started.runId, reviewOpened(complete.generation)).state,
    'reviewing_initial',
  );
});
...
test('candidate identity binds dirty baseline plus add/delete/rename/symlink/mode surfaces', () => {
  ...
  const baseline = getShipClosure({ projectRoot: root, feature: 'inventory', runId: started.runId })
    .repositories[0].baselineDigest;
  ...
  const completed = advance(
    ...
    'inventory',
    started.runId,
    taskCompleted(0, 'T-001', {
      filesWritten: [repositoryPath('added.txt'), repositoryPath('new-link')],
      filesModified: [
        repositoryPath('src/renamed.js'),
        repositoryPath('tests/smoke.test.mjs'),
        repositoryPath('input/tech/stack.md'),
      ],
    }),
  );
  advance(root, 'inventory', started.runId, reviewOpened(completed.generation));
  const candidate = getShipClosure({
    projectRoot: root,
    feature: 'inventory',
    runId: started.runId,
...
test('gate reuse includes dependency evidence and mandatory final suite always executes', () => {
  ...
  const gates = [
    {
      id: 'gate-a',
      repositoryKey: 'project',
      argv: [process.execPath, '-e', append(counterA)],
      inputs: [repositoryPath('src/app.js')],
      dependsOn: [],
      finalRelevantSuite: false,
    },
    {
      id: 'gate-b',
      repositoryKey: 'project',
      argv: [process.execPath, '-e', append(counterB)],
      inputs: [repositoryPath('tests/smoke.test.mjs')],
      dependsOn: ['gate-a'],
      finalRelevantSuite: true,
    },
  ...
  const started = startShip({
    projectRoot: root,
    feature: 'gate-custody',
    humanReviewConfirmed: true,
    gates,
  });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  completeAndOpen(root, 'gate-custody', started.runId);
  const initialEvidence = runShipGates({
    projectRoot: root,
    feature: 'gate-custody',
    runId: started.runId,
    phase: 'initial',
  ...
  assert.notEqual(
    targetedEvidence.find(({ gateId }) => gateId === 'gate-b').inputDigest,
    initialEvidence.find(({ gateId }) => gateId === 'gate-b').inputDigest,
  );
  state = getShipClosure({ projectRoot: root, feature: 'gate-custody', runId: started.runId });
  summary = advance(
    root,
  ...
  const finalEvidence = runShipGates({
    projectRoot: root,
    feature: 'gate-custody',
    runId: started.runId,
    phase: 'final',
...
test('a cross-repository bounded input invalidates reusable evidence when only that repository changes', () => {
  ...
  const gates = [
    {
      ...
      argv: [process.execPath, '-e', append(artifactCounter)],
    ...
    },
    {
      id: 'project-final',
      repositoryKey: 'project',
      argv: [process.execPath, '-e', append(finalCounter)],
      inputs: [repositoryPath('tests/smoke.test.mjs')],
      dependsOn: ['secondary-artifact'],
      finalRelevantSuite: true,
    },
  ];
  ...
  const started = startShip({
    projectRoot: root,
    feature: 'cross-repository-gate-input',
    humanReviewConfirmed: true,
  });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  ...
  let summary = advance(
    root,
    'cross-repository-gate-input',
    started.runId,
    reviewClosed(state, {
      phase: 'initial',
      findings: [finding({ paths: [repositoryPath('src/app.js')] })],
    }),
  );
  state = getShipClosure({
    projectRoot: root,
    feature: 'cross-repository-gate-input',
    runId: started.runId,
  });
  ...
  summary = advance(
    ...
    'cross-repository-gate-input',
    started.runId,
    reviewOpened(summary.generation),
  );
...
test('gate runner issues exact sealed repository roots and strips spoofed capabilities', () => {
  ...
  const observedPath = join(
    mkdtempSync(join(tmpdir(), 'openplanr-gate-root-observed-')),
    'env.json',
  );
...
test('projection crash repairs from receipt and never duplicates manifest or provenance truth', () => {
  ...
  const preparedForFinalization = prepareShip({
    projectRoot: root,
    feature: 'projection-repair',
    humanReviewConfirmed: true,
  });
  const hostileTarget = join(
    mkdtempSync(join(tmpdir(), 'openplanr-projection-target-')),
    'hostile-target',
  );
  ...
  assert.equal(
    existsSync(join(prepared.specDir, '.ship', 'receipts', `${started.runId}.json`)),
    true,
  );
  unlinkSync(qaReport);
  const repaired = finalizeShipClosure({
    projectRoot: root,
    feature: 'projection-repair',
    runId: started.runId,
  });
  ...
  const rows = readFileSync(join(prepared.specDir, '.run-manifest.jsonl'), 'utf8')
    .trim()
    .split('\n')
    .map(JSON.parse);
  assert.equal(rows.filter(({ stage }) => stage === `ship.closure:${started.runId}`).length, 1);
  const provenance = readFileSync(join(root, '.planr', 'provenance.jsonl'), 'utf8')
    .trim()
    .split('\n')
    .map(JSON.parse);
...
test('dual active/receipt crash recovery accepts only an exact active prefix', () => {
  ...
  const activePrefix = getShipClosure({
    projectRoot: root,
    feature: 'dual-custody',
    runId: started.runId,
  });
  ...
  assert.equal(
    getShipClosure({ projectRoot: root, feature: 'dual-custody', runId: started.runId }).recordType,
    'receipt',
  );
  ...
  errorCode(
    () => getShipClosure({ projectRoot: root, feature: 'dual-custody', runId: started.runId }),
    'E_SHIP_CLOSURE_DIVERGED',
  );
  assert.equal(
    existsSync(activePath),
    true,
    'divergent custody remains available for forensic recovery',
  );
});
...
test('replaying a superseded receipt cannot overwrite the latest leaf projections', () => {
  ...
  const second = finalizeShipClosure({
    projectRoot: root,
    feature: 'projection-leaf',
    runId: reopened.runId,
  });
  ...
  try {
    const replay = finalizeShipClosure({
      projectRoot: root,
      feature: 'projection-leaf',
      runId: first.runId,
    });
...
test('terminal projection repair refuses to re-certify source bytes changed after receipt', () => {
  ...
  errorCode(
    () =>
      finalizeShipClosure({ projectRoot: root, feature: 'projection-stale', runId: receipt.runId }),
    'E_SHIP_CANDIDATE_STALE',
  );
...
test('read custody rejects a symlinked active-directory ancestor', () => {
  ...
  errorCode(
    () => getShipClosure({ projectRoot: root, feature: 'read-symlink', runId: started.runId }),
    'E_SHIP_STORAGE_UNSAFE',
  );
});
...
test('disjoint explicit task runs retain mixed terminal projection without recertifying completed scope', () => {
  ...
  const blocked = finalizeShipClosure({
    projectRoot: root,
    feature: 'disjoint',
    runId: second.runId,
  });
  ...
  assert.match(
    readFileSync(join(prepared.specDir, 'tasks', 'T-001-hostile.md'), 'utf8'),
    /status: "done"/,
  );
  assert.match(
    readFileSync(join(prepared.specDir, 'tasks', 'T-002-hostile.md'), 'utf8'),
    /status: "blocked"/,
  );
  assert.match(
    readFileSync(join(prepared.specDir, 'stories', 'US-001-hostile.md'), 'utf8'),
    /status: "blocked"/,
  );
});
...
test('owner reopen permits only cross-repository input expansion and forms one linear chain', () => {
  ...
  writeFileSync(
    ...
    'BuildCommand: "node --check src/secondary.js"\nTestCommand: "node --test tests/secondary.test.mjs"\n',
  );
  writeFileSync(join(second, 'src', 'secondary.js'), 'export const secondary = true;\n');
  writeFileSync(
    join(second, 'tests', 'secondary.test.mjs'),
    "import test from 'node:test'; test('secondary', () => {});\n",
  );
  git(second, 'init', '-q');
  ...
  assert.deepEqual(
    active.repositories.map(({ repositoryKey }) => repositoryKey),
    ['project', 'secondary'],
  );
  assert.ok(active.gates.some(({ repositoryKey }) => repositoryKey === 'secondary'));
  advance(root, 'multi-reopen', started.runId, taskBlocked(0));
  const receipt = finalizeShipClosure({
    projectRoot: root,
    feature: 'multi-reopen',
    runId: started.runId,
  });
  errorCode(
    () => startShip({ projectRoot: root, feature: 'multi-reopen', humanReviewConfirmed: true }),
  ...
  rejectGateMutation((gates) => {
    gates[0].inputs = [repositoryPath('src/app.js')];
  });
  rejectGateMutation((gates) => {
    gates[0].argv = ['node', '--version'];
  });
  rejectGateMutation((gates) => {
    gates[0].repositoryKey = 'secondary';
  });
  rejectGateMutation((gates) => {
    gates[1].dependsOn = [];
  });
  ...
  const successor = getShipClosure({
    projectRoot: root,
    feature: 'multi-reopen',
    runId: reopened.runId,
  });
  ...
  assert.deepEqual(
    successor.repositories.map(({ repositoryKey }) => repositoryKey),
    ['project', 'secondary'],
  );
...
test('terminal start replay is exact-only and ordinary overlap cannot mint a new run', () => {
  ...
  writeFileSync(
    ...
    taskMarkdown({
      id: 'T-001',
      preserve: [storyPath, taskPath, specPath].map((absolute) =>
        repositoryPath(relative(root, absolute).split('\\').join('/')),
      ),
    }),
  );
  git(root, 'add', '.');
  ...
  assert.equal(terminal.state, 'blocked');
  const replay = createShipClosure({
    projectRoot: root,
    prepared: preview,
    runtime: 'codex',
    runId,
  ...
  assert.equal(replay.replayed, true);
  errorCode(
    () => createShipClosure({ projectRoot: root, prepared: preview, runtime: 'cursor', runId }),
    'E_SHIP_RUN_ID_CONFLICT',
  );
  errorCode(
    () => startShip({ projectRoot: root, feature: 'terminal-replay', humanReviewConfirmed: true }),
    'E_SHIP_REOPEN_REQUIRED',
  );
});
...
test('CLI exposes resumable compact custody and rejects caller-authored truth/hidden authority', () => {
  ...
  const completed = cli(
    root,
    ['advance-ship', 'cli-resume', '--run-id', started.runId, '--event-file', '-', '--json'],
    {
  ...
  assert.equal(
    cli(
      root,
      ['finalize-ship', 'cli-resume', '--run-id', started.runId, '--qa', 'passed', '--json'],
      { expectFailure: true },
    ).code,
    'E_SHIP_LEGACY_TRUTH_FORBIDDEN',
  );
  assert.equal(
    cli(
      root,
      ['start-ship', 'cli-resume', '--reviewed', '--run-id', `ship_${'f'.repeat(32)}`, '--json'],
      { expectFailure: true },
    ).code,
    'E_SHIP_OWNER_CONFIG_FORBIDDEN',
  );
});
...
test('active custody remains resumable after stack configuration becomes invalid', () => {
  ...
  assert.equal(
    finalizeShipClosure({ projectRoot: root, feature: 'stack-drift', runId: started.runId }).state,
    'passed',
  );
});
...
test('terminal summaries mark compatibility projections unverified after source or task-status tamper', () => {
  ...
  let preview = prepareShip({
    projectRoot: source.root,
    feature: 'projection-verify-source',
    humanReviewConfirmed: true,
  });
  assert.equal(
    preview.closure.terminal.find(({ runId }) => runId === sourceReceipt.runId).projectionVerified,
    true,
  );
  ...
  writeFileSync(
    taskFile,
    readFileSync(taskFile, 'utf8').replace('status: "done"', 'status: "blocked"'),
  );
  preview = prepareShip({
    projectRoot: task.root,
    feature: 'projection-verify-task',
    humanReviewConfirmed: true,
  });
  assert.equal(
    preview.closure.terminal.find(({ runId }) => runId === taskReceipt.runId).projectionVerified,
    false,
  );
});
...
test('a foreign valid receipt copied beneath a sibling feature is rejected by storage context', () => {
  ...
  const sourcePath = join(
    first.prepared.specDir,
    '.ship',
    'receipts',
    `${firstReceipt.runId}.json`,
  );
...
test('mandatory final relevant suite executes even when a dependency fails', () => {
  ...
  const gates = [
    {
      ...
      argv: [
        process.execPath,
        '-e',
        `const fs=require('node:fs');process.exit(fs.readFileSync(${JSON.stringify(join(root, '.gate-state'))},'utf8').trim()==='pass'?0:1)`,
      ],
  ...
  assert.equal(readFileSync(counter, 'utf8'), 'xx');
  assert.equal(
    finalizeShipClosure({ projectRoot: root, feature: 'final-after-failure', runId: started.runId })
      .state,
    'blocked',
  );
});
...
test('projecting a second feature preserves the first feature provenance event', () => {
  ...
  finalizeShipClosure({
    projectRoot: first.root,
    feature: 'provenance-b',
    runId: secondReceipt.runId,
  });
  const events = readFileSync(join(first.root, '.planr', 'provenance.jsonl'), 'utf8')
    .trim()
    .split('\n')
    .map(JSON.parse);
...
test('planning and closure reads reject intermediate specs symlinks and copied storage contexts', () => {
  ...
  errorCode(
    () =>
      prepareShip({
        projectRoot: symlinked.root,
        feature: 'specs-symlink',
        humanReviewConfirmed: true,
      }),
    'E_SHIP_STORAGE_UNSAFE',
  );
...
test('terminal summaries use one latest-owner verification result when successor bytes become stale', () => {
  ...
  assert.ok(
    preview.closure.terminal.every(({ projectionVerified }) => projectionVerified === false),
  );
});
...
test('projection verification requires exact ordered current repository membership', () => {
  ...
  assert.equal(
    verifyShipCompatibilityProjection(receipt, {
      projectRoot: root,
...
test('aggregate PASS never trusts an unreceipted planning task already marked done', () => {
  ...
  assert.match(readFileSync(join(prepared.specDir, 'qa-report.md'), 'utf8'), /Status: BLOCKED/);
  assert.match(
    readFileSync(join(prepared.specDir, basename(prepared.specDir) + '.md'), 'utf8'),
    /status: "in-pipeline"/,
  );
});

test('frozen runs ignore null or empty mutable gate configuration while resuming', () => {
  for (const [suffix, gates] of [
    ['null', null],
    ['empty', []],
  ]) {
    ...
    writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);
    assert.equal(
      getShipClosure({ projectRoot: root, feature: slug, runId: started.runId }).runId,
      started.runId,
    );
    completeAndOpen(root, slug, started.runId);
    closeCleanInitial(root, slug, started.runId);
    runShipGates({ projectRoot: root, feature: slug, runId: started.runId, phase: 'final' });
    assert.equal(
      finalizeShipClosure({ projectRoot: root, feature: slug, runId: started.runId }).state,
      'passed',
    );
  }
});
...
test('a no-review blocked receipt binds candidate sealing to closure finalization time', () => {
  ...
  const receipt = JSON.parse(readFileSync(summary.receiptPath, 'utf8'));
  receipt.candidateRevisions[0].sealedAt = new Date(
    Date.parse(receipt.terminal.at) - 1,
  ).toISOString();
