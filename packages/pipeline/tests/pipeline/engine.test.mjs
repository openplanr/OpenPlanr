...
function decompose(root, slug = 'auth') {
  ...
  writeFileSync(join(prepared.specDir, 'stories', 'US-001-login.md'), declaredArtifact({
    id: 'US-001',
    title: 'Log in',
    specId: 'SPEC-001',
    slug: 'login',
  ...
  writeFileSync(join(prepared.specDir, 'tasks', 'T-001-api.md'), declaredArtifact({
    id: 'T-001',
  ...
  writeFileSync(join(prepared.specDir, 'tasks', 'T-002-ui.md'), declaredArtifact({
    id: 'T-002',
...
function decomposeDeclared(root, { spec = {}, story = {}, task = {} } = {}) {
  ...
  if (Object.keys(spec).length > 0) {
    ...
    for (const [key, value] of Object.entries(spec)) {
      markdown = markdown.replace(new RegExp(`^${key}:.*$`, 'mu'), `${key}: ${JSON.stringify(value)}`);
    }
...
test('completePlan validates declared Protocol v1 story and task frontmatter', () => {
  ...
  const result = completePlan({ projectRoot: root, feature: 'auth', runtime: 'codex', runId: 'run-schema' });
  assert.equal(result.stories, 1);
...
test('completePlan validates and records the exact default-mode spec with us-N story discovery', () => {
  ...
  const result = completePlan({ projectRoot: root, feature: 'auth', runtime: 'codex', runId: 'run-default' });
...
test('completePlan keeps a truly schema-less default plan readable', () => {
  ...
  assert.equal(result.stories, 1);
  assert.equal(result.tasks, 1);
  assert.match(readFileSync(join(root, '.planr', 'provenance.jsonl'), 'utf8'), /input\/specs\/spec-auth\.md/u);
});
...
test('a Protocol v1 parent requires Protocol v1 child frontmatter', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'E_PLAN_ARTIFACT_SCHEMA'
  ...
  for (const fixture of cases) {
    ...
    assert.throws(
      ...
      (error) => {
        ...
        assert.match(error.details?.artifactPath ?? '', new RegExp(`${fixture.path.replaceAll('/', '\\/')}$`, 'u'));
...
test('an unknown task reports the searched root and every available selector', () => {
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assert.deepEqual(error.details?.candidates.map(({ selector }) => selector), [
        'US-001/T-001',
        'US-002/T-002',
      ]);
...
test('default task readiness resolves duplicate IDs inside each parent story', () => {
  ...
  assert.deepEqual(
    prepared.unresolvedTasks.map(({ selector, dependencySelectors }) => ({ selector, dependencySelectors })),
    [
...
test('a qualified default task selector chooses one usable story-scoped task', () => {
  ...
  assert.deepEqual(selected.unresolvedTasks.map(({ id, selector }) => ({ id, selector })), [
