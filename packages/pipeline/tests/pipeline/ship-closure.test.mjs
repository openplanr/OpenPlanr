...
function project(slug = 'closure') {
  ...
  writeFileSync(join(prepared.specDir, 'stories', 'US-001-close.md'), '---\nid: "US-001"\nstatus: "pending"\nupdated: "2026-08-21"\n---\n');
  writeFileSync(join(prepared.specDir, 'tasks', 'T-001-core.md'), [
    '---',
    'id: "T-001"',
    'storyId: "US-001"',
    'status: "pending"',
...
function finding({ id = null, disposition = 'open', evidence = 'The current value does not satisfy the reviewed acceptance behavior.' } = {}) {
  return {
    id,
    severity: 'P1',
    basis: 'correctness',
...
function closeReview(state, { phase, findings, reviewedFindingIds = [] }) {
  return {
    ...
    contributions: state.reviewerRoster.map((reviewerId) => ({
      ...
      evidenceDigest: sha256Jcs({ reviewerId, phase, candidateDigest: state.candidateRevisions.at(-1).digest }),
    })),
    findings,
    reviewedFindingIds,
...
function passWithoutCorrection(root, feature, started, { taskId = 'T-001', sourceChange = true } = {}) {
  if (sourceChange) writeFileSync(join(root, 'src', 'app.js'), `export const value = ${Date.now()};\n`);
  let result = advanceShip({ projectRoot: root, feature, runId: started.runId, event: taskCompleted(0, taskId) });
  result = advanceShip({ projectRoot: root, feature, runId: started.runId, event: openReview(result.generation) });
  ...
  return { finalized: finalizeShipClosure({ projectRoot: root, feature, runId: started.runId }), close };
}

test('one correction closes through one targeted review and immutable receipt', () => {
  const { root, prepared } = project();
  const preview = prepareShip({ projectRoot: root, feature: 'closure', humanReviewConfirmed: true });
  assert.deepEqual(Object.keys(preview).includes('tasks'), false, 'public JSON projection omits internal task paths');
  const started = startShip({ projectRoot: root, feature: 'closure', humanReviewConfirmed: true, runtime: 'codex' });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  let result = advanceShip({ projectRoot: root, feature: 'closure', runId: started.runId, event: taskCompleted(0) });
  result = advanceShip({ projectRoot: root, feature: 'closure', runId: started.runId, event: openReview(result.generation) });
  ...
  result = advanceShip({
    ...
    event: closeReview(state, {
      ...
      findings: [
        finding(),
        finding({ evidence: 'A second acceptance failure is part of the same consolidated review batch.' }),
      ],
    }),
  });
  ...
  result = advanceShip({
    ...
    event: closeReview(state, {
      ...
      findings: findingIds.map((findingId) => finding({
        id: findingId,
  ...
  const finalGates = runShipGates({ projectRoot: root, feature: 'closure', runId: started.runId, phase: 'final' });
  assert.equal(finalGates.evidence.find(({ gateId }) => gateId === 'test-1').status, 'passed', 'final relevant suite never reuses evidence');
  const finalized = finalizeShipClosure({ projectRoot: root, feature: 'closure', runId: started.runId });
  ...
  assert.match(readFileSync(join(prepared.specDir, '.pipeline-shipped'), 'utf8'), /closure_receipt_hash:/);
  assert.match(readFileSync(join(prepared.specDir, 'qa-report.md'), 'utf8'), /Status: PASS/);
  assert.match(readFileSync(join(prepared.specDir, 'tasks', 'T-001-core.md'), 'utf8'), /status: "done"/);
  assert.equal(finalizeShipClosure({ projectRoot: root, feature: 'closure', runId: started.runId }).replayed, true);
  assert.throws(
    () => advanceShip({ projectRoot: root, feature: 'closure', runId: started.runId, event: openReview(receipt.generation) }),
...
test('targeted blocking findings terminate and arbitrary late reviews cannot mint revisions', () => {
  ...
  const started = startShip({ projectRoot: root, feature: 'blocked', humanReviewConfirmed: true, runtime: 'codex' });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  let result = advanceShip({ projectRoot: root, feature: 'blocked', runId: started.runId, event: taskCompleted(0) });
  result = advanceShip({ projectRoot: root, feature: 'blocked', runId: started.runId, event: openReview(result.generation) });
  ...
  result = advanceShip({ projectRoot: root, feature: 'blocked', runId: started.runId, event: closeReview(state, { phase: 'initial', findings: [finding()] }) });
  state = getShipClosure({ projectRoot: root, feature: 'blocked', runId: started.runId });
  const findingId = state.reviews[0].findings[0].id;
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 2;\n');
  result = advanceShip({
    projectRoot: root, feature: 'blocked', runId: started.runId,
    event: { type: 'correction.registered', expectedGeneration: result.generation, impact: { summary: 'Attempt fix.', findingIds: [findingId], paths: [path('src/app.js')], affectedGateIds: ['build-1', 'test-1'] } },
  });
  result = advanceShip({ projectRoot: root, feature: 'blocked', runId: started.runId, event: openReview(result.generation) });
  ...
  result = advanceShip({
    projectRoot: root, feature: 'blocked', runId: started.runId,
    event: closeReview(state, { phase: 'targeted', reviewedFindingIds: [findingId], findings: [finding({ id: findingId, disposition: 'remains' })] }),
  });
  assert.equal(result.state, 'blocked');
  const finalized = finalizeShipClosure({ projectRoot: root, feature: 'blocked', runId: started.runId });
  assert.equal(finalized.state, 'blocked');
  for (let index = 0; index < 25; index += 1) {
    assert.throws(
      () => advanceShip({ projectRoot: root, feature: 'blocked', runId: started.runId, event: openReview(finalized.generation) }),
      (error) => error.code === 'E_SHIP_TERMINAL',
    );
  }
  assert.equal(JSON.parse(readFileSync(finalized.receiptPath, 'utf8')).candidateRevisions.length, 2);
});
...
test('structured Preserve detects directory and symlink changes across repositories', () => {
  ...
  writeFileSync(join(second, 'input', 'tech', 'stack.md'), [
    '# Stack',
  ...
  writeFileSync(join(second, 'tests', 'external.test.mjs'), "import test from 'node:test'; test('external', () => {});\n");
  ...
  config.shipClosure = { repositories: [
  ...
  writeFileSync(taskPath, readFileSync(taskPath, 'utf8').replace('preserve: []', [
    'preserve:',
  ...
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  let result = advanceShip({ projectRoot: root, feature: 'preserve', runId: started.runId, event: taskCompleted(0) });
  ...
  assert.throws(
    () => advanceShip({ projectRoot: root, feature: 'preserve', runId: started.runId, event: openReview(result.generation) }),
...
test('runtime-sealed multi-repository candidate uses the same canonical tuple order as closure validation', () => {
  ...
  config.shipClosure = {
    ...
    gates: [
      {
        id: 'openplanr-build', repositoryKey: 'openplanr', argv: ['node', '--check', 'src/index.js'],
        inputs: [{ repositoryKey: 'openplanr', path: '.' }], dependsOn: [], finalRelevantSuite: false,
      },
      {
        id: 'openplanr-web-test', repositoryKey: 'openplanr-web', argv: ['node', '--check', 'src/index.js'],
        inputs: [{ repositoryKey: 'openplanr-web', path: '.' }], dependsOn: ['openplanr-build'], finalRelevantSuite: false,
      },
      {
        id: 'project-test', repositoryKey: 'project', argv: ['node', '--test', 'tests/smoke.test.mjs'],
        inputs: [{ repositoryKey: 'project', path: '.' }], dependsOn: ['openplanr-web-test'], finalRelevantSuite: true,
      },
    ],
  };
  writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);

  const started = startShip({ projectRoot: root, feature: 'prefixed-repositories', humanReviewConfirmed: true });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 1;\n');
  ...
  const active = getShipClosure({ projectRoot: root, feature: 'prefixed-repositories', runId: started.runId });

  assert.equal(opened.state, 'reviewing_initial');
  assert.deepEqual(active.candidateRevisions[0].repositories.map(({ repositoryKey }) => repositoryKey), ['openplanr', 'openplanr-web', 'project']);
  assert.deepEqual(
...
test('receipt validation rejects a substituted self hash', () => {
  ...
  const forged = { ...active, recordType: 'receipt', state: 'blocked', repositories: active.repositories.map((repository) => ({ ...repository, root: null })), terminal: { status: 'blocked', at: new Date().toISOString(), reason: 'x', candidateDigest: `sha256:${'a'.repeat(64)}`, gateEvidenceDigest: `sha256:${'b'.repeat(64)}` }, receiptHash: `sha256:${'c'.repeat(64)}` };
  assert.throws(() => assertShipClosure(forged), (error) => error.code === 'E_SHIP_CLOSURE_INVALID');
});
...
test('public event replay is byte-identical before and after terminal closure', () => {
  ...
  const first = advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: completion });
  const replay = advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: completion });
  assert.equal(replay.replayed, true);
  assert.equal(replay.generation, first.generation);
  const opened = advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: openReview(first.generation) });
  ...
  const close = closeReview(state, { phase: 'initial', findings: [] });
  const ready = advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: close });
  ...
  const terminalReplay = advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: close });
  assert.equal(terminalReplay.replayed, true);
  assert.equal(terminalReplay.state, 'passed');
  assert.throws(
    () => advanceShip({ projectRoot: root, feature: 'replay', runId: started.runId, event: { ...close, summary: 'different bytes', eventId: state.events.at(-1)?.eventId } }),
...
test('unchanged correction is rejected and correction-required can close BLOCKED', () => {
  ...
  const started = startShip({ projectRoot: root, feature: 'uncorrectable', humanReviewConfirmed: true });
  writeFileSync(join(root, 'src', 'app.js'), 'export const value = 11;\n');
  let result = advanceShip({ projectRoot: root, feature: 'uncorrectable', runId: started.runId, event: taskCompleted(0) });
  result = advanceShip({ projectRoot: root, feature: 'uncorrectable', runId: started.runId, event: openReview(result.generation) });
  runShipGates({ projectRoot: root, feature: 'uncorrectable', runId: started.runId, phase: 'initial' });
  let state = getShipClosure({ projectRoot: root, feature: 'uncorrectable', runId: started.runId });
  result = advanceShip({ projectRoot: root, feature: 'uncorrectable', runId: started.runId, event: closeReview(state, { phase: 'initial', findings: [finding()] }) });
  ...
  assert.throws(
    () => advanceShip({
      ...
      runId: started.runId,
      event: { type: 'correction.registered', expectedGeneration: result.generation, impact: { summary: 'No byte change.', findingIds: [findingId], paths: [], affectedGateIds: [] } },
    }),
    (error) => error.code === 'E_SHIP_CORRECTION_INVALID',
  );
  const terminal = finalizeShipClosure({ projectRoot: root, feature: 'uncorrectable', runId: started.runId });
...
test('disjoint task receipts aggregate planning truth and reopen one exact prior scope', () => {
  ...
  writeFileSync(join(prepared.specDir, 'tasks', 'T-002-second.md'), [
    '---', 'id: "T-002"', 'storyId: "US-001"', 'status: "pending"', 'updated: "2026-08-21"', 'dependsOn: []', 'preserve: []', '---', '',
    '## Definition of done', '- [ ] complete',
  ].join('\n'));
  git(root, 'add', '.');
  git(root, 'commit', '-qm', 'second task');
  const first = startShip({ projectRoot: root, feature: 'disjoint', humanReviewConfirmed: true, taskId: 'T-001' });
  const firstPass = passWithoutCorrection(root, 'disjoint', first);
  assert.equal(existsSync(join(prepared.specDir, '.pipeline-shipped')), true, 'every terminal receipt derives a scoped compatibility marker');
  ...
  const reopened = reopenShip({ projectRoot: root, feature: 'disjoint', receiptHash: firstPass.finalized.receiptHash, reason: 'Recheck the first bounded task.', ownerConfirmed: true });
  assert.deepEqual(reopened.approvedScope.taskIds, ['T-001']);
  assert.equal(reopened.tasks[0].status, 'pending');
  const exactReplay = reopenShip({ projectRoot: root, feature: 'disjoint', receiptHash: firstPass.finalized.receiptHash, reason: 'Recheck the first bounded task.', ownerConfirmed: true });
...
test('config-owned multi-repository gates round-trip through CLI start, block, finalize, and reopen', () => {
  ...
  config.shipClosure = {
    ...
    gates: [
      { id: 'project-build', repositoryKey: 'project', argv: ['node', '--check', 'src/app.js'], inputs: [{ repositoryKey: 'project', path: '.' }], dependsOn: [], finalRelevantSuite: false },
      {
        id: 'secondary-build', repositoryKey: 'secondary', argv: ['node', '--check', 'src/index.js'],
        inputs: [{ repositoryKey: 'secondary', path: '.' }, { repositoryKey: 'project', path: 'src/app.js' }],
        dependsOn: ['project-build'], finalRelevantSuite: false,
      },
      { id: 'project-test', repositoryKey: 'project', argv: ['node', '--test', 'tests/smoke.test.mjs'], inputs: [{ repositoryKey: 'project', path: '.' }], dependsOn: ['secondary-build'], finalRelevantSuite: true },
    ],
  };
  writeFileSync(configPath, JSON.stringify(config));
  const cli = join(process.cwd(), 'bin', 'planr-pipeline.mjs');
  const run = (...args) => JSON.parse(execFileSync(process.execPath, [cli, ...args, '--json'], { cwd: root, encoding: 'utf8' }));
  const preview = run('prepare-ship', 'multi-cli');
  assert.deepEqual(preview.gateDescriptors.map(({ id }) => id), ['project-build', 'secondary-build', 'project-test']);
  const started = run('start-ship', 'multi-cli', '--reviewer', 'security-agent');
  assert.deepEqual(started.repositories.map(({ repositoryKey }) => repositoryKey), ['project', 'secondary']);
  assert.deepEqual(started.reviewerRoster, ['qa-agent', 'security-agent']);
  assert.deepEqual(started.gates.map(({ id }) => id), ['project-build', 'secondary-build', 'project-test']);
  const blocked = advanceShip({ projectRoot: root, feature: 'multi-cli', runId: started.runId, event: {
    type: 'task.blocked', expectedGeneration: 0, taskId: 'T-001', agent: 'backend-agent', reason: 'Environment cannot complete.', filesWritten: [], filesModified: [],
  } });
  ...
  config.shipClosure.gates[1].inputs.push({ repositoryKey: 'project', path: 'tests/smoke.test.mjs' });
  writeFileSync(configPath, JSON.stringify(config));
  const reopened = run('reopen-ship', 'multi-cli', '--receipt-hash', finalized.receiptHash, '--reason', 'Owner requests one bounded retry.');
  assert.deepEqual(reopened.repositories.map(({ repositoryKey }) => repositoryKey), ['project', 'secondary']);
  ...
  assert.equal(reopened.tasks[0].status, 'pending');
  assert.equal(run('reopen-ship', 'multi-cli', '--receipt-hash', finalized.receiptHash, '--reason', 'Owner requests one bounded retry.').replayed, true);
});
