...
} from '../../lib/pipeline/provenance.mjs';
...
test('invalid provenance history fails closed at the exact safe record context', () => {
  ...
  const cases = [
    { name: 'invalid first row', bytes: row(invalid) + row(middle) + row(last), line: 1, reason: 'schema-invalid', declared: '1.0.0' },
    { name: 'invalid middle row', bytes: row(first) + row(invalid) + row(last), line: 2, reason: 'schema-invalid', declared: '1.0.0' },
    { name: 'invalid last row', bytes: row(first) + row(middle) + row(invalid), line: 3, reason: 'schema-invalid', declared: '1.0.0' },
    { name: 'malformed row', bytes: row(first) + `{"schema_version":"1.0.0","secret":"${SECRET}"\n`, line: 2, reason: 'malformed-json', declared: null },
    { name: 'truncated tail', bytes: row(first) + JSON.stringify(middle), line: 2, reason: 'truncated-tail', declared: null },
    {
    ...
    { name: 'duplicate event identity', bytes: row(first) + row(first), line: 2, reason: 'duplicate-identity', declared: '1.0.0' },
  ];

  for (const fixture of cases) {
    writeFileSync(fixtureProject.target, fixture.bytes, { mode: 0o600 });
    const before = readFileSync(fixtureProject.target);
    const failure = failureFor(() => appendProvenanceEvent(
      fixtureProject.root,
    ...
    assert.deepEqual(failure.details?.context, {
      line: fixture.line,
      record: fixture.line,
    ...
    assert.equal(JSON.stringify(failure.toJSON()).includes(fixtureProject.root), false, fixture.name);
...
test('a generation change cannot be overwritten by a staged append', () => {
  ...
  const failure = failureFor(() => appendProvenanceEvent(fixtureProject.root, appended, {
    hooks: {
      afterStage() {
  ...
  const recoveryFailure = failureFor(() => appendProvenanceEvent(
    fixtureProject.root,
...
test('concurrent appenders serialize without losing or duplicating valid events', async () => {
  ...
  const results = await Promise.all(eventIds.map((eventId) => runAppendProcess(
    fixtureProject.root,
    eventId,
    { holdLock: true },
  )));
  assert.deepEqual(results.map(({ status }) => status), Array(eventIds.length).fill(0), results.map(({ stderr }) => stderr).join('\n'));
