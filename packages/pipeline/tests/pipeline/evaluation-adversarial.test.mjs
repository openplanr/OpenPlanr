...
const scenario = seal('evaluation-scenario', {
  ...
  expectedTrigger: {
    outcome: 'decline',
    rationale: 'The prompt names a task another surface owns.',
  },
  declaredPermissions: [{ capability: 'file-read', decision: 'granted' }],
  riskClass: 'read-only',
  fixtureRefs: [
    { fixtureId: `efx_${'2'.repeat(64)}`, contentDigest: digest('prompt-bytes'), role: 'prompt' },
  ],
...
const waiver = seal('evaluation-waiver', {
  ...
  reason: {
    code: 'accepted-risk',
    statement: 'The owner accepts one incomplete journey for this release.',
  },
  ownerSignature: {
...
function buildReceipt(overrides = {}) {
  const body = {
    ...
    inputs: {
      ...
      graderRegistryDigest: digest('grader-registry'),
      hostProfiles: [
        { hostProfileId: 'reference-host', hostProfileDigest: digest('host-profile') },
      ],
      budgetBaselineDigest: digest('baseline'),
      gatePolicyDigest: gatePolicy.gatePolicyDigest,
...
test('a scenario whose bytes changed but whose identity did not is refused', () => {
  ...
  const rewritten = {
    ...scenario,
    intent: 'The skill invokes on anything that mentions planning.',
  };
  assert.throws(
...
test('scenario identity follows meaning, not formatting, and a repeated key is never merged', () => {
  ...
  assert.throws(
    () =>
      assertUniqueJsonKeys(
        '{"kind":"evaluation-scenario","kind":"evaluation-corpus"}',
        'scenario source',
      ),
...
test('a changed grader digest invalidates exactly the evidence bound to it', () => {
  ...
  assert.deepEqual(
    reuse.invalidated.map(({ scenarioId, reason, changedInputs }) => ({
      scenarioId,
      reason,
  ...
  assert.deepEqual(
    dropped.invalidated.map(({ reason }) => reason),
    ['scenario-absent', 'scenario-absent'],
  );
});
  ...
  assert.throws(
    () =>
      assertEvaluationGraderRegistration({
        ...graderRegistration,
        credentialReference: {
          refKind: 'environment-name',
          refName: 'OPENPLANR_JUDGE_KEY',
          custody: 'host-owned',
        },
      }),
    (error) => error.code === 'E_EVALUATION_GRADER_INVALID',
...
test('a waiver cannot reach a P0 or P1 finding, schema validity, or package parity', () => {
  assert.deepEqual([...EVALUATION_UNWAIVABLE_METRICS].sort(), [
    'finding-severity-p0',
    'finding-severity-p1',
    'package-parity',
    'schema-validity',
...
test('a certification receipt reports readiness and can carry no release authority', () => {
  ...
  assert.throws(
    () =>
      assertSkillCertificationReceipt({
        ...ready,
        subject: { ...ready.subject, grant: 'publish' },
      }),
    (error) => error.code === 'E_EVALUATION_RECEIPT_AUTHORITY_CLAIM',
  ...
  assert.throws(
    () =>
      assertSkillCertificationReceipt(
        buildReceipt({ gateEvaluation: withUnmetGate, blockingMetrics: ['trigger-recall'] }),
      ),
    (error) => error.code === 'E_EVALUATION_CONTRACT_INVALID',
  );
  const blocked = buildReceipt({
    gateEvaluation: withUnmetGate,
    blockingMetrics: ['trigger-recall'],
    result: 'blocked',
  });
...
test('only the redacted aggregate report is publishable', () => {
  ...
  for (const declaration of Object.keys(aggregateReport.redaction).filter(
    (key) => key !== 'policyVersion',
  )) {
    assert.throws(
      () =>
  ...
  for (const smuggled of [
    ...
    assert.throws(
      ...
      (error) =>
        error.code === 'E_EVALUATION_PUBLISH_UNSAFE' ||
        error.code === 'E_EVALUATION_CONTRACT_INVALID',
