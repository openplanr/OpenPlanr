...
function claims(bundle) {
  return {
    ...
    screenshots: bundle.screenshots.map(({ id, bytes }) => ({
      id,
...
function session({ signedIn = false } = {}) {
  return {
    ...
    suppliedAt: '2026-08-24T10:00:00.000Z',
...
function attestation(
  ...
  const host = createTrustedBrowserQaRuntimeHost({
  ...
  const capability = establishBrowserQaRuntimeCapability({
    ...
    candidateDigest: subjectDigest,
    requirementDigest: classification.browserQa.requirementDigest,
    sessionBindingDigest: digest(browserSession),
    sessionId: browserSession.sessionId,
    signedIn: browserSession.signedIn,
    issuedAt: '2026-08-24T10:05:00.000Z',
    expiresAt: browserSession.expiresAt,
    ...overrides,
  });
  return attestBrowserQaEvidence({
    capability,
...
test('risk selection is permutation-stable and follows the fixed specialist registry order', () => {
  ...
  assert.deepEqual(
    shipReviewSpecialistRegistry().specialists.map(({ id }) => id),
    ['security', 'performance', 'migration', 'api-contract', 'data-integrity'],
  );
  const left = classifyShipRisk(
    input({
      changedPaths: [
      ...
      permissionEffects: true,
      performanceBudgets: true,
    }),
  );
  const right = classifyShipRisk(
    input({
      changedPaths: [
        { repositoryKey: 'project', path: 'migrations/001.sql' },
        { repositoryKey: 'project', path: 'src/dashboard/App.tsx' },
  ...
  assert.deepEqual(left.reviewerRoster, [
    'qa-agent',
    'security',
    'performance',
    'migration',
  ...
  assert.throws(
    () =>
      classifyShipRisk(input({ changedPaths: [{ repositoryKey: 'project', path: '../secret' }] })),
    { code: 'E_SHIP_RISK_PATH_INVALID' },
  );
  assert.throws(() => classifyShipRisk(input({ explicitRisks: ['security', 'security'] })), {
    code: 'E_SHIP_RISK_INPUT_INVALID',
  });
});
...
test('specialist results bind the selected candidate and grant contribution-only authority', () => {
  ...
  const value = {
    kind: 'specialist-review-result',
    schemaVersion: '1.0.0',
    protocolVersion: '1.1.0',
    specialistId: 'security',
    candidateDigest: subjectDigest,
    classificationDigest: classification.classificationDigest,
    summary: 'The bounded security review is complete.',
    findingIds: [],
    evidenceDigest: digest('security-evidence'),
    authority: 'contribution-only',
  };
  assert.equal(
    assertSpecialistReviewResult(value, { classification, candidateDigest: subjectDigest }),
    value,
  );
  assert.throws(
    () =>
      assertSpecialistReviewResult(
        { ...value, authority: 'correction' },
        { classification, candidateDigest: subjectDigest },
      ),
    { code: 'E_SHIP_SPECIALIST_RESULT_INVALID' },
  );
});
...
test('signed-in browser evidence consumes a fresh ephemeral session but persists only its digest', () => {
  ...
  assert.equal(
    assertBrowserQaSession(browserSession, { now: '2026-08-24T10:05:00.000Z' }),
    browserSession,
  );
  ...
  assert.equal(
    assertBrowserQaGateRecord(legacy),
    legacy,
    'historical unattested records remain readable',
  );
  assert.throws(() => assertBrowserQaGateRecord(legacy, { requireAttested: true }), {
...
test('caller-authored, rehashed, substituted, stale, foreign, and mismatched browser evidence cannot certify PASS', () => {
  ...
  assert.throws(
    () =>
      assertBrowserQaRecordedEventAuthority(issuedEvent.authority, {
        ...issuedEvent.event,
  ...
  assert.throws(() => attestation(classification, browserResult, browserSession, mismatched), {
    code: 'E_BROWSER_QA_EVIDENCE_MISMATCH',
  });
  assert.throws(
    () =>
      attestation(classification, browserResult, browserSession, artifacts(), {
...
test('mandatory unavailability blocks and unsafe/private browser evidence fails closed', () => {
  ...
  const unavailable = result(classification, {
    status: 'unavailable',
    routeIds: [],
    formIds: [],
    viewports: [],
  ...
  assert.equal(
    ...
    record,
    'unavailable mandatory evidence may block without fabricated custody',
  );
  assert.throws(
    () =>
      issueBrowserQaGateRecord({
        result: { ...unavailable, status: 'skipped' },
        required: true,
        candidateRevision: 1,
        candidateDigest: subjectDigest,
        requirementDigest: classification.browserQa.requirementDigest,
  ...
  assert.throws(
    () =>
      assertBrowserQaResult(
        { ...unavailable, origin: 'https://user:secret@example.com' },
        {
          candidateDigest: subjectDigest,
          requirementDigest: classification.browserQa.requirementDigest,
          required: true,
