...
const matrixJob = workflow.slice(workflow.indexOf('\n  pipeline-tests:'))
  .split(/\n  (?=[a-z][a-z0-9-]+:)/u)[1];
...
const entrypoints = [...matrixJob.matchAll(/^\s+script: ([a-z][a-z0-9:-]+)\s*$/gmu)]
  .map(([, script]) => script);
...
function leaves(script, parents = []) {
  ...
  assert.ok(!parents.includes(script), `Cyclic CI script chain: ${[...parents, script].join(' -> ')}`);
...
test('CI reaches native dispatch, governed landing and design handoff exactly once', () => {
  for (const fixture of ['nd1-parallel', 'nd2-advisory-locklist', 'nd3-dependson', 'nd4-coherent']) {
  ...
  for (const file of ['verify-governed-landing.mjs', 'verify-design-handoff-contracts.mjs']) {
    assert.equal(commands.filter((command) => command === `node conformance/${file}`).length, 1, file);
  }
});
...
test('ecosystem tests already exercise release reconciliation and installed contract compilation', () => {
  const ecosystem = commands.find((command) => /^node scripts\/run-test-group\.mjs tests\/ecosystem(?: |$)/u.test(command));
  ...
  for (const file of ['release-ledger-drift.test.mjs', 'operate-v2-phase2-package.test.mjs']) {
    assert.ok(!ecosystem.includes(`--exclude tests/ecosystem/${file}`), `${file} must not be excluded`);
  }
  const ledger = readFileSync(resolve(pipeline, 'tests/ecosystem/release-ledger-drift.test.mjs'), 'utf8');
  assert.match(ledger, /spawnSync\(process\.execPath, \[join\(root, 'conformance\/verify-release-ledger\.mjs'\)\]/u);
  const compilation = readFileSync(resolve(pipeline, 'tests/ecosystem/operate-v2-phase2-package.test.mjs'), 'utf8');
  assert.match(compilation, /run\(process\.execPath, \[\s*join\(installedPackage, 'conformance', 'verify-operate-v2-contract-compilation\.mjs'\)/u);
});
...
test('local CI parity runs the same pipeline entrypoints as Workspace CI', () => {
  ...
  const localEntrypoints = [...pipelineJob.matchAll(/^\s+'([a-z][a-z0-9:-]+)',?\s*$/gmu)]
    .map(([, script]) => script);
