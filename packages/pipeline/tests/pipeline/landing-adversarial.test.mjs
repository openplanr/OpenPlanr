...
function snapshot(plan, state = {}) {
  ...
  return {
    ...
    phaseReceiptHeadHash: null, phaseReceipts: [], confirmations: [],
    evidenceRecordsByOperation: {}, evidenceContextsByOperation: {}, startedAt: null,
  };
}

ownerTest('cloned host and non-durable intent acknowledgement fail before dispatch', ['confirm'], async () => {
  const trusted = memoryHost(fixture.plan);
  await assert.rejects(
    advanceLanding({
      host: structuredClone(trusted.host), plan: fixture.plan,
      operationId: fixture.operation.operationId, now: '2026-08-25T10:00:00.000Z',
    }),
  ...
  const host = createLandingOwnerRuntimeHost({
    ...
    commitIntent: async (request) => ({
      commitId: 'forged-commit', dispatcherId: 'forged-dispatcher',
    ...
    commitOutcome: async () => { throw new Error('unreachable'); },
  });
  await assert.rejects(
    advanceLanding({
      host, plan: fixture.plan, operationId: fixture.operation.operationId,
...
ownerTest('ack loss, reconciliation failure, and outcome CAS restart never redispatch', [
  'confirm', 'confirm', 'confirm', 'confirm', 'confirm',
], async () => {
  const reconciledResult = {
    status: 'succeeded', targetAfterHash: digest('target-after-reconciled'),
    completedAt: '2026-08-25T10:00:00.002Z', evidenceRecords: [], evidenceContexts: {},
    canary: null, containment: null, recovery: null,
  };
  const reconciled = memoryHost(fixture.plan, { dispatchThrows: true, reconcileResult: reconciledResult });
  const result = await advanceLanding({
    host: reconciled.host, plan: fixture.plan, operationId: fixture.operation.operationId,
    now: '2026-08-25T10:00:00.000Z',
  });
  ...
  await assert.rejects(
    advanceLanding({
      host: unresolved.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:00:00.000Z',
    }),
  ...
  const unknownRestart = memoryHost(fixture.plan, {
    ...
    reconcileResult: {
      disposition: 'unknown', targetAfterHash: null,
      completedAt: '2026-08-25T10:00:00.002Z',
    },
  });
  const uncertain = await advanceLanding({
    host: unknownRestart.host, plan: fixture.plan,
    operationId: fixture.operation.operationId, now: '2026-08-25T10:20:00.000Z',
  });
  ...
  await assert.rejects(
    advanceLanding({
      host: unacknowledged.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:00:00.000Z',
    }),
  ...
  const durableConfirmationHash = unacknowledged.state.pendingIntent.confirmation.confirmationHash;
  ...
  const resumed = await advanceLanding({
    host: restarted.host, plan: fixture.plan, operationId: fixture.operation.operationId,
  ...
  const replayed = await advanceLanding({
    host: lostOutcomeAck.host, plan: fixture.plan, operationId: fixture.operation.operationId,
    now: '2026-08-25T10:00:00.000Z',
  });
  ...
  await assert.rejects(
    advanceLanding({
      host: pendingRace.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:00:00.000Z',
    }),
    errorCode('E_LANDING_OUTCOME_NOT_PERSISTED'),
  );
  const raceLeft = memoryHost(fixture.plan, {
    state: pendingRace.state, reconcileResult: reconciledResult,
  });
  ...
  const raceResults = await Promise.all([
    advanceLanding({
      host: raceLeft.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:20:00.000Z',
    }),
    advanceLanding({
      host: raceRight.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:20:00.000Z',
    }),
  ]);
  assert.equal(raceResults[0].landingReceipt.receiptHash, raceResults[1].landingReceipt.receiptHash);
...
ownerTest('cancel and concurrent advance remain zero-or-one effect', ['cancel', 'confirm'], async () => {
  const cancelled = memoryHost(fixture.plan, { cancel: true });
  const result = await advanceLanding({
    host: cancelled.host, plan: fixture.plan, operationId: fixture.operation.operationId,
    now: '2026-08-25T10:00:00.000Z',
  });
  ...
  const calls = await Promise.allSettled([1, 2].map(() => advanceLanding({
    host: concurrent.host, plan: fixture.plan, operationId: fixture.operation.operationId,
    now: '2026-08-25T10:00:00.000Z',
  })));
...
ownerTest('pending intent self-binding and operation identity reject before reconciliation', ['confirm'], async () => {
  const pending = memoryHost(fixture.plan, { outcomeCommitThrows: true });
  await assert.rejects(
    advanceLanding({
      host: pending.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:00:00.000Z',
    }),
    errorCode('E_LANDING_OUTCOME_NOT_PERSISTED'),
  );
  ...
  const corrupted = memoryHost(fixture.plan, {
    ...
    reconcileResult: { disposition: 'unknown', targetAfterHash: null, completedAt: '2026-08-25T10:00:00.002Z' },
  });
  await assert.rejects(
    advanceLanding({
      host: corrupted.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:20:00.000Z',
    }),
  ...
  await assert.rejects(
    advanceLanding({
      host: wrongOperation.host, plan: fixture.plan,
      operationId: `lop_${'9'.repeat(32)}`, now: '2026-08-25T10:20:00.000Z',
    }),
  ...
  await assert.rejects(
    advanceLanding({
      host: missingIntent.host, plan: fixture.plan, operationId: fixture.operation.operationId,
      now: '2026-08-25T10:20:00.000Z',
    }),
    errorCode('E_LANDING_CUSTODY_INVALID'),
  );
...
directTest('plans require canonical topological order and forbid irreversible rollback binding', () => {
  const second = { ...structuredClone(fixture.operation), operationId: `lop_${'2'.padStart(32, '0')}` };
  const first = { ...structuredClone(fixture.operation), operationId: `lop_${'1'.padStart(32, '0')}` };
  assert.throws(() => prepareLanding({
    closureInspection: fixture.closureInspection,
    currentTargetHash: second.targetBeforeHash,
    operations: [second, first], baseRecords: fixture.baseRecords,
  ...
  const governed = {
    ...structuredClone(fixture.governed), effectClass: 'external-effect',
  };
  const rollbackPlan = {
    ...structuredClone(operating['operating-rollback-plan']),
    operationId: governed.operationId, effectClass: governed.effectClass,
    executor: structuredClone(governed.executor), capability: structuredClone(governed.capability),
    eligibility: 'eligible',
  };
  const rollbackResult = {
    ...structuredClone(operating['operating-rollback-result']),
    originalOperationId: governed.operationId, rollbackPlanId: rollbackPlan.rollbackPlanId,
    effectClass: governed.effectClass, executor: structuredClone(governed.executor),
    capability: structuredClone(governed.capability),
  };
  const registration = (id) => fixture.operationRegistry.operations.find(({ operationId }) => operationId === id);
  const mergeRegistration = registration('merge');
  const merge = {
    ...structuredClone(fixture.operation), operationId: `lop_${'1'.padStart(32, '0')}`,
    registryOperationId: 'merge', registrationHash: mergeRegistration.registrationHash,
    kind: 'merge', effectClass: mergeRegistration.effectClass,
  ...
  const rollback = {
    ...structuredClone(fixture.operation), operationId: `lop_${'2'.padStart(32, '0')}`,
    registryOperationId: 'rollback', registrationHash: rollbackRegistration.registrationHash,
    kind: 'rollback', dependsOn: [merge.operationId], effectClass: rollbackRegistration.effectClass,
  ...
  assert.throws(() => prepareLanding({
    closureInspection: fixture.closureInspection, currentTargetHash: merge.targetBeforeHash,
...
ownerTest('a non-passing deploy cannot bypass its frozen containment', ['confirm', 'confirm'], async () => {
  const executor = structuredClone(fixture.executor);
  const governed = { ...structuredClone(fixture.governed), effectClass: 'external-effect' };
  const rollbackPlan = {
    ...structuredClone(operating['operating-rollback-plan']), operationId: governed.operationId,
    effectClass: governed.effectClass, executor: structuredClone(governed.executor),
    capability: structuredClone(governed.capability), eligibility: 'eligible',
  };
  const registration = fixture.operationRegistry.operations.find(({ operationId }) => operationId === 'deploy');
  const containment = {
    policyHash: digest('deploy-containment'), state: 'preconfirmed', stopPromotion: true,
    stopNewTraffic: true, isolateFailedTarget: true, retainLastKnownGood: true,
    trafficStateHash: digest('traffic-contained'), residualStateHash: digest('residual-contained'),
    expiresAt: '2026-08-25T10:30:00.000Z', consequences: ['The failed target remains isolated.'],
    recoveryChoices: ['rollback', 'forward-fix'], authority: 'containment-only',
  };
  const deploy = {
    ...structuredClone(fixture.operation), registryOperationId: 'deploy',
    registrationHash: registration.registrationHash, kind: 'deploy',
  ...
  const plan = prepareLanding({
    closureInspection: fixture.closureInspection, currentTargetHash: deploy.targetBeforeHash,
    operations: [deploy], baseRecords: [executor, governed, rollbackPlan],
    operationRegistry: fixture.operationRegistry,
    createdAt: '2026-08-25T10:00:00.000Z', expiresAt: '2026-08-25T11:00:00.000Z',
  });
  const runtime = memoryHost(plan, {
    dispatchResult: {
      status: 'failed', targetAfterHash: null, completedAt: '2026-08-25T10:00:00.002Z',
      evidenceRecords: [], evidenceContexts: {}, canary: null, containment: null, recovery: null,
    },
  });
  await assert.rejects(
    advanceLanding({
      host: runtime.host, plan, operationId: deploy.operationId,
      now: '2026-08-25T10:00:00.000Z',
    }),
  ...
  const notStarted = memoryHost(plan, {
    dispatchThrows: true,
    reconcileResult: {
      disposition: 'not-started', targetAfterHash: null,
      completedAt: '2026-08-25T10:00:00.002Z',
    },
  });
  const blocked = await advanceLanding({
    host: notStarted.host, plan, operationId: deploy.operationId,
    now: '2026-08-25T10:00:00.000Z',
  });
