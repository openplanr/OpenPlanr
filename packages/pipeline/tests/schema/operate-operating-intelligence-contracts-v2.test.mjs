...
test('Phase 5 has the exact public operating-intelligence contract vocabulary', () => {
  assert.deepEqual(
    [...OPERATE_OPERATING_INTELLIGENCE_CONTRACT_KINDS_V2].sort(),
    [...vocabulary.contractIds].sort(),
  );
  assert.deepEqual(OPERATE_OPERATING_PROJECTION_IDENTITIES_V2, vocabulary.projectionIdentities);
  assert.deepEqual(
    [...OPERATE_OPERATING_PROVIDER_REGISTRATION_CONTRACT_KINDS_V2].sort(),
  ...
  assert.deepEqual(
    loadOperateOperatingIntelligenceContract('business-operating-snapshot-projection', {
      protocolVersion: '2.0.0',
    }).schema['x-openplanr-contract'],
    { id: 'business-operating-snapshot-projection', version: '1.0.0' },
  );
  assert.deepEqual(
    loadOperateOperatingIntelligenceContract('software-operating-snapshot-projection', {
      protocolVersion: '2.0.0',
    }).schema['x-openplanr-contract'],
...
test('API domain IDs use an explicit public domain-contract mapping', () => {
  for (const [domainId, domainContract] of Object.entries(vocabulary.apiDomainContracts)) {
    ...
    assert.deepEqual(
      validateProtocolArtifact('operate-domain-registration', registration, {
        protocolVersion: '2.0.0',
      }),
      [],
      domainId,
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', implicit, { protocolVersion: '2.0.0' })
      .length > 0,
  );
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', missingChallenger, {
      protocolVersion: '2.0.0',
    }).length > 0,
    'public business registration requires Challenger',
  );
});
...
test('Phase 5 provider declarations are data-only candidate descriptors', () => {
  ...
  assert.ok(
    ...
    'dynamic executable providers are rejected',
  );
  assert.ok(
    validateProtocolArtifact(
      'operate-snapshot-provider-registration',
    ...
    'provider declarations cannot authorize effects',
  );
});
...
test('operating actions and decisions fail closed when quality and revision fields are absent', () => {
  for (const field of [
    ...
    delete action[field];
    assert.ok(
      validateProtocolArtifact('operating-action', action, { protocolVersion: '2.0.0' }).length > 0,
      field,
    );
  }
  ...
  for (const field of [
    ...
    assert.ok(
      validateProtocolArtifact('operating-decision', decision, { protocolVersion: '2.0.0' })
        .length > 0,
      field,
    );
  }
...
test('advisor claims always carry evidence or an explicit assumption', () => {
  ...
  assert.ok(
    validateProtocolArtifact('operating-advisor-result', advisor, {
      protocolVersion: '2.0.0',
    }).length > 0,
    'an ungrounded speculative claim is rejected',
  );
  ...
  assert.ok(
    ...
    'known claims require evidence rather than assumptions alone',
  );
});
...
test('Challenger and Chair question coverage is closed and requires an explicit answer disposition', () => {
  for (const kind of ['operating-challenger-review', 'operating-decision-ledger']) {
    ...
    assert.ok(
      validateProtocolArtifact(kind, extra, { protocolVersion: '2.0.0' }).length > 0,
      `${kind}:closed`,
    );
    ...
    assert.ok(
      validateProtocolArtifact(kind, unjustified, { protocolVersion: '2.0.0' }).length > 0,
      `${kind}:not-applicable`,
    );
    ...
    assert.ok(
      validateProtocolArtifact(kind, unlinked, { protocolVersion: '2.0.0' }).length > 0,
      `${kind}:answered`,
    );
  }
});
...
test('Challenger and Chair dissent accept multiline 513–4096 characters and reject multiline 4097', () => {
  ...
  challenger.findings = [
    {
      ...
      contradictingEvidenceRefIds: [],
      rationale:
        'The accepted Advisor Artifact does not establish the asserted causal relationship.',
  ...
  assert.deepEqual(
    validateProtocolArtifact('operating-challenger-review', challenger, {
      protocolVersion: '2.0.0',
    }),
    [],
  ...
  assert.ok(
    ...
    '4097-character Challenger dissent is rejected',
  );
  ...
  assert.ok(
    ...
    '4097-character Chair-preserved dissent is rejected',
  );
});
...
test('a challenger-required intelligence plan selects the compiler-owned Challenger mandate', () => {
  ...
  assert.ok(
    validateProtocolArtifact('operating-intelligence-plan', absent, { protocolVersion: '2.0.0' })
      .length > 0,
  );
  ...
  assert.ok(
    validateProtocolArtifact('operating-intelligence-plan', malformed, { protocolVersion: '2.0.0' })
      .length > 0,
  );
});
...
test('a domain owns its role identity and label while the kernel owns only the scheduling kind', () => {
  ...
  advisor.label = 'CEO';
  assert.deepEqual(
    validateProtocolArtifact('operate-domain-registration', named, { protocolVersion: '2.0.0' }),
    [],
  );
  ...
  assert.deepEqual(
    validateProtocolArtifact('operate-domain-registration', parallel, { protocolVersion: '2.0.0' }),
    [],
  ...
  for (const field of ['roleKind', 'label']) {
    ...
    assert.ok(
      validateProtocolArtifact('operate-domain-registration', partial, { protocolVersion: '2.0.0' })
        .length > 0,
      field,
    );
  }
  ...
  foreignKind.roles.find(({ roleId }) => roleId === 'strategy-finance').roleKind =
    'strategy-finance';
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', foreignKind, {
      protocolVersion: '2.0.0',
    }).length > 0,
...
test('domain declarations carry data-only board vocabulary without authority fields', () => {
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', authority, { protocolVersion: '2.0.0' })
      .length > 0,
  );
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', executable, {
      protocolVersion: '2.0.0',
    }).length > 0,
  );
});
