...
const fixture = (name) => JSON.parse(readFileSync(new URL(`../../conformance/fixtures/operating-runtime-v2/${name}`, import.meta.url), 'utf8'));
...
const schema = (name) => JSON.parse(readFileSync(new URL(`../../schemas/v2.0.0/${name}.schema.json`, import.meta.url), 'utf8'));
const registry = JSON.parse(readFileSync(new URL('../../registry/operate-v2-contracts.json', import.meta.url), 'utf8'));
...
test('Phase 6 publishes one exact closed governed-execution contract family', () => {
  ...
  assert.deepEqual(Object.keys(valid).sort(), [...OPERATE_GOVERNED_EXECUTION_CONTRACT_KINDS_V2].sort());
  assert.deepEqual(OPERATE_GOVERNED_PROVIDER_REGISTRATION_CONTRACT_KINDS_V2, ['operate-capability-provider-registration', 'operate-executor-registration', 'operate-policy-provider-registration']);
  assert.deepEqual(OPERATE_GOVERNED_EFFECT_CLASSES_V2, ['read-only', 'machine-local-write', 'project-write', 'provider-call', 'external-effect', 'destructive']);
  assert.deepEqual(OPERATE_GOVERNED_POLICY_OUTCOMES_V2, ['automatic', 'deferred', 'named-multi-party', 'named-single-party', 'prohibited', 'rejected', 'threshold']);
  ...
  for (const kind of OPERATE_GOVERNED_EXECUTION_CONTRACT_KINDS_V2) {
    assert.equal(loadOperateGovernedExecutionContract(kind, { protocolVersion: '2.0.0' }).kind, kind);
    ...
    assert.ok(validateProtocolArtifact(kind, candidate, { protocolVersion: '2.0.0' }).length > 0, `${kind} negative fixture`);
  }
});
...
test('an Action carries either no Phase 6 bindings or the complete exact governed tuple', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-action', historical, { protocolVersion: '2.0.0' }), [], 'historical Phase 5 Action');
  const complete = { ...historical, ...bindings };
  assert.deepEqual(validateProtocolArtifact('operating-action', complete, { protocolVersion: '2.0.0' }), [], 'complete Phase 6 Action');

  for (const [field, value] of Object.entries(bindings)) {
    assert.ok(validateProtocolArtifact('operating-action', { ...historical, [field]: value }, {
      protocolVersion: '2.0.0',
    }).length > 0, `${field}: singleton rejected`);
    const partial = structuredClone(complete);
    delete partial[field];
    assert.ok(validateProtocolArtifact('operating-action', partial, {
      protocolVersion: '2.0.0',
    }).length > 0, `${field}: missing from otherwise complete tuple`);
  }
});
...
test('governed tools accept no caller authority or request fingerprint', () => {
  ...
  const requests = [
    ...
    ['operate.action.rollback', { action, originalOperationId: 'op_00000001', rollbackPlanId: 'rbp_00000001' }],
  ];
  const forbidden = [['requestFingerprint', 'sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'], ['grantId', 'cgr_caller'], ['approvalIds', ['aprv_caller']], ['executorId', 'caller-executor'], ['idempotencyKey', 'caller-key'], ['credential', 'secret']];
  for (const [operation, request] of requests) {
    const toolCall = { kind: 'operate-tool-call', schemaVersion: '1.0.0', protocolVersion: '2.0.0', direction: 'request', operation, request };
    const allowedAction = { tool: operation, arguments: request, label: operation, effect: 'project-write' };
    assert.deepEqual(validateProtocolArtifact('operate-tool-call', toolCall, { protocolVersion: '2.0.0' }), [], `${operation}: tool call`);
    assert.deepEqual(validateProtocolArtifact('operate-allowed-action', allowedAction, { protocolVersion: '2.0.0' }), [], `${operation}: allowed action`);
    for (const [field, value] of forbidden) {
      const unsafeToolCall = structuredClone(toolCall);
      unsafeToolCall.request[field] = value;
      assert.ok(validateProtocolArtifact('operate-tool-call', unsafeToolCall, { protocolVersion: '2.0.0' }).length > 0, `${operation}:${field}:tool`);
      const unsafeAllowedAction = structuredClone(allowedAction);
      unsafeAllowedAction.arguments[field] = value;
      assert.ok(validateProtocolArtifact('operate-allowed-action', unsafeAllowedAction, { protocolVersion: '2.0.0' }).length > 0, `${operation}:${field}:allowed-action`);
    }
  }
});

test('the public declaration exposes every governed schema surface without caller fingerprint input', () => {
  const declaration = readFileSync(new URL('../../lib/protocol/index.d.ts', import.meta.url), 'utf8');
  const eventTypes = ['policy.evaluated', 'approval.recorded', 'capability.availability-recorded', 'capability.granted', 'operation.intent-recorded', 'execution.result-recorded', 'rollback.plan-recorded', 'rollback.result-recorded'];
  const eventPayloads = ['OperatingPolicyEvaluationV2', 'OperatingApprovalRecordV2', 'OperatingCapabilityAvailabilityV2', 'OperatingCapabilityGrantV2', null, null, 'OperatingRollbackPlanV2', 'OperatingRollbackResultV2'];
  for (const [index, eventType] of eventTypes.entries()) {
    assert.ok(declaration.includes(`| '${eventType}'`), `${eventType}: event type`);
    assert.ok(declaration.includes(eventPayloads[index] === null
      ? `'${eventType}': {`
      : `'${eventType}': ${eventPayloads[index]};`), `${eventType}: payload map`);
  }
  ...
  assert.ok(declaration.includes("export type OperatingActionStateV2 = 'proposed' | 'approved' | 'queued' | 'in_progress' | 'completed' | 'blocked' | 'rejected' | 'deferred' | 'cancelled';"));
  const publicRequestMap = declaration.match(/export interface OperateToolRequestMapV2 \{([\s\S]*?)\n\}/u)?.[1] ?? '';
  assert.ok(publicRequestMap.includes("'operate.action.execute': { action: OperateActionIdentityV2 };"));
...
test('public cycle view and approval declarations exactly match their closed schemas', () => {
  const declaration = readFileSync(new URL('../../lib/protocol/index.d.ts', import.meta.url), 'utf8');
  const cycleView = declaration.match(/export interface OperatingCycleViewV2 \{([\s\S]*?)\n\}/u)?.[1] ?? '';
  assert.match(cycleView, /persistentWork: \{\s+ledger: OperatingWorkLedgerV2;\s+cycleLinks: OperatingWorkCycleLinkV2\[\];\s+\};/u);
  const requirement = structuredClone(valid['operating-approval-requirement']);
  requirement.parties[0].actorId = null;
  assert.deepEqual(validateProtocolArtifact('operating-approval-requirement', requirement, { protocolVersion: '2.0.0' }), []);
  for (const kind of ['operating-approval-requirement', 'operating-approval-record']) {
    ...
    assert.ok(validateProtocolArtifact(kind, destructive, { protocolVersion: '2.0.0' }).length > 0, `${kind}:destructive effect`);
  }
  const recordWithNullActor = structuredClone(valid['operating-approval-record']);
  recordWithNullActor.actor.actorId = null;
  assert.ok(validateProtocolArtifact('operating-approval-record', recordWithNullActor, { protocolVersion: '2.0.0' }).length > 0, 'an issued approval has an exact actor');
});

test('the public guide lists the exact Phase 6 cycle, assignment, tool, and effect vocabulary', () => {
  const guide = readFileSync(new URL('../../docs/protocol/operate-runtime-v2.md', import.meta.url), 'utf8');
  assert.match(guide, /created \| observing \| advising \| challenging \| synthesizing \| awaiting_review\napproved \| executing \| verifying \| closed \| blocked \| failed \| cancelled/u);
  assert.match(guide, /advisor \| challenger \| chair \| execution \| verification/u);
  const tools = guide.match(/The only trigger is `manual`\. The public tools are:\n\n```text\n([\s\S]*?)\n```/u)?.[1]?.split('\n') ?? [];
...
test('provider registrations are data-only, fail closed and cannot register destructive authority', () => {
  for (const kind of OPERATE_GOVERNED_PROVIDER_REGISTRATION_CONTRACT_KINDS_V2) {
    ...
    for (const patch of [{ modulePath: './provider.mjs' }, { sourcePath: './provider.mjs' }, { credential: 'secret' }, { grantId: 'cgr_caller' }, { approvalIds: ['aprv_caller'] }, { effectCeiling: 'destructive' }, { fallback: { kind: 'executor', errorCode: 'EXECUTOR_UNAVAILABLE' } }]) {
      assert.ok(validateProtocolArtifact(kind, { ...structuredClone(registration), ...patch }, { protocolVersion: '2.0.0' }).length > 0, `${kind}:${Object.keys(patch)[0]}`);
    }
  }
});
...
test('provider reconciliation is exactly false/none or true/deterministic', () => {
  ...
  for (const kind of OPERATE_GOVERNED_PROVIDER_REGISTRATION_CONTRACT_KINDS_V2) {
    for (const reconciliation of coherent) {
      ...
      assert.deepEqual(validateProtocolArtifact(kind, candidate, { protocolVersion: '2.0.0' }), [], `${kind}:${JSON.stringify(reconciliation)}`);
    }
    for (const reconciliation of contradictory) {
      const candidate = { ...structuredClone(valid[kind]), reconciliation };
      assert.ok(validateProtocolArtifact(kind, candidate, { protocolVersion: '2.0.0' }).length > 0, `${kind}:${JSON.stringify(reconciliation)}`);
    }
  }
});
...
test('policy tier and precedence are immutable and lower tiers may only narrow', () => {
  for (const kind of ['operating-action-policy', 'operate-policy-provider-registration']) {
    ...
    mismatch.tier = 'core';
    assert.ok(validateProtocolArtifact(kind, mismatch, { protocolVersion: '2.0.0' }).length > 0, `${kind}:core precedence mismatch`);
    const widening = structuredClone(valid[kind]);
    widening.narrowingOnly = false;
    assert.ok(validateProtocolArtifact(kind, widening, { protocolVersion: '2.0.0' }).length > 0, `${kind}:domain widening`);
  }
  const evaluation = structuredClone(valid['operating-policy-evaluation']);
  evaluation.policyTier = 'project';
  assert.ok(validateProtocolArtifact('operating-policy-evaluation', evaluation, { protocolVersion: '2.0.0' }).length > 0);
});
...
test('governed operations and terminal results retain exact execute-versus-rollback bindings', () => {
  ...
  assert.ok(validateProtocolArtifact('operating-governed-operation', invalidRollback, { protocolVersion: '2.0.0' }).length > 0);
  const rollback = {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-governed-operation', rollback, { protocolVersion: '2.0.0' }), []);
  const wrongExecutionKind = { ...structuredClone(valid['operating-execution-result']), operationKind: 'rollback' };
  assert.ok(validateProtocolArtifact('operating-execution-result', wrongExecutionKind, { protocolVersion: '2.0.0' }).length > 0);
  const wrongRollbackKind = { ...structuredClone(valid['operating-rollback-result']), operationKind: 'execute' };
  assert.ok(validateProtocolArtifact('operating-rollback-result', wrongRollbackKind, { protocolVersion: '2.0.0' }).length > 0);
});
...
test('every normative Phase 6 Action and Cycle edge has an independently specified strict Event payload', () => {
  ...
  const actionEdge = (event, from, to, bindings) => ({
    entityId: 'operating-action', event, from, to,
    payload: { action, from, to, ...bindings },
  });
  const cycleEdge = (event, from, to, bindings) => ({
    entityId: 'operating-cycle', event, from, to,
    payload: { cycleId: 'cyc_00000001', from, to, ...bindings },
  });
  const edges = [
    actionEdge('action.approved', 'proposed', 'approved', { operationId: null, resultId: null, reasonCode: null }),
    actionEdge('action.cancelled', 'approved', 'cancelled', { operationId: null, resultId: null, reasonCode: 'cancelled-before-dispatch' }),
    actionEdge('action.cancelled', 'queued', 'cancelled', { operationId, resultId: null, reasonCode: 'cancelled-while-queued' }),
    actionEdge('action.cancelled', 'in_progress', 'cancelled', { operationId, resultId: null, reasonCode: 'cancelled-in-progress' }),
    actionEdge('action.rejected', 'proposed', 'rejected', { operationId: null, resultId: null, reasonCode: 'policy-rejected' }),
    actionEdge('action.deferred', 'proposed', 'deferred', { operationId: null, resultId: null, reasonCode: 'approval-deferred' }),
    actionEdge('action.reopened', 'deferred', 'proposed', { operationId: null, resultId: null, reasonCode: null }),
    actionEdge('action.queued', 'approved', 'queued', { operationId, resultId: null, reasonCode: null }),
    actionEdge('action.queued', 'blocked', 'queued', { operationId, resultId, reasonCode: 'retry-queued' }),
    actionEdge('action.started', 'queued', 'in_progress', { operationId, resultId: null, reasonCode: null }),
    actionEdge('action.started', 'blocked', 'in_progress', { operationId, resultId, reasonCode: 'reconciled-resume' }),
    actionEdge('action.completed', 'in_progress', 'completed', { operationId, resultId, reasonCode: null }),
    actionEdge('action.blocked', 'queued', 'blocked', { operationId, resultId, reasonCode: 'dispatch-blocked' }),
    actionEdge('action.blocked', 'in_progress', 'blocked', { operationId, resultId, reasonCode: 'verification-blocked' }),
    cycleEdge('cycle.approved', 'awaiting_review', 'approved', { actionId: null, operationId: null, resultId: null, reasonCode: null }),
    cycleEdge('cycle.executing', 'approved', 'executing', { actionId: action.actionId, operationId, resultId: null, reasonCode: null }),
    cycleEdge('cycle.verifying', 'executing', 'verifying', { actionId: action.actionId, operationId, resultId, reasonCode: null }),
    cycleEdge('cycle.verifying', 'approved', 'verifying', { actionId: action.actionId, operationId: null, resultId: null, reasonCode: 'execution-not-required' }),
    cycleEdge('cycle.closed', 'verifying', 'closed', { actionId: action.actionId, operationId, resultId, reasonCode: null }),
    cycleEdge('cycle.closed', 'approved', 'closed', { actionId: null, operationId: null, resultId: null, reasonCode: 'no-execution-or-verification' }),
  ];
  const expectedRegistryEdges = edges.map(({ entityId, event, from, to }) => `${entityId}:${from}:${to}:${event}`).sort();
  const registeredEdges = registry.transitions
    ...
    .sort();
  assert.deepEqual(registeredEdges, expectedRegistryEdges, 'registry matches the independent normative edge matrix');
  ...
  for (const { entityId, event: type, from, payload } of edges) {
    ...
    assert.deepEqual(validateProtocolArtifact('operating-event', event, { protocolVersion: '2.0.0' }), [], edgeLabel);
    const fields = entityId === 'operating-action'
    ...
    for (const field of fields) {
      ...
      assert.ok(validateProtocolArtifact('operating-event', missing, { protocolVersion: '2.0.0' }).length > 0, `${edgeLabel}:missing ${field}`);
    }
    const mutations = entityId === 'operating-action'
      ? {
      ...
      : {
          ...
          operationId: payload.operationId === null ? operationId : null,
          resultId: payload.resultId === null ? resultId : null,
          reasonCode: type === 'cycle.closed' && from === 'verifying'
            ? 42
            : payload.reasonCode === null ? 'unexpected-reason' : null,
        };
    for (const [field, value] of Object.entries(mutations)) {
      const wrong = structuredClone(event);
      wrong.payload[field] = value;
      assert.ok(validateProtocolArtifact('operating-event', wrong, { protocolVersion: '2.0.0' }).length > 0, `${edgeLabel}:wrong ${field}`);
    }
  }
  ...
  const approvedDeferred = {
    ...
    payload: { action, from: 'approved', to: 'deferred', operationId: null, resultId: null, reasonCode: 'execution-deferred' },
  };
  assert.ok(validateProtocolArtifact('operating-event', approvedDeferred, { protocolVersion: '2.0.0' }).length > 0, 'approved->deferred remains review.submitted, not action.deferred');
});
...
test('Phase 6 runtime authority history and checkpoint hashes are atomically present or absent', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-runtime-state', historicalState, { protocolVersion: '2.0.0' }), []);
  ...
  assert.deepEqual(validateProtocolArtifact('operating-runtime-state', complete, { protocolVersion: '2.0.0' }), []);
  for (const [field, value] of Object.entries(authorityHistory)) {
    assert.ok(validateProtocolArtifact('operating-runtime-state', { ...historicalState, [field]: value }, { protocolVersion: '2.0.0' }).length > 0, `${field}:singleton`);
    const partial = structuredClone(complete);
    delete partial[field];
    assert.ok(validateProtocolArtifact('operating-runtime-state', partial, { protocolVersion: '2.0.0' }).length > 0, `${field}:missing`);
  }

  const checkpoint = structuredClone(allContracts['operating-checkpoint']);
  const hash = 'sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
  assert.deepEqual(validateProtocolArtifact('operating-checkpoint', checkpoint, { protocolVersion: '2.0.0' }), []);
  assert.deepEqual(validateProtocolArtifact('operating-checkpoint', { ...checkpoint, operationReplayIndexHash: hash, authorityHistoryHash: hash }, { protocolVersion: '2.0.0' }), []);
  assert.ok(validateProtocolArtifact('operating-checkpoint', { ...checkpoint, operationReplayIndexHash: hash }, { protocolVersion: '2.0.0' }).length > 0);
  assert.ok(validateProtocolArtifact('operating-checkpoint', { ...checkpoint, authorityHistoryHash: hash }, { protocolVersion: '2.0.0' }).length > 0);
});
