...
test('Protocol 2.0 schema inventory is the exact union of the runtime kernel and experience contracts', () => {
  ...
  const registered = listProtocolSchemas()
    ...
    .sort();
  const experienceFiles = OPERATE_EXPERIENCE_CONTRACT_KINDS_V2
    .map((kind) => loadOperateExperienceContract(kind, { protocolVersion: '2.0.0' }).path.split('/').at(-1))
    .sort();
  ...
  assert.deepEqual(OPERATE_ROLE_MANDATES_V2.map(({ id }) => id), ['advisor', 'chair', 'challenger']);
  assert.deepEqual(OPERATE_EXTENSION_CONTRACT_KINDS_V2, [
    'agent-runtime-manifest', 'operate-capability-provider-registration', 'operate-domain-registration',
    'operate-evidence-provider-registration', 'operate-evidence-resolver-registration', 'operate-executor-registration',
    'operate-live-evidence-provider-registration', 'operate-live-evidence-provider-registry',
    'operate-metric-provider-registration', 'operate-policy-provider-registration', 'operate-snapshot-provider-registration',
...
test('every current runtime schema has valid and invalid conformance examples', () => {
  ...
  for (const kind of OPERATE_RUNTIME_CONTRACT_KINDS) {
    ...
    assert.ok(
      validateProtocolArtifact(
        kind,
        invalidFromDescriptor(valid[kind], invalid[kind]),
...
test('owner Review gap summaries retain closed role and evidence identities', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-decision-ledger', sourceLedger, {
    protocolVersion: '2.0.0',
  ...
  assert.ok(validateProtocolArtifact('operating-review-read', missingRoleIdentity, {
    protocolVersion: '2.0.0',
  }).length, 'role gaps require their source Assignment identity, including explicit null');

  const missingEvidenceContract = clone(read);
  missingEvidenceContract.gaps[1].sourceContracts = [];
  assert.ok(validateProtocolArtifact('operating-review-read', missingEvidenceContract, {
    protocolVersion: '2.0.0',
  }).length, 'evidence gaps require at least one durable requested source contract');

  const privatePath = clone(receipt);
  privatePath.gaps[1].privatePath = '.planr/operate/state/private.json';
  assert.ok(validateProtocolArtifact('operating-review-receipt', privatePath, {
    protocolVersion: '2.0.0',
  }).length, 'Review gaps remain closed and cannot expose private runtime paths');
});
...
test('Assignment preserves certified Phase 5 grants while governed work requires exact cgr identities', () => {
  ...
  for (const capabilityGrantId of [
    ...
    assert.deepEqual(validateProtocolArtifact('operating-assignment', {
      ...assignment,
      capabilityGrantId,
    }, { protocolVersion: '2.0.0' }), [], capabilityGrantId);
  }
  ...
  assert.deepEqual(validateProtocolArtifact('operating-assignment', governed, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operating-assignment', {
    ...governed,
    capabilityGrantId: 'grant-asg_00000001',
  }, { protocolVersion: '2.0.0' }).length, 'execution work cannot reuse a Phase 5 grant identity');
  ...
  for (const capabilityGrantId of [
    ...
    assert.ok(validateProtocolArtifact('operating-assignment', {
      ...assignment,
      capabilityGrantId,
    }, { protocolVersion: '2.0.0' }).length, capabilityGrantId);
  }
});
...
test('context capture is a truthful non-intelligence Assignment with one exact result contract', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-assignment', assignment, {
    protocolVersion: '2.0.0',
  }), []);
  ...
  for (const patch of [
    ...
    assert.ok(validateProtocolArtifact('operating-assignment', {
      ...assignment,
      ...patch,
    }, { protocolVersion: '2.0.0' }).length > 0, JSON.stringify(patch));
  }

  const evidenceCapture = clone(valid['operating-context-capture']);
  assert.deepEqual(validateProtocolArtifact('operating-context-capture', evidenceCapture, {
    protocolVersion: '2.0.0',
  }), []);
  ...
  assert.deepEqual(validateProtocolArtifact('operating-context-capture', manifestCapture, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operating-context-capture', {
    ...manifestCapture,
...
test('each intelligence role kind accepts only its own exact output contract', () => {
  ...
  for (const assignmentKind of ['advisor', 'challenger', 'chair']) {
    const assignment = intelligenceAssignment(base, assignmentKind, outputContracts[assignmentKind]);
    assert.deepEqual(validateProtocolArtifact('operating-assignment', assignment, {
      protocolVersion: '2.0.0',
    }), [], `${assignmentKind} baseline`);
    for (const foreignKind of ['advisor', 'challenger', 'chair'].filter((kind) => kind !== assignmentKind)) {
      assert.ok(validateProtocolArtifact('operating-assignment', {
        ...assignment,
        outputContract: clone(outputContracts[foreignKind]),
      }, { protocolVersion: '2.0.0' }).length > 0, `${assignmentKind} cannot advertise ${foreignKind}`);
    }
  }
  for (const [assignmentKind, schemaId] of [
    ...
    }, { protocolVersion: '2.0.0' }).length > 0, `${schemaId}@1.0.0 is not an issuable intelligence contract`);
  }
});
...
test('bounded Action verification fixtures retain typed measurement and reject Assignment coercion', () => {
  ...
  for (const [kind, record] of [
    ...
    assert.deepEqual(validateProtocolArtifact(kind, record, { protocolVersion: '2.0.0' }), [], kind);
  }
  assert.ok(validateProtocolArtifact('operating-action', fixture('action-verification-invalid.json').action, {
    protocolVersion: '2.0.0',
  }).length > 0, 'an Action cannot carry an Assignment identity');
  for (const forbiddenField of ['assignmentId', 'operationId', 'capability', 'approval', 'policy']) {
...
test('a Chair Action hypothesis requires explicit dependencies and a verification method', () => {
  ...
  for (const field of ['dependsOnActionHypothesisIds', 'verificationMethod']) {
    ...
    assert.ok(validateProtocolArtifact('operating-decision-ledger', missing, {
      protocolVersion: '2.0.0',
    }).length > 0, field);
  }
});
...
test('Risk and Assumption revision lineage is schema-required, including a nullable initial predecessor', () => {
  for (const [kind, fields] of [
    ...
    for (const field of fields) {
      ...
      assert.ok(validateProtocolArtifact(kind, missing, { protocolVersion: '2.0.0' }).length > 0, `${kind}.${field} is required`);
    }
    assert.deepEqual(validateProtocolArtifact(kind, { ...clone(valid[kind]), predecessorRevisionId: null }, {
      protocolVersion: '2.0.0',
...
test('evidence registry declarations validate as exact public extension contracts', () => {
  ...
  for (const registration of evidenceRegistry.providers) {
    assert.deepEqual(validateProtocolArtifact('operate-evidence-provider-registration', registration, {
      protocolVersion: '2.0.0',
    }), []);
  }
  for (const registration of evidenceRegistry.resolvers) {
    assert.deepEqual(validateProtocolArtifact('operate-evidence-resolver-registration', registration, {
      protocolVersion: '2.0.0',
    }), []);
  }
});
...
test('public domain fixtures retain explicit API bindings and reject mismatched public projections', () => {
  assert.deepEqual(validateProtocolArtifact('operate-domain-registration', fixture('business-domain-valid.json'), {
    protocolVersion: '2.0.0',
  }), []);
  assert.deepEqual(validateProtocolArtifact('operate-domain-registration', fixture('software-domain-valid.json'), {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operate-domain-registration', fixture('operating-domain-invalid.json'), {
    protocolVersion: '2.0.0',
  }).length > 0);
});
...
test('the public plan helper accepts only an exact Protocol 2.0 intelligence plan', () => {
  ...
  assert.throws(() => assertOperateIntelligencePlanContractV2(fixture('intelligence-plan-invalid.json')), {
    code: 'E_PROTOCOL_ARTIFACT_INVALID',
  });
  const topologyBypass = clone(plan);
  topologyBypass.selectedRoles.find(({ roleId }) => roleId === 'chair').dependsOnRoleIds = ['advisor'];
  assert.deepEqual(validateProtocolArtifact('operating-intelligence-plan', topologyBypass, {
    protocolVersion: '2.0.0',
...
test('Planr and Operate Artifact resolver fixtures retain explicit v2 candidate and accepted Artifact identities', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-evidence-candidate', planr.candidate, {
    protocolVersion: '2.0.0',
  }), []);
  assert.deepEqual(validateProtocolArtifact('operating-evidence-candidate', artifact.candidate, {
    protocolVersion: '2.0.0',
  }), []);
  assert.deepEqual(validateProtocolArtifact('operating-artifact', artifact.acceptedArtifact.artifact, {
    protocolVersion: '2.0.0',
  }), []);
});
...
test('schema-valued additionalProperties rejects non-string contract versions', () => {
  ...
  assert.ok(validateProtocolArtifact('operating-cycle', {
    ...valid['operating-cycle'], contractVersions: { 'advisor-result': 2 },
  }, { protocolVersion: '2.0.0' }).some(({ path }) => path === '$.contractVersions.advisor-result'));
...
test('public value budgets are exact and every named deferred value fails through the package loader', () => {
  ...
  for (const state of publicSubset.cycleStates) {
    assert.deepEqual(validateProtocolArtifact('operating-cycle', {
  ...
  for (const assignmentKind of publicSubset.assignmentKinds) {
    ...
    assert.deepEqual(validateProtocolArtifact(
  ...
  for (const state of fixture('deferred-cycle-states-invalid.json').states) {
    ...
    if (['challenging', 'approved', 'executing', 'verifying'].includes(state)) assert.deepEqual(errors, [], state);
  ...
  for (const assignmentKind of fixture('deferred-assignment-kinds-invalid.json').assignmentKinds) {
    ...
    }, { protocolVersion: '2.0.0' }).length, assignmentKind);
  }
  for (const kind of fixture('deferred-triggers-invalid.json').triggers) {
    ...
    assert.ok(validateProtocolArtifact('operate-tool-call', rejected, {
      protocolVersion: '2.0.0',
    }).length, kind);
  }
});
...
test('all current actions require complete typed arguments and their compiled effect', () => {
  const argumentsByTool = {
    ...
    'operate.assignment.submit': {
      ...
      mediaType: 'application/json', encoding: 'utf-8', contentBase64: 'e30=',
      actor: { actorId: 'agent-001', kind: 'agent', runtime: 'codex' },
    },
    ...
    'operate.review.submit': {
      ...
      actor: { actorId: 'owner-001', kind: 'human', runtime: 'openplanr' },
      scope: { scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0' },
      disposition: 'approved', workDispositions: [],
    },
    'operate.action.approve': { action: { actionId: 'act_00000001', revision: 1, actionHash: digest('1') }, decision: 'approved' },
    'operate.action.execute': { action: { actionId: 'act_00000001', revision: 1, actionHash: digest('1') } },
    'operate.action.rollback': { action: { actionId: 'act_00000001', revision: 1, actionHash: digest('1') }, originalOperationId: 'op_00000001', rollbackPlanId: 'rbp_00000001' },
  };
  const effects = {
    'operate.cycle.start': 'project-write', 'operate.cycle.get': 'read-only', 'operate.cycle.resume': 'project-write',
    'operate.assignment.claim': 'machine-local-write', 'operate.assignment.submit': 'machine-local-write',
    'operate.artifact.get': 'read-only', 'operate.review.get': 'read-only', 'operate.review.submit': 'project-write',
    'operate.action.approve': 'project-write', 'operate.action.execute': 'project-write', 'operate.action.rollback': 'project-write',
  };
  ...
  for (const [tool, argumentsValue] of Object.entries(argumentsByTool)) {
    ...
    assert.deepEqual(validateProtocolArtifact('operate-allowed-action', action, {
      protocolVersion: '2.0.0',
    }), [], tool);
    const incomplete = clone(action);
    delete incomplete.arguments[Object.keys(incomplete.arguments)[0]];
    assert.ok(validateProtocolArtifact('operate-allowed-action', incomplete, {
      protocolVersion: '2.0.0',
    }).length, `${tool} rejects incomplete arguments`);
  }
  assert.ok(validateProtocolArtifact('operate-allowed-action', {
    ...valid['operate-allowed-action'], effect: 'external-effect',
  }, { protocolVersion: '2.0.0' }).length);
  assert.ok(validateProtocolArtifact('operate-allowed-action', {
    ...valid['operate-allowed-action'],
    arguments: {
      assignmentId: 'asg_00000001',
      actor: { actorId: 'agent-001', kind: 'agent', runtime: 'codex', sessionId: 'internal-only' },
    },
  }, { protocolVersion: '2.0.0' }).length, 'public claim action rejects a session binding');
});
...
test('event schema maintains strict type-specific payloads and request-bound intelligence facts', () => {
  ...
  const assignment = {
    ...valid['operating-assignment'], state: 'pending', availableAt: null, completedAt: null, claim: null,
  };
  ...
  const payloads = {
    ...
    'assignment.claimed': { assignmentId: assignment.assignmentId, actorId: 'agent-001', actorKind: 'agent', runtime: 'codex', claimId: 'claim-001', submissionId: 'sub_00000001' },
    'assignment.started': { assignmentId: assignment.assignmentId, attempt: 1 },
    'assignment.submitted': { assignmentId: assignment.assignmentId, submissionId: 'sub_00000001', rawHash: digest('a'), canonicalHash: null, sizeBytes: 21, mediaType: 'application/json', encoding: 'utf-8' },
    'artifact.created': valid['operating-artifact'],
    'assignment.validated': { assignmentId: assignment.assignmentId, submissionId: 'sub_00000001', artifactId: 'art_00000002', validatorVersion: '1.0.0' },
    'assignment.rejected': { assignmentId: assignment.assignmentId, submissionId: 'sub_00000001', violations: [{ path: '$.answer', code: 'required', message: 'answer is required' }], attempt: 1, attemptsRemaining: 2 },
    'assignment.abandoned': { assignmentId: assignment.assignmentId, reasonCode: 'legacy-absent', reason: 'No legacy role result exists.', recoveryDisposition: 'continue-partial' },
    'assignment.failed': { assignmentId: assignment.assignmentId, errorCode: 'runtime-failed', reason: 'Runtime stopped.', recoveryStatus: 'terminal' },
    ...
    'evidence.rejected': {
      resolution: {
        ...
        error: { code: 'SOURCE_NOT_FOUND', retryable: false, context: { evidenceKind: 'filesystem', sourceRootId: 'workspace-root' } },
      },
      requestHash: digest('c'),
      outcomeHash: digest('d'),
    },
    ...
    'scenario.recorded': intelligencePayload(fixture('operating-trigger-scenario-valid.json').scenario),
    'trigger.recorded': intelligencePayload(fixture('operating-trigger-scenario-valid.json').trigger),
  };

  const requestBoundIntelligenceTypes = new Set([
    'metric.observed', 'claim.recorded', 'risk.recorded', 'finding.recorded', 'assumption.recorded',
    'decision.revised', 'delta.derived', 'scenario.recorded', 'trigger.recorded',
  ]);
  ...
  for (const [type, payload] of Object.entries(payloads)) {
    sequence += 1;
    const event = {
      ...base, eventId: `evt-${sequence}`, sequence, type, payload,
    ...
    assert.ok(validateProtocolArtifact('operating-event', {
      ...event, payload: { ...payload, unexpected: true },
    }, { protocolVersion: '2.0.0' }).length, `${type} payload is closed`);
  }
  ...
  }, { protocolVersion: '2.0.0' }).length, 'intelligence facts require a canonical request fingerprint');
  ...
  assert.ok(validateProtocolArtifact('operating-event', {
    ...
    payload: { assignmentId: assignment.assignmentId, satisfiedDependencyIds: [], dependencyEventIds: [] },
  }, { protocolVersion: '2.0.0' }).length, 'legacy availability proof is not a v2 event payload');

  const created = { ...base, eventId: 'evt-created', sequence: 12, type: 'assignment.created', payload: assignment };
  for (const [label, payload] of [
    ['available-state bypass', { ...assignment, state: 'available', availableAt: '2026-08-08T08:00:01Z' }],
    ['availability timestamp bypass', { ...assignment, availableAt: '2026-08-08T08:00:01Z' }],
    ['claim-state bypass', { ...assignment, claim: { actorId: 'agent-001', actorKind: 'agent', runtime: 'codex', claimId: 'claim-001' } }],
  ]) {
    assert.ok(validateProtocolArtifact('operating-event', {
      ...created, payload,
    }, { protocolVersion: '2.0.0' }).length, label);
  }
});
...
test('raw bytes, canonical metadata, runtime replay state, and checkpoint stay exact', () => {
  ...
  assert.doesNotThrow(() => readOperatingRuntimeStateV2(valid['operating-runtime-state'], {
    protocolVersion: '2.0.0',
  }));
  assert.deepEqual(validateProtocolArtifact('operating-checkpoint', valid['operating-checkpoint'], {
    protocolVersion: '2.0.0',
  }), []);
  const badCanonical = { ...valid['operating-artifact'], canonicalHash: `sha256:${'A'.repeat(64)}` };
  assert.ok(validateProtocolArtifact('operating-artifact', badCanonical, {
    protocolVersion: '2.0.0',
  }).length);
  const missingDomainVersion = clone(valid['operating-artifact']);
  delete missingDomainVersion.domainVersion;
  assert.ok(validateProtocolArtifact('operating-artifact', missingDomainVersion, {
    protocolVersion: '2.0.0',
  }).length, 'Artifact must retain its explicit domain contract version');
});
...
test('v2 readers require explicit, matching identities with no default', () => {
  ...
  assert.throws(() => readOperatingRuntimeStateV2({
    ...valid['operating-runtime-state'], protocolVersion: '1.4.0',
  }, { protocolVersion: '2.0.0' }), { code: 'E_PROTOCOL_ARTIFACT_INVALID' });
  assert.throws(() => validateProtocolArtifact('operating-cycle', {
    ...valid['operating-cycle'], protocolVersion: undefined,
  }), { code: 'E_SCHEMA_VERSION_REQUIRED' });
  ...
  for (const [name, descriptor] of Object.entries(versionBoundaries)) {
    ...
    assert.ok(validateProtocolArtifact('operating-cycle', candidate, {
      protocolVersion: '2.0.0',
    }).length, name);
  }
});
...
test('metadata retrieval never materializes bytes and operation/body mismatches fail', () => {
  const metadataEnvelope = {
    ok: true, operation: 'operate.artifact.get',
    data: { metadata: valid['operating-artifact'], representation: 'metadata' },
    allowedActions: [],
  };
  assert.deepEqual(validateProtocolArtifact('operate-api-envelope', metadataEnvelope, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operate-api-envelope', {
    ...metadataEnvelope,
  ...
  const cycleEnvelope = {
    ...
    data: {
      ...
      progress: { total: 1, pending: 0, available: 1, active: 0, submitted: 0, validated: 0, rejected: 0, terminal: 0 },
  ...
  assert.deepEqual(validateProtocolArtifact('operate-api-envelope', cycleEnvelope, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operate-api-envelope', {
    ...cycleEnvelope,
    data: {
      ...cycleEnvelope.data,
      legacyReviewReference: { legacyCycleId: 'CYCLE-001', reviewState: 'reviewable', artifactIds: [] },
    },
  }, { protocolVersion: '2.0.0' }).length, 'v2 cycle views reject legacy routes');
  ...
  assert.deepEqual(validateProtocolArtifact('operate-api-envelope', acceptedEnvelope, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operate-api-envelope', {
    ...acceptedEnvelope,
  ...
  }, { protocolVersion: '2.0.0' }).length, 'v2 submission responses reject compatibility-shaped fields');

  const mixed = clone(valid['operate-tool-call']);
  mixed.operation = 'operate.cycle.get';
  assert.ok(validateProtocolArtifact('operate-tool-call', mixed, {
    protocolVersion: '2.0.0',
  }).length);
  assert.throws(() => assertProtocolArtifact('operate-tool-call', mixed, {
    protocolVersion: '2.0.0',
  }), { code: 'E_PROTOCOL_ARTIFACT_INVALID' });

  const responseCall = {
    kind: 'operate-tool-call', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    direction: 'response', operation: 'operate.cycle.get',
    response: {
      ok: false, operation: 'operate.cycle.get',
      error: { code: 'CYCLE_NOT_FOUND', message: 'Cycle not found.', retryable: false, context: {} },
      allowedActions: [],
    },
  };
  assert.deepEqual(validateProtocolArtifact('operate-tool-call', responseCall, {
    protocolVersion: '2.0.0',
  }), []);
  responseCall.response.operation = 'operate.cycle.resume';
  assert.ok(validateProtocolArtifact('operate-tool-call', responseCall, {
    protocolVersion: '2.0.0',
  }).length, 'nested response operation must equal the outer operation');
});
...
test('failure envelopes accept only safe, typed diagnostic context', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operate-api-envelope', envelope, {
    protocolVersion: '2.0.0',
  }), []);
  assert.ok(validateProtocolArtifact('operate-api-envelope', {
    ...envelope,
    error: { ...envelope.error, context: { ...envelope.error.context, rawSubmission: 'secret bytes' } },
  }, { protocolVersion: '2.0.0' }).length, 'error context rejects unapproved fields');
  assert.ok(validateProtocolArtifact('operate-api-envelope', {
    ...envelope,
    operation: 'operate.review.get',
    error: {
      code: 'REVIEW_NOT_AUTHORIZED', message: 'Review access is denied.', retryable: false,
      context: { reviewId: 'rev_00000001', ownerActorId: 'private-review-owner' },
    },
  }, { protocolVersion: '2.0.0' }).length, 'Review refusals reject private owner identity context');
});
...
test('allowed actions and tool calls reject every declared placeholder form', () => {
  ...
  for (const { tool, arguments: baseArguments, field, value } of cases) {
    ...
    assert.ok(validateProtocolArtifact('operate-allowed-action', action, {
      protocolVersion: '2.0.0',
    }).length, `${tool}:${field}`);
    const call = {
      kind: 'operate-tool-call', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
      direction: 'request', operation: tool, request: argumentsValue,
    };
    assert.ok(validateProtocolArtifact('operate-tool-call', call, {
      protocolVersion: '2.0.0',
    }).length, `${tool}:${field}:tool-call`);
  }

  for (const label of placeholders.labels) {
    assert.ok(validateProtocolArtifact('operate-allowed-action', {
      ...valid['operate-allowed-action'], label,
    }, { protocolVersion: '2.0.0' }).length, `action-label:${label}`);
  }
});
