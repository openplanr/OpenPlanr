...
const question = {
  ...
  type: 'text',
  label: 'Who owns final workflow decisions?',
...
const action = {
  ...
  schemaVersion: '1.0.0',
  protocolVersion: '1.2.0',
  id: 'apply-workflow-configuration',
  label: 'Apply this workflow configuration',
  command: `planr guided-answer submit --confirm ${digest('e')} --yes --json`,
...
test('confirmations and evidence classifications are exact-digest-bound', () => {
  const confirmation = {
    ...
    configHead: questionnaire.configHead,
    arguments: [],
    destinations: ['.planr/workflow/config.json'],
    writes: ['.planr/workflow/config.json'],
    createdAt,
    expiresAt,
  };
