...
function dereference(root, node) {
  ...
  for (
    let hops = 0;
    current && typeof current.$ref === 'string' && current.$ref.startsWith('#/') && hops < 8;
    hops += 1
  ) {
    current = current.$ref
      .slice(2)
...
test('the frozen evaluation family is exactly fourteen kinds, each resolvable only at the explicit version', () => {
  ...
  for (const kind of EVALUATION_CONTRACT_KINDS_V1) {
    ...
    assert.equal(
      resolved.schema.$id,
      `https://openplanr.dev/schemas/v${PROTOCOL_VERSION}/${kind}.schema.json`,
    );

    const registered = listProtocolSchemas().filter((entry) => entry.kind === kind);
    assert.deepEqual(registered, [
      { kind, protocolVersion: PROTOCOL_VERSION, path: resolved.path },
    ]);
  }
});
...
test('an implicit version, an unsupported version, and an unknown kind are each refused', () => {
  ...
  assert.throws(
    () =>
      loadEvaluationContract('evaluation-observation-summary', {
  ...
  for (const kind of EVALUATION_CONTRACT_KINDS_V1) {
    walkStructural(schemaOf(kind), kind, (node, path) => {
      ...
      assert.equal(
        ...
        false,
        `${path}: a nested object accepts unknown fields`,
      );
    });
  }
        ...
        if (/Digest$/u.test(field)) {
          ...
          for (const branch of branches) {
            assert.equal(
              branch.pattern,
              DIGEST_PATTERN,
              `${path}/${field}: digest is not pinned to sha256 hex`,
            );
          }
        }
        if (/At$/u.test(field)) {
          timestamps += 1;
          assert.equal(
            dereference(root, declared).format,
            'date-time',
            `${path}/${field}: timestamp is not RFC 3339`,
          );
        }
      }
    });
...
test('the shipped grader and host-profile registries satisfy their published schemas', () => {
  for (const [name, kind] of [
    ...
    assert.deepEqual(
      validateProtocolArtifact(kind, registry, { protocolVersion: PROTOCOL_VERSION }),
      [],
      name,
    );
  }
...
test('a registry member carrying an unknown field is refused by the envelope schema', () => {
  ...
  const errors = validateProtocolArtifact('evaluation-grader-registry', tampered, {
    protocolVersion: PROTOCOL_VERSION,
  });
  assert.ok(
    errors.some(
      (error) => error.rule === 'additionalProperties' && error.detail.includes('sandbox'),
    ),
