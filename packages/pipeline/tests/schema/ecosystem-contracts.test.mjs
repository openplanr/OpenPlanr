...
test('canonical adapter registry validates as the active shared adapter contract', () => {
  ...
  for (const adapter of registry.adapters) {
    ...
    assert.equal(Object.hasOwn(adapter.entrypoints, 'operate'), false);
  ...
  assert.equal(
    ...
    'advisory',
    'Codex advisory isolation is explicit shared adapter metadata',
  );
});
...
test('canonical role registry validates and contains exactly nine unique roles', () => {
  ...
  assert.equal(
    registry.roles.find((role) => role.id === 'entity-scaffold-agent').activation,
    'manual',
  );
});
...
test('runtime lock validates and rejects an unknown field', () => {
  const lock = {
    ...
    adapters: [
      { runtime: 'codex', version: '0.25.1', capabilityLevel: 'workflow', installScope: 'both' },
    ],
  };
...
test('the current ecosystem manifest contract closes over the shape the generator emits', () => {
  const current = readJson('schemas/v1.3.0/ecosystem-manifest.schema.json');
  const emitted = readJson('conformance/fixtures/release-ledger/ledger-valid.json')[
    'ecosystem-manifest'
  ];

  assert.deepEqual(validate(emitted, current), []);
  assert.ok(
    validate(emitted, schema('ecosystem-manifest')).length > 0,
    'the frozen v1.1.0 contract predates professionalSkills and landingWorkflow and stays frozen',
  );
  assert.ok(validate({ ...emitted, unknownSection: {} }, current).length > 0);
  for (const section of ['professionalSkills', 'landingWorkflow']) {
    const without = { ...emitted };
    delete without[section];
    assert.ok(validate(without, current).length > 0, `${section} is required, not optional`);
  }
});
