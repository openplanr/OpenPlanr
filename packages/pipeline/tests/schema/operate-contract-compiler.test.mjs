...
function temporaryProject(registry) {
  ...
  writeFileSync(join(project, 'registry/operate-v2-contracts.json'), `${JSON.stringify(isolatedRegistry, null, 2)}\n`);
  return project;
}
...
test('the canonical registry compiles to a frozen deterministic explicit-v2 catalog', () => {
  ...
  assert.deepEqual(first.runtimeContracts.map(({ id }) => id), [
    'operating-cycle',
  ...
  assert.equal(first.schemaRegistry['operating-artifact']['2.0.0'], 'schemas/v2.0.0/operating-artifact.schema.json');
  ...
  assert.deepEqual(first.governedExecution.effectClasses, [
    'read-only',
    'machine-local-write',
    'project-write',
    'provider-call',
  ...
  assert.deepEqual(first.evidence.sourceContracts.map(({ id }) => id), [
    'capacity-throughput',
    'channel-economics',
    'ci-test-evidence',
...
test('the catalog is source-order independent', () => {
  ...
  for (const field of [
    'contracts', 'roles', 'guards', 'transitions', 'operations', 'actions', 'dependencyPolicies', 'errors',
  ]) shuffled[field].reverse();

  assert.deepEqual(compileOperateContractRegistry(shuffled), compileOperateContractRegistry(source()));
});

test('the compiler rejects malformed, duplicate, unknown, unsafe, vendor, and consumer declarations', () => {
  const cases = [
    ['unknown declaration', (registry) => { registry.undeclared = {}; }, 'E_OPERATE_CONTRACT_UNKNOWN_DECLARATION'],
    ['duplicate contract version', (registry) => { registry.contracts.push(clone(registry.contracts[0])); }, 'E_OPERATE_CONTRACT_DUPLICATE'],
    ['unknown contract reference', (registry) => { registry.roles[0].output.schemaId = 'unknown-contract'; }, 'E_OPERATE_CONTRACT_UNKNOWN_REFERENCE'],
    ['unknown guard reference', (registry) => { registry.operations[0].guard = 'unknown-guard'; }, 'E_OPERATE_CONTRACT_UNKNOWN_REFERENCE'],
    ['unsafe schema path', (registry) => { registry.contracts[0].schemaPath = '../outside.schema.json'; }, 'E_OPERATE_CONTRACT_UNSAFE'],
    ['unowned dashboard declaration surface', (registry) => {
    ...
    ['unowned registry verification surface', (registry) => {
      registry.generation.verifiedTargets.find(({ kind }) => kind === 'registry').path =
        'registry/foreign.json';
    }, 'E_OPERATE_CONTRACT_UNSAFE'],
    ['vendor declaration', (registry) => { registry.contracts[0].docsSection = 'codex-runtime'; }, 'E_OPERATE_CONTRACT_NON_PORTABLE'],
    ['consumer declaration', (registry) => { registry.roles[0].docsSection = 'adatalabs-output'; }, 'E_OPERATE_CONTRACT_NON_PORTABLE'],
    ['implicit version policy', (registry) => { registry.protocol.versionPolicy = 'compatible'; }, 'E_OPERATE_CONTRACT_MALFORMED'],
    ['implicit public projection version', (registry) => { registry.contracts.find(({ id }) => id === 'business-operating-snapshot-projection').version = '2.0.0'; }, 'E_OPERATE_CONTRACT_UNKNOWN_REFERENCE'],
    ['implicit API domain-contract mapping', (registry) => { registry.extensions.domains[0] = { ...registry.extensions.domains[0], domainId: 'business', domainContract: { apiDomainId: 'business', id: 'derived-business-domain', version: '1.0.0' } }; }, 'E_OPERATE_CONTRACT_UNKNOWN_REFERENCE'],
    ['dynamic provider declaration', (registry) => { registry.extensions.snapshotProviders = [{ ...JSON.parse(readFileSync(join(root, 'conformance/fixtures/operating-runtime-v2/all-contracts-valid.json'), 'utf8'))['operate-snapshot-provider-registration'], implementation: { kind: 'module', path: './provider.mjs' } }]; }, 'E_OPERATE_CONTRACT_UNKNOWN_DECLARATION'],
    ['destructive executor declaration', (registry) => { registry.extensions.executors[0].effectCeiling = 'destructive'; }, 'E_OPERATE_CONTRACT_UNSAFE'],
    ['dynamic executor declaration', (registry) => { registry.extensions.executors[0].implementation = { kind: 'module', id: 'unsafe-executor' }; }, 'E_OPERATE_CONTRACT_UNSAFE'],
    ['unknown evidence source contract', (registry) => {
      const advisor = registry.extensions.domains[0].roles.find(({ roleKind }) => roleKind === 'advisor');
      advisor.evidenceRequirements[0].acceptedSourceContracts = [{ id: 'caller-asserted', version: '1.0.0' }];
    }, 'E_OPERATE_CONTRACT_MALFORMED'],
    ['advisor without evidence requirements', (registry) => {
      const advisor = registry.extensions.domains[0].roles.find(({ roleKind }) => roleKind === 'advisor');
...
test('the compiler is pure and the runtime loader remains an exact-v2 boundary', () => {
  ...
  assert.deepEqual(compilerImports, [], 'the registry is the sole hand-authored role and rubric source');
  ...
  assert.equal(loadOperateRuntimeContract('operating-artifact', { protocolVersion: '2.0.0' }).kind, 'operating-artifact');
  assert.throws(() => loadOperateRuntimeContract('operating-artifact'), { code: 'E_SCHEMA_VERSION_REQUIRED' });
  assert.throws(() => loadOperateRuntimeContract('operating-artifact', { protocolVersion: '1.4.0' }), {
