...
const registry = JSON.parse(readFileSync(
  new URL('../../registry/operate-v2-contracts.json', import.meta.url),
  'utf8',
));
...
test('compiled business roles carry the canonical mandate appendix', () => {
  ...
  for (const role of business.roles) {
    ...
    assert.deepEqual(role.mandate.allowedCapabilities, [...INTELLIGENCE_MANDATE_CEILING_V2.allowedCapabilities]);
    assert.deepEqual(role.mandate.forbiddenEffects, [...INTELLIGENCE_MANDATE_CEILING_V2.forbiddenEffects]);
  }
});
