...
const valid = JSON.parse(
  await readFile(new URL('experience-bridge-valid.json', fixtureRoot), 'utf8'),
);
const invalid = JSON.parse(
  await readFile(new URL('experience-bridge-invalid.json', fixtureRoot), 'utf8'),
);
...
test('experience contracts require exact protocol identity', () => {
  assert.throws(() => loadOperateExperienceContract('operate-experience-view', {}), {
    code: 'E_SCHEMA_VERSION_REQUIRED',
  });
  assert.throws(
    () => loadOperateExperienceContract('operate-experience-view', { protocolVersion: '1.4.0' }),
    { code: 'E_SCHEMA_VERSION_UNSUPPORTED' },
  );
  assert.throws(
    () => loadOperateExperienceContract('not-an-experience-contract', { protocolVersion: '2.0.0' }),
...
test('preview subjects and live-patch Event heads are conditionally closed', () => {
  ...
  assert.deepEqual(
    validateOperateExperienceArtifactV2('operate-experience-preview', {
      ...preview,
      subject: { ...preview.subject, revision: null },
    }),
    [],
  );
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-preview', {
      ...preview,
      subject: { ...preview.subject, kind: 'action', revision: null },
    }),
    [],
  );
  ...
  assert.deepEqual(
    validateOperateExperienceArtifactV2('operate-experience-live-patch', initialPatch),
    [],
  );
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-live-patch', {
      ...initialPatch,
      fromEventHead: {
        sequence: 0,
  ...
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-live-patch', {
      ...initialPatch,
      fromEventHead: { sequence: 1, hash: null },
    }),
    [],
  );
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-live-patch', {
      ...initialPatch,
      operations: [
        { op: 'replace', path: '/unknown', valueHash: initialPatch.patchHash, value: [] },
      ],
    }),
    [],
  );
  for (const [path, value] of [
    ['/domainMetrics', []],
    ['/claims', []],
    ['/replay', valid['operate-experience-view'].replay],
  ]) {
    assert.deepEqual(
      validateOperateExperienceArtifactV2('operate-experience-live-patch', {
        ...initialPatch,
        operations: [{ op: 'replace', path, valueHash: initialPatch.patchHash, value }],
      }),
      [],
    );
  }
...
test('screen-fidelity collections and replay proof are required and recursively closed', () => {
  ...
  const schema = loadOperateExperienceContract('operate-experience-view', {
    protocolVersion: '2.0.0',
  }).schema;
  assert.deepEqual(schema.$defs.cycle.properties.replayCheckpoint, {
    oneOf: [{ type: 'null' }, { $ref: '#/$defs/replayCheckpoint' }],
  });
  assert.deepEqual(schema.$defs.evidenceProvenance.required, [
    'sourceArtifactId',
    'evidenceArtifactId',
    'rawHash',
    'canonicalHash',
  ...
  assert.deepEqual(schema.$defs.history.properties.actorId, {
    type: ['string', 'null'],
    minLength: 1,
    maxLength: 128,
  });
  for (const field of ['domainMetrics', 'claims', 'replay']) {
    const missing = clone(view);
    delete missing[field];
    assert.notDeepEqual(
      validateOperateExperienceArtifactV2('operate-experience-view', missing),
      [],
      field,
    );
  }
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-view', {
      ...view,
      replay: { ...view.replay, liveAccessUsed: true },
    }),
    [],
  );
  const missingStateParity = clone(view);
  delete missingStateParity.replay.parityProof.stateParityVerified;
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-view', missingStateParity),
    [],
  );
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-view', {
      ...view,
      replay: { ...view.replay, hiddenReasoning: 'prohibited' },
    }),
    [],
  );
  assert.notDeepEqual(
    validateOperateExperienceArtifactV2('operate-experience-view', {
      ...view,
      domainMetrics: [{ metricId: 'met_invalid', inventedValue: 42 }],
    }),
    [],
  );
});
...
test('canonical public readers accept the synchronized valid fixtures', () => {
  ...
  assert.equal(
    readOperateExperienceViewV1(valid['operate-experience-view'], options),
    valid['operate-experience-view'],
  );
  assert.equal(
    readOperatingPlanningProposalV1(valid['operating-planning-proposal'], options),
    valid['operating-planning-proposal'],
  );
  assert.equal(
    readOperatingOriginV1(valid['operating-origin'], options),
    valid['operating-origin'],
  );
  assert.equal(
    readOperatingDeliveryEvidenceV1(valid['operating-delivery-evidence'], options),
    valid['operating-delivery-evidence'],
  );
});
