...
test('public domain-registration fixtures require exact API bindings and their own projection identity', () => {
  for (const registration of [business(), software()]) {
    ...
    assert.doesNotThrow(() =>
      assertProtocolArtifact('operate-domain-registration', registration, {
        protocolVersion: '2.0.0',
      }),
    );
  }
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', wrongBinding, {
      protocolVersion: '2.0.0',
    }).length > 0,
  );
  ...
  assert.ok(
    validateProtocolArtifact('operate-domain-registration', crossDomainProjection, {
      protocolVersion: '2.0.0',
    }).length > 0,
  );
});
