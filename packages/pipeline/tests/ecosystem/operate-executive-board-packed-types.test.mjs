...
test('packed trace and Executive Board declarations reject invalid proof, relation, and mode combinations', {
  ...
  try {
    const packed = run('npm', [
      'pack', '--ignore-scripts', '--json', '--cache', join(temporaryRoot, 'npm-cache'),
    ...
    run('tar', ['-xzf', join(temporaryRoot, filename), '-C', installedRoot, '--strip-components=1']);
    ...
    run(process.execPath, [
      typescript, '--noEmit', '--strict', '--target', 'ES2022', '--module', 'Node16',
      '--moduleResolution', 'Node16', join(consumerRoot, 'index.mts'),
    ], { cwd: consumerRoot });
  } finally {
