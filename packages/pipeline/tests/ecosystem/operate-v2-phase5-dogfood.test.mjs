...
test('a clean public consumer completes deterministic synthetic business and software operating cycles', { timeout: 120_000 }, () => {
  ...
  const extracted = spawnSync('tar', ['-xzf', packed.tarballPath, '-C', installedPackage, '--strip-components=1'], { encoding: 'utf8' });
  ...
  for (const journey of report.journeys) {
    assert.deepEqual(journey.stageOrder, PHASE_5_STAGES, `${journey.domainId}: complete public loop order`);
    assert.deepEqual(Object.keys(journey.stages), PHASE_5_STAGES, `${journey.domainId}: named checkpoints only`);
  ...
  assert.match(report.stop, /No policy, capability grant, approval, executor, connector, operating provider\/model dispatch, external effect, release, or remote action/u);
  assert.doesNotMatch(JSON.stringify(report), /contentBase64|rawBytes|resolverPayload|credential|operationId|executionAssignmentIds|externalEffect/iu);
});
