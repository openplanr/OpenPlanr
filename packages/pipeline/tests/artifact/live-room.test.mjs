...
test('live room links isolate reviewer owner-verdict and management capabilities', () => {
  ...
  assert.throws(() => parseLiveRoomLink(`https://user:secret@example.test/r/${roomId}#k=${key}`), {
    code: 'E_ARTIFACT_ROOM_INVALID',
  });
  assert.throws(() => parseLiveRoomLink(`http://example.test/r/${roomId}#k=${key}`), {
    code: 'E_ARTIFACT_ROOM_INVALID',
  });
  assert.equal(parseLiveRoomLink(`http://127.0.0.1:8787/r/${roomId}#k=${key}`).role, 'reader');
  assert.throws(() => parseLiveRoomLink(`https://example.test/r/${roomId}#k=${key}&k=${key}`), {
    code: 'E_ARTIFACT_ROOM_INVALID',
  });
});
...
test('room event encryption round trips and binds room plus reviewed digest', async () => {
  ...
  await assert.rejects(
    () => decryptLiveRoomEvent(encrypted, { key, roomId, reviewOf: 'b'.repeat(64) }),
    { code: 'E_ARTIFACT_DIGEST_MISMATCH' },
  );
});
...
test('append helper uploads ciphertext instead of feedback plaintext', async () => {
  ...
  const link = createLiveRoomLinks({
    roomId,
    key,
    writeCapability: write,
    ownerCapability: owner,
...
test('one hosted-style reviewer append and one owner digest-bound verdict converge through their separate authorities', async () => {
  const envelope = createArtifactEnvelope({
    artifacts: [{ id: 'artifact', title: 'Artifact', html: '<p>journey</p>' }],
  });
  ...
  const roomCiphertext = await encryptArtifactPayload(
    compressArtifactPayload(envelope).compressed,
    { key },
  );
  ...
  const client = {
    read: async () => ({
      ...
      operation: 'read',
      descriptor,
      iv: roomCiphertext.iv,
      ciphertext: roomCiphertext.ciphertext,
      expiresAt: '2026-07-22T00:00:00.000Z',
      commentsEnabled: true,
    ...
    append: async (_id, capability, { expectedGeneration, record }) => {
      ...
      const authorizedCapability =
        capability === write ? 'reviewer-write' : capability === owner ? 'owner-verdict' : null;
  ...
  const ownerReview = {
    ...afterReviewer.review,
    decision: 'approved',
    overall: 'Digest-bound owner verdict',
  };
...
test('one prepared creation survives a lost response and exact retry without rotating any secret', async () => {
  ...
  await assert.rejects(
    ...
    (failure) => {
      ...
      return (
        failure.code === 'E_ARTIFACT_ROOM_CREATE_AMBIGUOUS' &&
  ...
  assert.equal(
    JSON.stringify(ambiguousFailure.toJSON()).includes(requests[0].body.creationId),
    false,
  );
  assert.equal(
    JSON.stringify(ambiguousFailure.toJSON()).includes(requests[0].body.reviewerCapability),
    false,
  );
  ...
  await assert.rejects(
    ...
    (failure) =>
      failure.code === 'E_ARTIFACT_ROOM_INVALID' &&
...
test('legacy rooms hydrate as readable and permanently non-mutable', async () => {
  const envelope = createArtifactEnvelope({
    artifacts: [{ id: 'artifact', title: 'Artifact', html: '<p>legacy</p>' }],
  });
  let createRequest;
  ...
  const encrypted = await encryptArtifactPayload(compressArtifactPayload(envelope).compressed, {
    key,
  });
  const client = {
    read: async () => ({
      reviewOf: await (async () => {
  ...
  const hydrated = await (await import('../../lib/artifact/index.mjs')).hydrateLiveReviewRoom(
    link,
    { client },
  );
  ...
  await assert.rejects(
    () =>
      appendLiveRoomEvent(
        ...
        createLiveRoomEvent({
          roomId,
          reviewOf: createRequest.reviewOf,
          kind: 'pin',
          payload: pin(),
          eventId: 'legacy-mutation',
        }),
        { client },
      ),
    { code: 'E_ARTIFACT_ROOM_LEGACY_READ_ONLY' },
  );
});

test('v2 hydration authenticates the full chain and rejects a foreign server head', async () => {
  const envelope = createArtifactEnvelope({
    artifacts: [{ id: 'artifact', title: 'Artifact', html: '<p>signed</p>' }],
  });
  ...
  const descriptor = createLiveRoomDescriptor({
    roomId,
    reviewOf: signedReviewOf,
    ownerSigner,
    createdAt: '2026-07-15T00:00:00.000Z',
  });
  const semantic = createLiveRoomEvent({
    roomId,
    reviewOf: signedReviewOf,
    kind: 'pin',
    payload: pin(),
    eventId: 'signed-hydration',
    createdAt: '2026-07-15T00:00:01.000Z',
  });
  ...
  await assert.rejects(
    () =>
      hydrateLiveReviewRoom(link, {
        client: { read: async () => ({ ...room, head: `sha256:${'f'.repeat(64)}` }) },
      }),
  ...
  assert.throws(
    () =>
      createLiveRoomEvent({
        ...
        reviewOf,
        kind: 'owner_decision',
        payload: { decision: 'approved', injected: true },
      }),
    { code: 'E_ARTIFACT_ROOM_EVENT_INVALID' },
  );
  assert.throws(
    () =>
      createLiveRoomEvent({
        ...
        reviewOf,
        kind: 'pin_status',
        payload: { pinId: 'pin-1', status: 'reopened' },
      }),
    { code: 'E_ARTIFACT_ROOM_EVENT_INVALID' },
  );
});
