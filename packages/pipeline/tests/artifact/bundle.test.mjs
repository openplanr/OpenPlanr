...
function createSupportedGraph(root) {
  ...
  write(
    root,
    'assets/icons.svg',
    '<svg xmlns="http://www.w3.org/2000/svg"><symbol id="check"><path d="M0 0h1v1z"/></symbol></svg>',
  );
  write(root, 'styles/nested/base.css', '.base{background-image:url("../../assets/pixel.png")}');
  write(
    root,
    'styles/main.css',
    '@import "./nested/base.css";@font-face{font-family:Demo;src:url("../assets/font.woff2")}body{font-family:Demo}',
  );
  write(root, 'scripts/dep.js', 'export const message = "bundled module";');
  write(
    root,
    'scripts/module.js',
    'import { message } from "./dep.js"; document.documentElement.dataset.message = message;',
  );
...
test('supported HTML, JS/module, CSS, image, SVG, font, and srcset graph bundles deterministically', async () => {
  ...
  assert.equal(
    first.files.filter((path) => path === 'assets/pixel.png').length,
    1,
    'duplicate references read once',
  );
  assert.ok(
    first.assets.length < first.fileCount,
    'equal-content files are deduplicated in asset records',
  );
  const inputHashes = new Set(
    first.files.map((path) =>
      createHash('sha256')
  ...
  assert.ok(
    first.assets.every(({ sha256 }) => inputHashes.has(sha256)),
    'asset records describe decoded graph inputs only',
  );
...
test('remote resources can be explicitly rejected while forms and navigation always fail closed', async (t) => {
  const cases = [
    ...
    [
      'template form',
      '<template><form><button>send</button></form></template>',
      ARTIFACT_ERROR_CODES.UNSAFE_HTML,
    ],
    [
    ...
    [
      'noscript fallback',
      '<noscript><img src="//evil.test/p.png"></noscript>',
      ARTIFACT_ERROR_CODES.UNSAFE_HTML,
    ],
    [
      'noembed fallback',
      '<noembed><img src="//evil.test/p.png"></noembed>',
      ARTIFACT_ERROR_CODES.UNSAFE_HTML,
    ],
    [
      'noframes fallback',
      '<noframes><img src="//evil.test/p.png"></noframes>',
      ARTIFACT_ERROR_CODES.UNSAFE_HTML,
    ],
  ];
...
test('remote redirects are revalidated and content is fetched once per canonical URL', async () => {
  ...
  assert.deepEqual(
    ...
    ['https://assets.example.test/logo', 'https://cdn.example.test/logo.png'],
  );
  assert.match(result.html, /data:image\/png;base64,/);
});
  ...
  await t.test('downloaded scripts cannot retain network behavior', async () => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', {
        root,
        fetchImpl: async () =>
          new Response('fetch("https://api.example.test/private")', {
            headers: { 'content-type': 'text/javascript' },
          }),
        lookupImpl: publicLookup,
      }),
...
test('ambiguous srcset and active embedded SVG data fail closed', async (t) => {
  await t.test('ambiguous data URI candidate list', async () => {
    ...
    write(
      root,
      'index.html',
      '<img srcset="data:image/png;base64,iVBORw0KGgo=,//evil.test/p.png 2x">',
    );
    await assert.rejects(
  ...
  await t.test('base64 SVG cannot define a remote xml:base', async () => {
    const root = fixture();
    const svg = Buffer.from(
      '<svg xmlns="http://www.w3.org/2000/svg" xml:base="//evil.test/"><use href="#x"/></svg>',
    ).toString('base64');
    write(root, 'index.html', `<img src="data:image/svg+xml;base64,${svg}">`);
    await assert.rejects(
      bundleArtifact('index.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.UNSAFE_HTML),
    );
  });
});
...
test('external SVG dependency graphs are inspected and packaged or rejected', async (t) => {
  await t.test('nested local image is packaged', async () => {
    ...
    write(
      root,
      'icon.svg',
      '<svg xmlns="http://www.w3.org/2000/svg"><image href="pixel.png"/></svg>',
    );
  ...
  await t.test('SMIL cannot mutate a packaged URL attribute', async () => {
    ...
    write(
      root,
      'icon.svg',
      '<svg xmlns="http://www.w3.org/2000/svg"><image id="target"/><set href="#target" attributeName="href" to="//evil.test/p.png"/></svg>',
    );
    write(root, 'index.html', '<img src="icon.svg">');
    await assert.rejects(
      bundleArtifact('index.html', { root }),
  ...
  await t.test('inline SMIL is rejected before serialization', async () => {
    ...
    write(
      root,
      'index.html',
      '<svg><image id="target"/><animate href="#target" attributeName="href" values="#safe;//evil.test/p.png"/></svg>',
    );
    await assert.rejects(
      bundleArtifact('index.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.UNSAFE_HTML),
...
test('traversal, absolute paths, symlink escape, and missing assets use exact errors', async (t) => {
  await t.test('lexical traversal', async () => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.PATH_TRAVERSAL),
    );
  });
  ...
  await t.test('missing dependency', async () => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.UNRESOLVED_ASSET),
    );
  });
  await t.test('symlink escape', async (st) => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.SYMLINK_ESCAPE),
    );
  });
  ...
  await t.test('entry resolving to a directory', async () => {
    ...
    await assert.rejects(
      bundleArtifact('directory.html', { root }),
      expectCode(ARTIFACT_ERROR_CODES.INPUT_INVALID),
    );
  });
});
...
test('bare and nonliteral module imports are rejected', async (t) => {
  ...
  await t.test('new URL import metadata asset', async () => {
    ...
    write(
      root,
      'main.js',
      'const image = new URL("./pixel.png", import.meta.url); console.log(image);',
    );
  ...
  await t.test('CSS data URI import rule', async () => {
    ...
    write(
      root,
      'main.css',
      '@import "data:text/css;base64,QGltcG9ydCAnaHR0cHM6Ly9ldmlsLnRlc3QveC5jc3MnOw==";',
    );
  ...
  await t.test('JavaScript data URI import statement', async () => {
    ...
    write(
      root,
      'main.js',
      'import "data:text/javascript;base64,ZmV0Y2goJ2h0dHBzOi8vZXZpbC50ZXN0L3gnKQ==";',
    );
    write(root, 'index.html', '<script type="module" src="main.js"></script>');
    await assert.rejects(
      bundleArtifact('index.html', { root }),
...
test('static module extension and directory-index resolution are deterministic', async () => {
  ...
  write(
    root,
    'main.js',
    'import {value} from "./lib/value"; import {feature} from "./feature"; document.body.dataset.out=value+feature;',
  );
  write(root, 'index.html', '<script type="module" src="./main.js"></script>');
  const result = await bundleArtifact('index.html', { root });
...
test('byte and file limits fail before unbounded graph reads', async (t) => {
  await t.test('byte limit counts UTF-8 bytes', async () => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', { root, maxBytes: 12 }),
      expectCode(ARTIFACT_ERROR_CODES.BYTE_LIMIT),
    );
  });
  await t.test('configured file limit trips before the next file read', async () => {
    ...
    await assert.rejects(
      bundleArtifact('index.html', { root, maxFiles: 2 }),
  ...
  await t.test(
    'parallel module reads reserve unique realpaths before awaiting streams',
    async () => {
      const root = fixture();
  ...
  await t.test(
    'default 1000-file ceiling rejects the 1001st unique input',
    { timeout: 30_000 },
    async () => {
      const root = fixture();
      const refs = [];
...
test('remote-asset rejection is attribute-scoped, so URL-shaped prose survives', async (t) => {
  ...
  const allowed = [
    [
      'file:// quoted in escaped <code>',
      '<p>Run <code>open file://review/report.html</code> to preview.</p>',
    ],
    ['https URL in body text', '<p>See https://example.com for the changelog.</p>'],
    [
      'protocol-relative text in a comment',
      '<!-- mirror at //cdn.example.test/app.js --><p>ok</p>',
    ],
    ['ftp URL in body text', '<p>Legacy drop at ftp://example.test/private.txt is retired.</p>'],
    [
      'websocket URLs in body text',
      '<p>Sockets ws://example.test/socket and wss://example.test/socket.</p>',
    ],
  ];
  ...
  const blocked = [
    ...
    [
      'remote CSS url() in a style attribute',
      '<div style="background:url(https://cdn.example.test/a.png)">x</div>',
    ],
