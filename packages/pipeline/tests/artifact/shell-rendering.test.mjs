...
test('generated public projection matches canonical artifact bytes and digests', () => {
  const manifestBytes = readFileSync(join(root, ARTIFACT_SHELL_ASSET_PATHS.manifest), 'utf8');
  assert.equal(
    manifestBytes,
    readFileSync(join(canonicalArtifactRoot, ARTIFACT_SHELL_ASSET_PATHS.manifest), 'utf8'),
  );
  const manifest = JSON.parse(manifestBytes);
  const projectedPaths = [
    ARTIFACT_SHELL_ASSET_PATHS.manifest,
