...
const dirs = [];
const tmp = () => {
  const d = mkdtempSync(join(tmpdir(), 'planr-sf-'));
  dirs.push(d);
  return d;
};
afterEach(() => {
...
test('an openai session iterates on its own chain: previous_response_id + the default models', async () => {
  ...
  const fetchImpl = async (_url, init) => {
    ...
    return {
      ...
      json: async () => ({
        id: 'resp_2',
        output: [{ type: 'image_generation_call', result: 'AA==' }],
      }),
    };
  };
...
test('readFeedback: pending wins over submit and is CONSUMED on read', () => {
  ...
  assert.equal(
    existsSync(join(dir, PENDING_FILE)),
    false,
    'pending deleted — never double-applied',
  );
