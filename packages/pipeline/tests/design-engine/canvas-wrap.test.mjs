...
const CANVAS_RUNTIME = readFileSync(
  join(ROOT, 'templates', 'design', 'vendor', 'DesignCanvas.js'),
  'utf8',
);
...
test('imageDimensions reads an SVG viewBox, a PNG IHDR, and falls back', () => {
  ...
  const png = Buffer.concat([
    ...
    Buffer.from('IHDR'), // IHDR chunk header
    Buffer.from([0x00, 0x00, 0x00, 0x05]),
    Buffer.from([0x00, 0x00, 0x00, 0x03]), // W=5 H=3
  ]);
...
test('wrapInCanvas injects the data + neutralizes a </script> breakout (embedJson)', () => {
  ...
  const data = buildImageCanvasData({
    variantId: 'A',
    label: 'evil </script><img src=x onerror=alert(1)>',
    src: 'variant-A.svg',
    width: 10,
    height: 10,
  });
  ...
  assert.ok(
    !html.includes('</script><img src=x onerror=alert(1)>'),
    'no live </script> breakout survives',
  );
  assert.ok(html.includes('\\u003c'), 'angle brackets are \\u-escaped inside the script');
  assert.ok(
    html.includes('./vendor/DesignCanvas.js'),
    'the canvas references the vendored runtime',
  );
});
...
test('DesignCanvas emits stable unique Planr anchors without replacing legacy canvas attributes', () => {
  ...
  assert.match(
    CANVAS_SOURCE,
    /data-planr-id=\{focused \? undefined : planrAnchorId\}/,
    'the background artboard yields its anchor while its focus copy is mounted',
  );
  assert.match(
    CANVAS_SOURCE,
    /data-planr-id=\{planrAnchorId\} data-planr-screen=\{String\(sectionId\)\}/,
  ...
  assert.equal(
    typeof anchorId,
    'function',
    'compiled runtime exports its deterministic anchor helper',
  );
  ...
  assert.match(
    ...
    'the bridge can resolve the generated id',
  );
  assert.notEqual(
    anchorId('a-b', 'c'),
    anchorId('a', 'b-c'),
    'section/artboard boundaries cannot collide',
  );
  assert.notEqual(
    anchorId('résumé', '桌面'),
    anchorId('resume', '桌面'),
    'Unicode identities remain distinct',
  );
  assert.ok(
    anchorId('s'.repeat(900), 'a'.repeat(900)).length <= 512,
    'pathological ids remain bridge-bounded',
  );
});
...
test('discoverVariants prefers the canvas (.html) and degrades to the source image', () => {
  ...
  assert.equal(
    variants.find((v) => v.id === 'A').src,
    'variant-A.html',
    'A loads the canvas, not the image',
  );
});
