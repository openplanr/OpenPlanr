...
test('resolveProvider: auto is claude-svg with or without a key — a key alone never opts into openai', () => {
  ...
  assert.equal(
    resolveProvider({ auth: { apiKey: 'sk-present' } }).name,
    'claude-svg',
    'omitted request = auto',
  );
  assert.equal(
    resolveProvider({ requested: 'claude-svg', auth: { apiKey: 'sk' } }).name,
    'claude-svg',
  );
});
...
test('resolveProvider: openai only when requested; without a key the error names setup and the $0 default', () => {
  ...
  assert.throws(
    () => resolveProvider({ requested: 'midjourney', auth: { apiKey: 'sk' } }),
...
const okImage = (id) => ({
  ...
  json: async () => ({
    id,
    output: [
      { type: 'image_generation_call', result: Buffer.from('png-bytes').toString('base64') },
    ],
  }),
});
...
test('generateVariant: writes tmp file, chains previous_response_id, surfaces 429 as RATE_LIMITED', async () => {
  ...
  const fetchImpl = async (url, init) => {
    ...
    return okImage('resp_9');
  };
  const out = await generateVariant('brief', {
    apiKey: 'sk',
    fetchImpl,
    tmpDir: tmp(),
    previousResponseId: 'resp_8',
  });
  ...
  await assert.rejects(
    generateVariant('b', { apiKey: 'sk', fetchImpl: limited, tmpDir: tmp() }),
...
test('generateVariant: the request carries the default mainline + image models; flags override both', async () => {
  ...
  assert.equal(
    'previous_response_id' in calls[0].body,
    false,
    'a fresh generation opens a new chain',
  );
  ...
  assert.deepEqual(calls[1].body.tools[0], {
...
test('generateVariant: evolve sends the source image as input_image next to the brief', async () => {
  ...
  const fetchImpl = async (_url, init) => {
    calls.push(JSON.parse(init.body));
    return okImage('resp_2');
  };
  await generateVariant('more like this, warmer', {
    apiKey: 'sk',
    fetchImpl,
    tmpDir: tmp(),
    imageInputPath: '/from.png',
    readFile: () => Buffer.from('src-png'),
  });
  ...
  assert.equal(
    turn.content[1].image_url,
    `data:image/png;base64,${Buffer.from('src-png').toString('base64')}`,
  );
});
...
test('size + quality accept the documented values and reject the rest before any request', async () => {
  ...
  await assert.rejects(
    generateVariant('b', { ...base, quality: 'ultra' }),
...
test('checkQuality + extractAttributes use the mainline model for vision (overridable, no image tool)', async () => {
    ...
    return {
      ...
      json: async () => ({
        output: [
          {
            content: [
              { text: '{"pass": true, "issues": [], "fonts": ["Inter"], "colors": ["indigo"]}' },
            ],
          },
        ],
      }),
  ...
  assert.ok(
    calls.every((c) => c.tools === undefined),
    'vision calls carry no image_generation tool',
  );
...
test('iterate refuses a chain-less session; checkQuality parses the vision verdict', async () => {
  await assert.rejects(
    iterate({ lastResponseId: null }, 'fb', { apiKey: 'sk' }),
    /no lastResponseId/,
  );
  ...
  const verdictFetch = async () => ({
    ...
    status: 200,
    json: async () => ({
      output: [{ content: [{ text: 'verdict: {"pass": false, "issues": ["mark off-center"]}' }] }],
    }),
  });
