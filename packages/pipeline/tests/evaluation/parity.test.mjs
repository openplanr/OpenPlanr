...
test('the generated compatibility manifest matches the bytes on disk', () => {
  const parity = compareGeneratedAssets({ repoRoot: root });
...
test('parity compares exact bytes, not a version string', () => {
  const generated = renderProfessionalSkillsBundle({ projectRoot: root });
  const path = PROFESSIONAL_SKILLS_MANIFEST_PATH;
  const bytes = generated[path];
  assert.equal(evaluationContentDigest(readFileSync(join(root, path))), evaluationContentDigest(bytes));
...
test('every declared package export resolves to real bytes', () => {
  const parity = comparePackageExports({ repoRoot: root, packageJson });
  assert.deepEqual([...parity.mismatches], []);
  assert.equal(parity.present, parity.declared);
});
...
test('package-owned evaluation metadata is covered by the packed inventory', () => {
  const parity = comparePackedMembership({ repoRoot: root, packageJson });
  assert.deepEqual([...parity.mismatches], []);
  assert.equal(parity.present, parity.declared);
...
test('evaluation metadata the archive would not carry is a package parity failure', () => {
  const parity = comparePackedMembership({ repoRoot: root, packageJson: { ...packageJson, files: ['lib/'] } });
...
test('installed bytes that diverge from the source are a package parity failure', () => {
  const parity = comparePackedMembership({ repoRoot: root, packageJson, installedRoot: join(root, 'conformance') });
  assert.ok(parity.mismatches.length > 0);
...
test('the pipeline package excludes host prompt distributions', () => {
  for (const entry of ['skills/', 'adapters/', 'agents/', 'commands/', 'plugins/']) {
    assert.equal(packageJson.files.some((member) => member === entry || member.startsWith(entry)), false, entry);
  }
});

test('the declared archive carries the exact generated assets', () => {
  const archive = buildDeclaredArchive({ repoRoot: root });
  const generated = renderProfessionalSkillAssets(readProfessionalSkillsCatalog({ projectRoot: root }));
...
test('a declared member that would leave the installation root is refused', () => {
  assert.equal(assertNoSiblingDiscovery('/tmp/install-root', ['skills/planr-spec/SKILL.md']), '/tmp/install-root');
  assert.throws(
