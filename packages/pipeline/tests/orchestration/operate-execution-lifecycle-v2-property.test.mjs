...
test('all bounded legal terminal branches preserve immutable Action identity', () => {
  const timestamps = Array.from(
    { length: 64 },
    (_, index) => `2026-08-10T12:${String(index % 60).padStart(2, '0')}:00.000Z`,
  );
  for (const timestamp of timestamps) {
    ...
    for (const terminal of ['completed', 'blocked', 'cancelled']) {
      const value = transitionOperatingActionLifecycleV2(started, terminal, {
        updatedAt: timestamp,
      });
...
test('blocked recovery is explicit and cannot use a terminal reopen edge', () => {
  const action = structuredClone(actionFixture);
  const queued = transitionOperatingActionLifecycleV2(action, 'queued', {
    updatedAt: '2026-08-10T12:00:00.000Z',
  });
  const blocked = transitionOperatingActionLifecycleV2(queued, 'blocked', {
    updatedAt: '2026-08-10T12:01:00.000Z',
  });
  assert.equal(
    transitionOperatingActionLifecycleV2(blocked, 'queued', {
  ...
  assert.throws(
    () =>
      transitionOperatingActionLifecycleV2(blocked, 'completed', {
        updatedAt: '2026-08-10T12:02:00.000Z',
      }),
    (error) => error?.code === 'STATE_TRANSITION_INVALID',
  );
