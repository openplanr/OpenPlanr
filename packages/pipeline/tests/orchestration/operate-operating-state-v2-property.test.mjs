...
function walk(seed) {
  ...
  const artifact = {
    ...clone(valid['operating-artifact']), artifactId: `art_prop_${suffix}`, assignmentId,
    rawHash: rawHash(bytes), canonicalHash: null, sizeBytes: bytes.byteLength, inputArtifactIds: [], createdAt: TIME,
  };
  const assignment = {
    ...
    claim: { actorId: 'agent-001', actorKind: 'agent', runtime: 'codex', claimId: `claim_prop_${suffix}` },
    attemptPolicy: { ...valid['operating-assignment'].attemptPolicy, attempt: 1 },
  };
  const proof = bindAcceptedSubmissionProof({
    ...clone(valid['operating-submission']), submissionId: `sub_prop_${suffix}`, assignmentId, artifactId: artifact.artifactId,
    rawHash: artifact.rawHash, canonicalHash: null, sizeBytes: bytes.byteLength, acceptanceEventIds: [`evt_prop_accept_${suffix}`],
  }, artifact);
  const base = {
    ...
    assignments: [assignment], submissions: [proof.submission], artifacts: [artifact],
    submissionReplayIndex: [proof.replay],
  };
  const sourceRecord = (kind) => ({ ...clone(valid[kind]), sourceArtifactId: artifact.artifactId, createdAt: TIME, updatedAt: TIME });
  const request = {
    ...
    sourceArtifactIds: [artifact.artifactId], sourceRevisions: [{ sourceArtifactId: artifact.artifactId, revision: `r-${suffix}` }],
    collections: {
      objectives: [sourceRecord('operating-objective')], metrics: [sourceRecord('operating-metric')],
      findings: [], decisions: [], actions: [], risks: [sourceRecord('operating-risk')], assumptions: [sourceRecord('operating-assumption')],
    },
  };
  const draft = {
    snapshotId: `snp_prop_${suffix}`, stateId: `oms_prop_${suffix}`, timestamp: TIME, correlationId: `corr_prop_${suffix}`,
...
test('64 deterministic snapshot/state materialization and restart/replay walks are effect-free and model-free', () => {
  for (let seed = 1; seed <= 64; seed += 1) {
    ...
    const first = materializeOperatingStateSnapshotV2(request, draft, {
      initialState: base, artifactStore: store, replayHook: firstHook,
    });
    ...
    const replay = materializeOperatingStateSnapshotV2(clone(request), clone(draft), {
      initialState: rebuilt, artifactStore: store, replayHook,
    });
    ...
    assert.equal(sha256Jcs(replay.state), sha256Jcs(first.state), `seed ${seed}: durable state unchanged`);
  }
});
