...
test('snapshot candidate selection is stable across generated business and software accepted-input cases', () => {
  for (let index = 1; index <= 20; index += 1) {
    ...
    const artifact = {
      ...
      artifactId: `art_signal_property_${token}`,
      assignmentId: `asg_signal_property_${token}`,
      scopeId: `scope-signal-property-${token}`,
      domainId,
      domainVersion: '1.0.0',
      inputArtifactIds: [],
    };
    ...
    const transaction = buildOperatingSnapshotStateTransactionV2(
      ...
      {
        snapshotId: `snp_signal_property_${token}`,
        stateId: `oms_signal_property_${token}`,
