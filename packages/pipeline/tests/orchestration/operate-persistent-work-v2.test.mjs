...
function changeSet(overrides = {}) {
  return {
    ...
    findings: [{
      draftRef: 'draft_find_0001', title: 'Revenue risk', statement: 'Revenue is below plan.',
      state: 'open', ownerActorId: 'owner-001', revisitAt: null,
    }],
    decisions: [{
      draftRef: 'draft_decision_01', title: 'Prioritize retention', question: 'What should we prioritize?',
      rationale: 'Retention is the largest current risk.', evidenceRefIds: ['evr_00000001'],
      alternatives: ['Prioritize acquisition'], confidence: 0.8, assumptionIds: [],
      expectedUpside: 'Retention improves.', expectedDownside: 'Acquisition learning slows.', dissent: [],
      reopenConditions: ['Retention evidence changes materially.'], revisitConditions: ['Metric changes.'],
    ...
    actions: [{
      ...
      objectiveId: 'obj_retention_001', expectedResult: 'Customer interviews reveal retention friction.',
      metricId: 'met_retention_001', baseline: 0.4, target: 0.6, verificationWindow: 'next 30-day window',
...
function chairAssignment(state = 'pending') {
  return {
    ...
    assignmentId: CHAIR_ASSIGNMENT_ID, cycleId: 'cyc_00000001', assignmentKind: 'chair', roleId: 'chair',
    ...
    objective: 'Synthesize operating work.', state, dependsOn: [], dependencyPolicy: { kind: 'none' },
    inputArtifactIds: [...CHAIR_INPUT_ARTIFACT_IDS],
    outputContract: { schemaId: 'operating-decision-ledger', schemaVersion: '2.0.0', mediaType: 'application/json', encoding: 'utf-8', maxBytes: 65536 },
    capabilityGrantId: 'grant-chair-001', attemptPolicy: { maxAttempts: 1, attempt: state === 'running' ? 1 : 0, timeoutMs: 300000 },
    claim: state === 'running'
      ? { actorId: 'chair-001', actorKind: 'agent', runtime: 'codex', claimId: CHAIR_CLAIM_ID }
      : null,
    terminalOutcome: null, createdAt: TIME, availableAt: state === 'pending' ? null : TIME, completedAt: null,
  };
}
...
function baseState() {
  return {
    ...
    cycles: [{
      ...
      cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
      state: 'advising', inputBindingId: 'inb_00000001', contractVersions: { 'operating-work-change-set': '2.0.0' },
    ...
    inputBindings: [{
      ...
      inputBindingId: 'inb_00000001', cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
      sourceArtifactIds: [], runtimeBinding: { runtime: 'codex', adapterVersion: '1.0.0' }, capturedAt: TIME,
    }],
  };
}
...
function eventAfter(state, type, entityId, payload, eventId, causationId = null) {
  return createOperatingRuntimeEventV2({
    eventId, timestamp: TIME, cycleId: 'cyc_00000001', type, entityId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId, correlationId: 'corr-chair-001', payload,
...
function acceptedChairState(work = changeSet()) {
  ...
  const claimed = eventAfter(state, 'assignment.claimed', CHAIR_ASSIGNMENT_ID, {
    assignmentId: CHAIR_ASSIGNMENT_ID, actorId: 'chair-001', actorKind: 'agent', runtime: 'codex',
    claimId: CHAIR_CLAIM_ID, submissionId: CHAIR_SUBMISSION_ID,
  }, 'evt-chair-003');
  state = reduceOperatingRuntimeEventsV2([claimed], { initialState: state });
  const started = eventAfter(state, 'assignment.started', CHAIR_ASSIGNMENT_ID, {
    assignmentId: CHAIR_ASSIGNMENT_ID, attempt: 1,
  }, 'evt-chair-004', claimed.eventId);
  ...
  const submitted = eventAfter(state, 'assignment.submitted', CHAIR_ASSIGNMENT_ID, {
    ...
    encoding: 'utf-8',
  }, 'evt-chair-005', started.eventId);
  state = reduceOperatingRuntimeEventsV2([submitted], { initialState: state });
  const artifactCreated = eventAfter(state, 'artifact.created', CHAIR_ARTIFACT_ID, {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    artifactId: CHAIR_ARTIFACT_ID, artifactType: 'chair-result', assignmentId: CHAIR_ASSIGNMENT_ID,
    cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    schemaId: 'operating-decision-ledger', artifactSchemaVersion: '2.0.0',
    mediaType: 'application/json', encoding: 'utf-8', rawHash: canonicalHash, canonicalHash,
    sizeBytes: Buffer.byteLength(JSON.stringify(work)), storageClass: 'machine-local', sensitivity: 'internal',
    retentionClass: 'project', producer: { actorId: 'chair-001', roleId: 'chair', runtime: 'codex' },
    inputArtifactIds: [...CHAIR_INPUT_ARTIFACT_IDS], createdAt: NEXT,
  }, 'evt-chair-006', submitted.eventId);
  state = reduceOperatingRuntimeEventsV2([artifactCreated], { initialState: state });
  const validated = eventAfter(state, 'assignment.validated', CHAIR_ASSIGNMENT_ID, {
    assignmentId: CHAIR_ASSIGNMENT_ID, submissionId: CHAIR_SUBMISSION_ID,
    artifactId: CHAIR_ARTIFACT_ID, validatorVersion: '2.0.0',
  }, 'evt-chair-007', artifactCreated.eventId);
...
function workChangeSetArtifact(work = changeSet()) {
  ...
  return {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    artifactId: CHAIR_ARTIFACT_ID, artifactType: 'work-change-set', assignmentId: 'asg_legacy_work_materializer_001',
    cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    schemaId: 'operating-work-change-set', artifactSchemaVersion: '2.0.0',
    mediaType: 'application/json', encoding: 'utf-8', rawHash: canonicalHash, canonicalHash,
    sizeBytes: Buffer.byteLength(JSON.stringify(work)), storageClass: 'machine-local', sensitivity: 'internal',
    retentionClass: 'project', producer: { actorId: 'openplanr', roleId: 'work-materializer', runtime: 'portable' },
    inputArtifactIds: [], createdAt: NEXT,
  };
}
...
test('exact current Chair Artifact cannot enter the superseded work-change-set materializer', () => {
  ...
  assert.throws(() => materializeValidatedOperatingWorkV2({ artifactId: CHAIR_ARTIFACT_ID, changeSet: work }, {
    eventId: 'evt-work-001', timestamp: NEXT, correlationId: 'corr-work-001',
  }, { initialState: before }), ({ code }) => code === 'RESULT_CONTRACT_INVALID');
  assert.equal(sha256Jcs(before), beforeHash, 'rejected compatibility materialization is effect-free');
});
...
test('pure persistent-work builder derives canonical records and rejects forged bytes or local references', () => {
  ...
  assert.throws(() => buildPersistentWorkMaterializationPayloadV2({
    artifact: workChangeSetArtifact(work),
    changeSet: { ...work, findings: [{ ...work.findings[0], title: 'Changed title' }] },
    timestamp: NEXT,
  }), ({ code }) => code === 'RESULT_CONTRACT_INVALID');
  const invalidWork = changeSet({ actions: [{ ...work.actions[0], sourceFindingDraftRefs: ['draft_missing_1'] }] });
  assert.throws(() => buildPersistentPayload(invalidWork), ({ code }) => code === 'STATE_TRANSITION_INVALID');
});
...
test('pure persistent-work payload verification rejects forged runtime identities', () => {
  ...
  const payload = buildPersistentWorkMaterializationPayloadV2({ artifact, changeSet: work, timestamp: NEXT });
  const replay = buildPersistentWorkMaterializationPayloadV2({
    artifact: structuredClone(artifact), changeSet: structuredClone(work), timestamp: NEXT,
  });
  assert.deepEqual(replay, payload);
  const forged = structuredClone(payload);
  forged.actions[0].actionId = 'act_forged_001';
  assert.throws(() => assertPersistentWorkMaterializationPayloadV2(forged, {
    artifact, changeSet: work, timestamp: NEXT,
...
test('Phase 3 proposed work receives one deterministic complete governed Action authority tuple', () => {
  ...
  const input = {
    ...
    executionBinding: {
      policyId: 'bounded-project-write-policy', policyVersion: '1.0.0',
  ...
  const second = promotePersistentOperatingActionAuthorityV2(structuredClone(source), structuredClone(input));
  ...
  assert.throws(() => promotePersistentOperatingActionAuthorityV2(first, input),
