...
test('Action and Cycle traverse the canonical execution and verification lifecycles', () => {
  ...
  assert.deepEqual([originalAction.state, queued.state, started.state, completed.state], [
  ...
  assert.deepEqual([awaiting.state, approved.state, executing.state, verifying.state, closed.state], [
...
test('fresh execution rejects a foreign present verification relationship before commit, dispatch, or effect', async () => {
  ...
  await assert.rejects(() => runtime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('foreign checkpoint touched host'); } }),
    targetAdapter: new Proxy(targetAdapter, { get() { throw new Error('foreign checkpoint touched target'); } }),
  }), (error) => error?.code === 'OPERATING_SCOPE_INVALID');
...
test('governed execution atomically records the lifecycle around one result and replays without effects', async () => {
  ...
  const resultPosition = completed.events.findIndex(({ type }) => type === 'execution.result-recorded');
  assert.ok(completed.events.findIndex(({ eventId }) => eventId === identities.eventIds.cycleExecuting) < resultPosition);
  assert.ok(completed.events.findIndex(({ eventId }) => eventId === identities.eventIds.actionTerminal) > resultPosition);
  assert.equal(completed.state.actions.find(({ actionId }) => actionId === scenario.action.actionId).state, 'completed');
  assert.equal(completed.state.cycles.find(({ cycleId }) => cycleId === scenario.action.sourceCycleId).state, 'verifying');
  const verificationAssignment = completed.state.assignments.find(({ assignmentId }) => (
  ...
  const replay = await runtime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('lifecycle replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('lifecycle replay touched target'); } }),
  });
...
test('terminal verification Assignment scheduling is durable and rejects non-Event-backed lifecycle changes', async () => {
  ...
  const verificationCreated = completed.events.find(({ type, payload }) => (
    type === 'assignment.created' && payload.assignmentKind === 'verification'
  ));
  assert.ok(verificationCreated);

  const verificationCreatedIndex = completed.events.findIndex(({ eventId }) => (
    eventId === verificationCreated.eventId
  ));
  ...
  assert.deepEqual(creationSuffix.map(({ type }) => type), ['assignment.created', 'cycle.verifying']);
  const firstScheduled = scheduleOperatingRuntimeEventsV2(creationSuffix, {
    initialState: preCreation,
  });
  assert.deepEqual(firstScheduled.releaseEvents.map(({ type }) => type), ['assignment.available']);
  assert.equal(firstScheduled.state.assignments.find(({ assignmentKind }) => (
  ...
  const verificationAssignment = scheduled.state.assignments.find(({ assignmentKind }) => (
    assignmentKind === 'verification'
  ));
  ...
  assert.deepEqual(scheduled.releaseEvents.map(({ type }) => type), ['assignment.available']);
  ...
  const pendingIndex = forgedWithoutEvent.assignments.findIndex(({ assignmentKind }) => (
    assignmentKind === 'verification'
  ));
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2([], { initialState: forgedWithoutEvent }),
    (error) => error?.code === 'RESULT_CONTRACT_INVALID');

  const forgedTimestamp = structuredClone(scheduled.state);
  const availableIndex = forgedTimestamp.assignments.findIndex(({ assignmentKind }) => (
    assignmentKind === 'verification'
  ));
  forgedTimestamp.assignments[availableIndex].availableAt = '2026-08-10T12:00:01.000Z';
  assert.throws(() => reduceOperatingRuntimeEventsV2([], { initialState: forgedTimestamp }),
    (error) => error?.code === 'RESULT_CONTRACT_INVALID');
  assert.equal(targetAdapter.describe().effectCount, 1);
});
...
test('Event reduction, checkpoint loading, and replay reject foreign or ambiguous verification ownership', async () => {
  ...
  const assignmentIndex = completed.events.findIndex(({ type, payload }) => (
    type === 'assignment.created' && payload.assignmentKind === 'verification'
  ));
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2(
    [completed.events[assignmentIndex]],
  ...
  for (const field of ['scopeId', 'domainId', 'domainVersion']) {
    ...
    assert.throws(() => reduceOperatingRuntimeEventsV2([], { initialState: checkpoint }),
      (error) => error?.code === 'OPERATING_SCOPE_INVALID');
  }
  const canonicalAssignment = completed.state.assignments.find(({ assignmentKind }) => (
    assignmentKind === 'verification'
  ));
  ...
  const replayStore = {
    readSnapshot: () => structuredClone(durable),
    compareAndSwap: async () => { throw new Error('foreign replay attempted a commit'); },
  };
  ...
  await assert.rejects(() => replayRuntime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('foreign replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('foreign replay touched target'); } }),
  }), (error) => error?.code === 'OPERATING_SCOPE_INVALID');
  assert.equal(targetAdapter.describe().effectCount, 1);
});
