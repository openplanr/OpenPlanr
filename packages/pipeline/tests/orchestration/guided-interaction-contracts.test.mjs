...
test('all certified adapters declare truthful presentation modes only', () => {
  ...
  assert.equal(byId('claude-code').capabilities.interactiveQuestions, 'native');
  assert.equal(byId('cursor').capabilities.interactiveQuestions, 'chat');
});

test('normalization cannot manufacture authority from adapter capabilities', () => {
  const action = {
    id: 'begin-cycle',
    label: 'Apply a workflow configuration',
    command: 'planr guided-answer submit --json',
