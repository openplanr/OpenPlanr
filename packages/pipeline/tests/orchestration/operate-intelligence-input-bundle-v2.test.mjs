...
test('role bundle selection rejects a result Artifact substituted for exact materialized evidence custody', () => {
  ...
  const role = fixture('business-domain-valid.json').roles.find(({ roleId }) => roleId === 'strategy-finance');
  ...
  const position = evidenceArtifacts.findIndex(({ artifactId }) => artifactId === ref.evidenceArtifactId);
  ...
  assert.throws(() => prepareOperatingIntelligenceEvidenceSelectionV2({
...
test('role bundle selection matches exact source contracts instead of treating a broad Evidence kind as professional evidence', () => {
  const { result } = checkpoint();
  const role = fixture('business-domain-valid.json').roles.find(({ roleId }) => roleId === 'strategy-finance');
  const ref = structuredClone(result.state.evidenceRefs.find(({ evidenceRefId }) => (
    result.snapshot.evidenceRefIds.includes(evidenceRefId)
  )));
  const selection = (sourceContract) => prepareOperatingIntelligenceEvidenceSelectionV2({
    snapshot: result.snapshot,
    role,
  ...
  assert.deepEqual(generic.issuedEvidence, [], 'a generic context manifest cannot satisfy finance requirements');
  assert.deepEqual(generic.evidenceAbsenceDrafts.map(({ requirementId }) => requirementId), [
    'strategy-finance-evidence-1',
    'strategy-finance-evidence-2',
    'strategy-finance-evidence-3',
  ]);
  assert.deepEqual(generic.evidenceAbsenceDrafts.map(({ sourceContracts }) => sourceContracts), [
  ...
  assert.ok(generic.evidenceAbsenceDrafts.every(({ sourceEvidenceRefIds }) => (
  ...
  assert.deepEqual(finance.issuedEvidence.map(({ requirementId, evidenceRefId }) => ({ requirementId, evidenceRefId })), [{
    requirementId: 'strategy-finance-evidence-2',
    evidenceRefId: ref.evidenceRefId,
  }]);
  assert.deepEqual(finance.evidenceAbsenceDrafts.map(({ requirementId }) => requirementId), [
    'strategy-finance-evidence-1',
    'strategy-finance-evidence-3',
  ]);
});
...
test('role bundle selection fails closed when a persisted EvidenceRef loses runtime-owned source classification', () => {
  ...
  const ref = evidenceRefs.find(({ evidenceRefId }) => result.snapshot.evidenceRefIds.includes(evidenceRefId));
  ...
  assert.throws(() => prepareOperatingIntelligenceEvidenceSelectionV2({
    ...
    error.code === 'RESULT_CONTRACT_INVALID'
    && error.message.includes('EvidenceRef')
  ));
});
