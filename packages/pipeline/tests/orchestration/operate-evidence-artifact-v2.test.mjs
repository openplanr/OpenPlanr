...
test('Operate Artifact evidence references one accepted immutable v2 Artifact without a content copy or second acceptance', () => {
  ...
  assert.deepEqual(
    acceptedArtifact,
    before,
    'resolution is metadata-only and cannot mutate accepted Artifact state',
  );
...
test('Operate Artifact evidence validates accepted-v2/source binding/type/hash/access before reference', () => {
  ...
  assert.equal(
    resolve(base, { ...acceptedArtifact, accepted: false }).error.code,
    invalid.unacceptedArtifact,
  );
  assert.equal(
    resolve(base, acceptedArtifact, { operateArtifacts: [] }).error.code,
    invalid.missingArtifact,
  );
  assert.equal(resolve(base, acceptedArtifact, { capabilities: [] }).error.code, invalid.denied);
  assert.equal(
    resolve(base, valid.acceptedArtifact).error.code,
  ...
  assert.equal(
    resolve(otherScope, acceptedArtifact, {
...
test('an attempted non-Artifact dispatch is rejected as a precise kind mismatch before source access', () => {
  ...
  const result = resolve(candidate, classified(valid.acceptedArtifact), {
