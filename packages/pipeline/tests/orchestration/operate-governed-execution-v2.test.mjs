...
} from '../../lib/operate/governed-execution-v2.mjs';
import {
...
export function createGovernedExecutionCheckpointStore(initialState, { beforeCommit, afterCommit } = {}) {
  ...
  return {
    ...
    async compareAndSwap(input) {
      ...
      const matches = input.expectedEventHead.sequence === state.eventHead.sequence
...
export function governedExecutionScenario({
  ...
  action.preconditionArtifactIds = [...new Set([
  ...
  const domainPolicy = createOperatingActionPolicyV2({
    ...
    decisionMode: automatic
      ? 'automatic'
      : contracts['operating-action-policy'].decisionMode,
  ...
  const requirement = automatic ? null : createOperatingApprovalRequirementV2({
  ...
  const approval = automatic ? null : createOperatingApprovalRecordV2({
  ...
  const draft = {
    ...
    uncertainty: {
      ...
      ].map((field, index) => [field, `evt_${suffix}_uncertain_${String(index + 1).padStart(2, '0')}`])),
    },
  };
...
function replaceScenarioApprovalAuthority(state, {
...
function configureThresholdApprovalAuthority(state, suffix) {
  ...
  const records = parties.map((party, index) => createOperatingApprovalRecordV2({
...
export function reduceDirectResultTransaction(dispatchState, scenario, result, receipt = null) {
  ...
  const claimedAssignment = dispatchState.assignments.find(({ assignmentId }) => (
  ...
  const head = accepted.state.eventReplayIndex.find(({ sequence }) => (
    sequence === accepted.state.eventHead.sequence
  ));
...
registerTests && test('one approved Action becomes one Assignment, grant, operation, exact result Artifact, and contained effect', async () => {
  ...
  assert.deepEqual(
    completed.result.inputArtifactIds,
    [...new Set([
  ...
  assert.equal(completed.events.filter(({ type }) => type === 'operation.intent-recorded').length, 1);
  assert.equal(completed.events.filter(({ type }) => type === 'execution.result-recorded').length, 1);
  assert.equal(completed.state.operationReplayIndex[0].terminalResultId, scenario.draft.resultId);
  assert.equal(completed.state.approvalRecords[0].consumedByOperationId, scenario.draft.operationId);
  ...
  missingLifecycle.eventReplayIndex = missingLifecycle.eventReplayIndex.filter(({ type }) => type !== 'assignment.available');
  assert.throws(() => assertOperatingExecuteDispatchAuthorityChainV2({
    state: missingLifecycle,
    operationId: scenario.draft.operationId,
  }), ({ code }) => code === 'OPERATION_CONFLICT' || code === 'STATE_TRANSITION_INVALID');
});

registerTests && test('governed execution canonicalizes arbitrary lexical Artifact IDs before dispatch', async () => {
  const scenario = governedExecutionScenario({
    suffix: '00000032',
...
registerTests && test('raw Artifact staging failure after a proved effect remains explicitly proven-terminal and never redispatches', async () => {
  ...
  assert.equal(targetAdapter.describe().effectCount, 1);
  await assert.rejects(runtime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('retry touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('retry touched target'); } }),
  }), (error) => error.code === 'OPERATION_UNCERTAIN'
    && error.details.context.provenTerminal === true);
  assert.equal(targetAdapter.describe().effectCount, 1);
});

registerTests && test('write-then-throw byte-store acknowledgement loss proves exact success and never reuses its Artifact identity', async () => {
  ...
  const restartedReplay = await restarted.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('restart touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('restart touched target'); } }),
  });
...
registerTests && test('terminal reservation families are collision-free before target access', async () => {
  ...
  const targetAdapter = new Proxy({}, {
    get() { targetReads += 1; throw new Error('collision reached target'); },
  });
  const runtime = createOperatingGovernedExecutionRuntimeV2({
    initialState: scenario.initial,
    checkpointStore: createGovernedExecutionCheckpointStore(scenario.initial),
...
registerTests && test('proven success rebases after an unrelated checkpoint append and retains exact staged bytes', async () => {
  ...
  const checkpointStore = createGovernedExecutionCheckpointStore(scenario.initial, {
    beforeCommit(input, durable) {
      if (input.phase !== 'terminal-result'
        || durable.capabilityAvailability.some(({ availabilityId }) => (
          availabilityId === 'cava_unrelated00000012'
        ))) return;
      interleaved = true;
      const prior = durable.eventReplayIndex.find(({ sequence }) => sequence === durable.eventHead.sequence);
...
registerTests && test('two approved Actions may evolve one target sequentially while each receipt remains operation-local', async () => {
...
registerTests && test('exact completed retry returns the accepted chain without another target call or Event', async () => {
  const scenario = governedExecutionScenario();
  const targetAdapter = createDisposableLocalProjectTargetV2({
    target: scenario.action.targetBinding,
    initialValue: scenario.initialValue,
  ...
  const replay = await runtime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('replay touched target'); } }),
  });
  assert.equal(replay.replayed, true);
  assert.equal(replay.dispatchCount, 0);
...
registerTests && test('tampered payload, baseline, or result identity fails closed without accepting another chain', async () => {
  const scenario = governedExecutionScenario();
  const targetAdapter = createDisposableLocalProjectTargetV2({
    target: scenario.action.targetBinding,
    initialValue: scenario.initialValue,
  ...
  for (const [field, value, code = 'RESULT_CONTRACT_INVALID'] of [
    ...
    await assert.rejects(runtime.execute(scenario.request, divergentDraft, {
      trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
      targetAdapter,
    }), { code: ['resultId', 'resultArtifactId', 'completedAt'].includes(field) ? 'OPERATION_CONFLICT' : code });
  }
...
registerTests && test('canonical incomplete-dependency denial precedes contained target inspection', async () => {
  ...
  let targetReads = 0;
  const targetAdapter = new Proxy({}, {
    get() { targetReads += 1; throw new Error('authority denial touched target'); },
  });
  const runtime = createOperatingGovernedExecutionRuntimeV2({
    initialState: scenario.initial,
    checkpointStore: createGovernedExecutionCheckpointStore(scenario.initial),
...
registerTests && test('direct intent reduction rejects every fully rehashed caller-defined execute authority field', async () => {
  ...
  const intentOffset = completed.events.findIndex(({ type }) => type === 'operation.intent-recorded');
  const preIntentState = reduceOperatingRuntimeEventsV2(
    completed.events.slice(0, intentOffset),
    { initialState: scenario.initial },
  );
  const intent = completed.events[intentOffset];
  const vectors = [
    ['cross target', (operation) => { operation.target = { kind: 'project-record', id: 'other-target', revision: 'rev-other01' }; }],
    ['capability', (operation) => { operation.capability = { id: 'synthetic-contained-write', version: '1.0.0' }; }],
    ['effect', (operation) => { operation.effectClass = 'machine-local-write'; }],
    ['preconditions', (operation) => { operation.preconditionArtifactIds = []; }],
    ['undeclared input', (operation) => { operation.inputArtifactIds.push('art_undeclared00000024'); }],
    ['verification', (operation) => { operation.verificationPlanId = 'vfy_forged00000024'; }],
    ['executor', (operation) => { operation.executor = { executorId: 'open-reference-containment-executor', executorVersion: '1.0.0' }; }],
    ['connector', (operation) => { operation.connector = { id: 'synthetic-no-network-connector', version: '1.0.0' }; }],
    ['rollback class', (operation) => { operation.rollbackClass = 'manual'; }],
    ['rollback plan', (operation) => { operation.rollbackPlanId = 'rbp_forged00000024'; }],
    ['parent operation', (operation) => { operation.parentOperationId = 'op_parent00000024'; }],
    ['dispatch timestamp', (operation, event) => {
...
registerTests && test('Action revision hashing is immutable-content exact and lifecycle-stable in direct and checkpoint paths', async () => {
...
registerTests && test('deterministic policy output rejects a fully rehashed reason rewrite in direct and checkpoint paths', async () => {
...
registerTests && test('execute authority rejects a stale evaluation when a newer deterministic revision is effective', async () => {
  ...
  const preIntent = reduceOperatingRuntimeEventsV2(
    completed.events.slice(0, intentOffset),
    { initialState: forgedInitial },
  );
  assert.throws(
    () => reduceOperatingRuntimeEventsV2([completed.events[intentOffset]], { initialState: preIntent }),
    { code: 'OPERATION_CONFLICT' },
  );
});

registerTests && test('grant expiry cannot exceed availability, requirement, or approval ceilings in direct or checkpoint paths', async () => {
  ...
  const intentOffset = completed.events.findIndex(({ type }) => type === 'operation.intent-recorded');
  const availabilityOffset = completed.events.findIndex(({ type }) => (
    type === 'capability.availability-recorded'
  ));
  ...
  for (const [label, ceiling] of vectors) {
    ...
    checkpointEvents[grantOffset].payload = {
      ...clone(checkpointGrant), consumedAt: null,
    };
    checkpointEvents[grantOffset].payload.grantHash = sha256Jcs(
      Object.fromEntries(Object.entries(checkpointEvents[grantOffset].payload).filter(([key]) => key !== 'grantHash')),
    );
...
registerTests && test('execute validation evaluates complete threshold approval history in direct, checkpoint, and replay paths', async () => {
  ...
  assert.deepEqual(completed.operation.approvalIds, authority.approved.map(({ approvalId }) => approvalId).sort());
  assert.equal(completed.state.approvalRecords.length, 2);
  assert.ok(completed.state.approvalRecords.every(({ consumedByOperationId }) => (
    consumedByOperationId === scenario.draft.operationId
  )));
  ...
  const validDispatch = reduceOperatingRuntimeEventsV2(
    [completed.events[intentOffset]],
  ...
  assert.equal(authority.rejected.recordHash, sha256Jcs(
    Object.fromEntries(Object.entries(authority.rejected).filter(([key]) => key !== 'recordHash')),
  ));
  assert.throws(
    () => reduceOperatingRuntimeEventsV2([], { initialState: rejectedCheckpoint }),
    { code: 'OPERATION_CONFLICT' },
  );
  ...
  const validReplay = await replayRuntime.execute(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { replayHostCalls += 1; throw new Error('replay-host'); } }),
    targetAdapter: new Proxy({}, { get() { replayTargetCalls += 1; throw new Error('replay-target'); } }),
  });
...
registerTests && test('exact capability availability rejects fully rehashed authority rewrites in checkpoint and direct intent paths', async () => {
  ...
  const vectors = [
    ...
    ['provider selection', (availability) => { availability.provider.providerVersion = '1.0.1'; }],
    ['capability', (availability) => { availability.capability.id = 'synthetic-contained-write'; }],
    ['target', (availability) => { availability.target.id = 'other-target'; }],
    ['effect ceiling', (availability) => { availability.effectCeiling = 'machine-local-write'; }],
    ['check time', (availability) => { availability.checkedAt = '2026-08-10T11:59:59Z'; }],
    ['expiry bound', (availability) => { availability.expiresAt = '2100-01-01T00:00:00Z'; }],
  ];
  for (const [label, mutate] of vectors) {
  ...
  const forgedDirect = reduceOperatingRuntimeEventsV2(prefixEvents, { initialState: scenario.initial });
...
registerTests && test('assignment creation binds the exact non-genesis pre-transaction Event head', async () => {
  ...
  scenario.initial = reduceOperatingRuntimeEventsV2([seedEvent], { initialState: scenario.initial });
  const targetAdapter = createDisposableLocalProjectTargetV2({
    target: scenario.action.targetBinding,
    initialValue: scenario.initialValue,
  });
...
registerTests && test('direct reducer rejects every self-consistent target, baseline, effect, and timestamp forgery', async () => {
  ...
  const vectors = [
    ['target before', (value) => { value.targetBeforeHash = `sha256:${'1'.repeat(64)}`; value.baselineHash = value.targetBeforeHash; }],
    ['target after', (value) => { value.targetAfterHash = `sha256:${'2'.repeat(64)}`; }],
    ['baseline artifact', (value) => { value.baselineArtifactId = 'art_forged00000009'; }],
    ['changed flag', (value) => { value.effectSummary.changed = !value.effectSummary.changed; }],
    ['affected target', (value) => { value.effectSummary.affectedTargetIds = ['other-target']; }],
    ['effect summary', (value) => { value.effectSummary.summary = 'Unproven effect text.'; }],
    ['causal completion', (value) => { value.completedAt = '2026-08-10T11:59:59Z'; }],
    ['uncertain postcondition', (value) => { value.status = 'uncertain'; value.effectSummary.changed = false; value.effectSummary.summary = 'Contained dispatch outcome is uncertain; reconciliation is required before retry.'; }],
  ];
  for (const [label, mutate] of vectors) {
    const forged = clone(completed.result);
    mutate(forged);
    assert.throws(() => reduceDirectResult(dispatchState, scenario, withResultHash(forged), receipt), {
      code: 'RESULT_CONTRACT_INVALID',
    }, label);
  }
});

registerTests && test('all terminal statuses enforce closed receipt, postcondition, effect, and affected-target semantics', async () => {
  ...
  for (const status of ['succeeded', 'partial', 'failed', 'blocked', 'uncertain']) {
    ...
    assert.doesNotThrow(() => reduceDirectResult(dispatchState, scenario, valid, receipt), status);
    ...
    assert.throws(() => reduceDirectResult(
      dispatchState,
      scenario,
      withResultHash(forged),
      forgedReceipt,
    ), { code: 'RESULT_CONTRACT_INVALID' }, status);
  }
});

registerTests && test('checkpoint loading rejects cloned Action ownership and missing or mismatched intent proof', async () => {
  ...
  const intentEntry = mismatchedIntent.eventReplayIndex.find(({ eventId }) => (
    eventId === mismatchedIntent.governedOperations[0].intentEventId
  ));
...
registerTests && test('terminal checkpoint rejects rehashed execute authority and runtime-owned projection matrices', async () => {
  ...
  const authorityVectors = [
    ['cross target', (operation) => { operation.target = { kind: 'project-record', id: 'other-target', revision: 'rev-other01' }; }],
    ['undeclared input', (operation) => { operation.inputArtifactIds.push('art_undeclared00000025'); }],
    ['executor', (operation) => { operation.executor = { executorId: 'open-reference-containment-executor', executorVersion: '1.0.0' }; }],
    ['connector', (operation) => { operation.connector = { id: 'synthetic-no-network-connector', version: '1.0.0' }; }],
    ['rollback class', (operation) => { operation.rollbackClass = 'manual'; }],
    ['rollback plan', (operation) => { operation.rollbackPlanId = 'rbp_forged00000025'; }],
    ['parent operation', (operation) => { operation.parentOperationId = 'op_parent00000025'; }],
  ];
  for (const [label, mutate] of authorityVectors) {
    ...
    const historical = {
      ...
      updatedAt: operation.createdAt, operationHash: replay.operationHash,
    };
    mutate(historical);
  ...
  const projectionVectors = [
    ['assignment createdAt', (state) => { state.assignments[0].createdAt = '2026-08-10T11:59:59Z'; }],
    ['assignment availableAt', (state) => { state.assignments[0].availableAt = '2026-08-10T12:00:01Z'; }],
    ['assignment completedAt', (state) => { state.assignments[0].completedAt = '2026-08-10T12:01:01Z'; }],
    ['assignment attempts', (state) => { state.assignments[0].attemptPolicy.maxAttempts = 2; }],
    ['assignment timeout', (state) => { state.assignments[0].attemptPolicy.timeoutMs = 30001; }],
    ['accepted submission issuedAt', (state) => { state.submissions[0].issuedAt = '2026-08-10T12:00:01Z'; }],
    ['accepted submission resolvedAt', (state) => { state.submissions[0].resolvedAt = '2026-08-10T12:01:01Z'; }],
    ['unused reservation resolved', (state) => {
      const unused = state.submissions.find(({ state: submissionState }) => submissionState === 'issued');
      unused.resolvedAt = '2026-08-10T12:01:00Z';
    }],
    ['artifact createdAt', (state) => { state.artifacts[0].createdAt = '2026-08-10T12:01:01Z'; }],
    ['artifact sensitivity', (state) => { state.artifacts[0].sensitivity = 'confidential'; }],
    ['artifact retention', (state) => { state.artifacts[0].retentionClass = 'temporary'; }],
  ];
...
registerTests && test('checkpoint terminal loading rejects a fully rehashed failed-result poststate forgery', async () => {
  ...
  const terminalEvents = completed.events.filter(({ type }) => (
    ['assignment.submitted', 'artifact.created', 'assignment.validated', 'execution.result-recorded'].includes(type)
  )).map(clone);
...
registerTests && test('terminal checkpoint and direct reduction reject a tampered historical intent operation hash', async () => {
...
registerTests && test('terminal checkpoint rejects a rehashed operation timestamp divergent from its result and Event chain', async () => {
...
registerTests && test('completion after grant expiry is valid only because the one-use grant was consumed at durable dispatch', async () => {
  ...
  assert.equal(completed.state.capabilityGrants[0].consumedAt, scenario.draft.preparedAt);
  assert.ok(Date.parse(completed.state.capabilityGrants[0].consumedAt)
