...
import { createOperatingActionPolicyV2, evaluateOperatingActionPolicyV2 } from '../../lib/operate/policy-v2.mjs';
import {
  createOperatingApprovalRecordV2,
  createOperatingApprovalRequirementV2,
} from '../../lib/operate/approvals-v2.mjs';

const fixtureUrl = (name) => new URL(`../../conformance/fixtures/operating-runtime-v2/${name}`, import.meta.url);
...
const governed = JSON.parse(readFileSync(fixtureUrl('governed-execution-contracts-valid.json'), 'utf8'));
const clone = (value) => structuredClone(value);
const authTest = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url) ? test : () => {};

function thresholdContext(approvalCount) {
  return executeContext({ mode: 'threshold', partyCount: 3, approvalCount });
}

function canonicalAuthority(action, { mode = 'named-single-party', partyCount = 1, approvalCount = 1 } = {}) {
  ...
  const corePolicy = createOperatingActionPolicyV2({
    policyId: 'core-governed-action-policy', policyVersion: '1.0.0', domainId: action.domainId,
    actionKind: action.actionKind, capability: action.requestedCapability,
    effectClasses: [action.effectClass], targetKinds: [action.targetBinding.kind],
    decisionMode: 'automatic', approvalRequirementIds: [], rollbackRequired: true,
    verificationRequired: true, tier: 'core',
    provenance: { providerId: 'core-policy-provider', providerVersion: '1.0.0', sourceHash: `sha256:${'c'.repeat(64)}` },
  });
  ...
  const policyEvaluation = evaluateOperatingActionPolicyV2({
    action, configuredPolicies: [corePolicy, actionPolicy], evaluatedAt: '2026-08-10T08:00:00Z',
  });
  ...
  const requirement = approvalBearing ? createOperatingApprovalRequirementV2({
    policyRequirementId: templateId, evaluation: policyEvaluation, action, parties,
    threshold: mode === 'threshold' ? 2 : undefined,
    expiresAt: governed['operating-approval-requirement'].expiresAt,
    consumable: true,
  }) : null;
  const approvals = parties.slice(0, approvalBearing ? approvalCount : 0).map((party, index) => createOperatingApprovalRecordV2({
    approvalId: `aprv_0000000${index + 1}`, requirement, evaluation: policyEvaluation, action,
    partyId: party.partyId,
    actor: { kind: party.actorKind, actorId: party.actorId, capability: clone(party.requiredCapability) },
    decision: 'approved', issuedAt: '2026-08-10T08:01:00Z', expiresAt: requirement.expiresAt,
  }));
...
function rollbackContext(options = {}) {
  ...
  context.operation = {
    ...clone(governed['operating-governed-operation']),
    operationId: 'op_00000002', operationKind: 'rollback', assignmentId: 'asg_00000002',
    requestFingerprint: governed['operating-rollback-result'].requestFingerprint,
    grantId: 'cgr_00000002', verificationPlanId: context.rollbackPlan.verificationPlanId,
    rollbackPlanId: context.rollbackPlan.rollbackPlanId, parentOperationId: context.rollbackPlan.operationId,
  ...
  context.grant = {
    ...clone(governed['operating-capability-grant']),
    grantId: 'cgr_00000002', operationId: 'op_00000002', assignmentId: 'asg_00000002',
    evaluationId: context.policyEvaluation.evaluationId,
    approvalIds: context.approvals.map(({ approvalId }) => approvalId),
    scopeHash: context.approvalRequirements[0]?.scopeHash ?? governed['operating-capability-grant'].scopeHash,
...
function setPath(value, path, next) {
  ...
  const parent = parts.reduce((current, part) => current[Number.isInteger(Number(part)) ? Number(part) : part], value);
...
authTest('canonical authority decision authorizes an exact unchanged execute chain and is immutable', () => {
  const context = executeContext();
  assert.doesNotThrow(() => assertProtocolArtifact('operating-action', context.action, { protocolVersion: '2.0.0' }));
  ...
  assert.deepEqual(direct.map(({ tool }) => tool), ['operate.action.execute']);
  assert.deepEqual(runtime.filter(({ tool }) => tool.startsWith('operate.action.')).map(({ tool }) => tool), ['operate.action.execute']);
...
authTest('Action approval uses the same exact decision record and intelligence actors acquire no authority', () => {
  ...
  for (const decision of decisions) {
    const selected = { ...context, actionRequest: { ...clone(authorization.approveRequest), decision } };
    assert.equal(assertOperateAuthorityV2('operate.action.approve', selected).allowed, true, decision);
    assert.equal(assertOperateAuthorizedV2('operate.action.approve', selected).allowed, true, decision);
  }
  const advisor = {
    ...context,
    actor: {
      actorId: 'advisor-0001', kind: 'agent',
      capabilities: [{ id: 'action-approve', version: '1.0.0' }],
    },
  };
  assert.equal(evaluateOperateAuthorityV2('operate.action.approve', advisor).error.code, 'CAPABILITY_DENIED');
  assert.equal(deriveOperateAllowedActionsV2(advisor).some(({ tool }) => tool === 'operate.action.approve'), false);
});

authTest('a schema-valid wildcard forgery cannot let an outsider approve a named Action party', () => {
  const context = approveContext();
  context.approvalRequirements[0].parties[0].actorId = null;
  rehashApprovalRequirement(context.approvalRequirements[0]);
  context.actor = {
    actorId: 'outsider-0001', kind: 'human',
    capabilities: [{ id: 'action-approve', version: '1.0.0' }],
  };
  assert.doesNotThrow(() => assertProtocolArtifact(
    'operating-approval-requirement', context.approvalRequirements[0], { protocolVersion: '2.0.0' },
  ));
...
authTest('complete hostile matrix fails before any dispatch seam with registered safe errors', () => {
  for (const vector of invalid.vectors) {
    const context = executeContext();
    if (vector.field.startsWith('operationHistory.')) {
      context.operationHistory = [{
        ...clone(context.operation), state: 'succeeded', resultId: 'xres_00000001',
      }];
    }
    setPath(context, vector.field.startsWith('request.')
    ...
    let dispatchCalls = 0;
    context.providerDispatch = () => { dispatchCalls += 1; };
    context.modelDispatch = () => { dispatchCalls += 1; };
    context.connectorDispatch = () => { dispatchCalls += 1; };
    context.targetAccess = () => { dispatchCalls += 1; };
    context.effect = () => { dispatchCalls += 1; };
    const before = sha256Jcs({ ...context, providerDispatch: null, modelDispatch: null, connectorDispatch: null, targetAccess: null, effect: null });
    ...
    assert.equal(Object.keys(decision.error.context).every((key) => [
      'operation', 'cycleId', 'assignmentId', 'submissionId', 'submissionState',
      'reviewId', 'state', 'maxBytes',
    ].includes(key)), true, `${vector.name}: safe context only`);
    assert.equal(dispatchCalls, 0, `${vector.name}: no authority-side effects`);
    assert.equal(deriveOperateAllowedActionsV2(context).some(({ tool }) => tool === 'operate.action.execute'), false, vector.name);
    assert.equal(sha256Jcs({ ...context, providerDispatch: null, modelDispatch: null, connectorDispatch: null, targetAccess: null, effect: null }), before,
      `${vector.name}: denial leaves state unchanged`);
  }
});

authTest('exact terminal replay returns an immutable replay decision before live grant, provider, target, or executor checks', () => {
  const context = executeContext();
  context.operationHistory = [{
    ...clone(context.operation), state: 'succeeded', resultId: 'xres_00000001',
  }];
  ...
  delete context.executor;
  let effectCalls = 0;
  context.providerDispatch = () => { effectCalls += 1; };
  context.modelDispatch = () => { effectCalls += 1; };
  context.connectorDispatch = () => { effectCalls += 1; };
  context.targetAccess = () => { effectCalls += 1; };
  ...
  assert.equal(effectCalls, 0, 'guard, assert, and allowed-action derivation never dispatch replay effects');

  const missingGrant = { ...context, grant: undefined };
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', missingGrant).error.code, 'CAPABILITY_GRANT_INVALID');
  const forgedIssuer = {
    ...context,
    grant: { ...context.grant, issuer: { ...context.grant.issuer, id: 'forged-runtime' } },
  };
  assert.equal(evaluateOperateGuardV2('operate.action.execute', forgedIssuer).error.code, 'CAPABILITY_GRANT_INVALID');
  ...
  for (const [name, grantPatch] of [
    ['grant expired before operation start', {
      expiresAt: '2026-08-10T08:03:00Z', consumedAt: null, revokedAt: null,
    }],
    ['grant consumed before operation start', {
      expiresAt: '2026-08-10T08:07:00Z', consumedAt: '2026-08-10T08:03:00Z', revokedAt: null,
    }],
    ['grant revoked before operation start', {
      expiresAt: '2026-08-10T08:07:00Z', consumedAt: null, revokedAt: '2026-08-10T08:03:00Z',
    }],
  ]) {
  ...
  for (const [name, grantPatch] of [
    ...
    assert.equal(evaluateOperateAuthorityV2('operate.action.execute', {
      ...context, grant: { ...context.grant, ...grantPatch },
    }).replayed, true, name);
  }
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', {
    ...context,
    grant: { ...context.grant, consumedAt: null, revokedAt: context.operation.createdAt },
  }).error.code, 'CAPABILITY_GRANT_INVALID', 'revocation at operation start fails closed');
...
authTest('rollback authorization reuses the same exact guard and requires durable original-result history', () => {
  ...
  assert.deepEqual(
    deriveOperateAllowedActionsV2(context).filter(({ tool }) => tool.startsWith('operate.action.')).map(({ tool }) => tool),
    ['operate.action.rollback'],
  );
  const missingHistory = { ...context, operationHistory: [] };
  assert.equal(evaluateOperateAuthorityV2('operate.action.rollback', missingHistory).error.code, 'ROLLBACK_NOT_ELIGIBLE');
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.rollback', stalePlan).error.code, 'ROLLBACK_NOT_ELIGIBLE');
});

authTest('action-subject Review cannot substitute another Action version or use evidence text as authority', () => {
  const review = {
    kind: 'operating-review', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    reviewId: 'rev_00000001', cycleId: 'cyc_00000001',
    subject: { type: 'action', ...clone(authorization.executeRequest.action) },
    ownerActorId: 'owner-0001', state: 'pending', disposition: null, workDispositions: [],
    createdAt: authorization.now, updatedAt: authorization.now,
  };
  const cycle = {
    kind: 'operating-cycle', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'software', domainVersion: '1.0.0',
    state: 'awaiting_review', inputBindingId: 'inb_00000001', contractVersions: {}, trigger: { kind: 'manual' },
    focus: [], health: 'normal', activeReviewId: review.reviewId,
    createdAt: authorization.now, updatedAt: authorization.now,
  };
  const context = {
    actor: { actorId: 'owner-0001', kind: 'human', runtime: 'portable' },
    capabilities: [clone(authorization.toolCapabilities.reviewSubmit)],
    action: authorization.action, review, cycle,
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.review.submit', stale).error.code, 'ACTION_REVISION_MISMATCH');
  const agent = { ...context, actor: { actorId: 'owner-0001', kind: 'agent' } };
  assert.equal(evaluateOperateAuthorityV2('operate.review.submit', agent).error.code, 'REVIEW_NOT_AUTHORIZED');
});

authTest('versioned tool and actor capabilities plus runtime grant issuer are exact authority identities', () => {
  const mutations = [
    ['unversioned tool operation', (context) => { context.capabilities = ['operate.action.execute']; }],
    ['wildcard tool operation', (context) => { context.capabilities = ['*']; }],
    ['wrong tool version', (context) => { context.capabilities[0].version = '1.0.0'; }],
    ['wildcard beside exact tool identity', (context) => { context.capabilities.push('*'); }],
    ['unversioned actor capability', (context) => { context.actor.capabilities = ['bounded-project-write']; }],
    ['wildcard actor capability', (context) => { context.actor.capabilities = ['*']; }],
    ['wrong actor capability version', (context) => { context.actor.capabilities[0].version = '9.9.9'; }],
    ['wildcard beside exact actor identity', (context) => { context.actor.capabilities.push('*'); }],
    ['grant issuer substitution', (context) => { context.grant.issuer.id = 'another-runtime'; }],
  ];
  for (const [name, mutate] of mutations) {
    const context = executeContext();
    mutate(context);
    assert.equal(evaluateOperateAuthorityV2('operate.action.execute', context).allowed, false, name);
    assert.equal(deriveOperateAllowedActionsV2(context).some(({ tool }) => tool === 'operate.action.execute'), false, name);
  }
});

authTest('canonical policy identity, applicability, precedence, mode, provider, and execution bindings cannot be substituted', () => {
  const mutations = [
    ['policy hash', (context) => { context.policyEvaluation.policy.policyHash = `sha256:${'0'.repeat(64)}`; }],
    ['applied policy', (context) => { context.policyEvaluation.appliedPolicyRefs[0].policyId = 'alien-policy'; }],
    ['policy tier', (context) => {
      context.policyEvaluation.policyTier = 'project'; context.policyEvaluation.precedence = 200;
    }],
    ['precedence', (context) => { context.policyEvaluation.precedence = 200; }],
    ['automatic downgrade', (context) => {
      context.policyEvaluation.outcome = 'automatic'; context.policyEvaluation.approvalRequirementIds = [];
    }],
    ['provider', (context) => { context.policyEvaluation.evaluatedBy.providerId = 'alien-provider'; }],
    ['domain', (context) => { context.actionPolicy.domainId = 'business'; }],
    ['action kind', (context) => { context.actionPolicy.actionKind.id = 'another-action'; }],
    ['capability', (context) => { context.actionPolicy.capability.id = 'another-capability'; }],
    ['target kind', (context) => { context.actionPolicy.targetKinds = ['another-target']; }],
    ['effect', (context) => { context.actionPolicy.effectClasses = ['read-only']; }],
    ['rollback', (context) => { context.actionPolicy.rollbackRequired = false; }],
    ['verification', (context) => { context.actionPolicy.verificationRequired = false; }],
    ['approval requirement ids', (context) => { context.actionPolicy.approvalRequirementIds = ['aprq_00000009']; }],
  ];
  for (const [name, mutate] of mutations) {
    const context = executeContext();
    mutate(context);
...
authTest('live Action guards require complete configured policy truth and cannot omit core or an applicable project tier', () => {
  const missingRegistry = executeContext();
  delete missingRegistry.actionPolicies;
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', missingRegistry).error.code, 'POLICY_EVALUATION_REJECTED');

  const missingCore = executeContext();
  missingCore.actionPolicies = [clone(missingCore.actionPolicy)];
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', missingCore).error.code, 'POLICY_EVALUATION_REJECTED');
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', omittedProject).error.code, 'POLICY_EVALUATION_REJECTED');
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', prohibitedWithoutCore).error.code, 'POLICY_EVALUATION_REJECTED');
});

authTest('approval requirements, parties, and records are exact unique sets', () => {
  const mutations = [
    ['duplicate requirement', (context) => { context.approvalRequirements.push(clone(context.approvalRequirements[0])); }],
    ['duplicate approval id', (context) => { context.approvals.push(clone(context.approvals[0])); }],
    ['extra party record', (context) => {
      const extra = clone(context.approvals[0]);
      extra.approvalId = 'aprv_00000009';
      context.approvals.push(extra);
    }],
    ['undeclared party', (context) => { context.approvals[0].partyId = 'alien-party'; }],
    ['duplicate party declaration', (context) => {
    ...
    ['extra named actor', (context) => { context.approvalRequirements[0].namedActorIds.push('owner-0002'); }],
    ['extra actor kind', (context) => { context.approvalRequirements[0].requiredActorKinds.push('engine'); }],
    ['party capability version', (context) => {
...
authTest('named approval modes require exact non-null named parties and consistent all-party cardinality', () => {
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', anonymousAlternative).allowed, false);
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.approve', anonymousApprover).allowed, false);
  ...
  requirement.parties.push({
    ...clone(requirement.parties[0]), partyId: 'owner-2-party', actorId: 'owner-0002',
  }, {
    ...clone(requirement.parties[0]), partyId: 'anonymous-party', actorId: null,
  });
  requirement.namedActorIds.push('owner-0002');
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', inconsistentMulti).allowed, false);
});
...
authTest('automatic execution and rollback reject every stale requirement or approval input', () => {
  ...
  for (const createContext of [
    ...
    const operation = context.operation.operationKind === 'rollback'
      ? 'operate.action.rollback'
...
authTest('authority timestamps form one causal chain through the explicit decision time', () => {
  const mutations = [
    ['future evaluation', (context) => { context.policyEvaluation.evaluatedAt = '2026-08-10T08:04:00Z'; }],
    ['approval before evaluation', (context) => { context.approvals[0].issuedAt = '2026-08-10T07:59:00Z'; }],
    ['future approval', (context) => { context.approvals[0].issuedAt = '2026-08-10T08:04:00Z'; }],
    ['grant before approval', (context) => { context.grant.issuedAt = '2026-08-10T08:00:30Z'; }],
    ['future grant', (context) => { context.grant.issuedAt = '2026-08-10T08:04:00Z'; }],
    ['operation before grant', (context) => { context.operation.createdAt = '2026-08-10T08:01:30Z'; }],
    ['future operation', (context) => {
      context.operation.createdAt = '2026-08-10T08:04:00Z';
      context.operation.updatedAt = '2026-08-10T08:04:00Z';
    }],
    ['future availability check', (context) => { context.capabilityAvailability.checkedAt = '2026-08-10T08:04:00Z'; }],
    ['expired availability', (context) => { context.capabilityAvailability.expiresAt = context.now; }],
    ['future executor check', (context) => { context.executor.health.checkedAt = '2026-08-10T08:04:00Z'; }],
    ['expired executor health', (context) => { context.executor.health.expiresAt = context.now; }],
  ];
  for (const [name, mutate] of mutations) {
    const context = executeContext();
    mutate(context);
...
authTest('the governed operation envelope denies connectors, undeclared artifacts, rollback mismatch, and above-ceiling effects', () => {
  const mutations = [
    ['unregistered connector', (context) => {
      context.operation.connector = { id: 'synthetic-connector', version: '1.0.0' };
    }],
    ['extra input', (context) => { context.operation.inputArtifactIds.push('art_00000009'); }],
    ['extra precondition', (context) => { context.operation.preconditionArtifactIds.push('art_00000009'); }],
    ['missing current precondition', (context) => { context.currentPreconditionArtifactIds = []; }],
    ['rollback not applicable', (context) => { context.operation.rollbackClass = 'not-applicable'; }],
  ];
  for (const [name, mutate] of mutations) {
    const context = executeContext();
    mutate(context);
  ...
  assert.equal(deriveOperateAllowedActionsV2(aboveCeiling).some(({ tool }) => tool === 'operate.action.execute'), false);
});

authTest('Cycle and Action Review subjects bind the active Cycle identity, scope, and domain version', () => {
  const baseReview = {
    kind: 'operating-review', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    reviewId: 'rev_00000001', cycleId: 'cyc_00000001',
  ...
  const baseCycle = {
    ...
    cycleId: 'cyc_00000001', scopeId: 'scope-acme', domainId: 'software', domainVersion: '1.0.0',
    state: 'awaiting_review', inputBindingId: 'inb_00000001', contractVersions: {}, trigger: { kind: 'manual' },
    focus: [], health: 'normal', activeReviewId: 'rev_00000001',
    createdAt: authorization.now, updatedAt: authorization.now,
  };
  const base = {
    actor: { actorId: 'owner-0001', kind: 'human', runtime: 'portable' },
    capabilities: [clone(authorization.toolCapabilities.reviewSubmit)],
    cycle: baseCycle, review: baseReview,
    scope: { scopeId: baseCycle.scopeId, domainId: baseCycle.domainId, domainVersion: baseCycle.domainVersion },
    reviewRequest: {
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.review.submit', wrongCycleSubject).allowed, false);

  const wrongActionScope = clone(base);
  wrongActionScope.review.subject = { type: 'action', ...clone(authorization.executeRequest.action) };
  wrongActionScope.action = clone(authorization.action);
  wrongActionScope.action.domainVersion = '2.0.0';
  assert.equal(evaluateOperateAuthorityV2('operate.review.submit', wrongActionScope).allowed, false);
});
...
authTest('only one complete exact terminal governed operation is replayable', () => {
  ...
  terminal.operationHistory = [{
    ...clone(terminal.operation), state: 'succeeded', resultId: 'xres_00000001',
  }];
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', resultlessTerminal).allowed, false);
