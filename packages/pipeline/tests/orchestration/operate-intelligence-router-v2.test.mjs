...
function deltaCheckpoint(seed = '001') {
  ...
  const derived = deriveOperatingRuntimeDeltaV2(request, {
...
function acceptedProofRecords(target, seed = 'router_001') {
  const artifact = {
    ...
    artifactId: `art_${seed}`, artifactType: 'advisor-result', assignmentId: target.assignmentId,
    cycleId: target.cycleId, scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    schemaId: target.outputContract.schemaId, artifactSchemaVersion: target.outputContract.schemaVersion,
    mediaType: target.outputContract.mediaType, encoding: target.outputContract.encoding,
    rawHash: `sha256:${'a'.repeat(64)}`, canonicalHash: null, sizeBytes: 2,
    ...
    inputArtifactIds: [...target.inputArtifactIds], createdAt: PLAN_TIME,
  };
  const submission = {
    kind: 'operating-submission', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    submissionId: `sub_${seed}`, assignmentId: target.assignmentId, cycleId: target.cycleId,
    state: 'accepted', rawHash: artifact.rawHash, canonicalHash: artifact.canonicalHash,
    sizeBytes: artifact.sizeBytes, artifactId: artifact.artifactId,
    ...
    issuedAt: PLAN_TIME, resolvedAt: PLAN_TIME,
  };
  const replay = {
    submissionId: submission.submissionId, assignmentId: submission.assignmentId,
    rawHash: submission.rawHash, canonicalHash: submission.canonicalHash, sizeBytes: submission.sizeBytes,
    artifactId: submission.artifactId, acceptanceEventIds: [...submission.acceptanceEventIds],
...
test('intelligence plan fixtures and semantic minimum-board validation fail closed', () => {
  ...
  assert.deepEqual(validateProtocolArtifact('operating-intelligence-plan', valid, { protocolVersion: '2.0.0' }), []);
  assert.ok(validateProtocolArtifact('operating-intelligence-plan', invalid, { protocolVersion: '2.0.0' }).length > 0);
  assert.doesNotThrow(() => assertOperatingIntelligencePlanV2(valid));
  const overlap = clone(valid);
  overlap.omittedRoles.push({ roleId: 'advisor', roleKind: 'advisor', roleVersion: '2.0.0', reason: 'not-selected: Hostile duplicate.' });
  assert.throws(() => assertOperatingIntelligencePlanV2(overlap), { code: 'RESULT_CONTRACT_INVALID' });
});
...
test('pure routing produces one stable minimum useful board with explicit inputs, outputs, reasons, and dependencies', () => {
  ...
  assert.deepEqual(first.plan.selectedRoles.map(({ roleId }) => roleId), BUSINESS_BOARD_ROLE_IDS);
  assert.deepEqual(
    first.plan.selectedRoles.find(({ roleId }) => roleId === 'independent-challenge').dependsOnRoleIds,
  ...
  assert.equal(first.assignments.every((assignment) => (
    assignment.inputArtifactIds.includes(assignment.intelligenceContext.inputBundle.bundleArtifactId)
    && assignment.inputArtifactIds.every((artifactId) => (
      artifactId === assignment.intelligenceContext.inputBundle.bundleArtifactId
      || assignment.intelligenceContext.inputBundle.issuedEvidence.some(({ evidenceArtifactId }) => evidenceArtifactId === artifactId)
    ))
  )), true, 'each role receives only its role-scoped bundle and authorized materialized Evidence');
  assert.equal(first.assignments.every((assignment) => (
    sha256Jcs(assignment.intelligenceContext.sourceArtifactIds) === sha256Jcs(input.snapshot.sourceArtifactIds)
  )), true, 'snapshot sources remain provenance without becoming readable inputs');
  ...
  assert.equal(first.assignments.every(({ assignmentKind, outputContract }) => (
  ...
  assert.equal(first.assignments.every(({ roleVersion }) => roleVersion === '2.0.0'), true);
  assert.equal(Object.isFrozen(first), true);
  assert.equal(Object.isFrozen(first.plan.selectedRoles[0]), true);
  assert.doesNotMatch(first.plan.selectedRoles.map(({ reason }) => reason).join(' '), /\b(?:business|software)\b/iu);
});
...
test('a crafted required-Challenger plan cannot remove the Chair-to-Challenger edge', () => {
  ...
  craftedPlan.selectedRoles.find(({ roleId }) => roleId === 'chair').dependsOnRoleIds = ['strategy-finance'];
  ...
  assert.throws(() => assertOperatingIntelligencePlanV2(craftedPlan), { code: 'RESULT_CONTRACT_INVALID' });
  assert.throws(() => validateOperatingIntelligenceAssignmentGraphV2(craftedPlan, craftedAssignments), {
    code: 'STATE_TRANSITION_INVALID',
  });
  const sourceSubmission = value.derived.state.submissions.find(({ artifactId }) => (
    artifactId === craftedPlan.sourceArtifactId
  ));
  const craftedEvent = createOperatingRuntimeEventV2({
    eventId: 'evt_crafted_topology_001', timestamp: PLAN_TIME, cycleId: value.request.cycleId,
    type: 'intelligence.plan-recorded', entityId: craftedPlan.planId,
    actor: { kind: 'runtime', id: 'openplanr' },
    causationId: sourceSubmission.acceptanceEventIds.at(-1), correlationId: 'corr_crafted_topology_001',
    payload: craftedPlan,
  }, {
    previousEvent: {
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2([craftedEvent], {
...
test('a no-material-change routine omits Challenger explicitly while retaining advisor and Chair', () => {
  ...
  assert.deepEqual(result.plan.selectedRoles.map(({ roleId }) => roleId), [...EXECUTIVE_ADVISOR_IDS, 'chair']);
  assert.deepEqual(result.plan.omittedRoles.map(({ roleId }) => roleId), ['independent-challenge']);
  ...
  assert.deepEqual(absences.map(({ roleId, roleKind, absenceCode, sourceAssignmentId, sourceEventId }) => ({
    roleId, roleKind, absenceCode, sourceAssignmentId, sourceEventId,
  })), [{
...
test('routing rejects untyped omission reasons before producing work', () => {
  ...
  assert.throws(() => assertOperatingIntelligencePlanV2(tampered), { code: 'RESULT_CONTRACT_INVALID' });
});
...
test('scoped routing selects one executive lens and records typed absence for the others', () => {
  ...
  const result = planOperatingIntelligenceBoardV2(input);
  assert.deepEqual(
    result.plan.selectedRoles.filter(({ roleKind }) => roleKind === 'advisor').map(({ roleId }) => roleId),
    ['technology-risk'],
  );
  assert.deepEqual(
    result.plan.omittedRoles.filter(({ roleKind }) => roleKind === 'advisor').map(({ roleId }) => roleId).sort(),
    EXECUTIVE_ADVISOR_IDS.filter((roleId) => roleId !== 'technology-risk').sort(),
  );
  assert.ok(result.plan.omittedRoles.every(({ roleKind, reason }) => (
    roleKind !== 'advisor' || reason.startsWith('not-selected:')
  )));
...
test('routing rejects cross-scope inputs, authority-bearing domain requests, and undeclared input fields before producing work', () => {
  ...
  assert.throws(() => planOperatingIntelligenceBoardV2({
    ...input,
  ...
  assert.throws(() => planOperatingIntelligenceBoardV2({
    ...
    domainDescriptor: {
      ...
      requestedCapabilities: [{ id: 'repository-read', version: '1.0.0', reason: 'Not authority.' }],
    },
  ...
  hostileDescriptor.roles = hostileDescriptor.roles.map((role) => (
    role.roleId === 'strategy-finance'
      ? { ...role, roleId: 'finance-strategy', analysisRubric: { ...role.analysisRubric, requiredQuestions: ['Hostile rubric.'] } }
      : role
  ));
  assert.throws(() => planOperatingIntelligenceBoardV2({
    ...input,
...
test('routing rejects an evidence-empty snapshot before creating any Assignment intent', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'RESULT_CONTRACT_INVALID'
      && error.message.includes('evidence-empty snapshot'),
  );
});
...
test('runtime atomically records the plan and graph, releases only Advisor, and replays without dispatch', () => {
  ...
  const draft = {
    eventId: 'evt_router_plan_runtime_001', timestamp: PLAN_TIME, correlationId: 'corr_router_plan_runtime_001',
  };
  ...
  assert.deepEqual(first.events.map(({ type }) => type), [
    ...Array.from({ length: BUSINESS_BOARD_ROLE_IDS.length }, () => [
      ...
      'assignment.submitted', 'artifact.created', 'assignment.validated',
    ]).flat(),
  ...
  assert.deepEqual(
    first.assignments.filter(({ assignmentKind }) => assignmentKind === 'advisor').map(({ state }) => state),
    Array(EXECUTIVE_ADVISOR_IDS.length).fill('available'),
  );
  assert.equal(first.assignments.find(({ roleId }) => roleId === 'independent-challenge').state, 'pending');
  ...
  assert.throws(() => planOperatingRuntimeIntelligenceBoardV2(runtimeRequest, {
    ...draft,
    correlationId: 'corr_router_plan_divergent_001',
  }, { initialState: first.state, artifactStore: base.store }), { code: 'STATE_TRANSITION_INVALID' });
  assert.ok(base.store.readRaw({ artifactId: base.sourceArtifactId,
    rawHash: base.result.state.artifacts.find(({ artifactId }) => artifactId === base.sourceArtifactId).rawHash }).byteLength > 0);
});
...
test('Chair release accepts only validated Artifact proofs or descriptor-permitted typed terminal absences', () => {
  ...
  const advisor = {
    ...clone(advisorAssignments[0]), state: 'validated', availableAt: PLAN_TIME, completedAt: PLAN_TIME,
    claim: { actorId: 'agent-001', actorKind: 'agent', runtime: 'portable', claimId: 'claim-advisor-router-001' },
  };
  const otherAdvisors = advisorAssignments.slice(1).map((assignment, index) => absence(assignment, `advisor_${index + 2}`));
  const challenger = absence(board.assignments.find(({ roleId }) => roleId === 'independent-challenge'), 'challenger');
  ...
  challenger.inputArtifactIds = [...resolveOperatingAssignmentInputArtifactIdsV2(challenger, challengerProofs)];
  challenger.inputAbsences = [...resolveOperatingAssignmentInputAbsencesV2(challenger, {
  ...
  assert.equal(intent.dependencyProofs.filter(({ outcome }) => outcome === 'failed').length, otherAdvisors.length + 1);
  assert.equal(intent.dependencyProofs.find(({ outcome }) => outcome === 'validated').artifactId,
    proof.artifact.artifactId);
  assert.equal(intent.dependencyProofs.find(({ assignmentId }) => assignmentId === challenger.assignmentId).absence.code,
    'role-unavailable');
  assert.deepEqual(deriveOperatingAssignmentReleaseIntentsV2({
    assignments: [advisor, ...otherAdvisors, { ...challenger, state: 'running', terminalOutcome: null }, chair],
    submissions,
    artifacts: [proof.artifact], submissionReplayIndex: [proof.replay], intelligencePlans: [board.plan],
...
test('all unavailable Advisors terminalize an impossible Challenger and release Chair with typed absences only', () => {
  ...
  const advisors = planned.state.assignments.filter(({ assignmentKind }) => assignmentKind === 'advisor');
  let previousEvent = {
  ...
  const scheduled = scheduleOperatingRuntimeEventsV2(unavailableEvents, { initialState: planned.state });
  const challenger = scheduled.state.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const chair = scheduled.state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  assert.equal(challenger.state, 'failed');
  assert.equal(challenger.terminalOutcome.code, 'role-unavailable');
  assert.equal(scheduled.events.some((event) => (
    event.type === 'assignment.available' && event.entityId === challenger.assignmentId
  )), false, 'the impossible Challenger is never released as executable work');
  ...
  assert.deepEqual(scheduled.state.artifacts, planned.state.artifacts, 'scheduler invents no Advisor or Challenger result row');
});
...
test('intelligence board replay rejects arbitrary lifecycle input expansion without durable dependency proof', () => {
  ...
  assert.throws(() => planOperatingRuntimeIntelligenceBoardV2(clone(runtimeRequest), clone(draft), {
...
test('intelligence board replay rejects tampered role versions and rubrics', () => {
  ...
  const claimTarget = first.state.assignments.find(({ assignmentKind }) => assignmentKind === 'advisor');
  const claimRequest = {
  ...
  const claimDraft = {
    ...
    eventIds: { claimed: 'evt_checkpoint_tamper_claimed', started: 'evt_checkpoint_tamper_started' },
    timestamp: PLAN_TIME,
  ...
  const mandateTarget = tamperedMandate.assignments.find(({ assignmentId }) => assignmentId === claimTarget.assignmentId);
  mandateTarget.mandate = { ...mandateTarget.mandate, scope: 'Hostile schema-valid mandate rewrite.' };
  assert.throws(
    () => claimOperatingAssignmentV2(claimRequest, claimDraft, {
