...
function delta(overrides = {}) {
  return {
    ...
    deltaId: 'dlt_delta_property_001',
    scopeId: 'scope-acme',
    domainId: 'business',
    domainVersion: '1.0.0',
    priorSnapshotId: null,
    currentSnapshotId: 'snp_delta_property_001',
    ...
    exposedRiskIds: [],
    decisionRevisitIds: [],
    sourceArtifactId: 'art_delta_property_001',
...
test('materiality is a total deterministic function over every Delta material field', () => {
  ...
  assert.deepEqual(classifyOperatingDeltaMaterialityV2(delta()), {
    material: false,
  ...
  for (const field of fields) {
    const value = field.endsWith('Changes')
      ...
      : field === 'invalidatedAssumptionIds'
        ? ['asm_delta_property_001']
        : field === 'exposedRiskIds'
          ? ['rsk_delta_property_001']
          : field === 'decisionRevisitIds'
            ? ['dec_delta_property_001']
            : ['evr_delta_property_001'];
    assert.deepEqual(
      classifyOperatingDeltaMaterialityV2(delta({ [field]: value })),
      { material: true, reason: 'material-change' },
      field,
    );
  }
});
