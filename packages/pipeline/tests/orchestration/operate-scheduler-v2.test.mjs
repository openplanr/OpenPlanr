...
function assignment({
  ...
  return {
    ...
    assignmentId, cycleId, assignmentKind: 'context-capture', roleId: `role-${assignmentId}`,
    ...
    capabilityGrantId: `grant-${assignmentId}`, governedOperationId: null,
    ...
    terminalOutcome,
    createdAt: TIME,
    availableAt: ['available', 'claimed', 'running', 'submitted', 'validated', 'rejected', 'abandoned', 'failed'].includes(state) ? TIME : null,
...
function acceptedProofRecords(target, seed = '001') {
  const artifact = {
    ...
    artifactId: `art_scheduler_${seed}`, artifactType: 'advisor-result', assignmentId: target.assignmentId,
    ...
    inputArtifactIds: [...target.inputArtifactIds], createdAt: TIME,
  };
  const submission = {
    kind: 'operating-submission', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    submissionId: `sub_scheduler_${seed}`, assignmentId: target.assignmentId, cycleId: target.cycleId,
...
test('OP-12/INV-WF-005: scheduler validates a graph and releases pending work in stable Assignment-ID order', () => {
  ...
  assert.deepEqual(intents.map(({ assignmentId }) => assignmentId), [
    'asg_00000001', 'asg_00000002', 'asg_00000003',
  ]);
  assert.deepEqual(intents.map(({ dependencyProofs }) => dependencyProofs), [[], [], []]);
...
test('LIVE-EVIDENCE: scheduler admits only the Protocol 2.0 ingestion exception to ordinary verification Outcomes', () => {
  ...
  assert.throws(() => validateOperatingAssignmentGraphV2([{
    ...verification,
    outputContract: { ...verification.outputContract, schemaVersion: '1.0.0' },
  }]));
  assert.throws(() => validateOperatingAssignmentGraphV2([{
    ...verification,
    outputContract: { ...verification.outputContract, schemaId: 'operating-evidence-observation' },
  }]));
});
...
test('INV-WF-005/006: an intelligence plan compiles to one exact ordinary Assignment per selected role', () => {
  ...
  const ids = Object.fromEntries(plan.selectedRoles.map((role) => [
    role.roleId,
  ...
  const assignments = plan.selectedRoles.map((role) => ({
    ...assignment({
      ...
      dependencyPolicy: role.dependsOnRoleIds.length === 0 ? { kind: 'none' } : { kind: 'all-required' },
    }),
  ...
  const advisor = { ...structuredClone(assignments[0]), state: 'validated', availableAt: TIME, completedAt: TIME };
  const proof = acceptedProofRecords(advisor, 'topology');
  assert.throws(() => deriveOperatingAssignmentReleaseIntentsV2({
    assignments: [advisor, assignments[1], craftedAssignments[2]],
    submissions: [proof.submission], artifacts: [proof.artifact], submissionReplayIndex: [proof.replay],
    intelligencePlans: [plan],
  }), { code: 'STATE_TRANSITION_INVALID' }, 'persisted plan validation prevents an early Chair release');
});
...
test('OP-12/13: threshold accepts only declared terminal absences and carries exact Artifact/absence proofs', () => {
  ...
  const abandoned = assignment({
    assignmentId: 'asg_00000002', state: 'abandoned',
    terminalOutcome: {
      outcome: 'abandoned', eventId: 'evt-abandoned-001', code: 'timeout',
      reason: 'The bounded advisor window elapsed.', recoveryDisposition: 'continue-partial',
    },
  });
  const chair = assignment({
    assignmentId: 'asg_00000003', dependsOn: [validated.assignmentId, abandoned.assignmentId],
    dependencyPolicy: { kind: 'threshold', minimumSatisfied: 2, allowedTerminalOutcomes: ['abandoned'] },
  });
  ...
  const [intent] = deriveOperatingAssignmentReleaseIntentsV2({
    assignments: [chair, abandoned, validated], submissions, artifacts, submissionReplayIndex,
  });
  assert.equal(intent.assignmentId, chair.assignmentId);
  assert.deepEqual(intent.dependencyProofs.map(({ outcome }) => outcome), ['validated', 'abandoned']);
  ...
  assert.deepEqual(deriveOperatingAssignmentReleaseIntentsV2({
    assignments: [validated, abandoned, chair], submissions, artifacts, submissionReplayIndex,
  }), []);
});
...
test('OP-12/13: exact durable proof binding rejects foreign identity, contract, claim, hash, and replay mismatches', () => {
  const validated = {
    ...
    intelligenceContext: {
      intelligencePlanId: 'ipl_proof_target_0001', snapshotId: 'snp_proof_target_0001',
  ...
  for (const [label, mutate] of [
    ['foreign Artifact Assignment', (value) => { value.artifact.assignmentId = 'asg_foreign_0001'; }],
    ['foreign Artifact Cycle', (value) => { value.artifact.cycleId = 'cyc_foreign_0001'; }],
    ['raw-hash mismatch', (value) => { value.artifact.rawHash = `sha256:${'b'.repeat(64)}`; }],
    ['canonical-hash mismatch', (value) => { value.artifact.canonicalHash = `sha256:${'c'.repeat(64)}`; }],
    ['byte ceiling mismatch', (value) => {
    ...
    ['missing issued Artifact input', (value) => { value.artifact.inputArtifactIds = ['art_proof_input_0001']; }],
    ['reordered issued Artifact inputs', (value) => { value.artifact.inputArtifactIds.reverse(); }],
    ['foreign Artifact scope', (value) => { value.artifact.scopeId = 'scope-foreign'; }],
    ['foreign Artifact domain', (value) => { value.artifact.domainId = 'software'; }],
    ['foreign Artifact domain version', (value) => { value.artifact.domainVersion = '9.9.9'; }],
    ['missing retained claim', (value) => { value.assignment = { ...validated, claim: null }; }],
    ['producer actor mismatch', (value) => { value.artifact.producer.actorId = 'agent-foreign'; }],
    ['producer runtime mismatch', (value) => { value.artifact.producer.runtime = 'runtime-foreign'; }],
    ['producer role mismatch', (value) => { value.artifact.producer.roleId = 'role-foreign'; }],
    ['replay mismatch', (value) => { value.replay.artifactId = 'art_foreign_replay'; }],
  ]) {
    const hostile = structuredClone(proof);
    mutate(hostile);
    assert.throws(() => assertOperatingValidatedDependencyProofV2({ assignment: validated, ...hostile }), {
      code: 'STATE_TRANSITION_INVALID',
    }, label);
  }
});
...
test('INV-WF-005/006: duplicate, unknown, cross-cycle, self, cyclic, and malformed policies fail before a transaction can return state', () => {
  ...
  const invalidGraphs = [
    ...
    [assignment({ assignmentId: 'asg_00000002', dependsOn: ['asg_missing01'], dependencyPolicy: { kind: 'all-required' } })],
    [base, assignment({ assignmentId: 'asg_00000002', cycleId: 'cyc_other0001', dependsOn: [base.assignmentId], dependencyPolicy: { kind: 'all-required' } })],
    [assignment({ assignmentId: 'asg_00000003', dependsOn: ['asg_00000003'], dependencyPolicy: { kind: 'all-required' } })],
    [
      assignment({ assignmentId: 'asg_00000004', dependsOn: ['asg_00000005'], dependencyPolicy: { kind: 'all-required' } }),
      assignment({ assignmentId: 'asg_00000005', dependsOn: ['asg_00000004'], dependencyPolicy: { kind: 'all-required' } }),
    ],
    [assignment({ assignmentId: 'asg_00000006', dependsOn: ['asg_00000007'], dependencyPolicy: { kind: 'threshold', minimumSatisfied: 2, allowedTerminalOutcomes: [] } }), assignment({ assignmentId: 'asg_00000007' })],
  ];
  ...
  const malformed = assignment({ assignmentId: 'asg_00000008', dependsOn: ['asg_missing02'], dependencyPolicy: { kind: 'all-required' } });
  const source = createOperatingRuntimeEventV2({
    eventId: 'evt-create-invalid', timestamp: TIME, cycleId: malformed.cycleId,
    type: 'assignment.created', entityId: malformed.assignmentId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId: null, correlationId: 'corr-invalid', payload: malformed,
  });
...
test('OP-12/13: a scheduler transaction appends its one deterministic release Event and identical retry is effect-free', () => {
  ...
  const source = createOperatingRuntimeEventV2({
    eventId: 'evt-create-001', timestamp: TIME, cycleId: created.cycleId,
    type: 'assignment.created', entityId: created.assignmentId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId: null, correlationId: 'corr-create', payload: created,
  });
  ...
  const retry = scheduleOperatingRuntimeEventsV2([structuredClone(source)], { initialState: first.state });
  assert.equal(retry.releaseEvents.length, 0);
  assert.equal(sha256Jcs(retry.state), sha256Jcs(first.state));
  assert.throws(() => scheduleOperatingRuntimeEventsV2([first.releaseEvents[0]], { initialState: initial }));
});
...
test('OP-13: accepted submission atomically commits the source truth and every newly-ready dependency release', () => {
  ...
  const dependent = assignment({
    assignmentId: 'asg_00000002', dependsOn: [source.assignmentId], dependencyPolicy: { kind: 'all-required' },
  });
  const submission = {
    kind: 'operating-submission', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
  ...
  const request = {
    assignmentId: source.assignmentId, submissionId: submission.submissionId,
  ...
  const draft = {
    ...
    validatorVersion: '1.0.0', correlationId: 'corr-submit',
  ...
  assert.equal(accepted.state.assignments.find(({ assignmentId }) => assignmentId === dependent.assignmentId).state, 'available');
  assert.equal(Object.hasOwn(accepted.response, 'releasedAssignmentIds'), false);
  const replay = acceptOperatingAssignmentSubmissionV2(request, draft, { initialState: accepted.state });
