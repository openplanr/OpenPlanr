...
test('DEV simulator success cannot author terminal SHIP truth', () => {
  const dir = mkdtempSync(join(tmpdir(), 'openplanr-dev-success-'));
  try {
    const specDir = join(dir, '.planr', 'specs', 'SPEC-001-success');
    const result = simulateDevLoopWithRetries({
      specDir,
      taskIds: ['T-001'],
      llmSucceedsOnIteration: () => true,
    });
    assert.deepEqual(result, { 'T-001': 'done' });
    assert.equal(existsSync(join(specDir, '.pipeline-shipped')), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
