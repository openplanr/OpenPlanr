...
function validatedAssignment(value, artifact) {
  return {
    ...clone(value), assignmentId: artifact.assignmentId,
    ...
    state: 'validated', availableAt: TIME, completedAt: TIME,
    claim: { actorId: 'agent-001', actorKind: 'agent', runtime: 'codex', claimId: `claim-${artifact.assignmentId}` },
...
function sourceState({
  ...
  const sourceArtifact = {
    ...clone(valid['operating-artifact']), artifactId: 'art_source_001', inputArtifactIds: [], createdAt: TIME,
  };
  const targetBytes = Buffer.from('target evidence bytes\n');
  const targetArtifact = {
    ...clone(valid['operating-artifact']), artifactId: 'art_target_001', assignmentId: 'asg_target_001', inputArtifactIds: [], createdAt: TIME,
    artifactType: 'chair-result', schemaId: 'operating-decision-ledger', artifactSchemaVersion: '2.0.0',
    rawHash: rawHash(targetBytes), canonicalHash: null, sizeBytes: targetBytes.byteLength,
    ...targetArtifactOverrides,
  };
  const sourceCandidate = candidateFactory === null
  ...
  const claimLinks = links.map(({ localClaimId, relation, confidence }) => ({
    sourceArtifactId: sourceArtifact.artifactId, localClaimId, relation, confidence,
  }));
  ...
  const sourceSubmission = {
    ...clone(valid['operating-submission']), submissionId: 'sub_source_001', assignmentId: sourceArtifact.assignmentId,
    artifactId: sourceArtifact.artifactId, rawHash: sourceArtifact.rawHash, canonicalHash: sourceArtifact.canonicalHash,
    acceptanceEventIds: ['evt-source-accepted'],
  };
  const targetSubmission = {
    ...clone(sourceSubmission), submissionId: 'sub_target_001', assignmentId: targetArtifact.assignmentId,
    artifactId: targetArtifact.artifactId, rawHash: targetArtifact.rawHash, canonicalHash: targetArtifact.canonicalHash,
...
test('OpenPlanr cycle-context evidence receives only the closed context-manifest source contract', () => {
  ...
  const result = materializeOperatingEvidenceV2({ candidate: seeded.sourceCandidate }, draft({
  ...
  assert.notDeepEqual(result.evidenceRef.sourceContract, { id: 'prior-decisions', version: '1.0.0' });
});
...
function candidate(sourceArtifact, targetArtifact, overrides = {}) {
  return {
    ...
    candidateId: 'evc_materialize_001', scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    sourceArtifactId: sourceArtifact.artifactId, evidenceKind: 'operate-artifact',
    locator: {
      artifactId: targetArtifact.artifactId, expectedArtifactType: targetArtifact.artifactType,
      expectedSchemaId: targetArtifact.schemaId, expectedSchemaVersion: targetArtifact.artifactSchemaVersion,
...
function draft(overrides = {}) {
  return {
    resolutionId: 'evs_materialize_001', eventId: 'evt-evidence-001', timestamp: TIME,
    correlationId: 'corr-evidence-001', evidenceRefId: 'evr_materialize_001', evidenceArtifactId: 'art_evidence_001',
    ...overrides,
  };
}
...
test('a validated candidate atomically materializes one immutable evidence-snapshot, EvidenceRef, Event, and local proof edge', () => {
  const { state, sourceArtifact, targetArtifact, sourceCandidate, claimLinks, artifactStore, targetBytes } = sourceState();
  const before = sha256Jcs(state);
  const replayHook = createNoModelReplayHookV2();
  const result = materializeOperatingEvidenceV2({
    candidate: sourceCandidate,
    claimLinks,
  }, draft(), options(state, artifactStore, { replayHook }));
  ...
  assert.deepEqual(result.evidenceArtifact.inputArtifactIds, [sourceArtifact.artifactId, targetArtifact.artifactId].sort());
  ...
  assert.equal(result.state.findings.length, 0, 'a proof link does not create persistent operating work');
  assert.equal(result.state.decisions.length, 0);
  assert.equal(result.state.actions.length, 0);
  assert.deepEqual(readOperatingArtifactRawBytesV2(artifactStore, {
    artifactId: result.evidenceArtifact.artifactId,
    rawHash: result.evidenceArtifact.rawHash,
  }), targetBytes, 'snapshot bytes are staged under the runtime-owned Artifact identity');
...
test('identical evidence resolution replays without effects; divergent candidate, identity, resolver output, or link input fails before mutation', () => {
  ...
  const request = {
    candidate: sourceCandidate,
    claimLinks: [{ sourceArtifactId: sourceArtifact.artifactId, localClaimId: 'local-claim-001', relation: 'contradictedBy', confidence: 0.4 }],
  };
  const first = materializeOperatingEvidenceV2(request, draft(), options(state, artifactStore));
  const replayHook = createNoModelReplayHookV2();
  const replay = materializeOperatingEvidenceV2(clone(request), clone(draft()), options(first.state, artifactStore, { replayHook }));
  ...
  assert.throws(() => materializeOperatingEvidenceV2({
    ...request,
    candidate: { ...request.candidate, locator: { ...request.candidate.locator, expectedRawHash: `sha256:${'f'.repeat(64)}` } },
  }, draft(), options(first.state, artifactStore)), { code: 'STATE_TRANSITION_INVALID' });
  assert.throws(() => materializeOperatingEvidenceV2(request, draft({ eventId: 'evt-evidence-different' }), options(first.state, artifactStore)), {
...
test('registered resolver rejection is durably auditable but creates no EvidenceRef, snapshot Artifact, or graph edge', () => {
  ...
  const result = materializeOperatingEvidenceV2({
    ...
    resolutionId: 'evs_rejected_001', eventId: 'evt-evidence-rejected-001',
    evidenceRefId: undefined, evidenceArtifactId: undefined,
...
test('foreign, duplicate, untyped, or unresolved claim links fail without making a hidden Claim or operating-state mutation', () => {
  ...
  for (const claimLinks of [
    [{ sourceArtifactId: 'art_foreign_001', localClaimId: 'local-claim-001', relation: 'supportedBy', confidence: 0.5 }],
    [{ sourceArtifactId: sourceArtifact.artifactId, localClaimId: 'local-claim-001', relation: 'derivedFrom', confidence: 0.5 }],
    [
      { sourceArtifactId: sourceArtifact.artifactId, localClaimId: 'local-claim-001', relation: 'supportedBy', confidence: 0.5 },
      { sourceArtifactId: sourceArtifact.artifactId, localClaimId: 'local-claim-001', relation: 'supportedBy', confidence: 0.5 },
    ],
  ]) {
    assert.throws(() => materializeOperatingEvidenceV2({ ...request, claimLinks }, draft(), options(state, artifactStore)), {
  ...
  try {
    const seeded = sourceState({
      ...
      candidateFactory: ({ sourceArtifact, candidateId }) => ({
        kind: 'operating-evidence-candidate', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
        candidateId, scopeId: sourceArtifact.scopeId, domainId: sourceArtifact.domainId, domainVersion: sourceArtifact.domainVersion,
        sourceArtifactId: sourceArtifact.artifactId, evidenceKind: 'filesystem',
    ...
    const resolverContext = {
      ...
      filesystemRoots: [{ sourceRootId: 'workspace', rootPath: root, scope: {
        scopeId: seeded.sourceArtifact.scopeId, domainId: seeded.sourceArtifact.domainId, domainVersion: seeded.sourceArtifact.domainVersion,
    ...
    const first = materializeOperatingEvidenceV2(request, draft({
      resolutionId: 'evs_filesystem_replay_001', eventId: 'evt-filesystem-replay-001',
      evidenceRefId: 'evr_filesystem_replay_001', evidenceArtifactId: 'art_filesystem_replay_001',
    }), {
      initialState: seeded.state, registry: OPEN_REFERENCE_EVIDENCE_REGISTRY_V2,
      artifactStore: seeded.artifactStore, resolverContext,
    });
    writeFileSync(join(root, relativePath), 'changed after resolution\n');
    const replay = materializeOperatingEvidenceV2(clone(request), draft({
      resolutionId: 'evs_filesystem_replay_001', eventId: 'evt-filesystem-replay-001',
      evidenceRefId: 'evr_filesystem_replay_001', evidenceArtifactId: 'art_filesystem_replay_001',
    }), {
      initialState: first.state, registry: OPEN_REFERENCE_EVIDENCE_REGISTRY_V2,
      artifactStore: seeded.artifactStore, resolverContext,
    });
    assert.equal(replay.replayed, true, 'changed resolver input cannot reopen an exact durable request');
    assert.deepEqual(replay.events, []);
    assert.deepEqual(readOperatingArtifactRawBytesV2(seeded.artifactStore, {
      artifactId: first.evidenceArtifact.artifactId,
      rawHash: first.evidenceArtifact.rawHash,
    }), initialBytes, 'the snapshot is retrievable by its own immutable identity and hash');
    assert.doesNotMatch(JSON.stringify(first.events), /captured before mutation/, 'Event payloads contain no raw evidence bytes');
    assert.doesNotMatch(JSON.stringify(first.state), /captured before mutation/, 'projection state contains no raw evidence bytes');
  } finally {
...
test('an unregistered resolver-captured source contract is rejected before Artifact or Event state', () => {
  ...
  try {
    const seeded = sourceState({
      ...
      candidateFactory: ({ sourceArtifact, candidateId }) => ({
        kind: 'operating-evidence-candidate', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
        candidateId, scopeId: sourceArtifact.scopeId, domainId: sourceArtifact.domainId,
        domainVersion: sourceArtifact.domainVersion, sourceArtifactId: sourceArtifact.artifactId,
        evidenceKind: 'filesystem', locator: { sourceRootId: 'workspace', path: relativePath },
        provider: { id: 'local-filesystem-evidence-provider', version: '2.0.0' },
        resolver: { id: 'local-filesystem-evidence-resolver', version: '2.0.0' },
      }),
    });
    const before = sha256Jcs(seeded.state);
    assert.throws(() => materializeOperatingEvidenceV2({ candidate: seeded.sourceCandidate }, draft({
      ...
      && error.message.includes('source contract')
    ));
...
test('candidate and proof-link assertions must exactly match the accepted source Artifact body and source bytes must be present and intact', () => {
  ...
  assert.throws(() => materializeOperatingEvidenceV2(request, draft({ resolutionId: 'evs_absent_001', eventId: 'evt-absent-001' }), {
    initialState: seeded.state,
    registry: OPEN_REFERENCE_EVIDENCE_REGISTRY_V2,
  ...
  const tamperedStore = {
    ...
    readRaw: ({ artifactId, rawHash: expectedHash }) => artifactId === seeded.sourceArtifact.artifactId
      ? Buffer.from('{"evidenceCandidates":[],"evidenceClaimLinks":[]}')
      : seeded.artifactStore.readRaw({ artifactId, rawHash: expectedHash }),
  };
  assert.throws(() => materializeOperatingEvidenceV2(request, draft({ resolutionId: 'evs_tampered_001', eventId: 'evt-tampered-001' }), {
    initialState: seeded.state,
    registry: OPEN_REFERENCE_EVIDENCE_REGISTRY_V2,
  ...
  assert.throws(() => materializeOperatingEvidenceV2({
    ...request,
    candidate: { ...request.candidate, locator: { ...request.candidate.locator, expectedRawHash: `sha256:${'f'.repeat(64)}` } },
  }, draft({ resolutionId: 'evs_candidate_tampered_001', eventId: 'evt-candidate-tampered-001' }), options(seeded.state, seeded.artifactStore)), {
    code: 'RESULT_CONTRACT_INVALID',
  });
  assert.throws(() => materializeOperatingEvidenceV2({
    ...request,
    claimLinks: [{ ...request.claimLinks[0], confidence: 0.1 }],
  }, draft({ resolutionId: 'evs_link_tampered_001', eventId: 'evt-link-tampered-001' }), options(seeded.state, seeded.artifactStore)), {
    code: 'RESULT_CONTRACT_INVALID',
  });
  assert.equal(sha256Jcs(seeded.state), sourceHash, 'byte custody and source-payload failures cannot mutate the checkpoint');
});
