...
import { createOperatingApprovalRecordV2, createOperatingApprovalRequirementV2 } from '../../lib/operate/approvals-v2.mjs';
import { assertOperateAuthorityV2, evaluateOperateAuthorityV2 } from '../../lib/operate/authorization-v2.mjs';
import {
...
function authorityContext() {
  ...
  const requirement = createOperatingApprovalRequirementV2({
    policyRequirementId: templateId, evaluation: policyEvaluation, action,
    parties: clone(governed['operating-approval-requirement'].parties),
    expiresAt: governed['operating-approval-requirement'].expiresAt, consumable: true,
  });
  const party = requirement.parties[0];
  const approval = createOperatingApprovalRecordV2({
    approvalId: 'aprv_00000001', requirement, evaluation: policyEvaluation, action,
  ...
  const grant = {
    ...clone(governed['operating-capability-grant']), evaluationId: policyEvaluation.evaluationId,
    approvalIds: [approval.approvalId], scopeHash: requirement.scopeHash,
  };
  const operation = {
    ...clone(governed['operating-governed-operation']), evaluationId: policyEvaluation.evaluationId,
    approvalIds: [approval.approvalId],
  };
  return {
    actor: { actorId: 'operate-runtime-v2', kind: 'engine', capabilities: [clone(action.requestedCapability)] },
    capabilities: [clone(authorization.toolCapabilities.actionExecute)], now: authorization.now,
    action, actionRequest: clone(authorization.executeRequest), scope: clone(authorization.scope),
    target: clone(authorization.target), actionPolicy, actionPolicies: [corePolicy, actionPolicy],
    policyEvaluation, approvalRequirements: [requirement], approvals: [approval],
    capabilityAvailability: clone(governed['operating-capability-availability']), grant, operation,
    operationHistory: [], currentPreconditionArtifactIds: [...action.preconditionArtifactIds],
...
function projectBinding() {
  ...
  return {
    selection,
    binding: createTrustedExecutorBindingV2({ selection, trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2 }),
  };
}
...
function bindExecutorInput(context, binding, { payloadValue, baselineValue }) {
  ...
  const rollbackBaseline = context.operation.rollbackClass === 'not-applicable' ? null : {
    artifactId, contentHash: sha256Jcs(baselineValue), value: clone(baselineValue),
  };
  context.operation.requestFingerprint = deriveContainedExecutorRequestFingerprintV2({
    operation: context.operation, payload, rollbackBaseline,
  });
...
test('the disposable project executor applies once, replays idempotently, reconciles, and persists no Artifact or Event', () => {
  ...
  const executorInput = bindExecutorInput(context, binding, {
    payloadValue: { status: 'after', count: 1 }, baselineValue: initialValue,
  });
  ...
  const invocation = {
    authorityContext: context, authorityDecision: decision, executorInput,
    binding, executor: selection.registration, targetAdapter,
  };
  ...
  assert.deepEqual(OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.reconcile({
    targetAdapter, executorInput, binding, executor: selection.registration, rollbackPlan: null,
  ...
  divergentOperation.requestFingerprint = deriveContainedExecutorRequestFingerprintV2({
    operation: divergentOperation, payload: divergentPayload, rollbackBaseline: divergentBaseline,
  });
  ...
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.reconcile({
    targetAdapter, executorInput: divergentReconcileInput, binding,
    executor: selection.registration, rollbackPlan: null,
  }), { code: 'OPERATION_CONFLICT' });
  assert.equal(targetAdapter.describe().effectCount, 1);
  assert.equal(sha256Jcs(context), contextHash, 'executor did not mutate or persist into authority state');
  assert.equal(Object.keys(receipt).some((key) => /(?:artifact|event|resultId)/u.test(key)), false);
});
...
test('connector, decision, executor, and target substitution fail before a contained effect', () => {
  ...
  context.executor = clone(selection.registration);
  const executorInput = bindExecutorInput(context, binding, {
    payloadValue: { count: 1 }, baselineValue: { count: 0 },
  });
  const decision = assertOperateAuthorityV2('operate.action.execute', context);
  const targetAdapter = createDisposableLocalProjectTargetV2({ target: context.operation.target, initialValue: { count: 0 } });
  const input = {
    authorityContext: context, authorityDecision: decision, executorInput,
    binding, executor: selection.registration, targetAdapter,
  };
  ...
  }), { code: 'OPERATION_CONFLICT' }, 'obsolete top-level operation is not part of the runtime invocation');

  const forged = clone(decision);
  forged.checks = forged.checks.filter((check) => check !== 'trusted-executor-connector-binding');
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({ ...input, authorityDecision: forged }), {
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', disconnected).allowed, true,
    'legacy connectorless validation remains data-only and does not dispatch');
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
    ...input, authorityContext: disconnected,
  }), { code: 'OPERATION_CONFLICT' });
  assert.deepEqual(targetAdapter.read().value, { count: 0 });

  const syntheticTarget = createSyntheticNoNetworkTargetV2({
    target: { kind: 'synthetic-target', id: 'synthetic-0001', revision: 'rev-0001' }, initialValue: { count: 0 },
  });
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({ ...input, targetAdapter: syntheticTarget }), {
    code: 'OPERATION_CONFLICT',
  });
  assert.deepEqual(targetAdapter.read().value, { count: 0 });

  const substitutedTarget = createDisposableLocalProjectTargetV2({
    target: { ...clone(context.operation.target), id: 'record-substituted' }, initialValue: { count: 0 },
  });
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
    ...input, targetAdapter: substitutedTarget,
  }), { code: 'OPERATION_CONFLICT' });
  ...
  assert.equal(evaluateOperateAuthorityV2('operate.action.execute', {
    ...context, executorInput: tamperedEnvelope,
  }).allowed, false);
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
    ...input, executorInput: tamperedEnvelope,
  }), { code: 'OPERATION_CONFLICT' });
  assert.deepEqual(targetAdapter.read().value, { count: 0 });
});
...
test('missing Action keeps ACTION_NOT_FOUND precedence over valid or invalid connected containment input', () => {
  ...
  for (const invalidEnvelope of [false, true]) {
    ...
    assert.equal(decision.error.code, 'ACTION_NOT_FOUND', invalidEnvelope ? 'invalid envelope' : 'valid envelope');
  }
});
...
test('reversible project execution restores only its exact fingerprint-bound baseline', () => {
  ...
  const targetAdapter = createDisposableLocalProjectTargetV2({
    target: executeContext.operation.target, initialValue,
  });
  const executionReceipt = OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
    authorityContext: executeContext, authorityDecision: executeDecision,
    executorInput: executeInput, binding, executor: selection.registration, targetAdapter,
  });
  ...
  rollbackContext.rollbackPlan = {
    ...
    steps: clone(governed['operating-rollback-plan'].steps).map((step) => ({
      ...step, expectedTargetHash: sha256Jcs(initialValue),
    })),
  };
  rollbackContext.operationHistory = [{
    ...clone(executeContext.operation), state: 'succeeded', resultId: 'xres_00000001',
  }];
  rollbackContext.operation = {
    ...
    grantId: 'cgr_00000002', verificationPlanId: rollbackContext.rollbackPlan.verificationPlanId,
  ...
  const rollbackBaseline = {
    ...
    contentHash: sha256Jcs(initialValue), value: initialValue,
  };
  rollbackContext.operation.requestFingerprint = deriveContainedExecutorRequestFingerprintV2({
    operation: rollbackContext.operation, payload: rollbackPayload, rollbackBaseline,
  });
  ...
  const divergentParentDecision = assertOperateAuthorityV2('operate.action.rollback', rollbackContext);
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.rollback({
    authorityContext: rollbackContext, authorityDecision: divergentParentDecision,
    executorInput: rollbackInput, binding, executor: selection.registration, targetAdapter,
  }), { code: 'OPERATION_CONFLICT' });
  assert.deepEqual(targetAdapter.read().value, { status: 'after', count: 1 });
  ...
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.rollback({
    authorityContext: rollbackContext, authorityDecision: rollbackDecision,
    executorInput: rollbackInput, binding, executor: selection.registration, targetAdapter: restartedTarget,
  }), { code: 'OPERATION_CONFLICT' });
  ...
  assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.rollback({
    authorityContext: rollbackContext, authorityDecision: rollbackDecision,
    executorInput: divergent, binding, executor: selection.registration, targetAdapter,
  }), { code: 'OPERATION_CONFLICT' });
});
...
test('synthetic containment host is package-local, no-network, exact-bound, and rejects ambient targets', () => {
  ...
  const binding = createTrustedExecutorBindingV2({
    selection, trustedHost: OPEN_REFERENCE_CONTAINMENT_EXECUTOR_HOST_V2,
  });
  ...
  const target = createSyntheticNoNetworkTargetV2({
    target: { kind: 'synthetic-target', id: 'synthetic-0001', revision: 'rev-0001' }, initialValue: {},
  });
  assert.equal(Object.isFrozen(binding), true);
  assert.throws(() => OPEN_REFERENCE_CONTAINMENT_EXECUTOR_HOST_V2.reconcile({
    targetAdapter: { reconcile: () => ({ status: 'not-found', receipt: null }) },
  }), { code: 'OPERATION_CONFLICT' });

  const source = readFileSync(new URL('../../lib/operate/reference-governed-executors-v2.mjs', import.meta.url), 'utf8');
  assert.doesNotMatch(source, /from ['"]node:(?:fs|net|http|https|child_process|worker_threads)['"]/u);
...
test('contained adapters reject prohibited payloads before mutating explicit state', () => {
  const target = createDisposableLocalProjectTargetV2({
    target: { kind: 'project-record', id: 'record-contained', revision: 'rev-0001' }, initialValue: { safe: true },
  });
  assert.throws(() => createDisposableLocalProjectTargetV2({
    target: { kind: 'project-record', id: 'record-secret', revision: 'rev-0001' },
    initialValue: { credential: 'api-key-secret' },
  }), { code: 'CAPABILITY_DENIED' });
  for (const value of [
    'Credential Change', 'customer_contact', 'DESTROY', 'transfer funds',
    'payment.transfer', 'deployProduction', 'production merge', 'PUBLISH', 'mutate_secret',
  ]) {
    assert.throws(() => createDisposableLocalProjectTargetV2({
      target: { kind: 'project-record', id: `record-${value.length}`, revision: 'rev-0001' },
      initialValue: { safe: { value } },
    }), { code: 'CAPABILITY_DENIED' }, value);
    const normalizedKey = value.replace(/[^A-Za-z0-9]/gu, '_');
    assert.throws(() => createDisposableLocalProjectTargetV2({
      target: { kind: 'project-record', id: `record-key-${value.length}`, revision: 'rev-0001' },
      initialValue: { [normalizedKey]: true },
    }), { code: 'CAPABILITY_DENIED' }, `${value} key`);
  }
...
test('all consolidated core-prohibition key/value forms fail on the executor path with zero effects', () => {
  ...
  for (const payloadValue of vectors) {
    assert.throws(() => createDisposableLocalProjectTargetV2({
      target: { kind: 'project-record', id: `direct-${Object.keys(payloadValue)[0]}`, revision: 'rev-0001' },
    ...
    context.executor = clone(selection.registration);
    const executorInput = bindExecutorInput(context, binding, {
      payloadValue, baselineValue: { safe: true },
    });
    const decision = assertOperateAuthorityV2('operate.action.execute', context);
    const targetAdapter = createDisposableLocalProjectTargetV2({
      target: context.operation.target, initialValue: { safe: true },
    });
    assert.throws(() => OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2.execute({
      authorityContext: context, authorityDecision: decision, executorInput,
      binding, executor: selection.registration, targetAdapter,
    }), { code: 'CAPABILITY_DENIED' }, JSON.stringify(payloadValue));
    assert.deepEqual(targetAdapter.read(), {
      value: { safe: true }, revision: context.operation.target.revision, stateHash: sha256Jcs({ safe: true }),
  ...
  assert.deepEqual(payOnly.read().value, { pay: true },
  ...
  for (const standalone of ['ship', 'message', 'email']) {
    ...
    assert.deepEqual(target.read().value, { [standalone]: true },
