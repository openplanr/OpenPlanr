...
test('the public governed registry is closed, deterministic, exact-versioned, and independent of private implementations', () => {
  ...
  assert.deepEqual(registry.executors.map(({ executorId }) => executorId), [
    'open-reference-containment-executor',
    'open-reference-project-executor',
  ]);
  assert.equal(findOperateCapabilityProviderRegistrationV2(registry, 'open-reference-capability-provider', { providerVersion: '1.0.0' })?.implementation.source, 'built-in');
  assert.equal(findOperatePolicyProviderRegistrationV2(registry, 'open-reference-policy-provider', { providerVersion: '1.0.0' })?.narrowingOnly, true);
  assert.equal(findOperateExecutorRegistrationV2(registry, 'open-reference-project-executor', { executorVersion: '1.0.1' }), null);
  assert.doesNotMatch(JSON.stringify(registry), /(?:private-consumer|credential|secret|dynamic-import|https?:|\.\.[/\\])/iu);
  assert.equal(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSION_CATALOG_DIGEST_V2,
  ...
  const reservationChild = spawnSync(process.execPath, ['--input-type=module', '--eval', `
    import {
...
test('capability, policy, and executor discovery select only an exact healthy compatible declaration', () => {
  ...
  const capability = selectOperateCapabilityProviderV2(registry, criteria(selectionFixture.capabilitySelection));
  const policy = selectOperatePolicyProviderV2(registry, criteria(selectionFixture.policySelection));
  const project = selectOperateExecutorV2(registry, criteria(selectionFixture.projectExecutorSelection));
  const containment = selectOperateExecutorV2(registry, criteria(selectionFixture.containmentExecutorSelection));
  assert.deepEqual([
  ...
  const releaseWindowSelection = selectOperateExecutorV2(registry, {
    ...criteria(selectionFixture.projectExecutorSelection), now: '2026-08-12T12:00:00Z',
  });
  ...
  const expiredEquivalent = clone(expiredInput.executors.find(
  ...
  const unavailable = selectOperateExecutorV2(registry, {
    ...criteria(selectionFixture.projectExecutorSelection), executorVersion: '1.0.1',
  });
  assert.deepEqual(unavailable, {
    status: 'unavailable', registration: null, reasonCode: 'registration-not-found',
...
test('all governed selectors use an own-known-key null-prototype effect rank', () => {
  ...
  const selectors = [
    [selectOperateCapabilityProviderV2, selectionFixture.capabilitySelection, 'CAPABILITY_PROVIDER_UNAVAILABLE'],
    [selectOperatePolicyProviderV2, selectionFixture.policySelection, 'POLICY_PROVIDER_UNAVAILABLE'],
  ...
  for (const effectClass of ['toString', 'constructor', '__proto__', 'arbitrary-effect']) {
    ...
    for (const [select, selection, errorCode] of selectors) {
      ...
      assert.deepEqual(result, {
        status: 'unavailable', registration: result.registration,
  ...
  for (const [collection, index] of [['capabilityProviders', 0], ['policyProviders', 0], ['executors', 0]]) {
...
test('registration and health create no grant, approval, transition, target access, or fallback authority', () => {
  ...
  const availability = createOpenReferenceCapabilityAvailabilityV2({
    action, runtimeVersion: '0.42.0', checkedAt: '2026-08-12T12:00:00Z',
  ...
  const expired = selectOperateCapabilityProviderV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, {
    ...criteria(selectionFixture.capabilitySelection), now: '2027-08-12T00:00:00Z',
  });
  assert.equal(expired.status, 'unavailable');
  assert.equal(expired.reasonCode, 'health-expired');
  assert.deepEqual(expired.fallback, { kind: 'unavailable', errorCode: 'CAPABILITY_PROVIDER_UNAVAILABLE' });
});
...
test('trusted host bindings sit beneath one selected executor and cannot substitute its identity', () => {
  ...
  const binding = createTrustedExecutorBindingV2({ selection, trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2 });
  assert.deepEqual(assertTrustedExecutorBindingV2(binding, { executor: selection.registration }), binding);
  assert.deepEqual(assertOperateExecutorRegistrationV2(selection.registration), selection.registration);
  assert.deepEqual(binding.connector, { id: 'disposable-local-project-connector', version: '1.0.0' });
  assert.equal(Object.hasOwn(selection.registration, 'connector'), false);
  assert.throws(
    () => createTrustedExecutorBindingV2({ selection, trustedHost: OPEN_REFERENCE_CONTAINMENT_EXECUTOR_HOST_V2 }),
    { code: 'EXECUTOR_UNAVAILABLE' },
  );
  const tampered = clone(binding);
  tampered.connector = { id: 'substitute', version: '1.0.0' };
  assert.throws(() => assertTrustedExecutorBindingV2(tampered, { executor: selection.registration }), {
    code: 'EXECUTOR_UNAVAILABLE',
  });
  assert.throws(() => assertTrustedExecutorBindingV2(clone(binding), { executor: selection.registration }), {
    code: 'EXECUTOR_UNAVAILABLE',
  }, 'caller-constructed byte clones do not carry the host-owned binding proof');
  const forgedHost = {
    ...OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
    connector: { id: 'disposable-local-project-connector', version: '1.0.0', url: 'https://example.invalid' },
  };
...
test('hostile registration fields, duplicates, prohibited effects, and fallback substitution fail closed', () => {
  ...
  for (const vector of invalid.vectors.filter(({ kind }) => kind === 'registration')) {
    ...
    assert.throws(() => createOperateGovernedExtensionRegistryV2(input), { code: vector.errorCode }, vector.name);
  }
  ...
  for (const mutate of [
    (input) => { input.capabilityProviders[0].provenance.packageName = 'substitute-package'; },
    (input) => { input.policyProviders[0].health.expiresAt = '2026-08-12T08:00:00Z'; },
    (input) => { input.executors[1].implementation.id = 'open-reference-project-executor-v2-substitute'; },
  ]) {
  ...
  for (const { collection, index, idField, versionField } of [
    { collection: 'capabilityProviders', index: 0, idField: 'providerId', versionField: 'providerVersion' },
    { collection: 'policyProviders', index: 0, idField: 'providerId', versionField: 'providerVersion' },
    ...
    differentVersion[collection][index].implementation.id = `unreserved-implementation-${collection.toLowerCase()}-${index}`;
    assert.throws(() => createOperateGovernedExtensionRegistryV2(differentVersion), {
      code: 'E_EXTENSION_REGISTRATION_CONFLICT',
    ...
    assert.throws(() => createOperateGovernedExtensionRegistryV2(differentId), {
      code: 'E_EXTENSION_REGISTRATION_CONFLICT',
  ...
  for (const identifier of [
    'contact-customer', 'payment-send', 'deploy-production', 'merge-production',
    'transfer-funds', 'publish-now', 'delete-record', 'rotate-secret', 'change-credential',
    'contactCustomer', 'payment_send', 'deploy.production', 'production/merge',
  ]) {
    const input = clone(extensions);
  ...
  for (const identifier of [
    'release-public', 'release', 'remove-record', 'erase-record', 'wipe-record', 'remit-funds',
    'releasePublic', 'remove_record', 'erase.record', 'wipe/record', 'funds-remit',
    'pay-send', 'transfer-pay',
    'money-transfer', 'ship-production', 'deliver-production', 'delivery-production',
    'message-customer', 'email-customer', 'reachout-customer',
    'productionShip', 'prod_deliver', 'customer.email', 'customer/reachOut',
  ]) {
    assert.throws(() => createOperateGovernedExtensionRegistryV2(
      registrationsWithCommunityExecutor(identifier),
    ), { code: 'E_EXTENSION_REGISTRATION_INVALID' }, `non-reserved ${identifier}`);
  }
  const payOnly = createOperateGovernedExtensionRegistryV2(registrationsWithCommunityExecutor('pay'));
  assert.equal(payOnly.executors.some(({ executorId }) => executorId === 'community-contained-executor'), true,
  ...
  for (const standalone of ['ship', 'message', 'email']) {
    ...
    assert.equal(registry.executors.some(({ executorId }) => executorId === 'community-contained-executor'), true,
  ...
  nestedPrototype.executors[0].implementation = Object.assign(
    Object.create({ inherited: true }), nestedPrototype.executors[0].implementation,
  );
