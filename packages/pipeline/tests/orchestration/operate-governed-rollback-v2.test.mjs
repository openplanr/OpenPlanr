...
const EVENT_FIELDS = [
  ...
  'capabilityGranted', 'intentRecorded', ...TERMINAL_FIELDS,
];
...
export function governedRollbackDraft(suffix = '75000002') {
  return {
    ...
    eventIds: Object.fromEntries(EVENT_FIELDS.map((field, index) => (
    ...
    uncertainty: {
      ...
      eventIds: Object.fromEntries(TERMINAL_FIELDS.map((field, index) => (
...
export async function governedRollbackScenario({ executeSuffix = '75000001', rollbackSuffix = '75000002' } = {}) {
  ...
  const executionRuntime = createOperatingGovernedExecutionRuntimeV2({ initialState: execution.initial, checkpointStore: executionStore });
  ...
  return {
    execution, targetAdapter, completed, plan, planned,
    request, draft: governedRollbackDraft(rollbackSuffix),
  };
}
...
runTest('rollback is a separately authorized operation that restores the exact baseline once', async () => {
  const scenario = await governedRollbackScenario();
  const store = createGovernedExecutionCheckpointStore(scenario.planned.state);
  const runtime = createOperatingGovernedRecoveryRuntimeV2({ initialState: scenario.planned.state, checkpointStore: store });
  ...
  assert.equal(rolledBack.result.targetAfterHash, scenario.execution.request.rollbackBaseline.contentHash);
  ...
  const original = rolledBack.state.governedOperations.find(({ operationKind }) => operationKind === 'execute');
  assert.deepEqual(original, scenario.completed.operation, 'rollback never mutates original operation history');

  const exactRetry = await runtime.rollback(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('rollback replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('rollback replay touched target'); } }),
  });
  assert.equal(exactRetry.replayed, true);
  assert.equal(scenario.targetAdapter.describe().effectCount, 2);
  await assert.rejects(runtime.rollback(scenario.request, governedRollbackDraft('75000009'), {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
    targetAdapter: scenario.targetAdapter,
  }), { code: 'ROLLBACK_NOT_ELIGIBLE' });
  assert.equal(scenario.targetAdapter.describe().effectCount, 2, 'a fresh identity cannot reuse a consumed rollback plan');
});

runTest('rollback consumes one fresh complete approval disposition independent of the parent execution', async () => {
  const execution = governedExecutionScenario({ suffix: '75000018', automatic: false });
  const targetAdapter = createDisposableLocalProjectTargetV2({
    target: execution.action.targetBinding, initialValue: execution.initialValue,
  });
  ...
  const party = {
    partyId: 'rollback-owner-party', actorKind: 'human', actorId: 'rollback-owner-0001',
    requiredCapability: { id: 'action-approve', version: '1.0.0' },
  };
  const requirement = createOperatingApprovalRequirementV2({
    policyRequirementId: 'aprq_template01', evaluation, action: execution.action,
    parties: [party], expiresAt: '2026-08-11T08:00:00Z', consumable: true,
  });
  const approval = createOperatingApprovalRecordV2({
    approvalId: 'aprv_rollback75000018', requirement, evaluation, action: execution.action,
    partyId: party.partyId,
    actor: { kind: party.actorKind, actorId: party.actorId, capability: party.requiredCapability },
    decision: 'approved', issuedAt: '2026-08-10T12:01:45Z', expiresAt: '2026-08-11T08:00:00Z',
  });
  ...
  const planned = recordOperatingRollbackPlanV2({
    state: validatedState, plan, eventId: 'evt_rollback_plan_75000018',
  });
  ...
  const rollbackGrant = result.state.capabilityGrants.find(({ operationId }) => operationId === draft.operationId);
  const consumed = result.state.approvalRecords.find(({ approvalId }) => approvalId === approval.approvalId);
...
runTest('exact rollback replay reconstructs from Event and Artifact history without target access', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '75000003', rollbackSuffix: '75000004' });
  const store = createGovernedExecutionCheckpointStore(scenario.planned.state);
  const runtime = createOperatingGovernedRecoveryRuntimeV2({ initialState: scenario.planned.state, checkpointStore: store });
  const first = await runtime.rollback(scenario.request, scenario.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter,
  });
...
runTest('required rollback eligibility survives construction, direct reduction, checkpoint loading, and restart replay', async () => {
  const scenario = await governedRollbackScenario({
    executeSuffix: '75000020', rollbackSuffix: '75000021',
  });
  ...
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: planned.state, checkpointStore: store,
  });
  ...
  const replay = await restarted.rollback(request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('required replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('required replay touched target'); } }),
  });
  assert.equal(replay.replayed, true);
  assert.equal(replay.result.rollbackPlanId, requiredPlan.rollbackPlanId);
});

runTest('dispatching rollback reconciliation binds the durable plan and classifies exact postconditions', async () => {
  const absent = await governedRollbackScenario({
    executeSuffix: '75000022', rollbackSuffix: '75000023',
  });
  ...
  const absentRuntime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: absent.planned.state, checkpointStore: absentStore,
  });
  await assert.rejects(absentRuntime.rollback(absent.request, absent.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
  ...
  const operation = absentState.governedOperations.find(({ operationKind }) => (
    operationKind === 'rollback'
  ));
  const parentReceipt = absentState.operationReplayIndex.find(({ operationId }) => (
  ...
  for (const [field, invalidRevision] of [
    ...
    assert.deepEqual(classifyOperatingRollbackReconciliationReceiptV2({
      state: absentState,
      operationId: operation.operationId,
      response: { status: 'succeeded', receipt: invalid },
    }), { classification: 'unknown', receipt: null }, `${field} revision ${String(invalidRevision)}`);
  }
  ...
  for (const stalePostRevision of [baseReceipt.before.revision, parentReceipt.before.revision]) {
    ...
    }), { classification: 'unknown', receipt: null }, stalePostRevision);
  }

  const staleOperationState = structuredClone(absentState);
  staleOperationState.governedOperations.find(({ operationId }) => (
    operationId === operation.operationId
  )).target.id = 'foreign-target-with-stale-operation-hash';
  assert.throws(() => classifyOperatingRollbackReconciliationReceiptV2({
  ...
  stalePlanState.rollbackPlans.find(({ rollbackPlanId }) => (
    rollbackPlanId === absent.plan.rollbackPlanId
  )).baselineHash = `sha256:${'f'.repeat(64)}`;
  assert.throws(() => classifyOperatingRollbackReconciliationReceiptV2({
    state: stalePlanState,
    operationId: operation.operationId,
    response: { status: 'succeeded', receipt: baseReceipt },
  }), (error) => ['OPERATION_CONFLICT', 'ROLLBACK_NOT_ELIGIBLE', 'STATE_TRANSITION_INVALID'].includes(error.code));
...
runTest('stale target and expired or repeated plans fail closed without a second restoration', async () => {
  const stale = await governedRollbackScenario({ executeSuffix: '75000005', rollbackSuffix: '75000006' });
  ...
  await assert.rejects(staleRuntime.rollback(stale.request, stale.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: stale.targetAdapter,
  }), { code: 'ROLLBACK_NOT_ELIGIBLE' });
  assert.equal(staleRuntime.getState().rollbackResults.length, 0);

  const expired = await governedRollbackScenario({ executeSuffix: '75000007', rollbackSuffix: '75000008' });
  ...
  await assert.rejects(expiredRuntime.rollback(expired.request, expired.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: expired.targetAdapter,
  }), { code: 'ROLLBACK_NOT_ELIGIBLE' });
});

runTest('getter-backed CAS receipts cannot claim intent or terminal checkpoint ownership', async () => {
  function accessorReceipt(claimedState, durableState) {
    ...
    return Object.defineProperties({}, {
      ...
      state: {
        enumerable: true,
        get() {
          return committedRead <= 2 ? structuredClone(claimedState) : structuredClone(durableState);
        },
      },
    });
  }
  ...
  const intent = await governedRollbackScenario({
    executeSuffix: '75000038', rollbackSuffix: '75000039',
  });
  const intentStore = {
  ...
  await assert.rejects(intentRuntime.rollback(intent.request, intent.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
  ...
  assert.equal(intentStore.snapshot().governedOperations.filter(({ operationKind }) => (
    operationKind === 'rollback'
  )).length, 0);

  const terminal = await governedRollbackScenario({
    executeSuffix: '75000040', rollbackSuffix: '75000041',
  });
  ...
  await assert.rejects(terminalRuntime.rollback(terminal.request, terminal.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
  ...
  const terminalOperation = durable.governedOperations.find(({ operationKind }) => (
    operationKind === 'rollback'
  ));
  ...
  assert.equal(durable.artifacts.some(({ artifactId }) => (
    artifactId === terminal.draft.uncertainty.resultArtifactId
  )), false);
  assert.equal(terminalRuntime.dispatchCount, 1);
  assert.equal(terminal.targetAdapter.describe().restoreCallCount, 1);
});

runTest('proven rollback success rebases terminal CAS contention without a second host call', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '75000010', rollbackSuffix: '75000011' });
  ...
  const store = createGovernedExecutionCheckpointStore(scenario.planned.state, {
    beforeCommit(input, durable) {
      ...
      const availability = structuredClone(durable.capabilityAvailability.find(({ checkedAt }) => (
        checkedAt === scenario.draft.preparedAt
      )));
      ...
      const event = createOperatingRuntimeEventV2({
        eventId: 'evt_unrelated75000011', timestamp: scenario.draft.completedAt,
        cycleId: scenario.execution.action.sourceCycleId,
        type: 'capability.availability-recorded', entityId: availability.availabilityId,
        actor: { kind: 'engine', id: 'operate-runtime-v2' }, causationId: prior.eventId,
        correlationId: 'corr-unrelated-75000011', payload: availability,
  ...
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: scenario.planned.state, checkpointStore: store, artifactStore,
  });
  const rolledBack = await runtime.rollback(scenario.request, scenario.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter,
  });
...
runTest('rollback wrapper proves write-then-throw byte custody and restart replay reads the exact bytes', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '75000012', rollbackSuffix: '75000013' });
  ...
  const replay = await restarted.rollback(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('byte replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('byte replay touched target'); } }),
  });
  assert.equal(replay.replayed, true);
  assert.deepEqual(replay.result, first.result);
  assert.equal(restarted.dispatchCount, 0);
  ...
  const corruptReplay = createOperatingGovernedRecoveryRuntimeV2({
    ...
    artifactStore: {
      stageRaw() { return true; },
      readRaw() { return Buffer.from('{}', 'utf8'); },
    },
  });
  await assert.rejects(corruptReplay.rollback(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('corrupt replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('corrupt replay touched target'); } }),
  }), (error) => error.code === 'RESULT_CONTRACT_INVALID'
    && error.details.context.provenTerminal === true
  ...
  const unreadableReplay = createOperatingGovernedRecoveryRuntimeV2({
    ...
    artifactStore: {
      stageRaw() { return true; },
      readRaw() { throw Object.assign(new Error('fresh-runtime-read-failed'), {
        code: 'FRESH_RUNTIME_READ_FAILED',
      }); },
    },
  });
  await assert.rejects(unreadableReplay.rollback(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('unreadable replay touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('unreadable replay touched target'); } }),
  }), (error) => error.code === 'RESULT_CONTRACT_INVALID'
    && error.details.context.provenTerminal === true
    && error.details.context.recoveryDisposition === 'restore-exact-result-custody-before-retry');
  ...
  assert.equal(store.snapshot().rollbackResults.some(({ status }) => status === 'uncertain'), false);
  assert.equal(scenario.targetAdapter.describe().restoreCallCount, 1);
});

runTest('a proven rollback receipt is never downgraded when exact Artifact custody is unavailable', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '75000016', rollbackSuffix: '75000017' });
  const store = createGovernedExecutionCheckpointStore(scenario.planned.state);
  const artifactStore = rawArtifactStore({ fail: true });
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: scenario.planned.state, checkpointStore: store, artifactStore,
  });
  await assert.rejects(runtime.rollback(scenario.request, scenario.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter,
  }), (error) => error.code === 'OPERATION_UNCERTAIN'
    && error.details.context.provenTerminal === true);
  const durable = store.snapshot();
  const rollbackOperation = durable.governedOperations.find(({ operationKind }) => operationKind === 'rollback');
  ...
  assert.equal(scenario.targetAdapter.describe().effectCount, 2);
  await assert.rejects(runtime.rollback(scenario.request, scenario.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('proven retry touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('proven retry touched target'); } }),
  }), { code: 'OPERATION_UNCERTAIN' });
  assert.equal(scenario.targetAdapter.describe().restoreCallCount, 1);
});

runTest('post-proof malformed, rejected, divergent CAS and replay-read failures never create uncertainty or redispatch', async () => {
  const assertProvenTerminal = (error) => (
    error?.details?.context?.provenTerminal === true
  );
  for (const [index, mode] of ['malformed', 'rejected', 'divergent'].entries()) {
    ...
    const runtime = createOperatingGovernedRecoveryRuntimeV2({
      initialState: scenario.planned.state, checkpointStore,
    });
    await assert.rejects(runtime.rollback(scenario.request, scenario.draft, {
      trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
      targetAdapter: scenario.targetAdapter,
    }), assertProvenTerminal, mode);
    const durable = checkpointStore.snapshot();
    const rollbackOperation = durable.governedOperations.find(({ operationKind }) => (
      operationKind === 'rollback'
    ));
    ...
    assert.equal(
      durable.artifacts.some(({ artifactId }) => artifactId === scenario.draft.uncertainty.resultArtifactId),
      false,
      mode,
    );
    assert.equal(scenario.targetAdapter.describe().restoreCallCount, 1, mode);
    await assert.rejects(runtime.rollback(scenario.request, scenario.draft, {
      trustedHost: new Proxy({}, { get() { throw new Error(`${mode} retry touched host`); } }),
      targetAdapter: new Proxy({}, { get() { throw new Error(`${mode} retry touched target`); } }),
    }), assertProvenTerminal, `${mode} retry`);
    assert.equal(scenario.targetAdapter.describe().restoreCallCount, 1, mode);
  ...
  const replayRead = await governedRollbackScenario({
    executeSuffix: '75000036', rollbackSuffix: '75000037',
  });
  const bytes = new Map();
  let reads = 0;
  const artifactStore = {
    ...
    readRaw({ artifactId }) {
      reads += 1;
      if (reads >= 3) throw Object.assign(new Error('replay-read-failed'), {
  ...
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: replayRead.planned.state, checkpointStore, artifactStore,
  });
  await assert.rejects(runtime.rollback(replayRead.request, replayRead.draft, {
    trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2,
    targetAdapter: replayRead.targetAdapter,
  }), assertProvenTerminal);
  const durable = checkpointStore.snapshot();
  assert.equal(durable.governedOperations.find(({ operationKind }) => operationKind === 'rollback').state, 'succeeded');
  ...
  await assert.rejects(runtime.rollback(replayRead.request, replayRead.draft, {
    trustedHost: new Proxy({}, { get() { throw new Error('replay-read retry touched host'); } }),
    targetAdapter: new Proxy({}, { get() { throw new Error('replay-read retry touched target'); } }),
  }), assertProvenTerminal);
  assert.equal(replayRead.targetAdapter.describe().restoreCallCount, 1);
});

runTest('unsupported, irreversible, prohibited, and non-success parent rollback vectors fail closed', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '75000014', rollbackSuffix: '75000015' });
  ...
  assert.throws(() => buildOperatingRollbackPlanV2({
    operation: irreversible, result: scenario.completed.result,
    baseline: scenario.execution.request.rollbackBaseline,
    expiresAt: '2026-08-11T12:00:00Z',
  }), { code: 'ROLLBACK_NOT_ELIGIBLE' });
  ...
  for (const status of ['failed', 'uncertain']) {
    ...
    assert.throws(() => buildOperatingRollbackPlanV2({
      operation, result, baseline: scenario.execution.request.rollbackBaseline,
      expiresAt: '2026-08-11T12:00:00Z',
    }), { code: 'ROLLBACK_NOT_ELIGIBLE' }, status);
  }
  ...
  const partialPlan = buildOperatingRollbackPlanV2({
    operation: partialOperation, result: partialResult,
    baseline: scenario.execution.request.rollbackBaseline,
    expiresAt: '2026-08-11T12:00:00Z',
  });
  assert.equal(partialPlan.eligibility, 'uncertain');
  assert.throws(() => assertOperatingRollbackPolicyV2({
  ...
  unsupportedRegistry.executors = unsupportedRegistry.executors.filter(({ executorId }) => (
