...
function rehashRollbackResultCustody(state) {
  ...
  const replay = state.submissionReplayIndex.find(({ artifactId }) => artifactId === artifact.artifactId);
...
test('randomized exact rollback identities restore one baseline and preserve one original history', async () => {
  for (let seed = 1; seed <= 10; seed += 1) {
    ...
    assert.equal(result.state.governedOperations.filter(({ operationKind }) => operationKind === 'execute').length, 1, `seed ${seed}`);
    assert.equal(result.state.governedOperations.filter(({ operationKind }) => operationKind === 'rollback').length, 1, `seed ${seed}`);
...
test('divergent rollback plan, baseline, payload, and identity vectors never dispatch', async () => {
  const mutations = [
    (request) => { request.rollbackPlanId = 'rbp_divergent78000001'; },
    (request) => { request.rollbackBaseline.contentHash = `sha256:${'f'.repeat(64)}`; },
    (request) => { request.rollbackBaseline.value = { forged: true }; },
    (request) => { request.payload.artifactId = 'art_divergent78000001'; },
    (request) => { request.originalOperationId = 'op_divergent78000001'; },
  ];
  for (let index = 0; index < mutations.length; index += 1) {
    ...
    await assert.rejects(runtime.rollback(request, scenario.draft, {
      trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter,
...
test('Promise-concurrent identical rollback attempts elect one dispatcher and one restoration', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '78000020', rollbackSuffix: '79000020' });
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: scenario.planned.state,
    checkpointStore: createGovernedExecutionCheckpointStore(scenario.planned.state),
  });
  const environment = { trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter };
...
test('independent runtimes sharing one checkpoint cannot both own the rollback dispatch', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '78000030', rollbackSuffix: '79000030' });
  const store = createGovernedExecutionCheckpointStore(scenario.planned.state);
  const runtimeA = createOperatingGovernedRecoveryRuntimeV2({ initialState: scenario.planned.state, checkpointStore: store });
  const runtimeB = createOperatingGovernedRecoveryRuntimeV2({ initialState: scenario.planned.state, checkpointStore: store });
  const environment = { trustedHost: OPEN_REFERENCE_PROJECT_EXECUTOR_HOST_V2, targetAdapter: scenario.targetAdapter };
  const attempts = await Promise.allSettled([
  ...
  assert.equal(store.snapshot().governedOperations.filter(({ operationKind }) => operationKind === 'rollback').length, 1);
});

test('fully rehashed rollback authority and terminal projection mutations all fail checkpoint loading', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '78000040', rollbackSuffix: '79000040' });
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: scenario.planned.state,
    checkpointStore: createGovernedExecutionCheckpointStore(scenario.planned.state),
  });
  ...
  const rollbackOperationIndex = completed.state.governedOperations.findIndex(({ operationKind }) => operationKind === 'rollback');
  const rollbackGrantIndex = completed.state.capabilityGrants.findIndex(({ operationId }) => (
    operationId === scenario.draft.operationId
  ));
  const rollbackAvailabilityIndex = completed.state.capabilityAvailability.findIndex(({ checkedAt }) => (
    checkedAt === scenario.draft.preparedAt
  ));
  const rollbackAssignmentIndex = completed.state.assignments.findIndex(({ assignmentId }) => (
  ...
  const mutations = [
    ...
    ['complete approval disposition', (state) => {
  ...
  for (const [label, mutate] of mutations) {
    ...
    mutate(state);
    assert.throws(() => reduceOperatingRuntimeEventsV2([], { initialState: state }), (error) => (
      ['ACTION_REVISION_MISMATCH', 'APPROVAL_INVALID', 'CAPABILITY_GRANT_INVALID',
        'OPERATION_CONFLICT', 'POLICY_EVALUATION_REJECTED', 'RESULT_CONTRACT_INVALID',
        'STATE_TRANSITION_INVALID'].includes(error.code)
    ), label);
  }
});

test('fully rehashed direct intent and terminal Events cannot bypass the shared rollback authority validator', async () => {
  const scenario = await governedRollbackScenario({ executeSuffix: '78000041', rollbackSuffix: '79000041' });
  const runtime = createOperatingGovernedRecoveryRuntimeV2({
    initialState: scenario.planned.state,
    checkpointStore: createGovernedExecutionCheckpointStore(scenario.planned.state),
  });
  ...
  const intentIndex = completed.events.findIndex(({ type }) => type === 'operation.intent-recorded');
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2(
    rechain(forgedIntentEvents, scenario.planned.state),
    { initialState: scenario.planned.state },
  ), { code: 'OPERATION_CONFLICT' });

  const forgedRuntimeEvents = structuredClone(completed.events.slice(0, intentIndex + 1));
  forgedRuntimeEvents.find(({ type }) => type === 'assignment.claimed').payload.runtime = 'planr-pipeline@9.9.9';
  assert.throws(() => reduceOperatingRuntimeEventsV2(
    rechain(forgedRuntimeEvents, scenario.planned.state),
    { initialState: scenario.planned.state },
  ), { code: 'OPERATION_CONFLICT' });

  const intentState = reduceOperatingRuntimeEventsV2(
    completed.events.slice(0, intentIndex + 1),
    { initialState: scenario.planned.state },
  );
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2(
    rechain(terminalEvents, intentState),
