...
function inputs(seed = '001') {
  const artifact = {
    ...clone(valid['operating-artifact']), artifactId: `art_signal_${seed}`, assignmentId: `asg_signal_${seed}`,
    scopeId: `scope-signal-${seed}`, domainId: 'business', domainVersion: '1.0.0', inputArtifactIds: [],
  };
  ...
  }, { snapshotId: `snp_signal_${seed}`, stateId: `oms_signal_${seed}`, timestamp: '2026-08-09T12:00:00.000Z' });
  const metric = {
    ...clone(valid['operating-metric']), metricId: `met_signal_${seed}`,
    scopeId: artifact.scopeId, domainId: 'business', domainVersion: '1.0.0', sourceArtifactId: artifact.artifactId,
  };
  const observation = {
    ...clone(valid['operating-metric-observation']), observationId: `mob_signal_${seed}`, metricId: metric.metricId,
    scopeId: artifact.scopeId, domainId: 'business', domainVersion: '1.0.0', snapshotId: transaction.snapshot.snapshotId,
  ...
  const verificationPlan = {
    ...clone(valid['operating-action-verification-plan']), verificationPlanId: `vfy_signal_${seed}`,
    actionId: `act_signal_${seed}`, metricId: metric.metricId,
    scopeId: artifact.scopeId, domainId: 'business', domainVersion: '1.0.0', sourceArtifactId: artifact.artifactId,
  };
  const outcome = {
    ...clone(valid['operating-outcome']), outcomeId: `out_signal_${seed}`, actionId: verificationPlan.actionId,
    verificationPlanId: verificationPlan.verificationPlanId,
    scopeId: artifact.scopeId, domainId: 'business', domainVersion: '1.0.0', sourceArtifactId: verificationArtifact.artifactId,
  };
  return { artifact, verificationArtifact, ...transaction, metric, observation, verificationPlan, outcome };
}
...
test('closed built-ins select exact public domain bindings without dispatching code or an effect', () => {
  ...
  assert.equal(selectOperatingSnapshotProviderV2(undefined, {
    providerId: 'open-reference-snapshot-provider', providerVersion: '1.0.0', domainContract,
  }).status, 'selected');
  assert.equal(selectOperatingMetricProviderV2(undefined, {
    providerId: 'open-reference-metric-provider', providerVersion: '1.0.0', domainContract,
  }).status, 'selected');
  assert.equal(selectOperatingVerificationProviderV2(undefined, {
    providerId: 'open-reference-verification-provider', providerVersion: '1.0.0', domainContract,
  }).status, 'selected');
  assert.deepEqual(selectOperatingSnapshotProviderV2(undefined, {
    providerId: 'unknown-provider', providerVersion: '1.0.0', domainContract,
  }).error.code, 'OPERATING_PROVIDER_UNAVAILABLE');
  assert.equal(selectOperatingSnapshotProviderV2(undefined, {
...
test('reference providers validate and return normal candidates without mutating accepted inputs', () => {
  ...
  const snapshot = createOperatingSnapshotCandidateV2({
    providerId: 'open-reference-snapshot-provider', providerVersion: '1.0.0',
    snapshot: input.snapshot, state: input.state, acceptedArtifacts: [input.artifact],
  });
  const metric = createOperatingMetricObservationCandidateV2({
    providerId: 'open-reference-metric-provider', providerVersion: '1.0.0', metric: input.metric,
    observation: input.observation, snapshot: input.snapshot, acceptedArtifacts: [input.artifact],
  });
  const verification = createOperatingVerificationCandidateV2({
    ...
    verificationPlan: input.verificationPlan, outcome: input.outcome,
  ...
  assert.doesNotMatch(JSON.stringify([snapshot, metric, verification]), /raw|bytes|credential|secret|connector|executor/iu);
});
...
test('candidate validation fails closed on an unaccepted source, altered plan binding, or missing explicit version', () => {
  ...
  assert.throws(() => createOperatingMetricObservationCandidateV2({
    providerId: 'open-reference-metric-provider', providerVersion: '1.0.0', metric: input.metric,
    observation: { ...input.observation, sourceArtifactId: 'art_missing_002' }, snapshot: input.snapshot,
    acceptedArtifacts: [input.artifact],
  }), { code: 'OPERATING_PROVIDER_INPUT_INVALID' });
  assert.throws(() => createOperatingVerificationCandidateV2({
    providerId: 'open-reference-verification-provider', providerVersion: '1.0.0', verificationPlan: input.verificationPlan,
    outcome: { ...input.outcome, verificationPlanId: 'vfy_different_002' },
    acceptedArtifacts: [input.artifact, input.verificationArtifact],
  }), { code: 'OPERATING_PROVIDER_INPUT_INVALID' });
  assert.throws(() => selectOperatingSnapshotProviderV2(undefined, {
    providerId: 'open-reference-snapshot-provider', domainContract: input.snapshot.domainContract,
