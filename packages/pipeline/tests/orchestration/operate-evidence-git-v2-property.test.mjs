...
import { OPEN_REFERENCE_EVIDENCE_REGISTRY_V2, dispatchOperateEvidenceResolverV2 } from 'planr-pipeline/operate/evidence-v2';
...
test('Git resolver never converts an invalid range or unrequested ancestry into another resolver outcome', () => {
  ...
  try {
    ...
    for (let end = 4; end <= 128; end += 7) {
      ...
      const result = dispatchOperateEvidenceResolverV2(OPEN_REFERENCE_EVIDENCE_REGISTRY_V2, candidate, {
        scope: valid.scope,
    ...
    assert.equal(dispatchOperateEvidenceResolverV2(OPEN_REFERENCE_EVIDENCE_REGISTRY_V2, noAncestry, {
      scope: valid.scope,
      capabilities: ['evidence.git.read'],
      gitRepositories: [{
        ...valid.repository,
