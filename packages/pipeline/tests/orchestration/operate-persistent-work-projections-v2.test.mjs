...
function cycle(cycleId, state = 'closed') {
  return {
    kind: 'operating-cycle', schemaVersion: '1.0.0', protocolVersion: '2.0.0', cycleId, ...scope,
    state, inputBindingId: `inb_${cycleId.slice(4)}`, contractVersions: {}, trigger: { kind: 'manual' },
    focus: ['retention'], health: 'normal', activeReviewId: null, createdAt: TIME, updatedAt: TIME,
...
function state() {
  return {
    ...
    findings: [{
      ...clone(valid['operating-finding']), findingId: 'fnd_00000001', ...scope,
      sourceCycleId: 'cyc_00000001', sourceArtifactId: 'art_00000001', title: 'Retention risk', statement: 'Retention is declining.',
      state: 'deferred', ownerActorId: 'owner-001', revisitAt: null, createdAt: TIME, updatedAt: TIME,
    }],
    decisions: [{
      ...clone(valid['operating-decision']), decisionId: 'dec_00000001', ...scope,
      sourceCycleId: 'cyc_00000001', sourceArtifactId: 'art_00000001', title: 'Prioritize retention', question: 'What should we prioritize?',
      outcome: null, rationale: 'It is the largest current risk.', state: 'deferred', ownerActorId: 'owner-001', revisitAt: null,
      revision: 1, predecessorDecisionId: null, createdAt: TIME, updatedAt: TIME,
    }],
    actions: [{
      ...clone(valid['operating-action']), actionId: 'act_00000001', ...scope,
      sourceCycleId: 'cyc_00000001', sourceArtifactId: 'art_00000001', title: 'Interview customers', state: 'deferred',
      ownerActorId: 'owner-001', accountabilityDisposition: null, sourceDecisionId: 'dec_00000001', sourceFindingIds: ['fnd_00000001'],
      dependsOnActionIds: [], createdAt: TIME, updatedAt: TIME,
    }],
    reviews: [{
      kind: 'operating-review', schemaVersion: '1.0.0', protocolVersion: '2.0.0', reviewId: 'rev_00000002', cycleId: 'cyc_00000002',
      ownerActorId: 'owner-001', state: 'approved', disposition: 'approved', createdAt: TIME, updatedAt: TIME,
      workDispositions: [
...
test('scope ledger retains durable work and reconstructs source, touch, and carry-forward links deterministically', () => {
  ...
  assert.ok(first.cycleLinks.some((link) => link.relation === 'source' && link.cycleId === 'cyc_00000001'));
  assert.equal(first.cycleLinks.filter((link) => link.relation === 'carried-forward' && link.cycleId === 'cyc_00000002').length, 3);
  assert.equal(before.findings[0].state, 'deferred');
  assert.equal(first.cycleLinks.filter((link) => (
    link.relation === 'carried-forward' && link.cycleId === 'cyc_00000001'
  )).length, 0, 'a closed source Cycle cannot invent carry-forward without owned execution verification');
});
...
test('execution verification projection is scope-safe, order-stable, and retains later-cycle provenance', () => {
  const approvedAction = clone(JSON.parse(readFileSync(
  ...
  const verificationPlan = {
    ...
    window: action.verificationWindow, method: 'Compare the accepted observation with the target.',
  ...
  const contracts = JSON.parse(readFileSync(
    new URL('../../conformance/fixtures/operating-runtime-v2/governed-execution-contracts-valid.json', import.meta.url),
    'utf8',
  ));
  const result = {
  ...
  const sourceCycle = {
    ...clone(valid['operating-cycle']), cycleId: action.sourceCycleId,
    scopeId: action.scopeId, domainId: action.domainId, domainVersion: action.domainVersion,
    state: 'verifying', activeReviewId: null,
  };
  const laterCycle = {
    ...clone(sourceCycle), cycleId: 'cyc_projection_later_0001', inputBindingId: 'inb_projection_later_0001',
    state: 'advising', createdAt: result.completedAt, updatedAt: result.completedAt,
  };
  const assignment = buildOperatingTerminalVerificationAssignmentV2({
    action, cycle: sourceCycle, operation, result, verificationPlan, timestamp: result.completedAt,
  });
  const verification = JSON.parse(readFileSync(
    new URL('../../conformance/fixtures/operating-runtime-v2/action-verification-valid.json', import.meta.url),
    'utf8',
  ));
  const outcome = {
    ...clone(verification.outcome), actionId: action.actionId,
    scopeId: action.scopeId, domainId: action.domainId, domainVersion: action.domainVersion,
    verificationPlanId: verificationPlan.verificationPlanId, status: 'succeeded',
  };
  const learning = {
    ...clone(verification.learning), outcomeId: outcome.outcomeId,
    scopeId: action.scopeId, domainId: action.domainId, domainVersion: action.domainVersion,
    sourceArtifactId: outcome.sourceArtifactId, evidenceRefIds: [...outcome.evidenceRefIds],
  };
  const snapshot = {
    ...clone(valid['operating-snapshot']), snapshotId: 'snp_projection_later_0001',
    scopeId: action.scopeId, domainId: action.domainId, domainVersion: action.domainVersion,
    domainContract: { apiDomainId: action.domainId, id: `${action.domainId}-domain`, version: action.domainVersion },
    stateId: 'oms_projection_later_0001', createdAt: '2026-08-10T13:00:00Z',
  };
  const delta = {
    ...clone(valid['operating-delta']), deltaId: 'dlt_projection_later_0001',
    scopeId: action.scopeId, domainId: action.domainId, domainVersion: action.domainVersion,
    currentSnapshotId: snapshot.snapshotId, decisionRevisitIds: [action.sourceDecisionId],
    derivedAt: '2026-08-10T13:01:00Z',
  };
  const foreignSnapshot = {
    ...clone(snapshot), snapshotId: 'snp_projection_foreign_0001', domainId: 'business',
    domainContract: { apiDomainId: 'business', id: 'business-domain', version: action.domainVersion },
    createdAt: '2026-08-10T14:00:00Z',
  };
  const foreignDelta = {
    ...clone(delta), deltaId: 'dlt_projection_foreign_0001', domainId: 'business',
    currentSnapshotId: foreignSnapshot.snapshotId, derivedAt: '2026-08-10T14:01:00Z',
  };
  const input = {
    action, verificationPlan, operations: [operation], executionResults: [result],
    verificationAssignments: [assignment], outcomes: [outcome], learnings: [learning],
    deltas: [foreignDelta, delta], snapshots: [foreignSnapshot, snapshot],
    sourceCycle, cycle: laterCycle,
  };
  ...
  const reordered = derivePersistentOperatingExecutionVerificationProjectionV2({
    ...input, deltas: [...input.deltas].reverse(), snapshots: [...input.snapshots].reverse(),
  });
  ...
  assert.throws(() => derivePersistentOperatingExecutionVerificationProjectionV2({
    ...input,
    executionResults: [{ ...result, action: { ...result.action, revision: result.action.revision + 1 } }],
  ...
  for (const verificationAssignments of [[forged, assignment], [assignment, forged]]) {
    assert.throws(() => derivePersistentOperatingExecutionVerificationProjectionV2({
      ...input,
  ...
  assert.throws(() => derivePersistentOperatingExecutionVerificationProjectionV2({
    ...input,
    verificationAssignments: [{ ...clone(assignment), objective: 'Forged verification ownership.' }],
  }), (error) => error?.code === 'RESULT_CONTRACT_INVALID');
});
