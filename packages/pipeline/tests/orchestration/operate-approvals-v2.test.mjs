...
function authority(mode = 'named-single-party') {
  ...
  const selectedPolicy = policy(value, {
    tier: 'domain',
    decisionMode: mode,
    approvalRequirementIds: [requirementId],
  });
  ...
  const evaluation = evaluateOperatingActionPolicyV2({
    action: value,
    configuredPolicies: [corePolicy, selectedPolicy],
    evaluatedAt: TIME,
  });
  const parties =
    mode === 'named-single-party'
      ? [
          {
            partyId: 'owner-party',
            actorKind: 'human',
  ...
  return {
    action: value,
    policies: [corePolicy, selectedPolicy],
...
test('single-party approval binds exact Action/policy/target/actor scope and replay is harmless', () => {
  ...
  const first = appendOperatingApprovalRecordV2({
    records: [],
    record: approval,
    requirement: state.requirement,
  });
  const replay = appendOperatingApprovalRecordV2({
  ...
  assert.deepEqual(
    ...
    }),
    {
      ...
      requirementIds: [state.requirement.requirementId],
      reasonCode: 'approval-quorum-complete',
    },
  );
...
test('canonical requirement integrity rejects schema-valid forged parties, cardinality, and derived actor sets', () => {
  ...
  assert.doesNotThrow(() =>
    assertProtocolArtifact('operating-approval-requirement', forgedSingle, {
      protocolVersion: '2.0.0',
    }),
  );
  assert.throws(
    () => assertOperatingApprovalRequirementIntegrityV2(forgedSingle),
    ({ code }) => code === 'APPROVAL_INVALID',
  );
  assert.throws(
    () =>
      createOperatingApprovalRecordV2({
        approvalId: 'aprv_outsider01',
        requirement: forgedSingle,
  ...
  const thresholdTooSmall = rehashRequirement({
  ...
  for (const forged of [
    ...
    wrongNamedActors,
    wrongActorKinds,
    forgedDuplicateParty,
  ]) {
    assert.doesNotThrow(() =>
      assertProtocolArtifact('operating-approval-requirement', forged, {
        protocolVersion: '2.0.0',
      }),
    );
    assert.throws(
      () => assertOperatingApprovalRequirementIntegrityV2(forged),
      ({ code }) => code === 'APPROVAL_INVALID',
    );
  }
  ...
  assert.doesNotThrow(() =>
    assertProtocolArtifact('operating-approval-requirement', forgedWildcardThreshold, {
      protocolVersion: '2.0.0',
    }),
  );
  assert.throws(
    () => assertOperatingApprovalRequirementIntegrityV2(forgedWildcardThreshold),
    ({ code }) => code === 'APPROVAL_INVALID',
  );
});
...
test('threshold and named multi-party quorum reject partial, duplicate actor, stale, and consumed authority', () => {
  ...
  const consumed = consumeOperatingApprovalRecordsV2({
    approvals: [first, second],
    requirements: [state.requirement],
    approvalIds: [first.approvalId, second.approvalId],
    operationId: 'op_approval001',
  });
  assert.equal(
    consumed.history.every(({ consumedByOperationId }) => consumedByOperationId === null),
    true,
  );
  assert.equal(
    consumed.records.every(
      ({ consumedByOperationId }) => consumedByOperationId === 'op_approval001',
    ),
    true,
  );
  assert.throws(
...
test('actor-to-party uniqueness is global across requirements and non-consumable authority cannot be consumed', () => {
  ...
  const selectedPolicy = policy(value, {
    ...
    approvalRequirementIds: templateIds,
  });
  const evaluation = evaluateOperatingActionPolicyV2({
    action: value,
    configuredPolicies: [corePolicy, selectedPolicy],
  ...
  const duplicateActorRequirements = templateIds.map((policyRequirementId, index) =>
    createOperatingApprovalRequirementV2({
      policyRequirementId,
      evaluation,
      action: value,
  ...
  assert.throws(
    () =>
      evaluateOperatingApprovalSetV2({
        evaluation,
        action: value,
        requirements: duplicateActorRequirements,
        approvals: [],
        now: LATER,
      }),
    ({ code }) => code === 'APPROVAL_INVALID',
  ...
  const approval = createOperatingApprovalRecordV2({
    approvalId: 'aprv_noconsume1',
    requirement: nonConsumable,
    evaluation: single.evaluation,
    action: single.action,
    partyId: single.parties[0].partyId,
    actor: {
      kind: single.parties[0].actorKind,
      actorId: single.parties[0].actorId,
  ...
  assert.throws(
    () =>
      consumeOperatingApprovalRecordsV2({
        approvals: [approval],
        requirements: [widenedRequirement],
        approvalIds: [approval.approvalId],
        operationId: 'op_noconsume1',
      }),
    ({ code }) => code === 'APPROVAL_INVALID',
...
test('re-evaluation supersedes only prior authority while preserving history', () => {
  ...
  const partition = partitionSupersededOperatingAuthorityV2({
    ...
    requirements: [state.requirement],
    approvals: [approval],
    currentEvaluationId: nextEvaluation.evaluationId,
  });
...
test('Action-scoped Review binds one exact Action revision and is separate from Cycle review authority', () => {
  ...
  assert.deepEqual(review.subject, {
    ...
    actionHash: state.action.actionHash,
  });
  assert.equal(review.cycleId, state.action.sourceCycleId);
  assert.doesNotThrow(() =>
    assertProtocolArtifact('operating-review', review, { protocolVersion: '2.0.0' }),
  );
});
