...
const businessDomain = {
  ...
  ...structuredClone(OPERATE_CONTRACT_CATALOG_V2.extensions.domains.find(({ domainId }) => (
    domainId === 'business'
  ))),
};
...
test('executive board projection binds stable role identity and typed absence for business plans', () => {
  ...
  const indexes = {
    assignments: new Map(board.assignments.map((assignment) => [assignment.assignmentId, assignment])),
    artifacts: new Map(),
    claims: new Map(),
    findings: new Map(),
...
test('chair synthesis merges plan omissions with persisted terminal advisor absence gaps', () => {
  ...
  const chairAssignment = board.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  ...
  const gapRoleIds = executiveBoard.chairSynthesis.unresolvedGaps.map(({ roleId }) => roleId).sort();
...
test('executive board projection preserves durable Findings, structured decisions, typed evidence gaps, and 4096-character dissent', () => {
  ...
  const cycle = base.result.state.cycles[0];
  const advisorAssignment = board.assignments.find(({ assignmentKind }) => assignmentKind === 'advisor');
  const challengerAssignment = board.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const chairAssignment = board.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  const artifactFor = (assignment, artifactId, schemaId) => ({
    ...
    schemaId,
    artifactSchemaVersion: '2.0.0',
    producer: { actorId: `${assignment.roleId}-agent`, roleId: assignment.roleId, runtime: 'codex' },
    inputArtifactIds: [...assignment.inputArtifactIds],
  });
  const advisorArtifact = artifactFor(advisorAssignment, 'art_exec_board_advisor_001', 'operating-advisor-result');
  const challengerArtifact = artifactFor(challengerAssignment, 'art_exec_board_challenger_001', 'operating-challenger-review');
  const chairArtifact = artifactFor(chairAssignment, 'art_exec_board_chair_001', 'operating-decision-ledger');
  ...
  const finding = {
    ...
    findingId: 'fnd_exec_board_structured_001', scopeId: cycle.scopeId, domainId: 'business',
    domainVersion: cycle.domainVersion, sourceCycleId: cycle.cycleId,
    sourceArtifactId: challengerArtifact.artifactId, sourceAssignmentId: challengerAssignment.assignmentId,
    sourceLocalFindingId: localFindingId, findingType: 'unsupported', severity: 'high', confidence: 0.8,
    targets: [{
      advisorArtifactId: advisorArtifact.artifactId,
      analysisIds: [], claimIds: [localClaimId], measurementIds: [], riskIds: [], recommendationIds: [],
    }],
    supportingEvidenceRefIds: [], contradictingEvidenceRefIds: [base.evidenceRef.evidenceRefId],
    title: 'Evidence custody is incomplete', statement: 'The recommendation exceeds the available observation window.',
    ...
    state: 'open', ownerActorId: null, revisitAt: null,
    createdAt: '2026-08-09T12:04:00.000Z', updatedAt: '2026-08-09T12:04:00.000Z',
  };
  const ledger = {
    kind: 'operating-decision-ledger', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    assignmentId: chairAssignment.assignmentId, roleId: chairAssignment.roleId,
    roleVersion: chairAssignment.roleVersion, analysisProfile: structuredClone(chairAssignment.analysisProfile),
    ledgerId: deriveOperatingChairLedgerIdV2(chairAssignment.assignmentId),
    scopeId: cycle.scopeId, domainId: 'business', domainVersion: cycle.domainVersion,
    snapshotId: base.result.snapshot.snapshotId, intelligencePlanId: board.plan.planId,
    summary: 'Proceed only with the explicitly reversible bounded change.',
    advisorArtifactIds: [advisorArtifact.artifactId], challengerArtifactId: challengerArtifact.artifactId,
    advisorAbsenceGaps: [],
    evidenceGaps: [{
      absenceId: 'abs_evidence_exec_board_001', kind: 'evidence', requirementId: 'current-operating-evidence',
      evidenceKinds: ['operate-artifact'], absenceCode: 'stale',
      ...
      recoveryDisposition: 'refresh-or-issue-evidence', sourceEvidenceRefIds: [base.evidenceRef.evidenceRefId],
      sourceEventIds: [],
    }],
    decisions: [{
      localDecisionId: `decision:${chairAssignment.assignmentId}:1`,
      title: 'Run a bounded experiment', question: 'Which reversible move should be tested next?',
      outcome: 'Run the bounded experiment.', rationale: 'It preserves optionality while collecting decisive evidence.',
      sourceClaimRefs: [{ advisorArtifactId: advisorArtifact.artifactId, localClaimId }],
      sourceRecommendationRefs: [], challengerFindingIds: [localFindingId],
      ...
      alternativeDispositions: [{
        ...
        title: 'Wait for passive evidence', disposition: 'deferred',
        rationale: 'Passive evidence would arrive too slowly for the bounded decision window.',
      }],
      confidence: 0.72, assumptionIds: [base.assumption.assumptionId],
      upside: 'The experiment can validate the highest-value path quickly.',
      downside: 'The experiment consumes one delivery window.',
      uncertainty: 'Customer response remains uncertain.', reversibility: 'The experiment can be stopped without migration.',
      ownerActorId: 'owner-board-projection-001', revisitConditions: ['Revisit when the observation window closes.'],
      dissentIds: [localDissentId],
      actionHypotheses: [],
    }],
    sourceDispositions: [],
    dissent: [{
      sourceArtifactId: challengerArtifact.artifactId, localDissentId,
      findingIds: [localFindingId], statement: dissentStatement,
      evidenceRefIds: [base.evidenceRef.evidenceRefId],
  ...
  const indexes = {
    assignments: new Map(board.assignments.map((assignment) => [assignment.assignmentId, assignment])),
    artifacts: new Map([advisorArtifact, challengerArtifact, chairArtifact].map((artifact) => [artifact.artifactId, artifact])),
    claims: new Map(),
  ...
  const executiveBoard = buildExecutiveBoardForCycle({
    indexes, cycle,
    scope: { scopeId: cycle.scopeId, domainId: 'business', domainVersion: cycle.domainVersion },
    screen: screen(), intelligencePlan: board.plan, lensAbsences: [], assignmentAbsence: () => null,
  });
  ...
  assert.equal(executiveBoard.chairSynthesis.decisions[0].expectedDownside, ledger.decisions[0].downside);
  assert.equal(executiveBoard.chairSynthesis.decisions[0].alternativeDispositions[0].disposition, 'deferred');
