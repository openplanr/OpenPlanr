...
function policy(
  ...
  return createOperatingActionPolicyV2({
    policyId: `${tier}-action-policy`,
    policyVersion: '1.0.0',
    domainId: value.domainId,
    actionKind: value.actionKind,
    capability: value.requestedCapability,
    effectClasses,
    targetKinds,
    decisionMode,
    approvalRequirementIds,
    ...
    tier,
    provenance: {
      providerId: `${tier}-policy-provider`,
...
test('versioned policy evaluation is deterministic and applies core, project, then narrowing domain policy', () => {
  ...
  const policies = [
    policy(value, {
      ...
      effectClasses: ['project-write', 'read-only'],
      targetKinds: ['project-record', 'project-view'],
    }),
    policy(value, {
      tier: 'project',
      decisionMode: 'named-single-party',
      approvalRequirementIds: ['aprq_project01'],
    }),
    policy(value, {
      tier: 'domain',
      decisionMode: 'threshold',
      approvalRequirementIds: ['aprq_domain001'],
    }),
  ];
  const input = {
    action: value,
  ...
  const second = evaluateOperatingActionPolicyV2({
    ...clone(input),
    configuredPolicies: [...clone(policies)].reverse(),
  });
  assert.deepEqual(second, first);
  assert.equal(first.outcome, 'threshold');
  assert.deepEqual(
    first.appliedPolicyRefs.map(({ policyId }) => policyId),
  ...
  assert.doesNotThrow(() =>
    assertProtocolArtifact('operating-policy-evaluation', first, { protocolVersion: '2.0.0' }),
  );
});
...
test('lower policy cannot weaken, relabel, omit, or broaden higher policy authority', () => {
  ...
  const core = policy(value, {
    tier: 'core',
    decisionMode: 'named-single-party',
    approvalRequirementIds: ['aprq_core0001'],
  });
  ...
  assert.throws(
    () =>
      evaluateOperatingActionPolicyV2({
        action: value,
        configuredPolicies: [core, widened],
        evaluatedAt: '2026-08-10T08:00:00Z',
      }),
  ...
  const omitted = evaluateOperatingActionPolicyV2({
    action: value,
    configuredPolicies: [core, effective],
    evaluatedAt: '2026-08-10T08:00:00Z',
  });
  assert.throws(
    () =>
      assertOperatingPolicyEvaluationV2(omitted, {
        action: value,
        configuredPolicies: [core, project, effective],
      }),
    ({ code }) => code === 'POLICY_EVALUATION_REJECTED',
  );
});
...
test('configured applicability requires core and rejects ambiguous or omitted applicable tiers', () => {
  ...
  const omitted = evaluateOperatingActionPolicyV2({
    action: value,
    configuredPolicies: [core, domain],
    evaluatedAt: '2026-08-10T08:00:00Z',
  });
  assert.throws(
    () =>
      assertOperatingPolicyEvaluationV2(omitted, {
        action: value,
        configuredPolicies: [core, project, domain],
      }),
    ({ code }) => code === 'POLICY_EVALUATION_REJECTED',
  );
  const competingDomain = createOperatingActionPolicyV2({
    ...clone(domain),
    policyId: 'second-domain-policy',
  });
  assert.throws(
    () =>
      evaluateOperatingActionPolicyV2({
        action: value,
        configuredPolicies: [core, domain, competingDomain],
        evaluatedAt: '2026-08-10T08:00:00Z',
      }),
    ({ code }) => code === 'POLICY_EVALUATION_REJECTED',
  );
...
test('reference hard prohibitions are non-overridable and durable-safe', () => {
  for (const [field, identifier, effectClass] of [
    ...
    const core = policy(value, {
      ...
      targetKinds: [value.targetBinding.kind],
    });
    const evaluation = evaluateOperatingActionPolicyV2({
      action: value,
      configuredPolicies: [core],
      evaluatedAt: '2026-08-10T08:00:00Z',
    });
