...
function runningBinarySubmissionState() {
  ...
  appendRuntimeEvent(
    ...
    'asg_00000001',
    releasePayload([binaryAssignment], 'asg_00000001'),
    'evt-direct-003',
  );
  appendRuntimeEvent(
    events,
  ...
  if (nextState === 'running')
    return {
      attemptPolicy: {
        ...
        attempt: Math.min(current.attemptPolicy.maxAttempts, current.attemptPolicy.attempt + 1),
      },
    };
...
test('INV-WF-001: 2,000 seeded random legal walks preserve a forward or recovery path', () => {
  for (const [state, nextStates] of Object.entries(OPERATING_ASSIGNMENT_TRANSITIONS_V2)) {
    if (!['validated', 'abandoned', 'failed'].includes(state))
      assert.ok(nextStates.length > 0, state);
  }
...
test('every disallowed state edge fails with one code and leaves the source hash unchanged', () => {
  ...
  for (const from of states) {
    ...
    if (['running', 'submitted', 'rejected', 'validated'].includes(from))
      source.attemptPolicy = { ...source.attemptPolicy, attempt: 1 };
...
test('O2-P1-001: 256 seeded Event sequences preserve durable replay identity across restart', () => {
  for (let seed = 1; seed <= 256; seed += 1) {
    ...
    assert.equal(
      ...
      `seed ${seed}: same Event is a no-op replay`,
    );
    ...
    const conflicting = createOperatingRuntimeEventV2(
      {
        ...
        entityId: 'inb_00000001',
        actor: { kind: 'engine', id: 'openplanr' },
        causationId: checkpoint.eventReplayIndex.at(-1).eventId,
        correlationId: `conflict-${seed}`,
        payload: inputBoundPayload(),
      },
      {
        previousEvent: {
          sequence: checkpoint.eventHead.sequence,
          eventHash: checkpoint.eventHead.hash,
        },
      },
    );
    assert.throws(
    ...
    assert.equal(
      sha256Jcs(checkpoint),
      before,
      `seed ${seed}: conflict leaves checkpoint immutable`,
    );
  }
});
...
test('O2-P1-002: 256 seeded zero-dependency releases are pending-only, evented once, and restart-safe', () => {
  for (let seed = 1; seed <= 256; seed += 1) {
    ...
    const release = createOperatingRuntimeEventV2(
      {
        eventId: `evt-release-${seed}-003`,
        timestamp: TIME,
        cycleId: 'cyc_00000001',
        type: 'assignment.available',
        entityId: 'asg_00000001',
        actor: { kind: 'runtime', id: 'openplanr' },
        causationId: `evt-release-${seed}-002`,
    ...
    assert.equal(
      checkpoint.assignments[0].state,
      'available',
      `seed ${seed}: release reaches available`,
    );
    assert.equal(
      checkpoint.eventReplayIndex.filter(({ eventId }) => eventId === release.eventId).length,
      1,
    ...
    assert.equal(
      sha256Jcs(
        reduceOperatingRuntimeEventsV2([structuredClone(release)], { initialState: restarted }),
      ),
      sha256Jcs(checkpoint),
    ...
    assert.throws(
      () =>
        createOperatingRuntimeEventV2(
          {
            eventId: `evt-release-${seed}-invalid`,
            timestamp: TIME,
            cycleId: 'cyc_00000001',
            type: 'assignment.created',
            entityId: `asg_invalid_${String(seed).padStart(8, '0')}`,
            actor: { kind: 'runtime', id: 'openplanr' },
            causationId: checkpoint.eventReplayIndex.at(-1).eventId,
            correlationId: `corr-release-invalid-${seed}`,
            payload: {
              ...assignment(),
              assignmentId: `asg_invalid_${String(seed).padStart(8, '0')}`,
              state: 'available',
              availableAt: TIME,
            },
          },
    ...
    assert.equal(
      sha256Jcs(checkpoint),
      beforeInvalidCreation,
      `seed ${seed}: rejected creation leaves checkpoint unchanged`,
    );
  }
});
...
test('OP-01/07 property: 128 binary submissions preserve exact bytes, replay once, and reject changed-byte retries', () => {
  for (let seed = 1; seed <= 128; seed += 1) {
    ...
    const bytes = Buffer.from(
      Array.from({ length: 1 + Math.floor(random() * 64) }, () => Math.floor(random() * 256)),
    );
    ...
    const result = acceptOperatingAssignmentSubmissionV2(
      request,
      {
        ...
        correlationId: `corr-prop-${seed}`,
        eventIds: {
          submitted: `evt-prop-${seed}-006`,
          artifactCreated: `evt-prop-${seed}-007`,
          validated: `evt-prop-${seed}-008`,
        },
      },
      { initialState },
    );
    ...
    assert.equal(
      result.artifact.canonicalHash,
      null,
      `seed ${seed}: binary has no canonical substitution`,
    );
    const replay = acceptOperatingAssignmentSubmissionV2(
      request,
      {
        ...
        timestamp: TIME,
        validatorVersion: '1.0.0',
        correlationId: `corr-replay-${seed}`,
        eventIds: {
          submitted: `evt-replay-${seed}-006`,
          artifactCreated: `evt-replay-${seed}-007`,
          validated: `evt-replay-${seed}-008`,
        },
      },
    ...
    assert.throws(
      () =>
        acceptOperatingAssignmentSubmissionV2(
          { ...request, contentBase64: changed.toString('base64') },
          {
            ...
            timestamp: TIME,
            validatorVersion: '1.0.0',
            correlationId: `corr-conflict-${seed}`,
            eventIds: {
              submitted: `evt-conflict-${seed}-006`,
              artifactCreated: `evt-conflict-${seed}-007`,
              validated: `evt-conflict-${seed}-008`,
            },
          },
          { initialState: result.state },
        ),
...
test('submit guard/action equivalence exhaustively rejects valid-ID and contract-shaped mismatches', () => {
  ...
  assert.equal(
    deriveOperateAllowedActionsV2(base).some(({ tool }) => tool === 'operate.assignment.submit'),
    true,
  );
  ...
  assert.doesNotThrow(() =>
    assertProtocolArtifact('operating-submission', crossCycleSubmission, {
      protocolVersion: '2.0.0',
    }),
  );
  const cases = [
    [
      ...
      { submitRequest: { ...request, assignmentId: 'asg_00000002' } },
      'SUBMISSION_ID_CONFLICT',
    ],
    [
      'alternate submission ID',
      { submitRequest: { ...request, submissionId: 'sub_00000002' } },
      'SUBMISSION_ID_CONFLICT',
    ],
    [
    ...
    [
      'media mismatch',
      { submitRequest: { ...request, mediaType: 'text/plain' } },
      'RESULT_CONTRACT_INVALID',
    ],
    [
      'encoding mismatch',
      { submitRequest: { ...request, encoding: 'binary' } },
      'RESULT_CONTRACT_INVALID',
    ],
    [
      'malformed payload',
      { submitRequest: { ...request, contentBase64: '%%%%' } },
      'RESULT_CONTRACT_INVALID',
    ],
    [
      'oversized payload',
  ...
  for (const [label, overrides, code] of cases) {
    ...
    assert.equal(
      evaluateOperateGuardV2('operate.assignment.submit', context).error.code,
      code,
      label,
    );
    assert.equal(
  ...
  for (let index = 0; index < 1000; index += 1) {
    ...
    const context = {
      ...
      assignment: {
        ...
        availableAt: available ? TIME : null,
      },
    ...
    for (const action of actions)
      assert.equal(evaluateOperateGuardV2(action.tool, context).allowed, true);
...
test('governed Action capability strings alone never create an executable action', () => {
  for (const kind of ['agent', 'human', 'engine']) {
    ...
    assert.deepEqual(
      deriveOperateAllowedActionsV2(context).filter(({ tool }) =>
        tool.startsWith('operate.action.'),
      ),
      [],
      kind,
    );
...
test('rollback Event/Artifact history is an immutable reducer fixed point across restart clones', async () => {
  const scenario = await governedRollbackScenario({
    executeSuffix: '79999991',
    rollbackSuffix: '79999992',
  });
  ...
  for (let seed = 1; seed <= 32; seed += 1) {
    ...
    const rollbackReplay = restarted.operationReplayIndex.find(
      ({ operationKind }) => operationKind === 'rollback',
    );
    assert.equal(
      rollbackReplay.terminalResultId,
      restarted.rollbackResults[0].rollbackResultId,
      `seed ${seed}`,
    );
  }
});
