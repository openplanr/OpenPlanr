...
function acceptedArtifact(seed, timestamp = TIME) {
  ...
  const artifact = {
    ...clone(valid['operating-artifact']), artifactId: `art_snapshot_${seed}`, assignmentId,
    rawHash: rawHash(bytes), canonicalHash: null, sizeBytes: bytes.byteLength, inputArtifactIds: [], createdAt: timestamp,
  };
  const assignment = {
    ...clone(valid['operating-assignment']), assignmentId, state: 'validated', availableAt: timestamp, completedAt: timestamp,
    inputArtifactIds: [...artifact.inputArtifactIds],
    claim: { actorId: 'agent-001', actorKind: 'agent', runtime: 'codex', claimId: `claim_snapshot_${seed}` },
    attemptPolicy: { ...valid['operating-assignment'].attemptPolicy, attempt: 1 },
  };
  const proof = bindAcceptedSubmissionProof({
    ...clone(valid['operating-submission']), submissionId: `sub_snapshot_${seed}`, assignmentId,
    artifactId: artifact.artifactId, rawHash: artifact.rawHash, canonicalHash: null, sizeBytes: bytes.byteLength,
...
function collections(sourceArtifactId, timestamp) {
  const source = (kind) => ({ ...clone(valid[kind]), sourceArtifactId, createdAt: timestamp, updatedAt: timestamp });
  return {
    objectives: [source('operating-objective')], metrics: [source('operating-metric')],
    findings: [], decisions: [], actions: [], risks: [source('operating-risk')], assumptions: [source('operating-assumption')],
  };
}
...
test('snapshots retain only safe IDs/hashes, bind typed EvidenceRefs, and deterministically link to the prior scope snapshot', () => {
  ...
  const evidenceBytes = Buffer.from('opaque evidence capture that must remain in the byte store', 'utf8');
  const evidenceArtifact = {
    ...clone(valid['operating-artifact']), artifactId: 'art_snapshot_evidence_001', assignmentId: sourceOne.assignment.assignmentId,
    artifactType: 'evidence-snapshot', schemaId: 'operating-evidence-snapshot', artifactSchemaVersion: '1.0.0',
    mediaType: 'application/octet-stream', encoding: 'binary', rawHash: rawHash(evidenceBytes), canonicalHash: null,
    sizeBytes: evidenceBytes.byteLength, inputArtifactIds: [sourceOne.artifact.artifactId], createdAt: TIME,
  };
  const evidenceRef = {
    ...clone(valid['operating-evidence-ref']), evidenceRefId: 'evr_snapshot_001', candidateId: 'evc_snapshot_001',
    sourceArtifactId: sourceOne.artifact.artifactId, evidenceArtifactId: evidenceArtifact.artifactId,
    evidenceArtifactRawHash: evidenceArtifact.rawHash, evidenceArtifactCanonicalHash: null, resolvedAt: TIME,
  };
  ...
  const base = {
    ...
    assignments: [sourceOne.assignment], submissions: [sourceOne.submission],
    artifacts: [sourceOne.artifact, evidenceArtifact], evidenceRefs: [evidenceRef],
  ...
  const first = materializeOperatingStateSnapshotV2({
    ...
    sourceArtifactIds: [sourceOne.artifact.artifactId], evidenceRefIds: [evidenceRef.evidenceRefId],
    sourceRevisions: [{ sourceArtifactId: sourceOne.artifact.artifactId, revision: 'revision-001', evidenceRefIds: [evidenceRef.evidenceRefId] }],
    collections: collections(sourceOne.artifact.artifactId, TIME),
  }, {
    snapshotId: 'snp_snapshot_001', stateId: 'oms_snapshot_001', timestamp: TIME, correlationId: 'corr_snapshot_001',
  ...
  assert.doesNotMatch(JSON.stringify(first), /opaque snapshot source|opaque evidence capture/, 'state, snapshot, and Events contain no byte payload');
  ...
  assert.throws(() => materializeOperatingStateSnapshotV2({
    ...
    scope: { scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0' },
    domainContract: { apiDomainId: 'business', id: 'business-domain', version: '1.0.0' },
    sourceArtifactIds: [sourceTwo.artifact.artifactId], sourceRevisions: [{ sourceArtifactId: sourceTwo.artifact.artifactId, revision: 'revision-unlisted-evidence' }],
    collections: undeclaredEvidenceCollections,
  }, {
    snapshotId: 'snp_snapshot_unlisted_evidence', stateId: 'oms_snapshot_unlisted_evidence', timestamp: NEXT_TIME,
    correlationId: 'corr_snapshot_unlisted_evidence',
    eventIds: { snapshot: 'evt_snapshot_unlisted_evidence', state: 'evt_state_unlisted_evidence' },
  }, { initialState: nextBase, artifactStore: store }), { code: 'STATE_TRANSITION_INVALID' });
  const second = materializeOperatingStateSnapshotV2({
    cycleId: nextBase.cycles[0].cycleId,
    scope: { scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0' },
    domainContract: { apiDomainId: 'business', id: 'business-domain', version: '1.0.0' },
    sourceArtifactIds: [sourceTwo.artifact.artifactId], sourceRevisions: [{ sourceArtifactId: sourceTwo.artifact.artifactId, revision: 'revision-002' }],
    collections: collections(sourceTwo.artifact.artifactId, NEXT_TIME),
  }, {
    snapshotId: 'snp_snapshot_002', stateId: 'oms_snapshot_002', timestamp: NEXT_TIME, correlationId: 'corr_snapshot_002',
