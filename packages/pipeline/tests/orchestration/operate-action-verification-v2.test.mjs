...
function eventAfter(state, eventId, type, entityId, payload, timestamp, correlationId) {
  return createOperatingRuntimeEventV2({
    eventId, timestamp, cycleId: state.cycles[0].cycleId, type, entityId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId: latestEventId(state), correlationId, payload,
...
function claimAndStart(state, assignmentId, submissionId, suffix, timestamp) {
  ...
  const claimed = eventAfter(state, `evt_verify_${suffix}_claimed`, 'assignment.claimed', assignmentId, {
    assignmentId, actorId: `${assignment.roleId}-verify-001`, actorKind: 'agent', runtime: 'codex',
    claimId: `claim_verify_${suffix}`, submissionId,
  }, timestamp, correlationId);
  const afterClaim = reduceOperatingRuntimeEventsV2([claimed], { initialState: state });
  const started = eventAfter(afterClaim, `evt_verify_${suffix}_started`, 'assignment.started', assignmentId, {
    assignmentId, attempt: 1,
  }, timestamp, correlationId);
  return reduceOperatingRuntimeEventsV2([started], { initialState: afterClaim });
}

function acceptRole(state, assignmentId, submissionId, artifactId, artifactType, output, suffix, timestamp, artifactStore) {
  const assignment = state.assignments.find((candidate) => candidate.assignmentId === assignmentId);
  ...
  return acceptOperatingAssignmentSubmissionV2({
    ...
    mediaType: 'application/json', encoding: 'utf-8', contentBase64: bytes.toString('base64'),
  }, {
    artifactId, artifactType, inputArtifactIds: [...assignment.inputArtifactIds], timestamp,
    validatorVersion: '2.0.0', correlationId: `corr_verify_${suffix}`,
    eventIds: {
      submitted: `evt_verify_${suffix}_submitted`, artifactCreated: `evt_verify_${suffix}_artifact`,
      validated: `evt_verify_${suffix}_validated`,
    },
  }, { initialState: state, artifactStore }).state;
}
...
function issuedResultInputs(state, assignmentId, artifactStore) {
  ...
  return assignment.inputArtifactIds.flatMap((artifactId) => {
    ...
    if (!artifact || ![
      'operating-intelligence-input-bundle', 'operating-advisor-result',
      'operating-challenger-review', 'operating-decision-ledger',
    ].includes(artifact.schemaId)) return [];
    const rawBytes = readOperatingArtifactRawBytesV2(artifactStore, {
      artifactId,
      rawHash: artifact.rawHash,
    });
    return [{ artifactId, schemaId: artifact.schemaId, value: JSON.parse(Buffer.from(rawBytes).toString('utf8')) }];
  });
}

function advisorEvidenceLink(assignment) {
  const evidenceRefId = assignment.intelligenceContext.inputBundle.issuedEvidence[0]?.evidenceRefId ?? null;
...
function advisorResult(assignment, assumptionId, recommended) {
  ...
  value.claims = [{
    ...
    statement: 'A bounded retention measurement reduces the risk of changing allocation on a weak signal.',
...
function verificationCheckpoint() {
  ...
  const objective = {
    ...clone(valid['operating-objective']), objectiveId: 'obj_verify_001', metricIds: [metric.metricId],
    sourceArtifactId, evidenceRefIds: [evidenceRefId], createdAt: SNAPSHOT_TIME, updatedAt: SNAPSHOT_TIME,
  };
  const finding = {
    ...clone(valid['operating-finding']), findingId: 'fnd_verify_001', sourceCycleId: cycleId, sourceArtifactId,
    origin: 'persistent-work',
    createdAt: SNAPSHOT_TIME, updatedAt: SNAPSHOT_TIME,
  };
  for (const field of [
    'sourceAssignmentId', 'sourceLocalFindingId', 'findingType', 'severity', 'confidence',
    'targets', 'supportingEvidenceRefIds', 'contradictingEvidenceRefIds', 'rationale', 'correctionCondition',
  ...
  const snap = materializeOperatingStateSnapshotV2({
    ...
    sourceArtifactIds: [sourceArtifactId], evidenceRefIds: [evidenceRefId],
    sourceRevisions: [{ sourceArtifactId, revision: 'r2', evidenceRefIds: [evidenceRefId] }],
    collections: {
      objectives: [objective], metrics: [metric], findings: [finding], decisions: [seedDecision], actions: [], risks: [], assumptions: [assumption],
    },
  }, {
    snapshotId: 'snp_verify_001', stateId: 'oms_verify_001', timestamp: SNAPSHOT_TIME,
    correlationId: 'corr_verify_snapshot_001', eventIds: { snapshot: 'evt_verify_snapshot_001', state: 'evt_verify_state_001' },
  }, { initialState: base.result.state, artifactStore: base.store });
  return { base, snap, cycleId, sourceArtifactId, evidenceRefId, metric, objective, finding, assumption };
}

function acceptedLedgerInput({ mutateLedger } = {}) {
  const seeded = verificationCheckpoint();
  const delta = deriveOperatingRuntimeDeltaV2({
    cycleId: seeded.cycleId, snapshotId: seeded.snap.snapshot.snapshotId, stateId: seeded.snap.operatingState.stateId,
  }, {
    deltaId: 'dlt_verify_001', eventId: 'evt_verify_delta_001', timestamp: PLAN_TIME,
    correlationId: 'corr_verify_delta_001',
  }, { initialState: seeded.snap.state });
  const planned = planOperatingRuntimeIntelligenceBoardV2({
    cycleId: seeded.cycleId, snapshotId: seeded.snap.snapshot.snapshotId, stateId: seeded.snap.operatingState.stateId,
    deltaId: delta.delta.deltaId, focus: ['all'], domainDescriptor: fixture('business-domain-valid.json'),
    decisionOwnerActorId: 'owner-verify-001',
  }, {
    eventId: 'evt_verify_plan_001', timestamp: PLAN_TIME, correlationId: 'corr_verify_plan_001',
  ...
  for (const [index, advisor] of advisorAssignments.entries()) {
    ...
    state = claimAndStart(state, advisor.assignmentId, `sub_verify_${suffix}`, suffix, ADVISOR_TIME);
    const running = state.assignments.find(({ assignmentId }) => assignmentId === advisor.assignmentId);
    const output = advisorResult(running, seeded.assumption.assumptionId, index === 0);
    state = acceptRole(state, advisor.assignmentId, `sub_verify_${suffix}`, advisorArtifactId,
      'advisor-result', output, suffix, ADVISOR_TIME, seeded.base.store);
  ...
  state = claimAndStart(state, challenger.assignmentId, 'sub_verify_challenger_001', 'challenger', CHALLENGER_TIME);
  const runningChallenger = state.assignments.find(({ assignmentId }) => assignmentId === challenger.assignmentId);
  const challengerOutput = clone(createOperatingResultTemplateV2({
  ...
  challengerOutput.summary = 'The bounded recommendation is reversible, but its downside must remain explicit.';
  challengerOutput.analysisMarkdown = 'The challenge targets the exact accepted Advisor sources.';
  challengerOutput.inputAbsenceIds = runningChallenger.inputAbsences.map(({ absenceId }) => absenceId);
  challengerOutput.findings = [{
    localFindingId: findingId,
    title: 'Weak-signal downside',
    statement: 'The recommendation must not turn one bounded observation into an execution claim.',
    ...
    contradictingEvidenceRefIds: [],
    rationale: 'The source claim supports observation but not an irreversible allocation commitment.',
  ...
  challengerOutput.questionCoverage = challengerOutput.questionCoverage.map((coverage) => ({
    ...
    alternativeIds: coverage.questionId === 'challenge-missing-alternative'
  ...
  state = acceptRole(state, challenger.assignmentId, 'sub_verify_challenger_001', challengerArtifactId,
    'challenger-review', challengerOutput, 'challenger', CHALLENGER_TIME, seeded.base.store);
  state = claimAndStart(state, chair.assignmentId, 'sub_verify_chair_001', 'chair', CHAIR_TIME);
  const runningChair = state.assignments.find(({ assignmentId }) => assignmentId === chair.assignmentId);
  const ledger = clone(createOperatingResultTemplateV2({
  ...
  ledger.summary = 'Measure retention before changing allocation while preserving an observation-only boundary.';
  ledger.decisions = [{
    localDecisionId,
    title: 'Measure retention before changing allocation',
    ...
    actionHypotheses: [{
      ...
      verificationMethod: 'Compare the accepted cohort retention observation with the declared target.',
      sourceClaimRefs: [claimRef],
  ...
  ledger.questionCoverage = ledger.questionCoverage.map((coverage) => {
    if (coverage.questionId === 'synthesis-limiting-gaps' && runningChair.inputAbsences.length > 0) {
      return {
        ...coverage,
        disposition: 'gap',
  ...
  mutateLedger?.(ledger);
  state = acceptRole(state, chair.assignmentId, 'sub_verify_chair_001', chairArtifactId,
    'chair-result', ledger, 'chair', CHAIR_TIME, seeded.base.store);
  const ledgerResult = materializeOperatingDecisionLedgerV2({
    cycleId: seeded.cycleId, snapshotId: seeded.snap.snapshot.snapshotId, stateId: seeded.snap.operatingState.stateId,
    intelligencePlanId: planned.plan.planId, advisorArtifactIds: [...advisorArtifactIds], challengerArtifactId, chairArtifactId,
  }, {
...
function materialize(input) {
  return materializeOperatingActionVerificationV2({
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId, ledgerId: input.ledger.ledgerId,
  }, {
    eventIds: ['evt_verify_action_plan_001'], timestamp: ACTION_TIME, correlationId: 'corr_verify_action_plan_001',
  }, { initialState: input.state, artifactStore: input.base.store, replayHook: createNoModelReplayHookV2() });
}
...
test('a complete byte-proven Chair hypothesis atomically creates a distinct Action and observation-only verification plan', () => {
  ...
  assert.deepEqual(result.events.map(({ type }) => type), ['verification.plan-recorded']);
  const [action] = result.actions;
  const [plan] = result.verificationPlans;
  assert.equal(result.state.assignments.length, input.state.assignments.length, 'Actions never become Assignments');
  assert.equal(action.sourceDecisionId, result.state.decisions.find(({ sourceArtifactId }) => sourceArtifactId === input.chairArtifactId).decisionId);
  ...
  for (const forbiddenField of ['assignmentId', 'operationId', 'capability', 'approval', 'policy']) {
    ...
    assert.equal(Object.hasOwn(plan, forbiddenField), false, `verification plan has no ${forbiddenField}`);
  }
  const replay = materializeOperatingActionVerificationV2({
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId, ledgerId: input.ledger.ledgerId,
  }, {
    eventIds: ['evt_verify_action_plan_001'], timestamp: ACTION_TIME, correlationId: 'corr_verify_action_plan_001',
  }, { initialState: result.state, artifactStore: input.base.store, replayHook: createNoModelReplayHookV2() });
...
test('incomplete, foreign, duplicated, or placeholder Action hypotheses fail Chair preflight', () => {
  ...
  const cases = [
    (action) => { action.objectiveId = null; },
    (action) => { action.objectiveId = 'obj_foreign_objective_0001'; },
    (action) => { action.metricId = null; },
    (action) => { action.metricId = 'met_foreign_metric_0001'; },
    (action) => { action.baseline = null; },
    (action) => { action.target = null; },
    (action) => { action.sourceFindingIds = []; },
    (action) => { action.expectedResult = ' leading whitespace'; },
    (action) => { action.verificationWindow = 'trailing whitespace '; },
    (action) => { action.verificationMethod = '\nleading newline'; },
    (action) => { action.title = '<action-title>'; },
    (action) => { action.accountabilityDisposition = 'blocked'; },
    (action) => { action.sourceClaimRefs.push(clone(action.sourceClaimRefs[0])); },
    (_action, decision) => { decision.sourceClaimRefs.push(clone(decision.sourceClaimRefs[0])); },
  ];
...
test('verification-plan Action reconstruction proves the exact canonical ledger chain and rejects every substitution', () => {
  ...
  const rejects = (mutateState, mutateEvent = () => {}) => {
    ...
    assert.throws(() => reconstructOperatingVerificationPlanActionV2({
  ...
  rejects((state) => {
    state.eventReplayIndex.push(clone(
      state.eventReplayIndex.find((entry) => entry.eventId === event.eventId),
    ));
  });
  rejects(() => {}, (_state) => {
  ...
  rejects((state) => {
    ...
    embedded.metrics.push(clone(embedded.metrics.find(({ metricId }) => metricId === action.metricId)));
  });
  rejects((state) => {
    const embedded = state.operatingModelStates.find(
      (candidate) => candidate.stateId === input.snap.operatingState.stateId,
...
test('an accepted observation records a derived outcome and learning, and a later Delta reopens the source Decision without claiming execution', () => {
  ...
  const observation = {
    ...clone(valid['operating-metric-observation']), observationId: 'mob_verify_001', metricId: action.metricId,
    scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0', value: 0.85, unit: input.metric.unit,
    observedAt: OBSERVATION_TIME, snapshotId: input.snap.snapshot.snapshotId,
    evidenceRefIds: [input.evidenceRefId], sourceArtifactId: input.sourceArtifactId,
  };
  const observed = recordOperatingIntelligenceStateV2({
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId,
    claims: [], metricObservations: [observation], risks: [], assumptions: [], decisionRevisions: [],
  }, {
    timestamp: OBSERVATION_TIME, correlationId: 'corr_verify_observation_001',
    eventIds: { claims: [], metricObservations: ['evt_verify_observation_001'], risks: [], assumptions: [], decisionRevisions: [] },
  }, { initialState: actions.state });
  const verificationRequest = {
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId,
    actionId: action.actionId, verificationPlanId: plan.verificationPlanId, observationId: observation.observationId,
    learning: { statement: 'The measured cohort reached the stated target.', assumptionIds: [], decisionIds: [action.sourceDecisionId] },
  };
  const verificationDraft = {
    timestamp: OUTCOME_TIME, correlationId: 'corr_verify_outcome_001',
    eventIds: { outcome: 'evt_verify_outcome_001', learning: 'evt_verify_learning_001' },
  };
  const result = recordOperatingActionVerificationOutcomeV2(verificationRequest, verificationDraft, {
    initialState: observed.state, replayHook: createNoModelReplayHookV2(),
  });
  ...
  assert.throws(() => reduceOperatingRuntimeEventsV2([result.events[0]], {
    initialState: observed.state,
  }), { code: 'STATE_TRANSITION_INVALID' }, 'Outcome cannot be reduced without its paired Learning');
  const ackLostRetry = recordOperatingActionVerificationOutcomeV2(verificationRequest, verificationDraft, {
    initialState: result.state, replayHook: createNoModelReplayHookV2(),
  });
  assert.equal(ackLostRetry.replayed, true, 'a lost acknowledgement returns the durable Outcome/Learning pair');
  assert.deepEqual(ackLostRetry.events, []);
  assert.deepEqual(ackLostRetry.state, result.state);
  assert.throws(() => recordOperatingActionVerificationOutcomeV2({
    ...verificationRequest,
    learning: { ...verificationRequest.learning, statement: 'A divergent retry cannot replace the recorded learning.' },
  }, {
    ...verificationDraft,
    eventIds: { outcome: 'evt_verify_outcome_divergent_001', learning: 'evt_verify_learning_divergent_001' },
  }, { initialState: result.state, replayHook: createNoModelReplayHookV2() }), { code: 'CONCURRENT_MODIFICATION' });
  const delta = deriveOperatingRuntimeDeltaV2({
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId,
  }, {
    deltaId: 'dlt_verify_after_outcome_001', eventId: 'evt_verify_delta_after_outcome_001', timestamp: '2026-08-09T13:09:00.000Z',
...
test('forged plans, assignment coercion, malformed source Findings, and outcome replay divergence fail closed without partial records', () => {
  const input = acceptedLedgerInput();
  const before = sha256Jcs(input.state);
  assert.throws(() => materializeOperatingActionVerificationV2({
    cycleId: input.cycleId, snapshotId: input.snap.snapshot.snapshotId, stateId: input.snap.operatingState.stateId,
    ledgerId: input.ledger.ledgerId, assignmentId: 'asg_coercion_001',
  }, {
    eventIds: ['evt_verify_assignment_coercion_001'], timestamp: ACTION_TIME, correlationId: 'corr_verify_assignment_coercion_001',
  }, { initialState: input.state, artifactStore: input.base.store }), { code: 'RESULT_CONTRACT_INVALID' });
  assert.equal(sha256Jcs(input.state), before);
  ...
  const forgedEvent = createOperatingRuntimeEventV2({
    eventId: 'evt_verify_forged_plan_001', timestamp: ACTION_TIME, cycleId: input.cycleId,
    type: 'verification.plan-recorded', entityId: forged.payload.verificationPlanId,
    actor: { kind: 'runtime', id: 'openplanr' }, causationId: forged.causationId, correlationId: 'corr_verify_forged_plan_001', payload: forged.payload,
  }, { previousEvent: { sequence: input.state.eventHead.sequence, eventHash: input.state.eventHead.hash } });
  assert.throws(() => reduceOperatingRuntimeEventsV2([forgedEvent], {
    initialState: input.state, artifactStore: input.base.store,
  }), { code: 'STATE_TRANSITION_INVALID' });
  assert.equal(sha256Jcs(input.state), before);

  assert.throws(() => acceptedLedgerInput({
    mutateLedger: (ledger) => { ledger.decisions[0].actionHypotheses[0].sourceFindingIds = []; },
  }), { code: 'RESULT_CONTRACT_INVALID' }, 'malformed Action source custody fails before Chair Artifact acceptance');
});
