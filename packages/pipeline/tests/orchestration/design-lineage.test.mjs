...
function approvedHandoff() {
  const value = {
    ...
    requirements: [
      { id: 'REQ-001', kind: 'behavior', statement: 'Confirm the order.', sourceRefs: ['checkout-screen'], verification: ['Confirm action succeeds.'] },
      { id: 'REQ-002', kind: 'accessibility', statement: 'Name the total.', sourceRefs: ['billing-screen'], verification: ['Total has an accessible name.'] },
    ],
...
function planInput() {
  ...
  return {
    ...
    specId: 'SPEC-014',
    mappings: [
      { requirementId: 'REQ-001', acceptanceRefs: [{ storyId: 'US-045', acceptanceId: 'AC-013' }], taskIds: ['T-046'] },
      { requirementId: 'REQ-002', acceptanceRefs: [{ storyId: 'US-045', acceptanceId: 'AC-014' }], taskIds: ['T-047'] },
    ],
    stories: [{ id: 'US-045', acceptanceCriteria: [{ id: 'AC-013' }, { id: 'AC-014' }] }],
    tasks: [
      { id: 'T-046', storyId: 'US-045', acceptanceRefs: ['AC-013'], testRequirements: 'Prove AC-013 in browser verification.' },
      { id: 'T-047', storyId: 'US-045', acceptanceRefs: ['AC-014'], testRequirements: 'Prove AC-014 with accessibility checks.' },
    ],
  };
}
...
test('Plan composes closed requirement, acceptance, and task lineage', () => {
  ...
  assert.deepEqual(lineage.mappings.map(({ requirementId }) => requirementId), ['REQ-001', 'REQ-002']);
  assert.throws(() => composeDesignPlanningLineage({ ...input, mappings: input.mappings.slice(0, 1) }), /every implementation requirement/u);
  assert.throws(() => composeDesignPlanningLineage({
    ...input,
...
test('Plan writes generated artifacts and lineage atomically after validation', () => {
  ...
  try {
    const input = { ...planInput(), artifacts: [{ path: 'stories/US-045.md', bytes: '# Story\n' }] };
    ...
    assert.throws(() => writeDesignPlanningArtifacts(join(root, 'invalid'), {
      ...input,
...
test('a failed multi-file commit restores the exact prior state', () => {
  ...
  try {
    ...
    assert.throws(() => writeDesignPlanningArtifacts(root, {
      ...planInput(),
      artifacts: [
        { path: 'stories/US-045.md', bytes: '# New story\n' },
        { path: 'tasks/T-046.md', bytes: '# New task\n' },
      ],
    }, { replace: true, fs: { renameSync: failingRename } }), /simulated storage interruption/u);
...
test('Ship resolves only the selected task requirement and its exact source references', () => {
  ...
  try {
    ...
    writeFileSync(join(root, 'design/implementation-handoff/current.json'), JSON.stringify({
      id: input.handoff.id,
    ...
    assert.deepEqual(context.requirements.map(({ id }) => id), ['REQ-001']);
    assert.deepEqual(context.sources.map(({ id }) => id), ['checkout-screen']);
    assert.equal(resolveDesignPlanningLineage({ root, lineage: null, taskIds: ['T-046'] }).status, 'absent');
    writeFileSync(join(root, 'design/implementation-handoff/current.json'), JSON.stringify({
      id: input.handoff.id, version: 3, contentDigest: fixed('f'), status: 'approved',
    }));
    assert.equal(resolveDesignPlanningLineage({ root, lineage, taskIds: ['T-046'] }).status, 'stale');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
