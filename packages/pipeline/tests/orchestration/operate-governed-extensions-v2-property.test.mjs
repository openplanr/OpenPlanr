...
test('registry canonicalization and exact selection are deterministic across bounded input permutations', () => {
  ...
  for (let seed = 0; seed < 64; seed += 1) {
    ...
    assert.equal(
      sha256Jcs(selectOperateExecutorV2(registry, clone(baseCriteria))),
      sha256Jcs(
        selectOperateExecutorV2(OPEN_REFERENCE_OPERATE_GOVERNED_EXTENSIONS_V2, clone(baseCriteria)),
      ),
      `seed ${seed}: selection`,
    );
  }
...
test('bounded incompatibility and ceiling mutations always return explicit unavailable without substitution', () => {
  for (const vector of invalid.vectors.filter(({ kind }) => kind === 'executor-selection')) {
    ...
    assert.deepEqual(
      result.fallback,
      { kind: 'unavailable', errorCode: 'EXECUTOR_UNAVAILABLE' },
      vector.name,
    );
...
test('closed registration input rejects executable values and ambient identity without invoking them', () => {
  ...
  functionRegistration.executors[0].factory = () => {
    calls += 1;
  };
...
test('reserved identities and host connector proofs remain immutable across bounded hostile variants', () => {
  ...
  for (let seed = 0; seed < 64; seed += 1) {
    ...
    const target = [
      input.capabilityProviders[0],
      input.policyProviders[0],
      input.executors[seed % input.executors.length],
    ][seed % 3];
    ...
    assert.throws(
      () => createTrustedExecutorBindingV2({ selection, trustedHost: forgedHost }),
      {
...
test('reserved identities and prohibited registration semantics survive bounded version and spelling permutations', () => {
  ...
  for (let seed = 0; seed < 72; seed += 1) {
    ...
    assert.throws(
      () => createOperateGovernedExtensionRegistryV2(reserved),
      {
        code: 'E_EXTENSION_REGISTRATION_CONFLICT',
      },
    ...
    assert.throws(
      () => createOperateGovernedExtensionRegistryV2(prohibited),
      {
        code: 'E_EXTENSION_REGISTRATION_INVALID',
      },
...
test('executor envelope fingerprint rejects bounded payload, baseline, target, and prototype divergence', () => {
  const operation = fixture('governed-execution-contracts-valid.json')[
  ...
  for (let seed = 0; seed < 64; seed += 1) {
    ...
    if (seed % 4 === 1)
      divergent.rollbackBaseline.contentHash = `sha256:${(seed % 16).toString(16).repeat(64)}`;
    if (seed % 4 === 2) divergent.operation.target.id = `substitute-${seed}`;
    if (seed % 4 === 3)
      divergent.payload = Object.assign(Object.create({ inherited: true }), divergent.payload);
    assert.throws(
      () => assertContainedExecutorInputEnvelopeV2(divergent, { operation }),
      {
