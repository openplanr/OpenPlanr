...
function claimAssignment(state, assignmentId, suffix, timestamp, artifactStore) {
  ...
  const claimed = claimOperatingAssignmentV2({ assignmentId, actor }, {
  ...
  const inputArtifacts = claimed.response.assignment.inputArtifactIds.flatMap((artifactId) => {
    const artifact = claimed.state.artifacts.find((candidate) => candidate.artifactId === artifactId);
    assert.ok(artifact, `missing issued input Artifact ${artifactId}`);
    const rawBytes = readOperatingArtifactRawBytesV2(artifactStore, { artifactId, rawHash: artifact.rawHash });
    const rawText = Buffer.from(rawBytes).toString('utf8');
    if (!['operating-intelligence-input-bundle', 'operating-advisor-result', 'operating-challenger-review',
      'operating-decision-ledger'].includes(artifact.schemaId)) return [];
    return [{
      artifactId,
...
function evidenceLink(assignment) {
  ...
  assert.ok(evidenceRefId || absenceId, `Assignment ${assignment.assignmentId} has neither evidence nor a typed absence`);
  return {
...
function answerQuietChallengerCoverage(value) {
  ...
  value.questionCoverage = value.questionCoverage.map((coverage) => gapIds.length > 0
    ? {
        ...coverage,
        disposition: 'gap',
    ...
    : {
        ...
        justification: 'The accepted board contains no material recommendation or claim requiring this critique.',
      });
}
...
function answerQuietChairCoverage(value, assignment) {
  const absenceIds = assignment.inputAbsences.map(({ absenceId }) => absenceId);
  value.questionCoverage = value.questionCoverage.map((coverage) => absenceIds.length > 0
    ? {
        ...coverage,
        disposition: 'gap',
    ...
    : {
        ...
        justification: 'The accepted board contains only evidence-limited outcomes and no material recommendation.',
      });
}
...
function authoredAdvisorResult(assignment, {
  ...
  value.analysisMarkdown = 'The result preserves exact evidence custody and states its uncertainty explicitly.';
  ...
  value.claims = [{
    ...
    statement: 'A fresh retention measurement reduces the downside of changing allocation prematurely.',
  ...
  value.risks = [{
    ...
    likelihood: 0.5,
    impact: 'high',
    exposure: 'The operating scope may spend the next observation window on the wrong constraint.',
  ...
  if (withMeasurement) {
    assert.ok(link.evidenceRefIds.length > 0, 'typed measurement requires one exact issued EvidenceRef');
    value.measurements = [{
...
function authoredQuietAdvisorResult(assignment) {
  ...
  assert.ok(absence, `Quiet Advisor ${assignment.assignmentId} requires one exact evidence absence`);
  value.outcome = 'partial';
  value.summary = 'The issued evidence gaps do not support a distinct material recommendation.';
  value.analysisMarkdown = 'The Advisor preserves the exact gap custody and avoids inferring from unavailable evidence.';
  for (const answer of value.analysis.executiveQuestionAnswers) {
    answer.answer = 'The required evidence is absent, so this question remains explicitly unresolved.';
...
function authoredChallengerResult(assignment, inputArtifacts, advisorArtifactId, advisorOutput) {
  ...
  value.analysisMarkdown = 'The challenge targets the exact Advisor claim, risk, and recommendation.';
  value.inputAbsenceIds = assignment.inputAbsences.map(({ absenceId }) => absenceId);
  value.findings = [{
    localFindingId: findingId,
    title: 'Unpriced delay downside',
    statement: 'The recommendation does not fully price the opportunity cost of delaying acquisition learning.',
    ...
    supportingEvidenceRefIds: [...link.evidenceRefIds],
    contradictingEvidenceRefIds: [],
    rationale: 'The source claim supports measurement but does not quantify the delayed learning cost.',
  ...
  value.dissent = [{
    ...
    findingIds: [findingId],
    statement: 'Do not approve a measurement-only path unless the acquisition-learning delay is explicitly bounded.',
    evidenceRefIds: [...link.evidenceRefIds],
    resolutionCondition: 'Bound and price the acquisition-learning delay in the accepted decision.',
  }];
  value.gaps = value.gaps.map((gap) => ({
    ...gap,
...
function authoredQuietChallengerResult(assignment, inputArtifacts) {
  ...
  value.analysisMarkdown = 'The review preserves the exact predecessor and gap custody without inventing a second analysis.';
  value.gaps = value.gaps.map((gap) => ({
    ...gap,
...
function authoredChairLedger(assignment, inputArtifacts, advisorArtifactId, advisorOutput, challengerArtifactId, challengerOutput) {
  const value = structuredClone(createOperatingResultTemplateV2({ assignment, inputArtifacts }));
  ...
  value.summary = 'Approve a bounded retention measurement while explicitly limiting acquisition-learning delay.';
  value.decisions = [{
    localDecisionId: decisionId,
    title: 'Run a bounded retention measurement',
    question: 'How should the operating scope reduce retention uncertainty without hiding acquisition-learning delay?',
    outcome: 'Run one bounded retention measurement and cap the acquisition-learning delay to one window.',
    rationale: 'The Advisor claim supports a reversible measurement and the Challenger finding prices its downside.',
    ...
    alternativeDispositions: advisorOutput.alternatives.length === 0 ? [] : [{
      ...
      rationale: 'Holding all allocation is less informative than the bounded measurement.',
    }],
    confidence: 0.66,
    assumptionIds: [],
...
function authoredQuietChairLedger(assignment, inputArtifacts) {
  ...
  value.sourceDispositions = value.sourceDispositions.map((disposition) => ({
    ...disposition,
    rationale: 'The evidence-limited Advisor outcome is retained without inventing a recommendation.',
  }));
...
function acceptedBoard(seed = 'structured', {
  ...
  const delta = deriveOperatingRuntimeDeltaV2({ cycleId, snapshotId, stateId }, {
    deltaId: `dlt_${seed}`, eventId: `evt_${seed}_delta`, timestamp: DELTA_TIME,
    correlationId: `corr_${seed}_delta`,
  }, { initialState: base.result.state });
  const planned = planOperatingRuntimeIntelligenceBoardV2({
    cycleId, snapshotId, stateId, deltaId: delta.delta.deltaId, focus: ['all'],
    domainDescriptor: fixture('business-domain-valid.json'),
    decisionOwnerActorId: 'owner-ledger-001',
  }, {
    eventId: `evt_${seed}_plan`, timestamp: PLAN_TIME, correlationId: `corr_${seed}_plan`,
  ...
  for (const [position, plannedAssignment] of advisors.entries()) {
    ...
    const claimed = claimAssignment(state, plannedAssignment.assignmentId, suffix, ADVISOR_TIME, base.store);
    ...
    const artifactId = reverseAdvisorArtifactIds
      ...
      : `art_${suffix}`;
    const accepted = acceptResult(claimed, output, artifactId, 'advisor-result', suffix, ADVISOR_TIME, base.store);
  ...
  const challengerAssignment = state.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const challengerClaimed = claimAssignment(
    state, challengerAssignment.assignmentId, `${seed}_challenger`, CHALLENGER_TIME, base.store,
  );
  events.push(...challengerClaimed.events);
  const challengerInputs = protocolOrderedAdvisorInputs(
    planned.plan, advisorRecords, challengerClaimed.inputArtifacts,
  );
  ...
  const chairAssignment = state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  const chairClaimed = claimAssignment(state, chairAssignment.assignmentId, `${seed}_chair`, CHAIR_TIME, base.store);
  events.push(...chairClaimed.events);
  const chairInputs = protocolOrderedAdvisorInputs(planned.plan, advisorRecords, chairClaimed.inputArtifacts);
  ...
  const chairAccepted = acceptResult(
    chairClaimed, ledger, chairArtifactId, 'chair-result', `${seed}_chair`, CHAIR_TIME, base.store,
  );
  events.push(...chairAccepted.events);
  state = chairAccepted.state;
  return {
    base, cycleId, snapshotId, stateId, plan: planned.plan, state, events, replayBaseState,
    advisorRecords, challengerArtifactId, challengerOutput, chairArtifactId, ledger,
  };
}
...
test('accepted Advisor projection preserves typed measurements and exact result identity', () => {
  ...
  const bundleArtifact = board.state.artifacts.find(({ artifactId }) => artifactId === bundleArtifactId);
  const inputBundle = JSON.parse(Buffer.from(readOperatingArtifactRawBytesV2(board.base.store, {
...
test('Advisor evidence absences reach Challenger and Chair byte-identically through release and restart', () => {
  ...
  const advisor = board.advisorRecords.find(({ assignment }) => assignment.inputAbsences.some(({ kind }) => kind === 'evidence'));
  ...
  const challenger = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const chair = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  const exactAt = (assignment) => assignment.inputAbsences.find(({ absenceId }) => absenceId === sourceAbsence.absenceId);
...
test('public Chair template preserves absence-source Evidence metadata without treating it as a citation', () => {
  ...
  assert.ok(
    board.ledger.evidenceGaps.some(({ sourceEvidenceRefIds }) => sourceEvidenceRefIds.includes(evidenceRefId)),
  ...
  const chairAssignment = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  ...
  const absenceOnlyAdvisor = board.advisorRecords.find(({ assignment }) => (
    assignment.inputAbsences.some(({ sourceEvidenceRefIds }) => sourceEvidenceRefIds.includes(evidenceRefId))
      && !assignment.intelligenceContext.inputBundle.issuedEvidence.some((entry) => entry.evidenceRefId === evidenceRefId)
  ));
  assert.ok(absenceOnlyAdvisor, 'fixture must expose an Advisor with absence metadata but no citation custody');
...
test('Decision-ledger materialization preserves protocol plan order instead of Artifact lexical order', () => {
  ...
  const expectedPlanOrder = board.plan.selectedRoles
    .filter(({ roleKind }) => roleKind === 'advisor')
    .map(({ roleId, roleVersion }) => board.advisorRecords.find(({ assignment }) => (
      assignment.roleId === roleId && assignment.roleVersion === roleVersion
    ))?.artifactId);
  ...
  const projectedFinding = experience.cycles.find(({ cycleId }) => cycleId === board.cycleId)
    ?.executiveBoard?.challengerFindings?.findings.find(({ findingId }) => (
...
test('every advertised Review choice remains valid for an unowned materialized Finding', () => {
  ...
  assert.equal(sourceFinding.ownerActorId, null, 'Challenger materialization must not invent a human owner');
  ...
  const approvedChoices = read.dispositionChoices.filter(({ submitArguments }) => (
    submitArguments.disposition === 'approved'
  ));
  assert.deepEqual(approvedChoices.map(({ submitArguments }) => (
    submitArguments.workDispositions.find(({ entityId }) => entityId === sourceFinding.findingId)?.disposition
  )), ['accepted', 'deferred', 'rejected']);
  assert.deepEqual(read.dispositionChoices.map(({ submitArguments }) => submitArguments.disposition), [
  ...
  for (const [position, choice] of read.dispositionChoices.entries()) {
    const expectedFindingDisposition = choice.submitArguments.workDispositions.find(
      ({ entityId }) => entityId === sourceFinding.findingId,
    )?.disposition ?? sourceFinding.state;
    const committed = submitOperatingReviewV2(choice.submitArguments, {
    ...
    const committedFinding = committed.state.findings.find(({ findingId }) => (
      findingId === sourceFinding.findingId
    ));
    assert.equal(committedFinding.state, expectedFindingDisposition);
    if (choice.submitArguments.disposition === 'approved' && expectedFindingDisposition === 'deferred') {
      ...
      assert.equal(committed.state.cycles.find(({ cycleId }) => cycleId === board.cycleId).state, 'awaiting_review');
    }
...
test('multiline source text remains byte-exact through atomic Claim, Risk, Finding, and Decision materialization', () => {
  ...
  assert.equal(result.decisions[0].alternativeDispositions[0].rationale, source.alternativeRationale);
...
test('prepared Assignment preflight returns multiple bounded JSON-pointer schema issues without emitting state', () => {
  ...
  assert.ok(result.issues.some(({ path, message }) => path === '/' && message.includes("'summary'")));
...
test('Challenger and Chair question coverage rejects reordered and foreign role-local references', () => {
  ...
  const cases = [
    {
      ...
      mutate(value) { value.questionCoverage.reverse(); },
  ...
  for (const { label, assignmentId, source, mutate, message } of cases) {
    ...
    assert.ok(result.issues.some((issue) => message.test(issue.message)), label);
  }
});
...
test('one unavailable Advisor becomes one exact Challenger role gap and forged gap kinds or references fail', () => {
  ...
  const missing = board.state.assignments.find((assignment) => (
    assignment.assignmentKind === 'advisor' && assignment.state === 'failed'
  ));
  assert.ok(missing);
  const challenger = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const roleAbsence = challenger.inputAbsences.find((absence) => (
    absence.kind === 'role' && absence.sourceAssignmentId === missing.assignmentId
  ));
  assert.ok(roleAbsence);
  const exactGap = board.challengerOutput.gaps.find(({ inputAbsenceId }) => inputAbsenceId === roleAbsence.absenceId);
  ...
  for (const mutate of [
    ...
    const index = forged.gaps.findIndex(({ inputAbsenceId }) => inputAbsenceId === roleAbsence.absenceId);
...
test('4097-character multiline Challenger dissent is rejected before Artifact or Event emission', () => {
  ...
  const submittedIndex = board.events.findIndex((event) => (
    event.type === 'assignment.submitted' && event.entityId === challengerAssignmentId
  ));
  ...
  }, hostile, 'art_long_dissent_rejected', 'challenger-review', 'long_dissent_rejected', CHALLENGER_TIME,
  board.base.store), { code: 'RESULT_CONTRACT_INVALID' });
  assert.equal(sha256Jcs(beforeSubmission), beforeHash);
  assert.equal(beforeSubmission.eventReplayIndex.length, beforeEventCount);
  assert.throws(() => board.base.store.readRaw({
...
test('source bounds and placeholder titles fail preflight before durable materialization', () => {
  ...
  const challenger = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'challenger');
  const chair = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  const preflight = (assignmentId, value) => preflightOperatingAssignmentResultV2({
    assignmentId,
...
test('Chair preflight bounds aggregate Action hypotheses to the exact owner Review capacity', () => {
  ...
  const chair = board.state.assignments.find(({ assignmentKind }) => assignmentKind === 'chair');
  const bundleArtifact = board.state.artifacts.find(({ artifactId }) => (
    artifactId === chair.intelligenceContext.inputBundle.bundleArtifactId
  ));
  const bundle = JSON.parse(Buffer.from(readOperatingArtifactRawBytesV2(board.base.store, {
  ...
  const preflight = (value) => preflightOperatingIntelligenceResultV2({
    value,
    assignment: chair,
    plan: board.plan,
    snapshot: board.base.result.snapshot,
    operatingState: board.base.result.operatingState,
    inputBundle: bundle,
    advisorOutputs: board.advisorRecords.map(({ artifactId, output }) => ({ artifactId, output })),
  ...
  assert.ok(overCapacity.issues.some(({ message, context }) => (
...
test('Chair accepts a grounded no-alternative recommendation and rejects a foreign alternative reference', () => {
  ...
  assert.deepEqual(
    experience.cycles.find(({ cycleId }) => cycleId === board.cycleId)
      .executiveBoard.chairSynthesis.decisions[0].alternativeDispositions,
    [],
  );
...
test('parsed or incomplete Chair source coverage has no public materialization route', () => {
  ...
  hostile.sourceDispositions = hostile.sourceDispositions.filter(({ sourceKind }) => sourceKind !== 'challenger-finding');
  assert.throws(() => materializeOperatingDecisionLedgerV2({
    cycleId: board.cycleId,
    snapshotId: board.snapshotId,
    stateId: board.stateId,
    intelligencePlanId: board.plan.planId,
  ...
  }, { initialState: board.state, artifactStore: board.base.store }), { code: 'RESULT_CONTRACT_INVALID' });
});
