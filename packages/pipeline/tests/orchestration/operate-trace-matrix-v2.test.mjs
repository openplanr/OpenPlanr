...
test('trace matrix exposes one reciprocal requirement-to-evidence-to-action path at one exact scope and Event head', () => {
  const result = matrix();
  const claim = result.nodes.find(({ locator, proofState }) => (
    locator.kind === 'claim' && proofState === 'supported'
  ));
  assert.ok(claim);
  const artifactEdge = result.edges.find((edge) => (
    edge.relation === 'contains' && edge.to.id === claim.locator.id
  ));
  const assignmentEdge = result.edges.find((edge) => (
    edge.relation === 'produces' && edge.to.id === artifactEdge.from.id
  ));
  const seatEdge = result.edges.find((edge) => (
    edge.relation === 'issued-as' && edge.to.id === assignmentEdge.from.id
  ));
  const requirementEdge = result.edges.find((edge) => (
    edge.relation === 'assigned-to' && edge.to.id === seatEdge.from.id
  ));
  const evidenceEdge = result.edges.find((edge) => (
    edge.relation === 'supported-by' && edge.from.id === claim.locator.id
  ));
  const decisionEdge = result.edges.find((edge) => (
    edge.relation === 'informs' && edge.from.id === claim.locator.id && edge.to.kind === 'decision'
  ));
  const actionEdge = result.edges.find((edge) => (
    edge.relation === 'proposes' && edge.from.id === decisionEdge.to.id
  ));
  assert.ok(requirementEdge);
  assert.ok(evidenceEdge);
  assert.ok(actionEdge);
  for (const edge of result.edges) {
    const from = result.nodes.find(({ locator }) => locator.kind === edge.from.kind && locator.id === edge.from.id);
    const to = result.nodes.find(({ locator }) => locator.kind === edge.to.kind && locator.id === edge.to.id);
  ...
  assert.equal(located.outbound.some(({ edgeId }) => edgeId === evidenceEdge.edgeId), true);
  assert.equal(located.inbound.some(({ edgeId }) => edgeId === artifactEdge.edgeId), true);
...
test('terminal seats do not promote stale or uncited material claims to verified proof', () => {
  ...
  assert.equal(uncitedMatrix.nodes.find(({ locator }) => locator.kind === 'claim').proofState, 'unverified');
  assert.equal(uncitedMatrix.proof.unverifiedClaims, 1);
  assert.equal(uncitedMatrix.proof.terminalAssignments >= uncited.intelligencePlans[0].selectedRoles.length, true);
...
test('restricted proof is retained as a typed omission without exposing reciprocal edges', () => {
  ...
  for (const node of restricted) {
    ...
    assert.ok(result.omissions.some((omission) => (
...
test('orphan resolutions and substituted evidence or accepted-output custody fail closed', () => {
  ...
  for (const mutate of [
    (state) => { state.artifacts.find(({ artifactId }) => artifactId === state.evidenceRefs[0].evidenceArtifactId).artifactType = 'advisor-result'; },
    (state) => { state.artifacts.find(({ artifactId }) => artifactId === state.evidenceRefs[0].evidenceArtifactId).inputArtifactIds = []; },
    (state) => { state.artifacts.find(({ artifactId }) => artifactId === state.claims[0].sourceArtifactId).schemaId = 'operating-challenger-review'; },
  ]) {
    const substituted = clone(sourceState);
    mutate(substituted);
    assert.throws(() => buildOperatingTraceMatrixV2(substituted, { cycleId: journey.cycleId, planId }), {
      code: 'E_OPERATE_TRACE_DANGLING',
    });
  }
...
test('verified Action proof requires the exact current Metric, plan, observation, snapshot, source, and Evidence chain', () => {
  ...
  assert.equal(verifiedMatrix.nodes.find(({ locator }) => locator.kind === 'outcome').proofState, 'verified');

  const wrongObservation = clone(verified);
  wrongObservation.outcomes[0].observationIds = ['mob_forged_observation_001'];
  assert.throws(() => buildOperatingTraceMatrixV2(wrongObservation, {
    cycleId: outcomeJourney.cycleId,
  ...
  assert.throws(() => buildOperatingTraceMatrixV2(wrongPlan, {
    cycleId: outcomeJourney.cycleId,
    planId: wrongPlan.intelligencePlans[0].planId,
  }), { code: 'E_OPERATE_TRACE_DANGLING' });

  const staleMetric = clone(verified);
  staleMetric.operatingModelStates
    .find(({ snapshotId }) => snapshotId === [...staleMetric.operatingSnapshots]
      .sort((left, right) => left.createdAt.localeCompare(right.createdAt)).at(-1).snapshotId)
  ...
  assert.equal(staleMatrix.nodes.find(({ locator }) => locator.kind === 'outcome').proofState, 'insufficient');
  assert.equal(staleMatrix.nodes.find(({ locator }) => locator.kind === 'action').proofState, 'insufficient');
});
...
test('closed trace assertion rejects rehashed schema, ordering, dangling, duplicate, cyclic, and oversized attacks', () => {
  ...
  const schemaAttacks = [
    ...
    (candidate) => { candidate.nodes[0].accessState = 'leaked'; },
    (candidate) => { candidate.omissions[0].reasonCode = 'TRUST_ME'; },
  ];
  for (const mutate of schemaAttacks) {
    const candidate = clone(valid);
    mutate(candidate);
    assert.throws(() => assertOperatingTraceMatrixV2(rehash(candidate)), { code: 'E_PROTOCOL_ARTIFACT_INVALID' });
  }

  const unsorted = clone(valid);
  unsorted.nodes.reverse();
  assert.throws(() => assertOperatingTraceMatrixV2(rehash(unsorted)), { code: 'E_OPERATE_TRACE_GRAPH_INVALID' });
  ...
  assert.throws(() => assertOperatingTraceMatrixV2(rehash(dangling)), { code: 'E_OPERATE_TRACE_DANGLING' });

  const duplicate = clone(valid);
  duplicate.nodes.push(clone(duplicate.nodes[0]));
  duplicate.nodes.sort((left, right) => (
    `${left.locator.kind}:${left.locator.id}`.localeCompare(`${right.locator.kind}:${right.locator.id}`)
  ));
  assert.throws(() => assertOperatingTraceMatrixV2(rehash(duplicate)), { code: 'E_OPERATE_TRACE_DUPLICATE' });
  ...
  assert.throws(() => assertOperatingTraceMatrixV2(rehash(cyclic)), { code: 'E_OPERATE_TRACE_CYCLE' });

  const oversized = clone(valid);
  oversized.nodes = Array.from({ length: 8193 }, (_, index) => ({
    ...clone(valid.nodes[0]),
    locator: { ...clone(valid.nodes[0].locator), id: `oversized:${String(index).padStart(5, '0')}` },
    inboundEdgeIds: [],
    outboundEdgeIds: [],
  }));
  assert.throws(() => assertOperatingTraceMatrixV2(rehash(oversized)), { code: 'E_PROTOCOL_ARTIFACT_INVALID' });
});
...
test('omitted-role absence identity is deterministic and plan-bound', () => {
  ...
  const foreign = deriveOperatingOmittedRoleAbsenceIdV2('ipl_foreign_0001', 'independent-challenge', '2.0.0');
