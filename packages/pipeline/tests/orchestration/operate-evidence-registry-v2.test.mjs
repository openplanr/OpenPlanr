...
test('evidence registrations are deterministic explicit built-ins with exact versions', () => {
  ...
  const planrResolver = OPEN_REFERENCE_EVIDENCE_REGISTRY_V2.resolvers.find(
    ({ resolverId }) => resolverId === 'local-planr-evidence-resolver',
  );
  assert.ok(
    planrResolver.errorCodes.includes('EVIDENCE_SOURCE_SCOPE_MISMATCH'),
    'the Planr resolver advertises the same precise scope failure it can return',
  );
  assert.deepEqual(
    findOperateEvidenceProviderRegistrationV2(registry, 'local-git-evidence-provider', {
      providerVersion: '2.0.0',
    }),
    valid.providers.find(({ providerId }) => providerId === 'local-git-evidence-provider'),
  );
  assert.deepEqual(
    findOperateEvidenceResolverRegistrationV2(registry, 'local-git-evidence-resolver', {
      resolverVersion: '2.0.0',
    }),
    valid.resolvers.find(({ resolverId }) => resolverId === 'local-git-evidence-resolver'),
  );
  assert.equal(
    findOperateEvidenceProviderRegistrationV2(registry, 'missing-provider', {
      providerVersion: '2.0.0',
    }),
    null,
  );
  assert.equal(
    findOperateEvidenceResolverRegistrationV2(registry, 'missing-resolver', {
      resolverVersion: '2.0.0',
    }),
    null,
  );
...
test('dispatch preparation checks scope, kind, exact registrations, and runtime-held capabilities', () => {
  ...
  assert.equal(
    prepareOperateEvidenceDispatchV2(registry, unknownResolver, {
      scope: valid.scope,
      capabilities: valid.capabilities,
    }).error.code,
  ...
  unsupportedVersion.resolver.version = fixture(
    'evidence-registry-invalid.json',
  ).unsupportedResolverVersion;
  assert.equal(
    prepareOperateEvidenceDispatchV2(registry, unsupportedVersion, {
      scope: valid.scope,
      capabilities: valid.capabilities,
    }).error.code,
  ...
  assert.equal(
    prepareOperateEvidenceDispatchV2(registry, kindMismatch, {
      scope: valid.scope,
      capabilities: valid.capabilities,
    }).error.code,
...
test('duplicate, dynamic, non-built-in, or authority-bearing registrations fail before dispatch', () => {
  ...
  for (const descriptor of [
    ...
    invalid.wrongCapability,
    invalid.wrongEffect,
  ]) {
    const candidate = clone(valid);
    setPath(candidate, descriptor.target, descriptor.value);
    assert.throws(
      () => createOperateEvidenceRegistryV2(candidate),
      (error) =>
        error?.code === 'E_EVIDENCE_REGISTRATION_INVALID' ||
        error?.code === 'E_PROTOCOL_ARTIFACT_INVALID',
    );
  }
