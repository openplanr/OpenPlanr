...
function requirement(mode, seed) {
  return ['named-single-party', 'threshold', 'named-multi-party'].includes(mode)
    ? [`aprq_prop${String(seed).padStart(4, '0')}`]
    : [];
}
...
test('policy precedence property: 512 deterministic narrowing vectors never widen a higher tier', () => {
  for (let seed = 0; seed < 512; seed += 1) {
    ...
    const core = policy(value, {
      tier: 'core',
      decisionMode: coreMode,
      approvalRequirementIds: requirement(coreMode, seed),
    });
    const domain = policy(value, {
      tier: 'domain',
      decisionMode: domainMode,
      approvalRequirementIds: requirement(domainMode, seed + 600),
    });
      ...
      assert.throws(
        () =>
          evaluateOperatingActionPolicyV2({
            action: value,
            configuredPolicies: [domain, core],
