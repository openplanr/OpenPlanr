...
test('public domain discovery returns exact deeply frozen registry-owned start identities', () => {
  ...
  assert.deepEqual(
    domains.map(({ domainId, domainVersion, domainContract }) => ({
      domainId,
      domainVersion,
      domainContract,
    })),
  ...
  assert.equal(
    domains[0].roles.some(
      ({ roleId, roleKind }) => roleId === 'strategy-finance' && roleKind === 'advisor',
    ),
    true,
  );
  assert.equal(
    domains[0].roles.some(({ roleId, roleKind }) => roleId === 'chair' && roleKind === 'chair'),
    true,
  );
...
test('business and software projections rebuild deterministically from only their own state, registration, and exact Artifacts', () => {
  for (const domainId of ['business', 'software']) {
    ...
    assert.deepEqual(
      input,
      before,
      'projection is pure and does not mutate input state or Artifact metadata',
    );
...
test('projection rejects cross-domain state, missing sources, and unregistered public bindings before producing a view', () => {
  ...
  const registry = createOperateExtensionRegistryV2({
    domains: [
      phase5DomainRegistration('software-domain-valid.json'),
      phase5DomainRegistration('business-domain-valid.json'),
    ],
  });
  assert.deepEqual(
    resolvePublicOperatingDomainV2(registry, 'business', { domainVersion: '1.0.0' })
      ?.domainContract,
    { apiDomainId: 'business', id: 'business-domain', version: '1.0.0' },
  );
  assert.equal(
    resolvePublicOperatingDomainV2(registry, 'business', { domainVersion: '2.0.0' }),
    null,
  );
  assert.equal(
    resolvePublicOperatingDomainV2(OPEN_REFERENCE_OPERATE_EXTENSIONS_V2, 'synthetic-domain', {
      domainVersion: '1.0.0',
    }),
    null,
  );
...
test('public domain resolution and projection reject hostile registry-shaped inputs before use', () => {
  ...
  assert.throws(
    () => resolvePublicOperatingDomainV2(hostile, 'business', { domainVersion: '1.0.0' }),
    {
      code: 'OPERATING_DOMAIN_INPUT_INVALID',
    },
  );
