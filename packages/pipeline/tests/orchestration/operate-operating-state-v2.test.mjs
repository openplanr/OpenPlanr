...
function acceptedSource(seed = '001') {
  ...
  const bytes = Buffer.from(
    `opaque operating source ${seed}: never enters state or events`,
    'utf8',
  );
...
function stateSeed(seed = '001') {
  ...
  return {
    ...
    request: {
      ...
      sourceRevisions: [
        { sourceArtifactId: source.artifact.artifactId, revision: `revision-${seed}` },
      ],
      collections: {
        ...
        decisions: [],
        actions: [],
        risks: [sourced('operating-risk')],
        assumptions: [sourced('operating-assumption')],
      },
    },
    draft: {
      snapshotId: `snp_state_${seed}`,
      stateId: `oms_state_${seed}`,
      timestamp: TIME,
...
test('state and snapshot fixtures validate while malformed runtime hashes fail schema validation', () => {
  ...
  assert.deepEqual(
    validateProtocolArtifact('operating-model-state', state, { protocolVersion: '2.0.0' }),
    [],
  );
  assert.deepEqual(
    validateProtocolArtifact('operating-snapshot', snapshot, { protocolVersion: '2.0.0' }),
    [],
  );
  assert.ok(
    validateProtocolArtifact('operating-model-state', fixture('operating-state-invalid.json'), {
      protocolVersion: '2.0.0',
    }).length > 0,
  );
  assert.ok(
    validateProtocolArtifact('operating-snapshot', fixture('operating-snapshot-invalid.json'), {
      protocolVersion: '2.0.0',
    }).length > 0,
  );
});
...
test('one runtime transaction atomically materializes immutable seven-collection state and snapshot from accepted exact bytes', () => {
  ...
  assert.deepEqual(
    Object.keys(result.operatingState)
      .filter((key) =>
        [
          'objectives',
          'metrics',
  ...
  assert.doesNotMatch(
    ...
    /opaque operating source/,
    'raw Artifact bytes never enter public state or events',
  );
  assert.deepEqual(
      ...
      materializeOperatingStateSnapshotV2(
        {
          ...
          collections: {
            ...request.collections,
            objectives: [{ ...request.collections.objectives[0], title: 'Divergent title' }],
          },
        },
        draft,
        { initialState: first.state, artifactStore: store },
      ),
  ...
  assert.throws(
    () =>
      materializeOperatingStateSnapshotV2(
        {
          ...request,
          collections: incompleteCollections,
        },
        {
          ...draft,
          snapshotId: 'snp_state_incomplete_002',
          stateId: 'oms_state_incomplete_002',
          eventIds: {
            snapshot: 'evt_snapshot_incomplete_002',
            state: 'evt_state_incomplete_002',
          },
        },
        { initialState: first.state, artifactStore: store },
      ),
  ...
  assert.throws(
    () =>
      materializeOperatingStateSnapshotV2(
        ...
        {
          ...draft,
          eventIds: { ...draft.eventIds, state: 'evt_state_different_002' },
        },
        { initialState: first.state, artifactStore: store },
      ),
    { code: 'STATE_TRANSITION_INVALID' },
  ...
  assert.throws(
    () =>
      materializeOperatingStateSnapshotV2(
        {
          ...request,
          sourceArtifactIds: ['art_missing_002'],
          sourceRevisions: [{ sourceArtifactId: 'art_missing_002', revision: 'missing' }],
        },
        {
          ...draft,
          snapshotId: 'snp_state_missing_002',
          stateId: 'oms_state_missing_002',
          eventIds: {
            snapshot: 'evt_snapshot_missing_002',
            state: 'evt_state_missing_002',
          },
        },
        { initialState: first.state, artifactStore: store },
      ),
    { code: /ARTIFACT_NOT_FOUND|STATE_TRANSITION_INVALID/ },
  );
  ...
  assert.throws(
    () =>
      materializeOperatingStateSnapshotV2(
        ...
        {
          ...draft,
          snapshotId: 'snp_state_bytes_002',
          stateId: 'oms_state_bytes_002',
          eventIds: {
            snapshot: 'evt_snapshot_bytes_002',
            state: 'evt_state_bytes_002',
          },
        },
        { initialState: first.state, artifactStore: missingStore },
      ),
    ...
    for (const [collection, kind, id] of records) {
      ...
      if (caseName === 'foreign') {
        runtimeState.cycles.push({
          ...
          inputBindingId: 'inb_foreign_001',
          updatedAt: TIME,
        });
      }
      ...
      assert.throws(
        () =>
          materializeOperatingStateSnapshotV2(
            ...
            { initialState: runtimeState, artifactStore: monitoredStore },
          ),
        { code: 'OPERATING_SCOPE_INVALID' },
      );
      assert.equal(
        sha256Jcs(runtimeState),
        checkpoint,
        `${caseName} ${collection}: checkpoint remains unchanged`,
      );
      assert.deepEqual(
        runtimeState.eventReplayIndex,
        events,
        `${caseName} ${collection}: no events were appended`,
      );
