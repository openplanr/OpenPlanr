...
function build(domainId, index) {
  ...
  const artifact = {
    ...clone(valid['operating-artifact']), artifactId: `art_property_${token}`, assignmentId: `asg_property_${token}`,
    scopeId: `scope-property-${token}`, domainId, domainVersion: '1.0.0', inputArtifactIds: [],
  };
  ...
  const transaction = buildOperatingSnapshotStateTransactionV2({
    scope: { scopeId: artifact.scopeId, domainId, domainVersion: '1.0.0' }, domainContract,
  ...
  }, { snapshotId: `snp_property_${token}`, stateId: `oms_property_${token}`, timestamp: '2026-08-09T12:00:00.000Z' });
...
test('projection identity is stable and domain binding remains explicit across generated public inputs', () => {
  for (let index = 1; index <= 24; index += 1) {
    ...
    const projection = projectPublicOperatingDomainV2({ state: input.state, snapshot: input.snapshot, referencedArtifacts: [input.artifact] });
    ...
    assert.deepEqual(projection, projectPublicOperatingDomainV2({ state: input.state, snapshot: input.snapshot, referencedArtifacts: [input.artifact] }));
  }
});
