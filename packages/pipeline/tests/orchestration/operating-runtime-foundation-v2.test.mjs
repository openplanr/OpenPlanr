...
test('Phase 6 governed tools are compiler-owned rows with no authority-free default', () => {
  assert.deepEqual(Object.keys(OPERATE_GUARD_TABLE_V2).filter((operation) => operation.startsWith('operate.action.')).sort(), [
  ...
  assert.equal(evaluateOperateGuardV2('operate.action.execute', context).error.code, 'ACTION_NOT_FOUND');
...
function assignment(state = 'pending') {
  return {
    ...
    objective: 'Review the exact bound evidence.', state, dependsOn: [], dependencyPolicy: { kind: 'none' },
    ...
    claim: null, terminalOutcome: null, createdAt: TIME, availableAt: state === 'available' ? TIME : null, completedAt: null,
  };
}
...
test('Phase 5 intelligence plans are an explicit empty, rebuildable runtime projection', () => {
  ...
  assert.doesNotThrow(() => assertProtocolArtifact('operating-runtime-state', state, {
    protocolVersion: '2.0.0',
  }));
});
...
test('Action authority promotion is one exact replayable engine Event with no caller revision identity', () => {
  ...
  for (const field of [
    ...
    'requestedCapability', 'targetBinding', 'effectClass', 'preconditionArtifactIds',
    'executionBinding',
  ]) delete before[field];
  ...
  assert.equal(promoted.action.actionHash, derivePersistentOperatingActionRevisionHashV2(promoted.action));
  assert.doesNotThrow(() => assertProtocolArtifact('operating-event', promoted.events[0], {
    protocolVersion: '2.0.0',
  }));
  assert.deepEqual(
...
test('versioned policy and exact approval history survive replay into one canonical Action disposition', () => {
  ...
  const evaluation = evaluateOperatingActionPolicyV2({
    action, configuredPolicies: [corePolicy, policy], evaluatedAt: '2026-08-10T08:00:00Z',
  });
  const party = {
  ...
  const requirement = createOperatingApprovalRequirementV2({
    policyRequirementId: 'aprq_runtime01', evaluation, action, parties: [party],
    expiresAt: '2026-08-11T08:00:00Z', consumable: true,
  });
  const approval = createOperatingApprovalRecordV2({
    approvalId: 'aprv_runtime001', requirement, evaluation, action,
  ...
  let state = {
    ...createEmptyOperatingRuntimeStateV2('2026-08-10T08:00:00Z', {
      actionPolicies: [corePolicy, policy], approvalRequirements: [requirement],
    }),
    cycles: [{
      ...cycle(), domainId: action.domainId, domainVersion: action.domainVersion,
      scopeId: action.scopeId, updatedAt: '2026-08-10T08:00:00Z',
    }],
    actions: [action],
  };
  ...
  forgedCheckpoint.approvalRequirements[0] = rehashWithout(
    forgedCheckpoint.approvalRequirements[0], 'scopeHash',
  );
  assert.doesNotThrow(() => assertProtocolArtifact(
    'operating-runtime-state', forgedCheckpoint, { protocolVersion: '2.0.0' },
  ));
  assert.throws(() => reduceOperatingRuntimeEventsV2([], { initialState: forgedCheckpoint }),
    ({ code }) => code === 'APPROVAL_INVALID');
  const nextEvent = ({ eventId, timestamp, type, entityId, actor, requestHash, payload }) => createOperatingRuntimeEventV2({
    eventId, timestamp, cycleId: action.sourceCycleId, type, entityId, actor,
    causationId: null, correlationId: 'corr-governance-001', requestHash, payload,
  }, { previousEvent: state.eventHead.sequence === 0 ? null : {
    sequence: state.eventHead.sequence, eventHash: state.eventHead.hash,
  } });
  const policyEvent = nextEvent({
    eventId: 'evt-policy-001', timestamp: evaluation.evaluatedAt, type: 'policy.evaluated',
    entityId: evaluation.evaluationId, actor: { kind: 'runtime', id: 'openplanr' },
  ...
  assert.doesNotThrow(() => assertProtocolArtifact(
    'operating-approval-record', signedOutsiderApproval, { protocolVersion: '2.0.0' },
  ));
  const outsiderEvent = nextEvent({
    eventId: 'evt-approval-outsider-001', timestamp: signedOutsiderApproval.issuedAt,
    type: 'approval.recorded', entityId: signedOutsiderApproval.approvalId,
    actor: { kind: 'human', id: 'outsider-0001' }, payload: signedOutsiderApproval,
  });
  assert.throws(() => reduceOperatingRuntimeEventsV2([outsiderEvent], { initialState: state }),
    ({ code }) => code === 'APPROVAL_INVALID');
  const approvalEvent = nextEvent({
    eventId: 'evt-approval-001', timestamp: approval.issuedAt, type: 'approval.recorded',
    entityId: approval.approvalId, actor: { kind: 'human', id: 'owner-0001' }, payload: approval,
  });
  state = reduceOperatingRuntimeEventsV2([approvalEvent], { initialState: state });
  const approvedEvent = nextEvent({
    eventId: 'evt-action-approved-001', timestamp: '2026-08-10T08:02:00Z', type: 'action.approved',
    entityId: action.actionId, actor: { kind: 'engine', id: 'openplanr' },
    payload: {
      action: { actionId: action.actionId, revision: action.revision, actionHash: action.actionHash },
  ...
  assert.equal(reduceOperatingRuntimeEventsV2([approvedEvent], { initialState: state }).actions[0].state, 'approved');
  ...
  const reevaluationEvent = createOperatingRuntimeEventV2({
    eventId: 'evt-policy-reevaluated-001', timestamp: nextEvaluation.evaluatedAt,
    cycleId: action.sourceCycleId, type: 'policy.evaluated', entityId: nextEvaluation.evaluationId,
    actor: { kind: 'runtime', id: 'openplanr' }, causationId: approvedEvent.eventId,
    correlationId: 'corr-governance-001', requestHash: nextEvaluation.inputHash, payload: nextEvaluation,
  ...
  const consumedCheckpoint = { ...approved, approvalRecords: structuredClone(consumedAuthority.records) };
  const reevaluated = reduceOperatingRuntimeEventsV2([reevaluationEvent], { initialState: consumedCheckpoint });
  assert.deepEqual(
    reevaluated.policyEvaluations,
    [evaluation, nextEvaluation].sort((left, right) => left.evaluationId.localeCompare(right.evaluationId)),
  );
  ...
  assert.notEqual(reevaluated.approvalRequirements[0].requirementId, reevaluated.approvalRequirements[1].requirementId);
});
...
function append(events, type, entityId, payload, timestamp = TIME) {
  ...
  events.push(createOperatingRuntimeEventV2({
    ...
    timestamp, cycleId: 'cyc_00000001', type, entityId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId: previousEvent?.eventId ?? null,
    correlationId: 'corr-001', payload,
  }, { previousEvent }));
}

function eventAfterState(state, type, entityId, payload, eventId = 'evt-tail-001') {
  return createOperatingRuntimeEventV2({
    eventId, timestamp: NEXT_TIME, cycleId: 'cyc_00000001', type, entityId,
    actor: { kind: 'engine', id: 'openplanr' }, causationId: null,
    correlationId: 'corr-tail-001', payload,
  }, {
    previousEvent: {
...
function releasePayload({
  ...
  const intent = deriveOperatingAssignmentReleaseIntentsV2({
    assignments, submissions, artifacts, submissionReplayIndex, intelligencePlans,
  })
...
function happyPathEvents() {
  ...
  append(events, 'assignment.available', 'asg_00000001', releasePayload({ assignments: [assignment()] }, 'asg_00000001'));
  ...
  append(events, 'assignment.started', 'asg_00000001', { assignmentId: 'asg_00000001', attempt: 1 });
  ...
  append(events, 'artifact.created', 'art_00000001', {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0', artifactId: 'art_00000001',
    artifactType: 'cycle-evidence', assignmentId: 'asg_00000001', cycleId: 'cyc_00000001', scopeId: 'scope-acme',
    domainId: 'business', domainVersion: '1.0.0', schemaId: 'operating-context-capture', artifactSchemaVersion: '2.0.0', mediaType: 'application/json',
    encoding: 'utf-8', rawHash: RAW_HASH, canonicalHash: null, sizeBytes: 21, storageClass: 'machine-local',
    sensitivity: 'internal', retentionClass: 'project',
    producer: { actorId: 'agent-001', roleId: 'context-evidence', runtime: 'codex' }, inputArtifactIds: [], createdAt: NEXT_TIME,
  }, NEXT_TIME);
  append(events, 'assignment.validated', 'asg_00000001', {
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001', artifactId: 'art_00000001',
...
test('claim returns the complete runtime-issued Assignment and Artifact reads stay actor-, scope-, and custody-bound', () => {
  ...
  assert.throws(() => claimOperatingAssignmentV2({
    ...request,
    actor: { actorId: 'agent-foreign', kind: 'agent', runtime: 'codex' },
  }, draft, { initialState: claimed.state }), { code: 'ASSIGNMENT_ALREADY_CLAIMED' });

  const artifact = {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    artifactId: 'art_00000001', artifactType: 'advisor-result', assignmentId: available.assignmentId,
    cycleId: available.cycleId, scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    schemaId: 'advisor-result', artifactSchemaVersion: '1.0.0', mediaType: 'application/json', encoding: 'utf-8',
    rawHash: RAW_HASH, canonicalHash: null, sizeBytes: 21, storageClass: 'machine-local', sensitivity: 'internal',
    retentionClass: 'project', producer: { actorId: 'agent-001', roleId: available.roleId, runtime: 'codex' },
    inputArtifactIds: [], createdAt: TIME,
  };
  const scope = { scopeId: artifact.scopeId, domainId: artifact.domainId, domainVersion: artifact.domainVersion };
  const agentRead = {
    artifactId: artifact.artifactId, representation: 'metadata', actor: fullActor(), scope,
    assignmentId: available.assignmentId,
  };
  const readContext = {
    capabilities: ['operate.artifact.get'], actor: fullActor(), artifact,
    assignment: claimed.response.assignment, artifactRequest: agentRead,
  };
  assert.equal(evaluateOperateGuardV2('operate.artifact.get', readContext).allowed, true);
  assert.equal(evaluateOperateGuardV2('operate.artifact.get', {
    ...readContext,
    actor: { actorId: 'agent-foreign', kind: 'agent', runtime: 'codex' },
  ...
  const humanContext = {
    capabilities: ['operate.artifact.get'], actor: human, artifact, artifactRequest: humanRequest,
...
test('decoded-json reads are exact UTF-8, canonical-hash, scope, actor, and Assignment-custody bound', () => {
  ...
  const claimed = claimOperatingAssignmentV2({ assignmentId: available.assignmentId, actor: fullActor() }, {
    claimId: 'claim-decoded-json-001',
    submissionId: 'sub_decoded_json_001',
    eventIds: { claimed: 'evt-decoded-json-claimed-001', started: 'evt-decoded-json-started-001' },
    timestamp: TIME,
  ...
  const artifact = {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    artifactId: inputArtifactId, artifactType: 'cycle-evidence', assignmentId: available.assignmentId,
    cycleId: available.cycleId, scopeId: 'scope-acme', domainId: 'business', domainVersion: '1.0.0',
    schemaId: 'operating-context-capture', artifactSchemaVersion: '2.0.0', mediaType: 'application/json', encoding: 'utf-8',
    rawHash: `sha256:${createHash('sha256').update(rawBytes).digest('hex')}`,
    canonicalHash: sha256Jcs(content), sizeBytes: rawBytes.byteLength,
  ...
  const request = {
    ...
    actor: fullActor(),
    assignmentId: available.assignmentId,
    scope: { scopeId: artifact.scopeId, domainId: artifact.domainId, domainVersion: artifact.domainVersion },
  };
  const decoded = readOperatingArtifactV2(request, {
    initialState: state, artifactStore: store, capabilities: ['operate.artifact.get'],
  });
  ...
  let deniedReads = 0;
  assert.throws(() => readOperatingArtifactV2({
    ...request,
    actor: { actorId: 'agent-foreign', kind: 'agent', runtime: 'codex' },
  }, {
    initialState: state,
    capabilities: ['operate.artifact.get'],
    artifactStore: { readRaw: () => { deniedReads += 1; return rawBytes; } },
  ...
  refusesBytes(Buffer.from('{"nested":{"value":8},"list":["exact","json"]}', 'utf8'), {}, 'ARTIFACT_HASH_MISMATCH');
  refusesBytes(Buffer.from('not json', 'utf8'), { mediaType: 'text/plain', encoding: 'binary', canonicalHash: null }, 'RESULT_CONTRACT_INVALID');
});

test('pre-intelligence context capture claims and submits one exact runtime-owned DTO', () => {
  const contextAssignment = {
    kind: 'operating-assignment', schemaVersion: '1.0.0', protocolVersion: '2.0.0',
    assignmentId: 'asg_context_001', cycleId: 'cyc_00000001', assignmentKind: 'context-capture',
    roleId: 'context-evidence', objective: 'Capture the exact Cycle evidence request.',
    state: 'available', dependsOn: [], dependencyPolicy: { kind: 'none' },
    inputArtifactIds: [], inputAbsences: [], intelligenceContext: null,
    outputContract: {
      schemaId: 'operating-context-capture', schemaVersion: '2.0.0',
      mediaType: 'application/json', encoding: 'utf-8', maxBytes: 262144,
    },
    capabilityGrantId: 'grant-context-001', governedOperationId: null,
    attemptPolicy: { maxAttempts: 3, attempt: 0, timeoutMs: 300000 },
    claim: null, terminalOutcome: null, createdAt: TIME, availableAt: TIME, completedAt: null,
    mandate: null, analysisRubric: null,
  };
  const claimed = claimOperatingAssignmentV2({
    assignmentId: contextAssignment.assignmentId,
    actor: fullActor(),
  }, {
    claimId: 'claim-context-001', submissionId: 'sub_context_001',
    eventIds: { claimed: 'evt-context-claimed-001', started: 'evt-context-started-001' },
    timestamp: TIME, correlationId: 'corr-context-001',
  }, {
  ...
  const request = {
    ...
    actor: fullActor(), mediaType: 'application/json', encoding: 'utf-8',
    contentBase64: Buffer.from(JSON.stringify(capture), 'utf8').toString('base64'),
  };
  const acceptance = {
    artifactId: 'art_context_001', artifactType: 'cycle-evidence', inputArtifactIds: [],
    timestamp: NEXT_TIME, validatorVersion: '1.0.0', correlationId: 'corr-context-001',
    eventIds: {
  ...
  const staged = [];
  assert.throws(() => acceptOperatingAssignmentSubmissionV2({
    ...request,
    contentBase64: Buffer.from(JSON.stringify({ ...capture, invented: true }), 'utf8').toString('base64'),
  }, acceptance, {
...
test('OP-05/06 and INV-GUIDE-001..005: one guard table owns refusals and complete allowed actions', () => {
  ...
  const context = {
    ...
    cycle: cycle(), assignment: running,
    submission: submissionRecord(),
    actor: fullActor(),
  };
  assert.equal(evaluateOperateGuardV2('operate.assignment.submit', context).allowed, false);
  assert.equal(deriveOperateAllowedActionsV2(context).some(({ tool }) => tool === 'operate.assignment.submit'), false,
    'a submit action is omitted until its complete content body is known');

  context.submitRequest = {
    assignmentId: running.assignmentId, submissionId: 'sub_00000001', mediaType: 'application/json',
    encoding: 'utf-8', contentBase64: intelligenceSubmissionBase64('context-capture'), actor: fullActor(),
  };
  const tools = deriveOperateAllowedActionsV2(context).map(({ tool }) => tool);
  assert.deepEqual(tools, ['operate.assignment.submit', 'operate.cycle.get', 'operate.cycle.resume']);
  ...
  for (const { label, request, code } of submitViolations) {
    ...
    assert.equal(evaluateOperateGuardV2('operate.assignment.submit', invalidContext).error.code, code, label);
    assert.equal(deriveOperateAllowedActionsV2(invalidContext).some(({ tool }) => tool === 'operate.assignment.submit'), false, label);
  }
  const resolvedContext = {
    ...context,
    submission: submissionRecord({
      state: 'rejected', rawHash: RAW_HASH, canonicalHash: null, sizeBytes: 2,
      acceptanceEventIds: ['evt-rejected'], responseData: { accepted: false }, resolvedAt: NEXT_TIME,
    }),
  };
  assert.equal(evaluateOperateGuardV2('operate.assignment.submit', resolvedContext).error.code, 'ASSIGNMENT_ALREADY_SUBMITTED');

  const available = assignment('available');
  const claimContext = { capabilities: ['operate.assignment.claim'], assignment: available, actor: fullActor() };
  assert.deepEqual(deriveOperateAllowedActionsV2(claimContext).map(({ tool }) => tool), ['operate.assignment.claim']);
  assert.equal(evaluateOperateGuardV2('operate.assignment.claim', { ...claimContext, capabilities: [] }).error.code, 'CAPABILITY_DENIED');
  ...
  assert.deepEqual(failure.allowedActions.map(({ tool }) => tool), ['operate.cycle.get']);

  const placeholderContext = {
    capabilities: ['operate.cycle.start'],
    startRequest: {
      scope: { scopeId: '<scope-id>', domainId: 'business', domainVersion: '1.0.0' },
      focus: [], trigger: { kind: 'manual' }, mode: 'standard',
      ownerActorId: 'owner-placeholder-001', deliveryRoute: 'observe-only',
    },
  };
...
test('INV-OBS-001: strict provenance and base-plus-tail reduction are deterministic and no-model', () => {
  ...
  const second = reduceOperatingRuntimeEventsV2(structuredClone(events), { initialState: structuredClone(base) });
  ...
  assert.deepEqual(first.submissionReplayIndex[0].acceptanceEventIds, ['evt-006', 'evt-007', 'evt-008']);
});

test('OP-01/07, AC-09, and INV-ART-001..006: direct submission stages exact bytes and commits one replayable acceptance', () => {
  const beforeAcceptance = reduceOperatingRuntimeEventsV2(happyPathEvents().slice(0, 5), { initialState: baseState() });
  ...
  const request = {
    ...
    mediaType: 'application/json', encoding: 'utf-8', contentBase64: rawBytes.toString('base64'),
    actor: fullActor(),
  };
  const draft = {
    artifactId: 'art_00000001', artifactType: 'cycle-evidence', inputArtifactIds: [],
    timestamp: NEXT_TIME, validatorVersion: '1.0.0', correlationId: 'corr-submit-001',
  ...
  }), { code: 'CAPABILITY_DENIED' }, 'a foreign claimant fails before staging bytes or emitting Events');
  ...
  assert.equal(sha256Jcs(beforeAcceptance), beforeHash, 'the pure transaction never mutates its input projection');
  assert.equal(replayHook.dispatchCount, 0, 'acceptance/replay never dispatches a model');
  assert.equal(first.replayed, false);
  assert.deepEqual(first.stagedRawBytes, rawBytes, 'Unicode and whitespace bytes are retained exactly for durable staging');
  assert.equal(first.events.length, 3, 'submission, Artifact, and validation are one ordered transaction');
  ...
  assert.deepEqual(first.artifact.producer, {
  ...
  assert.notEqual(first.artifact.rawHash, first.artifact.canonicalHash, 'semantic canonical form never replaces exact-byte identity');
  ...
  const sameCanonicalDifferentBytes = Buffer.from('{"kind":"operating-context-capture","schemaVersion":"1.0.0","protocolVersion":"2.0.0","contextKind":"cycle-evidence","scope":{"scopeId":"scope-acme","domainId":"business","domainVersion":"1.0.0"},"focus":["café Δ"],"trigger":{"kind":"manual"}}', 'utf8');
  for (const [label, mutate] of [
    ['actor', (context) => {
      context.actor.actorId = 'agent-foreign';
      context.submitRequest.actor.actorId = 'agent-foreign';
    }],
    ['submission', (context) => { context.submitRequest.submissionId = 'sub_foreign_0001'; }],
    ['body', (context) => { context.submitRequest.contentBase64 = sameCanonicalDifferentBytes.toString('base64'); }],
    ['media type', (context) => { context.submitRequest.mediaType = 'text/plain'; }],
    ['encoding', (context) => { context.submitRequest.encoding = 'binary'; }],
    ['custody', (context) => { context.artifact.inputArtifactIds = ['art_foreign_input']; }],
  ]) {
  ...
  assert.equal(replay.replayed, true);
  assert.deepEqual(replay.response, first.response, 'same runtime-issued submission ID and bytes returns the original acknowledgement');
  ...
  for (const [label, initialState, hostileDraft] of [
    ...
    ['live undeclared input', beforeAcceptance, { ...draft, inputArtifactIds: ['art_foreign_input'] }],
  ...
  assert.throws(
    () => acceptOperatingAssignmentSubmissionV2(request, { ...draft, inputArtifactIds: ['art_foreign_input'] }, {
      initialState: first.state,
    }),
  ...
  for (const [label, mutate] of [
    ...
    ['checkpoint undeclared input', (state) => { state.artifacts[0].inputArtifactIds = ['art_foreign_input']; }],
    ['checkpoint domain version', (state) => { state.artifacts[0].domainVersion = '9.9.9'; }],
    ['checkpoint producer actor', (state) => { state.artifacts[0].producer.actorId = 'agent-foreign'; }],
    ['checkpoint producer runtime', (state) => { state.artifacts[0].producer.runtime = 'runtime-foreign'; }],
  ]) {
  ...
  assert.throws(
    () => acceptOperatingAssignmentSubmissionV2({ ...request, contentBase64: sameCanonicalDifferentBytes.toString('base64') }, draft, {
      initialState: first.state,
    }),
  ...
  assert.equal(sha256Jcs(first.state), acceptedHash, 'different-byte retry cannot mutate accepted state');

  const invalidJson = Buffer.from('{', 'utf8');
  assert.throws(
    () => acceptOperatingAssignmentSubmissionV2({ ...request, contentBase64: invalidJson.toString('base64') }, draft, {
      initialState: beforeAcceptance,
    }),
    (error) => error.code === 'RESULT_CONTRACT_INVALID',
  );
  assert.equal(sha256Jcs(beforeAcceptance), beforeHash, 'validation failure exposes no partial Artifact or projection');
...
test('OP-01: binary direct submission remains byte-safe and has no canonical-hash substitute', () => {
  ...
  beforeAcceptance.assignments[0].outputContract = {
    ...beforeAcceptance.assignments[0].outputContract,
    mediaType: 'application/octet-stream', encoding: 'binary', maxBytes: 64,
  };
  const rawBytes = Buffer.from([0x00, 0xff, 0x20, 0x0a, 0x80, 0x41]);
  const result = acceptOperatingAssignmentSubmissionV2({
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001',
    mediaType: 'application/octet-stream', encoding: 'binary', contentBase64: rawBytes.toString('base64'),
    actor: fullActor(),
  }, {
    artifactId: 'art_00000002', artifactType: 'advisor-result', inputArtifactIds: [],
    timestamp: NEXT_TIME, validatorVersion: '1.0.0', correlationId: 'corr-binary-001',
    eventIds: { submitted: 'evt-006', artifactCreated: 'evt-007', validated: 'evt-008' },
  }, { initialState: beforeAcceptance });
...
test('O2-P1-001 / OP-07 / INV-OBS-001..003: Event identity survives checkpoint recovery and restart', () => {
  const events = happyPathEvents();
  const checkpointState = reduceOperatingRuntimeEventsV2(events.slice(0, 2), { initialState: baseState() });
  const checkpoint = {
    kind: 'operating-checkpoint', schemaVersion: '1.0.0', protocolVersion: '2.0.0', createdAt: NEXT_TIME,
    ...
    recordDigests: [], blobHashes: [], recoveryVersion: '1.0.0',
  };
  assert.doesNotThrow(() => assertProtocolArtifact('operating-checkpoint', checkpoint, { protocolVersion: '2.0.0' }));
  assert.deepEqual(checkpointState.eventReplayIndex, events.slice(0, 2).map(expectedEventReplayEntry));

  const replayed = reduceOperatingRuntimeEventsV2([structuredClone(events[0])], {
    initialState: structuredClone(checkpointState),
  });
  assert.equal(sha256Jcs(replayed), sha256Jcs(checkpointState), 'same Event content replays without an append');

  const recoveredWithRetryAndTail = reduceOperatingRuntimeEventsV2([
    structuredClone(events[0]),
    ...structuredClone(events.slice(2)),
  ], { initialState: structuredClone(checkpointState) });
  const recoveredWithTailOnly = reduceOperatingRuntimeEventsV2(events.slice(2), {
    initialState: structuredClone(checkpointState),
  });
  assert.equal(sha256Jcs(recoveredWithRetryAndTail), sha256Jcs(recoveredWithTailOnly),
  ...
  const conflictingReuse = eventAfterState(restartedState, 'assignment.available', 'asg_00000001',
    releasePayload({ assignments: [assignment()] }, 'asg_00000001'), 'evt-001');
  assert.throws(
  ...
  assert.equal(sha256Jcs(restartedState), before, 'a divergent retry fails before checkpoint state mutation');
});
...
test('O2-P1-002: assignment availability is evented exactly once with dependency proof and replay provenance', () => {
  ...
  const pending = reduceOperatingRuntimeEventsV2(zeroDependencyEvents, { initialState: baseState() });
  const release = eventAfterState(pending, 'assignment.available', 'asg_00000001',
    releasePayload(pending, 'asg_00000001'), 'evt-zero-release-001');
  const zeroDependencyAvailable = reduceOperatingRuntimeEventsV2([release], { initialState: pending });
  ...
  assert.equal(
    sha256Jcs(reduceOperatingRuntimeEventsV2([structuredClone(release)], { initialState: structuredClone(zeroDependencyAvailable) })),
  ...
  const duplicate = eventAfterState(zeroDependencyAvailable, 'assignment.available', 'asg_00000001',
    releasePayload({ assignments: [assignment()] }, 'asg_00000001'), 'evt-zero-release-002');
  assert.throws(
    () => reduceOperatingRuntimeEventsV2([duplicate], { initialState: zeroDependencyAvailable }),
    (error) => error.code === 'STATE_TRANSITION_INVALID',
  );
  assert.equal(sha256Jcs(zeroDependencyAvailable), beforeDuplicate, 'a second availability Event cannot mutate the checkpoint');

  const checkpointBase = reduceOperatingRuntimeEventsV2(happyPathEvents(), { initialState: baseState() });
  const dependent = {
    ...assignment(),
  ...
  const valid = eventAfterState(checkpointBase, 'assignment.available', dependent.assignmentId, validPayload);
  const available = reduceOperatingRuntimeEventsV2([valid], { initialState: checkpointBase });
  assert.equal(available.assignments.find(({ assignmentId }) => assignmentId === dependent.assignmentId).state, 'available');

  for (const [label, payload] of [
    ['missing dependency proof', { ...validPayload, dependencyProofs: [] }],
    ['extra dependency proof', {
      ...validPayload,
      dependencyProofs: [...validPayload.dependencyProofs, {
        ...validPayload.dependencyProofs[0], assignmentId: 'asg_00000003', eventId: 'evt-extra-001', artifactId: 'art_00000003',
      }],
    }],
    ['missing validation event', { ...validPayload, dependencyEventIds: [] }],
    ['nonexistent validation event', { ...validPayload, dependencyEventIds: ['evt-does-not-exist'] }],
  ]) {
    const initial = structuredClone(checkpointBase);
    const before = sha256Jcs(initial);
    const invalid = eventAfterState(initial, 'assignment.available', dependent.assignmentId, payload);
    assert.throws(
  ...
  for (const [label, mutate] of [
    ['foreign Artifact Assignment binding', (state) => {
      state.artifacts[0].assignmentId = 'asg_foreign_0001';
    }],
    ['foreign Artifact Cycle binding', (state) => {
      state.artifacts[0].cycleId = 'cyc_foreign_0001';
    }],
    ['Artifact raw-hash mismatch', (state) => {
      state.artifacts[0].rawHash = `sha256:${'d'.repeat(64)}`;
    }],
    ['Artifact canonical-hash mismatch', (state) => {
      state.artifacts[0].canonicalHash = `sha256:${'e'.repeat(64)}`;
    }],
    ['Artifact byte ceiling mismatch', (state) => {
      const sizeBytes = state.assignments[0].outputContract.maxBytes + 1;
      state.artifacts[0].sizeBytes = sizeBytes;
      state.submissions[0].sizeBytes = sizeBytes;
      state.submissions[0].responseData.sizeBytes = sizeBytes;
    ...
    const invalid = eventAfterState(hostile, 'assignment.available', dependent.assignmentId, validPayload,
      `evt-hostile-${label.replaceAll(' ', '-').toLowerCase()}`);
    assert.throws(() => reduceOperatingRuntimeEventsV2([invalid], { initialState: hostile }),
      (error) => error.code === 'STATE_TRANSITION_INVALID', label);
    assert.equal(sha256Jcs(hostile), before, label);
  }

  const unknownBase = structuredClone(checkpointBase);
  const unknown = unknownBase.assignments.find(({ assignmentId }) => assignmentId === dependent.assignmentId);
  unknown.dependsOn = ['asg_00000003'];
  const unknownEvent = eventAfterState(unknownBase, 'assignment.available', dependent.assignmentId, validPayload);
  assert.throws(
  ...
  crossCycleBase.assignments.find(({ assignmentId }) => assignmentId === 'asg_00000001').cycleId = 'cyc_00000002';
  const crossCycle = eventAfterState(crossCycleBase, 'assignment.available', dependent.assignmentId, validPayload);
  assert.throws(
  ...
  for (const [label, createdAssignment] of [
    ...
    assert.throws(
      () => append(bypassEvents, 'assignment.created', createdAssignment.assignmentId, createdAssignment, NEXT_TIME),
  ...
  assert.equal(rebuilt.assignments.find(({ assignmentId }) => assignmentId === dependent.assignmentId).state, 'available',
...
function appendSubmittedAttempt(events, attempt, rawHash) {
  append(events, 'assignment.submitted', 'asg_00000001', {
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001', rawHash, canonicalHash: null,
    sizeBytes: 21, mediaType: 'application/json', encoding: 'utf-8',
  }, NEXT_TIME);
  append(events, 'assignment.rejected', 'asg_00000001', {
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001',
    violations: [{ path: '$.result', code: 'invalid', message: 'The result contract is invalid.' }],
    attempt,
...
test('rejected attempts reopen the same issued identity only on the next bounded attempt', () => {
  ...
  append(events, 'assignment.started', 'asg_00000001', { assignmentId: 'asg_00000001', attempt: 2 }, NEXT_TIME);
  ...
  append(events, 'assignment.submitted', 'asg_00000001', {
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001', rawHash: RETRY_HASH, canonicalHash: null,
    sizeBytes: 21, mediaType: 'application/json', encoding: 'utf-8',
  }, NEXT_TIME);
  append(events, 'artifact.created', 'art_00000002', {
    kind: 'operating-artifact', schemaVersion: '1.0.0', protocolVersion: '2.0.0', artifactId: 'art_00000002',
    artifactType: 'cycle-evidence', assignmentId: 'asg_00000001', cycleId: 'cyc_00000001', scopeId: 'scope-acme',
    domainId: 'business', domainVersion: '1.0.0', schemaId: 'operating-context-capture', artifactSchemaVersion: '2.0.0', mediaType: 'application/json',
    encoding: 'utf-8', rawHash: RETRY_HASH, canonicalHash: null, sizeBytes: 21, storageClass: 'machine-local',
    sensitivity: 'internal', retentionClass: 'project',
    producer: { actorId: 'agent-001', roleId: 'context-evidence', runtime: 'codex' }, inputArtifactIds: [], createdAt: NEXT_TIME,
  }, NEXT_TIME);
  append(events, 'assignment.validated', 'asg_00000001', {
    assignmentId: 'asg_00000001', submissionId: 'sub_00000001', artifactId: 'art_00000002',
    validatorVersion: '1.0.0',
  }, NEXT_TIME);
  ...
  assert.deepEqual(state.submissionReplayIndex[0].acceptanceEventIds, ['evt-009', 'evt-010', 'evt-011']);
});
...
test('final rejection is durable but cannot reopen, and attempt counters fail closed', () => {
  ...
  append(events, 'assignment.started', 'asg_00000001', { assignmentId: 'asg_00000001', attempt: 3 }, NEXT_TIME);
  ...
  const reopen = eventAfterState(finalRejected, 'assignment.started', 'asg_00000001', {
    assignmentId: 'asg_00000001', attempt: 4,
  }, 'evt-reopen-final');
  assert.throws(
  ...
  const rejectedBase = reduceOperatingRuntimeEventsV2(firstRejectedAttemptEvents(), { initialState: baseState() });
  rejectedBase.submissionReplayIndex.push({
    submissionId: 'sub_00000001', assignmentId: 'asg_00000001', rawHash: RAW_HASH, canonicalHash: null,
    sizeBytes: 21, artifactId: 'art_00000009', acceptanceEventIds: ['evt-accepted-existing'],
    responseData: { accepted: true },
  });
  const before = sha256Jcs(rejectedBase);
  const blocked = eventAfterState(rejectedBase, 'assignment.started', 'asg_00000001', {
...
test('strict chain and semantic failures preserve the input projection', () => {
  ...
  assert.throws(() => verifyOperatingRuntimeEventChainV2(tampered), (error) => error.code === 'STATE_TRANSITION_INVALID');
  assert.equal(sha256Jcs(base), before);

  const wrongVersion = structuredClone(happyPathEvents()[0]);
  wrongVersion.protocolVersion = '1.4.0';
  assert.throws(() => verifyOperatingRuntimeEventChainV2([wrongVersion]), (error) => (
  ...
  for (const requiredField of ['actor', 'causationId', 'correlationId', 'entityId', 'payload']) {
    ...
    assert.throws(() => verifyOperatingRuntimeEventChainV2([invalid]), (error) => error.code === 'STATE_TRANSITION_INVALID', requiredField);
  }
...
test('illegal assignment transitions use a stable code and never mutate the source', () => {
  ...
  assert.throws(() => transitionOperatingAssignmentV2(source, 'validated'), (error) => (
