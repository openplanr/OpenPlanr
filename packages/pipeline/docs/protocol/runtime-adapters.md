...
5. New SHIP runs produce one schema-valid immutable closure receipt; human QA,
...
The orchestrator dispatches every ready task as a native parallel subagent call,
then drains newly ready dependents without ending the user invocation.
...
- **No concurrency knob.** The host's native concurrency cap is the only throttle.
...
- **Runtime-owned state.** Agents return structured results; only the closure engine writes task status, receipts, compatibility evidence, and provenance.
...
- **Cursor adapter:** the generated `.cursor/rules/openplanr.mdc` is the single
...
| Self-heal stack.md | ✅ | ✅ | ✅ |
| R6 adaptive recovery guidance | ✅ | ✅ | ✅ |
| Explicit release-candidate closure | ✅ | ✅ | ✅ |
| Explicit release compatibility projection | ✅ | ✅ | ✅ |
