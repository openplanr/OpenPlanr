...
4. After each task, the orchestrator submits its structured result to the closure
   runtime, which owns all persistent bookkeeping.
...
Cursor and any host without native subagents execute the same ready graph
sequentially without ending the user invocation. `--task T-NNN` is the only
single-task path.
