...
- Codex uses native subagents when available, while Cursor uses same-invocation
  sequential fallback; both drain the coherent task DAG. Explicit `--task T-NNN`
  is the sole single-task scope.
- No `.planr-worktrees/` directory and no `planr-wt/*` branch is ever created.

---

*Core user story for SPEC-014 native parallel dispatch (supersedes the SPEC-013
wave scheduler story). SPEC-025 supersedes only its former per-task resumption
behavior with monotonic, same-invocation closure.*
