...
export const PROTECTED_USER_OWNED_PATHS = Object.freeze({
  ...
  'lib/pipeline/engine.mjs': '3fceb9ddb78287a276a448b03ffdd137e6314f056b0ba4b7a6d4e2a736d69cc3',
  'tests/pipeline/engine.test.mjs': '4f9bb8c7bde90ea8f569fa810e7e5d20ae61ac83a3fa0ab71adea375102c2631',
});
...
const textExtensions = new Set([
  '.cjs', '.css', '.d.mts', '.d.ts', '.feature', '.html', '.js', '.json', '.jsonl',
  '.md', '.mdc', '.mjs', '.sh', '.svg', '.ts', '.txt', '.yaml', '.yml',
]);
const outputPaths = new Set(canonicalOutputPaths);
const operatingNeedle = /\boperate\b|operating[-\s]|operatingboard|assignment\.submit|harness\s+(?:prepare|record|finalize|heartbeat)/i;
const legacyNeedle = /compatibility-v1_4|v1[_-]?[234]|operating-board|operating-mandate|harness\s+(?:prepare|record|finalize|heartbeat)|legacy-operating/i;
...
function command(commandName, args, { cwd, allowFailure = false } = {}) {
  ...
  if (!allowFailure && result.status !== 0) {
    fail(`${commandName} ${args.join(' ')} failed: ${result.stderr || result.stdout || `exit ${result.status}`}`);
  }
  return result;
}
...
function readText(path) {
  const extension = path.endsWith('.d.mts') ? '.d.mts' : path.endsWith('.d.ts') ? '.d.ts' : extname(path);
...
function parseStatus(root) {
  ...
  entries.sort((left, right) => `${left.path}:${left.index}:${left.worktree}`.localeCompare(`${right.path}:${right.index}:${right.worktree}`));
...
function repositoryDescriptor(definition, root) {
  ...
  if (existsSync(packagePath)) {
    try { packageName = JSON.parse(readFileSync(packagePath, 'utf8')).name ?? packageName; } catch { /* descriptive fallback */ }
  }
...
function looksLikeOperatingSurface(path, text) {
  ...
  if (/^(?:lib\/operate\/|schemas\/v[0-9]|conformance\/|commands\/|templates\/runtime\/|agents\/operating\/|adapters\/|registry\/)/.test(lower)) {
...
function classifyPlanrPipelineSurface(path, text) {
  ...
  if (/^schemas\/v2\.0\.0\/(?:business-scope-binding|operate-v14-compatibility-transaction)\.schema\.json$/.test(path)) return 'DELETE_LEGACY';
  ...
  if (/^conformance\/verify-operating-(?:board(?:-amendment|-v1_[34])?|mandate-vocabulary|records-migration)\.mjs$/.test(path)) return 'DELETE_LEGACY';
  if (/^tests\/schema\/operating-schemas-v1_[34]\.test\.mjs$/.test(path)) return 'DELETE_LEGACY';
  if (path === 'tests/ecosystem/operate-release-contract.test.mjs' || path === 'scripts/operate-release-canary.mjs') return 'DELETE_LEGACY';
  if (/^schemas\/v2\.0\.0\//.test(path)
    || /^conformance\/fixtures\/operating-runtime-v2\//.test(path)
    || /^conformance\/verify-operate(?:-v2-absence|-ing-runtime-v2)\.mjs$/.test(path)
    || /^tests\/ecosystem\/operate-(?:reset-inventory|v2-(?:development-package|legacy-absence))\.test\.mjs$/.test(path)
    ...
    ].includes(path)) return 'KEEP_V2';
  if (path === 'CHANGELOG.md' || path === 'scripts/inventory-operate-surfaces.mjs') return 'KEEP_SHARED_FOUNDATION';
  if (/^lib\/pipeline\//.test(path) || /^lib\/dashboard\/(?!operate-reader\.mjs|app\/views\/operate\.js)/.test(path)) return 'KEEP_SHARED_FOUNDATION';
...
function markdownLinks(path, text) {
  ...
  for (const match of text?.matchAll(/\]\(([^)\s]+)(?:\s+[^)]*)?\)/g) ?? []) {
    ...
    if (!target.startsWith('#') && !target.startsWith('/') && !/^[a-z][a-z0-9+.-]*:/i.test(target)) links.push(target.split('#', 1)[0]);
  }
...
function importSpecifiers(text) {
  const imports = [];
  for (const match of text?.matchAll(/(?:\bfrom\s*|\bimport\s*\(|\brequire\s*\()(['"])([^'"]+)\1/g) ?? []) imports.push(match[2]);
...
function resolveImport(path, specifier, allPaths) {
  ...
  const alternatives = [base, `${base}.mjs`, `${base}.js`, `${base}.ts`, `${base}.d.mts`, `${base}.d.ts`, `${base}/index.mjs`, `${base}/index.js`];
...
function packageReachability(path, packageJson) {
  ...
  for (const [exportName, target] of Object.entries(packageJson.exports ?? {})) {
    ...
    if (targets.some((value) => typeof value === 'string' && value.replace(/^\.\//, '') === path)) references.push(`exports:${exportName}`);
  }
  const topLevel = path.split('/', 1)[0];
  if ((packageJson.files ?? []).some((entry) => entry.replace(/\/$/, '') === topLevel || entry === path)) references.push(`files:${topLevel}`);
...
function generatorEvidence(path) {
  if (
    ...
    || /^adapters\/(?:claude-code\/README\.md|codex\/project-guidance\.md|cursor\/rules\/openplanr\.mdc)$/.test(path)
...
function makeSurface(repository, path, text, packageJson) {
  const classification = repository.id === 'planr-pipeline'
    ? classifyPlanrPipelineSurface(path, text)
    : 'DEFER_OWNER';
  ...
  const surface = {
    ...
    ownership: repository.id === 'planr-pipeline' ? 'contract-owner-or-shared-foundation' : 'independent-downstream-owner',
    evidence: {
      ...
      testCoverage: path.startsWith('tests/') || path.startsWith('conformance/') ? [`self:${path}`] : [],
      documentationLinks: /\.mdc?$/i.test(path) ? [`self:${path}`] : [],
      downstreamReferences: repository.id === 'planr-pipeline' ? [] : [`repository:${repository.id}`],
    },
    reachability: {
      method: 'static-import-and-content-reference-scan; dynamic resolution is conservatively unresolved',
      incoming: [],
      outgoing: [],
      unresolved: [],
    },
...
function inventorySchema() {
  ...
  return {
    ...
    required: ['schemaVersion', 'inventoryId', 'classifications', 'repositories', 'surfaces', 'evidenceGaps', 'protectedPreserveEvidence'],
    properties: {
      schemaVersion: { const: INVENTORY_SCHEMA_VERSION },
      inventoryId: { const: 'operate-2.0-legacy-surface-inventory' },
      classifications: { type: 'array', minItems: INVENTORY_CLASSIFICATIONS.length, items: { enum: INVENTORY_CLASSIFICATIONS } },
      repositories: {
        type: 'array', minItems: knownRepositories.length,
        items: {
          ...
          properties: {
            ...
            worktree: {
              type: 'object', additionalProperties: false, required: ['state', 'entries'],
              properties: {
                state: { enum: ['clean', 'dirty', 'unavailable'] },
                entries: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['path', 'index', 'worktree'], properties: { path: portablePathSchema, index: { type: 'string' }, worktree: { type: 'string' } } } },
              },
            },
            writeAuthority: { enum: ['authorized-current-task', 'read-only-inspection', 'unavailable'] },
      ...
      surfaces: {
        ...
        items: {
          type: 'object', additionalProperties: false,
          required: ['id', 'repositoryId', 'path', 'kind', 'classification', 'rationale', 'ownership', 'evidence', 'reachability', 'proposedProof'],
          properties: {
            id: { type: 'string', minLength: 3 }, repositoryId: { enum: knownRepositories.map(({ id }) => id) }, path: portablePathSchema,
            kind: { type: 'string', minLength: 1 }, classification: { enum: INVENTORY_CLASSIFICATIONS }, rationale: { type: 'string', minLength: 1 }, ownership: { type: 'string', minLength: 1 },
            evidence: { type: 'object', additionalProperties: false, required: ['observations', 'imports', 'markdownLinks', 'packageReachability', 'generatorEvidence', 'schemaRegistration', 'testCoverage', 'documentationLinks', 'downstreamReferences'], properties: {
              observations: { type: 'array', minItems: 1, items: { type: 'string' } }, imports: { type: 'array', items: { type: 'string' } }, markdownLinks: { type: 'array', items: { type: 'string' } }, packageReachability: { type: 'array', items: { type: 'string' } }, generatorEvidence: { type: 'array', items: portablePathSchema }, schemaRegistration: { type: 'array', items: { type: 'string' } }, testCoverage: { type: 'array', items: { type: 'string' } }, documentationLinks: { type: 'array', items: { type: 'string' } }, downstreamReferences: { type: 'array', items: { type: 'string' } },
            } },
            reachability: { type: 'object', additionalProperties: false, required: ['method', 'incoming', 'outgoing', 'unresolved'], properties: { method: { type: 'string', minLength: 1 }, incoming: { type: 'array', items: { type: 'string' } }, outgoing: { type: 'array', items: { type: 'string' } }, unresolved: { type: 'array', items: { type: 'string' } } } },
            proposedProof: { type: 'object', additionalProperties: false, required: ['mode', 'requirement'], properties: { mode: { enum: ['absence', 'replacement', 'owner-proof', 'preserve'] }, requirement: { type: 'string', minLength: 1 } } },
            protectedPreserve: { type: 'object', additionalProperties: false, required: ['recordedSha256', 'observedSha256', 'preserveOnly'], properties: { recordedSha256: { pattern: '^[a-f0-9]{64}$' }, observedSha256: { pattern: '^[a-f0-9]{64}$' }, preserveOnly: { const: true } } },
          },
        },
      },
      evidenceGaps: { type: 'array', items: { type: 'string' } },
      protectedPreserveEvidence: { type: 'array', minItems: 3, items: { type: 'object', additionalProperties: false, required: ['path', 'recordedSha256', 'observedSha256', 'matchesRecorded', 'preserveOnly'], properties: { path: portablePathSchema, recordedSha256: { pattern: '^[a-f0-9]{64}$' }, observedSha256: { pattern: '^[a-f0-9]{64}$' }, matchesRecorded: { const: true }, preserveOnly: { const: true } } } },
    },
  };
}
...
function attachReachability(surfaces) {
  ...
  for (const surface of surfaces) {
    if (!surfacesByRepository.has(surface.repositoryId)) surfacesByRepository.set(surface.repositoryId, new Map());
  ...
  for (const surface of surfaces) {
    ...
    for (const specifier of surface.evidence.imports) {
      ...
      surfacesByRepository.get(surface.repositoryId).get(target)?.reachability.incoming.push(`import:${surface.path}`);
    }
    surface.evidence.imports = [...new Set(normalizedImports)].sort();
    surface.evidence.markdownLinks = surface.evidence.markdownLinks.map((target) => {
  ...
  for (const surface of surfaces) {
    for (const property of ['incoming', 'outgoing', 'unresolved']) surface.reachability[property] = [...new Set(surface.reachability[property])].sort();
  }
}

export function buildOperateSurfaceInventory({ root = repositoryRoot } = {}) {
  const repositories = knownRepositories.map((definition) => repositoryDescriptor(definition, root));
  const surfaces = [];
  for (const repository of repositories) {
    ...
    for (const path of listFiles(directory)) {
      ...
      if (looksLikeOperatingSurface(path, text)) surfaces.push(makeSurface(repository, path, text, manifest));
    }
  }
  surfaces.sort((left, right) => left.id.localeCompare(right.id));
  attachReachability(surfaces);
  const protectedPreserveEvidence = Object.entries(PROTECTED_USER_OWNED_PATHS).map(([path, recordedSha256]) => {
    const observedSha256 = sha256File(join(root, path));
    return { path, recordedSha256, observedSha256, matchesRecorded: observedSha256 === recordedSha256, preserveOnly: true };
  }).sort((left, right) => left.path.localeCompare(right.path));
  const evidenceGaps = repositories.filter(({ availability }) => availability === 'unavailable')
    .map(({ id, evidenceGap }) => `${id}: ${evidenceGap}`).sort();
  const inventory = {
...
function assertPortableValue(value, label = '$') {
  ...
  if (Array.isArray(value)) value.forEach((item, index) => assertPortableValue(item, `${label}[${index}]`));
  else if (value && typeof value === 'object') Object.entries(value).forEach(([key, item]) => assertPortableValue(item, `${label}.${key}`));
}

export function validateOperateSurfaceInventory(inventory) {
  const schemaErrors = validateJson(inventory, inventorySchema());
  if (schemaErrors.length > 0) fail(`Inventory schema validation failed: ${schemaErrors.map(({ path, rule }) => `${path} ${rule}`).join(', ')}`);
  if (new Set(inventory.classifications).size !== INVENTORY_CLASSIFICATIONS.length) fail('Inventory classifications must contain every allowed classification exactly once.');
  const seen = new Set();
  for (const surface of inventory.surfaces) {
    ...
    if (!INVENTORY_CLASSIFICATIONS.includes(surface.classification)) fail(`Unsupported classification for ${key}`);
    if (surface.classification === 'DELETE_LEGACY') {
      if (surface.path.endsWith('/')) fail(`Directory-wide deletion is forbidden: ${surface.path}`);
      if (surface.proposedProof.mode !== 'absence' || surface.evidence.observations.length === 0 || !surface.reachability.method) fail(`DELETE_LEGACY lacks evidence or an absence proof: ${key}`);
    }
    if (surface.evidence.observations.includes('generated:declared-generator') && surface.evidence.generatorEvidence.length === 0) fail(`Generated surface has no generator evidence: ${key}`);
  }
  for (const evidence of inventory.protectedPreserveEvidence) {
    ...
    const surface = inventory.surfaces.find((candidate) => candidate.repositoryId === 'planr-pipeline' && candidate.path === evidence.path);
    if (!surface?.protectedPreserve?.preserveOnly) fail(`Missing protected Preserve surface evidence: ${evidence.path}`);
  }
  for (const repository of inventory.repositories) {
    if (repository.id !== 'planr-pipeline' && repository.availability === 'available' && repository.writeAuthority !== 'read-only-inspection') fail(`Downstream authority must remain read-only: ${repository.id}`);
  }
...
export function renderInventoryMarkdown(inventory) {
  const lines = [
    ...
    '| --- | --- | --- | --- | --- |',
    ...inventory.repositories.map((repository) => `| ${repository.id} | ${repository.availability} | ${repository.revision} | ${repository.worktree.state} | ${repository.writeAuthority} |`),
    '',
    '## Classified surfaces',
    '',
    '| Repository | Path | Classification | Kind | Proof mode |',
    '| --- | --- | --- | --- | --- |',
    ...inventory.surfaces.map((surface) => `| ${surface.repositoryId} | ${surface.path} | ${surface.classification} | ${surface.kind} | ${surface.proposedProof.mode} |`),
    '',
    ...
    '## Evidence gaps',
    '',
    ...(inventory.evidenceGaps.length > 0 ? inventory.evidenceGaps.map((gap) => `- ${gap}`) : ['- None. Downstream repositories remain read-only in this task.']),
    '',
    '## Protected user-owned Preserve evidence',
    '',
    '| Path | Recorded SHA-256 | Observed SHA-256 | Preserve only |',
    '| --- | --- | --- | --- |',
    ...inventory.protectedPreserveEvidence.map((evidence) => `| ${evidence.path} | ${evidence.recordedSha256} | ${evidence.observedSha256} | ${evidence.preserveOnly} |`),
    '',
  ];
  return `${lines.join('\n')}\n`;
}
...
export function renderDownstreamManifest(inventory) {
  const downstream = inventory.surfaces.filter(({ repositoryId }) => repositoryId !== 'planr-pipeline');
  const lines = [
    ...
    '| --- | --- | --- | --- | --- |',
    ...downstream.map((surface) => `| ${surface.repositoryId} | ${surface.path} | ${surface.classification} | read-only-inspection | ${surface.proposedProof.mode} |`),
    '',
    ...(downstream.length > 0
...
function resolveDisposableOutputDirectory(root, outputDirectory) {
  ...
  const canonicalTargets = new Set(canonicalOutputPaths.map((path) => canonicalOutputPath(root, path)));
  if (
...
function parseCliArguments(argv) {
  ...
  fail('Usage: inventory-operate-surfaces.mjs [--check | --write --output-dir .planr/tmp/operate-inventory/<name>]');
}
