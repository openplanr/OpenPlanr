...
function buildReviewSchemaSlices(sourceRoot) {
  const schemas = new Map(
    reviewInputs.map(([exportName, relativePath]) => {
  ...
  const enqueue = (sourceFilename, reference) => {
    ...
    const filename = referencePath === '' ? sourceFilename : referencePath.split('/').at(-1);
  ...
  return [...frequencies]
    .sort(
      ([leftToken, leftCount], [rightToken, rightCount]) =>
        rightCount - leftCount || (leftToken < rightToken ? -1 : leftToken > rightToken ? 1 : 0),
    )
    .map(([token]) => token);
}
...
function readSchemaDefinitions(
  selectedInputs,
  { sourceRoot, schemaOverrides = new Map(), exportNameFor = (exportName) => exportName },
) {
...
function buildSchemaGenerationPlan(sourceRoot) {
  ...
  const canonicalInputs = inputs.filter(([, relativePath]) => !fullRelativePaths.has(relativePath));
...
function generateSchemaData(definitions, { reviewSpecific, reexports = [], encode }) {
  const lines = [
  ...
  if (reexports.length > 0) {
    lines.push(
      ...
      ...reexports.map(({ sourceName, exportName }) =>
        sourceName === exportName ? `\t${sourceName},` : `\t${sourceName} as ${exportName},`,
      ),
...
export function generateDashboardSurfaceSchemaData({ sourceRoot = repositoryRoot } = {}) {
...
export function generateDashboardReviewSchemaData({ sourceRoot = repositoryRoot } = {}) {
  const plan = buildSchemaGenerationPlan(sourceRoot);
...
export function generateDashboardSchemaTokenCodecData({ sourceRoot = repositoryRoot } = {}) {
  return generateSchemaTokenCodecData(buildSchemaGenerationPlan(sourceRoot).tokens);
}

export function deriveDashboardSchemaGenerationValues({ sourceRoot = repositoryRoot } = {}) {
  const plan = buildSchemaGenerationPlan(sourceRoot);
  const review = Object.fromEntries(
    plan.reviewDefinitions.map(({ exportName, schema }) => [exportName, structuredClone(schema)]),
  );
  const surface = Object.fromEntries(
    inputs.map(([exportName, relativePath]) => [
      exportName,
...
function runCli(argv) {
  ...
  const customOutputRequested =
    outputOption !== null || reviewOutputOption !== null || codecOutputOption !== null;
      ...
      ? [
          {
            path: resolve(
              reviewOutputOption ??
                resolve(sourceRoot, 'lib/dashboard/generated/operate-review-schema-data.mjs'),
            ),
      ...
      ? [
          {
            path: resolve(
              codecOutputOption ??
                resolve(sourceRoot, 'lib/dashboard/generated/operate-schema-token-codec.mjs'),
            ),
  ...
  for (const target of targets) {
    if (argv.includes('--check')) {
      ...
      if (current !== target.expected) {
        process.stderr.write(`${target.driftCode}: generated ${target.label} is stale\n`);
        process.exitCode = 1;
      } else {
