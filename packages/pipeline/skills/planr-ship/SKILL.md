...
preparation belong to `planr-land`.
...
2. `input/tech/stack.md`, its `ActiveStackFiles`, installed stack defaults, and matching
   `the active host project stack directory/` overrides;
...
```bash
node the installed planr-pipeline package root/scripts/discover-verification.mjs --project . --task <task-path>
```
...
When Outcome is `completed`, append `Next: planr-land` to the human-readable
