<!-- {{AGENTS_ROOT}}/shared/modes/spec-driven/designer.md: spec-driven-mode-only content for designer-agent. Loaded by agents/designer-agent.md when MODE=spec-driven. T-002 of SPEC-002. -->
...
|-------|--------|----------|
| Feature/spec slug (`$ARGUMENTS`) | `{{WORKFLOW_PREFIX}}plan` orchestrator | Yes |
...
- Expand `~user/foo` through the host's user-directory resolver; never hard-code a home path.
- Resolve bare relative paths against the **project root** (working directory), NOT against `{{PIPELINE_PACKAGE_ROOT}}`
...
```
0. Receive spec slug from {{WORKFLOW_PREFIX}}plan as $ARGUMENTS (the {slug} in SPEC-NNN-{slug})
